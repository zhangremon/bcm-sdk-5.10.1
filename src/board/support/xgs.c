...
board_xgs_probe(board_driver_t *driver, int num, bcm_info_t *info)
{
    ...
    if (num == PRIVATE(num_device)) {
        ...
        if (PRIVATE(num_supported_device) % PRIVATE(num_device)) {
            BOARD_VERB(("%s: devtab not modulo numdev\n", driver->name));
            return BCM_E_CONFIG;
        }
    ...
    for (i=0; i<PRIVATE(num_internal_connection); i++) {
        ...
        for (j=0; j<end; j++) {
            ...
            if (i_port != BCM_GPORT_TYPE_NONE) {
                if (replace < 0) {
                    BOARD_VERB(("Adding internal connection %d to conn %d\n",
                                i, j));
        ...
        if (d_idx >= 0) {
            ...
            for (j=d_idx; j<end; j++) {
                BOARD_VERB(("Copying %d to %d\n", j+1, j));
                conn[j] = conn[j+1];
            }
            end -= 1;
            BOARD_VERB(("Delete, end now %d\n", end));
        } else if (replace < 0) {
            /* add the internal entry to the end of the list */
            BOARD_VERB(("Copying internal %d to %d\n", i, end));
            sal_memcpy(&conn[end], &internal[i], sizeof(conn[end]));
            end += 1;
            BOARD_VERB(("Add internal, end now %d\n", end));
        }
    }

    BOARD_VERB(("Board: %d internal, %d stack connections\n",
...
/* TODO: If driver static connection list includes external stack
   ports, then assume the list is complete, otherwise probe for stack
   ports and merge the stack port list. What if you don't want any
   external ports? NONE,NONE at the end?

   For now: always merge external stack ports. */
...
#endif
        BCM_IF_ERROR_RETURN(board_led_init(unit,
                                           PRIVATE(led_port_offset),
                                           PRIVATE(num_led_program),
                                           PRIVATE(led_program)));
    }
