...
_board_connect_vertex(tree_t *t, vertex_t *vertex, void *user_data)
{
    ...
    if (!bcm_unit_local(vertex->unit)) {
        BOARD_VERB(("Board programming for remote unit %d not required.\n",
                    vertex->unit));
        return BCM_E_NONE;
    }

    BOARD_VERB(("Board programming unit %d\n", vertex->unit));
    for (edge = vertex->edge; edge != NULL; edge = edge->next) {
    ...
    if (BCM_FAILURE(rv)) {
        BOARD_VERB(("Board programming unit %d failed (%d)\n",
                    vertex->unit, rv));
    }
    return rv;
}
