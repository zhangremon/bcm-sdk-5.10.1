/*
 * $Id: populate.c 1.1 Broadcom SDK $
 ...
 * Purpose:
 *     Set the board ID based on the chip family.
 * Parameters:
 *     unit   - (IN)    unit
...
board_cpudb_get(board_driver_t *driver, cpudb_ref_t db_ref)
{
    ...
    if (entry->base.num_units == 1) {
        (void)(_board_probe_board_id(unit, db_ref));
    }
