...
static spinlock_t _ai_lock = SPIN_LOCK_UNLOCKED;
...
sal_alloc(unsigned int sz, char *s)
{
     ...
     */
    if ((ai = kmalloc(sizeof(alloc_info_t) + 4 * szw, GFP_ATOMIC)) != NULL) {
        ai->src = FROM_KMALLOC;
    } else if ((ai = vmalloc(sizeof(alloc_info_t) + 4 * szw)) != NULL) {
        ai->src = FROM_VMALLOC;
    } else if ((ai = kmalloc_giant(sizeof(alloc_info_t) + 4 * szw)) != NULL) {
