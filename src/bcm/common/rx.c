...
{  
    ...
    if (rx_ctl[unit]->user_cfg.rx_alloc != RX_DEFAULT_ALLOC ||
            rx_ctl[unit]->user_cfg.rx_free != RX_DEFAULT_FREE) {
        RX_DEBUG((DK_WARN,
               "RX WARNING: Not using rx_pool alloc/free with RPC.\n"));
    }
#endif
    ...
    if (RX_IS_LOCAL(unit) && SOC_UNIT_VALID(unit)) {
        ...
        if (soc_feature(unit, soc_feature_cos_rx_dma) && 
                first_rx_chan != -1) {
#ifdef BCM_CMICM_SUPPORT
            /* Clear the ARM queue use record during this reinit. */
            for (cmc = 0; cmc < SOC_CMCS_NUM(unit); cmc++) {
                SHR_BITCLR_RANGE(CPU_ARM_QUEUE_BITMAP(unit, cmc),
                                 0, NUM_CPU_COSQ(unit))
            }
#endif
            /* enable all cosq on the first Rx channel */
            BCM_IF_ERROR_RETURN
                (_bcm_common_rx_queue_channel_set(unit, -1, first_rx_chan));
...
_bcm_common_rx_reasons_get(int unit, bcm_rx_reasons_t *reasons)
{
    ...
    if (NULL != SOC_DCB(unit).rx_egr_reason_map) {
        egr_map = SOC_DCB(unit).rx_egr_reason_map;
    }
    ...
    for (ix = 0; ix < max_index ; ix++) {
         ...
         if (NULL != egr_map && egr_map[ix] != socRxReasonInvalid &&
             egr_map[ix] != socRxReasonCount) {
             BCM_RX_REASON_SET(*reasons, (bcm_rx_reason_t)egr_map[ix]);
         }
    }
                                 ...
                                 bcm_rx_chan_t chan_id)
{
    ...
    if(soc_feature(unit, soc_feature_cmicm)) {
        ...
        if (queue_id < 0) { /* All COS Queues */
            /* validate the queue range of CMC is non overlapping with
             * other CMCs
             */
            for (ix = 0; ix < SOC_CMCS_NUM(unit); ix++) {
                if ((ix != cmc) &&
                    (!SHR_BITNULL_RANGE(CPU_ARM_QUEUE_BITMAP(unit, ix),
                                       startq, numq))) {
                    return BCM_E_EXISTS;
                } else {
                    if (ix == cmc) {
                        SHR_BITSET_RANGE(CPU_ARM_QUEUE_BITMAP(unit, cmc),
                                        startq, numq);
                    }
                }
            }
            ...
            for (ix = 0; ix < SOC_CMCS_NUM(unit); ix++) {
                if ((ix != cmc) &&
                    SHR_BITGET(CPU_ARM_QUEUE_BITMAP(unit, cmc), queue_id)) {
            ...
            for (ix = start_chan_id; ix < (start_chan_id + BCM_RX_CHANNELS); ix++) {
                ...
                if (ix == (uint32)chan_id) {
                    ...
                    /*
                     * We need to record the queues used by the ARMs so
                     * we don't allow the API to redirect them.
                     */

                    SHR_BITSET(CPU_ARM_QUEUE_BITMAP(unit, cmc), queue_id);
    ...
    *chan_id = -1;
    ...
    if(soc_feature(unit, soc_feature_cmicm)) {
        for (ix = 0; ix < chan_id_max; ix++) { 
            ...
            chan_off = ix % BCM_RX_CHANNELS;
            reg_addr = (queue_id < 32) ? 
                        CMIC_CMCx_CHy_COS_CTRL_RX_0_OFFSET(cmc,ix) :
                        CMIC_CMCx_CHy_COS_CTRL_RX_1_OFFSET(cmc,ix);
            bit = 1 << (queue_id % 32);
            reg_val = soc_pci_read(unit, reg_addr);
            if (reg_val & bit) {
                if (*chan_id == -1) {
...
rx_update_dv(int unit, int chan, dv_t *dv)
{
    ...
    if (!rx_control.thread_running || NO_REAL_CALLOUTS(unit)) {
...
/* Multiple DCB DMA fixup routine */
    ...
    if (vni) {
        if (SOC_IS_TR_VL(unit)) {
            if (_bcm_vfi_used_get(unit, vni, _bcmVfiTypeMpls)) {
                _BCM_VPN_SET(*vpn, _BCM_VPN_TYPE_VPLS, vni);
            } else if (_bcm_vfi_used_get(unit, vni, _bcmVfiTypeMim)) {
                _BCM_VPN_SET(*vpn, _BCM_VPN_TYPE_MIM, vni);
            }
        }
    }
}
    ...
    if (soc_feature(unit, soc_feature_higig2)) {
        ...
        case 2:      /* PPD2 */
            ...
            if (soc_higig2_field_get(unit, xgh2, HG_multipoint)) {
                ...
                physical = soc_higig2_field_get(unit, xgh2, HG_dst_type) != 0;
...
rx_intr_process_packet(int unit, dv_t *dv, dcb_t *dcb, bcm_pkt_t *pkt)
{
    ...
    if (SOC_IS_RCPU_UNIT(unit)) {
        ...
        if (SOC_IS_DRACO15(unit)) {
            pkt->prio_int = SOC_DCB_RX_PRIO_GET(unit, dcb);
        } else {
            pkt->prio_int = BCM_PKT_VLAN_PRI(pkt);
            pkt->vlan = BCM_PKT_VLAN_ID(pkt);
        }
#ifdef BCM_TUCANA_SUPPORT
        if (soc_feature(unit, soc_feature_dcb_st0_bug)) {
            if (!IS_HG_PORT(unit, pkt->rx_port)){
                pkt->src_port = pkt->rx_port & 0x1f;
            ...
            if (BCM_SUCCESS(rv)) {
                if (0x1 == 
                    ...
                    if (_bcm_vfi_used_get(unit, vfi, _bcmVfiTypeMpls)) {
                        _BCM_VPN_SET(pkt->vlan, _BCM_VPN_TYPE_VPLS, vfi);
                    } 
                } 
            }
        }
...
rx_rcpu_process_packet(int unit, bcm_pkt_t *pkt)
{
    ...
    for (rco = rx_ctl[unit]->rc_callout; rco; rco = rco->rco_next) {
        ...
        case BCM_RX_HANDLED:
            handled = TRUE;
            bcm_pkt_rx_free(unit, pkt);    /* Free the packet */
...
#ifdef BCM_SIRIUS_SUPPORT
    if (SOC_UNIT_VALID(unit) && SOC_IS_SIRIUS(unit)) {
