/*
 * $Id: link.c 1.19.20.2 Broadcom SDK $
...
_bcm_linkscan_init(int unit, _bcm_ls_driver_t *driver)
{
...
#ifdef BCM_WARM_BOOT_SUPPORT
    SOC_SCACHE_HANDLE_SET(wb_hdl, unit, BCM_MODULE_LINKSCAN, 0);
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        if (size > 0) {
            scache_size = sizeof(_ls_control_t) + SOC_WB_SCACHE_CONTROL_SIZE;
            LINK_VERB(("LINKSCAN: allocating 0x%x (%d) bytes of scache:",  scache_size, scache_size));
            rv = soc_scache_alloc(unit, wb_hdl, scache_size);
        }
    }
#endif /* BCM_WARM_BOOT_SUPPORT */

    if ((lc = sal_alloc(sizeof(_ls_control_t), "link_control")) == NULL) {
        return BCM_E_MEMORY;
    }
    sal_memset(lc, 0, sizeof(_ls_control_t));
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        free_lc = lc;
        if (size > 0) {
            rv = soc_scache_ptr_get(unit, wb_hdl, &ptr, &scache_size);
            wb_cache = (soc_wb_cache_t*)ptr;

            /* adjust the scache size for the control word */
            scache_size = SOC_WB_SCACHE_SIZE(scache_size);
        }
        ...
        if (BCM_SUCCESS(rv)) {
            if (wb_cache) {
                ...
                sal_free(lc);
                free_lc = NULL;
                lc = (_ls_control_t*)wb_cache->cache;
                ...
                if (SOC_WARM_BOOT(unit)) {
                    ...
                    wb_cache->version = LINK_WB_CURRENT_VERSION;
                    sal_memset(lc, 0, sizeof(*lc));
                }

                LINK_VERB(("Obtained scache pointer=%p, %d bytes, "
                                "version=%d.%d\n",
                         wb_cache->cache, scache_size,
                         SOC_SCACHE_VERSION_MAJOR(wb_cache->version),
                         SOC_SCACHE_VERSION_MINOR(wb_cache->version)));

                if (wb_cache->version > LINK_WB_CURRENT_VERSION) {
                    LINK_ERR(("Upgrade scenario not supported.  "
                                   "Current version=%d.%d  found %d.%d\n",
                            SOC_SCACHE_VERSION_MAJOR(LINK_WB_CURRENT_VERSION),
                            SOC_SCACHE_VERSION_MINOR(LINK_WB_CURRENT_VERSION),
                            SOC_SCACHE_VERSION_MAJOR(wb_cache->version),
                            SOC_SCACHE_VERSION_MINOR(wb_cache->version)));
                    rv = BCM_E_CONFIG;

                } else if (wb_cache->version < LINK_WB_CURRENT_VERSION) {
                    LINK_ERR(("Downgrade scenario not supported.  "
                                   "Current version=%d.%d  found %d.%d\n",
                            SOC_SCACHE_VERSION_MAJOR(LINK_WB_CURRENT_VERSION),
                            SOC_SCACHE_VERSION_MINOR(LINK_WB_CURRENT_VERSION),
                            SOC_SCACHE_VERSION_MAJOR(wb_cache->version),
...
bcm_common_linkscan_detach(int unit)
{
    ...
    if (BCM_SUCCESS(rv) && size) {
        rv = soc_scache_ptr_get(unit, wb_hdl, &ptr, (uint32*)&size);
        wb_cache = (soc_wb_cache_t*)ptr;
    }
