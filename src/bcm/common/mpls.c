...
bcm_mpls_tunnel_switch_t_init(bcm_mpls_tunnel_switch_t *info)
{
    if (info != NULL) {
        ...
        info->inner_label = BCM_MPLS_LABEL_INVALID;
...
void
bcm_mpls_vpn_t_init(bcm_mpls_vpn_t *vpn_info)
{
    if (vpn_info != NULL) {
...
void
bcm_mpls_l3_initiator_t_init(bcm_mpls_l3_initiator_t *ftn)
{
    if (ftn != NULL) {
...
void
bcm_mpls_switch_t_init(bcm_mpls_switch_t *mswitch)
{
    if (mswitch != NULL) {
...
/*
 * Function:
 *      bcm_mpls_circuit_t_init
 * Purpose:
 *      Initialize mpls circuit structure
 * Parameters:
 *      mpls_vc - Pointer to the struct to be init'ed
 */

void
bcm_mpls_circuit_t_init(bcm_mpls_circuit_t *mpls_vc)
{
    if (mpls_vc != NULL) {
 ...
 * Purpose:
 *      Initialize mpls info structure
 * Parameters:
 *      mpls_vc - Pointer to the struct to be init'ed
 */

void
bcm_mpls_info_t_init(bcm_mpls_info_t *mpls_info)
{
    if (mpls_info != NULL) {
