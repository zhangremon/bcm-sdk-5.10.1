...
_bcm_tx_hg2hdr_setup(bcm_pkt_t *pkt)
{
    ...
    int multicast, fwd_type, mc_index_offset;
#if defined(BCM_BRADLEY_SUPPORT)
    ...
    case 2:
        ...
        if (multicast) {
            soc_higig2_field_set(unit, xgh, HG_dst_vp, pkt->multicast_group);
        } else {
    ...
    if (multicast) {
        ...
        soc_higig2_field_set(unit, xgh, HG_mgid,
                             pkt->multicast_group + mc_index_offset);
    } else {
...
_bcm_tx_hghdr_setup(bcm_pkt_t *pkt)
{
    ...
    if ((pkt->opcode == SOC_HIGIG_OP_MC) ||
        ...
        soc_higig_field_set(unit, xgh, HG_l2mc_ptr, pkt->multicast_group);
    } else {
...
_bcm_tx_sltag_setup(bcm_pkt_t *pkt)
{
    ...
    if (pkt->opcode == BCM_HG_OPCODE_CPU) {
        ...
        if (bcm_stk_sl_simplex_count_get(pkt->unit, &slcount) < 0) {
            slcount = 0;
        }
        if (slcount > 0) {
            slcount -= 1;
             ...
             bcm_pkt_cb_f chain_done_cb, void *cookie, int per_pkt_cb)
{
    ...
    dv_info->pkt = sal_alloc((sizeof(bcm_pkt_t *) * pkt_count), "tx_dv pkt");
    if (dv_info->pkt == NULL) {
        soc_dma_dv_free(unit, dv);
        sal_free(dv_info);
        return NULL;
...
_dcb_flags_get(int unit, bcm_pkt_t *pkt, dv_t *dv)
{
    ...
    if (SOC_IS_XGS3_SWITCH(unit)) {
        ...
        return dcb_flags;
    } else if (SOC_IS_XGS_SWITCH(unit)) { /* Not for Hercules */
        /* If soc is draco/tucana/lynx, special dcb setup for higig info */
        SOC_DMA_DPORT_SET(dcb_flags, pkt->dest_port);
        SOC_DMA_DMOD_SET(dcb_flags, pkt->dest_mod);
        SOC_DMA_MHOP_SET(dcb_flags, pkt->opcode);
    } 
#if defined(BCM_SIRIUS_SUPPORT)
    else if (SOC_IS_SIRIUS(unit)) {
        /*
...
_soc_tx_pkt_setup(int unit, bcm_pkt_t *pkt, soc_tx_param_t *tx_param)
{
    ...
    if (SOC_IS_TUCANA(unit) || SOC_IS_DRACO1(unit) || SOC_IS_LYNX(unit)) {
        tx_param->flags |= SOC_DMA_F_PKT_PROP;
    }
#endif
...
_tx_pkt_desc_add(int unit, bcm_pkt_t *pkt, dv_t *dv, int pkt_idx)
{
    ...
    /* Decide which source mod/port to use */
    src_mod  = dv->tx_param.src_mod;
    src_port = dv->tx_param.src_port;
    prio_int = dv->tx_param.prio_int;
        ...
        if (pkt->flags & BCM_TX_FAST_PATH) {
            for (i = 0; i < pkt->blk_count; i++) {
                ...
                if (SOC_IS_DRACO15(unit)) {
                    /*
                     * Get the pointer to the DCB that just has been allocated
                     */
                    cur_dcb = SOC_DCB_IDX2PTR(unit,
                                              dv->dv_dcb,
                                              dv->dv_vcnt - 1);
                    SOC_DCB_TX_SRCMOD_SET(unit, cur_dcb, src_mod);
                    SOC_DCB_TX_SRCPORT_SET(unit, cur_dcb, src_port);
                    SOC_DCB_TX_PRIO_SET(unit, cur_dcb, prio_int);
                    SOC_DCB_TX_PFM_SET(unit, cur_dcb, pkt->pfm);
                }
...
#if defined(BCM_SIRIUS_SUPPORT)
	{
        ...
        if (SOC_IS_DRACO15(unit)) {
            /* Get the pointer to the DCB that just has been allocated */
            cur_dcb = SOC_DCB_IDX2PTR(unit, dv->dv_dcb, dv->dv_vcnt - 1);
            SOC_DCB_TX_SRCMOD_SET(unit, cur_dcb, src_mod);
            SOC_DCB_TX_SRCPORT_SET(unit, cur_dcb, src_port);
            SOC_DCB_TX_PRIO_SET(unit, cur_dcb, prio_int);
            SOC_DCB_TX_PFM_SET(unit, cur_dcb, pkt->pfm);
