/*
 * $Id: l2cache.c 1.54 Broadcom SDK $
...
/* Debug Macros */
#define BCM_L2CACHE_ERR(stuff)          BCM_DEBUG(BCM_DBG_L2 | BCM_DBG_ERR, stuff)
#define BCM_L2CACHE_WARN(stuff)         BCM_DEBUG(BCM_DBG_L2 | BCM_DBG_WARN, stuff)
#define BCM_L2CACHE_VERB(stuff)         BCM_DEBUG(BCM_DBG_L2 | BCM_DBG_VERBOSE, stuff)
#define BCM_L2CACHE_VVERB(stuff)        BCM_DEBUG(BCM_DBG_L2 | BCM_DBG_VVERBOSE, stuff)
#define BCM_L2CACHE_MSG(stuff)          BCM_DEBUG(BCM_DBG_NORMAL, stuff)
...
do { \
    ...
    if (!_l2_cache[unit].lock) { \
        L2CACHE_ERR(("%s: l2_cache unitialized on unit:%d \n", \
                     FUNCTION_NAME(), unit)); \
        return BCM_E_INIT; \
    } \
} while (0)
...
do { \
    if (sal_mutex_take(_l2_cache[unit].lock, sal_mutex_FOREVER)) { \
        L2CACHE_ERR(("%s: sal_mutex_take failed for unit %d. \n", \
                    FUNCTION_NAME(), unit)); \
...
do { \
    if (sal_mutex_give(_l2_cache[unit].lock)) { \
        L2CACHE_ERR(("%s: sal_mutex_give failed for unit %d. \n", \
                    FUNCTION_NAME(), unit)); \
        return BCM_E_INTERNAL; \
    } \
} while (0)
                              ...
                              uint32 *pfte, int fte_type)
{
    ...
    if (addr->flags & BCM_L2_CACHE_MULTICAST) {
        ...
        if (rv != BCM_E_NONE) {
            L2CACHE_ERR(("%s: invalid (dest_modid, dest_port)\n", 
                         FUNCTION_NAME()));
            return rv;
        }
    ...
    if (rv != BCM_E_NONE) {
        L2CACHE_ERR(("%s: failed to allocate an %s (%d) FTE.\n", 
...
_bcm_fe2000_l2cache_entry_store(int unit, int index, int subtype) {
    ...
    BCM_L2CACHE_VVERB(("L2CP_CACHE entry stored, index: 0x%x flags: 0x%08x\n", index, layout->flags[index]));
    BCM_L2CACHE_VVERB(("   entry->flags: 0x%08x\n", entry->flags));
    BCM_L2CACHE_VVERB(("   entry->cookie: 0x%x\n", entry->cookie));
    BCM_L2CACHE_VVERB(("   entry->fte_idx: 0x%x\n", entry->fte_idx));
    BCM_L2CACHE_VVERB(("   entry->encap_id: 0x%x\n", entry->encap_id));
    BCM_L2CACHE_VVERB(("   entry->src_port: 0x%x\n", entry->src_port));
    ...
    if (index >= L2CACHE_MAX_L2CP_IDX) {
        ...
        if (entry->flags & L2CACHE_ENTRY_DMAC) {
            ...
            BCM_L2CACHE_VVERB(("   entry->vlan: 0x%x\n", layout->mac_entry[index].vlan));
            BCM_L2CACHE_VVERB(("   entry->mac: %02x:%02x:%02x:%02x:%02x:%02x\n", layout->mac_entry[index].mac[0],
                                  ...
                                  fe2k_l2cache_wb_mem_layout_t *layout)
{
    ...
    if (!(entry->flags & L2CACHE_ENTRY_DMAC) ){
        if (entry->flags & L2CACHE_ENTRY_SUBTYPE) {
            ...
            if (rv != BCM_E_NONE) {
                L2CACHE_ERR(("%s: Error reading l2cpslow table, subtype 0x%x, port 0x%x, \n", FUNCTION_NAME(), subtype, port));
                return rv;
            }
            ...
            if (rv != BCM_E_NONE) {
                L2CACHE_ERR(("%s: Error reading l2cp table, dmac 0x%x port 0x%x\n", FUNCTION_NAME(), dmac, port));
                return rv;
            }
        ...
        if (entry->flags & L2CACHE_ENTRY_DMAC) {
            ...
            if ((entry->flags & L2CACHE_ENTRY_PREFIX5_LOOKUP) & !(rsvd_mac)) {
                ...
                if (rv != BCM_E_NONE){
                    L2CACHE_ERR(("%s: Entry not found in L2CPMac table\n", FUNCTION_NAME()));
                    return BCM_E_INTERNAL;
                }
            }
            ...
            rv = soc_sbx_g2p3_cmac_get(unit, layout->mac_entry[mac_index].mac, (pL2Key->ulVlan & 0xfff),
                                      &sbx_mac_data);
            if (rv != BCM_E_NONE) {
                L2CACHE_ERR(("%s: Error reading mac from L2 table\n", FUNCTION_NAME()));
                return rv;
            }
    ...
    if ( entry->flags & L2CACHE_ENTRY_FTE_VALID) {
        ...
        if (rv != SOC_E_NONE) {
            L2CACHE_ERR(("%s: Error getting ft entry:%d)\n", FUNCTION_NAME(), entry->fte_idx));
            return rv;
        }
        ...
        if ( !(entry->flags & L2CACHE_ENTRY_FTE_MPLS_GPORT) ) {
            ...
            if ( (entry->fte_idx >= SBX_GLOBAL_GPORT_FTE_BASE(unit)) &&
                ...
                if (rv != BCM_E_NONE) {
                    L2CACHE_ERR(("%s: Error re-allocating fte 0x%x\n", FUNCTION_NAME(), entry->fte_idx));
                    return rv;
                }
            }else{
                rv = _sbx_gu2_resource_alloc(unit,
    ...
    BCM_L2CACHE_VVERB(("L2CP_CACHE entry recovered, index: 0x%x flags: 0x%08x\n", index, layout->flags[index]));
    BCM_L2CACHE_VVERB(("   entry->flags: 0x%08x\n", entry->flags));
    BCM_L2CACHE_VVERB(("   entry->cookie: 0x%x\n", entry->cookie));
    BCM_L2CACHE_VVERB(("   entry->fte_idx: 0x%x\n", entry->fte_idx));
    BCM_L2CACHE_VVERB(("   entry->encap_id: 0x%x\n", entry->encap_id));
                                       ...
                                       fe2k_l2cache_wb_mem_layout_t *layout)
{
    ...
    if (rv != SOC_E_NONE) {
        L2CACHE_ERR(("%s: Error getting L2CP SLOW entry (port:%d "
                     "subtype:%d)\n", FUNCTION_NAME(), 0, 11));
        return rv;
    }
    ...
    *pass_thru_ohi = l2cpslow.ftidx;
    ...
    if (rv != BCM_E_NONE) {
        L2CACHE_ERR(("%s: Error re-allocating pass-thru ohi 0x%x\n", FUNCTION_NAME(), *pass_thru_ohi));
        return rv;
    }
    ...
    if (rv != SOC_E_NONE) {
        L2CACHE_ERR(("%s: Error getting oi2e entry:%d)\n", FUNCTION_NAME(), *pass_thru_ohi));
        return rv;
    }
    ...
    if (rv != BCM_E_NONE) {
        L2CACHE_ERR(("%s: Error re-allocating ete encap 0x%x\n", FUNCTION_NAME(), encap_ete_idx));
        return rv;
    }
    ...
    if (rv != SOC_E_NONE) {
        L2CACHE_ERR(("%s: Error getting eteencap entry:%d)\n", FUNCTION_NAME(), encap_ete_idx));
        return rv;
    }
    ...
    rv = _sbx_gu2_resource_alloc(unit, SBX_GU2K_USR_RES_ETE_L2, 
                                 1, &ete_idx, flags);
    if (rv != BCM_E_NONE) {
        L2CACHE_ERR(("%s: Error re-allocating ete l2 0x%x\n", FUNCTION_NAME(), ete_idx));
        return rv;
    }
                                ...
                                int subtype_in, int idx)
{
    ...
    if ((port != BCM_L2_SRCPORT_MASK_ALL) && (port > SBX_MAX_PORTS)) {
        L2CACHE_ERR(("%s: Invalid port (%d) \n", FUNCTION_NAME(), port));
        return BCM_E_PARAM;
    }

    if (entry->flags & L2CACHE_ENTRY_SUBTYPE) {
        if (dmac != 0x02 ) {
            L2CACHE_ERR(("%s: Invalid DMAC LSB(0x%x) specified for l2cp slow "
    ...
    for (port = port_min; port < port_max; port++) {
        for (dmac = dmac_min; dmac < dmac_max; dmac++) {
            ...
            if (rv != SOC_E_NONE) {
                L2CACHE_ERR(("%s: Error setting L2CP entry (port:%d lsb:%d)\n",
        ...
        for (subtype = st_min; subtype < st_max; subtype++) {
            ...
            if (rv != SOC_E_NONE) {
                L2CACHE_ERR(("%s: Error setting L2CP SLOW entry (port:%d "
...
_bcm_fe2000_g2p3_l2_cache_hw_entry_delete(int unit, int idx)
{
    ...
    if (flags & L2CACHE_ENTRY_DMAC) {
        ...
        if (rv != BCM_E_NONE) {
            L2CACHE_VVERB(("%s: Error (%s) removing L2 entry from DMAC table\n",
                                              ...
                                              bcm_l2_cache_addr_t *addr)
{
    ...
    if (flags & L2CACHE_ENTRY_DMAC) {
        ...
        pL2Key = (_l2_cache_key *)entry->cookie;
        if (!pL2Key) {
            L2CACHE_ERR(("%s: Invalid L2CACHE entry\n", FUNCTION_NAME()));
            return BCM_E_INTERNAL;
        }
    ...
    if (flags & L2CACHE_ENTRY_FTE_VALID) {
        ...
        if (flags & L2CACHE_ENTRY_FTE_MPLS_GPORT) {
            ...
            rv = soc_sbx_modid_get(unit, fab_node_id, fab_port, 
                                   &addr->dest_modid);
            if (rv != BCM_E_NONE) {
                L2CACHE_ERR(("%s: Tunnel dest_modid get failed. \n", 
                             FUNCTION_NAME()));
                return rv;
            }
                                       ...
                                       int idx)
{
    ...
    if (rsvd_dmac && !(entry->flags & L2CACHE_ENTRY_DMAC)) {
        ...
        if (rv != BCM_E_NONE) {
            ...
            if (rv != BCM_E_NONE) {
                L2CACHE_ERR(("%s: Error storing warmboot information for entry 0x%x",
                             FUNCTION_NAME(), idx));
            }
#endif
        }
...
_bcm_fe2000_g2p3_l2_cache_entries_init(int unit, uint32 fte_idx)
{
    ...
    if (BCM_L2_SRCPORT_MASK_ALL <= SBX_MAX_PORTS) {
        L2CACHE_WARN(("%s: BCM_L2_SRCPORT_MASK_ALL is less than SBX_MAX_PORTS> "
                               ...
                               uint32 *cpu_fte, uint32 *l2cpmac_table)
{
    ...
    if (!SOC_WARM_BOOT(unit) && (size > 0)) {
        BCM_L2CACHE_VVERB(("L2CACHE: allocating 0x%x (%d) bytes of scache:",  
    ...
    if (BCM_FAILURE(rv)) {
        L2CACHE_ERR(("%s: programming exception table failed (%s)\n", 
                     FUNCTION_NAME(), _SHR_ERRMSG(rv)));
        return rv;
    }
#endif
    ...
    rv = _sbx_gu2_resource_alloc(unit, SBX_GU2K_USR_RES_FTE_UNICAST, 1, 
                                 &fte_idx, 0);
    if (rv != BCM_E_NONE) {
        L2CACHE_ERR(("%s: failed to allocate an FTE. \n", FUNCTION_NAME()));
        return rv;
    }
    ...
    if (rv != BCM_E_NONE) {
        L2CACHE_ERR(("%s: failed to set cpu FTE 0x%x for l2cp\n", 
    ...
    if (rv != BCM_E_NONE) {
        _sbx_gu2_resource_free(unit, SBX_GU2K_USR_RES_FTE_UNICAST, 1, 
                       &fte_idx, 0);
        L2CACHE_ERR(("%s: failed to allocate an OHI\n", FUNCTION_NAME()));
        return rv;
    }
    ...
    if (rv != BCM_E_NONE) {
        ...
        L2CACHE_ERR(("%s: allocate ENCAP ETE failed.\n", FUNCTION_NAME()));
        return rv;
    }
    ...
    if (rv != BCM_E_NONE) {
        ...
        _sbx_gu2_resource_free(unit, SBX_GU2K_USR_RES_ETE_ENCAP, 1, 
                               &encap_ete_idx, 0);
        L2CACHE_ERR(("%s: failed to allocate an ETE\n", FUNCTION_NAME()));
        return rv;
    }
...
_bcm_fe2000_l2_cache_entry_delete(int unit, int idx)
{
    ...
    if (!(flags & L2CACHE_ENTRY_VALID)) {
        L2CACHE_ERR(("%s: Invalid L2CP entry at idx %d\n", FUNCTION_NAME(), idx));
        return BCM_E_PARAM;
    }
                                      ...
                                      bcm_l2_cache_addr_t *addr)
{
    ...
    flags = entry->flags;
    if (!(flags & L2CACHE_ENTRY_VALID)) {
        L2CACHE_ERR(("%s: Invalid entry at idx %d\n", FUNCTION_NAME(), idx));
        return BCM_E_PARAM;
    }
                                      ...
                                      bcm_l2_cache_addr_t *addr)
{
    ...
    if (!(mask_null || mask_em || mask_prefix5)) {
        L2CACHE_ERR(("%s: Unsupported mask null:%d em:%d p5:%d\n",
    ...
    if ((rsvd_dmac) && !(mask_null || mask_em || mask_prefix5)) {
        L2CACHE_ERR(("%s: Unsupported mask null:%d em:%d p5:%d rsvd_dmac:%d\n", 
    ...
    if (addr->src_port_mask && (flags & (~(BCM_L2_CACHE_CPU | 
        ...
        L2CACHE_ERR(("%s: src_port_mask supported only with BCM_L2_CACHE_CPU,"
    ...
    if (addr->src_port_mask && (!rsvd_dmac)) {
        ...
        L2CACHE_ERR(("%s: src_port_mask supported only with rsvd_dmac "
                     "addresses \n", FUNCTION_NAME()));
        return BCM_E_PARAM;
    }
    if (flags & BCM_L2_CACHE_LOOKUP) {
        ...
        if (addr->flags & BCM_L2_CACHE_MULTICAST) {
            if (!addr->group) {
                L2CACHE_ERR(("%s: Group not specified with BCM_L2_CACHE_MULTICAST"
                             "flag \n", FUNCTION_NAME()));
                return BCM_E_PARAM;
            }
            /* encap_id is ignored */
            if (addr->encap_id) {
                L2CACHE_VVERB(("%s: Ignoring encap_id. \n", FUNCTION_NAME()));
            }
            entry->flags |= L2CACHE_ENTRY_FTE_VALID;
            entry->flags |= L2CACHE_ENTRY_MULTICAST;
        }
    ...
    if (addr->flags & BCM_L2_CACHE_TUNNEL) {
        ...
        if (BCM_GPORT_IS_MPLS_PORT(addr->dest_port)) {
            ...
            if (!_FE2K_MPLS_PORT_FTE_VALID(unit, fte_idx)) {
                L2CACHE_ERR(("%s: Invalid mpls gport in dest_port (%d) \n",
            ...
            if ((addr->dest_modid != mymodid) && 
                ...
                L2CACHE_ERR(("%s:encap_id not specified for remote "
                             "dest_modid\n", FUNCTION_NAME()));
                return BCM_E_PARAM;
            }
        }
...
_bcm_fe2000_l2_cache_index_get(int unit, bcm_l2_cache_addr_t *addr, int *real_idx)
{
    int rv = BCM_E_NONE;
    int port, subtype = 0;
    int mask_null, mask_em, mask_prefix5, rsvd_dmac;
...
bcm_fe2000_l2_cache_delete_all(int unit)
{
    ...
    for (index=0; index<L2CACHE_MAX_IDX; index++) {
        ...
        if (entry->flags & L2CACHE_ENTRY_VALID) {
            rv = _bcm_fe2000_l2_cache_entry_delete(unit, index);
            if (rv != BCM_E_NONE) {
                L2CACHE_ERR(("%s: Failed deleting entry with index - %d\n", 
                             FUNCTION_NAME(), index));
                break;
            }
        }
...
bcm_fe2000_l2_cache_detach(int unit)
{
    ...
    if (rv != BCM_E_NONE) {
        L2CACHE_ERR(("%s: Error deleting existing entries. Init failed.\n",
                     FUNCTION_NAME()));
        return rv;
    }
#ifdef BCM_WARM_BOOT_SUPPORT
...
bcm_fe2000_l2_cache_init(int unit)
{
    ...
    if (!_l2_cache_glob_lock) {
        ...
        if (!local_lock) {
            L2CACHE_ERR(("%s: sal_mutex_create failed. \n", FUNCTION_NAME()));
            return BCM_E_RESOURCE;
        }
        if (sal_mutex_take(local_lock, sal_mutex_FOREVER) == 0) {
            ...
            if (sal_mutex_give(local_lock) != 0) {
                L2CACHE_ERR(("%s: sal_mutex_give failed.\n", FUNCTION_NAME()));
                sal_mutex_destroy(local_lock);
                return BCM_E_INTERNAL;
            }
        } else {
            L2CACHE_ERR(("%s: sal_mutex_take failed. \n", FUNCTION_NAME()));
            sal_mutex_destroy(local_lock);
            return BCM_E_INTERNAL;
        }
    ...
    if (_l2_cache[unit].lock) {
        ...
        L2CACHE_WARN(("%s: Re-initializing. Clearing all existing entries\n",
                      FUNCTION_NAME()));
        rv = bcm_fe2000_l2_cache_delete_all(unit);
        if (rv != BCM_E_NONE) {
            L2CACHE_ERR(("%s: Error deleting existing entries. Init failed.\n",
        ...
        if (!local_lock) {
            L2CACHE_ERR(("%s: mutex create failed. \n", FUNCTION_NAME()));
            rv = BCM_E_RESOURCE;
        }
        ...
        if (rv == BCM_E_NONE) {
            ...
            if (rv != BCM_E_NONE) {
                L2CACHE_ERR(("%s: list create for user L2CACHE entries failed."
                             " Unit %d\n", FUNCTION_NAME(), unit));
            }
        }
...
bcm_fe2000_l2_cache_size_get(int unit, int *size)
{
    ...
    *size = L2CACHE_MAX_IDX;
    /* TODO: Or should we return current number of entries ? */
                        ...
                        int *index_used)
{
    ...
    if ((addr->flags & BCM_L2_CACHE_SUBTYPE) && 
        ...
        L2CACHE_ERR(("%s: Invalid Mac address for IEEE slow protocol. \n", 
                     FUNCTION_NAME()));
        return BCM_E_PARAM;
    }
    ...
    if (!addr->src_port_mask) {
        ...
        if (BCM_GPORT_IS_MPLS_PORT(addr->src_port)) {
            L2CACHE_ERR(("Source GPort is not supported.\n"));
            return BCM_E_PARAM;
        }
        ...
        if (addr->src_port) {
            ...
            L2CACHE_WARN(("%s: Ignoring the src_port specified \n", 
                          FUNCTION_NAME()));
        }
    } else {
        L2CACHE_ERR(("Un-supported src_port_mask specified. \n"));
        return BCM_E_PARAM;
    }
...
bcm_fe2000_l2_cache_get(int unit, int index, bcm_l2_cache_addr_t *addr)
{
    ...
    if (rv != BCM_E_NONE) {
        L2CACHE_ERR(("%s: Error reading entry at index %d \n", 
                     FUNCTION_NAME(), index));
        return rv;
    }
