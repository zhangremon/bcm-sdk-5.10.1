...
if (((p) < (min))  || ((p) > (max))) {      \
  PORT_ERR((_SBX_D((_u), "Invalid parameter: %s=%d min=%d max=%d\n"), \
                 #p , (p), (min), (max)));                \
  return BCM_E_PARAM; \
}
 ...
 * TPID
 *
 * G2P3 supports only 1 customer and 2 provider TPIDs for the entire
...
typedef struct _g2p3_port_s {

    _tpid_count_t  tpid_count;
    int            *profile_count;
} _g2p3_port_t;

static _g2p3_port_t  _port_state[BCM_LOCAL_UNITS_MAX];

int _bcm_fe2000_g2p3_port_qos_profile_dump(int unit, int profile);
 ...
 */
int
_bcm_fe2000_g2p3_port_ilib_common_init(int unit)
{
    int rv;
    ...
    rv = soc_sbx_g2p3_max_qos_profile_index_get (unit, &max_qos_profile_idx);
    if (BCM_FAILURE(rv)) {
        PORT_ERR(("bcm_port_init: unit=%d rv=%d soc_sbx_g2p3_max_qos_profile_index_get\n",
                  unit, rv));
        return rv;
    } 
    ...
    if (_port_state[unit].profile_count == NULL) {
        PORT_ERR(("bcm_port_init: sal_alloc failed\n"));
        return BCM_E_MEMORY;
    } 
...
int
_bcm_fe2000_g2p3_port_ilib_common_detach(int unit)
{
    if (_port_state[unit].profile_count != NULL) {
...
int
_bcm_fe2000_g2p3_port_ilib_entry_init(int unit,
                                      bcm_port_t port,
                                      bcm_vlan_data_t *vd)
{
    soc_sbx_g2p3_p2e_t    sbx_port;
    soc_sbx_g2p3_ep2e_t   egrport;
    soc_sbx_g2p3_lp_t     sbx_logical_port;
    int                   rv, temp_rv;
    int                   modid;
     ...
     */
    soc_sbx_g2p3_p2e_t_init(&sbx_port);

    sbx_port.customer = 1;
    sbx_port.nativevid  = 0 /* NOT vd->vlan_tag */;
    /* by default enable mpls processing on all ports. To filter
     * mpls packets per port, used port switch controls */
    if( SOC_SBX_CFG(unit)->mplstp_ena) {
        sbx_port.mplstp = 1;
    }

    temp_rv = soc_sbx_g2p3_p2e_set(unit, port, &sbx_port);
    if (BCM_FAILURE(temp_rv)) {
        rv = temp_rv;
        PORT_ERR(("bcm_port_init: unit=%d rv=%d port=%d "
                  "oc_sbx_g2p3_p2e_set\n",
                  unit, rv, port));
        return rv;
    }

    soc_sbx_g2p3_ep2e_t_init(&egrport);

    temp_rv = soc_sbx_g2p3_ep2e_set(unit, port, &egrport);
    if (BCM_FAILURE(temp_rv)) {
        rv = temp_rv;
        PORT_ERR(("bcm_port_init: unit=%d rv=%d port=%d "
     ...
     */
    soc_sbx_g2p3_lp_t_init(&sbx_logical_port);

    BCM_IF_ERROR_RETURN(bcm_stk_my_modid_get(unit, &modid));

    sbx_logical_port.pid = SOC_SBX_PORT_SID(unit, modid, port);

    temp_rv = soc_sbx_g2p3_lp_set(unit, port, &sbx_logical_port);
    if (BCM_FAILURE(temp_rv)) {
        rv = temp_rv;
        PORT_ERR(("bcm_port_init: unit=%d rv=%d port=%d soc_sbx_g2p3_lp_set\n",
                  unit, rv, port));
    }
...
int
_bcm_fe2000_g2p3_port_ilib_egr_init(int unit, bcm_port_t port)
{
     ...
     */
    etel2 = SOC_SBX_PORT_ETE(unit, port);
    ohi   = SOC_SBX_PORT_OHI(port);

    soc_sbx_g2p3_oi2e_t_init(&sbx_ohi);
    sbx_ohi.eteptr = etel2;

    temp_rv = soc_sbx_g2p3_oi2e_set(unit, ohi - SBX_RAW_OHI_BASE, &sbx_ohi);
    if (BCM_FAILURE(temp_rv)) {
        rv = temp_rv;
        PORT_ERR(("bcm_port_init: unit=%d port=%d soc_sbx_g2p3_oi2e_set\n",
                  unit, port));
    }

    soc_sbx_g2p3_etel2_t_init(&sbx_etel2);
    sbx_etel2.mtu = SBX_DEFAULT_MTU_SIZE;
    sbx_etel2.remark = PORT(unit, port).egr_remark_table_idx; 
    temp_rv = soc_sbx_g2p3_etel2_set(unit, etel2, &sbx_etel2);
    PORT_VERB(("write tag %d:eteL2[%08X]\n", unit, etel2));
    if (BCM_FAILURE(temp_rv)) {
        rv = temp_rv;
        PORT_ERR(("bcm_port_init: unit=%d port=%d soc_sbx_g2p3_etel2_set\n",
                  unit, port));
    }

    /*
     * In g2p3, untagged is the same as tagged, stripping comes from
     * setting epv2e.strip = 1.
     */
    temp_rv = soc_sbx_g2p3_etel2_set(unit,
                                     SOC_SBX_PORT_UT_ETE(unit, port),
                                     &sbx_etel2);
    PORT_VERB(("write untag %d:eteL2[%08X]\n",
               unit,
               SOC_SBX_PORT_UT_ETE(unit, port)));
    if (BCM_FAILURE(temp_rv)) {
        rv = temp_rv;
...
int
_bcm_fe2000_g2p3_port_ilib_tpid_init(int unit, bcm_port_t port)
{
    soc_sbx_g2p3_tpid_t  g2p3_tpid;

    if (SOC_WARM_BOOT(unit)) {
        soc_sbx_g2p3_p2e_t  p2e;
        int                 rv;

        rv = soc_sbx_g2p3_p2e_get(unit, port, &p2e);
        if (BCM_FAILURE(rv)) {
            PORT_ERR((_SBX_D(unit, "Failed to recover: p2e[%d] "
                             "read failed: %s\n"),
                      port, bcm_errmsg(rv)));
            return rv;
        }
        ...
        PORT_VERB((_SBX_D(unit, "Port %d p2e.provider=%d twin=%d stpid1=%d\n"),
                   port, p2e.provider, p2e.twintpid, p2e.stpid1));
        PORT_VERB((_SBX_D(unit, "c/t/0/1=%d/%d/%d/%d\n"),
    ...
    soc_sbx_g2p3_tpid_t_init(&g2p3_tpid);
    BCM_IF_ERROR_RETURN
        (soc_sbx_g2p3_tpid_get(unit, SB_G2P3_FE_CTPID_INDEX, &g2p3_tpid));
    if (g2p3_tpid.tpid != TPID_CTPID_DEFAULT) {
        g2p3_tpid.tpid = TPID_CTPID_DEFAULT;
        BCM_IF_ERROR_RETURN
            (soc_sbx_g2p3_tpid_set(unit, SB_G2P3_FE_CTPID_INDEX, &g2p3_tpid));
    }

    soc_sbx_g2p3_tpid_t_init(&g2p3_tpid);
    BCM_IF_ERROR_RETURN
        (soc_sbx_g2p3_tpid_get(unit, SB_G2P3_FE_STPID0_INDEX, &g2p3_tpid));
    if (g2p3_tpid.tpid != TPID_STPID0_DEFAULT) {
        g2p3_tpid.tpid = TPID_STPID0_DEFAULT;
        BCM_IF_ERROR_RETURN
            (soc_sbx_g2p3_tpid_set(unit, SB_G2P3_FE_STPID0_INDEX, &g2p3_tpid));
    }

    soc_sbx_g2p3_tpid_t_init(&g2p3_tpid);
    BCM_IF_ERROR_RETURN
        (soc_sbx_g2p3_tpid_get(unit, SB_G2P3_FE_STPID1_INDEX, &g2p3_tpid));
    if (g2p3_tpid.tpid != TPID_STPID1_DEFAULT) {
        g2p3_tpid.tpid = TPID_STPID1_DEFAULT;
        BCM_IF_ERROR_RETURN
            (soc_sbx_g2p3_tpid_set(unit, SB_G2P3_FE_STPID1_INDEX, &g2p3_tpid));
    }
...
int
_bcm_fe2000_g2p3_port_ilib_fte_init(int unit, int node, bcm_port_t port)
{
    int                 temp_rv, rv = BCM_E_NONE;
    soc_sbx_g2p3_ft_t   sbx_fte;
    int                 queue;
    int                 fte_idx;
     ...
     */
    soc_sbx_g2p3_ft_t_init(&sbx_fte);
    sbx_fte.qid     = queue;

    temp_rv = soc_sbx_g2p3_ft_set(unit, fte_idx, &sbx_fte);
    if (BCM_FAILURE(temp_rv)) {
        rv = temp_rv;
        PORT_ERR(("bcm_port_init: unit=%d node %d port%d fte_idx=%d rv=%d "
...
int
bcm_fe2k_egr_path_get(int unit, uint32 fti,
                      ...
                      uint32               *etei)
{
    ...
    if (BCM_FAILURE(rv)) {
        PORT_ERR((_SBX_D(unit, "Failed to read FT[0x%x]: %d (%s)\n"),
                  fti, rv, bcm_errmsg(rv)));
        return rv;
    }

    rv = soc_sbx_g2p3_oi2e_get(unit, ((ft->oi) - SBX_RAW_OHI_BASE), oh);
    if (BCM_FAILURE(rv)) {
        PORT_ERR((_SBX_D(unit, "Failed to read oi2e[0x%x] FT[0x%x]: %d (%s)\n"),
                  ft->oi, fti, rv, bcm_errmsg(rv)));
        return rv;
    }

    rv = soc_sbx_g2p3_eteencap_get(unit, oh->eteptr, eteencap);
    if (BCM_FAILURE(rv)) {
        PORT_ERR((_SBX_D(unit, "Failed to read ETE Encap[0x%x] FT[0x%x] "
                         "oi2e[0x%x]: %d (%s)\n"),
    ...
    if (eteencap->l2ete == 0) {
        /* if there is no l2 ete in the encap, oh->eteptr must point
         * to an l2 ete and not an encap
         */
        *etei = oh->eteptr;
    } else {
        *etei = eteencap->l2ete;
    ...
    rv = soc_sbx_g2p3_etel2_get(unit, *etei, etel2);
    if (BCM_FAILURE(rv)) {
        PORT_ERR((_SBX_D(unit, "Failed to read ETE L2[0x%x] FT[0x%x] "
                         "oi2e[0x%x] Ete Encap [0x%x]: %d (%s)"),
                  *etei, fti, ft->oi, *etei, rv, bcm_errmsg(rv)));
        return rv;
    }
 ...
 */
int
_bcm_fe2000_g2p3_lp_path_get(int unit, bcm_port_t port,
                             soc_sbx_g2p3_ft_t *ft, uint32 *ftIdx,
                             soc_sbx_g2p3_oi2e_t *oh,
                             soc_sbx_g2p3_etel2_t *ete, uint32 *eteIdx)
{
    soc_sbx_g2p3_eteencap_t eteencap;
    int rv = BCM_E_INTERNAL;

    rv = _bcm_fe2000_vlan_fte_gport_get(unit, port, ftIdx);
    if (rv != BCM_E_NONE) {
        return rv;
    }

    rv = bcm_fe2k_egr_path_get(unit, *ftIdx, ft, oh, &eteencap, ete, eteIdx);
...
/*
 *   Function
 *      _bcm_fe2000_g2p3_port_lp_discard_action
 *   Purpose
 ...
 */
int
_bcm_fe2000_g2p3_port_lp_discard_access(int unit,
                                        bcm_port_t port,
                                        int *discardTagged,
                                        int *discardUntagged,
                                        int set)
{
    ...
    uint32                ftIdx, eteIdx;
    soc_sbx_g2p3_ft_t     ft;
    soc_sbx_g2p3_oi2e_t   outHeader;
    soc_sbx_g2p3_etel2_t  ete;

    rv = _bcm_fe2000_g2p3_lp_path_get(unit, port, &ft, &ftIdx,
                                      &outHeader, &ete, &eteIdx);

    if (set) {
        ete.dropuntagged = *discardUntagged;
        ete.droptagged   = *discardTagged;
        rv = soc_sbx_g2p3_etel2_set(unit, eteIdx, &ete);
        if (BCM_FAILURE(rv)) {
            PORT_ERR(("%s: unit=%d Failed to write ETE on "
...
/*
 *   Function
 *      _bcm_fe2000_g2p3_port_lp_untagged_vlan_access
 *   Purpose
 ...
 */
int
_bcm_fe2000_g2p3_port_lp_untagged_vlan_access(int unit,
                                              bcm_port_t port,
                                              bcm_vlan_t *vid,
                                              int set)
{
...
/*
 *   Function
 *      _bcm_fe2000_g2p3_port_lp_untagged_priority_access
 *   Purpose
 ...
 */
int
_bcm_fe2000_g2p3_port_lp_untagged_priority_access(int unit,
                                                  bcm_port_t port,
                                                  int *priority,
                                                  int set)
{
...
int
_bcm_fe2000_g2p3_port_vlan_lp_set(int unit, bcm_module_t modid,
 ...
 */
int
_bcm_fe2000_g2p3_port_ilib_lp_init(int unit, bcm_module_t modid,
                                   bcm_port_t port)
{
    int                 rv = BCM_E_NONE;
    uint32_t            lp_index;
    ...
    soc_sbx_g2p3_lp_t_init(&sbx_logical_port);
    rv = soc_sbx_g2p3_lp_get(unit, lp_index, &sbx_logical_port);
    if (BCM_E_NONE == rv) {
    sbx_logical_port.pid = SOC_SBX_PORT_SID(unit, fab_node, fab_port);
    rv = soc_sbx_g2p3_lp_set(unit, lp_index, &sbx_logical_port);
    }
    ...
    soc_sbx_g2p3_ep2e_t_init(&egrport);
    rv = soc_sbx_g2p3_ep2e_get(unit, port, &egrport);
    if (rv == BCM_E_NONE) {
        egrport.pid = SOC_SBX_PORT_SID(unit, fab_node, fab_port);
        rv = soc_sbx_g2p3_ep2e_set(unit, port, &egrport);
    }
        ...
        if (rv == BCM_E_NONE) {
            ...
            for (idx=0; idx < vlan_count; idx++) {
                 if (BCM_PBMP_MEMBER(vlan_list[idx].port_bitmap,
                                     port)) {
                      rv = _bcm_fe2000_g2p3_port_vlan_lp_set(unit,
                                                        modid,
                                                        port,
...
int
_bcm_fe2000_g2p3_port_ilib_well_known_egr_init(int unit)
{
    int                     rv = BCM_E_NONE;
    soc_sbx_g2p3_etel2_t    sbx_etel2;
    soc_sbx_g2p3_oi2e_t     sbx_oi2e;
    uint32                  ete_l2;
    ...
    if ((!_sbx_port_handler[unit]->ete_l2_valid) ||
        (_sbx_gu2_resource_test(unit, SBX_GU2K_USR_RES_ETE_L2,
        ...
        if (BCM_FAILURE(rv)) {
            ...
            PORT_ERR(("bcm_port_init: unit=%d Failed to alloc ete\n", unit));
            return rv;
        }
    ...
    if (SOC_WARM_BOOT(unit)) {
        rv = soc_sbx_g2p3_oi2e_get(unit, SBX_RAW_OHI_BASE - SBX_RAW_OHI_BASE,
                                   &sbx_oi2e);
        if (BCM_FAILURE(rv)) {
            PORT_ERR((_SBX_D(unit, "Failed to get raw OHI: %s\n"),
                      bcm_errmsg(rv)));
            return rv;
        }

        if (sbx_oi2e.eteptr != ete_l2) {
            PORT_ERR((_SBX_D(unit, "Warmboot Failed.  "
    ...
    sbx_etel2.stpcheck = 0;
    sbx_etel2.mtu = SBX_DEFAULT_MTU_SIZE;
    sbx_etel2.nostrip = 0;
    sbx_etel2.noclass = 1;
    sbx_etel2.usevid = 1;
    sbx_etel2.vid = 0xfff;
    sbx_etel2.nosplitcheck = 1;

    rv = soc_sbx_g2p3_etel2_set(unit, ete_l2, &sbx_etel2);

    if (BCM_SUCCESS(rv)) {
        soc_sbx_g2p3_oi2e_t_init(&sbx_oi2e);
        sbx_oi2e.eteptr = ete_l2;
        rv = soc_sbx_g2p3_oi2e_set(unit, SBX_RAW_OHI_BASE - SBX_RAW_OHI_BASE, &sbx_oi2e);
    }
 ...
 */
int
_bcm_fe2000_port_qos_map_create(int unit, int flags, bcm_port_t port)
{
    int                    rv, map_id, hw_id;
    uint32                 etei;
    soc_sbx_g2p3_etel2_t   ete;
    ...
    map_id = _bcm_fe2000_qos_hw_id_to_map_id(flags, map_id);

    rv = bcm_qos_map_create(unit, flags, &map_id);
    if (BCM_FAILURE(rv)) {
        PORT_ERR((_SBX_D(unit, "Port %d failed to create qos map: %s\n"),
                  port, bcm_errmsg(rv)));
        return rv;
    }
    
    /* save it, update porte etes */
    hw_id = _bcm_fe2000_qos_map_id_to_hw_id(flags, map_id);

    PORT_VERB((_SBX_D(unit, "Port %d changing remark idx from 0x%x to 0x%x\n"),
    ...
    PORT_VERB((_SBX_D(unit, "Updating port %d ete 0x%x remark\n"),
               port, etei));

    rv = soc_sbx_g2p3_etel2_get(unit, etei, &ete);
    if (BCM_FAILURE(rv)) {
        PORT_ERR((_SBX_D(unit, "failed to read ete 0x%04x: %d (%s)\n"), 
    ...
    rv = soc_sbx_g2p3_etel2_set(unit, etei, &ete);
    if (BCM_FAILURE(rv)) {
        PORT_ERR((_SBX_D(unit, "failed to write ete 0x%04x: %d (%s)\n"), 
                  etei, rv, bcm_errmsg(rv)));
        return rv;
    }
...
int
_bcm_fe2000_g2p3_port_qos_init(int unit)
{
    bcm_port_t            port;
    bcm_pbmp_t            pbmp;
    int                   rv, etei;
    ...
    BCM_PBMP_ITER(pbmp, port) {
        ...
        if (SOC_WARM_BOOT(unit)) {
            ...
            rv = soc_sbx_g2p3_etel2_get(unit, etei, &ete);
            if (BCM_FAILURE(rv)) {
                PORT_ERR((_SBX_D(unit, "Failed to get ete[0x%x]: %s\n"),
                          etei, bcm_errmsg(rv)));
                return rv;
            }
            ...
            if (ete.remark != SBX_QOS_DEFAULT_REMARK_IDX) {
                ...
                rv = _bcm_fe2000_port_qos_map_create(unit, flags, port);
                if (BCM_FAILURE(rv)) {
                    PORT_ERR((_SBX_D(unit, "Failed to create qos map: %s\n"),
                              bcm_errmsg (rv)));
                    return rv;
                }
            }
...
int
_bcm_fe2000_g2p3_port_qos_detach(int unit)
{
    bcm_port_t            port;
    bcm_pbmp_t            pbmp;
    int                   map_id, rv;
    ...
    BCM_PBMP_ITER(pbmp, port) {
        ...
        {
            map_id = PORT(unit, port).egr_remark_table_idx;
            map_id = _bcm_fe2000_qos_hw_id_to_map_id(BCM_QOS_MAP_EGRESS, 
                                                     map_id);

            rv = bcm_qos_map_destroy(unit, map_id);
            if (BCM_FAILURE(rv)) {
                PORT_ERR((_SBX_D(unit, "Failed to destroy qos map 0x%x (0x%x)"
                                 ": %s\n"),
                          map_id, PORT(unit, port).egr_remark_table_idx,
                          bcm_errmsg(rv)));
...
/*
 * Function:
 *     _bcm_fe2000_g2p3_port_ctpid_set
 * Description:
 *     Set the Customer Tag Protocol ID for a port.
 *     G2P3 has only 1 CTPID to be shared for all ports.
 * Parameters:
 *     unit - Device number
 ...
 */
STATIC int
_bcm_fe2000_g2p3_port_ctpid_set(int unit, bcm_port_t port, uint16 tpid)
{
    soc_sbx_g2p3_tpid_t  g2p3_tpid;
    ...
    if (g2p3_tpid.tpid == tpid) {  /* Nothing to do */
        return BCM_E_NONE;
    }
    ...
    if (TPID_COUNT(unit).ctpid > 1) {
        PORT_ERR(("%s: unit=%d CTPID in use by %d ports, cannot change.\n",
                  FUNCTION_NAME(), unit, TPID_COUNT(unit).ctpid));
            return BCM_E_RESOURCE;
    }

    if (TPID_COUNT(unit).twin > 1) {
        PORT_ERR(("%s: unit=%d CTPID used as STPID by %d ports, cannot change.\n",
...
/*
 * Function:
 *     _bcm_fe2000_g2p3_port_stpid_set
 * Description:
 *     Set the Provider Tag Protocol ID for a port.
 *     G2P3 has 2 STPID to be shared for all ports, and only 1 can be
 *     active per port.
 * Parameters:
 *     unit - Device number
 *     port - Device port number
 *     tpid - Tag Protocol ID
 *     p2e  - (IN/OUT) G2P3 p2e configuration
 * Returns:
 *     BCM_E_NONE - Success
 *     BCM_E_XXX  - Failure
 * Notes:
 ...
 */
STATIC int
_bcm_fe2000_g2p3_port_stpid_set(int unit, bcm_port_t port, uint16 tpid,
                                soc_sbx_g2p3_p2e_t *p2e,
                                soc_sbx_g2p3_ep2e_t *ep2e, int *out_tpid_idx)
{
    soc_sbx_g2p3_tpid_t  g2p3_tpid, g2p3_tpid2, g2p3_ctpid;
    ...
    /* Get active STPID */
    if (p2e->stpid1) {  /* S-TPID1 */
        tpid_index  = SB_G2P3_FE_STPID1_INDEX;
        tpid_index2 = SB_G2P3_FE_STPID0_INDEX;
        p_count     =  &(TPID_COUNT(unit).stpid1);
        p_count2    =  &(TPID_COUNT(unit).stpid0);

    } else  {  /* S-TPID0 */
        tpid_index  = SB_G2P3_FE_STPID0_INDEX;
        tpid_index2 = SB_G2P3_FE_STPID1_INDEX;
    ...
    rv = soc_sbx_g2p3_tpid_get(unit, SB_G2P3_FE_CTPID_INDEX, &g2p3_ctpid);
    if (BCM_FAILURE(rv)) {
        PORT_ERR(("%s: unit=%d Error=%d (%s) failed to get CTPID.\n",
    ...
    rv = soc_sbx_g2p3_tpid_get(unit, tpid_index, &g2p3_tpid);
    if (BCM_FAILURE(rv)) {
        PORT_ERR(("%s: unit=%d Error=%d (%s) failed to get STPID %d.\n",
                  FUNCTION_NAME(), unit, rv, bcm_errmsg(rv), tpid_index));
        return rv;
    }

    if ( ((p2e->twintpid == 1) && g2p3_ctpid.tpid == tpid) ||
         ((p2e->twintpid == 0) && g2p3_tpid.tpid  == tpid) )
    {
        /* Nothing to do */
        return BCM_E_NONE;
    }
    ...
    soc_sbx_g2p3_tpid_t_init(&g2p3_tpid2);
    rv = soc_sbx_g2p3_tpid_get(unit, tpid_index2, &g2p3_tpid2);
    if (BCM_FAILURE(rv)){
        PORT_ERR(("%s: unit=%d Error=%d (%s) failed to get STPID %d.\n",
    ...
    select_twin = FALSE;
    if (g2p3_ctpid.tpid == tpid) {
        select_tpid2 = FALSE;
        select_twin  = TRUE;
    } else if (g2p3_tpid.tpid == tpid) {
        select_tpid2 = FALSE;
    } else if ((g2p3_tpid2.tpid == tpid) || (*p_count2 == 0))  {
        select_tpid2 = TRUE;
    } else if (*p_count <= 1) {
        select_tpid2 = FALSE;
    } else {
    ...
    if (select_twin) {
        ...
        rv = soc_sbx_g2p3_p2e_set(unit, port, p2e);
        if (BCM_FAILURE(rv)) {
            PORT_ERR(("%s: unit=%d Error=%d (%s) failed to set p2e.\n",
                      FUNCTION_NAME(), unit, rv, bcm_errmsg(rv)));
            return rv;
        }
        ...
        if (BCM_FAILURE(rv)) {
            PORT_ERR(("%s: unit=%d Error=%d (%s) failed to set ep2e.\n",
                      FUNCTION_NAME(), unit, rv, bcm_errmsg(rv)));
            return rv;
        }
        ...
        if (select_tpid2) {
            ...
            /* Set new TPID value */
            if (g2p3_tpid2.tpid != tpid) {
                g2p3_tpid2.tpid = tpid;
                BCM_IF_ERROR_RETURN
                    (soc_sbx_g2p3_tpid_set(unit, tpid_index2, &g2p3_tpid2));
            }

            /* Select secondary Provider TPID */
            if (tpid_index2 == SB_G2P3_FE_STPID1_INDEX) {
                p2e->stpid1 = 1;
                ep2e->stpid1 = 1;
                ep2e->stpid0 = 0;
            } else {
            ...
            BCM_IF_ERROR_RETURN(soc_sbx_g2p3_p2e_set(unit, port, p2e));
            BCM_IF_ERROR_RETURN(soc_sbx_g2p3_ep2e_set(unit, port, ep2e));
            ...
            g2p3_tpid.tpid = tpid;
            BCM_IF_ERROR_RETURN
                (soc_sbx_g2p3_tpid_set(unit, tpid_index, &g2p3_tpid));
            ...
            if (updateTwinCount) {
                if (tpid_index == SB_G2P3_FE_STPID0_INDEX) {
                    ep2e->stpid0 = 1;
                    ep2e->stpid1 = 0;
                } else {
                    ep2e->stpid0 = 0;
    ...
    PORT_VERB(("%s %d: tpidCounts c/1/2/t=%d/%d/%d/%d\n",
               FUNCTION_NAME(), unit, TPID_COUNT(unit).ctpid,
               TPID_COUNT(unit).stpid0, TPID_COUNT(unit).stpid1,
               TPID_COUNT(unit).twin));
...
/*
 * Function:
 *     _bcm_fe2000_g2p3_port_tpid_set
 * Description:
 ...
 */
int
_bcm_fe2000_g2p3_port_tpid_set(int unit, bcm_port_t port, uint16 tpid)
{
    int                  rv;
    soc_sbx_g2p3_p2e_t   p2e;
    ...
    BCM_IF_ERROR_RETURN(soc_sbx_g2p3_ep2e_get(unit, port, &ep2e));
    if (!p2e.customer && !p2e.provider) {
        return BCM_E_CONFIG;
    }

    if (p2e.customer) {
        rv = _bcm_fe2000_g2p3_port_ctpid_set(unit, port, tpid);
    } else {
        rv = _bcm_fe2000_g2p3_port_stpid_set(unit, port, tpid, &p2e,
                                             &ep2e, &tpid_idx);
    }
...
/*
 * Function:
 *     _bcm_fe2000_g2p3_port_tpid_get
 * Description:
 ...
 */
int
_bcm_fe2000_g2p3_port_tpid_get(int unit, bcm_port_t port, uint16 *tpid)
{
    soc_sbx_g2p3_p2e_t  p2e;
    int                 tpid_index;
    ...
    if (p2e.customer || p2e.twintpid) {  /* Customer C-TPID */
        tpid_index = SB_G2P3_FE_CTPID_INDEX;
    ...
    if (tpid_index >= 0) {
        ...
        soc_sbx_g2p3_tpid_t_init(&g2p3_tpid);
        BCM_IF_ERROR_RETURN
            (soc_sbx_g2p3_tpid_get(unit, tpid_index, &g2p3_tpid));
        *tpid = g2p3_tpid.tpid;
...
/*
 * Function:
 *     _bcm_fe2000_g2p3_port_inner_tpid_set
 * Purpose:
 ...
 */
int
_bcm_fe2000_g2p3_port_inner_tpid_set(int unit, bcm_port_t port, uint16 tpid)
{
    soc_sbx_g2p3_p2e_t   p2e;
    ...
    return _bcm_fe2000_g2p3_port_ctpid_set(unit, port, tpid);
}


/*
 * Function:
 *     _bcm_fe2000_g2p3_port_inner_tpid_get
 * Purpose:
 ...
 */
int
_bcm_fe2000_g2p3_port_inner_tpid_get(int unit, bcm_port_t port, uint16 *tpid)
{
    soc_sbx_g2p3_p2e_t   p2e;
    soc_sbx_g2p3_tpid_t  g2p3_tpid;
...
/*
 * Function:
 *     _bcm_fe2000_g2p3_port_tpid_add
 * Purpose:
 ...
 *     BCM_E_UNAVAIL
 * Notes:
 * In G2P3, each given port can only have 1 'active' Provider STPID, 0 or 1.
 * (set in the soc_sbx_g2p3_p2e_t configuration).
 *
 * G2P3 has 2 provider STPIDs. The second STPID is used as an extra
 * TPID available for ports to use (set in bcm_port_tpid_set,
 * when in provider mode).
 */
int
_bcm_fe2000_g2p3_port_tpid_add(int unit, bcm_port_t port,
...
/*
 * Function:
 *     _bcm_fe2000_g2p3_port_tpid_delete
 * Purpose:
 ...
 *     BCM_E_UNAVAIL
 * Notes:
 *     API 'port_tpid_add()' is not available in G2P3, similarly,
 *     'port_tpid_delete()' returns 'unavail'.
 *     If user wants to set a TPID to zero, the 'port_tpid_set()'
 *     API can be used.
 */
int
_bcm_fe2000_g2p3_port_tpid_delete(int unit, bcm_port_t port, uint16 tpid)
{
    return BCM_E_UNAVAIL;
}
...
/*
 * Function:
 *     _bcm_fe2000_g2p3_port_tpid_delete_all
 * Purpose:
 ...
 */
int
_bcm_fe2000_g2p3_port_tpid_delete_all(int unit, bcm_port_t port)
{
    return BCM_E_UNAVAIL;
}

int
_bcm_fe2000_g2p3_port_dtag_mode_set(int unit, bcm_port_t port, int mode)
{
    soc_sbx_g2p3_p2e_t    p2e;
    soc_sbx_g2p3_ep2e_t   ep2e;
    int                   rv = BCM_E_NONE;
    ...
    case BCM_PORT_DTAG_MODE_INTERNAL:
        ...
        ep2e.stpid0   = !p2e.stpid1 && !p2e.twintpid;
        ep2e.stpid1   = p2e.stpid1 && !p2e.twintpid;
        break;
    ...
    if (rv == BCM_E_NONE) {
        /* let VLAN refresh nativeVid information */
...
int
_bcm_fe2000_g2p3_port_dtag_mode_get(int unit, bcm_port_t port, int *mode)
{
    soc_sbx_g2p3_p2e_t    p2e;
    soc_sbx_g2p3_ep2e_t   ep2e;
    int                   rv = BCM_E_NONE;
...
int
_bcm_fe2000_g2p3_port_strip_tag(int unit, bcm_port_t port, int strip)
{
    soc_sbx_g2p3_p2e_t portData;
    int                rv;

    rv = soc_sbx_g2p3_p2e_get(unit, port, &portData);
    if (BCM_FAILURE(rv)) {
        PORT_ERR(("%s: unit=%d port=%d rv=%d Failed to read port data. \n",
                  FUNCTION_NAME(), unit, port, rv));
    }
    portData.defstrip = !!strip;
    rv = soc_sbx_g2p3_p2e_set(unit, port, &portData);
    if (rv == BCM_E_NONE) {
        /* let VLAN refresh nativeVid information */
        rv = bcm_fe2000_g2p3_vlan_port_nativevid_touch(unit, port);
    }
...
int
_bcm_fe2000_g2p3_port_untagged_priority_get(int unit,
                                            bcm_port_t port,
                                            int *priority)
{
    soc_sbx_g2p3_p2e_t sbx_port;
    ...
    rv = soc_sbx_g2p3_p2e_get(unit, port, &sbx_port);
    if (BCM_FAILURE(rv)) {
        return rv;
    }
...
int
_bcm_fe2000_g2p3_port_qos_map_get(int unit,
                                  bcm_port_t port,
                                  bcm_vlan_t vid,
                                  int isPortVid,
                                  int de, int pri,
                                  soc_sbx_g2p3_qos_t *qos)
{
    ...
    if (isPortVid) {
        rv = bcm_fe2000_lp_get_by_pv(unit, port, vid, &lp);
        if (BCM_FAILURE(rv)) {
            PORT_ERR((_SBX_D(unit, "unable to find or access lp for "
                             "pv2e[%d,%03X]: %d (%s)\n"), 
        ...
        if (BCM_FAILURE(rv)) {
            PORT_ERR((_SBX_D(unit, "unable to read lp[%08X]: %d (%s)\n"),
    ...
    /* get the QoS data */
    rv = soc_sbx_g2p3_qos_get(unit, de, pri, lp.qos, qos);
    if (BCM_FAILURE(rv)) {
        PORT_ERR((_SBX_D(unit, "unable to read qos[%02X][%d][%d]: %d (%s)\n"),
...
int
_bcm_fe2000_g2p3_port_dscp_qos_map_get(int unit,
                                  bcm_port_t port,
                                  bcm_vlan_t vid,
                                  int isPortVid,
                                  int dscp,
                                  soc_sbx_g2p3_dscp_qos_t *qos)
{
    int rv;
    soc_sbx_g2p3_lp_t   lp;
    ...
    /* get the QoS data */
    rv = soc_sbx_g2p3_dscp_qos_get(unit, dscp, lp.qos, qos);
    if (BCM_FAILURE(rv)) {
        PORT_ERR((_SBX_D(unit, "unable to read dscp_qos[%02X][%d]: %d (%s)\n"),
...
int
_bcm_fe2000_g2p3_port_vlan_priority_map_get(int unit,
                                            bcm_port_t port,
                                            int pkt_pri,
                                            int cfi,
                                            int *internal_pri,
...
{
    int                   rv;
    soc_sbx_g2p3_qos_t    qosMap;

    rv = _bcm_fe2000_g2p3_port_qos_map_get(unit, port, 0, FALSE,
                                           cfi, pkt_pri, &qosMap);

    if (BCM_FAILURE(rv)) {
        PORT_ERR((_SBX_D(unit, "Failed to get qos map for port %d pri/cfi=%d/%d "
...
int
_bcm_fe2000_g2p3_port_vlan_priority_dscp_map_get(int unit,
                                                  bcm_port_t port,
                                                  int pkt_dscp,
                                                  int *internal_pri,
                                                  bcm_color_t *color,
...
{
    int                   rv;
    soc_sbx_g2p3_dscp_qos_t    qosMap;

    rv = _bcm_fe2000_g2p3_port_dscp_qos_map_get(unit, port, 0, FALSE,
                                           pkt_dscp, &qosMap);

    if (BCM_FAILURE(rv)) {
        PORT_ERR((_SBX_D(unit, "Failed to get qos map for port %d dscp=%d "
...
int
_bcm_fe2000_g2p3_port_vlan_pri_map_get(int unit,
                                       bcm_port_t port,
                                       bcm_vlan_t vid,
                                       int pkt_pri,
                                       int cfi,
                                       ...
                                       bcm_color_t *color)
{
    ...
    rv = _bcm_fe2000_g2p3_port_qos_map_get(unit, port, vid,
                                           TRUE /* isPortVid */,
                                           cfi, pkt_pri, &qos);
...
int
_bcm_fe2000_g2p3_port_vlan_priority_mapping_get(int unit,
                                                bcm_port_t port,
                                                bcm_vlan_t vid,
                                                int pkt_pri,
                                                int cfi,
...
STATIC int
_bcm_fe2000_g2p3_port_qos_map_alloc(int unit,
                                    uint32_t *pProfileId)
{
    int                rv;
    int                pri;
    ...
    int                dscp;
    soc_sbx_g2p3_qos_t qos;
    soc_sbx_g2p3_dscp_qos_t dscp_qos;
    ...
    rv = _sbx_gu2_resource_alloc(unit,
                                 ...
                                 0);
    if (BCM_FAILURE(rv)) {
        PORT_ERR(("unable to allocate QOS map on unit %d: %d (%s)\n",
                  unit,
                  rv,
                  _SHR_ERRMSG(rv)));
        return rv;
            ...
            if (BCM_E_NONE == rv) {
                rv = soc_sbx_g2p3_qos_set(unit,
                                          ...
                                          *pProfileId,
                                          &qos);
                if (BCM_E_NONE != rv) {
                    PORT_ERR(("unable to write %d:qos[%02X][%d,%d]: %d (%s)\n",
                              unit,
                              *pProfileId,
                              cfi,
                              pri,
                ...
                PORT_ERR(("unable to read %d:qos[%02X][%d,%d]: %d (%s)\n",
                          unit,
                          SBX_QOS_DEFAULT_PROFILE_IDX,
                          cfi,
                          pri,
   ...
   for(dscp = 0; (dscp <64) && (BCM_E_NONE ==rv) ; dscp ++) {
            ...
            if (BCM_E_NONE == rv) {
                rv = soc_sbx_g2p3_dscp_qos_set(unit,
                                               ...
                                               *pProfileId,
                                               &dscp_qos);
                if (BCM_E_NONE != rv) {
                    PORT_ERR(("unable to write %d:dscp_qos[%02X][%d]: %d (%s)\n",
                              unit,
                              *pProfileId,
                              dscp,
                              rv,
                ...
                PORT_ERR(("unable to read %d:dscp_qos[%02X][%d]: %d (%s)\n",
                          unit,
                          SBX_QOS_DEFAULT_PROFILE_IDX,
                          dscp,
                          rv,
    ...
    if (BCM_E_NONE != rv) {
        ...
        _sbx_gu2_resource_free(unit,
                               ...
                               pProfileId,
                               0);
        PORT_ERR(("freed QOS map due to errors initialising it\n"));
    }
...
int
_bcm_fe2000_g2p3_port_profile_copy(int unit,
                                   uint32_t destProfileIdx,
                                   uint32_t srcProfileIdx)
{
    int rv;
    int de, pri;
    int dscp;
    ...
    PORT_VERB((_SBX_D(unit, "Copy profile %d to %d\n"), srcProfileIdx, destProfileIdx));
    for (de = 0; de < 2; de++) {
        for (pri = 0; pri < 8; pri++) {

            rv = soc_sbx_g2p3_qos_get(unit, de, pri, srcProfileIdx, &qos);
            if (BCM_FAILURE(rv)) {
                PORT_ERR((_SBX_D(unit, "unable to read qos map[%d,%d,%d] : %d (%s)\n"),
                          de, pri, srcProfileIdx, rv, bcm_errmsg(rv)));
                return rv;
            }

            rv = soc_sbx_g2p3_qos_set(unit, de, pri, destProfileIdx, &qos);
            if (BCM_FAILURE(rv)) {
                PORT_ERR((_SBX_D(unit, "unable to write qos map[%d,%d,%d] : %d (%s)\n"),
    ...
    for (dscp = 0; dscp < 64; dscp++) {
            ...
            rv = soc_sbx_g2p3_dscp_qos_get(unit, dscp, srcProfileIdx, &dscp_qos);
            if (BCM_FAILURE(rv)) {
                PORT_ERR((_SBX_D(unit, "unable to read dscp_qos map[%d,%d] : %d (%s)\n"),
                          dscp, srcProfileIdx, rv, bcm_errmsg(rv)));
                return rv;
            }

            rv = soc_sbx_g2p3_dscp_qos_set(unit, dscp, destProfileIdx, &dscp_qos);
            if (BCM_FAILURE(rv)) {
                PORT_ERR((_SBX_D(unit, "unable to write qos map[%d,%d] : %d (%s)\n"),
...
/*
 *   Function
 *      _bcm_fe2000_g2p3_port_profile_shareable
 *   Purpose
 ...
 */
int
_bcm_fe2000_g2p3_port_profile_shareable(int unit, uint32_t curProfileIdx,
                                        int deChange, int priChange,
                                        int dscpChange, int dscp_modify,
                                        soc_sbx_g2p3_qos_t *qosChange,
                                        soc_sbx_g2p3_dscp_qos_t *dscpQosChange,
                                        uint32_t *shrProfileIdx)
{
    int        rv = BCM_E_NOT_FOUND;
    uint32_t   searchIdx;
    int        sharePriProfile, shareDscpProfile;
    int        de, pri,dscp;
    soc_sbx_g2p3_qos_t *pQosRequest;
    soc_sbx_g2p3_qos_t  qosRequest, qosInUse;
    soc_sbx_g2p3_dscp_qos_t *pDscpQosRequest;
    soc_sbx_g2p3_dscp_qos_t  dscpQosRequest, dscpQosInUse;

    if (BCM_DEBUG_CHECK (BCM_DBG_PORT | BCM_DBG_VERBOSE)) {

        if(!dscp_modify)
        {
            PORT_OUT((_SBX_D(unit, "cprof=%d de/pri=%d/%d qosMap:\n  "), curProfileIdx,
                      deChange, priChange));
            soc_sbx_g2p3_qos_print(unit, qosChange);
            PORT_OUT(("\n"));
       } else {
            PORT_OUT((_SBX_D(unit, "cprof=%d dscp=%d dscpQosMap:\n  "), curProfileIdx,
                         dscpChange));
            soc_sbx_g2p3_dscp_qos_print(unit, dscpQosChange);
            PORT_OUT(("\n"));
       }
    }

    /* all three must be set to see this amount of data - */
    if (BCM_DEBUG_CHECK (BCM_DBG_PORT | BCM_DBG_VVERBOSE | BCM_DBG_VERBOSE)) {
        PORT_OUT((_SBX_D(unit, "Finding shareable map for profile=%d\n"), 
                  curProfileIdx));
        _bcm_fe2000_g2p3_port_qos_profile_dump(unit, curProfileIdx);
    }
    ...
    for (searchIdx = 0;
        ...
        if (PROFILE_REF_COUNT(unit, searchIdx) ||
            ...
            /* all three must be set to see this amount of data - */
            if (BCM_DEBUG_CHECK (BCM_DBG_PORT | BCM_DBG_VVERBOSE | BCM_DBG_VERBOSE)) {
                PORT_OUT((_SBX_D(unit, "Comparing against profile=%d\n"), searchIdx));
                _bcm_fe2000_g2p3_port_qos_profile_dump(unit, searchIdx);
            }
            ...
            for (de=0; de < 2 && sharePriProfile; de++) {
                for (pri=0; pri < 8 && sharePriProfile; pri++) {
                    ...
                    rv = soc_sbx_g2p3_qos_get(unit, de, pri, searchIdx,
                                              &qosInUse);
                    if (BCM_FAILURE(rv)) {
                        PORT_ERR((_SBX_D(unit, "Error %d(%s) reading qos table.  "
                    ...
                    if (deChange == de && priChange == pri && !dscp_modify) {
                        ...
                        rv = soc_sbx_g2p3_qos_get(unit, de, pri, curProfileIdx,
                                                  &qosRequest);
                        if (BCM_FAILURE(rv)) {
                            PORT_ERR((_SBX_D(unit, "Error %d(%s) reading given qos table.  "
                                         "profile=%d de/pri=%d/%d\n"),
                                      rv,_SHR_ERRMSG(rv),
                    ...
                    if (sal_memcmp(pQosRequest, &qosInUse, sizeof(qosInUse)) != 0) {
                        PORT_VERB((_SBX_D(unit, "profiles differ, cannot be shared. \n"
            ...
            for(dscp = 0; dscp <64 && shareDscpProfile; dscp ++) {
                    ...
                    rv = soc_sbx_g2p3_dscp_qos_get(unit, dscp, searchIdx,
                                              &dscpQosInUse);
                    if (BCM_FAILURE(rv)) {
                        PORT_ERR((_SBX_D(unit, "Error %d(%s) reading dscp qos table.  "
                                     "profile=%d dscp=%d\n"),
                                  rv,_SHR_ERRMSG(rv),
                                  searchIdx, dscp));
                        return rv;
                    ...
                    if (dscpChange == dscp && dscp_modify) {
                        ...
                        rv = soc_sbx_g2p3_dscp_qos_get(unit, dscp, curProfileIdx,
                                                  &dscpQosRequest);
                        if (BCM_FAILURE(rv)) {
                            PORT_ERR((_SBX_D(unit, "Error %d(%s) reading given dscp qos table.  "
                                         "profile=%d dscp=%d\n"),
                                      rv,
                                      _SHR_ERRMSG(rv),
                                      curProfileIdx, dscp));
            ...
            if (shareDscpProfile && sharePriProfile) {
                PORT_VERB((_SBX_D(unit, "profiles match, sharing profile=%d\n"),
                           searchIdx));
    ...
    if (rv == BCM_E_NOT_FOUND &&
        ...
        PORT_VERB((_SBX_D(unit, "Only user of profile=%d, marked as shareable\n"),
                   curProfileIdx));
...
/*
 *   Function
 *      _bcm_fe2000_g2p3_port_qos_profile_manage
 *   Purpose
 ...
 */
 STATIC int
_bcm_fe2000_g2p3_port_qos_profile_manage(int unit,
                                         bcm_port_t port,
                                         bcm_vlan_t vid,
                                         int isPortVid,
                                         int de, int pri,
                                         int dscp, int dscp_change,
                                         soc_sbx_g2p3_qos_t *qosMapChange,
                                         soc_sbx_g2p3_dscp_qos_t *dscpQosMapChange)
{
    int                         rv;
    int                         updateMap = FALSE;
    _bcm_fe2k_vlan_divergence_t divergence = _FE2K_VLAN_DIVERGE_QOS;
    int                         causeDivergence;
    int                         forceAlloc = 0;
    uint32_t                    shrProfileIdx;
    uint32_t                    oldProfileIdx;
    ...
    if (isPortVid) {
        ...
        rv = bcm_fe2000_lp_get_by_pv(unit, port, vid, &lp);
        if (BCM_FAILURE(rv)) {
            PORT_ERR((_SBX_D(unit, "failed to get lp for port/vid=%d/0x%x: %d (%s)\n"),
                      port, vid, rv, bcm_errmsg(rv)));
            return rv;
        }
    } else {
        ...
        rv = soc_sbx_g2p3_lp_get(unit, port, &lp);
        if (BCM_FAILURE(rv)) {
            PORT_ERR((_SBX_D(unit, "failed to read lp for port %d: %d (%s)\n"), 
                      port, rv, bcm_errmsg(rv)));
            return rv;
        }
    }
    ...
    if (causeDivergence) {
        rv = bcm_fe2000_lp_check_diverged(unit, port, vid, &divergence);
        if (BCM_E_NONE != rv) {
            PORT_ERR((_SBX_D(unit, "unable to check lp divergence for pv2e[%d,0x%03X]"
                         ": %d (%s)\n"),  port, vid, rv, bcm_errmsg(rv)));
            return rv;
        }
        forceAlloc = !(divergence & _FE2K_VLAN_DIVERGE_QOS);
    }
    ...
    if (forceAlloc) {
        ...
        PORT_VERB((_SBX_D(unit, "Divergence caused; force new profile allocation\n")));
        rv = BCM_E_NOT_FOUND;  /* allocate a new profile */
    } else {

        /* Find a shareable qos profile, if one exists */
        rv = _bcm_fe2000_g2p3_port_profile_shareable(unit, lp.qos,
                                                     de, pri,
                                                     dscp, dscp_change,
                                                     qosMapChange,
                                                     dscpQosMapChange,
    ...
    if (rv == BCM_E_NOT_FOUND ||
         ...
         */
        if (rv == BCM_E_NOT_FOUND) {
            rv = _bcm_fe2000_g2p3_port_qos_map_alloc(unit, &shrProfileIdx);
            if (BCM_FAILURE(rv)) {
                PORT_ERR((_SBX_D(unit, "unable to allocate QOS map for pv2e[%d,%03X]:"
                             " %d (%s)\n"), port, vid, rv, bcm_errmsg(rv)));
                return rv;
            }

            /* copy the existing to the newly allocated */
            _bcm_fe2000_g2p3_port_profile_copy(unit,
        ...
        if (isPortVid) {
            rv = bcm_fe2000_lp_set_by_pv_and_diverge(unit, port, vid,
                                                     _FE2K_VLAN_DIVERGE_QOS,
                                                     &lp);

            if (BCM_FAILURE(rv)) {
                PORT_ERR((_SBX_D(unit, "unable to write lp for pv2e[%d,%03X]: %d (%s)\n"),
                          port, vid, rv, bcm_errmsg(rv)));
                ...
                _sbx_gu2_resource_free(unit,
                                       ...
                                       &shrProfileIdx,
                                       0);
                PORT_ERR((_SBX_D(unit, "freed QOS map due to errors using it\n")));
                return rv;
            }
            ...
            if (BCM_FAILURE(rv)) {
                PORT_ERR((_SBX_D(unit, "unable to write lp for port %d: %d (%s)\n"),
                          port, rv, bcm_errmsg(rv)));

                /* could not write newly allocated qos map; free it */
                _sbx_gu2_resource_free(unit,
                                       SBX_GU2K_USR_RES_QOS_PROFILE, 1,
                                       &shrProfileIdx, 0);
                PORT_ERR((_SBX_D(unit, "freed QOS map due to errors using it\n")));
                return rv;
            }
            ...
            rv = bcm_fe2000_vlan_port_lp_replicate(unit, port,
                                                   BCM_FE2K_LP_COPY_QOS,
                                                   &tempLp, &lp);
            if (BCM_FAILURE(rv)) { 
                PORT_ERR((_SBX_D(unit, "unable to update pv2e entries for new QOS:"
                             " %d (%s)\n"), rv, bcm_errmsg(rv)));
                return rv;
            }
        }
    ...
    if (BCM_FAILURE(rv)) {
        PORT_ERR((_SBX_D(unit, "unexpected error while searching for a shareable "
                     "qos profile: %d (%s)\n"),
                  rv, bcm_errmsg(rv)));
        return rv;
    }
    ...
    if ((shrProfileIdx != oldProfileIdx) && !forceAlloc) {
        ...
        if (PROFILE_REF_COUNT(unit, oldProfileIdx) == 0 &&
            ...
            PORT_VERB((_SBX_D(unit, "Freeing profile=%d\n"), oldProfileIdx));
            _sbx_gu2_resource_free(unit,
                                   SBX_GU2K_USR_RES_QOS_PROFILE, 1,
                                   &oldProfileIdx, 0);
        }
    ...
    PORT_VERB((_SBX_D(unit, "updated profile reference counts to:  "
        ...
        lp.qos != SBX_QOS_DEFAULT_PROFILE_IDX)
    {
       ...
       if(!dscp_change)
       {
            rv = soc_sbx_g2p3_qos_set(unit, de, pri, lp.qos, qosMapChange);
            if (BCM_FAILURE(rv)) { 
                PORT_ERR((_SBX_D(unit, "unable to update qos map profile= %d de,pri=%d,%d"
                             " :%d (%s)\n"), lp.qos, de, pri, 
                          rv, bcm_errmsg(rv)));
                return rv;
            }
       ...
       {
            rv = soc_sbx_g2p3_dscp_qos_set(unit, dscp, lp.qos, dscpQosMapChange);
            if (BCM_FAILURE(rv)) { 
                PORT_ERR((_SBX_D(unit, "unable to update dscp_qos map profile= %d dscp=%d"
                             " :%d (%s)\n"), lp.qos, dscp, 
                          rv, bcm_errmsg(rv)));
                return rv;
           }
...
/*
 *   Function
 *      _bcm_fe2000_g2p3_port_egr_remark_set
 *   Purpose
 ...
 */
static int
_bcm_fe2000_g2p3_port_egr_remark_set(int unit, bcm_port_t port, 
                                     int remark, uint32 dscp_remark)
{
    ...
    etei = SOC_SBX_PORT_ETE(unit, port);
    PORT_VERB((_SBX_D(unit, "Updating port %d ete 0x%04x\n"), port, etei));
                                              ...
                                              bcm_vlan_t vid)
{
    ...
    PORT_VERB((_SBX_D(unit, "Setting port/vid %d/0x%x back to default\n"), port, vid));

    rv = SOC_SBX_G2P3_PV2E_GET(unit, port, vid, &pv2e);
    if (BCM_FAILURE(rv)) {
        PORT_ERR((_SBX_D(unit, "failed to get pv2e for port/vid=%d/0x%x: %d (%s)\n"),
                  port, vid, rv, bcm_errmsg(rv)));
        return rv;
    }
    ...
    PORT_VERB((_SBX_D(unit, "current qos profile=%d refCount=%d\n"), 
    ...
    if (PROFILE_REF_COUNT(unit, lp.qos)) {
        ...
        PORT_ERR((_SBX_D(unit, "invalid reference count for qos profile %d\n"), lp.qos));
        return BCM_E_INTERNAL;
    }
    ...
    rv = soc_sbx_g2p3_lp_set(unit, pv2e.lpi, &lp);
    if (BCM_FAILURE(rv)) {
        PORT_ERR((_SBX_D(unit, "failed to write lp for port/vid=%d/0x%x: %d (%s)\n"),
                  port, vid, rv, bcm_errmsg(rv)));
        return rv;
    }
...
int
_bcm_fe2000_g2p3_port_vlan_priority_map_set(int unit,
                                            bcm_port_t port,
                                            int pkt_pri,
                                            int cfi,
                                            int internal_pri,
                                            ...
                                            int policer_offset)
{
    int rv;
    soc_sbx_g2p3_qos_t qosMap;
    soc_sbx_g2p3_dscp_qos_t dscpQosMap;
    ...
    soc_sbx_g2p3_qos_t_init(&qosMap);
    soc_sbx_g2p3_dscp_qos_t_init(&dscpQosMap);
    ...
    rv = _bcm_fe2000_g2p3_port_qos_profile_manage(unit, port, 0, FALSE,
                                                  cfi, pkt_pri,0, FALSE,
                                                  &qosMap, &dscpQosMap);
    return rv;
}
...
int
_bcm_fe2000_g2p3_port_vlan_pri_map_set(int unit,
                                       bcm_port_t port,
                                       bcm_vlan_t vid,
                                       int pkt_pri,
                                       int cfi,
                                       ...
                                       int color)
{
    ...
    rv = _bcm_fe2000_g2p3_port_qos_profile_manage(unit, port, vid, TRUE,
                                                  cfi, pkt_pri,0, FALSE,
                                                  &qosMap, &dscpQosMap);
    return rv;
}
...
int
_bcm_fe2000_g2p3_port_vlan_priority_unmap_get(int unit,
                                              bcm_port_t port,
                                              int internal_pri,
                                              bcm_color_t color,
                                              int *pkt_pri,
...
{
    int                   rv;
    soc_sbx_g2p3_remark_t zEgrRemark;
    ...
    if (BCM_E_NONE == rv) {
        ...
        *cfi      = zEgrRemark.cfi;
        ...
        PORT_ERR(("unable to read %d:remark[%02X][%d,%d,%d]: %d (%s)\n",
                  unit,
                  PORT(unit,port).egr_remark_table_idx,
                  0,
                  color,
...
int
_bcm_fe2000_g2p3_port_vlan_priority_unmap_set(int unit,
                                              bcm_port_t port,
                                              int internal_pri,
                                              bcm_color_t color,
                                              int pkt_pri,
                                              int cfi)
{
    ...
    PORT_VERB((_SBX_D(unit, "Port %d updating remark=%d\n"),
    ...
    if (BCM_E_NONE != rv) {
        PORT_ERR(("unable to update %d:remark[%02X][%d,%d,%d]: %d (%s)\n",
                  unit,
                  PORT(unit,port).egr_remark_table_idx,
                  0,
                  color,
...
/*
 * Function:
 *     _bcm_fe2000_g2p3_port_frame_max_access
 * Description:
 ...
 */
int
_bcm_fe2000_g2p3_port_frame_max_access(int unit, bcm_port_t port,
                                       int *size, int set)
{
    soc_sbx_g2p3_etel2_t  ete;
    int                   eteIdx;
    ...
    soc_sbx_g2p3_etel2_t_init(&ete);
    BCM_IF_ERROR_RETURN(soc_sbx_g2p3_etel2_get(unit, eteIdx, &ete));
    ...
    ete.mtu = *size-4;
    BCM_IF_ERROR_RETURN(soc_sbx_g2p3_etel2_set(unit, eteIdx, &ete));
...
int
_bcm_fe2000_g2p3_port_qos_profile_dump(int unit, int profile)
{
    int de, pri, dscp;
    soc_sbx_g2p3_qos_t qosEnt;
    soc_sbx_g2p3_dscp_qos_t dscpQosEnt;
    int rv;

    soc_cm_print("de pri | e dp cos fcos mefcos\n");
    soc_cm_print("-------|---------------------\n");
    for (de=0; de < 2; de++) {
        for (pri=0; pri < 8; pri++) {
            rv = soc_sbx_g2p3_qos_get(unit, de, pri, profile, &qosEnt);
            if (BCM_SUCCESS(rv)) {
                soc_cm_print("%2d %3d | %d %2d %3d %4d %6d\n",
                             de, pri, qosEnt.e, qosEnt.dp, qosEnt.cos,
                             qosEnt.fcos, qosEnt.mefcos);
            }
        }
    }
    soc_cm_print("dscp | e dp cos fcos mefcos\n");
    soc_cm_print("-----|---------------------\n");

    for (dscp=0; dscp < 64; dscp++) {
        rv = soc_sbx_g2p3_dscp_qos_get(unit, dscp, profile, &dscpQosEnt);
            if (BCM_SUCCESS(rv)) {
                soc_cm_print("%2d | %d %2d %3d %4d %6d\n",
...
int
_bcm_fe2000_g2p3_port_vlan_dscp_map_set(int unit,
                                        bcm_port_t port,
                                        bcm_vlan_t vlan,
                                        int dscp,
                                        int internal_pri,
                                        bcm_color_t color)
{
    ...
    rv = _bcm_fe2000_g2p3_port_qos_profile_manage(unit, port, vlan, TRUE,
                                                  0,0,dscp,TRUE,
                                                  &qosMap,&dscpQosMap );
    return rv;
...
int
_bcm_fe2000_g2p3_port_dscp_map_set(int unit,
                                   bcm_port_t port,
                                   int dscp,
                                   int internal_pri,
                                   bcm_color_t color)
...
int
_bcm_fe2000_g2p3_port_vlan_dscp_map_get(int unit,
                                        bcm_port_t port,
                                        bcm_vlan_t vid,
                                        int pkt_dscp,
                                        int *internal_pri,
                                        bcm_color_t *color)
{
    ...
    rv = _bcm_fe2000_g2p3_port_dscp_qos_map_get(unit, port, vid,
                                                TRUE /* isPortVid */,
                                                pkt_dscp, &qos);

    if (BCM_FAILURE(rv)) {
        PORT_ERR((_SBX_D(unit, "Failed to get dscp_qos map for port %d dscp=%d/$d "
                     ": %d %s\n"), port, pkt_dscp, rv, bcm_errmsg(rv)));
        return rv;
    }
...
int
_bcm_fe2000_g2p3_port_dscp_map_get(int unit,
                                   bcm_port_t port,
                                   int pkt_dscp,
                                   int *internal_pri,
                                   int *color)
{
    ...
    rv = _bcm_fe2000_g2p3_port_dscp_qos_map_get(unit, port, 0,
                                                FALSE /* isPortVid */,
                                                pkt_dscp, &qos);
...
int
_bcm_fe2000_g2p3_port_dscp_unmap_set(int unit,
                                     bcm_port_t port,
                                     int internal_pri,
                                     bcm_color_t color,
                                     int pkt_dscp)
{
    ...
    rv = soc_sbx_g2p3_etel2_get(unit, etel2, &sbx_etel2);
    PORT_VERB(("Read tag %d:eteL2[%08X]\n", unit, etel2));
    if (BCM_FAILURE(rv)) {
        PORT_ERR(("port_dscp_unmap_set: unit=%d port=%d soc_sbx_g2p3_etel2_get\n",
                  unit, port));
        return rv;
    }

    sbx_etel2.dscpremark = 1;
    rv = soc_sbx_g2p3_etel2_set(unit, etel2, &sbx_etel2);
    PORT_VERB(("Write tag %d:eteL2[%08X]\n", unit, etel2));
    if (BCM_FAILURE(rv)) {
        PORT_ERR(("port_dscp_unmap_set: unit=%d port=%d soc_sbx_g2p3_etel2_set\n",
                  unit, port));
        return rv;
    }
...
int
_bcm_fe2000_g2p3_port_dscp_unmap_get(int unit,
                                     bcm_port_t port,
                                     int internal_pri,
                                     bcm_color_t color,
                                     int *pkt_dscp)
{
    ...
    PORT_VERB(("Read tag %d:eteL2[%08X]\n", unit, etel2));
    if (BCM_FAILURE(rv)) {
        PORT_ERR(("port_dscp_unmap_get: unit=%d port=%d soc_sbx_g2p3_etel2_get\n",
                  unit, port));
        return rv;
    }

    /* check if dscp remark is configured. Else return error */
    if( ! sbx_etel2.dscpremark)
    {
      PORT_VERB(("DSCP remark not configure for unit %d, port %d\n", unit, port));
      return BCM_E_DISABLED;
    }
...
/*
 * Function:
 *     _bcm_fe2000_g2p3_port_dscp_map_mode_get
 * Description:
...
int
_bcm_fe2000_g2p3_port_dscp_map_mode_get(int unit, bcm_port_t port, int *mode)
{
    int rv;
    soc_sbx_g2p3_lp_t lp;
...
/*
 * Function:
 *     _bcm_fe2000_g2p3_port_dscp_map_mode_set
 * Description:
...
int
_bcm_fe2000_g2p3_port_dscp_map_mode_set(int unit, bcm_port_t port, int mode)
{
    int rv;
    soc_sbx_g2p3_lp_t lp;
    ...
    rv = soc_sbx_g2p3_lp_set(unit, port, &lp);
    if (BCM_FAILURE(rv)) {
        PORT_ERR((_SBX_D(unit, "failed to write lp for port %d: %d (%s)\n"), 
                  port, rv, bcm_errmsg(rv)));
        return rv;
    }
    return rv;
...
/*
 *   Function
 *      _bcm_fe2000_g2p3_port_qosmap_set
 *   Purpose
 ...
 */
int
_bcm_fe2000_g2p3_port_qosmap_set(int unit, bcm_port_t port,
                                 ...
                                 uint32 ingFlags, uint32 egrFlags)
{
    ...
    PORT_VERB((_SBX_D(unit, "(%d,%d,%d) enter\n"), port, ingrMap, egrMap));

    if (ingrMap >= 0) {
        bcm_vlan_t vid;
        uint32_t   logicalPort = 0, ltype = BCM_GPORT_INVALID;
        uint32_t *fastLpi = sal_alloc(sizeof(uint32_t) * (BCM_VLAN_MAX + 1),
                            "fastLpi");
        if (fastLpi == NULL) {
            PORT_ERR((_SBX_D(unit, "unable to alloc %d bytes for"
                                   " unit %d port temp data\n"),
                      sizeof(uint32_t) * (BCM_VLAN_MAX + 1),unit));
            return BCM_E_INTERNAL;
        }

        rv = soc_sbx_g2p3_pv2e_lpi_fast_get(unit, 0, port,
                                            0xFFF, port,
    ...
    rv = _bcm_fe2000_g2p3_port_egr_remark_set(unit, port, egrMap, 
...
/*
 *   Function
 *      _bcm_fe2000_g2p3_port_qosmap_get
 *   Purpose
 ...
 */
int
_bcm_fe2000_g2p3_port_qosmap_get(int unit,bcm_port_t  port, 
...
/*
 *   Function
 *      _bcm_fe2000_g2p3_port_vlan_qosmap_set
 *   Purpose
 ...
 */
int
_bcm_fe2000_g2p3_port_vlan_qosmap_set(int unit, bcm_port_t port,
                                      bcm_vlan_t vid, 
                                      int ing_idx, int egr_idx,
                                      uint32 ing_flags, uint32 egr_flags)
{
    int                    rv = BCM_E_NONE;
    soc_sbx_g2p3_lp_t      lp, port_lp, default_lp;
    soc_sbx_g2p3_pv2e_t    pv2e;
    uint32                 lpi, alloc_lp;
    
    alloc_lp = FALSE;

    PORT_VERB((_SBX_D(unit, "(%d, 0x%03x, %d, %d) enter\n"),
    ...
    if (ing_idx >= 0) {
        ...
        rv = SOC_SBX_G2P3_PV2E_GET(unit, port, vid, &pv2e);
        if (BCM_FAILURE(rv)) {
            PORT_ERR((_SBX_D(unit, "failed to read pv2e[%d,0x%03x]:"
                             " %d (%s)\n"), 
                      port, vid, rv, bcm_errmsg(rv)));
            return rv;
        }

        rv = soc_sbx_g2p3_lp_get(unit, port, &port_lp);
        if (BCM_FAILURE(rv)) {
            PORT_ERR((_SBX_D(unit, "failed to port lp[%d]: %d (%s)\n"),
                      port, rv, bcm_errmsg(rv)));
            return rv;
        }
        ...
        if (ing_idx == 0 && lpi != port) {
             ...
             */
            soc_sbx_g2p3_lp_t_init(&default_lp);

            rv = soc_sbx_g2p3_lp_get(unit, lpi, &lp);
            if (BCM_FAILURE(rv)) {
                PORT_ERR((_SBX_D(unit, "failed to read lp for port,vid"
                                 " %d,0x%03x: %d (%s)\n"),
                          port, vid, rv, bcm_errmsg(rv)));
                return rv;
            }
                ...
                (sal_memcmp(&lp, &default_lp, sizeof(lp)) == 0))
            {
                ...
                PORT_VERB((_SBX_D(unit, "Freeing Logical Port 0x%0x\n"), lpi));
                rv = _sbx_gu2_resource_free(unit, SBX_GU2K_USR_RES_LPORT,
                                            1, &lpi, 0);
                pv2e.lpi = 0;

            } else {
                PORT_VERB((_SBX_D(unit, "Logical Port is non-default, keeping.\n"), 
                           lpi));
            }
            
            if (BCM_FAILURE(rv)) {
                PORT_ERR((_SBX_D(unit, "failed to free logical port 0x%x"
                                 ": %d (%s)\n"),
            ...
            if (BCM_FAILURE(rv)) {
                PORT_ERR((_SBX_D(unit, "failed to allocate logical port"
                                 ": %d (%s)\n"),
                          rv, bcm_errmsg(rv)));
                return rv;
            }
            ...
            PORT_VERB((_SBX_D(unit, "Allocated Logical Port 0x%0x\n"), 
                       lpi));
        }  /* ing_idx != 0 && lpi == port */
        
        PORT_VERB((_SBX_D(unit, "Modifying Logical Port 0x%0x\n"), lpi));
        ...
        if (BCM_FAILURE(rv)) {
            PORT_ERR((_SBX_D(unit, "failed to write lp[0x%x] for port,vid"
                             " %d,0x%03x: %d (%s)\n"),
                      lpi, port, vid, rv, bcm_errmsg(rv)));
            return rv;
        }
        
        rv = SOC_SBX_G2P3_PV2E_SET(unit, port, vid, &pv2e);
        if (BCM_FAILURE(rv)) {
            PORT_ERR((_SBX_D(unit, "failed to read pv2e[%d,0x%03x]:"
                             " %d (%s)\n"), 
                      port, vid, rv, bcm_errmsg(rv)));
    ...
    rv = _bcm_fe2000_g2p3_port_egr_remark_set(unit, port, egr_idx,
                                              !!(egr_flags & BCM_QOS_MAP_L3));
...
/*
 *   Function
 *      _bcm_fe2000_g2p3_port_vlan_qosmap_get
 *   Purpose
 *      Set the qos profile and remark index for the given port,vid
 *   Parameters
 *      (in)  unit       - BCM device number
 ...
 */
int
_bcm_fe2000_g2p3_port_vlan_qosmap_get(int unit, bcm_port_t port,
                                      bcm_vlan_t vid, 
                                      int *ing_idx, int *egr_idx,
                                      uint32 *ing_flags, uint32 *egr_flags)
{
    ...
    PORT_VERB((_SBX_D(unit, "(%d, 0x%03x) enter\n"), port, vid));

    rv = SOC_SBX_G2P3_PV2E_GET(unit, port, vid, &pv2e);
    if (BCM_FAILURE(rv)) {
        PORT_ERR((_SBX_D(unit, "failed to read pv2e[%d,0x%03x]: %d (%s)\n"),
                  port, vid, rv, bcm_errmsg(rv)));
        return rv;
    }
    ...
    rv = soc_sbx_g2p3_lp_get(unit, lpi, &lp);
    if (BCM_FAILURE(rv)) {
        PORT_ERR((_SBX_D(unit, "failed to read lp[%d]: %d (%s)\n"),
                  lpi, rv, bcm_errmsg(rv)));
        return rv;
    }
