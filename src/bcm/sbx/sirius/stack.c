/*
 * $Id: stack.c 1.35.42.1 Broadcom SDK $
                             ...
                             bcm_gport_t *queue_groups)
{
    ...
    unsigned int queue;
    bcm_gport_t qs[256];
    bcm_fabric_qsel_offset_t qso[256];
    int xgsMode = TRUE;
    ...
    if (sir->userPredAction[steer_id - 1]) {
        ...
        BCM_IF_ERROR_RETURN(bcm_fabric_predicate_action_get(unit,
                                                            predActId,
                                                            &predActInfo));
    ...
    if (sir->userAction[steer_id - 1]) {
        ...
        BCM_IF_ERROR_RETURN(bcm_fabric_action_get(unit,
                                                  actId,
                                                  &actInfo));
    ...
    if ((~0) == queueSeg) {
        ...
        BCM_IF_ERROR_RETURN(bcm_sirius_fabric_qsel_create(unit,
                              ...
                              bcm_gport_t fabric_port)
{
	...
	if (soc_feature(unit, soc_feature_source_port_priority_flow_control)) {
		...
		BCM_ERR(("ERROR: %s, 0x%x gport type not supported , unit %d\n",
	    ...
	    BCM_ERR(("ERROR: %s, (Source Port, PG) based PFC is not supported on unit %d\n", FUNCTION_NAME(), unit));	    
	...
	if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: %s, unable to retrieve modid\n", FUNCTION_NAME()));
