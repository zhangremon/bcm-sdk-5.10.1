/*
 * $Id: port.c 1.137.2.10 Broadcom SDK $
...
#define PORT_FUNCMSG(string)    "%s[%d]%s" string, __FILE__, __LINE__, FUNCTION_NAME()
#define PORT_FUNCMSG1(string)   "%s[%d]%s: " string, __FILE__, __LINE__, FUNCTION_NAME()
...
#define PORT_LOCK(unit)  \
    if (sal_mutex_take(SOC_SBX_STATE(unit)->port_state->port_lock, sal_mutex_FOREVER)) { \
        ...
        PORT_ERR((PORT_FUNCMSG1("unable to obtain PORT lock on unit %d\n"), \
                  unit)); \
...
#define PORT_UNLOCK(unit) \
    if (sal_mutex_give(SOC_SBX_STATE(unit)->port_state->port_lock)) { \
        /* Cound not obtain unit lock  */ \
        PORT_ERR((PORT_FUNCMSG1("unable to release PORT lock on unit %d\n"), \
                  unit)); \
        return BCM_E_INTERNAL; \
    }
...
_bcm_sirius_port_prbs_rx_setup(int unit, bcm_port_t port, int nSi, int bEnablePrbs)
{
   ...
   uint32 uRegValue;
    ...
    SOC_DEBUG_PRINT((DK_PORT, "#### %s: Begin.\n", FUNCTION_NAME()));
    SOC_DEBUG_PRINT((DK_PORT, "#### %s: u=%d p=%d nSi=0x%x bEnablePrbs=0x%x\n",
                     FUNCTION_NAME(), unit, port, nSi, bEnablePrbs));
    if (SOC_SBX_STATE(unit)->port_state->uPrbsModeSi) {
        SOC_DEBUG_PRINT((DK_PORT, "#### %s: PRBS Mode: si\n", FUNCTION_NAME()));
	if (nSi < 12) {
	...
	if ((SOC_SBX_CFG(unit)->bSerdesEncoding) || (port_speed == 3125)) { 
	    ...
	    if (port_speed == 6250) {	    
                ...
                SOC_DEBUG_PRINT((DK_PORT, "#### %s: Doing scrambler phy stuff.\n", FUNCTION_NAME()));
		SOC_IF_ERROR_RETURN
        ...
        SOC_DEBUG_PRINT((DK_PORT, "#### %s: Doing 8b10b phy stuff.\n", FUNCTION_NAME()));
	SOC_IF_ERROR_RETURN
	...
	if ((port_speed > 3125) && (SOC_SBX_CFG(unit)->bSerdesEncoding == 0)) {
            SOC_DEBUG_PRINT((DK_PORT, "#### %s: Doing 64b66b phy stuff.\n", FUNCTION_NAME()));
        ...
        if (nSi < 12) {
            ...
            SOC_DEBUG_PRINT((DK_PORT, "#### %s: (Prbs_Mon_En) Writing nSi: %d, SC_TOP_SI_CONFIG0: 0x%08x\n",
            ...
            SOC_DEBUG_PRINT((DK_PORT, "#### %s: (Prbs_Mon_En) Writing nSi: %d, SI_CONFIG0: 0x%08x\n",
                            FUNCTION_NAME(), nSi, uRegValue));
        }
    ...
    /* But, on the receive side, set the status select so that the receive status is set up for PRBS */
        ...
        SOC_DEBUG_PRINT((DK_PORT, "#### %s: PRBS Mode: hc\n", FUNCTION_NAME()));
	SOC_IF_ERROR_RETURN
	...
	/* detect in the hypercore                                                     */
        SOC_DEBUG_PRINT((DK_PORT, "#### %s: 1: Doing 8b10b phy stuff.\n", FUNCTION_NAME()));
	...
	if ((port_speed > 3125) && (SOC_SBX_CFG(unit)->bSerdesEncoding == 0)) {
            ...
            SOC_DEBUG_PRINT((DK_PORT, "#### %s: 1: Doing 64b66b phy stuff.\n", FUNCTION_NAME()));
	    /* Disable or enable 64/66 encoding */
	    SOC_IF_ERROR_RETURN
		(soc_phyctrl_control_set(unit, port, SOC_PHY_CONTROL_64B66B, bEnablePrbs?0:1));
	}
    }
    SOC_DEBUG_PRINT((DK_PORT, "#### %s: Begin. Done...\n", FUNCTION_NAME()));
    return BCM_E_NONE;
}
...
_bcm_sirius_port_prbs_tx_setup(int unit, bcm_port_t port, int nSi, int bEnablePrbs)
{
   ...
   int port_speed;
	...
	if (nSi < 12) {
	    SOC_IF_ERROR_RETURN(READ_SC_TOP_SI_CONFIG0r(unit, nSi, &uRegValue));
            SOC_DEBUG_PRINT((DK_PORT, "#### %s: (Prbs_Gen_En) Reading nSi: %d, SC_TOP_SI_CONFIG0: 0x%08x\n",
                            FUNCTION_NAME(), nSi, uRegValue));
	    soc_reg_field_set(unit, SC_TOP_SI_CONFIG0r, &uRegValue, PRBS_GENERATOR_ENABLEf, (bEnablePrbs)?1:0);
	    SOC_IF_ERROR_RETURN(WRITE_SC_TOP_SI_CONFIG0r(unit, nSi, uRegValue));
            SOC_DEBUG_PRINT((DK_PORT, "#### %s: (Prbs_Gen_En) Writing nSi: %d, SC_TOP_SI_CONFIG0: 0x%08x\n",
                            FUNCTION_NAME(), nSi, uRegValue));
	} else {
	    SOC_IF_ERROR_RETURN(READ_SI_CONFIG0r(unit, (nSi - 12), &uRegValue));
            SOC_DEBUG_PRINT((DK_PORT, "#### %s: (Prbs_Gen_En) Reading nSi: %d, SI_CONFIG0: 0x%08x\n",
                            FUNCTION_NAME(), nSi, uRegValue));
	    soc_reg_field_set(unit, SI_CONFIG0r, &uRegValue, PRBS_GENERATOR_ENABLEf, (bEnablePrbs)?1:0);
	    SOC_IF_ERROR_RETURN(WRITE_SI_CONFIG0r(unit, (nSi - 12), uRegValue));
            SOC_DEBUG_PRINT((DK_PORT, "#### %s: (Prbs_Gen_En) Writing nSi: %d, SI_CONFIG0: 0x%08x\n",
                            FUNCTION_NAME(), nSi, uRegValue));
	}
...
bcm_sirius_port_speed_set(int unit, bcm_port_t port, int speed)
{
    ...
    int subport = 0;
    int intf = 0, portnum = 0, fifo = 0, egroup_num = -1, num_fifos = 0;
	...
	if (sp_info == NULL || sp_info->valid == FALSE) {
        ...
        if ((rv = bcm_sbx_port_get_intf_portoffset(unit, subport, &intf, &portnum)) == BCM_E_NONE) {
	  
	  if (intf >= SB_FAB_DEVICE_SIRIUS_RQ0_INTF) {
	    portnum = 0;
	  }
	  sir->uSubportSpeed[intf][portnum][0][SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_MAX_SIZE] = speed;
	  for (fifo=0; fifo <= sp_info->egroup[0].num_fifos; fifo++) {
	    sir->uSubportSpeed[intf][portnum][0][fifo] = speed;
          }
	...
	if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: Could not retrieve egroup information from port 0x%x\n",port));
	    PORT_UNLOCK(unit);
	    return rv;
	}
        if ((rv = bcm_sbx_port_get_intf_portoffset(unit, subport, &intf, &portnum)) == BCM_E_NONE) {
	  ...
	  sir->uSubportSpeed[intf][portnum][egroup_num][SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_MAX_SIZE] = speed;
	  if (num_fifos % SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_HW_SIZE) {
	      ...
	      num_fifos += (num_fifos % SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_HW_SIZE);
	  }
	  for (fifo=0; fifo <= num_fifos; fifo++)
            sir->uSubportSpeed[intf][portnum][egroup_num][fifo] = speed;
                                ...
                                bcm_gport_t *subport)
{
    ...
    if ((0 > port) || (SB_FAB_DEVICE_SIRIUS_NUM_HG_PORTS <= port)) {
        /* port is not a valid 'bitmappable' port */
        return BCM_E_PARAM;
    }
	...
	for (subport=0; subport < SB_FAB_DEVICE_SIRIUS_MAX_FABRIC_PORTS; subport++) {
		...
		for (idx = 0; idx < SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_MAX_SIZE; idx++) {
                    if (0 == (sp_info->flags & (SBX_SUBPORT_FLAG_TRUNK_MCAST |
                        ...
                        *speed += sp_info->egroup[idx].port_speed;
                    }
		}
 	    }
	}
...
bcm_sirius_port_speed_get(int unit, bcm_port_t port, int *speed)
{
    ...
    int nSi = -1;
    int subport = 0;
    int intf = 0, portnum = 0, egroup_num = -1;
    int mac_lb;
    uint32 uRegValue;
    ...
    if (BCM_GPORT_IS_CHILD(port)) {
	  ...
	  if (intf >= SB_FAB_DEVICE_SIRIUS_RQ0_INTF) {
	    portnum = 0;
            ...
            *speed = sir->uSubportSpeed[intf][portnum][0][SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_MAX_SIZE];
            rv = BCM_E_NONE;
        }
    } else if (BCM_GPORT_IS_EGRESS_CHILD(port)) {
        subport = BCM_GPORT_EGRESS_CHILD_PORT_GET(port);
	  ...
	  if (intf >= SB_FAB_DEVICE_SIRIUS_RQ0_INTF) {
        ...
        subport = -1;
	rv = bcm_sbx_cosq_egress_group_info_get(unit, port, &subport, &egroup_num, NULL);
	if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: Could not retrieve egroup information from port 0x%x\n", port));
	    PORT_UNLOCK(unit);
	    return rv;
	  ...
	  *speed = sir->uSubportSpeed[intf][portnum][egroup_num][SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_MAX_SIZE];
	  rv = BCM_E_NONE;
        }
    } else if (IS_SFI_PORT(unit, port) || IS_SCI_PORT(unit, port)) {
...
bcm_sirius_port_link_get(int unit, bcm_port_t port, int *up)
{
    ...
    *up = FALSE;
    ...
    if (IS_SFI_PORT(unit, port) || IS_SCI_PORT(unit, port)) {
        ...
        /* map port to serdes */
        SIRIUS_SOC_IF_ERROR_UNLOCK_RETURN(_bcm_sirius_port_to_serdes(unit, port, &nSi));
        SOC_DEBUG_PRINT((DK_PORT, "#### %s: u=%d p=%d mapped to Si=%d\n",
		...
		*up = FALSE;
	    ...
	    soc_cm_debug(DK_PORT, "port %d siState 0x%x\n", port, uSiState);
	}	
                                              ...
                                              int *value)
{
    ...
    if (BCM_GPORT_IS_MODPORT(port) ||
        ...
        if (SB_FAB_DEVICE_SIRIUS_CPU_PORT == portIndex) {
            ...
            BCM_ERR(("unit %d gport %08X(%d) not valid for"
        ...
        if (SOC_E_NONE == rv) {
            if (write) {
                if (BCM_GPORT_IS_UCAST_QUEUE_GROUP(*value)) {
                    ...
                    BCM_ERR(("bcmPortControlDefaultQueue can not accept"
                             " value %08X\n", *value));
                    return BCM_E_PARAM;
                }
                if (regData & (~0xFFFF)) {
                    BCM_WARN(("queue ID %08X bits %08X were ignored\n",
                              regData,
                              regData & (~0xFFFF)));
                }
                soc_reg_field_set(unit, regId, &regVal, fieldId, regData & 0xFFFF);
                rv = soc_reg32_write(unit,
                                     soc_reg_addr(unit, regId, REG_PORT_ANY, 0),
                                     regVal);
            } else { /* if (write) */
                regData = soc_reg_field_get(unit, regId, regVal, fieldId);
                rv = _bcm_sirius_fabric_qsel_entry_gport_extrapolate(unit, regData, value);
            } /* if (write) */
        } /* if (SOC_E_NONE == rv) */
        if (SOC_E_NONE != rv) {
            BCM_ERR(("unable to %s bcmPortControlDefaultQueue"
    ...
    } else { /* if (port is MODPORT or EGRESS_MODPORT or (!GPORT)) */
        /* this control is not on CHILD ports */
        BCM_ERR(("port control bcmPortControlDefaultQueue is only"
                 " valid for modport or egress_modport\n"));
        return BCM_E_UNAVAIL;
    } /* if (port is MODPORT or EGRESS_MODPORT or (!GPORT)) */
    return rv;
}
                            ...
                            bcm_port_control_t type, int value)
{
    ...
    SOC_DEBUG_PRINT((DK_PORT, "#### %s: u=%d p=%d type=%d value=0x%x\n",
    ...
    if (BCM_GPORT_IS_CHILD(port) ||
        ...
        if (BCM_E_NONE != rv) {
            /* something went wrong looking for it, assume not local */
            return rv;
        }
        if (myMod != pMod) {
            /* not my module */
    ...
    if (BCM_GPORT_IS_CHILD(port) ||
		...
		if ( (value > LINK_SPECIFIC_CONFIG_EQUALIZATION_HYPERCORE_MAX) || (value < 0)) {
		    ...
		    sal_snprintf(SOC_PORT_NAME(unit,port), sizeof(si->port_name[port]),
                ...
                /* the drive strength.                                          */
                SOC_SBX_CFG_SIRIUS(unit)->uSerdesAbility[nSi] = value;

		if (IS_SFI_PORT(unit, port)) {
		    sfi_port = port - SOC_PORT_MIN(unit, sfi);
		    bcm_sbx_fabric_get_sfi_xbar_map(unit, SOC_SBX_CONTROL(unit)->node_id, sfi_port, &lxbar);
		    bcm_sirius_fabric_map_link_status(unit, lxbar, sfi_port);
		} else {
		    bcm_sirius_fabric_unmap_link_status(unit, sfi_port);
		}
		...
		if ((value == 0) || (value == 1)) {
			...
			if ((SOC_SBX_CFG_SIRIUS(unit)->uSerdesAbility[nSi] != BCM_PORT_ABILITY_SFI_SCI) &&
			    ...
			    BCM_ERR(("ERROR: ability must be SCI for port(%d) if setting arbiter active\n"));
			...
			switch (nSi) {
				...
				break;
			    default:
				BCM_ERR(("ERROR: unit(%d) Cannot set port(%d) si(%d) ability(%d) to active arbiter(0) invalid configuration\n",
					 unit, port, nSi, SOC_SBX_CFG_SIRIUS(unit)->uSerdesAbility[nSi]));
				BCM_ERR(("Valid configurations for arbiter id 0, can use ports sfi0, sfi1, sci0 if ability set to SCI or SFI_SCI\n"));
				PORT_UNLOCK(unit);
				return BCM_E_PARAM;
				break;
			}
			...
			switch (nSi) {
				...
				break;
			    default:
				BCM_ERR(("ERROR: unit(%d) Cannot set port(%d) si(%d) ability(%d) to active arbiter(1) invalid configuration\n",
					 unit, port, nSi, SOC_SBX_CFG_SIRIUS(unit)->uSerdesAbility[nSi]));
				BCM_ERR(("Valid configurations for arbiter id 0, can use ports sfi2, sfi3, sci1 if ability set to SCI or SFI_SCI\n"));
				PORT_UNLOCK(unit);
				return BCM_E_PARAM;
				break;
			}
			...
			BCM_ERR(("Disable currently unsupported, not required to switch between SCI muxing\n"));
			PORT_UNLOCK(unit);
			return BCM_E_PARAM;
		    }
		}
                            ...
                            bcm_port_control_t type, int *value)
{
	...
	*value = SOC_SBX_CFG(unit)->pfc_cos_enable;
	...
	uLaneMask = 1 << (SIRIUS_HYPERCORE_8019_LANE_SHIFT * uLaneAddr);
            ...
            case bcmPortControlPrbsRxStatus:
                ...
                if (SOC_SBX_STATE(unit)->port_state->uPrbsModeSi) {
                    if (nSi < 12) {
                        ...
                        SOC_DEBUG_PRINT((DK_PORT, "#### %s: Reading nSi: %d, SC_TOP_SI_PRBS_STATUS: 0x%08x\n",
                                        FUNCTION_NAME(), nSi, uRegValue));
                        *value = soc_reg_field_get(unit, SC_TOP_SI_PRBS_STATUSr, uRegValue, PRBS_ERR_CNTf);
                        ...
                        SOC_DEBUG_PRINT((DK_PORT, "#### %s: Reading nSi: %d, SI_PRBS_STATUS: 0x%08x\n",
                                        FUNCTION_NAME(), nSi, uRegValue));
		...
		*value = SOC_SBX_CFG_SIRIUS(unit)->linkDriverConfig[nSi].uDriverEqualization;
		...
		if (SOC_SBX_STATE(unit)->port_state->uDriverEqualizationFarEnd[nSi] == -1) {
		    BCM_ERR(("User has not set the bcmPortControlSerdesDriverEqualizationFarEnd value, please set the near end and the far end value of this field\n"));
		    BCM_ERR(("Please start with 0 and increase the value based upon the information provided through tuning\n"));
		    PORT_UNLOCK(unit);
		    return BCM_E_PARAM;
		}
		...
		if (rv) {
		    BCM_ERR(("Error in tuning unit(%d) port(%d)\n", unit, port)); 
		} else {
			...
			if (SOC_SBX_STATE(unit)->port_state->uDriverEqualizationFarEnd[nSi] < 15) {
			    ...
			    soc_cm_print("unit(%d) port(%d): Please increase far end postcursor value from (%d) to (%d) and retune\n", 
					 unit,
					 port,
			...
			soc_cm_print("unit(%d) port(%d): Far end postcursor value selection is good value(%d)\n", 
				     unit,
				     port,
				     SOC_SBX_STATE(unit)->port_state->uDriverEqualizationFarEnd[nSi]);
		...
		if (nSi > 5) {
		    BCM_ERR(("unit(%d) ERROR: Port(%d) cannote be configured as active arbiter\n", unit, port));
		}
		...
		/* 5/1 to sci 1 */
		    ...
		    *value = 1;
		    ...
		    soc_cm_print("Unit(%d) Sirius device port(%d) not configured as sci link\n", unit, port);
		    *value = -1;
		}
		break;
	...
	case bcmPortControlFabricHeaderFormat:
	  *value = SOC_SBX_STATE(unit)->port_state->fabric_header_format;
	  ...
	  if (rv) {
	      ...
	      *value = uData;
	      if (uData == FALSE) {
		  BCM_ERR(("Tuning failed unit(%d) port(%d)\n", unit, port)); 
		  rv = BCM_E_FAIL;
	      } else {
		  rv = BCM_E_NONE;
	      }	 
...
_bcm_sirius_port_probe(int unit, bcm_port_t p, int *okay)
{
    ...
    *okay = FALSE;
	...
	SOC_IF_ERROR_DEBUG_PRINT
	  (rv, (DK_VERBOSE, "soc_phyctrl_speed_set failed port(%d): %s\n", p, bcm_errmsg(rv)));
	if (rv) {
	    return rv;
	}
	...
	SOC_IF_ERROR_DEBUG_PRINT
	  (rv, (DK_VERBOSE, "soc_sirius_hc_encoding_set failed port(%d): %s\n", p, bcm_errmsg(rv)));
	if (rv) {
	    return rv;
	}
    }	
...
{
    int                 rv = BCM_E_NONE, port_enable, eg=0, attid=0;
    bcm_port_t          p;
    pbmp_t              okay_ports;
    pbmp_t              all_ports;
	...
	for (p = 0; p < SB_FAB_DEVICE_SIRIUS_MAX_FABRIC_PORTS; p++) {
	    ...
	    SOC_SBX_STATE(unit)->port_state->subport_info[p].flags = 0;
	    ...
	    SOC_SBX_STATE(unit)->port_state->subport_info[p].unused_fifos = 0;
		...
		SOC_SBX_STATE(unit)->port_state->subport_info[p].egroup[eg].num_fifos = 0;
		SOC_SBX_STATE(unit)->port_state->subport_info[p].egroup[eg].egroup_gport = -1;
		SOC_SBX_STATE(unit)->port_state->subport_info[p].egroup[eg].mem_index = 0;
		SOC_SBX_STATE(unit)->port_state->subport_info[p].egroup[eg].mem_size = 0;
		SOC_SBX_STATE(unit)->port_state->subport_info[p].egroup[eg].port_speed = 0;
                for (attid=0; attid <  SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_MAX_SIZE; attid++) {
                  SOC_SBX_STATE(unit)->port_state->subport_info[p].egroup[eg].fcd[attid] = -1;
                }
	    }
	}
    }
...
bcm_sirius_port_congestion_init(int unit)
{
    ...
    if (congestion_info[unit] == NULL) {
        congestion_info[unit] = sal_alloc((sizeof(bcm_sbx_port_congestion_info_t) * 2 *
	      ...
	      "congestion state");
        if (congestion_info[unit] == NULL) {
            PORT_ERR((PORT_FUNCMSG("congestion init: unit %d unable to allocate memory\n"), unit));
            return(BCM_E_MEMORY);
        }
    }
				    ...
				    int out_of_band)
{
	...
	/* TODO: Fill in LBID and PPD_TYPE                                       */
	...
	if (config->flags & BCM_PORT_CONGESTION_CONFIG_TX) {
	    ...
	    SOC_IF_ERROR_RETURN(READ_MAC_HCFC_CTRLr(unit, port, &val64));
            soc_reg64_field_set(unit, MAC_HCFC_CTRLr, &val64, HCFC_TX_ENf, 1);
            soc_reg64_field_set(unit, MAC_HCFC_CTRLr, &val64, HCFC_OUT_OF_BAND_MODEf,
				(out_of_band?1:0));
	    soc_reg64_field_set(unit, MAC_HCFC_CTRLr, &val64, HCFC_IGNORE_CRCf, 1);
	...
	if (config->flags & BCM_PORT_CONGESTION_CONFIG_RX) {
	     ...
	     */
	    SOC_IF_ERROR_RETURN(READ_MAC_HCFC_CTRLr(unit, port, &val64));
            soc_reg64_field_set(unit, MAC_HCFC_CTRLr, &val64, HCFC_RX_ENf, 1);
            soc_reg64_field_set(unit, MAC_HCFC_CTRLr, &val64, HCFC_OUT_OF_BAND_MODEf,
				(out_of_band?1:0));
	    soc_reg64_field_set(unit, MAC_HCFC_CTRLr, &val64, HCFC_IGNORE_CRCf, 1);
            SOC_IF_ERROR_RETURN(WRITE_MAC_HCFC_CTRLr(unit, port, val64));
		...
		case 8:
		    ...
		    break;
		default:
		    PORT_ERR((PORT_FUNCMSG("HCFC reception %d bits per port not supported on unit %d\n"),
	...
	/* bigmac SAFC */
	/* TODO: SAFC rx for inline-TME mode?
	 */
	SOC_IF_ERROR_RETURN(READ_MAC_TXLLFCCTRLr(unit, port, &val64));
	soc_reg64_field_set(unit, MAC_TXLLFCCTRLr, &val64, LLFC_ENf, 1);
				      ...
				      bcm_port_congestion_config_t *config, void *info)
{
    ...
    if (rv != BCM_E_NONE) {
        PORT_ERR((PORT_FUNCMSG("unable to get module id, unit %d\n"), unit));
        return(rv);
    }
    ...
    if (modid != my_modid) {
        PORT_ERR((PORT_FUNCMSG("module id is not this unit, unit %d, modid: %d expected modid: %d\n"), unit, modid, my_modid));
        return(BCM_E_UNAVAIL);
    }

    if (BCM_GPORT_IS_CONGESTION(port)) {
	if ((port_nbr < 0) || (port_nbr > 1)) {
	    PORT_ERR((PORT_FUNCMSG("Invalid Out of band HCFC port, unit %d, port %d\n"), unit, port_nbr));
	    return(BCM_E_PARAM);
	}
	...
	if (!IS_HG_PORT(unit, port_nbr)) {
	    PORT_ERR((PORT_FUNCMSG("Invalid HiGig port, unit %d, port %d\n"), unit, port_nbr));
	    return(BCM_E_PARAM);
	}
    ...
    if ( (config->flags & BCM_PORT_CONGESTION_CONFIG_MAX_SPLIT4) ||
        ...
        PORT_ERR((PORT_FUNCMSG("Unsupported flags Option specified, unit %d, port %d flag 0x%x\n"), unit, port_nbr, config->flags));
        return(BCM_E_PARAM);
    }
    if (!((config->flags & BCM_PORT_CONGESTION_CONFIG_E2ECC) ||
	  (config->flags & BCM_PORT_CONGESTION_CONFIG_HCFC) ||
	  (config->flags & BCM_PORT_CONGESTION_CONFIG_SAFC)) ) {
        PORT_ERR((PORT_FUNCMSG("Need to specify Flow Control Mode, unit %d, port %d flag 0x%x\n"), unit, port_nbr, config->flags));
        return(BCM_E_PARAM);
    }
    if (!((config->flags & BCM_PORT_CONGESTION_CONFIG_RX) ||
	  (config->flags & BCM_PORT_CONGESTION_CONFIG_TX))) {
        PORT_ERR((PORT_FUNCMSG("Need to specify Flow Control Direction, unit %d, port %d flag 0x%x\n"), unit, port_nbr, config->flags));
        return(BCM_E_PARAM);
    }
    ...
    if (rv != BCM_E_NONE) {
        PORT_ERR((PORT_FUNCMSG("Error configuring congestion HiGig Port, unit %d, port %d\n"), unit, port_nbr));
        return(rv);
    }
	...
	if (rv != BCM_E_NONE) {
	    PORT_ERR((PORT_FUNCMSG("Error configuring congestion (cosq), unit %d, port %d\n"), unit, port_nbr));
	    return(rv);
	}
    }
			      ...
			      int *scheduler_node)
{
	...
	if (port >= SB_FAB_DEVICE_SIRIUS_MAX_FABRIC_PORTS) {
	    BCM_ERR(("ERROR: %s, child gport 0x%x out of range, unit %d\n",
	...
	if (sp_info->valid == FALSE) {
	    BCM_ERR(("ERROR: %s, invalid child gport 0x%x, unit %d\n",
		     FUNCTION_NAME(), gport, unit));
	    return BCM_E_PARAM;
	}	
	...
	if (BCM_GPORT_IS_CHILD(gport)) {
		...
		BCM_ERR(("ERROR: %s, child gport 0x%x has no ingress scheduler resource allocated, unit %d\n",
			 FUNCTION_NAME(), gport, unit));
		return BCM_E_PARAM;	    
	    }
	    ...
	    *scheduler_node = sp_info->ts_scheduler_node;
		...
		BCM_ERR(("ERROR: %s, child gport 0x%x has no egress scheduler resource allocated, unit %d\n",
			 FUNCTION_NAME(), gport, unit));
		return BCM_E_PARAM;	    
	    }
		...
		*scheduler_node = sp_info->es_scheduler_level2_node;
		...
		if (*scheduler_level == SIRIUS_ES_LEVEL_GROUP_SHAPER) {
			...
			*scheduler_node = sp_info->group_shaper;
			...
			BCM_ERR(("ERROR: %s, child gport 0x%x is in trunk, but no group shaper resources allocated, unit %d\n",
				 FUNCTION_NAME(), gport, unit));
			return BCM_E_PARAM;
		    }
		} else {
	...
	if (bcm_sbx_cosq_egress_group_info_get(unit, gport, &port, &eg_n, NULL) != BCM_E_NONE) { 
	    BCM_ERR(("ERROR: %s, gport 0x%x not allocated on unit %d\n",FUNCTION_NAME(), gport, unit));
	    return BCM_E_PARAM;
	}
	
	if ((port < 0) || (port >= SB_FAB_DEVICE_SIRIUS_MAX_FABRIC_PORTS)) {
	    BCM_ERR(("ERROR: %s, egress group gport 0x%x out of range, unit %d\n",
		     FUNCTION_NAME(), gport, unit));
	    return BCM_E_PARAM;
	}
	...
	sp_info = &(SOC_SBX_STATE(unit)->port_state->subport_info[port]);
	if (sp_info->valid == FALSE) {
	    BCM_ERR(("ERROR: %s, invalid egress group gport 0x%x, unit %d\n",
		     FUNCTION_NAME(), gport, unit));
	    return BCM_E_PARAM;
	}	
	...
	*scheduler_level = 7;
	...
	if (IS_GX_PORT(unit, port)) {
		...
		BCM_ERR(("ERROR: %s, gport 0x%x not line-side Higig port, unit %d\n",
			 FUNCTION_NAME(), gport, unit));
		return BCM_E_PARAM;
	    }
	    ...
	    *scheduler_node = SB_FAB_DEVICE_SIRIUS_INGRESS_SCHEDULER_CPU_NODE;
		...
		BCM_ERR(("ERROR: %s, gport 0x%x not valid requeue port, unit %d\n",
			 FUNCTION_NAME(), gport, unit));
		return BCM_E_PARAM;
	    }
	    *scheduler_node = intf + SB_FAB_DEVICE_SIRIUS_INGRESS_SCHEDULER_RQ0_NODE;
	} else {
	    BCM_ERR(("ERROR: %s, No ingress scheduler resource for gport 0x%x, unit %d\n",
		     FUNCTION_NAME(), gport, unit));
	    return BCM_E_PARAM;
	}
	...
	*scheduler_level = SIRIUS_ES_LEVEL_INTERFACE;
	...
	if (IS_GX_PORT(unit, port)) {
	    ...
	    *scheduler_node = intf + SB_FAB_DEVICE_SIRIUS_EGRESS_SCHEDULER_RQ0_NODE;
	    ...
	    BCM_ERR(("ERROR: %s, No egress scheduler resource for gport 0x%x, unit %d\n",
		     FUNCTION_NAME(), gport, unit));
	    return BCM_E_PARAM;
	}
                                               ...
                                               int *scheduler_nodes)
{
    ...
    if (BCM_GPORT_IS_EGRESS_CHILD(gport)) {
	port = BCM_GPORT_EGRESS_CHILD_PORT_GET(gport);
	BCM_ERR(("ERROR: %s, scheduler setting not supported, gport 0x%x, unit %d\n", FUNCTION_NAME(), gport, unit));
        return(BCM_E_UNAVAIL);
    }
    else if (BCM_GPORT_IS_EGRESS_GROUP(gport)) {
        port = BCM_GPORT_EGRESS_GROUP_GET(gport);
    ...
    if (BCM_GPORT_IS_EGRESS_GROUP(gport)) {
        sp_info = &(SOC_SBX_STATE(unit)->port_state->subport_info[port]);
	if ((sp_info == NULL) || (sp_info->valid == FALSE)) {
	    BCM_ERR(("ERROR: %s, invalid fabric_group %d, unit %d\n", FUNCTION_NAME(), port, unit));
	    return(BCM_E_PARAM);
	}
        if (cosq >= sp_info->egroup[0].num_fifos) {
	    BCM_ERR(("ERROR: %s, invalid cos %d, noFifos %d unit %d\n", FUNCTION_NAME(),
                                           ...
                                           int *egress_groups)
{
    ...
    if (!BCM_GPORT_IS_MODPORT(sp_info->parent_gport)) {
        BCM_ERR(("ERROR: %s, egress_group: 0x%x parent 0x%x is not of type ModPort\n",
    ...
    if (!IS_HG_PORT(unit, higig_port)) {
        BCM_ERR(("ERROR: %s, parent 0x%x, is not a HiGig interface\n", FUNCTION_NAME(), sp_info->parent_gport));
        return(BCM_E_PARAM);
    }
    ...
    if (rv == BCM_E_NOT_FOUND) {
        BCM_ERR(("%s, No HiGig Trunk membership, HiGig Port: %d, HiGig Gport: 0x%x\n",
    ...
    (*nbr_egress_groups) = 0;
    for (i = 0, j = 0; i < info.num_ports; i++) {
	if (BCM_GPORT_IS_MODPORT(info.tp[i])) {
	    info.tm[i] = BCM_GPORT_MODPORT_MODID_GET(info.tp[i]);
	    info.tp[i] = BCM_GPORT_MODPORT_PORT_GET(info.tp[i]);
	}

        if (info.tm[i] != modid) {
            BCM_ERR(("%s HiGig Port (%d) on another module (%d) expected module (%d)\n", 
                                                    FUNCTION_NAME(), info.tp[i], info.tm[i], modid));
            continue;
        }
        if (!IS_HG_PORT(unit, info.tp[i])) {
            BCM_ERR(("ERROR: %s, trunk port %d, is not HiGig Port\n", FUNCTION_NAME(), info.tp[i]));
            continue;
        }

        higig_port = info.tp[i] - SOC_PORT_MIN(unit, hg);
        if (higig_port >= SB_FAB_DEVICE_SIRIUS_NUM_HG_LINE_PORTS) {
            BCM_ERR(("ERROR: %s, trunk port %d not in valid range\n", FUNCTION_NAME(), info.tp[i]));
            continue;
        }
	    ...
	    BCM_WARN(("ERROR: %s, NoFifos %d, does not match with the other entries NoFifos %d\n",
...
static int
_bcm_sirius_fifo_size_set(int unit, int fcd, int cosq, uint32 bytes_min, uint32 bytes_max)
{
    int  rv = BCM_E_NONE;
    dt_mem_entry_t dt_entry;
    uint32 pages_min = 0, pages_max = 0;
...
static int
_bcm_sirius_fifo_size_get(int unit, int fcd, int cosq, uint32 *bytes_min, uint32 *bytes_max)
{
    int  rv = BCM_E_NONE;
    dt_mem_entry_t dt_entry;
    uint32 pages_min = 0, pages_max = 0;
			 ...
			 uint32  bytes_max)
{
	...
	(cosq == SOC_SBX_CFG_SIRIUS(unit)->mcast_nef_fifo)) {
        BCM_DEBUG(BCM_DBG_ERR, ("FUNCTION %s: Invalid unicast cosq %d\n", FUNCTION_NAME(), cosq));
        return(BCM_E_PARAM);
    }
    ...
    if (is_empty == FALSE) {
        BCM_DEBUG(BCM_DBG_ERR, ("FIFO not empty fifo: %d, fcd: %d\n", fifo_parm_index, fcd));
    ...
    rv = _bcm_sirius_fifo_size_set(unit, fcd, cosq, bytes_min, bytes_max);
			 ...
			 uint32  *bytes_max)
{
    ...
    rv = _bcm_sirius_fifo_size_get(unit, fcd, cosq, bytes_min, bytes_max);
    return(rv);
}
				   ...
				   uint32  bytes_max)
{
    ...
    if ((fcd < SB_FAB_DEVICE_SIRIUS_MAX_FC_DOMAINS-1) || (fcd > SB_FAB_DEVICE_SIRIUS_MAX_FC_DOMAINS)) {
        BCM_DEBUG(BCM_DBG_ERR, ("FUNCTION %s: Invalid multicast fcd %d\n", FUNCTION_NAME(), fcd));
        return(BCM_E_PARAM);
    }

    if ((cosq != SOC_SBX_CFG_SIRIUS(unit)->mcast_ef_fifo) && 
	(cosq != SOC_SBX_CFG_SIRIUS(unit)->mcast_nef_fifo)) {
        BCM_DEBUG(BCM_DBG_ERR, ("FUNCTION %s: Invalid multicast cosq %d\n", FUNCTION_NAME(), cosq));
        return(BCM_E_PARAM);
    }

    if (bytes_min < 128) {
        BCM_DEBUG(BCM_DBG_ERR, ("FUNCTION %s: Invalid bytes_min %d\n", FUNCTION_NAME(), bytes_min));
	return(BCM_E_PARAM);
    } 

    if (bytes_max < 128) {
        BCM_DEBUG(BCM_DBG_ERR, ("FUNCTION %s: Invalid bytes_max %d\n", FUNCTION_NAME(), bytes_max));
	return(BCM_E_PARAM);
    } 
