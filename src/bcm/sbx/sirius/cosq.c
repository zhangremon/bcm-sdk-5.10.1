/*
 * $Id: cosq.c 1.349.2.25 Broadcom SDK $
...
#define COSQ_DEBUG(flags, stuff)  BCM_DEBUG(flags | BCM_DBG_COSQ, stuff)

#define COSQ_NORMAL(stuff)        COSQ_DEBUG(BCM_DBG_NORMAL, stuff)
#define COSQ_WARN(stuff)          COSQ_DEBUG(BCM_DBG_WARN, stuff)
#define COSQ_ERR(stuff)           COSQ_DEBUG(BCM_DBG_ERR, stuff)
#define COSQ_VERB(stuff)          COSQ_DEBUG(BCM_DBG_VERBOSE, stuff)
#define COSQ_VVERB(stuff)         COSQ_DEBUG(BCM_DBG_VVERBOSE, stuff)
...
static void
_bcm_sirius_rate_delta_max_index_for_port_rate_chosen(uint32 bag_rate_kbps,
...
#define BCM_SIRIUS_SCHEDULER_STATE_INIT_VERIFY(unit) \
	...
	(gs_state[unit] == NULL)) {					 \
        BCM_ERR(("scheduler states not initialized, unit(%d)\n", unit)); \
	return BCM_E_INIT;					         \
    }
...
_bcm_sirius_cosq_delete_sysport(int unit, uint32 sysport)
{
    ...
    if (BCM_E_NONE == result) {
        ...
        BCM_ERR(("unable to read %d:MDB[%08X]: %d (%s)\n",
                 unit,
                 mgid,
                 result,
                 _SHR_ERRMSG(result)));
    ...
    if ((BCM_E_NONE == result) &&
        ...
        if ((BCM_E_NONE != result) && (BCM_E_INIT != result)) {
             ...
             */
            BCM_ERR(("unable to delete unit %d multicast/distribution group"
    ...
    if ((BCM_E_NONE != result) ||
        ...
        if (SOC_E_NONE != result) {
            BCM_ERR(("unable to write unit %d sysport %d pointer: %d (%s)\n",
                     unit,
                     sysport,
                     result,
                     _SHR_ERRMSG(result)));
...
bcm_sirius_scheduler_init(int unit, int create_ports)
{
    ...
    int modid, port, fifo, eg, hg, intf, intf_gport, subport, flags, port_offset, intf_subports;
		...
		creditor_state_map = 0x4;
		    ...
		    switch (profile) {
			    ...
			    break;
	             ...
	             BCM_ERR(("ERROR: %s, node template not supported %d, Unit(%d)\n", 
    ...
    if (rv != BCM_E_NONE) {
	BCM_ERR(("ERROR: failed to allocate group max shaper for multicast trunking child gport\n"));
    }
	...
	for (node = 0; node < num_node; node++) {
	    ...
	    if (rv != SOC_E_NONE) {
		BCM_ERR(("ES failed to init level %d node %d\n", level, node));
		return rv;
	    }
	}
    }

    /* create the eggress schedulers for interface ports */
	...
	rv = bcm_sirius_cosq_scheduler_allocate(unit, SIRIUS_ES_LEVEL_INTERFACE, 
						intf, 1, TRUE, &node);
	if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: %s, failed to allocate level 3 egress scheduler node, Unit(%d)\n",
		     FUNCTION_NAME(), unit));
	    return BCM_E_RESOURCE;
	}
    }
	...
	for (level = 6; level >= 1; level--) {
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_ERR(("ERROR: %s, failed to allocation child level %d scheduler node %d,  Unit(%d)\n",
	...
	for (level = 6; level >= 2; level--) {
		...
		if (SOC_SBX_CFG_SIRIUS(unit)->uNumTsNode[level] < 3) {
		    BCM_ERR(("ERROR: %s, TS level %d has less than 3 scheduler node, Unit(%d)\n",
		...
		if (rv != SOC_E_NONE) {
		    BCM_ERR(("ERROR: %s, failed to reinit ingress scheduler level %d node %d, Unit(%d)\n",
			     FUNCTION_NAME(), level, node, unit));
		    return rv;
		}
		...
		if (rv != SOC_E_NONE) {
		    BCM_ERR(("ERROR: %s, Failed to init TS level %d node %d first child node, Unit(%d)\n",
			     FUNCTION_NAME(), level, node, unit));		    
		    return rv;
		}
	    }
		...
		if (rv != SOC_E_NONE) {
		    BCM_ERR(("ERROR: %s, Failed to init TS leaf level node %d destination, Unit(%d)\n",
	...
	if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: %s, failed to create level 6 ingress scheduler node %d, Unit(%d)\n",
	...
	if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: %s, failed to connect level 6 ingress scheduler node %d,  Unit(%d)\n",
		     FUNCTION_NAME(), level6_node, unit));
	    return BCM_E_RESOURCE;
	}
	...
	if (rv != SOC_E_NONE) {
	    BCM_ERR(("%s, failed to get modid, Unit(%d)\n", FUNCTION_NAME(), unit));
	    return rv;
	}	
	...
	PBMP_ALL_ITER(unit, port) {
		...
		if (IS_HG_PORT(unit, port)) {
		    ...
		    intf_subports = 1;
		    subport = SB_FAB_DEVICE_SIRIUS_CPU_HANDLE; /* Must be last port created */
		}
		...
		for (port_offset = 0; port_offset < intf_subports; port_offset++) {
		    /* speed configuration */
		    if (IS_HG_PORT(unit, port)) {
			for (eg = 0; eg < SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_MAX_SIZE; eg++) {
			    SOC_SBX_CFG_SIRIUS(unit)->uSubportSpeed[intf][port_offset][eg][SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_MAX_SIZE] = 
				soc_property_port_get(unit, subport, spn_PORT_INIT_SPEED, SOC_SBX_CFG_SIRIUS(unit)->uSubportSpeed[intf][port_offset][0][SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_MAX_SIZE]);
			    for (fifo = 0; fifo < SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_MAX_SIZE; fifo++) {
				char name[10];
				sal_sprintf(name, "fifo%d", fifo);
				SOC_SBX_CFG_SIRIUS(unit)->uSubportSpeed[intf][port_offset][eg][fifo] = 
				    soc_property_suffix_num_get(unit, subport, spn_PORT_INIT_SPEED, name, SOC_SBX_CFG_SIRIUS(unit)->uSubportSpeed[intf][port_offset][0][SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_MAX_SIZE]);
			    }
			}
		    ...
		    if (rv != BCM_E_NONE) {
		    	    BCM_ERR(("ERROR: %s, failed to create subports on port %d, Unit(%d)\n", 
                                     FUNCTION_NAME(), port, unit));
			return rv;
		    } else {
			BCM_DEBUG(BCM_DBG_COSQ, ("created fabric gport(0x%x) on port(0x%x)\n",
						 subport, intf_gport));
		    }
		    subport++;
		}
                                             ...
                                             int internal_port)
{
	...
	BCM_ERR(("ERROR: %s, unsupported type for gport 0x%X, unit %d\n",
		 FUNCTION_NAME(),
                 physical_gport,
                 unit));
	return BCM_E_PARAM;
    ...
    if ( (SOC_SBX_STATE(unit)->port_state == NULL) || (internal_port < 0) ||
	 (internal_port >= SB_FAB_DEVICE_SIRIUS_MAX_FABRIC_PORTS) ) {
        BCM_ERR(("Unit %d subport %d out of range [0-SB_FAB_DEVICE_SIRIUS_MAX_FABRIC_PORTS)\n",
		 unit, internal_port));
        return BCM_E_PARAM;	
    }
    ...
    if ( (sp_info == NULL) || (sp_info->valid != TRUE) ||
	 ...
	 !(sp_info->flags & SBX_SUBPORT_FLAG_ON_ES) ) {
        BCM_ERR(("ERROR: %s, invalid internal port %d, unit %d\n",
    ...
    for (fifo = level0_node; fifo < level0_node + sp_info->egroup[0].num_fifos; fifo++) {
        ...
        if (BCM_E_NONE != rv) {
            BCM_ERR(("ERROR: %s, failed to free level 0 egress scheduler"
    ...
    if (BCM_E_NONE != rv) {
	BCM_ERR(("ERROR: %s, failed to create level 1 egress scheduler node"
    ...
    if (BCM_E_NONE != rv) {
	BCM_ERR(("ERROR: %s, failed to free level 2 egress scheduler node"
                 " on gport 0x%x, Unit(%d)\n",
		 FUNCTION_NAME(),
                 physical_gport,
                 unit));
    ...
    rv = soc_sirius_config_ff(unit, internal_port, 0, FF_MEM_DELETE);
    bcm_sbx_cosq_egress_group_free(unit, sp_info->egroup[0].egroup_gport);
					    ...
					    int *internal_port)
{
    ...
    if (flags & SBX_SUBPORT_FLAG_INTERNAL) {
		...
		*internal_port = BCM_GPORT_EGRESS_CHILD_PORT_GET(original_subport);
		...
		BCM_ERR(("ERROR: %s, failed to create internal Multicast Trunk port on ES using gport 0x%x, Unit(%d)\n",
    ...
    rv = bcm_sirius_fabric_port_create_es(unit, physical_gport, -1, 
					  ...
					  flags, &target);
    if (BCM_E_NONE != rv) {
	BCM_ERR(("ERROR: %s, failed to create internal port on ES on gport 0x%x, Unit(%d)\n",
	...
	*internal_port = target;
    ...
    if (rv != BCM_E_NONE) {
	BCM_ERR(("ERROR: Could not determine subport offsets\n"));
	return rv;
    }
	...
	if (BCM_GPORT_IS_CHILD(original_subport)) {
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_ERR(("ERROR: %s, original_subport 0x%x does not contain fabric_port, unit %d\n",
			 FUNCTION_NAME(), original_subport, unit));
		return BCM_E_PARAM;
	    }
	} else {
	    BCM_ERR(("ERROR: %s, unsupported gport type original child gport 0x%x for internal trunking port creation, Unit(%d)\n",
		     FUNCTION_NAME(), original_subport, unit));
	    return BCM_E_PARAM;
	}
	...
	if ((orig_sp_info->valid == TRUE) && (orig_sp_info->flags & SBX_SUBPORT_FLAG_ON_ES)) {
		...
		if (rv != BCM_E_NONE) {
		    BCM_ERR(("ERROR: %s, failed to allocate group shaper resource gport 0x%x, Unit(%d)\n",
			     FUNCTION_NAME(), original_subport, unit));
		    return rv;
		}
		...
		for (node = orig_sp_info->egroup[0].es_scheduler_level0_node; 
		    ...
		    if (rv != BCM_E_NONE) {
			BCM_ERR(("ERROR: %s, failed to config group shaper membership for level 0 node %d, Unit(%d)\n",
				 FUNCTION_NAME(), node, unit));
			return rv;
		    }
		}
	    ...
	    BCM_ERR(("ERROR: %s, invalid original child gport 0x%x for internal trunking port creation, Unit(%d)\n",
		     FUNCTION_NAME(), original_subport, unit));
	    return BCM_E_PARAM;
	}
    } else {
	...
	for (node = sp_info->egroup[0].es_scheduler_level0_node; 
	    ...
	    SOC_SBX_CFG_SIRIUS(unit)->uSubportSpeed[intf][0][0][node - sp_info->egroup[0].es_scheduler_level0_node] = 1000;
	}
	SOC_SBX_CFG_SIRIUS(unit)->uSubportSpeed[intf][0][0][SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_MAX_SIZE] = 1000;
    }
    
    
    rv = soc_sirius_config_ff(unit, *internal_port, 0, FF_MEM_ALLOCATE);
    if (rv != BCM_E_NONE) {
	BCM_ERR(("Unit %d Internal Egress Fifo Port creation failed\n",
		 unit));
	return rv;
    }

    COSQ_VERB(("Unit %d Creating Internal Egress Fifo Port, phy_gport: 0x%x, type: 0x%x, orig_subport: 0x%x, internal_port: 0x%x\n",
                         ...
                         bcm_gport_t gport, int32 *p_dest_switch_modid, int32 *p_dest_switch_port)
{
    ...
    if (BCM_GPORT_IS_MODPORT(gport)) {
        ...
        if ((logical_eg < 0) || (logical_eg >= SOC_SBX_CFG(unit)->num_egress_group) ||
            ...
            BCM_ERR(("ERROR: %s, egress group %d not valid,  Unit(%d)\n",
    ...
    if (rv) {
        BCM_ERR(("ERROR: error returned from bcm_sbx_stk_fabric_map_get_switch_port gport(0x%08x) error(%d)\n", p_bwstate->gport, rv));
        return rv;
    }
                          ...
                          int inhibit_write)
{
	...
	if (BCM_GPORT_IS_EGRESS_GROUP(p_bwstate->gport)) {
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_ERR(("[%d]:unable to write to unit %d:gmt for sysport %d destination port(%d)\n",
			 __LINE__, 
			 unit,
			 sysport,
			 fct));
	...
	if (queue_region == bcm_sbx_cosq_queue_region_local) {
	    ...
	    if (rv) {
		BCM_ERR(("ERROR: write to Q2SC table failed for queue(%d)\n", queue));
		return rv;
	    }

	    rv = soc_sirius_qs_sysport_to_queue_set(unit, mc, sysport, base_queue);
	    if (rv) {
		BCM_ERR(("ERROR: %s, write to S2Q table failed for queue(%d)\n", FUNCTION_NAME(), queue));
		return rv;
	    }

	    rv = soc_sirius_qs_sysport_to_node_set(unit, sysport, dest_node);
	    if (rv) {
		BCM_ERR(("ERROR: write to S2N table failed for queue(%d)\n", queue));
		return rv;
	    }
	}
    }
    ...
    if ((dest_type != BCM_INT_SBX_DEST_TYPE_MULTICAST) && (base_queue == queue) && 
	(SOC_SBX_CFG(unit)->uInterfaceProtocol == SOC_SBX_IF_PROTOCOL_XGS)) {
        /* write the queue map table with base queue if unicast      */
        /* For a multicast queue, this will be done when the call to */
        /* bcm_multicast_fabric_distribution_set() is done.  The RB  */
        /* does a lookup on mcgroup which is not known during the    */
        /* bcm_fabric_distribution_add() call.                       */

        
        gport = p_bwstate->gport;
        rv = _bcm_sirius_cosq_destmod_destport_get(unit, queue, gport, &dest_switch_modid, &dest_switch_port);
        if (rv != BCM_E_NONE) {
            return(rv);
        }

        if (dest_node == BCM_STK_MOD_TO_NODE(mymodid)) {
        /* to see if its a cpu port, see what interface the dest_port belongs to */
        rv = bcm_sbx_port_get_intf_portoffset(unit, dest_port, &intf, &subport);
        if (rv) {
            BCM_ERR(("ERROR mapping a fabric port to an interface"));
            return rv;
        }
        ...
        if (intf == SB_FAB_DEVICE_SIRIUS_CPU_INTF) {
            ...
            rv = soc_sirius_rb_higig2_remote_cpu_config(unit,
                                       dest_switch_modid, dest_switch_port,
                                       base_queue);
            if (rv != SOC_E_NONE) {
                BCM_ERR(("Error programming the REMOTE CPU information \n"));
                return rv;
            }
        }
        }
        if (inhibit_write) {
            ...
            rv = soc_sirius_rb_higig2_header_unicast_queue_map_config(unit,
                                                                      ...
                                                                      base_queue);
            if (rv) {
                BCM_ERR(("ERROR: write to RB queue map table failed switch"
    ...
    else if ((dest_type == BCM_INT_SBX_DEST_TYPE_MULTICAST) && (base_queue == queue) && 
	              (SOC_SBX_CFG(unit)->uInterfaceProtocol == SOC_SBX_IF_PROTOCOL_XGS)) {
    ...
    if (queue_region == bcm_sbx_cosq_queue_region_global) {
        if (SOC_SBX_CFG(unit)->sp_mode == SOC_SBX_SP_MODE_ACCOUNT_IN_BAG) {
            ...
            if (rv) {
                BCM_ERR(("ERROR: set ingress params configuration (strict priority) error(%d)\n", rv));
                return(rv);
            }
        }
    }
                           ...
                           int port)
{
    ...
    bcm_if_t encaps[BCM_TRUNK_MAX_PORTCNT];
    dmt_mem_entry_t dmt_entry;
    bcm_sbx_subport_info_t *sp_info = NULL;
    int fabric_port = port, eg_n = 0;
    int modid = 0, mymodid = 0, fifo;
    int rv = BCM_E_NONE;

    result = bcm_sirius_trunk_find_and_get(unit, module, gport, &tid, &tinfo);
    if (BCM_E_NONE == result) {
        ...
        if (BCM_E_NONE == result) {
            ...
            sal_memset(&(encaps[0]),
                       0x00,
                       sizeof(encaps[0]) * BCM_TRUNK_MAX_PORTCNT);
            ...
            result = bcm_sirius_multicast_egress_set_override_oi(unit,
                                                                 ...
                                                                 FALSE);
            if (BCM_E_NONE != result) {
                BCM_ERR(("unable to add %d targets to distribution group on"
            ...
            BCM_ERR(("unable to create distribution group on unit %d sysport"
	...
	if (SOC_SBX_CFG(unit)->bUcqResourceAllocationMode == FALSE) {
	    ...
	    if (BCM_E_NONE != result) {
		BCM_ERR(("[%d]:unable to write to unit %d:gmt for sysport %d"
			 " destination port(%d)\n",
			 __LINE__, 
			 unit,
			 sysport,
	    ...
	    if (modid == mymodid) {
		/* setup DMT table */
		BCM_IF_ERROR_RETURN(READ_DMT_MEMm(unit, MEM_BLOCK_ANY, sysport, &dmt_entry));
		...
		if (SOC_SBX_CFG(unit)->bEgressFifoIndependentFlowControl) {
		     ...
		     */
		    soc_mem_field32_set(unit, DMT_MEMm, &dmt_entry, DOMAIN_NUMBER0f, sp_info->egroup[eg_n].nef_fcd);
		    soc_mem_field32_set(unit, DMT_MEMm, &dmt_entry, DOMAIN_NUMBER1f, sp_info->egroup[eg_n].ef_fcd);
		    BCM_IF_ERROR_RETURN(WRITE_DMT_MEMm(unit, MEM_BLOCK_ANY, (BCM_INT_SBX_SYSPORT_TO_NEF_SYSPORT(unit, sysport)), &dmt_entry));
		} else {
		    /* only even domains are used */
		    soc_mem_field32_set(unit, DMT_MEMm, &dmt_entry, DOMAIN_NUMBER0f, sp_info->egroup[eg_n].ef_fcd);
		    soc_mem_field32_set(unit, DMT_MEMm, &dmt_entry, DOMAIN_NUMBER1f, sp_info->egroup[eg_n].ef_fcd);	    
		}
		BCM_IF_ERROR_RETURN(WRITE_DMT_MEMm(unit, MEM_BLOCK_ANY, sysport, &dmt_entry));
	    }
...
_bcm_sirius_cosq_delete_queue(int unit, int queue, bcm_sbx_cosq_queue_region_type_t queue_region, int is_user_invoked)
{
    ...
    if (mc == 0) {
      ...
      gport = p_bwstate->gport;
      if (BCM_GPORT_IS_MODPORT(gport)) {
        BCM_GPORT_EGRESS_CHILD_SET(gport, BCM_GPORT_MODPORT_MODID_GET(p_bwstate->gport), BCM_GPORT_MODPORT_PORT_GET(p_bwstate->gport));
      }
	...
	return rv;
      ...
      if (SOC_SBX_CFG(unit)->uInterfaceProtocol == SOC_SBX_IF_PROTOCOL_XGS) {
          ...
          } /* if (queue was not created inhibiting qsel updates) */
      } /* if (XGS mode) */
      ...
      if (rv) {
	BCM_ERR(("ERROR: write to GMT table failed sysport(%d) destination port(%d)\n", sysport, 0));
	return rv;
      }
#endif 
	...
	while (p_qstate->mgid_list->data > 0) {
	  ...
	  if (pElem == NULL) {
	    BCM_ERR(("ERROR: MGID element invalid\n"));
	    return BCM_E_MEMORY;
	  }
	  ...
	  if (rv) {
	    BCM_ERR(("ERROR: Read of RB queue map table failed switch mgid(%d)\n", mgid));
	    return rv;
	}
	  if (temp == base_queue) {
	    rv = soc_sirius_rb_higig2_header_multicast_queue_map_config(unit, mgid, 0xffff);
	    if (rv) {
	      BCM_ERR(("ERROR: write to RB queue map table failed switch mgid(%d) base queue(%d)\n", mgid, base_queue));
	      return rv;
	    }
	  }
	}
    ...
    /* TODO: Move this to the common code */
    /*
     * set anemic configuration
     */
    if (is_user_invoked == TRUE) {
	...
	rv = soc_sirius_qs_sysport_to_queue_set(unit, mc, sysport, 0xffff);
	if (rv) {
	    BCM_ERR(("ERROR: %s, write to S2Q table failed for queue(%d)\n", FUNCTION_NAME(), queue));
	    return rv;
	}
			    ...
			    bcm_sbx_cosq_queue_region_type_t queue_region) 
{
	...
	BCM_ERR(("ERROR: Unsuported gport type 0x%x\n", gport));
	rv = BCM_E_PARAM;
	return rv;	
    }

    if (bcm_sbx_cosq_egress_group_info_get(unit, gport, &fabric_port, &eg_num, &num_fifos) != BCM_E_NONE) { 
	BCM_ERR(("ERROR: fifo gport cos %d unknown\n", cosq));
	rv = BCM_E_PARAM;
	return rv;	
    }
	
    if (cosq >= num_fifos) {
	BCM_ERR(("ERROR: Cosq %d exceeds %d fifos on gport 0x%x\n", cosq, num_fifos, gport));
	rv = BCM_E_PARAM;
	return rv;	
    }
    ...
    if (enable != 0) {
	BCM_DEBUG(BCM_DBG_COSQ, ("gport(0x%x) fifo(%d) already ENABLED\n",
				 gport, cosq));
	return BCM_E_NONE;
    }
    ...
    if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: fifo flush failed\n", fifo));
	    return rv;	
    }
    ...
    if (rv != BCM_E_NONE) {
	BCM_ERR(("ERROR: fifo empty check for fifo %d failed\n", fifo));
        goto err;
    }
    
    if (is_empty == FALSE) {
        BCM_DEBUG(BCM_DBG_COSQ, ("FIFO not empty fifo: %d, gport: 0x%x\n", fifo, gport));
	
        /* Flush port */
        rv = soc_sirius_port_flush(unit, fifo);
        if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: fifo flush for fifo %d failed\n", fifo));
            goto err;
        }
    }
			     ...
			     bcm_sbx_cosq_queue_region_type_t queue_region) 
{
    ...
    if (enable == 0) {
	BCM_DEBUG(BCM_DBG_COSQ, ("gport(0x%x) fifo(%d) already DISABLED\n",
				 gport, cosq));
	return BCM_E_NONE;
    }
...
bcm_sirius_cosq_enable_get(int  unit, 
    ...
    if (bcm_sbx_cosq_egress_group_info_get(unit, gport, &fabric_port, &eg_num, &num_fifos) != BCM_E_NONE) { 
	BCM_ERR(("ERROR: fifo gport 0x%x cos %d unknown\n", gport, cosq));
	return BCM_E_PARAM;
    }
...
bcm_sirius_cosq_set_ingress_params(int unit,
    ...
    if (rv) {
	BCM_ERR(("ERROR: _bcm_sirius_cosq_select_queue_params - oldParams, Unit:%d\n", unit));
        return(rv);
    }
    ...
    if (rv) {
	BCM_ERR(("ERROR: _bcm_sirius_cosq_select_queue_params - newParams, Unit:%d\n", unit));
        return(rv);
    }
    ...
    if ( (hold_ts != old_hold_ts) || (q_type != old_q_type) ) {
        ...
        rv = soc_sirius_qs_queue_parameter_set(unit, queue, q_type, hold_ts);
        if (rv) {
	    BCM_ERR(("ERROR: soc_sirius_qs_queue_parameter_set - QueueType, Unit:%d\n", unit));
            return(rv);
        }
    }
	...
	_bcm_sirius_rate_delta_max_index_for_port_rate_chosen(p_newbwparams->bag_rate_kbps,
							      &rate_delta_max_index);
	
	COSQ_VVERB(("  VoQ RateDelatMaxindex(%d)\n", rate_delta_max_index));
	...
	if (p_qstate->ingress.enabled == TRUE) {
	    ...
	    if (rv) {
		BCM_ERR(("ERROR: _bcm_sirius_cosq_add_delete_queue - Delete, Unit:%d\n", unit));
		return(rv);
	    }
	    ...
	    if (rv) {
	    BCM_ERR(("ERROR: _bcm_sirius_cosq_add_delete_queue - Add, Unit:%d\n", unit));
            return(rv);
	    }
	}
	...
	rv = soc_sirius_qm_queue_buffs_profile_entry_set(unit, queue, p_newqparams->template, TRUE);
        if (rv) {
	    BCM_ERR(("ERROR: queue_buffs_profile update failed, Unit:%d\n", unit));
            return(rv);
        }
    }
	...
	switch (p_newqparams->bw_mode) {
		...
		if ( (weight < 0) || (weight > 0x7F ) ) {
		    ...
		    BCM_ERR(("ERROR: %s weight %d out of range [0-127], Unit:%d\n",
	...
	if (weight != old_weight) {
	    ...
	    if (rv != SOC_E_NONE) {
		BCM_ERR(("ERROR: %s failed to get leaf node for local queue %d, Unit:%d\n",
	    ...
	    if (rv != SOC_E_NONE) {
		BCM_ERR(("ERROR: %s, can not config queue %d weight %d on level 1 node %d offset %d, Unit(%d)\n",
			 FUNCTION_NAME(), queue, weight, level1_node, child_offset, unit));
		return rv;
	    } else {
		BCM_DEBUG(BCM_DBG_VERBOSE, ("queue(%d) weight (%d) configed in level 1 node %d offset %d\n",
	...
	if (cir != old_cir) {
	    ...
	    if (rv != SOC_E_NONE) {
		BCM_ERR(("ERROR: %s failed to convert rate %d kbps to mant/exp format\n",
			 FUNCTION_NAME(), cir));
		return(rv);
	    }
	    ...
	    if (rv != SOC_E_NONE) {
		BCM_ERR(("ERROR: %s failed to config rate %d kbps to mant/exp format\n",
			 FUNCTION_NAME(), cir));
		return(rv);
	    } else {
		BCM_DEBUG(BCM_DBG_VERBOSE, ("queue(%d) configed to have CIR of %d kbps\n",
					    queue, cir));
	    }
	}
    }
                                   ...
                                   int enable_shaping)
{
	...
	if (voq_shape_inc == 1) {
		...
		if (shape_limit_kbps != 0) {
		    BCM_ERR(("ERROR: %s no hardware ingress shaper resource for VOQ queue %d \n",
			     FUNCTION_NAME(), queue));
		    return BCM_E_RESOURCE;
		} else {
		    return BCM_E_NONE;
	...
	if (local_shape_inc == 1) {
		...
		if (shape_limit_kbps != 0) {
		    BCM_ERR(("ERROR: %s no hardware ingress shaper resource for Local queue %d \n",
			     FUNCTION_NAME(), queue));
		    return BCM_E_RESOURCE;
		} else {
		    return BCM_E_NONE;
	...
	if (shape_limit_kbps != 0) {
	    BCM_ERR(("ERROR: %s no hardware ingress shaper resource for queue %d \n",
		     FUNCTION_NAME(), queue));
	    return BCM_E_RESOURCE;
	} else {
	    return BCM_E_NONE;
    ...
    if (enable_shaping == TRUE) {
	    ...
	    rv = soc_sbx_div64(shape_limit_kbps * round_ns, 1000000, &min_burst_size);
	    if (rv != SOC_E_NONE) {
	        BCM_ERR(("ERROR: %s failed to calculate burst size for rate of %d kbps\n",
		     FUNCTION_NAME(), shape_limit_kbps));
                return(rv);
	    } else {
	    ...
	    rv = soc_sbx_fabric_util_num_to_mant_exp(unit, SHAPER_LEAK_0m, SHAPE_THRESH_EXPf,
            ...
            if (rv != SOC_E_NONE) {
	        BCM_ERR(("ERROR: %s failed to convert burst size %d bits to mant/exp format\n",
    ...
    BCM_DEBUG(BCM_DBG_VERBOSE,("%s base queue %d num_cos %d use ingress shaper table %d index %d\n",
                  ...
                  int gain)
{
	...
	BCM_ERR(("ERROR: %s, wred template %d out of range [0-0x1FF],  Unit(%d)\n",
		 FUNCTION_NAME(), template, unit));
	return BCM_E_PARAM;
    }

    if ( (gain < 0) || (gain > 0xF) ) {
	BCM_ERR(("ERROR: %s, wred gain %d out of range [0-0xF],  Unit(%d)\n",
                                  ...
                                  bcm_cosq_gport_discard_t *discard)
{
	...
	BCM_ERR(("ERROR: %s, template %d out of range [0-511],  Unit(%d)\n",
		 FUNCTION_NAME(), template, unit));
	return BCM_E_PARAM;
    }

    if ( color >= BCM_SBX_COSQ_DISCARD_MAX_COLORS ) {
	BCM_ERR(("ERROR: %s, color %d out of range [0-3],  Unit(%d)\n",
	...
	if ((chip_params.m_nTmax > 0xFFFF) ||
	    ...
	    (chip_params.m_nSlope > 0xFFF)) {
            BCM_ERR(("ERROR: %s, wred parameter out of range,  Unit(%d)\n",
                     FUNCTION_NAME(), unit));
	    return BCM_E_PARAM;
	}
    ...
    if (rv != SOC_E_NONE) {
	BCM_ERR(("ERROR: soc_sirius_qm_queue_min_buff_set, Unit:%d\n", unit));
	return(rv);
    }
    
    rv = soc_sirius_qm_queue_max_buff_set(unit, template, queue_size);
    if (rv != SOC_E_NONE) {
	BCM_ERR(("ERROR: soc_sirius_qm_queue_max_buff_set, Unit:%d\n", unit));
	return(rv);
    }
                                      ...
                                      int   enable)
{
    ...
    BCM_DEBUG(BCM_DBG_VERBOSE,("gport %d base q %d num_cos %d max_queues %d\n", \
    ...
    if(enable == TRUE) {
        ...
        if (!soc_feature(unit, soc_feature_cosq_gport_stat_ability)) {
            BCM_ERR(("soc_feature_cosq_gport_stat_ability Unavailable"));
        ...
        if (current_enable == TRUE) {
            BCM_ERR(("Counter currently active\n"));
            return BCM_E_INTERNAL;
        }
        ...
        int   *enable)
{
    ...
    BCM_DEBUG(BCM_DBG_VERBOSE,("gport %d base q %d num_cos %d\n", gport, base_queue, num_cos));
	...
	*enable = 0;
    ...
    BCM_DEBUG(BCM_DBG_VERBOSE,("Enable %d,base_queue %d, actual_base_queue %d\n",*enable,base_queue,actual_base_queue));
        ...
        uint64  value)
{
    ...
    if (!soc_feature(unit, soc_feature_cosq_gport_stat_ability)) {
        BCM_DEBUG(BCM_DBG_VERBOSE,("soc_feature_cosq_gport_stat_ability Unavailable"));
        return BCM_E_UNAVAIL;
    }
    ...
    BCM_DEBUG(BCM_DBG_VERBOSE, ("soc_sbx_counter_port_set %d  %d %d cntr %d 0x%llx\n",unit,block,set,stat,value));
    rv = soc_sbx_counter_set(unit,  block, set, stat, 0);
                               ...
                               uint64   *value)
{
    ...
    BCM_DEBUG(BCM_DBG_VERBOSE,("soc_sbx_sirius_counter_port_get %d %d %d cntr %d 0x%llx\n",unit,block,set,stat,*value));
		     ...
		     sbBool_t enable)
{
	...
	if (sir->cs.flags & CS_SLQ_ENABLE) {
		...
		COSQ_ERR(("profile_set: Unit %d SLQ %d not within range of"
			  " queue_base %d\n", unit, slq, queue_base));
		return BCM_E_PARAM;
	    }
	    if (sir->cs.slq_q_active & (1 << cosq)) {
		COSQ_ERR(("profile_set: Unit %d Cosq %d currently active.\n",
			  unit, cosq));
		return BCM_E_BUSY;
	    }
	} else {
		...
		COSQ_ERR(("profile_set: Unit %d SLQ queue_base %d not available\n",
			  unit, queue_base));
		return BCM_E_PARAM;
	    }
	}
	...
	if (sir->cs.flags & CS_SLQ_ENABLE) {
		...
		COSQ_ERR(("profile_set: Unit %d Cosq %d currently inactive.\n",
			  unit, cosq));
		return BCM_E_PARAM;
	    }
	    /*
...
static int bcm_sirius_get_egress_info(uint32 unit, bcm_gport_t gport, uint8_t *bsize, uint8_t *level,
				      uint16_t *cntrId, int *intf)
{
    int rv, index = 0;
	...
	rv = bcm_sbx_cosq_egress_group_info_get(unit, gport, &index, &eg_n, NULL);
        if (rv != BCM_E_NONE) {
            COSQ_ERR(("Invalid gport(0x%x). Unable to resolve to an intf \n",
                      gport));
            return rv;
        }
	...
	*level = BCM_META0_SUBPORT_ID;
	...
	if (SCHED_EGRESS_CHECK(index) == 0)
	    return BCM_E_PARAM;
	*cntrId = (uint16_t) SCHED_LOOKUP(index);
	*bsize = 8;
	*intf = 0;
...
ep_seg_done:
	 ...
	 */
	COSQ_ERR(("ep_stat: unit %d no segments available for Egress Stat"
		  " type %d\n", unit, type));
	return BCM_E_RESOURCE;
    }
	...
	if (segment < 0) {
		...
		COSQ_ERR(("gport_profile_set: unit %d no segments available"
			      ...
			      uint32 shared)
{
	...
	if (found == FALSE) {
		...
		if (soc_sbx_sirius_get_segment(unit, CU_NUM_PHYSICAL, &segment) == SOC_E_UNAVAIL) {
		    COSQ_ERR(("intf_stat: unit %d no segments available for Ingress"
	...
	if ((color_map & (1 << dp)) != 0) {
		...
		if (segment == -1) {
		     ...
		     */
		    if (soc_sbx_sirius_get_segment(unit, CU_NUM_PHYSICAL, &segment) == SOC_E_UNAVAIL) {
			COSQ_ERR(("intf_stat: unit %d no segments available for"
		...
		COSQ_ERR(("intf_stat: unit %d type %d color %d entry already"
			  " configured\n",
			  unit,
			  type,
			  dp));
		...
		COSQ_ERR(("intf_stat: unit %d type %d color %d entry not"
			  " configured\n", unit, type, dp));
		return BCM_E_PARAM;
	    }
			  ...
			  uint32 shared)
{
	...
	COSQ_WARN(("QID %d not configured\n",cntrId));
	return BCM_E_CONFIG;
    }
	...
	if (color_map) {
		...
		COSQ_ERR(("queue_stat: unit %d no segments available"
			    ...
			    int32 enable)
{
	...
	if (soc_reg_field_get(unit, QM_CS_CONFIG0r, regval, DEQ_CS_STATS_ENf) == 0) {
		...
		COSQ_ERR(("dequeue_stat: unit %d no segments available for Dequeue Stat\n", unit));
		return BCM_E_RESOURCE;
	    }
				       ...
				       bcm_cosq_gport_stat_profile_t profile)
{
	...
	if ((index == 0) && (type == 0x7) && (type2 == 0x7)) {
	    COSQ_ERR(("%s: unit %d Admission Control resources not configured\n", FUNCTION_NAME(), unit));
	    return BCM_E_PARAM;
	} else if (index > 2) {
	    COSQ_ERR(("%s: unit %d Admission Control resources not available\n", FUNCTION_NAME(), unit));
	    return BCM_E_PARAM;
	}
	...
	if (lgl_gport != BCM_GPORT_TYPE_NONE) {
		...
		if (!shared) {
		    ...
		    if (uStatus != BCM_E_NONE) {
			BCM_ERR(("Dequeue stat %s failed for 0x%x\n",(deq) ? "enable" : "disable", lgl_gport));
			return uStatus;
		    }
		}		
		...
		COSQ_ERR(("gport_profile_set: unit %d unknown Logical Interface"
			  " %x\n", unit, lgl_gport));
	...
	switch(qm_addr_sel) {
	    ...
	    default:
		COSQ_ERR(("gport_profile_set: unit %d unknown mode\n", unit));
		return BCM_E_PARAM;
	}
    }
...
#define CHECK_EP_STAT(_a, _b, _c, _l, _id)				\
	...
	if (soc_mem_field32_get(unit, EP_STATS_CTRLm, &ep_stats_ctrl, _a) != 0) { \
	    ...
	    if ((_meta0 == _l) && (sir->cs.segment[_segment].cntrIdMap & (1 < _id))) \
	...
	switch(qm_addr_sel) {
		...
		for (type=0; type<16; type++)
			...
			for (dp=0; dp < 8; dp++) {
			    ...
			    if (soc_mem_field32_get(unit, STATSCFGm, &statsCfg, RECORD2_EVENT_SELf)) {
				segment = soc_mem_field32_get(unit, STATSCFGm, &statsCfg, RECORD2_SEGMENT_SELf);
				if (sir->cs.segment[segment].cntrIdMap & (1 << (intf))) {
				    if (shared) {
	    ...
	    case qm_addr_queue:
		if ((cntrId >> CS_CFG_GRP_SHIFT) < 32)
		    COMPILER_64_SET(temp, 0, (uint64) 1 << (cntrId >> CS_CFG_GRP_SHIFT));
		else
		    COMPILER_64_SET(temp, 1 << ((cntrId >> CS_CFG_GRP_SHIFT) - 32), 0);
		for (type=0; type<16; type++)
		    if ((sir->type_res_flags[PRED_TYPE_RB][type] & (1 << (intf + 4))) != 0) {
			for (dp=0; dp < 8; dp++) {
			    BCM_IF_ERROR_RETURN( READ_STATSCFGm(unit, MEM_BLOCK_ANY, ((type << 3) + dp), &statsCfg));
			    if (soc_mem_field32_get(unit, STATSCFGm, &statsCfg, RECORD0_EVENT_SELf)) {
				segment = soc_mem_field32_get(unit, STATSCFGm, &statsCfg, RECORD0_SEGMENT_SELf);
				if (COMPILER_64_AND(temp, sir->cs.segment[segment].cntrIdMap) != 0) {
				    if (shared) {
					if (sir->cs.segment[segment].shared)
					    CS_STAT_COLOR_SET(dp);
				    } else {
		...
		COSQ_ERR(("gport_profile_set: unit %d stats_label mode unsupported\n",
			  unit));
		return BCM_E_UNAVAIL;
		break;

	    case qm_addr_histogram:
		COSQ_ERR(("gport_profile_set: unit %d histogram mode unsupported\n",
			  unit));
		return BCM_E_UNAVAIL;
		break;
	    default:
				     ...
				     uint64 value)
{
    ...
    uint8_t bit_size = 0, level = 0, type = 0;
    uint16_t cntrId = 0;
    int segment = -1;
	...
	COSQ_ERR(("stat_set: unit %d value %llu not 0\n", unit, value));
	return BCM_E_PARAM;
    }
	...
	if (rv != BCM_E_NONE) {
	    COSQ_ERR(("Invalid gport(0x%x) for stat %d\n", gport, stat));
	    return rv;
	}
	COMPILER_64_SET(sir->cs.fd_drop[type][port*2+idx], COMPILER_64_HI(value),COMPILER_64_LO(value));
	return BCM_E_NONE;
    }
	...
	sal_memset(&tdata, 0, sizeof(bcm_trunk_add_info_t));
	...
	if (BCM_GPORT_IS_EGRESS_MODPORT(gport)) {
	    ...
	    if (rv != BCM_E_NONE) {
		COSQ_ERR(("Invalid gport(0x%x). Unable to resolve a port\n",
			  gport));
		return rv;
	    }
	    idx = cosq;
	...
	rv = bcm_sirius_trunk_find_and_get(unit, mod, gport, &tid, &tdata);
	if (rv == BCM_E_NONE) {
	    if ((tdata.num_ports > 0) && (tdata.num_ports <= SB_FAB_DEVICE_SIRIUS_NUM_HG_FABRIC_PORTS)) {
		loopcnt = tdata.num_ports;
	    } else {
		loopcnt = 1;
	    }
	} else {
	...
	for (i=0; i < loopcnt; i++) {
	     ...
	     */
	    if (rv == BCM_E_NONE) {
		BCM_IF_ERROR_RETURN(bcm_sirius_get_egress_info(unit, tdata.tp[i], &bit_size, &level, &cntrId, &intf));
	    }
	    cntrId += idx;
	     ...
	     */
	    if (type == 16) {
		COSQ_ERR(("stat_set: unit %d Egress type unknown gport %d flags %d"
	     ...
	     */
	    if (segment < 0) {
		COSQ_ERR(("stat_set: unit %d Egress level %d not configured on"
			  " type %d\n", unit, level, type));
		return BCM_E_UNAVAIL;
	    }
	     ...
	     */
	    if ((sir->cs.segment[segment].cntrIdMap & (1 << (cntrId >> CS_CFG_GRP_SHIFT))) == 0) {
		COSQ_ERR(("stat_set: unit %d stat %d for level %d not currently"
			  " configured\n", unit, cntrId, level));
		return BCM_E_PARAM;
	    }
		...
		case bcmCosqGportOutBytes:
		    ...
		    break;
		default:
		    COSQ_ERR(("stat_set: unit %d Egress stat %d invalid\n", unit, stat));
		    return BCM_E_PARAM;
	    }
	}
	return BCM_E_NONE;
	...
	switch (stat) {
		...
		break;
	    default:
		soc_cm_print("gport_profile_set: unit %d stat %d not supported under SLQ\n", unit, stat);
		return BCM_E_PARAM;
	}
    }
	...
	switch(stat) {
		...
		break;
	    default:
		COSQ_ERR(("stat_set: unit %d, Ingress stat %d not valid\n",
			  unit, stat));
		return BCM_E_PARAM;
	}
	...
	if (dp < 8) {
		...
		COSQ_ERR(("stat_set: unit %d Ingress stat %d not configured\n",
			  unit, stat));
		return BCM_E_UNAVAIL;
	    }
	} else if (!shared && (dp == 8)) {
	...
	if (COMPILER_64_AND(temp, sir->cs.segment[segment].cntrIdMap) == 0) {
	    COSQ_ERR(("stat_set: unit %d Ingress stat %d for segment %d not currently"
		      " configured\n", unit, stat, segment));
	    return BCM_E_PARAM;
	}
    }
...
set_stat:
	...
	COSQ_ERR(("%s: unit %d Illegal gport 0x%x\n",FUNCTION_NAME(),gport));
	return BCM_E_PARAM;
    }
				     ...
				     uint64 *value)
{
	...
	for (i=0; i < loopcnt; i++) {
	     ...
	     */
	    if (type == 16) {
		COSQ_ERR(("stat_get: unit %d Egress type unknown gport %d flags %d"
			  " stat %d\n", unit, gport, flags, stat));
		return BCM_E_UNAVAIL;
	    }
	     ...
	     */
	    if (segment < 0) {
		COSQ_ERR(("stat_get: unit %d Egress level %d not configured on"
			  " type %d\n", unit, level, type));
		return BCM_E_UNAVAIL;
	    }
	     ...
	     */
	    if ((sir->cs.segment[segment].cntrIdMap & (1 << (cntrId >> CS_CFG_GRP_SHIFT))) == 0) {
		COSQ_ERR(("stat_get: unit %d stat %d for level %d not currently"
			  " configured\n", unit, cntrId, level));
		return BCM_E_PARAM;
	    }
		...
		case bcmCosqGportOutBytes:
		    ...
		    break;
		default:
		    COSQ_ERR(("stat_get: unit %d Egress stat %d invalid\n", unit, stat));
		    return BCM_E_PARAM;
	    }
	}
	return BCM_E_NONE;
	...
	switch (stat) {
		...
		break;
	    default:
		soc_cm_print("gport_profile_get: unit %d stat %d not supported under SLQ\n", unit, stat);
		return BCM_E_PARAM;
	}
    }
	...
	switch(stat) {
		...
		break;
	    default:
		COSQ_ERR(("stat_get: unit %d, Ingress stat %d not valid\n",
			  unit, stat));
		return BCM_E_PARAM;
	}
	...
	if (dp < 8) {
		...
		COSQ_ERR(("stat_get: unit %d Ingress stat %d not configured\n",
			  unit, stat));
		return BCM_E_UNAVAIL;
	    }
	} else if (!shared && (dp == 8)) {
	...
	if (COMPILER_64_AND(temp, sir->cs.segment[segment].cntrIdMap) == 0) {
	    COSQ_ERR(("stat_get: unit %d Ingress stat %d for segment %d not currently"
		      " configured\n", unit, stat, segment));
	    return BCM_E_PARAM;
	}
...
get_stat:
	...
	case bcmCosqGportOverSubscribeGuaranteedDroppedBytes:
		...
		*value = sir->cs.slq_stats[cosq*16 + 29];
	    break;
	default:
	    COSQ_ERR(("gport_profile_get: unit %d unknown mode\n", unit));
	    return BCM_E_PARAM;
    }
    return BCM_E_NONE;
}
					   ...
					   uint64 value)
{
	...
	COSQ_ERR(("stat_multi_set: unit %d value %llu not 0\n", unit, value));
	return BCM_E_PARAM;
    }
	...
	for (j = 0; j < stat_count; j++) {
		 ...
		 */
		if (type == 16) {
		    COSQ_ERR(("stat_multi_set: unit %d Egress type unknown gport %d"
			      " stat %d\n", unit, gport, stat));
		    return BCM_E_UNAVAIL;
		}
		 ...
		 */
		if (segment < 0) {
		    COSQ_ERR(("stat_multi_set: unit %d Egress level %d not configured on"
			      " type %d\n", unit, level, type));
		    return BCM_E_UNAVAIL;
		}
		 ...
		 */
		if ((sir->cs.segment[segment].cntrIdMap & (1 << (cntrId >> CS_CFG_GRP_SHIFT))) == 0) {
		    COSQ_ERR(("stat_multi_set: unit %d stat %d for level %d not currently"
			      " configured\n", unit, cntrId, level));
		    return BCM_E_PARAM;
		}
		...
		for (k = 0; k < num_cos; k++) {
			...
			case bcmCosqGportOutBytes:
			    ...
			    break;
			default:
			    COSQ_ERR(("stat_multi_set: unit %d Egress stat %d invalid\n", unit, stat));
			    rv = BCM_E_PARAM;
		    }
		}
	    }
	...
	for (j = 0; j < stat_count; j++) {
		...
		case bcmCosqGportBlackDiscardDroppedBytes:
		    ...
		    break;
		default:
		    COSQ_ERR(("stat_multi_set: unit %d Ingress stat %d not supported in multi mode\n",
			      unit, stat));
		    rv = BCM_E_UNAVAIL;
		    continue;
	    }
	    ...
	    if (COMPILER_64_AND(temp, sir->cs.segment[segment].cntrIdMap) == 0) {
		COSQ_ERR(("stat_multi_set: unit %d Ingress stat %d for segment %d not currently"
			  " configured\n", unit, stat, segment));
		continue;
	    }
					   ...
					   uint64 *value_array)
{
	...
	if (value_count < (num_cos * stat_count)) {
	    COSQ_ERR(("stat_multi_get: unit %d value_count %d less than stats requested %d\n", unit, value_count, (num_cos * stat_count)));
	    return BCM_E_PARAM;
	}
	...
	for (i=0; i < loopcnt; i++) {
	     ...
	     */
	    if (type == 16) {
		COSQ_ERR(("stat_multi_get: unit %d Egress type unknown gport %d\n", unit, gport));
		return BCM_E_UNAVAIL;
	    }
	     ...
	     */
	    if (segment < 0) {
		COSQ_ERR(("stat_multi_get: unit %d Egress level %d not configured on type %d\n", unit, level, type));
		return BCM_E_UNAVAIL;
	    }
	     ...
	     */
	    if ((sir->cs.segment[segment].cntrIdMap & (1 << (cntrId >> CS_CFG_GRP_SHIFT))) == 0) {
		COSQ_ERR(("stat_multi_get: unit %d stat %d for level %d not currently"
			  " configured\n", unit, cntrId, level));
		return BCM_E_PARAM;
	    }
		...
		for (j = 0; j < stat_count; j++) {
			...
			case bcmCosqGportOutBytes:
			    ...
			    break;
			default:
			    COSQ_ERR(("stat_multi_get: unit %d Egress stat %d invalid\n", unit, stat));
			    return BCM_E_PARAM;
		    }
		}
		COMPILER_64_SET(value_array[k*stat_count + j], u64_H(value), u64_L(value));
	    }
	}
	return BCM_E_NONE;
    }
	...
	for (j = 0; j < stat_count; j++) {
		...
		case bcmCosqGportBlackDiscardDroppedBytes:
		    ...
		    break;
		default:
		    COSQ_ERR(("stat_multi_get: unit %d Ingress stat %d not supported in multi mode\n",
			      unit, stat));
		    rv = BCM_E_UNAVAIL;
		    continue;
	    }
	    ...
	    if (COMPILER_64_AND(temp, sir->cs.segment[segment].cntrIdMap) == 0) {
		COSQ_ERR(("stat_multi_get: unit %d Ingress stat %d for segment %d not currently"
			  " configured\n", unit, stat, segment));
		continue;
	    }
                                ...
                                uint32 bytes_max)
{
	...
	fcd = SB_FAB_DEVICE_SIRIUS_INGRESS_MCAST_FC_DOMAIN*2;
    } else if (cosq == SOC_SBX_CFG_SIRIUS(unit)->mcast_nef_fifo) {
	fcd = SB_FAB_DEVICE_SIRIUS_INGRESS_MCAST_FC_DOMAIN*2 + 1;
    }
    ...
    if (rc != BCM_E_NONE) {
	BCM_ERR(("ERROR: %s, gport 0x%x does not contain fabric_port, unit %d\n",
		 FUNCTION_NAME(), gport, unit));
	return BCM_E_PARAM;
    }

    if (cosq >= num_fifos) {
        BCM_ERR(("cos level out of range (0-%d).\n", num_fifos-1));
        return BCM_E_PARAM;
    }
				 ...
				 int egress)
{
    ...
    int node, base_node, queue, base_queue, fifo, cs_l = -1, ps_l = -1, eg_n = 0, subport = 0;
    ...
    COSQ_VVERB(("parent_gport: 0x%x, child_gport: 0x%x\n", parent_gport, child_gport));
	...
	if (BCM_GPORT_IS_SCHEDULER(parent_gport) ||
		...
		BCM_ERR(("ERROR: %s, Scheduler unknown for gport(0x%x), Unit(%d)\n", FUNCTION_NAME(), parent_gport, unit));
		return BCM_E_INTERNAL;
	    }
	...
	if (BCM_GPORT_IS_SCHEDULER(child_gport)) {
	    ...
	    if ( !BCM_GPORT_IS_SCHEDULER(parent_gport) ) {
		BCM_ERR(("ERROR: %s, egress group must attach to a scheduler gport,  Unit(%d)\n",
			 FUNCTION_NAME(), unit));
		return BCM_E_PARAM;
	    }
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_ERR(("ERROR: %s, fabric_port %d does not contain child_gport 0x%x, unit %d\n",
	...
	if (topDown) {
		...
		BCM_ERR(("ERROR: %s, invalid parent level %d node %d, unit %d\n",
		...
		BCM_ERR(("ERROR: %s, Parent node not initialized,  Unit(%d)\n", FUNCTION_NAME(), unit));
		return BCM_E_INTERNAL;
	    }
		...
		case SIRIUS_ES_LEVEL_FIFO:
		    ...
		    if ( !BCM_GPORT_IS_SCHEDULER(parent_gport) ) {
			BCM_ERR(("ERROR: %s, Level %d egress scheduler must attach to an egress scheduler gport,  Unit(%d)\n",
				 FUNCTION_NAME(), child_level, unit));
			return BCM_E_PARAM;
		    }
			...
			if (sp_info->egroup[eg_n].es_scheduler_level0_node == 0xffff) {
			    ...
			    if (rv != BCM_E_NONE) {
				BCM_ERR(("ERROR: %s, failed to allocation child level %d egress scheduler node,  Unit(%d)\n",
					 FUNCTION_NAME(), child_level, unit));
				return BCM_E_RESOURCE;
			    }
			}
		    ...
		    /*
		     * Allocate fifo group memory resources if not in fabric_egress_setup
		     */
		...
		case SIRIUS_ES_LEVEL_SUBPORT:
			...
			BCM_ERR(("ERROR: %s, Level %d egress scheduler must attach to an egress child or scheduler gport,  Unit(%d)\n",
				 FUNCTION_NAME(), child_level, unit));
			return BCM_E_PARAM;
		    }
		    ...
		    if (rv != BCM_E_NONE) {
			BCM_ERR(("ERROR: %s, ES failed to attach subport node %d\n", FUNCTION_NAME(), child_node));
			return rv;
		    }
		...
		case SIRIUS_ES_LEVEL_CHANNEL:
			...
			BCM_ERR(("ERROR: %s, Level %d egress scheduler must attach to an egress modport gport,  Unit(%d)\n",
				 FUNCTION_NAME(), child_level, unit));
			return BCM_E_PARAM;
		    }
	...
	if (BCM_GPORT_IS_UCAST_QUEUE_GROUP(child_gport)) {
	    ...
	    rv = bcm_sbx_port_get_scheduler(unit, child_gport, &child_level, &child_node);
	    if (rv != BCM_E_NONE) {
		BCM_ERR(("ERROR: %s, Scheduler unknown for subport gport(0x%x), Unit(%d)\n", FUNCTION_NAME(), child_gport, unit));
		return BCM_E_INTERNAL;
	    }
	    ...
	    rv = bcm_sbx_port_get_scheduler(unit, child_gport, &child_level, &child_node);
	    if (rv != BCM_E_NONE) {
		BCM_ERR(("ERROR: %s, Scheduler unknown for scheduler gport(0x%x), Unit(%d)\n",
			 FUNCTION_NAME(), child_gport, unit));
		return rv;
	    }
	...
	if ( (child_level == -1) && ((parent_level != -1) && (parent_node != -1)) ) {
		...
		if (SOC_SBX_CFG_SIRIUS(unit)->nNodeUserManagementMode != TRUE) {
		    BCM_ERR(("ERROR: %s, gport 0x%x already attached,  Unit(%d)\n",
			     FUNCTION_NAME(), child_gport, unit));
		    return BCM_E_PARAM;
		}
	    }
	...
	if (topDown) {
		...
		if (hide_level1 == FALSE) {
			...
			BCM_ERR(("ERROR: %s, cos level out of range [0-%d),  Unit(%d)\n",
			...
			if (rv != BCM_E_NONE) {
			    BCM_ERR(("ERROR: %s, failed to allocation child level %d node %d,  Unit(%d)\n",
			...
			if (rv != BCM_E_NONE) {
			    BCM_ERR(("ERROR: %s, failed to connect child level %d node %d to parent level %d node %d,  Unit(%d)\n",
		    ...
		    if (num_child != 0) {
			BCM_ERR(("ERROR: %s, Parent node already has child attached,  Unit(%d)\n", FUNCTION_NAME(), unit));
			return BCM_E_PARAM;
		    }
		    ...
		    if (rv != BCM_E_NONE) {
			BCM_ERR(("ERROR: %s, can not allocate level 1 node,  Unit(%d)\n", FUNCTION_NAME(), unit));
			return BCM_E_RESOURCE;
		    }
			...
			if (rv != BCM_E_NONE) {
			    BCM_ERR(("ERROR: %s, failed to connect level 1 node %d to parent level %d node %d,  Unit(%d)\n",
			...
			if (num_level1_nodes > 1) {
			    ...
			    rv = soc_sirius_ts_node_mapping_config(unit, 1, level1_nodes[node],
								   ...
								   level1_nodes[0]);
			    if (rv != SOC_E_NONE) {
				BCM_ERR(("ERROR: %s, fail to config ingress scheduler level 1 node %d bucket info, Unit(%d)\n",
			    ...
			    if (rv != SOC_E_NONE) {
				BCM_ERR(("ERROR: %s, failed to config ingress creditor node profile for level 1 node %d, Unit(%d)\n",
					 FUNCTION_NAME(), level1_nodes[node], unit));
				return rv;
			    } else {
				BCM_DEBUG(BCM_DBG_VERBOSE,("%s config TS level 1 node %d use node_profile %d, Unit(%d)\n",
		    ...
		    rv = soc_sirius_ts_node_mapping_config(unit, parent_level, parent_node,
							   ...
							   SOC_SIRIUS_API_PARAM_NO_CHANGE);
		    if (rv != SOC_E_NONE) {
			BCM_ERR(("ERROR: %s, fail to config ingress scheduler level 2 node %d bucket info, Unit(%d)\n",
		...
		if (parent_level == 2) {
			...
			BCM_ERR(("ERROR: %s, Parent node already has queue group attached,  Unit(%d)\n", FUNCTION_NAME(), unit));
			return BCM_E_PARAM;
		    }
		}
		...
		if (ps_i_state->num_child == 0) {
			...
			child_node = parent_node * 8 + ps_i_state->num_child;
		    } else {
			...
			if ( (ps_l >= 0) && (num_nodes > 0) ) {
			    ...
			    if (rv != BCM_E_NONE) {
				BCM_ERR(("ERROR: %s, can not reserve %d child nodes,  Unit(%d)\n", 
		    ...
		    rv = bcm_sirius_cosq_scheduler_allocate(unit, child_level, child_node, 1, egress, &child_node);
		    if (rv != BCM_E_NONE) {
			BCM_ERR(("ERROR: %s, can not allocate child node,  Unit(%d)\n", FUNCTION_NAME(), unit));
			return BCM_E_RESOURCE;
		    }
		} else {
			...
			BCM_ERR(("ERROR: %s, can not attach more than 8 children to gport 0x%x,  Unit(%d)\n",
				 FUNCTION_NAME(), parent_gport, unit));
			return BCM_E_RESOURCE;
		    }
			...
			if (rv != BCM_E_NONE) {
				...
				if (rv != BCM_E_NONE) {
				    BCM_ERR(("ERROR: %s, failed to allocate node at level %d,  Unit(%d)\n",
					     FUNCTION_NAME(), child_level, unit));
				    return rv;
				}
				...
				if (rv != BCM_E_NONE) {
				    ...
				    BCM_ERR(("ERROR: %s, level %d node %d already used,  Unit(%d)\n",
		...
		COSQ_VVERB(("Update is_map, cs_l: 0x%x, level: 0x%x,"
				     ...
				     bcm_cos_queue_t *cosq, bcm_gport_t *out_gport)
{
	...
	if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: %s, fail to map gport 0x%x to its hardware resources, Unit(%d)\n",
	...
	if (BCM_GPORT_IS_EGRESS_CHILD(in_gport)) {
		...
		*out_gport = BCM_GPORT_INVALID;
	    ...
	    if (cos < 0) {
		BCM_ERR(("ERROR: %s, ingress level %d node %d has no parent, unit %d\n",
			 FUNCTION_NAME(), level, node, unit));
		return BCM_E_PARAM;
	    }
	    ...
	    *out_gport = BCM_GPORT_INVALID;
		...
		if ((egress_group < 0) || (egress_group >= SOC_SBX_CFG(unit)->num_egress_group) ||
		    ...
		    BCM_ERR(("ERROR: %s, egress group 0x%x not created,  Unit(%d)\n",
			     FUNCTION_NAME(), in_gport, unit));
		    return BCM_E_PARAM;
		}
		...
		BCM_ERR(("ERROR: %s gport 0x%x is at lowest level and has no children.\n",
	...
	if ( (BCM_GPORT_IS_UCAST_QUEUE_GROUP(in_gport)) ||
	    ...
	    if (cos >= 0) {
		BCM_ERR(("ERROR: %s gport 0x%x has no children\n",
			  FUNCTION_NAME(), in_gport));
		return BCM_E_PARAM;
	    }
	    ...
	    if (rv != SOC_E_NONE) {
		BCM_ERR(("ERROR: %s, failed to get leaf node for subscriber queue %d,  Unit(%d)\n",
		...
		if (rv != SOC_E_NONE) {
		    BCM_ERR(("ERROR: %s, fail to get ingress scheduler level %d node %d hierachy state, Unit(%d)\n",
		...
		if ((is_state[unit][2][parent].in_use == FALSE) ||
		    ...
		    BCM_ERR(("ERROR: %s, ingress scheduler hierachy state inconsistent, Unit(%d)\n",
			     FUNCTION_NAME(), unit));
		    return BCM_E_INTERNAL;
		} else {
		...
		*cosq = SOC_SBX_CFG_SIRIUS(unit)->b8kNodes?8:4;
		...
		BCM_ERR(("ERROR: %s gport 0x%x has no parent\n",
			  FUNCTION_NAME(), in_gport));
		return BCM_E_PARAM;
	    }
		...
		*cosq = is_state[unit][parent_level][parent].num_child;
		...
		if ((level > 2) || 
			...
			*out_gport = BCM_GPORT_INVALID;
		    ...
		    if (rv != SOC_E_NONE) {
			BCM_ERR(("ERROR: %s, failed to find queue mapped to leaf node %d, Unit(%d)\n", FUNCTION_NAME(), leaf_node));
			return rv;
		    }

		    rv = soc_sirius_qs_queue_to_leaf_node_get(unit, base_queue, &mc, &node);
		    if (rv != SOC_E_NONE) {
			BCM_ERR(("ERROR: %s, failed to find leaf node mapped to queue %d, Unit(%d)\n", FUNCTION_NAME(), base_queue));
			return rv;
		    }
...
bcm_sirius_cosq_sibling_node_swap(int unit, int level, int orig_node, int dest_node, int egress)
{
	...
	rv = bcm_sirius_cosq_scheduler_allocate(unit, level, BCM_INT_SBX_REQUESTED_SCHEDULER_AUTO_SELECT,
						1, egress, &tmp_node);
	if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: %s, failed to allocation temporary node on level %d for node swap,  Unit(%d)\n",
		     FUNCTION_NAME(), level, unit));
	    return BCM_E_RESOURCE;
	}
	...
	if (rv != SOC_E_NONE) {
	    BCM_ERR(("ERROR: %s, failed to clone ingress scheduler level %d node %d to node %d, Unit(%d)\n",
	...
	/* free the tmp node */
        ...
        if (BCM_E_NONE != rv) {
            BCM_ERR(("ERROR: %s, failed to free level %d node %d ingress scheduler",
        ...
        for (tmp_node = is_d->first_child; tmp_node < is_d->first_child + is_d->num_child; tmp_node++) {
            ...
            if (level > 1) {
                rv = soc_sirius_ts_node_hierachy_config(unit, level-1, tmp_node, dest_node,
                                                        ...
                                                        SOC_SIRIUS_API_PARAM_NO_CHANGE);
                if (rv != SOC_E_NONE) {
                    BCM_ERR(("ERROR: %s, failed to update ingress scheduler level %d node %d parent, Unit(%d)\n",
                             FUNCTION_NAME(), level, tmp_node, unit));
                    return rv;
                }
            }
...
bcm_sirius_cosq_ingress_scheduler_shift(int unit, int level, int node, int direction)
{
	...
	if (loop_cnt >= SOC_SBX_CFG_SIRIUS(unit)->uNumTsNode[level]) {
	    ...
	    BCM_ERR(("ERROR: %s, ingress scheduler level %d start node %d shift internal failure, Unit(%d)\n",
	...
	if (dest_node < 0) {
	    BCM_ERR(("ERROR: %s, ingress scheduler level %d exhausted, Unit(%d)\n",
		     FUNCTION_NAME(), level, unit));
	    return BCM_E_RESOURCE;
	}
	is_d = &is_state[unit][level][dest_node];
	if ( is_d->in_use != FALSE ) {
	    BCM_ERR(("ERROR: %s, ingress scheduler level %d dest node %d state inconsistent, Unit(%d)\n",
	...
	if ( ((is_c->in_use == TRUE) && (is_c->parent < 0)) ||
	     (is_c->in_use == FALSE) ) {
	    BCM_ERR(("ERROR: %s, ingress scheduler level %d node %d state inconsistent, Unit(%d)\n",
		     FUNCTION_NAME(), level, tmp_node, unit));
	    return BCM_E_INTERNAL;
	...
	rv = soc_sirius_ts_node_hierachy_config(unit, level+1, is_c->parent,
						...
						SOC_SIRIUS_API_PARAM_NO_CHANGE);
	if (rv != SOC_E_NONE) {
	    BCM_ERR(("ERROR: %s, failed to update ingress scheduler level %d node %d first_child, Unit(%d)\n",
		     FUNCTION_NAME(), level, is_c->parent, unit));
	    return rv;
	}
				 ...
				 int egress)
{
    ...
    int node, base_node, queue, base_queue, fifo, cs_l = -1, ps_l = -1, eg_n = 0, num_fifos = -1, subport = 0;
	...
	switch (child_level) {
		...
		for (node = base_node; node < (base_node + num_fifos); node++) {
		    ...
		    if (rv != BCM_E_NONE) {
			BCM_ERR(("ERROR: %s, ES failed to detach fifo node %d\n", FUNCTION_NAME(), node));
			return rv;
		    }
			...
			if (rv != BCM_E_NONE) {
			    BCM_ERR(("ERROR: %s, failed to free child level %d node %d (was %d) egress scheduler node,  Unit(%d)\n",
		...
		/*
		 * free fifo group memory resources if not in fabric_egress_setup
		 */
		if (!SOC_SBX_CFG(unit)->fabric_egress_setup) {
		...
		if (rv != BCM_E_NONE) {
		    BCM_ERR(("ERROR: %s, ES failed to detach subport node %d\n", FUNCTION_NAME(), child_node));
		    return rv;
		}
	...
	if (child_level == 0) {
		...
		for (node = base_node; node < (base_node + num_child_level1); node++) {
		    ...
		    if (rv != BCM_E_NONE) {
			BCM_ERR(("ERROR: %s, failed to disconnect child level %d node %d to parent level %d node %d,  Unit(%d)\n",
		    ...
		    rv = bcm_sirius_cosq_scheduler_free(unit, child_level, tmp_node, egress);
		    if (rv != BCM_E_NONE) {
			BCM_ERR(("ERROR: %s, can not free level %d node %d (was %d),  Unit(%d)\n", FUNCTION_NAME(),
		...
		for (node = (num_level1_nodes-1); node >= 0; node--) {
		    ...
		    if (rv != BCM_E_NONE) {
			BCM_ERR(("ERROR: %s, failed to disconnect level 1 node %d from parent level %d node %d,  Unit(%d)\n",
				 FUNCTION_NAME(), level1_nodes[node], parent_level, parent_node, unit));
			return rv;
		    }
		}
		...
		for (node = (num_level1_nodes-1); node >= 0; node--) {
		    ...
		    if (rv != BCM_E_NONE) {
			BCM_ERR(("ERROR: %s, can not free level 1 node %d,  Unit(%d)\n", FUNCTION_NAME(), level1_children[node], unit));
			return rv;
		    }
		}
	    }
	    ...
	    if (rv != SOC_E_NONE) {
		BCM_ERR(("ERROR: %s, failed to get child level %d node %d bucket info,  Unit(%d)\n",
			 FUNCTION_NAME(), child_level, child_node, unit));
		return BCM_E_RESOURCE;
	    }
		...
		for (tmp_node = 0; tmp_node < SOC_SBX_CFG_SIRIUS(unit)->uNumTsNode[child_level]; tmp_node++) {
			...
			rv = soc_sirius_ts_node_mapping_config(unit, child_level, tmp_node,
							       ...
							       bucket_new);
			if (rv != SOC_E_NONE) {
			    BCM_ERR(("ERROR: %s, fail to update ingress scheduler level %d node %d to use bucket %d, Unit(%d)\n",
					    ...
					    int weight)
{
	...
	BCM_ERR(("ERROR: %s, egress scheduler level %d does not support EF scheduling discipline, Unit(%d)\n",
		 FUNCTION_NAME(), level, unit));
        return(BCM_E_UNAVAIL);
    }
	...
	BCM_ERR(("ERROR: %s, egress scheduler level %d node %d not in use, Unit(%d)\n",
		 FUNCTION_NAME(), level, node, unit));
	return BCM_E_PARAM;
    }

    if ( (config_mode == SIRIUS_ES_SCHEDULER_MODE_SP) && ((weight < 0) || (weight > max_pri)) ) {
        BCM_ERR(("ERROR: %s, es strict pri mode invalid priority %d, valid range [0-%d]. Unit(%d)\n",
		 FUNCTION_NAME(), weight, max_pri, unit));
        return BCM_E_PARAM;
    }

    if ( (config_mode == SIRIUS_ES_SCHEDULER_MODE_WERR) && ((weight < 0) || (weight > 0x7F)) ) {
        BCM_ERR(("ERROR: %s, es werr mode invalid weight %d, valid range [0-127]. Unit(%d)\n",
		 FUNCTION_NAME(), weight, unit));
        return BCM_E_PARAM;
    }

    if ( (config_mode == SIRIUS_ES_SCHEDULER_MODE_EF) && (weight != 0) ) {
        BCM_ERR(("ERROR: %s, es ef mode invalid weight %d, Unit(%d), overriding it with 0\n",
		                                        FUNCTION_NAME(), weight, unit));
        weight = 0;
    }
	...
	if (rv != SOC_E_NONE) {
	    BCM_ERR(("ERROR: %s, es failed to get level %d node %d parent node. Unit(%d)\n",
		     FUNCTION_NAME(), level, node, unit));
	    return rv;
	}
    ...
    if (rv != SOC_E_NONE) {
	BCM_ERR(("ERROR: %s, failed to config min egress scheduler for level %d node %d, Unit(%d)\n",
		 FUNCTION_NAME(), level, node, unit));
	return rv;
    }
    ...
    if (rv != SOC_E_NONE) {
	BCM_ERR(("ERROR: %s, failed to config max egress scheduler for level %d node %d, Unit(%d)\n",
		 FUNCTION_NAME(), level, node, unit));
	return rv;
    }
					 ...
					 int bits_thresh_max)
{
    ...
    int rate = 0;
    int enable = FALSE;
    int default_threshold = 32000; /* default burst size to 4000 Bytes, safe burst size for 1G ports */
	...
	if ((gs_state[unit] == NULL) || (node < 0) || (node >= SB_FAB_DEVICE_SIRIUS_NUM_EGRESS_GROUP_SHAPER)
	    ...
	    BCM_ERR(("ERROR: %s, egress group max shaper node %d not in use, Unit(%d)\n",
		     FUNCTION_NAME(), node, unit));
	    return BCM_E_PARAM;
	}
    } else {
	...
	BCM_ERR(("ERROR: %s, egress scheduler level %d nodes do not support min shapers, Unit(%d)\n",
		 FUNCTION_NAME(), level, unit));
	return BCM_E_PARAM;
    }
	...
	if (rv != SOC_E_NONE) {
	    BCM_ERR(("ERROR: %s, failed to config min egress shaper for level %d node %d, Unit(%d)\n",
		     FUNCTION_NAME(), level, node, unit));
	    return rv;
	}
    } else {
	if ((kbits_sec_min != SOC_SIRIUS_API_PARAM_NO_CHANGE) &&
	    (kbits_sec_min != 0)) {
	    BCM_ERR(("ERROR: %s, group min shaper is not supported by sirius, Unit(%d)\n",
		     FUNCTION_NAME(), unit));
	...
	BCM_ERR(("ERROR: %s, For egress shaper with rate %d kbps, burst size is required to be "
    ...
    if (rv != SOC_E_NONE) {
	BCM_ERR(("ERROR: %s, failed to config max egress shaper for level %d node %d, Unit(%d)\n",
		 FUNCTION_NAME(), level, node, unit));
	return rv;
    }
					     ...
					     int weight)
{
	...
	BCM_ERR(("ERROR: %s, ingress scheduler level %d node %d not supported, Unit(%d)\n",
		 FUNCTION_NAME(), level, node, unit));
	return BCM_E_PARAM;
    }
    ...
    if (s_i->in_use != TRUE) {
	BCM_ERR(("ERROR: %s, ingress scheduler level %d node %d not in use, Unit(%d)\n",
		 FUNCTION_NAME(), level, node, unit));
	return BCM_E_PARAM;
    }

    if ((s_i->parent < 0) || (s_i->parent >= SOC_SBX_CFG_SIRIUS(unit)->uNumTsNode[level+1])) {
	BCM_ERR(("ERROR: %s, ingress scheduler level %d node %d not attached, Unit(%d)\n",
		 FUNCTION_NAME(), level, node, unit));
	return BCM_E_PARAM;
    }
	...
	BCM_ERR(("ERROR: %s, ingress scheduler level %d node %d state not consistent, Unit(%d)\n",
		 FUNCTION_NAME(), level+1, s_i->parent, unit));
	return BCM_E_INTERNAL;
    }
	...
	BCM_ERR(("ERROR: %s, ingress scheduler level %d node %d hierachy state inconsistent, Unit(%d)\n",
		 FUNCTION_NAME(), level, node, unit));	    
	return BCM_E_INTERNAL;
    }
	...
	BCM_ERR(("ERROR: %s, ingress scheduler level %d node %d hierachy inconsistent, Unit(%d)\n",
		 FUNCTION_NAME(), level, node, unit));	    
	return BCM_E_PARAM;
    }
	...
	/* check how many children its parent has, if only 1 child, assuming this
	 * is a pass-through node, otherwise it's part of flat scheduler
	 */
	if (s_p->num_child == 1) {
	    /* pass-through node */
	    ...
	    rv = soc_sirius_ts_node_mapping_config(unit, level+1, s_i->parent,
						   ...
						   SOC_SIRIUS_API_PARAM_NO_CHANGE);
	    if (rv != SOC_E_NONE) {
		BCM_ERR(("ERROR: %s, fail to config ingress scheduler level %d node %d bucket info, Unit(%d)\n",
			 FUNCTION_NAME(), level+1, s_i->parent, unit));
		return rv;
	    } else {
		BCM_DEBUG(BCM_DBG_VERBOSE,("%s config TS level %d node %d to node_type %d bucket_type %d, Unit(%d)\n",
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_ERR(("ERROR: %s, fail to select a bucket for ingress flat scheduler level %d node %d, Unit(%d)\n",
			 FUNCTION_NAME(), level, node, unit));
		return rv;
	    }
	}
	...
	if (rv != SOC_E_NONE) {
	    BCM_ERR(("ERROR: %s, fail to config ingress scheduler level %d node %d weight, Unit(%d)\n",
		 FUNCTION_NAME(), level, node, unit));	    
	    return rv;
	} else {
	    BCM_DEBUG(BCM_DBG_VERBOSE,("%s config TS level %d node %d child offset %d to weight %d, Unit(%d)\n",
	...
	BCM_DEBUG(BCM_DBG_VERBOSE,("%s config ingress scheduler level %d node %d node_type %d bucket_type %d, Unit(%d)\n",
     ...
     * if (rv != SOC_E_NONE) {
     *     BCM_ERR(("ERROR: %s, fail to init ingress scheduler level %d node %d state, Unit(%d)\n",
     *	       FUNCTION_NAME(), level, node, unit));	    
     *	   return rv;
     * }
     */
    ...
    if (is_state[unit][level][node].adopt_all == TRUE) {
        ...
        if (rv != BCM_E_NONE) {
            BCM_ERR(("ERROR: %s, in getting ingress scheduler node profile (0x%x)\n", FUNCTION_NAME(), rv));
            return(rv);
        }
    }
    ...
    if (rv != SOC_E_NONE) {
	BCM_ERR(("ERROR: %s, failed to config ingress creditor node profile for level %d node %d, Unit(%d)\n",
		 FUNCTION_NAME(), level, node, unit));
	return rv;
    } else {
	BCM_DEBUG(BCM_DBG_VERBOSE,("%s config TS level %d node %d use node_profile %d, Unit(%d)\n",
					       ...
					       int kbits_sec_max)
{
	...
	BCM_ERR(("ERROR: %s Level %d rate accuracy comprimised using Last Node %d and Leak Cycles %d, unit %d\n", FUNCTION_NAME(),level, last_node, leak_cycles, unit));
	rv = BCM_E_FAIL;
    } else if (kbits_sec_max > (full_bucket * 4)) {
	BCM_WARN(("WARNING: %s Level %d rate precision may be affected using Last Node %d and Leak Cycles %d, unit %d\n", FUNCTION_NAME(),level, last_node, leak_cycles, unit));
    }
					  ...
					  int bits_thresh_max)
{
    ...
    if (rv != BCM_E_NONE) {
	BCM_ERR(("ERROR: %s, Shaper Accuracy and Precision severely affected, Unit(%d)\n",
		 FUNCTION_NAME(), unit));
	return BCM_E_PARAM;
    }
    ...
    if (rv != SOC_E_NONE) {
	BCM_ERR(("ERROR: %s, failed to config ingress creditor for level %d node %d, Unit(%d)\n",
		 FUNCTION_NAME(), level, node, unit));
	return rv;
    } else {
	BCM_DEBUG(BCM_DBG_VERBOSE,("%s config ingress creditor level %d node %d, Unit(%d)\n",
				   FUNCTION_NAME(), level, node, unit));
    }
	...
	(is_state[unit][level][node].scheduler_mode == BCM_COSQ_SP6)) {
        ...
        if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: %s, failed to config ingress params for level %d node %d, Unit(%d)\n",
		 FUNCTION_NAME(), level, node, unit));
	...
	if (enable == TRUE) {
	    ...
	    if (is_state[unit][level][node].scheduler_mode == BCM_COSQ_SP1) {
		node_profile = SIRIUS_TS_NODE_PROFILE_TEMPLATE_SP1;
	    }
	    if (is_state[unit][level][node].scheduler_mode == BCM_COSQ_SP2) {
		node_profile = SIRIUS_TS_NODE_PROFILE_TEMPLATE_SP2;
	    }
	    if (is_state[unit][level][node].scheduler_mode == BCM_COSQ_SP3) {
		node_profile = SIRIUS_TS_NODE_PROFILE_TEMPLATE_SP3;
	    }
	    if (is_state[unit][level][node].scheduler_mode == BCM_COSQ_SP4) {
		node_profile = SIRIUS_TS_NODE_PROFILE_TEMPLATE_SP4;
	    }
	    if (is_state[unit][level][node].scheduler_mode == BCM_COSQ_SP5) {
    ...
    if (rv != SOC_E_NONE) {
	BCM_ERR(("ERROR: %s, failed to config max ingress shaper for level %d node %d, Unit(%d)\n",
		 FUNCTION_NAME(), level, node, unit));
	return rv;
    } else {
	BCM_DEBUG(BCM_DBG_VERBOSE,("%s config ingress shaper level %d node %d, Unit(%d)\n",
				   FUNCTION_NAME(), level, node, unit));
    }
                            ...
                            bcm_sbx_cosq_control_t *control_info)
{
    ...
    uint32 pages = 0, method = 0, uRegValue = 0;
    uint32 thresh0 = 0, thresh1 = 0, thresh2 = 0, thresh3 = 0, fifo = 0;
    ...
    bcm_trunk_t tid;
    bcm_trunk_add_info_t info; /* could be allocated dynamically */
	...
	case bcmCosqControlBandwidthBurstMax:
		...
		if ((BCM_GPORT_IS_CHILD(gport) && (soc_feature(unit, soc_feature_standalone) || soc_feature(unit, soc_feature_hybrid))) || 
		    ...
		    if (rv != BCM_E_NONE) {
			BCM_DEBUG(BCM_DBG_COSQ, ("gport control set, unsupported gport on unit %d\n", unit));
			rv = BCM_E_PARAM;
			return rv;
		    }
		    ...
		    if (rv != BCM_E_NONE) {
			BCM_DEBUG(BCM_DBG_COSQ, ("ERROR: %s BandwidthBurstMax set failed, Unit(%d)\n",
						 FUNCTION_NAME(), unit));
		    }
		    return rv;
		}
		...
		if ( (level != SIRIUS_ES_LEVEL_GROUP_SHAPER) && 
		     ((level < 0) || (node < 0)) ) {
		    BCM_ERR(("ERROR: unattached scheduler (0x%x), please attach first\n",
			     gport));
		    rv = BCM_E_PARAM;
		    return rv;
		}
		...
		if ( BCM_GPORT_IS_EGRESS_GROUP(gport) ) {
			...
			BCM_ERR(("ERROR: fifo scheduler cos %d out of range [0-%d]\n",
				 cosq, num_fifos));
			rv = BCM_E_PARAM;
			return rv;	
		    }
		...
		BCM_ERR(("ERROR:unsupported gport type\n"));
		rv = BCM_E_PARAM;
	    }
	    break;
	case bcmCosqControlBandwidthBurstMin:
		...
		if ((BCM_GPORT_IS_CHILD(gport) && (soc_feature(unit, soc_feature_standalone) || soc_feature(unit, soc_feature_hybrid))) || 
		    (BCM_GPORT_IS_SCHEDULER(gport) && BCM_INT_SBX_SCHEDULER_IS_INGRESS(gport))) {		    
		    BCM_DEBUG(BCM_DBG_COSQ, ("ERROR: %s BandwidthBurstMin not supported on ingress scheduler, Unit(%d)\n",
						 FUNCTION_NAME(), unit));
		    rv = BCM_E_UNAVAIL;
		    return rv;
		}
        ...
        case bcmCosqControlFabricConnectMinUtilization:
            ...
            if (SOC_E_NONE == rv) {
                ...
                q_state[base_queue].ingress.anemic_watermark_select = template;
	...
	case bcmCosqControlFabricPortScheduler:
		...
		if (BCM_GPORT_IS_CHILD(value)) {
		    ...
		    BCM_DEBUG(BCM_DBG_COSQ, ("gport control set, unsupported gport type for value 0x%x on unit %d\n",
					     value, unit));
		    rv = BCM_E_PARAM;
		    return rv;			
		}
		...
		if (handle >= SB_FAB_DEVICE_SIRIUS_MAX_FABRIC_PORTS) {
		    BCM_ERR(("ERROR: %s, gport 0x%x uses invalid handle %d, unit %d\n",
		...
		if ( sp_info->valid == 0) {
		    BCM_DEBUG(BCM_DBG_COSQ, ("ERROR %s, gport 0x%x not in use, unit %d",
					     FUNCTION_NAME(), gport, unit));
		    rv = BCM_E_PARAM;
		    return rv;			
		}
		...
		if (subport != handle) {
		    BCM_ERR(("ERROR: %s, fabric_port %d used at egress group creation does not match what's specified %d here, unit %d\n",
		...
		if (((sp_info->flags & SBX_SUBPORT_FLAG_TRUNK_MCAST) == 0) && 
		    ...
		    BCM_ERR(("ERROR: %s, es_fabric_port_hierachy_set was set to 1, egress schedulers are hooked"
		...
		rv = soc_sirius_es_node_get_info(unit, SIRIUS_ES_LEVEL_FIFO, node,
						 &subport, &num_cos);
		if (rv != SOC_E_NONE) {
		    BCM_ERR(("ERROR: %s, failed to get info for fifo level egress node %d, unit %d\n",
			     FUNCTION_NAME(), node, unit));	
		    return rv;
		}
		...
		if (sp_info->flags & SBX_SUBPORT_FLAG_TRUNK_MCAST) {
			...
			if ((port < 0) || (port > 3)) {
			    BCM_ERR(("ERROR: %s, gport 0x%x not connected to hgig, incorrect port %d\n",
				     FUNCTION_NAME(), gport, unit));
			    return BCM_E_INTERNAL;
			}
			rv = _bcm_sirius_aggregate_data_map_internal_port(unit, port, handle);
			if (rv != BCM_E_NONE) {
			    BCM_ERR(("ERROR: %s, FAILED to create internal higig ports, unit %d\n",
				     FUNCTION_NAME(), unit));
			    return rv;
			} else {
				...
				if (num_fifos != SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_HW_SIZE) {
				    BCM_WARN(("WARNING: %s, Multicast defaults unchanged, Admission control settings ignored, unit %d\n",
					      FUNCTION_NAME(), unit));
				}
			    } else {
		...
		BCM_ERR(("ERROR: %s, scheduler gport 0x%x can not be mapped to a fabric port, unit %d\n",
			 FUNCTION_NAME(), gport, unit));
		rv = BCM_E_PARAM;
	    } else {
		BCM_ERR(("ERROR: %s, gport 0x%x can not be mapped to a fabric port, unit %d\n",
			 FUNCTION_NAME(), gport, unit));
		rv = BCM_E_PARAM;
	    }
	    break;
	...
	case bcmCosqControlFabricPortIngressScheduler:
		...
		BCM_ERR(("ERROR: %s, tm_fabric_port_hierachy_set was set to 1, ingress schedulers are hooked"
			 " up by SDK already, this API is not supported in this mode, unit %d\n",
			 FUNCTION_NAME(), unit));
		rv = BCM_E_UNAVAIL;
	    } else {
		...
		if (BCM_GPORT_IS_SCHEDULER(gport) && BCM_INT_SBX_SCHEDULER_IS_INGRESS(gport)) {
		    ...
		    if (level < 4) {
			BCM_DEBUG(BCM_DBG_COSQ, ("gport 0x%x is at level %d, only scheduler level 4 and above"
			...
			BCM_DEBUG(BCM_DBG_COSQ, ("gport 0x%x is at node %d, out of level %d node range %d on unit %d",
			...
			BCM_DEBUG(BCM_DBG_COSQ, ("gport 0x%x is at level %d node %d not used on unit %d",
			...
			BCM_DEBUG(BCM_DBG_COSQ, ("gport control set, invalid gport for value 0x%x on unit %d\n",
						 value, unit));
			rv = BCM_E_PARAM;
			return rv;			
		    }
			...
			BCM_DEBUG(BCM_DBG_COSQ, ("flow control domain %d out of range [0, %d] on unit %d",
		    ...
		    BCM_ERR(("ERROR: %s, gport 0x%x is not an ingress scheduler, unit %d\n",
			     FUNCTION_NAME(), gport, unit));
		    rv = BCM_E_PARAM;
		}
	    }
	...
	case bcmCosqControlEgressFlowControlThreshold0:
		...
		if (sp_info->flags & SBX_SUBPORT_FLAG_TRUNK_MCAST) {
		    ...
		    if (cosq >= num_fifos) {
			BCM_ERR(("ERROR: %s, cosq %d exceeds %d fifos for gport 0x%x, unit %d\n",
				 FUNCTION_NAME(), cosq, num_fifos, gport, unit));
			return BCM_E_PARAM;
		    }
		    
		    fifo = sp_info->egroup[eg_n].es_scheduler_level0_node + cosq;
		    if (bcm_sbx_cosq_fcd_get_from_fifo(unit, fifo, &fcd) == BCM_E_NOT_FOUND) {
			fcd = sp_info->egroup[eg_n].ef_fcd;
		    }
		}

		pages =  ((uint32) value / 128) + ((((uint32) value % 128) == 0) ? 0 : 1);
		if (pages > 0x7fff) {
		    BCM_ERR(("ERROR: %s, value %u out of range 0 - 4194176, unit %d\n",
			     FUNCTION_NAME(), value, unit));
		    return BCM_E_PARAM;
		}

		/* Get current settings */
		/* Fcd range 0-262, DT_MEM range 0-131 */
		BCM_IF_ERROR_RETURN(READ_DT_MEMm(unit, MEM_BLOCK_ANY, fcd/2, &dt_entry));
		method = soc_mem_field32_get(unit, DT_MEMm, &dt_entry, METHODf);
		...
		thresh0 = soc_mem_field32_get(unit, DT_MEMm, &dt_entry, THRESHOLD0f);
		thresh1 = soc_mem_field32_get(unit, DT_MEMm, &dt_entry, THRESHOLD1f);
		thresh2 = soc_mem_field32_get(unit, DT_MEMm, &dt_entry, THRESHOLD2f);
		thresh3 = soc_mem_field32_get(unit, DT_MEMm, &dt_entry, THRESHOLD3f);
		...
		if (SOC_SBX_CFG_SIRIUS(unit)->bExtendedPortMode == TRUE) {
			...
			BCM_ERR(("ERROR: Threshold2 & Threshold3 types unsupported in Extended Port Mode\n"));
			rv = BCM_E_PARAM;
			return rv;
		    }
		    if ((fcd % 2) != 0) {
			if ((type == bcmCosqControlEgressFlowControlThreshold0) ||
			    (type == bcmCosqControlEgressFlowControlThreshold1))
			    type += 2;
		    }
		}
		...
		if (method == 0) {
			...
			soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD0f, pages);
			soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD2f, pages);
			if (pages > thresh1) {
			    soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD1f, pages);
			    soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD3f, pages);
			    BCM_WARN(("WARN: %s threshold0/2 (0x%x) larger than threshold1/3 (0x%x),"
				      "updating threshold1/3\n",
				      FUNCTION_NAME(), pages, thresh1));
			} else {
			    soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD1f, pages);
			    soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD3f, pages);
			    if (pages < thresh0) {
				soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD0f, pages);
				soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD2f, pages);
				BCM_WARN(("WARN: %s threshold1/3 (0x%x) less than threshold0/2 (0x%x),"
					  "updating threshold0/2\n",
					  FUNCTION_NAME(), pages, thresh0));
			    }
			}
		    }
		} else {
			...
			if (pages > thresh1) {
			    soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD1f, pages);
			    BCM_WARN(("WARN: %s threshold0 (0x%x) larger than threshold1 (0x%x),"
				      "updating threshold1\n",
				      FUNCTION_NAME(), pages, thresh1));
			}
			if (SOC_SBX_CFG_SIRIUS(unit)->bExtendedPortMode == FALSE) {
			    if (pages > thresh2) {
				soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD2f, pages);
				BCM_WARN(("WARN: %s threshold0 (0x%x) larger than threshold2 (0x%x),"
					  "updating threshold2\n",
					  FUNCTION_NAME(), pages, thresh2));
			    }
			    if (pages > thresh3) {
				soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD3f, pages);
				BCM_WARN(("WARN: %s threshold0 (0x%x) larger than threshold3 (0x%x),"
					  "updating threshold3\n",
					  FUNCTION_NAME(), pages, thresh3));
			    }
			}
			...
			if (pages < thresh0) {
			    soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD0f, pages);
			    BCM_WARN(("WARN: %s threshold1 (0x%x) less than threshold0 (0x%x),"
				      "updating threshold0\n",
				      FUNCTION_NAME(), pages, thresh0));
			}
			if (SOC_SBX_CFG_SIRIUS(unit)->bExtendedPortMode == FALSE) {
			    if (pages > thresh2) {
				soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD2f, pages);
				BCM_WARN(("WARN: %s threshold1 (0x%x) larger than threshold2 (0x%x),"
					  "updating threshold2\n",
					  FUNCTION_NAME(), pages, thresh2));
			    }
			    if (pages > thresh3) {
				soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD3f, pages);
				BCM_WARN(("WARN: %s threshold1 (0x%x) larger than threshold3 (0x%x),"
					  "updating threshold3\n",
					  FUNCTION_NAME(), pages, thresh3));
			    }
			}
		    } else if (type == bcmCosqControlEgressFlowControlThreshold2) {
			soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD2f, pages);
			if (SOC_SBX_CFG_SIRIUS(unit)->bExtendedPortMode == FALSE) {
			    if (pages < thresh0) {
				soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD0f, pages);
				BCM_WARN(("WARN: %s threshold2 (0x%x) less than threshold0 (0x%x),"
					  "updating threshold0\n",
					  FUNCTION_NAME(), pages, thresh0));
			    }
			    if (pages < thresh1) {
				soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD1f, pages);
				BCM_WARN(("WARN: %s threshold2 (0x%x) less than threshold1 (0x%x),"
					  "updating threshold1\n",
					  FUNCTION_NAME(), pages, thresh1));
			    }
			}
			if (pages > thresh3) {
			    soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD3f, pages);
			    BCM_WARN(("WARN: %s threshold2 (0x%x) larger than threshold3 (0x%x),"
				      "updating threshold3\n",
				      FUNCTION_NAME(), pages, thresh3));
			}
		    } else {
			...
			if (SOC_SBX_CFG_SIRIUS(unit)->bExtendedPortMode == FALSE) {
			    if (pages < thresh0) {
				soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD0f, pages);
				BCM_WARN(("WARN: %s threshold3 (0x%x) less than threshold0 (0x%x),"
					  "updating threshold0\n",
					  FUNCTION_NAME(), pages, thresh0));
			    }
			    if (pages < thresh1) {
				soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD1f, pages);
				BCM_WARN(("WARN: %s threshold3 (0x%x) less than threshold1 (0x%x),"
					  "updating threshold1\n",
					  FUNCTION_NAME(), pages, thresh1));
			    }
			}
			if (pages < thresh2) {
			    soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD2f, pages);
			    BCM_WARN(("WARN: %s threshold3 (0x%x) less than threshold2 (0x%x),"
				      "updating threshold2\n",
				      FUNCTION_NAME(), pages, thresh2));
			}
		    }
		...
		/* Fcd range 0-262, DT_MEM range 0-131 */
		BCM_IF_ERROR_RETURN(WRITE_DT_MEMm(unit, MEM_BLOCK_ANY, fcd/2, &dt_entry));
	...
	case bcmCosqControlClassMap:
	    ...
	    if (fifo == 0) {
		BCM_ERR(("ERROR: %s, Multicast Egress Group resource not allocated\n",FUNCTION_NAME()));
		return BCM_E_UNAVAIL;
	    }

	    if (cosq >= fifo) {
		BCM_ERR(("ERROR: %s, Multicast Egress Group configured for %d fifos, cosq exceeds limit.\n",FUNCTION_NAME(),fifo));
		return BCM_E_PARAM;
	    }
		...
		BCM_ERR(("ERROR:unsupported Class Map Value\n"));
		rv = BCM_E_PARAM;
		return rv;
	    }
	...
	case bcmCosqControlDropLimitAlpha:
		...
		if ((value < 0) || (value > 8)) {
		    BCM_ERR(("ERROR: Drop Limit Alpha range 0-8\n"));
		    rv = BCM_E_PARAM;
		    return rv;	
		}
		...
		if (cosq >= num_fifos) {
		    BCM_ERR(("ERROR: %s, cosq %d out of range (0-%d) for child_gport 0x%x, unit %d\n",
		...
		if (SOC_SBX_CFG(unit)->uInterfaceProtocol == SOC_SBX_IF_PROTOCOL_XGS) {
			...
			rv = bcm_sirius_trunk_find_and_get(unit, modid, sp_info->parent_gport, &tid, &info);
			if (rv == BCM_E_NOT_FOUND) {
			    COSQ_VERB(("%s, No HiGig Trunk membership, HiGig Gport: 0x%x\n",
				       FUNCTION_NAME(), gport));
			    rv = BCM_E_NONE;
			    goto droplimitalpha;
			}
			else if (rv != BCM_E_NONE) {
			    BCM_ERR(("%s, error in getting HiGig Trunk membership, HiGig Gport: 0x%x\n",
  				     FUNCTION_NAME(), gport));
			    return rv;;
			}
			
			COSQ_VERB(("%s HiGig Trunk found for Gport: 0x%x NbrPorts: %d Irunk: %d\n",
				   FUNCTION_NAME(), gport, info.num_ports, tid));
			...
			for (i = 0; i < info.num_ports; i++) {
			    ...
			    COSQ_VERB(("%s HiGig Port (%d) module (%d) part of trunk (%d)\n",
				       FUNCTION_NAME(), info.tp[i], info.tm[i], tid));

			    if (BCM_GPORT_IS_MODPORT(info.tp[i])) {
				port = BCM_GPORT_MODPORT_PORT_GET(info.tp[i]);
			    } else if (BCM_GPORT_IS_EGRESS_MODPORT(info.tp[i])) {
				port = BCM_GPORT_EGRESS_MODPORT_PORT_GET(info.tp[i]);
			    } else {
				BCM_ERR(("%s, error in HiGig Trunk member, HiGig Gport: 0x%x\n",
  				     FUNCTION_NAME(), info.tp[i]));
			    }
			    sp_info = &(SOC_SBX_STATE(unit)->port_state->subport_info[SB_FAB_DEVICE_SIRIUS_MCAST_HANDLE0+port]);
			    if ( sp_info->valid == 0) {
				BCM_DEBUG(BCM_DBG_COSQ, ("ERROR %s, MCAST gport for gport 0x%x not valid, unit %d",
							 FUNCTION_NAME(), info.tp[i], unit));
				rv = BCM_E_PARAM;
				return rv;			
			    }
	...
	case bcmCosqControlResumeLimitBytes:
		...
		if (value > 0x7fff) {
		    BCM_ERR(("ERROR: Drop Resume Bytes range 0-0x7fff\n"));
		    rv = BCM_E_PARAM;
		    return rv;	
		}
	...
	case bcmCosqControlFlowControlState:
		...
		for (fifo = sp_info->egroup[eg_n].es_scheduler_level0_node; 
		    ...
		    rv = soc_sirius_es_fc_map_table_get(unit, fifo/SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_HW_SIZE,
							0, &fc_state_index);
		    if (rv != SOC_E_NONE) {
			BCM_ERR(("ERROR:failed to get flow control state index for gport 0x%x\n", gport));
			return rv;
		    }

		    if (fc_state_index > SOC_MEM_INFO(unit, FLOW_CONTROL_STATE_TABLEm).index_max) {
			BCM_ERR(("ERROR: flow control state index %d is not valid for gport 0x%x\n",
                            ...
                            bcm_sbx_cosq_control_t *control_info)
{
    ...
    dt_mem_entry_t dt_entry;
    uint32 threshf = THRESHOLD0f, fifo = 0;
    int mymodid = -1, fcd = -1;
    int handle;
    eg_fd_fifo_thresh_entry_t fifo_thresh;
    eg_fd_fifo_thresh_offset_red_entry_t thresh_offset_red;
	...
	case bcmCosqControlBandwidthBurstMax:
		...
		rv = bcm_sbx_port_get_scheduler(unit, gport, &level, &node);
		if (rv != BCM_E_NONE) {
		    BCM_DEBUG(BCM_DBG_COSQ, ("gport control get, unsupported gport on unit %d\n", unit));
		    rv = BCM_E_PARAM;
		    return rv;
		}
		...
		if (rv != BCM_E_NONE) {
		    BCM_DEBUG(BCM_DBG_COSQ, ("ERROR: %s BandwidthBurstMax get failed, Unit(%d)\n",
					     FUNCTION_NAME(), unit));
		}
	...
	case bcmCosqControlFabricPortIngressScheduler:
		...
		rv = bcm_sbx_port_get_scheduler(unit, gport, &level, &node);
		if (rv != BCM_E_NONE) {
		    BCM_ERR(("ERROR: %s, gport control set, unsupported gport on unit %d\n",
			    FUNCTION_NAME(), unit));
		    rv = BCM_E_PARAM;
		    return rv;
		}
		...
		if (level < 4) {
		    BCM_ERR(("ERROR: %s, gport 0x%x is at level %d, only scheduler level 4 and above"
		...
		if (node >= SOC_SBX_CFG_SIRIUS(unit)->uNumTsNode[level]) {
		    BCM_ERR(("ERROR: %s, gport 0x%x is at node %d, out of level %d node range %d on unit %d",
		...
		s_i_state = &is_state[unit][level][node];
		if ((s_i_state == NULL) || (s_i_state->in_use == FALSE)) {
		    BCM_ERR(("ERROR %s, gport 0x%x is at level %d node %d not used on unit %d",
		...
		if (SOC_SBX_CFG(unit)->bEgressFifoIndependentFlowControl) {
		    ...
		    max_fc_domain = SB_FAB_DEVICE_SIRIUS_INGRESS_MCAST_FC_DOMAIN;
		}
		...
		if (rv != BCM_E_NONE) {
		    BCM_ERR(("ERROR: %s, failed to find flow control domain for gport 0x%x on unit %d",
			     FUNCTION_NAME(), gport, unit));
		    return rv;
		}

		if (SOC_SBX_STATE(unit)->port_state->subport_info == NULL) {
		    BCM_ERR(("ERROR: $s, no subport info on unit %d", FUNCTION_NAME(), unit));
		...
		if (rv != BCM_E_NONE) {
		    BCM_ERR(("ERROR: %s, no subport matches flow control domain on unit %d", FUNCTION_NAME(), unit));
		    return rv;
		} else {
		    BCM_IF_ERROR_RETURN(bcm_stk_modid_get(unit, &mymodid));
		...
		BCM_ERR(("ERROR: %s, gport 0x%x is not an ingress schedulers, unit %d\n",
			 FUNCTION_NAME(), gport, unit));
		rv = BCM_E_PARAM;
	    }
	    break;
	...
	case bcmCosqControlEgressFlowControlThreshold0:
		...
		/* Get current settings */
		BCM_IF_ERROR_RETURN(READ_DT_MEMm(unit, MEM_BLOCK_ANY, fcd/2, &dt_entry));
		*value = 128 * soc_mem_field32_get(unit, DT_MEMm, &dt_entry, threshf);
	...
	case bcmCosqControlDropLimitAlpha:
		...
		if (SOC_SBX_CFG(unit)->uInterfaceProtocol == SOC_SBX_IF_PROTOCOL_XGS) {
			...
			for (i = 0; i < info.num_ports; i++) {
				...
				*value = -1;
				BCM_DEBUG(BCM_DBG_COSQ, ("ERROR %s, Gport 0x%x Fifo Entry %d configured not dynamic, unit %d",
	...
	case bcmCosqControlDropLimitBytes:
		...
		if (SOC_SBX_CFG(unit)->uInterfaceProtocol == SOC_SBX_IF_PROTOCOL_XGS) {
			...
			for (i = 0; i < info.num_ports; i++) {
				...
				*value = -1;
				BCM_DEBUG(BCM_DBG_COSQ, ("ERROR %s, Gport 0x%x Fifo Entry %d configured for dynamic, unit %d",
							 FUNCTION_NAME(), gport, fifo, unit));
				rv = BCM_E_PARAM;
			    }
			    return rv;
				  ...
				  bcm_sbx_cosq_queue_params_ingress_t *p_qparams)
{
	...
	COSQ_VVERB(("  Number Current Buffers(%d)\n", nbr_bufs));
	COSQ_VVERB(("  Guaranteed Buffers(%d)\n", current_buffs_guaranteed));
	current_buffs_guaranteed += nbr_bufs;
	COSQ_VVERB(("  Modified Guaranteed Buffers(%d)\n", current_buffs_guaranteed));
	if (current_buffs_guaranteed >= (SOC_SBX_CFG_SIRIUS(unit)->uMaxBuffers - 128)) {
	     ...
	     */
	    SOC_ERROR_PRINT((DK_ERR, "Run out of guarranteed buffers when adding queue(%d)\n", queue));
	    return BCM_E_RESOURCE;
	}
	...
	COSQ_VVERB(("  bufferTemplate(%d)\n", buffs_template));
	    ...
	    /* The bw_group contains the bag rate  */
	    _bcm_sirius_rate_delta_max_index_for_port_rate_chosen(p_bwparams->bag_rate_kbps,
								  &rate_delta_max_index);
	...
	if (!queue_add_done) {
	    SOC_ERROR_PRINT((DK_ERR, "Timeout awaiting queue_add completion queue(%d)\n", queue));
	    rv = BCM_E_TIMEOUT;
	}
	else if (soc_reg_field_get(unit, QMB_QUEUE_CONFIG_CTRLr, regval, CQ_ERRf)) {
	    SOC_ERROR_PRINT((DK_ERR, "queue_add failed in hardware for queue(%d)\n", queue));

	}
	else {
	    COSQ_VERB(("queue_add succeeded: rate_delta_max_index(%d)"
                       " buffs_template(%d)\n", rate_delta_max_index,
                       buffs_template));
	}
    }
	...
	soc_reg_field_set(unit, QMB_QUEUE_CONFIG_CTRLr, &regval, CQ_REQf, 1);
  	COSQ_VERB(("  Write QMB_QUEUE_CONFIG_CTRL(0x%x)\n", regval));
	BCM_IF_ERROR_RETURN(WRITE_QMB_QUEUE_CONFIG_CTRLr(unit, regval));
	...
	if (!queue_add_done) {
	    SOC_ERROR_PRINT((DK_ERR, "Timeout awaiting queue_delete completion queue(%d)\n", queue));
	    rv = BCM_E_TIMEOUT;
	}
	else if (soc_reg_field_get(unit, QMB_QUEUE_CONFIG_CTRLr, regval, CQ_ERRf)) {
	    SOC_ERROR_PRINT((DK_ERR, "queue_delete failed in hardware for queue(%d)\n", queue));
            rv = BCM_E_FAIL;
	}
	else {
  	    COSQ_VERB(("queue_delete succeeded for queue(%d)\n",queue));
	}
        ...
        if (rv == BCM_E_NONE) {
            ...
            if (rc != BCM_E_NONE) {
	        SOC_ERROR_PRINT((DK_ERR, "error reading QBUFPROFILE table, queue(%d)\n", queue));
            }
            else {
  	        COSQ_VVERB(("  queue(%d) QBUFFSPROFILE(%d)\n", queue, profileEntry.entry_data[0]));

                queue_enable = soc_QBUFFSPROFILEm_field32_get(unit, &profileEntry, ENABLEf);
                if (queue_enable == TRUE) {
	            SOC_ERROR_PRINT((DK_ERR, "  queue delete failed to update enable bit in QBUFPROFILE, queue(%d)\n", queue));
                }
                else {
  	            COSQ_VVERB(("  queue(%d) Enable(%d)\n", queue, queue_enable));
                }
            }
        }
...
bcm_sirius_cosq_group_shaper_allocate(int unit, int egress, int *shaper)
{
	...
	if (*shaper < 0) {
	    ...
	    if (rv != SOC_E_NONE) {
		BCM_ERR(("ERROR: failed to init egress group shaper (%d)\n",
			 *shaper));
		return rv;
	    }
	}
    } else {
	/* TODO: ingress group shapers */
	rv = BCM_E_UNAVAIL;
    }
...
bcm_sirius_cosq_group_shaper_free(int unit, int egress, int shaper)
{
	...
	if ( (s_g_state == NULL) ||
	    ...
	    BCM_ERR(("ERROR: egress group shaper (%d) doesn't exist\n", shaper));
	    return BCM_E_PARAM;
	}
	...
	if (s_g_state->in_use == FALSE) {
	    BCM_ERR(("ERROR: egress group shaper (%d) not in use\n", shaper));
	    return BCM_E_PARAM;
	}
				   ...
				   int num_nodes, int egress, int *p_node)
{
    ...
    *p_node = - 1;
	...
	if (found == TRUE) {
		...
		if (rv != SOC_E_NONE) {
		    BCM_ERR(("ERROR: %s, can not init egress scheduler level %d node %d, Unit(%d)\n",
	...
	if (found == TRUE) {
		...
		if (rv != SOC_E_NONE) {
		    BCM_ERR(("ERROR: %s, can not init ingress scheduler level %d node %d, Unit(%d)\n",
			     FUNCTION_NAME(), level, *p_node, unit));
		    return BCM_E_INTERNAL;
		}
	    }
...
bcm_sirius_cosq_scheduler_free(int unit, int level, int node, int egress)
{
	...
	if (s_e_state->in_use != TRUE) {
	    BCM_WARN(("WARN: %s, egress scheduler level %d node %d is not in use Unit(%d)\n",
		     FUNCTION_NAME(), level, node, unit));
	}
	BCM_SIRIUS_EGRESS_SCHEDULER_STATE_INIT_PTR(s_e_state);
    } else {
	...
	if (s_i_state->in_use != TRUE) {
	    ...
	    BCM_WARN(("WARN: %s, ingress scheduler level %d node %d is not in use Unit(%d)\n",
		      FUNCTION_NAME(), level, node, unit));
	    */
	    return BCM_E_NONE;
	} else {
	    if (s_i_state->num_child != 0) {
		BCM_ERR(("ERROR: %s, ingress scheduler level %d node %d still has children, can not be freed Unit(%d)\n",
			 FUNCTION_NAME(), level, node, unit));
		return BCM_E_PARAM;
	    } else {
		BCM_SIRIUS_INGRESS_SCHEDULER_STATE_INIT_PTR(s_i_state);
    ...
    if (rv != SOC_E_NONE) {
	BCM_ERR(("ERROR: %s, can not re-init %s scheduler level %d node %d, Unit(%d)\n",
...
bcm_sirius_cosq_scheduler_init(int unit, int level, int node, int egress)
{
    ...
    rv = soc_sirius_scheduler_init(unit, level, node, egress);
    if (rv != SOC_E_NONE) {
	BCM_ERR(("ERROR: %s, can not init %s scheduler level %d node %d, Unit(%d)\n",
		 FUNCTION_NAME(), egress?"egress":"ingress", level, node, unit));
    }
				  ...
				  int queue, int egress, int fifo_num)
{
	...
	switch (child_level) {
		...
		if ( fifo_num == -1 ) {
		    BCM_ERR(("ERROR: %s, egress scheduler level 0 child node has no allocated fifos for Unit (%d)\n",
			     FUNCTION_NAME(), child_node, unit));
		    return BCM_E_PARAM;
		}

		if ( (child_node < 0) || (child_node >= SB_FAB_DEVICE_SIRIUS_NUM_EGRESS_SCHEDULER_L0) ) {
		    BCM_ERR(("ERROR: %s, egress scheduler level 0 child node %d out of range [0-%d] Unit(%d)\n",
		...
		if ( (parent_node < 0) || (parent_node >= SB_FAB_DEVICE_SIRIUS_NUM_EGRESS_SCHEDULER_L1) ) {
		    BCM_ERR(("ERROR: %s, egress scheduler level 0 parent node %d out of range [0-%d] Unit(%d)\n",
		...
		if (BCM_E_NONE != rv) {
		    BCM_ERR(("Unable to read %d:subportMap[%d]: %d (%s)\n", unit, parent_node,
			     rv, _SHR_ERRMSG(rv)));
		    return rv;
		}
		...
		if ( (child_node < 0) || (child_node >= SB_FAB_DEVICE_SIRIUS_NUM_EGRESS_SCHEDULER_L1) ) {
		    BCM_ERR(("ERROR: %s, egress scheduler level 1 child node %d out of range [0-%d] Unit(%d)\n",
		...
		if ( (parent_node < 0) || (parent_node >= SB_FAB_DEVICE_SIRIUS_NUM_EGRESS_SCHEDULER_L2) ) {
		    BCM_ERR(("ERROR: %s, egress scheduler level 2 parent node %d out of range [0-%d] Unit(%d)\n",
		...
		if (BCM_E_NONE != rv) {
		    BCM_ERR(("Unable to read %d:channelMap[%d]: %d (%s)\n", unit, parent_node,
			     rv, _SHR_ERRMSG(rv)));
		    return rv;
		}
		...
		if ( (child_node < 0) || (child_node >= SB_FAB_DEVICE_SIRIUS_NUM_EGRESS_SCHEDULER_L2) ) {
		    BCM_ERR(("ERROR: %s, egress scheduler level 2 child node %d out of range [0-%d] Unit(%d)\n",
		...
		if ( (parent_node < 0) || (parent_node >= SB_FAB_DEVICE_SIRIUS_NUM_EGRESS_SCHEDULER_L3) ) {
		    BCM_ERR(("ERROR: %s, egress scheduler level 3 parent node %d out of range [0-%d] Unit(%d)\n",
		...
		break;
	    default:
		BCM_ERR(("ERROR: %s, egress scheduler level 3 to 4 is fix mapped, Unit(%d)\n", FUNCTION_NAME(), unit));
		return BCM_E_INTERNAL;
	}
	...
	if (ps_i_state->num_child == 0) {
	    ...
	    BCM_ERR(("ERROR: %s, Attached more than %d children to a parent node,  Unit(%d)\n",
		     FUNCTION_NAME(), max_num_child, unit));
	    return BCM_E_PARAM;
	} else if ( (child_level != 0) && 
		    (child_node != (ps_i_state->first_child + ps_i_state->num_child)) &&
		    (child_node != ps_i_state->first_child) ) {
	    BCM_ERR(("ERROR: %s, child level %d node %d not expected first %d or last child %d of parent node, Unit(%d)\n",
		     FUNCTION_NAME(), child_level, child_node, ps_i_state->first_child,
		     (ps_i_state->first_child + ps_i_state->num_child), unit));
	    return BCM_E_PARAM;
	} else if (child_level == 0) {
	    /* following check assuming level 0 has to attach lowest node first */
	    if ( (child_node >= (ps_i_state->first_child + max_num_child)) ||
		 (child_node <= ps_i_state->first_child)) {
		BCM_ERR(("ERROR: %s, child level %d node %d not in valid change range of parent node, Unit(%d)\n",
			 FUNCTION_NAME(), child_level, child_node, unit));
		return BCM_E_PARAM;
	    }
	}
	...
	if (child_level == 0) {
	    ...
	    rv = soc_sirius_qs_queue_to_leaf_node_set(unit, queue,
						      ...
						      child_node);
	    if (rv != SOC_E_NONE) {
		BCM_ERR(("ERROR: %s, map queue to leaf node failed, Unit(%d)\n", FUNCTION_NAME(), unit));
		return rv;
	    }
	    ...
	    if (rv != SOC_E_NONE) {
		BCM_ERR(("ERROR: %s, map leaf node to queue failed, Unit(%d)\n", FUNCTION_NAME(), unit));
		return rv;
	    }
	    ...
	    if (rv != SOC_E_NONE) {
		BCM_ERR(("ERROR: %s, set leaf node destination failed, Unit(%d)\n", FUNCTION_NAME(), unit));
		return rv;
	    }
	} else {
	    ...
	    rv = soc_sirius_ts_node_hierachy_config(unit, child_level, child_node, parent_node, SOC_SIRIUS_API_PARAM_NO_CHANGE,
						    SOC_SIRIUS_API_PARAM_NO_CHANGE);
	    if (rv != SOC_E_NONE) {
		BCM_ERR(("ERROR: %s, can not attach level %d children node %d to the parent node %d,  Unit(%d)\n",
	    ...
	    rv = soc_sirius_ts_node_hierachy_config(unit, parent_level, parent_node, SOC_SIRIUS_API_PARAM_NO_CHANGE,
						    ps_i_state->first_child, ps_i_state->num_child);
	    if (rv != SOC_E_NONE) {
		BCM_ERR(("ERROR: %s, can not attach level %d parent node %d to the child node %d,  Unit(%d)\n",
    ...
    if (rv == BCM_E_NONE) {
	COSQ_DEBUG(BCM_DBG_VERBOSE, ("connected %s level %d node %d to level %d node %d.\n",
				     ...
				     int queue, int egress, int *discon_child)
{
    ...
    if (!discon_child) {
        BCM_ERR(("ERROR: %s discon_child argument must not be NULL\n",
                 FUNCTION_NAME()));
        return BCM_E_INTERNAL;
    }
    ...
    *discon_child = child_node;
	...
	if (child_level == 0) {
		...
		BCM_ERR(("ERROR: %s, child node %d not the child of the parent node %d. range [%d-%d],  Unit(%d)\n",
	    ...
	    /* queue to leaf node table, multicast bit is left alone */
	    rv = soc_sirius_qs_queue_to_leaf_node_set(unit, queue, 0, 0xFFFF);
	    if (rv != SOC_E_NONE) {
		BCM_ERR(("ERROR: %s, map queue to leaf node failed, Unit(%d)\n", FUNCTION_NAME(), unit));
		return rv;
	    }
		...
		if (child_node != last_child) {
		    BCM_ERR(("ERROR: %s, child node %d not the last child %d of the parent node %d.,  Unit(%d)\n",
		...
		if (rv != BCM_E_NONE) {
		    BCM_ERR(("ERROR: %s, failed to swap ingress scheduler level %d node %d to node %d, Unit(%d)\n",
	    ...
	    rv = soc_sirius_ts_node_hierachy_config(unit, parent_level, parent_node, SOC_SIRIUS_API_PARAM_NO_CHANGE,
						    ...
						    ps_i_state->num_child);
	    if (rv != SOC_E_NONE) {
		BCM_ERR(("ERROR: %s, can not detach level %d child node %d from the parent node %d,  Unit(%d)\n",
			 FUNCTION_NAME(), child_level, child_node, parent_node, unit));
		return rv;
	    }
	}
    ...
    if (rv == BCM_E_NONE) {
	COSQ_DEBUG(BCM_DBG_VERBOSE, ("disconnected %s level %d node %d from level %d node %d.\n",
				     egress?"ES":"TS", child_level, child_node, parent_level, parent_node));
    }
				   ...
				   int *attach_id)
{
    ...
    int fcd = 0;
    dt_mem_entry_t dt_entry;
    int method=0, thresh0=0, thresh1=0, thresh2=0, thresh3=0;
	...
	if (SOC_SBX_CFG(unit)->bTmeMode == SOC_SBX_QE_MODE_TME) {
	    ...
	    if (rv) {
		BCM_ERR(("ERROR: %s write to Q2SC table failed for queue(%d)\n", FUNCTION_NAME(), queue));
		return(rv);
	    }
	...
	if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: %s [%d]:unable to write to unit %d:gmt for sysport %d destination port(%d)\n",
		     FUNCTION_NAME(),
		     __LINE__, 
		     unit,
		     sysport,
	...
	if (SOC_SBX_STATE(unit)->sysport_state[sysport].fifo == -1) {
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_ERR(("ERROR: bcm_cosq_gport_sched_set() must be called, possible error,  Unit(%d)\n", unit));	    
		return (BCM_E_PARAM);
	    }
	...
	BCM_IF_ERROR_RETURN(READ_DMT_MEMm(unit, MEM_BLOCK_ANY, sysport, &dmt_entry));
	soc_mem_field32_set(unit, DMT_MEMm, &dmt_entry, DOMAIN_NUMBER0f, fcd);
	soc_mem_field32_set(unit, DMT_MEMm, &dmt_entry, DOMAIN_NUMBER1f, fcd);
	BCM_IF_ERROR_RETURN(WRITE_DMT_MEMm(unit, MEM_BLOCK_ANY, sysport, &dmt_entry));

	if (SOC_SBX_CFG(unit)->bEgressFifoIndependentFlowControl) {
	    method =  0;
	    if (eSp_info->egroup[eEg_n].port_speed < 1001) {
		thresh0 = 0x0186;
		thresh1 = 0x7FFF;
	    } else {
		thresh0 = 3000;
	...
	sal_memset(&dt_entry, 0, sizeof(dt_entry));
	soc_mem_field32_set(unit, DT_MEMm, &dt_entry, METHODf, method);
	soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD0f, thresh0);
	soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD1f, thresh1);
	soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD2f, thresh2);
	soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD3f, thresh3);
				   ...
				   int attach_id)
{
	...
	BCM_ERR(("ERROR: %s, cannot retrieve system modid on unit %d\n",
		 FUNCTION_NAME(), unit));
	return BCM_E_INTERNAL;
    }
	...
	BCM_ERR(("ERROR: %s, Unexpected gport from queue %d, 0x%x\n",
	...
	rv = soc_sirius_fd_unicast_gmt_set(unit, sysport, 0);
	if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: %s [%d]:unable to write to unit %d:gmt for sysport %d\n",
		     FUNCTION_NAME(),
		     __LINE__, 
		     unit,
		     sysport));
	...
	BCM_IF_ERROR_RETURN(READ_DMT_MEMm(unit, MEM_BLOCK_ANY, sysport, &dmt_entry));
	if (SOC_SBX_CFG(unit)->bEgressFifoIndependentFlowControl) {
	    soc_mem_field32_set(unit, DMT_MEMm, &dmt_entry, DOMAIN_NUMBER0f, SIRIUS_FR_FC_MC_DOMAIN_NON_EF - 2);
	    soc_mem_field32_set(unit, DMT_MEMm, &dmt_entry, DOMAIN_NUMBER1f, SIRIUS_FR_FC_MC_DOMAIN_EF - 2);
	} else {
	    soc_mem_field32_set(unit, DMT_MEMm, &dmt_entry, DOMAIN_NUMBER0f, SIRIUS_FR_FC_MC_DOMAIN - 2);
	    soc_mem_field32_set(unit, DMT_MEMm, &dmt_entry, DOMAIN_NUMBER1f, SIRIUS_FR_FC_MC_DOMAIN - 2);	    
	}
	BCM_IF_ERROR_RETURN(WRITE_DMT_MEMm(unit, MEM_BLOCK_ANY, sysport, &dmt_entry));
	...
	if (SOC_SBX_CFG(unit)->bEgressFifoIndependentFlowControl) {
	    ...
	    soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD0f, 0x186);
	    soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD1f, 0x7FFF);
	    soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD2f, 0x186);
	    soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD3f, 0x7FFF);
	} else {
	    soc_mem_field32_set(unit, DT_MEMm, &dt_entry, METHODf, 1);
	    soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD0f, 0xC3);
	    soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD1f, 0x186);
	    soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD2f, 0x249);
	    soc_mem_field32_set(unit, DT_MEMm, &dt_entry, THRESHOLD3f, 0x7FFF);
	}
	SOC_IF_ERROR_RETURN(WRITE_DT_MEMm(unit, MEM_BLOCK_ANY, SOC_SBX_STATE(unit)->sysport_state[sysport].fcd/2, &dt_entry));
    } else if ((modid == mymodid) && ((SOC_SBX_STATE(unit)->sysport_state[sysport].use_cnt == 2) &&
				      (p_qstate->sysport == sysport) && 
				      (p_qstate->default_sysport == sysport))) {
	...
	if (SOC_SBX_CFG(unit)->bTmeMode != SOC_SBX_QE_MODE_TME) {
	    rv = soc_sirius_qs_sysport_to_queue_set(unit, mc, sysport, 0xffff);
	    if (rv) {
		BCM_ERR(("ERROR: %s write to S2Q table failed for sysport(%d)\n", FUNCTION_NAME(), sysport));
		return rv;
	    }
	}
...
static void _bcm_sirius_rate_delta_max_index_for_port_rate_chosen(uint32 bag_rate_kbps,
								  int32 *prate_delta_max_index)
{
				     ...
				     int32 *p_hold_ts, int32 *p_qtype)
{
	...
	if (SOC_SBX_CFG(unit)->local_template_id == SOC_SBX_QOS_TEMPLATE_TYPE0) {
		...
		case BCM_COSQ_AF2:
			...
			*p_qtype = SIRIUS_Q_TYPE_LOCAL_AF2;
		    ...
		    break;
		default:
		    COSQ_ERR(("Unsupported bw_mode(%d) found during AddQueue"
			      " operation\n", p_qparams->bw_mode));
		    return -1;
	    }
	}
		...
		default:
            ...
            COSQ_ERR(("Unsupported scheduling discipline template (%d)\n",
		...
		*p_qtype = SIRIUS_Q_TYPE_AF;
	    ...
	    case BCM_COSQ_SP_GLOBAL :
                ...
                *p_qtype = uPriority;
                ...
                COSQ_VVERB(("bw mode PATH_SP_GLOBAL, priority set to 0x%x\n",
                            *p_qtype));
    ...
    COSQ_VVERB(("Scheduling Discipline: 0x%x, Queue Type: 0x%x\n", bw_mode, (*p_qtype)));
			    ...
			    bcm_cosq_subscriber_map_t *map)
{
	...
	COSQ_VVERB(("Supported in Hybrid mode only\n"));
	return BCM_E_UNAVAIL;
    }

    if (SOC_SBX_CFG(unit)->uInterfaceProtocol == SOC_SBX_IF_PROTOCOL_XGS) {
	COSQ_VVERB(("Not supported in XGS mode\n"));
	return BCM_E_UNAVAIL;
    }
	...
	case BCM_COSQ_SUBSCRIBER_MAP_PORT_VLAN:
	    ...
	    if (sp_info->valid == FALSE) {
		BCM_ERR(("ERROR: %s, invalid fabric_port %d, unit %d\n",
			       ...
			       bcm_cosq_subscriber_map_t *map)
{
    ...
    switch(map->flags) {
	case BCM_COSQ_SUBSCRIBER_MAP_PORT_VLAN:
	    soc_mem_field32_set(unit, EP_OI2QB_MAPm, &oi2qb, QUEUE_BASEf, 0xffff);
	    rv = WRITE_EP_OI2QB_MAPm(unit, MEM_BLOCK_ANY, map->vlan + minOitt, &oi2qb);
	    if (rv != SOC_E_NONE) {
		return(rv);
	    }
				 ...
				 void *user_data)
{
    ...
    if (SOC_SBX_CFG(unit)->bTmeMode != SOC_SBX_QE_MODE_HYBRID) {
	COSQ_VVERB(("Supported in Hybrid mode\n"));
	return BCM_E_UNAVAIL;
    }
	...
	rv = cb(unit, 0, 0);
	if (BCM_FAILURE(rv)) {
	    soc_cm_print("Retrieving Map failed (%s)\n", bcm_errmsg(rv));
        }
    } else {
...
bcm_sirius_fc_init(int unit)
{
    ...
    fc_state[unit] = sal_alloc((sizeof(bcm_sbx_sirius_fc_state) * 132), "egress fc state");
    if (fc_state[unit] == NULL) {
        return(BCM_E_MEMORY);
    }
    sal_memset(fc_state[unit], 0x00, (sizeof(bcm_sbx_sirius_fc_state) * 132));
                      ...
                      int force_all)
{
    ...
    if (force_all == TRUE) {
	    ...
	    /* TODO: support 8 FIFO model */
            ...
            COSQ_VERB(("Flow Control Translate Table, Index: %d, Mask: 0x%x HwMask: 0x%x\n",
...
{
    int rv = BCM_E_NONE;
    int int_pri, port_nbr, fifo_nbr, flow_control_mask;
    flow_control_state_table_entry_t fc_state_table_entry;
    ...
    if (force_all == TRUE) {
        /* do all the configuration */
        for (int_pri = 0; int_pri < BCM_SBX_COSQ_STANDARD_FIFO_SET_MAX_NO_COS; int_pri++) {
            flow_control_mask = (cur_fc_map_state + int_pri)->flow_control_mask;
            if (flow_control_mask == 0) {
                for (port_nbr = 0; port_nbr < SB_FAB_DEVICE_SIRIUS_MAX_PHYSICAL_PORTS; port_nbr++) {
                    fifo_nbr = (port_nbr * BCM_SBX_COSQ_STANDARD_FIFO_SET_MAX_NO_COS) + int_pri;

                    /* clear Flow Control State (FLOW_CONTROL_STATE_TABLE) */
                    /* NOTE: no operation is required as flow control will get cleared with */
                    /*       the next message that is received. As is also no mechanism to  */
                    /*       clear flow control for a particular FIFO                       */
                }
            }
        }
    }
...
bcm_sirius_cosq_fifo_set_get(int unit, bcm_gport_t fabric_port, int *fifo_map, int *nbr_fifos, int max_fifo_map_size)
{
    ...
    if (rv != BCM_E_NONE) {
	BCM_ERR(("ERROR: %s, retreiving fabric_port info, unit: %d, rv: 0x%x\n",
    ...
    if ( (SOC_SBX_CFG(unit)->fabric_egress_setup == TRUE) ||
        ...
        /* TODO: should handle the case that fabric port uses multiple fifo sets */

        /* TODO: Also return the set of FIFOs for XGS oversubscibed model        */
    ...
    sp_info = &(SOC_SBX_STATE(unit)->port_state->subport_info[port]);
    if ( (sp_info == NULL) || (sp_info->valid != TRUE) ) {
	BCM_ERR(("ERROR: %s, invalid fabric port 0x%X%d, unit %d\n",
    ...
    /* TODO: Joey: Should it be egroup[egroup_num] or egroup[cur_group]? */
...
bcm_sirius_cosq_module_congestion_set(int unit, bcm_port_t higig_port, bcm_module_t module)
{
    ...
    if (rv != BCM_E_NONE) {
	BCM_ERR(("ERROR: %s, retreiving congestion size, unit: %d, module: 0x%x\n",
                                                       FUNCTION_NAME(), unit, module));
        return(rv);
    }

    if (max_ports == 0)  {
        COSQ_VERB(("%s HiGig Port (%d) module (%d) has no ports reserved\n",
    ...
    rv = bcm_sirius_cosq_module_congestion_allocate_get(unit, module, &cfg_max_ports,
                                                                             &fc_state_index);
    if (rv != BCM_E_NONE) {
	BCM_VERB(("%s, No congestion resources present, unit: %d, module: 0x%x\n",
                                                       FUNCTION_NAME(), unit, module));
        return(rv);
    }
    ...
    if (fifo_map == NULL) {
	BCM_VERB(("%s, Temp memory allocation failed, unit: %d\n",
		  FUNCTION_NAME(), unit));
	return BCM_E_MEMORY;
    }
    ...
    for (nbr_port = 0; nbr_port < max_ports; nbr_port++) {
	    ...
	    BCM_ERR(("ERROR: %s, retreiving fabric port, unit: %d, switch gport: 0x%x\n",
        ...
        if (BCM_GPORT_IS_CHILD(fabric_port)) {
	        ...
	        COSQ_VERB(("%s, Skiping unit: %d, switch gport: 0x%x\n",
                                                       FUNCTION_NAME(), unit, switch_port));
                continue;
            }
        }
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_ERR(("ERROR: %s, nexpected fabric port,  Switch Port: 0x%x, fabric port: 0x%x\n",
        ...
        if (subport != -1) {
            ...
            if (rv != BCM_E_NONE) {
	        BCM_ERR(("ERROR: %s, err: 0x%x\n", FUNCTION_NAME(), rv));
                goto err;
            }
            if (IS_CPU_PORT(unit, port)) {
                COSQ_VERB(("Switch Port: 0x%x, fabric port: 0x%x corresponds to CPU port\n",
	...
	if ((SOC_SBX_CFG_SIRIUS(unit)->bExtendedPortMode == TRUE) &&
        ...
        if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: %s, retreiving fifos for fabric port, unit: %d, fabric gport: 0x%x\n",
                                                       FUNCTION_NAME(), unit, switch_port));
            goto err;
        }

        COSQ_VERB(("Switch Port: 0x%x, fabric port: 0x%x FifoMap: %d, nbrFifos: %d\n",
	...
	if (SOC_SBX_CFG_SIRIUS(unit)->bExtendedPortMode == TRUE) {
        ...
        for (i = 0; i < nbr_fifos; i++) {
            ...
            if (rv != BCM_E_NONE) {
	        BCM_ERR(("ERROR: %s, configuring fc map table, unit: %d, fifo Base: 0x%x Index: 0x%x\n",
    ...
    rv = bcm_sirius_trunk_find_and_get(unit, modid, higig_gport, &tid, &info);
    if (rv == BCM_E_NOT_FOUND) {
	COSQ_VERB(("%s, No HiGig Trunk membership, HiGig Port: %d, HiGig Gport: 0x%x\n",
                                                FUNCTION_NAME(), higig_port, higig_gport));
        rv = BCM_E_NONE;
        goto done;
    }
    else if (rv != BCM_E_NONE) {
	BCM_ERR(("%s, error in getting HiGig Trunk membership, HiGig Port: %d, HiGig Gport: 0x%x\n",
                                                FUNCTION_NAME(), higig_port, higig_gport));
         rv = BCM_E_NONE;
        goto done;
    }

    COSQ_VERB(("%s HiGig Trunk found for HiGig Port/Gport: %d(0x%x) NbrPorts: %d Irunk: %d\n",
                FUNCTION_NAME(), higig_port, higig_gport, info.num_ports, tid));
    ...
    for (i = 0; i < info.num_ports; i++) {
        ...
        BCM_GPORT_MODPORT_SET(higig_gport, modid, info.tp[i]);
        rv = bcm_sirius_aggregate_gport_translate(unit, 0 /* flags */, modid, modid, higig_gport, &target[0], &target_count, NULL);
        if (rv != BCM_E_NONE) {
            BCM_ERR(("%s Error getting the internal Mc port for HiGig port/Gport %d(0x%x))\n",
                                                    FUNCTION_NAME(), info.tp[i], higig_gport));
            goto err;
        }
        if (target[0] >= SB_FAB_DEVICE_SIRIUS_MAX_PHYSICAL_PORTS) {
            BCM_ERR(("%s Unexpected internal Mc port (%d) for HiGig port/Gport %d(0x%x))\n",
                                               FUNCTION_NAME(), target, info.tp[i], higig_gport));
            continue;
        }

        COSQ_VERB(("%s found internal Mc port (%d) for HiGig port/Gport %d(0x%x))\n",
                                              FUNCTION_NAME(), target, info.tp[i], higig_gport));
                               ...
                               bcm_port_congestion_config_t *config)
{
    ...
    do {
        ...
        if (rv == BCM_E_NOT_FOUND) {
            ...
            if (rv == BCM_E_NOT_FOUND) {
	        BCM_VERB(("%s, retreiving switch port, unit: %d, fabric gport: 0x%x\n",
    ...
    if (nbr_modules == 0) {
	BCM_VERB(("%s, no Switch port found, unit: %d, higig gport: 0x%x\n",
	...
	if (max_ports == 0) {
	    ...
	    BCM_ERR(("ERROR: %s, retreiving congestion resources, unit: %d, module: 0x%x\n",
                                                       FUNCTION_NAME(), unit, module));
            return(rv);
        }
	...
	if (SOC_SBX_CFG_SIRIUS(unit)->bExtendedPortMode) {
        ...
        if (cfg_max_ports == 0) {
            ...
            if (rv != BCM_E_NONE) {
	        BCM_ERR(("ERROR: %s, allocating congestion resources, unit: %d, module: 0x%x\n",
                                                                   FUNCTION_NAME(), unit, module));
                return(rv);
            }
            ...
            if (rv != BCM_E_NONE) {
	        BCM_ERR(("ERROR: %s, configuring congestion resources, unit: %d, module: 0x%x rc: 0x%x\n",
                                                     FUNCTION_NAME(), unit, module, rv));
                return(rv);
            }
        }
        else if (cfg_max_ports != alloc_max_ports) { /* consistency check */
	    BCM_ERR(("ERROR: %s, inconsistency in congestion resource allocation, unit: %d, module: 0x%x Expected: 0x%x Configured: 0x%x\n",
        ...
        if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: %s, configuring congestion, unit: %d, module: 0x%x, rv: 0x%x\n",
                                                                FUNCTION_NAME(), unit, module, rv));
        }
    }
                                 ...
                                 void *info)
{
    ...
    if (port != BCM_GPORT_INVALID) {
	    ...
	    BCM_ERR(("ERROR: %s, Invalid gport: 0x%x, unit: %d, Expecting Child Gport\n",
                                                                FUNCTION_NAME(), port, unit));
            return(BCM_E_PARAM);
        }
	    ...
	    BCM_ERR(("ERROR: %s, Invalid child port: %d, unit: %d, Expecting mc child port\n",
    ...
    if (port != BCM_GPORT_INVALID) {
	BCM_ERR(("ERROR: %s, invalid gport, unit: %d, gport: 0x%x, expected gport: 0x%x\n",
                               FUNCTION_NAME(), unit, port, BCM_GPORT_INVALID));
        return(BCM_E_PARAM);
    }

    /* currently only supporting 4 FIFO model */
    /* TODO: support the 8 FIFO model */
    if ( (int_pri > (BCM_SBX_COSQ_STANDARD_FIFO_SET_MAX_NO_COS - 1)) && (int_pri != -1) ) {
	BCM_ERR(("ERROR: %s, invalid int_pri, unit: %d, int_pri: 0x%x, max int_pri: 0x%x\n",
	...
	BCM_ERR(("ERROR: %s, invalid flow control mask, unit: %d, flow_control_mask: 0x%x\n",
    ...
    if (int_pri != -1) {
        ...
        COSQ_VERB(("Set Flow Control, InternalPri: %d, Mask: 0x%x\n", int_pri, flow_control_mask));
    }
    else {
        for (pri = 0; pri < BCM_SBX_COSQ_STANDARD_FIFO_SET_MAX_NO_COS; pri++) {
    ...
    for (pri = 0; pri < BCM_SBX_COSQ_STANDARD_FIFO_SET_MAX_NO_COS; pri++) {
        COSQ_VERB(("Set Flow Control Configuration, InternalPri: %d, Mask: 0x%x\n",
    ...
    if (rv != BCM_E_NONE) {
	BCM_ERR(("ERROR: %s, configuring flow control mapping, unit: %d, rc: 0x%x\n",
                                                                FUNCTION_NAME(), unit, rv));
        return(rv);
    }
    ...
    if (rv != BCM_E_NONE) {
	BCM_ERR(("ERROR: %s, clearing flow control state, unit: %d, rc: 0x%x\n",
                                                                FUNCTION_NAME(), unit, rv));
        return(rv);
    }
    ...
    for (pri = 0; pri < BCM_SBX_COSQ_STANDARD_FIFO_SET_MAX_NO_COS; pri++) {
        ...
        COSQ_VERB(("Flow Control Configuration, InternalPri: %d, Mask: 0x%x\n",
                            ...
                            bcm_cos_queue_t cosq)
{
    ...
    if (SOC_SBX_CFG(unit)->uInterfaceProtocol != SOC_SBX_IF_PROTOCOL_XGS) {
	BCM_ERR(("ERROR: %s, Unit: %d, functionality not supported for interface protocol: 0x%x\n",
						      ...
						      int *bucket)
{
	...
	if ( (s_tmp->scheduler_mode != BCM_COSQ_NONE) ) {
	     ...
	     */
	    BCM_DEBUG(BCM_DBG_COSQ, ("converge on level %d node %d up % levels, unit %d\n",
...
_bcm_sirius_cosq_map_sp2gsp(int unit, int queue, int32 *p_bw_mode, int32 *p_sp_priority)
{
    ...
    int32_t bw_group;
    bcm_sbx_cosq_bw_group_state_t *p_bwstate;
    int32_t base_queue, cur_queue;
    int32_t num_cos, sp_num_cos;
    int rc = BCM_E_NONE;
...
_bcm_sirius_cosq_update_gsp(int unit, int queue, int cur_q_type, int bw_mode)
{
    ...
    for (cur_queue = base_queue, sp_num_cos = 0;
	   ...
	   rc = soc_sirius_qs_queue_parameter_get(unit, cur_queue, &q_type, &hold_ts);
           if (rc != SOC_E_NONE) {
               BCM_ERR(("ERROR: queue param table read failure\n"));
               rc = -1;
               break;
           }
           ...
           if (expected_qtype != q_type) {
               ...
               if (rc != SOC_E_NONE) {
                   BCM_ERR(("ERROR: queue param table write failure\n"));
                   rc = -1;
                   break;
               }
           }
...
_bcm_sirius_cosq_ingress_shaper_default_burst_get(int unit, int queue, uint32 *burst_size)
{
    ...
    rc = soc_sbx_div64(p_qstate[queue].ingress.shape_limit_kbps * round_ns, 1000000, &min_burst_size);
    if (rc != SOC_E_NONE) {
        BCM_ERR(("ERROR: %s failed to calculate burst size for rate of %d kbps\n",
...
_bcm_sirius_cosq_qsel_offset_set(int unit, bcm_gport_t gport, int value)
{
    ...
    if (soc_sirius_cos_map_block_check(unit, value) != SOC_E_FULL) {
        BCM_ERR(("ERROR: %s, Unit: %d, cos_map: %d not present\n", FUNCTION_NAME(), unit, value));
        return(BCM_E_PARAM);
    }
    ...
    if (SHR_BITGET(SOC_SBX_SIRIUS_STATE(unit)->inhibitQsel, base_queue)) {
        BCM_ERR(("ERROR: %s, Unit: %d, quue not part of standard SDK model\n", FUNCTION_NAME(), unit));
        return(BCM_E_PARAM);
    }
    ...
    if (BCM_GPORT_IS_UCAST_QUEUE_GROUP(gport) ||
                           BCM_GPORT_IS_UCAST_SUBSCRIBER_QUEUE_GROUP(gport)) {
        /* cos map is always updated */
        p_bwstate->cos_map = value;

        bw_state_gport = p_bwstate->gport;
        rc = _bcm_sirius_cosq_destmod_destport_get(unit, base_queue, bw_state_gport, &dest_switch_modid, &dest_switch_port);
        if (rc != BCM_E_NONE) {
            return(rc);
        }

        rc = soc_sirius_rb_higig2_header_unicast_queue_map_config(unit,
                                        dest_switch_modid, dest_switch_port, base_queue);
    }
    else {
        /* currently update cos map if not associated with any mcGroup */
        
        if (p_base_qstate->mgid_list != NULL) {
            BCM_ERR(("ERROR: %s, Unit: %d, mgid list present\n", FUNCTION_NAME(), unit));
            return(BCM_E_PARAM);
        }
        p_bwstate->cos_map = value;
    }
                                                                 ...
                                                                 int mode, int *node_profile)
{
    ...
    if (SOC_SBX_CFG(unit)->node_template_id == SOC_SBX_NODE_QOS_TEMPLATE_TYPE0) {
	    ...
	    case BCM_COSQ_NONE:
	        ...
	        break;
	    default:
	        BCM_ERR(("ERROR: %s, ingress scheduler level %d node %d unsupported sched mode, Unit(%d)\n",
        ...
        BCM_ERR(("ERROR: %s, unsupportd node scheduling discipline template %d, Unit(%d)\n",
                                      FUNCTION_NAME(), SOC_SBX_CFG(unit)->node_template_id, unit));
        return(BCM_E_PARAM);
    }
                            ...
                            bcm_sbx_cosq_control_t *control_info)
{
    ...
    if (!(BCM_GPORT_IS_SCHEDULER(gport) && BCM_INT_SBX_SCHEDULER_IS_INGRESS(gport))) {		    
        BCM_DEBUG(BCM_DBG_COSQ, ("gport control set, adopt pri setting not support on gport(0x%x) on unit %d\n", gport, unit));
        return(BCM_E_UNAVAIL);
    }
    ...
    if ((value != TRUE) && (value != FALSE)) {
        BCM_DEBUG(BCM_DBG_COSQ, ("gport control set - adopt all priority, unsupported value (%d) on unit %d\n", value, unit));
        rc = BCM_E_PARAM;
        return(rc);
    }
                            ...
                            bcm_sbx_cosq_control_t *control_info)
{
    ...
    if (!(BCM_GPORT_IS_SCHEDULER(gport) && BCM_INT_SBX_SCHEDULER_IS_INGRESS(gport))) {		    
        BCM_DEBUG(BCM_DBG_COSQ, ("gport control get, adopt pri setting not support on gport(0x%x) on unit %d\n", gport, unit));
        return(BCM_E_UNAVAIL);
    }
    ...
    if (value == NULL) {
        BCM_DEBUG(BCM_DBG_COSQ, ("gport control get - value is a NULL pointer unit %d\n", unit));
        rc = BCM_E_PARAM;
        return(rc);
    }
...
bcm_sirius_cosq_multipath_allocate(int unit, int egress, int *p_multipath)
{
	...
	if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: failed to allocate egress group max shaper\n"));
	}
    } else {
...
bcm_sirius_cosq_multipath_free(int unit, int egress, int multipath)
{
	...
	if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: failed to free egress group max shaper\n"));
	}
    } else {
	/* do nothing, there is no physical resource associated with mulipath ingress
	 * shapers
...
bcm_sirius_cosq_multipath_add(int unit, int egress, int multipath, int level, int node)
{
	...
	for (i = 0; i < BCM_INT_SBX_MAX_EGRESS_MUTIPATH_MEMBER_ARRAY_SIZE; i++) {
		...
		if ( (member[i] & (1<<j)) != 0 ) {
		    rv = soc_sirius_es_node_group_shaper_member_config(unit, 
								       ...
								       TRUE);
		    if (rv != SOC_E_NONE) {
			BCM_ERR(("ERROR: %s, failed to enable egress multipath shaper for fifo %d on unit %d\n",
	...
	if ((level < 2) || (level > 6)) {
            BCM_ERR(("ERROR: %s, ingress multipath only support level [2-6] on unit %d\n",
		     FUNCTION_NAME(), unit));
	    return BCM_E_PARAM;
	}
	...
	if (rv != SOC_E_NONE) {
	    return rv;
            ...
            BCM_ERR(("ERROR: %s, level %d node %d is a relay node, can not be part of multipath\n",
	...
	for (i = 0; i < BCM_INT_SBX_MAX_EGRESS_MUTIPATH_MEMBER_ARRAY_SIZE; i++) {
		...
		if ( (member[i] & (1<<j)) != 0 ) {
		    rv = soc_sirius_es_node_group_shaper_member_config(unit, 
								       ...
								       FALSE);
		    if (rv != SOC_E_NONE) {
			BCM_ERR(("ERROR: %s, failed to disable egress multipath shaper for fifo %d on unit %d\n",
				 FUNCTION_NAME(), (i*32+j), unit));
			return rv;
		    }
		    ps_e_state->member[i] &= (~(1<<j));
	...
	if (ps_i_state->node == node) {
		...
		for (tmp_node = 0; tmp_node < SOC_SBX_CFG_SIRIUS(unit)->uNumTsNode[level]; tmp_node++) {
			...
			if (new_master < 0) {
				...
				BCM_ERR(("ERROR: %s, ingress multipath failed to config new multipath master on unit %d\n",
					 FUNCTION_NAME(), unit));
				return rv;
			    }
			} else {
				...
				BCM_ERR(("ERROR: %s, ingress multipath failed to point to new multipath master on unit %d\n",
					 FUNCTION_NAME(), unit));
				return rv;
			    }
			}
			      ...
			      int *member_gport_info_array)
{
    bcm_sbx_cosq_ingress_multipath_state_t *ps_i_state;
    bcm_sbx_subport_info_t *sp_info = NULL;
    int level, node, tmp_node, port, found;
				...
				int *pfc_id)
{
	...
	BCM_ERR(("ERROR: %s, priority flow control not supported on Unit(%d)\n",
		 FUNCTION_NAME(), unit));
	return BCM_E_UNAVAIL;
    }
    ...
    if ((cosq < 0) || (cosq >= 8)) {
	BCM_ERR(("ERROR: %s, cosq out of valid range [0-7], Unit(%d)\n",
	...
	if (config.xoff_threshold > 0x3FFFF) {
	    BCM_ERR(("ERROR: %s, xoff_threshold 0x%x can not be larger than 0x3FFFF, Unit(%d)\n",
	...
	BCM_ERR(("ERROR: %s, drop_threshold 0x%x can not be larger than 0x3FFFF, Unit(%d)\n",
		 FUNCTION_NAME(), config.drop_threshold, unit));
	return BCM_E_PARAM;
    }

    if (config.xon_threshold >= config.xoff_threshold) {
	BCM_ERR(("ERROR: %s, xon_threshold 0x%x can not equal or larger"
	...
	if (config.drop_threshold < config.xoff_threshold) {
	    BCM_ERR(("ERROR: %s, drop_threshold 0x%x must be larger"
		     "than xoff_threshold 0x%x, Unit(%d)\n", FUNCTION_NAME(),
	...
	if (rev_id == BCM88230_B0_REV_ID) {
		...
		BCM_ERR(("ERROR: %s, For current chip revision, xoff_threshold(0x%x) - xon_threshold(0x%x) "
	...
	if (!soc_feature(unit, soc_feature_source_port_priority_flow_control)) {
	    ...
	    BCM_ERR(("ERROR: %s, (Source port, PG) PFC not supported on Unit(%d)\n",
		     FUNCTION_NAME(), unit));
	    return BCM_E_UNAVAIL;
	}
...
{
    int rv = BCM_E_UNAVAIL;
    int port, module, out_of_band = FALSE;
	...
	BCM_ERR(("ERROR: %s, channel base of channel id 0x%X is out of range [0-0x1F], unit %d\n",
		 FUNCTION_NAME(), channel_id, unit));
	return (BCM_E_PARAM);
    }
    
    if (config.port_bits == 0) {
	BCM_ERR(("ERROR: %s, invalid %d bits per port config, unit %d\n",
		 FUNCTION_NAME(), config.port_bits, unit));
	return (BCM_E_PARAM);
    }

    if ((channel_offset + config.port_bits - 1) > SIRIUS_HCFC_MAX_CHANNEL_OFFSET) {
	BCM_ERR(("ERROR: %s, channel offset of channel id 0x%X is out of range [0-95] with %d"
	...
	if (flags & BCM_PORT_CONGESTION_MAPPING_CLEAR) {
	    BCM_ERR(("ERROR: %s, no resoure found for specified channel base 0x%x\n",
	...
	if (flags & BCM_PORT_CONGESTION_MAPPING_CLEAR) {
		...
		if (rv != BCM_E_NONE) {
		    BCM_ERR(("ERROR: %s, failed to free congestion resources, unit: %d, module: 0x%x rc: 0x%x\n",
			     FUNCTION_NAME(), unit, module, rv));		    
		    return rv;
		}
		...
		if (rv != BCM_E_NONE) {
		    BCM_ERR(("ERROR: %s, failed to free congestion resources, unit: %d, module: 0x%x\n",
			     FUNCTION_NAME(), unit, module));
		    return(rv);
		}
    ...
    if (rv != BCM_E_NONE) {
	BCM_ERR(("ERROR: %s, unexpected fabric port: 0x%x\n", FUNCTION_NAME(), gport));
	return rv;
    }
	...
	if (IS_CPU_PORT(unit, port)) {
	    COSQ_VERB(("fabric port: 0x%x corresponds to CPU port\n", gport));
    ...
    COSQ_VERB(("fabric port: 0x%x FifoMap: %d, nbrFifos: %d\n", gport, (*fifo_map), nbr_fifos));
    ...
    for (i = 0; i < nbr_fifos; i++) {
	if ((config.port_bits == 8) && (nbr_fifos >= 8) && (i==4)) {
				    ...
				    bcm_gport_t gport, uint32 flags, int *channel_id)
{
    ...
    if (rv != BCM_E_NONE) {
	BCM_ERR(("ERROR: %s, failed to get fc map table, unit: %d, fifo Base: 0x%x\n",
    ...
    if ((fc_state_index >= 0x132) ||
	((fc_state[unit]+fc_state_index)->in_use != TRUE)) {
	BCM_ERR(("ERROR: %s, fabric port 0x%x not mapped to valid channel_base/offset unit: %d\n",
		 FUNCTION_NAME(), gport, unit));
	if (fifo_map != NULL) {
	    sal_free(fifo_map);
	}
    ...
    if (is_enable == FALSE) {
	BCM_ERR(("ERROR: %s, fabric port 0x%x not mapped to valid channel_base/offset in HCFC message unit: %d\n",
		 FUNCTION_NAME(), gport, unit));
	if (fifo_map != NULL) {
	    sal_free(fifo_map);
	}
...
bcm_sirius_cosq_set_template_pfc(int unit, int32 template, int pfc_en, int pfc_pg)
{
	...
	if (soc_feature(unit, soc_feature_source_port_priority_flow_control)) {
		...
		BCM_ERR(("ERROR: %s, priority group %d out of range [0-7] supported by unit: %d\n",
