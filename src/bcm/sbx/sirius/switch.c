/* $Id: switch.c 1.15.2.3 Broadcom SDK $
...
int
_bcm_switch_stat_threshold_get(int unit, int32_t *threshold)
{
    ...
    if (soc_reg_field_get(unit, CS_ACE_CTRLr, regval, THRESHOLD_EJECT_ENf) == 0) {
      ...
      *threshold = 0;
      ...
      if(soc_sbx_div64(((uint64_t) (regval + 3) * 100ull), 0x7ffffff, &result) == -1) {
	return (BCM_E_FAIL);
      }
      *threshold = result;
      return(BCM_E_NONE);
			      ...
			      int *val)
{
	...
	break;
    default:
        soc_cm_debug(DK_ERR, "Unsupported switch control type (%d) \n", type);
        rv = BCM_E_UNAVAIL;
        break;
    }
...
int
_bcm_switch_stat_threshold_set(int unit, int32_t threshold)
{
    uint32 regval = 0;
	...
	return(BCM_E_PARAM);
      ...
      temp = (0x7ffffffull * (uint64_t) threshold);
...
_bcm_switch_stat_interval_set(int unit, int interval)
{
    ...
    if (((uint32)interval) >= 0xffffffff) {
        /* Passed in interval is too large. Set the background eject rate to
           max 32bit value */
        data = 0xffffffff;
        soc_cm_debug(DK_WARN, "Specified interval is too large. Interval "
    ...
    if (rv != SOC_E_NONE) {
        soc_cm_debug(DK_ERR, "Failed to set background eject rate. switch "
...
_bcm_sirius_switch_egress_pkt_age_set(int unit, int egress_pkt_age)
{
    ...
    BCM_DEBUG(BCM_DBG_VERBOSE, ("Egress Packet Age: 0x%x(%d)\n", egress_pkt_age, egress_pkt_age));

    /* Is aging disabled */
    if (egress_pkt_age == 0) {
        BCM_DEBUG(BCM_DBG_VERBOSE, ("Disable Egress Packet Age\n"));
    ...
    if (egress_pkt_age > ((500 * 0x3FFF * 15) / 1000)) {
        BCM_DEBUG(BCM_DBG_VERBOSE, ("Egress Packet Age, h/w granulatity in msec\n"));
        is_gran_ms = 1;
    }
    ...
    BCM_DEBUG(BCM_DBG_VERBOSE, ("Egress Packet Age, h/w age: 0x%x(%d), start_limit: 0x%x(%d)\n", hw_age, hw_age, start_limit, start_limit));

    min_rounding_accuracy = 0;
    min_limit = start_limit;
    for (i = start_limit; i < 14; i++) {
        gran = hw_age >> i;
        BCM_DEBUG(BCM_DBG_VERBOSE, ("Egress Packet Age, gran: 0x%x(%d)\n", gran, gran));
        if ((hw_age - (gran << i)) == 0) {
            min_limit = i;
            min_rounding_accuracy = 0;
            BCM_DEBUG(BCM_DBG_VERBOSE, ("Egress Packet Age, sel_min_limit: %d round_inaccuracy: %d\n", min_limit, min_rounding_accuracy));
            break;
        }
        rounding_accuracy = ((gran + 1) << i) - hw_age;

        BCM_DEBUG(BCM_DBG_VERBOSE, ("Egress Packet Age, rounded_gran: 0x%x(%d) rounded_age: 0x%x(%d) hw_age: 0x%x(%d), min_limit: %d round_inaccuracy: %d\n", (gran + 1), (gran + 1), ((gran + 1) << i), ((gran + 1) << i), hw_age, hw_age, i, rounding_accuracy));

        if (i == start_limit) {
            min_rounding_accuracy = rounding_accuracy;
            min_limit = i;
            BCM_DEBUG(BCM_DBG_VERBOSE, ("Egress Packet Age, cur_min_limit: %d round_inaccuracy: %d\n", min_limit, min_rounding_accuracy));
        }
    ...
    BCM_DEBUG(BCM_DBG_VERBOSE, ("Egress Packet Age, gran: 0x%x(%d) min_limit: %d\n", gran, gran, min_limit));
			      ...
			      int val)
{
    ...
    case bcmSwitchPFCQueue7Class:
	if ( (val < 0) || (val > 7) ) {
	    soc_cm_debug(DK_ERR, "Unsupported PFC priority (%d) \n", val);
	    return BCM_E_PARAM;
	}
