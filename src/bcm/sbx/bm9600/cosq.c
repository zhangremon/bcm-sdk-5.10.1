/*
 * $Id: cosq.c 1.74.6.1 Broadcom SDK $
...
bcm_bm9600_cosq_init(int unit)
{
...
#ifdef BCM_EASY_RELOAD_SUPPORT
    uint32_t num_queues_in_bag, base_queue_of_bag, bag_rate_bytes_per_epoch;
    ...
    if (SOC_IS_RELOADING(unit)) {
	    ...
	    for (bw_group = 0; bw_group < SOC_SBX_CFG(unit)->num_bw_groups; bw_group++) {
	        ...
	        if (rv != SOC_E_NONE) {
		    BCM_ERR(("ERROR: BM9600 read of bag table failed for bw_group(%d)\n", bw_group));
		    return BCM_E_FAIL;
	        }
		    ...
		    BCM_DEBUG(BCM_DBG_COSQ, ("EASY_RELOAD bw_group(%d) base_queue_of_bag(%d)\n", bw_group, base_queue_of_bag));
	        }
	    }
        }
#endif
	...
	for (bw_group = 0; bw_group < SOC_SBX_CFG(unit)->num_bw_groups; bw_group++) {
		...
		if (rv != SOC_E_NONE) {
		    BCM_ERR(("ERROR: BM9600 write of PRT table failed for bw_group(%d)\n", bw_group));
		    return BCM_E_FAIL;
		}
	    }
	}
	...
	for (sysport=0; sysport<SOC_SBX_CFG(unit)->num_sysports; sysport++) {
	    ...
	    if (rv != SOC_E_NONE) {
		BCM_ERR(("ERROR: %s, Failed to write sysport map table, Unit(%d)\n",
			 FUNCTION_NAME(), unit));
...

{
	...
	if (_bcm_bm9600_cosq_get_token_state(unit)) {
	    BCM_ERR(("ERROR: EASY_RELOAD, cold boot required because internal state is unknown - crashed during sysport/portset setup\n"));
	    return BCM_E_INTERNAL;
	}
#endif
    }
	...
	BCM_ERR(("ERROR: Sysport required for the unicast queues"));
	return BCM_E_INTERNAL;
    }
	...
	if (SOC_IS_RELOADING(unit)) {
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_ERR(("ERROR: EASY_RELOAD sysport mapping table read failed for sysport(%d) coldboot required\n", sysport));
		return BCM_E_INTERNAL;
	    }

	    if (portset == 0xff) {
		BCM_ERR(("ERROR: EASY_RELOAD portset entry invalid for sysport(%d), coldboot required\n", sysport));
		return BCM_E_INTERNAL;
	    }

	    BCM_DEBUG(BCM_DBG_COSQ, ("EASY_RELOAD: portset(%d) for sysport(%d)\n", portset, sysport));

	    rv = soc_bm9600_portset_info_table_read(unit, portset, &virtual, &eopp, &start_port, &eg_node);

	    BCM_DEBUG(BCM_DBG_COSQ, ("EASY_RELOAD: portset(%d) virtual(%d) eopp(%d) start_port(%d) eg_node(%d)\n",
				     portset, virtual, eopp, start_port, eg_node));

	    if (eg_node != dest_node) {
		BCM_ERR(("ERROR: EASY_RELOAD egress node(%d) from portset info doesn't match node(%d) in GPORT_ADD\n",
			 eg_node, dest_node));
		return BCM_E_INTERNAL;
	    }
	    ...
	    /* Physical port */
	    if (!virtual) {
		if (dest_port != (start_port + offset)) {
		    BCM_ERR(("ERROR: EASY_RELOAD egress port(%d) from portset info doesn't match port(%d) in GPORT_ADD\n",
			     start_port + offset, dest_port));
		    return BCM_E_INTERNAL;
		...
		if ((offset >= 0) && (offset < 4)) {
			...
			BCM_ERR(("ERROR: EASY_RELOAD egress virtual port(%d) from portset info doesn't match port(%d) in GPORT_ADD\n",
		...
		else if ((offset >= 8) && (offset < 12) && (eopp&3)) {
		    if (dest_port != start_port) {
			BCM_ERR(("ERROR: EASY_RELOAD egress virtual port(%d) from portset info doesn't match port(%d) in GPORT_ADD\n",
				 start_port, dest_port));
			return BCM_E_INTERNAL;
		...
		else if ((offset >= 12) && (offset < 16) && (eopp&7)) {
		    if (dest_port != start_port) {
			BCM_ERR(("ERROR: EASY_RELOAD egress virtual port(%d) from portset info doesn't match port(%d) in GPORT_ADD\n",
				 start_port, dest_port));
			return BCM_E_INTERNAL;
		...
		/* Not a valid configuration...eopp setting invalid with offset */
		    ...
		    BCM_ERR(("ERROR: EASY_RELOAD virtual portset info invalid start_port(%d) eopp(%d)\n",
			     start_port, eopp));
		    return BCM_E_INTERNAL;
		}
	    }
		...
		BCM_ERR(("ERROR: EASY_RELOAD sysport array table entry(%d) doesn't match sysport(%d)\n",
		...
		if ( (SOC_SBX_STATE(unit)->sysport_state[sysport].node != dest_node) ||
		     (SOC_SBX_STATE(unit)->sysport_state[sysport].port != dest_port) ) {
		    BCM_ERR(("ERROR: Sysport (%d) already mapped to Node/Port(%d/%d)",
...
bcm_bm9600_cosq_set_ingress_params(int unit,
    ...
    if (rv) {
	BCM_ERR(("ERROR: set ingress params BM9600 configuration bw_group(%d) error(%d)\n", bw_group, rv));
	return rv;
    }
    return rv;
}
				  ...
				  int gain)
{
    ...
    BCM_DEBUG(BCM_DBG_COSQ, ("Update WRED template, queue(%d) template(%d) gain(%d) on BM9600(%d)\n",
			     queue, template, gain, unit));

    /* this is a 32 bit entry but the field is 16 bits */
    addr = queue >> 1;

    status = soc_bm9600_BwR0WdtRead(unit, addr, (uint32_t*)&read_data);

    BCM_DEBUG(BCM_DBG_COSQ, ("WDT entry read(0x%x)\n", read_data));

    if (status == SOC_E_NONE) {

	sbZfFabBm9600BwR0WdtEntry_Unpack(&zfWdtEntry, (uint8_t*)&read_data, 4);
	...
	sbZfFabBm9600BwR0WdtEntry_Pack(&zfWdtEntry, (uint8_t*)&write_value, 4);
	BCM_DEBUG(BCM_DBG_COSQ, ("WDT entry value to write(0x%x)\n", write_value));

	status = soc_bm9600_BwR0WdtWrite(unit, addr, write_value);
    }
    if (status != SOC_E_NONE) {
	BCM_ERR(("ERROR: %s, soc_bm9600_BwR0WdtRead/Write, Unit(%d)\n", FUNCTION_NAME(), unit));
	rv = BCM_E_INTERNAL;
    }
				  ...
				  int *gain)
{
    ...
    status = soc_bm9600_BwR0WdtRead(unit, addr, (uint32_t*)&read_data);
    if (status) {
	BCM_ERR(("ERROR: %s, soc_bm9600_BwR0WdtRead, Unit(%d)\n", FUNCTION_NAME(), unit));
    }
    else {
	...
	BCM_DEBUG(BCM_DBG_COSQ, ("Read WRED template, queue(%d) template(%d) gain(%d) on BM9600(%d)\n",
				 queue, *template, *gain, unit));
    ...
    int *attach_id)
{
	...
	if (rv != BCM_E_NONE) {
            BCM_ERR(("ERROR: bcm_cosq_sched_set() must be called, possible error,  Unit(%d)\n", unit));	    
	    return (BCM_E_PARAM);
	}
...

	{
	    ...
	    rv = bcm_bm9600_map_sysport_to_nodeport(unit, sysport, p_bwstate->dest_node, fcd);
		...
		if (soc_feature(unit, soc_feature_egr_independent_fc)) {
		    rv = bcm_bm9600_map_sysport_to_nodeport(unit,
							    ...
							    p_bwstate->dest_node,
							    BCM_INT_SBX_PORT_TO_NEF_PORT(fcd));
		}
	    }
	} /* not reloading */
                                  ...
                                  bcm_cosq_gport_discard_t *discard)
{
	...
	case BCM_SBX_COSQ_DISCARD_COLOR_RED_DP2:
        ...
        default:
            BCM_ERR(("ERROR: %s, color (%d) out of range [0-2],  Unit(%d)\n", FUNCTION_NAME(), color, unit));
            rv = BCM_E_INTERNAL;
            goto err;
            break;
    }
	...
	addr = template * 8;
        ...
        if (status) {
            BCM_ERR(("ERROR: %s, soc_bm9600_BwR1Wct0AWrite, Unit(%d), Color(%d)\n", FUNCTION_NAME(), unit, color));
            rv = BCM_E_INTERNAL;
            goto err;
        }
        status = soc_bm9600_BwR1Wct0AWrite(unit, (addr + 1), write_value[0]);
        if (status) {
            BCM_ERR(("ERROR: %s, soc_bm9600_BwR1Wct0BWrite, Unit(%d), Color(%d)\n", FUNCTION_NAME(), unit, color));
            rv = BCM_E_INTERNAL;
            goto err;
        }
#else /* !(0) */
        ...
        switch (color) {
            ...
            case BCM_SBX_COSQ_DISCARD_COLOR_YELLOW_DP1:
                ...
                if (status) {
                    BCM_ERR(("ERROR: %s, soc_bm9600_BwR1Wct1AWrite, Unit(%d), Color(%d)\n", FUNCTION_NAME(), unit, color));
                    rv = BCM_E_INTERNAL;
                    goto err;
                }
                ...
                if (status) {
                    BCM_ERR(("ERROR: %s, soc_bm9600_BwR1Wct1BWrite, Unit(%d), Color(%d)\n", FUNCTION_NAME(), unit, color));
                    rv = BCM_E_INTERNAL;
                    goto err;
                }
            ...
            default:
                ...
                if (status) {
                    BCM_ERR(("ERROR: %s, soc_bm9600_BwR1Wct2AWrite, Unit(%d), Color(%d)\n", FUNCTION_NAME(), unit, color));
                    rv = BCM_E_INTERNAL;
                    goto err;
                }
                ...
                if (status) {
                    BCM_ERR(("ERROR: %s, soc_bm9600_BwR1Wct2BWrite, Unit(%d), Color(%d)\n", FUNCTION_NAME(), unit, color));
                    rv = BCM_E_INTERNAL;
                    goto err;
                }
        ...
        if (rv != BCM_E_NONE) {
            BCM_ERR(("ERROR: %s, _bcm_bm9600_get_nbr_node_enabled, Unit(%d)\n", FUNCTION_NAME(), unit));
            goto err;
        }
        ...
        sbZfFabBm9600BwR1Wct0AEntry_Pack(&chip_params0, (uint8_t *)&write_value[0],
        ...
        sbZfFabBm9600BwR1Wct0BEntry_Pack(&chip_params1, (uint8_t *)&write_value[1],
					 (SB_ZF_FAB_WRED_PARAMETERS_SIZE / 2));
        ...
        status = soc_bm9600_BwR1Wct0AWrite(unit, addr, write_value[0]);
        if (status) {
            BCM_ERR(("ERROR: %s, soc_bm9600_BwR1Wct0AWrite,  Unit(%d)\n", FUNCTION_NAME(), unit));
            rv = BCM_E_INTERNAL;
            goto err;
        }
        ...
        if (status) {
            BCM_ERR(("ERROR: %s, soc_bm9600_BwR1Wct0BWrite,  Unit(%d)\n", FUNCTION_NAME(), unit));
            rv = BCM_E_INTERNAL;
            goto err;
        }
#else /* !(0) */
...

{
    ...
    BCM_DEBUG(BCM_DBG_COSQ | BCM_DBG_VERBOSE, ("update bw_group(%d)\n",bw_group));
    ...
    uu_epoch_length_in_ns = ((SOC_SBX_CFG(unit)->epoch_length_in_timeslots) * (SOC_SBX_STATE(unit)->fabric_state->timeslot_size)); /* timeslot size ns */;
    ...
    BCM_DEBUG(BCM_DBG_COSQ | BCM_DBG_VERBOSE, ("bag_rate_kbps=(%d) bag_rate_bytes_per_epoch(%d)\n",
	...
	if (add) {
		...
		sigma = (int32_t) bag_rate_bytes_per_epoch;
	    }
	    ...
	    BCM_DEBUG(BCM_DBG_COSQ | BCM_DBG_VERBOSE, ("queue(%d) guarantee in kbps(%d) sigma(%d) gamma(%d)\n", queue, guarantee_in_kbps, sigma, gamma));
	} /* end if (add) */
	...
	BCM_DEBUG(BCM_DBG_COSQ | BCM_DBG_VERBOSE, ("num_queues_in_bag(%d)\n", num_queues_in_bag));
    ...
    if (rv != SOC_E_NONE) {
	BCM_ERR(("ERROR: BM9600 Write to BAG tables failed for bw_group(%d)\n", bw_group));
	return BCM_E_FAIL;
    }
    BCM_DEBUG(BCM_DBG_COSQ | BCM_DBG_VERBOSE, ("Bag update successful for bw_group(%d)\n",bw_group));
...
_bcm_bm9600_portset_state_init(int unit)
{
	...
	while (ps != 0xff) {
	    ...
	    if (rv != SOC_E_NONE) {
		BCM_ERR(("ERROR: Failed to read portset link table for row %d", ps));
		return BCM_E_INTERNAL;
	    }
	    ps_state[unit][ps].next = next_ps;
	    ps_state[unit][ps].prev = prev_ps;

	    BCM_DEBUG(BCM_DBG_COSQ, ("Portset(%d) next(%d) prev(%d)\n", ps, next_ps, prev_ps));
			  ...
			  int *portset)
{
    ...
    if (bFound == FALSE) {
	BCM_ERR(("There are no free portset.  Resource error\n"));
	rv = BCM_E_RESOURCE;
    } else {
	ps = *portset;
	p_psstate = &ps_state[unit][ps];

	BCM_DEBUG(BCM_DBG_COSQ, ("Portset %d allocated\n", ps));

	rv = _bcm_bm9600_init_portset(unit, ps, p_psstate);
	if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: Failed to init portset %d", ps));
	    return BCM_E_INTERNAL;
	}
    }
			 ...
			 bcm_sbx_cosq_portset_state_t *p_psstate)
{
    ...
    rv = soc_bm9600_portset_info_table_write(unit, ps,
					     ...
					     p_psstate->eg_node);
    if (rv != SOC_E_NONE) {
	BCM_ERR(("ERROR: Failed to update portset info table"));
	return BCM_E_INTERNAL;
    }
    ...
    rv = soc_bm9600_nm_sysport_array_table_write(unit,
						 ...
						 1      /* new row */);
    if (rv != SOC_E_NONE) {
	BCM_ERR(("ERROR: Failed to update sysport array table"));
	return BCM_E_INTERNAL;
    }
    ...
    if (rv != SOC_E_NONE) {
	BCM_ERR(("ERROR: Failed to init port priority table"));
	return BCM_E_INTERNAL;
    }

    BCM_DEBUG(BCM_DBG_COSQ, ("Portset %d inited\n", ps));
			   ...
			   int portset)
{
	 ...
	 */
	BCM_DEBUG(BCM_DBG_COSQ, ("Portset %d replace old head %d\n", portset, ps_head[unit]));
	...
	if (rv != SOC_E_NONE) {
	    BCM_ERR(("ERROR: Failed to update portset link table for row %d", portset));
	    return BCM_E_INTERNAL;
	}
	...
	BCM_DEBUG(BCM_DBG_COSQ, ("Portset %d insert between %d/%d\n", portset,
	...
	if (rv != SOC_E_NONE) {
	    BCM_ERR(("ERROR: Failed to update portset link table for row %d", prev_ps));
	    return BCM_E_INTERNAL;
	}
			   ...
			   int portset)
{
	...
	BCM_DEBUG(BCM_DBG_COSQ, ("Portset %d deleted, new ps_head %d\n", portset,
				 next_ps));
	...
	BCM_DEBUG(BCM_DBG_COSQ, ("Portset %d deleted between %d/%d\n", portset,
				 prev_ps, next_ps));
			 ...
			 int portset)
{
    ...
    if (ps_state[unit] == NULL) {
	BCM_ERR(("Portset not inited, wrong unit?.\n"));
	return BCM_E_UNIT;
    }

    if ( (portset < 0) || (portset >= BM9600_MAX_PORTSETS)) {
	BCM_ERR(("Portset out of range (0 - %d).\n", BM9600_MAX_PORTSETS));
	return BCM_E_PARAM;
    }
				       ...
				       int *bIn_use)
{
	...
	*port = (p_psstate->start_port + offset);
    ...
    if ( (*port == -1) || (*node == -1) ) {
	BCM_ERR(("Port/Node retrieved from portset/offset (%d / %d) is not valid.\n", portset, offset));
	rv = BCM_E_INTERNAL;
    }
				   ...
				   int port)
{
	...
	if (bAlloc_v) {
		...
		BCM_ERR(("ERROR: %s, Virtual portset should not be used when there is only 1 sysport\n",
			 FUNCTION_NAME()));
		return BCM_E_INTERNAL;
	    }
				  ...
				  int new_row)
{
    ...
    if (sysport == BCM_INT_SBX_INVALID_SYSPORT) {
	BCM_ERR(("ERROR: %s, Invalid sysport %d, Unit(%d)\n",
    ...
    if (rv != SOC_E_NONE) {
	BCM_ERR(("ERROR: %s, Failed to write sysport array table, Unit(%d)\n",
		 FUNCTION_NAME(), unit));
	return BCM_E_INTERNAL;
    }
			 ...
			 int new_offset)
{
	...
	BCM_ERR(("ERROR: %s, Invalid old sysport %d, Unit(%d)\n",
    ...
    rv = soc_bm9600_nm_sysport_array_table_write(unit, old_portset, old_offset,
						 0xFFF, 0);
    if (rv != SOC_E_NONE) {
	BCM_ERR(("ERROR: %s, Failed to move sysport(%d) from (%d/%d) to (%d/%d) in sysport array, Unit(%d)\n",
	...
	if (rv != SOC_E_NONE) {
	    BCM_ERR(("ERROR: %s, Failed to move sysport(%d) from (%d/%d) to (%d/%d), Unit(%d)\n",
		     FUNCTION_NAME(), old_sysport, old_portset, old_offset, new_portset,
		     new_offset, unit));
	    return BCM_E_INTERNAL;
	}
						   ...
						   int add)
{
    ...
    if (rebuild_ps_state == NULL) {
      BCM_DEBUG(BCM_DBG_COSQ, ("Failed to alloc rebuid_ps_state buffer"));
      return BCM_E_MEMORY;
    }
    ...
    if (nps_map == NULL) {
      BCM_DEBUG(BCM_DBG_COSQ, ("Failed to alloc nps_map buffer")); 
	...
	for (ps = ps_head[unit]; ps != 0xFF; ps = ps_state[unit][ps].next) {
		...
		if ( (os_in_use == TRUE) && (os_node == node) && (os_port == port) ) {
			...
			BCM_DEBUG(BCM_DBG_COSQ, ("Move sysport %d from physical portset %d to virtual portset\n",
	...
	for (ps = node_head, nps_count = 0;
		...
		if ((add == TRUE) && (added == FALSE) && (port < os_port)) {
			...
			nps_count ++;
			BCM_DEBUG(BCM_DBG_COSQ, ("To_virtual sysport %d added into nps map\n",
		    ...
		    nps_count ++;
		    added = TRUE;
		    BCM_DEBUG(BCM_DBG_COSQ, ("sysport %d added into nps map\n", sysport));
		}
	...
	for (nps = 0; nps < nps_count; nps++) {
	    BCM_DEBUG(BCM_DBG_COSQ, ("nps_map entry %d: node %d port %d sysport %d ps %d os %d\n",
	...
	for (nps = 0; nps < nps_count; nps++) {
		...
		BCM_DEBUG(BCM_DBG_COSQ, ("Add sysport %d to ps/os=%d/%d with eopp = 0x%x\n",
	...
	if (rebuilded == FALSE) {
		...
		if (to_physical == TRUE) {
		    ...
		    if (rv != BCM_E_NONE) {
			BCM_DEBUG(BCM_DBG_COSQ, ("Failed to move sysport %d from (%d/%d) into physical portset (%d/%d) during rebuild\n",
						 to_physical_sysport.sysport, to_physical_sysport.ps, to_physical_sysport.os, ps, os));
			sal_free(rebuild_ps_state);
			sal_free(nps_map);
			return rv;
		    } else {
			BCM_DEBUG(BCM_DBG_COSQ, ("Moved sysport %d from (%d/%d) into physical portset (%d/%d) during rebuild\n",
						 to_physical_sysport.sysport, to_physical_sysport.ps, to_physical_sysport.os, ps, os));
		    }
		...
		return BCM_E_NONE;
		    ...
		    BCM_DEBUG(BCM_DBG_COSQ, ("Failed to detect portset difference during rebuild\n"));
		    sal_free(rebuild_ps_state);
		    sal_free(nps_map);
		    return BCM_E_INTERNAL;
		}
	...
	if (rv != BCM_E_NONE) {
	    BCM_DEBUG(BCM_DBG_COSQ, ("Failed to alloc temp portsets during rebuild\n"));
	    sal_free(rebuild_ps_state);
	    sal_free(nps_map);
	    return rv;
	}
	...
	if (rv != BCM_E_NONE) {
	    BCM_DEBUG(BCM_DBG_COSQ, ("Failed to insert temp portsets during rebuild\n"));
	    sal_free(rebuild_ps_state);
	    sal_free(nps_map);
	    return rv;
	}
	...
	if ((rebuild_ps_count - rebuild_ps) > 1) {
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_DEBUG(BCM_DBG_COSQ, ("Failed to alloc 2nd temp portsets during rebuild\n"));
		sal_free(rebuild_ps_state);
		sal_free(nps_map);
		return rv;
	    }
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_DEBUG(BCM_DBG_COSQ, ("Failed to insert 2nd temp portsets during rebuild\n"));
		sal_free(rebuild_ps_state);
		sal_free(nps_map);
		return rv;
	    }
	...
	for (ps = tmp_ps1; rebuild_ps < rebuild_ps_count;
		...
		BCM_DEBUG(BCM_DBG_COSQ, ("Portset %d should be empty before rebuild\n", ps));
		sal_free(rebuild_ps_state);
		sal_free(nps_map);
		return BCM_E_INTERNAL;
	    }
		...
		if ( (rebuild_ps_state[rebuild_ps].offset_in_use & (1 << os)) != 0 ) {
			...
			if (rv != BCM_E_NONE) {
			    BCM_DEBUG(BCM_DBG_COSQ, ("Failed to add sysport %d into portset during rebuild\n", sysport));
			    sal_free(rebuild_ps_state);
			    sal_free(nps_map);
			    return rv;
			}
			BCM_DEBUG(BCM_DBG_COSQ, ("Adding sysport %d to (%d/%d)\n", sysport, ps, os));
		    } else {
			for (nps = 0; nps < nps_count; nps++) {
			...
			if (nps == nps_count) {
			    ...
			    BCM_DEBUG(BCM_DBG_COSQ, ("Failed to find sysport %d in the node/port/sysport map\n",
			...
			if (ps_state[unit][nps_map[nps].ps].sysports[nps_map[nps].os] != rebuild_ps_state[rebuild_ps].sysports[os]) {
			    BCM_DEBUG(BCM_DBG_COSQ, ("nps sysport %d not maching with ps sysport %d\n",
			...
			BCM_DEBUG(BCM_DBG_COSQ, ("Moving sysport %d from ps/os (%d/%d) to (%d/%d)\n",
	...
	if (add == TRUE) {
		...
		BCM_DEBUG(BCM_DBG_COSQ, ("Moving sysport %d from ps/os (%d/%d) to virtual ps/os(%d/%d)\n",
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_DEBUG(BCM_DBG_COSQ, ("Failed to add sysport %d into portset\n", sysport));
		sal_free(rebuild_ps_state);
		sal_free(nps_map);
		return rv;
	    }
