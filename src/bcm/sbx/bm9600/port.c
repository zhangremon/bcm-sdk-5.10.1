/*
 * $Id: port.c 1.86.2.5 Broadcom SDK $
...
{    
	...
	case BCM_PORT_ABILITY_SCI:
	    if (port >= SBX_MAXIMUM_NODES) {
		SOC_ERROR_PRINT((DK_ERR,
				 "ability_matching_speed_set: (%d) SCI port out of range(%d)\n",
				 unit, port));
		return BCM_E_PARAM;
	    }
	    ...
	    if ((node < 0) || (node >= BCM_STK_MAX_MODULES)) {
		SOC_ERROR_PRINT((DK_ERR,
...
#if 000
    soc_cm_print("port(%d) speed(%d) encoding(%s)\n", port, speed, encoding?"8b10b":"64b66b");
#endif
...
bcm_bm9600_port_init(int unit)
{
	...
	for (p = 0; p <  SB_FAB_DEVICE_MAX_FABRIC_PORTS; p++) {
...
static int
_bcm_bm9600_xb_iport_enable_set(int unit, int32_t nLink, int nAbility)
{
    uint32_t uIprtEnableIndex;
    uint32_t uIprtEnable;
    int rv = BCM_E_NONE;
	...
	case 5:
	    ...
	    break;
	default:
	    BCM_ERR(("Invalid xb_iprt_enable index(%d) must be less than 6\n", uIprtEnableIndex));
	    return BCM_E_PARAM;
    }
...
#if 000
	    soc_cm_print("even channel only nLink(%d) uIprtEnable(0x%08x)\n", nLink, uIprtEnable);
#endif
	    break;
	case BCM_PORT_ABILITY_DUAL_SFI:
	    ...
	    uIprtEnable |= 3 << ((nLink % BM9600_XB_IPORT_ENABLE_INDEX_DIVISOR) * 2);
#if 000
	    soc_cm_print("even and odd channels nLink(%d) uIprtEnable(0x%08x)\n", nLink, uIprtEnable);
#endif
	    break;
	case BCM_PORT_ABILITY_SFI_SCI:
	    /* Enable only the odd */
	    ...
	    break;
	default:
	    BCM_ERR(("Invalid port ability(%d)\n", nAbility));
	    return rv;
    }
                            ...
                            bcm_port_control_t type, int value)
{
    ...
    switch (type) {
		...
		*/
	    ...
	    if (enable) {
		status = _bcm_bm9600_xb_iport_enable_set(unit, nLink, value);
		if (status) {
		    return status;
		}
	    }
...
bcm_bm9600_port_enable_set(int unit, bcm_port_t port, int enable)
{
    ...
    rv = _bcm_bm9600_xb_iport_enable_set(unit, port, nAbility);
    if (rv) {
      return rv;
    }
...
#if 000
	soc_cm_print("xb_config out of reset for port(%d)\n", port);
#endif
	thin_delay(250000);
