                                          ...
                                          bcm_port_t dst_xbport)
{
	...
	/* TODO: remove the sequencing requirement
	 *   (1) use xbar <-> link map to get the link for xcfg update
	 *   (2) update the xcfg mem
	 *   (3) update the (xbar,node) <-> xcfg mapping
	 */
	...
	for (uPlane = 0; uPlane < HW_QE2000_NUM_SCI_LINKS; uPlane++) {
	    ...
	    if (rv != BCM_E_NONE) {
		soc_cm_debug(DK_ERR, "bcm_qe2000_fabric_crossbar_connection_set failed on plane %d xbar %d\n",
			     uPlane, xbar);
		return rv;
	    }
		...
		if (rv != BCM_E_NONE) {
		    soc_cm_debug(DK_ERR, "soc_qe2000_sfi_rd_wr_xcfg failed on plane %d xbar %d\n",
				 uPlane, xbar);
		    return rv;
		}
		remapped = TRUE;
	...
	if (remapped == FALSE) {
	    soc_cm_debug(DK_ERR, "logical xbar %d has no physical link mapped on either plane, setup remap first\n",
			 xbar);
	    return BCM_E_PARAM;
	} else {
	    return BCM_E_NONE;
                                       ...
                                       bcm_port_t port)
{
    ...
    /* TODO: remove the sequencing requirement
     *  (1) put the old link used for the xbar to loopback in xcfg mem
     *  (2) config the xfg for the new link, update all nodes for the xbar
     *  (3) update xbar <-> link mapping
     */
                                      ...
                                      uint64 xbars)
{
    ...
    uint64_t uuNodeBytesPerEpoch = 0x0ULL;
    uint64_t uuEpochLengthInNs;
    uint64_t uuLengthSumAdjustInBytes = 0x0ULL;
    ...
    int32  nSfiCount;
    int32  nHalfBus;
    uint64_t uuNodeMaxRateInBitsPerSec = 20000000ULL; /* 20 Gbps */
...
#define BCM_QE2000_FABRIC_BITS_PER_BYTE 8
    ...
    uuEpochLengthInNs = p_fabric_state->timeslot_size * SOC_SBX_CFG(unit)->epoch_length_in_timeslots;

    
    if(soc_sbx_div64((uuEpochLengthInNs * uuNodeMaxRateInBitsPerSec), (BCM_QE2000_FABRIC_NANOSEC_PER_SEC * BCM_QE2000_FABRIC_BITS_PER_BYTE), 
		     &nodeBytesPerEpoch) == -1) {
      return BCM_E_INTERNAL;
    }

    uuNodeBytesPerEpoch = nodeBytesPerEpoch;

    BCM_DEBUG(BCM_DBG_COSQ, ("timeslot_size(%d) epoch_length_in_timeslots(%d) epoch_length_in_ns(%lld)\n",
			     p_fabric_state->timeslot_size, SOC_SBX_CFG(unit)->epoch_length_in_timeslots, uuEpochLengthInNs));

    BCM_DEBUG(BCM_DBG_COSQ, ("uuNodeBytesPerEpoch(%lld)\n", uuNodeBytesPerEpoch));
    ...
    if (soc_sbx_div64((uuNodeMaxRateInBitsPerSec * 3 * SB_FAB_DEVICE_QE2000_NUM_QUEUES), 
		      (8 * BCM_QE2000_FABRIC_MICROSEC_PER_SEC), &lengthSumAdjustInBytes) == -1) {
	return BCM_E_INTERNAL;
    }
    uuLengthSumAdjustInBytes = lengthSumAdjustInBytes;
    
    SAND_HAL_WRITE(unit, KA, QM_DC_CONFIG0,
		   SAND_HAL_SET_FIELD(KA, QM_DC_CONFIG0, MAX_ARRIVAL_BYTES, uuNodeBytesPerEpoch));

    SAND_HAL_WRITE(unit, KA, QM_DC_CONFIG1,
		   SAND_HAL_SET_FIELD(KA, QM_DC_CONFIG1, LENGTH_SUM_ADJUST, uuLengthSumAdjustInBytes));
    ...
    if (nRateConversion == -1) {
      soc_cm_debug(DK_ERR, "nRateConversion failed\n");
      return BCM_E_INTERNAL;
    }
                              ...
                              int arg)
{
	...
	case bcmFabricMode:
	    ...
	    SOC_SBX_CFG(unit)->reset_ul = 1;
            ...
            /* TODO: cleanup of memory allocated/data structures */
