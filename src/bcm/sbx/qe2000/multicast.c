/*
 * $Id: multicast.c 1.21.42.2 Broadcom SDK $
                            ...
                            bcm_multicast_t *group)
{
    ...
    /* consistency checks */
    if (group == NULL) {
        BCM_ERR(("ERROR: %s, NULL group parameter, Unit(%d)\n", FUNCTION_NAME(), unit));
        return(BCM_E_PARAM);
    }
    ...
    if ( ((flags & ~(BCM_MULTICAST_TYPE_MASK | BCM_MULTICAST_WITH_ID | BCM_MULTICAST_DISABLE_SRC_KNOCKOUT)) != 0) ||
        ...
        BCM_ERR(("ERROR: %s, Invalid flag parameter, Unit(%d), flags(0x%x)\n",
	...
	if ((flags & BCM_MULTICAST_WITH_ID)==0) {
	    BCM_ERR(("ERROR: %s, Easy Reload - when reloading, all mcgroups must be added with flag BCM_MULTICAST_WITH_ID\n"));
	    return (BCM_E_PARAM);
	}
#endif
    }
    ...
    if (flags & BCM_MULTICAST_WITH_ID) {
        if (!soc_qe2000_mvt_entry_id_valid(unit, (*group))) {
            BCM_ERR(("ERROR: %s, Invalid group parameter, Unit(%d), group(0x%x)\n",
        ...
        if (rv != BCM_E_NONE) {
            BCM_ERR(("ERROR: %s, soc_qe2000_mvt_entry_reserve_id, Unit(%d), Err: 0x%x\n",
        ...
        if (rv != BCM_E_NONE) {
            BCM_ERR(("ERROR: %s, soc_qe2000_mvt_entry_allocate, Unit(%d), Err: 0x%x\n",
                      FUNCTION_NAME(), unit, rv));
            return(rv);
        }
        (*group) = entry_id;
    ...
    if (rv != BCM_E_NONE) {
        BCM_ERR(("ERROR: %s, soc_qe2000_mvt_entry_get_frm_id, Unit(%d), Err: 0x%x\n",
                      FUNCTION_NAME(), unit, rv));
        goto err;
    }
    ...
    if (rv != BCM_E_NONE) {
        BCM_ERR(("ERROR: %s, soc_qe2000_mvt_entry_set_frm_id, Unit(%d), Err: 0x%x\n",
                      FUNCTION_NAME(), unit, rv));
        goto err;
    }
                               ...
                               uint32 *flags)
{
    ...
    /* consistency checks */
    if (!flags) {
        BCM_ERR(("%s obligatory out argument is NULL\n",
                 FUNCTION_NAME()));
        return BCM_E_PARAM;
    }
    if (!soc_qe2000_mvt_entry_id_valid(unit, group)) {
                                    ...
                                    void *user_data)
{
    ...
    /* consistency checks */
    if (!flags) {
        BCM_ERR(("%s callback function may not be NULL\n",
                 FUNCTION_NAME()));
        return BCM_E_PARAM;
    }
                             ...
                             bcm_multicast_t group)
{
    ...
    if (i >= SBX_MAX_MVT_CHAINED) {
        BCM_ERR(("ERROR: %s, Inconsistency in MVT Chain, Unit(%d), Chain  length greater then: 0x%x\n",
                                ...
                                bcm_if_t encap_id)
{
    ...
    if (!BCM_GPORT_IS_MODPORT(port)) {
        BCM_ERR(("ERROR: %s, Invalid gport type, Unit(%d), gport(0x%x)\n", FUNCTION_NAME(), unit, port));
        return(BCM_E_PARAM);
    }
    ...
    if  ( (num_port >= SOC_PORT_MIN(unit, spi_subport)) &&
        ...
        BCM_ERR(("ERROR: %s, Invalid port, Unit(%d), port(0x%x)\n", FUNCTION_NAME(), unit, num_port));
        return(BCM_E_PARAM);
    }
   ...
   /* Search chain for matching encap_id or for free entry */
	...
	if (SOC_IS_RELOADING(unit)) {
		...
		if (rv != BCM_E_NONE) {
		    BCM_ERR(("ERROR: %s, reserving entry for soc_qe2000_mvt_entry_reserve_id, Unit(%d), Err: 0x%x\n",
			     FUNCTION_NAME(), unit, rv));
		    return(rv);
		}
	    }
...
#ifdef BCM_EASY_RELOAD_SUPPORT
	    if (found != TRUE) {
		BCM_ERR(("ERROR: %s, Inconsistency in MVT Chain, Unit(%d), EASY_RELOAD, reloading but chain MVT entry not found: 0x%x\n",
			 FUNCTION_NAME(), unit, SBX_MAX_MVT_CHAINED));
		rv = BCM_E_INTERNAL;
		goto err;
	    }
                                   ...
                                   bcm_if_t encap_id)
{
    ...
    /* check for error condition */
    if (found != TRUE) {
        BCM_ERR(("ERROR: %s, MVT entry not found, Unit(%d)\n", FUNCTION_NAME(), unit));
        rv = BCM_E_NOT_FOUND;
        goto err;
    }
    ...
    if (SOC_PBMP_IS_NULL(mvtEntry.ports)) {
        ...
        if (entry_index == group && mvtEntry.next != SBX_MVT_ID_NULL) { 
            ...
            rv = soc_qe2000_mvt_entry_get_frm_id(unit, mvtEntry.next, 
                                                 &mvtNextEntry);
            if (rv != BCM_E_NONE) {
                BCM_ERR(("ERROR: %s, soc_qe2000_mvt_entry_get_frm_id, "
                         "Unit(%d), Err: 0x%x\n", FUNCTION_NAME(), unit, rv));
                goto err;
            }
    ...
    if (remove_index != SBX_MVT_ID_NULL) {
        ...
        if (previous_entry_index != 0) {
            ...
            rv = soc_qe2000_mvt_entry_get_frm_id(unit, previous_entry_index,
                                                 &previousMvtEntry);
            if (rv != BCM_E_NONE) {
                BCM_ERR(("ERROR: %s, soc_qe2000_mvt_entry_get_frm_id, Unit(%d),"
                         " Err: 0x%x\n", FUNCTION_NAME(), unit, rv));
                goto err;
            }
            ...
            rv = soc_qe2000_mvt_entry_set_frm_id(unit, previous_entry_index,
                                                 &previousMvtEntry);
            if (rv != BCM_E_NONE) {
                BCM_ERR(("ERROR: %s, soc_qe2000_mvt_entry_set_frm_id, Unit(%d), "
                         "Err: 0x%x\n", FUNCTION_NAME(), unit, rv));
                goto err;
            }
        }
        ...
        if (tmp != BCM_E_NONE) {
            BCM_WARN(("ERROR: %s, soc_qe2000_mvt_entry_set_frm_id, Unit(%d), "
                      "Err: 0x%x\n", FUNCTION_NAME(), unit,tmp));
        }
         
        tmp = soc_qe2000_mvt_entry_free_frm_id(unit, remove_index);
        if (tmp != BCM_E_NONE) {
            BCM_WARN(("ERROR: %s, soc_qe2000_mvt_entry_free_frm_id, Unit(%d), "
                      "Err: 0x%x\n", FUNCTION_NAME(), unit, tmp));
        }
    }
                                ...
                                bcm_if_t *encap_id_array)
{
    ...
    if (rv != BCM_E_NONE) {
        BCM_ERR(("ERROR: %s, bcm_qe2000_multicast_egress_delete_all, Unit(%d), Err: 0x%x\n",
                      FUNCTION_NAME(), unit, rv));
        goto err;
    }
    ...
    for (i = 0; i < port_count; i++) {
        ...
        if (rv != BCM_E_NONE) {
            BCM_ERR(("ERROR: %s, bcm_qe2000_multicast_egress_add, Unit(%d), Err: 0x%x\n",
                      FUNCTION_NAME(), unit, rv));
            goto err;
        }
    }
                                ...
                                int *port_count)
{
    ...
    if (port_max < 0) {
        BCM_ERR(("ERROR: %s, invalid port_max parameter, Unit(%d), port_max: 0x%x\n",
    ...
    if (port_count == NULL) {
        BCM_ERR(("ERROR: %s, port_count parameter is NULL, Unit(%d)\n", FUNCTION_NAME(), unit));
        return(BCM_E_PARAM);
    }
    if (!soc_qe2000_mvt_entry_id_valid(unit, group)) {
                                         ...
                                         int source_knockout)
{
    ...
    if ( (source_knockout != FALSE) && (source_knockout != TRUE) ) {
        BCM_ERR(("ERROR: %s, Invalid source knockout parameter, Unit(%d), SrcKnockout(0x%x)\n",
...
bcm_qe2000_multicast_state_get(int unit, char *pbuf)
{
    ...
    if (rv != BCM_E_NONE) {
	BCM_ERR(("ERROR: %s, soc_qe2000_mvt_state_get, Unit(%d), Err: %s\n", FUNCTION_NAME(), unit, bcm_errmsg(rv)));
    }
