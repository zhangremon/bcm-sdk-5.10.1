/*
 * $Id: port.c 1.51.2.1 Broadcom SDK $
...
{
    if (port == 0) {
        SAND_HAL_RMW_FIELD(unit, KA,
                           SC_SI0_DEBUG1,
                           FORCE_SERDES_TX_LOW, (enable)?0:1);
        SAND_HAL_RMW_FIELD(unit, KA,
                           SC_SI0_CONFIG0,
                           ENABLE, (enable)?1:0);
    } else {
...
_qe2000_spi_subport_init(int unit, bcm_port_t port)
{
    ...
    port_config = sal_alloc((sizeof(qe2000_ei_port_config_t) * SB_FAB_DEVICE_QE2000_MAX_PORT),
                                                                             "ei port info");
    if (port_config == NULL) {
        BCM_ERR(("Unable to allocate memory for ei port configuration"));
        return(BCM_E_MEMORY);
    }
                            ...
                            bcm_port_control_t type, int *value)
{
	...
	*value = SOC_SBX_STATE(unit)->port_state->fabric_header_format;
	...
	nSciLink = SOC_PORT_BLOCK_INDEX(unit, port);
		...
		if (SOC_SBX_CFG_QE2000(unit)->uuRequeuePortsMask[spi] & (1LL << spi_port)) {
		    *value = 1;
		} else {
		    *value = 0;
		}
                                ...
                                uint32 uShaperId, int nType, int bAdd)
{
    ...
    soc_cm_print("*** bcm_qe2000_eg_mem_read:Data 0x%x 0x%x 0x%x ShaperType %d Id %d\n", uData0, uData1, uData2, nType, uShaperId);
#endif
    if (rv != BCM_E_NONE) {
        return(rv);
    }
    ...
    /*
    soc_cm_print("_bcm_qe2000_fifo_egress_set ret: 0x%x (Fifo Index %d: 0x%x 0x%x 0x%x)\n",
				   ...
				   uint32 *pShaperIdHiPort, uint32 *pShaperIdLoFifo)
{
    ...
    rv = soc_qe2000_eg_mem_read(unit, nFifoParmIndex, 0x2, &uData0, &uData1, &uData2);
#ifdef EGRESS_SHAPER_DEBUG
    soc_cm_print("*** bcm_qe2000_eg_mem_read:Data 0x%x 0x%x 0x%x\n", uData0, uData1, uData2);
#endif
    if (rv != BCM_E_NONE) {
        return(rv);
    }
                                ...
                                uint32 kbits_sec, uint32 kbits_burst)
{
    int rv = BCM_E_NONE;
    uint32_t uData0, uData1, uData2;
    uint64_t uuMaxRateInBitsPerSec;
    uint32   maxRateInBitsPerSec = 0;
    uint32_t uShapeRate;
    uint32_t uBurstRate;
    uint32_t uFifoSrcMask;
    int  nType;
    int  nFifoParmIndex;
    int  bAdd;
    sal_usecs_t t;
    ...
    /*
    soc_cm_print("bcm_qe2000_port_rate_egress_set (Port %d: ShaperId %d 0x%x 0x%x)\n", port, uShaperId, kbits_sec, kbits_burst);
    */

    if (!soc_feature(unit, soc_feature_egress_metering)) {
        BCM_DEBUG(BCM_DBG_PORT,("soc_feature_egress_metering Unavailable"));
        return BCM_E_UNAVAIL;
     }

    if (kbits_burst > 190) {
      BCM_WARN(("Burst size exceeded: Max Burst size(%d,000)bits will be adjusted to 190 kbits\n", kbits_burst));
      kbits_burst = 190;
    }
    ...
    */
    ...
    uuMaxRateInBitsPerSec = kbits_sec;
    
    if (soc_sbx_div64((uuMaxRateInBitsPerSec * 384), 625, &maxRateInBitsPerSec) == -1) {
      return BCM_E_INTERNAL;
    }
    ...
    /*
    soc_cm_print("kbits_sec %d => uShapeRate %d. kbits_burst %d => uBurstRate %d\n",
        ...
        bAdd = FALSE;
        BCM_DEBUG(BCM_DBG_PORT,("Zero rate specified. Disabling Shaper"));
        /*    soc_cm_print("Zero rate speicified. Disabling shaper\n"); */
    ...
    rv = soc_qe2000_eg_mem_write(unit, uShaperId, 0x5, uData0, uData1, uData2);
    BCM_DEBUG(BCM_DBG_PORT,("Set Shaper %d [0x%x 0x%x]\n", uShaperId, uData1, uData0));
    /*
    soc_cm_print("bcm_qe2000_port_rate_egress_set ret: 0x%x (Port %d: 0x%x 0x%x 0x%x)\n", rv, port, uData0, uData1, uData2);
    */
    rv = soc_qe2000_eg_mem_read(unit, uShaperId, 0x5, &uData0, &uData1, &uData2);
    /*
    soc_cm_print("bcm_qe2000_eg_mem_read:Data 0x%x 0x%x 0x%x\n", uData0, uData1, uData2);
                                ...
                                uint32 *kbits_sec, uint32 *kbits_burst)
{
    ...
    uuMaxRateInBitsPerSec = uShapeRate;

    

    if (soc_sbx_div64((uuMaxRateInBitsPerSec * 625), 384, &maxRateInBitsPerSec) == -1) {
      return BCM_E_INTERNAL;
    }
    ...
    *kbits_burst = uBurstRate / 1000;  /* convert bit to kbits */
    ...
    /*
    soc_cm_print("Max 0x%llx Burst 0x%x\n", maxRateInBitsPerSec, uBurstRate);
    soc_cm_print("bcm_qe2000_eg_mem_read:Data 0x%x 0x%x 0x%x\n", uData2, uData1, uData0);
    soc_cm_print("bcm_qe2000_port_rate_egress_get ret: 0x%x. Port %d: Rate %d kb Burst %d kb (0x%x 0x%x)\n",
        ...
        uint32 kbits_burst)
{
    ...
    soc_cm_print("bcm_qe2000_port_rate_egress_traffic_set... 0x%x 0x%x 0x%x 0x%x 0x%x\n",
    ...
    */
    ...
    BCM_DEBUG(BCM_DBG_PORT,("Set Shaper %d [0x%x 0x%x]\n", uShaperId, uData1, uData0));
#ifdef EGRESS_TRAFFIC_DEBUG
    soc_cm_print("bcm_qe2000_port_rate_egress_traffic_set ret: 0x%x (Port %d: 0x%x 0x%x 0x%x)\n", rv, port, uData2, uData1, uData0);
#endif
...
#ifdef EGRESS_TRAFFIC_DEBUG
    soc_cm_print("bcm_qe2000_port_rate_egress_traffic_get... ShaperId %d traffic_type 0x%x\n",
				...
				int enable)
{
    ...
    if (is_empty == FALSE) {
        BCM_DEBUG(BCM_DBG_ERR, ("FIFO not empty fifo: %d, port: %d\n", fifo_parm_index, port));
    ...
    BCM_DEBUG(BCM_DBG_PORT,("Port Remap Entry (%d) 0x%x\n", uData0));
