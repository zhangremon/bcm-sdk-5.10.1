...
bcm_qe2000_vlan_init(int unit)
{
    ...
    if (NULL == sbx_vlan_state[unit]) {
        ...
        if (NULL == sbx_vlan_state[unit]) {
            rv = BCM_E_MEMORY;
            BCM_ERR(("ERROR: %s, Cannot allocate memory, unit(%d), Err: %s (%d)\n",
    ...
    for (vid = 0; vid <= SBX_MVT_ID_VSI_END; vid++) {
        ...
        if (rv != BCM_E_NONE) {
            BCM_ERR(("ERROR: %s, soc_qe2000_mvt_entry_get_frm_id, Unit(%d), Err: %d, Vid: %d\n",
        ...
        if (rv != BCM_E_NONE) {
            BCM_ERR(("ERROR: %s, soc_qe2000_mvt_entry_set_frm_id, Unit(%d), Err: %d\n",
                       ...
                       bcm_vlan_t vid)
{
    ...
    if (vid > SBX_MVT_ID_VSI_END) {
        BCM_ERR(("ERROR: %s, Invalid Vlan Id, Unit(%d), VlanId: %d\n",
    ...
    if (rv != BCM_E_NONE) {
        BCM_ERR(("ERROR: %s, soc_qe2000_mvt_entry_reserve_id, Unit(%d), Err: %d\n",
                            FUNCTION_NAME(), unit, rv));
        return(rv);
    }
    ...
    rv = soc_qe2000_mvt_entry_get_frm_id(unit, vid, &mvtEntry);
    if (rv != BCM_E_NONE) {
        BCM_ERR(("ERROR: %s, soc_qe2000_mvt_entry_get_frm_id, Unit(%d), Err: %d\n",
                      FUNCTION_NAME(), unit, rv));
        goto err;
    }
                                 ...
                                 int diags)
{
    ...
    if (rv != BCM_E_NONE) {
        if (diags) {
            BCM_ERR(("ERROR: %s, soc_qe2000_mvt_entry_free_frm_id, Unit(%d), Err: %d\n",
                                FUNCTION_NAME(), unit, rv));
        }
        if (vid != sbx_vlan_state[unit]->default_vid) {
...
bcm_qe2000_vlan_destroy_all(int unit)
{
    ...
    for (vid = 0; vid <= SBX_MVT_ID_VSI_END; vid++) {
        ...
        if (rv != BCM_E_NONE) {
            BCM_ERR(("ERROR: %s, bcm_qe2000_vlan_destroy, Unit(%d), Err: %d\n",
                      FUNCTION_NAME(), unit, rv));
            goto err;
        }
    }
                          ...
                          bcm_pbmp_t ubmp)
{
    ...
    if (BCM_PBMP_NOT_NULL(tbmpa)) {
        BCM_ERR(("ERROR: %s, Invalid Ports, Unit(%d), VlanId: %d\n", FUNCTION_NAME(), unit, vid));
        return(BCM_E_PARAM);
    }
             ...
             bcm_pbmp_t ubmp)
{
    ...
    if (rv == BCM_E_NONE) {
         ...
         */
        rv = bcm_qe2000_trunk_vlan_port_fixup(unit,
                                              vid,
                            ...
                            bcm_vlan_t vid)
{
    ...
    if (vid != BCM_VLAN_VID_DISABLE) {
        ...
        if (rv != BCM_E_NONE) {
            BCM_ERR(("ERROR: %s, bcm_qe2000_vlan_port_add, Unit(%d), Err: 0x%x\n",
                      FUNCTION_NAME(), unit, rv));
            goto err;
        }
    }
    ...
    if (sbx_vlan_state[unit]->default_vid != BCM_VLAN_VID_DISABLE) {
        ...
        if (rv != BCM_E_NONE) {
            BCM_ERR(("ERROR: %s, bcm_qe2000_vlan_port_remove, Unit(%d), Err: 0x%x\n",
                      FUNCTION_NAME(), unit, rv));
        }
    }
