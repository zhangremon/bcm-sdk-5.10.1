/*
 * $Id: cosq.c 1.99.14.3 Broadcom SDK $
...
static void
_bcm_qe2000_cosq_queue_depth_denormalize(int32_t *p_min_physical_queue_depth_in_bytes, int32_t *p_max_physical_queue_depth_in_bytes, sbBool_t bAdd);
...
static void _bcm_qe2000_cosq_anemic_watermark_select_for_params_chosen(bcm_sbx_cosq_queue_params_ingress_t *p_qparams,
                                       int32_t *p_anemic_watermark_select);

static int
_bcm_qe2000_cosq_add_delete_queue(int unit, int add, int32_t queue, int32_t rate_delta_max_index,
...
_bcm_qe2000_cosq_select_queue_params(int unit, int queue, bcm_sbx_cosq_queue_params_ingress_t *p_qparams,
                     int32_t *p_hold_ts, int32_t *p_qtype,
                     uint32_t *p_rate_a, uint32_t *p_rate_b,
                     bcm_sbx_cosq_queue_region_type_t queue_region);

static int
_bcm_qe2000_cosq_set_ingress_shaper(int unit,
                    int32_t queue,
                    uint32_t shape_limit_kbps,
                    int enable_shaping);
...
_bcm_qe2000_anemic_age_threshold_key_for_params_chosen(int unit, int queue, bcm_sbx_cosq_queue_params_ingress_t *p_qparams,
                               int32_t *p_anemic_age_threshold_key);

static void
_bcm_qe2000_rate_delta_max_index_for_port_rate_chosen(int unit, int bw_mode, uint32_t bag_rate_kbps,
                              int32_t *prate_delta_max_index);
...
static INLINE uint32 _bcm_qe2000_cosq_shape_burst_rate_256kbits_get(int unit, uint32_t per_queue_shape_burst_kbps);
static INLINE uint32 _bcm_qe2000_cosq_burst_get(int unit, int num_cos, uint32_t shape_rate_256_kbits);
static INLINE uint32 _bcm_qe2000_cosq_queuegroup_to_queue_shape_burst_get(int unit, int num_cos, uint32_t queuegroup_shape_burst_kbits);
...
bcm_qe2000_cosq_init(int unit)
{
    ...
    if (rv != BCM_E_NONE) {
        BCM_ERR(("ERROR: Unit(%d) error starting scoreboard thread", unit));
	return rv;
    } 
    ...
    if (unit >= HW_QE2000_MAX_UNIT_NBR) {
        BCM_ERR(("ERROR: Unexpected Unit, Max Qe2000 Unit(%d) Unit(%d)", HW_QE2000_MAX_UNIT_NBR, unit));
        return(rv);
    }
    ...
    if (rv != BCM_E_NONE) {
	BCM_ERR(("ERROR: VNP memory allocation error\n"));
    }
	...
	if (rv != SOC_E_NONE) {
	    BCM_ERR(("ERROR: timeout writing to q2ec memory queue(%d)", queue));
	...
	for (bw_group = 0; bw_group < SOC_SBX_CFG(unit)->num_bw_groups; bw_group++) {
	    ...
	    if (rv != SOC_E_NONE) {
		BCM_ERR(("ERROR: EASY_RELOAD: Reading Bw port configuration entry bw_group(%d) failed.\n", bw_group));
		return BCM_E_TIMEOUT;
	    }

	    sbZfFabQe2000BwPortConfigEntry_Unpack(&zfPortConfigEntry, (uint8_t*)data, 6);
	...
	for (bw_group = 0; bw_group < SOC_SBX_CFG(unit)->num_bw_groups; bw_group++) {
	    ...
	    sbZfFabQe2000BwPortConfigEntry_Pack(&zfPortConfigEntry, (uint8_t*)data, 6);
	    ...
	    if (rv != SOC_E_NONE) {
		BCM_ERR(("ERROR: Writing Bw port configuration table entry (%d) failed.\n", bw_group));
		return BCM_E_FAIL;
	    }
	}
    }
                          ...
                          bcm_sbx_cosq_bw_group_params_t *p_bwparams, int is_user_invoked)
{
    ...
    int32 status;
    int32_t mc=0;
    int32_t rate_delta_max_index = 0;
    uint32_t rate_a = 0, rate_b = 0;
    int32_t anemic_age_threshold_key;
    int32_t qtype;
    int32_t hold_ts;
    bcm_sbx_cosq_queue_state_t *queue_state;
    int32_t bw_group;
    int32_t idx = 0;
    int32_t remap_node = dest_node;
    int32_t remap_port = dest_port;
    /* int32_t initial_sysport = sysport; */
    int32_t ef;
    int32_t is_allocated = FALSE;

    BCM_DEBUG(BCM_DBG_COSQ, ("Adding queue(%d)\n",queue));
    ...
    if (queue_region == bcm_sbx_cosq_queue_region_local) {
        ...
        if (dest_type == BCM_INT_SBX_DEST_TYPE_MULTICAST) {
            BCM_DEBUG(BCM_DBG_COSQ, ("Adding queue(%d) as multicast queue\n",queue));
        } else {
	...
	/* only allocate for base queue in queue group */
        if ((is_user_invoked == TRUE) && (dest_cos == 0)) {
            ...
            if (vnp_id < 0) {
                BCM_DEBUG(BCM_DBG_COSQ, ("Error: virtual node port allocation failed\n"));
		if (SOC_IS_RELOADING(unit)) {
#ifdef BCM_EASY_RELOAD_SUPPORT
	    ...
	    if (vnp_id == -1){
		BCM_ERR(("ERROR: Could not find virtual np "));
		status = BCM_E_MEMORY;
		goto err;
	    }
	}
	...
	if ((is_user_invoked == TRUE) &&
	    ...
	    if (status != 0) {
		BCM_ERR(("ERROR: timeout on lna memory access"));
	    ...
	    if (status != 0) {
		BCM_ERR(("ERROR: timeout on port remap memory access"));
		status = BCM_E_TIMEOUT;
		goto err;
	    }
	}
	...
	if (sysport != BCM_INT_SBX_INVALID_SYSPORT) {
		...
		/* Extended eset */
		BCM_DEBUG(BCM_DBG_COSQ, ("extended eset Remapping sysport(0x%x) to port(0x%x)\n", sysport, remap_port));
		ef = (p_qparams->bw_mode == BCM_COSQ_EF);
		...
		BCM_DEBUG(BCM_DBG_COSQ, ("other Remapping sysport(0x%x) to port(0x%x)\n", sysport, remap_port));
		ef = (p_qparams->bw_mode == BCM_COSQ_EF);
		status = bcm_qe2000_cosq_sysport_port_remap(unit, sysport, ef, remap_port);
		if (status != BCM_E_NONE) {
		    BCM_ERR(("ERROR: Failed to remap sysport\n"));
...

   {
        ...
        if (status != SOC_E_NONE) {
            BCM_ERR(("ERROR: select queue params failure\n"));
            goto err;
        }
        ...
        if (status != SOC_E_NONE) {
            BCM_ERR(("ERROR: Rate A table write failed\n"));
            status = -1;
            goto err;
        }
        ...
        if (status != SOC_E_NONE) {
            BCM_ERR(("ERROR: rate B table write failed\n"));
            status = -1;
            goto err;
        }
    }
...

    {
        ...
        if (status != SOC_E_NONE) {
            BCM_ERR(("ERROR: age threshold key set write failed\n"));
            goto err;
        }
    }
    ...
    if (status != SOC_E_NONE) {
        BCM_ERR(("ERROR: queue depth/length set failed\n"));
        goto err;
    }
    ...
    if (status != SOC_E_NONE) {
        BCM_ERR(("add/delete queue failed\n"));
        goto err;
    }
    ...
    status = soc_qe2000_priority_set (unit, idx, 0 /* shaped */, 0 /* pri */,
                                      0 /* next_pri */);
    if (status != SOC_E_NONE) {
        BCM_ERR(("priority table clear failed for queue(%d) idx(%d)\n", queue, idx));
        goto err;
    }
...
_bcm_qe2000_cosq_delete_queue(int unit, int queue, bcm_sbx_cosq_queue_region_type_t queue_region, int is_user_invoked) {
    ...
    int32_t dest_type; /* unicast/multicast */
    int32_t dest_node;
    int32_t dest_port;
    int32_t dest_cos;
    uint32_t data0, data1, data2, data3;

    int32_t last_queue = 1; /* ab 022806 this is last queue so clear E2Q  - in future this will come from up above*/
    int32_t cos_per_port = 16; /* ab 022806 this should come down too for FIC mode */
    uint32_t shape_limit_kbps;
    uint32_t tmp_cos;
    uint32_t tmp_base_queue = 0;
    uint32_t enable = 0;
    ...
    if (status != SOC_E_NONE) {
        BCM_ERR(("ERROR: Q2EC read failed\n"));
        return BCM_E_MEMORY;
    }
    if (dest_type == BCM_INT_SBX_DEST_TYPE_MULTICAST) {
        mc = 1;
    ...
    if (queue_region == bcm_sbx_cosq_queue_region_local){
        ...
        status = soc_qe2000_e2q_get(unit, dest_type, (dest_node<<6), (dest_port&0x3F), (int32_t*)&tmp_base_queue, (int32_t*)&enable);
        if (status != SOC_E_NONE) {
            BCM_ERR(("ERROR: E2Q get failed\n"));
            return BCM_E_MEMORY;
        }
    ...
    if (status != SOC_E_NONE) {
        BCM_ERR(("ERROR:delete queue failed\n"));
        return BCM_E_MEMORY;
    }
    ...
    if (status != SOC_E_NONE) {
        BCM_ERR(("ERROR: QM memory read failed\n"));
        return BCM_E_MEMORY;
    }
    ...
    status = soc_qe2000_qm_mem_write(unit, queue, 0x02,
                                     data0, data1, data2, data3);
    if (status != SOC_E_NONE) {
        BCM_ERR(("ERROR: QM memory write failed\n"));
        return BCM_E_MEMORY;
    }
			    ...
			    bcm_sbx_cosq_queue_region_type_t queue_region)
{
	...
	BCM_ERR(("ERROR: gport 0x%x not associated with base queue\n", gport));
	return BCM_E_PARAM;
    }
    ...
    BCM_DEBUG(BCM_DBG_COSQ,("Enable FIFO gport = 0x%x, sysport = 0x%x local = %d\n", gport, sysport, local));
			     ...
			     bcm_sbx_cosq_queue_region_type_t queue_region) 
{
    ...
    BCM_DEBUG(BCM_DBG_COSQ,("Disable FIFO gport = 0x%x, sysport = 0x%x local = %d\n", gport, sysport, local));
			   ...
			   int *enable)
{
    ...
    BCM_DEBUG(BCM_DBG_COSQ,("Enable get gport = 0x%x, sysport = 0x%x, local =  %d\n", gport, sysport, local));
...
bcm_qe2000_cosq_overlay_queue(int unit,
    ...
    BCM_DEBUG(BCM_DBG_COSQ, ("Overlaying queue(%d)\n",queue));

    if (sysport == BCM_INT_SBX_INVALID_SYSPORT) {
        BCM_ERR(("ERROR: Overlay sysport not specified\n"));
    ...
    if (dest_type == BCM_INT_SBX_DEST_TYPE_MULTICAST) {
        BCM_ERR(("ERROR: Overlay not support multicast\n"));
        return BCM_E_PARAM;
    }
	...
	if (SOC_SBX_CONTROL(unit)->node_id == remap_node) {
	    ...
	    BCM_DEBUG(BCM_DBG_COSQ, ("Remapping sysport(0x%x) to port(0x%x)\n", sysport, remap_port));

	    queue_state = (bcm_sbx_cosq_queue_state_t*)SOC_SBX_STATE(unit)->queue_state;
	    ef = (queue_state[queue].ingress.bw_mode == BCM_COSQ_EF);
...
bcm_qe2000_cosq_delete_overlay_queue(int unit,
	...
	BCM_INT_SBX_COSQ_GET_NODE_PORT_FROM_SYSPORT(unit, base_sysport, &dest_node, &dest_port, &dest_mc);
        ...
        BCM_DEBUG(BCM_DBG_COSQ, ("Removing sysport(0x%x) overlay on queue(0x%x)\n", sysport, queue));

	/* port remap table is left alone, will be updated if the sysport is used
	 * for something else later on
	 */

	/* TODO: convert queue to Best effort, do we really need to do this?
            status = soc_qe2000_queue_para_get(unit, queue, &local, &hold_ts, &qtype);
            if (status != SOC_E_NONE) {
            ...
            status = soc_qe2000_queue_para_set(unit, queue, local, hold_ts, qtype);
            if (status != SOC_E_NONE) {
                BCM_ERR(("ERROR: queue param table read failure\n"));
                return -1;
            }
...
bcm_qe2000_cosq_set_ingress_params(int unit,
    ...
    int32_t status = SOC_E_NONE;
    int32_t enable, old_anemic_watermark_select, old_min_buffers, old_max_buffers;
    sbBool_t add_delete_queue = FALSE;
    int32_t dest_mc, mc;
    int32_t dest_node;
    int32_t dest_port;
    int32_t dest_cos;
    uint32_t old_rate_a = 0, old_rate_b = 0;
    int32_t old_qtype;
    int32_t old_hold_ts;
    uint32_t rate_a = 0, rate_b = 0;
    int32_t qtype;
    int32_t hold_ts;
    int32_t anemic_age_threshold_key;
    int32_t old_anemic_age_threshold_key;
    ...
    int32_t remap_node;
    int32_t remap_port;
    int32_t anemic_watermark_select;
    sbBool_t disable_length_adjustment, configure_rate_delta_max;
    int adjust;
     ...
     */
    {
        ...
        if ( status != SOC_E_NONE ) {
            BCM_ERR(("ERROR: Failed to read queue state memory for queue(%d)\n",queue));
            return status;
        }
        ...
        if ( enable == 1 ) {
            ...
            BCM_ERR(("ERROR: reconfiguration of queue which is not enabled\n"));
    ...
    if ( add_delete_queue == TRUE ) {
        ...
        if (status) {
            BCM_ERR(("ERROR: q2ec table read error\n"));
            return status;
        }
        ...
        if (status) {
            BCM_ERR(("ERROR: error retreiving queue packet length adjustment\n"));
            return status;
        }
     ...
     */
    {
        ...
        if ( (p_newqparams->anemic_age_threshold_key != BCM_DEFAULT_INT32_VALUE) &&
            ...
            if (status) {
                BCM_ERR(("ERROR: Failed to modify Anemic Age Threshold for queue(%d)\n",queue));
                return status;
            }
        }
        ...
        if ( (p_newqparams->template != p_oldqparams->template) ||
            ...
            if ( status ) {
                BCM_ERR(("ERROR:Failed to modify WRED template and gain for queue(%d)\n",queue));
                return status;
            }
        }
        ...
        if ( (p_newqparams->shape_limit_kbps != p_oldqparams->shape_limit_kbps) ||
            ...
            if ( status ) {
                BCM_ERR(("ERROR: Failed to modify shaper config for queue(%d)\n",queue));
                return status;
            }
        }

        configure_rate_delta_max = FALSE;
        disable_length_adjustment = FALSE;
        ...
        if (SOC_SBX_CFG(unit)->sp_mode == SOC_SBX_SP_MODE_ACCOUNT_IN_BAG) {
            ...
            disable_length_adjustment = (p_newqparams->bw_mode == BCM_COSQ_SP) ? TRUE : FALSE;
        }
        ...
        if (configure_rate_delta_max == TRUE) {
            ...
            if ( status ) {
                BCM_ERR(("ERROR: Failed to modify qla_demand_mask for queue(%d)\n",queue));
                return status;
            }
#endif /* 0 */
     ...
     */
        if ( (p_newqparams->queue_header_adjust_sign != p_oldqparams->queue_header_adjust_sign) ||
            ...
            if ( status ) {
                BCM_ERR(("ERROR: Failed to modify Queue header adjust sign/bytes for queue(%d)\n",queue));
                return status;
            }
        }
    }
	...
	if ( (p_bwstate->overlay_in_use == TRUE) &&
        ...
        if ( ((SOC_SBX_CFG(unit)->uFabricConfig == SOC_SBX_SYSTEM_CFG_VPORT_LEGACY) ||
	      (SOC_SBX_CFG(unit)->uFabricConfig == SOC_SBX_SYSTEM_CFG_VPORT_MIX)) &&
	     (SOC_SBX_CONTROL(unit)->node_id == remap_node) ) {
	...
	if (status) {
	    BCM_ERR(("ERROR: set ingress params configuration bw_group(%d) error(%d)\n", bw_group, status));
	    return status;
	}
    }
    ...
    if (SOC_SBX_CFG(unit)->sp_mode == SOC_SBX_SP_MODE_ACCOUNT_IN_BAG) {
        if (p_newqparams->bw_mode == BCM_COSQ_SP) {
            ...
            status = _bcm_qe2000_cosq_update_gsp(unit, queue);
            if (status) {
                BCM_ERR(("ERROR: set ingress params configuration (strict priority) bw_group(%d) error(%d)\n", bw_group, status));
                return(status);
            }
        }
    }
				  ...
				  int gain)
{
     ...
     *     definitions
     */
    uint32_t data0 = ( ((template << 4) & 0x000000F0) | (gain & 0x0000000F) );
    uint32_t status;
    int rv = BCM_E_NONE;

    status = soc_qe2000_qm_mem_write(unit, queue, 0x0F, data0, 0x00, 0x00, 0x00);
    if (status) {
        BCM_ERR(("ERROR: %s, soc_qe2000_qm_mem_write,  Unit(%d) queue(%d)\n", FUNCTION_NAME(), unit, queue));
        rv = BCM_E_INTERNAL;
    }
    return rv;
}
				  ...
				  int *gain)
{
    ...
    BCM_DEBUG(BCM_DBG_COSQ, ("Retrieve WRED template, queue(%d) on QE2000(%d)\n", queue, unit));
    ...
    if (rv) {
	BCM_ERR(("ERROR: read of template/gain from WRED table on QE2000 queue(%d) timed out\n", queue));
	return rv;
    }
...
{
    int rv = BCM_E_NONE;
    uint8_t data[SB_ZF_FAB_WRED_PARAMETERS_SIZE];
    uint32_t *p_data, data0, data1, data2, data3;
    ...
    if (discard->drop_probability == 0) { /* disabling drop probability */
        ...
        if (status) {
           BCM_ERR(("ERROR: %s, soc_qe2000_qm_mem_write,  Unit(%d)\n", FUNCTION_NAME(), unit));
           rv = BCM_E_INTERNAL;
           goto err;
        }
                               ...
                               uint64_t   *value)
{
    ...
    BCM_DEBUG(BCM_DBG_VERBOSE,("soc_sbx_qe2000_counter_port_get %d %d %d cntr %d 0x%llx\n",unit,block,set,stat,*value));
...

{
    ...
    bcm_sbx_cosq_bw_group_state_t *p_bwstate;
    int gamma = 0, sigma = 0;
    uint32_t guarantee_in_kbps = 0;
    uint64_t uu_epoch_length_in_ns = 0;
    uint64_t uu_guarantee_in_kbytes;
    uint32_t guarantee_in_bytes_per_epoch = 0;
    uint32_t bag_rate_bytes_per_epoch;
    uint64_t uu_bag_rate_kbytes;
    int32 queue;
    int32 num_sp_queues;
    ...
    if ( soc_feature(unit, soc_feature_standalone) ) {
	uu_epoch_length_in_ns = SB_FAB_TME_EPOCH_IN_NS;
    } else {
	uu_epoch_length_in_ns = (SOC_SBX_CFG(unit)->epoch_length_in_timeslots * p_fabricstate->timeslot_size) /* ns */;
    }
    ...
    for (queue=base_queue; queue < base_queue + p_bwstate->num_cos; queue++) {
        ...
        if (p_qstate[queue].ingress.bw_mode== BCM_COSQ_AF) {
            ...
            uu_guarantee_in_kbytes = guarantee_in_kbps/8;
	    
	    if (soc_sbx_div64((uu_guarantee_in_kbytes * uu_epoch_length_in_ns), 1000000, &guarantee_in_bytes_per_epoch) == -1) {
	      ...
	      return BCM_E_INTERNAL;
            ...
            BCM_DEBUG(BCM_DBG_COSQ, ("queue(%d) guarantee in bytes/epoch(%d)\n", queue, guarantee_in_bytes_per_epoch));
        }
        ...
        BCM_DEBUG(BCM_DBG_COSQ, ("gamma set to %d\n", gamma));

        if (rv != SOC_E_NONE) {
            BCM_ERR(("Writing BAA configuration entry (%d) failed.\n", queue));
            return BCM_E_FAIL;
        }
    }
    ...
    uu_bag_rate_kbytes = p_bwstate->path.bag_rate_kbps / 8;
    
    if (soc_sbx_div64((uu_bag_rate_kbytes * uu_epoch_length_in_ns), 1000000, &bag_rate_bytes_per_epoch) == -1) {
      BCM_ERR(("ERROR: update BAG rate per epoch failed\n"));
      return BCM_E_INTERNAL;
    }
    bag_rate_bytes_per_epoch >>= SOC_SBX_CFG(unit)->demand_scale;
    BCM_DEBUG(BCM_DBG_COSQ, ("Bag rate: %d\n", bag_rate_bytes_per_epoch));
    ...
    BCM_DEBUG(BCM_DBG_COSQ, ("Reading Bw Port Cfg: 0x%08x 0x%08x 0x%08x 0x%08x\n", data[0],data[1],data[2],data[3]));

    if (rv != SOC_E_NONE) {
        BCM_DEBUG(BCM_DBG_COSQ, ("Reading Bw port configuration entry bw_group(%d) failed.\n", bw_group));
        return BCM_E_FAIL;
    }
    ...
    BCM_DEBUG(BCM_DBG_COSQ, ("Updating Port Configuration Table entry (%d) num flows in the bag (%d)\n",
    ...
    if (rv != SOC_E_NONE) {
        BCM_ERR(("Writing Bw port configuration table entry (%d) failed.\n", bw_group));
        return BCM_E_FAIL;
    }
...
_bcm_qe2000_cosq_init_virtual_node_port(int unit)
{
    ...
    BCM_DEBUG(BCM_DBG_COSQ, ("initialize virtual node port list\n"));
...
bcm_qe2000_cosq_detach(int unit)
{
   ...
   if (rv != BCM_E_NONE) {
        BCM_ERR(("ERROR: Unit(%d) error detaching scoreboard thread", unit));
	return rv;
    } 
    return BCM_E_NONE;
}
...
_bcm_qe2000_reserve_arb_group(int unit, int32 vnp_id, int32 dest_node, int32 dest_port)
{
    ...
    soc_cm_print("l1_arb_id(%d) l2_arb_id(%d)\n", l1_arb_id, l2_arb_id);
#endif
}
...
_bcm_qe2000_allocate_virtual_node_port_multicast(int unit, int32 base_queue)
{
	...
	BCM_DEBUG(BCM_DBG_COSQ, ("allocate multicast virtual node/port(%d) ref_count(%d) base_queue(%d)\n",
				 vnp_id, virtual_node_port[unit][vnp_id].ref_count, base_queue));
	
#ifdef DEBUG_TME_ARB_PORT_OPTIMIZE	
	soc_cm_print("0:  allocate multicast virtual node/port(%d) ref_count(%d) base_queue(%d)\n",
		     vnp_id, virtual_node_port[unit][vnp_id].ref_count, base_queue);
#endif	
...
#ifdef DEBUG_TME_ARB_PORT_OPTIMIZE	    
	    soc_cm_print("1:  allocate multicast virtual node/port(%d) ref_count(%d) base_queue(%d)\n",
			 vnp_id, virtual_node_port[unit][vnp_id].ref_count, base_queue);
#endif		
	    _bcm_qe2000_reserve_arb_group(unit, vnp_id, -1, -1);
...
_bcm_qe2000_allocate_virtual_node_port_unicast(int unit, int32 base_queue, int32 dest_node, int32 dest_port)
{
	...
	BCM_DEBUG(BCM_DBG_COSQ, ("allocate virtual node/port(%d) ref_count(%d) base_queue(%d) dest node/port(%d/%d)\n",
				 vnp_id, virtual_node_port[unit][vnp_id].ref_count, base_queue, dest_node, dest_port));
	
#ifdef DEBUG_TME_ARB_PORT_OPTIMIZE	
	soc_cm_print("0:  allocate virtual node/port(%d) ref_count(%d) base_queue(%d) dest node/port(%d/%d)\n",
		     vnp_id, virtual_node_port[unit][vnp_id].ref_count, base_queue, dest_node, dest_port);
#endif	
	...
	if (virtual_node_port[unit][vnp_id].free == TRUE) {
	    ...
	    BCM_DEBUG(BCM_DBG_COSQ, ("allocate virtual node/port(%d) ref_count(%d) base_queue(%d)\n",
				     vnp_id, virtual_node_port[unit][vnp_id].ref_count, base_queue));
	    
#ifdef DEBUG_TME_ARB_PORT_OPTIMIZE	    
	    soc_cm_print("1:  allocate virtual node/port(%d) ref_count(%d) base_queue(%d) dest node/port(%d/%d)\n",
			 vnp_id, virtual_node_port[unit][vnp_id].ref_count, base_queue, dest_node, dest_port);
#endif	    
	    _bcm_qe2000_reserve_arb_group(unit, vnp_id, dest_node, dest_port);
	...
	BCM_DEBUG(BCM_DBG_COSQ, ("re-use virtual node/port(%d) ref_count(%d) base_queue(%d)\n",
				 vnp_id, virtual_node_port[unit][vnp_id].ref_count, base_queue));
#ifdef DEBUG_TME_ARB_PORT_OPTIMIZE	
	soc_cm_print("2:  allocate virtual node/port(%d) ref_count(%d) base_queue(%d) dest node/port(%d/%d)\n",
		     vnp_id, virtual_node_port[unit][vnp_id].ref_count, base_queue, dest_node, dest_port);
#endif		
	_bcm_qe2000_reserve_arb_group(unit, vnp_id, dest_node, dest_port);
...
#ifdef DEBUG_TME_ARB_PORT_OPTIMIZE	    
		soc_cm_print("2:  WARNING: allocate virtual node/port(%d) ref_count(%d) base_queue(%d) dest node/port(%d/%d)\n",
			     vnp_id, virtual_node_port[unit][vnp_id].ref_count, base_queue, dest_node, dest_port);
#endif		    
...
#ifdef DEBUG_TME_ARB_PORT_OPTIMIZE		
		soc_cm_print("2:  allocate empty arbiter node virtual node/port(%d) ref_count(%d) base_queue(%d) dest node/port(%d/%d)\n",
			     vnp_id, virtual_node_port[unit][vnp_id].ref_count, base_queue, dest_node, dest_port);
#endif		
...
_bcm_qe2000_cosq_allocate_virtual_node_port(int unit, int32 base_queue, int32 dest_node, int32 dest_port, int32 multicast)
{
	...
	if (status != SOC_E_NONE) {
	    BCM_ERR(("ERROR: EASY_RELOAD: Q2EC read failed for queue(%d)\n", base_queue));
	    return  BCM_E_TIMEOUT;
	}

	/* For unused entries, more than 5 bits will be set in the node field */
	if ((dest_node1 > 0x1f) || (dest_port1 > 0x1f)) {
	    BCM_ERR(("ERROR: EASY_RELOAD: Q2EC read of virtual node port node(%d) port(%d) invalid\n", dest_node, dest_port));
	    return BCM_E_INTERNAL;
	}
	...
	BCM_DEBUG(BCM_DBG_COSQ, ("RELOADING: virtual node/port(%d) ref_count(%d) base_queue(%d)\n",
				 vnp_id, virtual_node_port[unit][vnp_id].ref_count, base_queue));
#endif
    } else {
#ifdef TME_ARB_PORT_OPTIMIZE
...
#ifdef DEBUG_TME_ARB_PORT_OPTIMIZE
	    soc_cm_print("old 0:  allocate virtual node/port(%d) ref_count(%d) base_queue(%d) dest node/port(%d/%d)\n",
			 vnp_id, virtual_node_port[unit][vnp_id].ref_count, base_queue, dest_node, dest_port);
#endif
...
#ifdef DEBUG_TME_ARB_PORT_OPTIMIZE
		soc_cm_print("old 1:  allocate virtual node/port(%d) ref_count(%d) base_queue(%d) dest node/port(%d/%d)\n",
			     vnp_id, virtual_node_port[unit][vnp_id].ref_count, base_queue, dest_node, dest_port);
#endif
		return(vnp_id);
	    }
...
_bcm_qe2000_cosq_get_virtual_node_port(int unit, int32 base_queue)
{
    ...
    for (vnp_id=0; vnp_id < BCM_SBX_DEVICE_COSQ_MAX_LNAS; vnp_id++) {
        if (virtual_node_port[unit][vnp_id].base_queue == arb_port_base_queue) {
            BCM_DEBUG(BCM_DBG_COSQ, ("found virtual node port(%d) base_queue(%d)\n", vnp_id, base_queue));
            return(vnp_id);
        }
    }
    return(-1);
...
_bcm_qe2000_cosq_free_virtual_node_port(int unit, int32 base_queue)
{
    ...
    for (vnp_id = 0; vnp_id < BCM_SBX_DEVICE_COSQ_MAX_LNAS; vnp_id++) {
        ...
        BCM_DEBUG(BCM_DBG_COSQ, ("free virtual node/port(%d) ref_count(%d) base_queue(%d)\n",
                             vnp_id, virtual_node_port[unit][vnp_id].ref_count, base_queue));


#ifdef TME_ARB_PORT_OPTIMIZE
        soc_cm_print("free virtual node/port(%d) ref_count(%d) base_queue(%d)\n",
		     vnp_id, virtual_node_port[unit][vnp_id].ref_count, base_queue);
#endif
    ...
    BCM_ERR(("ERROR: attempt to free virtual node port(%d) which has not been allocated\n", vnp_id));
    return(-1);
}
    ...
    for ( buf_index=0; buf_index<2; buf_index++ ) {
        ...
        for ( shift=0; shift<8; shift++ ) {
            ...
            if ( (value == 0) && !((buf_index == 0) && (*p_min_physical_queue_depth_in_bytes == 0))) {
                ...
                if ( bAdd ) {
                    BCM_DEBUG(BCM_DBG_COSQ, ("Could not resolve queue depth (%d).  Fixing queue depth at (%d bytes)\n", bufs_unshifted[buf_index], 4*4080));
                    value = 4;
                    shift = 0;
                }
            }
            ...
            BCM_DEBUG(BCM_DBG_COSQ,("(%s) Value(0x%x, %d) Shift(0x%x, %d)\n",
                  ...
                  bcm_sbx_cosq_queue_params_ingress_t *p_qparams)
{
    ...
    int32_t min_bufs;
    int32_t max_bufs;
    int32_t min_physical_queue_depth_in_bytes = 0;
    int32_t max_physical_queue_depth_in_bytes = 0;
    int32_t local = 0;
    int32_t qetype = 0;
    int32_t qla_demand_mask = 0;
    int32_t queue_header_adjust_sign = 0;
    int32_t queue_header_adjust = 0;
    int32_t gamma = 0;
    int32_t template = 0;
    int32_t gain = 0;
    int32_t ingress_spi4 = 0;
    int32_t ingress_spi4_port = 0;
    int32_t bufs[2];
    int32_t total_min_bufs_allocated;
    int32_t anemic_watermark_select;
    int32_t total_bufs_available = 0;
    int32_t min_bufs_allocated = 0;
    int32_t  minimum_bufs;
    int32_t  delta;
    ...
    BCM_DEBUG(BCM_DBG_COSQ,("(%s) MinBytes(0x%x, %d) MaxBytes(0x%x, %d)\n",
                            ((add == TRUE) ? "Add" : "Delete"),
    ...
    _bcm_qe2000_cosq_queue_depth_denormalize(&bufs[0], &bufs[1], add);
        ...
        if ( add ) {
            BCM_DEBUG(BCM_DBG_COSQ,("Queue parameter qm_queue_config_data1.min_buf (0x%04x) (num buffers: %d)\n", min_bufs, ((min_bufs>>3)<<(min_bufs&0x7))));
            BCM_DEBUG(BCM_DBG_COSQ,("Queue parameter qm_queue_config_data0.max_bufs (0x%04x) (num buffers: %d)\n", max_bufs, ((max_bufs>>3)<<(max_bufs&0x7))));
        }
        ...
        if (add) {
            ...
            if ((min_bufs_allocated + minimum_bufs) > total_bufs_available) {
                ...
                BCM_DEBUG(BCM_DBG_COSQ,("Minimum Buffers Exceeds the Available Buffers, TotalBufs: 0x%x MinBufAllocated: 0x%x MinBufAllocateReq: 0x%x MinBufAllocated: 0x%x\n",
    ...
    BCM_DEBUG(BCM_DBG_COSQ,("%s Queue(%d) local(%d)\n", (add==TRUE)?"Adding":"Deleting", queue, local));
    BCM_DEBUG(BCM_DBG_COSQ,("%s Queue(%d) Full Params: max_bufs(%d) ingress_spi4(%d) ingress_spi4_port(%d)\n", (add==TRUE)?"Adding":"Deleting", queue,
                            max_bufs, ingress_spi4, ingress_spi4_port));
    BCM_DEBUG(BCM_DBG_COSQ,("%s Queue(%d) Full Params: qetype(%d) rate_delta_max_index(%d) qla_demand_mask(%d)\n", (add==TRUE)?"Adding":"Deleting", queue,
                            qetype, rate_delta_max_index, qla_demand_mask));
    BCM_DEBUG(BCM_DBG_COSQ,("%s Queue(%d) Full Params: queue_header_adjust_sign(%d) queue_header_adjust(%d) min_bufs(%d)\n", (add==TRUE)?"Adding":"Deleting", queue,
                            queue_header_adjust_sign, queue_header_adjust, min_bufs));
    BCM_DEBUG(BCM_DBG_COSQ,("%s Queue(%d) Full Params: gamma(%d) template(%d) gain(%d)\n", (add==TRUE)?"Adding":"Deleting", queue,
                            gamma, template, gain));
    ...
    if ( timeout == 0 ) {
        if ( add ) {
            BCM_ERR(("queue add failed\n"));
            return -1;
        } else {
            BCM_ERR(("queue delete failed\n"));
            return -1;
        }
    }
    ...
    /* address 22048: register QM_CONFIG1 total_min_bufs_allocated needs to be set */
    {
        ...
        if ( add ) {
            if (min_buffers_for_queue[unit][queue] != 0 ) {
                BCM_WARN(("WARNING: Found min_buffers_for_queue[%d][%d]!=0 before setting value (%d)\n", unit, queue, min_buffers_for_queue[unit][queue]));
            }
        ...
        if ( total_min_bufs_allocated < 0 ) {
            ...
            soc_cm_print("Found total_min_bufs_allocated<0.  Resetting to 0, but system may be unstable.\n");
            total_min_bufs_allocated = 0;
        } else {
            BCM_DEBUG(BCM_DBG_COSQ,("Setting total_min_bufs_allocated to %d\n",total_min_bufs_allocated));
        }
...
_bcm_qe2000_cosq_select_queue_params(int unit, int queue, bcm_sbx_cosq_queue_params_ingress_t *p_qparams,
                     ...
                     bcm_sbx_cosq_queue_region_type_t queue_region)
{
    uint32_t guaranteed_rate_in_kbps;
    uint32_t rate_in_256_kbits_integer;
    uint32_t rate_in_256_kbits_fraction;
    int32 bw_mode;
    int32 sp_priority;
    ...
    *p_rate_a = *p_rate_b = 0;
    ...
    switch ( bw_mode ) {
        ...
        case BCM_COSQ_SP_GLOBAL :
            {
                ...
                *p_qtype = uPriority;
                ...
                BCM_DEBUG(BCM_DBG_COSQ, ("bw mode PATH_SP_GLOBAL, priority set to 0x%x\n", *p_qtype));
        ...
        guaranteed_rate_in_kbps  = p_qparams->bw_value.guarantee_kbps;
                ...
                *p_rate_b = *p_rate_a;
                ...
                BCM_DEBUG(BCM_DBG_COSQ, ("AF set guarantee(%d)b/s integer(%d) per 256Kns fraction (%d) per 256Kns\n",
                     guaranteed_rate_in_kbps,
                     (int32_t)rate_in_256_kbits_integer,
                     (int32_t)rate_in_256_kbits_fraction));
            }
            /* TODO - need to set a fixed rate (in rate table) if we are standalone (standalone does not support sigma).
                - also, the rate will need to be removed from the logical port rate...so this configuration
                - add queue stage.  Better is when the logical port is updated due to the flow being enabled
                - at that time we need to remove the bandwidth of the flow from the logical port bandwidth
             */
            break;
        case BCM_COSQ_BE :
            *p_qtype = 0x0;
        ...
        default:
            BCM_DEBUG(BCM_DBG_COSQ, ("Unsupported bw_mode(%d) found during AddQueue operation\n", p_qparams->bw_mode));
            return -1;
    }
	...
	BCM_DEBUG(BCM_DBG_COSQ, ("hold_ts=0x%x\n", *p_hold_ts));
    }
...
_bcm_qe2000_cosq_set_queue_group_ingress_shaper(int unit,
    ...
    int32_t queue;
    uint32_t per_queue_shape_limit_kbps;
    bcm_sbx_cosq_queue_state_t *p_qstate;
    ...
    BCM_DEBUG(BCM_DBG_COSQ, ("Setting per queue shape(%d) based on port_shape(%d) and num_queues(%d)\n",
    ...
    for ( queue = base_queue;
        ...
        if ( status ) {
            ...
            BCM_ERR(("Failed to setup shaper for queue(%d)\n",queue));
            saved_status = status;
        }
...
_bcm_qe2000_cosq_set_ingress_shaper(int unit,
    ...
    uint32_t shape_max_burst_table_value = 0;
    uint32_t shape_rate_256_kbits = _bcm_qe2000_cosq_shape_burst_rate_256kbits_get(unit, shape_limit_kbps);
    ...
    if (enable_shaping ) {
        BCM_DEBUG(BCM_DBG_COSQ, ("Shaping parameter shape_rate_256_kbits (%d)\n", shape_rate_256_kbits));
        BCM_DEBUG(BCM_DBG_COSQ, ("Enable shaping, max burst (%d) bits\n", shape_max_burst_table_value));
    } else {
        BCM_DEBUG(BCM_DBG_COSQ, ("Disable shaping, max burst (%d) bits\n", shape_max_burst_table_value));
    }
...
static void _bcm_qe2000_rate_delta_max_index_for_port_rate_chosen(int unit, int bw_mode,
                                  uint32_t bag_rate_kbps,
                                  int32_t *prate_delta_max_index)
{
#if 0
    uint32_t uPercentOffset = 2; /* 17 percent */
#endif /* 0 */
    /*
...
_bcm_qe2000_cosq_enable_disable_queue(int unit, int queue, int enable)
{
    ...
    BCM_DEBUG(BCM_DBG_COSQ, ("Queue is (%s)\n", ((enable == TRUE) ? "ENABLED" : "DISABLED")));
    return(status);
}
...
bcm_qe2000_cosq_sysport_port_remap(int unit, int sysport, int ef, int remap_port)
{
    ...
    if ( ( remap_port < 0 ) ||
         ( remap_port > 49 ) ) {
        BCM_DEBUG(BCM_DBG_COSQ, ("Remap_port (%d) is out of range\n", remap_port));
        return BCM_E_PARAM;
    }
                            ...
                            bcm_sbx_cosq_control_t *control_info)
{
    ...
    switch (type) {
        ...
        case bcmCosqControlFabricConnectMax:
	    SOC_IF_ERROR_RETURN(bcm_sbx_cosq_get_base_queue_from_gport(unit, gport, &base_queue, &num_cos));
	    SOC_IF_ERROR_RETURN(soc_qe2000_queue_para_get(unit, base_queue+cosq, &local, &q_type, &hold_ts));
	    *value = hold_ts + 1;
	    break;
                                ...
                                uint32 bytes_max)
{
	...
	rc = bcm_qe2000_port_multicast_size_set(unit, cosq, bytes_min, bytes_max);
        ...
        BCM_ERR(("cos level out of range (0-3) valid value passed (%d)\n", cosq));
        rc = BCM_E_PARAM;
    }
				   ...
				   bcm_cosq_subscriber_map_t *map)
{
	...
	case BCM_COSQ_SUBSCRIBER_MAP_PORT_VLAN:
	    ...
	    if (rv != SB_ELIB_OK) {
		soc_cm_print("ERROR: showing vlan(%d) for %s[u:%d] failed\n", vlan,
	    ...
	    if (rv != SB_ELIB_OK) {
		soc_cm_print("ERROR: setting vlan(%d) for %s[u:%d] failed\n", vlan,
		       SOC_CHIP_STRING(unit),unit);
		rv = -1;
	    } else {
		rv = 0;
	    ...
	    if (rv != SB_ELIB_OK) {
		soc_cm_print("ERROR: showing port(%d) for %s[u:%d] failed\n", port,
			     SOC_CHIP_STRING(unit),unit);
		rv = -1;
	    } 
	    ...
	    if (rv != SB_ELIB_OK) {
		soc_cm_print("ERROR: setting port(%d) for %s[u:%d] failed\n", port,
			     SOC_CHIP_STRING(unit),unit);
		rv = -1;
	    } else {
		rv = 0;
...
bcm_qe2000_cosq_subscriber_map_delete_all(int unit)
{
    ...
    uint32_t nAddr, nBase, nEnd, nESize;
...
_bcm_qe2000_cosq_queue_burst_set(int unit, int base_queue, bcm_cos_queue_t cosq, int num_cos, int queue, int burst_rate_kbits, int is_burst_size_context)
{
    ...
    bcm_sbx_cosq_queue_state_t *p_qstate;
    uint32_t shape_rate_kbits, per_queue_shape_rate_kbits;
    uint32_t burst_rate_256_kbits;
    uint32_t shape_max_burst_table_value;
    int32_t cur_queue;
    ...
    if (SOC_SBX_CFG_QE2000(unit)->nQueuesPerShaperIngress == 1) { /* per queue shaping */
        ...
        if (rv != SOC_E_NONE) {
            BCM_ERR(("ERROR: Failed to configure Queue Burst Size, Queue: 0x%x\n", queue));
        }
    }
    else { /* queue group shaping */
        if (is_burst_size_context == TRUE) {
            if (cosq != 0) {
                BCM_ERR(("ERROR: Logical Queue Shaping Burst Size can be applied only to cos=0, cos=%d\n", cosq));
                rv = BCM_E_PARAM;
                return(rv);
            }
        }
