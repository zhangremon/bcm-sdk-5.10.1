/*
 * $Id: trunk.c 1.20.42.1 Broadcom SDK $
...
/* Debug Macros */
#define BCM_TRUNK_ERR(stuff)          BCM_DEBUG(BCM_DBG_TRUNK | BCM_DBG_ERR, stuff)
#define BCM_TRUNK_WARN(stuff)         BCM_DEBUG(BCM_DBG_TRUNK | BCM_DBG_WARN, stuff)
                                          ...
                                          bcm_module_t *fabric_mod, bcm_port_t *fabric_port)
{
    ...
    if (rc != BCM_E_NONE) {
        BCM_TRUNK_ERR(("Invalid Module 0x%x, Port: %d\n", mod, port));
        return(rc);
    }
...
_bcm_qe2000_trunk_debug(int unit)
{
    ...
    for (index = 0; index < tc->ngroups; index++) {
        ...
        if (ti->trunk_id == BCM_TRUNK_INVALID) {
            ...
            BCM_TRUNK_MSG(("      dlf index       : %d\n", ti->dlf_index));
            BCM_TRUNK_MSG(("      mc index        : %d\n", ti->mc_index));
            if (0 < ti->num_ports) {
                BCM_TRUNK_MSG(("         ports        : %d:%d", ti->tm[0], ti->tp[0]));
                for (jindex = 1; jindex < ti->num_ports; jindex++)
                    BCM_TRUNK_MSG((",  %d:%d", ti->tm[jindex], ti->tp[jindex]));
                BCM_TRUNK_MSG(("\n"));
            }
        }
    }
...
_bcm_qe2000_trunk_set(int unit, bcm_trunk_t tid, bcm_trunk_add_info_t *add_info)
{
    ...
    for (index = 0; index < add_info->num_ports; index++) {
        BCM_TRUNK_VVERB((" %d:%d", add_info->tm[index], add_info->tp[index]));
    }
    ...
    if (req_config->num_ports > BCM_TRUNK_MAX_PORTCNT) {
        BCM_TRUNK_ERR(("Invalid number of ports %d, Max Allowed: %d\n",
    ...
    for (index = 0; index < req_config->num_ports; index++) {
        ...
        if (!BCM_STK_MOD_IS_NODE(module)) {
            BCM_TRUNK_ERR(("Invalid Module 0x%x, Index: %d\n", module, index));
            result = BCM_E_PARAM;
            goto err;
        }
        ...
        if (port < 0) {
            BCM_TRUNK_ERR(("Invalid Port: 0x%x on Module: 0x%x, Index: %d\n",
                                                            port, module, index));
            result = BCM_E_PARAM;
            goto err;
        }
        ...
        if (result == BCM_E_NONE) {     /* mod:port was found to exist in a trunk */
            ...
            BCM_TRUNK_ERR(("error bcm_trunk_find module: 0x%x, port: 0x%x, Err(0x%x, %s)\n",
    ...
    if (removed.num_ports != 0) {
        BCM_TRUNK_VVERB(("Ports removed: [n=%d - ", removed.num_ports));
        for (index = 0; index < removed.num_ports; index++) {
            BCM_TRUNK_VVERB((" %d:%d", removed.tm[index], removed.tp[index]));
        }
        BCM_TRUNK_VVERB(("])\n"));
    }
    ...
    if (removed.num_ports != 0) {
        ...
        if (result != BCM_E_NONE) {
            BCM_TRUNK_ERR(("error bcm_vlan_list_by_pbmp unit: 0x%x, Err(0x%x, %s)\n",
        ...
        for (index = 0; index < count; index++) {
            ...
            if (result != BCM_E_NONE) {
                BCM_TRUNK_ERR(("error _bcm_qe2000_vlan_port_add unit: 0x%x, vlan: 0x%x, Err(0x%x, %s)\n",
                                unit, listp[index].vlan_tag, result, bcm_errmsg(result)));
                break;
            }
            else {
                SOC_PBMP_FMT(listp[index].port_bitmap, pfmt);
                BCM_TRUNK_VVERB(("\tTo VLAN %d, Ports %s added back.\n", listp[index].vlan_tag, pfmt));
            }
        }
    ...
    if (BCM_PBMP_NOT_NULL(aiPbmp)) {
        ...
        for (index = 0; index < count; index++) {
            ...
            if (BCM_PBMP_NOT_NULL(wPbmp)) {
                ...
                if (result != BCM_E_NONE) {
                    BCM_TRUNK_ERR(("error _bcm_qe2000_vlan_port_remove unit: 0x%x, vlan: 0x%x, Err(0x%x, %s)\n",
                                    unit, listp[index].vlan_tag, result, bcm_errmsg(result)));
                    break;
                }
                else {
                    SOC_PBMP_FMT(wPbmp, pfmt);
                    BCM_TRUNK_VVERB(("\tTo VLAN %d, Removed ports %s.\n", listp[index].vlan_tag, pfmt));
                }
    ...
    BCM_TRUNK_VVERB(("%s(%d, %d, *) - Exit(%d: %s)\n", 
...
/*
 *  Function
 *    bcm_qe2000_trunk_vlan_port_fixup
 * Purpose
...
extern int
bcm_qe2000_trunk_vlan_port_fixup(const int unit,
                                 ...
                                 const bcm_pbmp_t pbmp)
{
    ...
    BCM_TRUNK_VERB(("%s[%d] %s(%d, %d, *) - Enter\n",
                    __FILE__,
                    __LINE__,
                    FUNCTION_NAME(),
                    unit,
    ...
    BCM_PBMP_ITER(pbmp, port) {
        ...
        BCM_TRUNK_VVERB(("%s[%d] %s: found aggregate %d in VLAN %03X\n",
                         __FILE__,
                         __LINE__,
                         FUNCTION_NAME(),
                         tid,
        ...
        if (BCM_E_NONE != tempRes) {
            result = tempRes;
            BCM_TRUNK_ERR(("%s[%d] %s: unable to get unit %d trunk %d member"
            ...
            BCM_TRUNK_VVERB(("%s[%d] %s: using index %d as designate\n",
                             __FILE__,
                             __LINE__,
                             FUNCTION_NAME(),
                             desIndex));
            ...
            for (index = 0; index < ti.num_ports; index++) {
                ...
                BCM_TRUNK_VVERB(("%s[%d] %s: mod %d, port %d in aggregate "
                                 "(%s (%s%s\n",
                                 __FILE__,
                                 __LINE__,
                                 FUNCTION_NAME(),
            ...
            if (ti.tm[desIndex] == myModId) {
                ...
                BCM_TRUNK_VVERB(("%s[%d] %s: local designate index %d,"
    ...
    if (BCM_PBMP_NOT_NULL(pbmpRemove)) {
        ...
        if (BCM_E_NONE != tempRes) {
            result = tempRes;
            BCM_TRUNK_ERR(("%s[%d] %s: unable to remove aggregate non-designate"
    ...
    if (BCM_PBMP_NOT_NULL(pbmpAdd)) {
        ...
        if (BCM_E_NONE != tempRes) {
            result = tempRes;
            BCM_TRUNK_ERR(("%s[%d] %s: unable to add aggregate designate"
    ...
    BCM_TRUNK_VERB(("%s[%d] %s(%d, %d, *) - Exit(%s)\n",
                    __FILE__,
                    __LINE__,
                    FUNCTION_NAME(),
                    unit,
...
bcm_qe2000_trunk_psc_set(int unit, bcm_trunk_t tid, int psc)
{
    ...
    BCM_TRUNK_VERB(("%s(%d, %d, %d) - Exit(%d: %s)\n", 
...
bcm_qe2000_trunk_set(int unit, bcm_trunk_t tid, bcm_trunk_add_info_t *add_info)
{
    ...
    for (index = 0; index < add_info->num_ports; index++) {
        BCM_TRUNK_VERB((" %d:%d", add_info->tm[index], add_info->tp[index]));
    }
...
bcm_qe2000_trunk_get(int unit, bcm_trunk_t tid, bcm_trunk_add_info_t *t_data)
{
    ...
    for (index = 0; index < t_data->num_ports; index++) {
        BCM_TRUNK_VVERB((" %d:%d", t_data->tm[index], t_data->tp[index]));
    }
    BCM_TRUNK_VVERB(("]) - Exit(%d: %s)\n", result, bcm_errmsg(result)));
...
bcm_qe2000_trunk_destroy(int unit, bcm_trunk_t tid)
{
    ...
    if (ti->trunk_id != BCM_TRUNK_INVALID) {
        ...
        if (0 < ti->num_ports) {
            ...
            add_info.psc = tc->psc;
            result = bcm_trunk_set(unit, tid, &add_info);
        }
...
{
    BCM_TRUNK_VERB(("%s(%d, %d, %d, %02X:%02X:%02X:%02X:%02X:%02X) - Exit(%d: %s)\n", 
                      ...
                      bcm_trunk_t *tid)
{
    ...
    *tid = BCM_TRUNK_INVALID;
    ...
    BCM_TRUNK_VERB(("%s(%d, %d, %d, %d) - Exit(%d: %s)\n", 
...
{
    BCM_TRUNK_VERB(("%s(%d, %d, %d, %d, %d) - Exit(%d: %s)\n", 
