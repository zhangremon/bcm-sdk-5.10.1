...
error:
    ...
    if (rv) {
        soc_cm_debug(DK_ERR, "bcmTx: Error=%d %s\n", rv, msg);
    }
...
int
bcm_sbx_tx_pkt_l2_map(int unit, bcm_pkt_t *pkt, bcm_mac_t dest_mac, int vid)
{
	    ...
	    if (!(tx_pkt->flags & BCM_TX_SBX_READY)) {
	      bcm_tx_pkt_setup(unit, tx_pkt);
            ...
            if (tx_pkt->tot_len < (ENET_MIN_PKT_SIZE - ENET_FCS_SIZE)) {
                TX_WARN(("Packet too small: pktLen=%d min=%d, skipping\n",
...
_sbx_tx_thread(void *param)
{
    ...
    while(1) {
        ...
        if (sal_sem_take(pkt_tx_sem, sal_sem_FOREVER) < 0) {
            soc_cm_debug(DK_ERR, "TX thread error\n");
            break;
        }
        ...
        while (BCM_SUCCESS(rv)) {
            ...
            if (BCM_SUCCESS(rv) && entry) {
                soc_cm_debug(DK_TX | DK_VERBOSE, 
                             "tx done first_pkt=0x%08x cookie=0x%08x\n",
...
int
bcm_sbx_tx_dv_dump(int unit, void *dv_p)
{
    return BCM_E_UNAVAIL;
}
