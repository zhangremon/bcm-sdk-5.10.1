/*
 * $Id: cosq.c 1.279.2.9 Broadcom SDK $
...
static int
_bcm_sbx_cosq_fcd_free(int unit, int fcd);
                         ...
                         bcm_cos_queue_t cosq)
{
    ...
    if (rc != BCM_E_NONE) {
        BCM_DEBUG(BCM_DBG_COSQ, ("error(0x%x) cosq_mapping_set()\n", rc));
    }
...
int
bcm_sbx_cosq_port_mapping_set(int unit,
                              bcm_port_t port,
                              bcm_cos_t priority,
                              bcm_cos_queue_t cosq)
{
                         ...
                         bcm_cos_queue_t *cosq)
{
    ...
    if (cosq == NULL) {
        BCM_SBX_UNLOCK(unit);
        BCM_ERR(("ERROR: API error - cosq pointer not being passed\n"));
        return(BCM_E_PARAM);
    }

    rc = MBCM_SBX_DRIVER_CALL(unit, mbcm_cosq_mapping_get, unit, priority, cosq);
    if (rc != BCM_E_NONE) {
        BCM_DEBUG(BCM_DBG_COSQ, ("error(0x%x) cosq_mapping_get()\n", rc));
    }
...
int
bcm_sbx_cosq_port_mapping_get(int unit,
                              bcm_port_t port,
                              bcm_cos_t priority,
                              bcm_cos_queue_t *cosq)
{
...
int
bcm_sbx_cosq_sched_set(int unit,
                       int mode,
                       const int weights[],
                       int delay)
{
...
int
bcm_sbx_cosq_port_sched_set(int unit,
                            bcm_pbmp_t pbm,
                            int mode,
                            const int weights[],
                            int delay)
...
int
bcm_sbx_cosq_sched_get(int unit,
                       int *mode,
                       int weights[],
                       int *delay)
{
...
int
bcm_sbx_cosq_port_sched_get(int unit,
                            bcm_pbmp_t pbm,
                            int *mode,
                            int weights[],
                            int *delay)
...
int
bcm_sbx_cosq_sched_weight_max_get(int unit,
                                  int mode,
                                  int *weight_max)
{
    return BCM_E_UNAVAIL;
...
int
bcm_sbx_cosq_port_bandwidth_set(int unit,
                                bcm_port_t port,
                                bcm_cos_queue_t cosq,
...
int
bcm_sbx_cosq_port_bandwidth_get(int unit,
                                bcm_port_t port,
                                bcm_cos_queue_t cosq,
                                uint32 *kbits_sec_min,
                                uint32 *kbits_sec_max,
...
int
bcm_sbx_cosq_discard_set(int unit,
                         uint32 flags)
{
    return BCM_E_UNAVAIL;
}
...
int
bcm_sbx_cosq_discard_get(int unit,
                         uint32 *flags)
{
    return BCM_E_UNAVAIL;
}
...
int
bcm_sbx_cosq_discard_port_set(int unit,
                              bcm_port_t port,
                              bcm_cos_queue_t cosq,
                              uint32 color,
                              int drop_start,
...
int
bcm_sbx_cosq_discard_port_get(int unit,
                              bcm_port_t port,
                              bcm_cos_queue_t cosq,
                              uint32 color,
                              int *drop_start,
                       ...
                       bcm_gport_t *req_gport)
{
    ...
    int egress_group_requested = BCM_INT_SBX_REQUESTED_SCHEDULER_AUTO_SELECT;
    int sysport = -1;
    int queue = -1;
    ...
    BCM_DEBUG(BCM_DBG_COSQ, ("gport add request, unit(%d) physical_port(0x%x) num_cos_levels(%d) flags(0x%x) *req_gport(0x%x)\n",
    ...
    if (BCM_COSQ_GPORT_SCHEDULER & flags) {
        BCM_DEBUG(BCM_DBG_COSQ, ("allocate scheduler gport\n"));
        user_scheduler = TRUE;
    }

    if (BCM_COSQ_GPORT_EGRESS_GROUP & flags) {
	BCM_DEBUG(BCM_DBG_COSQ, ("allocate egress group gport\n"));
	user_egroup = TRUE;
    }

    if (BCM_COSQ_GPORT_MULTIPATH & flags) {
	BCM_DEBUG(BCM_DBG_COSQ, ("allocate multipath gport\n"));
	user_multipath = TRUE;
	if (flags & ~BCM_COSQ_GPORT_MULTIPATH) {
	    BCM_ERR(("ERROR: BCM_COSQ_GPORT_MULTIPATH can not be used with other flags\n"));
	    BCM_SBX_UNLOCK(unit);
	    return BCM_E_PARAM;
	}
    }
    ...
    if (!req_gport) {
        BCM_ERR(("ERROR: user needs to provide pointer to req_gport to be filled with gport info\n"));
        BCM_SBX_UNLOCK(unit);
        return BCM_E_PARAM;
    }
	...
	if ((BCM_COSQ_GPORT_WITHOUT_SHAPING & flags) &&
	    (BCM_COSQ_GPORT_WITH_SHAPING & flags)) {
	    BCM_ERR(("ERROR: Can not set both BCM_COSQ_GPORT_WITH_SHAPING/BCM_COSQ_GPORT_WITHOUT_SHAPING flag\n"));
	    BCM_SBX_UNLOCK(unit);
	    return BCM_E_PARAM;		
	}	

	if (!SOC_IS_SIRIUS(unit)) {
	    BCM_ERR(("ERROR: BCM_COSQ_GPORT_WITH_SHAPING/BCM_COSQ_GPORT_WITHOUT_SHAPING flags not supported on unit %d\n", unit));
	    BCM_SBX_UNLOCK(unit);
	    return BCM_E_PARAM;
	}
	    ...
	    BCM_ERR(("ERROR: all FIC queues has ingress shaper, BCM_COSQ_GPORT_WITHOUT_SHAPING flags not supported under fic mode %d\n", unit));
	    BCM_SBX_UNLOCK(unit);
	    return BCM_E_PARAM;
	}
	...
	if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: failed to get local queue shaper increment config on unit %d\n", unit));
	    BCM_SBX_UNLOCK(unit);
	    return BCM_E_PARAM;
	} else if (shaper_inc != 1) {
	    BCM_ERR(("ERROR: BCM_COSQ_GPORT_WITH_SHAPING/BCM_COSQ_GPORT_WITHOUT_SHAPING requires local queue"
	...
	if (soc_feature(unit, soc_feature_hybrid) ||
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_ERR(("ERROR: failed to get voq queue shaper increment config on unit %d\n", unit));
		BCM_SBX_UNLOCK(unit);
		return BCM_E_PARAM;
	    } else if (shaper_inc != 1) {
		BCM_ERR(("ERROR: BCM_COSQ_GPORT_WITH_SHAPING/BCM_COSQ_GPORT_WITHOUT_SHAPING requires voq queue"
			 "shaper increment be 1 on unit %d\n", unit));
		BCM_SBX_UNLOCK(unit);
		return BCM_E_PARAM;		
	    }
	    ...
	    BCM_ERR(("ERROR: BCM_COSQ_GPORT_WITH_SHAPING/BCM_COSQ_GPORT_WITHOUT_SHAPING flags not supported with other flags %d\n", unit));
	    BCM_SBX_UNLOCK(unit);
	    return BCM_E_PARAM;
	}
...
#ifdef BCM_EASY_RELOAD_SUPPORT
        if (!(BCM_COSQ_GPORT_WITH_ID & flags)) {
            BCM_ERR(("ERROR: user needs to provide gport with ID if reloading (in easy_reload mode)\n"));
            BCM_SBX_UNLOCK(unit);
            return BCM_E_PARAM;
        }
#endif
    ...
    if (BCM_COSQ_GPORT_OVERLAY & flags) {
        ...
        if (user_allocated_req_gport != TRUE) {
            BCM_ERR(("ERROR: GPORT Overlay requires User-defined (predefined) QID to be passed in\n"));
            BCM_SBX_UNLOCK(unit);
            return BCM_E_PARAM;
        } else {
            BCM_DEBUG(BCM_DBG_COSQ, ("user qid overlay\n"));
	...
	if (BCM_GPORT_IS_MODPORT(physical_port)) {
	    ...
	    BCM_ERR(("ERROR: Invalid physical port for multipath\n"));
	    BCM_SBX_UNLOCK(unit);
	    return BCM_E_PARAM;	    
	}

	if (!SOC_IS_SIRIUS(unit)) {
	    BCM_ERR(("ERROR: multipath gport not supported on unit %d\n", unit));
	    BCM_SBX_UNLOCK(unit);
	    return BCM_E_UNAVAIL;
	}
	...
	if (BCM_GPORT_IS_MODPORT(physical_port)) {
	    ...
	    BCM_ERR(("ERROR: Invalid physical port for scheduler\n"));
	    BCM_SBX_UNLOCK(unit);
	    return BCM_E_PARAM;	    
	}

	if (!SOC_IS_SIRIUS(unit)) {
	    BCM_ERR(("ERROR: scheduler gport not supported on unit %d\n", unit));
	    BCM_SBX_UNLOCK(unit);
	    return BCM_E_UNAVAIL;
	}
	...
	if (BCM_COSQ_GPORT_WITH_ID & flags) {
		...
		if (SOC_SBX_CFG_SIRIUS(unit)->nNodeUserManagementMode < 0) {
		    ...
		    rv = BCM_E_PARAM;
		    BCM_ERR(("ERROR: SDK managed scheduler resource automatically before,"
		...
		rv = MBCM_SBX_DRIVER_CALL(unit, mbcm_cosq_scheduler_allocate, unit, level, node,
					  1, egress, &scheduler);
		if (rv != BCM_E_NONE) {
		    BCM_ERR(("ERROR: scheduler allocate 0x%x failed error(%d)\n", *req_gport, rv));
		}
		BCM_SBX_UNLOCK(unit);
		return rv;
	    } else {
		rv = BCM_E_PARAM;
		BCM_ERR(("ERROR: If user wants to manage scheduler resource directly, has to specify a"
		...
		if (SOC_SBX_CFG_SIRIUS(unit)->nNodeUserManagementMode < 0) {
		    ...
		    rv = BCM_E_PARAM;
		    BCM_ERR(("ERROR: User managed scheduler resource directly before,"
	...
	if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: Could not allocate scheduler rv %d\n", rv));
	    BCM_SBX_UNLOCK(unit);
	    return rv;
	}
	...
	if ((egress == FALSE) && (num_cos_levels > 0) && 
		...
		BCM_WARN(("Could not preallocate more than 8 children ingress schedulers, default to 8\n"));
		num_cos_levels = 8;
	...
	*req_gport = scheduler;
	...
	if (BCM_GPORT_IS_CHILD(physical_port)) {
	    ...
	    BCM_ERR(("ERROR: Unsupported physical gport when creating egress group\n"));
	    BCM_SBX_UNLOCK(unit);
	    return BCM_E_PARAM;	    
	}	

	if ((SOC_SBX_CFG(unit)->fabric_egress_setup) && (egress == FALSE)) {
	    BCM_DEBUG(BCM_DBG_COSQ, ("Egress groups unavailable when fabric_egress_setup=1\n"));
	    return BCM_E_UNAVAIL;
	}
	
	/* Make sure egress group handle returned is similiar and consistent with fabric ports */
	/* For Polaris, user passes egress_group gport from sirius */
	if (SOC_IS_SBX_BME(unit) && user_allocated_req_gport) {
	    egress_group_requested = BCM_GPORT_EGRESS_GROUP_GET(*req_gport);
	} else {
	    egress_group_requested = BCM_INT_SBX_REQUESTED_SCHEDULER_AUTO_SELECT;
	}
	...
	if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: Could not allocate egress group\n"));
	    BCM_SBX_UNLOCK(unit);
	    return rv;
	}
	...
	*req_gport = egress_group;
	...
	port = BCM_GPORT_MODPORT_PORT_GET(physical_port);
        ...
        if (modid < BCM_MODULE_FABRIC_BASE) {
            BCM_ERR(("ERROR: gport modid invalid(%d) minimum (%d)\n", modid, BCM_MODULE_FABRIC_BASE));
            rv = BCM_E_PARAM;
            BCM_SBX_UNLOCK(unit);
            return rv;
        }
	...
	if (node < BCM_STK_MAX_MODULES) {
	    protocol = SOC_SBX_STATE(unit)->stack_state->protocol[node];
            ...
            BCM_ERR(("ERROR: unknown node type for modid(%d)\n", modid));
            rv = BCM_E_INIT;
            BCM_SBX_UNLOCK(unit);
            return rv;
	}
	...
	if ( (protocol == bcmModuleProtocol3) || (protocol == bcmModuleProtocol4) || (protocol == bcmModuleProtocol5)) {
		...
		BCM_ERR(("ERROR: gport type MODPORT, must be cpu port\n"));
		BCM_SBX_UNLOCK(unit);
		return BCM_E_PARAM;
	    }
	} else {
	    ...
	    if (fabric_port < 0) {
		BCM_ERR(("ERROR: gport type MODPORT, port field invalid\n"));
		BCM_SBX_UNLOCK(unit);
		return BCM_E_PARAM;
	    }
	}

        BCM_DEBUG(BCM_DBG_COSQ, ("fabric_port=%d\n", fabric_port));
        ...
        /* TODO: don't assume fabric port is the same as the front panel port */
	fabric_port = -1;
    ...
    if ((BCM_COSQ_GPORT_LOCAL & flags) ||
        soc_feature(unit, soc_feature_standalone)) {
        BCM_DEBUG(BCM_DBG_COSQ, ("locally switched queue\n"));
	...
	if (SOC_IS_SBX_QE2000(unit) || SOC_IS_SBX_SIRIUS(unit)) {
	  ...
	  if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR (%s) getting modid for unit:%d\n",bcm_errmsg(rv),unit));
	    BCM_SBX_UNLOCK(unit);
	    return(rv);
	  }
    ...
    /* TODO: Add consistency checks on other gport types and also base it on Fabric Mode */
    if (BCM_GPORT_IS_MODPORT(physical_port)) {
        if (queue_region == bcm_sbx_cosq_queue_region_global) {
            ...
            if (num_cos_levels > max_cos) {
                ...
                BCM_ERR(("ERROR:invalid queue group size(%d) max supported(%d)\n",
            ...
            if (num_cos_levels > max_cos) {
		       ...
		       ((num_cos_levels % SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_HW_SIZE) != 0)) {
                ...
                BCM_ERR(("ERROR:invalid queue group size(%d)\n",(int)num_cos_levels));
                return(rv);
            }
        }
    }
    ...
    if (user_allocated_req_gport) {
      ...
      if (queue_region == bcm_sbx_cosq_queue_region_global) {
        ...
        BCM_DEBUG(BCM_DBG_COSQ, ("sysport=%d\n", sysport_requested));
        ...
        if (SOC_IS_RELOADING(unit) && (SOC_SBX_CFG(unit)->uFabricConfig != SOC_SBX_SYSTEM_CFG_DMODE)) {
            ...
            if (sysport_requested == BCM_INT_SBX_REQUESTED_SYSPORT_AUTO_SELECT) {
                rv = BCM_E_PARAM;
                BCM_ERR(("User must specify SYSPORT with ID if EASY_RELOAD and reloading software\n"));
                BCM_SBX_UNLOCK(unit);
                return rv;
            }
#endif
      ...
      BCM_DEBUG(BCM_DBG_COSQ, ("base_queue_requested=%d\n", base_queue_requested));

        if (base_queue_requested > SOC_SBX_CFG(unit)->num_queues) {

            BCM_ERR(("Queue(%d) requested is out of range (0-%d)\n",
    ...
    if (user_gport_overlay ) {
        ...
        BCM_DEBUG(BCM_DBG_COSQ, ("Gport Overlay 0x%x found num_cos 0x%x sysport 0x%x\n",
	...
	if (dp_state[unit] != NULL) {
        ...
        BCM_DEBUG(BCM_DBG_COSQ, ("Gport overlay sysport allocate sysport_requested %d Node %d returned sysport 0x%x\n",
                                 sysport_requested, node, sysport));

        if (rv != BCM_E_NONE) {
            BCM_ERR(("Could not allocate sysport\n"));
            BCM_SBX_UNLOCK(unit);
            return rv;
        }
        ...
        if (num_cos_levels > p_bwstate_in->num_cos) {
            BCM_ERR((" cos overlay count request(%d) exceeds allocated group size (%d)\n",
	  ...
	  if (queue_region == bcm_sbx_cosq_queue_region_global) {
        ...
        BCM_DEBUG(BCM_DBG_COSQ, ("Setting overlay traffic of size %d at 0x%x. Original Base at 0x%x\n",
        ...
        for ( queue=base_queue; queue < base_queue+num_cos_levels; queue++ ) {
            ...
            BCM_DEBUG(BCM_DBG_COSQ, ("Overlay queue %d on Unit %d switch to sysport %d\n",
                                     queue, unit, sysport));
            ...
            if (rv != BCM_E_NONE) {
                BCM_ERR(("ERROR: overlay queue(%d) failed error(%d)\n", queue, rv));
                BCM_SBX_UNLOCK(unit);
                return rv;
            }
        }

        BCM_DEBUG(BCM_DBG_COSQ, ("Overlay Node %d Fabric port %d mc %d num_cos %d base_queue 0x%x dest_type 0x%x\n",
        ...
        BCM_DEBUG(BCM_DBG_COSQ, ("**** DEBUG Overlay Ret GPORT 0x%x SysPort 0x%x base_queue 0x%x\n", *req_gport, sysport, base_queue));
	...
	if (BCM_COSQ_GPORT_WITHOUT_SHAPING & flags) {
        ...
        if (rv < 0) {
            BCM_ERR(("ERROR: queue group resource unavailable\n"));
            BCM_SBX_UNLOCK(unit);
            return BCM_E_RESOURCE;
        }
        BCM_DEBUG(BCM_DBG_COSQ, ("base_queue(%d) sysport(%d) dest node(%d) dest qe port(%d)\n",
	...
	p_bwstate->dest_port_kbits_sec_max = SOC_SBX_CFG(unit)->bcm_cosq_all_min_bw_kbps;
        ...
        for ( cos = 0; cos < num_cos_levels; cos++) {
            BCM_DEBUG(BCM_DBG_COSQ, ("gport add unicast queue(%d) for destination node(%d) fabric_port(%d) cos(%d)\n", queue, node, fabric_port, cos));
            ...
            if (rv != BCM_E_NONE) {
                BCM_ERR(("ERROR: add queue(%d) failed error(%d)\n", queue, rv));
                BCM_SBX_UNLOCK(unit);
                return rv;
            }
		...
		if (rv != BCM_E_NONE) {
		    BCM_ERR(("ERROR: queue size set(%d) failed error(%d)\n", queue, rv));
		    BCM_SBX_UNLOCK(unit);		    
		    return rv;
		}		
	    }
	    ...
	    if (BCM_GPORT_IS_EGRESS_GROUP(physical_port)) {
		rv = bcm_sbx_cosq_egress_group_info_get(unit, physical_port, &fabric_port, &eg_n, &num_fifos);
		if (rv != BCM_E_NONE) {
		...
		if (BCM_GPORT_IS_EGRESS_CHILD(physical_port)) {
		    modid = BCM_GPORT_EGRESS_CHILD_MODID_GET(physical_port);
		} else {
		    modid = BCM_GPORT_CHILD_MODID_GET(physical_port);
		}
		...
		if (modid == my_modid) {
			...
			if (sp_info[fabric_port].egroup[0].ef_fcd < SB_FAB_DEVICE_SIRIUS_MAX_FC_DOMAINS) {
			    ...
			    BCM_ERR(("ERROR: %s, Egress Group not initialized for fabric_port %d, unit %d\n",
            ...
            rv = MBCM_SBX_DRIVER_MAYBE_CALL(unit,
                                            ...
                                            (unit,
                                             ...
                                             fifo_idx));
            if (BCM_E_NONE != rv) {
                BCM_ERR(("Unable to set target for unit %d port %08X module %d"
                       ...
                       uint32 *flags)
{
    ...
    BCM_DEBUG(BCM_DBG_COSQ, ("gport get request, unit(%d) gport(0x%x) *physical_port(%d) *num_cos_levels(0x%x)\n",
    ...
    if ((!physical_port) || (!num_cos_levels) || (!flags)) {
        BCM_ERR(("ERROR: API error - required pointers not being passed\n"));
        BCM_SBX_UNLOCK(unit);
        return BCM_E_PARAM;
    }
    ...
    *num_cos_levels = -1;
    ...
    if (p_qstate->state != BCM_INT_SBX_QUEUE_STATE_IN_USE) {
        BCM_WARN(("gport(0x%x) base_queue(%d) not in use\n", gport, base_queue));
        rv = BCM_E_PARAM;
        BCM_SBX_UNLOCK(unit);
        return rv;
    }
    ...
    *flags = p_bwstate->flags;
    ...
    BCM_DEBUG(BCM_DBG_COSQ, ("gport(0x%x) base_queue(%d) num_cos_levels(%d) node(%d) port(%d)\n",
                                     ...
                                     bcm_gport_t *req_gport)
{
    ...
    BCM_DEBUG(BCM_DBG_COSQ, ("gport fabric distribution add unit(%d) ds_id(0x%x) num_cos_levels(%d) flags(0x%x) *req_gport(0x%x)\n",
...
/*     } */
    ...
    if (!req_gport) {
        BCM_ERR(("ERROR: user needs to provide pointer to req_gport\n"));
        BCM_SBX_UNLOCK(unit);
        return BCM_E_PARAM;
    }
    ...
    /* Decode flags */
    if (BCM_COSQ_GPORT_WITH_ID & flags) {
        BCM_DEBUG(BCM_DBG_COSQ, ("user allocated base queue\n"));
        user_allocated_req_gport = TRUE;
    }
	...
	if (BCM_GPORT_IS_MCAST_SUBSCRIBER_QUEUE_GROUP(*req_gport)) {
	  ...
	  base_queue_requested = BCM_GPORT_MCAST_QUEUE_GROUP_QID_GET(*req_gport);
        ...
        if (base_queue_requested < 0) {
            BCM_ERR(("ERROR: req_gport value invalid\n"));
            BCM_SBX_UNLOCK(unit);
            return BCM_E_PARAM;
        }
    }
    ...
    for (cos = 0 ; cos < num_cos_levels; cos++) {
        ...
        BCM_DEBUG(BCM_DBG_COSQ, ("gport add allocate multicast queue(%d) for eset(%d) destination node(%d) port(%d) cos(%d)\n", queue, eset, node, port, cos));
                                     ...
                                     uint32 *flags)
{
    ...
    if ((!req_gport) || (!num_cos_levels) || (!flags)) {
        BCM_ERR(("ERROR: parameter error - required pointers not being passed\n"));
        BCM_SBX_UNLOCK(unit);
        return BCM_E_PARAM;
    }
    if (ds_id >= SOC_SBX_CFG(unit)->num_ds_ids) {
        BCM_ERR(("ERROR: parameter error, ds_id(%d) out of valid range (0-%d)\n", ds_id,
    ...
    *num_cos_levels = -1;
    ...
    if (base_queue == -1) {
        BCM_ERR(("Queue not allocated for this distribution_group(%d)\n", ds_id));
        BCM_SBX_UNLOCK(unit);
        return BCM_E_PARAM;
    }
    ...
    *flags = bw_group_state[unit][bw_group].flags;
    ...
    BCM_DEBUG(BCM_DBG_COSQ, ("gport(0x%x) base_queue(%d) num_cos_levels(%d) ds_id(%d)\n",
                          ...
                          bcm_gport_t gport)
{
    ...
    if (BCM_GPORT_IS_UCAST_QUEUE_GROUP(gport)) {
        ...
        if (rv != BCM_E_NONE) {
            BCM_ERR(("delete %s scheduler gport(0x%x) failed\n", (egress?"egress":"ingress"), gport));
        }
        BCM_SBX_UNLOCK(unit);
        return rv;
	...
	rv = bcm_sbx_cosq_egress_group_free(unit, gport);
        if (rv != BCM_E_NONE) {
            BCM_ERR(("delete egress group 0x%x failed\n", gport));
        }
        BCM_SBX_UNLOCK(unit);
        return rv;    
        ...
        if (rv != BCM_E_NONE) {
            BCM_ERR(("delete %s multipath gport(0x%x) failed\n", (egress?"egress":"ingress"), gport));
        }
        BCM_SBX_UNLOCK(unit);
        return rv;	
    } else {
    ...
    if (p_bwstate->in_use) {
        ...
        if(p_bwstate->overlay_in_use) {
            if (base_queue == p_bwstate->overlay_base_queue) {
                ...
                /*  we should convert all queues back to best effort,
                 *  unmap and deallocate the sysport for the overlay gport,
                 *  and change the sysport of overlay queues to orignial
                 *  sysport.
                 *  TODO: convert all queues back to best effort.
                 *    are we sure we need to do this?
                 */
                num_cos = p_bwstate->overlay_num_cos;
                if ( (num_cos < 0) || (num_cos > p_bwstate->num_cos) ) {
                    BCM_ERR(("delete overlay gport(0x%x) failed\n", gport));
                    BCM_SBX_UNLOCK(unit);
                ...
                for (queue = base_queue; queue<base_queue+num_cos; queue++) {
                    ...
                    if (rv != BCM_E_NONE) {
                        BCM_ERR(("delete overlay queue(%d) failed\n", queue));
                        BCM_SBX_UNLOCK(unit);
                        return BCM_E_INTERNAL;
                    }
                ...
                /* original gport */
                BCM_ERR(("Overlay queue exists base queue(%d) num_cos(%d). Delete overlay gport first\n",
            ...
            BCM_DEBUG(BCM_DBG_COSQ, ("delete gport(0x%x) base_queue(%d) num_cos_levels(%d)\n",
                                     gport, base_queue, num_cos));
        }
    } else {
        BCM_ERR(("ERROR: bw_group(%d) not in use\n", bw_group));
        rv = BCM_E_RESOURCE;
    }
                            ...
                            void *user_data)
{
	...
	if (queue_region == bcm_sbx_cosq_queue_region_global)
      ...
      if (cmd == BCM_SBX_COSQ_GPORT_DELETE) {
        ...
        if (*p_gport != BCM_GPORT_INVALID && p_bwstate->cosq_init_added == FALSE) {
          ...
          BCM_DEBUG(BCM_DBG_COSQ,("deleting gport(0x%x) base_queue(%d) sysport(%d)\n",*p_gport,base_queue,sys_port));
          rv = cb(unit,0/*port*/,0/*numq*/,0/*flags*/,*p_gport,0);
          if (BCM_FAILURE(rv)) {
            soc_cm_print("Deleting gport(0x%x) failed (%s)\n",*p_gport,
                         bcm_errmsg(rv));
            retval = rv;
          }
        } else {
          BCM_DEBUG(BCM_DBG_COSQ,("skipping gport(0x%x) queue(%d) sysport(%d) - (%s)\n",
  ...
  if (bQueuesInUse == FALSE) {
    BCM_DEBUG(BCM_DBG_COSQ,("%s[u:%d] has no queues in use.\n",
                            ...
                            uint32 bytes_max)
{
	...
	BCM_GPORT_IS_EGRESS_GROUP(gport)) {
        rv = bcm_sbx_stk_modid_get(unit, &my_modid);
	if (rv != BCM_E_NONE) {
            BCM_DEBUG(BCM_DBG_COSQ, ("gport size set, error getting modid for unit\n", unit));
	    BCM_SBX_UNLOCK(unit);
	    return rv;
	}
	...
	if (BCM_GPORT_IS_EGRESS_MODPORT(gport)) {
		...
		BCM_ERR(("ERROR: %s, egress group gport 0x%x not valid, unit %d\n",
	    ...
	    BCM_ERR(("Unknown port derived from gport (0x%x)\n", gport));
	    BCM_SBX_UNLOCK(unit);
	    return BCM_E_PARAM;
	}
        ...
        if (rv != BCM_E_NONE) {
            BCM_DEBUG(BCM_DBG_COSQ, ("gport size set, error setting Egress Size\n"));
        }
        BCM_SBX_UNLOCK(unit);
        return(rv);
    ...
    BCM_DEBUG(BCM_DBG_COSQ, ("gport size set queue(%d) cos(%d)\n", queue, cosq));
	...
	(bytes_min != 0)) {
	BCM_ERR(("ERROR: %s, Unit(%d) Cos %d has PFC feature enabled, queue MinThreshold must be 0.\n",
	...
	for (color=0; color < BCM_SBX_COSQ_DISCARD_MAX_COLORS; color++) {
		...
		BCM_ERR(("ERROR: %s, Unit(%d) Invalid max size 0x%x conflicts with discard template color 0x%x: "
    ...
    rv = MBCM_SBX_DRIVER_CALL(unit, mbcm_cosq_set_ingress_params,unit,
			      ...
			      &sizeInfo);
    if (rv != BCM_E_NONE) {
	BCM_DEBUG(BCM_DBG_COSQ, ("gport size set, error setting ingress params\n"));
    }
                            ...
                            uint32 *bytes_max)
{
	...
	if (BCM_GPORT_IS_EGRESS_MODPORT(gport)) {
        ...
        rv = MBCM_SBX_DRIVER_CALL(unit, mbcm_cosq_egress_size_get, unit,
                                           gport, cosq, bytes_min, bytes_max);
        if (rv != BCM_E_NONE) {
            BCM_DEBUG(BCM_DBG_COSQ, ("gport size get, error getting Egress Size\n"));
        }
    ...
    *bytes_max = queue_state[unit][queue].ingress.max_physical_queue_depth_bytes;
    ...
    BCM_DEBUG(BCM_DBG_COSQ, ("gport(0x%x) queue(%d) cosq(%d) min depth bytes(%d) max depth bytes(%d)\n",
                                 ...
                                 uint32 flags)
{
	...
	if (scheduler == TRUE) {
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_DEBUG(BCM_DBG_COSQ, ("%s: can not find ingress scheduler for gport 0x%x on unit %d\n",
					 FUNCTION_NAME(), gport, unit));
		BCM_SBX_UNLOCK(unit);
		return rv;	    
	    }
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_DEBUG(BCM_DBG_COSQ, ("%s, error setting ingress params\n",
					 FUNCTION_NAME()));
		BCM_SBX_UNLOCK(unit);
		return rv;	    
	    }
	    ...
	    BCM_DEBUG(BCM_DBG_COSQ, ("gport bandwidth set queue(%d) base(%d) cos(%d)\n", queue, base_queue, cosq));
		...
		if (rv != BCM_E_NONE) {
		    BCM_DEBUG(BCM_DBG_COSQ, ("gport bandwidth set, error setting ingress shaper\n"));
		    BCM_SBX_UNLOCK(unit);
		    return rv;	    
		}
	    } else {
		...
		if (flags & BCM_COSQ_ALL) {
		    ...
		    rv = MBCM_SBX_DRIVER_CALL(unit, mbcm_cosq_set_ingress_params, unit,
					      ...
					      NULL);
		    if (rv != BCM_E_NONE) {
			BCM_DEBUG(BCM_DBG_COSQ, ("gport bandwidth set, error setting ingress params\n"));
		    }
		}
	    }
	...
	if ( BCM_GPORT_IS_EGRESS_GROUP(gport) ) {
	    rv = bcm_sbx_port_get_scheduler(unit, gport, &level, &node);	    
	    if (rv != BCM_E_NONE) {
		BCM_DEBUG(BCM_DBG_COSQ, ("gport bandwidth set, unsupported gport on unit %d\n", unit));
		BCM_SBX_UNLOCK(unit);
		return rv;	    
	    }
		...
		BCM_ERR(("ERROR: fifo scheduler cos %d unknown\n", cosq));
		rv = BCM_E_PARAM;
		BCM_SBX_UNLOCK(unit);
		return rv;	
	    }
	...
	if (level == SB_FAB_DEVICE_GROUP_SHAPER_LEVEL) {
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_DEBUG(BCM_DBG_COSQ, ("gport bandwidth set, error setting egress max shaper params\n"));
	    }
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_DEBUG(BCM_DBG_COSQ, ("gport bandwidth set, error setting egress min shaper params\n"));
	    }
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_DEBUG(BCM_DBG_COSQ, ("gport bandwidth set, error setting egress params\n"));
	    }
	}
    }
                                 ...
                                 uint32 *flags)
{
    ...
    *kbits_sec_max = -1;
	...
	if (scheduler == TRUE) {
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_DEBUG(BCM_DBG_COSQ, ("%s, error getting ingress params\n",
					 FUNCTION_NAME()));
		BCM_SBX_UNLOCK(unit);
		return rv;	    
	    }
		...
		*flags = BCM_COSQ_ALL;
	    ...
	    BCM_DEBUG(BCM_DBG_COSQ, ("gport(0x%x) base_queue(%d) cosq(%d) queue(%d) kbits_sec_min(%d) kbits_sec_max(%d)\n",
	...
	if (rv != BCM_E_NONE) {
	    BCM_DEBUG(BCM_DBG_COSQ, ("gport bandwidth get, unsupported gport on unit %d\n", unit));
	    rv = BCM_E_PARAM;
	    BCM_SBX_UNLOCK(unit);
	    return rv;	    
	}
	...
	*flags = 0;
	...
	if (level == SB_FAB_DEVICE_GROUP_SHAPER_LEVEL) {
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_DEBUG(BCM_DBG_COSQ, ("gport bandwidth get, error getting egress max shaper params\n"));
	    }
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_DEBUG(BCM_DBG_COSQ, ("gport bandwidth get, error getting egress min shaper params\n"));
	    }
	} else {
	    rv = MBCM_SBX_DRIVER_CALL(unit, mbcm_cosq_get_egress_shaper_params, unit,
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_DEBUG(BCM_DBG_COSQ, ("gport bandwidth get, error getting egress params\n"));
	    }
	}
    }
                             ...
                             int weight)
{
	...
	if (scheduler == TRUE) {
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_DEBUG(BCM_DBG_COSQ, ("gport sched set, unsupported gport on unit %d\n", unit));
		rv = BCM_E_PARAM;
		BCM_SBX_UNLOCK(unit);
		return rv;	    
	    }
		...
		return rv;		
            ...
            switch (SOC_SBX_CFG(unit)->node_template_id) {
		        ...
		        case BCM_COSQ_NONE:
		            break;
		        default:
		            BCM_ERR(("ERROR: unsupported mode value for ingress schedulers(%d)\n",
			     mode));
		            rv = BCM_E_PARAM;
		            BCM_SBX_UNLOCK(unit);
		            return rv;
		        ...
		        default:
			     ...
			     mode));
                    ...
                    break;
                default:
                    BCM_ERR(("ERROR: unsupported local scheduling template value (%d)\n",
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_DEBUG(BCM_DBG_COSQ, ("gport sched set, error setting ingress params\n"));
	    }
		...
		BCM_ERR(("ERROR:invalid cosq value(%d) only 0-7 currently supported\n", (int)cosq));
		rv = BCM_E_PARAM;
		BCM_SBX_UNLOCK(unit);
		return rv;
	    }
	    ...
	    if (cosq >= num_cos) {
		BCM_ERR(("ERROR:invalid cosq value(%d) outside of num_cos_levels(%d)\n", (int)cosq, num_cos));
		rv = BCM_E_PARAM;
		BCM_SBX_UNLOCK(unit);
		return rv;
	    }
		...
		switch (SOC_SBX_CFG(unit)->local_template_id) {
			...
			switch (mode) {
			    ...
			    case BCM_COSQ_AF2:
			    case BCM_COSQ_BE:
				BCM_DEBUG(BCM_DBG_COSQ, ("mode(%d)\n", mode));
				break;
			    default:
				BCM_ERR(("ERROR: unsupported mode value (%d)\n", mode));
				rv = BCM_E_PARAM;
				BCM_SBX_UNLOCK(unit);
				return rv;
			}
	    ...
	    BCM_DEBUG(BCM_DBG_COSQ, ("gport sched set queue(%d) base(%d) cos(%d)\n", queue, base_queue, cosq));
	...
	if (rv != BCM_E_NONE) {
	    BCM_DEBUG(BCM_DBG_COSQ, ("gport sched set, error in determinhing egress external/internl ports, unit %d\n", unit));
	    BCM_SBX_UNLOCK(unit);
	    return(rv);	    
	}
	    ...
	    switch (mode) {
	        ...
	        case BCM_COSQ_WEIGHTED_FAIR_QUEUING:
                case BCM_COSQ_EF:
                    break;
	        default:
		    BCM_ERR(("ERROR: unsupported mode value for egress schedulers(%d)\n",
			 mode));
		    rv = BCM_E_PARAM;
		    BCM_SBX_UNLOCK(unit);
		    return rv;
	    ...
	    if (rv != BCM_E_NONE) {
	        BCM_DEBUG(BCM_DBG_COSQ, ("gport sched set, error setting egress params\n"));
	    }
        }
        else {
	    switch (mode) {
	    ...
	    if (rv != BCM_E_NONE) {
	        BCM_DEBUG(BCM_DBG_COSQ, ("gport sched set, error in determinhing egress port configuration, unit %d\n", unit));
	        BCM_SBX_UNLOCK(unit);
	        return(rv);	    
	    }
	    ...
	    if ( BCM_GPORT_IS_EGRESS_GROUP(gport) ) {
	        ...
	        if (rv != BCM_E_NONE) {
	            BCM_DEBUG(BCM_DBG_COSQ, ("gport sched set, error in determinhing egress group configuration, unit %d\n", unit));
	            BCM_SBX_UNLOCK(unit);
	            return(rv);	    
	        }
            }
	        ...
	        if (rv != BCM_E_NONE) {
	            BCM_DEBUG(BCM_DBG_COSQ, ("gport sched set, error setting egress multicast params\n"));
	        }
            }
        }
    }
                             ...
                             int *weight)
{
    ...
    *weight = -1;
	...
	if (scheduler == TRUE) {
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_DEBUG(BCM_DBG_COSQ, ("%s, unsupported gport on unit %d\n", FUNCTION_NAME(), unit));
		rv = BCM_E_PARAM;
		BCM_SBX_UNLOCK(unit);
		return rv;	    
	    }
	    ...
	    if ( (level < 0) || (node < 0) ) {
		BCM_ERR(("ERROR: %s, unattached scheduler (0x%x), please attach first\n",
			 FUNCTION_NAME(), gport));
		rv = BCM_E_PARAM;
		BCM_SBX_UNLOCK(unit);
		return rv;		
	    ...
	    *weight = queue_state[unit][queue].ingress.bw_value.wfq_weight;
	    ...
	    BCM_DEBUG(BCM_DBG_COSQ, ("gport(0x%x) base_queue(%d) cosq(%d) queue(%d) mode(%d) weight(%d)\n",
	...
	if (rv != BCM_E_NONE) {
	    ...
	    if (rv != BCM_E_NONE) {
	        BCM_DEBUG(BCM_DBG_COSQ, ("gport sched get, error setting egress params\n"));
	    }
        }
        else {
            rv = MBCM_SBX_DRIVER_CALL(unit, mbcm_port_egress_multicast_scheduler_get, unit, gport,
                               ...
                               bcm_cosq_gport_discard_t *discard)
{
    ...
    if (!(soc_feature(unit, soc_feature_discard_ability))) {
        BCM_ERR(("ERROR: %s, Discard functionality not supported,  Unit(%d)\n", FUNCTION_NAME(), unit));
        BCM_SBX_UNLOCK(unit);
        return(BCM_E_UNAVAIL);
    }
    ...
    /* Consistency Checks */
    if (discard == NULL) {
        BCM_ERR(("ERROR: %s, Discard parameter is NULL, Unit(%d)\n", FUNCTION_NAME(), unit));
        BCM_SBX_UNLOCK(unit);
        return(BCM_E_PARAM);
    }
    ...
    if ( (color_val != BCM_COSQ_DISCARD_COLOR_GREEN) && (color_val != BCM_COSQ_DISCARD_COLOR_YELLOW) &&
	 ...
	 ((color & ~BCM_COSQ_DISCARD_COLOR_ALL) != 0) ) {
        BCM_ERR(("ERROR: %s, Invalid Color Parameter, Unit(%d)\n", FUNCTION_NAME(), unit));
        BCM_SBX_UNLOCK(unit);
        return(BCM_E_PARAM);
    }
    ...
    if ( (color_val == BCM_COSQ_DISCARD_COLOR_BLACK) &&
	 !soc_feature(unit, soc_feature_discard_ability_color_black)) {
        BCM_ERR(("ERROR: %s, Color black not support on this device, Unit(%d)\n", FUNCTION_NAME(), unit));
        BCM_SBX_UNLOCK(unit);
        return(BCM_E_PARAM);
    }
    ...
    if ( (discard->drop_probability < BCM_SBX_COSQ_MIN_DROP_PROBABILITY) ||
	 (discard->drop_probability > BCM_SBX_COSQ_MAX_DROP_PROBABILITY) ) {
        BCM_ERR(("ERROR: %s, Invalid Drop Probability, Unit(%d), DP: %d\n", FUNCTION_NAME(), unit, discard->drop_probability));
        BCM_SBX_UNLOCK(unit);
        return(BCM_E_PARAM);
    }
    ...
    if (discard->drop_probability != 0) {
        if ( (discard->gain != -1) && ((discard->gain < BCM_SBX_COSQ_MIN_GAIN) ||
            ...
            BCM_ERR(("ERROR: %s, Invalid Gain Parameter, Unit(%d)\n", FUNCTION_NAME(), unit));
            BCM_SBX_UNLOCK(unit);
            return(BCM_E_PARAM);
        }
    }
    ...
    if (rv != BCM_E_NONE) { /* VOQ is not valid */
        BCM_ERR(("ERROR: %s, Invalid VOQ, Unit(%d)\n", FUNCTION_NAME(), unit));
        BCM_SBX_UNLOCK(unit);
        return(BCM_E_PARAM);
    }
                               ...
                               bcm_cosq_gport_discard_t *discard)
{
    ...
    if (discard_state[unit] == NULL) {
        BCM_ERR(("ERROR: %s, Discard Data Structures Not allocated,  Unit(%d)\n", FUNCTION_NAME(), unit));
        BCM_SBX_UNLOCK(unit);
        return(BCM_E_INTERNAL);
    }
                              ...
                              int enable)
{
	...
	BCM_ERR(("ERROR: Unsupported gport type 0x%x\n", gport));
	rv = BCM_E_PARAM;
    }
                              ...
                              int *enable)
{
	...
	*enable = queue_state[unit][queue].ingress.enabled;
	...
	BCM_DEBUG(BCM_DBG_COSQ, ("gport(0x%x) queue(%d) cosq(%d) (%s)\n",
	...
	BCM_DEBUG(BCM_DBG_COSQ, ("gport(0x%x) fifo(%d) (%s)\n",
...

{
    ...
    if ( (BCM_GPORT_IS_UCAST_QUEUE_GROUP(input_gport)) ||
	 ...
	 (BCM_GPORT_IS_MCAST_SUBSCRIBER_QUEUE_GROUP(input_gport)) ||
        ...
        BCM_ERR(("Unsupported child gport type\n"));
	BCM_SBX_UNLOCK(unit);
        return BCM_E_PARAM;
    }
    ...
    if ( (BCM_GPORT_IS_CHILD(sched_gport)) ||
        ...
        BCM_ERR(("Unsupported parent gport type\n"));
	BCM_SBX_UNLOCK(unit);
        return BCM_E_PARAM;
    }

    if ( child_egress != parent_egress ) {
        BCM_ERR(("Child and parent gport direction not match\n"));
	BCM_SBX_UNLOCK(unit);
        return BCM_E_PARAM;
    }
    ...
    if (rv != BCM_E_NONE) {
        BCM_ERR(("ERROR: %s attach gport(0x%x) to (0x%x) failed error(%d)\n", 
                          ...
                          bcm_cos_queue_t cosq)
{
    ...
    if (rv != BCM_E_NONE) {
        BCM_ERR(("ERROR: %s detach gport(0x%x) to (0x%x) failed error(%d)\n", 
		 FUNCTION_NAME(), input_gport, sched_gport, rv));
        BCM_SBX_UNLOCK(unit);
        return rv;
    }
    ...
    int *attach_id)
{
    ...
    int fcd_max = SB_FAB_DEVICE_SIRIUS_MAX_FC_DOMAINS + 3;
	...
	BCM_ERR(("ERROR: %s, ingress_queue 0x%x is not a base queue, unit %d\n",
	...
	BCM_ERR(("ERROR: %s, ingress_int_pri %d exceeds %d cos for ingress_queue 0x%x, unit %d\n",
	...
	((ingress_int_pri == BCM_COS_MULTICAST_EF) || 
	 (ingress_int_pri == BCM_COS_MULTICAST_NON_EF))) {
	BCM_ERR(("ERROR: %s, ingress_int_pri %d indicates multicast, but ingress_queue %d not multicast queue, unit %d\n",
		 FUNCTION_NAME(), ingress_int_pri, ingress_queue, unit));
	BCM_SBX_UNLOCK(unit);
	return BCM_E_PARAM;
    }
    ...
    if (p_bwstate->dest_type == BCM_INT_SBX_DEST_TYPE_MULTICAST) {
	    ...
	    BCM_ERR(("ERROR: %s, multicast egress_queue is not NULL 0x%x\n", egress_queue));
	    BCM_SBX_UNLOCK(unit);
	    return(BCM_E_PARAM);
        }
        if ((egress_int_pri != BCM_COS_MULTICAST_EF) && (egress_int_pri != BCM_COS_MULTICAST_NON_EF)) {
	    BCM_ERR(("ERROR: %s, multicast egress_int_pri invalid 0x%x\n", egress_int_pri));
	    BCM_SBX_UNLOCK(unit);
	    return(BCM_E_PARAM);
        }
        ...
        if (rv != BCM_E_NONE) {
            BCM_ERR(("ERROR: %s ingress queue/cos (0x%x/%d) to egress cos (0x%x) failed error(%d)\n",
	...
	if (with_id != TRUE) {
	    BCM_ERR(("ERROR: %s %s node must use BCM_COSQ_GPORT_QUEUE_ATTACH_WITH_ID flag\n",
	...
	if (p_qstate->sysport != p_qstate->default_sysport) {
	    ...
	    BCM_WARN(("WARNING: %s, Ingress Queue 0x%x cos %d is currently connected to egress queue 0x%x attach_id 0x%x, unit %d\n",
	...
	if (mymodid == -1) {
		...
		BCM_ERR(("ERROR: %s illegal attach_id 0x%x\n",
	...
	if (BCM_GPORT_IS_EGRESS_CHILD(egress_queue)) {
		...
		BCM_ERR(("ERROR: %s egress queue 0x%x not connected to ingress queue 0x%x, port mismatch\n",
			 FUNCTION_NAME(),egress_queue, ingress_queue));
		BCM_SBX_UNLOCK(unit);
		return BCM_E_RESOURCE;
	    }
	    if (BCM_STK_MOD_TO_NODE(BCM_GPORT_EGRESS_CHILD_MODID_GET(egress_queue)) != node) {
		BCM_ERR(("ERROR: %s egress queue 0x%x not connected to ingress queue 0x%x, node mismatch\n",
			 FUNCTION_NAME(),egress_queue, ingress_queue));
		BCM_SBX_UNLOCK(unit);
		return BCM_E_RESOURCE;
	    }
	...
	if (requested_sysport == BCM_INT_SBX_REQUESTED_SYSPORT_AUTO_SELECT) {
	    BCM_ERR(("ERROR: %s Requested sysport out of range(%d)\n", FUNCTION_NAME(), requested_sysport));
	    BCM_SBX_UNLOCK(unit);
	    return BCM_E_RESOURCE;
	}
	...
	if (sysport_state[unit][requested_sysport].use_cnt == 0) {
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_ERR(("ERROR: %s Could not allocate sysport 0x%x\n", FUNCTION_NAME(), requested_sysport));
		BCM_SBX_UNLOCK(unit);
		return rv;
	    }
	} else {
	...
	if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: %s ingress queue (0x%x) to egress queue (0x%x) failed error(%d)\n", 
	...
	BCM_ERR(("ERROR: %s, egress_int_pri %d exceeds %d fifos for egress_queue 0x%x, unit %d\n",
	...
	if ((flags & BCM_COSQ_GPORT_QUEUE_ATTACH_WITH_ID) == 0) {
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_ERR(("ERROR: %s Could not allocate fcd\n", FUNCTION_NAME()));
		BCM_SBX_UNLOCK(unit);
		return rv;
	    }
	    ...
	    if (rv != BCM_E_NONE) {
		_bcm_sbx_cosq_fcd_free(unit, fcd);
		BCM_ERR(("ERROR: %s Could not allocate sysport\n", FUNCTION_NAME()));
		BCM_SBX_UNLOCK(unit);
		return rv;
	    }
	} else {
    ...
    if (rv != BCM_E_NONE) {
        BCM_ERR(("ERROR: %s ingress queue (0x%x) to egress queue (0x%x) failed error(%d)\n", 
		 FUNCTION_NAME(), ingress_queue, egress_queue, rv));
	if (_bcm_sbx_cosq_fcd_free(unit, fcd) == BCM_E_NONE) {
    ...
    int attach_id)
{
    ...
    if (p_bwstate->dest_type == BCM_INT_SBX_DEST_TYPE_MULTICAST) {
	    ...
	    BCM_ERR(("ERROR: %s, multicast egress_int_pri pointer is NULL\n"));
	    BCM_SBX_UNLOCK(unit);
	    return(BCM_E_PARAM);
        }
	...
	if (mymodid != -1) {
	    ...
	    BCM_ERR(("ERROR: %s unsupported on remote node, unit %d\n",
		     FUNCTION_NAME(), unit));
	    BCM_SBX_UNLOCK(unit);
	    return BCM_E_UNAVAIL;
	}
	if (p_qstate->sysport != p_qstate->default_sysport) {
		...
		*egress_int_pri = -1; 
		...
		BCM_ERR(("ERROR: %s, Attach Id 0x%x does not match 0x%x associated with queue 0x%x\n",
			 FUNCTION_NAME(), attach_id, temp, ingress_queue));
		BCM_SBX_UNLOCK(unit);
		return BCM_E_PARAM;
	    }
	} else {
	    BCM_ERR(("ERROR: %s, Ingress queue 0x%x currently not associated with an Egress queue, unit %d\n",
		     FUNCTION_NAME(), ingress_queue, unit));
	    BCM_SBX_UNLOCK(unit);
	    return BCM_E_PARAM;
	}
    ...
    *egress_int_pri = -1; 
	...
	BCM_ERR(("ERROR: %s, ingress_queue 0x%x does not contain attach_id 0x%x at int_pri %d, unit %d\n",
    ...
    int attach_id)
{
    ...
    if (p_bwstate->dest_type == BCM_INT_SBX_DEST_TYPE_MULTICAST) {
        ...
        if (rv != BCM_E_NONE) {
            BCM_ERR(("ERROR: %s ingress queue/cos (0x%x/%d) failed error (%d)\n",
	...
	BCM_ERR(("ERROR: %s ingress queue/cos (0x%x/%d) not associated with attach_id 0x%x\n",
	...
	if (sp_info->egroup[eg_n].fcd[ingress_int_pri] != attach_id) {
	    BCM_ERR(("ERROR: %s, Ingress Queue 0x%x does not contain attach_id 0x%x at int_pri %d, unit %d\n",
		     FUNCTION_NAME(), ingress_queue, attach_id, ingress_int_pri, unit));
	    BCM_SBX_UNLOCK(unit);
	    return BCM_E_PARAM;
	}
	...
	if (attach_id == BCM_INT_SBX_INVALID_FCD) {
	    BCM_ERR(("ERROR: %s, Invalid attach ID 0x%x,  unit %d\n",
    ...
    if (rv != BCM_E_NONE) {
	BCM_ERR(("ERROR: %s ingress queue (0x%x) attach_id (0x%x) failed error(%d)\n", 
...
_bcm_sbx_cosq_get_base_queue_from_gport(int unit, bcm_gport_t gport, int *p_base_queue, int *p_num_cos_levels)
{
	...
	(BCM_GPORT_IS_CHILD(gport) == FALSE) &&
        ...
        BCM_ERR(("ERROR: gport %08X type is not supported\n"));
        rv = BCM_E_PARAM;
        return rv;
    }
    ...
    if (BCM_GPORT_IS_MODPORT(gport) ||
        ...
        if (modid < BCM_MODULE_FABRIC_BASE) {
            BCM_ERR(("ERROR: gport modid invalid value (%d) is less than minimum (%d)\n",
                     modid, BCM_MODULE_FABRIC_BASE));
            rv = BCM_E_PARAM;
        }
        node = modid - BCM_MODULE_FABRIC_BASE;
    ...
    *p_num_cos_levels = num_cos;
    ...
    BCM_DEBUG(BCM_DBG_COSQ, ("base queue(%d) num cos levels(%d)\n", *p_base_queue, num_cos));
    return rv;
}
...
_bcm_sbx_cosq_queue_state_init(int unit, int32 num_cos) 
{
    ...
    if (rc != BCM_E_NONE) {
        BCM_ERR(("Unit:%d - %s, internal error setting queue_regions \n", 
                 unit, FUNCTION_NAME()));
        goto err;
    }
...
_bcm_sbx_cosq_sysport_state_init(int unit)
{
    ...
    /*
     * TODO: Revisit when only QE4000 is in system, we may not need sysport
     *       grouping or reservation of sysport for extended esets
     */
...
_bcm_sbx_cosq_fcd_state_init(int unit)
{
    ...
    fcd_state[unit] = sal_alloc(sizeof(bcm_sbx_cosq_fcd_state_t) * fcd_max,
				"flow control domain state memory");
    
    if (fcd_state[unit] == NULL) {
	BCM_ERR(("ERROR: %s, sal_alloc flow control domain,  Unit(%d)\n", FUNCTION_NAME(), unit));
	return(BCM_E_MEMORY);
    }
    
    for (fcd = 0; fcd < fcd_max; fcd++) {
...
_bcm_sbx_cosq_bw_group_get_available_bw(int unit, int bw_group) {
    ...
    bcm_sbx_cosq_bw_group_state_t *p_bwstate;
    int32_t available_bw_kbps;
    int32_t num_cos;
    int32_t total_queue_bw_kbps = 0;
    int32_t queue;
    int32_t base_queue;
...
_bcm_sbx_cosq_bw_group_allocate(int unit, int *p_bw_group, int auto_select)
{
    ...
    if (auto_select) {
        /* TODO: this should be a managed linked list. free and in use lists */
        for (bw_group = 0; bw_group < SOC_SBX_CFG(unit)->num_bw_groups; bw_group++) {
            if (bw_group_state[unit][bw_group].in_use == FALSE) {
                ...
                *p_bw_group = bw_group;
                BCM_DEBUG(BCM_DBG_COSQ, ("Allocated bw_group(%d)\n", bw_group));
                return rv;
            }
        }
        ...
        if (bw_group_state[unit][bw_group].in_use == FALSE) {
            ...
            BCM_ERR(("bw_group requested is already in use(%d)\n", bw_group));
            rv = BCM_E_RESOURCE;
        }
    }
    BCM_ERR(("There are no free bw_groups.  Resource error\n"));
    rv = BCM_E_RESOURCE;
    return rv;
}
		...
		*p_sysport = SIRIUS_TS_LOCAL_SYSPORT_BASE + sp_info->egroup[egroup_num].ef_fcd/2;
		...
		if (dp_state[unit] != NULL) {
			...
			if (dp_state[unit] != NULL) {
			    dp_state[unit][fabric_port].ref_cnt++;
                        ...
                        BCM_DEBUG(BCM_DBG_COSQ, ("Allocated sysport(%d) for node(%d) in sysport group(%d)\n",
		...
		if (dp_state[unit] != NULL) {
		    dp_state[unit][fabric_port].ref_cnt++;
                ...
                BCM_DEBUG(BCM_DBG_COSQ, ("Requested sysport(%d) available\n", requested_sysport));
		
                return rv;
            } else {
                *p_sysport = BCM_INT_SBX_INVALID_SYSPORT;
                BCM_DEBUG(BCM_DBG_COSQ, ("requested sysport(%d) is not available\n", requested_sysport));
                rv = BCM_E_RESOURCE;
                return rv;
            }
        }
	...
	if (queue_region == bcm_sbx_cosq_queue_region_local) {
		...
		for (sysport = SIRIUS_TS_LOCAL_SYSPORT_BASE + 256; 
		    ...
		    *p_sysport = sysport;
		    ...
		    BCM_DEBUG(BCM_DBG_COSQ, ("Allocated sysport(%d) for node(%d)\n",
					     sysport, dest_node));
		    rv = BCM_E_NONE;
		    return rv;
		}
		...
		if ((requested_sysport < SIRIUS_TS_LOCAL_SYSPORT_BASE) ||
		    ...
		    *p_sysport = BCM_INT_SBX_INVALID_SYSPORT;
		    BCM_DEBUG(BCM_DBG_COSQ, ("requested sysport(%d) out of range\n", requested_sysport));
		    rv = BCM_E_RESOURCE;
		...
		*p_sysport = sysport;
        ...
        BCM_ERR(("Unsupported fabric system mode\n"));
    }

    BCM_ERR(("There are no free sysports.  Resource error\n"));
    *p_sysport = BCM_INT_SBX_INVALID_SYSPORT;
    rv = BCM_E_RESOURCE;
    return rv;
}
			   ...
			   int *p_fcd)
{
	...
	for (fcd = 0; fcd < SB_FAB_DEVICE_SIRIUS_MAX_FC_DOMAINS; fcd++) {
	    if (fcd_state[unit][fcd].in_use == FCD_FREE) {
		/* Allocate this flow control domain */
		*p_fcd = fcd;
		if (flags == FCD_ALLOC) {
		    fcd_state[unit][fcd].in_use = FCD_ALLOC;
		} else {
		    fcd_state[unit][fcd].in_use = FCD_TAKE;
		}
		fcd_state[unit][fcd].ref_cnt++;
		fcd_state[unit][fcd].fifo = fifo;
		fcd_state[unit][fcd].sysport = sysport;
		fcd_state[unit][fcd].node = dest_node;
		fcd_state[unit][fcd].port = fabric_port;
		
		BCM_DEBUG(BCM_DBG_COSQ, ("Allocated fcd(%d) for node(%d)\n",
					 fcd, dest_node));
	...
	fcd_state[unit][requested_fcd].in_use = FCD_TAKE;
	fcd_state[unit][requested_fcd].ref_cnt++;
	fcd_state[unit][requested_fcd].fifo = -1;
	fcd_state[unit][requested_fcd].sysport = -1;
	fcd_state[unit][requested_fcd].node = -1;
	fcd_state[unit][requested_fcd].port = -1;
	*p_fcd = requested_fcd;
    } else {
	/* if requested an fcd, check if it's available */
	if (fcd_state[unit][requested_fcd].in_use == FCD_FREE) {
	    if (flags == FCD_ALLOC) {
		fcd_state[unit][requested_fcd].in_use = FCD_ALLOC;
	    } else {
		fcd_state[unit][requested_fcd].in_use = FCD_TAKE;
	    }
	    fcd_state[unit][requested_fcd].ref_cnt++;
	    fcd_state[unit][requested_fcd].fifo = fifo;
	    fcd_state[unit][requested_fcd].sysport = sysport;
	    fcd_state[unit][requested_fcd].node = dest_node;
	    fcd_state[unit][requested_fcd].port = fabric_port;
	    *p_fcd = requested_fcd;

	    BCM_DEBUG(BCM_DBG_COSQ, ("Requested flow control domain(%d) available\n", requested_fcd));
		
	    return rv;
	} else {
	    *p_fcd = BCM_INT_SBX_INVALID_FCD;
	    BCM_DEBUG(BCM_DBG_COSQ, ("requested fcd(%d) is not available\n", requested_fcd));
	    rv = BCM_E_RESOURCE;
	    return rv;
	}
    }

    BCM_ERR(("There are no free flow control domains.  Resource error\n"));
    *p_fcd = BCM_INT_SBX_INVALID_FCD;
    rv = BCM_E_RESOURCE;
    return rv;
}
...
int
bcm_sbx_cosq_fcd_get_from_fifo(int unit, int fifo, int *p_fcd)
{
    int rv = BCM_E_NONE;
    int fcd = BCM_INT_SBX_INVALID_FCD;
...
_bcm_sbx_cosq_fcd_free(int unit, int fcd)
{
    ...
    if ( (fcd >= 0) && (fcd < SB_FAB_DEVICE_SIRIUS_MAX_FC_DOMAINS) ) {
        ...
        if ((fcd_state[unit][fcd].in_use != FCD_FREE) && (fcd_state[unit][fcd].ref_cnt == 1)) {
            /* make fcd state as not in use */
            fcd_state[unit][fcd].in_use = FCD_FREE;
            fcd_state[unit][fcd].ref_cnt = 0;
            fcd_state[unit][fcd].fifo = -1;
            fcd_state[unit][fcd].sysport = BCM_INT_SBX_INVALID_SYSPORT;
            fcd_state[unit][fcd].node = -1;
            fcd_state[unit][fcd].port = -1;
	} else {
            fcd_state[unit][fcd].ref_cnt--;
	    rv = BCM_E_BUSY;
	}
    } else {
	if (fcd > (SB_FAB_DEVICE_SIRIUS_MAX_FC_DOMAINS + 2)) {
	    rv = BCM_E_PARAM;
	}
    }
...
_bcm_sbx_cosq_queue_group_allocate(int unit,
    ...
    if (queue_region_p->is_valid != TRUE) {
        BCM_ERR(("Requested base region (%d) not valid\n", queue_region));
        rv = BCM_E_RESOURCE;
        return rv;
    }

    *p_allocated_base_queue = -1;
    BCM_DEBUG(BCM_DBG_COSQ, ("Allocating queue: requested_base_queue=%d\n", requested_base_queue));
	...
	(requested_base_queue != BCM_INT_SBX_REQUESTED_QUEUE_AUTO_SELECT_WITHOUT_SHAPING) &&
	 ...
	 (requested_base_queue > (queue_region_p->end - 1)))) {
        BCM_ERR(("Requested base queue out of range(%d)\n", requested_base_queue));
        rv = BCM_E_RESOURCE;
        return rv;
    }
    ...
    if ((requested_sysport != BCM_INT_SBX_REQUESTED_SYSPORT_AUTO_SELECT) &&
        ...
        (requested_sysport > (SOC_SBX_CFG(unit)->num_sysports -1))) {
        BCM_ERR(("Requested sysport out of range(%d)\n", requested_sysport));
        rv = BCM_E_RESOURCE;
        return rv;
    }
    ...
    if ( (requested_base_queue != BCM_INT_SBX_REQUESTED_QUEUE_AUTO_SELECT) &&
	 ...
	 (requested_base_queue != BCM_INT_SBX_REQUESTED_QUEUE_AUTO_SELECT_WITHOUT_SHAPING) ) {
        if ( *p_allocated_base_queue != requested_base_queue ) {
            BCM_WARN(("WARNING: Could not allocate %d cos levels with requested base queue(%d) node %d port %d.\n",
    ...
    if (found_base_queue == FALSE ) {
        if ( (requested_base_queue == BCM_INT_SBX_REQUESTED_QUEUE_AUTO_SELECT) ||
	     (requested_base_queue == BCM_INT_SBX_REQUESTED_QUEUE_AUTO_SELECT_WITH_SHAPING) ||
	     (requested_base_queue == BCM_INT_SBX_REQUESTED_QUEUE_AUTO_SELECT_WITHOUT_SHAPING) ) {
            BCM_WARN(("Could not find (auto-select) base queue with %d cos levels available\n",
                     num_cos));
            rv = BCM_E_INTERNAL;
        } else {
            BCM_WARN(("%d cos levels are not available from requested base queue(%d)\n",
    ...
    BCM_DEBUG(BCM_DBG_COSQ, ("Allocated queue group base(%d)\n", base_queue));
    ...
    if ((!SOC_IS_RELOADING(unit)) || (SOC_IS_SBX_QE2000(unit)))  {
        ...
        if (rv != BCM_E_NONE) {
            BCM_ERR(("Could not allocate bw_group\n"));
            return rv;
        }
        BCM_DEBUG(BCM_DBG_COSQ, ("Allocated bw group(%d)\n", bw_group));
	...
	for (bw_group = 0; bw_group < SOC_SBX_CFG(unit)->num_bw_groups; bw_group++) {
		...
		BCM_DEBUG(BCM_DBG_COSQ, ("EASY_RELOAD bw_group(%d) base_queue_of_bag(0x%x) base_queue_cached(0x%x)\n", bw_group, 
					 p_bwstate_cache[bw_group].base_queue_of_bag,
					 base_queue_cached));

		if (base_queue_cached == base_queue) {
		    BCM_DEBUG(BCM_DBG_COSQ, ("EASY_RELOAD bw_group(%d) for queue(%d) validated\n", bw_group, base_queue));
		    break;
		}
	    }
	}
        ...
        if ((bw_group >=0 ) || (bw_group < SOC_SBX_CFG(unit)->num_bw_groups)) {
            ...
            if (rv != BCM_E_NONE) {
                BCM_ERR(("ERROR: EASY_RELOAD Could not reserve bw_group(%d)\n", bw_group));
                return BCM_E_INTERNAL;
            }
            BCM_DEBUG(BCM_DBG_COSQ, ("EASY_RELOAD: Reloaded bw group(%d)\n", bw_group));

	    queue_state[unit][base_queue].bw_group = bw_group;
        } else {
            BCM_ERR(("ERROR: EASY_RELOAD could not find bw_group allocated in hardware for base queue(%d)\n", base_queue));
            return BCM_E_INTERNAL;
        }
#endif /* BCM_EASY_RELOAD_SUPPORT */
    }
    ...
    BCM_DEBUG(BCM_DBG_COSQ, ("Allocated sysport(%d)\n", *p_allocated_sysport));
...
_bcm_sbx_cosq_queue_group_free(int unit, int32 base_queue, int32 num_cos) {
    ...
    if (rv != BCM_E_NONE) {
        BCM_ERR(("Could not free bw_group for base_queue(%d)\n", base_queue));
        return FALSE;
    }
    ...
    for ( queue=base_queue; queue < base_queue+num_cos; queue++ ) {
        ...
        if (p_qstate->state != BCM_INT_SBX_QUEUE_STATE_IN_USE) {
            BCM_ERR(("ERROR: Attempted to free queue(0x%x) which is not currently in use\n", queue));
            rv = BCM_E_INTERNAL;
        }
...
_bcm_sbx_cosq_discard_state_init(int unit)
{
    ...
    if (!SAL_BOOT_BCMSIM && soc_property_get(unit, spn_DIAG_EMULATOR_PARTIAL_INIT, 0)) {
        /*
         * BCM_DEBUG(BCM_DBG_VERBOSE,("#### %s: DIAG_EMULATOR_PARTIAL_INIT is settt\n", FUNCTION_NAME()));
         */
        return BCM_E_NONE;
    }
...
_bcm_sbx_cosq_scheduler_state_init(int unit)
{
    ...
    if ((ingress_scheduler_state[unit] == NULL) && (SOC_SBX_CFG(unit)->num_ingress_scheduler != 0)) {
        BCM_ERR(("ERROR: %s, sal_alloc,  Unit(%d)\n", FUNCTION_NAME(), unit));
        return(BCM_E_MEMORY);
    }
    ...
    if ((egress_scheduler_state[unit] == NULL) && (SOC_SBX_CFG(unit)->num_egress_scheduler != 0)) {
        BCM_ERR(("ERROR: %s, sal_alloc,  Unit(%d)\n", FUNCTION_NAME(), unit));
        return(BCM_E_MEMORY);
    }
    ...
    for (i = 0; i < SOC_SBX_CFG(unit)->num_egress_scheduler; i++) {
        ...
        (egress_scheduler_state[unit] + i)->child_port = -1;
    }

    /* allocate egress group state */
    egress_group_state[unit] = sal_alloc(sizeof(bcm_sbx_cosq_egress_group_state_t) * SOC_SBX_CFG(unit)->num_egress_group,
                                              "egress_group_memory");
    if ((egress_group_state[unit] == NULL) && (SOC_SBX_CFG(unit)->num_egress_group !=0)) {
        BCM_ERR(("ERROR: %s, sal_alloc,  Unit(%d)\n", FUNCTION_NAME(), unit));
        return(BCM_E_MEMORY);
    }
    ...
    if ((ingress_multipath_state[unit] == NULL) && (SOC_SBX_CFG(unit)->num_ingress_multipath != 0)) {
        BCM_ERR(("ERROR: %s, sal_alloc,  Unit(%d)\n", FUNCTION_NAME(), unit));
        return(BCM_E_MEMORY);
    }
...
_bcm_sbx_allocate_template(int unit, int *template)
{
    ...
    if (found == FALSE) {
        BCM_ERR(("ERROR: %s, template resource not available, Unit(%d)\n", FUNCTION_NAME(), unit));
        return(BCM_E_RESOURCE);
    }
    ...
    BCM_DEBUG(BCM_DBG_COSQ,("allocated template(%d)\n", i));
    return(rv);
}
...
_bcm_sbx_deallocate_template(int unit, int template)
{
    ...
    BCM_DEBUG(BCM_DBG_COSQ,("deallocated template(%d)\n", template));
    return(rv);
}
                                           ...
                                           int queue_sz)
{
    ...
    if ((max_queue_depth < discard->max_thresh) ||
        ...
        BCM_ERR(("ERROR: %s, Invalid Discard paramerers, Unit(%d), "
                 "MinThreshold: 0x%x MaxThreshold: 0x%x EcnThrehold: 0x%x\n",
    ...
    if (discard->max_thresh < discard->min_thresh) {
        BCM_ERR(("ERROR: %s, Max Threshoold needs to be bigger then Min threshold, Unit(%d), "
    ...
    if ( (discard->ecn_thresh > discard->max_thresh) ||
        ...
        BCM_ERR(("ERROR: %s, Invalid ecn threshold, Unit(%d), "
                               ...
                               bcm_cosq_gport_discard_t *discard)
{
	...
	if (_bcm_sbx_cosq_queue_is_local(unit, queue) == FALSE) {
	    BCM_ERR(("ERROR: %s, Discard functionality not supported on queue %d when device is"
	...
	if ((color_val == BCM_COSQ_DISCARD_COLOR_ALL) ||
		...
		rv = _bcm_sbx_allocate_template(unit, &template);
		if (rv != BCM_E_NONE) {
		    BCM_ERR(("%s: ERROR: _bcm_sbx_allocate_template, Unit(%d)\n", FUNCTION_NAME(), unit));
		    return rv;
		}
		...
		if (rv != BCM_E_NONE) {
		    BCM_ERR(("ERROR: %s, mbcm_sbx_driver, Unit(%d), Error: 0x%x\n", FUNCTION_NAME(), unit, rv));
	...
	for (lcl_color = 0; lcl_color < nbr_color; lcl_color++) {
		...
		if (rv == BCM_E_NONE) {
		    ...
		    BCM_ERR(("ERROR: %s, error in bcm_sbx_stk_get_modules_enabled"
			     ", Unit(%d), Error: 0x%x\n", FUNCTION_NAME(), unit, rv));
		    goto err;
		}
                               ...
                               bcm_cosq_gport_discard_t *discard)
{
    ...
    if (SOC_IS_RELOADING(unit)) {
	    ...
	    (template > SOC_SBX_CFG(unit)->nDiscardTemplates)) {
            BCM_ERR(("ERROR: %s, Incorrect Template: 0x%x\n", FUNCTION_NAME(), template));
            return(BCM_E_INTERNAL);
        }
...
bcm_sbx_cosq_template_pfc_config(int unit, int32 template)
{
    ...
    if (discard_state[unit] == NULL) {
	BCM_ERR(("ERROR: %s, discard state not init on Unit(%d)\n",
		 FUNCTION_NAME(), unit));
	return BCM_E_INIT;
    }

    if (template >= SOC_SBX_CFG(unit)->nDiscardTemplates) {
	BCM_ERR(("ERROR: %s, template id %d out of range on Unit(%d)\n",
		 FUNCTION_NAME(), template, unit));
	return BCM_E_PARAM;
    }

    if ((discard_state[unit] + template)->is_free == TRUE) {
	BCM_ERR(("ERROR: %s, template id %d not in use on Unit(%d)\n",
		 FUNCTION_NAME(), template, unit));
	return BCM_E_PARAM;
    }
...
bcm_sbx_cosq_bw_group_verify_queues(int unit, int bw_group) {
    ...
    if ( available_bw_kbps < 0 ) {
        BCM_ERR(("ERROR: Not enough bw_group bandwidth for flow guarantees: group BW(%d kbps) Available BW(%d kbps).\n",
                     available_bw_kbps, p_bwstate->dest_port_kbits_sec_max));
        return BCM_E_CONFIG;
    }

    BCM_DEBUG(BCM_DBG_COSQ, ("bandwidth available(%d)\n",available_bw_kbps));

    /* TODO: check QOS compatibility at this point        */
...
bcm_sbx_cosq_update_given_weights(int unit, int bw_group) {
    ...
    uint32_t total_q_weights = 0;
    int32_t total_given_weights = 0;
    uint32_t weight = 0;
    bcm_sbx_cosq_bw_group_state_t *p_bwstate;
    int32_t base_queue;
    uint32_t weight_sum;
    ...
    BCM_DEBUG(BCM_DBG_COSQ, ("update given weights bw_group(%d) num_cos(%d)\n",
                             bw_group, num_cos));
    ...
    BCM_DEBUG(BCM_DBG_COSQ, ("total_q_weights(%d)\n", total_q_weights));
    ...
    if ( total_q_weights ) {
        ...
        for ( queue=base_queue; queue< base_queue+num_cos; queue++ ) {
            ...
            if ( weight != 0 ) {
                ...
                BCM_DEBUG(BCM_DBG_COSQ, ("queue(%d) weight(%d) ", queue, weight));
                BCM_DEBUG(BCM_DBG_COSQ, ("total_given_weights(%d) given_weight(%d)\n",
                                         total_given_weights, p_qstate->ingress.given_weight));
	    } else {
        ...
        if ( total_given_weights != weight_sum ) {
            ...
            while ( total_given_weights > weight_sum ) {
                ...
                for ( queue=base_queue; queue < base_queue+num_cos; queue++ ) {
                    ...
                    total_given_weights--;
                    BCM_DEBUG(BCM_DBG_COSQ,("queue(%d) reduce given_weight(%d) total_given_weight(%d)\n",
            ...
            while ( total_given_weights < weight_sum ) {
                ...
                for ( queue=base_queue; queue< base_queue+num_cos; queue++ ) {
                    ...
                    p_qstate->ingress.given_weight++;
                    total_given_weights++;
                    BCM_DEBUG(BCM_DBG_COSQ,("queue(%d) increase given_weight(%d) total_given_weights(%d)\n",
                                            queue, p_qstate->ingress.given_weight, total_given_weights));
                    if ( total_given_weights >= weight_sum ) {
                        break;
                    }
        ...
        for (queue=base_queue; queue<base_queue + num_cos; queue++) {
            ...
            BCM_DEBUG(BCM_DBG_COSQ,("queue(%d) weight(%d) given_weight(%d) total_given_weights(%d)\n", queue,
                                    weight, p_qstate->ingress.given_weight, total_given_weights));
        }

        if (total_given_weights != weight_sum) {
             BCM_DEBUG(BCM_DBG_COSQ, ("queue(%d) weight(%d) given_weight(%d) total_given_weights(%d) out of range - must be %d\n",
    ...
    for (template = 0; template < nbr_template; template++) {
        for (lcl_color = 0; lcl_color < max_color; lcl_color++) {
          rv = MBCM_SBX_DRIVER_CALL(unit, mbcm_cosq_gport_discard_set,unit, 0, 0,
				    ...
				    &(discard_state[unit] + template)->config[lcl_color]);
            if (rv != BCM_E_NONE) {
                BCM_ERR(("ERROR: %s, mbcm_cosq_gport_discard_set, Unit(%d) Err: 0x%x\n",
                         FUNCTION_NAME(), unit, rv));
                result = rv;
            }
        }
...
bcm_sbx_cosq_get_gport_cos_from_qid(int unit, int queue, bcm_gport_t *gport, bcm_cos_queue_t *dest_cosq)
{
   ...
   if (bcm_sbx_cosq_queue_available(unit, queue)) {
      BCM_ERR(("ERROR: Queue(%d) not allocated\n"));
      return BCM_E_PARAM;
    }
    ...
    if (BCM_E_NONE == rv) {
        switch (type) {
            default:
                ...
                if (rv != BCM_E_NONE) {
                    BCM_ERR(("ERROR: %s, mbcm_cosq_gport_control_set, Unit(%d) Err: 0x%x\n",
                                                                   FUNCTION_NAME(), unit, rv));
                }
                break;
        }
        ...
        switch (type) {
            default:
                rv = MBCM_SBX_DRIVER_CALL(unit, mbcm_cosq_control_get,unit, port, cosq,
                                  type, arg, &control_info);
                if (rv != BCM_E_NONE) {
                    BCM_ERR(("ERROR: %s, mbcm_cosq_gport_control_get, Unit(%d) Err: 0x%x\n",
                                                                    FUNCTION_NAME(), unit, rv));
                }
                break;
        }
...
_bcm_sbx_cosq_scheduler_free(int unit, int egress, int gport)
{
	...
	if (rv != BCM_E_NONE) {
	    BCM_ERR(("ERROR: %s, failed to free %d level %d scheduler nodes %d for gport 0x%x, Unit(%d)\n",
	...
	scheduler = BCM_INT_SBX_SCHEDULER_ID_GET(gport);
		...
		BCM_ERR(("ERROR: egress scheduler id %d out of range\n", scheduler));
                return BCM_E_PARAM;
            }
            if ( (p_e_scheduler + scheduler)->in_use == FALSE ) {
		BCM_ERR(("ERROR: egress scheduler %d not used\n",scheduler));
		return BCM_E_PARAM;
            } else {
		/* hardware resource should already be freed during detach
		rv = bcm_sbx_port_get_scheduler(unit, gport, &level, &node);
		if (rv != BCM_E_NONE) {
		    BCM_DEBUG(BCM_DBG_COSQ, ("%s: can not find egress scheduler for gport 0x%x on unit %d\n",
					     FUNCTION_NAME(), gport, unit));
		    return rv;	    
		}
		...
		if (rv != BCM_E_NONE) {
		    BCM_ERR(("ERROR: %s, failed to free %d level %d egress scheduler nodes %d for gport 0x%x, Unit(%d)\n",
			     FUNCTION_NAME(), level, node, gport, unit));
		    return rv;
		}      
		*/
		...
		(p_e_scheduler + scheduler)->child_port = -1;
	    }
        } else {
            if (p_i_scheduler == NULL) {
                return BCM_E_UNAVAIL;
		...
		BCM_ERR(("ERROR: ingress scheduler id %d out of range\n",scheduler));
                return BCM_E_PARAM;
            }
            if ( (p_i_scheduler + scheduler)->in_use == FALSE ) {
		BCM_ERR(("ERROR: ingress scheduler %d not used\n",scheduler));
		return BCM_E_PARAM;
	    } else {
		/* hardware resource should already be freed during detach
		rv = bcm_sbx_port_get_scheduler(unit, gport, &level, &node);
		...
		if (rv != BCM_E_NONE) {
		    BCM_ERR(("ERROR: %s, failed to free %d level %d ingress scheduler nodes %d for gport 0x%x, Unit(%d)\n",
			     FUNCTION_NAME(), level, node, gport, unit));
		    return BCM_E_RESOURCE;
		}	
		*/
...
_bcm_sbx_cosq_multipath_free(int unit, int egress, int gport)
{
	...
	if ( (multipath < 0) || (multipath >= SOC_SBX_CFG(unit)->num_egress_multipath) ) {
	    BCM_ERR(("ERROR: egress multipath id %d out of range\n", multipath));
	    return BCM_E_PARAM;
	}
	if ( (p_e_multipath + multipath)->in_use == FALSE ) {
	    BCM_ERR(("ERROR: egress multipath %d not used\n",multipath));
	    return BCM_E_PARAM;
	} else {
	    ...
	    if (member != 0) {
		BCM_ERR(("ERROR: egress multipath %d still has member in it, can not be freed\n",multipath));
		return BCM_E_PARAM;
	    }
	    ...
	    if (rv != BCM_E_NONE) {
		BCM_ERR(("ERROR: %s, failed to free egress multipath %d for gport 0x%x, Unit(%d)\n",
	...
	if ( (multipath < 0) || (multipath >= SOC_SBX_CFG(unit)->num_ingress_multipath) ) {
	    BCM_ERR(("ERROR: ingress multipath id %d out of range\n",multipath));
	    return BCM_E_PARAM;
	}
	if ( (p_i_multipath + multipath)->in_use == FALSE ) {
	    BCM_ERR(("ERROR: ingress multipath %d not used\n",multipath));
	    return BCM_E_PARAM;
	} else if ((p_i_multipath + multipath)->num_nodes != 0) {
	    BCM_ERR(("ERROR: ingress multipath %d still has member in it, can not be freed\n",multipath));
	    return BCM_E_PARAM;
	} else {
				    ...
				    int num_fifos, bcm_gport_t *p_eg_gport)
{
    ...
    int32 requested_fcd = BCM_INT_SBX_REQUESTED_FCD_AUTO_SELECT;
    int intf = 0, port = 0, fifo = 0, fcd = BCM_INT_SBX_INVALID_FCD, speed = 0, rv = BCM_E_NONE;
    int idx = 0, enum_fifos = 0, efifos = (num_fifos % SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_HW_SIZE);
	...
	BCM_ERR(("ERROR: Only up to %d fifos per egress group allowed\n",SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_MAX_SIZE));
	BCM_SBX_UNLOCK(unit);
	return BCM_E_PARAM;	    
    }
	...
	rv = bcm_sbx_port_get_intf_portoffset(unit, child_port, &intf, &port);
	if (rv != BCM_E_NONE) {
	...
	if ((enum_fifos + num_fifos) > SB_FAB_DEVICE_SIRIUS_MAX_PER_HG_SUBPORTS ) {
	    BCM_ERR(("ERROR: %s, %d fifos exceeds maximum allowable fifos for child_port %d unit %d\n",
    ...
    if (idx == SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_MAX) {
	    BCM_ERR(("ERROR: %s, max fifos in use for child_port %d unit %d\n",
		     FUNCTION_NAME(), child_port, unit));
	    return BCM_E_RESOURCE;
    }
    ...
    *p_eg_gport = -1;
    ...
    /* in sirius extended port mode, force the fcd to be same as the fabric port handle */
    if (SOC_IS_SIRIUS(unit) && 
	SOC_SBX_CFG_SIRIUS(unit)->bExtendedPortMode &&
	((sp_info->flags & SBX_SUBPORT_FLAG_INTERNAL) == 0)) {
	efifos = 0; /* why we need efifos, this might double number of fifos when num_fifos < 4? */
    }
	...
	if (!SOC_IS_SBX_BME(unit)) {
	    if (num_fifos > sp_info->unused_fifos) {
		rv = MBCM_SBX_DRIVER_CALL(unit, mbcm_cosq_scheduler_allocate, unit, SIRIUS_ES_LEVEL_FIFO, 
					  BCM_INT_SBX_REQUESTED_SCHEDULER_AUTO_SELECT, num_fifos + efifos, TRUE, &level0_node);
		
		if (rv != BCM_E_NONE) {
		    (p_eg + temp_eg)->eg_in_use = FALSE;
		    (p_eg + temp_eg)->child_port = -1;
		    BCM_ERR(("ERROR: %s, failed to allocate %d level 0 egress scheduler nodes for gport 0x%x, Unit(%d)\n",
	    ...
	    if (num_fifos > sp_info->unused_fifos) {
		level0_node = logical_eg * num_fifos;
	    }
	}
    } else {
	...
	if ( (p_eg + logical_eg)->eg_in_use == FALSE ) {
	    *p_eg_gport = logical_eg;
	    ...
	    BCM_ERR(("ERROR: %s, egress group %d in use, Unit(%d)\n",
	...
	if (!SOC_IS_SBX_BME(unit)) {
	    if (num_fifos > sp_info->unused_fifos) {
		rv = MBCM_SBX_DRIVER_CALL(unit, mbcm_cosq_scheduler_allocate, unit, SIRIUS_ES_LEVEL_FIFO, 
					  logical_eg * SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_HW_SIZE, num_fifos + efifos, TRUE, &level0_node);
		if (rv != BCM_E_NONE) {
	    ...
	    if (num_fifos > sp_info->unused_fifos) {
		level0_node = logical_eg * SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_HW_SIZE;
	    }
	}
    }
	...
	/*
	 * If we have extra fifos and can keep contiguity, then use them.
	 */

	if (level0_node == -1) {
	    sp_info->egroup[idx].es_scheduler_level0_node = (sp_info->egroup[idx-1].es_scheduler_level0_node + 
							     sp_info->egroup[idx-1].num_fifos);
	    sp_info->egroup[idx].num_fifos = num_fifos;
	    sp_info->unused_fifos -= num_fifos;
		...
		if (rv != BCM_E_NONE) {
		    BCM_ERR(("Could not allocate fcd\n"));
		    return rv;
		}
	...
	if (!SOC_IS_SBX_BME(unit)) {
	    /*
	     * Allocate port speed if not present
	     */
	    
	    speed = sp_info->egroup[idx].num_fifos;
	    if (speed % SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_HW_SIZE) {
		/* Initialize fifo speeds in chunks of SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_HW_SIZE */
		speed += (num_fifos % SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_HW_SIZE);
	    }
	    if (SOC_SBX_CFG_SIRIUS(unit)->uSubportSpeed[intf][port][idx][SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_MAX_SIZE] == 0) {
		SOC_SBX_CFG_SIRIUS(unit)->uSubportSpeed[intf][port][idx][SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_MAX_SIZE] = 1000;
		for(fifo = 0; fifo < speed; fifo++) {
		    SOC_SBX_CFG_SIRIUS(unit)->uSubportSpeed[intf][port][idx][fifo] = 1000;
		}
	    }
	    sp_info->egroup[idx].port_speed = SOC_SBX_CFG_SIRIUS(unit)->uSubportSpeed[intf][port][idx][SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_MAX_SIZE];
	}
				  ...
				  int *egroup_num, int *num_fifos)
{
    ...
    bcm_sbx_subport_info_t *sp_info;
    bcm_sbx_cosq_egress_group_state_t *p_eg = NULL;
    int fport = 0, idx = 0, logical_eg = 0, max = 0;
    ...
    if (!(BCM_GPORT_IS_EGRESS_CHILD(egress_group) || 
	  BCM_GPORT_IS_CHILD(egress_group) || 
	  BCM_GPORT_IS_EGRESS_GROUP(egress_group))) {
	return BCM_E_PARAM;
    }
	...
	if (BCM_GPORT_IS_CHILD(egress_group) ||
	    ...
	    sp_info = &(SOC_SBX_STATE(unit)->port_state->subport_info[fport]);
	    *fabric_port = fport;
	    if (egroup_num != NULL) {
		*egroup_num = 0;
	    }
	...
	p_eg = SOC_SBX_STATE(unit)->egress_group_state;
	logical_eg = BCM_GPORT_EGRESS_GROUP_GET(egress_group);
	...
	if ((p_eg + logical_eg)->eg_in_use == FALSE) {
	    BCM_ERR(("gport Egress Group not in use\n"));
	    return BCM_E_PARAM;
	}
	fport = (p_eg + logical_eg)->child_port;
	if ((fport < 0) || (fport >=  SB_FAB_DEVICE_MAX_FABRIC_PORTS)) {
	    BCM_ERR(("ERROR: %s, egress_group 0x%x does not associated with any fabric port, unit %d\n",
		     FUNCTION_NAME(), egress_group, unit));	    
	    return BCM_E_PARAM;	    
	}
	sp_info = &(SOC_SBX_STATE(unit)->port_state->subport_info[fport]);
    ...
    if (BCM_GPORT_IS_EGRESS_CHILD(egress_group)) {
	logical_eg = BCM_GPORT_EGRESS_CHILD_PORT_GET(egress_group);
	max = 1;
    } else if (BCM_GPORT_IS_CHILD(egress_group)) {
	logical_eg = BCM_GPORT_CHILD_PORT_GET(egress_group);
	max = 1;
    } else {
	logical_eg = BCM_GPORT_EGRESS_GROUP_GET(egress_group);
...
bcm_sbx_cosq_egress_group_free(int unit, bcm_gport_t eg_gport)
{
    ...
    bcm_sbx_subport_info_t *sp_info;
    int child_port = 0;
    int intf = 0, port = 0, fifo = 0, rv = 0;
    int idx = 0, attid = 0, efifos = 0;
	...
	if (!SOC_IS_SBX_BME(unit)) {
	    ...
	    /*
	     * Deallocate port speed if not present
	     */
	    
	    SOC_SBX_CFG_SIRIUS(unit)->uSubportSpeed[intf][port][idx][SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_MAX_SIZE] = 0;
	    for(fifo = 0; fifo < sp_info->egroup[idx].num_fifos; fifo++) {
		SOC_SBX_CFG_SIRIUS(unit)->uSubportSpeed[intf][port][idx][fifo] = 0;
	    }
	}
	...
	if (SOC_IS_SIRIUS(unit) && 
	    ...
	    efifos = sp_info->egroup[idx].num_fifos % SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_HW_SIZE;
	}

	/* dispose of fifo level */
	for (fifo = sp_info->egroup[idx].es_scheduler_level0_node; 
	     fifo < sp_info->egroup[idx].es_scheduler_level0_node + sp_info->egroup[idx].num_fifos + efifos;
	     fifo++) {
	...
	rv = _bcm_sbx_cosq_fcd_free(unit, sp_info->egroup[idx].ef_fcd);
	if (BCM_E_NONE != rv) {
	    BCM_ERR(("ERROR: %s, failed to free fcd %d for gport 0x%x, Unit(%d)\n",
		     FUNCTION_NAME(),
	...
	if (SOC_IS_SIRIUS(unit) && 
	    (SOC_SBX_CFG_SIRIUS(unit)->bExtendedPortMode == 0) &&
	    (SOC_SBX_CFG(unit)->bEgressFifoIndependentFlowControl == 1)) {
	    rv = _bcm_sbx_cosq_fcd_free(unit, sp_info->egroup[idx].nef_fcd);
	    if (BCM_E_NONE != rv) {
		BCM_ERR(("ERROR: %s, failed to free fcd %d for gport 0x%x, Unit(%d)\n",
			 FUNCTION_NAME(),
	...
	sp_info->egroup[idx].port_speed = 0;
	for (attid = 0; attid < SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_MAX_SIZE; attid++) {
	  sp_info->egroup[idx].fcd[attid] = BCM_INT_SBX_INVALID_FCD;
	}
	(p_eg + logical_eg)->child_port = -1;
...
bcm_sbx_cosq_multicast_queue_group_get(int unit, bcm_fabric_distribution_t ds_id, int32 *p_base_queue)
{
    ...
    if (base_queue == -1) {
        soc_cm_print("Queue not allocated for this distribution_group(%d)\n", ds_id);
    }
    else {
      soc_cm_print("ds_id(%d) associated with base queue(%d)\n", ds_id, base_queue);
    }
...
bcm_sbx_gport_fifo_get(int unit, bcm_gport_t gport, int *p_fifo, bcm_module_t *module)
{
	...
	if ((p_eg + logical_eg)->eg_in_use == FALSE) {
        ...
        *module = BCM_GPORT_EGRESS_GROUP_MODID_GET(gport);
        ...
        BCM_ERR(("gport type unsupported\n"));
        rv = BCM_E_PARAM;
    }
    return rv;
}
...
bcm_sbx_cosq_detach(int unit)
{
    ...
    rv = MBCM_SBX_DRIVER_MAYBE_CALL(unit,
                                    ...
                                    (unit));
    if (BCM_E_NONE != rv) {
        BCM_ERR(("error %d (%s) detaching unit %d cosq\n",
                 rv,
                 _SHR_ERRMSG(rv),
                 unit));
        BCM_SBX_UNLOCK(unit);
...
{
    int                  rv = BCM_E_NONE;
    int                  num_cos, tmp_len=0, i=0, size;
    uint8                *scache_ptr, *ptr, dummy = 0;
    ...
    if (SOC_SBX_STATE(unit) == NULL) {
        BCM_ERR(("%s: Internal error. Invalid cosq state (NULL) \n", 
                 FUNCTION_NAME()));
        return BCM_E_INTERNAL;
    }

    scache_handle = SOC_SBX_STATE(unit)->cosq_scache_handle;
    num_cos = sbx_num_cosq[unit];

    /* During cold-boot. Allocate a stable cache */
    if (SOC_WARM_BOOT(unit) == FALSE) {
	scache_len += sizeof(sbx_num_cosq[unit]);
	scache_len += sizeof(SOC_SBX_CFG(unit)->max_ports);
        scache_len += sizeof(SOC_SBX_CFG(unit)->nMaxFabricPortsOnModule);
	scache_len += sizeof(SOC_SBX_CFG(unit)->uFabricConfig);
	scache_len += sizeof(SOC_SBX_CFG(unit)->use_extended_esets);
	scache_len += sizeof(SOC_SBX_CFG(unit)->arbitration_port_allocation);
	scache_len += sizeof(SOC_SBX_CFG(unit)->cfg_node_00_31_mask);
	scache_len += sizeof(SOC_SBX_CFG(unit)->cfg_node_32_63_mask);
	scache_len += sizeof(SOC_SBX_CFG(unit)->cfg_node_64_95_mask);
	scache_len += sizeof(SOC_SBX_CFG(unit)->num_ds_ids);
       
	/* Allocate Queue State Buffer */
	scache_len += sizeof(SOC_SBX_CFG(unit)->num_queues);
	tmp_len = 0;
	/* Compress Queue State */
	tmp_len = sizeof(int32);  /* state */
	tmp_len += sizeof(int16); /* bw_group */
	tmp_len += sizeof(int16); /* sysport */
	tmp_len += sizeof(int16); /* default_sysport */

	/* bcm_sbx_cosq_queue_params_ingress_t */
	tmp_len += sizeof(int8);  /* bw_mode */
	tmp_len += sizeof(int8);  /* DUMMY 1 */
	tmp_len += sizeof(bcm_sbx_cosq_bw_value_t);
	tmp_len += sizeof(int32); /* given_weight */
	tmp_len += sizeof(int32); /* min_physical_queue_depth_bytes */
	tmp_len += sizeof(int32); /* max_physical_queue_depth_bytes */
	tmp_len += sizeof(int8);  /* anemic_watermark_select */
	tmp_len += sizeof(int8);  /* ingress_spi4 */
	tmp_len += sizeof(int8);  /* ingress_spi4_port */
	tmp_len += sizeof(int8);  /* DUMMY 2 */
	tmp_len += sizeof(int32); /* qla_demand_mask */
	tmp_len += sizeof(int8);  /* queue_header_adjust_sign */
	tmp_len += sizeof(int8);  /* queue_header_adjust */
	tmp_len += sizeof(int8);  /* anemic_age_threshold_key */
	tmp_len += sizeof(int8);  /* DUMMY 3 */
	tmp_len += sizeof(int32); /* shape_limit_kbps */
	tmp_len += sizeof(int8);  /* enable_shaping */
	tmp_len += sizeof(int8);  /* DUMMY 4 */
	tmp_len += sizeof(int8);  /* DUMMY 5 */
	tmp_len += sizeof(int8);  /* DUMMY 6 */
	tmp_len += sizeof(int32); /* shape_burst_kbps */
	tmp_len += sizeof(int8);  /* local */
	tmp_len += sizeof(int8);  /* enabled **MOVED FROM BOTTOM OF STRUCTURE** */
	tmp_len += sizeof(int16); /* hold_pri_num_timeslots */
	tmp_len += sizeof(int32); /* template */
	tmp_len += sizeof(int32); /* gain */
	tmp_len += sizeof(uint8); /* enq_stat_in_use*/
	tmp_len += sizeof(uint8); /* deq_stat_in_use*/
	tmp_len += sizeof(uint8); /* attached_fifo*/
	tmp_len += sizeof(uint8); /* DUMMY 7 */

#ifdef BCM_SIRIUS_SUPPORT
	if (SOC_IS_SIRIUS(unit)) {
	    num_ds_ids = ((SOC_SBX_CFG(unit)->use_extended_esets) ? 
			  SOC_SBX_CFG(unit)->num_ds_ids : 
			  SB_FAB_SWITCH_CONFIG_MAX_NUM_ESETS);
	    tmp_len += (sizeof(bcm_sbx_mgid_list_t) * num_ds_ids);
	}
#endif

	scache_len += (tmp_len * SOC_SBX_CFG(unit)->num_queues);

	/* queue region */
	scache_len += (sizeof(bcm_sbx_cosq_queue_region_t) * bcm_sbx_cosq_queue_region_last);

	/* bandwidth group state */
	scache_len += sizeof(SOC_SBX_CFG(unit)->num_bw_groups);
	tmp_len = 0;
	/* Compress bandwidth group State */
	tmp_len += sizeof(bcm_gport_t);
	tmp_len += sizeof(int16); /* base_queue */
	tmp_len += sizeof(int8);  /* num_cos */
	tmp_len += sizeof(int8);  /* in_use */
	tmp_len += sizeof(int16); /* dest_node */
	tmp_len += sizeof(int16); /* dest_port */
	tmp_len += sizeof(int8);  /* dest_mc */
	tmp_len += sizeof(int8);  /* DUMMY 1 */
	tmp_len += sizeof(int16); /* dest_type */
	tmp_len += sizeof(int32); /* dest_port_kbits_sec_max */
	tmp_len += sizeof(bcm_sbx_cosq_bw_group_params_t);
	tmp_len += sizeof(int16);  /* flags */
	tmp_len += sizeof(int8);   /* overlay_in_use */
	tmp_len += sizeof(int8);  /* DUMMY 2 */
	tmp_len += sizeof(int16);  /* overlay_dest_node */
	tmp_len += sizeof(int16);  /* overlay_dest_port */
	tmp_len += sizeof(int16);  /* overlay_base_queue */
	tmp_len += sizeof(int8);   /* overlay_num_cos */
	tmp_len += sizeof(int8);   /* cosq_init_added */

	scache_len += (tmp_len * SOC_SBX_CFG(unit)->num_bw_groups);

	/* ds ids */
	if (ds_id_base_queue[unit] != NULL) {
	    scache_len += (sizeof(int) * SOC_SBX_CFG(unit)->num_ds_ids);
	}

	/* sysport state */
	scache_len += sizeof(SOC_SBX_CFG(unit)->num_sysports);
	scache_len += (sizeof(bcm_sbx_cosq_sysport_state_t) * SOC_SBX_CFG(unit)->num_sysports);

	/* sysport_group_state */
	scache_len += (sizeof(bcm_sbx_cosq_sysport_group_state_t) * BCM_INT_SBX_MAX_SYSPORT_GROUP);

        if ( soc_feature(unit, soc_feature_hybrid) ) {
	    /* Allocate device_cosq structures */
	    scache_len += _bcm_sbx_wb_device_cosq_state_sync(unit, 2);
	}
	
	/* fcd state */
	if (fcd_state[unit] != NULL) {
	    scache_len += (sizeof(bcm_sbx_cosq_fcd_state_t) * (SB_FAB_DEVICE_SIRIUS_MAX_FC_DOMAINS + 2));
	}

	/* Discard state */
	scache_len += sizeof(SOC_SBX_CFG(unit)->nDiscardTemplates);
	if (discard_state[unit] != NULL) {
	    scache_len += (sizeof(bcm_sbx_cosq_discard_state_t) * 
			   ((SOC_SBX_CFG(unit)->nDiscardTemplates) ? SOC_SBX_CFG(unit)->nDiscardTemplates : 1));
	}

	/* scheduler state */
	scache_len += sizeof(SOC_SBX_CFG(unit)->num_ingress_scheduler);
	if (ingress_scheduler_state[unit] != NULL) {
	    scache_len += (sizeof(bcm_sbx_cosq_ingress_scheduler_state_t) * SOC_SBX_CFG(unit)->num_ingress_scheduler);
	}

	scache_len += sizeof(SOC_SBX_CFG(unit)->num_egress_scheduler);
	if (egress_scheduler_state[unit] != NULL) {
	    scache_len += (sizeof(bcm_sbx_cosq_egress_scheduler_state_t) * SOC_SBX_CFG(unit)->num_egress_scheduler);
	}

	scache_len += sizeof(SOC_SBX_CFG(unit)->num_egress_group);
	if (egress_group_state[unit] != NULL) {
	    scache_len += (sizeof(bcm_sbx_cosq_egress_group_state_t) * SOC_SBX_CFG(unit)->num_egress_group);
	}

	/* multipath state */
	scache_len += sizeof(SOC_SBX_CFG(unit)->num_ingress_multipath);
	if (ingress_multipath_state[unit] != NULL) {
	    scache_len += (sizeof(bcm_sbx_cosq_ingress_multipath_state_t) * SOC_SBX_CFG(unit)->num_ingress_multipath);
	}

	scache_len += sizeof(SOC_SBX_CFG(unit)->num_egress_multipath);
	if (egress_multipath_state[unit] != NULL) {
	    scache_len += (sizeof(bcm_sbx_cosq_egress_multipath_state_t) * SOC_SBX_CFG(unit)->num_egress_multipath);
	}
	...
	if (SOC_IS_SBX_QE2000(unit)) {
	    ...
	    scache_len += sizeof(SOC_SBX_CONTROL(unit)->numQueues);
	    scache_len += _bcm_qe2000_wb_cosq_state_sync(unit, 2);
	}

        rv = soc_scache_alloc(unit, scache_handle, scache_len);
        if (rv != SOC_E_NONE) {
	...
	sbx_num_cosq[unit] = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	SOC_SBX_CFG(unit)->max_ports = *(((uint32 *) ptr)); ptr += sizeof(uint32);
        SOC_SBX_CFG(unit)->nMaxFabricPortsOnModule = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	SOC_SBX_CFG(unit)->uFabricConfig = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	SOC_SBX_CFG(unit)->use_extended_esets = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	SOC_SBX_CFG(unit)->arbitration_port_allocation = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	SOC_SBX_CFG(unit)->cfg_node_00_31_mask = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	SOC_SBX_CFG(unit)->cfg_node_32_63_mask = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	SOC_SBX_CFG(unit)->cfg_node_64_95_mask = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	SOC_SBX_CFG(unit)->num_ds_ids = *(((uint32 *) ptr)); ptr += sizeof(uint32);

	/* Decompress Queue State */
	SOC_SBX_CFG(unit)->num_queues = *(((uint32 *) ptr)); ptr += sizeof(uint32);

	for (i=0; i < SOC_SBX_CFG(unit)->num_queues; i++) {
	    queue_state[unit][i].state = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	    queue_state[unit][i].bw_group = *(((uint16 *) ptr)); ptr += sizeof(uint16);
	    queue_state[unit][i].sysport = *(((uint16 *) ptr)); ptr += sizeof(uint16);
	    queue_state[unit][i].default_sysport = *(((uint16 *) ptr)); ptr += sizeof(uint16);

	    /* bcm_sbx_cosq_queue_params_ingress_t */
	    queue_state[unit][i].ingress.bw_mode = *(((uint8 *) ptr)); ptr += sizeof(uint8);
	    dummy = *(((uint8 *) ptr)); ptr += sizeof(uint8);
	    queue_state[unit][i].ingress.bw_value.guarantee_kbps = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	    queue_state[unit][i].ingress.bw_value.sp_priority = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	    queue_state[unit][i].ingress.bw_value.wfq_weight = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	    queue_state[unit][i].ingress.given_weight = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	    queue_state[unit][i].ingress.min_physical_queue_depth_bytes = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	    queue_state[unit][i].ingress.max_physical_queue_depth_bytes = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	    queue_state[unit][i].ingress.anemic_watermark_select = *(((uint8 *) ptr)); ptr += sizeof(uint8);
	    queue_state[unit][i].ingress.ingress_spi4 = *(((uint8 *) ptr)); ptr += sizeof(uint8);
	    queue_state[unit][i].ingress.ingress_spi4_port = *(((uint8 *) ptr)); ptr += sizeof(uint8);
	    dummy = *(((uint8 *) ptr)); ptr += sizeof(uint8);
	    queue_state[unit][i].ingress.qla_demand_mask = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	    queue_state[unit][i].ingress.queue_header_adjust_sign = *(((uint8 *) ptr)); ptr += sizeof(uint8);
	    queue_state[unit][i].ingress.queue_header_adjust      = *(((uint8 *) ptr)); ptr += sizeof(uint8);
	    queue_state[unit][i].ingress.anemic_age_threshold_key = *(((uint8 *) ptr)); ptr += sizeof(uint8);
	    dummy = *(((uint8 *) ptr)); ptr += sizeof(uint8);
	    queue_state[unit][i].ingress.shape_limit_kbps = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	    queue_state[unit][i].ingress.enable_shaping = *(((uint8 *) ptr)); ptr += sizeof(uint8);
	    dummy = *(((uint8 *) ptr)); ptr += sizeof(uint8);
	    dummy = *(((uint8 *) ptr)); ptr += sizeof(uint8);
	    dummy = *(((uint8 *) ptr)); ptr += sizeof(uint8);
	    queue_state[unit][i].ingress.shape_burst_kbps = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	    queue_state[unit][i].ingress.local = *(((uint8 *) ptr)); ptr += sizeof(uint8);
	    queue_state[unit][i].ingress.enabled = *(((uint8 *) ptr)); ptr += sizeof(uint8);
	    queue_state[unit][i].ingress.hold_pri_num_timeslots = *(((uint16 *) ptr)); ptr += sizeof(uint16);
	    queue_state[unit][i].ingress.template = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	    queue_state[unit][i].ingress.gain = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	    queue_state[unit][i].enq_stat_in_use = *(((uint8 *) ptr)); ptr += sizeof(uint8);
	    queue_state[unit][i].deq_stat_in_use = *(((uint8 *) ptr)); ptr += sizeof(uint8);
	    queue_state[unit][i].attached_fifo = *(((uint8 *) ptr)); ptr += sizeof(uint8);
	    dummy = *(((uint8 *) ptr)); ptr += sizeof(uint8);
	}
	...
	if (SOC_IS_SIRIUS(unit)) {
	    num_ds_ids = ((SOC_SBX_CFG(unit)->use_extended_esets) ? 
			  SOC_SBX_CFG(unit)->num_ds_ids : 
			  SB_FAB_SWITCH_CONFIG_MAX_NUM_ESETS);
	    ptr += (sizeof(bcm_sbx_mgid_list_t) * num_ds_ids);
	}
#endif

	/* queue region */
	for (i = 0; i < bcm_sbx_cosq_queue_region_last; i++) {
	    queue_regions[unit][i].is_valid = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	    queue_regions[unit][i].start = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	    queue_regions[unit][i].end = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	}
	
	/* Compress bandwidth group State */
	SOC_SBX_CFG(unit)->num_bw_groups = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	
	for (i=0; i < SOC_SBX_CFG(unit)->num_bw_groups; i++) {
	    bw_group_state[unit][i].gport                   = *(((int *) ptr))   ; ptr += sizeof(bcm_gport_t);
	    bw_group_state[unit][i].base_queue              = *(((uint16 *) ptr)); ptr += sizeof(uint16);
	    bw_group_state[unit][i].num_cos                 = *(((uint8 *) ptr)) ; ptr += sizeof(uint8);
	    bw_group_state[unit][i].in_use                  = *(((uint8 *) ptr)) ; ptr += sizeof(uint8);
	    bw_group_state[unit][i].dest_node               = *(((uint16 *) ptr)); ptr += sizeof(uint16);
	    bw_group_state[unit][i].dest_port               = *(((uint16 *) ptr)); ptr += sizeof(uint16);
	    bw_group_state[unit][i].dest_mc                 = *(((uint8 *) ptr)) ; ptr += sizeof(uint8);
	    dummy                                           = *(((uint8 *) ptr)) ; ptr += sizeof(uint8);
	    bw_group_state[unit][i].dest_type               = *(((uint16 *) ptr)); ptr += sizeof(uint16);
	    bw_group_state[unit][i].dest_port_kbits_sec_max = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	    bw_group_state[unit][i].path.bag_rate_kbps      = *(((int32 *) ptr)) ; ptr += sizeof(uint32);
	    bw_group_state[unit][i].flags                   = *(((uint16 *) ptr)); ptr += sizeof(uint16);
	    bw_group_state[unit][i].overlay_in_use          = *(((uint8 *) ptr)) ; ptr += sizeof(uint8);
	    dummy                                           = *(((uint8 *) ptr)) ; ptr += sizeof(uint8);
	    bw_group_state[unit][i].overlay_dest_node       = *(((uint16 *) ptr)); ptr += sizeof(uint16);
	    bw_group_state[unit][i].overlay_dest_port       = *(((uint16 *) ptr)); ptr += sizeof(uint16);
	    bw_group_state[unit][i].overlay_base_queue      = *(((uint16 *) ptr)); ptr += sizeof(uint16);
	    bw_group_state[unit][i].overlay_num_cos         = *(((uint8 *) ptr)) ; ptr += sizeof(uint8);
	    bw_group_state[unit][i].cosq_init_added         = *(((uint8 *) ptr)) ; ptr += sizeof(uint8);
	}
	
	/* ds ids */
	if (ds_id_base_queue[unit] != NULL) {
	    for (i=0; i < SOC_SBX_CFG(unit)->num_ds_ids; i++) {
		ds_id_base_queue[unit][i] = *(((int *) ptr)); ptr += sizeof(int);
	    }
	}

	/* sysport state */
	SOC_SBX_CFG(unit)->num_sysports = *(((uint32 *) ptr)); ptr += sizeof(uint32);
    size = (sizeof(bcm_sbx_cosq_sysport_state_t) * 
            SOC_SBX_CFG(unit)->num_sysports);
    if (sysport_state[unit] != NULL) {
        sal_memcpy(sysport_state[unit], (((uint8 *)ptr)), size);
        ptr += size;
    }
	
	/* sysport_group_state */
    size = (sizeof(bcm_sbx_cosq_sysport_group_state_t) * 
            BCM_INT_SBX_MAX_SYSPORT_GROUP);
    if (sysport_group_state[unit] != NULL) {
	...
	SOC_SBX_STATE(unit)->cosq_index = ptr;
        ...
        _bcm_sbx_wb_device_cosq_state_sync(unit, TRUE);
    }
	...
	/* fcd state */
	if (fcd_state[unit] != NULL) {
	    sal_memcpy(fcd_state[unit], (((uint8 *)ptr)), (sizeof(bcm_sbx_cosq_fcd_state_t) * (SB_FAB_DEVICE_SIRIUS_MAX_FC_DOMAINS + 2)));
	    ptr += (sizeof(bcm_sbx_cosq_fcd_state_t) * (SB_FAB_DEVICE_SIRIUS_MAX_FC_DOMAINS + 2));
	}
	
	/* Discard state */
	SOC_SBX_CFG(unit)->nDiscardTemplates = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	if (discard_state[unit] != NULL) {
	    sal_memcpy(discard_state[unit], (((uint8 *)ptr)),
		       (sizeof(bcm_sbx_cosq_discard_state_t) * 
			((SOC_SBX_CFG(unit)->nDiscardTemplates) ? SOC_SBX_CFG(unit)->nDiscardTemplates : 1)));
	...
	/* scheduler state */
	SOC_SBX_CFG(unit)->num_ingress_scheduler = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	if (ingress_scheduler_state[unit] != NULL) {
	    sal_memcpy(ingress_scheduler_state[unit], (((uint8 *)ptr)),
		       (sizeof(bcm_sbx_cosq_ingress_scheduler_state_t) * SOC_SBX_CFG(unit)->num_ingress_scheduler));
	    ptr += (sizeof(bcm_sbx_cosq_ingress_scheduler_state_t) * SOC_SBX_CFG(unit)->num_ingress_scheduler);
	}
	
	SOC_SBX_CFG(unit)->num_egress_scheduler = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	if (egress_scheduler_state[unit] != NULL) {
	    sal_memcpy(egress_scheduler_state[unit], (((uint8 *)ptr)), 
		       (sizeof(bcm_sbx_cosq_egress_scheduler_state_t) * SOC_SBX_CFG(unit)->num_egress_scheduler));
	    ptr += (sizeof(bcm_sbx_cosq_egress_scheduler_state_t) * SOC_SBX_CFG(unit)->num_egress_scheduler);
	}
						 
	SOC_SBX_CFG(unit)->num_egress_group = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	if (egress_group_state[unit] != NULL) {
	    sal_memcpy(egress_group_state[unit], (((uint8 *)ptr)), 
		       (sizeof(bcm_sbx_cosq_egress_group_state_t) * SOC_SBX_CFG(unit)->num_egress_group));
	    ptr += (sizeof(bcm_sbx_cosq_egress_group_state_t) * SOC_SBX_CFG(unit)->num_egress_group);
	}

	/* multipath state */
	SOC_SBX_CFG(unit)->num_ingress_multipath = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	if (ingress_multipath_state[unit] != NULL) {
	    sal_memcpy(ingress_multipath_state[unit], (((uint8 *)ptr)),
		       (sizeof(bcm_sbx_cosq_ingress_multipath_state_t) * SOC_SBX_CFG(unit)->num_ingress_multipath));
	    ptr += (sizeof(bcm_sbx_cosq_ingress_multipath_state_t) * SOC_SBX_CFG(unit)->num_ingress_multipath);
	}
	
	SOC_SBX_CFG(unit)->num_egress_multipath = *(((uint32 *) ptr)); ptr += sizeof(uint32);
	if (egress_multipath_state[unit] != NULL) {
	    sal_memcpy(egress_multipath_state[unit], (((uint8 *)ptr)), 
		       (sizeof(bcm_sbx_cosq_egress_multipath_state_t) * SOC_SBX_CFG(unit)->num_egress_multipath));
	    ptr += (sizeof(bcm_sbx_cosq_egress_multipath_state_t) * SOC_SBX_CFG(unit)->num_egress_multipath);
	}
	...
	if (SOC_IS_SBX_QE2000(unit)) {
	    ...
	    SOC_SBX_CONTROL(unit)->numQueues = *(((int *) ptr)); ptr += sizeof(int);

	    SOC_SBX_STATE(unit)->cosq_index = ptr;

	    _bcm_qe2000_wb_cosq_state_sync(unit, TRUE);
...
bcm_sbx_wb_cosq_state_sync(int unit, int sync)
{
    ...
    rv = soc_scache_ptr_get(unit, scache_handle, &scache_ptr, &scache_len);
    if ((rv != SOC_E_NONE) || (scache_ptr == NULL) || (scache_len == 0)) {
        BCM_ERR(("%s: Error(%s) reading scache. scache_ptr:0x%lx and len:%d\n",
                 FUNCTION_NAME(), soc_errmsg(rv), scache_ptr, scache_len));
        return rv;
    ...
    *(((uint32 *) ptr)) = sbx_num_cosq[unit]                            ; ptr += sizeof(uint32);
    *(((uint32 *) ptr)) = SOC_SBX_CFG(unit)->max_ports                  ; ptr += sizeof(uint32);
    *(((uint32 *) ptr)) = SOC_SBX_CFG(unit)->nMaxFabricPortsOnModule    ; ptr += sizeof(uint32);
    *(((uint32 *) ptr)) = SOC_SBX_CFG(unit)->uFabricConfig              ; ptr += sizeof(uint32);
    *(((uint32 *) ptr)) = SOC_SBX_CFG(unit)->use_extended_esets         ; ptr += sizeof(uint32);
    *(((uint32 *) ptr)) = SOC_SBX_CFG(unit)->arbitration_port_allocation; ptr += sizeof(uint32);
    *(((uint32 *) ptr)) = SOC_SBX_CFG(unit)->cfg_node_00_31_mask        ; ptr += sizeof(uint32);
    *(((uint32 *) ptr)) = SOC_SBX_CFG(unit)->cfg_node_32_63_mask        ; ptr += sizeof(uint32);
    *(((uint32 *) ptr)) = SOC_SBX_CFG(unit)->cfg_node_64_95_mask        ; ptr += sizeof(uint32);
    *(((uint32 *) ptr)) = SOC_SBX_CFG(unit)->num_ds_ids                 ; ptr += sizeof(uint32);
       
    /* Compress Queue State */
    *(((uint32 *) ptr)) = SOC_SBX_CFG(unit)->num_queues; ptr += sizeof(uint32);

    for (i=0; i < SOC_SBX_CFG(unit)->num_queues; i++) {
	*(((uint32 *) ptr)) = queue_state[unit][i].state           ; ptr += sizeof(uint32);
	*(((uint16 *) ptr)) = queue_state[unit][i].bw_group        ; ptr += sizeof(uint16);
	*(((uint16 *) ptr)) = queue_state[unit][i].sysport         ; ptr += sizeof(uint16);
	*(((uint16 *) ptr)) = queue_state[unit][i].default_sysport ; ptr += sizeof(uint16);

	/* bcm_sbx_cosq_queue_params_ingress_t */
	*(((uint8 *) ptr))  = queue_state[unit][i].ingress.bw_mode                       ; ptr += sizeof(uint8);
	*(((uint8 *) ptr))  = dummy                                                      ; ptr += sizeof(uint8);
	*(((uint32 *) ptr)) = queue_state[unit][i].ingress.bw_value.guarantee_kbps       ; ptr += sizeof(uint32);
	*(((uint32 *) ptr)) = queue_state[unit][i].ingress.bw_value.sp_priority          ; ptr += sizeof(uint32);
	*(((uint32 *) ptr)) = queue_state[unit][i].ingress.bw_value.wfq_weight           ; ptr += sizeof(uint32);
	*(((uint32 *) ptr)) = queue_state[unit][i].ingress.given_weight                  ; ptr += sizeof(uint32);
	*(((uint32 *) ptr)) = queue_state[unit][i].ingress.min_physical_queue_depth_bytes; ptr += sizeof(uint32);
	*(((uint32 *) ptr)) = queue_state[unit][i].ingress.max_physical_queue_depth_bytes; ptr += sizeof(uint32);
	*(((uint8 *) ptr))  = queue_state[unit][i].ingress.anemic_watermark_select       ; ptr += sizeof(uint8);
	*(((uint8 *) ptr))  = queue_state[unit][i].ingress.ingress_spi4                  ; ptr += sizeof(uint8);
	*(((uint8 *) ptr))  = queue_state[unit][i].ingress.ingress_spi4_port             ; ptr += sizeof(uint8);
	*(((uint8 *) ptr))  = dummy                                                      ; ptr += sizeof(uint8);
    *(((uint32 *) ptr)) = queue_state[unit][i].ingress.qla_demand_mask               ; ptr += sizeof(uint32);
	*(((uint8 *) ptr))  = queue_state[unit][i].ingress.queue_header_adjust_sign      ; ptr += sizeof(uint8);
	*(((uint8 *) ptr))  = queue_state[unit][i].ingress.queue_header_adjust           ; ptr += sizeof(uint8);
	*(((uint8 *) ptr))  = queue_state[unit][i].ingress.anemic_age_threshold_key      ; ptr += sizeof(uint8);
	*(((uint8 *) ptr))  = dummy                                                      ; ptr += sizeof(uint8);
	*(((uint32 *) ptr)) = queue_state[unit][i].ingress.shape_limit_kbps              ; ptr += sizeof(uint32);
	*(((uint8 *) ptr))  = queue_state[unit][i].ingress.enable_shaping                ; ptr += sizeof(uint8);
	*(((uint8 *) ptr))  = dummy                                                      ; ptr += sizeof(uint8);
	*(((uint8 *) ptr))  = dummy                                                      ; ptr += sizeof(uint8);
	*(((uint8 *) ptr))  = dummy                                                      ; ptr += sizeof(uint8);
	*(((uint32 *) ptr)) = queue_state[unit][i].ingress.shape_burst_kbps              ; ptr += sizeof(uint32);
	*(((uint8 *) ptr))  = queue_state[unit][i].ingress.local                         ; ptr += sizeof(uint8);
	*(((uint8 *) ptr))  = queue_state[unit][i].ingress.enabled                       ; ptr += sizeof(uint8);
	*(((uint16 *) ptr)) = queue_state[unit][i].ingress.hold_pri_num_timeslots        ; ptr += sizeof(uint16);
	*(((uint32 *) ptr)) = queue_state[unit][i].ingress.template                      ; ptr += sizeof(uint32);
	*(((uint32 *) ptr)) = queue_state[unit][i].ingress.gain                          ; ptr += sizeof(uint32);
    }
    ...
    /* queue region */
    for (i = 0; i < bcm_sbx_cosq_queue_region_last; i++) {
	*(((uint32 *) ptr)) = queue_regions[unit][i].is_valid; ptr += sizeof(uint32);
	*(((uint32 *) ptr)) = queue_regions[unit][i].start   ; ptr += sizeof(uint32);
	*(((uint32 *) ptr)) = queue_regions[unit][i].end     ; ptr += sizeof(uint32);
    }
    
    /* bandwidth group state */
    *(((uint32 *) ptr)) = SOC_SBX_CFG(unit)->num_bw_groups; ptr += sizeof(uint32);

    /* Compress bandwidth group State */
    for (i=0; i < SOC_SBX_CFG(unit)->num_bw_groups; i++) {
	*(((int *) ptr)) = bw_group_state[unit][i].gport; ptr += sizeof(bcm_gport_t);
	*(((uint16 *) ptr)) = bw_group_state[unit][i].base_queue ; ptr += sizeof(uint16);
	*(((uint8 *) ptr)) = bw_group_state[unit][i].num_cos ; ptr += sizeof(uint8);
	*(((uint8 *) ptr)) = bw_group_state[unit][i].in_use  ; ptr += sizeof(uint8);
	*(((uint16 *) ptr)) = bw_group_state[unit][i].dest_node ; ptr += sizeof(uint16);
	*(((uint16 *) ptr)) = bw_group_state[unit][i].dest_port ; ptr += sizeof(uint16);
	*(((uint8 *) ptr)) = bw_group_state[unit][i].dest_mc  ; ptr += sizeof(uint8);
	*(((uint8 *) ptr))  = dummy                           ; ptr += sizeof(uint8);
	*(((uint16 *) ptr)) = bw_group_state[unit][i].dest_type ; ptr += sizeof(uint16);
	*(((uint32 *) ptr)) = bw_group_state[unit][i].dest_port_kbits_sec_max; ptr += sizeof(uint32);
	*(((int32 *) ptr)) = bw_group_state[unit][i].path.bag_rate_kbps; ptr += sizeof(uint32);
	*(((uint16 *) ptr)) = bw_group_state[unit][i].flags              ; ptr += sizeof(uint16);
	*(((uint8 *) ptr))  = bw_group_state[unit][i].overlay_in_use     ; ptr += sizeof(uint8);
	*(((uint8 *) ptr))  = dummy                                      ; ptr += sizeof(uint8);
	*(((uint16 *) ptr)) = bw_group_state[unit][i].overlay_dest_node  ; ptr += sizeof(uint16);
	*(((uint16 *) ptr)) = bw_group_state[unit][i].overlay_dest_port  ; ptr += sizeof(uint16);
	*(((uint16 *) ptr)) = bw_group_state[unit][i].overlay_base_queue; ptr += sizeof(uint16);
	*(((uint8 *) ptr))  = bw_group_state[unit][i].overlay_num_cos   ; ptr += sizeof(uint8);
	*(((uint8 *) ptr))  = bw_group_state[unit][i].cosq_init_added   ; ptr += sizeof(uint8);
    }

    /* ds ids */
    if (ds_id_base_queue[unit] != NULL) {
        for (i=0; i < SOC_SBX_CFG(unit)->num_ds_ids; i++) {
            *(((int *) ptr)) = ds_id_base_queue[unit][i]; ptr += sizeof(int);
        }
    }

    /* sysport state */
    *(((uint32 *) ptr)) = SOC_SBX_CFG(unit)->num_sysports; 
    ptr += sizeof(uint32);
    size = (sizeof(bcm_sbx_cosq_sysport_state_t) * 
            SOC_SBX_CFG(unit)->num_sysports);
    if ((SOC_SBX_CFG(unit)->num_sysports > 0) && (sysport_state[unit] != NULL)) {
        sal_memcpy((((uint8 *)ptr)), sysport_state[unit], size);
        ptr += size;
    }
    ...
    if ( soc_feature(unit, soc_feature_hybrid) ) {
        ...
        _bcm_sbx_wb_device_cosq_state_sync(unit, FALSE);
    }
    ptr = SOC_SBX_STATE(unit)->cosq_index;

    /* fcd state */
    if (fcd_state[unit] != NULL) {
        size = (sizeof(bcm_sbx_cosq_fcd_state_t) * 
                (SB_FAB_DEVICE_SIRIUS_MAX_FC_DOMAINS + 2));
        sal_memcpy((((uint8 *)ptr)), fcd_state[unit], size);
        ptr += size;
    }

    /* Discard state */
    *(((uint32 *) ptr)) = SOC_SBX_CFG(unit)->nDiscardTemplates; 
    ptr += sizeof(uint32);
    if (discard_state[unit] != NULL) {
        size = (sizeof(bcm_sbx_cosq_discard_state_t) * 
                ((SOC_SBX_CFG(unit)->nDiscardTemplates) ? 
                 SOC_SBX_CFG(unit)->nDiscardTemplates : 1));
        sal_memcpy((((uint8 *)ptr)), discard_state[unit], size);
        ptr += size;
    }

    /* scheduler state */
    *(((uint32 *) ptr)) = SOC_SBX_CFG(unit)->num_ingress_scheduler; 
    ptr += sizeof(uint32);
    if (ingress_scheduler_state[unit] != NULL) {
        size = (sizeof(bcm_sbx_cosq_ingress_scheduler_state_t) * 
                SOC_SBX_CFG(unit)->num_ingress_scheduler);
        sal_memcpy((((uint8 *)ptr)), ingress_scheduler_state[unit], size);
        ptr += size;
    }

    *(((uint32 *) ptr)) = SOC_SBX_CFG(unit)->num_egress_scheduler; 
    ptr += sizeof(uint32);
    if (egress_scheduler_state[unit] != NULL) {
        size = (sizeof(bcm_sbx_cosq_egress_scheduler_state_t) * 
                SOC_SBX_CFG(unit)->num_egress_scheduler);
        sal_memcpy((((uint8 *)ptr)), egress_scheduler_state[unit], size);
        ptr += size;
    }

    *(((uint32 *) ptr)) = SOC_SBX_CFG(unit)->num_egress_group; 
    ptr += sizeof(uint32);
    if (egress_group_state[unit] != NULL) {
        size = (sizeof(bcm_sbx_cosq_egress_group_state_t) * 
                SOC_SBX_CFG(unit)->num_egress_group);
        sal_memcpy((((uint8 *)ptr)), egress_group_state[unit], size);
        ptr += size;
    }

    /* multipath state */
    *(((uint32 *) ptr)) = SOC_SBX_CFG(unit)->num_ingress_multipath; 
    ptr += sizeof(uint32);
    if (ingress_multipath_state[unit] != NULL) {
        size = (sizeof(bcm_sbx_cosq_ingress_multipath_state_t) * 
                SOC_SBX_CFG(unit)->num_ingress_multipath);
        sal_memcpy((((uint8 *)ptr)), ingress_multipath_state[unit], size);
        ptr += size;
    }

    *(((uint32 *) ptr)) = SOC_SBX_CFG(unit)->num_egress_multipath; 
    ptr += sizeof(uint32);
    if (egress_multipath_state[unit] != NULL) {
        size = (sizeof(bcm_sbx_cosq_egress_multipath_state_t) * 
                SOC_SBX_CFG(unit)->num_egress_multipath);
        sal_memcpy((((uint8 *)ptr)), egress_multipath_state[unit], size);
        ptr += size;
    }
    ...
    if (SOC_IS_SBX_QE2000(unit)) {
        ...
        *(((int *) ptr)) = SOC_SBX_CONTROL(unit)->numQueues; ptr += sizeof(int);

        SOC_SBX_STATE(unit)->cosq_index = ptr;

        _bcm_qe2000_wb_cosq_state_sync(unit, FALSE);
...
bcm_sbx_cosq_init(int unit)
{
	...
	SOC_SCACHE_HANDLE_SET(SOC_SBX_STATE(unit)->cosq_scache_handle, unit, BCM_MODULE_COSQ, 0);
	rv = bcm_sbx_wb_cosq_state_init(unit);
	if (rv != BCM_E_NONE) {
	    BCM_ERR(("%s: error in WarmBoot fabric state init \n", 
		     FUNCTION_NAME()));
	}
	}
    }
...
bcm_sbx_cosq_gport_flow_control_get(int unit, bcm_gport_t port, bcm_cos_t int_pri, uint32 *flow_control_mask)
{
    ...
    if (!flow_control_mask) {
        BCM_ERR(("ERROR: parameter error - required pointer (flow_control_mask) not being passed\n"));
        return(BCM_E_PARAM);
    }
                                                                                ...
                                                                                uint32 flags)
{
    ...
    if (flags & ~(BCM_COSQ_SCHED_CONFIG_EXPEDITE)) {
        BCM_ERR(("ERROR: unsupported flags value (%d)\n", flags));
        return(BCM_E_PARAM);
    } 
...
int bcm_sbx_cosq_gport_destmod_attach(int unit, bcm_gport_t port,
...
int bcm_sbx_cosq_gport_destmod_detach(int unit, bcm_gport_t port,
                                      bcm_port_t ingress_port, 
                                      bcm_module_t dest_modid, 
                                      int fabric_egress_port)
{
				     ...
				     bcm_cos_queue_t member_gport_cosq)
{
    ...
    if (BCM_COSQ_GPORT_IS_MULTIPATH(multipath_gport) == FALSE) {
        BCM_ERR(("ERROR: gport 0x%x not multipath gport\n", multipath_gport));
	return BCM_E_PARAM;
    }
	...
	if ((BCM_GPORT_IS_EGRESS_GROUP(member_gport) == FALSE) &&
	    ...
	    BCM_ERR(("ERROR: gport 0x%x not supported by egress multipath shaper\n", member_gport));
	    return BCM_E_PARAM;
	}
	...
	if (p_e_multipath->in_use != TRUE) {
	    BCM_ERR(("ERROR: egress multipath id %d not created\n", multipath));
	    return BCM_E_PARAM;
	}
    } else {
	if (!((BCM_GPORT_IS_CHILD(member_gport)) ||
	    ...
	    BCM_ERR(("ERROR: gport 0x%x type not supported by ingress multipath\n", member_gport));
	    return BCM_E_PARAM;
	}
	...
	if (p_i_multipath->in_use != TRUE) {
	    BCM_ERR(("ERROR: ingress multipath id %d not created\n", multipath));
	    return BCM_E_PARAM;
	}
    }
	...
	if (rv != BCM_E_NONE) {
	    BCM_ERR(("%s: can not find egress scheduler for gport 0x%x on unit %d\n",
	...
	if (rv != BCM_E_NONE) {
	    BCM_ERR(("%s: can not find ingress scheduler for gport 0x%x on unit %d\n",
		     FUNCTION_NAME(), member_gport, unit));
	    return rv;	    
	}
	...
	if ((p_i_multipath->level != -1) &&
	    ...
	    BCM_ERR(("ERROR: ingress scheduler 0x%x not at same level as multipath gport\n", member_gport));
	    return BCM_E_PARAM;	    
	}
				...
				int *pfc_id)
{
	...
	BCM_ERR(("ERROR: invalid xoff_threshold\n"));
	return BCM_E_PARAM;
    }

    if (config.xon_threshold < 0) {
	BCM_ERR(("ERROR: invalid xon_threshold\n"));
	return BCM_E_PARAM;
    }

    if (config.drop_threshold < 0) {
	BCM_ERR(("ERROR: invalid drop_threshold\n"));
	return BCM_E_PARAM;
    }

    if (config.reserved_buffers < 0) {
	BCM_ERR(("ERROR: invalid reserved_buffers\n"));
	return BCM_E_PARAM;
    }

    rv = MBCM_SBX_DRIVER_CALL(unit, mbcm_cosq_pfc_config_set,
			      unit, gport, cosq, flags, config, NULL);
