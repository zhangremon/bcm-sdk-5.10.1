/*
 * $Id: field.c 1.1.2.15 Broadcom SDK $
 *
...
_bcm_ea_field_operator_equal(
	...
	FP_VVERB(("_bcm_ea_field_operator_equal\n"));
	while (i < len){
		FP_VVERB(("mask[%d]=%02X, data[%d]=%02X\n", i, mask[i], i, data[i]));
		if (mask[i] == data[i]){
			iflag++;
		}
		i++;
	}
	FP_VVERB(("iflag=%d, len=%d\n", iflag, len));
	if (iflag == len){
...
_bcm_ea_field_operator_equal_v2(
	...
	FP_VVERB(("_bcm_ea_field_operator_equal_v2\n"));
	mflag = 0;
	while (i < len){
		FP_VVERB(("mask[%d]=%02X, data[%d]=%02X\n", i, mask[i], i, data[i]));
		if (mask[i] == 0xff){
...
_bcm_ea_field_operator_nevermatch(
	...
	int mflag = 0;  
	int fflag = 0;
	FP_VVERB(("_bcm_ea_field_operator_nevermatch\n"));
	while (i < len){
		FP_VVERB(("mask[%d]=%02X, data[%d]=%02X\n", i, mask[i], i, data[i]));
		if (data[i] != 0){
			dflag = 1;
	...
	FP_VVERB(("mflag=%d, dflag=%d, len=%d\n", mflag, dflag, len));
...
_bcm_ea_field_operator_notequal(
	...
	FP_VVERB(("_bcm_ea_field_operator_notequal\n"));
	while (i < len){
		FP_VVERB(("mask[%d]=%02X, data[%d]=%02X\n", i, mask[i], i, data[i]));
...
_bcm_ea_field_operator_notequal_v2(
	...
	FP_VVERB(("_bcm_ea_field_operator_notequal_v2\n"));
	while (i < len){
		FP_VVERB(("mask[%d]=%02X, data[%d]=%02X\n", i, mask[i], i, data[i]));
...
_bcm_ea_field_operator_alwaysmatch(
	...
	FP_VVERB(("_bcm_ea_field_operator_alwaysmatch\n"));
	while (i < len) {
		FP_VVERB(("mask[%d]=%02X, data[%d]=%02X\n", i, mask[i], i, data[i]));
		if (data[i] == 0){
			dflag++;
...
_bcm_ea_field_operator_cond(uint8 *data, uint8 *mask, int len){
	if (OPERATOR_TRUE ==
			_bcm_ea_field_operator_notequal(data, mask, len)){
