/*
 * $Id: port.c 1.1.2.39 Broadcom SDK $
 *
	    ...
	    int *value)
{
	...
	soc_cm_debug(DK_PORT,"enter _bcm_ea_port_control_get()\n");
	if (TK371X_PON_PORT_VALID(port)){
		soc_cm_debug(DK_PORT,"port type: PON=%d\n", port);
		switch (type){
			case bcmPortControlPonFecMode:{
				OamExtFECMode fec;
				ret_val = _soc_ea_fec_mode_get(unit, port, &fec);
				if (OK != ret_val){
					soc_cm_debug(DK_PORT,"calling _soc_ea_fec_mode_get failed\n");
					soc_cm_debug(DK_PORT,"return value: BCM_E_RESOURCE\n");
					return BCM_E_FAIL;
				}
				if (fec.rxFEC == 0){
					if (fec.txFEC == 0){
				...
				soc_cm_debug(DK_PORT,"type=bcmPortControlPonFecMode\n");
				soc_cm_debug(DK_PORT,"fec.rxFEC=%d\n", fec.rxFEC);
				soc_cm_debug(DK_PORT,"fec.txFEC=%d\n", fec.txFEC);
				soc_cm_debug(DK_PORT,"*value=%d\n", *value);
				return BCM_E_NONE;
			}
			case bcmPortControlPonUserTraffic:
				*value = _user_traffic_status;
				soc_cm_debug(DK_PORT,"type=bcmPortControlPonUserTraffic.\n");
				return BCM_E_NONE;
			case bcmPortControlPonMultiLlid:{
				uint32 llid;
				int state;
				soc_cm_debug(DK_PORT,"type=bcmPortControlPonMultiLlid.\n");
				ret_val = soc_auth_result_get(unit, &state);
				if (ret_val != SOC_E_NONE){
					return BCM_E_FAIL;
				}
				soc_cm_debug(DK_PORT,"soc_auth_result_get return val=%d, state=%d.\n", ret_val, state);
				if (state == TRUE){
					*value = -1;
					return BCM_E_NONE;
				}
				...
				if (ret_val != SOC_E_NONE){
					soc_cm_debug(DK_PORT,"_soc_ea_real_llid_count_get return BCM_E_RESOURCE, %d\n", ret_val);
					return BCM_E_FAIL;
				}
				soc_cm_debug(DK_PORT,"_soc_ea_real_llid_count_get llid_count=%d\n", llid);
				*value = llid;
				return BCM_E_NONE;
			}
			case bcmPortControlPonEncryptKeyExpiryTime:{
				uint16 time;
				soc_cm_debug(DK_PORT,"type=bcmPortControlPonEncryptKeyExpiryTime.\n");
				ret_val = _soc_ea_encrypt_key_expiry_time_get(unit, port, &time);
				if (OK != ret_val){
					soc_cm_debug(DK_PORT,"_soc_ea_encrypt_key_expiry_time_get return BCM_E_RESOURCE, %d\n", ret_val);
					return BCM_E_FAIL;
				}
				*value = (int)time;
				soc_cm_debug(DK_PORT,"_soc_ea_real_llid_count_get time=%d\n", time);
				return BCM_E_NONE;
			}
			case bcmPortControlPonHoldoverState:{
				...
				soc_cm_debug(DK_PORT,"type=bcmPortControlPonHoldoverState.\n");
				ret_val = CtcExtOamGetHoldover(unit, port, &state, &time);
				if (ret_val != OK){
					soc_cm_debug(DK_PORT,"CtcExtOamGetHoldover return BCM_E_RESOURCE, %d\n", ret_val);
					return BCM_E_FAIL;
				}
				soc_cm_debug(DK_PORT,"CtcExtOamGetHoldover state=%d, time=%d\n", state, time);
				*value = state;
				return BCM_E_NONE;
			}
			case bcmPortControlPonHoldoverTime:{
				uint32 state;
				uint32 time;
				soc_cm_debug(DK_PORT,"type=bcmPortControlPonHoldoverTime.\n");
				ret_val = CtcExtOamGetHoldover(unit, port, &state, &time);
				if (ret_val != OK){
					soc_cm_debug(DK_PORT,"CtcExtOamGetHoldover return BCM_E_RESOURCE, %d\n", ret_val);
					return BCM_E_FAIL;
		...
		soc_cm_debug(DK_PORT,"port type: UNI=%d\n", port);
		switch (type){
			case bcmPortControlEthPortAutoNegFailureAlarmState:{
				uint8 state;
				soc_cm_debug(DK_PORT,"type=bcmPortControlEthPortAutoNegFailureAlarmState.\n");
				ret_val = _soc_ea_alarm_enable_get(unit, port,
						SOC_EA_EVENT_AUTONEG_FAILURE_ALARM, &state);
				if (ret_val != OK){
					soc_cm_debug(DK_PORT,"_soc_ea_alarm_enable_get return BCM_E_RESOURCE, %d\n", ret_val);
					return BCM_E_FAIL;
				}
				if (state > 0){
					*value = 1;
				...
				soc_cm_debug(DK_PORT,"_soc_ea_alarm_enable_get state=%d\n", state);
				return BCM_E_NONE;
			}
			case bcmPortControlEthPortLosAlarmState:{
				uint8 state;
				soc_cm_debug(DK_PORT,"type=bcmPortControlEthPortLosAlarmState.\n");
				ret_val = _soc_ea_alarm_enable_get(unit, port,
			...
			case bcmPortControlEthPortFailureAlarmState:{
				uint8 state;
				soc_cm_debug(DK_PORT,"type=bcmPortControlEthPortFailureAlarmState.\n");
				ret_val = _soc_ea_alarm_enable_get(unit, port,
			...
			case bcmPortControlEthPortLoopbackAlarmState:{
				uint8 state;
				soc_cm_debug(DK_PORT,"type=bcmPortControlEthPortLoopbackAlarmState.\n");
				ret_val = _soc_ea_alarm_enable_get(unit, port,
			...
			case bcmPortControlEthPortCongestionAlarmState:{
				uint8 state;
				soc_cm_debug(DK_PORT,"type=bcmPortControlEthPortCongestionAlarmState.\n");
				ret_val = _soc_ea_alarm_enable_get(unit, port,
		...
		soc_cm_debug(DK_PORT,"port type: LLID=%d\n", port);
		return BCM_E_UNAVAIL;
	}else{
		return BCM_E_PORT;
	}
	...
	if (TK371X_PON_PORT_VALID(port)){
		...
		switch (type){
			case bcmPortControlPonFecMode:{
				...
				fec.rxFEC = fec.txFEC = value;
				ret_val = _soc_ea_fec_mode_set(unit, port, &fec);
				if (OK != ret_val){
					soc_cm_debug(DK_PORT,"calling _soc_ea_fec_mode_set failed\n");
					soc_cm_debug(DK_PORT,"return value: BCM_E_RESOURCE\n");
					return BCM_E_FAIL;
				}
				soc_cm_debug(DK_PORT,"type=bcmPortControlPonFecMode\n");
			...
			case bcmPortControlPonUserTraffic:
				...
				if (_user_traffic_status == 0){
					...
					soc_cm_debug(DK_PORT,"calling TkExtOamSetEnaUserTraffic, ret_val=%d\n", ret_val);
					if (ret_val != OK){
						return BCM_E_FAIL;
					}
				}
			...
			case bcmPortControlPonMultiLlid:{
				...
				if (value == -1){
					...
					soc_cm_debug(DK_PORT,"soc_auth_result_set fail return value SOC_E_NONE.\n");
					return BCM_E_NONE;
					...
					soc_cm_debug(DK_PORT,"soc_auth_result_set success return value SOC_E_NONE.\n");
					return BCM_E_NONE;
				...
				soc_cm_debug(DK_PORT,"calling CtcExtOamSetMulLlidCtrl, ret_val=%d\n", ret_val);
				return BCM_E_NONE;
			}
			case bcmPortControlPonEncryptKeyExpiryTime: {
				uint16 time;
				...
				soc_cm_debug(DK_PORT,"calling TkExtOamSetEncryptKeyExpiryTime time=%d, ret_val=%d\n",time, ret_val);
				return BCM_E_NONE;
			}
			case bcmPortControlPonHoldoverState:{
				uint32 state;
				...
				soc_cm_debug(DK_PORT,"calling CtcExtOamGetHoldoverret_val=%d\n", ret_val);
				soc_cm_debug(DK_PORT,"state=%d\n",state);
				soc_cm_debug(DK_PORT,"time=%d\n", time);
				state = value;
				ret_val = CtcExtOamSetHoldover(unit, port, state, time);
				if (ret_val != OK){
					return BCM_E_FAIL;
				}
				soc_cm_debug(DK_PORT,"calling CtcExtOamSetHoldover=%d\n", ret_val);
				soc_cm_debug(DK_PORT,"state=%d\n",state);
				soc_cm_debug(DK_PORT,"time=%d\n", time);
				return BCM_E_NONE;
			}
		...
		switch (type){
			case bcmPortControlEthPortAutoNegFailureAlarmState: {
				...
				soc_cm_debug(DK_PORT,"calling _soc_ea_alarm_enable_set=%d\n", ret_val);
				soc_cm_debug(DK_PORT,"state=%d\n",state);
				return BCM_E_NONE;
			}
			case bcmPortControlEthPortLosAlarmState:{
	    ...
	    int *enable)
{
	...
	if (port < _BCM_TK371X_PON_PORT_BASE ||
		...
		soc_cm_debug(DK_PORT,"_bcm_ea_port_enable_get return BCM_E_PORT, invalid port=%d\n", port);
		return BCM_E_PORT;
		...
		soc_cm_debug(DK_PORT,"TkExtOamGetEponAdmin:unit=%d, port=%d, state=%d, rv=%d\n", unit, port, state, rv);
	}else if (port < _BCM_TK371X_LLID_PORT_BASE){
		/* getting the UNI port status */
		rv = _soc_ea_phy_admin_state_get((uint8)unit, 0, port, &state);
		soc_cm_debug(DK_PORT,"_soc_ea_phy_admin_state_get:unit=%d, port=%d, state=%d,rv=%d\n", unit, port, state, rv);
	}else {
		soc_cm_debug(DK_PORT,"_bcm_ea_port_enable_get return BCM_E_UNAVAIL, invalid port=%d\n", port);
		return BCM_E_UNAVAIL;
	}
	    ...
	    int enable)
{
	...
	if (port < _BCM_TK371X_PON_PORT_BASE ||
			port > (_BCM_TK371X_LLID_PORT_BASE + _BCM_TK371X_MAX_LLID_PORT_NUM)){
		soc_cm_debug(DK_PORT,"_bcm_ea_port_enable_set return BCM_E_PORT, invalid port=%d\n", port);
		return BCM_E_PORT;
	}else if (port < _BCM_TK371X_UNI_PORT_BASE){
		state = (enable == 1) ? 0 : 1;
		rv = TkExtOamSetEponAdmin(unit, port, state);
		soc_cm_debug(DK_PORT,"TkExtOamSetEponAdmin:unit=%d, port=%d, enable=%d, rv=%d\n", unit, port, enable, rv);
	}else if (port < _BCM_TK371X_LLID_PORT_BASE){
		/* getting the UNI port status */
		rv = _soc_ea_phy_admin_state_set((uint8)unit, 0, port, enable);
		soc_cm_debug(DK_PORT,"_soc_ea_phy_admin_state_set:unit=%d, port=%d, enable=%d, rv=%d\n", unit, port, enable, rv);
	}else {
		return BCM_E_UNAVAIL;
	}
	if (rv != OK){
	    ...
	    int *size)
{
	...
	soc_cm_debug(DK_PORT,"_bcm_ea_port_frame_max_get..., port=%d\n", port);
	    ...
	    int size)
{
	...
	soc_cm_debug(DK_PORT,"_bcm_ea_port_frame_max_set...\n");
	...
	soc_cm_debug(DK_PORT,"_soc_ea_mtu_set: UNI Port=%d, rv=%d, max_frame=%d", port, rv, framesize);
	return BCM_E_NONE;
}
...
_bcm_ea_port_learn_get(
	    ...
	    uint32 *flags)
{
	soc_cm_debug(DK_PORT,"_bcm_ea_port_learn_get...\n");
	if (port < _BCM_TK371X_UNI_PORT_BASE || port >= _BCM_TK371X_LLID_PORT_BASE){
		soc_cm_debug(DK_PORT,"Return BCM_E_UNAVAIL, invalid port=%d\n", port);
		return BCM_E_UNAVAIL;
	}
	    ...
	    uint32 remove)
{
	...
	soc_cm_debug(DK_PORT,"_bcm_ea_port_learn_modify...\n");
	if (port < _BCM_TK371X_UNI_PORT_BASE || port >= _BCM_TK371X_LLID_PORT_BASE){
		soc_cm_debug(DK_PORT,"Return BCM_E_UNAVAIL, invalid port=%d\n", port);
		return BCM_E_UNAVAIL;
	}
    ...
    soc_cm_debug(DK_PORT,"_bcm_ea_port_learn_get:unit=%d port=%d rv=%d flags=%d\n",
             unit, port, rv, flags);
	if (BCM_E_NONE != rv){
		return rv;
	}
	...
	soc_cm_debug(DK_PORT,"_bcm_ea_port_learn_set:unit=%d port=%d rv=%d add=%d\n",
		 unit, port, rv, add);
	return rv;
}
	    ...
	    uint32 flags)
{
	...
	rv1 = _soc_ea_mac_learning_set((uint8)unit, 0,
			port, (uint8)mode);
	soc_cm_debug(DK_PORT,"_soc_ea_mac_learning_set:unit=%d port=%d rv=%d mode=%d\n",
             unit, port, rv1, mode);
	if (rv1 != OK){
		return BCM_E_FAIL;
	}
	...
	soc_cm_debug(DK_PORT,"_soc_ea_forward_mode_set:unit=%d port=%d rv=%d mode=%d\n",
...
_bcm_tk371x_port_laser_para_update(
	...
	para->rx_power = diag.txPower;
	para->tx_power = diag.rxPower;
	return BCM_E_NONE;
}
	    ...
	    int *speed)
{
	...
	soc_cm_debug(DK_PORT,"_bcm_ea_port_speed_get...\n");
	if (port < _BCM_TK371X_UNI_PORT_BASE || port >= _BCM_TK371X_LLID_PORT_BASE){
		soc_cm_debug(DK_PORT,"return BCM_E_PORT, invalid port is %d\n", port);
		return BCM_E_PORT;
	}

	rc = _soc_ea_auto_neg_get((uint8)unit, 0,
			(uint8)port, &auto_neg_state, &_speed, &duplex_state);
	soc_cm_debug(DK_PORT,"_soc_ea_auto_neg_get, rv=%d,"
				 "unit = %d, port = %d\n"
	    ...
	    int speed)
{
	...
	soc_cm_debug(DK_PORT,"_bcm_ea_port_speed_set...\n");
	if (port < _BCM_TK371X_UNI_PORT_BASE || port >= _BCM_TK371X_LLID_PORT_BASE){
		soc_cm_debug(DK_PORT,"return BCM_E_PORT, invalid port is %d\n", port);
		return BCM_E_PORT;
	}
	...
	soc_cm_debug(DK_PORT,"_soc_ea_auto_neg_get,"
				 "unit = %d, port = %d\n"
				 "autoneg_state=%d, speed=%d duplex=%d\n",
				 unit, port,
	...
	soc_cm_debug(DK_PORT,"_soc_ea_auto_neg_set, rv=%d,"
				 "unit = %d, port = %d\n"
...
_bcm_ea_port_link_status_get(int unit, bcm_port_t port, int *up){
	...
	soc_cm_debug(DK_PORT,"_bcm_ea_port_link_status_get..\n");
	if (port == _BCM_TK371X_PON_PORT_BASE){
		rv = TkExtOamGetMLLIDLinkStatus(unit, port, &pon_status);
		soc_cm_debug(DK_PORT,"TkExtOamGetMLLIDLinkStatus: unit=%d, port=%d, rv=%d\n", unit, port, rv);
		if (rv != OK){
			return BCM_E_FAIL;
		}
		*up = pon_status.connection;
		...
		soc_cm_debug(DK_PORT,"_soc_ea_eth_link_state_get: unit=%d, port=%d, rv=%d\n", unit, port, rv);
		if (rv != OK){
			return BCM_E_FAIL;
		}
		*up = status;
