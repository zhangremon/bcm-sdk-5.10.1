/*
 * $Id: init.c 1.1.2.7 Broadcom SDK $
 *
	    ...
	    bcm_info_t *info)
{
	...
	if (ERROR == _soc_ea_jedec_id_get((uint8)unit, 0, &vendor)){
		soc_cm_print("_soc_ea_jedec_id_get BCM_E_INTERNAL = %d\n", BCM_E_INTERNAL);
		rc = BCM_E_INTERNAL;
	}
	info->vendor = (int)vendor;

	if (ERROR == _soc_ea_chip_id_get((uint8)unit, 0, &device)){
		soc_cm_print("_soc_ea_chip_id_get BCM_E_INTERNAL = %d\n", BCM_E_INTERNAL);
		rc = BCM_E_INTERNAL;
	}
	info->device = (uint32)device;

	if (ERROR == _soc_ea_revision_id_get((uint8)unit, &info->revision)){
		soc_cm_print("_soc_ea_revision_id_get BCM_E_INTERNAL = %d\n", BCM_E_INTERNAL);
		rc = BCM_E_INTERNAL;
	}
	info->capability = 0;
