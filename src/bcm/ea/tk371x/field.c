/*
 * $Id: field.c 1.1.2.61 Broadcom SDK $
 *
...
_bcm_tk371x_field_bcmActionToRuleAction_update(
    ...
    if (bcmFieldActionRedirectPort == act[0] && _FP_ACTION_INVALID_VAL == act[1]){
        ...
        *action = *(fe->bcm_action);
    	 ...
    	 * */
    	fe->soc_action = _bcmTk371xFieldSetVidAndSetAddVlanTagAndForward;
    	fe->rule_para.vid_cos = (uint16)fe->bcm_action->param0;
       	*soc_act = fe->soc_action;
        *action = *(fe->bcm_action);
    }else if(bcmFieldActionDropCancel == act[0] &&
        ...
        *action = *(fe->bcm_action);
		   	   ...
		   	   bcmFieldActionDropCancel == act[1] &&
		   	   _FP_ACTION_INVALID_VAL == act[2]){
    	/* the soc action value is 0x16
    	 * 1. Replace outer VLAN tag priority
    	 * 2. Set forward action
    	 * */
        ...
        *action = *(fe->bcm_action);
    	 ...
    	 * 2. Set Add Vlan tag action to true
    	 * 3. Set VID
    	 * 4. Set the forward action
    	 * */
    	fe->soc_action = _bcmTk371xFieldReplaceTagAndSetVidAndForward;
    	fe->rule_para.vid_cos = (uint16)fe->bcm_action->param0;
	    ...
	    bcm_field_group_t group)
{
	...
	FP_VVERB(("FP: bcm_tk371x_field_group_create_id(unit=%d)\n", unit));
	fg = NULL;
    FIELD_IS_INIT(unit);
    BCM_IF_ERROR_RETURN(_bcm_tk371x_field_control_get(unit, fc));
    if (pri < 0 || pri > 15){
...
/*
 * Function:
 *      bcm_tk371x_field_qualify_UserDefined
 * Purpose:
...
{
	FP_VVERB(("bcm_tk371x_field_qualify_UserDefined(%d,%d)\n", unit, entry));
    FIELD_IS_INIT(unit);
	return BCM_E_UNAVAIL;
}
...
/*
 * Function:
 *      bcm_tk371x_field_qualify_UserDefined_get
 * Purpose:
 *      Get match criteria for bcmFieildQualifyUserDefined
 *                     qualifier from the field entry.
 * Parameters:
...
bcm_tk371x_field_qualify_UserDefined_get(
	    ...
	    uint8 mask[BCM_FIELD_USER_FIELD_SIZE])
{
	FP_VVERB(("bcm_tk371x_field_qualify_UserDefined_get(%d,%d)\n", unit, entry));
    FIELD_IS_INIT(unit);
	return BCM_E_UNAVAIL;
}
...
/*
 * Function: bcm_tk371x_field_udf_create
 *
 * Purpose:
 ...
 */
int
bcm_tk371x_field_udf_create(
	    int unit,
	    bcm_field_udf_spec_t *udf_spec,
	    bcm_field_udf_t *udf_id)
{
	FP_VVERB(("FP: bcm_tk371x_field_udf_create(unit=%d)\n", unit));
	return BCM_E_UNAVAIL;
}

/*
 * Function: bcm_tk371x_field_udf_create_id
 *
 * Purpose:
 ...
 */
int
bcm_tk371x_field_udf_create_id(
	    int unit,
	    bcm_field_udf_spec_t *udf_spec,
	    bcm_field_udf_t udf_id)
{
	FP_VVERB(("FP: bcm_tk371x_field_udf_create_id(unit=%d)\n", unit));
	return BCM_E_UNAVAIL;
}

/*
 * Function: bcm_tk371x_field_udf_destroy
 *
 * Purpose:
 ...
 */
int
bcm_tk371x_field_udf_destroy(
	    int unit,
	    bcm_field_udf_t udf_id)
{
	FP_VVERB(("FP: bcm_tk371x_field_udf_destroy(unit=%d)\n", unit));
	return BCM_E_UNAVAIL;
}

/*
 * Function: bcm_tk371x_field_udf_get
 *
 * Purpose:
 ...
 */
int
bcm_tk371x_field_udf_get(
	    int unit,
	    bcm_field_udf_spec_t *udf_spec,
	    bcm_field_udf_t udf_id)
{
	FP_VVERB(("FP: bcm_tk371x_field_udf_get(unit=%d)\n", unit));
	return BCM_E_UNAVAIL;
}

/*
 * Function: bcm_tk371x_field_udf_spec_get
 *
 * Purpose:
 ...
 */
int
bcm_tk371x_field_udf_spec_get(
	    int unit,
	    bcm_field_udf_spec_t *udf_spec,
	    uint32 *flags,
	    uint32 *offset)
{
	FP_VVERB(("FP: bcm_tk371x_field_udf_spec_get(unit=%d)\n", unit));
	return BCM_E_UNAVAIL;
}
...
/*
 * Function: bcm_tk371x_field_udf_spec_set
 *
 * Purpose:
 ...
 */
int
bcm_tk371x_field_udf_spec_set(
	    int unit,
	    bcm_field_udf_spec_t *udf_spec,
	    uint32 flags,
	    uint32 offset)
{
	FP_VVERB(("FP: bcm_tk371x_field_udf_spec_set(unit=%d)\n", unit));
	return BCM_E_UNAVAIL;
}

/*
 * Function:
 *      bcm_tk371x_field_udf_spec_t_init
 * Purpose:
 *      Initialize a UDF data type.
 */
void
bcm_tk371x_field_udf_spec_t_init(bcm_field_udf_spec_t *udf_spec)
{
	FP_VVERB(("FP: bcm_tk371x_field_udf_spec_t_init\n"));
	if (udf_spec != NULL){
...
/*
 * Function:
 *      bcm_tk371x_field_qualify_SrcMacGroup_get
 * Purpose:
...
bcm_tk371x_field_qualify_SrcMacGroup_get(
		...
		uint32 *offset)
{
	FP_VVERB(("FP: bcm_tk371x_field_qualify_SrcMacGroup_get(unit=%d)\n", unit));
	return BCM_E_UNAVAIL;
}

/*
 * Function: bcm_tk371x_field_qualify_SrcMacGroup
 *
 * Purpose:
 ...
 */
int
bcm_tk371x_field_qualify_SrcMacGroup(
		int unit,
		bcm_field_entry_t field,
		uint32 flags,
		uint32 offset)
{
	FP_VVERB(("FP: bcm_tk371x_field_qualify_SrcMacGroup(unit=%d)\n", unit));
	return BCM_E_UNAVAIL;
}
		...
		bcm_field_entry_t eid, uint8 *data,uint8 *mask)
{
	...
	while (fe_cond != NULL){
		if (fe_cond->qual_id == qual_id){
			sal_memcpy((uint8*)data,
					&(fe_cond->common.value[MAX_ENTRY_RULE_VAL_LEN - len]), len);
			if (fe_cond->match == _BCM_TK371X_FIELD_MATCH_EXISTS){
		...
		bcm_field_entry_t entry, uint16 data, uint16 mask)
{
	...
	int qset = _bcmTk371xFieldEntryCondQsetLlidIndex;
	int len = sizeof(uint16);
	uint8 match;
