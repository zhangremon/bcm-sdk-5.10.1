...
extern uint32 kt_dip128_start_offset;
extern uint32 kt_sip128_start_offset;
extern uint32 kt_depth128;
extern uint32 kt_dip64_start_offset;
extern uint32 kt_sip64_start_offset;
...
{   
    ...
    BCM_KT_DEFIP_PAIR128_TOTAL(unit) = soc_mem_index_count(unit, L3_DEFIP_PAIR_128m);
    BCM_KT_DEFIP_PAIR128_URPF_OFFSET(unit) = kt_sip128_start_offset;
    BCM_KT_DEFIP_PAIR128_IDX_MAX(unit) = kt_depth128 - 1;

    mem_sz = kt_depth128 * sizeof(_bcm_l3_defip_pair128_entry_t);
                            ...
                            int nh_ecmp_idx)
{
    ...
    /*for (idx = 0; idx <= (kt_depth128 - 1); idx++) {*/
    ...
    /*for (idx = (kt_depth128 - 1); idx >= 0; idx--) {*/
...
_bcm_kt_defip_pair128_delete(int unit, _bcm_defip_cfg_t *lpm_cfg) 
{
    ...
    if (rv == BCM_E_NOT_FOUND) {
        ...
        if (BCM_SUCCESS(rv1)) {
            ...
            if (BCM_TRX_DEFIP128_USED_COUNT(unit) > 0) {
