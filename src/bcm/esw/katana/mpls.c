/*
 * $Id: mpls.c 1.10.2.14 Broadcom SDK $
...
_bcm_kt_mpls_p2mp_loopback_enable(int unit)
{
   ...
   soc_LPORT_TABm_field32_set(unit, &lport_profile, PORT_TYPEf, 0x2);
   soc_LPORT_TABm_field32_set(unit, &lport_profile, MPLS_ENABLEf, 0x1);   
...
_bcm_kt_mpls_p2mp_loopback_disable(int unit)
{
   ...
   soc_LPORT_TABm_field32_set(unit, &lport_profile, MPLS_ENABLEf, 0x0);   
   soc_LPORT_TABm_field32_set(unit, &lport_profile, V4L3_ENABLEf, 0x0);
   soc_LPORT_TABm_field32_set(unit, &lport_profile, V6L3_ENABLEf, 0x0);
   rv = soc_mem_write(unit, LPORT_TABm,
                        MEM_BLOCK_ALL, 0, &lport_profile);
   return rv;
}
...
_bcm_kt_mpls_entry_delete(int unit, mpls_entry_entry_t *ment, 
                                                       bcm_mpls_switch_action_t info_action, int index)
{   
    ...
    if (forwarding_field_type == _BCM_MPLS_FORWARD_MULTICAST_GROUP) {
        ...
        if ((two_pass) && (info_action != -1)) {
            if ((info_action == BCM_MPLS_SWITCH_ACTION_SWAP) || 
                 (info_action == BCM_MPLS_SWITCH_ACTION_PHP)) {
...
_bcm_kt_mpls_entry_get_data(int unit, mpls_entry_entry_t *ment,
                            bcm_mpls_tunnel_switch_t *info)
{
    int rv=BCM_E_NONE, action, nh_index, ecmp_group, vrf, ipmc_index;
    ...
    case 0x3:
        ...
        if (forwarding_type == _BCM_MPLS_FORWARD_NEXT_HOP) {
              ...
              /* TODO: After MC_EGRESS_TYPE gets defined
                  info->mc_group = Convert index to Group;
              */
        }
...
/*
 * Function:
 *      _bcm_esw_mpls_port_stat_get_table_info
 * Description:
 ...
 */
STATIC
bcm_error_t _bcm_esw_mpls_port_stat_get_table_info(
            int                        unit,
            bcm_vpn_t                  vpn,
            bcm_gport_t                port,
...
/*
 * Function:
 *      _bcm_esw_mpls_label_stat_get_table_info
 * Description:
 *      Provides relevant flex table information(table-name,index with
...
STATIC
bcm_error_t _bcm_esw_mpls_label_stat_get_table_info(
            int                        unit,
...
bcm_kt_mpls_tunnel_switch_delete(int unit, bcm_mpls_tunnel_switch_t *info)
{
    ...
    rv = _bcm_kt_mpls_entry_delete(unit, &ment, info->action, index);
...
bcm_kt_mpls_tunnel_switch_delete_all(int unit)
{
    ...
    for (i = 0; i < num_entries; i++) {
        ...
        rv = _bcm_kt_mpls_entry_delete(unit, &ment, -1, i );
        if (rv < 0) {
            return rv;
        }
    }
            ...
            uint32      stat_counter_id)
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_esw_mpls_port_stat_get_table_info(
            ...
            uint32           stat_counter_id)
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_esw_mpls_label_stat_get_table_info(
            ...
            bcm_stat_value_t     *counter_values)
{
    ...
    if (((bcm_mpls_stat_t)stat == bcmMplsInPkts) ||
        ((bcm_mpls_stat_t)stat == bcmMplsOutPkts)) {
         byte_flag=0;
    } else {
         byte_flag=1;
    }
