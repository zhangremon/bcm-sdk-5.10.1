/*
 * $Id: field.c 1.172.2.5 Broadcom SDK $
    ...
    bcm_policer_t pid)
{
    ...
    fp_meter_table_entry_t fp_meter_table_entry;
    ...
    if (_FP_METER_BMP_TEST(fs_p->meter_bmp, meter_index) != 0)
    {
    ...
    else
    {
        ...
        case BCM_FIELD_METER_MODE_FLOW:
            if (POLICY_FIELD_GET(METER_UPDATE_EVENf) != 0)
            {
                policer_config_p->mode = bcmPolicerModePeak;
                meter_offset = 0;
            }
            else if (POLICY_FIELD_GET(METER_UPDATE_ODDf) != 0)
            {
            ...
            rv = soc_mem_read(unit, FP_METER_TABLEm, MEM_BLOCK_ANY,
                meter_table_index, &fp_meter_table_entry);
            ...
            refresh_count = soc_mem_field32_get(unit, FP_METER_TABLEm,
                &fp_meter_table_entry, REFRESHCOUNTf);

            bucket_count = soc_mem_field32_get(unit, FP_METER_TABLEm,
                &fp_meter_table_entry, BUCKETCOUNTf);

            bucket_size = soc_mem_field32_get(unit, FP_METER_TABLEm,
                &fp_meter_table_entry, BUCKETSIZEf);

            if (policer_config_p->mode == bcmPolicerModePeak)
            {
        ...
        _FP_METER_BMP_ADD(fs_p->meter_bmp, meter_index);
    }
