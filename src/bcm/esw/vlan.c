/*
 * $Id: vlan.c 1.363.2.25 Broadcom SDK $
    ...
    if (SOC_WARM_BOOT(unit)) {
        ...
        if (soc_feature(unit, soc_feature_vlan_vp)) {
...
#endif /* BCM_ENDURO_SUPPORT */
            {
                ...
                BCM_IF_ERROR_RETURN(bcm_trident_vlan_virtual_init(unit));
#endif /* BCM_TRIDENT_SUPPORT */
            } 
        }
#endif /* (BCM_TRIDENT_SUPPORT || BCM_ENDURO) && INCLUDE_L3 */
...
bcm_esw_vlan_detach(int unit)
{
...
#ifdef BCM_ENDURO_SUPPORT
            if (SOC_IS_ENDURO(unit)) {
                BCM_IF_ERROR_RETURN(bcm_enduro_vlan_virtual_detach(unit));
            } else
#endif /* BCM_ENDURO_SUPPORT */
            {
#ifdef BCM_TRIDENT_SUPPORT
                BCM_IF_ERROR_RETURN(bcm_trident_vlan_virtual_detach(unit));
#endif /* BCM_TRIDENT_SUPPORT */
            } 
        }
#endif /* (BCM_TRIDENT_SUPPORT || BCM_ENDURO) && INCLUDE_L3 */
    ...
    if (soc_feature(unit, soc_feature_vp_group_ingress_vlan_membership) ||
        soc_feature(unit, soc_feature_vp_group_egress_vlan_membership)) {
        BCM_IF_ERROR_RETURN(bcm_td_vp_group_detach(unit));
    }
#endif /* BCM_TRIDENT_SUPPORT && INCLUDE_L3 */
...
bcm_esw_vlan_create(int unit, bcm_vlan_t vid)
{
    ...
    if (rv == BCM_E_NONE) {
        ...
        rv = _bcm_vlan_port_add(unit, vid, stacked, empty_pbmp, stacked);
    }
#endif  /* BCM_VLAN_NO_AUTO_STACK */
    BCM_UNLOCK(unit);
...
bcm_esw_vlan_port_add(int unit, bcm_vlan_t vid, pbmp_t pbmp, pbmp_t ubmp)
{
    ...
    BCM_LOCK(unit);
    rv = _bcm_vlan_port_add(unit, vid, pbmp, ubmp, pbmp);
    BCM_UNLOCK(unit);
    ...
    if (SOC_MEM_FIELD_VALID(unit, VLAN_TABm, VIRTUAL_PORT_ENf)) {
        ...
        if (soc_mem_field32_get(unit, VLAN_TABm, &vtab, VIRTUAL_PORT_ENf)) {
            ...
            if (_BCM_MULTICAST_IS_VLAN(mc_group)) {
                ...
                {
#if defined(BCM_TRIDENT_SUPPORT)
                    rv = bcm_trident_update_vlan_pbmp(unit, vid,
                            &l2_l3_pbmp); 
#endif /* BCM_TRIDENT_SUPPORT */
                }
                if (rv < 0) {
            ...
            if (_BCM_MULTICAST_IS_NIV(mc_group)) {
                rv = bcm_trident_update_vlan_pbmp(unit, vid, &l2_l3_pbmp); 
                if (rv < 0) {
                    BCM_UNLOCK(unit);
                    return rv;
                }
...
{   
    ...
    if (SOC_IS_TRX(unit) && soc_feature(unit, soc_feature_vlan_action)) {
        ...
        rv = _bcm_trx_vlan_port_default_action_set(unit, local_port, action);
                                            ...
                                            bcm_vlan_action_set_t *action)
{
    ...
    if (SOC_IS_TRX(unit) && soc_feature(unit, soc_feature_vlan_action)) {
        ...
        rv = _bcm_trx_vlan_port_egress_default_action_set(unit, local_port, action);
    }
#endif /* BCM_TRX_SUPPORT */
                                               ...
                                               int *stop)
{
    ...
    *stop = FALSE;
    return usr_cb_st->usr_cb(unit, trvs_str->port_class, old_vlan, new_vlan, 
                             trvs_str->action->priority, trvs_str->user_data); 
}
                              ...
                              bcm_vlan_control_port_t type, int arg)
{
    if (port == -1) {
        ...
        if (BCM_GPORT_IS_SET(port)) {
            if (BCM_FAILURE(_bcm_vlan_control_trust_gport(type))) {
                bcm_trunk_t     tgid;
                bcm_module_t    modid;
                int             id;
...
#if defined(INCLUDE_L3)
    /* Is it a VPN? */
    if (_BCM_VPN_IS_VPWS(vlan) || _BCM_VPN_IS_VPLS(vlan)) {
 ...
 *
 */
bcm_error_t bcm_esw_vlan_stat_attach(
            int	        unit,
            bcm_vlan_t	vlan,
            uint32      stat_counter_id)
{
    ...
    if (!((object >= bcmStatObjectIngPort) &&
          (object <= bcmStatObjectEgrL3Intf))) {
           VLAN_ERR(("Invalid bcm_stat_object_t passed %d \n",object));
           return BCM_E_PARAM;
    }
    /* Validate group_mode */
    if(!((group_mode >= bcmStatGroupModeSingle) &&
         (group_mode <= bcmStatGroupModeDvpType))) {
          VLAN_ERR(("Invalid bcm_stat_group_mode_t passed %d \n",group_mode));
          return BCM_E_PARAM;
    }
 ...
 *
 */
bcm_error_t bcm_esw_vlan_stat_detach(
            int	        unit,
            bcm_vlan_t	vlan)
{
#if defined(BCM_KATANA_SUPPORT)
 ...
 *
 */
bcm_error_t bcm_esw_vlan_stat_counter_get( 
            int	             unit, 
            bcm_vlan_t       vlan, 
 ...
 *
 */
bcm_error_t bcm_esw_vlan_stat_counter_set( 
            int	             unit, 
            bcm_vlan_t       vlan,
            bcm_vlan_stat_t  stat,
            uint32           num_entries,
 ...
 *
 */
bcm_error_t bcm_esw_vlan_stat_id_get(
            int              unit,
            bcm_vlan_t       vlan,
            bcm_vlan_stat_t  stat,
            uint32           *stat_counter_id)
                   ...
                   bcm_stat_flex_table_info_t *table_info)
{
    ...
    vlan_xlate_entry_t return_vent={{0}};
    int                index=0;
 ...
 *
 */
bcm_error_t bcm_esw_vlan_translate_stat_attach(
            int                      unit,
            bcm_gport_t              port,
            bcm_vlan_translate_key_t key_type,
            bcm_vlan_t               outer_vlan,
 ...
 *
 */
bcm_error_t bcm_esw_vlan_translate_stat_detach(
            int                      unit,
            bcm_gport_t              port,
            bcm_vlan_translate_key_t key_type,
            bcm_vlan_t               outer_vlan,
 ...
 *
 */
bcm_error_t bcm_esw_vlan_translate_stat_counter_get(
            int                      unit,
            bcm_gport_t              port,
            bcm_vlan_translate_key_t key_type,
            bcm_vlan_t               outer_vlan,
 ...
 *
 */
bcm_error_t bcm_esw_vlan_translate_stat_counter_set(
            int                      unit,
            bcm_gport_t              port,
            bcm_vlan_translate_key_t key_type,
            bcm_vlan_t               outer_vlan,
 ...
 *
 */
bcm_error_t bcm_esw_vlan_translate_stat_id_get(
            int                      unit,
            bcm_gport_t              port,
            bcm_vlan_translate_key_t key_type,
            bcm_vlan_t               outer_vlan,
  ...
  if (!soc_feature(unit,soc_feature_advanced_flex_counter)) {
    BCM_IF_ERROR_RETURN
        (_bcm_esw_vlan_translate_stat_param_valid(unit, port, key_type,
                                                  outer_vlan, inner_vlan,
                                                  &vxlt_fsh));
    return _bcm_esw_flex_stat_ext_get(unit, _bcmFlexStatTypeVxlt,
                                      vxlt_fsh,
 ...
 if (!soc_feature(unit,soc_feature_advanced_flex_counter)) {
    BCM_IF_ERROR_RETURN
        (_bcm_esw_vlan_translate_stat_param_valid(unit, port, key_type,
                                                  outer_vlan, inner_vlan,
                                                  &vxlt_fsh));
    return _bcm_esw_flex_stat_ext_get32(unit, _bcmFlexStatTypeVxlt,
                                        vxlt_fsh,
                           _bcm_esw_vlan_translate_stat_to_flex_stat(stat), val);
  }
#if defined(BCM_KATANA_SUPPORT)
 ...
 *
 */
bcm_error_t bcm_esw_vlan_translate_egress_stat_attach(
            int        unit,
            int        port_class,
            bcm_vlan_t outer_vlan,
            bcm_vlan_t inner_vlan,
 ...
 *
 */
bcm_error_t bcm_esw_vlan_translate_egress_stat_detach(
            int unit,
            int port_class,
            bcm_vlan_t outer_vlan,
            bcm_vlan_t inner_vlan)
 ...
 *
 */
bcm_error_t bcm_esw_vlan_translate_egress_stat_counter_get(
            int              unit,
            int              port_class,
            bcm_vlan_t       outer_vlan,
            bcm_vlan_t       inner_vlan,
 ...
 *
 */
bcm_error_t bcm_esw_vlan_translate_egress_stat_counter_set(
            int              unit,
            int              port_class,
            bcm_vlan_t       outer_vlan,
            bcm_vlan_t       inner_vlan,
 ...
 *
 */
bcm_error_t bcm_esw_vlan_translate_egress_stat_id_get(
            int             unit,
            int 	    port_class,
            bcm_vlan_t 	    outer_vlan,
            bcm_vlan_t      inner_vlan,
  ...
  if (!soc_feature(unit,soc_feature_advanced_flex_counter)) {
    BCM_IF_ERROR_RETURN
        (_bcm_esw_vlan_translate_egress_stat_param_valid(unit, port_class,
                                                  outer_vlan, inner_vlan,
                                                  &vxlt_fsh));
    return _bcm_esw_flex_stat_ext_get(unit, _bcmFlexStatTypeEgrVxlt,
                                      vxlt_fsh,
 ...
 if (!soc_feature(unit,soc_feature_advanced_flex_counter)) {
    BCM_IF_ERROR_RETURN
        (_bcm_esw_vlan_translate_egress_stat_param_valid(unit, port_class,
                                                  outer_vlan, inner_vlan,
                                                  &vxlt_fsh));
    return _bcm_esw_flex_stat_ext_get32(unit, _bcmFlexStatTypeEgrVxlt,
                                        vxlt_fsh,
                _bcm_esw_vlan_translate_egress_stat_to_flex_stat(stat),
                                        val);
  }
  ...
  if (!soc_feature(unit,soc_feature_advanced_flex_counter)) {
    BCM_IF_ERROR_RETURN
        (_bcm_esw_vlan_translate_egress_stat_param_valid(unit, port_class,
                                                  outer_vlan, inner_vlan,
                                                  &vxlt_fsh));
    return _bcm_esw_flex_stat_ext_set(unit, _bcmFlexStatTypeEgrVxlt,
                                      vxlt_fsh,
                _bcm_esw_vlan_translate_egress_stat_to_flex_stat(stat),
                                      val);
  }
  ...
  if (!soc_feature(unit,soc_feature_advanced_flex_counter)) {
    BCM_IF_ERROR_RETURN
        (_bcm_esw_vlan_translate_egress_stat_param_valid(unit, port_class,
                                                  outer_vlan, inner_vlan,
                                                  &vxlt_fsh));
    return _bcm_esw_flex_stat_ext_set32(unit, _bcmFlexStatTypeEgrVxlt,
                                        vxlt_fsh,
                _bcm_esw_vlan_translate_egress_stat_to_flex_stat(stat),
                                        val);
  }
                                             ...
                                             uint64 *value_arr)
{
  ...
  if (!soc_feature(unit,soc_feature_advanced_flex_counter)) {
    ...
    return _bcm_esw_flex_stat_ext_multi_get(unit, _bcmFlexStatTypeEgrVxlt,
                                            vxlt_fsh,
                                        nstat, fs_arr, value_arr);
  }
#if defined(BCM_KATANA_SUPPORT)
                                               ...
                                               uint32 *value_arr)
{
  ...
  if (!soc_feature(unit,soc_feature_advanced_flex_counter)) {
    ...
    return _bcm_esw_flex_stat_ext_multi_get32(unit, _bcmFlexStatTypeEgrVxlt,
                                              vxlt_fsh,
                                        nstat, fs_arr, value_arr);
  }
#if defined(BCM_KATANA_SUPPORT)
                                             ...
                                             uint64 *value_arr)
{
  ...
  if (!soc_feature(unit,soc_feature_advanced_flex_counter)) {
    ...
    return _bcm_esw_flex_stat_ext_multi_set(unit, _bcmFlexStatTypeEgrVxlt,
                                            vxlt_fsh,
                                        nstat, fs_arr, value_arr);
  }
#if defined(BCM_KATANA_SUPPORT)
                                               ...
                                               uint32 *value_arr)
{
  ...
  if (!soc_feature(unit,soc_feature_advanced_flex_counter)) {
    ...
    return _bcm_esw_flex_stat_ext_multi_set32(unit, _bcmFlexStatTypeEgrVxlt,
                                              vxlt_fsh,
                                        nstat, fs_arr, value_arr);
  }
#if defined(BCM_KATANA_SUPPORT)
 ...
 *      bcm_vlan_gport_add
 * Purpose:
 *      Add a Gport to the specified vlan. Adds WLAN/VLAN/NIV ports to
                       ...
                       int flags)
{
...
#if (defined(BCM_TRIDENT_SUPPORT) || defined(BCM_ENDURO_SUPPORT)) && defined(INCLUDE_L3)
    if (soc_feature(unit, soc_feature_vlan_vp) && BCM_GPORT_IS_VLAN_PORT(port)) {
#if defined(BCM_ENDURO_SUPPORT)
        if (SOC_IS_ENDURO(unit)) {
        ...
        {
#if defined(BCM_TRIDENT_SUPPORT)
            rv = bcm_trident_vlan_gport_add(unit, vlan, port, flags);
...
#if defined(BCM_TRIDENT_SUPPORT) && defined(INCLUDE_L3)
    if (soc_feature(unit, soc_feature_niv) && BCM_GPORT_IS_NIV_PORT(port)) {
        rv = bcm_trident_vlan_gport_add(unit, vlan, port, flags);
    } else
#endif /* (BCM_TRIDENT_SUPPORT || BCM_ENDURO_SUPPORT) && INCLUDE_L3 */

    {
        ...
        if (SOC_IS_FBX(unit)) {
            rv = _bcm_vlan_port_add(unit, vlan, pbmp, ubmp, ing_pbmp);
        } else {
...
bcm_esw_vlan_gport_delete(int unit, bcm_vlan_t vlan, bcm_gport_t port)
{
    ...
    if (soc_feature(unit, soc_feature_vlan_vp) && BCM_GPORT_IS_VLAN_PORT(port)) {
        ...
        {
#if defined(BCM_TRIDENT_SUPPORT)
            rv = bcm_trident_vlan_gport_delete(unit, vlan, port);
#endif /* BCM_TRIDENT_SUPPORT */
        }
    } else
#endif /* (BCM_TRIDENT_SUPPORT || BCM_ENDURO_SUPPORT) && INCLUDE_L3 */
    ...
    if (SOC_MEM_FIELD_VALID(unit, VLAN_TABm, VIRTUAL_PORT_ENf)) {
        ...
        if (soc_mem_field32_get(unit, VLAN_TABm, &vtab, VIRTUAL_PORT_ENf)) {
            ...
            if (_BCM_MULTICAST_IS_VLAN(mc_group)) {
                ...
                {
#if defined(BCM_TRIDENT_SUPPORT)
                    rv = bcm_trident_vlan_gport_delete_all(unit, vlan);
#endif /* BCM_TRIDENT_SUPPORT */
                }
                if (BCM_FAILURE(rv)) {
                    soc_mem_unlock(unit, VLAN_TABm);
...
#endif /* (BCM_TRIDENT_SUPPORT || BCM_EDNURO_SUPPORT) && INCLUDE_L3 */
                       ...
                       int *flags)
{
    ...
    if (soc_feature(unit, soc_feature_vlan_vp) && BCM_GPORT_IS_VLAN_PORT(port)) {
        ...
        {
#if defined(BCM_TRIDENT_SUPPORT)
            rv = bcm_trident_vlan_gport_get(unit, vlan, port, flags);
#endif /* BCM_TRIDENT_SUPPORT */
        }
    } else
#endif /* (BCM_TRIDENT_SUPPORT || BCM_ENDURO_SUPPORT) && INCLUDE_L3 */
                         ...
                         bcm_vlan_port_t *vlan_port)
{
    ...
    if (soc_feature(unit, soc_feature_vlan_vp)) {
        ...
        {
#if defined(BCM_TRIDENT_SUPPORT)
            return bcm_trident_vlan_vp_create(unit, vlan_port);
#endif /* BCM_TRIDENT_SUPPORT */
        }
    }
#endif /* (BCM_TRIDENT_SUPPORT || BCM_ENDURO) && INCLUDE_L3 */
                         ...
                         bcm_gport_t gport)
{
    ...
    if (soc_feature(unit, soc_feature_vlan_vp)) {
        ...
        {
#if defined(BCM_TRIDENT_SUPPORT)
            return bcm_trident_vlan_vp_destroy(unit, gport);
#endif /* BCM_TRIDENT_SUPPORT */
        }
    }
#endif /* (BCM_TRIDENT_SUPPORT || BCM_ENDURO_SUPPORT) && INCLUDE_L3 */
                       ...
                       bcm_vlan_port_t *vlan_port)
{
    ...
    if (soc_feature(unit, soc_feature_vlan_vp)) {
        ...
        {
#if defined(BCM_TRIDENT_SUPPORT)
            return bcm_trident_vlan_vp_find(unit, vlan_port);
#endif /* BCM_TRIDENT_SUPPORT */
        }
    }
#endif /* (BCM_TRIDENT_SUPPORT || BCM_ENDURO_SUPPORT) && INCLUDE_L3 */
...
_bcm_vlan_sw_dump(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_vlan_vp)) {
        ...
        {
#ifdef BCM_TRIDENT_SUPPORT
            bcm_trident_vlan_vp_sw_dump(unit);
#endif /* BCM_TRIDENT_SUPPORT */
        } 
    }
#endif /* (BCM_TRIDENT_SUPPORT || BCM_ENDURO) && INCLUDE_L3 */
