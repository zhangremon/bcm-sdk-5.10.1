/* $Id: external_lpm.c 1.33 Broadcom SDK $
...
_bcm_tr_ext_lpm_state_recover(int unit, int v6)
{
    ...
    for (idx = 0; idx < soc_mem_index_count(unit, mem); idx++) {
        ...
        if (v6) { 
            _TR_LPM_FIB6(unit)[idx].vrf = vrf;
        } else {
            _TR_LPM_FIB4(unit)[idx].vrf = vrf;
        }
