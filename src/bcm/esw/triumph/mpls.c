/*
 * $Id: mpls.c 1.362.2.42 Broadcom SDK $
...
{
    int num_vfi, num_vrf, num_vp, num_vc, num_ip_tnl, num_mpls_map;
...
_bcm_tr_mpls_limited_reinit(int unit)
{
    ...
    /* Warm Boot */
        ...
        if (stable_size > sizeof(int32)) {
            ...
            SHR_BITCOPY_RANGE(mpls_info->ing_exp_map_bitmap, 0, mpls_state, 0,
                                                               num_ing_exp_map);
        }
    } else {
    /* Cold Boot */
    ...
    for (i = index_min; i <= index_max; i++) {
        ...
        if (_bcm_vp_used_get(unit, vp, _bcmVpTypeMpls)) {
            if (soc_MPLS_ENTRYm_field32_get(unit, &mpls_entry, Tf) == 0x1) {
                mpls_info->match_key[vp].trunk_id =
                        soc_MPLS_ENTRYm_field32_get(unit, &mpls_entry, TGIDf);
            } else {
                mpls_info->match_key[vp].port = 
                        soc_MPLS_ENTRYm_field32_get(unit, &vent, PORT_NUMf);
                mpls_info->match_key[vp].modid = 
                           soc_MPLS_ENTRYm_field32_get(unit, &vent, MODULE_IDf);
                if ((mpls_info->match_key[vp].port == 0) &&
...
_bcm_tr_mpls_extended_reinit(int unit)
{
    ...
    int       num_vfi, num_vrf, num_vp, num_vc;
    int       num_ing_exp_map, num_egr_l2_exp_map;
    int       vpws_vp_map_size;
    uint8     *mpls_state, vp_nh_idx;
    ...
    /* Warm Boot */
        ...
        if (stable_size > alloc_sz) {
            ...
            num_nh = soc_mem_index_count(unit, EGR_L3_NEXT_HOPm);
            SHR_BITCOPY_RANGE(mpls_info->vrf_bitmap, 0, mpls_state, 0, num_vrf);
            mpls_state += SHR_BITALLOCSIZE(num_vrf);
            SHR_BITCOPY_RANGE(mpls_info->vpws_bitmap, 0, mpls_state, 0,
                                                                    (num_vp/2));
            mpls_state += SHR_BITALLOCSIZE(num_vp/2);
            SHR_BITCOPY_RANGE(mpls_info->vc_c_bitmap, 0, mpls_state, 0, num_vc);
            mpls_state += SHR_BITALLOCSIZE(num_vc);
            SHR_BITCOPY_RANGE(mpls_info->vc_nc_bitmap, 0, mpls_state, 0,
                                                                        num_vc);
            mpls_state += SHR_BITALLOCSIZE(num_vc);
            if (num_pw_term) {
                SHR_BITCOPY_RANGE(mpls_info->pw_term_bitmap, 0, mpls_state, 0,
                                                                   num_pw_term);
                mpls_state += SHR_BITALLOCSIZE(num_pw_term);
            }
         
            if (num_pw_init) {
                SHR_BITCOPY_RANGE(mpls_info->pw_init_bitmap, 0, mpls_state, 0,
                                                                  num_pw_init); 
                mpls_state += SHR_BITALLOCSIZE(num_pw_init);
            }
            SHR_BITCOPY_RANGE(mpls_info->egr_mpls_bitmap, 0, mpls_state, 0,
                                                                   num_mpls_map);
            mpls_state += SHR_BITALLOCSIZE(num_mpls_map);
            SHR_BITCOPY_RANGE(mpls_info->ing_exp_map_bitmap, 0, mpls_state, 0,
                                                               num_ing_exp_map);
            mpls_state += SHR_BITALLOCSIZE(num_ing_exp_map);
            SHR_BITCOPY_RANGE(mpls_info->egr_l2_exp_map_bitmap, 0, mpls_state,
                                                         0, num_egr_l2_exp_map);
            mpls_state += SHR_BITALLOCSIZE(num_egr_l2_exp_map);
            SHR_BITCOPY_RANGE(mpls_info->tnl_bitmap, 0, mpls_state, 0,
                                                              (num_ip_tnl * 4));
            mpls_state += SHR_BITALLOCSIZE(num_ip_tnl * 4);
            SHR_BITCOPY_RANGE(mpls_info->ip_tnl_bitmap, 0, mpls_state, 0,
                                                                    num_ip_tnl);
            mpls_state += SHR_BITALLOCSIZE(num_ip_tnl);
        
            /* Reconstruct match_index by reading tables */
            rv = _bcm_tr_mpls_match_key_recover(unit);
            if (BCM_FAILURE(rv)) {
                return rv;
            }
            ...
            sal_memcpy(&nh_if, mpls_state, sizeof(int));
            vp_nh_idx = 0;
            while (vp_nh_idx < num_nh && nh_if != 0xdeadbeef) {
                 if (nh_if) {
                     BCM_IF_ERROR_RETURN(_bcm_tr_mpls_add_egrNhop(unit, nh_if));
                     BCM_IF_ERROR_RETURN(_bcm_tr_mpls_add_vp_nh(unit, nh_if,
                                                                    vp_nh_idx));
                 }
                 vp_nh_idx++;
...
_bcm_tr_mpls_ingexpbitmap_sync(int unit)
{
    ...
    SHR_BITCOPY_RANGE(mpls_state, 0, mpls_info->ing_exp_map_bitmap, 0,
                                                          num_ing_exp_map);
...
bcm_tr_mpls_sync(int unit)
{
    ...
    rv = BCM_E_NONE;
    SHR_BITCOPY_RANGE(mpls_state, 0, mpls_info->vrf_bitmap, 0, num_vrf);
    mpls_state += SHR_BITALLOCSIZE(num_vrf);
    SHR_BITCOPY_RANGE(mpls_state, 0, mpls_info->vpws_bitmap, 0, (num_vp/2));
    mpls_state += SHR_BITALLOCSIZE(num_vp/2);
    SHR_BITCOPY_RANGE(mpls_state, 0, mpls_info->vc_c_bitmap, 0, num_vc);
    mpls_state += SHR_BITALLOCSIZE(num_vc);
    SHR_BITCOPY_RANGE(mpls_state, 0, mpls_info->vc_nc_bitmap, 0, num_vc);
    mpls_state += SHR_BITALLOCSIZE(num_vc);
    if (num_pw_term) {
        SHR_BITCOPY_RANGE(mpls_state, 0, mpls_info->pw_term_bitmap, 0, num_pw_term);
        mpls_state += SHR_BITALLOCSIZE(num_pw_term);
    }
    if (num_pw_init) {
        SHR_BITCOPY_RANGE(mpls_state, 0, mpls_info->pw_init_bitmap, 0, num_pw_init);
        mpls_state += SHR_BITALLOCSIZE(num_pw_init);
    }
    
    SHR_BITCOPY_RANGE(mpls_state, 0, mpls_info->egr_mpls_bitmap, 0, num_mpls_map);
    mpls_state += SHR_BITALLOCSIZE(num_mpls_map);
    SHR_BITCOPY_RANGE(mpls_state, 0, mpls_info->ing_exp_map_bitmap, 0, num_ing_exp_map);
    mpls_state += SHR_BITALLOCSIZE(num_ing_exp_map);
    SHR_BITCOPY_RANGE(mpls_state, 0, mpls_info->egr_l2_exp_map_bitmap, 0, num_egr_l2_exp_map);
    mpls_state += SHR_BITALLOCSIZE(num_egr_l2_exp_map);
    SHR_BITCOPY_RANGE(mpls_state, 0, mpls_info->tnl_bitmap, 0, (num_ip_tnl * 4));
    mpls_state += SHR_BITALLOCSIZE(num_ip_tnl * 4);
    SHR_BITCOPY_RANGE(mpls_state, 0, mpls_info->ip_tnl_bitmap, 0, num_ip_tnl);
    mpls_state += SHR_BITALLOCSIZE(num_ip_tnl);
        ...
        if (_bcm_tr_mpls_nhop_headPtr[unit][i] != NULL) {
            for (nhop_ptr = _bcm_tr_mpls_nhop_headPtr[unit][i];
                ...
                for (vp_ptr = nhop_ptr->vp_head_ptr;
                     vp_ptr != NULL;
                     vp_ptr = vp_ptr->link) {
                    vp_state_ptr = (uint8 *) (mpls_state + (vp_ptr->vp_nh_idx * sizeof(int)));
                    sal_memcpy(vp_state_ptr, &nhop_ptr->egr_if, sizeof(int));
                } /* for each vp on nh */
                /* sal_memcpy(vp_count_ptr, &vp_count, sizeof(int)); */
            } /* for each nh */
        }
    }
    vp_count = 0xdeadbeef;
    mpls_state += sizeof(int) * num_nh;
    sal_memcpy(mpls_state, &vp_count, sizeof(int));
...
{
    _bcm_tr_mpls_bookkeeping_t *mpls_info = MPLS_INFO(unit);
    int rv=0, i, num_vfi, num_vp, num_vpws, num_vc, num_vrf;
...
bcm_tr_mpls_vpn_id_create(int unit, bcm_mpls_vpn_config_t *info)
{
    ...
    if (info->flags & BCM_MPLS_VPN_VPWS) {
        ...
        if (info->flags & BCM_MPLS_VPN_WITH_ID) {
            if ( info->vpn < 0x2000 || info->vpn > 0x2FFF ) {
                return BCM_E_PARAM;
            }
            vpn_id = _BCM_MPLS_VPN_ID_GET(info->vpn);
        ...
        if (info->flags & BCM_MPLS_VPN_WITH_ID) {
            if ( info->vpn < 0x3000 || info->vpn > (0x3000+num_vfi-1) ) {
                return BCM_E_PARAM;
            }
         ...
         */
        if (info->flags & BCM_MPLS_VPN_WITH_ID) {
            if ( info->vpn < 0x1000 || info->vpn > (0x1000 + SOC_VRF_MAX(unit)) ) {
                return BCM_E_PARAM;
            }
                       ...
                       bcm_mpls_vpn_config_t *info)
{
    if (_BCM_MPLS_VPN_IS_VPWS(vpn)) {
        ...
        if ( vpn < 0x2000 || vpn > 0x2FFF ) {
            return BCM_E_PARAM;
        }

        vpn_id = _BCM_MPLS_VPN_ID_GET(vpn);
        ...
        if ( vpn < 0x3000 || vpn > (0x3000+num_vfi-1) ) {
            return BCM_E_PARAM;
        }
...
bcm_tr_mpls_vpn_id_destroy (int unit, bcm_vpn_t vpn)
{
    ...
    if (_BCM_MPLS_VPN_IS_VPWS(vpn)) {
        ...
        if ( vpn < 0x1000 || vpn > (0x1000 + SOC_VRF_MAX(unit)) ) {
             return BCM_E_PARAM;
        }
...
/*
 * Function:
 *      bcm_tr_mpls_egress_intf_find
 * Purpose:
 *      Get egress_object id for a given nhi
 * Parameters:
 *      unit  - (IN)  Device Number
 *      nh_index    - (IN)  Next Hop Index
 *      egress_if  - (OUT) Egress object
 * Returns:
 *      BCM_E_XXX
 */

int
bcm_tr_mpls_egress_intf_find(int unit, int nh_index, bcm_if_t *egress_if)
{
...
STATIC int 
_bcm_tr_mpls_create_vp_nh ( int unit, bcm_if_t  egress_tunnel_if, int  *vp_nh_index) 
{
  int rv;
  bcm_l3_egress_t nh_info;
  uint32 nh_flags;
               ...
               egr_mpls_vc_and_swap_label_table_entry_t *vc_entry)
{
    ...
    if (mpls_port != NULL) {
         entry_label = soc_EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm_field32_get(unit, 
                        vc_entry, MPLS_LABELf);
         if (entry_label != mpls_port->egress_label.label) {
             return BCM_E_NOT_FOUND;
         }
         entry_label_action = soc_EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm_field32_get(unit,
                        vc_entry, MPLS_LABEL_ACTIONf);
         if (label_action != entry_label_action) {
             return BCM_E_NOT_FOUND;
         }
         ttl = soc_EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm_field32_get(unit,
                        vc_entry, MPLS_TTLf);
         if (mpls_port->egress_label.flags & BCM_MPLS_EGRESS_LABEL_TTL_SET &&
             ttl != mpls_port->egress_label.ttl) {
             return BCM_E_NOT_FOUND;
         }
         cw = soc_EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm_field32_get(unit,
         ...
         exp = soc_EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm_field32_get(unit,
                        vc_entry, MPLS_EXPf);
         pri = soc_EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm_field32_get(unit,
                        vc_entry, NEW_PRIf);
         cfi = soc_EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm_field32_get(unit,
                        vc_entry, NEW_CFIf);
         ...
         sd_tag = soc_EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm_field32_get(unit,
                        vc_entry, SD_TAG_VIDf);
         sd_tag_action_present = soc_EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm_field32_get(unit,
                        vc_entry, SD_TAG_ACTION_IF_PRESENTf);
         sd_tag_action_not_present = soc_EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm_field32_get(unit,
         ...
         if ((egr_obj->mpls_flags & BCM_MPLS_EGRESS_LABEL_EXP_SET) ||
            (egr_obj->mpls_flags & BCM_MPLS_EGRESS_LABEL_PRI_SET)) {
              ...
              cfi = soc_EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm_field32_get(unit,
                        vc_entry, NEW_CFIf);
              exp_select = soc_EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm_field32_get(unit,
              ...
              exp_select = soc_EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm_field32_get(unit,
                        vc_entry, MPLS_EXP_SELECTf);
              pri_mapping_ptr = soc_EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm_field32_get(unit,
                        vc_entry, MPLS_EXP_MAPPING_PTRf);
              if (exp_select != 1) {
                  return BCM_E_NOT_FOUND;
              }
              if ( pri_mapping_ptr != MPLS_INFO(unit)->egr_mpls_hw_idx[egr_obj->mpls_qos_map_id]) {
                  return BCM_E_NOT_FOUND;              
              }
                       ...
                       int action, int *vc_swap_index)
{
    ...
    for (idx = 0; idx < num_vc; idx++) {
         if ((_BCM_MPLS_VC_NON_COUNT_USED_GET(unit, idx) &&
              ...
              BCM_IF_ERROR_RETURN(
                   soc_mem_read(unit, EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm,
                             MEM_BLOCK_ALL, (num_vc+idx), &vc_entry));
              if (BCM_SUCCESS(
         ...
         } else if (_BCM_MPLS_VC_COUNT_USED_GET(unit, idx)) {
              BCM_IF_ERROR_RETURN(
              soc_mem_read(unit, EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm,
                             MEM_BLOCK_ALL, idx, &vc_entry));
              if (BCM_SUCCESS(
                    _bcm_tr_mpls_vc_swap_compare(unit, mpls_port, egr_obj, action, &vc_entry))) {
                   *vc_swap_index = idx;
                   _bcm_tr_mpls_vc_and_swap_ref_count_adjust (unit, *vc_swap_index, 1);
                                       ...
                                       int hw_map_idx)
{
        ...
        if ((mpls_port->egress_label.flags & BCM_MPLS_EGRESS_LABEL_EXP_SET) ||
            ...
            soc_mem_field32_set(unit, EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm,
                                vc_entry, MPLS_EXP_MAPPING_PTRf,
                                hw_map_idx);
        } else  if (mpls_port->egress_label.flags & BCM_MPLS_EGRESS_LABEL_EXP_COPY) { 
            ...
            soc_mem_field32_set(unit, EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm,
                                vc_entry, MPLS_EXP_MAPPING_PTRf,
                                hw_map_idx);
        } else { /* Default: Use EXP from swapped label. Use EXP-map for PRI/CFI */
            soc_mem_field32_set(unit, EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm,
                                vc_entry, MPLS_EXP_SELECTf,
                                0x3); /* USE_SWAP */
            /* Use EXP-PRI-map for PRI/CFI */
...
STATIC int
_bcm_tr_mpls_port_untagged_profile_set (int unit, bcm_port_t port)
{
...
/*
 * Function:
 *      _bcm_tr_mpls_port_untagged_profile_reset
 * Purpose:
...
STATIC int
_bcm_tr_mpls_port_untagged_profile_reset(int unit, bcm_port_t port)
{
    int rv;
    port_tab_entry_t ptab;
    uint32 ing_profile_idx = 0xffffffff;
                         ...
                         int *tpid_index )
{
    ...
    if (mpls_port->flags & BCM_MPLS_PORT_EGRESS_TUNNEL) {
        ...
        if ((mpls_port->flags & BCM_MPLS_PORT_SERVICE_VLAN_ADD) ||
            ...
            /* TPID value is used */
            BCM_IF_ERROR_RETURN(
                _bcm_fb2_outer_tpid_entry_add(unit, mpls_port->service_tpid, tpid_index));
            soc_mem_field32_set(unit, EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm,
        ...
        if ((mpls_port->flags & BCM_MPLS_PORT_SERVICE_VLAN_ADD) ||
            ...
            (mpls_port->flags & BCM_MPLS_PORT_SERVICE_TPID_REPLACE)) {
            /* TPID value is used */
            BCM_IF_ERROR_RETURN(_bcm_fb2_outer_tpid_entry_add(unit, mpls_port->service_tpid, tpid_index));
                            ...
                            bcm_port_t *local_port, int *is_local)
{
    ...
    int num_mpls_map, map_id, hw_map_idx;
    void *entries[1];
    ...
    *is_local = 0;
    ...
    if (mpls_port->flags & BCM_MPLS_PORT_EGRESS_TUNNEL) {
        ...
        num_mpls_map = soc_mem_index_count(unit, EGR_MPLS_EXP_MAPPING_1m) / 64;
        map_id = mpls_port->egress_label.qos_map_id;
        if (map_id < 0 || map_id >= num_mpls_map) {
            rv = BCM_E_PARAM;
            goto cleanup;
        }
        hw_map_idx = MPLS_INFO(unit)->egr_mpls_hw_idx[map_id];
        ...
        if (!BCM_XGS3_L3_EGRESS_IDX_VALID(unit, mpls_port->egress_tunnel_if)) {
            ...
            if (soc_feature(unit, soc_feature_mpls_enhanced) && soc_feature(unit, soc_feature_gport_service_counters)) {
                 egr_nh_info.vintf_ctr_index = 
        ...
        rv = _bcm_tr_mpls_vc_and_swap_table_entry_set(unit, mpls_port,
                                                                    &vc_entry, hw_map_idx);
        _BCM_MPLS_CLEANUP(rv);
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_esw_modid_is_local(unit, ing_nh_info.module, is_local));
    if (*is_local) {
        bcm_port_t phys_port;
        uint32 port_flags;
        phys_port = ing_nh_info.port; 
      ...
      _BCM_MPLS_FAILOVER_VALID_RANGE ( mpls_port->failover_id ) {
         ...
         if (mpls_port->flags & BCM_MPLS_PORT_FAILOVER) {
            rv = bcm_tr2_failover_prot_nhi_create( 
                unit, failover_nh_index);
            _BCM_MPLS_CLEANUP(rv);

            rv = bcm_tr2_failover_prot_nhi_set(
                unit, failover_nh_index, 
                (uint32) *vp_nh_index, mpls_port->failover_id );
            _BCM_MPLS_CLEANUP(rv);

         } else {

            rv = bcm_tr2_failover_prot_nhi_create( 
                unit, (uint32) *vp_nh_index);
            _BCM_MPLS_CLEANUP(rv);
            ...
            _BCM_MPLS_CLEANUP(rv);
             ...
             rv = bcm_tr2_failover_prot_nhi_update(unit, old_vp_nh_index, (uint32) *vp_nh_index );
            _BCM_MPLS_CLEANUP(rv);
      }
    }
#endif /* BCM_TRIUMPH2_SUPPORT  */
...
{
    int rv=BCM_E_NONE, is_local, num_vc, old_macda_idx = -1, old_vc_swap_idx = -1;
...
_bcm_tr_mpls_l2_nh_info_get(int unit, bcm_mpls_port_t *mpls_port, int nh_index)
{
    ...
    if (entry_type == 0x2) {
        ...
        if (network_port_flag) {
            mpls_port->flags |= BCM_MPLS_PORT_EGRESS_TUNNEL;
            rv = bcm_tr_mpls_egress_intf_find(unit, nh_index, 
 ...
 _bcm_tr_mpls_match_trunk_add(int unit, bcm_trunk_t tgid, int vp)
{
    ...
    int port_idx;
    int modid_local = 0;
    int local_src_trk_idx[BCM_TRUNK_MAX_PORTCNT];
    int local_port_out[BCM_TRUNK_MAX_PORTCNT];
    int rv = BCM_E_NONE;
    int src_trk_idx = -1;
    bcm_module_t mod_out;
    bcm_port_t port_out;
    ...
    for (port_idx = 0; port_idx < tinfo.num_ports; port_idx++) {
        BCM_IF_ERROR_RETURN(
            _bcm_esw_stk_modmap_map(unit, BCM_STK_MODMAP_SET,
                        tinfo.tm[port_idx], tinfo.tp[port_idx], &mod_out, &port_out));
        BCM_IF_ERROR_RETURN(
        ...
        BCM_IF_ERROR_RETURN(
        _bcm_tr_mpls_match_trunk_idx_get(unit, mod_out, port_out, &src_trk_idx));
        local_src_trk_idx[port_idx] = src_trk_idx;
        local_port_out[port_idx] = port_out;
...
void
_bcm_tr_mpls_match_clear (int unit, int vp)
{
    _bcm_tr_mpls_bookkeeping_t *mpls_info = MPLS_INFO(unit);
...
{
    _bcm_tr_mpls_bookkeeping_t *mpls_info = MPLS_INFO(unit);
    int rv = BCM_E_NONE, num_bits_for_port, gport_id;
    bcm_module_t mod_out;
    bcm_port_t port_out;
    bcm_trunk_t trunk_id;
        ...
        if (BCM_GPORT_IS_TRUNK(mpls_port->port)) {
             ...
             BCM_IF_ERROR_RETURN(rv);       
              ...
              if (SOC_IS_TRIDENT(unit) || SOC_IS_KATANA(unit)) {
                ...
                src_trk_idx = (mod_out << num_bits_for_port) | port_out;
            }
            rv = soc_mem_field32_modify(unit, SOURCE_TRUNK_MAP_TABLEm, 
                                        src_trk_idx, SOURCE_VPf, vp);
            BCM_IF_ERROR_RETURN(rv);
            ...
            rv = _bcm_tr_mpls_port_untagged_profile_set(unit, port_out);
            BCM_IF_ERROR_RETURN(rv);
...
_bcm_tr_mpls_match_delete(int unit, int vp)
{
    ...
    if  (mpls_info->match_key[vp].flags == _BCM_MPLS_PORT_MATCH_TYPE_VLAN) {
        ...
        (void) _bcm_tr_mpls_match_clear (unit, vp);
         _bcm_tr_mpls_port_match_count_adjust(unit, vp, -1);
        ...
        _bcm_tr_mpls_port_match_count_adjust(unit, vp, -1);
         ...
         rv = _bcm_tr_mpls_port_untagged_profile_reset(unit, port);
         BCM_IF_ERROR_RETURN(rv);
                             ...
                             if (soc_feature(unit, soc_feature_mpls_failover)) {
                                  ...
                                  if (mpls_port->flags & BCM_MPLS_PORT_REPLACE) {
                                       ...
                                       nh_index = soc_ING_DVP_TABLEm_field32_get(unit, &dvp3,
                                                                          NEXT_HOP_INDEXf);
                                       if (soc_SOURCE_VPm_field32_get(unit, &svp3, SD_TAG_MODEf)) {
                                           /* SD-tag mode, save the old TPID enable bits */
    ...
    if (_BCM_MPLS_VPN_IS_VPWS(vpn)) {
        ...
        if (rv >= 0) {
            ...
            if (soc_feature(unit, soc_feature_mpls_failover)) {
                ...
                failover_nh_index  = soc_mem_field32_get(unit, INITIAL_PROT_NHI_TABLEm,
        ...
        int is_module_local=0, is_trunk=0;
        ...
        is_trunk = soc_mem_field32_get(unit, ING_L3_NEXT_HOPm, &ing_nh, Tf);
        if (!is_trunk) {
            BCM_IF_ERROR_RETURN( _bcm_esw_modid_is_local(unit, module_id, &is_module_local));
        }
        if (is_module_local) {
            local_port = soc_mem_field32_get(unit, ING_L3_NEXT_HOPm, &ing_nh, PORT_NUMf);
        } else if (is_trunk) {
        ...
        if (soc_feature(unit, soc_feature_mpls_failover)) {
            ...
            if (failover_nh_index > 0) {
              for (primary_nh_index=0; primary_nh_index < num_entry; primary_nh_index++) {
                ...
                if (failover_nh_index  == (int)soc_mem_field32_get(unit, INITIAL_PROT_NHI_TABLEm,
                                                                   &prot_nhi_entry, PROT_NEXT_HOP_INDEXf) ) {
...
void 
_bcm_tr_mpls_port_match_count_adjust(int unit, int vp, int step)
{
    _bcm_tr_mpls_bookkeeping_t *mpls_info = MPLS_INFO(unit);
                                 ...
                                 bcm_mpls_egress_label_t *label_array)
{
    ...
    for (i = 0; i < num_labels; i++) {
        if ((label_array[i].label > 0xfffff) ||
            (label_array[i].qos_map_id < 0) ||
            (label_array[i].qos_map_id >= num_mpls_map) ||
   ...
   for (i = 0; i < num_labels; i++) {
    ...
    offset = mpls_index & 0x3;
    for (i = 0; i < num_labels; i++) {
        map_id = label_array[i].qos_map_id;
        hw_map_idx = MPLS_INFO(unit)->egr_mpls_hw_idx[map_id];
...
bcm_tr_mpls_l3_label_add (int unit, bcm_l3_egress_t *egr, int nh_index, uint32 flags)
{
    ...
    egr_l3_next_hop_entry_t egr_nh;
    uint32    entry_type;
    int       map_id, hw_map_idx, num_mpls_map;
    ...
    if ((entry_type == 1) && (egr->mpls_label != 0) && (flags & BCM_L3_REPLACE)) {
        /* Be sure that the existing entry is already setup to
         * egress into an MPLS tunnel. If not, return BCM_E_PARAM.
         */
        entry_type = 
    ...
    }else if ((entry_type == 1) && (egr->mpls_label ==0) && (flags & BCM_L3_REPLACE)) {
    ...
    if (BCM_XGS3_L3_MPLS_LBL_VALID(egr->mpls_label)) {
       soc_mem_field32_set(unit, EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm, 
              &vc_entry, MPLS_LABELf, egr->mpls_label);
       soc_mem_field32_set(unit, EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm, 
              &vc_entry, MPLS_LABEL_ACTIONf, _BCM_MPLS_ACTION_PUSH); /* PUSH one label */
    } else {
       soc_mem_field32_set(unit, EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm, 
              &vc_entry, MPLS_LABEL_ACTIONf, _BCM_MPLS_ACTION_NOOP); /* NOOP */
    }
    ...
    if ((egr->mpls_flags & BCM_MPLS_EGRESS_LABEL_EXP_SET) ||
         ...
         soc_mem_field32_set(unit, EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm,
                                       &vc_entry, MPLS_EXP_MAPPING_PTRf, hw_map_idx);

    } else if (egr->mpls_flags & BCM_MPLS_EGRESS_LABEL_EXP_COPY) { 

         soc_mem_field32_set(unit, EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm,
                                       &vc_entry, MPLS_EXP_SELECTf, 0x2); /* USE_INNER */
         soc_mem_field32_set(unit, EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm,
                                       &vc_entry, MPLS_EXP_MAPPING_PTRf, hw_map_idx);
    } else {
...
bcm_tr_mpls_swap_nh_info_add (int unit, bcm_l3_egress_t *egr, int nh_index, uint32 flags)
{
    ...
    if ((egr->mpls_flags & BCM_MPLS_EGRESS_LABEL_EXP_SET) ||
        ...
        soc_mem_field32_set(unit, EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm, &vc_entry,
                            MPLS_EXP_MAPPING_PTRf, hw_map_idx); 
...
bcm_tr_mpls_l3_nh_info_add(int unit, bcm_mpls_tunnel_switch_t *info, int *nh_index)
{
    ...
    /* Retrieve hardware index for mapping pointer */
    num_mpls_map = soc_mem_index_count(unit, EGR_MPLS_EXP_MAPPING_1m) / 64;
    map_id = info->egress_label.qos_map_id;
    if (map_id < 0 || map_id >= num_mpls_map) {
        rv = BCM_E_PARAM;
        goto cleanup;
    }
...
bcm_tr_mpls_l3_nh_info_get(int unit, bcm_mpls_tunnel_switch_t *info, int nh_index)
{
    egr_l3_next_hop_entry_t egr_nh;
    egr_mpls_vc_and_swap_label_table_entry_t vc_entry;
    int rv, vc_swap_idx, ix;

    rv = bcm_tr_mpls_egress_intf_find(unit, nh_index, &info->egress_if);
    BCM_IF_ERROR_RETURN(rv);
...
{
    int table_num, num_exp_map, cng, rv, index, idx2;
    ...
    if ((exp_map_id & _BCM_TR_MPLS_EXP_MAP_TABLE_TYPE_MASK) == 
        ...
        /* Combined index used for EGR_MPLS_EXP_MAPPING_2m */
        idx2 = index + offset;
        ...
        /*
         * Note that EGR_MPLS_EXP_MAPPING_2 is currently used only by the
         * MPLS module, so we can manage it directly.
         * If this table is ever shared, it must be added to the combo_map
         * profile memory used above.
         */

        /* Update EGR_MPLS_EXP_MAPPING_2m using same index as combo_map */
        if (SOC_MEM_IS_VALID(unit, EGR_MPLS_EXP_MAPPING_2m)) {
            /* Clear old entry if we have been moved */
            if (idx2 != index + offset) {
                idx2 = index + offset;
                                   ...
                                   bcm_gport_t port, int enable)
{
    ...
    if (BCM_GPORT_IS_MPLS_PORT(port)) {
         ...
         */
        return bcm_esw_port_stat_enable_set(unit, port, enable);
    }
...
_bcm_mpls_sw_dump(int unit)
{
    ...
    soc_cm_print("\n  Match Info    : \n");
    for (i = 0; i < num_vp; i++) {
        if ((mpls_info->match_key[i].trunk_id == 0) && 
            (mpls_info->match_key[i].modid == 0) &&
