/*
 * $Id: mbcm.c 1.37.18.1 Broadcom SDK $
...
mbcm_init(int unit)
{
    ...
    if (SOC_IS_DRACO(unit)) {
        mbcm_driver[unit] = &mbcm_draco_driver;
        bcm_chip_family_set(unit, BCM_FAMILY_DRACO); 
	return BCM_E_NONE;
    }
    ...
    if (SOC_IS_DRACO15(unit)) {
        mbcm_driver[unit] = &mbcm_draco_driver;
        bcm_chip_family_set(unit, BCM_FAMILY_DRACO15); 
	return BCM_E_NONE;
    }
    ...
    if (SOC_IS_HERCULES1(unit)) {
        ...
        bcm_chip_family_set(unit, BCM_FAMILY_HERCULES); 
	return BCM_E_NONE;
    }
    ...
    if (SOC_IS_LYNX(unit)) {
        mbcm_driver[unit] = &mbcm_lynx_driver;
        bcm_chip_family_set(unit, BCM_FAMILY_LYNX); 
	return BCM_E_NONE;
    }
#endif	/* BCM_LYNX_SUPPORT */
#ifdef	BCM_TUCANA_SUPPORT
    if (SOC_IS_TUCANA(unit)) {
        mbcm_driver[unit] = &mbcm_tucana_driver;
        bcm_chip_family_set(unit, BCM_FAMILY_TUCANA); 
	return BCM_E_NONE;
    }
    ...
    if (SOC_IS_EASYRIDER(unit)) {
        mbcm_driver[unit] = &mbcm_easyrider_driver;
        bcm_chip_family_set(unit, BCM_FAMILY_EASYRIDER); 
        return BCM_E_NONE;
    }
