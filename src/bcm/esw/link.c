/*
 * $Id: link.c 1.151.2.7 Broadcom SDK $
...
#if defined(BCM_HURRICANE_SUPPORT) || defined(BCM_TRIDENT_SUPPORT) || defined(BCM_HAWKEYE_SUPPORT)
...
_bcm_esw_link_failover_link_up(int unit, int port)
{
    ...
    BCM_IF_ERROR_RETURN
        (soc_reg32_get(unit, lag_failover_config_reg, port, 0, 
                       &lag_fail_config));

    /* Unimac may take several tries before clearing the loopback state. */
    while (retries--) {
        soc_reg_field_set(unit, lag_failover_config_reg,
                          &lag_fail_config, REMOVE_FAILOVER_LPBKf, 0);
        BCM_IF_ERROR_RETURN
            (soc_reg32_set(unit, lag_failover_config_reg,
        ...
        while (!soc_timeout_check(&to)) {
            BCM_IF_ERROR_RETURN
                (soc_reg32_get(unit, lag_failover_status_reg,
                               port, 0, &lag_fail_status));
            failed =
                soc_reg_field_get(unit, lag_failover_status_reg, 
...
_bcm_esw_linkscan_update_port(int unit, int port)
{
	...
	rv = BCM_E_NONE;
        ...
        if (SOC_IS_HURRICANE (unit) || SOC_IS_TD_TT (unit) ||
            SOC_IS_HAWKEYE (unit) || SOC_IS_KATANA(unit)) {
