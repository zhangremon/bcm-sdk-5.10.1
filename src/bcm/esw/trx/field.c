/* $Id: field.c 1.229.2.42 Broadcom SDK $
                                ...
                                SHR_BITDCL *pfs_bmp)
{
    ...
    for (idx = 0; idx < BCM_PBMP_PORT_MAX; idx++) {
        ...
        if ((SHR_BITGET(pfs_bmp, idx)) &&
            (PBMP_MEMBER(port_config.hg, idx) || IS_HL_PORT(unit, idx))) {
#else
        if ((SHR_BITGET(pfs_bmp, idx)) &&
                                    ...
                                    int selcode_index)
{
    ...
    if ((fg->flags & _FP_GROUP_INTRASLICE_DOUBLEWIDE) &&
        ...
        for (idx = 0; idx < pfs_idx_count; idx++) {
            ...
            /* Set slice mode. */
            fld = _trx_slice_wide_mode_field[slice_num];
            ...
            fld = _trx_slice_pairing_field[slice_num / 2];
            soc_FP_PORT_FIELD_SELm_field32_set(unit, buf, fld, value);
                        ...
                        _field_entry_t *f_ent, uint32 *buf)
{
    ...
    if (_BCM_FIELD_STAGE_INGRESS == fg->stage_id) {
        ...
        if (SOC_IS_TRIUMPH2(unit) && FP_GLOBAL_MASK_TCAMm == mem) {
            tcam = &f_ent->extra_tcam;
            key_field = IPBMf;
...
/*
 * Function:
 *     _field_trx_action_fabric_tag_set
 * Purpose:
 ...
 */
STATIC int
_field_trx_action_fabric_tag_set(int unit, soc_mem_t mem,
                                 int value, uint32 *buf)
{
    ...
    traffic_manager = (value & BCM_FABRIC_QUEUE_CUSTOMER) ? 0x1 : 0x0;
    PolicySet(unit, mem, buf, EH_TMf, traffic_manager);

    /* Set queue number. */
    PolicySet(unit, mem, buf, EH_QUEUE_TAGf, value & 0xffff);
                               ...
                               _field_action_t *fa, uint32 *buf)
{
    ...
    switch (fa->action) {
      ...
      case bcmFieldActionRedirectTrunk:    /* param0 = trunk ID */
          shift_val = SOC_IS_TR_VL(unit) ? ((SOC_IS_TD_TT(unit) || SOC_IS_KATANA(unit)) ? 9 : 7) : 8;
      ...
      case bcmFieldActionRedirectMcast:
          ...
          shift_val = SOC_IS_TR_VL(unit) ? ((SOC_IS_TD_TT(unit) || SOC_IS_KATANA(unit)) ? 13 : 12) : 13;
          PolicySet(unit, mem, buf, REDIRECTIONf,
                         (redir_field | (2 << shift_val)));
          PolicySet(unit, mem, buf, G_PACKET_REDIRECTIONf, 0x3);
          break;
      case bcmFieldActionRedirectVlan:
          shift_val = (SOC_IS_TD_TT(unit) || SOC_IS_KATANA(unit)) ? 13 : 12;
      ...
      case bcmFieldActionOuterVlanNew:
          if (_BCM_FIELD_STAGE_LOOKUP == f_ent->group->stage_id) {
              ...
              if (SOC_IS_TRIUMPH2(unit) || SOC_IS_TD_TT(unit)
                  || SOC_IS_KATANA(unit) || SOC_IS_APOLLO(unit)
            ...
            for (f_ent_act = f_ent->actions; fa != NULL; fa = fa->next) {
                if (bcmFieldActionUcastCosQNew == f_ent_act->action) {
                    ...
                    if (BCM_GPORT_IS_UCAST_QUEUE_GROUP(f_ent_act->param[0])) {
                        BCM_IF_ERROR_RETURN
                            (_bcm_td_cosq_index_resolve(unit, f_ent_act->param[0], 0,
                                _BCM_TD_COSQ_INDEX_STYLE_UCAST_QUEUE, NULL,
                                    &ucosq, NULL));
                    } else {
                          ...
                          int tcam_idx, _field_action_t *fa, uint32 *buf)
{
    ...
    case bcmFieldActionCosQNew:
        ...
        if (SOC_IS_TD_TT(unit) || SOC_IS_KATANA(unit)) {
            BCM_IF_ERROR_RETURN(
                _bcm_field_trx_ucast_mcast_action_update(unit, mem,
    ...
    case bcmFieldActionGpPrioIntCancel:
        ...
        break;
    case bcmFieldActionAddClassTag:
        PolicySet(unit, mem, buf, l3sw_or_change_l2,
    ...
    case bcmFieldActionFabricQueue:
        ...
        BCM_IF_ERROR_RETURN
            (_field_trx_action_fabric_tag_set(unit, mem, fa->param[0], buf));
        break;
#if defined(BCM_TRIDENT_SUPPORT)
    case bcmFieldActionCompressSrcIp6:
            if (_BCM_FIELD_STAGE_LOOKUP == f_ent->group->stage_id) {
    ...
    if (_BCM_FIELD_STAGE_LOOKUP == f_ent->group->stage_id) {
        ...
        case bcmFieldActionIncomingMplsPortSet:
            *result = soc_feature(unit, soc_feature_mpls) ? TRUE : FALSE;
            break;
        case bcmFieldActionOamPbbteLookupEnable:
            *result = (SOC_IS_ENDURO(unit) || SOC_IS_KATANA(unit)) ? TRUE : FALSE;
            break;
        case bcmFieldActionUpdateCounter:
        ...
        case bcmFieldActionOuterVlanCfiNew:
#ifdef BCM_TRIDENT_SUPPORT
            *result = (SOC_IS_TD_TT(unit) || SOC_IS_KATANA(unit));
            break;
    case bcmFieldActionCompressSrcIp6:
    case bcmFieldActionCompressDstIp6:
        if (SOC_IS_TD_TT(unit) &&
        ...
        switch (action) {
          ...
          case bcmFieldActionRedirectBcastPbmp:
              if (SOC_IS_TD_TT(unit) || SOC_IS_ENDURO(unit) ||
                  SOC_IS_KATANA(unit) || SOC_IS_TRIUMPH2(unit)
                  || SOC_IS_APOLLO(unit)) {
                  *result = TRUE;
              } else {
                  *result = FALSE;
...
#if defined(INCLUDE_L3)
              if ((_BCM_FIELD_STAGE_INGRESS == f_ent->group->stage_id) &&
                  (SOC_IS_TD_TT(unit) || SOC_IS_TRIUMPH2(unit)
                  || SOC_IS_APOLLO(unit) || SOC_IS_KATANA(unit)
                  || SOC_IS_ENDURO(unit))) {
          ...
          case bcmFieldActionDynamicHgTrunkCancel:
              *result = SOC_MEM_FIELD_VALID(unit, FP_POLICY_TABLEm,
                                            DISABLE_DYNAMIC_LOAD_BALANCINGf);
              break;
          case bcmFieldActionUcastCosQNew:
          case bcmFieldActionGpUcastCosQNew:
          case bcmFieldActionYpUcastCosQNew:
          ...
          case bcmFieldActionYpMcastCosQNew:
          case bcmFieldActionRpMcastCosQNew:
              *result = SOC_IS_TD_TT(unit) ? TRUE : FALSE;
              break;
          case bcmFieldActionServicePoolIdNew:
...
_bcm_field_trx_udf_flags_to_index (int unit, int flags, uint32 *tbl_idx)
{
    ...
    *tbl_idx |= index;
    ...
    if ((*tbl_idx >  (uint32)soc_mem_index_max(unit, FP_UDF_OFFSETm)) &&
           ...
           /* Pair index is (Pool number * Pairs in Pool + Pair number) */
            ...
            if (SOC_IS_TD_TT(unit)) {
...
#endif
                {
                    ...
                    /* Set the meter mode */
                    soc_mem_field32_set(unit, mem, buf, pair_mode_field, meter_pair_mode);
    ...
    for (idx = 0; idx < idx_count; idx++) {
        if (stage_fc->data_ctrl->tcam_entry_arr[idx].ref_count) {
            ...
            /* Parse entry key to udf flags. */
                        ...
                        /* Compare if the flag bit settings are same */
                        if ((flags_hw  ^ flags_sw) == 0) {
                *tcam_idx = idx;
                break;
            }
        }
...
/*
 * Function:
 *     _field_trx2_udf_offset_entry_read
 * Purpose:
 ...
 */
STATIC int
_field_trx2_udf_offset_entry_read(int unit, int entry_idx,
...

{
    ...
    offset_base_encoding =
        soc_mem_field32_get(unit, FP_UDF_OFFSETm, (uint32 *)&offset_buf,
                                   ...
                                   bcm_field_data_packet_format_t *pkt_fmt)
{
    ...
    if ((ethertype == 0x800) && (inner_iptype ==  iptype_none_val)) {
        ...
        switch (soc_mem_field32_get(unit,
                                    FP_UDF_TCAMm,
                                    hw_buf,
                                    FC_HDR_ENCODE_1f
                                    )
        ...
        case 1:
            pkt_fmt->fibre_chan_outer = BCM_FIELD_DATA_FORMAT_FIBRE_CHAN;
            break;

        case 2:
            pkt_fmt->fibre_chan_outer = BCM_FIELD_DATA_FORMAT_FIBRE_CHAN_VIRTUAL;
            break;

        case 3:
            pkt_fmt->fibre_chan_outer = BCM_FIELD_DATA_FORMAT_FIBRE_CHAN_ENCAP;
            break;

        case 4:
            pkt_fmt->fibre_chan_outer = BCM_FIELD_DATA_FORMAT_FIBRE_CHAN_ROUTED;
            break;
        ...
        fc_header_encode = soc_mem_field32_get(unit, FP_UDF_TCAMm, hw_buf,
                                FC_HDR_ENCODE_2f);
        if (0 != fc_header_encode) {
            switch (fc_header_encode) {
                case 1:
                                  ...
                                  uint32 *hw_buf, uint8 *priority)
{
    ...
    if (pkt_fmt->tunnel == BCM_FIELD_DATA_FORMAT_TUNNEL_NONE) {
        ...
        *priority += _FP_DATA_QUALIFIER_PRIO_FCOE_FORMAT;
        ...
        soc_mem_field32_set(unit, FP_UDF_TCAMm, hw_buf, FC_HDR_ENCODE_2_MASKf,
            0x0);
    } else {
        return (BCM_E_UNAVAIL);
    }
...
/*
 * Function:
 *     _field_trx2_udf_tcam_entry_init
 * Purpose:
 *     Write the info in udf_spec to hardware
 * Parameters:
 *     unit     - (IN) BCM device number.
 ...
 */
STATIC int
_field_trx2_udf_tcam_entry_init(int unit, int flags, uint32 *hw_buf, uint8 *priority)
{
    ...
    /* Set vlan_tag format.*/
    rv = _field_trx2_udf_tcam_entry_vlanformat_init(unit, pkt_fmt.vlan_tag,
                                                   hw_buf, priority);
    BCM_IF_ERROR_RETURN(rv);

    /* Set l2 format.*/
    rv = _field_trx2_udf_tcam_entry_l2format_init(unit, pkt_fmt.l2,
                                                 hw_buf, priority);
    BCM_IF_ERROR_RETURN(rv);

    /* Set l3 fields.*/
    rv = _field_trx2_udf_tcam_entry_l3_init(unit, &pkt_fmt,
                                           hw_buf, priority);
    BCM_IF_ERROR_RETURN(rv);
    return (BCM_E_NONE);
}
 ...
 */
STATIC int
_field_trx2_udf_spec_clear(int unit,
                           _field_stage_t *stage_fc,
                           ...
                           uint32 udf_num, uint32 user_num)
{
    ...
    for (idx = 0; idx < COUNTOF(udf_spec->offset); idx++) {
        ...
        rv = _field_trx2_udf_tcam_entry_init(unit, flags, (uint32*)&tcam_buf, &priority);
        BCM_IF_ERROR_RETURN(rv);
        ...
        if (0 == tcam_entry_arr[tcam_idx].ref_count) {
            ...
            rv = _field_trx2_udf_offset_entry_write
                (unit, tcam_idx, udf_num, user_num,
 ...
 */
int
_field_trx2_udf_spec_remove(int unit, _field_stage_t *stage_fc,
                            int udf_num, int user_num)
{
    ...
    qid = udf_num * stage_fc->data_ctrl->num_elems + user_num;

    f_dq_prev = f_dq = stage_fc->data_ctrl->data_qual;

    while (NULL != f_dq) {
        if (qid == f_dq->qid) {
            rv = _field_trx2_udf_spec_clear(unit, stage_fc,
                                            f_dq->spec, udf_num, user_num);
            BCM_IF_ERROR_RETURN(rv);
 ...
 */
int
_field_trx2_udf_spec_insert(int unit, _field_stage_t *stage_fc,
                            int udf_num, int user_num,
                            bcm_field_udf_spec_t *spec)
{
    _field_data_qualifier_p f_dq;      /* Field data qualifier pointer.  */
    int rv;                            /* Operation return status.       */
    ...
    f_dq->qid = udf_num * stage_fc->data_ctrl->num_elems + user_num;
 ...
 */
int
_bcm_field_trx2_udf_delete(int unit, bcm_field_udf_spec_t *udf_spec,
                           uint32 udf_num, uint32 user_num)
{
    _field_stage_t *stage_fc;        /* Field stage control.      */
    int rv;                          /* Operation return status.  */
    ...
    rv = _field_trx2_udf_spec_remove(unit, stage_fc, udf_num, user_num);
    BCM_IF_ERROR_RETURN(rv);
    return (BCM_E_NONE);
}
 ...
 */
int
_bcm_field_trx2_udf_write(int unit, bcm_field_udf_spec_t *udf_spec,
                     uint32 udf_num, uint32 user_num)
{
    fp_udf_tcam_entry_t tcam_buf;              /* Udf tcam entry.           */
    _field_stage_t *stage_fc;                  /* Field stage control.      */
    int offset_base;                           /* Udf base offset.          */
    int tcam_idx;                              /* Tcam insertion index.     */
    int idx_max;                               /* Udf spec index max.       */
    int flags;                                 /* Udf API flags.            */
    int idx;                                   /* Udf spec iteration index. */
    int rv;                                    /* Operation return status.  */
    uint8                   priority = 0;     /* Udf tcam entry priority.   */
    ...
    for (idx = 0; idx < COUNTOF(udf_spec->offset); idx++) {
        ...
        BCM_IF_ERROR_RETURN(rv);
                ...
                /* reset priority to zero */
                priority = 0;

        /* Initialize udf tcam entry. */
        sal_memset(&tcam_buf, 0, sizeof(fp_udf_tcam_entry_t));
        rv = _field_trx2_udf_tcam_entry_init(unit, flags, (uint32*)&tcam_buf,
                                                                                                &priority);
        BCM_IF_ERROR_RETURN(rv);
        ...
        offset_base = (flags & (BCM_FIELD_USER_HIGIG | BCM_FIELD_USER_HIGIG2)) ?
                   bcmFieldDataOffsetBaseHigigHeader : \
                   bcmFieldDataOffsetBasePacketStart;
        rv =  _field_trx2_udf_offset_entry_write(unit, tcam_idx,
                                                 udf_num, user_num,
                                                 offset_base,
                                                 udf_spec->offset[idx]);
        BCM_IF_ERROR_RETURN(rv);
    ...
    rv = _field_trx2_udf_spec_insert(unit, stage_fc, udf_num, user_num,
                                     udf_spec);
    BCM_IF_ERROR_RETURN(rv);
 ...
 */
int
_bcm_field_trx2_udf_read(int unit, bcm_field_udf_spec_t *udf_spec,
                      uint32 udf_num, uint32 user_num)
{
    bcm_field_data_offset_base_t offset_base;
    ...
    for (idx = 0; idx < idx_count; idx++) {
        if (stage_fc->data_ctrl->tcam_entry_arr[idx].ref_count) {
            ...
            /* Parse entry key to udf flags. */
            rv = _bcm_field_trx2_udf_tcam_entry_parse(unit, entry_ptr, &flags);
            if (BCM_FAILURE(rv)) {
                break;
            }
            ...
            /* Get offset value. */
            rv = _field_trx2_udf_offset_entry_read(unit, idx, udf_num,
                                                   user_num, &offset_base,
                                                   &udf_spec->offset[tbl_idx]);
            if (BCM_FAILURE(rv)) {
                break;
            }
...
STATIC int
_field_tr2_meter_rate_burst_recover(uint32 unit,
                                    uint32 meter_table,
                                    uint32 mem_idx,
                                    uint32   *prate,
                                    uint32   *pburst
                                    )
{
    ...
    uint32 refresh_rate, granularity, bucket_count, bucket_max;

    rv = soc_mem_read(unit, meter_table, MEM_BLOCK_ANY, mem_idx, ent);

    if (BCM_FAILURE(rv)) {
        return rv;
    }

    refresh_rate = soc_mem_field32_get(unit,
                                       meter_table,
                                       ent,
                                       REFRESHCOUNTf
                                       );
    granularity = soc_mem_field32_get(unit,
                                      meter_table,
                                      ent,
                                      METER_GRANf
                                      );
    bucket_count = soc_mem_field32_get(unit,
                                       meter_table,
                                       ent,
                                       BUCKETCOUNTf
                                       );
    bucket_max = soc_mem_field32_get(unit,
                                     meter_table,
                                     ent,
                                     BUCKETSIZEf
                                     );
...
STATIC int
_field_tr2_meter_recover(int             unit,
                         _field_entry_t  *f_ent,
                         int             part,
                         bcm_policer_t   pid,
                         uint32          level,
                         ...
                         )
{
    ...
    uint32 *prate, *pburst;
    uint32 stage_id, meter_pair_mode=0, index_field, modifier_field;
    uint32 pair_index, modifier_mode, pool_index=0, hw_index=0, meter_offset;
    ...
    case  _BCM_FIELD_STAGE_INGRESS:
        ...
        pair_index = soc_mem_field32_get(unit,
                                         policy_mem,
                                         policy_buf,
                                         SOC_IS_TD_TT(unit)
                                             ? METER_PAIR_INDEX_EVENf
                                             : index_field
                                         );

        modifier_mode = soc_mem_field32_get(unit,
                                            policy_mem,
                                            policy_buf,
                                            modifier_field
                                            );
        /* Calculate pool_index and hw_index */
        for (hw_index = pair_index, pool_index = 0;
             hw_index >= stage_fc->meter_pool[pool_index]->size;
             hw_index -= stage_fc->meter_pool[pool_index]->size,
                 ++pool_index);
    ...
    if (((stage_id != _BCM_FIELD_STAGE_EGRESS)
         && _FP_METER_BMP_TEST(f_mp->meter_bmp, hw_index)
         )
        ...
        case BCM_FIELD_METER_MODE_FLOW: /* 1 */
            ...
            case _BCM_FIELD_STAGE_EXTERNAL:
                if (SOC_IS_TD_TT(unit)) {
                    if (soc_mem_field32_get(unit, policy_mem, policy_buf, METER_TEST_ODDf)) {
                ...
                mem_idx = stage_fc->slices[f_pl->pool_index].start_tcam_idx
                    + (2 * f_pl->hw_index) + meter_offset;
                _field_tr2_meter_rate_burst_recover(unit,
                                                    FP_METER_TABLEm,
                                                    mem_idx,
                                                    prate,
                                                    pburst);
        ...
        case BCM_FIELD_METER_MODE_srTCM_COLOR_AWARE: /* 7 */
            if (stage_id == _BCM_FIELD_STAGE_EGRESS) {
                ...
                f_pl->cfg.mode = modifier_mode ?
                    bcmPolicerModeSrTcmModified : bcmPolicerModeSrTcm;
            }
        ...
        case 5:
            if (fg->stage_id != _BCM_FIELD_STAGE_EGRESS) {
                f_pl->cfg.mode = modifier_mode ?
                    bcmPolicerModeCoupledTrTcmDs : bcmPolicerModeTrTcmDs;
            }
            break;
        default:
            ;
        ...
        if (stage_id == _BCM_FIELD_STAGE_EGRESS) {
            ...
            _FP_METER_BMP_ADD(f_mp->meter_bmp, hw_index);
            f_mp->free_meters--;
        }
                           ...
                           )
{
    ...
    _field_policer_t *f_pl = NULL;
    _field_entry_policer_t *f_ent_pl;
    int rv, policer_id, meter_mem_idx, meter_hw_idx;
    uint32 ent[SOC_MAX_MEM_FIELD_WORDS], ent_pl_flags = 0;
    uint32 stage_id, meter_pair_mode;
    uint32 pair_index, modifier_mode, pool_index, hw_index, meter_offset;
    _field_meter_pool_t *f_mp = NULL;
    _meter_config_t meter_conf;
    soc_mem_t meter_mem;
    uint32 *prate, *pburst;
    ...
    rv = _field_stage_control_get(unit, stage_id, &stage_fc);
    BCM_IF_ERROR_RETURN(rv);
    meter_mem = (stage_id == _BCM_FIELD_STAGE_INGRESS) ? FP_METER_TABLEm :
                       EFP_METER_TABLEm;
    ...
    if (stage_id == _BCM_FIELD_STAGE_INGRESS) {
        ...
        meter_conf.meter_update_odd = soc_mem_field32_get(unit,
                                                          ...
                                                          policy_buf,
                                                          METER_UPDATE_ODDf);
        meter_conf.meter_test_odd = soc_mem_field32_get(unit, EFP_POLICY_TABLEm,
                                                        policy_buf,
                                                        METER_TEST_ODDf);
        meter_conf.meter_update_even = soc_mem_field32_get(unit,
                                                           EFP_POLICY_TABLEm,
    ...
    if (((stage_id == _BCM_FIELD_STAGE_INGRESS) &&
        ...
        /* Calculate meter index */
        if (stage_fc->flags & _FP_STAGE_GLOBAL_METER_POOLS) {
            /* Hw index is (Pool number * 2 * Pairs in Pool + 2 * Pair number) */
            meter_mem_idx = (2 * f_pl->pool_index *
                             stage_fc->meter_pool[f_pl->pool_index]->size) +
                             (2 * f_pl->hw_index);
        } else {
            /* Hw index is (Slice number + 2 * Pair number) */
            meter_mem_idx  = stage_fc->slices[f_pl->pool_index].start_tcam_idx +
                                (2 * f_pl->hw_index);
        }
        ...
        switch (meter_conf.meter_mode) {
            ...
            case BCM_FIELD_METER_MODE_FLOW: /* 1 */
                ...
                meter_hw_idx = meter_mem_idx + meter_offset;
                rv = soc_mem_read(unit, meter_mem, MEM_BLOCK_ANY,
                                  meter_hw_idx, ent);
                if (BCM_FAILURE(rv)) {
                    sal_free(f_pl);
                    return rv;
...
STATIC int
_field_tr2_actions_recover_action_add(int                unit,
                                      _field_entry_t     *f_ent,
                                      bcm_field_action_t action,
                                      uint32             param0,
                                      uint32             param1,
                           ...
                           )
{
    ...
    uint32 fldval, param0, param1, hw_index, append, param2 = 0, param3 = 0;
    ...
    void *entries[1];                   /* Redirection profile table pointer */
    ...
    for (i = 0; i < num_fields; i++) {
            ...
            switch (fldval) {
                ...
                case 1:
                    ...
                    if (PolicyGet(unit, policy_mem, policy_entry, ECMPf) == 0) {
                         ...
                         * FP entry */
                        rv = _field_tr2_actions_recover_action_add(unit,
                        ...
                        */
                        ...
                        if (soc_mem_field32_get(
                            ...
                            */
                            ...
                            rv = _field_tr2_actions_recover_action_add(
                                     unit,
                                     f_ent,
        ...
        case METER_PAIR_MODEf:
            BCM_IF_ERROR_RETURN(
                SOC_IS_SC_CQ(unit)
                ? _field_sc_cq_meter_recover(unit, f_ent, part, pid, policy_entry)
                : _field_tr2_meter_recover(unit, f_ent, part, pid, 0, policy_mem, policy_entry)
                                );
        ...
        case METER_SHARING_MODEf:
            BCM_IF_ERROR_RETURN
                (_field_tr2_meter_recover(unit, f_ent, part, pid, 1, policy_mem, policy_entry));
                ...
                if (SOC_IS_TD_TT(unit)) {
                    if ((1 == PolicyGet(unit, policy_mem, policy_entry,
                        ...
                        if (uc_cosq == mc_cosq) {
                             ...
                             */
                            rv = _field_tr2_actions_recover_action_add(unit,
                                    f_ent, bcmFieldActionUcastCosQNew, uc_cosq, 0,
                                    0, 0, hw_index);
                            if (BCM_FAILURE(rv)) {
                                return (rv);
                            }
                            rv = _field_tr2_actions_recover_action_add(unit,
                                    f_ent, bcmFieldActionMcastCosQNew, mc_cosq, 0,
                                    0, 0, hw_index);
                            if (BCM_FAILURE(rv)) {
                                return (rv);
                            }
        ...
        case MPLS_ACTIONf:
            switch (fldval) {
                case 1:
                    ...
                    BCM_GPORT_MPLS_PORT_ID_SET(param0, PolicyGet(unit, policy_mem,
                                               policy_entry, SVPf));
                    append = 1;
                    break;
                case 3:
        ...
        case DISABLE_VLAN_CHECKSf:
            if (!soc_mem_field_valid(unit, policy_mem, MPLS_ACTIONf)
                || PolicyGet(unit, policy_mem, policy_entry, MPLS_ACTIONf) != 1
                ) {
                if (fldval) {
        ...
        case G_PACKET_REDIRECTIONf:
            {
                uint32 redir = PolicyGet(unit,
                                         policy_mem,
                                         policy_entry,
                                         REDIRECTIONf
                                         );
                unsigned sh = SOC_IS_TR_VL(unit)
                    ? ((SOC_IS_TD_TT(unit) || SOC_IS_KATANA(unit)) ? 9 : 7)
                    : 8,
                    m = (1 << sh) - 1;
...
{
    int parts_count = 1, rv = BCM_E_NONE;
    ...
    if (fg_ptr == NULL) {
        ...
        if (port != -1) {
            ...
            SOC_PBMP_ASSIGN(fg_ptr->pbmp, PBMP_PORT_ALL(unit));
            SOC_PBMP_OR(fg_ptr->pbmp, PBMP_CMIC(unit));
        }
...
STATIC void
_field_tr2_group_part_fixup(_bcm_field_group_qual_t *grp_qual, uint8 *cnt_arr,
...
STATIC int
_field_tr2_stage_reinit_all_groups_fixup(int unit, _field_control_t *fc, unsigned stage_id)
{
    int            rv = BCM_E_NONE;
    unsigned       cnt_arr_sz;
    uint8          *cnt_arr;
        ...
        */
        ...
        for (part_idx = 0; part_idx < parts_cnt; ++part_idx) {
            ...
            for (i = 0; i < fg->group_status.entry_count; ++i) {
                _field_entry_t *f_ent               = fg->entry_arr[i];
            ...
            */
            ...
            _field_tr2_group_part_fixup(grp_qual, cnt_arr, tcam_mask, part_idx);
        }
    }
                                         ...
                                         )
{
    ...
    vmap_cnt =
        ...
        SOC_IS_TD_TT(unit) | SOC_IS_KATANA(unit) ? 1 :
        _FP_VMAP_CNT;
...
int
_field_tr2_scache_slice_group_recover(int              unit,
                                      _field_control_t *fc,
                                      int              slice_num,
                                      int              *multigroup
                                      )
{
    ...
    while (next_group_valid) {
        ...
        for (i = 0; i < qset_count; i++) {
            BCM_FIELD_QSET_ADD(new_grp->qset, buf[fc->scache_pos]);
            fc->scache_pos++; /* Next QID */

            FP_VVERB(("Read qualifier @ %d.\n", fc->scache_pos - 1));
        }
...
int
_field_tr2_entry_info_retrieve(int               unit,
...
/*
 * Function:
 *     _field_tr2_tunnel_type_get
 *
 * Purpose:
 *     Retrieves Tunnel Type information from installed TCAM entry.
 *
 * Parameters:
 *     unit             - (IN)BCM device number
 ...
 */
STATIC void
_field_tr2_tunnel_type_get(int unit, int slice_idx, uint32 *fp_tcam_buf,
                              int slice_ent_cnt, _field_stage_t *stage_fc,
                              int8 *tunnel_type_sel)
{
    _field_slice_t *fs;
    ...
    for (idx = 0; idx < slice_ent_cnt; idx++) {
        ...
        if (soc_FP_TCAMm_field32_get(unit, tcam_entry, VALIDf) != 0) {
            ...
            _field_extract((uint32 *)tcam_entry, 21, 4, &tunnel_type);
            switch (tunnel_type) {
                case 0x1:
                    *tunnel_type_sel = (int8) bcmFieldTunnelTypeIp;
                    break;
                case 0x2:
                    *tunnel_type_sel = (int8) bcmFieldTunnelTypeMpls;
                    break;
                case 0x3:
                    *tunnel_type_sel = (int8) bcmFieldTunnelTypeMim;
                    break;
                case 0x4:
                    *tunnel_type_sel = (int8) bcmFieldTunnelTypeWlanWtpToAc;
                    break;
                case 0x5:
                    *tunnel_type_sel = (int8) bcmFieldTunnelTypeWlanAcToAc;
                    break;
                case 0x6:
                    *tunnel_type_sel = (int8) bcmFieldTunnelTypeAutoMulticast;
                    break;
                case 0x7:
                    *tunnel_type_sel = (int8) bcmFieldTunnelTypeTrill;
                    break;
                default:
                    ;
            }
            if (*tunnel_type_sel != _FP_SELCODE_DONT_CARE) {
                break;
            }
        }
    }
                                ...
                                )
{
    ...
    bcm_pbmp_t temp_pbm_mask;
    uint32  tcam_dual_pipe_entry[SOC_MAX_MEM_FIELD_WORDS];
#endif
    ...
    for (slice_idx = 0; slice_idx < stage_fc->tcam_slices; slice_idx++) {
        ...
        if (fc->l2warm) {
            rv = _field_tr2_scache_slice_group_recover(unit,
                                                       fc,
                                                       slice_idx,
                                                       &multigroup
                                                       );
        ...
        PBMP_ALL_ITER(unit, port) {
            ...
            if (SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit)
                || SOC_IS_VALKYRIE2(unit)) {
                _field_tr2_tunnel_type_get(unit, slice_idx, fp_tcam_buf,
        ...
        for (idx = 0; idx < slice_ent_cnt; idx++) {
            ...
            sid = pid = -1;
            if (fc->l2warm) {
                rv = _field_tr2_entry_info_retrieve(unit,
                                                    &eid,
                                                    &prio,
                                                    fc,
                                                    multigroup,
    ...
    _field_tr2_stage_reinit_all_groups_fixup(unit, fc, _BCM_FIELD_STAGE_INGRESS);
...
static uint16 list_key1[27] = {bcmFieldQualifyL4Ports, bcmFieldQualifyIpFrag,
                      ...
                      bcmFieldQualifyPortClass, bcmFieldQualifyOutPort,
...
static uint16 list_key2[24] = {bcmFieldQualifyL4Ports, bcmFieldQualifyIpProtocol,
                      ...
                      bcmFieldQualifyDstHiGig, bcmFieldQualifyPortClass,
                               ...
                               _field_stage_t *stage_fc)
{
    ...
    int i, pri_tcam_idx, part_index, slice_number;
    int list_size[2], prio, prev_prio, expanded[4];
    uint16 *list_arr[2];
    ...
    for (slice_idx = 0; slice_idx < stage_fc->tcam_slices; slice_idx++) {
        ...
        if (efp_slice_mode <= 5) { /* Valid values */
            ...
            SOC_PBMP_ASSIGN(fg->pbmp, PBMP_PORT_ALL(unit));
            SOC_PBMP_OR(fg->pbmp, PBMP_CMIC(unit));
    ...
    _field_tr2_stage_reinit_all_groups_fixup(unit, fc, _BCM_FIELD_STAGE_EGRESS);
                               ...
                               _field_stage_t *stage_fc)
{
    ...
    for (slice_idx = 0; slice_idx < stage_fc->tcam_slices; slice_idx++) {
        ...
        /* Ignore secondary slice in paired mode */
        fld = _trx_slice_pairing_field[slice_idx / 2];
        ...
        hw_sels.pri_slice[0].fpf2 = soc_reg_field_get(unit, VFP_KEY_CONTROLr,
                                    vfp_key, _trx_vfp_field_sel[slice_idx][0]);
        hw_sels.pri_slice[0].fpf3 = soc_reg_field_get(unit, VFP_KEY_CONTROLr,
                                    vfp_key, _trx_vfp_field_sel[slice_idx][1]);
        hw_sels.pri_slice[0].ip_header_sel = soc_reg_field_get(unit,
                                           VFP_KEY_CONTROL_2r, vfp_key2,
                                           _trx_vfp_ip_header_sel[slice_idx]);
        /* If intraslice, get double-wide key - only 2 options */
        if (intraslice) {
            dbl_wide_key = soc_reg_field_get(unit, VFP_KEY_CONTROLr, vfp_key,
                                     _trx_vfp_double_wide_sel[slice_idx]);
            hw_sels.pri_slice[1].intraslice = TRUE;
            hw_sels.pri_slice[1].fpf2 = dbl_wide_key;
            hw_sels.pri_slice[1].ip_header_sel = soc_reg_field_get(unit,
                                           VFP_KEY_CONTROL_2r, vfp_key2,
                                           _trx_vfp_ip_header_sel[slice_idx]);
        }
        /* If paired, get secondary slice's selectors */
        if (paired) {
            ...
            hw_sels.sec_slice[0].fpf2 = soc_reg_field_get(unit,
                                                          VFP_KEY_CONTROLr,
                                                          vfp_key,
                                          _trx_vfp_field_sel[slice_idx + 1][0]);
            hw_sels.sec_slice[0].fpf3 = soc_reg_field_get(unit,
                                                          VFP_KEY_CONTROLr,
                                                          vfp_key,
                                          _trx_vfp_field_sel[slice_idx + 1][1]);
            hw_sels.sec_slice[0].ip_header_sel = soc_reg_field_get(unit,
                                           VFP_KEY_CONTROL_2r, vfp_key2,
                                           _trx_vfp_ip_header_sel[slice_idx]);
            if (intraslice) {
                dbl_wide_key_sec = soc_reg_field_get(unit, VFP_KEY_CONTROLr,
                            vfp_key, _trx_vfp_double_wide_sel[slice_idx]);
                hw_sels.sec_slice[1].intraslice = TRUE;
                hw_sels.sec_slice[1].fpf2 = dbl_wide_key_sec;
                hw_sels.sec_slice[1].ip_header_sel = soc_reg_field_get(unit,
                                               VFP_KEY_CONTROL_2r, vfp_key2,
                                           _trx_vfp_ip_header_sel[slice_idx]);
            }
        }
        ...
        for (idx = 0; idx < slice_ent_cnt / ratio; idx++) {
            ...
            for (i = 0; i < parts_count; i++) {
                ...
                if (SOC_IS_KATANA(unit)) {
                    ...
                    _bcm_esw_get_policer_id_from_index_offset(unit, policer_index, 
    ...
    _field_tr2_stage_reinit_all_groups_fixup(unit, fc, _BCM_FIELD_STAGE_LOOKUP);
...
STATIC unsigned
_field_tr2_ext_scache_size(int       unit,
                           unsigned  part_idx,
                           soc_mem_t *mems
                           )
{
                                   ...
                                   )
{
    ...
    return (_field_tr2_ext_scache_size(unit, part_idx, mems));
}
                           ...
                           )
{
    ...
    h->scache_nwords = _field_tr2_ext_scache_size(h->unit,
                                                  part_idx,
                                                  h->mems
                                                  );
...
_field_tr2_ext_scache_wr(struct _field_tr2_ext_scache_rw *h, uint8 val)
{
    if (h->scache_nwords != 0) {
        ...
        *p = (*p & ~(0xff << sh)) | val << sh;
        ...
        fc->scache_ptr[fc->scache_pos] = val;
        ++fc->scache_pos;
    }
...
_field_tr2_ext_scache_rd(struct _field_tr2_ext_scache_rw *h, uint8 *val)
{
    if (h->scache_nwords != 0) {
        ...
        *val = fc->scache_ptr[fc->scache_pos];
        ++fc->scache_pos;
    }
        ...
        for (entry_cnt = entry_idx = 0;
            ...
            */
            ...
            if (tcam_data_mask_mem != 0) {
                ...
                rv = soc_mem_read(unit, tcam_mask_mem, MEM_BLOCK_ANY, entry_idx, tcam_buf.entry_data);
                if (rv != BCM_E_NONE) {
                    goto cleanup;
                }
    ...
    for (slice_idx = 0; slice_idx < stage_fc->tcam_slices; ++slice_idx) {
        ...
        for (entry_cnt = entry_idx = 0;
            ...
            */
            ...
            f_ent->tcam.key_hw  = sal_alloc(f_ent->tcam.key_size, "");
            f_ent->tcam.mask_hw = sal_alloc(f_ent->tcam.key_size, "");
