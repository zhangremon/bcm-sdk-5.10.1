/*
 * $Id: virtual.c 1.49.2.6 Broadcom SDK $
...
_bcm_virtual_reinit(int unit) {
    ...
    /* Extended scache support available */
        ...
        num_vfi = soc_mem_index_count(unit, VFIm);
        num_vp = soc_mem_index_count(unit, SOURCE_VPm);
        SOC_SCACHE_HANDLE_SET(scache_handle, unit, BCM_MODULE_AUTH, 0);
    ...
    if (SOC_WARM_BOOT(unit)) {
        ...
        if (soc_feature(unit, soc_feature_vlan_vp)) {
...
#endif /* BCM_ENDURO_SUPPORT */
            {
                ...
                rv = bcm_trident_vlan_virtual_reinit(unit);
#endif /* BCM_TRIDENT_SUPPORT */
            } 
        }
                             ...
                             int fs_idx)
{
    ...
    if (_bcm_vfi_used_get(unit, vfi, type)) {
        ...
        if (BCM_SUCCESS(rv)) {
            rv = soc_mem_read(unit, EGR_VFIm, MEM_BLOCK_ANY, vfi,
                              &egr_vfi_entry);
            if (BCM_SUCCESS(rv)) {
                ...
                rv = soc_mem_write(unit, EGR_VFIm, MEM_BLOCK_ANY, vfi,
                                   &egr_vfi_entry);
            }
        }
    } else {
