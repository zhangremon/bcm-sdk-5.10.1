/*
 * $Id: multicast.c 1.73.2.16 Broadcom SDK $
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        if (soc_feature(unit, soc_feature_trill)) {
            if (valid) {
                ...
                /* Check EGR_IPMC.IPMC_GROUP_TYPE for TRILL specific IPMC Group */
                rv = READ_EGR_IPMCm(unit, MEM_BLOCK_ANY, ipmc_id, &egr_ipmc);
                if(BCM_SUCCESS(rv)) {
                          ...
                          bcm_port_t port, bcm_if_t encap_id)
{
    ...
    if (soc_feature(unit, soc_feature_trill) && 
        ...
        return bcm_td_trill_egress_add(unit, port, group, encap_id);
    }
#endif /* BCM_TRIDENT_SUPPORT */
                             ...
                             bcm_port_t port, bcm_if_t encap_id)
{
    ...
    if (soc_feature(unit, soc_feature_trill) && 
        (BCM_GPORT_IS_TRILL_PORT(port)) ) {
        return bcm_td_trill_egress_delete(unit, port, mc_index, encap_id);
    }
#endif /* BCM_TRIDENT_SUPPORT */
...
bcm_esw_multicast_init(int unit)
{
    ...
    if (SOC_IS_XGS3_FABRIC(unit)) {
        ...
        return _bcm_esw_fabric_multicast_init(unit);
    }
#endif /* BCM_XGS3_FABRIC_SUPPORT */
...
bcm_esw_multicast_detach(int unit)
{
    ...
    if (SOC_IS_XGS3_FABRIC(unit)) {
        /* Fabrics must track the multicast groups here, since they
         * are not recorded in the mcast or IPMC modules. */
        return _bcm_esw_fabric_multicast_detach(unit);
    }
#endif /* BCM_XGS3_FABRIC_SUPPORT */

#if defined(BCM_TRX_SUPPORT) && defined(INCLUDE_L3)
    if (SOC_IS_TRX(unit)) {
        return bcm_trx_multicast_detach(unit);
    }
#endif /* BCM_TRX_SUPPORT && INCLUDE_L3 */
    return BCM_E_NONE;
}
                                         ...
                                         bcm_gport_t subscriber_queue)
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_esw_ipmc_set_remap_group(unit, mc_index, port_out, 1)); 

    /* Add port to the IPMC_L3_BITMAP or L2_BITMAP as decided above */
    rv = _bcm_esw_multicast_ipmc_read(unit, mc_index, &l2_pbmp, &l3_pbmp);

   /* decide for work around based on num_copies
    *  num_copies=0, no need for any work around, non-zero, then its a new chain
    */
