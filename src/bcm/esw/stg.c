/*
 * $Id: stg.c 1.54.2.3 Broadcom SDK $
...
#define CHECK_VID(unit, vid) do { \
	if (vid > BCM_VLAN_MAX) return BCM_E_BADID; \
	} while (0);
...
/* Function:
 *      _bcm_stg_mstp_port_vlan_enable_set
 * Purpose:
 *      Set the enable bit for a (port, VLAN) mask.
 */
STATIC int
_bcm_stg_mstp_port_vlan_enable_set(int unit, bcm_port_t port,
                                   bcm_vlan_t vid, int enable)
{
#ifdef	INCLUDE_L3
    if (SOC_IS_TUCANA(unit)) {
        return _bcm_tucana_ipmc_repl_vlan_stp_set(unit, port, vid, enable);
    }
#endif

    if (SOC_IS_LYNX(unit)) {
        egress_vlan_stg_entry_t entry;
        int index = vid / BCM_LYNX_VLANS_PER_MSTP_ENTRY;
        int bit = vid % BCM_LYNX_VLANS_PER_MSTP_ENTRY;
        int rv;
        soc_block_t blk = SOC_PORT_BLOCK(unit, port);
        uint32 stg_vector, bit_pos;

        soc_mem_lock(unit, EGRESS_VLAN_STGm);
        if ((rv = READ_EGRESS_VLAN_STGm(unit, blk, index, &entry)) < 0) {
            soc_mem_unlock(unit, EGRESS_VLAN_STGm);
            return rv;
        }

        stg_vector =
            soc_EGRESS_VLAN_STGm_field32_get(unit, &entry, STG_VECTORf);
        bit_pos = 1 << bit;
        if (((stg_vector & bit_pos) != 0) != enable) {
            stg_vector ^= bit_pos;
            soc_EGRESS_VLAN_STGm_field32_set(unit, &entry,
                                             STG_VECTORf, stg_vector);
            rv = WRITE_EGRESS_VLAN_STGm(unit, blk, index, &entry);
        }
...
_bcm_stg_mstp_vlan_update(int unit, bcm_stg_t stg, bcm_vlan_t vid)
{
    ...
    PBMP_E_ITER(unit, port) {
        ...
        BCM_IF_ERROR_RETURN
            (_bcm_stg_mstp_port_vlan_enable_set(unit, port, vid,
                           _BCM_STG_STP_TO_MSTP_MASK_ENABLE(stp_state)));
    }
...
bcm_esw_stg_init(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_mstp_mask) && SOC_IS_LYNX(unit)) {
        egress_vlan_stg_entry_t entry;
        int                     index, min_index, max_index;
        bcm_port_t              port;

        min_index = soc_mem_index_min(unit, EGRESS_VLAN_STGm);
        max_index = soc_mem_index_max(unit, EGRESS_VLAN_STGm);
        sal_memcpy(&entry, soc_mem_entry_null(unit, EGRESS_VLAN_STGm),
                   soc_mem_entry_words(unit, EGRESS_VLAN_STGm) * 4);

        /* Init MSTP mask memory */
        PBMP_E_ITER(unit, port) {
            soc_block_t blk = SOC_PORT_BLOCK(unit, port);
            for (index = min_index; index <= max_index; index++) {
                BCM_IF_ERROR_RETURN
                    (WRITE_EGRESS_VLAN_STGm(unit, blk, index, &entry));
            }
        }
    }
                          ...
                          int stp_state)
{
    ...
    while (v != BCM_VLAN_INVALID) {
        BCM_IF_ERROR_RETURN
            (_bcm_stg_mstp_port_vlan_enable_set(unit, port, v, enable));
        v = si->vlan_next[v];
    }
...
_bcm_stg_sw_dump(int unit)
{
    ...
    soc_cm_print("  Memory     : %d(%4d-%4d)\n",
                 si->stg_mem, si->stg_min, si->stg_max);
    soc_cm_print("  Default STG: %4d\n", si->stg_defl);
    soc_cm_print("  Count      : %4d\n", si->stg_count);

    if (si->stg_max >= BCM_VLAN_COUNT) {
        soc_cm_print("More STGs than VLANs!\n");
