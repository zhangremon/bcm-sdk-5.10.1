/*
 * $Id: port.c 1.191.2.5 Broadcom SDK $
...
#ifdef BCM_EASYRIDER_SUPPORT 
    /* HIGIG physical port 0 info located in HG_PORT_TABLEm */ 
    if(SOC_IS_EASYRIDER(unit) && (port == SOC_PORT_MAX(unit, hg))) {
        /* In HG-HG mode, only the "true" HG uses HG_PORT_TABLE */
        ...
        if (SOC_MEM_IS_VALID(unit, IPORT_TABLEm)) {
            ...
            BCM_IF_ERROR_RETURN
                (soc_mem_write(unit, PORT_TABm, MEM_BLOCK_ALL,
                               SOC_INFO(unit).cpu_hg_index, &ptab_entry));
        }
    }
...
_bcm_fb_port_cfg_init(int unit, bcm_port_t port, bcm_vlan_data_t *vlan_data)
{
    ...
    if (IS_CPU_PORT(unit,port)) {
        ...
        if (SOC_MEM_IS_VALID(unit, IPORT_TABLEm)) {
            ...
            BCM_IF_ERROR_RETURN
                (soc_mem_write(unit, PORT_TABm, MEM_BLOCK_ALL,
                               SOC_INFO(unit).cpu_hg_index, &ptab));
        }
                              ...
                              uint32 kbits_sec, uint32 kbits_burst)
{
    ...
    if (SOC_IS_FIREBOLT2(unit) || SOC_IS_RAVEN(unit) || SOC_IS_HAWKEYE(unit) || SOC_IS_HURRICANE(unit)) {
    ...
    if (SOC_IS_FIREBOLT2(unit) || SOC_IS_RAVEN(unit) || 
        SOC_IS_HAWKEYE(unit) || SOC_IS_SC_CQ(unit) || SOC_IS_HURRICANE(unit)) {
...
/*
 * Function:
 *      _bcm_er_port_nni_field_set
 * Purpose:
 *      Set port network to network indication bit
 * Parameters:
 *      unit       - (IN)SOC unit number.
 *      port       - (IN)Port number.
 *      port_cfg   - (IN)API port information structure.
 ...
 */
STATIC INLINE int
_bcm_er_port_nni_field_set(int unit, bcm_port_t port, bcm_port_cfg_t *port_cfg)
{
    int nni_val;             /* Network to Network register new value.*/        

    nni_val = (port_cfg->pc_nni_port) ? 1 : 0;                                  

    BCM_IF_ERROR_RETURN(soc_reg_field32_modify(unit, EGR_PORTr, port,
                                               NNIf, nni_val));

    BCM_IF_ERROR_RETURN(soc_reg_field32_modify(unit, IGR_PORTr,
                                               port, NNI_PORTf, nni_val));

    if (IS_CPU_PORT(unit, port)) {
        BCM_IF_ERROR_RETURN(soc_reg_field32_modify(unit, IEGR_PORTr, port,
                                                   NNIf, nni_val));

        BCM_IF_ERROR_RETURN(soc_reg_field32_modify(unit, IGR_IPORTr, port,
                                                   NNI_PORTf, nni_val));
    }
    return (BCM_E_NONE);
}
 ...
 */
int
_bcm_er_port_cfg_init(int unit, bcm_port_t port, bcm_vlan_data_t *vlan_data)
{
    port_tab_entry_t ptab;             /* Port table entry.       */
    hg_port_table_entry_t hgptab;      /* Higig port table entry. */
    iport_table_entry_t iptab;         /* Cpu port table.         */
    uint32 higig_port;                 /* Higig port indicator.   */

    higig_port = IS_HG_PORT(unit,port) ? 1 : 0;

    /* Set port is higig to EGR_PORTr & IGR_PORTr registers. */
    BCM_IF_ERROR_RETURN
        (soc_reg_field32_modify(unit, EGR_PORTr, port, HIGIGf, higig_port));
    BCM_IF_ERROR_RETURN
        (soc_reg_field32_modify(unit, IGR_PORTr, port, HIGIGf, higig_port));

    /* Write port table entry. */
    sal_memcpy(&ptab, soc_mem_entry_null(unit, PORT_TABm), sizeof (ptab));
    if (!higig_port) {
        /* Also includes the "non-HiGig-format" settings for the CPU port */
        soc_PORT_TABm_field32_set(unit, &ptab, MAC_BASED_VID_ENABLEf, 1);
        soc_PORT_TABm_field32_set(unit, &ptab, SUBNET_BASED_VID_ENABLEf, 1);
        soc_PORT_TABm_field32_set(unit, &ptab, PORT_VIDf, vlan_data->vlan_tag);

        /* When looking up VRF_VFI_INTF table, ignore the port_id.
         * The key to the VRF_VFI_INTF table will only be vid. This
         * table is used in L3 for determining the VRF. It is also used
         * for L2 MPLS for determining the VFI.
         */
        soc_PORT_TABm_field32_set(unit, &ptab, IGNORE_PORT_IDf, 1);
    }

    if (port == SOC_PORT_MAX(unit, hg)) {
        /* Reset HG_PORT_TABLEm entry for higig port. */
        sal_memcpy(&hgptab, soc_mem_entry_null(unit, HG_PORT_TABLEm), 
                   sizeof (hgptab));
        SOC_IF_ERROR_RETURN
            (BCM_XGS3_MEM_WRITE(unit, HG_PORT_TABLEm, 0, &hgptab));
    } else {
        SOC_IF_ERROR_RETURN(BCM_XGS3_MEM_WRITE(unit, PORT_TABm, port, &ptab));
    }

    if (IS_CPU_PORT(unit,port)) {
        /* Configure the "HiGig-format" settings for the CPU port */
        sal_memcpy(&iptab, soc_mem_entry_null(unit, IPORT_TABLEm), 
                   sizeof (iptab));

        BCM_IF_ERROR_RETURN
            (BCM_XGS3_MEM_WRITE(unit, IPORT_TABLEm, port, &iptab));

        BCM_IF_ERROR_RETURN
            (soc_reg_field32_modify(unit, IEGR_PORTr, port, HIGIGf, 1));
        BCM_IF_ERROR_RETURN
            (soc_reg_field32_modify(unit, IGR_IPORTr, port, HIGIGf, 1));
    }
...
/*
 * Function:
 *   	bcm_er_port_rate_egress_set
 * Purpose:
 *	     Set egress rate limiting parameters for the Easyrider chip.
 * Parameters:
 *	unit        -(IN)SOC unit number.
 *	port        -(IN)Port number.
 *	kbits_sec   -(IN)Rate in kilobits (1000 bits) per second.
 *	    		    Rate of 0 disables rate limiting.
 *	kbits_burst -(IN)Not applicable for Easyrider.
 * Returns:
 *	BCM_E_XXX
 */
int bcm_er_port_rate_egress_set(int unit, bcm_port_t port, 
                                uint32 kbits_sec, uint32 kbits_burst)
{
    uint32 rate;         /* Egress rate limit. */

    /* Calculate egress rate limit. */
    if (kbits_sec) {
        BCM_XGS3_PORT_RATE_64KBIT_QUANTUM(kbits_sec, rate)
    } else {
        rate = (1 << soc_reg_field_length(unit, SHAPERMAXBWPORTr, MAX_BWf)) - 1;
    }

    /* Update egress rate limit. */
    BCM_IF_ERROR_RETURN
        (soc_reg_field32_modify(unit, SHAPERMAXBWPORTr, port, MAX_BWf, rate));
    return (BCM_E_NONE);
}

/*
 * Function:
 *	bcm_er_port_rate_egress_get
 * Purpose:
 *	Get egress rate limiting parameters from the Easyrider chip.
 * Parameters:
 *	unit -        (IN)SOC unit number.
 *	port -        (IN)Port number.
 *	kbits_sec -  (OUT) Rate in kilobits (1000 bits) per second, or
 *	     		  zero if rate limiting is disabled.
 *	kbits_burst - (OUT) Always returns 0.
 * Returns:
 *	BCM_E_XXX
 */
int bcm_er_port_rate_egress_get(int unit, bcm_port_t port,
                                uint32 *kbits_sec, uint32 *kbits_burst)
{
    uint32 regval;        /* SHAPERMAXBWPORTr register value. */
    uint32 rate;          /* Calculated egress rate.          */ 
    ...
    /* Burst rate is always unlimited. */ 
    *kbits_burst = 0; 

    /* Read port egress shaper register. */
    BCM_IF_ERROR_RETURN(
        READ_SHAPERMAXBWPORTr(unit, port, &regval));
    rate = soc_reg_field_get(unit, SHAPERMAXBWPORTr, regval, MAX_BWf); 
    if (!rate) {
        *kbits_sec = 0;
    }
    else {
        /* Convert the MAX_BW field to kbits/sec (1000 bits/sec). */
        *kbits_sec = 64 * rate;
    }
    return (BCM_E_NONE);
}
...
{
#ifdef BCM_EASYRIDER_SUPPORT
    uint32 rval;                   /* ER EGR_PORT regiester value */  
#endif /* BCM_EASYRIDER_SUPPORT */
    ...
    if (SOC_IS_EASYRIDER(unit)) {
        /* Get NNI bit from EGR_PORTr */
        SOC_IF_ERROR_RETURN(READ_EGR_PORTr(unit, port, &rval));
        port_cfg->pc_nni_port = soc_reg_field_get(unit, EGR_PORTr, rval, NNIf);
    }
#endif /* BCM_EASYRIDER_SUPPORT */
...
#ifdef BCM_EASYRIDER_SUPPORT 
    if (SOC_IS_EASYRIDER(unit)) {
        BCM_IF_ERROR_RETURN(_bcm_er_port_nni_field_set(unit, port, port_cfg));
    }
#endif /* BCM_EASYRIDER_SUPPORT */
...
#ifdef BCM_EASYRIDER_SUPPORT
    if (SOC_IS_EASYRIDER(unit)) {
        return _bcm_er_port_cfg_init(unit, port, vlan_data);
    }
#endif /* BCM_EASYRIDER_SUPPORT */
    return (BCM_E_NONE);
}
