/* $Id: field.c 1.496.2.14 Broadcom SDK $
                        ...
                        bcm_policer_t pid)
{
    ...
    int rv, idx, policer_id, found, mem_idx, meter_offset;
    uint32 ent[SOC_MAX_MEM_FIELD_WORDS], ent_pl_flags = 0;
    uint32 refresh_count, bucket_count, bucket_size;
    ...
    /* Search if policer has already been detected */
    /* Go over the policers in the slice */
    if (1 == _FP_METER_BMP_TEST(fs->meter_bmp, meter_conf->meter_idx)) {
        ...
        switch (meter_conf->meter_mode) {
            ...
            case BCM_FIELD_METER_MODE_FLOW: /* 1 */
                ...
                if (f_pl->cfg.mode == bcmPolicerModePeak) {
                    f_pl->cfg.pkbits_sec = refresh_count * 
                ...
                if (f_pl->cfg.mode == bcmPolicerModeCommitted) {
                    f_pl->cfg.ckbits_sec = refresh_count * 
                                           _FP_POLICER_REFRESH_RATE; 
#ifdef BCM_FIREBOLT2_SUPPORT
                    if (SOC_IS_FIREBOLT2(unit)) {
            ...
            for (idx = 0; idx < _bcmFieldQualifyCount; idx++) {
                ...
                for (j = 0; j < f_qual_arr->conf_sz; j++) {
                            ...
                            if ((fg_ptr->sel_codes[i].fpf1 != 3) && 
                                ...
                                if ((idx == bcmFieldQualifyInnerVlan) || 
                                    ...
                                    if (fg_ptr->sel_codes[i].inner_vlan_overlay
                                     ...
                                     (idx == bcmFieldQualifyLookupClass0) ||
                                     (idx == bcmFieldQualifySrcMacGroup)) {
                                    if (fg_ptr->sel_codes[i].inner_vlan_overlay
                                        == 1) {
                                        selector_match = 1; 
                                    }
                    ...
                    if (selector_match) {
                        ...
                        for (k = 0; k < fg_ptr->qual_arr[i].size; k++) {
                            if (fg_ptr->qual_arr[i].qid_arr[k] == idx) {
                                break; /* Qualifier already added */
                            }
                        }
                        if (k == fg_ptr->qual_arr[i].size) {
                            size = fg_ptr->qual_arr[i].size + 1;
                            qid_arr = NULL;
                            ...
                            if (size) {
                                sal_memcpy(qid_arr, fg_ptr->qual_arr[i].qid_arr,
                                           fg_ptr->qual_arr[i].size * 
                                           sizeof (uint16));
                                sal_memcpy(q_offset_arr, 
                                           fg_ptr->qual_arr[i].offset_arr, 
                                           fg_ptr->qual_arr[i].size * 
                                           sizeof (_bcm_field_qual_offset_t));
                                if (fg_ptr->qual_arr[i].size) {
                                    sal_free(fg_ptr->qual_arr[i].qid_arr);
                                    sal_free(fg_ptr->qual_arr[i].offset_arr);
                                }
                            }
                            fg_ptr->qual_arr[i].qid_arr = qid_arr;
                            fg_ptr->qual_arr[i].offset_arr = q_offset_arr;
                            fg_ptr->qual_arr[i].qid_arr[size - 1] = idx;
                            fg_ptr->qual_arr[i].offset_arr[size - 1].field = 
                                f_qual_arr->conf_arr[j].offset.field;
                            fg_ptr->qual_arr[i].offset_arr[size - 1].offset = 
                                f_qual_arr->conf_arr[j].offset.offset;
                            fg_ptr->qual_arr[i].offset_arr[size - 1].width = 
                                f_qual_arr->conf_arr[j].offset.width;
                            fg_ptr->qual_arr[i].offset_arr[size - 1].offset1 = 
                                f_qual_arr->conf_arr[j].offset.offset1;
                            fg_ptr->qual_arr[i].offset_arr[size - 1].width1 = 
                                f_qual_arr->conf_arr[j].offset.width1;
                            fg_ptr->qual_arr[i].offset_arr[size - 1].offset2 = 
                                f_qual_arr->conf_arr[j].offset.offset2;
                            fg_ptr->qual_arr[i].offset_arr[size - 1].width2 = 
                                f_qual_arr->conf_arr[j].offset.width2;
                            fg_ptr->qual_arr[i].size = size;
        ...
        for (idx = 0; idx < fs->entry_count / ratio; idx++) {
            ...
            for (i = 0; i < parts_count; i++) {
                ...
                /* Iterate over the HW qualifiers and match offsets with
                   the non-zero masks */
                for (j = 0; j < fg->qual_arr[i].size; j++) {
                    qual_idx = fg->qual_arr[i].qid_arr[j];
                    ...
                    for (k = 0; k < f_qual_arr->conf_sz; k++) {
                        if (_mask_is_set(unit, &f_qual_arr->conf_arr[k].offset,
                            ...
                            if (qualified) {
                                fg->qual_arr[i].offset_arr[j].field = 
                                    f_qual_arr->conf_arr[k].offset.field;
                                fg->qual_arr[i].offset_arr[j].offset = 
                                    f_qual_arr->conf_arr[k].offset.offset;
                                fg->qual_arr[i].offset_arr[j].width = 
                                    f_qual_arr->conf_arr[k].offset.width;
                                fg->qual_arr[i].offset_arr[j].offset1 = 
                                    f_qual_arr->conf_arr[k].offset.offset1;
                                fg->qual_arr[i].offset_arr[j].width1 = 
                                    f_qual_arr->conf_arr[k].offset.width1;
                                fg->qual_arr[i].offset_arr[j].offset2 = 
                                    f_qual_arr->conf_arr[k].offset.offset2;
                                fg->qual_arr[i].offset_arr[j].width2 = 
                               ...
                               _field_stage_t *stage_fc)
{
    ...
    for (slice_idx = 0; slice_idx < stage_fc->tcam_slices; slice_idx++) {
        ...
        if (efp_slice_mode != 3) { /* Invalid value */
            ...
            BCM_FIELD_QSET_ADD(fg->qset, bcmFieldQualifyPacketFormat);
            for (i = 0; i < parts_count; i++) {
                for (j = 0; j < list_size[i]; j++) {
                    ...
                    if (size) {
                        sal_memcpy(qid_arr, fg->qual_arr[i].qid_arr,
                                   fg->qual_arr[i].size * 
                                   sizeof (uint16));
                        sal_memcpy(q_offset_arr, 
                                   fg->qual_arr[i].offset_arr, 
                                   fg->qual_arr[i].size * 
                                   sizeof (_bcm_field_qual_offset_t));
                        if (fg->qual_arr[i].size) {
                            sal_free(fg->qual_arr[i].qid_arr);
                            sal_free(fg->qual_arr[i].offset_arr);
                        }
                    }
                    fg->qual_arr[i].qid_arr = qid_arr;
                    fg->qual_arr[i].offset_arr = q_offset_arr;
                    fg->qual_arr[i].qid_arr[size - 1] = list_arr[i][j];
                    fg->qual_arr[i].offset_arr[size - 1].field = 
                        f_qual_arr->conf_arr[k].offset.field;
                    fg->qual_arr[i].offset_arr[size - 1].offset = 
                        f_qual_arr->conf_arr[k].offset.offset;
                    fg->qual_arr[i].offset_arr[size - 1].width = 
                        f_qual_arr->conf_arr[k].offset.width;
                    fg->qual_arr[i].offset_arr[size - 1].offset1 = 
                        f_qual_arr->conf_arr[k].offset.offset1;
                    fg->qual_arr[i].offset_arr[size - 1].width1 = 
                        f_qual_arr->conf_arr[k].offset.width1;
                    fg->qual_arr[i].offset_arr[size - 1].offset2 = 
                        f_qual_arr->conf_arr[k].offset.offset2;
                    fg->qual_arr[i].offset_arr[size - 1].width2 = 
                        f_qual_arr->conf_arr[k].offset.width2;
                    fg->qual_arr[i].size = size;
                }
            }
            /* Associate slice(s) to group */
                         ...
                         int *tcam_idx_old, int *tcam_idx_new)
{
    ...
    fp_gm_fields_x_entry_t fp_gm_fields_x[_FP_MAX_ENTRY_WIDTH];
    fp_gm_fields_y_entry_t fp_gm_fields_y[_FP_MAX_ENTRY_WIDTH];
    ...
    for (idx = 0; idx < parts_count; idx++) {
        ...
        if (SOC_IS_TD_TT(unit) && (stage_id == _BCM_FIELD_STAGE_INGRESS)) {
            if ((fg->flags & _FP_GROUP_SPAN_DOUBLE_SLICE)
                && (0x1 & idx)) {
                rv = READ_FP_GM_FIELDS_Xm(unit, MEM_BLOCK_ANY,
                        tcam_idx_old[idx], &fp_gm_fields_x[idx]);
                BCM_IF_ERROR_RETURN(rv);
                rv = READ_FP_GM_FIELDS_Ym(unit, MEM_BLOCK_ANY,
                        tcam_idx_old[idx], &fp_gm_fields_y[idx]);
                BCM_IF_ERROR_RETURN(rv);
            } else {
    ...
    for (idx = parts_count - 1; idx >= 0; idx--) {
        ...
        if ((SOC_IS_TD_TT(unit)) && (stage_id == _BCM_FIELD_STAGE_INGRESS)) {
            if ((fg->flags & _FP_GROUP_SPAN_DOUBLE_SLICE)
                && (0x1 & idx)) {
                rv = WRITE_FP_GM_FIELDS_Xm(unit, MEM_BLOCK_ALL,
                        tcam_idx_new[idx], &fp_gm_fields_x[idx]);
                BCM_IF_ERROR_RETURN(rv);
                rv = WRITE_FP_GM_FIELDS_Ym(unit, MEM_BLOCK_ALL,
                        tcam_idx_new[idx], &fp_gm_fields_y[idx]);
                BCM_IF_ERROR_RETURN(rv);
            } else {
                rv = WRITE_FP_GLOBAL_MASK_TCAM_Xm(unit, MEM_BLOCK_ALL,
                        tcam_idx_new[idx], &fp_global_mask_x[idx]);
                BCM_IF_ERROR_RETURN(rv);
                rv = WRITE_FP_GLOBAL_MASK_TCAM_Ym(unit, MEM_BLOCK_ALL,
                        tcam_idx_new[idx], &fp_global_mask_y[idx]);
                BCM_IF_ERROR_RETURN(rv);
            }
        } else
#endif
                      ...
                      int idx, uint64 *packet_count, uint64 *byte_count)
{
    ...
    if (NULL != mem_x_buf) {
        ...
        rv = _field_br_counter_read(unit, memacc_x, mem_x_buf,
                                   cntrs64_buf, &value);
    }
    ...
    if (NULL != mem_y_buf) {
        ...
        rv = _field_br_counter_read(unit, memacc_y, mem_y_buf,
                                   cntrs64_buf, &value);
    }
                            ...
                            bcm_field_data_packet_format_t *pkt_fmt)
{
    ...
    switch (ip_idx) {
      ...
      case 0xc:
          ...
          /* SOC_IS_TR_VL: BCM_FIELD_USER_IP_NOTUSED */
          break;
      case 0xd:
          if (SOC_IS_SC_CQ(unit)) {
    ...
    for (idx = 0; idx <= _FP_DATA_DATA1_WORD_MAX; idx++) {
        ...
        if(SOC_IS_ENDURO(unit) || SOC_IS_HURRICANE(unit)) {
            if ((0 == idx) && !(f_dq->hw_bmap & (1 << 0x1)) &&
                              !(f_dq->hw_bmap & (1 << 0x3))) {
                offset_idx = _FP_DATA_DATA0_WORD_MAX;
            } else if((2 == idx) && !(f_dq->hw_bmap & (1 << 0x1)) &&
                                    !(f_dq->hw_bmap & (1 << 0x3))) {
                offset_idx = _FP_DATA_DATA0_WORD_MAX - idx;
            } else {
                offset_idx = idx;
            }
        } else
...
_field_fb_functions_init(int unit, _field_funct_t *functions)
{
    ...
    functions->fp_udf_delete           = _bcm_field_fb_udf_write;
