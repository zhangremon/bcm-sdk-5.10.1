/*
 * $Id: vlan.c 1.177.2.17 Broadcom SDK $
...
_bcm_fb2_outer_tpid_entry_add(int unit, uint16 tpid, int *index)
{
    ...
    if (free_index >= 0) {
        ...
        if (SOC_IS_SC_CQ(unit)) {
            BCM_IF_ERROR_RETURN(
                _bcm_trx_egr_src_port_outer_tpid_set(unit, free_index, TRUE));
        }
#endif /* BCM_TRX_SUPPORT */
...
_vlan_profile_write(int unit, int index, _vlan_profile_t *entry)
{
    ...
    /* Drop non-unicast, broadcast or multicast packets that miss L2 lookup. */     if (SOC_MEM_FIELD_VALID(unit, mem, L2_NON_UCAST_DROPf)) {
...
_bcm_xgs3_vlan_table_init(int unit, bcm_vlan_data_t *vd, int table)
{
    ...
    if (SOC_IS_EASYRIDER(unit)) {
        soc_mem_field32_set(unit, table, &ve, LEARNf, 1);
    }
 ...
 || defined(BCM_RAVEN_SUPPORT)
    ...
    if (SOC_MEM_IS_VALID(unit, VLAN_MPLSm) && !SOC_IS_RCPU_ONLY(unit)) {
...
_bcm_xgs3_vlan_table_create(int unit, bcm_vlan_t vid, int table)
{
...
#if defined(BCM_EASYRIDER_SUPPORT)
    if (SOC_IS_EASYRIDER(unit)) {
        soc_mem_field32_set(unit, table, &vt, LEARNf, 1);
    }
#endif /* BCM_EASYRIDER_SUPPORT */
    ...
    if (SOC_MEM_FIELD_VALID(unit, table, VRF_IDf)) {
...
{
    if (SOC_IS_HAWKEYE(unit) && !SOC_IS_RCPU_ONLY(unit)) {
        uint32 rval;
...
bcm_xgs3_vlan_create(int unit, bcm_vlan_t vid)
{
   ...
   int l3IntfMapMode=0;
    ...
    if (SOC_MEM_IS_VALID(unit, VLAN_MPLSm)) {
        ...
        if (l3IntfMapMode) {
            /* Disable 1:1 mapping */
            soc_mem_field32_set(unit, VLAN_MPLSm, &vlan_mpls, L3_IIFf, BCM_L3_INGRESS_INTERFACE_INVALID);
        } else {
 ...
 *     cosq  - (IN) CosQ value.
 *     flags - (IN) Enable/disable. 
 * Return :
 *    BCM_E_XXX
 */
STATIC int
_vlan_cosq_map_set(int unit, bcm_vlan_t vid, bcm_cos_queue_t cosq, uint32 flags)
{
    ...
    soc_mem_field32_set(unit, VLAN_COS_MAPm, &vcm, VALIDf, 
                        ((flags & BCM_VLAN_COSQ_ENABLE) ? 1 : 0));

    if (flags & BCM_VLAN_COSQ_ENABLE) {
 ...
 *     vid   - (IN) Vlan table entry buffer.
 *     cosq  - (OUT) CosQ value.
 *     flags - (IN/OUT) Valid cosq found.
 * Return :
 *    BCM_E_XXX
 */
STATIC int
_vlan_cosq_map_get(int unit, bcm_vlan_t vid, bcm_cos_queue_t *cosq, uint32 *flags) 
{
    vlan_cos_map_entry_t vcm;
    int rv;
    ...
    *flags |= BCM_VLAN_COSQ_ENABLE;
    /* Subtract 8 since VLAN COS queues are from 8..23 */
    *cosq = soc_mem_field32_get(unit, VLAN_COS_MAPm, &vcm, VLAN_COSf) - 8;
    return (BCM_E_NONE);
}
#endif /* BCM_TRIUMPH_SUPPORT */
 ...
 || defined(BCM_RAVEN_SUPPORT)
    if (soc_feature(unit, soc_feature_vlan_ctrl)) {
        ...
        if (SOC_MEM_FIELD_VALID(unit, VLAN_TABm, VRF_IDf)) {              
           control->vrf = soc_mem_field32_get(unit, VLAN_TABm, &vt, VRF_IDf);
                    ...
                    && (!ingress_map_mode) && !SOC_IS_HURRICANE(unit)) {
        ...
        if (((SOC_IS_TRIDENT(unit)) || (SOC_IS_KATANA(unit))) && 
            ...
            if (SOC_MEM_FIELD_VALID(unit, L3_IIFm, URPF_MODEf)) {
                 rv = _bcm_td_l3_intf_urpf_mode_get(unit, vid, &control->urpf_mode);
                 if (BCM_FAILURE(rv)) {
                    soc_mem_unlock(unit, VLAN_TABm);
                    return rv; 
                 }
            ...
            if (SOC_MEM_FIELD_VALID(unit, L3_IIFm, URPF_DEFAULTROUTECHECKf)) {
               rv =  _bcm_td_l3_intf_urpf_default_route_get(unit, vid, &enable);
               if (BCM_FAILURE(rv)) {
                   soc_mem_unlock(unit, VLAN_TABm);
                   return rv;
               }
    ...
    if (soc_feature(unit, soc_feature_l3) && (ingress_map_mode)) {
        ...
        if (SOC_MEM_IS_VALID(unit, VLAN_COS_MAPm)) {
            rv = _vlan_cosq_map_get(unit, vid, &control->cosq, 
                                    &control->flags);
        }
#endif
                                ...
                                bcm_vlan_control_vlan_t *control)
{
    ...
    int                disable_mask;
#if defined(BCM_TRIDENT_SUPPORT) && defined(INCLUDE_L3)
    bcm_multicast_t   trill_l3mc_group=0;
...
#if defined(BCM_TRIUMPH_SUPPORT)
    if (SOC_MEM_IS_VALID(unit, VLAN_COS_MAPm)) {
        rv = _vlan_cosq_map_set(unit, vid, control->cosq,
                                control->flags);
        BCM_IF_ERROR_RETURN(rv);
    }
    ...
    if (soc_feature(unit, soc_feature_l3) && (!ingress_map_mode)) {
        if (!(control->flags & BCM_VLAN_L2_CLASS_ID_ONLY )) {
           if ((SOC_MEM_FIELD_VALID(unit, VLAN_TABm, L3_IIFf) || 
               ...
               rv =  _bcm_tr_l3_intf_class_set(unit, vid, control->if_class);
               BCM_IF_ERROR_RETURN(rv);
           }
        }
        if (SOC_MEM_FIELD_VALID(unit, L3_IIFm, ALLOW_GLOBAL_ROUTEf)) {
    ...
    if (soc_feature(unit, soc_feature_l3) && (!ingress_map_mode)) {
         if (SOC_IS_TRIDENT(unit) || SOC_IS_KATANA(unit)) {
              if (SOC_MEM_FIELD_VALID(unit, L3_IIFm, URPF_DEFAULTROUTECHECKf)) {
                   rv =  _bcm_td_l3_intf_urpf_default_route_set(unit, vid,
                        (0 == (control->flags & BCM_VLAN_L3_URPF_DEFAULT_ROUTE_CHECK_DISABLE)));
                   BCM_IF_ERROR_RETURN(rv);
              }
         }
         if (SOC_MEM_FIELD_VALID(unit, L3_IIFm, URPF_MODEf)) {
              rv =  _bcm_td_l3_intf_urpf_mode_set(unit, vid, control->urpf_mode);
              BCM_IF_ERROR_RETURN(rv);
         }
    }
#endif /* BCM_TRIDENT_SUPPORT && INCLUDE_L3 */
        ...
        if (SOC_MEM_FIELD_VALID(unit, VLAN_TABm, 
         ...
         if ( rv == BCM_E_NONE ) {
              if (BCM_SUCCESS(bcm_td_trill_multicast_check(unit, control->trill_nonunicast_group))) {
                   ...
                   if(BCM_SUCCESS(bcm_td_trill_multicast_group_get(unit, control->trill_nonunicast_group,
                              &trill_l3mc_group))) {
                        soc_mem_field32_set(unit, VLAN_TABm, &vt, 
                                TRILL_DOMAIN_NONUC_REPL_INDEXf,
                                _BCM_MULTICAST_ID_GET(trill_l3mc_group));
                   }
                   if (SOC_MEM_FIELD_VALID(unit, VLAN_TABm, 
