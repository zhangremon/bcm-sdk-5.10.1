/*
 * $Id: mcast.c 1.55.6.3 Broadcom SDK $
...
int bcm_draco_l2_addr_delete_mcast(int unit, uint32 flags);
...
STATIC int
_bcm_er_mcast_delete_callback(int unit, uint32 *l2_entry, int index,
                              soc_mem_t tmem, void *data)
{
    ...
    soc_mem_mac_addr_get(unit, tmem, l2_entry, MAC_ADDRf, mac);
    if (!BCM_MAC_IS_MCAST(mac)) {
        return BCM_E_NONE;
    }
    SOC_IF_ERROR_RETURN
        (soc_mem_delete(unit, tmem, MEM_BLOCK_ALL, l2_entry));

    return BCM_E_NONE;
}

STATIC int
_bcm_er_l2_addr_delete_mcast(int unit)
{
    if (SAL_BOOT_SIMULATION && !SAL_BOOT_BCMSIM) {
	return BCM_E_NONE;
    }

    return soc_er_l2_traverse(unit, _bcm_er_mcast_delete_callback, NULL);
}
...
bcm_xgs3_mcast_init(int unit)
{
    ...
    if (SOC_IS_EASYRIDER(unit)) {
        ...
        L2MC_L2MEM(unit) = L2_ENTRY_INTERNALm;
        L2MC_MCMEM(unit) = L2MC_TABLEm;
#endif /* BCM_EASYRIDER_SUPPORT */
    } else {
    ...
    if (!SAL_BOOT_BCMSIM && !SAL_BOOT_QUICKTURN && !SOC_IS_RCPU_ONLY(unit)) {
        SOC_IF_ERROR_RETURN
            (soc_mem_clear(unit, L2MC_MCMEM(unit), MEM_BLOCK_ALL, FALSE));
    }

    /* Delete all multicast entries from L2 */
    if (!SAL_BOOT_QUICKTURN && !SOC_IS_RCPU_ONLY(unit)) {
        if (SOC_IS_EASYRIDER(unit)) {
#ifdef	BCM_EASYRIDER_SUPPORT
            BCM_IF_ERROR_RETURN
                (_bcm_er_l2_addr_delete_mcast(unit));
#endif
        }
