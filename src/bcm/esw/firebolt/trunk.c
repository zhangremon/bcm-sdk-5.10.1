/*
 * $Id: trunk.c 1.150.2.3 Broadcom SDK $
...
typedef struct {
    uint8         num_ports;
    uint8         psc[BCM_TRUNK_MAX_PORTCNT];
    uint16        modport[BCM_TRUNK_MAX_PORTCNT];
    uint32        flags[BCM_TRUNK_MAX_PORTCNT];
} _xgs3_hw_tinfo_t;
...
STATIC int _bcm_xgs3_trunk_swfailover_set(int unit,
                                          ...
                                          int rtag,
                                          int nports,
                                          int mods[BCM_TRUNK_MAX_PORTCNT],
                                          int ports[BCM_TRUNK_MAX_PORTCNT],
                                          uint32 member_flags[BCM_TRUNK_MAX_PORTCNT]);
...
_bcm_xgs3_trunk_mod_port_map_reinit(int unit) 
{
    ...
    bcm_module_t     om[BCM_TRUNK_MAX_PORTCNT]; /* new mod list */
    bcm_port_t       op[BCM_TRUNK_MAX_PORTCNT]; /* new port list */
    uint32           of[BCM_TRUNK_MAX_PORTCNT]; /* new flag list */
    int              i, found;
...
{
    bcm_port_t        port;
    int                localcount, localports[BCM_TRUNK_MAX_PORTCNT];
    int                index_count, max_ports = BCM_SWITCH_TRUNK_MAX_PORTCNT;

    localcount = 0;
#if defined(BCM_SCORPION_SUPPORT)
    if (SOC_IS_SC_CQ(unit)) {
        max_ports = BCM_TRUNK_MAX_PORTCNT;
    }
#endif
    ...
    if (SOC_IS_EASYRIDER(unit)) {
        pbmp_t                mask_ports;
        uint32                mask_reg, pword, opword;
        int                i, imax, rv;

        rv = BCM_E_NONE;
        imax = SOC_REG_NUMELS(unit, NONUCAST_TRUNK_BLOCK_MASKr);
        index_count = 0;
        for (i = 0; i < imax; i++) {
            rv = READ_NONUCAST_TRUNK_BLOCK_MASKr(unit, i, &mask_reg);
            if (rv < 0) {
                break;
            }
            pword = soc_reg_field_get(unit, NONUCAST_TRUNK_BLOCK_MASKr,
                                      mask_reg, BLOCK_MASKf);
            opword = pword;
            SOC_PBMP_CLEAR(mask_ports);
            SOC_PBMP_WORD_SET(mask_ports, 0, pword);
            SOC_PBMP_REMOVE(mask_ports, old_ports);
            SOC_PBMP_OR(mask_ports, new_ports);
            if (localcount > 0) {
                if (my_modid_bmap & (1 << (i % member_count)))  {
                    SOC_PBMP_PORT_REMOVE(mask_ports,
                                         localports[index_count % localcount]);
                    index_count++;
                }
            }
            pword = SOC_PBMP_WORD_GET(mask_ports, 0);
            if (pword == opword) {        /* no change */
                continue;
            }
            soc_reg_field_set(unit, NONUCAST_TRUNK_BLOCK_MASKr,
                              &mask_reg, BLOCK_MASKf, pword);
            rv = WRITE_NONUCAST_TRUNK_BLOCK_MASKr(unit, i, mask_reg);
            if (rv < 0) {
                break;
            }
        }
                               ...
                               bcm_port_t *ftp, bcm_module_t *ftm)
{
    ...
    uint16 fail_modport;
    bcm_gport_t gport;
    bcm_port_t local_port, port_list[BCM_TRUNK_MAX_PORTCNT];
    bcm_module_t modid_list[BCM_TRUNK_MAX_PORTCNT];
                             ...
                             bcm_trunk_add_info_t *add_info)
{
    ...
    _xgs3_hw_tinfo_t           *hwft;
    int num_hwft_ports;
    bcm_port_t   port_array[BCM_TRUNK_MAX_PORTCNT];
    bcm_module_t mod_array[BCM_TRUNK_MAX_PORTCNT];
...
{
    int                i, local_id;
    int port[BCM_TRUNK_MAX_PORTCNT];
    ...
    if (SOC_IS_HBX(unit) || SOC_IS_TRX(unit)) {
        ...
        if(!SOC_IS_HURRICANE(unit)) { /* PR 34459 */
            soc_reg64_field32_set(unit, HG_TRUNK_GROUPr, &val64,
                                  HIGIG_TRUNK_SIZEf, add_info->num_ports-1);
        }
        soc_reg64_field32_set(unit, HG_TRUNK_GROUPr, &val64,
...
#ifdef BCM_TRX_SUPPORT
        if (SOC_IS_TRX(unit)) {
            int vlan_range_idx, old_vlan_range_idx;
            int i;

            /* Get trunk group's VLAN_RANGE_IDX */
            vlan_range_idx = soc_mem_field32_get(unit, SOURCE_TRUNK_MAP_TABLEm,
                   ...
                   trunk_private_t *t_info)
{
    ...
    int                         hwmod[BCM_TRUNK_MAX_PORTCNT];
    int                         egr_hwmod[BCM_TRUNK_MAX_PORTCNT] = { 0 };
    int                         uc_egr_hwmod[BCM_TRUNK_MAX_PORTCNT] = { 0 };
    int                         hwport[BCM_TRUNK_MAX_PORTCNT];
    int                         egr_hwport[BCM_TRUNK_MAX_PORTCNT] = { 0 };
    int                         uc_egr_hwport[BCM_TRUNK_MAX_PORTCNT] = { 0 };
    uint32                      egr_member_flags[BCM_TRUNK_MAX_PORTCNT] = { 0 };
    ...
    /*
     * dlf/mc/ipmc indices are not used in XGS3 devices.
     * non-unicast traffic is spread over multiple ports through hashing
     * (see _bcm_xgs3_trunk_block_mask)
     * if dlf_index == mc_index == ipmc_index is unspecified then determine
     * the set of load balance eligible ports.
     */
    if ((add_info->dlf_index != add_info->mc_index) ||
        (add_info->dlf_index != add_info->ipmc_index) ||
        (add_info->mc_index != add_info->ipmc_index) ||
        (add_info->dlf_index >= add_info->num_ports)) {
        return BCM_E_PARAM;
    }
    ...
    t_info->ipmc_index_used = add_info->dlf_index;
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_xgs3_trunk_psc_to_rtag(unit, add_info->psc, &(t_info->rtag)));
                           ...
                           trunk_private_t *t_info)
{
    if ((SOC_IS_FB_FX_HX(unit) || SOC_IS_RAVEN(unit)) &&
        ...
        t_data->num_ports = 0;
        PBMP_ITER(pbmp, port) {
            t_data->tm[t_data->num_ports] = modid;
    ...
    if (SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit) || 
        SOC_IS_VALKYRIE2(unit)) {
        uint64        val64;
        int        i, port_limit, port;

        COMPILER_64_ZERO(val64);
        SOC_IF_ERROR_RETURN
            (READ_HG_TRUNK_GROUPr(unit, hgtid, &val64));
        i = soc_reg64_field32_get(unit, HG_TRUNK_GROUPr, val64,
                                  HIGIG_TRUNK_SIZEf);

        t_data->num_ports = i + 1;
        port_limit = t_data->num_ports;
        for (i = 0; i < port_limit; i++) {
            port = soc_reg64_field32_get(unit, HG_TRUNK_GROUPr, val64,
                _hg_trunk[i]);

            BCM_IF_ERROR_RETURN(_bcm_esw_stk_modmap_map(unit,
                BCM_STK_MODMAP_GET, 0, port, &t_data->tm[i], &t_data->tp[i]));
        }
        return BCM_E_NONE;
    }
#endif /* BCM_TRIUMPH2_SUPPORT */
    ...
    if (SOC_IS_HBX(unit) || SOC_IS_TRX(unit)) {
        ...
        for (i = 0; i < port_limit; i++) {
            t_data->tm[i] = 0;
            t_data->tp[i] = soc_reg64_field32_get(unit, HG_TRUNK_GROUPr,
                                                  val64, _hg_trunk[i]);
        }
#if defined(BCM_SCORPION_SUPPORT)
        if (SOC_IS_SC_CQ(unit) &&
            (t_data->num_ports > BCM_SWITCH_TRUNK_MAX_PORTCNT)) {
            SOC_IF_ERROR_RETURN
                (READ_HG_TRUNK_GROUP_HIr(unit, hgtid, &val64));
            for (i = BCM_SWITCH_TRUNK_MAX_PORTCNT;
                 i < t_data->num_ports; i++) {
                t_data->tm[i] = 0;
                t_data->tp[i] =
                    soc_reg64_field32_get(unit, HG_TRUNK_GROUP_HIr, val64,
                         _hg_trunk_hi[i - BCM_SWITCH_TRUNK_MAX_PORTCNT]);
            }
        }
#endif
                                   ...
                                   bcm_pbmp_t ports_status)
{
    ...
    PBMP_ITER(ports_active, port) {
        ...
        if (tid >= 1 && tid < _BCM_FP_TRUNK_NUM_MAX + 1) {
            ...
            uint16               uc_modport[BCM_TRUNK_MAX_PORTCNT];
            int                  max_ports = BCM_SWITCH_TRUNK_MAX_PORTCNT;
            uint32               my_modid_bmap = 0;
            uint32               nuc_member_count = 0;
            uint32               nuc_tpbm = 0;
...
_xgs3_trunk_reinit(int unit, int ngroups_fp, trunk_private_t *trunk_info)
{
    ...
    if (SOC_IS_FBX(unit) || SOC_IS_EASYRIDER(unit)) {
        ...
        trunk_private_t *t_info; 
        trunk_group_entry_t tg_entry;
        bcm_module_t egr_hw_mods[BCM_TRUNK_MAX_PORTCNT];
        bcm_port_t egr_hw_ports[BCM_TRUNK_MAX_PORTCNT];
        uint32 egr_member_flags[BCM_TRUNK_MAX_PORTCNT] = { 0 };
        bcm_module_t om[BCM_TRUNK_MAX_PORTCNT];
        pbmp_t op[BCM_TRUNK_MAX_PORTCNT];
        int max_ports = BCM_SWITCH_TRUNK_MAX_PORTCNT;
        source_trunk_map_table_entry_t  stm; /* Hw entry buffer. */
    ...
    for (tid = min_tid; tid <= max_tid; tid++, trunk_info++) {
        ...
        if (SOC_IS_FB_FX_HX(unit)) {
            PBMP_ITER(pbmp, port) {
                ...
                BCM_IF_ERROR_RETURN(
                                    soc_xgs3_port_to_higig_bitmap(unit, port, 
                                                                  (uint32 *)&(t_data->tp[t_data->num_ports])));
                BCM_IF_ERROR_RETURN(
                                    soc_xgs3_higig_bitmap_to_higig_port_num
                                    (t_data->tp[t_data->num_ports], 
                                     (uint32 *)&(t_data->tp[t_data->num_ports])));
                t_data->num_ports += 1;
            }
        } 
        ...
        for (i = 0; i < t_data->num_ports; i++) {
            MEMBER_INFO(unit, tid).modport[i] = t_data->tp[i];
        }
        MEMBER_INFO(unit, tid).num_ports = t_data->num_ports;
        ...
        if (SOC_IS_FB_FX_HX(unit)) {
            ...
            t_data->psc = soc_reg_field_get(unit, 
                                            HIGIG_TRUNK_GROUPr,
                                            hg_group_entry,
                                            (tid == min_tid) ?           \
                                            HIGIG_TRUNK_RTAG0f : \
            ...
            /* Humv / Bradley / Scorpion / Conqueror / Triumph / Valkyrie */
            t_data->psc = soc_reg64_field32_get(unit, 
                                                HG_TRUNK_GROUPr,
                                                hg_trunk_group,
                                                HIGIG_TRUNK_RTAGf);
        }
        ...
        if (soc_feature(unit, soc_feature_hg_trunk_failover)) {
            ...
            hwft->num_ports = t_data->num_ports;
            for (tix = 0; tix < t_data->num_ports; tix++) {
                hwft->modport[tix] = t_data->tp[tix];
