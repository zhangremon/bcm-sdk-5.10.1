/*
 * $Id: ipmc.c 1.162.2.7 Broadcom SDK $
...
/*
 * Function:
 *      _bcm_er_ipmc_l3entry_list_add
 * Purpose:
 *      Add a L3 entry to IPMC group's linked list of L3 entries
 * Parameters:
 *      unit       - (IN) BCM device number.
 *      ipmc_index - (IN) IPMC group ID.
 *      l3index    - (IN) L3 entry index.
 *      ip6        - (IN) Indicates whether L3 entry is an IPv6 entry.
 * Returns:
 *      BCM_E_XXX
 */
STATIC int
_bcm_er_ipmc_l3entry_list_add(int unit, int ipmc_index,
                              int l3index, int ip6)
{
    ...
    ipmc_l3entry->l3index = l3index;
    ipmc_l3entry->ip6 = ip6;
 ...
 */
STATIC int
_bcm_er_ipmc_l3entry_list_del(int unit, int ipmc_index,
                              int l3index, int ip6)
{
    _bcm_esw_ipmc_l3entry_t *ipmc_l3entry;
    ...
    while (ipmc_l3entry != NULL) {
        if ((ipmc_l3entry->l3index == l3index) &&
            (ipmc_l3entry->ip6 == ip6)) {
...
/*
 * Function:
 *      _bcm_er_ipmc_l3entry_list_get_ip6
 * Purpose:
 *      Get IPv6 indication for a given L3 entry index in IPMC group's linked list.
 * Parameters:
 *      unit       - (IN) BCM device number.
 *      ipmc_index - (IN) IPMC group ID.
 *      l3index    - (IN) L3 entry index.
 *      ip6        - (OUT) Indicates whether L3 entry is an IPv6 entry.
 * Returns:
 *      BCM_E_XXX
 */
STATIC int
_bcm_er_ipmc_l3entry_list_get_ip6(int unit, int ipmc_index,
                                  int l3index, int *ip6)
{
    _bcm_esw_ipmc_l3entry_t *ipmc_l3entry;
...
/*
 * Function:
 *      _bcm_er_ipmc_l3entry_list_get_l3index
 * Purpose:
 *      Get the first L3 entry index in IPMC group's linked list.
 * Parameters:
 *      unit       - (IN) BCM device number.
 *      ipmc_index - (IN) IPMC group ID.
 *      l3index    - (OUT) L3 entry index.
 * Returns:
 *      BCM_E_XXX
 */
STATIC int
_bcm_er_ipmc_l3entry_list_get_l3index(int unit, int ipmc_index,
                                      int *l3index)
{
    _bcm_esw_ipmc_l3entry_t *ipmc_l3entry;
...
/*
 * Function:
 *      _bcm_er_ipmc_l3entry_list_size_get
 * Purpose:
 ...
 */
STATIC int
_bcm_er_ipmc_l3entry_list_size_get(int unit, int ipmc_index,
        int *size)
{
    _bcm_esw_ipmc_l3entry_t *ipmc_l3entry;
...
/*
 * Function:
 *      _bcm_er_ipmc_get_by_index
 * Purpose:
 *      Get IPMC entry at index
 * Parameters:
 *      unit - SOC unit number.
 *      index - Index to IPMC table
 *      ipmc - (OUT) decoded entry
 * Returns:
 *      BCM_E_XXX
 */
STATIC int
_bcm_er_ipmc_get_by_index(int unit, int index, bcm_ipmc_addr_t *ipmc)
{
    int idx_min, idx_max, idx;
    int found = FALSE;

    if (ipmc->flags & BCM_IPMC_IP6) {
        ipmc_group_v6_entry_t ipmc_entry;

        idx_min = soc_mem_index_min(unit, IPMC_GROUP_V6m);
        idx_max = soc_mem_index_max(unit, IPMC_GROUP_V6m);
        for (idx = idx_min; idx <= idx_max; idx++) {
             sal_memset(&ipmc_entry, 0, sizeof(ipmc_group_v6_entry_t));
             BCM_IF_ERROR_RETURN(
                 soc_mem_read(unit, IPMC_GROUP_V6m,
                              MEM_BLOCK_ANY, idx, &ipmc_entry));
             ipmc->ipmc_index = soc_IPMC_GROUP_V6m_field32_get(unit,
                                               &ipmc_entry, L3_IPMC_INDEXf);
             if (ipmc->ipmc_index == index) {
                 found = TRUE;
                 break;
             }
        }
        if (found) {
            soc_mem_ip6_addr_get(unit, IPMC_GROUP_V6m, &ipmc_entry,
                        DIPf, ipmc->mc_ip6_addr, SOC_MEM_IP6_FULL_ADDR);
            ipmc->mc_ip6_addr[0] = 0xff;
            soc_mem_ip6_addr_get(unit, IPMC_GROUP_V6m, &ipmc_entry,
                    SIPf, ipmc->s_ip6_addr, SOC_MEM_IP6_FULL_ADDR);
            if (soc_IPMC_GROUP_V6m_field32_get(unit, &ipmc_entry, RPEf)) {
                ipmc->flags |= BCM_IPMC_SETPRI;
                ipmc->cos = soc_IPMC_GROUP_V6m_field32_get(unit,
                                                           &ipmc_entry, PRIf);
            } else {
                ipmc->flags &= ~BCM_IPMC_SETPRI;
                ipmc->cos = -1;
            ...
            ipmc->vid =
                  soc_IPMC_GROUP_V6m_field32_get(unit, &ipmc_entry, VLANf);
        } else {
            return SOC_E_NOT_FOUND;
        }
    } else {
        ipmc_group_v4_entry_t ipmc_entry;

        idx_min = soc_mem_index_min(unit, IPMC_GROUP_V4m);
        idx_max = soc_mem_index_max(unit, IPMC_GROUP_V4m);
        for (idx = idx_min; idx <= idx_max; idx++) {
             sal_memset(&ipmc_entry, 0, sizeof(ipmc_group_v4_entry_t));
             BCM_IF_ERROR_RETURN(
                 soc_mem_read(unit, IPMC_GROUP_V4m, MEM_BLOCK_ANY,
                              idx, &ipmc_entry));

             ipmc->ipmc_index = soc_IPMC_GROUP_V4m_field32_get(unit,
                                               &ipmc_entry, L3_IPMC_INDEXf);
             if (ipmc->ipmc_index == index) {
                 found = TRUE;
                 break;
        ...
        if (found) {
            ipmc->mc_ip_addr =
                  soc_IPMC_GROUP_V4m_field32_get(unit, &ipmc_entry, DIPf);
            ipmc->s_ip_addr =
                  soc_IPMC_GROUP_V4m_field32_get(unit, &ipmc_entry, SIPf);
            if (soc_IPMC_GROUP_V4m_field32_get(unit, &ipmc_entry, RPEf)) {
                ipmc->flags |= BCM_IPMC_SETPRI;
                ipmc->cos = soc_IPMC_GROUP_V4m_field32_get(unit,
                                                           &ipmc_entry, PRIf);
            } else {
                ipmc->flags &= ~BCM_IPMC_SETPRI;
                ipmc->cos = -1;
            ...
            ipmc->vid =
                  soc_IPMC_GROUP_V4m_field32_get(unit, &ipmc_entry, VLANf);
            ipmc->vrf =
                  soc_IPMC_GROUP_V4m_field32_get(unit, &ipmc_entry, VRFf);
        } else {
            return SOC_E_NOT_FOUND;
        }
    }
...
/*
 * Function:
 *      _bcm_er_ipmc_del_by_index
 * Purpose:
 *      Delete IPMC entry at index
 * Parameters:
 *      unit - SOC unit number.
 *      index - Index to IPMC table
 *      ipv6_flag  - 0 for IPV4; 1 for IPV6
 * Returns:
 *      BCM_E_XXX
 */
STATIC int
_bcm_er_ipmc_del_by_index(int unit, int index, int ipv6_flag)
{
    int rv, idx_min, idx_max, idx, ptr;
    int found = FALSE;
    ...
    if (ipv6_flag) {
        ...
        for (idx = idx_min; idx <= idx_max; idx++) {
             sal_memset(&ipmc_entry, 0, sizeof(ipmc_group_v6_entry_t));
             BCM_IF_ERROR_RETURN(
                 soc_mem_read(unit, IPMC_GROUP_V6m, MEM_BLOCK_ANY,
                              idx, &ipmc_entry));
             ptr = soc_IPMC_GROUP_V6m_field32_get(unit, &ipmc_entry,
                                                  L3_IPMC_INDEXf);
             if (ptr == index) {
                 found = TRUE;
                 break;
        ...
        if (found) {
            soc_mem_lock(unit, IPMC_GROUP_V6m);
            rv = soc_mem_delete(unit, IPMC_GROUP_V6m, COPYNO_ALL, &ipmc_entry);
            soc_mem_unlock(unit, IPMC_GROUP_V6m);
        } else {
                return SOC_E_NOT_FOUND;
        }
    } else {
        ...
        for (idx = idx_min; idx <= idx_max; idx++) {
             ...
             BCM_IF_ERROR_RETURN(
                 soc_mem_read(unit, IPMC_GROUP_V4m, MEM_BLOCK_ANY,
                              idx, &ipmc_entry));
             ptr = soc_IPMC_GROUP_V4m_field32_get(unit, &ipmc_entry,
                                                  L3_IPMC_INDEXf);
             if (ptr == index) {
                 found = TRUE;
                 break;
        ...
        if (found) {
            soc_mem_lock(unit, IPMC_GROUP_V4m);
            rv = soc_mem_delete(unit, IPMC_GROUP_V4m, COPYNO_ALL, &ipmc_entry);
            soc_mem_unlock(unit, IPMC_GROUP_V4m);
        } else {
            return SOC_E_NOT_FOUND;
        }
    }
 ...
 *      _bcm_er_ipmc_add
 * Purpose:
 *      Add an entry in IPMC table.
 * Parameters:
 *      unit - SOC unit number.
 *      ipmc - Pointer to memory for IPMC table related information.
 * Returns:
 *      BCM_E_XXX
 */
STATIC int
_bcm_er_ipmc_add(int unit, bcm_ipmc_addr_t *ipmc)
{
    int hw_index, rv = BCM_E_NONE;

    if (ipmc->flags & BCM_IPMC_IP6) {
        ipmc_group_v6_entry_t ipmc_key, ipmc_entry;

        if (!BCM_IP6_MULTICAST(ipmc->mc_ip6_addr)) {
            return BCM_E_PARAM;
        }

        sal_memset(&ipmc_key, 0, sizeof(ipmc_group_v6_entry_t));
        ipmc->mc_ip6_addr[0] = 0x0;
        soc_mem_ip6_addr_set(unit, IPMC_GROUP_V6m, &ipmc_key,
                DIPf, ipmc->mc_ip6_addr, SOC_MEM_IP6_FULL_ADDR);
        soc_mem_ip6_addr_set(unit, IPMC_GROUP_V6m, &ipmc_key,
                SIPf, ipmc->s_ip6_addr, SOC_MEM_IP6_FULL_ADDR);
        soc_IPMC_GROUP_V6m_field32_set(unit, &ipmc_key,
                                       VLANf, ipmc->vid);
        rv = soc_mem_search(unit, IPMC_GROUP_V6m, MEM_BLOCK_ANY,
                            &hw_index, &ipmc_key, &ipmc_entry, 0);
        ipmc->mc_ip6_addr[0] = 0xff;
        ...
        sal_memset(&ipmc_entry, 0, sizeof(ipmc_group_v6_entry_t));
        ipmc->mc_ip6_addr[0] = 0x0;
        soc_mem_ip6_addr_set(unit, IPMC_GROUP_V6m, &ipmc_entry,
                DIPf, ipmc->mc_ip6_addr, SOC_MEM_IP6_FULL_ADDR);
        soc_mem_ip6_addr_set(unit, IPMC_GROUP_V6m, &ipmc_entry,
                SIPf, ipmc->s_ip6_addr, SOC_MEM_IP6_FULL_ADDR);
        soc_IPMC_GROUP_V6m_field32_set(unit, &ipmc_entry,
                                       L3_IPMC_INDEXf, ipmc->ipmc_index);
        soc_IPMC_GROUP_V6m_field32_set(unit, &ipmc_entry,
                                       VLANf, ipmc->vid);
        if (ipmc->flags & BCM_IPMC_SETPRI) {
            soc_IPMC_GROUP_V6m_field32_set(unit, &ipmc_entry, RPEf, 1);
            soc_IPMC_GROUP_V6m_field32_set(unit, &ipmc_entry, 
                                           PRIf, ipmc->cos);
        }
        ipmc->mc_ip6_addr[0] = 0xff;
        ...
        soc_mem_lock(unit, IPMC_GROUP_V6m);
        rv = soc_mem_insert(unit, IPMC_GROUP_V6m, COPYNO_ALL, &ipmc_entry);
        soc_mem_unlock(unit, IPMC_GROUP_V6m);
    } else {
        ipmc_group_v4_entry_t ipmc_key, ipmc_entry;

        if (!BCM_IP4_MULTICAST(ipmc->mc_ip_addr)) {
            return BCM_E_PARAM;
        }

        sal_memset(&ipmc_key, 0, sizeof(ipmc_group_v4_entry_t));
        soc_IPMC_GROUP_V4m_field32_set(unit, &ipmc_key,
                                       DIPf, ipmc->mc_ip_addr);
        soc_IPMC_GROUP_V4m_field32_set(unit, &ipmc_key,
                                       SIPf, ipmc->s_ip_addr);
        soc_IPMC_GROUP_V4m_field32_set(unit, &ipmc_key,
                                       VLANf, ipmc->vid);
        soc_IPMC_GROUP_V4m_field32_set(unit, &ipmc_key,
                                       VRFf, ipmc->vrf);
        rv = soc_mem_search(unit, IPMC_GROUP_V4m,
                            MEM_BLOCK_ANY, &hw_index, &ipmc_key, &ipmc_entry, 0);
        if (rv < 0 && rv != SOC_E_NOT_FOUND) {
            return rv;
        } else if (rv == 0) {
            return BCM_E_EXISTS;
        ...
        sal_memset(&ipmc_entry, 0, sizeof(ipmc_group_v4_entry_t));
        soc_IPMC_GROUP_V4m_field32_set(unit, &ipmc_entry,
                                     DIPf, ipmc->mc_ip_addr);
        soc_IPMC_GROUP_V4m_field32_set(unit, &ipmc_entry,
                                     SIPf, ipmc->s_ip_addr);
        if (ipmc->flags & BCM_IPMC_SETPRI) {
            soc_IPMC_GROUP_V4m_field32_set(unit, &ipmc_entry, RPEf, 1);
            soc_IPMC_GROUP_V4m_field32_set(unit, &ipmc_entry,
                                           PRIf, ipmc->cos);
        } 
        soc_IPMC_GROUP_V4m_field32_set(unit, &ipmc_entry,
        ...
        soc_mem_lock(unit, IPMC_GROUP_V4m);
        rv = soc_mem_insert(unit, IPMC_GROUP_V4m, COPYNO_ALL, &ipmc_entry);
        soc_mem_unlock(unit, IPMC_GROUP_V4m);
    }

    if (BCM_SUCCESS(rv)) {
        rv = _bcm_er_ipmc_l3entry_list_add(unit, ipmc->ipmc_index, 
                                           ipmc->ipmc_index, 
 ...
 */
STATIC int
_bcm_er_ipmc_get(int unit, bcm_ipmc_addr_t *ipmc)
{
    int hw_index, rv = BCM_E_NONE;
    ...
    if (ipmc->flags & BCM_IPMC_IP6) {
        ...
        soc_IPMC_GROUP_V4m_field32_set(unit, &ipmc_key,
                                       VRFf, ipmc->vrf);
        rv = soc_mem_search(unit, IPMC_GROUP_V4m, MEM_BLOCK_ANY,
                            &hw_index, &ipmc_key, &ipmc_entry, 0);
        if (rv == SOC_E_NOT_FOUND) {
            return BCM_E_NOT_FOUND;
        } else if (rv < 0) {
 ...
 *      _bcm_er_ipmc_del
 * Purpose:
 *      Delete an entry in IPMC table.
 * Parameters:
 *      unit - SOC unit number.
 *      ipmc - Pointer to memory for IPMC table related information.
 *      modify_l3entry_list - Controls whether to modify IPMC group's linked
 *                            list of L3 entries.
 * Returns:
 *      BCM_E_XXX
 */
STATIC int
_bcm_er_ipmc_del(int unit, bcm_ipmc_addr_t *ipmc, int modify_l3entry_list)
{
    int rv, hw_index, ipmc_index;
    ...
    if (ipmc->flags & BCM_IPMC_IP6) {
        ...
        ipmc_index = soc_IPMC_GROUP_V6m_field32_get(unit,
                                          &ipmc_entry, L3_IPMC_INDEXf);

        soc_mem_lock(unit, IPMC_GROUP_V6m);
        rv = soc_mem_delete(unit, IPMC_GROUP_V6m, COPYNO_ALL, &ipmc_key);
        soc_mem_unlock(unit, IPMC_GROUP_V6m);
    } else {
        ipmc_group_v4_entry_t ipmc_key, ipmc_entry;
        ...
        ipmc_index = soc_IPMC_GROUP_V4m_field32_get(unit,
                                          &ipmc_entry, L3_IPMC_INDEXf);
 
        soc_mem_lock(unit, IPMC_GROUP_V4m);
        rv = soc_mem_delete(unit, IPMC_GROUP_V4m, COPYNO_ALL, &ipmc_key);
        soc_mem_unlock(unit, IPMC_GROUP_V4m);
    }

    if (BCM_SUCCESS(rv) && modify_l3entry_list) {
        rv = _bcm_er_ipmc_l3entry_list_del(unit, ipmc_index, 
                                           ipmc_index, 
                                           (ipmc->flags & BCM_IPMC_IP6) ? 1 : 0); 
    }
...
/*
 * Function:
 *      _bcm_er_ipmc_replace
 * Purpose:
 *      Replace an entry in IPMC table.
 * Parameters:
 *      unit - SOC unit number.
 *      ipmc - Pointer to memory for IPMC table related information.
 * Returns:
 ...
 */
STATIC int
_bcm_er_ipmc_replace(int unit, bcm_ipmc_addr_t *ipmc)
{
    int hw_index, rv = BCM_E_NONE;
    ...
    if (ipmc->flags & BCM_IPMC_IP6) {
        ...
        if (ipmc->flags & BCM_IPMC_SETPRI) { 
            soc_IPMC_GROUP_V6m_field32_set(unit, &ipmc_entry, RPEf, 1);
            soc_IPMC_GROUP_V6m_field32_set(unit, &ipmc_entry, PRIf, ipmc->cos);
        } else {
            soc_IPMC_GROUP_V6m_field32_set(unit, &ipmc_entry, RPEf, 0);
        }
        ...
        soc_l3x_lock(unit);
        rv = WRITE_IPMC_GROUP_V6m(unit, MEM_BLOCK_ALL,
                                  hw_index, &ipmc_entry);
        soc_l3x_unlock(unit);
        ...
        if (ipmc->flags & BCM_IPMC_SETPRI) { 
            soc_IPMC_GROUP_V4m_field32_set(unit, &ipmc_entry, RPEf, 1);
            soc_IPMC_GROUP_V4m_field32_set(unit, &ipmc_entry, PRIf, ipmc->cos);
        } else {
            soc_IPMC_GROUP_V4m_field32_set(unit, &ipmc_entry, RPEf, 0);
        }
        if (ipmc->flags & BCM_IPMC_USE_IPMC_INDEX) {
            soc_IPMC_GROUP_V4m_field32_set(unit, &ipmc_entry,
                                           L3_IPMC_INDEXf, ipmc->ipmc_index);
        ...
        soc_l3x_lock(unit);
        rv = WRITE_IPMC_GROUP_V4m(unit, MEM_BLOCK_ALL,
                                  hw_index, &ipmc_entry);
        soc_l3x_unlock(unit);
    }
                    ...
                    _bcm_esw_ipmc_l3entry_t *use_ipmc_l3entry)
{
    ...
    /* On ER_C0, it is possible to use the VALID bit in 
     * the L3_IPMC entry as a HIT bit. In this case, an invalid
     * entry is identified by empty L3/L2 PBMPs.
     */
    if (SOC_IS_EASYRIDER(unit) && IPMC_VALID_AS_HIT(unit)) {
        if (BCM_PBMP_IS_NULL(ipmc->l3_pbmp) &&
            BCM_PBMP_IS_NULL(ipmc->l2_pbmp)) {
            ipmc->v = 0;
        } else {
            ipmc->v = 1;
            if (soc_L3_IPMCm_field32_get(unit, entry, VALIDf)) {
                ipmc->flags |= BCM_IPMC_HIT;
                if (ipmc->flags & BCM_IPMC_HIT_CLEAR) {
                    soc_L3_IPMCm_field32_set(unit, entry, VALIDf, 0);
                    BCM_IF_ERROR_RETURN
                        (soc_mem_write(unit, L3_IPMCm, MEM_BLOCK_ALL, index, entry));
                }
            }
        }
    } else {
    ...
    if (ipmc->v) {
        if (SOC_IS_FBX(unit) && do_l3_lkup) {
            ...
            BCM_IF_ERROR_RETURN
                (_bcm_er_ipmc_l3entry_list_get_ip6(unit, hw_index, hw_index, &ipmc_ip6));
            if (ipmc_ip6) {
                ipmc->flags |= BCM_IPMC_IP6;
            } else {
                ipmc->flags &= ~BCM_IPMC_IP6;
            ...
            if (er_index_get) {
                rv = _bcm_er_ipmc_get_by_index(unit, hw_index, ipmc);
            }
            /* Else the info was already retreived before this
             * function so don't perform an expensive get_by_index. */
            if (rv < 0) {
                return (rv);
            }
#endif /* BCM_EASYRIDER_SUPPORT */
...
#define IPMC_MTU_MAX(_u_) \
    ((1 << soc_mem_field_length((_u_), L3_IPMCm, MIN_MTUf) ) - 1)
/*
 * Function:
 *      _bcm_er_ipmc_min_mtu_get
 * Purpose:
 *      Find the lowest MTU out of a set of ports.
 * Parameters:
 *      unit - BCM device number
 *      pbmp - ports to check MTU values
 *      min_mtu - (OUT) MTU of port with smallest MTU
 * Returns:
 *      BCM_E_NONE     - Success
 *      BCM_E_INTERNAL - Hardware read failure
 */
int
_bcm_er_ipmc_min_mtu_get(int unit, pbmp_t pbmp, uint32 *min_mtu)
{
    bcm_port_t          port;
    uint32              mtu_port;

    *min_mtu = IPMC_MTU_MAX(unit);

    PBMP_ITER(pbmp, port) {
        SOC_IF_ERROR_RETURN(READ_EGR_PORT_MTUr(unit, port, &mtu_port));
        if (*min_mtu > mtu_port) {
            *min_mtu = mtu_port;
        }
    }
...
#define BCM_ER_IPMC_BUCKET_SIZE (100)
/*
 * Function:
 *      bcm_er_ipmc_port_mtu_update
 * Parameters: 
 *      unit - (IN)SOC unit number.  
 *      port - (IN)Port with update mtu.  
 * Purpose:
 *      Update ipmc entries min mtu on port min mtu change.
 * Returns:  
 *      BCM_E_XXX 
 */
int
bcm_er_ipmc_port_mtu_update(int unit, bcm_port_t port)
{
    uint32 orig_min_mtu;      /* Original entry mtu value.    */
    uint32 new_min_mtu;       /* New calculated mtu value.    */
    ipmc_entry_t *ipmc_entry; /* IPMC entry address.          */
    pbmp_t l3_pbmp;           /* IPMC entry l3 ports bitmap.  */
    int alloc_size;           /* Allocation size.             */ 
    int todo_count;           /* IPMC unchecked indexes count.*/
    char *ipmc_tbl;           /* Buffer to read ipmc table.   */
    int start_dma_idx;        /* Last tbl read start index.   */
    int end_dma_idx;          /* Last tbl read end index.     */
    int idx;                  /* Iteration index.             */
    int rv = BCM_E_NONE;      /* Operation return status.     */

    /* Get ipmc table info. */
    todo_count = IPMC_GROUP_NUM(unit) - 1;
    start_dma_idx = soc_mem_index_min(unit, L3_IPMCm);

    /* Calculate table size. */
    alloc_size = BCM_ER_IPMC_BUCKET_SIZE * sizeof(ipmc_entry_t);

    /* Allocate memory buffer. */
    ipmc_tbl = soc_cm_salloc(unit, alloc_size, "IPMC_tbl_shadow");
    if (ipmc_tbl == NULL) {
        return (BCM_E_MEMORY);
    }
    ...
    while(todo_count > 0) {
        /* Skip unused entries. */
        while (!IPMC_USED_ISSET(unit, start_dma_idx) && (todo_count > 0)) {
            start_dma_idx++;
            todo_count--;
        }
        ...
        /* Calculate indexes range for dma. */
        end_dma_idx = start_dma_idx + BCM_ER_IPMC_BUCKET_SIZE;
        if(IPMC_GROUP_NUM(unit) < end_dma_idx) {
            end_dma_idx = IPMC_GROUP_NUM(unit);  
        }

        /* Read table to the buffer. */
        if (soc_mem_read_range(unit, L3_IPMCm, MEM_BLOCK_ANY,
                               start_dma_idx, 
                               end_dma_idx - 1,
                               ipmc_tbl) < 0) {
            soc_cm_sfree(unit, ipmc_tbl);
            IPMC_UNLOCK(unit);
            return (BCM_E_INTERNAL);
        }

        /* Loop over all the table & update min mtu if needed. */
        for (idx = 0; idx < (end_dma_idx - start_dma_idx); idx++) {

            /* If entry is not used continue. */
            if (!IPMC_USED_ISSET(unit, (start_dma_idx + idx))) {
                continue;
            }

            /* Get ipmc entry pointer. */
            ipmc_entry = soc_mem_table_idx_to_pointer(unit, L3_IPMCm, 
                                                      ipmc_entry_t *, 
                                                      ipmc_tbl, idx);

            /* Skip invalid entries. */
            if (!IPMC_VALID_AS_HIT(unit) &&
                !soc_L3_IPMCm_field32_get(unit, ipmc_entry, VALIDf)) { 
                continue;
            }
            ...
            /* If port is not a member continue to the next entry. */
            if (!PBMP_MEMBER(l3_pbmp, port)) {
                continue;
            }

            /* Get entry min mtu value. */
            orig_min_mtu = soc_L3_IPMCm_field32_get(unit, ipmc_entry, MIN_MTUf);

            /* Calculate new mtu value */ 
            rv = _bcm_er_ipmc_min_mtu_get(unit, l3_pbmp, &new_min_mtu);
            if (rv < 0) {
                break;
            }

            /* If value didn't change move on. */ 
            if (orig_min_mtu == new_min_mtu) {
                continue;
            }

            /* Set new min mtu value. */
            soc_L3_IPMCm_field32_set(unit, ipmc_entry, MIN_MTUf, new_min_mtu);

            /* Write entry to hw. */
            rv = soc_mem_write(unit, L3_IPMCm, MEM_BLOCK_ALL, 
                               (start_dma_idx + idx), ipmc_entry);
            if (rv < 0) {
                break;
            }
        }
        ...
        /* Decrement total number of entries to read. */
        todo_count -= (end_dma_idx - start_dma_idx);
        start_dma_idx = end_dma_idx;
    }
    IPMC_UNLOCK(unit);
    soc_cm_sfree(unit, ipmc_tbl);
    return (rv);
...
_xgs3_ipmc_write(int unit, int ipmc_id, bcm_ipmc_addr_t *ipmc)
{
    ...
    /* On Easyrider C0, it is possible to use the VALID
     * bit in the L3_IPMC entry as a HIT bit. In this case, an invalid
     * entry is identified by empty L3/L2 PBMPs.
     */
    if (IPMC_VALID_AS_HIT(unit)) {
        if (!ipmc->v) {
            SOC_PBMP_CLEAR(l2_pbmp);
            SOC_PBMP_CLEAR(l3_pbmp);
        } else {
            SOC_PBMP_ASSIGN(l2_pbmp, ipmc->l2_pbmp);
            SOC_PBMP_ASSIGN(l3_pbmp, ipmc->l3_pbmp);
        }
    } else {
    ...
    /* Verify l3_pbmp fits into L3_BITMAP field. */
    if (soc_mem_field_pbmp_fit
        (unit, L3_IPMCm, L3_BITMAPf, &SOC_PBMP_WORD_GET(l3_pbmp, 0))) {
        return BCM_E_PARAM;
    }
    soc_L3_IPMCm_field32_set(unit, &entry, L3_BITMAPf,
                             SOC_PBMP_WORD_GET(l3_pbmp, 0));
    soc_L3_IPMCm_field32_set(unit, &entry, L2_BITMAPf,
                             SOC_PBMP_WORD_GET(l2_pbmp, 0));
#ifdef BCM_BRADLEY_SUPPORT
    if (SOC_IS_HBX(unit)) {
        BCM_IF_ERROR_RETURN
            (_bcm_xgs3_ipmc_bitmap_set(unit, ipmc_id, l2_pbmp));
    }
#endif /* BCM_BRADLEY_SUPPORT */

#ifdef BCM_EASYRIDER_SUPPORT
    /* If needed calculate and set MIN_MTUf. */
    if (soc_mem_field_length(unit, L3_IPMCm, MIN_MTUf)) {
        uint32  min_mtu_l3;
        BCM_IF_ERROR_RETURN
            (_bcm_er_ipmc_min_mtu_get(unit, ipmc->l3_pbmp, &min_mtu_l3));
        soc_L3_IPMCm_field32_set(unit, &entry, MIN_MTUf, min_mtu_l3);
    }
#endif /* BCM_EASYRIDER_SUPPORT */
...
bcm_fb_er_ipmc_init(int unit)
{
    ...
    bcm_fb_er_ipmc_delete_all(unit);
    ...
    if (SOC_IS_EASYRIDER(unit) &&
        SOC_REG_FIELD_VALID(unit, SEER_CONFIGr, L3_IPMC_VALID_AS_HITf)) {
        int val;

        val = soc_property_get(unit, spn_L3_IPMC_VALID_AS_HIT, 1);
        BCM_IF_ERROR_RETURN
            (soc_reg_field32_modify(unit, SEER_CONFIGr, REG_PORT_ANY,
                                    L3_IPMC_VALID_AS_HITf, (val) ? 1 : 0));
        IPMC_VALID_AS_HIT(unit) = (val) ? 1 : 0;
    }
#endif
...
#if defined(BCM_EASYRIDER_SUPPORT)
        if (SOC_IS_EASYRIDER(unit)) {
            /* Get the first l3index in the IPMC group's linked list */
            BCM_IF_ERROR_RETURN
                (_bcm_er_ipmc_l3entry_list_get_l3index(unit, index, &l3index));
        }
#endif /* BCM_EASYRIDER_SUPPORT */
...
bcm_fb_er_ipmc_lookup(int unit, int *index, bcm_ipmc_addr_t *ipmc)
{
    ...
    if (SOC_IS_FBX(unit)) {
        ...
        BCM_IF_ERROR_RETURN(_bcm_er_ipmc_get(unit, ipmc));
#endif /* BCM_EASYRIDER_SUPPORT */
    }

    ipmc_id = SOC_IS_FBX(unit) ? l3cfg.l3c_ipmc_ptr : ipmc->ipmc_index;
    BCM_IF_ERROR_RETURN
        (_xgs3_ipmc_read(unit, ipmc_id, &ipmc_entry));

    l3_id = SOC_IS_FBX(unit) ? l3cfg.l3c_hw_index : ipmc->ipmc_index;
    BCM_IF_ERROR_RETURN
    ...
    if (index != NULL) {
        *index = SOC_IS_FBX(unit) ? l3cfg.l3c_ipmc_ptr : ipmc->ipmc_index;
    }
...
_bcm_fb_ipmc_add(int unit, bcm_ipmc_addr_t *ipmc)
{
    ...
    if (BCM_SUCCESS(rv)) {
        rv = _bcm_xgs3_ipmc_l3entry_list_add(unit, ipmc->ipmc_index, 
                                             l3cfg); 
    }
    return (rv);
}
...
#if defined(BCM_EASYRIDER_SUPPORT)
        BCM_IF_ERROR_RETURN(_bcm_er_ipmc_l3entry_list_size_get(unit,
                    ipmc_index, &l3entry_list_size));
#endif /* BCM_EASYRIDER_SUPPORT */
    } 
...
bcm_fb_er_ipmc_add(int unit, bcm_ipmc_addr_t *ipmc)
{
    ...
    rv = bcm_fb_er_ipmc_lookup(unit, &old_ipmc_index, &ipmc_lookup_data);
    if (BCM_SUCCESS(rv)) { 
        if (!(ipmc->flags & BCM_IPMC_REPLACE)) {
           return (BCM_E_EXISTS);
        } else {
    ...
    if (new_entry) {
        ...
        rv = _xgs3_ipmc_write(unit, ipmc->ipmc_index, ipmc);
        if (BCM_FAILURE(rv)) {
        ...
        rv = bcm_fb_er_ipmc_put(unit, ipmc->ipmc_index, ipmc);
        if (BCM_FAILURE(rv)) {
            if (SOC_IS_FBX(unit)) {
#if defined(BCM_FIREBOLT_SUPPORT)
...
#if defined(BCM_EASYRIDER_SUPPORT)
                _bcm_er_ipmc_del(unit, ipmc, TRUE);
#endif /* BCM_EASYRIDER_SUPPORT */
            }
            bcm_xgs3_ipmc_id_free(unit, ipmc->ipmc_index);
            return rv;
        ...
        if (delete_old_ipmc_repl_tree) {
            ...
            BCM_IF_ERROR_RETURN(_xgs3_ipmc_read(unit, old_ipmc_index, &ipmc_entry));
            ...
            if (repl) {
                BCM_IF_ERROR_RETURN(bcm_esw_ipmc_repl_delete_all(unit, old_ipmc_index,
                                                                 repl_pbmp));
            }
        }
    } 
...
bcm_fb_er_ipmc_put(int unit, int index, bcm_ipmc_addr_t *ipmc)
{
    ...
    if (SOC_IS_FBX(unit)) {
        ...
        if (ipmc->flags & BCM_IPMC_USE_IPMC_INDEX) {
            if (old_ipmc_index != ipmc->ipmc_index) {
                BCM_IF_ERROR_RETURN
                    (_bcm_xgs3_ipmc_l3entry_list_add(unit, ipmc->ipmc_index,
                                                     l3cfg)); 
                BCM_IF_ERROR_RETURN
    ...
    if (SOC_IS_EASYRIDER(unit)) {
#if defined(BCM_EASYRIDER_SUPPORT)
        bcm_ipmc_addr_t ipmc_rpl;
        int old_ipmc_index;

        sal_memcpy(&ipmc_rpl, ipmc, sizeof(bcm_ipmc_addr_t));

        BCM_IF_ERROR_RETURN(_bcm_er_ipmc_get(unit, ipmc));
        old_ipmc_index = ipmc->ipmc_index;

        BCM_IF_ERROR_RETURN(_bcm_er_ipmc_replace(unit, &ipmc_rpl));

        if (ipmc_rpl.flags & BCM_IPMC_USE_IPMC_INDEX) {
            if (old_ipmc_index != ipmc_rpl.ipmc_index) {
                BCM_IF_ERROR_RETURN
                    (_bcm_er_ipmc_l3entry_list_add(unit, ipmc_rpl.ipmc_index,
                                                   ipmc_rpl.ipmc_index,
                                                   (ipmc_rpl.flags & BCM_IPMC_IP6) ? 1 : 0)); 
                BCM_IF_ERROR_RETURN
                    (_bcm_er_ipmc_l3entry_list_del(unit, old_ipmc_index,
                                                   old_ipmc_index,
                                                   (ipmc_rpl.flags & BCM_IPMC_IP6) ? 1 : 0)); 
            }
        }
    ...
    return _xgs3_ipmc_write(unit, index, ipmc);
}
...
_bcm_fb_er_ipmc_delete(int unit, bcm_ipmc_addr_t *ipmc, int modify_l3entry_list)
{
    ...
    if (!(ipmc->flags & BCM_IPMC_KEEP_ENTRY)) {
        ...
        if (SOC_IS_FBX(unit)) {
            ...
            BCM_IF_ERROR_RETURN(_bcm_er_ipmc_del(unit, ipmc, modify_l3entry_list));
#endif /* BCM_EASYRIDER_SUPPORT */
        }
...
#ifdef BCM_BRADLEY_SUPPORT
    if (!IPMC_USED_ISSET(unit, ipmc_id) && SOC_IS_HBX(unit)) {
        BCM_IF_ERROR_RETURN
...
bcm_fb_er_ipmc_delete_all(int unit)
{
    ...
    rv = soc_mem_clear(unit, L3_IPMCm, MEM_BLOCK_ALL, TRUE);
    IPMC_UNLOCK(unit);
    ...
    for (i = 0; i < IPMC_GROUP_NUM(unit); i++) {
        ...
        while (ipmc_l3entry != NULL) {
            if (SOC_IS_FBX(unit)) {
                ...
                BCM_IF_ERROR_RETURN
                    (bcm_xgs3_l3_del(unit, &l3cfg));
#if defined(BCM_BRADLEY_SUPPORT)
                if (SOC_IS_HBX(unit)) {
                    BCM_IF_ERROR_RETURN
                        (_bcm_xgs3_ipmc_bitmap_clear(unit, i));
                }
#endif
#endif /* BCM_FIREBOLT_SUPPORT */
#if defined(BCM_EASYRIDER_SUPPORT)
                ...
                BCM_IF_ERROR_RETURN
                    (_bcm_er_ipmc_del_by_index(unit, ipmc_l3entry->l3index,
                                               ipmc_l3entry->ip6));
#endif /* BCM_EASYRIDER_SUPPORT */
            }
            IPMC_GROUP_INFO(unit, i)->l3entry_list = ipmc_l3entry->next;
...
bcm_fb_ipmc_repl_init(int unit)
{
    ...
    if (!SAL_BOOT_QUICKTURN) {
        ...
        if (!SOC_IS_HB_GW(unit) && !SOC_IS_RAVEN(unit) && !SOC_IS_ENDURO(unit)) {
            ...
            if (!SOC_IS_SC_CQ(unit) && !SOC_IS_HURRICANE(unit)) {
                SOC_IF_ERROR_RETURN
                            ...
                            int check_port)
{
    ...
    if (!SOC_IPMCREPLSHR_GET(unit) && !SOC_IS_HURRICANE(unit) &&
        !soc_feature(unit, soc_feature_ipmc_repl_penultimate)) {
        return BCM_E_CONFIG;
    }
...
#if defined(BCM_EASYRIDER_SUPPORT)
            if (SOC_IS_EASYRIDER(unit)) {
                rv = _bcm_er_ipmc_l3entry_list_add(unit, ipmc_ptr, i, 1);
            } else 
#endif
            {
#if defined(BCM_FIREBOLT_SUPPORT)
...
#if defined(BCM_EASYRIDER_SUPPORT)
    if (SOC_IS_EASYRIDER(unit)) {
        soc_cm_print("    L3 index (index:value) :");
        if (ipmc_info->ipmc_group_info != NULL) {
            for (i = 0, j = 0; i < IPMC_GROUP_NUM(unit); i++) {
                ipmc_l3entry = IPMC_GROUP_INFO(unit, i)->l3entry_list;
                while (ipmc_l3entry != NULL) {
                    ...
                    soc_cm_print("  %5d:%-5d", i, ipmc_l3entry->l3index);
                    j++;
                    ipmc_l3entry = ipmc_l3entry->next;
                }
            }
