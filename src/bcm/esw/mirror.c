/*
 * $Id: mirror.c 1.288.2.9 Broadcom SDK $
                        ...
                        
{
    ...
    if ((0 == directed) && !SOC_WARM_BOOT(unit)){
        ...
        if (FALSE == isLocal) {
            if (!SOC_IS_TUCANA(unit) || dest_mod > mymodid + NUM_MODID(unit)) {
                BCM_IF_ERROR_RETURN
                                   ...
                                   bcm_mirror_destination_t *mirror_dest)
{
    ...
    *(MIRROR_DEST(unit, mirror_dest->mirror_dest_id)) = *mirror_dest; 
    (MIRROR_DEST(unit, mirror_dest->mirror_dest_id))->flags &=
        (_BCM_MIRROR_DESTINATION_LOCAL | BCM_MIRROR_DEST_TUNNELS); 
    ...
    if (0 != (flags & BCM_MIRROR_PORT_INGRESS)) {
        ...
        if (soc_mem_field_valid(unit, IM_MTP_INDEXm, Tf)) {
            if (0 != soc_IM_MTP_INDEXm_field32_get(unit, &im_mtp, Tf)) {
                tgid = soc_IM_MTP_INDEXm_field32_get(unit, &im_mtp,
                                                      PORT_NUMf);
                *modid = 0;
                BCM_IF_ERROR_RETURN 
        ...
        if (soc_mem_field_valid(unit, EM_MTP_INDEXm, Tf)) {
            if (0 != soc_EM_MTP_INDEXm_field32_get(unit, &em_mtp, Tf)) {
                tgid = soc_EM_MTP_INDEXm_field32_get(unit, &em_mtp,
                                                      PORT_NUMf);
                *modid = 0;
                BCM_IF_ERROR_RETURN 
                    (_bcm_mirror_gport_construct(unit, tgid, 0,
        ...
        if (0 != soc_EP_REDIRECT_EM_MTP_INDEXm_field32_get(unit,
                                                           &epm_mtp, Tf)) {
            tgid = soc_EP_REDIRECT_EM_MTP_INDEXm_field32_get(unit, &epm_mtp,
                                                             PORT_NUMf);
            *modid = 0;
            BCM_IF_ERROR_RETURN 
                (_bcm_mirror_gport_construct(unit, tgid, 0,
                                   ...
                                   void *user_data)
{
    ...
    for (entry_count = 0; entry_count < entry_num; entry_count++) {
        ...
        if (BCM_SUCCESS(rv)) {
            ...
            FP_ERR(("Mirror module reload, ignore FP error report\n"));
        }
        ...
        if (BCM_SUCCESS(rv)) {
            ...
            BCM_IF_ERROR_RETURN(bcm_esw_field_group_get(unit, group,
                &group_qset));
    ...
    if (SOC_IS_XGS3_SWITCH(unit)) {
        PBMP_PORT_ITER(unit, port_ix) {
...
#endif /* BCM_TRIDENT_SUPPORT */
            {
                ...
                mc_enable = soc_reg64_field32_get(unit, MIRROR_CONTROLr,
                                                  reg_val, M_ENABLEf);
            }
            if (mc_enable) {
                enabled = TRUE;
        ...
        PBMP_PORT_ITER(unit, port_ix) {
            ...
            if (mc_enable) {
...
#endif /* BCM_TRIDENT_SUPPORT */
                            {
                                idx = soc_reg64_field32_get(unit,
...
#endif /* BCM_TRIUMPH2_SUPPORT */
                {
                    ...
                    if (enable) {
                        idx = soc_reg64_field32_get(unit, MIRROR_CONTROLr,
                                 ...
                                 bcm_pbmp_t *dest_bitmap)
{
    ...
    /* If mtp_index is not marked as egress (& not in Warm Boot) - error */
    if (!MIRROR_CONFIG_SHARED_MTP(unit, mtp_index).egress &&
        !SOC_WARM_BOOT(unit)) {
        return BCM_E_PARAM;
    }

    BCM_IF_ERROR_RETURN(soc_mem_read(unit, mem[mtp_index], MEM_BLOCK_ANY, port,
                                     &entry));
                                 ...
                                 bcm_pbmp_t *dest_bitmap)
{
    ...
    /* If mtp_index is not marked as egress - error */
    if (!MIRROR_CONFIG_SHARED_MTP(unit, mtp_index).egress) {
        return BCM_E_PARAM;
    }
    ...
    soc_mem_pbmp_field_set(unit, mem[mtp_index], &entry, BITMAPf, dest_bitmap);
    BCM_IF_ERROR_RETURN
                                 ...
                                 bcm_pbmp_t *dest_bitmap)
{
    ...
    if (soc_feature(unit, soc_feature_mirror_flexible)) {
        /* If mtp_index is not marked as egress - error */
        if (!MIRROR_CONFIG_SHARED_MTP(unit, mtp_index).egress) {
            /* Not configured for egress mirroring */
            return BCM_E_PARAM;
        }
    } else if (mtp_index >= MIRROR_CONFIG(unit)->port_em_mtp_count) {
        /* Out of range */
        return BCM_E_PARAM;
    }
...
/*
 * Function:
 *     _bcm_tr2_mirror_dest_get_all
 * Purpose:
 *     Get all mirroring destinations on devices with TR2 mirroring.   
 * Parameters:
 *     unit             - (IN) BCM device number. 
 ...
 */
STATIC int
_bcm_tr2_mirror_dest_get_all(int unit, uint32 flags, int mirror_dest_size,
                             bcm_gport_t *mirror_dest, int *mirror_dest_count)
{
    int idx = 0;
    int index = 0;
    int egress = (flags & BCM_MIRROR_PORT_EGRESS) ? TRUE : FALSE;
    ...
    for (idx = 0; idx < BCM_MIRROR_MTP_COUNT; idx++) {
        if ((index < mirror_dest_size) && 
            (MIRROR_CONFIG_SHARED_MTP_REF_COUNT(unit, idx)) && 
            egress == MIRROR_CONFIG_SHARED_MTP(unit, idx).egress) {
            mirror_dest[index++] = MIRROR_CONFIG_SHARED_MTP_DEST(unit, idx);
        }
    } 
...
#ifdef BCM_TRIUMPH2_SUPPORT 
    if (soc_feature(unit, soc_feature_mirror_flexible)) {
        return _bcm_tr2_mirror_dest_get_all(unit, flags, mirror_dest_size, 
...
_bcm_er_mtp_init(int unit, int index, bcm_gport_t *trunk_arr, int flags)
{
    ...
    BCM_IF_ERROR_RETURN(
        _bcm_esw_gport_resolve(unit, mirror_dest, &modid, &port, &tgid, &id));
                                  ...
                                  int flags, void **entries)
{
    ...
    /* Get mtp configuration structure by direction & index. */
    mtp_cfg = &(MIRROR_CONFIG_SHARED_MTP(unit, index));
                                        ...
                                        void **entries)
{
    ...
    /* Set protocol to GRE. 0x88be */
    soc_EGR_MIRROR_ENCAP_DATA_1m_field32_set(unit, data_1_entry_p,
                         ERSPAN__ERSPAN_HEADER_GREf, 0x88be);
...
_bcm_trident_mtp_init(int unit, int index, bcm_gport_t *trunk_arr, int flags)
{
    ...
    /* Currently module_id_7 and port_num_7 will be used by SW to store 
       The trunk id of programmed ports until required field will be 
       added into regs fil by HW */
    static const soc_field_t port_field[] = {
        PORT_NUM_0f, PORT_NUM_1f, PORT_NUM_2f, PORT_NUM_3f, 
        PORT_NUM_4f, PORT_NUM_5f, PORT_NUM_6f};
    static const soc_field_t module_field[] = {
            MODULE_ID_0f, MODULE_ID_1f, MODULE_ID_2f, MODULE_ID_3f, 
            MODULE_ID_4f, MODULE_ID_5f, MODULE_ID_6f};
    ...
    if (BCM_GPORT_IS_TRUNK(mirror_dest)) {
        ...
        BCM_IF_ERROR_RETURN(
            bcm_esw_trunk_get(unit, trunk, &tinfo));
        
        /* Unable to mirror to empty trunk */
        if (0 == tinfo.num_ports) {
            return BCM_E_PORT;
        }
        if (tinfo.num_ports > BCM_TD_MIRROR_TRUNK_MAX_PORTCNT) {
            tinfo.num_ports = BCM_TD_MIRROR_TRUNK_MAX_PORTCNT;
        }
        soc_IM_MTP_INDEXm_field32_set(unit, mtp, Tf, 1);
        soc_IM_MTP_INDEXm_field32_set(unit, mtp, COUNTf, (tinfo.num_ports - 1));
        soc_IM_MTP_INDEXm_field32_set(unit, mtp, PORT_NUM_7f, trunk);
        for (idx = 0; idx < tinfo.num_ports; idx++) {
            /* trunk variable will not be used from this point, OK to update */
            BCM_IF_ERROR_RETURN(
        ...
        /* Here it is safe to assume that mirror_dest is in MODPORT format */
    ...
    for (idx = 0; idx < BCM_SWITCH_TRUNK_MAX_PORTCNT; idx++, offset++) {
        ...
        /* EGR_EP_REDIRECT_EM_MTP_INDEX has same layout as EGR_IM_MTP_INDEX */
        if (soc_feature(unit, soc_feature_egr_mirror_true) &&
            (flags & BCM_MIRROR_PORT_EGRESS_TRUE)) {
            BCM_IF_ERROR_RETURN
    ...
    if (BCM_GPORT_IS_TRUNK(mirror_dest)) {
        ...
        for (idx = 0; idx < BCM_SWITCH_TRUNK_MAX_PORTCNT; idx++) {
            ...
            if (isGport) {
                ...
                gport_st.gport_type = BCM_GPORT_TYPE_MODPORT;
                gport_st.modid = tinfo.tm[idx % tinfo.num_ports];
                gport_st.port = tinfo.tp[idx % tinfo.num_ports];
                BCM_IF_ERROR_RETURN(
                    _bcm_esw_gport_construct(unit, &gport_st, &gport[idx]));
            }
        }
    } else {
        bcm_module_t    modid;
                                     ...
                                     int *index_used)
{
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        MIRROR_CONFIG_EGR_MTP_REF_COUNT(unit, idx)++;
            ...
            MIRROR_CONFIG_EGR_MTP_REF_COUNT(unit, idx) += (1 << BCM_MIRROR_MTP_REF_FP_OFFSET);
        }
        *index_used = idx;
        return (rv);
    }
                                         ...
                                         int mtp_index)
{
    ...
    /* if mtp is enabled on port - do nothing */
    if (enable & (1 << mtp_index)) {
        return (BCM_E_NONE);
    }
                                    ...
                                    int egress)
{
    ...
    mirror_control_entry_t entry;   /* MTP control memory value (Trident)   */
    ...
    if (egress) {
        ...
        BCM_IF_ERROR_RETURN
            (_bcm_esw_mirror_egress_get(unit, port, &enable));
    } else {
    ...
    /* Read from memory into SW structure */
    if (SOC_IS_TD_TT(unit) || SOC_IS_KATANA(unit)) {
        BCM_IF_ERROR_RETURN
            (READ_MIRROR_CONTROLm(unit, MEM_BLOCK_ANY, port, &entry));
    } else {
    ...
    for (mtp_index = 0; mtp_index < BCM_MIRROR_MTP_COUNT; mtp_index++) {
        /* Check if MTP index is enabled on port */
        if (!(enable & (1 << mtp_index))) {
            continue;
        }
        /* Check if MTP index direction matches */
        ...
        if (SOC_IS_TD_TT(unit) || SOC_IS_KATANA(unit)) {
            mtp_value = soc_mem_field32_get(unit, MIRROR_CONTROLm, &entry, 
                                            _mtp_index_field[mtp_index]);
        } else {
            mtp_value = soc_reg64_field32_get(unit, MIRROR_CONTROLr, reg_val,
                                              _mtp_index_field[mtp_index]);
        }

        dest_array[index++] = MIRROR_CONFIG_SHARED_MTP_DEST(unit, mtp_value);
    }
    return (BCM_E_NONE);
}
...
_bcm_xgs3_mirror_enable_set(int unit, int port, int enable)
{
    ...
    if (SOC_IS_TD_TT(unit) || SOC_IS_KATANA(unit)) {
        ...
        BCM_IF_ERROR_RETURN
            (soc_mem_field32_modify(unit, MIRROR_CONTROLm,
                                    SOC_INFO(unit).cpu_hg_index, M_ENABLEf,
                                    enable));
        }
    } else {
        BCM_IF_ERROR_RETURN
...
_bcm_esw_mirror_egress_get(int unit, bcm_port_t port, int *enable)
{
    ...
    bcm_pbmp_t dest_bitmap;
    int value = 0;
    int mtp_index, mtp_count;

    mtp_count = soc_feature(unit, soc_feature_mirror_flexible) ?
        BCM_MIRROR_MTP_COUNT :
        MIRROR_CONFIG(unit)->port_em_mtp_count;

    /* Get destination port bitmap from first valid port. */
    PBMP_ALL_ITER(unit, port_iterator) {
        for (mtp_index = 0; mtp_index < mtp_count; mtp_index ++) {
            if (soc_feature(unit, soc_feature_mirror_flexible)) {
                if (!MIRROR_CONFIG_SHARED_MTP(unit, mtp_index).egress &&
                    !SOC_WARM_BOOT(unit)) {
                    continue;
            ...
            BCM_IF_ERROR_RETURN
                (_bcm_esw_mirror_egr_dest_get(unit, port_iterator, mtp_index,
                                              &dest_bitmap));
...
_bcm_esw_mirror_egress_set(int unit, bcm_port_t port, int enable)
{
    ...
    PBMP_ALL_ITER(unit, port_iterator) {
        for (mtp_index = 0; mtp_index < mtp_count; mtp_index ++) {
            ...
            BCM_IF_ERROR_RETURN
                (_bcm_esw_mirror_egr_dest_set(unit, port_iterator, mtp_index,
                                              &dest_bitmap));
        }
    }
...
_bcm_esw_mirror_hw_clear(int unit)
{
    ...
    uint64 mc_reg;                 /* MTP control register value.  */
    ...
    if (SOC_IS_XGS3_SWITCH(unit)) {
        ...
        if (soc_feature(unit, soc_feature_mirror_flexible) ||
            ...
            if (SOC_IS_TD_TT(unit) || SOC_IS_KATANA(unit)) {
                ...
                COMPILER_64_ZERO(mc_reg);
                for (mtp_index = 0; mtp_index < BCM_MIRROR_MTP_COUNT;
                     mtp_index++) {
                    soc_reg64_field32_set(unit, MIRROR_CONTROLr, &mc_reg,
                                          _mtp_index_field[mtp_index],
                                          mtp_index);
                    soc_reg64_field32_set(unit, MIRROR_CONTROLr, &mc_reg,
                                          _non_uc_mtp_index_field[mtp_index],
            ...
            if (soc_feature(unit, soc_feature_mirror_flexible) ||
                ...
                if (SOC_IS_TD_TT(unit) || SOC_IS_KATANA(unit)) {
                    ...
                    if (IS_CPU_PORT(unit, port) && 
                        ...
                        BCM_IF_ERROR_RETURN
                            (WRITE_MIRROR_CONTROLm(unit, MEM_BLOCK_ANY,
                                   SOC_INFO(unit).cpu_hg_index, &mc_entry));
                    }
                } else {
                    BCM_IF_ERROR_RETURN
                            ...
                            uint32 flags, int *mtp_index) 
{
    ...
    if ((flags & BCM_MIRROR_PORT_EGRESS_TRUE) && !directed) {
        MIRROR_UNLOCK(unit);
        return (BCM_E_CONFIG);
    }
...
_bcm_esw_mirror_fp_dest_delete(int unit, int mtp_index, uint32 flags)
{
    ...
    if (soc_feature(unit, soc_feature_mirror_flexible))
    {
        if (flags & BCM_MIRROR_PORT_EGRESS_TRUE) {
            ...
            mirror_dest_id = MIRROR_CONFIG_SHARED_MTP_DEST(unit, mtp_index);
        }
...
bcm_esw_mirror_to_set(int unit, bcm_port_t port)
{
    ...
    if (SOC_IS_XGS3_SWITCH(unit)) {
        ...
        if (soc_feature(unit, soc_feature_mirror_flexible)) {
             ...
             * the ingress and MTP 3 for the egress. */
                             ...
                             uint16 tpid, uint16 vlan)
{
    ...
    egr_port_entry_t egr_port_entry;
    void *entries[EGR_MIRROR_ENCAP_ENTRIES_NUM];
    soc_field_t fields[2] = { MIRROR_ENCAP_ENABLEf, MIRROR_ENCAP_INDEXf };
    uint32 values[2];
    ...
    if (0 != hw_buffer) {
        ...
        if (BCM_SUCCESS(rv)) {
            ...
            values[0] = 1;
            values[1] = profile_index;
            rv = soc_mem_fields32_modify(unit, EGR_PORTm,
                                         port, 2, fields, values);
        }
    } else {
        ...
        if (BCM_SUCCESS(rv) &&
            (0 != soc_EGR_PORTm_field32_get(unit, &egr_port_entry,
                                            MIRROR_ENCAP_ENABLEf))) {
            profile_index = soc_EGR_PORTm_field32_get(unit, &egr_port_entry,
                                                      MIRROR_ENCAP_INDEXf);
            values[0] = 0;
            values[1] = 0;
            rv = soc_mem_fields32_modify(unit, EGR_PORTm,
                                         port, 2, fields, values);
            if (BCM_SUCCESS(rv)) {
                rv = _bcm_egr_mirror_encap_entry_delete(unit, profile_index);
            }
        } /* Else do nothing */
    }
                              ...
                              uint32 flags, bcm_gport_t mirror_dest) 
{
    ...
    if (0 == directed) {
        ...
        /* Trident does not support non directed mirroring */
        if (SOC_IS_TD_TT(unit) || SOC_IS_KATANA(unit)) {
            return (BCM_E_CONFIG);
        }
...
_bcm_mirror_sw_dump(int unit)
{
    ...
    if (BCM_FAILURE(_bcm_esw_directed_mirroring_get(unit, &directed))) {
        /* Unable to retrieve directed mirroring status, abort */
        return;
    }
