/*
 * $Id: trunk.c 1.28.2.6 Broadcom SDK $
...
#define _BCM_FP_TRUNK_NUM_MAX         1024
#define _BCM_FABRIC_TRUNK_NUM_MAX     32

#define _BCM_FP_TRUNK_MAX_PORTCNT     256 
#define _BCM_FABRIC_TRUNK_MAX_PORTCNT 32

#define _BCM_FP_TRUNK_RTAG1_6_MAX_PORTCNT     16
#define _BCM_FABRIC_TRUNK_RTAG1_6_MAX_PORTCNT 16

#define _BCM_TRUNK_FAILOVER_MAX_PORTCNT 8

#define _BCM_FABRIC_TRUNK_SWF_TID_OFFSET 1025

#ifdef BCM_KATANA_SUPPORT

#define _BCM_KATANA_FP_TRUNK_NUM_MAX         128
#define _BCM_KATANA_FABRIC_TRUNK_NUM_MAX     2

#define _BCM_KATANA_FP_TRUNK_MAX_PORTCNT     8 
#define _BCM_KATANA_FABRIC_TRUNK_MAX_PORTCNT 4

#define _BCM_KATANA_FP_TRUNK_RTAG1_6_MAX_PORTCNT     8
#define _BCM_KATANA_FABRIC_TRUNK_RTAG1_6_MAX_PORTCNT 4

#define _BCM_KATANA_TRUNK_FAILOVER_MAX_PORTCNT 4

#define _BCM_KATANA_FABRIC_TRUNK_SWF_TID_OFFSET 127
...
/* ------------------------------------------------- */
typedef struct _trident_dlb_bookkeeping_s {
    SHR_BITDCL *hg_dlb_id_used_bitmap;
    SHR_BITDCL *hg_dlb_flowset_block_bitmap; /* Each block corresponds to 512 entries */
    int hg_dlb_sample_rate;
    int hg_dlb_tx_load_min_th;
    int hg_dlb_tx_load_max_th;
    int hg_dlb_qsize_min_th;
    int hg_dlb_qsize_max_th;
    uint8 hg_dlb_quality_tx_load_percent[SOC_MAX_NUM_PORTS];
    soc_profile_mem_t *hg_dlb_quality_map_profile;
} _trident_dlb_bookkeeping_t;

STATIC _trident_dlb_bookkeeping_t *_trident_dlb_bk[BCM_MAX_NUM_UNITS];

#define DLB_INFO(_unit_) (_trident_dlb_bk[_unit_])

#define _BCM_HG_DLB_ID_USED_GET(_u_, _idx_) \
    SHR_BITGET(DLB_INFO(_u_)->hg_dlb_id_used_bitmap, _idx_)
#define _BCM_HG_DLB_ID_USED_SET(_u_, _idx_) \
    SHR_BITSET(DLB_INFO(_u_)->hg_dlb_id_used_bitmap, _idx_)
#define _BCM_HG_DLB_ID_USED_CLR(_u_, _idx_) \
    SHR_BITCLR(DLB_INFO(_u_)->hg_dlb_id_used_bitmap, _idx_)

#define _BCM_HG_DLB_FLOWSET_BLOCK_USED_GET(_u_, _idx_) \
    SHR_BITGET(DLB_INFO(_u_)->hg_dlb_flowset_block_bitmap, _idx_)
#define _BCM_HG_DLB_FLOWSET_BLOCK_USED_SET(_u_, _idx_) \
    SHR_BITSET(DLB_INFO(_u_)->hg_dlb_flowset_block_bitmap, _idx_)
#define _BCM_HG_DLB_FLOWSET_BLOCK_USED_CLR(_u_, _idx_) \
    SHR_BITCLR(DLB_INFO(_u_)->hg_dlb_flowset_block_bitmap, _idx_)
#define _BCM_HG_DLB_FLOWSET_BLOCK_USED_SET_RANGE(_u_, _idx_, _count_) \
    SHR_BITSET_RANGE(DLB_INFO(_u_)->hg_dlb_flowset_block_bitmap, _idx_, _count_)
#define _BCM_HG_DLB_FLOWSET_BLOCK_USED_CLR_RANGE(_u_, _idx_, _count_) \
    SHR_BITCLR_RANGE(DLB_INFO(_u_)->hg_dlb_flowset_block_bitmap, _idx_, _count_)
#define _BCM_HG_DLB_FLOWSET_BLOCK_TEST_RANGE(_u_, _idx_, _count_, _result_) \
    SHR_BITTEST_RANGE(DLB_INFO(_u_)->hg_dlb_flowset_block_bitmap, _idx_, _count_, _result_)
...
/* --------------------------------- */
/* Trunk add info internal structure */
/* --------------------------------- */
 ...
 */
typedef struct _trident_trunk_add_info_s {
...
STATIC int _bcm_trident_hg_dlb_quality_assign(int unit, int tx_load_percent,
        dlb_hgt_port_quality_mapping_entry_t *entry_arr);
...
_bcm_trident_trunk_member_bk_init(int unit) 
{
    ...
    /* Reserve entry 0 of TRUNK_MEMBERm. Invalid trunk groups have
     * their TRUNK_GROUP table entry's BASE_PTR and TG_SIZE fields set to 0.
     * Configure entry 0's module ID to max module ID, port number to invalid
     * port number.
     */ 
    if (!SOC_WARM_BOOT(unit)) {
        sal_memset(&trunk_member_entry, 0, sizeof(trunk_member_entry_t));
        soc_TRUNK_MEMBERm_field32_set(unit, &trunk_member_entry, MODULE_IDf,
                SOC_MODID_MAX(unit));
        soc_TRUNK_MEMBERm_field32_set(unit, &trunk_member_entry, PORT_NUMf,
                SOC_MAX_NUM_PORTS);
        SOC_IF_ERROR_RETURN
            (WRITE_TRUNK_MEMBERm(unit, MEM_BLOCK_ALL, 0, &trunk_member_entry));
    }
...
_bcm_trident_hg_dlb_quality_map_profile_init(int unit) 
{
    ...
    if (NULL == _trident_dlb_bk[unit]->hg_dlb_quality_map_profile) {
        _trident_dlb_bk[unit]->hg_dlb_quality_map_profile =
    ...
    profile = _trident_dlb_bk[unit]->hg_dlb_quality_map_profile;
    soc_profile_mem_t_init(profile);

    mem = DLB_HGT_PORT_QUALITY_MAPPINGm;
    entry_words = sizeof(dlb_hgt_port_quality_mapping_entry_t) / sizeof(uint32);
    SOC_IF_ERROR_RETURN
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        /* Assign quality in the entry array, with 50% weight for port load,
         * 50% for queue size.
         */
        tx_load_percent = 50;
        ...
        PBMP_ALL_ITER(unit, port) {
            BCM_IF_ERROR_RETURN
                (READ_DLB_HGT_QUANTIZE_CONTROLr(unit, port, &quantize_control_reg));
            ...
            BCM_IF_ERROR_RETURN
                (WRITE_DLB_HGT_QUANTIZE_CONTROLr(unit, port, quantize_control_reg));
            port_count++;
            DLB_INFO(unit)->hg_dlb_quality_tx_load_percent[port] = tx_load_percent;
        }  
...
/*
 * Function:
 *      _bcm_trident_dlb_deinit
 * Purpose:
 *      Deallocate _trident_dlb_bk
 * Parameters:
 *      unit - (IN)SOC unit number. 
 * Returns:
 *      None
 */
STATIC void
_bcm_trident_dlb_deinit(int unit) 
{
    int rv;
    uint32 arb_control_reg;

    if (_trident_dlb_bk[unit]) {
        if (_trident_dlb_bk[unit]->hg_dlb_id_used_bitmap) {
            sal_free(_trident_dlb_bk[unit]->hg_dlb_id_used_bitmap);
            _trident_dlb_bk[unit]->hg_dlb_id_used_bitmap = NULL;
        }
        if (_trident_dlb_bk[unit]->hg_dlb_flowset_block_bitmap) {
            sal_free(_trident_dlb_bk[unit]->hg_dlb_flowset_block_bitmap);
            _trident_dlb_bk[unit]->hg_dlb_flowset_block_bitmap = NULL;
        }
        if (_trident_dlb_bk[unit]->hg_dlb_quality_map_profile) {
            soc_profile_mem_destroy(unit,
                    _trident_dlb_bk[unit]->hg_dlb_quality_map_profile);
            sal_free(_trident_dlb_bk[unit]->hg_dlb_quality_map_profile);
            _trident_dlb_bk[unit]->hg_dlb_quality_map_profile = NULL;
        }

        sal_free(_trident_dlb_bk[unit]);
        _trident_dlb_bk[unit] = NULL;
    }

    if (soc_reg_field_valid
                (unit, AUX_ARB_CONTROL_2r, DLB_HGT_REFRESH_ENABLEf)) {
...
/*
 * Function:
 *      _bcm_trident_dlb_init
 * Purpose:
 *      Allocate and initialize _trident_dlb_bk
 * Parameters:
 *      unit - (IN)SOC unit number. 
 * Returns:
 *      BCM_X_XXX
 */
STATIC int 
_bcm_trident_dlb_init(int unit) 
{
    int num_hg_dlb_id;
    ...
    if (_trident_dlb_bk[unit] == NULL) {
        _trident_dlb_bk[unit] = 
            sal_alloc(sizeof(_trident_dlb_bookkeeping_t), "_trident_dlb_bk");
        if (_trident_dlb_bk[unit] == NULL) {
            return BCM_E_MEMORY;
        }
    }
    sal_memset(_trident_dlb_bk[unit], 0,
            sizeof(_trident_dlb_bookkeeping_t));

    num_hg_dlb_id = soc_mem_index_count(unit, DLB_HGT_GROUP_CONTROLm);
    if (_trident_dlb_bk[unit]->hg_dlb_id_used_bitmap == NULL) {
        _trident_dlb_bk[unit]->hg_dlb_id_used_bitmap = 
            sal_alloc(SHR_BITALLOCSIZE(num_hg_dlb_id), "hg_dlb_id_used_bitmap");
        if (_trident_dlb_bk[unit]->hg_dlb_id_used_bitmap == NULL) {
            _bcm_trident_dlb_deinit(unit);
            return BCM_E_MEMORY;
        }
    }
    sal_memset(_trident_dlb_bk[unit]->hg_dlb_id_used_bitmap, 0,
    ...
    if (_trident_dlb_bk[unit]->hg_dlb_flowset_block_bitmap == NULL) {
        _trident_dlb_bk[unit]->hg_dlb_flowset_block_bitmap = 
    ...
    sal_memset(_trident_dlb_bk[unit]->hg_dlb_flowset_block_bitmap, 0,
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        rv = _bcm_trident_hg_dlb_sample_rate_thresholds_set(unit, 1900000,
                1250, 8750);
        if (BCM_FAILURE(rv)) {
            _bcm_trident_dlb_deinit(unit);
            return rv;
        ...
        /* Configure DLB_HGT_LINK_CONTROL */
        ...
        /* Configure HGT_DLB_CONTROL table */
        ...
        /* Configure per-port threshold scaling factor and 
        ...
        PBMP_ALL_ITER(unit, port) {
            ...
            soc_reg_field_set(unit, DLB_HGT_QUANTIZE_CONTROLr,
                    &quantize_control_reg, PORT_LOADING_THRESHOLD_SCALING_FACTORf, 1);
            soc_reg_field_set(unit, DLB_HGT_QUANTIZE_CONTROLr,
                    &quantize_control_reg, PORT_QSIZE_THRESHOLD_SCALING_FACTORf, 1);
            ...
            rv = READ_DLB_HGT_PORT_QUALITY_MEASURE_UPDATE_CONTROLr(unit, port,
            ...
            soc_reg_field_set(unit, DLB_HGT_PORT_QUALITY_MEASURE_UPDATE_CONTROLr,
                    &measure_update_control_reg, ENABLE_MEASURE_COLLECTIONf, 1);
            soc_reg_field_set(unit, DLB_HGT_PORT_QUALITY_MEASURE_UPDATE_CONTROLr,
                    &measure_update_control_reg, ENABLE_MEASURE_AVERAGE_CALCULATIONf, 1);
            soc_reg_field_set(unit, DLB_HGT_PORT_QUALITY_MEASURE_UPDATE_CONTROLr,
                    &measure_update_control_reg, ENABLE_PORT_QUALITY_UPDATEf, 1);
            rv = WRITE_DLB_HGT_PORT_QUALITY_MEASURE_UPDATE_CONTROLr(unit, port,
                    measure_update_control_reg);
            if (BCM_FAILURE(rv)) {
                _bcm_trident_dlb_deinit(unit);
                return rv;
...
_bcm_trident_hg_trunk_override_profile_init(int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        ref_count = soc_mem_index_count(unit, VLAN_TABm) + 
            ...
            soc_mem_index_count(unit, IFP_REDIRECTION_PROFILEm); 
...
static soc_field_t _hg_trunk_failover_portf[_BCM_TRUNK_FAILOVER_MAX_PORTCNT] = {
        ...
        int hw_count, bcm_port_t *ftp)
{
    ...
    int    trunk_failover_max_portcnt = 0;
    
#ifdef BCM_KATANA_SUPPORT
    if (SOC_IS_KATANA (unit)) {
        trunk_failover_max_portcnt = _BCM_KATANA_TRUNK_FAILOVER_MAX_PORTCNT;
    } else
#endif
    {
        trunk_failover_max_portcnt = _BCM_TRUNK_FAILOVER_MAX_PORTCNT;
    }
    ...
    if (hw_count) { /* Enabling HG trunk failover */
        ...
        for (ix = 0; ix < trunk_failover_max_portcnt; ix++) {
            soc_HG_TRUNK_FAILOVER_SETm_field32_set(unit, &fail_entry,
...
static soc_field_t _trunk_failover_portf[_BCM_TRUNK_FAILOVER_MAX_PORTCNT] = {
    PORT0f, PORT1f, PORT2f, PORT3f,
    PORT4f, PORT5f, PORT6f, PORT7f
};

static soc_field_t _trunk_failover_modulef[_BCM_TRUNK_FAILOVER_MAX_PORTCNT] = {
        ...
        int hw_count, bcm_port_t *ftp, bcm_module_t *ftm)
{
    ...
    SOC_IF_ERROR_RETURN
        (READ_LAG_FAILOVER_STATUSr(unit, failport, &val));
    if (soc_reg_field_get(unit, LAG_FAILOVER_STATUSr,
    ...
    SOC_IF_ERROR_RETURN
        (READ_LAG_FAILOVER_CONFIGr(unit, failport, &val));
    oval = val;
    soc_reg_field_set(unit, LAG_FAILOVER_CONFIGr, &val,
            LAG_FAILOVER_ENf, (hw_count != 0) ? 1 : 0);
    if (val != oval) {
        SOC_IF_ERROR_RETURN
            (WRITE_LAG_FAILOVER_CONFIGr(unit, failport, val));
    }
        ...
        int *array_count)
{
    ...
    SOC_IF_ERROR_RETURN
        (READ_LAG_FAILOVER_CONFIGr(unit, failport, &val));
    enable = soc_reg_field_get(unit, LAG_FAILOVER_CONFIGr, val,
            LAG_FAILOVER_ENf);
        ...
        bcm_port_t *ftp, bcm_module_t *ftm)
{
    ...
    uint16 fail_modport;
    bcm_gport_t gport;
    bcm_port_t local_port, port_list[_BCM_TRUNK_FAILOVER_MAX_PORTCNT];
    bcm_module_t modid_list[_BCM_TRUNK_FAILOVER_MAX_PORTCNT];
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_trident_trunk_psc_to_rtag(unit, psc, &rtag));

#ifdef BCM_KATANA_SUPPORT
    if(SOC_IS_KATANA(unit)) {
        tid_ix = tid + (hg_trunk ? _BCM_KATANA_FP_TRUNK_NUM_MAX : 0);
    } else
#endif
    {
    ...
    if (flags) {
        ...
        for (ix = ((tix + 1) % tports);  /* Start after fail port */
            ...
            hw_count++;
            if (hw_count == trunk_failover_max_portcnt) {
                break;
            }
            if (next) {
                break;
        ...
        if (count) {
            for (ix = 0; ix < count && ix < trunk_failover_max_portcnt; ix++) {
                port_list[ix] = ftp[ix];
    ...
    if (hg_trunk) {
        BCM_IF_ERROR_RETURN
            (_bcm_trident_trunk_hwfailover_hg_write(unit, local_port, rtag,
                                                    hw_count, port_list));
    } else {
        BCM_IF_ERROR_RETURN
            (_bcm_trident_trunk_hwfailover_write(unit, local_port, rtag,
...
_bcm_trident_trunk_failover_set(int unit, 
        bcm_trunk_t tid, 
        int hg_trunk,
        _trident_trunk_add_info_t *add_info)
{
    int rv = BCM_E_NONE;
        ...
        bcm_pbmp_t ports)
{
...
#ifdef BCM_KATANA_SUPPORT
    if(SOC_IS_KATANA(unit)) {
        swft = &swf->tinfo[hgtid + _BCM_KATANA_FP_TRUNK_NUM_MAX];
        fabric_trunk_swf_tid_offset = _BCM_KATANA_FABRIC_TRUNK_SWF_TID_OFFSET;
    } else
#endif
    {
        swft = &swf->tinfo[hgtid + _BCM_FP_TRUNK_NUM_MAX];
        fabric_trunk_swf_tid_offset = _BCM_FABRIC_TRUNK_SWF_TID_OFFSET;
    }
    ...
    if (swft->num_ports > 0) {
        swft->modport = sal_alloc(sizeof(uint16) * swft->num_ports, "swfail_tinfo_modport");
        ...
        uint32 flags)
{
    ...
    for (; i <= imax; i++) {
        ...
        /* Entries 0 to 255 of NONUCAST_TRUNK_BLOCK_MASK table is used for
         * IPMC trunk resolution. If the flag BCM_TRUNK_FLAG_IPMC_CLEAVE is
         * set, IPMC trunk resolution will be disabled in hardware.
         */
        if (!((i < 256) && (flags & BCM_TRUNK_FLAG_IPMC_CLEAVE))) {
            SOC_PBMP_OR(pbmp, new_ports);
            if (localcount > 0) {
                if (SHR_BITGET(my_modid_bmap, ((i & 0xff) % member_count))) {
 ...
 */
STATIC int
_bcm_trident_trunk_fp_set(int unit,
        bcm_trunk_t tid,
        _trident_trunk_add_info_t *add_info,
        trunk_private_t *t_info)
{
...
#ifdef BCM_KATANA_SUPPORT
    if (SOC_IS_KATANA (unit)) {
        if (add_info->num_ports > _BCM_KATANA_FP_TRUNK_MAX_PORTCNT ||
            add_info->num_ports < 1) {
            return BCM_E_PARAM;
        }

        if (t_info->rtag >= 1 && t_info->rtag <= 6) {
            if (add_info->num_ports > _BCM_KATANA_FP_TRUNK_RTAG1_6_MAX_PORTCNT) {
                return BCM_E_PARAM;
            }
        }
    } else
#endif
    {
        if (add_info->num_ports > _BCM_FP_TRUNK_MAX_PORTCNT ||
            add_info->num_ports < 1) {
            return BCM_E_PARAM;
        }

        if (t_info->rtag >= 1 && t_info->rtag <= 6) {
            if (add_info->num_ports > _BCM_FP_TRUNK_RTAG1_6_MAX_PORTCNT) {
                return BCM_E_PARAM;
            }
        }
    }
    ...
    my_modid_bmap = sal_alloc(SHR_BITALLOCSIZE(add_info->num_ports), "my_modid_bmap");
    if (my_modid_bmap == NULL) {
        _BCM_TRIDENT_TRUNK_FP_SET_FREE_MEM_RETURN(BCM_E_MEMORY);
    }
    sal_memset(my_modid_bmap, 0, SHR_BITALLOCSIZE(add_info->num_ports));
    ...
    if (num_uc_egr_ports > 0) {
         ...
         * If RTAG is 1-6, hardware ignores the TG_SIZE field and assumes 
         * the trunk group occupies _BCM_FP_TRUNK_RTAG1_6_MAX_PORTCNT
...
#endif
            {
                num_entries = _BCM_FP_TRUNK_RTAG1_6_MAX_PORTCNT;
            }
        } else {
        ...
        if (base_ptr > max_base_ptr) {
             ...
             * TRUNK_MEMBERm table.
             */
            _BCM_TRIDENT_TRUNK_FP_SET_FREE_MEM_RETURN(BCM_E_RESOURCE);
        }
    ...
    } else { /* zero unicast egress trunk member ports */
        /* Update TRUNK_GROUP table */
        ...
        old_tg_size = 1 + soc_TRUNK_GROUPm_field32_get(unit, &tg_entry,
                TG_SIZEf);
        old_rtag = soc_TRUNK_GROUPm_field32_get(unit, &tg_entry,
                RTAGf);
    ...
    for (i = 0; i < add_info->num_ports; i++) {
        ...
        if (add_info->member_flags[i] & BCM_TRUNK_MEMBER_INGRESS_DISABLE) {
            BCM_IF_ERROR_RETURN(_bcm_trident_trunk_set_port_property(unit, mod, 
                                                                  port, -1));
        } else {
            BCM_IF_ERROR_RETURN(_bcm_trident_trunk_set_port_property(unit, mod, 
                                                                  port, tid));
    ...
    /* If trunk group was previously in use, clear the previously
     * used region in TRUNK_MEMBERm and fp_trunk_member_bitmap.
     */
    if (t_info->in_use) {
...
#endif
                {
                    old_num_entries = _BCM_FP_TRUNK_RTAG1_6_MAX_PORTCNT;
                }
            } else {
                old_num_entries = old_tg_size;
            }

            _BCM_FP_TRUNK_MEMBER_USED_CLR_RANGE(unit, old_base_ptr,
                    old_num_entries);
        ...
        trunk_private_t *t_info)
{
    ...
    if (SOC_PBMP_NOT_NULL(old_trunk_pbmp)) {
        SOC_PBMP_CLEAR(new_trunk_pbmp);
        rv = _bcm_trident_trunk_block_mask(unit, old_trunk_pbmp, new_trunk_pbmp, 
                new_trunk_pbmp, 0, 0, t_info->flags);
        if (BCM_FAILURE(rv)) {
            sal_free(leaving_members);
            return rv;
        }
...
_bcm_trident_trunk_fp_get(int unit,
        bcm_trunk_t tid,
        _trident_trunk_add_info_t *t_data,
        trunk_private_t *t_info)
{
    int i;
    bcm_module_t mod;
    ...
    if (t_data->num_ports == 0) {
        ...
        if (MEMBER_INFO(unit, tid).num_ports <= t_data->num_ports) {
            ...
            t_data->flags |= BCM_TRUNK_FLAG_MORE_MEMBERS;
        }
        ...
        int min_th, int max_th)
{
     ...
     *     number of 256ns time units = 10^9 ns / 256 ns / sample_rate
     *                                = 3906250 / sample_rate
     */ 
    ...
    DLB_INFO(unit)->hg_dlb_sample_rate = sample_rate;
     ...
     *       to its max value of 255. Hence, the formula will be rewritten as
     *       ((mbps * 256) / 8000) * num_time_units.
     */
    ...
    DLB_INFO(unit)->hg_dlb_tx_load_min_th = min_th;
    DLB_INFO(unit)->hg_dlb_tx_load_max_th = max_th;
...
_bcm_trident_hg_dlb_sample_rate_set(int unit, int sample_rate)
{
    BCM_IF_ERROR_RETURN(_bcm_trident_hg_dlb_sample_rate_thresholds_set(unit,
                sample_rate, DLB_INFO(unit)->hg_dlb_tx_load_min_th,
                DLB_INFO(unit)->hg_dlb_tx_load_max_th));
    return BCM_E_NONE;
}
...
{
    BCM_IF_ERROR_RETURN(_bcm_trident_hg_dlb_sample_rate_thresholds_set(unit,
                DLB_INFO(unit)->hg_dlb_sample_rate, min_th,
                DLB_INFO(unit)->hg_dlb_tx_load_max_th));
    return BCM_E_NONE;
}
...
{
    BCM_IF_ERROR_RETURN(_bcm_trident_hg_dlb_sample_rate_thresholds_set(unit,
                DLB_INFO(unit)->hg_dlb_sample_rate,
                DLB_INFO(unit)->hg_dlb_tx_load_min_th,
                max_th));
    return BCM_E_NONE;
}
...
_bcm_trident_hg_dlb_qsize_thresholds_set(int unit, int min_th, int max_th)
{
    ...
    SOC_IF_ERROR_RETURN
        (WRITE_DLB_HGT_GLB_QUANTIZE_THRESHOLDSm(unit, MEM_BLOCK_ALL, 0,
                                                &thresholds_entry));
    DLB_INFO(unit)->hg_dlb_qsize_min_th = min_th;
    DLB_INFO(unit)->hg_dlb_qsize_max_th = max_th;
...
_bcm_trident_hg_dlb_qsize_min_th_set(int unit, int min_th)
{
    BCM_IF_ERROR_RETURN(_bcm_trident_hg_dlb_qsize_thresholds_set(unit,
                min_th, DLB_INFO(unit)->hg_dlb_qsize_max_th));
...
{
    BCM_IF_ERROR_RETURN(_bcm_trident_hg_dlb_qsize_thresholds_set(unit,
                DLB_INFO(unit)->hg_dlb_qsize_min_th, max_th));
...
_bcm_trident_hg_dlb_qsize_weight_set(int unit, int weight)
{
    ...
    SOC_IF_ERROR_RETURN
        (READ_DLB_HGT_QUALITY_MEASURE_CONTROLr(unit, &measure_control_reg));
    soc_reg_field_set(unit, DLB_HGT_QUALITY_MEASURE_CONTROLr,
        &measure_control_reg, PORT_QSIZE_WEIGHTf, weight);
    SOC_IF_ERROR_RETURN
        (WRITE_DLB_HGT_QUALITY_MEASURE_CONTROLr(unit, measure_control_reg));
    return BCM_E_NONE;
}
...
_bcm_trident_hg_dlb_qsize_weight_get(int unit, int *weight)
{
    ...
    SOC_IF_ERROR_RETURN
        (READ_DLB_HGT_QUALITY_MEASURE_CONTROLr(unit, &measure_control_reg));
    *weight = soc_reg_field_get(unit, DLB_HGT_QUALITY_MEASURE_CONTROLr,
        measure_control_reg, PORT_QSIZE_WEIGHTf);
...
_bcm_trident_hg_dlb_tx_load_cap_set(int unit, int cap)
{
    ...
    SOC_IF_ERROR_RETURN
        (READ_DLB_HGT_QUALITY_MEASURE_CONTROLr(unit, &measure_control_reg));
    soc_reg_field_set(unit, DLB_HGT_QUALITY_MEASURE_CONTROLr,
        &measure_control_reg, CAP_LOADING_AVERAGEf, cap);
    SOC_IF_ERROR_RETURN
        (WRITE_DLB_HGT_QUALITY_MEASURE_CONTROLr(unit, measure_control_reg));
    return BCM_E_NONE;
}
...
_bcm_trident_hg_dlb_tx_load_cap_get(int unit, int *cap)
{
    ...
    *cap= soc_reg_field_get(unit, DLB_HGT_QUALITY_MEASURE_CONTROLr,
        measure_control_reg, CAP_LOADING_AVERAGEf);
...
_bcm_trident_hg_dlb_qsize_cap_set(int unit, int cap)
{
    ...
    SOC_IF_ERROR_RETURN
        (READ_DLB_HGT_QUALITY_MEASURE_CONTROLr(unit, &measure_control_reg));
    soc_reg_field_set(unit, DLB_HGT_QUALITY_MEASURE_CONTROLr,
        &measure_control_reg, CAP_QSIZE_AVERAGEf, cap);
    SOC_IF_ERROR_RETURN
        (WRITE_DLB_HGT_QUALITY_MEASURE_CONTROLr(unit, measure_control_reg));
...
_bcm_trident_hg_dlb_qsize_cap_get(int unit, int *cap)
{
    ...
    SOC_IF_ERROR_RETURN
        (READ_DLB_HGT_QUALITY_MEASURE_CONTROLr(unit, &measure_control_reg));
    *cap= soc_reg_field_get(unit, DLB_HGT_QUALITY_MEASURE_CONTROLr,
        measure_control_reg, CAP_QSIZE_AVERAGEf);
...
_bcm_trident_hg_dlb_config_get(int unit, bcm_switch_control_t type, int *arg)
{
    switch (type) {
        case bcmSwitchFabricTrunkDynamicSampleRate:
            *arg = DLB_INFO(unit)->hg_dlb_sample_rate;
            break;
        ...
        case bcmSwitchFabricTrunkDynamicEgressBytesMinThreshold:
            *arg = DLB_INFO(unit)->hg_dlb_tx_load_min_th;
            break;

        case bcmSwitchFabricTrunkDynamicEgressBytesMaxThreshold:
            *arg = DLB_INFO(unit)->hg_dlb_tx_load_max_th;
            break;

        case bcmSwitchFabricTrunkDynamicQueuedBytesMinThreshold:
            *arg = DLB_INFO(unit)->hg_dlb_qsize_min_th;
            break;

        case bcmSwitchFabricTrunkDynamicQueuedBytesMaxThreshold:
            *arg = DLB_INFO(unit)->hg_dlb_qsize_max_th;
            break;
        ...
        dlb_hgt_port_quality_mapping_entry_t *entry_arr)
{
    ...
    for (quantized_tx_load = 0; quantized_tx_load < 8; quantized_tx_load++) {
        for (quantized_qsize = 0; quantized_qsize < 8; quantized_qsize++) {
            /* Quantized port load and queue size range from 0 to 7, with
             * 7 being most congested. Quality also ranges from
             * 0 to 7, but with 7 being highest quality, i.e. least congested.
             */
            quality = 7 - (quantized_tx_load * tx_load_percent +
        ...
        int tx_load_percent)
{
    ...
    if (_trident_dlb_bk[unit]->hg_dlb_quality_map_profile == NULL) {
        BCM_IF_ERROR_RETURN
            (_bcm_trident_hg_dlb_quality_map_profile_init(unit));
    }
    profile = _trident_dlb_bk[unit]->hg_dlb_quality_map_profile;
    ...
    soc_mem_lock(unit, DLB_HGT_PORT_QUALITY_MAPPINGm);
    entries = entry_arr;
    rv = soc_profile_mem_add(unit, profile, &entries, entries_per_profile,
            &base_index);
    sal_free(entry_arr);
    if (SOC_FAILURE(rv)) {
        soc_mem_unlock(unit, DLB_HGT_PORT_QUALITY_MAPPINGm);
        return rv;
    }
...
/*
 * Function:
 *      _bcm_trident_hg_dlb_config_port_set
 * Purpose:
 *      Set per-port Higig dynamic load balancing configuration parameters.
 * Parameters:
 *      unit - (IN) SOC unit number.
 *      port - (IN) Port number.
 *      type - (IN) Configuration parameter type.
 ...
 */
int
_bcm_trident_hg_dlb_config_port_set(int unit, bcm_port_t port,
        bcm_switch_control_t type, int arg)
{
    switch (type) {
        case bcmSwitchFabricTrunkDynamicEgressBytesPercent:
            return _bcm_trident_hg_dlb_quality_map_set(unit, port, arg);
...
/*
 * Function:
 *      _bcm_trident_hg_dlb_config_port_get
 * Purpose:
 *      Get per-port Higig dynamic load balancing configuration parameters.
 * Parameters:
 *      unit - (IN) SOC unit number.
 *      port - (IN) Port number.
 *      type - (IN) Configuration parameter type.
 ...
 */
int
_bcm_trident_hg_dlb_config_port_get(int unit, bcm_port_t port,
        bcm_switch_control_t type, int *arg)
{
    switch (type) {
        case bcmSwitchFabricTrunkDynamicEgressBytesPercent:
            *arg = DLB_INFO(unit)->hg_dlb_quality_tx_load_percent[port];
            break;
 ...
 */
STATIC int
_bcm_trident_hg_dlb_id_alloc(int unit, int *dlb_id)
{
    int i;
 ...
 * Parameters:
 *      unit   - (IN) SOC unit number. 
 *      dlb_id - (IN) Higig DLB group ID.
 * Returns:
 *      BCM_E_xxx
 */
STATIC int
_bcm_trident_hg_dlb_free_resource(int unit, int dlb_id)
{
    ...
    SOC_IF_ERROR_RETURN
        (READ_DLB_HGT_GROUP_CONTROLm(unit, MEM_BLOCK_ANY, dlb_id,
...
#endif
        {
            num_entries = _BCM_FABRIC_TRUNK_RTAG1_6_MAX_PORTCNT;
        }
    } else {
        num_entries = tg_size;
    }
 ...
 *      hgtid - (IN) Higig trunk group ID.
 *      add_info - (IN) Pointer to trunk add info structure.
 *      new_trunk_pbmp - (IN) Higig trunk group's new port bitmap.
 * Returns:
 *      BCM_E_xxx
 */
STATIC int
        ...
        pbmp_t new_trunk_pbmp)
{
    ...
    int alloc_size;
    dlb_hgt_flowset_port_entry_t *block_ptr, *entry_ptr;
    int i, k;
    ...
    BCM_IF_ERROR_RETURN(_bcm_trident_hg_dlb_id_alloc(unit, &dlb_id));
    ...
    total_blocks = soc_mem_index_count(unit, DLB_HGT_FLOWSET_PORTm) >> 9;
    ...
    alloc_size = num_entries_per_block * sizeof(dlb_hgt_flowset_port_entry_t);
    block_ptr = soc_cm_salloc(unit, alloc_size,
            "Block of DLB_HGT_FLOWSET_PORT entries");
    if (NULL == block_ptr) {
        BCM_IF_ERROR_RETURN(_bcm_trident_hg_dlb_id_free(unit, dlb_id));
        return BCM_E_MEMORY;
    }
    ...
    for (i = 0; i < num_blocks; i++) {
        for (k = 0; k < num_entries_per_block; k++) {
            entry_ptr = soc_mem_table_idx_to_pointer(unit,
                    ...
                    block_ptr, k);
            soc_DLB_HGT_FLOWSET_PORTm_field32_set(unit, entry_ptr, VALIDf, 1);
            soc_DLB_HGT_FLOWSET_PORTm_field32_set(unit, entry_ptr, PORT_NUMf,
                    add_info->tp[(i * num_entries_per_block + k) % add_info->num_ports]);
        }
        index_min = entry_base_ptr + i * num_entries_per_block;
        index_max = index_min + num_entries_per_block - 1;
        rv = soc_mem_write_range(unit, DLB_HGT_FLOWSET_PORTm, MEM_BLOCK_ALL,
    ...
    /* Set hg_dlb_flowset_block_bitmap */
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_trident_hg_dlb_dynamic_size_encode(add_info->dynamic_size,
                                                 &flow_set_size));
    soc_DLB_HGT_GROUP_CONTROLm_field32_set(unit, &dlb_hgt_group_control_entry,
    ...
    SOC_IF_ERROR_RETURN
        (WRITE_DLB_HGT_GROUP_CONTROLm(unit, MEM_BLOCK_ALL, dlb_id,
                                      &dlb_hgt_group_control_entry));
    ...
    if (SOC_MEM_IS_VALID(unit, HGT_DLB_CONTROLm)) {
        ...
        SOC_IF_ERROR_RETURN
            (WRITE_HGT_DLB_CONTROLm(unit, MEM_BLOCK_ALL, hgtid,
    ...
    soc_mem_pbmp_field_set(unit, DLB_HGT_GROUP_MEMBERSHIPm,
            &dlb_hgt_group_membership_entry, PORT_MAPf, &new_trunk_pbmp);
    SOC_IF_ERROR_RETURN
        (WRITE_DLB_HGT_GROUP_MEMBERSHIPm(unit, MEM_BLOCK_ALL, dlb_id,
                                         &dlb_hgt_group_membership_entry));

    /* Store Higig trunk group size in HG_TRUNK_GROUP table for warm boot
     * recovery.
     */
    sal_memset(&hg_trunk_group_entry, 0, sizeof(hg_trunk_group_entry_t));
        ...
        trunk_private_t *t_info)
{
     ...
     * If RTAG is 1-6, hardware ignores the TG_SIZE field and assumes 
     * the Higig trunk group occupies _BCM_FABRIC_TRUNK_RTAG1_6_MAX_PORTCNT
...
/*
 * Function:
 *      bcm_trident_trunk_fabric_set
 * Purpose:
 *      Set fabric trunk members.
 * Parameters:
 *      unit     - (IN) SOC unit number. 
 *      hgtid    - (IN) Higig trunk group ID.
 *      add_info - (IN) Pointer to trunk add info structure.
 ...
 */
STATIC int
_bcm_trident_trunk_fabric_set(int unit,
        bcm_trunk_t hgtid,
        _trident_trunk_add_info_t *add_info,
        trunk_private_t *t_info)
{
    ...
    int          old_dlb_enable = 0, old_dlb_id = 0, dlb_enable = 0;
    pbmp_t       old_trunk_pbmp, new_trunk_pbmp;
...
#ifdef BCM_KATANA_SUPPORT
    if (SOC_IS_KATANA (unit)) {
        if (add_info->num_ports > _BCM_KATANA_FABRIC_TRUNK_MAX_PORTCNT ||
            add_info->num_ports < 1) {
            return BCM_E_PARAM;
        }

        if (t_info->rtag >= 1 && t_info->rtag <= 6) {
            if (add_info->num_ports > _BCM_KATANA_FABRIC_TRUNK_RTAG1_6_MAX_PORTCNT) {
                return BCM_E_PARAM;
            }
        }

        tid = hgtid + _BCM_KATANA_FP_TRUNK_NUM_MAX;
    } else
#endif
    {
        if (add_info->num_ports > _BCM_FABRIC_TRUNK_MAX_PORTCNT ||
            add_info->num_ports < 1) {
            return BCM_E_PARAM;
        }

        if (t_info->rtag >= 1 && t_info->rtag <= 6) {
            if (add_info->num_ports > _BCM_FABRIC_TRUNK_RTAG1_6_MAX_PORTCNT) {
                return BCM_E_PARAM;
            }
        }
    ...
    /* If Higig trunk group was previously in use, free up the previously
     * used resources.
     */
    if (t_info->in_use) {
        if (SOC_MEM_IS_VALID(unit, HGT_DLB_CONTROLm)) {
            SOC_IF_ERROR_RETURN
                (READ_HGT_DLB_CONTROLm(unit, MEM_BLOCK_ANY, hgtid,
                                      &hgt_dlb_control_entry));
            old_dlb_enable = soc_HGT_DLB_CONTROLm_field32_get(unit,
                                      &hgt_dlb_control_entry, GROUP_ENABLEf);
            old_dlb_id = soc_HGT_DLB_CONTROLm_field32_get(unit,
                                      &hgt_dlb_control_entry, DLB_IDf);
        }
        if (old_dlb_enable) {
            BCM_IF_ERROR_RETURN(_bcm_trident_hg_dlb_free_resource(unit, old_dlb_id));
        } else {
    ...
    if ((add_info->psc == BCM_TRUNK_PSC_DYNAMIC) ||
        ...
        trunk_private_t *t_info)
{
    hgt_dlb_control_entry_t hgt_dlb_control_entry;
    dlb_hgt_group_control_entry_t dlb_hgt_group_control_entry;
    dlb_hgt_flowset_port_entry_t dlb_hgt_flowset_port_entry;
    hg_trunk_group_entry_t hg_tg_entry;
    hg_trunk_member_entry_t hg_trunk_member_entry;
    int dlb_enable = 0, dlb_id = 0;
    int base_ptr, tg_size;
    bcm_module_t modid;
    bcm_port_t *hwport;
    int i;
    int rv = BCM_E_NONE;

    /* Although Higig trunk members can be retrieved from HG_TRUNK_BITMAP
     * table, the relative ordering of trunk members cannot be.
     * Hence, Higig trunk members are retrieved from DLB_HGT_FLOWSET_PORT
     * or HG_TRUNK_MEMBER tables.
     */
    ...
    if (dlb_enable) {
       dlb_id = soc_HGT_DLB_CONTROLm_field32_get(unit,
        ...
        base_ptr = soc_DLB_HGT_GROUP_CONTROLm_field32_get(unit,
                &dlb_hgt_group_control_entry, FLOW_SET_BASEf);

        /* Recover Higig trunk size from HG_TRUNK_GROUP table */
        SOC_IF_ERROR_RETURN
            (READ_HG_TRUNK_GROUPm(unit, MEM_BLOCK_ANY, hgtid,
                                  &hg_tg_entry));
        ...
        for (i = 0; i < tg_size; i++) {
            rv = READ_DLB_HGT_FLOWSET_PORTm(unit, MEM_BLOCK_ANY,
                    base_ptr + i, &dlb_hgt_flowset_port_entry);
            if (BCM_FAILURE(rv)) {
                sal_free(hwport);
                return rv;
            }
            hwport[i] = soc_DLB_HGT_FLOWSET_PORTm_field32_get
                (unit, &dlb_hgt_flowset_port_entry, PORT_NUMf);
        }

    } else { /* DLB not enabled */
    ...
    if (t_data->num_ports == 0) {
        t_data->num_ports = tg_size;
    } else {
        if (tg_size <= t_data->num_ports) {
            t_data->num_ports = tg_size;
        } else {
            t_data->flags |= BCM_TRUNK_FLAG_MORE_MEMBERS;
        }
 ...
 * Purpose:
 *      Allocate member_flags, tp, and tm of _trident_trunk_add_info_t.
 * Parameters:
 ...
 */
STATIC int
_trident_trunk_add_info_member_alloc(_trident_trunk_add_info_t *add_info, int num_ports)
{
 ...
 * Purpose:
 *      Free member_flags, tp, and tm of _trident_trunk_add_info_t.
 * Parameters:
 *      add_info  - (IN) Pointer to trident trunk add info structure.
 * Returns:
 *      BCM_E_xxx
 */
STATIC void 
_trident_trunk_add_info_member_free(_trident_trunk_add_info_t *add_info)
{
 ...
 */
STATIC int
_bcm_trident_trunk_set(int unit,
        bcm_trunk_t tid,
        _trident_trunk_add_info_t *add_info,
        trunk_private_t *t_info)
{
    ...
    if (chip_info.trunk_fabric_id_min >= 0 &&
        ...
        /* Higig trunks */
        hgtid = tid - chip_info.trunk_fabric_id_min;
        return _bcm_trident_trunk_fabric_set(unit, hgtid, add_info, t_info);
    } 

    /* Front panel trunks */
    return _bcm_trident_trunk_fp_set(unit, tid, add_info, t_info);
}

/*
 * Function:
 *      bcm_trident_trunk_set
 * Purpose:
 *      Set trunk members.
 * Parameters:
 *      unit     - (IN) SOC unit number. 
 *      tid      - (IN) Trunk group ID.
 *      t_add    - (IN) Pointer to trunk add info structure.
 *      t_info   - (IN) Pointer to trunk info.
 * Returns:
 *      BCM_E_xxx
 */
int
bcm_trident_trunk_set(int unit,
        bcm_trunk_t tid,
        ...
        trunk_private_t *t_info)
{
    ...
    if (t_add->psc <= 0) {
        t_add->psc = BCM_TRUNK_PSC_DEFAULT;
    }
#ifdef BCM_KATANA_SUPPORT
    if(SOC_IS_KATANA(unit)) {
        if ((t_add->psc == BCM_TRUNK_PSC_DYNAMIC) ||
        (t_add->psc == BCM_TRUNK_PSC_DYNAMIC_ASSIGNED) ||
        (t_add->psc == BCM_TRUNK_PSC_DYNAMIC_OPTIMAL))
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_trident_trunk_psc_to_rtag(unit, t_add->psc, &(t_info->rtag)));

    t_info->flags = t_add->flags;

    BCM_IF_ERROR_RETURN(bcm_esw_trunk_chip_info_get(unit, &chip_info));

    if (chip_info.trunk_id_min >= 0 &&
        tid >= chip_info.trunk_id_min &&
        tid <= chip_info.trunk_id_max) {
        ...
        if ((t_add->dlf_index != t_add->mc_index) ||
                (t_add->dlf_index != t_add->ipmc_index)) {
            return BCM_E_PARAM;
        }
        ...
        t_info->dlf_index_spec =
            ...
            t_info->ipmc_index_used = t_add->dlf_index;
    } else {
        /* Higig trunk */
        if (dlb_enable) {
            ...
            /* Verify dynamic_size */
            BCM_IF_ERROR_RETURN
                (_bcm_trident_hg_dlb_dynamic_size_encode(t_add->dynamic_size,
                                                  &dynamic_size_encode));
            t_info->dynamic_size = t_add->dynamic_size;

            /* Verify dynamic_age */
            if ((t_add->dynamic_age < 16) ||
                (t_add->dynamic_age > 0x7fff)) {
                return BCM_E_PARAM;
            }
            t_info->dynamic_age = t_add->dynamic_age;
        }

    }

    if (t_add->num_ports < 1) {
        if (t_info->in_use) {
            /* Delete existing trunk member ports */
            BCM_IF_ERROR_RETURN
                (bcm_trident_trunk_destroy(unit, tid, t_info));
        }
    } else {
        _trident_trunk_add_info_t add_info;
        int i;

        sal_memset(&add_info, 0, sizeof(_trident_trunk_add_info_t));

        add_info.flags = t_add->flags;
        add_info.num_ports = t_add->num_ports;
        add_info.psc = t_add->psc;
        add_info.ipmc_psc = t_add->ipmc_psc;
        add_info.dlf_index = t_add->dlf_index;
        add_info.mc_index = t_add->mc_index;
        add_info.ipmc_index = t_add->ipmc_index;
        add_info.dynamic_size = t_add->dynamic_size;
        add_info.dynamic_age = t_add->dynamic_age;

        BCM_IF_ERROR_RETURN
            (_trident_trunk_add_info_member_alloc(&add_info, t_add->num_ports));

        for (i = 0; i < t_add->num_ports; i++) {
            add_info.member_flags[i] = t_add->member_flags[i];
            add_info.tp[i] = t_add->tp[i];
            add_info.tm[i] = t_add->tm[i];
        }

        rv = _bcm_trident_trunk_set(unit, tid, &add_info, t_info);
    ...
    bcm_trunk_member_t *member_array)
{
    ...
    add_info.flags = t_info->flags;
    add_info.num_ports = member_count;
    add_info.psc = t_info->psc;
    add_info.ipmc_psc = t_info->ipmc_psc;
    add_info.dlf_index = t_info->dlf_index_spec;
    add_info.mc_index = t_info->mc_index_spec;
    add_info.ipmc_index = t_info->ipmc_index_spec;
    add_info.dynamic_size = t_info->dynamic_size;
    add_info.dynamic_age = t_info->dynamic_age;
        ...
        trunk_private_t *t_info)
{
    ...
    sal_memset(&t_data, 0, sizeof(_trident_trunk_add_info_t));

    t_data.num_ports = BCM_TRUNK_MAX_PORTCNT;

    BCM_IF_ERROR_RETURN
        (_trident_trunk_add_info_member_alloc(&t_data, BCM_TRUNK_MAX_PORTCNT));

    rv = _bcm_trident_trunk_get(unit, tid, &t_data, t_info);
    if (BCM_FAILURE(rv)) {
        _trident_trunk_add_info_member_free(&t_data);
        return rv;
    }

    trunk_data->flags = t_data.flags;
    trunk_data->num_ports = t_data.num_ports;
    trunk_data->psc = t_data.psc;
    trunk_data->ipmc_psc = t_data.ipmc_psc;
    trunk_data->dlf_index = t_data.dlf_index;
    trunk_data->mc_index = t_data.mc_index;
    trunk_data->ipmc_index = t_data.ipmc_index;
    trunk_data->dynamic_size = t_data.dynamic_size;
    trunk_data->dynamic_age = t_data.dynamic_age;

    for (i = 0; i < t_data.num_ports; i++) {
        trunk_data->member_flags[i] = t_data.member_flags[i];
        trunk_data->tp[i] = t_data.tp[i];
        trunk_data->tm[i] = t_data.tm[i];
    }
...
/*
 * Function:
 *      bcm_trident_trunk_member_get
 * Purpose:
 *      Retrieve members of a trunk group.
 * Parameters:
 *      unit   - (IN) Unit number.
 *      tid    - (IN) Trunk id.
 *      t_info - (IN) Pointer to trunk info.
    ...
    int *member_count) 
{
    ...
    t_data.num_ports = member_max;

    if (member_max > 0) {
        BCM_IF_ERROR_RETURN
            (_trident_trunk_add_info_member_alloc(&t_data, member_max));
    }
...
/* ----------------------------------------------------------------------------- 
 *
 * Software failover trigger routines
 *
 * ----------------------------------------------------------------------------- 
 */
...
#endif
        {
            old_num_entries = _BCM_FABRIC_TRUNK_RTAG1_6_MAX_PORTCNT;
        }
    } else {
        old_num_entries = old_tg_size;
    }
        ...
        bcm_pbmp_t ports_status)
{
    ...
    if (SOC_IS_KATANA (unit)) {
        fabric_trunk_swf_tid_offset = _BCM_KATANA_FABRIC_TRUNK_SWF_TID_OFFSET;
        fabric_fp_trunk_num_max = _BCM_KATANA_FP_TRUNK_NUM_MAX;
    } else
#endif
    {
        fabric_trunk_swf_tid_offset = _BCM_FABRIC_TRUNK_SWF_TID_OFFSET;
        fabric_fp_trunk_num_max = _BCM_FP_TRUNK_NUM_MAX;
    }
    ...
    PBMP_ITER(ports_active, port) {
        ...
        if (tid >= 1 && tid < fabric_fp_trunk_num_max + 1) {
            pbmp_t               new_trunk_pbmp;
            pbmp_t               old_trunk_pbmp;
            uint16               modport;
            int                  i, j, nmp, rv;
            ...
            if (nmp > 0) {
                mods = sal_alloc(sizeof(bcm_module_t) * nmp, "mods");
                if (mods == NULL) {
                    return BCM_E_MEMORY;
                }
                for (i = 0; i < nmp; i++) {
                ...
                nuc_tpbm = sal_alloc(SHR_BITALLOCSIZE(nmp), "nuc_tpbm");
                if (nuc_tpbm == NULL) {
                    sal_free(mods);
                    return BCM_E_MEMORY;
                }
                sal_memset(nuc_tpbm, 0, SHR_BITALLOCSIZE(nmp));

                rv = _bcm_trident_nuc_tpbm_get(unit, nmp, mods, nuc_tpbm);
                if (BCM_FAILURE(rv)) {
                    sal_free(mods);
                    sal_free(nuc_tpbm);
                    return rv;
                ...
                my_modid_bmap = sal_alloc(SHR_BITALLOCSIZE(nmp), "my_modid_bmap");
                if (my_modid_bmap == NULL) {
                    sal_free(mods);
                    sal_free(nuc_tpbm);
                    return BCM_E_MEMORY;
                }
                sal_memset(my_modid_bmap, 0, SHR_BITALLOCSIZE(nmp));
                ...
                (tid < (fabric_fp_trunk_num_max +
                        fabric_trunk_swf_tid_offset))) {
            /* higig trunk */
            BCM_IF_ERROR_RETURN(
                    _bcm_trident_trunk_fabric_swfailover_trigger
                    (unit,
...
#ifdef BCM_KATANA_SUPPORT
            if (SOC_IS_KATANA (unit)) {
                _BCM_FP_TRUNK_MEMBER_USED_SET_RANGE(unit, base_ptr,
                        _BCM_KATANA_FP_TRUNK_RTAG1_6_MAX_PORTCNT);
            } else
#endif
            {
                _BCM_FP_TRUNK_MEMBER_USED_SET_RANGE(unit, base_ptr,
                        _BCM_FP_TRUNK_RTAG1_6_MAX_PORTCNT);
            }
        } else {
            _BCM_FP_TRUNK_MEMBER_USED_SET_RANGE(unit, base_ptr, tg_size);
        }
...
/*
 * Handle state recovery for Higig DLB parameters 
 */
STATIC int
_bcm_trident_hg_dlb_reinit(int unit)
{
    int rv = BCM_E_NONE;
    uint32 measure_control_reg;
    int num_time_units;
     ...
     */
    DLB_INFO(unit)->hg_dlb_sample_rate = 3906250 / num_time_units;

    /* Recover max and min port thresholds */
    ...
    /* Threshold (in mbps) = (bytes per sampling period) * 8 bits per byte /
     *                       (sampling period in seconds) / 10^6
     *                     = (bytes per sampling period) * 8 bits per byte /
     *                       (num_time_units * 256 ns per time unit * 10^-9) /
     *                       10^6
     *                     = (bytes per sampling period) / num_time_units *
     *                       8000 / 256
     */ 
    DLB_INFO(unit)->hg_dlb_tx_load_max_th = (max_th_bytes / num_time_units *
                                             8000) >> 8;
    min_th_bytes = soc_DLB_HGT_GLB_QUANTIZE_THRESHOLDSm_field32_get
            (unit, &thresholds_entry, PORT_LOADING_THRESHOLD_1f); 
    DLB_INFO(unit)->hg_dlb_tx_load_min_th = (min_th_bytes / num_time_units *
                                             8000) >> 8;

    /* Recover max and min queue size thresholds */

    max_th_cells = soc_DLB_HGT_GLB_QUANTIZE_THRESHOLDSm_field32_get
            (unit, &thresholds_entry, PORT_QSIZE_THRESHOLD_7f); 
    DLB_INFO(unit)->hg_dlb_qsize_max_th = max_th_cells << 7;
    min_th_cells = soc_DLB_HGT_GLB_QUANTIZE_THRESHOLDSm_field32_get
            (unit, &thresholds_entry, PORT_QSIZE_THRESHOLD_1f); 
    DLB_INFO(unit)->hg_dlb_qsize_min_th = min_th_cells << 7;

    /* Recover port quality mapping profiles */
    ...
    PBMP_ALL_ITER(unit, port) {
        ...
        /* quality = 7 - (quantized_tx_load * tx_load_percent +
         *                quantized_qsize * (100 - tx_load_percent)) / 100;
         * quality = 7 - ((quantized_tx_load - quantized_qsize) *
         *                tx_load_percent +
         *                quantized_qsize * 100) / 100;
         * quality = 7 - ((quantized_tx_load - quantized_qsize) *
         *                tx_load_percent / 100 + quantized_qsize);
         * tx_load_percent = (7 - quality - quantized_qsize) * 100 /
         *                   (quantized_tx_load - quantized_qsize); 
         *
         * The index to DLB_HGT_PORT_QUALITY_MAPPING table consists of
         * quantized_tx_load * 8 + quantized_qsize, where both
         * quantized_tx_load and quantized_qsize ranges from 0 to 7.
         *
         * With quantized_tx_load = 7 and quantized_qsize = 0, the index
         * to DLB_HGT_PORT_QUALITY_MAPPING table is 56, and
         * tx_load_present simplies to (7 - quality) * 100 / 7;
         */
        BCM_IF_ERROR_RETURN
            (READ_DLB_HGT_PORT_QUALITY_MAPPINGm(unit, MEM_BLOCK_ANY,
                                                base_index + 56,
        ...
        DLB_INFO(unit)->hg_dlb_quality_tx_load_percent[port] = (7 - quality) *
                                                               100 / 7;
    }
                          ...
                          trunk_private_t *trunk_info)
{
    ...
    int entry_base_ptr, dynamic_size_encode, dlb_mode, dynamic_age;
    ...
    for (tid = min_tid; tid <= max_tid; tid++, trunk_info++) {
        ...
        if (dlb_enable) {
            ...
            trunk_info->rtag = 7;
            /* trunk_info->flags already recovered from scache */
            if (0 == dynamic_size_encode) {
                trunk_info->dynamic_size = 0;
            } else if (1 == dynamic_size_encode) {
                trunk_info->dynamic_size = 512;
            } else {
                trunk_info->dynamic_size = 512 * (2 << (dynamic_size_encode - 2)); 
            }
            ...
            for (i = 0; i < tg_size; i++) {
                rv = READ_DLB_HGT_FLOWSET_PORTm(unit, MEM_BLOCK_ANY,
                        entry_base_ptr + i, &dlb_hgt_flowset_port_entry);
                if (BCM_FAILURE(rv)) {
                    sal_free(hwport);
                    return rv;
                }
...
#endif
                {
                    _BCM_HG_TRUNK_MEMBER_USED_SET_RANGE(unit, base_ptr,
                        _BCM_FABRIC_TRUNK_RTAG1_6_MAX_PORTCNT);
                }
            } else {
                _BCM_HG_TRUNK_MEMBER_USED_SET_RANGE(unit, base_ptr, tg_size);
            }
    ...
    BCM_IF_ERROR_RETURN(_bcm_trident_hg_dlb_reinit(unit));
...
_bcm_trident_trunk_sw_dump(int unit)
{
...
#ifdef BCM_KATANA_SUPPORT
    if (SOC_IS_KATANA (unit)) {
        fp_trunk_num_max = _BCM_KATANA_FP_TRUNK_NUM_MAX;
    } else
#endif
    {
        fp_trunk_num_max = _BCM_FP_TRUNK_NUM_MAX;
    }
    ...
    soc_cm_print("   Queue size min threshold %d cells\n",
            DLB_INFO(unit)->hg_dlb_qsize_min_th);
    soc_cm_print("   Queue size max threshold %d cells\n",
            DLB_INFO(unit)->hg_dlb_qsize_max_th);
    for (i = 0; i < SOC_MAX_NUM_PORTS; i++) {
        if (0 != DLB_INFO(unit)->hg_dlb_quality_tx_load_percent[i]) {
            soc_cm_print("   Port %d quality Tx load percent %d\n",
                    i, DLB_INFO(unit)->hg_dlb_quality_tx_load_percent[i]);
        }
    }
    num_entries_per_profile = 64;
    num_profiles = soc_mem_index_count(unit, DLB_HGT_PORT_QUALITY_MAPPINGm) /
    ...
    for (i = 0; i < num_profiles; i++) {
        ...
        soc_cm_print("   Quality mapping profile %d ref count %d\n",
                i, ref_count);
    }
