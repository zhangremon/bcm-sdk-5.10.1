/*
 * $Id: port.c 1.35.18.1 Broadcom SDK $
...
#undef RECONFIGURE_PORT_TYPE_INFO
    ...
    SOC_IF_ERROR_RETURN(WRITE_EGR_ENABLEr(unit, port + 1, rval64));
    SOC_IF_ERROR_RETURN(WRITE_EGR_ENABLEr(unit, port + 2, rval64));
    SOC_IF_ERROR_RETURN(WRITE_EGR_ENABLEr(unit, port + 3, rval64));
    ...
    COMPILER_64_ZERO(rval64);
    soc_reg64_field32_set(unit, EGR_ENABLEr, &rval64, PRT_ENABLEf, 
                          (value == 1) ? 1 : 0);
    SOC_IF_ERROR_RETURN(WRITE_EGR_ENABLEr(unit, port + 1, rval64));
    SOC_IF_ERROR_RETURN(WRITE_EGR_ENABLEr(unit, port + 2, rval64));
    SOC_IF_ERROR_RETURN(WRITE_EGR_ENABLEr(unit, port + 3, rval64));
                                               ...
                                               soc_profile_mem_t *prof)
{
    ...
    memset(pri_map, 0, sizeof(ing_pri_cng_map_entry_t) * 16);
    memset(&phb, 0, sizeof(phb));
...
/* Helper function for setting fields from the LPORT table.
   Assumes locks are taken outside. */
#ifdef INCLUDE_L3
int
_bcm_tr2_lport_field_set(int unit, bcm_gport_t wlan_port_id, soc_field_t field,
                         int value)
{
...
int
_bcm_tr2_lport_field_get(int unit, bcm_gport_t wlan_port_id, 
                         soc_field_t field, int *value)
{
