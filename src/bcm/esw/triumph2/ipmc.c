/*
 * $Id: ipmc.c 1.29.6.5 Broadcom SDK $
                            ...
                            bcm_port_t port, int count)
{
    ...
    BCM_IF_ERROR_RETURN(soc_mem_read(unit, L3_IPMCm, MEM_BLOCK_ANY,
                                     ipmc_id, entry));
    num_copy = soc_mem_field32_get(unit, L3_IPMCm, entry, 
                                    EXT_Q_NUM_COPIESf);
                                     ...
                                     int *q_num)
{
    ...
    for (i = 0; i < 2; i++) {     
        if ((queue_id = soc_MMU_EXT_MC_QUEUE_LIST1m_field32_get(unit, 
                                    &qlist1_entry, base_qid[i])) != 0) {    
            ptr = soc_MMU_EXT_MC_QUEUE_LIST0m_field32_get(unit, &qlist1_entry, 
                                                          repl_ptr[i]);
            type = soc_MMU_EXT_MC_QUEUE_LIST0m_field32_get(unit, &qlist1_entry, 
                                                           ptr_type[i]);
            copy = soc_MMU_EXT_MC_QUEUE_LIST0m_field32_get(unit, &qlist1_entry, 
                                                           l2_copy[i]);
            buf_type = soc_MMU_EXT_MC_QUEUE_LIST0m_field32_get(unit, &qlist1_entry, 
                                                           buff_type[i]);
            vlan_last = soc_MMU_EXT_MC_QUEUE_LIST0m_field32_get(unit, &qlist1_entry, 
                                                           ipmc_last[i]);
    ...
    if (next_ptr1 != extq_ptr) {
        if ((rv = _bcm_kt_get_subscriber_queue_entries(unit, next_ptr1, q_repl_ptr, 
                                                       q_vec, q_num)) < 0) {
            goto q_entries_done;                                                                                                
        }                                                    
    } 
    ...
    if (next_ptr2 != extq_ptr) {
        if ((rv = _bcm_kt_get_subscriber_queue_entries(unit, next_ptr2, q_repl_ptr, 
                                                       q_vec, q_num)) < 0) {
            goto q_entries_done;                                                                                                        
        }                                                
    }    
...
STATIC int
_kt_extq_repl_next_free_ptr(int unit)
{
    int                 ix, bit;
    SHR_BITDCL          not_ptrs;
                        ...
                        int *extq_index, int *count)
{
    ...
    for (i = 0; i < node_max; i++) {
        ...
        cur_node->index = _kt_extq_repl_next_free_ptr(unit);
        if (cur_node->index < 0) {
            return BCM_E_RESOURCE;
        }
    ...
    for (i = 0; i < IPMC_EXTQ_TOTAL(unit); i++) {
        if (q_repl_ptr[i] != 0) {
            if (q_count < 2) {
                ...
                soc_MMU_EXT_MC_QUEUE_LIST1m_field32_set(unit, &qlist1_entry, 
                                                        l2_copy[q_count], copy);
                soc_MMU_EXT_MC_QUEUE_LIST0m_field32_set(unit, &qlist1_entry, 
                                                        buff_type[q_count], buf_type);
                soc_MMU_EXT_MC_QUEUE_LIST0m_field32_set(unit, &qlist1_entry, 
                                                        ipmc_last[q_count], vlan_last);                
                soc_MMU_EXT_MC_QUEUE_LIST1m_field32_set(unit, &qlist1_entry, 
