/*
 * $Id: oam.c 1.79.2.3 Broadcom SDK $
 * 
        ...
        ++group_index)
    {
        ...
        if (maid_reduction_valid || ma_state_valid)
        {
            ...
            if (SOC_WARM_BOOT_SCACHE_IS_LIMITED(unit) || (stable_size == 0)) {
                ...
                /* Use stored values */
                memcpy(group_p->name, group_names_p, BCM_OAM_GROUP_NAME_LENGTH);
    ...
    bcm_oam_endpoint_info_t *endpoint_info)
{
    ...
    {
        /* Other types not supported in tr2x */
    ...
    if (endpoint_info->flags & BCM_OAM_ENDPOINT_WITH_ID)
    {
		...
		for (chk_ep_index = 0; chk_ep_index < oam_info_p->endpoint_count;
				++chk_ep_index) {
			if (!replace && oam_info_p->endpoints[chk_ep_index].in_use && !is_remote &&
    ...
    else
    {
		...
		for (endpoint_index = 0; endpoint_index < oam_info_p->endpoint_count;
				++endpoint_index) {
			if (oam_info_p->endpoints[endpoint_index].in_use && !is_remote &&
    ...
    quantization_index =
        _bcm_tr2x_oam_quantize_ccm_period(endpoint_info->ccm_period);
...
int _bcm_tr2x_oam_sync(int unit)
{
        ...
        ++group_index)
    {
        ...
        if (group_p->in_use)
        {
            memcpy(group_names_p, group_p->name, BCM_OAM_GROUP_NAME_LENGTH);
