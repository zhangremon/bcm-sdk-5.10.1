/*
 * $Id: failover.c 1.12.64.2 Broadcom SDK $
...
_bcm_tr2_failover_reinit(int unit)
{
    ...
    for (idx = index_min; idx <= index_max; idx++) {
        ...
        if (soc_INITIAL_PROT_GROUP_TABLEm_field32_get(unit, &prot_grp_entry, STATUSf) != 0) {
...
STATIC int
_bcm_tr2_failover_id_validate (int unit, bcm_failover_t failover_id)
{
    int num_entry;
...
bcm_tr2_failover_create(int unit, uint32 flags, bcm_failover_t *failover_id)
{
    ...
    if ((flags & BCM_FAILOVER_WITH_ID) || (flags & BCM_FAILOVER_REPLACE )) {
         rv = _bcm_tr2_failover_id_validate ( unit, *failover_id );
         ...
         } else if (flags & BCM_FAILOVER_REPLACE) {
              if (!_BCM_FAILOVER_PROT_GROUP_MAP_USED_GET(unit, *failover_id)) {
    ...
    if (BCM_SUCCESS(rv)) {
         ...
         soc_mem_field32_set(unit, INITIAL_PROT_GROUP_TABLEm, &prot_group_entry, STATUSf, 0);
...
bcm_tr2_failover_set(int unit, bcm_failover_t failover_id, int value)
{
    ...
    BCM_IF_ERROR_RETURN( _bcm_tr2_failover_id_validate ( unit, failover_id ));
    if (!_BCM_FAILOVER_PROT_GROUP_MAP_USED_GET(unit, failover_id)) {
         return BCM_E_NOT_FOUND;
    }
    if ((value < 0) || (value > 1)) {
...
_bcm_tr2_failover_sw_dump(int unit)
{
    ...
    soc_cm_print("Protection Group usage bitmap:\n");
