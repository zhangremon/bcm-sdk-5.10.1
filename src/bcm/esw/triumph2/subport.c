/*
 * $Id: subport.c 1.21 Broadcom SDK $
...
cleanup:
    ...
    for (;priority >= 0; priority--) {
        rv = READ_ING_DVP_TABLEm(unit, MEM_BLOCK_ALL, (vp_base+priority), &dvp);
   ...
   /* *********************************************************** */
    ...
    if (soc_feature(unit, soc_feature_subport_enhanced)) {
       ...
       nh_index = soc_ING_DVP_TABLEm_field32_get(unit, &dvp, NEXT_HOP_INDEXf);
        ...
        soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh, VC_AND_SWAP_INDEXf, config->pkt_vlan);
...
bcm_tr2_subport_port_delete(int unit, bcm_gport_t port)
{
    ...
    modid = (BCM_GPORT_SUBPORT_PORT_GET(port) >> 12) & 0xf;
    if (modid != my_modid) {
        return BCM_E_PORT;
    }
                         ...
                         bcm_subport_config_t *config)
{
    ...
    if (soc_feature(unit, soc_feature_subport_enhanced)) {
        ...
        BCM_IF_ERROR_RETURN(rv);
        ovid = soc_mem_field32_get(unit, EGR_L3_NEXT_HOPm, &egr_nh, OVIDf);
        config->pkt_vlan = soc_mem_field32_get(unit, EGR_L3_NEXT_HOPm, &egr_nh, VC_AND_SWAP_INDEXf);
    }
                                          ...
                                          int fs_idx)
{
    ...
    if (BCM_GPORT_IS_SUBPORT_PORT(port)) {
        rv = _bcm_esw_subport_port_flex_stat_index_set(unit, port, fs_idx);
    } else if (BCM_GPORT_IS_SUBPORT_GROUP(port)) {
        rv = _bcm_esw_subport_group_flex_stat_index_set(unit, port, fs_idx);
    }
