/*
 * $Id: policer.c 1.6.6.7 Broadcom SDK $
...
static bcm_policer_global_meter_action_bookkeep_t
          global_meter_action_bookkeep[BCM_MAX_NUM_UNITS]\
                                 [BCM_POLICER_GLOBAL_METER_MAX_ACTIONS];
    ...
    switch (mode_type_v) {
       ...
       case compressed_mode:
       {
           ...
           if ((pkt_attr_bits_v.cng != 0) || (pkt_attr_bits_v.int_pri)) {
               for (index = 0; index < BCM_SVC_METER_MAP_SIZE_64; index++) {
                   soc_mem_write(unit, 
    ...
    uint32 *total_bits)
{
    ...
    if ((pkt_attr_bits_v->cng != 0) || (pkt_attr_bits_v->int_pri)) {
        /* Sanity Check */
        if (soc_mem_index_count(unit,ING_SVM_PRI_CNG_MAPm) != 
            sizeof(compressed_pri_cnf_attr_map_t) * BCM_SVC_METER_MAP_SIZE_64) {

            POLICER_VVERB(("%s : Invalid Table index for  \
                                 ING_SVM_PRI_CNG_MAP\n", FUNCTION_NAME()));
            return BCM_E_INTERNAL;
        }
        for (index = 0; index < BCM_SVC_METER_MAP_SIZE_64; index++) {
    ...
    if ((pkt_attr_bits_v->vlan_format != 0) || (pkt_attr_bits_v->outer_dot1p) ||
                                              (pkt_attr_bits_v->inner_dot1p)) {
        /* Sanity Check */
        if (soc_mem_index_count(unit,ING_SVM_PKT_PRI_MAPm) != 
               sizeof(compressed_pkt_pri_attr_map_t) * 
                                                BCM_SVC_METER_MAP_SIZE_256) {
            POLICER_VVERB(("%s : Invalid Table index for  \
                                 ING_SVM_PKT_PRI_MAP\n", FUNCTION_NAME()));
            return BCM_E_INTERNAL;
        }
    ...
    if (pkt_attr_bits_v->tos != 0){
        /* Sanity Check */
        if (soc_mem_index_count(unit,ING_SVM_TOS_MAPm) != 
                sizeof(compressed_tos_attr_map_t) * 
                                               BCM_SVC_METER_MAP_SIZE_256) {
            POLICER_VVERB(("%s : Invalid Table index for  \
                                 ING_SVM_TOS_MAP\n", FUNCTION_NAME()));
            return BCM_E_INTERNAL;
    ...
    if ((pkt_attr_bits_v->pkt_resolution != 0) || (pkt_attr_bits_v->svp_type) ||
                                                 (pkt_attr_bits_v->drop))   {
        /* Sanity Check */
        if (soc_mem_index_count(unit,ING_SVM_PKT_RES_MAPm) != 
               sizeof(compressed_pkt_res_attr_map_t) * 
                                                  BCM_SVC_METER_MAP_SIZE_256) {
            POLICER_VVERB(("%s : Invalid Table index for  \
                                 ING_SVM_PKT_RES_MAP\n", FUNCTION_NAME()));
            return BCM_E_INTERNAL;
    ...
    uint32                       *total_bits)
{
    ...
    for (index = 1;index < BCM_POLICER_SVC_METER_MAX_MODE; index++) {
        if (_bcm_policer_svc_meter_get_mode_info(unit, index, &mode_info) == 
            ...
            if ((udf_attr->udf_pkt_attr_bits_v.udf0 == udf_pkt_attr.\
                                        udf_pkt_attr_bits_v.udf0) && 
                                              ...
                                              bcm_policer_t *pid)
{
    ...
    size_of_pool =  SOC_INFO(unit).global_meter_size_of_pool;
    if (index > 0) {
...
bcm_esw_global_meter_init(int unit)
{
    ...
    int mode = 0;
    uint32 num_pools = 0, size_pool = 0, total_size = 0;
    ...
    num_pools =  SOC_INFO(unit).global_meter_pools;
    size_pool =  SOC_INFO(unit).global_meter_size_of_pool;
    total_size = num_pools * size_pool;   
    if ((total_size) > (soc_mem_index_max(unit, SVM_METER_TABLEm) + 1))
    {
        POLICER_ERR(("%s : Number global meter exceeding \
    ...
    /* Initialize meter action  bookkeeping data structure */
    for (index = 0; index < total_size; index++) {
        sal_memset(&global_meter_action_bookkeep[unit][index],
                     0,  sizeof(bcm_policer_global_meter_action_bookkeep_t));
    }
    ...
    uint32 param0)
{
    ...
    switch (action) { 
       ...
       case bcmPolicerActionGpCngNew :
           ...
           green_action[0] = green_action[0] | 
                   (1 << BCM_POLICER_GLOBAL_METER_ACTION_CHANGE_CNG_BIT_POS);
           break;       
       case bcmPolicerActionYpCngNew :
           SHR_BITCLR_RANGE(yellow_action, 
                        BCM_POLICER_GLOBAL_METER_ACTION_CHANGE_CNG_BIT_POS, 2);
           yellow_action[0] = yellow_action[0] | 
                   (2 << BCM_POLICER_GLOBAL_METER_ACTION_CHANGE_CNG_BIT_POS);
           break;       
       case bcmPolicerActionRpCngNew :
           SHR_BITCLR_RANGE(red_action, 
                        BCM_POLICER_GLOBAL_METER_ACTION_CHANGE_CNG_BIT_POS, 2);
           red_action[0] = red_action[0] | 
                   (3 << BCM_POLICER_GLOBAL_METER_ACTION_CHANGE_CNG_BIT_POS);
           break;       
                                                  ...
                                                  uint32 policer_index) 
{
    ...
    for (index = 0;index < numbers; index++) {
        ...
        rv = _bcm_gloabl_meter_unreserve_bloc_horizontally(unit, pool, index); 
        if (!BCM_SUCCESS(rv)) {
            POLICER_VVERB(("%s : Unable to free policer handle \n", \
                                                       FUNCTION_NAME()));
            return BCM_E_INTERNAL;
                                             ...
                                             uint8 *pid_offset) 
{
    ...
    if (direction == GLOBAL_METER_ALLOC_VERTICAL) {
        ...
        for (index = 1; index < numbers; index++) {
            rv = shr_aidxres_list_reserve_block(
                   meter_alloc_list_handle[unit][pool + pid_offset[index]], 
                                                pol_index, 1);
            if (!BCM_SUCCESS(rv)) {
            ...
            rv = _bcm_global_meter_reserve_bloc_horizontally(unit,
                             (pool+(pid_offset[index])), pol_index);
            if (!BCM_SUCCESS(rv)) {
                rv = shr_aidxres_list_free(
                        meter_alloc_list_handle[unit][pool], pol_index);
                if (!BCM_SUCCESS(rv)) {
                  ...
                  _global_meter_policer_control_t *policer_control)
{
    ...
    uint32 coupling = 0, refresh_mode = 0, policy_index = 0;
    uint32  refreshmax = 0; 
    int index = 0;
    ...
    mode_modifier = (pol_cfg->flags & BCM_POLICER_COLOR_BLIND) >> 2;
    ...
    if (SOC_MEM_FIELD_VALID(unit, SVM_METER_TABLEm, REFRESH_MODEf)) {
        soc_SVM_METER_TABLEm_field_set(unit, &data, REFRESH_MODEf, 
                                                        &refresh_mode);
    }
    if (SOC_MEM_FIELD_VALID(unit, SVM_METER_TABLEm, COUPLING_FLAGf)) {
                   ...
                   int *new_index) 
{
    ...
    int rv = BCM_E_NONE;
    int offset1 = 0, offset2 = 0, pool1 = 0, pool2 = 0; 
    int new_offset = 0; 
    int size_pool = 0, num_pools = 0;
    ...
    offset2 = policer_id & offset_mask;
                         ...
                         _global_meter_policer_control_t **policer_p)
{
    ...
    int policer_index = 0;
    svm_meter_table_entry_t meter_entry;
    uint32 coupling_flag = 0, index = 0, index_max = 0;
    int rv = BCM_E_NONE;
    int hash_max = 0; 
    int offset_mask = 0;
    int size_pool = 0, num_pools = 0;
                ...
                if (global_meter_pl->direction == 
                    ...
                    /* read coupling flag */
                        ...
                        _bcm_esw_get_policer_table_index(unit, 
                                        global_meter_pl->pid, &policer_index); 
                        rv = READ_SVM_METER_TABLEm(unit, MEM_BLOCK_ANY, 
                                                 policer_index, &meter_entry);
                        if (BCM_SUCCESS(rv)) {
                   ...
                   bcm_policer_config_t *pol_cfg)
{
   ...
   /* we come here from bcm_esw_policer_get,
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        if (BCM_FAILURE(rv)) {
            POLICER_ERR(("%s : Unable to read policer config from hw \n", \
                                    FUNCTION_NAME(), policer_id));
            return (rv);
        }
    }
   ...
   /* we come here from bcm_esw_policer_get,
    ...
    if (BCM_SUCCESS(rv)) {
       /* Add code to write this to HW */
        ...
        if (BCM_FAILURE(rv)) {
            POLICER_ERR(("%s : Unable to write policer config to hw \n", \
                                    FUNCTION_NAME(), policer_id));
            return (rv);
        }
                               ...
                               bcm_policer_svc_meter_attr_t *mode_attr) 
{
    ...
    switch (mode) {
        ...
        {
            *npolicers = 1;
            rv =  _bcm_esw_policer_set_offset_table_map_to_a_value(19, 0, 
        ...
        case bcmPolicerGroupModeTrafficType:
        {
            *npolicers = 3;
            mode_attr->uncompressed_attr_selectors_v.\
               ...
               BCM_POLICER_SVC_METER_UNCOMPRESSED_USE_PKT_RESOLUTION_ATTR_BITS;
            table_offset[0] = L2UC_PKT;
            table_offset[1] = KNOWN_L3UC_PKT;
            table_offset[2] = UNKNOWN_L3UC_PKT;
            table_offset[3] = KNOWN_L2MC_PKT;
            table_offset[4] = UNKNOWN_L2MC_PKT;
            table_offset[5] = L2BC_PKT;
            value[3] = 1;
            value[4] = 1;
            value[5] = 2;
        ...
        case bcmPolicerGroupModeDlfAll:
        {
            *npolicers = 2;
            mode_attr->uncompressed_attr_selectors_v.\
                     uncompressed_attr_bits_selector =
               BCM_POLICER_SVC_METER_UNCOMPRESSED_USE_PKT_RESOLUTION_ATTR_BITS;
            rv =  _bcm_esw_policer_set_offset_table_map_to_a_value(19, 1, 
                      &mode_attr->uncompressed_attr_selectors_v.offset_map[0]); 
            BCM_IF_ERROR_RETURN(rv);
            table_offset[0] = L2DLF_PKT;
            value[0] = 0;
        ...
        case bcmPolicerGroupModeTyped:
        {
            *npolicers = 4;
            mode_attr->uncompressed_attr_selectors_v.\
                      uncompressed_attr_bits_selector =
               BCM_POLICER_SVC_METER_UNCOMPRESSED_USE_PKT_RESOLUTION_ATTR_BITS;
            table_offset[0] = UNKNOWN_L3UC_PKT;
            table_offset[1] = KNOWN_L3UC_PKT;
            table_offset[2] = L2UC_PKT;
            table_offset[3] = KNOWN_L2MC_PKT;
            table_offset[4] = UNKNOWN_L2MC_PKT;
            table_offset[5] = L2BC_PKT;
            value[1] = 1;
        ...
        case bcmPolicerGroupModeTypedAll:
        {
            *npolicers = 5;
            mode_attr->uncompressed_attr_selectors_v.\
                      uncompressed_attr_bits_selector =
               BCM_POLICER_SVC_METER_UNCOMPRESSED_USE_PKT_RESOLUTION_ATTR_BITS;
            rv =  _bcm_esw_policer_set_offset_table_map_to_a_value(19, 4, 
                      &mode_attr->uncompressed_attr_selectors_v.offset_map[0]); 
            BCM_IF_ERROR_RETURN(rv);
            table_offset[0] = UNKNOWN_L3UC_PKT;
            table_offset[1] = KNOWN_L3UC_PKT;
        ...
        case bcmPolicerGroupModeSingleWithControl:
        {
            *npolicers = 2;
            ...
            rv =  _bcm_esw_policer_set_offset_table_map_to_a_value(19, 0, 
                      &mode_attr->uncompressed_attr_selectors_v.offset_map[0]); 
            BCM_IF_ERROR_RETURN(rv);
            table_offset[0] = CONTROL_PKT;
            table_offset[1] = BPDU_PKT;
            value[0] = 1;
            value[1] = 1;
        ...
        case bcmPolicerGroupModeTrafficTypeWithControl:
        {
            *npolicers = 4;
            ...
            table_offset[4] = UNKNOWN_L2MC_PKT;
            table_offset[5] = L2BC_PKT;
            table_offset[6] = CONTROL_PKT;
            table_offset[7] = BPDU_PKT;
            value[3] = 1;
            value[4] = 1;
            value[5] = 2;
            value[6] = 3;
        ...
        case bcmPolicerGroupModeDlfAllWithControl:
        {
            *npolicers = 3;
            mode_attr->uncompressed_attr_selectors_v.\
                      uncompressed_attr_bits_selector =
               BCM_POLICER_SVC_METER_UNCOMPRESSED_USE_PKT_RESOLUTION_ATTR_BITS;
            rv =  _bcm_esw_policer_set_offset_table_map_to_a_value(19, 2, 
                      &mode_attr->uncompressed_attr_selectors_v.offset_map[0]); 
            BCM_IF_ERROR_RETURN(rv);
            table_offset[0] = CONTROL_PKT;
            table_offset[1] = BPDU_PKT;
            table_offset[2] = L2DLF_PKT;
            value[2] = 1;
        ...
        case bcmPolicerGroupModeTypedWithControl:  
        {
            *npolicers = 5;
            ...
            table_offset[1] = BPDU_PKT;
            table_offset[2] = UNKNOWN_L3UC_PKT;
            table_offset[3] = L2UC_PKT;
            table_offset[4] = KNOWN_L3UC_PKT;
            table_offset[5] = KNOWN_L2MC_PKT;
            table_offset[6] = UNKNOWN_L2MC_PKT;
            table_offset[7] = L2BC_PKT;
            value[2] = 1;
            value[3] = 2;
            value[4] = 2;
            value[5] = 3;
        ...
        case bcmPolicerGroupModeTypedAllWithControl:
        {
            *npolicers = 6;
            mode_attr->uncompressed_attr_selectors_v.\
                      uncompressed_attr_bits_selector =
               BCM_POLICER_SVC_METER_UNCOMPRESSED_USE_PKT_RESOLUTION_ATTR_BITS;
            rv =  _bcm_esw_policer_set_offset_table_map_to_a_value(19, 5, 
                      &mode_attr->uncompressed_attr_selectors_v.offset_map[0]); 
            BCM_IF_ERROR_RETURN(rv);
            table_offset[0] = CONTROL_PKT;
            table_offset[1] = BPDU_PKT;
        ...
        /* ******************************************************** */
        /* A set of 8(2^3) policers selected based on Vlan priority */
        ...
        case bcmPolicerGroupModeSvpType:
        {
            *npolicers = 2;
            mode_attr->uncompressed_attr_selectors_v.\
                   uncompressed_attr_bits_selector =
                     BCM_POLICER_SVC_METER_UNCOMPRESSED_USE_SVP_TYPE_ATTR_BITS;
            rv =  _bcm_esw_policer_set_offset_table_map_to_increasing_value(2,
                      &mode_attr->uncompressed_attr_selectors_v.offset_map[0]); 
            BCM_IF_ERROR_RETURN(rv);
            break;
        } 
        ...
        case bcmPolicerGroupModeDlfIntPri:  
        {
            *npolicers = 17;
            ...
            for (offset = 0; offset < 4; offset++) { 
                pkt_res_map[(L2DLF_PKT << 2) + offset] = 1 << 2;
            }
            /* Reset pri_cnf map */
            pri_cnf_map = &mode_attr->compressed_attr_selectors_v.\
                                             compressed_pri_cnf_attr_map_v[0];
            for (map_index = 0; map_index < BCM_SVC_METER_MAP_SIZE_64; 
                                                               map_index++) {
        ...
        case bcmPolicerGroupModeTypedIntPri:
        {
            *npolicers = 20;
            ...
            /* set pkt_resolution map for DLF to 1 */
            for (offset = 0; offset < 4; offset++) { 
                pkt_res_map[(UNKNOWN_L3UC_PKT << 2) + offset] = 1 << 2;
                pkt_res_map[(L2UC_PKT << 2) + offset] = 2 << 2;
                pkt_res_map[(KNOWN_L3UC_PKT << 2) + offset] = 2 << 2;
                pkt_res_map[(KNOWN_L2MC_PKT << 2) + offset] = 3 << 2;
                pkt_res_map[(UNKNOWN_L2MC_PKT << 2) + offset] = 3 << 2;
                pkt_res_map[(L2BC_PKT << 2) + offset] = 4 << 2;
            }
            /* Reset pri_cng map */
            pri_cnf_map = &mode_attr->compressed_attr_selectors_v.\
                                             compressed_pri_cnf_attr_map_v[0];
        ...
        case bcmPolicerGroupModeDlfIntPriWithControl:
        {
            *npolicers = 18;
            ...
            for (offset = 0; offset < 4; offset++) { 
                pkt_res_map[(CONTROL_PKT << 2) + offset] = 1 << 2;
                pkt_res_map[(BPDU_PKT << 2) + offset] = 1 << 2;
                pkt_res_map[(L2DLF_PKT << 2) + offset] = 2 << 2;
            }
            /* Reset pri_cng map */
            pri_cnf_map = &mode_attr->compressed_attr_selectors_v.\
                                             compressed_pri_cnf_attr_map_v[0];
        ...
        case bcmPolicerGroupModeTypedIntPriWithControl:
        {
            *npolicers = 21;
            ...
            for (offset = 0; offset < 4; offset++) { 
                pkt_res_map[(CONTROL_PKT << 2) + offset] = 1 << 2;
                pkt_res_map[(BPDU_PKT << 2) + offset] = 1 << 2;
                pkt_res_map[(UNKNOWN_L3UC_PKT << 2)+ offset] = 2 << 2;
                pkt_res_map[(L2UC_PKT << 2) + offset] = 3 << 2;
                pkt_res_map[(KNOWN_L3UC_PKT << 2) + offset] = 3 << 2;
                pkt_res_map[(KNOWN_L2MC_PKT << 2) + offset] = 4 << 2;
                pkt_res_map[(UNKNOWN_L2MC_PKT << 2) + offset] = 4 << 2;
                pkt_res_map[(L2BC_PKT << 2) + offset] = 5 << 2;
            }
            /* Reset pri_cnf map */
            pri_cnf_map = &mode_attr->compressed_attr_selectors_v.\
                                             compressed_pri_cnf_attr_map_v[0];
                                   ...
                                   int *npolicers)
{
    ...
    offset_mask = SOC_INFO(unit).global_meter_size_of_pool - 1;
                           ...
                           _global_meter_policer_control_t *policer_control) 
{
   ...
   if (offset_mode >= 1) {
       ...
       if (mode == GLOBAL_METER_MODE_CASCADE) { 
           ...
           numbers++;
           for (index =pool + 1; index < num_pools; pool++) {
               pol_index |= index << _shr_popcount(size_pool - 1); 
               rv = soc_mem_read(unit, SVM_METER_TABLEm, MEM_BLOCK_ANY, 
                                                       policer_index, &data);
               if (!BCM_SUCCESS(rv)) {
                   sal_free(policer_control);
...
int _bcm_policer_config_reinit_from_table(int unit, soc_mem_t table,void *data)
{
    ...
    _global_meter_policer_control_t *policer_control = NULL;
    int rv = BCM_E_NONE;
    max_index = soc_mem_index_count(unit, table);
    if (table == PORT_TABm) {
        max_index = BCM_PORT_TAB_MAX_INDEX + 1;
    }
...
int _bcm_esw_global_meter_offset_mode_reinit(int unit)
{
    ...
    for (mode = 1; mode < BCM_POLICER_SVC_METER_MAX_MODE; mode++) { 
        soc_reg64_get(unit, _pkt_attr_sel_key_reg[mode],
                                        REG_PORT_ANY, 0, &selector_key_value);
