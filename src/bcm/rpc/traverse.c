/*
 * $Id: traverse.c 1.5 Broadcom SDK $
...
RLINK_TRAV_THREADED_SERVER
  ...
  server. The non-threaded server required a memory buffer that can
  accomodate the *RPC response* for the entire table. This is likely
  to be larger than the actual table size.
...
RLINK_TRAV_THREAD_STACK
  ...
  Size of the traverse thread stack. Ignored for Ignored for a
  non-threaded server implementation.
  
RLINK_TRAV_THREAD_PRIO

  Priority of the traverse thread. Ignored for Ignored for a
  non-threaded server implementation.
                                   ...
                                   rlink_type_t ty)
{
    ...
    RLINK_DEBUG((DK_VERBOSE, "TRAVERSE server_message %d\n", ty));
    ...
    if (BCM_SUCCESS(rv)) {
        RLINK_DEBUG((DK_VERBOSE, "TRAVERSE server_message sleeping %d\n",
                     rlink_traverse_client_timeout));
                                 ...
                                 uint32 *key)
{
    ...
    RLINK_DEBUG((DK_VERBOSE, "TRAVERSE request_start\n"));
    ...
    RLINK_DEBUG((DK_VERBOSE, "TRAVERSE request id:%x:%x\n",
                 req->c_id, req->s_id));
    bcm_rx_pool_own(req->tx_buf,(void *)__FUNCTION__);
...
bcm_rlink_traverse_reply_get(int unit, bcm_rlink_traverse_data_t *req)
{
    ...
    RLINK_DEBUG((DK_VERBOSE, "TRAVERSE reply_get\n"));
    ASSERT_TRAVC(parent);
    switch (parent->state) {
    case client_start:
                                ...
                                bcm_rlink_traverse_data_t *req)
{
    ...
    RLINK_DEBUG((DK_VERBOSE, "TRAVERSE request_done\n"));
    ASSERT_TRAVC(parent);
    rv = parent->rrv;
    switch (parent->state) {
    case client_start:
...
_bcm_rlink_traverse_client_message(_rlink_travs_t *server, rlink_type_t ty)
{
    ...
    RLINK_DEBUG((DK_VERBOSE, "TRAVERSE client_message waking travs\n"));
#if 0
    ...
    if (ty == RLINK_TYPE_MORE) {
        ...
        RLINK_DEBUG((DK_VERBOSE, "TRAVERSE client_message sleeping travt\n"));
...
bcm_rlink_traverse_reply_check(bcm_rlink_traverse_data_t *data, int size)
{
    ...
    RLINK_DEBUG((DK_VERBOSE, "TRAVERSE reply_check%s\n",
                 data ? "" : " - no data"));
...
bcm_rlink_traverse_reply_done(bcm_rlink_traverse_data_t *data, int rv)
{
    ...
    RLINK_DEBUG((DK_VERBOSE, "TRAVERSE reply_done%s\n",
                 data ? "" : " - no data"));
    if (data) {
...
bcm_rlink_traverse_device_clear(int unit)
{
    ...
    if (BCM_SUCCESS(rv)) {
        RLINK_DEBUG((DK_VERBOSE, "TRAVERSE server_clear waking travt\n"));
        RLINK_TRAVT_WAKE;
    }
...
_bcm_rlink_travs_thread(void *cookie)
{
	...
	/* execute traverse */
	if (cur) {
            RLINK_DEBUG((DK_VERBOSE, "TRAVERSE begin\n"));
	    cur->handler(&cur->reply);
            ...
            RLINK_TRAVS_UNLOCK;
            RLINK_DEBUG((DK_VERBOSE, "TRAVERSE end\n"));
            cur->magic = ~cur->magic;
            sal_free(cur);
            cur = NULL;
        } else {
            RLINK_DEBUG((DK_VERBOSE, "TRAVERSE travs_thread sleeping\n"));
            RLINK_TRAVT_SLEEP;
        }
    }
...
_bcm_rlink_travs_thread_signal(void)
{
    ...
    RLINK_DEBUG((DK_VERBOSE, "TRAVERSE signal waking travt\n"));
    RLINK_TRAVT_WAKE;

    /* wait for traverse server to signal back */
    RLINK_DEBUG((DK_VERBOSE, "TRAVERSE signal sleeping travs\n"));
    rv = RLINK_TRAVS_SLEEP;
    RLINK_DEBUG((DK_VERBOSE, "TRAVERSE signal woken up travs\n"));

    return rv < 0 ? BCM_E_TIMEOUT : BCM_E_NONE;
}

STATIC int
_bcm_rlink_traverse_run(void)
{
...
_bcm_rlink_set_current_len(int len)
{
    if (len > _rlink_trav_info[_rlink_trav_info_offset].len) {
        /* .len may not be entirely accurate, but should be at least an
           overestimate */
        RLINK_DEBUG((DK_WARN, "TRAVERSE set_current_len len=%d actual=%d\n",
...
_bcm_rlink_traverse_run(void)
{
    bcm_rlink_traverse_data_t tdata;
    bcm_rlink_traverse_data_t *tdatap = &tdata;

    RLINK_DEBUG((DK_VERBOSE, "TRAVERSE traverse run sync begin\n"));
    ...
    /* use the traverse buffer instead of the reply buffer */
    tdata.tx_buf = tdata.tx_ptr = _rlink_trav_buffer;

    /* init response info */
    _rlink_trav_info_offset = 0;
    _rlink_trav_info[_rlink_trav_info_offset].num = 0;
    _rlink_trav_info[_rlink_trav_info_offset].len = 0;
    _rlink_trav_info[_rlink_trav_info_offset].buf = tdata.tx_ptr;
    ...
    RLINK_DEBUG((DK_VERBOSE, "TRAVERSE traverse run sync end\n"));
    rv = BCM_E_NONE;
}
                           ...
                           uint8 *rx_buf, int len)
{
    ...
    if (tx_buf) {
        bcm_rx_pool_own(tx_buf, (void *)__FUNCTION__);
        bcm_rx_pool_own(rx_buf, (void *)__FUNCTION__);
        ...
        if (server) {
            ...
            if (BCM_SUCCESS(rv)) {
                assert(actual > 0);
                RLINK_DEBUG((DK_VERBOSE, "TRAVERSE traverse_request %d\n",
                             actual));
        ...
        RLINK_DEBUG((DK_WARN,
                     "TRAVERSE traverse_request could not alloc tx\n"));
        atp_rx_free(rx_buf, rx_pkt);
        rv = BCM_E_MEMORY;
    }
...
_bcm_rlink_trav_msg_start(bcm_rlink_traverse_data_t *data)
{
    ...
    if (trav) {
        ...
        RLINK_DEBUG((DK_VERBOSE, "TRAVERSE msg_start: cur:%d\n",
                     duration));
        if (rlink_traverse_transaction_timeout <= 0 ||
            ...
            RLINK_DEBUG((DK_WARN, "TRAVERSE msg_start: busy\n"));
            _bcm_rlink_trav_prep_error(data, BCM_E_BUSY);
            return BCM_E_NONE;
        } else {
            RLINK_DEBUG((DK_WARN, "TRAVERSE msg_start: old transaction\n"));
...
_bcm_rlink_travs_get(bcm_rlink_traverse_data_t *data, _rlink_travs_t **tp)
{
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        *tp = trav;
        ...
        RLINK_DEBUG((DK_ERR, "TRAVERSE could not find id %x:%x\n",
                     data->c_id, data->s_id));
        _bcm_rlink_trav_prep_error(data, BCM_E_NOT_FOUND);
    }
...
_bcm_rlink_trav_msg_next(bcm_rlink_traverse_data_t *data)
{
    ...
    if (BCM_SUCCESS(rv)) {
        RLINK_DEBUG((DK_VERBOSE, "TRAVERSE msg_next waking travt id=%x:%x\n",
                     data->c_id, data->s_id));
#if RLINK_TRAV_THREADED_SERVER
        _bcm_rlink_travs_thread_signal();
#else
        ...
        RLINK_DEBUG((DK_VERBOSE, "TRAVERSE msg_next id %x:%x not found\n",
                     data->c_id, data->s_id));
    }
...
_bcm_rlink_trav_msg_quit(bcm_rlink_traverse_data_t *data)
{
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        RLINK_DEBUG((DK_VERBOSE, "TRAVERSE msg_quit id %x:%x not found\n",
                     data->c_id, data->s_id));
    }
...
_bcm_rlink_travc_get(bcm_rlink_traverse_data_t *data, _rlink_travc_t **tp)
{
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        *tp = trav;
        ...
        RLINK_DEBUG((DK_ERR, "TRAVERSE could not find c_id %x\n",
                     data->c_id));
    }
...
_bcm_rlink_trav_msg_error(bcm_rlink_traverse_data_t *data)
{
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        trav->state = client_done;
        RLINK_DEBUG((DK_VERBOSE, "TRAVERSE msg_err (%d) waking\n",
                     trav->rrv));
...
_bcm_rlink_trav_msg_more(bcm_rlink_traverse_data_t *data)
{
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        RLINK_DEBUG((DK_VERBOSE, "TRAVERSE msg_more (%d) waking\n",
                     trav->rrv));
...
_bcm_rlink_trav_msg_done(bcm_rlink_traverse_data_t *data)
{
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        BCM_UNPACK_U32(trav->data->rx_ptr, trav->rrv);
        BCM_UNPACK_U32(trav->data->rx_ptr, trav->num);
        RLINK_DEBUG((DK_VERBOSE, "TRAVERSE msg_done (%d) waking\n",
                     trav->rrv));
