/*
 * $Id: vlan.c 1.217.2.1 Broadcom SDK $
...
#define _BCM5389_A1_DROP_VLAN_DOMAIN 4095
...
int _bcm_robo_vlan_vtcfp_delete_all(int unit, int vt_mode){
    ...
    VT_CFP_LOCK;
    soc_cm_debug(DK_VLAN, "%s,%d,Start to clear vt_mode=%d!\n", 
...
_bcm_robo_egress_vt_init(int unit){
    ...
    if (list == NULL){
        /* init the bitmap */
        evt_size = SOC_MEM_SIZE(unit, EGRESS_VID_REMARKm);
...
_bcm_robo_vlan_init(int unit, bcm_vlan_data_t *vd)
{
    ...
    if (SOC_IS_ROBO5389(unit)){
        SOC_IF_ERROR_RETURN(REG_READ_MODEL_IDr(unit, &model));
        SOC_IF_ERROR_RETURN(REG_READ_CHIP_REVIDr(unit, &rev));
    ...
    if (bcm5389a1) {
        BCM_IF_ERROR_RETURN(DRV_MEM_WRITE
                        (unit, DRV_MEM_VLAN, _BCM5389_A1_DROP_VLAN_DOMAIN, 1, 
                        (uint32 *)&ve_drop));
    }
                                  ...
                                  bcm_vlan_action_set_t *action)
{
    ...
    if (!tag_action_map) {
        /* no any action really be configured */
        return BCM_E_PARAM;
    }
                                  ...
                                  bcm_vlan_t vlan_id)
{
    ...
    int rv = BCM_E_NONE;
    int rv2 = BCM_E_NONE; /* return value that no need to check or return. */
    _robo_vlan_translate_action_param_t vt_act_param;
    ...
    for (i = 0; i < vt_act_cnt; i++) {
        ...
        if (action_current.new_outer_vlan == vlan_id) {
            ...
            if (rv < 0) {
                ...
                rv2 = _robo_vlan_translate_action_add(unit, &vt_act_param, &action_current);
                soc_cm_debug(DK_ERR, 
                    "Updating vlan translation action failed\n");
                return rv;
            }
            ...
            void *user_data)
{
    ...
    tbl_size = SOC_MEM_SIZE(unit, EGRESS_VID_REMARKm);
    ...
    switch (type) {
      ...
      case bcmVlanTranslate:
        ...
        if (soc_feature(unit, soc_feature_vlan_translation)) {
            ...
            if (SOC_IS_ROBO53242(unit) || SOC_IS_ROBO53262(unit) || 
                    SOC_IS_ROBO53115(unit) || SOC_IS_TBX(unit) ||
              ...
              bcm_vlan_control_port_t type, int arg)
{
    ...
    switch (type) {
        ...
        case bcmVlanPortUseInnerPri:
            BCM_IF_ERROR_RETURN(
                        _bcm_robo_port_config_set(unit, port, 
                             _bcmPortUseInnerPri, (arg ? TRUE :FALSE)));
            break;
        case bcmVlanPortUseOuterPri:
            BCM_IF_ERROR_RETURN(
                        _bcm_robo_port_config_set(unit, port, 
                             _bcmPortUseOuterPri, (arg ? TRUE :FALSE)));
            break;
...
{   
    ...
    if (SOC_IS_TBX(unit) && soc_feature(unit, soc_feature_vlan_action)) {
        ...
        BCM_IF_ERROR_RETURN(_robo_vlan_translate_action_add(unit, &vt_act_param, action));
        return BCM_E_NONE;
    }
#endif
                                           ...
                                           void *user_data)
{
    ...
    if (SOC_IS_ROBO53242(unit)||SOC_IS_ROBO53262(unit)) {
        ...
        index_min = soc_robo_mem_index_min(unit, PROTOCOL2VLANm);
        index_max = soc_robo_mem_index_max(unit, PROTOCOL2VLANm);
                            ...
                            void *user_data)
{
    ...
    if (SOC_IS_ROBO53242(unit)||SOC_IS_ROBO53262(unit)) {
        ...
        index_min = soc_robo_mem_index_min(unit, VLAN2VLANm);
        index_max = soc_robo_mem_index_max(unit, VLAN2VLANm);
        for (i = index_min; i <= index_max; i++) {
                            ...
                            void *user_data)
{
    ...
    soc_cm_debug(DK_VLAN, "BCM_API:%s...\n",FUNCTION_NAME());
