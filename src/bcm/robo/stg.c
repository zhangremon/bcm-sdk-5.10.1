...
/*
 * $Id: stg.c 1.50.44.3 Broadcom SDK $
...
static int bcm5324_stg_status[BCM_MAX_NUM_UNITS][SOC_MAX_NUM_PORTS];
                ...
                bcm_port_t port, int stp_state)
{
    ...
    if (SOC_IS_ROBO5324(unit)) {
        ...
        /* 
         * BCM5324 enable PASS_BPDU_REVMCASTf in register 0x2 0x40 bit 0.
         * All STP status except FORWARDING should set to Disable 
         */
        switch(stp_state) {
            case BCM_STG_STP_DISABLE:
            case BCM_STG_STP_BLOCK:
...
#else
        /* 
         * The Learning and Listening states of all BCM5324 ports are identical.
         * Keep a S/W value for BCM5324 to return proper stp_state
         */
        switch(stp_state) {
            case BCM_STG_STP_DISABLE:
                port_status = DRV_PORTST_DISABLE;
        ...
        bcm5324_stg_status[unit][port] = stp_state;
    } else {
        switch(stp_state) {
            case BCM_STG_STP_DISABLE:
                port_status = DRV_PORTST_DISABLE;
                ...
                bcm_port_t port, int *stp_state)
{
    ...
    *stp_state = bcm_shadow_stg_status[unit][port];
    ...
    if (SOC_IS_ROBO5324(unit)) {
         ...
         * Keep a S/W value for BCM5324 to return proper stp_state
         */
        *stp_state = bcm5324_stg_status[unit][port];
    } else {
   ...
   /*
    ...
    if (soc_feature(unit, soc_feature_stg)) {
        ...
        if (SOC_IS_ROBO5397(unit)) {
             ...
             * Spanning tree state of WAN port should be set in default stg table.
             * So that packets CPU sends to WAN port can be forwarded.
             */
...
int bcm_robo_stg_init(int unit)
{
    ...
    if (SOC_IS_ROBO5324(unit)) {
        ...
        PBMP_ALL_ITER(unit, port) {
            DRV_MSTP_PORT_SET
                    (unit, 1, port, DRV_PORTST_FORWARD);
            bcm5324_stg_status[unit][port] = BCM_STG_STP_FORWARD;
        }
    }
