/*
 * $Id: port.c 1.296.2.8 Broadcom SDK $
    ...
    BCM_PBMP_ITER(PBMP_ALL(unit), port){
        ...
        if (IS_GE_PORT(unit, port)){
            ...
            if (phy_apd_war_info->auto_pd_war_lock == NULL) {
                ...
                soc_cm_print("%s, Memory issue on port%d AutoPD WAR\n",
                        FUNCTION_NAME(), port);
            }
        ...
        uint32 sw_flags, uint32 *soc_flags)
{
    ...
    if (temp_val) {
        soc_cm_debug(DK_WARN, "Invalid LEARN flags at 0x%x!\n", temp_val);
        return (BCM_E_PARAM);
    }
...
bcm_robo_port_init(int unit)
{
    ...
    soc_cm_debug(DK_PORT, "%s,Unit %d Init...", FUNCTION_NAME(), unit);
    /*
...
bcm_robo_port_untagged_priority_get(int unit, bcm_port_t port, int *priority)
{
    ...
    vid = BCM_VLAN_CTRL_ID(outer_tag);
    pri = BCM_VLAN_CTRL_PRIO(outer_tag);
    cfi = BCM_VLAN_CTRL_CFI(outer_tag);
...
#define DSCP_CODE_POINT_MAX (DSCP_CODE_POINT_CNT - 1)
    ...
    if (soc_feature(unit, soc_feature_dscp)) {
        ...
        if (prio < 0) {
            ...
            bcm_robo_port_untagged_priority_get(unit, port, &prio);
        } 
                               ...
                               int pkt_dscp)
{
    ...
    soc_cm_debug(DK_PORT, "BCM API : bcm_robo_port_dscp_unmap_set()..\n");
                                    ...
                                    int *pkt_dscp)
{
    ...
    soc_cm_debug(DK_PORT, "BCM API : bcm_robo_port_dscp_unmap_get()..\n");
...
bcm_robo_port_speed_set(int unit, bcm_port_t port, int speed)
{
    ...
    soc_cm_debug(DK_PORT, "BCM API:%s,p=%d,spd=%d\n",
...
_bcm_robo_port_update(int unit, bcm_port_t port, int link)
{
    ...
    if (BCM_FAILURE(rv)) {
        SOC_DEBUG_PRINT((DK_WARN, "u=%d p=%d DRV_PORT_SW_MAC_UPDATE rv=%d\n",
                            unit, port, rv));
    }
     ...
     *          - bcm5324/5348/5347/53242/53262/5396/5389/5397/5398/5395/
     *              53115/53118/53101/53125/53128
    ...
    if (SOC_IS_ROBO5324(unit) || SOC_IS_ROBO5324_A1(unit) || 
            SOC_IS_ROBO5396(unit) || SOC_IS_ROBO5389(unit) || 
                    ...
                    uint32 phy_reg_addr, uint32 * phy_data)
{
    ...
    if (flags & BCM_PORT_PHY_CLAUSE45) {
        ...
        rv = DRV_MIIM_READ(unit, phy_id, phy_reg, &phy_rd_data);
    }
                    ...
                    uint32 phy_reg_addr, uint32 phy_data)
{
    ...
    if (flags & BCM_PORT_PHY_CLAUSE45) {
        ...
        rv = DRV_MIIM_WRITE(unit, phy_id, phy_reg, phy_wr_data);
    }
                         ...
                         uint32 phy_reg_addr, uint32 phy_data, uint32 phy_mask)
{
    ...
    soc_cm_debug(DK_PORT, "BCM API : bcm_robo_port_phy_modify()..\n");
    BCM_IF_ERROR_RETURN(
        _bcm_robo_port_gport_validate(unit, port, &port));
    if (flags & BCM_PORT_PHY_NOMAP) {
        phy_id = port;
    ...
    bcm_phy_config_t *  config)
{
    ...
    rv = DRV_PORT_GET(unit, port, drv_type,(uint32 *)config);
    ...
    bcm_phy_config_t *  config)
{
    ...
    rv = DRV_PORT_SET(unit, t_pbm,drv_type, (uint32)config);
    return (rv);
}
...
bcm_robo_port_loopback_set(int unit, bcm_port_t port, int loopback)
{
    ...
    soc_timeout_t to;
#endif /* BCM_53125 || BCM_53128 */
#endif /* BCM_53101 || BCM_53125 || BCM_53128 */
#ifdef BCM_TB_SUPPORT
    char *s;
#endif
    ...
    if ((SOC_CONTROL(unit)->int_cpu_enabled) &&
        (loopback == BCM_PORT_LOOPBACK_PHY)) {
...
_bcm_robo_port_pfm_set(int unit, bcm_port_t port, int mode)
{
    ...
    switch (mode) {
        case BCM_PORT_MCAST_FLOOD_NONE:
            ...
            reg_len = DRV_REG_LENGTH_GET( unit, MLF_DROP_MAPr );
            ...
            if (SOC_IS_ROBO_GE_SWITCH(unit) || SOC_IS_ROBO53101(unit)) {
                        ...
                        bcm_pbmp_t pbmp)
{
    ...
    BCM_PBMP_ITER(i_pbm, p) {
        BCM_PBMP_CLEAR(t_pbm);
        BCM_IF_ERROR_RETURN(DRV_PORT_VLAN_GET(unit, p, &t_pbm));
                        ...
                        bcm_pbmp_t *pbmp)
{
    ...
    if ((modid != 0) || (!SOC_PORT_VALID(unit, port))) {
        return BCM_E_PARAM;
    }
    ...
    BCM_PBMP_CLEAR(t_pbm);
    BCM_IF_ERROR_RETURN(DRV_PORT_VLAN_GET(unit, port, &t_pbm));
             ...
             uint32 flags)
{
    ...
    if (SOC_IS_TBX(unit)){
         ...
         */
        BCM_IF_ERROR_RETURN(bcm_robo_port_flood_block_get(
        ...
        BCM_IF_ERROR_RETURN(DRV_PORT_BLOCK_GET(
        ...
        if (changed) {
            ...
            BCM_IF_ERROR_RETURN(DRV_PORT_BLOCK_SET(
        ...
        BCM_IF_ERROR_RETURN(DRV_PORT_VLAN_GET(unit, ingress_port, &t_pbm));
        ...
        if ((!flags) || (flags & BCM_PORT_FLOOD_BLOCK_ALL)) {
            if (BCM_PBMP_NEQ(t_pbm, opbm)) {
                BCM_IF_ERROR_RETURN(DRV_PORT_VLAN_SET(
...
#if defined(BCM_53115) || defined(BCM_53125_A0)  
...
bcm_robo_port_selective_get(int unit, bcm_port_t port, bcm_port_info_t *info)
{
    ...
    if (mask & BCM_PORT_ATTR_ENCAP_MASK){
        r = bcm_robo_port_encap_get(unit, port, &info->encap_mode);
        if (r == BCM_E_UNAVAIL){
            r = BCM_E_NONE;     /* for unsupported chip */
        }
        BCM_IF_ERROR_RETURN(r);
...
bcm_robo_port_selective_set(int unit, bcm_port_t port, bcm_port_info_t *info)
{
    ...
    if (mask & BCM_PORT_ATTR_ENCAP_MASK){
        r = bcm_robo_port_encap_set(unit, port, info->encap_mode);
        SOC_IF_ERROR_DEBUG_PRINT(r, (DK_VERBOSE,
             "bcm_port_pause_addr_get failed: %s\n", bcm_errmsg(r)));
        if (r == BCM_E_UNAVAIL){
            r = BCM_E_NONE;     /* for unsupported chip */
    ...
    if (SOC_IS_TBX(unit)) {
        ...
        for (pkt_pri = 0; pkt_pri <= 7; pkt_pri++) {
            ...
            BCM_IF_ERROR_RETURN(DRV_PORT_PRI_MAPOP_GET
                    ...
                    pkt_pri, cfi, &internal_pri, (uint32 *)&t_color));
                                ...
                                int prio, bcm_color_t color)
{
    ...
    if (SOC_IS_TBX(unit)) {
        ...
        for (cfi = 0; cfi <= 1; cfi++) {
            ...
            BCM_IF_ERROR_RETURN(DRV_PORT_PRI_MAPOP_GET
                    (unit, port, DRV_PORT_OP_PCP2TC, 
                    prio, cfi, &internal_pri, (uint32 *)&t_color));
 ...
 *      2. In Robo chip, the color and cfi parameters are supported for TB only
 *          so far (not supported for bcm5395/bcm53115/bcm53118).
                       ...
                       int cfi, int *internal_pri, bcm_color_t *color)
{
    ...
    if (pkt_pri == -1 && cfi == -1) {
        /* For the untagged packets */
        BCM_IF_ERROR_RETURN(DRV_PORT_GET(
            unit, port, DRV_PORT_PROP_UNTAG_DEFAULT_TC, &val));
        *internal_pri = val;
        if (SOC_IS_TBX(unit)) {
            BCM_IF_ERROR_RETURN(DRV_PORT_GET(
                unit, port, DRV_PORT_PROP_DEFAULT_DROP_PRECEDENCE, 
                &val));
            *color = _BCM_COLOR_DECODING(unit, val);
        }
        return BCM_E_NONE;
    }      
    ...
    if (SOC_IS_TBX(unit)) {
        ...
        BCM_IF_ERROR_RETURN(DRV_PORT_PRI_MAPOP_GET
                (unit, port, DRV_PORT_OP_PCP2TC, pkt_pri,
                 cfi, (uint32 *) internal_pri, (uint32 *) color));
        ...
        case bcmPortControlEEETransmitEventCount:
            ...
            if ((rv = soc_phyctrl_control_get(unit, port,
                ...
                *value = val;
                ...
                rv = soc_robo_counter_get32(unit, port, EEE_LPI_EVENTr, &val);
                *value = val;
            } else {
                rv = BCM_E_UNAVAIL;
            }
        ...
        case bcmPortControlEEETransmitDuration:
            ...
            if ((rv = soc_phyctrl_control_get(unit, port,
                ...
                *value = val;
                ...
                rv = soc_robo_counter_get32(unit, port, EEE_LPI_DURATIONr, &val);
                *value = val;
            } else {
                rv = BCM_E_UNAVAIL;
            }
                ...
                bcm_port_t port, int link)
{
    ...
    if (!IS_GE_PORT(unit, port)){
        soc_cm_debug(DK_WARN, "%s, Auto-PowerDown WAR is for GE PHY only!\n", 
            FUNCTION_NAME());
        return BCM_E_NONE;
    }
                ...
                bcm_port_t port, uint32 power_type)
{
    ...
    soc_cm_debug(DK_PORT, "%s, port=%d, power=%s\n", FUNCTION_NAME(), port,
    ...
    if (SOC_IS_ROBO5398(unit) || SOC_IS_ROBO5397(unit) ||
        ...
        switch(mode) {
            case PHY_HIGH_PERFORMANCE_MODE:
                ...
                DRV_MIIM_WRITE(unit, phy_addr, 0x18, 0x2007);
                DRV_MIIM_READ(unit, phy_addr, 0x18, &tmp);
                tmp &= ~0x40;
                tmp &= ~(0x7 << 13);
                DRV_MIIM_WRITE(unit, phy_addr, 0x18, tmp);
                
                /* Set expansion register F9h, bit1 = 0 */
                DRV_MIIM_WRITE(unit, phy_addr, 0x17, 0x0ff9);
                DRV_MIIM_READ(unit, phy_addr, 0x15, &tmp);
                if (tmp & 0x2) {
                    tmp &= ~0x2;
                    DRV_MIIM_WRITE(unit, phy_addr, 0x15, tmp);
                }
                break;
            ...
            case PHY_STANDBY_POWER_DOWN_MODE:
                ...
                DRV_MIIM_READ(unit, phy_addr, 0x0, &tmp);
                tmp |= 0x800;
                DRV_MIIM_WRITE(unit, phy_addr, 0x0, tmp);
                break;
            ...
            case PHY_AUTO_POWER_DOWN_MODE:
                ...
                DRV_MIIM_WRITE(unit, phy_addr, 0x1c, 0x2800);
                DRV_MIIM_READ(unit, phy_addr, 0x1c, &tmp);
                tmp |= 0x8020; /* auto power-down mode */
                DRV_MIIM_WRITE(unit, phy_addr, 0x1c, tmp);
                
                /* Enable Auto-MDIX When autoneg disabled */
                /* register 18h, shadow 111, bit 9 = 1 */
                DRV_MIIM_WRITE(unit, phy_addr, 0x18, 0x7007);
                DRV_MIIM_READ(unit, phy_addr, 0x18, &tmp);
                tmp |= 0x8200;
                DRV_MIIM_WRITE(unit, phy_addr, 0x18, tmp);
                break;
...
bcm_robo_port_phy_power_mode_select(int unit, bcm_port_t port)
{
    ...
    if (SOC_IS_ROBO5398(unit) || SOC_IS_ROBO5397(unit) ||
        ...
        bcm_robo_port_discard_get(unit, port, &discard_mode);
        bcm_robo_port_speed_get(unit, port, &link_speed);
        ...
        bcm_robo_linkscan_mode_set(unit, port, BCM_LINKSCAN_MODE_NONE); 

        /* Cable Checker */
        rv = bcm_robo_port_cable_diag(unit, port, &cd);
        ...
        if ((rv == BCM_E_NONE) && 
            ...
            sal_usleep(10);     
            DRV_MIIM_READ(unit, phy_addr, 0x1, &tmp);
            if (tmp & 0x4) {
                link_up = 1;
            } else {
                link_up = 0;
        ...
        bcm_robo_linkscan_mode_set(unit, port, BCM_LINKSCAN_MODE_SW);
        bcm_robo_port_discard_set(unit, port, discard_mode);
    } else {
        return BCM_E_UNAVAIL;
    }
                       ...
                       bcm_trunk_t *trunk_id, int *id)
{
    ...
    if (SOC_GPORT_IS_TRUNK(gport)) {
        ...
        if (!SOC_PORT_ADDRESSABLE(unit, local_port)) {
            soc_cm_debug(DK_PORT,"%s, local_port=0x%x, not addressable!\n",
        ...
        if (!SOC_MODID_ADDRESSABLE(unit, local_modid)) {
            soc_cm_debug(DK_PORT, "%s, local_mod=0x%x, not addressable!\n",
