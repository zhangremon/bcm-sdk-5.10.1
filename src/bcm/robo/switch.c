/*
 * $Id: switch.c 1.93.2.7 Broadcom SDK $
...
_bcm_robo_dos_attack_group_init(int unit)
{
    ...
    if (SOC_IS_ROBO5395(unit)) {
        /* 
          * The following DoS attack cases need to be done by Field.
          * bcmSwitchDosAttackPingFlood
          * bcmSwitchDosAttackTcpSmurf
          */
    ...
    /* 
      * Per-port DoS attack cases of BCM5395 can't be represented 
      * in one group because of the content arrangement of slices.
      * Create another group id for TCP related cases.
      */
    if (SOC_IS_ROBO5395(unit)) {
        BCM_FIELD_QSET_INIT(qset);
        /* 
          * The following DoS attack cases need to be done by Field.
          * bcmSwitchDosAttackTcpFlags
          * bcmSwitchDosAttackTcpFlagsSF
          * bcmSwitchDosAttackTcpFlagsFUP
          * bcmSwitchDosAttackSynFrag
          * bcmSwitchDosAttackFlagZeroSeqZero
          * bcmSwitchDosAttackSynFlood
          */
        BCM_FIELD_QSET_ADD(qset, bcmFieldQualifyTcpControl);
        BCM_FIELD_QSET_ADD(qset, bcmFieldQualifyInPorts);
        ...
        dos_attack_tcp_group_id[unit] = tmp_group;
    }
...
_bcm_robo_dos_attack_set(int unit, bcm_port_t port, int dos_attack_type, int param)
{
    ...
    switch (dos_attack_type) {
            ...
            if (param) {
                ...
                if (!entry) {
                    ...
                    if (port != DOS_ATTACK_ALL_PORT) {
                        ...
                        if (SOC_IS_ROBO5347(unit) || SOC_IS_ROBO5348(unit)) {
                            BCM_IF_ERROR_RETURN(
                                bcm_robo_field_qualify_SrcPort
                    ...
                    BCM_IF_ERROR_RETURN(
                        bcm_robo_field_counter_create(unit, entry));
                    BCM_IF_ERROR_RETURN(
        ...
        case bcmSwitchDosAttackTcpFlags:
            ...
            if (SOC_IS_ROBO5395(unit)) {
                /* 
                  * Field group id for BCM5395 TCP related DoS attack cases
                  * is different from other chips and cases.
                  */
                _bcm_robo_dos_attack_group_get(unit,
                        _BCM_SWITCH_DOS_TCP_GROUP, &group);
            }
            if (param) {
                /*
        ...
        case bcmSwitchDosAttackPingFlood:
            if (param) {
                ...
                if (!entry) {
                    ...
                    BCM_IF_ERROR_RETURN(
                        bcm_robo_field_meter_create(unit, entry));
                    BCM_IF_ERROR_RETURN(
                        bcm_robo_field_meter_set(unit, entry, BCM_FIELD_METER_COMMITTED, 64, 1024));
                    BCM_IF_ERROR_RETURN(
...
_bcm_robo_dos_attack_get(int unit, bcm_port_t port, int dos_attack_type, int *param)
{
    ...
    switch(dos_attack_type) {
        ...
        case bcmSwitchDosAttackV4FirstFrag:
            ...
            break;
        case bcmSwitchDosAttackTcpFlags:
            if (SOC_IS_TBX(unit) || SOC_IS_ROBO5395(unit)){
                /* TCP Control in TB is unavail */
                return BCM_E_UNAVAIL;
            }
                  ...
                  int *arg)
{
    ...
    if (type == bcmSwitchFiltersControlModes || 
        ...
        rv = _bcm_robo_bypass_modes_get(unit, type, &bypass_modes);
        *arg = bypass_modes;
        return rv;
    }
