...
STATIC int         robo_rx_dflt_alloc(int unit, int size, uint32 flags, void **pool);
STATIC int         robo_rx_dflt_free(int unit, void *buf);
...
_pkt_robo_reasons_get(int unit, bcm_pkt_t *pkt, uint32 reasons)
{
    ...
    if (SOC_IS_ROBO5395(unit) || SOC_IS_ROBO53242(unit) ||
...
robo_rx_intr_process_packet(int unit, eth_dv_t *dv, eth_dcb_t *dcb, bcm_pkt_t *pkt)
{
    ...
    if (SOC_IS_ROBO53242(unit) || SOC_IS_ROBO53262(unit)) {
        ...
        rx_reasons = dv->dv_reason_5395;
        _pkt_robo_reasons_get(unit, pkt, rx_reasons);
        pkt->prio_int = dv->dv_tc_5395;
        pkt->rx_port = dv->dv_src_portid_5395;
        if (dv->dv_opcode == BRCM_OP_EGR_TS) {
            if (SOC_ROBO_DV_TS_TX_EVENT_5395(dv)) {
                /* 
        ...
        _pkt_robo_reasons_get(unit, pkt, rx_reasons);
        DRV_QUEUE_QOS_CONTROL_GET(unit, DRV_QOS_CTL_USE_TC, &temp);
        if (temp) {
...
STATIC int 
robo_rx_dflt_alloc(int unit, int size, uint32 flags, void **pool)
{
    COMPILER_REFERENCE(flags);
    *pool = soc_cm_salloc(unit, size, "robo_rx_dflt_alloc");
    if (*pool == NULL){
        return BCM_E_MEMORY;
    }
    return BCM_E_NONE;
...
STATIC int
robo_rx_dflt_free(int unit, void *buf)
{
...
bcm_robo_rx_cosq_mapping_reasons_get(int unit, bcm_rx_reasons_t * reasons)
{
    ...
    if (SOC_IS_ROBO5395(unit) || SOC_IS_ROBO53242(unit) ||
        ...
        sz = _bcm_robo_rx_cpu_cosmap_key_max;
        for (ix = 0; ix < sz; ix++) {
             if (_bcm_robo_cpu_cos_map_key[ix] != bcmRxReasonInvalid) {
                 BCM_RX_REASON_SET(*reasons, _bcm_robo_cpu_cos_map_key[ix]);
             }
        }
    } else if (SOC_IS_TBX(unit)) {
        sz = _bcm_robo_tb_rx_cpu_cosmap_key_max;
        for (ix = 0; ix < sz; ix++) {
             if (_bcm_robo_tb_cpu_cos_map_key[ix] != bcmRxReasonInvalid) {
                 BCM_RX_REASON_SET(*reasons, _bcm_robo_tb_cpu_cos_map_key[ix]);
             }
        }
    } else {
        return BCM_E_UNAVAIL;
