/*
 * $Id: drv.c 1.2.2.40 Broadcom SDK $
...
soc_ea_private_db_sync(int unit, int flag)
{
    ...
    if(soc_attached(unit)){
        ...
        while(db_type < socEaChipInfoCount){
            rv = _soc_ea_chip_info_sync(unit, db_type, flag);
            if(rv != SOC_E_NONE){
                soc_cm_debug(DK_ERR,"Ea unit %d db %d sync failed.\n",unit, 
                    db_type);
            }
            db_type++;
        }
        ...
        while(db_type < socEaChipControlCount){
            rv = _soc_ea_chip_control_sync(unit, db_type, flag);
            if(rv != SOC_E_NONE){
                soc_cm_debug(DK_ERR,"Ea unit %d db %d sync failed.\n",unit, 
                    db_type);
            }
...
soc_ea_private_db_dump(int unit)
{
    ...
    if (NULL == SOC_CONTROL(unit)) {
        soc_cm_debug(DK_ERR,"Ea err code:%d",SOC_E_UNIT);
        return;
    }
    ...
    if(si->private == NULL){
        soc_cm_debug(DK_ERR,"Ea err code:%d",SOC_E_INIT);
        return ;
    }

    db = (soc_ea_private_db_t *)si->private;

    soc_cm_print("private info\n");
    soc_cm_print("info.flag = 0x%08x\n",db->chip_info.flag);
    soc_cm_print("loadinfo\t\n");
    for(i = 0; i < socEaFileCount; i++){
        soc_cm_print("load.%d.ver = 0x%04x\t\n",i,
            db->chip_info.load_info[i].legacy_ver);
        soc_cm_print("load.%d.crc = 0x%08x\t\n",i,
            db->chip_info.load_info[i].crc);
    }

    soc_cm_print("onu info\t\n");
    soc_cm_print("onuinfo.firmware_ver = 0x%04x\t\n",
        db->chip_info.onu_info.firmware_ver);
    soc_cm_print("pon base mac::%02x:%02x:%02x:%02x:%02x:%02x\t\n",
    ...
    soc_cm_print("uni base mac::%02x:%02x:%02x:%02x:%02x:%02x\t\n",
    ...
    soc_cm_print("onuinfo.port_count = 0x%04x\t\n",
        db->chip_info.onu_info.port_count);
    soc_cm_print("onuinfo.max_link_count = 0x%04x\t\n",
        db->chip_info.onu_info.max_link_count);
    
    soc_cm_print("onuinfo.up_queue_count = 0x%04x\t\n",
        db->chip_info.onu_info.up_queue_count);
    soc_cm_print("onuinfo.max_queue_count_per_link = 0x%04x\t\n",
        db->chip_info.onu_info.max_queue_count_per_link);
    soc_cm_print("onuinfo.up_queue_unit = 0x%04x\t\n",
        db->chip_info.onu_info.up_queue_unit);

    soc_cm_print("onuinfo.dn_queue_count = 0x%04x\t\n",
        db->chip_info.onu_info.dn_queue_count);
    soc_cm_print("onuinfo.max_queue_count_per_port = 0x%04x\t\n",
        db->chip_info.onu_info.max_queue_count_per_port);
    soc_cm_print("onuinfo.dn_queue_unit = 0x%04x\t\n",
        db->chip_info.onu_info.dn_queue_unit);

    soc_cm_print("onuinfo.up_queue_total_size = 0x%08x\t\n",
        db->chip_info.onu_info.up_queue_total_size);
    soc_cm_print("onuinfo.max_queue_count_per_port = 0x%08x\t\n",
        db->chip_info.onu_info.max_queue_count_per_port);
    soc_cm_print("onuinfo.jedec_id = 0x%04x\t\n",
        db->chip_info.onu_info.jedec_id);
    soc_cm_print("onuinfo.chip_id = 0x%04x\t\n",
        db->chip_info.onu_info.chip_id);
    soc_cm_print("onuinfo.chip_ver = 0x%08x\t\n",
        db->chip_info.onu_info.chip_ver);

    soc_cm_print("chip_revision = 0x%08x\t\n",db->chip_info.chip_revision);
    soc_cm_print("extned id\t\n");
    for(i = 0; i < sizeof(soc_ea_extend_id_t); i++){
        if(i != 0 && i%16 == 0){
            soc_cm_print("\t\n%02x",db->chip_info.extend_id[i]);
        }else{
            soc_cm_print("%02x ",db->chip_info.extend_id[i]);
        }
    }

    soc_cm_print("\t\noam ver = %x\t\n",db->chip_info.oam_ver);
    soc_cm_print("llid count = %d\t\n", db->chip_info.llid_count);
    soc_cm_print("port info\t\n");
    for(i = 0; i < SDK_MAX_NUM_OF_PORT; i++){
        if(!SOC_PORT_VALID(unit,i)){
            break;
        }
        soc_cm_print("port.%d.interface = %d\t\n",i,db->chip_info.port[i].interface);
        soc_cm_print("port.%d.autoneg = %d\t\n",i,db->chip_info.port[i].autoneg);
        soc_cm_print("port.%d.speed = %d\t\n",i,db->chip_info.port[i].speed);
        soc_cm_print("port.%d.duplex = %d\t\n",i,db->chip_info.port[i].duplex);
        soc_cm_print("port.%d.advert_local = %d\t\n",i,
            db->chip_info.port[i].advert_local);
        soc_cm_print("port.%d.advert_remote = %d\t\n",i,
            db->chip_info.port[i].advert_remote);
        soc_cm_print("port.%d.ability_speed_half_duplex = %d\t\n",i,
            db->chip_info.port[i].ability_speed_half_duplex);
        soc_cm_print("port.%d.ability_speed_full_duplex = %d\t\n",i,
            db->chip_info.port[i].ability_speed_full_duplex);
        soc_cm_print("port.%d.ability_pause = %d\t\n",i,
            db->chip_info.port[i].ability_pause);
        soc_cm_print("port.%d.ability_medium = %d\t\n",i,
            db->chip_info.port[i].ability_medium);
        soc_cm_print("port.%d.ability_loopback = %d\t\n",i,
            db->chip_info.port[i].ability_loopback);
        soc_cm_print("port.%d.ability_encap = %d\t\n",i,
            db->chip_info.port[i].ability_encap);
        soc_cm_print("port.%d.mdix = %d\n",i,db->chip_info.port[i].mdix);
        soc_cm_print("port.%d.mdix_status = %d\t\n",i,
            db->chip_info.port[i].mdix_status);
        soc_cm_print("port.%d.bcast_limit = %d\t\n",i,
            db->chip_info.port[i].bcast_limit);
        soc_cm_print("port.%d.bcast_limit_enable = %d\t\n",i,
            db->chip_info.port[i].bcast_limit_enable);
        soc_cm_print("port.%d.phy_master = %d\t\n",i,
            db->chip_info.port[i].phy_master); 
    }

    soc_cm_print("control info\t\n");
    soc_cm_print("control.flag = 0x%08x\t\n",db->chip_control.flag);
    soc_cm_print("control.mllid_num = %d\t\n",db->chip_control.mllid_num);
    soc_cm_print("qconfig info\t\n");
    soc_cm_print("qconfig.link_count = %d\t\n",db->chip_control.q_config.link_count);
    for(i = 0; i < db->chip_control.q_config.link_count; i++){
        if(i >= MAX_CNT_OF_LINK)break;
        soc_cm_print("link%d.qcount = %d\t\n",i,
        ...
        for(j = 0; j < db->chip_control.q_config.link_info[i].q_count; j++){
            ...
            soc_cm_print("link%d.q%d.size = %d\t\n",i,j,
...
soc_ea_pre_attach(int unit)
{
    ...
    if (!soc->oam_ctrlops) {
        soc_cm_debug(DK_ERR, "EA unit %d attached control interface failed\n",
                     unit);
...
_ea_device_probe_thread(void *param)
{
    ...
    while (ea_probe_thread_running) {
        ...
        for (cur_unit= 0; (cur_unit < soc_ndev); cur_unit++) { 
            if (SOC_IS_EA(cur_unit)) {
                soc = SOC_CONTROL(cur_unit);
                ...
                ctrl_if->request(cur_unit, info_req_buf, info_req_len);
            }
        }
        ea_probe_count++;
    ...
    for (cur_unit= 0; (cur_unit < soc_ndev); cur_unit++) { 
        if (SOC_IS_EA(cur_unit)) {
            ...
            if (!(soc->soc_flags & SOC_F_ATTACHED)) {
                soc_cm_debug(DK_WARN, "Unit %d attaching failed\n", cur_unit);
                soc_ea_detach(cur_unit);
            }
...
static void
_oam_ctrl_temp_cbk_func(int unit, eth_dv_t *dv,  eth_dcb_t *dcb)
{
    char    *p;
    uint8   path_id = unit;
    
    p = &((char *) (INT_TO_PTR(dcb->dcb_vaddr)))[40];   
...
static void
_oam_ctrl_probe_cbk_func(int unit, eth_dv_t *dv,  eth_dcb_t *dcb)
{
    soc_control_t   *soc;
    soc_info_t      *si;
    soc_ea_oam_ctrl_if_t *ctrl_if;
    ...
    if (!(soc->soc_flags & SOC_F_ATTACHED)) {
        ...
        soc_cm_print("Attaching unit %d\n",unit);
    }

    
    for (test_unit= 0; (test_unit < soc_ndev); test_unit++) {
        if (SOC_IS_EA(test_unit)) {
            if (soc->soc_flags & SOC_F_ATTACHED) {
                /* This unit is attached succefully */
                attached_unit++;
            }
