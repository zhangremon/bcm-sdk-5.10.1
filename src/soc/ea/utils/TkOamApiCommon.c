/*
 * $Id: TkOamApiCommon.c 1.1.2.12 Broadcom SDK $
...
#include <soc/ea/TkNetIf.h>
#include <soc/ea/TkInit.h>  /* FixMe */
#include <soc/ea/TkDebug.h>


static uint32   TkExtOamReplyTimeoutMs = 40000000;
               ...
               uint8 branch, uint16 leaf, uint8 * pRxBuf, uint32 * pRxLen)
{
     ...
     * index of object 
     */
    pObjectInst = (OamVarContainer *) ((uint32) tk + 1);
     ...
     * Variable Descriptor 
     */
    pVarDesc = (OamVarDesc *) ((uint32) pObjectInst + objLen);
    ...
    gTagLinkCfg[pathId].timeOut = TkExtOamGetReplyTimeout();
    if (OK ==
        ...
        size =
            ...
            sizeof(OamVarDesc);
        dataSize = *(uint8 *) ((uint32) pMsgBuf + size);
        ...
        dataSize = *(uint8 *) ((uint32) pMsgBuf + size);    /* Variable
                                                             * Width */

        if (dataSize < OamVarErrNoError) {  /* < 0x80 */
            *pRxLen = dataSize;
            bcopy((uint8 *) ((uint32) pMsgBuf + size + 1), pRxBuf,
                  dataSize);
            TkOamMemPut(pathId,(void *) pMsgBuf);  /* free memory */
            return (OK);
        } else {
                    ...
                    uint32 * pRxLen)
{
    ...
    if (OK ==
        ...
        *pRxLen = dataSize;
        bcopy((uint8 *) ((uint32) pMsgBuf + size), pRxBuf, dataSize);
            ...
            uint8 branch, uint16 leaf, uint8 * pRxBuf, uint32 * pRxLen)
{
     ...
     * Variable Descriptor 
     */
    pVarDesc = (OamVarDesc *) ((uint32) tk + 1);
    pVarDesc->branch = branch;  /* branch (07 or 09) */
    pVarDesc->leaf = soc_htons(leaf);   /* leaf */
               ...
               uint8 branch, uint16 leaf, uint8 * pTxBuf, uint8 txLen)
{
     ...
     * Variable Container 
     */
    pVarCont = (OamVarContainer *) ((uint32) pObjectInst + objLen);
    ...
    if (OK ==
        ...
        size =
            sizeof(OamOuiVendorExt) + sizeof(OamTkExt) +
            sizeof(OamVarDesc);
        objValLen = *(uint8 *) ((uint32) pMsgBuf + size);
        if (OamVarErrNoError > objValLen) {
            size += objValLen + sizeof(OamVarDesc) + 1;
            retCode = *(uint8 *) ((uint32) pMsgBuf + size);
        } else {
            retCode = objValLen;
        }
            ...
            uint8 branch, uint16 leaf, uint8 * pTxBuf, uint8 txLen)
{
     ...
     * Variable Container 
     */
    pVarCont = (OamVarContainer *) ((uint32) tk + 1);
    pVarCont->branch = branch;  /* branch (07 or 09) */
    pVarCont->leaf = soc_htons(leaf);   /* leaf */
    if ((NULL == pTxBuf) && (0 == txLen)) {
        pVarCont->length = 0x80;
...
void
TkExtOamSetReplyTimeout(uint32 val)
{
    TkExtOamReplyTimeoutMs = val;
}

uint32
TkExtOamGetReplyTimeout(void)
{
    return TkExtOamReplyTimeoutMs;
}
