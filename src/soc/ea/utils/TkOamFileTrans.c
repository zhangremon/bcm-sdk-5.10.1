/*
 * $Id: TkOamFileTrans.c 1.1.2.3 Broadcom SDK $
...
#include <soc/ea/FileTransTk.h>
#include <soc/ea/OamUtils.h>
#include <soc/ea/TkDebug.h>

#include <soc/ea/TkOnuApi.h>        /* FixMe */
...
TkExtOamFileSendAck(uint8 pathId, uint8 err)
{
    ...
    uint16          flags = 0x0050;
    uint8          *pMsgBuf = NULL;
    uint32          origOut = TkExtOamGetReplyTimeout();
     ...
     * for App. The middle reply need about > 0.02s. 
     */
    TkExtOamSetReplyTimeout(24000);
    if (OK ==
        TkOamRequest(pathId, 0,
                     (OamFrame *) pMsgBuf, size,
                     (OamPdu *) pMsgBuf, &RespLen)) {
        OamTkFileAck   *pTkRcvAck =
            (OamTkFileAck *) ((uint32) pMsgBuf + sizeof(OamOuiVendorExt));
        TkExtOamSetReplyTimeout(origOut);
