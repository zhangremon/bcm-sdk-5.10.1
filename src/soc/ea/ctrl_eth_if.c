/*
 * $Id: ctrl_eth_if.c 1.2.2.13 Broadcom SDK $
...
#define EA_OAM_PKT_SIZE_DFLT 512
#define SOC_EA_OAM_DMA 3
#define SOC_EA_OAM_RX_DMA_COUNT 64


typedef void (*cb_dv_done_packet)(int unit, eth_dv_t *dv,  eth_dcb_t *dcb);

static cb_dv_done_packet oam_rx_rtn_func = NULL;

static void
_oam_cb_func(int unit, eth_dv_t *dv,  eth_dcb_t *dcb)
{
    if  (oam_rx_rtn_func) {
        (*oam_rx_rtn_func)(unit, dv, dcb);
...
ether_ctrl_init(int unit)
{
    ...
    dv = soc_eth_dma_dv_alloc(attached_switch_unit, DV_RX, 
                                SOC_EA_OAM_RX_DMA_COUNT);      
    soc_eth_dma_dv_reset(DV_RX, dv);
    dv->dv_next = NULL;
    dv->dv_channel = SOC_EA_OAM_DMA;
    dv->dv_unit = unit;
    dv->dv_done_chain = NULL;
    dv->dv_done_packet = _oam_cb_func;
    
    for (i = 0; i < SOC_EA_OAM_RX_DMA_COUNT; i++) {       
        if ((buf = soc_cm_salloc(attached_switch_unit, 
                    EA_OAM_PKT_SIZE_DFLT, "oam rx")) == NULL) {
            soc_cm_debug(DK_WARN, "ERROR: Out of memory at buf #%d\n",i); 
            return;
        }
        soc_eth_dma_desc_add(dv,  PTR_TO_INT(buf), EA_OAM_PKT_SIZE_DFLT);
    }
