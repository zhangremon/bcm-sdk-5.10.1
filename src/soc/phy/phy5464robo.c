/*
 * $Id: phy5464robo.c 1.63.66.4 Broadcom SDK $
...
#define ERRATA_FIXUPS       0   /* rev A0 errata */
...
#define DPRINTF         if (DPRINTF_ENABLE && \
                    soc_cm_debug_check(DK_PHY)) \
                    soc_cm_print
...
_robo_phy_5464_medium_check(int unit, soc_port_t port, int *medium)
{
    ...
    if (DPRINTF_ENABLE && soc_cm_debug_check(DK_VERBOSE)) {
        DPRINTF("_phy_5464_medium_check: "
                "u=%d p=%d fiber_pref=%d 0x1c(11111)=%04x\n",
                unit, port, pc->fiber.preferred, tmp);
        DPRINTF("_phy_5464_medium_check: u=%d p=%d fiber=%d\n",
                unit, port, !copper);
    }
...
_robo_phy_5464_reset_setup(int unit, soc_port_t port)
{
    ...
    DPRINTF("_robo_phy_5464_reset_setup: u=%d p=%d medium=%s\n",
        unit, port,
    ...
    if (PHY_IS_BCM5464(pc) && pc->phy_rev == PHY_REV_A0 &&
        ...
        DPRINTF("_phy_5464_reset_setup: u=%d p=%d ERRATA\n", unit, port);
    ...
    if (PHY_IS_BCM5488(pc) || PHY_IS_BCM5482(pc)) {
     ...
     /* Set Power/MII control register */
    if(SOC_IS_ROBO5398(unit)||SOC_IS_ROBO5397(unit) ||
        SOC_IS_ROBO5395(unit)) {
        
        /* write to reg 18h, shadow 010b */
        SOC_IF_ERROR_RETURN (
            WRITE_PHY5464_MII_POWER_CTRLr(unit, pc,0xc040));

        /* Enable Auto Power-Down Mode */
        /* register 1Ch, shadow 01010, bit 5 =1 */
        /* auto power-down mode */
        SOC_IF_ERROR_RETURN (
            WRITE_PHY5464_AUTO_POWER_DOWNr(unit, pc, 0x20));
        
        /* Enable Auto-MDIX When autoneg disabled */
        /* register 18h, shadow 111, bit 9 = 1 */
        SOC_IF_ERROR_RETURN (
            MODIFY_PHY5464_MII_MISC_CTRLr(unit, pc, 0x200, 0x200));
    ...
    if (phy53115_an100tx_war){ /* config set to stop this workaround */
        ...
        if (PHY_REVISION_CHECK(pc, PHY_BCM53115_MODEL, 
            ...
            DPRINTF("_robo_phy_5464_reset_setup,"
...
robo_phy_5464_init(int unit, soc_port_t port)
{
    ...
    DPRINTF("robo_phy_5464_init: u=%d p=%d phy_addr=%d\n",
   ...
   if (PHY_IS_BCM5461(pc)) {
    ...
    if ((SOC_IS_DRACO(unit) ||
        (soc_feature(unit, soc_feature_dodeca_serdes))) && (int_pc != NULL)) {
        if (PHY_IS_BCM5461(pc)) {
            if (!type_s) {
                return SOC_E_FAIL;
            }
    ...
    if (!fiber_capable) {
        ...
        tmp = 0;
        /* Set the variable, "fiber_preferred", when PHY chip is S-TYPE */
    ...
    DPRINTF("phy_5464_init: u=%d p=%d "
            "type=546%d%s automedium=%d fiber_pref=%d detect=%d if=%s\n",
            unit, port,
    ...
    DPRINTF("phy_5464_init: u=%d p=%d COPPER "
    ...
    DPRINTF("phy_5464_init: u=%d p=%d FIBER "
            "enable=%d pref=%d an=%d an_adv=0x%x\n",
            unit, port,
...
_robo_phy_5464_medium_change(int unit, soc_port_t port, int force_update)
{
    ...
    if (medium == SOC_PORT_MEDIUM_COPPER) {
        if ((!PHY_COPPER_MODE(unit, port)) || force_update) {
            DPRINTF("_phy_5464_link_auto_detect: u=%d p=%d [F->C]\n",
                    unit, port);
        ...
        if (PHY_COPPER_MODE(unit, port) || force_update) {
            DPRINTF("_phy_5464_link_auto_detect: u=%d p=%d [C->F]\n",
                    unit, port);
...
robo_phy_5464_duplex_set(int unit, soc_port_t port, int duplex)
{
    ...
    DPRINTF("robo_phy_5464_duplex_set: u=%d p=%d d=%d\n",
            unit, port, duplex);
...
robo_phy_5464_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    DPRINTF("robo_phy_5464_speed_set: u=%d p=%d s=%d fiber=%d\n",
            ...
            if (SOC_IS_ROBO53115(unit)){
                ...
                if (phy53115_an100tx_war) {
                    if(PHY_REVISION_CHECK(pc, PHY_BCM53115_MODEL, 
                                    PHY_BCM53115_OUI, BCM53115_A0_REV_ID)){
                        if(speed == 100){
                            DPRINTF("robo_phy_5464_speed_set,"
                                    "Set ge%d WAR for phy53115_a0!!\n", 
                                    port);
                            rv = WRITE_PHY_REG(unit, pc, 0x17, 0x0fff);
                            rv |= WRITE_PHY_REG(unit, pc, 0x15, 0x20c0);
                            if (rv){
                                DPRINTF("robo_phy_5464_speed_set,%d,"
...
robo_phy_5464_master_set(int unit, soc_port_t port, int master)
{
    ...
    DPRINTF("robo_phy_5464_master_set: u=%d p=%d master=%d fiber=%d\n",
...
robo_phy_5464_autoneg_set(int unit, soc_port_t port, int autoneg)
{
    ...
    DPRINTF("phy_5464_autoneg_set: u=%d p=%d autoneg=%d auto=%d rv=%d\n",
...
robo_phy_5464_adv_local_set(int unit, soc_port_t port, soc_port_mode_t mode)
{
    ...
    DPRINTF("phy_5464_adv_local_set: u=%d p=%d mode=0x%x rv=%d\n",
            unit, port, mode, rv);
...
robo_phy_5464_lb_set(int unit, soc_port_t port, int enable)
{
    ...
    DPRINTF("phy_5464_lb_set: u=%d p=%d en=%d rv=%d\n", 
             unit, port, enable, rv);
    return rv;
...
robo_phy_5464_link_down(int unit, soc_port_t port)
{
    ...
    if (PHY_IS_BCM5464(pc) && pc->phy_rev == PHY_REV_A0 &&
        ...
        DPRINTF("5464_link_down: port=%d activated\n", port);
...
void _phy53115_a0_iop_errata(int unit, soc_port_t port)
{
    ...
    if (regrw_rv) {
        DPRINTF("_phy53115_a0_iop_errata,%d,"
                "port%d, failed on writing to PHY reg!!\n", 
                __LINE__,port);
    } else {
        DPRINTF("_phy53115_a0_iop_errata,%d,port%d, IOP-WAR-#1 Done!!\n", 
                __LINE__,port);
    }
}
...
void _phy53115_b0_iop_errata(int unit, soc_port_t port, int link)
{
    ...
    if (regrw_rv){
        DPRINTF("_phy53115_b0_iop_errata,%d,port%d failed on PHY Reg read!\n", 
                __LINE__,port);
    }
    ...
    if (link){  /* 2.1 link-up procedure */
        ...
        if (PHY53115_ERRATA_IOP_GET(port)){
            ...
            if (regrw_rv){
                DPRINTF("_phy53115_b0_iop_errata,%d,"
                        "port%d failed on writing to PHY Reg!!\n", 
                        __LINE__,port);
            }
        ...
        if (phy53115_errata_iop_cnt[port] >= 
            ...
            DPRINTF("_phy53115_b0_iop_errata,%d,"
                    "port%d IOP-WAR-#2,HALFOUT change to 0x%04x!!\n", 
                    __LINE__,port, temp);
            ...
            if (regrw_rv){
                DPRINTF("_phy53115_b0_iop_errata,"
                        "%d,port%d failed on writing to PHY Reg!!\n", 
                        __LINE__,port);
            }
...
void _phy53115_a0_errata(int unit, soc_port_t port, int link)
{
    ...
    if (rv){
        DPRINTF("_phy53115_a0_errata,%d,port%d failed on PHY Reg read!\n", 
                __LINE__,port);
    }
    ...
    DPRINTF("_phy53115_a0_errata,%d,"
            "phy53115_a0 an100tx sw-info stat=0x%08x,link=0x%08x!!\n", 
            __LINE__, phy53115_errata_an100tx_status, 
            phy53115_errata_an100tx_linkup);
    DPRINTF("_phy53115_a0_errata,%d,Set ge%d workaround for phy53115_a0!!\n", 
            __LINE__, port);
...
void _phy53115_a0_errata_link_check(int unit, soc_port_t port, int link)
{
    ...
    if (link) {
        ...
        if (PHY53115_ERRATA_AN100TX_STAT_GET(port) && 
            ...
            DPRINTF("_phy53115_a0_errata_link_check,%d,"
        ...
        if (PHY53115_ERRATA_AN100TX_STAT_GET(port) && 
                PHY53115_ERRATA_AN100TX_LINK_GET(port)){
            DPRINTF("_phy53115_a0_errata_link_check,%d,"
                    "ge%d reset to free phy53115_a0 workaround !!\n", 
                    __LINE__, port);
            ...
            DPRINTF("_phy53115_a0_errata_link_check,%d,"
                    "ge%d reset for phy53115_a0 workaround failed!!\n", 
                    __LINE__, port);
            robo_phy_5464_init(unit, port);
        }
    }
...
robo_phy_5464_shadow_dump(int unit, soc_port_t port)
{
    ...
    for (i = 0; i <= 7; i++) {
        ...
        if (regrw_rv){
            soc_cm_print("robo_phy_5464_shadow_dump,%d, "
