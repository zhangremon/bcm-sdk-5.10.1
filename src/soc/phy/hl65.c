/*
 * $Id: hl65.c 1.148.14.4 Broadcom SDK $
...
_phy_hl65_independent_lane_init(int unit, soc_port_t port)
{
    ...
    if ((pc->phy_mode == PHYCTRL_DUAL_LANE_PORT) && IS_HG_PORT(unit,port)) {
        SOC_IF_ERROR_RETURN
...
_phy_hl65_control_prbs_polynomial_set(int unit, soc_port_t port, uint32 value)
{
    ...
    /* uPrbsLaneMask = 0x3 << (BM9600_HYPERCORE_8019_LANE_SHIFT * uLaneAddr);
     * uData &= ~uPrbsLaneMask;
     * uData |= value << (BM9600_HYPERCORE_8019_LANE_SHIFT * uLaneAddr);
     */
                                          ...
                                          uint32 value)
{
    ...
    if (value) {
        ...
        SOC_IF_ERROR_RETURN
            (WRITE_HL65_DIGITAL_MISC1r(unit, pc, 0x7220));
        ...
        SOC_IF_ERROR_RETURN
            (WRITE_HL65_XGXSBLK1_LANEPRBSr(unit, pc, 0x8888));
