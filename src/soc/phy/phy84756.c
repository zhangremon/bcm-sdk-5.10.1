/*
 * $Id: phy84756.c 1.9.4.7 Broadcom SDK $
...
phy_84756_init(int unit, soc_port_t port)
{
    ...
    bmacsec_dev_addr_t fcmap_dev_addr;
    int line_mode;
    int fiber_preferred;
    int rv = SOC_E_NONE;
    int macsec_enable = 0, fcmap_enable = 0, mmi_mdio_addr;
    ...
    rv = bmacsec_phy84756_phy_mac_addr_set(phy_dev_addr, 
                                          macsec_dev_addr, 
                                          pc->macsec_dev_port,
    ...
    if (fiber_preferred) {
        line_mode = BMACSEC_PHY84756_INTF_SFI;
        ...
        line_mode = BMACSEC_PHY84756_INTF_SGMII;
    ...
    rv = bmacsec_phy84756_init(phy_dev_addr, 1, 
                               line_mode, 
                               macsec_enable, 
                               fcmap_enable, 
                               phy_ext_rom_boot);
...
phy_84756_firmware_set(int unit, int port, int offset, uint8 *data,int len)
{
    ...
    rv = bmacsec_phy84756_spi_firmware_update(phy_dev_addr, data, len);
    if (rv != BMACSEC_E_NONE) {
            SOC_DEBUG_PRINT((DK_WARN,
...
phy_84756_probe(int unit, phy_ctrl_t *pc)
{
    ...
    if (0xff == soc_property_port_get(unit, pc->port, spn_FCMAP_ENABLE, 0)) {
        soc_cm_print("Using Standalone PHY driver for BCM84756\n");
        return SOC_E_NONE;
    }
    return SOC_E_NOT_FOUND;
