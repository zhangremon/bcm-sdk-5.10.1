/*
 * $Id: phy54682.c 1.39.2.18 Broadcom SDK $
...
_phy_54682_no_reset_setup(int unit, soc_port_t port)
{
   ...
   /* SGMII interface to MAC */
    ...
    SOC_IF_ERROR_RETURN(
        MODIFY_PHY_REG(unit, pc, 0x18, 0x0c00, 0x0c00));
    SOC_IF_ERROR_RETURN(
        WRITE_PHY_REG(unit, pc, 0x17, 0x4022));
    SOC_IF_ERROR_RETURN(
        WRITE_PHY_REG(unit, pc, 0x15, 0x017b));
    SOC_IF_ERROR_RETURN(
        MODIFY_PHY_REG(unit, pc, 0x18, 0x0400, 0x0c00));
...
_phy_54682_eee_auto_enable(int unit, soc_port_t port, int enable)
{
    ...
    if (PHY_IS_BCM54685E_A0A1(pc)) {
        ...
        SOC_IF_ERROR_RETURN
            (PHY54682_REG_MODIFY(unit, pc, 0x00, 0x0F7F,0x15, 0x1, 0x7)); /* exp 7f */
                     ...
                     soc_phy_control_t type, uint32 *value)
{
    ...
    case SOC_PHY_CONTROL_EEE:
        ...
        SOC_IF_ERROR_RETURN
            (READ_PHY54682_EEE_TEST_CTRL_Br(unit, pc, &data));
    ...
    case SOC_PHY_CONTROL_EEE_AUTO:
        ...
        SOC_IF_ERROR_RETURN
           (_phy_54682e_blk_top_lvl_reg_read(unit, pc, 0, 7, &data));
...
phy_54682_link_up(int unit, soc_port_t port)
{
    ...
    /* revs A0 and A1 of supported PHYs */
        ...
        SOC_IF_ERROR_RETURN
            (READ_PHY54682_EEE_ADVr(unit, pc, &eee_advert));
        if (eee_advert & 0x4) {
            sal_usleep(500000);
            SOC_IF_ERROR_RETURN(
...
phy_54682_timesync_config_set(int unit, soc_port_t port, soc_port_phy_timesync_config_t *conf)
{
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_FLAGS) {
        ...
        if (conf->flags & SOC_PORT_PHY_TIMESYNC_ENABLE) {
            ...
            SOC_IF_ERROR_RETURN
                (MODIFY_PHY54682_MODE_CTRLr(unit, pc, 0x00, 0x06));
            ...
            SOC_IF_ERROR_RETURN
                (PHY54682_REG_MODIFY(unit, pc, 0x00, 0x0F7F,0x15, 0x7, 0x7)); /* exp 7f */
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_LOOP_FILTER) {
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x42, (uint16)((COMPILER_64_LO(conf->phy_1588_dpll_loop_filter) >> 16) & 0xffff)));
    }
