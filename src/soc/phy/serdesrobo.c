/*
 * $Id: serdesrobo.c 1.12 Broadcom SDK $
...
phy_serdesrobo_link_get(int unit, soc_port_t port, int *link)
{
    ...
    if ( SOC_IS_ROBO5396(unit) )  {
        ...
        reg_addr = (DRV_SERVICES(unit)->reg_addr)( unit, SDMIISTSr, port, 0);
        reg_len  = (DRV_SERVICES(unit)->reg_length_get)( unit, SDMIISTSr ) ;
        ...
        SOC_IF_ERROR_RETURN( (DRV_SERVICES(unit)->reg_field_get)
                (unit, SDMIISTSr, &reg_value, LINK_STAf, &temp));
        *link = (int) temp ;
...
{
    uint32  reg_addr, reg_value, reg_len, temp = (uint32) duplex ;
    ...
    if ( SOC_IS_ROBO5396(unit) )  {
        soc_cm_debug( DK_PHY,
                ...
                unit, port, duplex ) ;
        reg_addr = (DRV_SERVICES(unit)->reg_addr)( unit, SDMIICTLr, port, 0);
        reg_len  = (DRV_SERVICES(unit)->reg_length_get)( unit, SDMIICTLr ) ;
        SOC_IF_ERROR_RETURN( (DRV_SERVICES(unit)->reg_read)
                ( unit, reg_addr, &reg_value, reg_len) ) ;
        SOC_IF_ERROR_RETURN( (DRV_SERVICES(unit)->reg_field_set)
                (unit, SDMIICTLr, &reg_value, DUPLEX_MODf, &temp));
...
phy_serdesrobo_an_set(int unit, soc_port_t port, int an)
{
    uint8	phy_addr = PORT_TO_PHY_ADDR_INT(unit, port);
    uint16	sgmii_ctrl;
    uint32      reg_addr, reg_value, reg_len, temp = (uint32) an ;
    ...
    if ( SOC_IS_ROBO5396(unit) )  {
        ...
        SOC_IF_ERROR_RETURN( (DRV_SERVICES(unit)->reg_read)
                ( unit, reg_addr, &reg_value, reg_len) ) ;
        SOC_IF_ERROR_RETURN( (DRV_SERVICES(unit)->reg_field_set)
                (unit, SDMIICTLr, &reg_value, AN_ENf, &temp));
    ...
    if ( SOC_IS_ROBO5396(unit) )  {
        ...
        *an = (int) temp ;
        ...
        SOC_IF_ERROR_RETURN( (DRV_SERVICES(unit)->reg_read)
                ( unit, reg_addr, &reg_value, reg_len) ) ;
        SOC_IF_ERROR_RETURN( (DRV_SERVICES(unit)->reg_field_get)
                (unit, SDMIISTSr, &reg_value, AUTO_NEGO_COMPf, &temp));
        *an_done = (int) temp ;
...
phy_serdesrobo_lb_set(int unit, soc_port_t port, int enable)
{
    uint8	phy_addr = PORT_TO_PHY_ADDR_INT(unit, port);
    uint16	ctrl;
    uint32	reg_addr, reg_value, reg_len, temp = (uint32) enable ;
    ...
    if ( SOC_IS_ROBO5396(unit) )  {
        ...
        SOC_IF_ERROR_RETURN( (DRV_SERVICES(unit)->reg_read)
                ( unit, reg_addr, &reg_value, reg_len) ) ;
        SOC_IF_ERROR_RETURN( (DRV_SERVICES(unit)->reg_field_set)
                (unit, SDMIICTLr, &reg_value, LOOPBACKf, &temp));
