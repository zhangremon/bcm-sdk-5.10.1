/*
 * $Id: phy54640.c 1.41.2.17 Broadcom SDK $
...
_phy_54640_eee_auto_enable(int unit, soc_port_t port, int enable)
{
    ...
    if (enable == 1) {
        ...
        if (PHY_IS_BCM54640E_A0A1(pc)) {
            ...
            SOC_IF_ERROR_RETURN
                (PHY54640_REG_MODIFY(unit, pc, 0x00, 0x0F7F,0x15, 0x7, 0x7)); /* exp 7f */
        ...
        SOC_IF_ERROR_RETURN
            (PHY54640_REG_MODIFY(unit, pc, 0x00, 0x0F7F,0x15, 0x1, 0x7)); /* exp 7f */
                     ...
                     soc_phy_control_t type, uint32 *value)
{
    ...
    switch(type) {
        ...
        case SOC_PHY_CONTROL_EEE_AUTO:
            ...
            SOC_IF_ERROR_RETURN
               (_phy_54640e_blk_top_lvl_reg_read(unit, pc, 0, 7, &data));
...
phy_54640_link_up(int unit, soc_port_t port)
{
    ...
    /* revs A0 and A1 of supported PHYs */
        ...
        SOC_IF_ERROR_RETURN
            (READ_PHY54640_EEE_ADVr(unit, pc, &eee_advert));
        if (eee_advert & 0x4) {
            sal_usleep(500000);
            SOC_IF_ERROR_RETURN(
...
phy_54640_timesync_config_set(int unit, soc_port_t port, soc_port_phy_timesync_config_t *conf)
{
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_LOOP_FILTER) {
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54640e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x42, (uint16)((COMPILER_64_LO(conf->phy_1588_dpll_loop_filter) >> 16) & 0xffff)));
    }
