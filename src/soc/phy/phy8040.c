/*
 * $Id: phy8040.c 1.12 Broadcom SDK $
...
_phy_8040_mux(int unit, soc_port_t port)
{
    ...
    soc_cm_debug(DK_PHY, "8040: u=%d port=%d primary=%d secondary=%d.\n", 
...
phy_8040_init(int unit, soc_port_t port)
{
    ...
    for (num_pc = 0,i = 0; i < dc->num_mports; i++) {
        ...
        if (new_pc->pd != NULL) {
            ...
            num_pc += 1;
            SOC_DEBUG_PRINT((DK_PHY, "_phy_8040_init: found next phy device"
                     ...
                     soc_phy_control_t type, uint32 value)
{
    ...
    if ((type != SOC_PHY_CONTROL_INTERFACE) && 
        (type != SOC_PHY_CONTROL_INTERFACE_MAX)) {
