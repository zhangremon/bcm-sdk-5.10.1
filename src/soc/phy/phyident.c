/*
 * $Id: phyident.c 1.323.2.19 Broadcom SDK $
                          ...
                          uint16 *phy_addr, uint16 *phy_addr_int)
{
    ...
    static const uint16 _katana_phy_ext_addr_mxqport[] = {
        0x0c, /* Port  25 Ext Bus=0/1 ExtAddr=0x0c */
        0x10, /* Port  26 Ext Bus=0/1 ExtAddr=0x10 */
        0x14, /* Port  27 Ext Bus=0/1 ExtAddr=0x14 */
        0x18, /* Port  28 Ext Bus=0/1 ExtAddr=0x18 */
        0x19, /* Port  29 Ext Bus=0/1 ExtAddr=0x19 */
        0x1a, /* Port  30 Ext Bus=0/1 ExtAddr=0x1a */
        0x1b, /* Port  31 Ext Bus=0/1 ExtAddr=0x1b */
        0x15, /* Port  32 Ext Bus=0/1 ExtAddr=0x15 */
        0x16, /* Port  33 Ext Bus=0/1 ExtAddr=0x16 */
        0x17, /* Port  34 Ext Bus=0/1 ExtAddr=0x17 */
    };
        ...
        *phy_addr_int = _katana_phy_int_addr_mxqport[port-25];
        switch (dev_id) {
            ...
            case BCM56447_DEVICE_ID:
                ...
                break;
            default:
                /* ToDo : Implement other variants as needed */
                break;
        }
    }
}
...
#ifdef BCM_KATANA_SUPPORT
         if (SOC_IS_KATANA(unit) && IS_GE_PORT(unit,port)) {
             uint32 rval;
             int port_mode = 2;

             if (IS_MXQ_PORT(unit,port)) {
                 SOC_IF_ERROR_RETURN(READ_XPORT_MODE_REGr(unit, port, &rval));
                 port_mode = soc_reg_field_get(unit, XPORT_MODE_REGr, rval, CORE_PORT_MODEf);
             }
             if (port_mode) {
                 return TRUE;
             } else {
               ...
               soc_phy_info_t *pi, phy_ctrl_t *int_pc)
{
    ...
    if (IS_GE_PORT(unit, port) && (&phy_xgxs16g1l_ge == int_phyd)) {
	  ...
	  int_pc->flags |= PHYCTRL_MDIO_ADDR_SHARE;
            switch(SOC_PORT_INFO(unit, port).bindex) {
            case 8:
                int_pc->lane_num = 0;
                break;
