/*
 * $Id: phy543xx.c 1.8.2.12 Broadcom SDK $
...
_phy_54380_medium_check(int unit, soc_port_t port, int *medium)
{
    ...
    rv = bmacsec_phy54380_medium_get(phy_dev_addr, &bmacsec_port_medium); 
...
STATIC int
_phy_54380_no_reset_setup(int unit, soc_port_t port, int fiber)
{
    phy_ctrl_t    *int_pc;
    phy_ctrl_t    *pc;
    bmacsec_dev_addr_t phy_dev_addr;
    ...
    SOC_DEBUG_PRINT((DK_PHY, "_phy_54380_reset_setup: u=%d p=%d medium=%s\n",
                     unit, port,
...
phy_54380_init(int unit, soc_port_t port)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "phy_54xxx_init: u=%d p=%d\n",
                     unit, port));
    ...
    if (pc->phy_model == BMACSEC_PHY_BCM54340_MODEL) {
    ...
    if (mmi_mdio_addr == -1) {
        soc_cm_debug(DK_WARN, "phy_54380_init: "
    ...
    rv = bmacsec_phy54380_phy_mac_addr_set(phy_dev_addr, macsec_dev_addr, 
                                   ...
                                   mac_config_flag); 
    if(rv != BMACSEC_E_NONE) {
        SOC_DEBUG_PRINT((DK_WARN, "phy_54380_init: bmacsec_phy_mac_addr_set "
    ...
    SOC_DEBUG_PRINT((DK_PHY, "phy_54380_init: "
...
phy_54380_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    if (pc->automedium || PHY_FIBER_MODE(unit, port)) {
        ...
        SOC_DEBUG_PRINT((DK_PHY, "phy_54380_enable_set: "
                         ...
                         soc_port_medium_t force_medium)
{
    ...
    if (medium == SOC_PORT_MEDIUM_COPPER) {
        if (!PHY_COPPER_MODE(unit, port) || force_update) { /* Was fiber */ 
            ...
            SOC_IF_ERROR_RETURN
                (_phy_54380_no_reset_setup(unit, port, 0));
        ...
        if (PHY_COPPER_MODE(unit, port) || force_update) { /* Was copper */
            ...
            PHY_FLAGS_SET(unit, port, PHY_FLAGS_MEDIUM_CHANGE);
            SOC_IF_ERROR_RETURN
                (_phy_54380_no_reset_setup(unit, port, 1));
 ...
 */
STATIC int
phy_54380_cable_diag(int unit, soc_port_t port,
                    soc_port_cable_diag_t *status)
{
    ...
    if (rv <= 0) {      /* don't reset if > 0 -- link was up */
        phy_dev_addr = SOC_MACSECPHY_MDIO_ADDR(unit, pc->phy_id, 0); 
        rv2 = bmacsec_phy54380_init(phy_dev_addr, pc->macsec_dev_addr, 
                               1,
                               pc->automedium,
                               pc->fiber.preferred,
                               pc->fiber_detect,
...
/* CLOCK CONTROLS  */
    ...
    case SOC_PHY_CONTROL_EEE:
        ...
        if (value == 1) {
            /* Enable EEE */
            SOC_IF_ERROR_RETURN
                (bmacsec_phy_54380_eee_enable(phy_dev_addr, port_mode, 
                          (bmacsec_port_ability_t*)&phy_config.advert_ability, 1));

        } else {
            /* Disable EEE */
            SOC_IF_ERROR_RETURN
                (bmacsec_phy_54380_eee_enable(phy_dev_addr, port_mode, 
                          (bmacsec_port_ability_t*)&phy_config.advert_ability, 0)); 
        }
    ...
    case SOC_PHY_CONTROL_EEE_AUTO:
        ...
        if (value == 1) {
            /* Enable AutoGrEEEn */
            SOC_IF_ERROR_RETURN
                (bmacsec_phy_54380_eee_auto_enable(phy_dev_addr, 
                         (bmacsec_port_t)port, port_mode, 1));

        } else {
            /* Disable AutoGrEEEn */
            SOC_IF_ERROR_RETURN
                (bmacsec_phy_54380_eee_auto_enable(phy_dev_addr, 
                          (bmacsec_port_t)port, port_mode, 0));
        }
                     ...
                     soc_phy_control_t type, uint32 *value)
{
    ...
    switch(type) {
        ...
        case SOC_PHY_CONTROL_EEE_AUTO:
            ...
            bmacsec_phy_54380_get_auto_eee_control_status(phy_dev_addr, (bmacsec_port_t)port, value);
        break;
