/*
 * $Id: phy_mac_ctrl.c 1.3.2.2 Broadcom SDK $
...
extern phy_mac_drv_t unimac_drv;
#if 0
extern phy_mac_drv_t bigmac_drv;
#endif
extern phy_mac_drv_t xmac_drv;
                         ...
                         blmi_dev_io_f   mmi_cbf)
{
    ...
    if (mac_type == PHY_MAC_CORE_UNIMAC) {
        sal_memcpy(&mmc->mac_drv, &unimac_drv, sizeof(phy_mac_drv_t));
#if 0
    } else if (mac_type == PHY_MAC_CORE_BIGMAC) {
        sal_memcpy(&mmc->mac_drv, &bigmac_drv, sizeof(phy_mac_drv_t));
#endif
    } else if (mac_type == PHY_MAC_CORE_XMAC) {
        sal_memcpy(&mmc->mac_drv, &xmac_drv, sizeof(phy_mac_drv_t));
    } else {
        sal_free(mmc);
        mmc = NULL;
    }
