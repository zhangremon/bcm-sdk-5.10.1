/*
 * $Id: wc40.c 1.120.2.22 Broadcom SDK $ 
...
#define WC40_CL73_SOFT_KR2(_pc)  ((DEV_CFG_PTR(_pc)->cl73an == WC40_CL73_KR2) || \
				  (DEV_CFG_PTR(_pc)->cl73an == WC40_CL73_KR2_HP) )
...
STATIC int _wc40_soft_an_cl73kr2_hp(int unit, soc_port_t port, int an);
STATIC int _wc40_soft_an_cl73kr2_check(int unit, soc_port_t port);
STATIC int _wc40_soft_an_cl73kr2_hp_check(int unit, soc_port_t port);
...
phy_wc40_link_get(int unit, soc_port_t port, int *link)
{
    ...
    *link = FALSE;
    ...
    } else if (DEV_CFG_PTR(pc)->cl73an == WC40_CL73_KR2_HP) {
        SOC_IF_ERROR_RETURN
            (_wc40_soft_an_cl73kr2_hp_check(unit,port));
    }
...
_phy_wc40_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    if ((force_cl72 != FORCE_CL72_ENABLED(pc)) || (force_cl72 == TRUE)) {
        SOC_IF_ERROR_RETURN
            (_phy_wc40_force_cl72_config(unit,port,force_cl72));
        FORCE_CL72_ENABLED(pc) = force_cl72;
    }
...
_wc40_soft_an_cl73kr2_init(int unit, soc_port_t port)
{
    ...
    DEV_CTRL_PTR(pc)->soft_kr2.cnt = 0;
...
STATIC int
_wc40_soft_an_cl73kr2_hp(int unit, soc_port_t port, int an)
{
    return SOC_E_NONE;
}
...
STATIC int
_wc40_soft_an_cl73kr2_hp_check(int unit, soc_port_t port)
{
    return SOC_E_NONE;
}
...
_wc40_soft_an_cl73kr2_check(int unit, soc_port_t port)
{
    ...
    if (data16 & (1 << 6)) {
        ...
        lp_nxp0 = lp_nxp0 & AN_IEEE1BLK_AN_LP_XNP_ABILITY0_MSG_MASK;

        /* received base page, start soft autoneg */
        if ((lp_nxp0 == 0) && (lp_nxp1 == 0) && (lp_nxp2 == 0)) {
           DEV_CTRL_PTR(pc)->soft_kr2.state = WC40_SOFT_KR2_STATE_INIT; 
        ...
        if (DEV_CTRL_PTR(pc)->soft_kr2.state == WC40_SOFT_KR2_STATE_INIT) {
            ...
            if ((lp_nxp0 == 0) && (lp_nxp1 == 0) && (lp_nxp2 == 0)) {
                ...
                SOC_IF_ERROR_RETURN
                    (WRITE_WC40_AN_IEEE1BLK_AN_XNP_TRANSMIT1r(unit, pc, 0x00, WC40_BRCM_OUI0));
                SOC_IF_ERROR_RETURN
                    (WRITE_WC40_AN_IEEE1BLK_AN_XNP_TRANSMIT2r(unit, pc, 0x00, WC40_BRCM_OUI1));

                /* indicate MP-5 page and next page set */
                SOC_IF_ERROR_RETURN
                    (WRITE_WC40_AN_IEEE1BLK_AN_XNP_TRANSMIT0r(unit, pc, 0x00, 
                       0x5 | AN_IEEE1BLK_AN_XNP_TRANSMIT0_NP_MASK));
            ...
            if ((lp_nxp0 == WC40_AUTONEG_MSG_MP5) && (lp_nxp1 == WC40_BRCM_OUI0) &&
                (lp_nxp2 == WC40_BRCM_OUI1)) {
            
                /* match BRCM MP-5, now send BAM3 page indicating KR2 */
                SOC_IF_ERROR_RETURN
                    (WRITE_WC40_AN_IEEE1BLK_AN_XNP_TRANSMIT1r(unit, pc, 0x00, 
                            DEV_CTRL_PTR(pc)->soft_kr2.ld_kr2? WC40_BRCM_ETA_KR2: 0));
            ...
            if ((lp_nxp0 == WC40_BRCM_CODE_BAM3) && (lp_nxp2 == 0)) {
                ...
                /* KR2 match */
                if (lp_nxp0 == WC40_BRCM_CODE_BAM3 && lp_nxp1 == WC40_BRCM_ETA_KR2) {
...
phy_wc40_an_set(int unit, soc_port_t port, int an)
{
    ...
    if (WC40_CL73_SOFT_KR2(pc)) {
        if (DEV_CFG_PTR(pc)->cl73an == WC40_CL73_KR2) {
            ...
            SOC_IF_ERROR_RETURN
                (_wc40_soft_an_cl73kr2_hp(unit,port,an));
        }
    } else {
