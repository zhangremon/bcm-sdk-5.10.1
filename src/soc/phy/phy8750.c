...
/*
 * $Id: phy8750.c 1.13.2.2 Broadcom SDK $
...
phy_8750_init(int unit, soc_port_t port)
{
    ...
    if ((chip_id != 0x8750) && (chip_id != 0x8752) && (chip_id != 0x8754)) {
        soc_cm_print("Unsupported PHY: Chip Id = %x\n", chip_id); 
        return SOC_E_FAIL;
    }
    ...
    if (ucode_ver >= 0x0301) {
        ...
        if (data16 != 0x600D || SOC_FAILURE(rv)) {
            soc_cm_debug(DK_WARN,"8750: p=%d SPI-ROM load: Bad Checksum (0x%x)\n",
                     port, data16);
        ...
        soc_cm_debug(DK_WARN,"8750: p=%d SPI-ROM load: Invalid Ucode version (0x%x)\n",
                     port, ucode_ver);
             /* let it continue to allow other ports to initialize */
    }

    soc_cm_debug(DK_PHY, "8750: u=%d port%d rom code ver: 0x%x: init.\n",
         unit, port, ucode_ver);
    ...
    soc_cm_print("BCM%x (Rev. %d): u=%d port%d rom code ver. 0x%x\n",
...
phy_8750_an_set(int unit, soc_port_t port, int an)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "phy_8750_an_set: u=%d p=%d an=%d\n",
                     unit, port, an));
...
phy_8750_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    soc_cm_debug(DK_PHY,"phy_8750_speed_set: u=%d p=%d speed=%d\n", 
                 unit, port,speed);
...
_phy8754_mdio_firmware_download(int unit, int port, int offset, uint8 *array,int datalen)
{
    ...
    if (data16 != 0x4321) {
        /* Download failure */
        soc_cm_print("_phy8754_mdio_firmware_download fail,p=%d done msg 0x%x\n",
                     port,data16);
        return SOC_E_FAIL;
    }
    ...
    SOC_DEBUG_PRINT((DK_PHY, "_phy8754_mdio_firmware_download: p=%d msgout 0x%x\n", 
                     port,data16));
    ...
    if (data16 != 0x600D) {
        /* Bad CHECKSUM */
        soc_cm_print("_phy8754_mdio_firmware_download, bad cksum: p=%d, cksum=0x%x\n",
                    port,data16);
        return SOC_E_FAIL;
    }
...
phy_8750_rom_wait(int unit, int port)
{
    ...
    if (!(rd_data & 0x0100)) {
        soc_cm_debug(DK_ERR,"phy_8750_rom_program: u = %d p = %d "
                            "timeout 1\n", unit, port);
...
phy_8750_firmware_set(int unit, int port, int offset, uint8 *array,int datalen)
{
    ...
    soc_cm_debug(DK_PHY, "phy_8750_rom_program: u=%d p=%d done\n", unit, port);
