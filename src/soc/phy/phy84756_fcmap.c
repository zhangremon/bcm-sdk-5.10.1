/*
 * $Id: phy84756_fcmap.c 1.6.2.11 Broadcom SDK $
 *
 ...
 */
#define PHY84756_PHY_INFO(pc)  ((phy84756_map_t *)(pc->driver_data))
...
{
    phy_ctrl_t *pc;
    blmi_dev_addr_t phy_dev_addr;
    blmi_dev_addr_t fcmap_dev_addr;
    blmi_dev_addr_t macsec_dev_addr;
    int line_mode;
    int fiber_preferred;
    int rv = SOC_E_NONE;
    int fcmap_enable = 0, macsec_enable = 0,  mmi_mdio_addr;
    ...
    pc->driver_data = (void *)sal_alloc (sizeof (phy84756_map_t), 
                              pc->pd->drv_name);
    if (pc->driver_data == NULL) {
        return SOC_E_MEMORY;
    }
    sal_memset(pc->driver_data, 0, sizeof(phy84756_map_t));
    ...
    if (fcmap_enable) {
        ...
        fcmap_dev_addr = SOC_FCMAPPHY_MDIO_ADDR(unit, mmi_mdio_addr, 1);
        pc->fcmap_dev_addr    = fcmap_dev_addr;
        pc->fcmap_uc_dev_addr = phy_dev_addr;
    ...
    SOC_DEBUG_PRINT((DK_PHY, "phy_84756_fcmap_init: u=%d p=%d\n",unit, port));
...
phy_84756_fcmap_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    /* PMA/PMD */
    SOC_IF_ERROR_RETURN(
    BFCMAP_MOD_PHY84756_LN_DEV1_PMD_CTRLr(pc, power, 
                                         BFCMAP_PHY84756_PMD_CTRL_PD));
    /* PCS */
    SOC_IF_ERROR_RETURN(
            BFCMAP_MOD_PHY84756_LN_DEV3_PCS_CTRLr(pc, power, 
                                         BFCMAP_PHY84756_PCS_PCS_CTRL_PD));
    /* AN */
    SOC_IF_ERROR_RETURN(
            BFCMAP_MOD_PHY84756_LN_DEV7_AN_MII_CTRLr(pc, power, 
...
phy_84756_fcmap_an_set(int unit, soc_port_t port, int autoneg)
{
    ...
    SOC_IF_ERROR_RETURN
        (BFCMAP_WR_PHY84756_LN_DEV1_PMDr(pc, 0x0096, autoneg ? 0x2 : 0x0));

    SOC_IF_ERROR_RETURN(
        BFCMAP_RD_PHY84756_XFI_DEV7_AN_CTRLr(pc, &data));
    if (autoneg) {
        data |= (BFCMAP_PHY84756_AN_AN_CTRL_AE |
                BFCMAP_PHY84756_AN_AN_CTRL_RAN);
    } else {
        data &= ~(BFCMAP_PHY84756_AN_AN_CTRL_AE |
                  BFCMAP_PHY84756_AN_AN_CTRL_RAN);
    }
    SOC_IF_ERROR_RETURN(
        BFCMAP_WR_PHY84756_XFI_DEV7_AN_CTRLr(pc, data));
                     ...
                     int *autoneg, int *autoneg_done)
{
    int           rv;
    phy_ctrl_t   *pc;
    buint16_t an_ctrl = 0, an_stat = 0;
    buint16_t an_mii_ctrl = 0, an_mii_stat = 0;
    bfcmap_phy84756_intf_t line_mode;
    ...
    *autoneg_done = FALSE;
    ...
    if (line_mode == BFCMAP_PHY84756_INTF_SFI) {
        SOC_IF_ERROR_RETURN(
            BFCMAP_RD_PHY84756_LN_DEV7_AN_CTRLr(pc, &an_ctrl));
        SOC_IF_ERROR_RETURN(
            BFCMAP_RD_PHY84756_LN_DEV7_AN_STATr(pc, &an_stat));
        *autoneg = (an_ctrl & BFCMAP_PHY84756_AN_AN_CTRL_AE) ? 1 : 0;
        *autoneg_done = (an_stat & BFCMAP_PHY84756_AN_AN_STAT_AN_DONE) ? 1 : 0;
    } else {
        SOC_IF_ERROR_RETURN(
            BFCMAP_RD_PHY84756_LN_DEV7_AN_MII_CTRLr(pc, &an_mii_ctrl));
        SOC_IF_ERROR_RETURN(
            BFCMAP_RD_PHY84756_LN_DEV7_AN_MII_STATr(pc, &an_mii_stat));
        *autoneg = (an_mii_ctrl & BFCMAP_PHY84756_AN_MII_CTRL_AE) ? 1 : 0;
        *autoneg_done = 
            (an_mii_stat & BFCMAP_PHY84756_AN_MII_STAT_AN_DONE) ? 1 : 0;
    }
...
phy_84756_fcmap_probe(int unit, phy_ctrl_t *pc)
{
        ...
        if (0xff != soc_property_port_get(unit, pc->port, spn_FCMAP_ENABLE, 0)) {
            soc_cm_print("Using SDK PHY driver for BCM84756/7/9\n");
            return SOC_E_NONE;
        }
        return SOC_E_NOT_FOUND;
#else
        soc_cm_print("Using SDK PHY driver for BCM84756/7/9 (no MACSEC support)\n");
        return SOC_E_NONE;
#endif
}
    ...
    if (phy_ext_boot) {
        ...
        SOC_IF_ERROR_RETURN(
            bfcmap_phy84756_mdio_firmware_download(pc,
                                                    bcm84756_fw, bcm84756_fw_length));
    } 
                    ...
                    int addr, int count, void *data_array,buint32_t ram_start)
{
    ...
    if (count > BFCMAP_PHY84756_BSC_XFER_MAX) {
        return SOC_E_PARAM;
    }

    data_type = BFCMAP_PHY84756_I2C_DATA_TYPE(opr);
    access_type = BFCMAP_PHY84756_I2C_ACCESS_TYPE(opr);

    if (access_type == BFCMAP_PHY84756_I2CDEV_WRITE) {
        for (i = 0; i < count; i++) {
            if (data_type == BFCMAP_PHY84756_I2C_8BIT) {
                SOC_IF_ERROR_RETURN
    ...
    data16 |= (dev_addr<<9);
    if (access_type == BFCMAP_PHY84756_I2CDEV_WRITE) {
        data16 |= BFCMAP_PHY84756_WR_FREQ_400KHZ;
    }
    ...
    if (access_type == BFCMAP_PHY84756_I2CDEV_WRITE) {
        data16 =  0x8000 | BFCMAP_PHY84756_BSC_WRITE_OP;
    } else {
        data16 =  0x8000 | BFCMAP_PHY84756_BSC_READ_OP;
    }

    if (data_type == BFCMAP_PHY84756_I2C_16BIT) {
        data16 |= (1 << 12);
    }
    ...
    while (iter < 100000) {
        ...
        if (((data16 & BFCMAP_PHY84756_2W_STAT) == BFCMAP_PHY84756_2W_STAT_COMPLETE)) {
            break;
        }
        iter++;
    }
...
{
    return _phy_84756_bsc_rw(pc, dev_addr,BFCMAP_PHY84756_I2CDEV_READ,
             offset, nbytes, (void *)read_array,BFCMAP_PHY84756_READ_START_ADDR);
                     ...
                     buint8_t *write_array)   /* buffer to hold written data */
{
    ...
    for (j = 0; j < (nbytes/BFCMAP_PHY84756_BSC_WR_MAX); j++) {
        rv = _phy_84756_bsc_rw(pc, dev_addr,BFCMAP_PHY84756_I2CDEV_WRITE,
                    offset + j * BFCMAP_PHY84756_BSC_WR_MAX, BFCMAP_PHY84756_BSC_WR_MAX,
                    (void *)(write_array + j * BFCMAP_PHY84756_BSC_WR_MAX),
                    BFCMAP_PHY84756_WRITE_START_ADDR);
        if (rv != SOC_E_NONE) {
            return rv;
        }
        sal_usleep(20000);
    }
    if (nbytes%BFCMAP_PHY84756_BSC_WR_MAX) {
        rv = _phy_84756_bsc_rw(pc, dev_addr,BFCMAP_PHY84756_I2CDEV_WRITE,
                offset + j * BFCMAP_PHY84756_BSC_WR_MAX, nbytes%BFCMAP_PHY84756_BSC_WR_MAX,
                (void *)(write_array + j * BFCMAP_PHY84756_BSC_WR_MAX),
                BFCMAP_PHY84756_WRITE_START_ADDR);
    }
    return rv;
