/*
 * $Id: phy5421S.c 1.40 Broadcom SDK $
...
_phy_5421S_medium_select(int unit, soc_port_t port, int medium)
{
    ...
    DPRINTF("5421S_medium_select: port=%d medium=%d fp=%d\n",
...
_phy_5421S_reset_setup(int unit, soc_port_t port)
{
    ...
    DPRINTF("_phy_5421S_reset_setup: u=%d p=%d\n", unit, port);
...
phy_5421S_init(int unit, soc_port_t port)
{
    ...
    DPRINTF("phy_5421S_init: u=%d p=%d\n", unit, port);
...
phy_5421S_link_get(int unit, soc_port_t port, int *link)
{
    ...
    if (medium != pc->medium) {
        ...
        DPRINTF("phy_5421S_link_get: u=%d p=%d: medium %d-->%d\n",
...
phy_5421S_duplex_set(int unit, soc_port_t port, int duplex)
{
    ...
    DPRINTF("phy_5421S_duplex_set: u=%d p=%d d=%d m=%d\n",
...
phy_5421S_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    DPRINTF("phy_5421S_speed_set: u=%d p=%d s=%d m=%d\n",
...
phy_5421S_autoneg_set(int unit, soc_port_t port, int autoneg)
{
    ...
    DPRINTF("phy_5421S_autoneg_set: u=%d p=%d autoneg=%d m=%d\n",
...
phy_5421S_adv_local_set(int unit, soc_port_t port, soc_port_mode_t mode)
{
    ...
    DPRINTF("phy_5421S_adv_local_set: u=%d p=%d mode=0x%x m=%d\n",
...
phy_5421S_lb_set(int unit, soc_port_t port, int enable)
{
    ...
    DPRINTF("phy_5421S_lb_set: u=%d p=%d en=%d m=%d\n",
...
phy_5421S_interface_set(int unit, soc_port_t port, soc_port_if_t pif)
{
    ...
    DPRINTF("phy_5421S_interface_set: u=%d p=%d pif=%d\n", unit, port, pif);
    ...
    if (SOC_IS_ESW(unit)) {
       ...
       soc_reg64_field32_set(unit, GPCSCr, &gpcsc, EWRAPf, tbi);
    ...
    DPRINTF("5421S_if_set: port=%d tbi=%d reg 0x10=0x%x\n",
            port, tbi, mii_ecr);
    ...
    if ((tbi && !(mii_ecr & MII_ECR_10B)) ||
        ...
        DPRINTF("5421S_if_set: port=%d set reg 0x10=0x%x and toggle TBI\n",
                port, mii_ecr);
...
phy_5421S_ability_get(int unit, soc_port_t port, soc_port_mode_t *mode)
{
    ...
    DPRINTF("phy_5421S_ability_get: u=%d p=%d mode=0x%x\n",
            unit, port, *mode);
...
phy_5421S_link_down(int unit, soc_port_t port)
{
    ...
    DPRINTF("5421S_link_down: port=%d m=%d\n", port, pc->medium);
