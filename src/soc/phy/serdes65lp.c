/*
 * $Id: serdes65lp.c 1.40.20.1 Broadcom SDK $
...
_phy_serdes65lp_init_no_reset(int unit, soc_port_t port)
{
    ...
    if (!PHY_FLAGS_TST(unit, port, PHY_FLAGS_INIT_DONE)) {
        /* Since physerdes65lp driver reset function vector does not reset
         * the SerDes, reset the SerDes in initialization first. 
         * The internal SERDES PHY's reset bit is self-clearing.
         * MII_CTRL_RESET on slice 1 will reset all other slices. Do it
         * only once 
         */
        SOC_IF_ERROR_RETURN
        ...
        while (!soc_timeout_check(&to)) {
            rv = READ_SERDES65LP_MII_CTRLr(unit, pc, &data16);
