/*
 * $Id: phyctrl.c 1.94.2.8 Broadcom SDK $
...
{ 
    ...
    if (SOC_FAILURE(rv)) {
        PHYCTRL_DEBUG((DK_WARN, "soc_phyctrl_diag_ctrl failed %d\n", rv));
    }
    return rv;
}
...
soc_phy_primary_and_offset_get(int unit, soc_port_t port, soc_port_t *primary_port, int *offset) 
{
    ...
    if (!(primary_and_offset = soc_property_port_get(unit, port, spn_PHY_PORT_PRIMARY_AND_OFFSET, 0))) {
        return  SOC_E_FAIL;
    }
