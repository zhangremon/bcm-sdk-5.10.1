...
soc_pci_off2name(int unit, uint32 offset)
{
    ...
    if (offset < 0x50) {
        ...
        sal_strcpy(buf, "CMIC_LED_CTRL");
    } else if (led && offset == 0x1004) {
        sal_strcpy(buf, "CMIC_LED_STATUS");
...
soc_pci_getreg(int unit, uint32 addr, uint32 *datap)
{
    *datap = CMREAD(unit, addr);
    soc_cm_debug(DK_PCI, "PCI%d memR(0x%x)=0x%x\n", unit, addr, *datap);
    return SOC_E_NONE;
}
...
{
    soc_cm_debug(DK_PCI, "PCI%d memW(0x%x)=0x%x\n", unit, addr, data);
