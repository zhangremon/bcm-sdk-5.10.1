 ...
 *   - IPIC on Draco (Port 12 - the uplink port)
 *
 */
...
_port2pllf(int unit, soc_port_t port)
{
    ...
    if (SOC_IS_EASYRIDER(unit)) {
        switch(port) {
            case 12: return PLL_CONTROL_12f;
            case  0: return PLL_CONTROL_13f;
            default: return INVALIDf;
        }
    }
    ...
    *pa_flag = 0;
    if (soc_feature(unit, soc_feature_xgxs_lcpll)) {
        ...
        for (idx = 0; idx < sizeof(mac_big_hg_speed_list) /
            ...
            if (speed == mac_big_hg_speed_list[idx].speed) {
                *pa_flag = mac_big_hg_speed_list[idx].pa_flag;
                break;
            }
        }
    }
...
mac_big_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    DBG_10G_VERB(("mac_big_enable_set: unit %d port %s enable=%s\n",
...
_fusioncore_phy_xgxs1_init(int unit, soc_port_t port)
{
    ...
    if (phyrev == 1) {
        ...
        uint16 plldiv = ((SOC_IS_DRACO15(unit) ||
                          SOC_IS_HERCULES15(unit)) ? 0x0000 : 0x6c33);
    ...
    if (SAL_BOOT_QUICKTURN) {
        DBG_10G_VERB(("SIMULATION: _fusioncore_phy_xgxs1_init: unit %d "
    ...
    if (!locked) {
        ...
        DBG_10G_VERB(("_fusioncore_phy_xgxs1_init: unit %d port %s "
...
{
    DBG_10G_VERB(("soc_fusioncore_reset: unit %d port %s\n",
                  unit, SOC_PORT_NAME(unit, port)));
...
_mac_big_init(int unit, soc_port_t port)
{
    ...
    DBG_10G_VERB(("_mac_big_init: unit %d port %s\n",
                  unit, SOC_PORT_NAME(unit, port)));
    ...
    if (SOC_IS_SIRIUS(unit)) {
       ...
       soc_reg64_field32_set(unit, MAC_TXCTRLr, &tx_ctrl, CRC_MODEf,
                             SOC_IS_HERCULES1(unit) ? BIGMAC_CRC_KEEP :
                             BIGMAC_CRC_REGEN);
#if defined(BCM_SIRIUS_SUPPORT)
    }
#endif
...
mac_big_enable_get(int unit, soc_port_t port, int *enable)
{
    ...
    *enable = soc_reg64_field32_get(unit, MAC_CTRLr, ctrl, RXENf);
    ...
    DBG_10G_VERB(("mac_big_enable_get: unit %d port %s enable=%s\n",
                  unit, SOC_PORT_NAME(unit, port),
                  *enable ? "True" : "False"));
...
{
    DBG_10G_VERB(("mac_big_duplex_set: unit %d port %s duplex=%s\n",
                  unit, SOC_PORT_NAME(unit, port),
                  duplex ? "Full" : "Half"));
    return SOC_E_NONE;
}
...
mac_big_duplex_get(int unit, soc_port_t port, int *duplex)
{
    *duplex = TRUE; /* Always full duplex */
    ...
    DBG_10G_VERB(("mac_big_duplex_get: unit %d port %s duplex=%s\n",
                  unit, SOC_PORT_NAME(unit, port),
                  *duplex ? "Full" : "Half"));
    return SOC_E_NONE;
}
...
mac_big_pause_set(int unit, soc_port_t port, int pause_tx, int pause_rx)
{
    ...
    DBG_10G_VERB(("mac_big_pause_set: unit %d port %s RX=%s TX=%s\n",
                  unit, SOC_PORT_NAME(unit, port),
                  pause_rx ? "on" : "off",
                  pause_tx ? "on" : "off"));
...
mac_big_pause_get(int unit, soc_port_t port, int *pause_tx, int *pause_rx)
{
    ...
    *pause_tx = soc_reg64_field32_get(unit, MAC_TXCTRLr, tx, PAUSEENf);
    ...
    DBG_10G_VERB(("mac_big_pause_get: unit %d port %s RX=%s TX=%s\n",
                  unit, SOC_PORT_NAME(unit, port),
                  *pause_rx ? "on" : "off",
                  *pause_tx ? "on" : "off"));
    return SOC_E_NONE;
...
{
    DBG_10G_VERB(("mac_big_speed_set: unit %d port %s speed=%dMb\n",
                  unit, SOC_PORT_NAME(unit, port),
                  speed));
...
mac_big_speed_get(int unit, soc_port_t port, int *speed)
{
    *speed = 10000;
    ...
    if (soc_reg_field_valid(unit, MAC_XGXS_STATr, SPEED_10000f)) {
        ...
        DBG_10G_VERB(("mac_big_speed_get: unit %d port %s speed=%dMb\n",
                      unit, SOC_PORT_NAME(unit, port),
                      *speed));
        return SOC_E_NONE;
    }
...
mac_big_loopback_set(int unit, soc_port_t port, int lb)
{
    ...
    DBG_10G_VERB(("mac_big_loopback_set: unit %d port %s loopback=%s\n",
                  unit, SOC_PORT_NAME(unit, port),
                  lb ? "local" : "no"));
...
mac_big_loopback_get(int unit, soc_port_t port, int *lb)
{
    ...
    *lb = local | remote;
    ...
    DBG_10G_VERB(("mac_big_loopback_get: unit %d port %s loopback=%s\n",
                  unit, SOC_PORT_NAME(unit, port),
...
mac_big_pause_addr_set(int unit, soc_port_t port, sal_mac_addr_t m)
{
    ...
    DBG_10G_VERB(("mac_big_pause_addr_set: unit %d port %s MAC=<"
...
mac_big_pause_addr_get(int unit, soc_port_t port, sal_mac_addr_t m)
{
    ...
    DBG_10G_VERB(("mac_big_pause_addr_get: unit %d port %s MAC=<"
                  "%02x:%02x:%02x:%02x:%02x:%02x>\n",
                  unit, SOC_PORT_NAME(unit, port),
                  m[0], m[1], m[2], m[3], m[4], m[5]));
    return SOC_E_NONE;
...
{
    DBG_10G_VERB(("mac_big_interface_set: unit %d port %s interface=%s\n",
                  unit, SOC_PORT_NAME(unit, port),
...
mac_big_interface_get(int unit, soc_port_t port, soc_port_if_t *pif)
{
    ...
    *pif = SOC_PORT_IF_MII;
    ...
    DBG_10G_VERB(("mac_big_interface_get: unit %d port %s interface=%s\n",
                  unit, SOC_PORT_NAME(unit, port),
                  mac_big_port_if_names[*pif]));
    return SOC_E_NONE;
}
...
mac_big_frame_max_set(int unit, soc_port_t port, int size)
{
    ...
    DBG_10G_VERB(("mac_big_frame_max_set: unit %d port %s size=%d\n",
                  unit, SOC_PORT_NAME(unit, port),
                  size));
...
mac_big_frame_max_get(int unit, soc_port_t port, int *size)
{
    ...
    DBG_10G_VERB(("mac_big_frame_max_get: unit %d port %s size=%d\n",
                  unit, SOC_PORT_NAME(unit, port),
                  *size));
    return rv;
}
                ...
                soc_port_duplex_t duplex, int ifg)
{
    ...
    DBG_10G_VERB(("mac_big_ifg_set: unit %d port %s speed=%dMb duplex=%s "
                  "ifg=%d\n",
                  unit, SOC_PORT_NAME(unit, port),
                ...
                soc_port_duplex_t duplex, int *ifg)
{
    ...
    DBG_10G_VERB(("mac_big_ifg_get: unit %d port %s speed=%dMb duplex=%s "
                  "ifg=%d\n",
                  unit, SOC_PORT_NAME(unit, port),
                  speed, duplex ? "True" : "False", *ifg));
    return SOC_E_NONE;
...
mac_big_loopback_remote_set(int unit, soc_port_t port, int lb)
{
    ...
    DBG_10G_VERB(("mac_big_loopback_remote_set: unit %d port %s loopback=%s\n",
                  unit, SOC_PORT_NAME(unit, port),
                  lb ? "remote" : "no"));
...
mac_big_encap_set(int unit, soc_port_t port, int mode)
{
    ...
    DBG_10G_VERB(("mac_big_encap_set: unit %d port %s encapsulation=%s\n",
                  unit, SOC_PORT_NAME(unit, port),
...
mac_big_encap_get(int unit, soc_port_t port, int *mode)
{
    ...
    *mode = soc_reg64_field32_get(unit, MAC_RXCTRLr, rx_ctrl, HDRMODEf);
    ...
    DBG_10G_VERB(("mac_big_encap_get: unit %d port %s encapsulation=%s\n",
                  unit, SOC_PORT_NAME(unit, port),
                  mac_big_encap_mode[*mode]));
    return SOC_E_NONE;
}
...
{
#if defined(BCM_XGS3_SUPPORT) || defined(BCM_LYNX_SUPPORT) || defined(BCM_SIRIUS_SUPPORT)
    if (SOC_IS_XGS3_SWITCH(unit) || SOC_IS_LYNX(unit) || SOC_IS_SIRIUS(unit)) {
        ...
        if (SOC_IS_LYNX(unit)) {
            if (value) {
                SOC_IF_ERROR_RETURN(soc_reg_field32_modify(unit, MAC_TXSPARE0r,
                                    port, WAN_MODEf, 1));
            } else {
                SOC_IF_ERROR_RETURN(soc_reg_field32_modify(unit, MAC_TXSPARE0r,
                                    port, WAN_MODEf, 0));
...
#endif /* BCM_XGS3_SUPPORT || BCM_LYNX_SUPPORT || BCM_SIRIUS_SUPPORT */
                    ...
                    int value)
{
    ...
    DBG_10G_VERB(("mac_big_control_set: unit %d port %s type=%d value=%d\n",
                  unit, SOC_PORT_NAME(unit, port),
                  type, value));
                    ...
                    int *value)
{
    ...
    DBG_10G_VERB(("mac_big_control_get: unit %d port %s type=%d value=%d "
                  "rv=%d\n",
                  unit, SOC_PORT_NAME(unit, port),
                  type, *value, rv));
                          ...
                          soc_port_ability_t *ability)
{
    ...
    if (!IS_HG_PORT(unit , port)) {
        DBG_10G_VERB(("mac_big_ability_local_get: unit %d port %s "
