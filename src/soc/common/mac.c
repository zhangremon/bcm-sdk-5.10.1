/*
 * $Id: mac.c 1.34.6.3 Broadcom SDK $
 ...
 * set the PAUSE_CONTROL register at initialization time.  The 5690
 * counts in different units depending on which mac mode is in use and
 * so the PAUSE_CONTROL register has to be updated whenever the mac mode
 * is flipped:
 *	PAUSE_VAL(32) in 1000 mode: 5690 1000 mac counts in 32 bit times
 *	PAUSE_VAL(4) in 10/100 mode: 5690 10/100 mac counts in 4 bit times
 */
...
soc_mac_speed_get(int unit, soc_port_t port, int *speed)
{
    ...
    if (IS_FE_PORT(unit, port)) {
        ...
        if (soc_reg64_field32_get(unit, FE_SUPPr, fereg, SPEEDf)) {
            *speed = 100;
        } else {
            *speed = 10;
        }
        ...
        case SOC_MAC_MODE_10_100:       /* Get the speed from the mac */
            ...
            if (soc_reg64_field32_get(unit,
                          _REG_NAME(unit, port, FE_SUPPr, GTH_FE_SUPPr),
                          fereg, SPEEDf)) {
                *speed = 100;
            } else {
                *speed = 10;
...
{
    if (SOC_IS_TUCANA(unit)) { /* Tucana puts speed into port table */
        port_tab_entry_t ptab;
        uint32 sp_sel;

        SOC_IF_ERROR_RETURN(soc_mem_read(unit, PORT_TABm, MEM_BLOCK_ANY,
                                         port, &ptab));
        soc_mem_field_get(unit, PORT_TABm, ptab.entry_data,
                          SPEED_SELECTf, &sp_sel);
        return sp_sel;
#ifdef  BCM_XGS3_SWITCH_SUPPORT
    } else if (soc_feature(unit, soc_feature_trimac)) {
        ...
        return soc_reg_field_get(unit, CONFIGr, configreg, SPEED_SELECTf);
    }
...
_port_sp_sel_set(int unit, int port, uint32 sp_sel)
{
    if (SOC_IS_TUCANA(unit)) { /* Tucana puts speed into port table */
        ...
        SOC_IF_ERROR_RETURN(soc_mem_read(unit, PORT_TABm, MEM_BLOCK_ANY,
                                         port, &ptab));
        soc_mem_field_set(unit, PORT_TABm, ptab.entry_data,
                          SPEED_SELECTf, &sp_sel);
        ...
        soc_reg_field_set(unit, CONFIGr, &configreg, SPEED_SELECTf,
            sp_sel);
    ...
    if(IS_GE_PORT(unit, port) && soc_feature(unit, soc_feature_unimac)) {
        ...
        *mode = SOC_MAC_MODE_1000_T; /* Default */
        ...
        if (soc_feature(unit, soc_feature_fe_gig_macs)) {
            ...
            } else if (!SOC_IS_DRACO(unit) && (speed_sel != 0)) {
                *mode = SOC_MAC_MODE_10_100;
            } else if (speed_sel == 1) {
                *mode = SOC_MAC_MODE_10_100;
            }
...
soc_drain_cells(int unit, soc_port_t port)
{
    ...
    if (SOC_IS_DRACO(unit)) {
        ...
        SOC_IF_ERROR_RETURN
            (READ_GPC_EGR_PKT_DROP_CTLr(unit, port, &pktd_ctl));
        COMPILER_64_SET(pktd_set, 0, 1);
        SOC_IF_ERROR_RETURN
            (WRITE_GPC_EGR_PKT_DROP_CTLr(unit, port, pktd_set));
        SOC_IF_ERROR_RETURN
    ...
    if (SOC_IS_TUCANA(unit)) {
        if (IS_GE_PORT(unit, port)) {
            ...
            SOC_IF_ERROR_RETURN
                (mac_fe_pause_get(unit, port, &pause_tx, &pause_rx));
            SOC_IF_ERROR_RETURN
                (mac_fe_pause_set(unit, port, 0, 0));
            SOC_IF_ERROR_RETURN
                (READ_EPC_EGR_PKT_DROP_CTLr(unit, port, &pktd_ctl));
            COMPILER_64_SET(pktd_set, 0,
                            1 << SOC_PORT_INFO(unit, port).bindex);
            SOC_IF_ERROR_RETURN
                (WRITE_EPC_EGR_PKT_DROP_CTLr(unit, port, pktd_set));
        }
    }
#endif
    ...
    if (SOC_IS_DRACO(unit)) {
        ...
        SOC_IF_ERROR_RETURN
            (WRITE_GPC_EGR_PKT_DROP_CTLr(unit, port, pktd_ctl));
        SOC_IF_ERROR_RETURN
    ...
    if (SOC_IS_TUCANA(unit)) {
        if (IS_GE_PORT(unit, port)) {
            ...
            SOC_IF_ERROR_RETURN
                (WRITE_EPC_EGR_PKT_DROP_CTLr(unit, port, pktd_ctl));
            SOC_IF_ERROR_RETURN
                (mac_fe_pause_set(unit, port, pause_tx, pause_rx));
        }
    }
#endif
...
/*
 * Function:
 *      _soc_mac_rsv_mask_rflt_set
 * Purpose:
 *      Enable or disable RECEIVE_FRAME_LONG_TRUNCATED drop condition.
 * Parameters:
 *      unit - StrataSwitch unit #.
 *      port - Port number on unit.
 *      enable - (TRUE/FALSE) set or clear rflt bit of RSV_MASK.
 * Returns:
 *      SOC_E_XXX
 * Notes:
 *      Useful for 10/100 mode XGS switches.
 */

STATIC int
_soc_mac_rsv_mask_rflt_set(int unit, soc_port_t port, int enable)
{
    uint64 rsv_mask;

    if (soc_feature(unit, soc_feature_rsv_mask)) {
        SOC_IF_ERROR_RETURN(READ_RSV_MASKr(unit, port, &rsv_mask));
        /* Set the RECEIVE_FRAME_LONG_TRUNCATED bit */
        if (enable) {
            COMPILER_64_SET(rsv_mask, 0,
                            COMPILER_64_LO(rsv_mask) | 0x04000000);
        } else {
            COMPILER_64_SET(rsv_mask, 0,
                            COMPILER_64_LO(rsv_mask) & ~0x04000000);
        }
        SOC_IF_ERROR_RETURN(WRITE_RSV_MASKr(unit, port, rsv_mask));
    }
...
soc_mac_mode_set(int unit, soc_port_t port, soc_mac_mode_t new_mode)
{
    ...
    case SOC_MAC_MODE_10_100:
        ...
        if (soc_feature(unit, soc_feature_rsv_mask)) {
            SOC_IF_ERROR_RETURN
                (_soc_mac_rsv_mask_rflt_set(unit, port, 0)); /* disable */
        }
	if (SOC_IS_DRACO1(unit)) {
	    uint64 pctrl;
	    SOC_IF_ERROR_RETURN(READ_PAUSE_CONTROLr(unit, port, &pctrl));
	    soc_reg64_field32_set(unit, PAUSE_CONTROLr, &pctrl, VALUEf,
				  PAUSE_VAL(4));
    ...
    case SOC_MAC_MODE_1000_T:
        ...
        if (soc_feature(unit, soc_feature_rsv_mask)) {
            SOC_IF_ERROR_RETURN
                (_soc_mac_rsv_mask_rflt_set(unit, port, 1)); /* enable */
        }
	if (SOC_IS_DRACO1(unit)) {
	    uint64 pctrl;
	    SOC_IF_ERROR_RETURN(READ_PAUSE_CONTROLr(unit, port, &pctrl));
...
mac_fe_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    /*
     * The 5690 SERDES PHY is kept in reset whenever the MAC is
    ...
    defined(BCM_RAPTOR_SUPPORT) || defined(BCM_DRACO_SUPPORT)
    if (SOC_IS_DRACO(unit) || soc_feature(unit, soc_feature_dodeca_serdes)) {
        if (enable) {
            SOC_IF_ERROR_RETURN
                (soc_phyctrl_notify(unit, port, 
...
#endif /* BCM_FIREBOLT_SUPPORT || BCM_EASYRIDER_SUPPORT || BCM_RAPTOR_SUPPORT */
...
mac_fe_ipg_update(int unit, int port)
{
     ...
     */
    COMPILER_64_SET(ipg64, 0, ipg);
    SOC_IF_ERROR_RETURN(FE_WRITE(FE_IPGTr, GTH_FE_IPGTr, unit, port, ipg64));
...
mac_fe_init(int unit, soc_port_t port)
{
     ...
     */
    SOC_IF_ERROR_RETURN(FE_READ(FE_MAC2r, GTH_FE_MAC2r, unit, port, &reg64));
    /* FULL_DUP=0 for workaround */
    FE_SET(unit, port, FE_MAC2r, GTH_FE_MAC2r, FULL_DUPf, reg64, 0);
    FE_SET(unit, port, FE_MAC2r, GTH_FE_MAC2r, EXC_DEFf, reg64, 1);
    FE_SET(unit, port, FE_MAC2r, GTH_FE_MAC2r, LG_CHKf, reg64, 0);
    FE_SET(unit, port, FE_MAC2r, GTH_FE_MAC2r, HUGE_FRf, reg64, 0);
    FE_SET(unit, port, FE_MAC2r, GTH_FE_MAC2r, DEL_CRCf, reg64, 0);
    FE_SET(unit, port, FE_MAC2r, GTH_FE_MAC2r, CRC_ENf, reg64, 0);
    FE_SET(unit, port, FE_MAC2r, GTH_FE_MAC2r, PAD_ENf, reg64, 0);
    FE_SET(unit, port, FE_MAC2r, GTH_FE_MAC2r, VLAN_PADf, reg64, 0);
    FE_SET(unit, port, FE_MAC2r, GTH_FE_MAC2r, AUTO_PADf, reg64, 0);
    FE_SET(unit, port, FE_MAC2r, GTH_FE_MAC2r, PURE_PADf, reg64, 0);
    FE_SET(unit, port, FE_MAC2r, GTH_FE_MAC2r, LONG_PREf, reg64, 0);
    FE_SET(unit, port, FE_MAC2r, GTH_FE_MAC2r, NO_BOFFf, reg64, 0);
    FE_SET(unit, port, FE_MAC2r, GTH_FE_MAC2r, BP_NO_BOFFf, reg64, 0);
    if (!SOC_IS_XGS3_SWITCH(unit)) {
        FE_SET(unit, port, FE_MAC2r, GTH_FE_MAC2r, AUTOZf, reg64, 0);
    }
    SOC_IF_ERROR_RETURN(FE_WRITE(FE_MAC2r, GTH_FE_MAC2r, unit, port, reg64));

    SOC_IF_ERROR_RETURN(FE_READ(FE_CLRTr, GTH_FE_CLRTr, unit, port, &reg64));
    FE_SET(unit, port, FE_CLRTr, GTH_FE_CLRTr, COL_WINf, reg64, 0x37);
    FE_SET(unit, port, FE_CLRTr, GTH_FE_CLRTr, RETRYf, reg64, 0xf);
    SOC_IF_ERROR_RETURN(FE_WRITE(FE_CLRTr, GTH_FE_CLRTr, unit, port, reg64));

    if (soc_feature(unit, soc_feature_fe_maxframe)) {
        COMPILER_64_SET(reg64, 0, 1518 + 1);
        SOC_IF_ERROR_RETURN(WRITE_FE_MAXFr(unit, port,  reg64));
    }

    SOC_IF_ERROR_RETURN(FE_READ(FE_IPGRr, GTH_FE_IPGRr, unit, port, &reg64));
    FE_SET(unit, port, FE_IPGRr, GTH_FE_IPGRr, IPGR2f, reg64, 0x12);
    FE_SET(unit, port, FE_IPGRr, GTH_FE_IPGRr, IPGR1f, reg64, 0xc);
    SOC_IF_ERROR_RETURN(FE_WRITE(FE_IPGRr, GTH_FE_IPGRr, unit, port, reg64));

    SOC_IF_ERROR_RETURN(FE_READ(FE_SUPPr, GTH_FE_SUPPr, unit, port, &reg64));
    FE_SET(unit, port, FE_SUPPr, GTH_FE_SUPPr, SPEEDf, reg64, 1);
    FE_SET(unit, port, FE_SUPPr, GTH_FE_SUPPr, BIT_MODEf, reg64, 0);

    SOC_IF_ERROR_RETURN(FE_WRITE(FE_SUPPr, GTH_FE_SUPPr, unit, port, reg64));
    SOC_IF_ERROR_RETURN(FE_READ(FE_TESTr, GTH_FE_TESTr, unit, port, &reg64));
    FE_SET(unit, port, FE_TESTr, GTH_FE_TESTr, SHORT_QNTAf, reg64, 0);
    FE_SET(unit, port, FE_TESTr, GTH_FE_TESTr, TEST_PAUSEf, reg64, 0);
    FE_SET(unit, port, FE_TESTr, GTH_FE_TESTr, TEST_BACKf, reg64, 0);
    SOC_IF_ERROR_RETURN(FE_WRITE(FE_TESTr, GTH_FE_TESTr, unit, port, reg64));

    SOC_IF_ERROR_RETURN(FE_READ(FE_MAC1r, GTH_FE_MAC1r, unit, port, &reg64));
    if (!SOC_IS_XGS_SWITCH(unit)) {
        FE_SET(unit, port, FE_MAC1r, GTH_FE_MAC1r, SOFT_RSTf, reg64, 0);
    }

    /* Enable pause on FE ports */
    FE_SET(unit, port, FE_MAC1r, GTH_FE_MAC1r, TX_PAUf, reg64, 1);
    FE_SET(unit, port, FE_MAC1r, GTH_FE_MAC1r, RX_PAUf, reg64, 1);

    FE_SET(unit, port, FE_MAC1r, GTH_FE_MAC1r, LBACKf, reg64, 0);
    FE_SET(unit, port, FE_MAC1r, GTH_FE_MAC1r, RX_ENf, reg64, 0);
    SOC_IF_ERROR_RETURN(FE_WRITE(FE_MAC1r, GTH_FE_MAC1r, unit, port, reg64));

    /* Clear station address */
    COMPILER_64_ZERO(reg64);
    SOC_IF_ERROR_RETURN(FE_WRITE(ESA0r, GTH_ESA0r, unit, port, reg64));
    SOC_IF_ERROR_RETURN(FE_WRITE(ESA1r, GTH_ESA1r, unit, port, reg64));
    SOC_IF_ERROR_RETURN(FE_WRITE(ESA2r, GTH_ESA2r, unit, port, reg64));

    /* Set IPG to match initial operating mode */
    SOC_IF_ERROR_RETURN(mac_fe_ipg_update(unit, port));

    /* Allow dribble packets */
    if (soc_feature(unit, soc_feature_rsv_mask)) {
        SOC_IF_ERROR_RETURN
            (_soc_mac_rsv_mask_rflt_set(unit, port, 0)); /* disable */
    }
    ...
    if (!IS_GE_PORT(unit, port)) {
        ...
        COMPILER_64_SET(port_bit, 0, 1 << (port % 8));
...
{
    uint64      esa0, esa1, esa2;

    COMPILER_64_ZERO(esa0);
    COMPILER_64_ZERO(esa1);
    COMPILER_64_ZERO(esa2);
...
mac_fe_frame_max_set(int unit, soc_port_t port, int size)
{
    ...
    return FE_WRITE(FE_MAXFr, GTH_FE_MAXFr, unit, port, val64);
}
...
mac_fe_frame_max_get(int unit, soc_port_t port, int *size)
{
    ...
    SOC_IF_ERROR_RETURN(FE_READ(FE_MAXFr, GTH_FE_MAXFr, unit, port, &val64));
...
mac_ge_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    ogmacc1 = gmacc1;
    if (enable) {
        soc_reg64_field32_set(unit, GMACC1r, &gmacc1, TXEN0f, 1);
        soc_reg64_field32_set(unit, GMACC1r, &gmacc1, RXEN0f, 1);
    } else {            /* Turn off Receive Enable */
        soc_reg64_field32_set(unit, GMACC1r, &gmacc1, RXEN0f, 0);
    }
    if (COMPILER_64_NE(gmacc1, ogmacc1)) {
...
mac_ge_enable_get(int unit, soc_port_t port, int *enable)
{
    ...
    if ((mode == SOC_MAC_MODE_10_100) || (mode == SOC_MAC_MODE_10)) {
        ...
        *enable = soc_reg64_field32_get(unit, GMACC1r, gmacc1, RXEN0f);
    }
...
mac_ge_ipg_update(int unit, int port)
{
    ...
    ogmacc2 = gmacc2;
    soc_reg64_field32_set(unit, GMACC2r, &gmacc2, IPGTf, ipg);
    if (COMPILER_64_NE(gmacc2, ogmacc2)) {
...
{
    uint64              gmacc0, gmacc1, gmacc2, val64;
    uint64              ogmacc0, ogmacc1, ogmacc2;
    uint32              val32;
    uint64 pctrl;
    ...
    soc_reg64_field32_set(unit, GMACC0r, &gmacc0, L10Bf, 0);
    soc_reg64_field32_set(unit, GMACC0r, &gmacc0, L32Bf, 0);
    soc_reg64_field32_set(unit, GMACC0r, &gmacc0, TMDSf, 0);

    soc_reg64_field32_set(unit, GMACC1r, &gmacc1, FULLDf, 1);  /* arbitrary */
    if (IS_ST_PORT(unit, port)) {
        /* Flow control should not be enabled in stacking mode. */
        soc_reg64_field32_set(unit, GMACC1r, &gmacc1, FCTXf, 0);  /* Flow ctl */
        soc_reg64_field32_set(unit, GMACC1r, &gmacc1, FCRXf, 0);  /* Flow ctl */
    } else {
        soc_reg64_field32_set(unit, GMACC1r, &gmacc1, FCTXf, 1);  /* Flow ctl */
        soc_reg64_field32_set(unit, GMACC1r, &gmacc1, FCRXf, 1);  /* Flow ctl */
    }
    soc_reg64_field32_set(unit, GMACC1r, &gmacc1, LONGPf, 1);
    soc_reg64_field32_set(unit, GMACC1r, &gmacc1, MIFGf, 2);
    soc_reg64_field32_set(unit, GMACC1r, &gmacc1, GLVRf, 1);
    soc_reg64_field32_set(unit, GMACC1r, &gmacc1, TXEN0f, 0);
    soc_reg64_field32_set(unit, GMACC1r, &gmacc1, RXEN0f, 0);
    soc_reg64_field32_set(unit, GMACC1r, &gmacc1, JUMBOf, 1);  /* HUGEN=0 */
    if (!SOC_IS_XGS3_SWITCH(unit)) {
        soc_reg64_field32_set(unit, GMACC1r, &gmacc1, AUTOZf, 0);
    }

    COMPILER_64_ZERO(pctrl);
    soc_reg64_field32_set(unit, PAUSE_CONTROLr, &pctrl, ENABLEf, 1);
    if (SOC_IS_DRACO1(unit)) {
        /* this will be updated in soc_mac_mode_set */
        soc_reg64_field32_set(unit, PAUSE_CONTROLr, &pctrl, VALUEf,
            PAUSE_VAL(32));
    } else {
        soc_reg64_field32_set(unit, PAUSE_CONTROLr, &pctrl, VALUEf,
    ...
    SOC_IF_ERROR_RETURN(WRITE_TEST2r(unit, port, val64));
    ...
    if (COMPILER_64_NE(gmacc0, ogmacc0)) {
    ...
    /* Clear station address */
    COMPILER_64_ZERO(val64);
    SOC_IF_ERROR_RETURN(WRITE_GSA0r(unit, port, val64));
    SOC_IF_ERROR_RETURN(WRITE_GSA1r(unit, port, val64));
    ...
    case SOC_CHIP_BCM5695:
        val32 = 0;
        soc_reg_field_set(unit, GTH_FE_IPGRr, &val32, IPGR1f, 0x6);
        soc_reg_field_set(unit, GTH_FE_IPGRr, &val32, IPGR2f, 0xf);
        SOC_IF_ERROR_RETURN(WRITE_GTH_FE_IPGRr(unit, port, val32));
        break;
    case SOC_CHIP_BCM56601:
    case SOC_CHIP_BCM56602:
    case SOC_CHIP_BCM56504:
    ...
    case SOC_CHIP_BCM88732:
        COMPILER_64_ZERO(val64);
        soc_reg64_field32_set(unit, FE_IPGRr, &val64, IPGR1f, 0x6);
        soc_reg64_field32_set(unit, FE_IPGRr, &val64, IPGR2f, 0xf);
        SOC_IF_ERROR_RETURN(WRITE_FE_IPGRr(unit, port, val64));
        break;
    default:
        break;
    }
    ...
    if (SOC_IS_EASYRIDER(unit)) {
        SOC_IF_ERROR_RETURN(READ_EGR_PORTr(unit, port, &val32));
        soc_reg_field_set(unit, EGR_PORTr, &val32, PORT_ENABLEf, 1);
        SOC_IF_ERROR_RETURN(WRITE_EGR_PORTr(unit, port, val32));
    } else
...
#endif /* BCM_FIREBOLT_SUPPORT */
    {
        ...
        SOC_IF_ERROR_RETURN(WRITE_GEGR_ENABLEr(unit, port, val64));
    }

    /* Block ingress control frames at the GPIC (vs. at the MAC) */
    if (soc_feature(unit, soc_feature_block_ctrl_ingress)) {
        uint64 rsv_mask;
        SOC_IF_ERROR_RETURN(READ_RSV_MASKr(unit, port, &rsv_mask));
        /* Set the RECEIVE_FRAME_CONTROL_FRAME bit */
...
mac_ge_interface_set(int unit, soc_port_t port, soc_port_if_t pif)
{
    ...
    case SOC_PORT_IF_SGMII:
        soc_reg64_field32_set(unit, GMACC0r, &gmacc0, TMDSf, 1); /* GMII */
        soc_reg64_field32_set(unit, GPCSCr, &gpcsc, RCSELf, 1);  /* 125 MHz */
        break;
    case SOC_PORT_IF_TBI:
        soc_reg64_field32_set(unit, GMACC0r, &gmacc0, TMDSf, 0); /* TBI */
        soc_reg64_field32_set(unit, GPCSCr, &gpcsc, RCSELf, 0);  /* 62.5 MHz */
        break;
    default:
        return SOC_E_UNAVAIL;
    }

    if (COMPILER_64_NE(gpcsc, ogpcsc)) {
...
mac_ge_interface_get(int unit, soc_port_t port, soc_port_if_t *pif)
{
    ...
    *pif = (soc_reg64_field32_get(unit, GMACC0r, gmacc0, TMDSf) ?
            SOC_PORT_IF_GMII :
            SOC_PORT_IF_TBI);
...
mac_ge_duplex_set(int unit, soc_port_t port, int duplex)
{
    ...
    if ((mode == SOC_MAC_MODE_10_100) || (mode == SOC_MAC_MODE_10)) {
        ...
        SOC_IF_ERROR_RETURN(READ_GMACC1r(unit, port, &gmacc1));
        ogmacc1 = gmacc1;
        soc_reg64_field32_set(unit, GMACC1r, &gmacc1, 
                              FULLDf, duplex ? 1 : 0);
        if (COMPILER_64_NE(gmacc1, ogmacc1)) {
            SOC_IF_ERROR_RETURN(WRITE_GMACC1r(unit, port, gmacc1));
...
mac_ge_duplex_get(int unit, soc_port_t port, int *duplex)
{
    ...
    if ((mode == SOC_MAC_MODE_10_100) || (mode == SOC_MAC_MODE_10)) {
        ...
        *duplex = soc_reg64_field32_get(unit, GMACC1r, gmacc1, FULLDf);
...
mac_se_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    soc_reg64_field32_set(unit, GMACC0r, &val64, SRSTf, 1);
    ...
    SOC_IF_ERROR_RETURN(READ_GMACC0r(unit, port, &val64));
    soc_reg64_field32_set(unit, GMACC0r, &val64, SRSTf, 0);
    SOC_IF_ERROR_RETURN(WRITE_GMACC0r(unit, port, val64));
...
mac_ge_pause_set(int unit, soc_port_t port, int pause_tx, int pause_rx)
{
    ...
    if ((mode == SOC_MAC_MODE_10_100) || (mode == SOC_MAC_MODE_10)) {
        ...
        ogmacc1 = gmacc1;
        if (pause_tx >= 0) {
            soc_reg64_field32_set(unit, GMACC1r, &gmacc1, FCTXf,
                                  pause_tx ? 1 : 0);
        }
        if (pause_rx >= 0) {
            soc_reg64_field32_set(unit, GMACC1r, &gmacc1, FCRXf,
                                  pause_rx ? 1 : 0);
        }
        if (COMPILER_64_NE(gmacc1, ogmacc1)) {
            SOC_IF_ERROR_RETURN(WRITE_GMACC1r(unit, port, gmacc1));
...
mac_ge_pause_addr_set(int unit, soc_port_t port, sal_mac_addr_t pause_mac)
{
    ...
    if ((mode == SOC_MAC_MODE_10_100) || (mode == SOC_MAC_MODE_10)) {
        ...
        COMPILER_64_ZERO(gsa0);
        COMPILER_64_ZERO(gsa1);

        soc_reg64_field32_set(unit, GSA0r, &gsa0, STAD1f,
                          pause_mac[0] << 24 |
                          pause_mac[1] << 16 |
                          pause_mac[2] << 8 |
                          pause_mac[3] << 0);

        soc_reg64_field32_set(unit, GSA1r, &gsa1, STAD2f,
                          pause_mac[4] << 8 |
                          pause_mac[5]);
...
mac_ge_pause_get(int unit, soc_port_t port, int *pause_tx, int *pause_rx)
{
    ...
    if ((mode == SOC_MAC_MODE_10_100) || (mode == SOC_MAC_MODE_10)) {
        ...
        *pause_tx = soc_reg64_field32_get(unit, GMACC1r, gmacc1, FCTXf) ?
            TRUE : FALSE;
        *pause_rx = soc_reg64_field32_get(unit, GMACC1r, gmacc1, FCRXf) ?
            TRUE : FALSE;
...
mac_ge_pause_addr_get(int unit, soc_port_t port, sal_mac_addr_t pause_mac)
{
    ...
    if ((mode == SOC_MAC_MODE_10_100) || (mode == SOC_MAC_MODE_10)) {
        ...
        stad1 = soc_reg64_field32_get(unit, GSA0r, gsa0, STAD1f);
        stad2 = soc_reg64_field32_get(unit, GSA1r, gsa1, STAD2f);
...
mac_ge_loopback_set(int unit, soc_port_t port, int loopback)
{
    ...
    if ((mode == SOC_MAC_MODE_10_100) || (mode == SOC_MAC_MODE_10)) {
        ...
        uint64 gmacc0, ogmacc0;

        /* L10B also works, but not on Draco/Raptor so we'll use L32B */
        SOC_IF_ERROR_RETURN(READ_GMACC0r(unit, port, &gmacc0));
        ogmacc0 = gmacc0;
        soc_reg64_field32_set(unit, GMACC0r, &gmacc0, L32Bf, loopback ? 1 : 0);
        if (COMPILER_64_NE(gmacc0, ogmacc0)) {
            SOC_IF_ERROR_RETURN(WRITE_GMACC0r(unit, port, gmacc0));
        }
...
mac_ge_loopback_get(int unit, soc_port_t port, int *loopback)
{
    ...
    if ((mode == SOC_MAC_MODE_10_100) || (mode == SOC_MAC_MODE_10)) {
        ...
        *loopback = soc_reg64_field32_get(unit, GMACC0r, gmacc0, L32Bf);
...
mac_ge_frame_max_set(int unit, soc_port_t port, int size)
{
    ...
    COMPILER_64_SET(val64, 0, size);
    return WRITE_MAXFRr(unit, port, val64);
}
...
mac_ge_frame_max_get(int unit, soc_port_t port, int *size)
{
    ...
    if ((mode == SOC_MAC_MODE_10_100) || (mode == SOC_MAC_MODE_10)) {
        ...
        SOC_IF_ERROR_RETURN(READ_MAXFRr(unit, port, &val64));
        COMPILER_64_TO_32_LO(*size, val64);
                   ...
                   int value)
{
    ...
    case SOC_MAC_CONTROL_RX_SET:
        ...
        copy = regval;
        if (value) {
            soc_reg64_field32_set(unit, GMACC1r, &regval, RXEN0f, 1);
        } else {
            soc_reg64_field32_set(unit, GMACC1r, &regval, RXEN0f, 0);
        }
        if (COMPILER_64_NE(regval, copy)) {
                   ...
                   int *value)
{
    ...
    case SOC_MAC_CONTROL_RX_SET:
        ...
        SOC_IF_ERROR_RETURN(READ_GMACC1r(unit, port, &regval));
        *value = soc_reg64_field32_get(unit, GMACC1r, regval, RXEN0f);
        break;
    default:
        return SOC_E_UNAVAIL;
    }
