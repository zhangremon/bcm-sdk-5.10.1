...
/*
 * Function:
 *      soc_cmicm_ipoll_thread
 * Description:
 *      Thread context for CMICm interrupt handlers in polled IRQ mode
 * Parameters:
 ...
 */
STATIC void
soc_cmicm_ipoll_thread(void *data)
{
    int dev, spl, udelay, cmc;
    ...
    while (_ihandlers) {
        ...
        for (dev = 0; dev < SOC_MAX_NUM_DEVICES; dev++) {
            if (_ictrl[dev].handler != NULL && !_ictrl[dev].paused) {
                cmc = SOC_PCI_CMC(dev);
                if (soc_pci_read(dev, CMIC_CMCx_IRQ_STAT0_OFFSET(cmc)) & SOC_CMCx_IRQ0_MASK(dev,cmc)) {
    ...
    if (start_thread) {
        ...
        if (soc_feature(dev, soc_feature_cmicm)) {
            sal_thread_create("bcmPOLL",
                              ...
                              soc_cmicm_ipoll_thread, INT_TO_PTR(udelay));
        } else
#endif
        {
            sal_thread_create("bcmPOLL",
