/*
 * $Id: unimac.c 1.46.4.3 Broadcom SDK $
...
{
    uint32              command_config, ocommand_config, speed;
    int                 frame_max;
    int                 ignore_pause;
    uint64              rval64;
...
mac_uni_loopback_set(int unit, soc_port_t port, int loopback)
{
    ...
    if (auto_cfg[unit][port] && (!SOC_IS_HAWKEYE(unit))) {
        ...
        SOC_IF_ERROR_RETURN(soc_phyctrl_loopback_set(unit,port,TRUE));
        sal_usleep(40000);
        SOC_IF_ERROR_RETURN(READ_COMMAND_CONFIGr(unit, port, &command_config));
        soc_reg_field_set(unit, COMMAND_CONFIGr, &command_config,
                          ENA_EXT_CONFIGf, loopback? 0 : 1);
        SOC_IF_ERROR_RETURN(WRITE_COMMAND_CONFIGr(unit, port, command_config));
        SOC_IF_ERROR_RETURN(soc_phyctrl_loopback_set(unit,port,phy_lb));
    }
                    ...
                    int value)
{
    ...
    case SOC_MAC_CONTROL_EEE_TX_IDLE_TIME:
        ...
        if (!soc_reg_field_valid(unit, EEE_DELAY_ENTRY_TIMERr, EEE_LPI_TIMERf)) {
            return SOC_E_UNAVAIL;
        }
        SOC_IF_ERROR_RETURN(soc_reg_field32_modify(unit, EEE_DELAY_ENTRY_TIMERr, 
    ...
    case SOC_MAC_CONTROL_EEE_TX_WAKE_TIME:
        ...
        SOC_IF_ERROR_RETURN(soc_reg_field32_modify(unit, EEE_WAKE_TIMERr, 
    ...
    case SOC_MAC_CONTROL_PFC_REFRESH_TIME:
        ...
        *value = soc_reg_field_get(unit, UNIMAC_PFC_CTRLr, rval,
        ...
        case SOC_MAC_CONTROL_EEE_TX_IDLE_TIME:
            ...
            SOC_IF_ERROR_RETURN(READ_EEE_DELAY_ENTRY_TIMERr(unit, port, &rval));
