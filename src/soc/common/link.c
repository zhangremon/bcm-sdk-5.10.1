/*
 * $Id: link.c 1.40.2.1 Broadcom SDK $
...
if(SOC_IS_KATANA(unit)) {
    ...
    if ((IS_XE_PORT(unit, port) || IS_HG_PORT(unit, port)) &&
        ...
        reg = protocol_map_reg[port_bit / 32];
    ...
    if (SOC_REG_IS_VALID(unit, bus_map_reg[0])) {
        reg = bus_map_reg[port_bit / 10];
        field = bus_map_9_0_field[port_bit % 10];
        if (SOC_REG_IS_VALID(unit, reg)) {
            addr = soc_reg_addr(unit, reg, REG_PORT_ANY, 0);
            rval = soc_pci_read(unit, addr);
            soc_reg_field_set(unit, bus_map_reg[0], &rval, field, bus_sel);
            soc_pci_write(unit, addr, rval);
        }
    } else {
        reg = INVALIDr;
    ...
    if (!PHY_EXTERNAL_MODE(unit, port)) {
        reg = int_sel_map_reg[port_bit / 32];
        if (SOC_REG_IS_VALID(unit, reg)) {
            addr = soc_reg_addr(unit, reg, REG_PORT_ANY, 0);
            rval = soc_pci_read(unit, addr);
            rval |= 1 << (port_bit % 32);
