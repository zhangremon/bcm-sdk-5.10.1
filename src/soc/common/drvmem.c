/*
 * $Id: drvmem.c 1.30.4.3 Broadcom SDK $
...
/*
 * Function:     soc_memacc_field64_get
 ...
 */
uint32
soc_mem_addr(int unit, soc_mem_t mem, int blk, int index)
{
    uint32              blkoff;
    soc_mem_info_t      *mip;
 ...
 */
uint32
soc_mem_addr_get(int unit, soc_mem_t mem, soc_block_t block, int index,
                 uint8 *acc_type)
{
    soc_mem_info_t      *mip;

    if (!soc_feature(unit, soc_feature_new_sbus_format)) {
        return soc_mem_addr(unit, mem, block, index);
    }
    if (!SOC_MEM_IS_VALID(unit, mem)) {
#if defined(BCM_ESW_SUPPORT) && !defined(SOC_NO_NAMES)
        soc_cm_print("mem %s is invalid\n", soc_mem_name[mem]);
