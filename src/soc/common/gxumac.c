/*
 * $Id: gxumac.c 1.15.14.2 Broadcom SDK $
...
_mac_combo_select_mac(int unit, soc_port_t port, soc_mac_mode_t mode)
{
    if (SOC_DEBUG_CHECK(SOC_DBG_10G | SOC_DBG_VERBOSE) ||
        ...
        soc_cm_print("_mac_combo_select_mac: unit %d port %s mode %s\n",
    ...
    if (IS_HG_PORT(unit, port)) {
        ...
        if (SOC_INFO(unit).port_speed_max[port] > 0 &&
            ...
            if (!((SOC_INFO(unit).port_speed_max[port] >= 40000) &&
                (speed == 42000))) { /* make exception for 42G speed*/
                return SOC_E_PARAM;
            }
        }
    } else {
...
#ifdef BCM_CMAC_SUPPORT
    if (mode == SOC_MAC_MODE_100000) {
        return _mac_combo_100g_mac(unit)->md_control_set(unit, port, type,
                                                         value);
    } else
#endif 
    if (mode == SOC_MAC_MODE_10000) {
        return _mac_combo_10g_mac(unit)->md_control_set(unit, port, type,
                                                        value);
    } else {
        return _mac_combo_ge_mac(unit)->md_control_set(unit, port, type,
                                                       value);
    }
}
