/*
 * $Id: et_soc.c 1.3.2.6 Broadcom SDK $
...
et_soc_rx_chain(int unit, eth_dv_t *dv_chain)
{
    ...
    if ((chan < 0) || (chan >= max_channels)) {
        soc_cm_debug(DK_WARN, "et_soc_rx_chain: invalid dma channel\n");
        chan =
...
eth_dv_t *
et_soc_rx_chain_get(int unit, int chan)
{
    eth_dv_t *dv = NULL;
    int max_channels;
...
#endif
    if ((chan < 0) || (chan >= max_channels)) {
        soc_cm_debug(DK_WARN, "et_soc_rx_chain_get(): invalid dma channel\n");
        return dv;
    }
