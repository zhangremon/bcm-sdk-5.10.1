...
soc_unicore_reset(int unit, soc_port_t port)
{
    uint8               phy_addr;
    uint64              xgxs_ctrl;
    uint32              reg_val, lcpll, gport_config;
    ...
    DBG_10G_VERB(("gxmac: unit %d port %s: unicore reset\n",
    ...
    if (!SOC_WARM_BOOT(unit) && !SOC_IS_RELOADING(unit)) {
        ...
        if (SAL_BOOT_QUICKTURN) {
            DBG_10G_VERB(("SIMULATION: xmac: unit %d port %s: "
...
soc_unicore_pll_lock_wait(int unit, int port)
{
    ...
    DBG_10G_VERB(("unit %d port %s: Unicore PLL locked in %d usec\n",
                  unit, SOC_PORT_NAME(unit, port),
                         ...
                         soc_mac_mode_t mode, int enable)
{
    ...
    soc_reg64_field32_set(unit, GMACC1r, &gmacc1, RXEN0f, gmac_en);
    ...
    ofe_mac1 = fe_mac1;
    soc_reg64_field32_set(unit, FE_MAC1r, &fe_mac1, RX_ENf, femac_en);
    if (COMPILER_64_NE(fe_mac1, ofe_mac1)) {  
...
gxmac_ipg_update(int unit, int port)
{
    ...
    case 10:
    case 100:
        COMPILER_64_ZERO(fe_ipg);
        soc_reg64_field32_set(unit, FE_IPGTr, &fe_ipg, IPGTf, ipg);
...
{
    uint64              gmacc0, gmacc1, gmacc2, val64, pctrl;

    /*
     * Perform length check on Ethernet frames and do not allow huge frames
     */
    SOC_IF_ERROR_RETURN(READ_FE_MAC2r(unit, port, &val64));
    soc_reg64_field32_set(unit, FE_MAC2r, &val64, FULL_DUPf, 1);
    soc_reg64_field32_set(unit, FE_MAC2r, &val64, EXC_DEFf, 1);
    soc_reg64_field32_set(unit, FE_MAC2r, &val64, LG_CHKf, 0);
    soc_reg64_field32_set(unit, FE_MAC2r, &val64, HUGE_FRf, 0);
    soc_reg64_field32_set(unit, FE_MAC2r, &val64, DEL_CRCf, 0);
    soc_reg64_field32_set(unit, FE_MAC2r, &val64, CRC_ENf, 0);
    soc_reg64_field32_set(unit, FE_MAC2r, &val64, PAD_ENf, 0);
    soc_reg64_field32_set(unit, FE_MAC2r, &val64, VLAN_PADf, 0);
    soc_reg64_field32_set(unit, FE_MAC2r, &val64, AUTO_PADf, 0);
    soc_reg64_field32_set(unit, FE_MAC2r, &val64, PURE_PADf, 0);
    soc_reg64_field32_set(unit, FE_MAC2r, &val64, LONG_PREf, 0);
    soc_reg64_field32_set(unit, FE_MAC2r, &val64, NO_BOFFf, 0);
    soc_reg64_field32_set(unit, FE_MAC2r, &val64, BP_NO_BOFFf, 0);
    SOC_IF_ERROR_RETURN(WRITE_FE_MAC2r(unit, port, val64));

    SOC_IF_ERROR_RETURN(READ_FE_CLRTr(unit, port, &val64));
    soc_reg64_field32_set(unit, FE_CLRTr, &val64, COL_WINf, 0x37);
    soc_reg64_field32_set(unit, FE_CLRTr, &val64, RETRYf, 0xf);
    SOC_IF_ERROR_RETURN(WRITE_FE_CLRTr(unit, port, val64));
    ...
    COMPILER_64_ZERO(val64);
    soc_reg64_field32_set(unit, FE_IPGTr, &val64, IPGTf, 0xb);
    SOC_IF_ERROR_RETURN(WRITE_FE_IPGTr(unit, port, val64));

    SOC_IF_ERROR_RETURN(READ_FE_SUPPr(unit, port, &val64));
    soc_reg64_field32_set(unit, FE_SUPPr, &val64, SPEEDf, 1);
    soc_reg64_field32_set(unit, FE_SUPPr, &val64, BIT_MODEf, 0);
    SOC_IF_ERROR_RETURN(WRITE_FE_SUPPr(unit, port, val64));

    SOC_IF_ERROR_RETURN(READ_FE_TESTr(unit, port, &val64));
    soc_reg64_field32_set(unit, FE_TESTr, &val64, SHORT_QNTAf, 0);
    soc_reg64_field32_set(unit, FE_TESTr, &val64, TEST_PAUSEf, 0);
    soc_reg64_field32_set(unit, FE_TESTr, &val64, TEST_BACKf, 0);
    SOC_IF_ERROR_RETURN(WRITE_FE_TESTr(unit, port, val64));

    SOC_IF_ERROR_RETURN(READ_FE_MAC1r(unit, port, &val64));
    soc_reg64_field32_set(unit, FE_MAC1r, &val64, LBACKf, 0);
    soc_reg64_field32_set(unit, FE_MAC1r, &val64, RX_ENf, 0);
    SOC_IF_ERROR_RETURN(WRITE_FE_MAC1r(unit, port, val64));

    /* Clear station address */
    COMPILER_64_ZERO(val64);
    SOC_IF_ERROR_RETURN(WRITE_ESA0r(unit, port, val64));
    SOC_IF_ERROR_RETURN(WRITE_ESA1r(unit, port, val64));
    SOC_IF_ERROR_RETURN(WRITE_ESA2r(unit, port, val64));
    ...
    soc_reg64_field32_set(unit, GMACC2r, &gmacc2, IPGTf, 0xc);
...
gxmac_init(int unit, soc_port_t port)
{
    ...
    DBG_10G_VERB(("gxmac_init: unit %d port %s\n",
                  unit, SOC_PORT_NAME(unit, port)));
    ...
    SOC_IF_ERROR_RETURN(WRITE_MAC_TXCTRLr(unit, port, tx_ctrl));
    SOC_IF_ERROR_RETURN(WRITE_MAC_RXCTRLr(unit, port, rx_ctrl));
#if defined(BCM_XGS3_SUPPORT) || defined(BCM_LYNX_SUPPORT)
    if (SOC_IS_XGS3_SWITCH(unit) || SOC_IS_LYNX(unit)) {
...
#endif /* BCM_XGS3_SUPPORT || BCM_LYNX_SUPPORT */
...
{
    uint64 ctrl, fe_mac1, gmacc1;

    /* Instead of checking the current mode we read from all MACs */

    /* TriMAC */
    SOC_IF_ERROR_RETURN(READ_FE_MAC1r(unit, port, &fe_mac1));
    *enable = soc_reg64_field32_get(unit, FE_MAC1r, fe_mac1, RX_ENf);
    ...
    *enable |= soc_reg64_field32_get(unit, MAC_CTRLr, ctrl, RXENf);
    ...
    DBG_10G_VERB(("gxmac_enable_get:u=%d port=%d enable=%c\n",
...
gxmac_duplex_set(int unit, soc_port_t port, int duplex)
{
    ...
    DBG_10G_VERB(("gxmac_duplex_set: unit %d port %s %s duplex\n",
                  unit, SOC_PORT_NAME(unit, port),
                  duplex ? "FULL" : "HALF"));
    ...
    if (IS_GE_PORT(unit, port)) {
        ...
        if (cur_speed <= 100) { 
            uint64 fe_mac2, ofe_mac2;
            duplex = duplex ? 1 : 0;

            SOC_IF_ERROR_RETURN(READ_FE_MAC2r(unit, port, &fe_mac2));
            ofe_mac2 = fe_mac2;
            soc_reg64_field32_set(unit, FE_MAC2r, &fe_mac2, FULL_DUPf, duplex);
            if (COMPILER_64_NE(fe_mac2, ofe_mac2)) {
    ...
    *duplex = TRUE; /* HG and XE ports are always full duplex */
    ...
    if (IS_GE_PORT(unit, port)){
        ...
        if (cur_speed <= 100) {
            ...
            SOC_IF_ERROR_RETURN(READ_FE_MAC2r(unit, port, &fe_mac2));
            *duplex = soc_reg64_field32_get(unit, FE_MAC2r, fe_mac2, FULL_DUPf);
        }
    } 
    DBG_10G_VERB(("gxmac_duplex_get: unit %d port %s %s duplex\n",
                  unit, SOC_PORT_NAME(unit, port),
                  *duplex ? "FULL" : "HALF"));
...
gxmac_pause_set(int unit, soc_port_t port, int pause_tx, int pause_rx)
{
    ...
    if (pause_tx >= 0) {
        soc_reg64_field32_set(unit, GMACC1r, &gmacc1, FCTXf, pause_tx);
        soc_reg64_field32_set(unit, FE_MAC1r, &fe_mac1,
                              TX_PAUf, pause_tx);
    }
    if (pause_rx >= 0) {
        soc_reg64_field32_set(unit, GMACC1r, &gmacc1, FCRXf, pause_rx);
        soc_reg64_field32_set(unit, FE_MAC1r, &fe_mac1,
                              RX_PAUf, pause_rx);
    }
    ...
    DBG_10G_VERB(("gxmac_pause_set: unit %d port %s RX=%s TX=%s\n",
                  unit, SOC_PORT_NAME(unit, port),
                  pause_rx ? "on" : "off",
                  pause_tx ? "on" : "off"));
...
gxmac_pause_get(int unit, soc_port_t port, int *pause_tx, int *pause_rx)
{
    ...
    *pause_rx = soc_reg64_field32_get(unit, GMACC1r, gmacc1, FCRXf);
    ...
    DBG_10G_VERB(("gxmac_pause_get: unit %d port %s RX=%s TX=%s\n",
                  unit, SOC_PORT_NAME(unit, port),
                  *pause_rx ? "on" : "off",
                  *pause_tx ? "on" : "off"));
...
gxmac_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    DBG_10G_VERB(("gxmac_speed_set: unit %d port %s speed %dMb\n",
...
gxmac_speed_get(int unit, soc_port_t port, int *speed)
{
    ...
    DBG_10G_VERB(("gxmac_speed_get: unit %d port %s speed %dMb\n",
                  unit, SOC_PORT_NAME(unit, port), *speed));
    return rv;
}
...
gxmac_loopback_set(int unit, soc_port_t port, int lb)
{
    ...
    DBG_10G_VERB(("gxmac_loopback_set: unit %d port %s %s loopback\n",
                  unit, SOC_PORT_NAME(unit, port),
                  lb ? "local" : "no"));
    ...
    SOC_IF_ERROR_RETURN(READ_FE_MAC1r(unit, port, &fe_mac1));
    ofe_mac1 = fe_mac1;
    soc_reg64_field32_set(unit, FE_MAC1r, &fe_mac1, LBACKf, lb); 
    if (COMPILER_64_NE(fe_mac1, ofe_mac1)) {
        SOC_IF_ERROR_RETURN(WRITE_FE_MAC1r(unit, port, fe_mac1));
    }
    ...
    SOC_IF_ERROR_RETURN(READ_GMACC0r(unit, port, &gmacc0));
    ogmacc0 = gmacc0;
    soc_reg64_field32_set(unit, GMACC0r, &gmacc0, L32Bf, lb);
...
gxmac_loopback_get(int unit, soc_port_t port, int *lb)
{
    ...
    *lb = local | remote;
    ...
    DBG_10G_VERB(("gxmac_loopback_get: unit %d port %s %s loopback\n",
                  unit, SOC_PORT_NAME(unit, port),
...
gxmac_pause_addr_set(int unit, soc_port_t port, sal_mac_addr_t mac)
{
    ...
    soc_reg64_field32_set(unit, ESA0r, &esa0, STAD0f,
                          (mac[4] << 8) | mac[5]);
    soc_reg64_field32_set(unit, ESA1r, &esa1, STAD0f,
                          (mac[2] << 8) | mac[3]);
    soc_reg64_field32_set(unit, ESA2r, &esa2, STAD0f,
                          (mac[0] << 8) | mac[1]);
    ...
    soc_reg64_field32_set(unit, GSA0r, &gsa0, STAD1f, (w0 << 16) | w1);
    soc_reg64_field32_set(unit, GSA1r, &gsa1, STAD2f, w2);
    ...
    DBG_10G_VERB(("gxmac_pause_addr_set: unit %d port %s MAC=<"
...
gxmac_pause_addr_get(int unit, soc_port_t port, sal_mac_addr_t mac)
{
    ...
    DBG_10G_VERB(("gxmac_pause_addr_get: unit %d port %s MAC=<"
                  "%02x:%02x:%02x:%02x:%02x:%02x>\n",
                  unit, SOC_PORT_NAME(unit, port),
                  mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]));
...
gxmac_interface_set(int unit, soc_port_t port, soc_port_if_t pif)
{
    ...
    DBG_10G_VERB(("gxmac_interface_set: unit %d port %s %s interface\n",
...
gxmac_interface_get(int unit, soc_port_t port, soc_port_if_t *pif)
{
    ...
    DBG_10G_VERB(("gxmac_interface_get: unit %d port %s %s interface\n",
                  unit, SOC_PORT_NAME(unit, port), gxmac_port_if_names[*pif]));
...
gxmac_frame_max_set(int unit, soc_port_t port, int size)
{
    ...
    /* TriMAC */
    SOC_IF_ERROR_RETURN(WRITE_MAXFRr(unit, port, val64));

    size += 1; /* XGS3 FE MAC_FR is one greater than MAX len */
    COMPILER_64_SET(val64, 0, size);
    return (WRITE_FE_MAXFr(unit, port, val64));
}
...
gxmac_encap_set(int unit, soc_port_t port, int mode)
{
    ...
    DBG_10G_VERB(("gxmac_encap_set: unit %d port %s %s encapsulation\n",
...
gxmac_encap_get(int unit, soc_port_t port, int *mode)
{
    ...
    *mode = soc_reg64_field32_get(unit, MAC_RXCTRLr, rx_ctrl, HDRMODEf);
    ...
    DBG_10G_VERB(("gxmac_encap_get: unit %d port %s %s encapsulation\n",
                  unit, SOC_PORT_NAME(unit, port), gxmac_encap_mode[*mode]));
...
gxmac_frame_spacing_stretch_set(int unit, soc_port_t port, int value)
{
 ...
 if (SOC_IS_XGS3_SWITCH(unit) || SOC_IS_LYNX(unit)) {
     ...
     if (SOC_IS_LYNX(unit)) {
         if (value) {
             SOC_IF_ERROR_RETURN(
             soc_reg_field32_modify(unit, MAC_TXSPARE0r, port,
                                    WAN_MODEf, 1));
         } else {
             SOC_IF_ERROR_RETURN(
             soc_reg_field32_modify(unit, MAC_TXSPARE0r, port,
              ...
              soc_mac_control_t type, int value)
{
 ...
 DBG_10G_VERB(("gxmac_control_set: unit %d port %s: %d = %d\n",
                unit, SOC_PORT_NAME(unit, port),
                type, value));
 ...
 switch(type) {
     case SOC_MAC_CONTROL_RX_SET:
         ...
         copy = regval;
         soc_reg64_field32_set(unit, GMACC1r, &regval, RXEN0f, value ? 1 : 0);
         ...
         copy = regval;
         soc_reg64_field32_set(unit, FE_MAC1r, &regval, RX_ENf, value ? 1 : 0);
         if (COMPILER_64_NE(regval, copy)) {
             SOC_IF_ERROR_RETURN(WRITE_FE_MAC1r(unit, port, regval));
         }
         SOC_IF_ERROR_RETURN(READ_MAC_CTRLr(unit, port, &regval));
         copy = regval;
         soc_reg64_field32_set(unit, MAC_CTRLr, &regval, RXENf, value ? 1 : 0);
         if (COMPILER_64_NE(regval, copy)) {
             SOC_IF_ERROR_RETURN(WRITE_MAC_CTRLr(unit, port, regval));
         }
         break;
              ...
              soc_mac_control_t type, int *value)
{
 ...
 switch(type) {
     case SOC_MAC_CONTROL_RX_SET:
         SOC_IF_ERROR_RETURN(READ_GMACC1r(unit, port, &regval));
         val = soc_reg64_field32_get(unit, GMACC1r, regval, RXEN0f);
         SOC_IF_ERROR_RETURN(READ_FE_MAC1r(unit, port, &regval));
         val |= soc_reg64_field32_get(unit, FE_MAC1r, regval, RX_ENf);
         SOC_IF_ERROR_RETURN(READ_MAC_CTRLr(unit, port, &regval));
         val |= soc_reg64_field32_get(unit, MAC_CTRLr, regval, RXENf);
         *value = val ? 1 : 0;
         break;
     case SOC_MAC_CONTROL_FRAME_SPACING_STRETCH:
         if (SOC_REG_IS_VALID(unit, MAC_TXCTRLr)) {
             SOC_IF_ERROR_RETURN
                 (READ_MAC_TXCTRLr(unit, port, &regval));
             *value = soc_reg64_field32_get(unit, MAC_TXCTRLr, regval,
                                            THROTDENOMf);
             rv = SOC_E_NONE;
         }
         break;
 ...
 DBG_10G_VERB(("gxmac_control_get: unit %d port %s: %d = %d: rv = %d\n",
                unit, SOC_PORT_NAME(unit, port),
                type, *value, rv));
