/*
 * $Id: l2x.c 1.48.2.3 Broadcom SDK $
...
#define SOURCE_VP_SIZE              (8192)  /* 8K source_vp entries */
...
_soc_l2x_cml_struct_alloc(int unit)
{
    ...
    cml_alloc_size = sizeof(int) * SOC_MAX_NUM_PORTS;
    ...
    cml_alloc_size = sizeof(int) * SOURCE_VP_SIZE;
...
soc_l2x_attach(int unit)
{
    ...
    SOC_CONTROL(unit)->l2x_age_timeout = soc_property_get(unit,
                                                          spn_ARL_CLEAN_TIMEOUT_USEC,
                                                          5000000);
#ifdef BCM_EASYRIDER_SUPPORT
    if (SOC_IS_EASYRIDER(unit)) {
        SOC_CONTROL(unit)->l2_traverse_size = soc_property_get(unit, spn_L2DELETE_CHUNKS,
...
STATIC int perform_port_age_simulation(int unit)
{
	...
	if ( port_age_enable==FALSE) {
	        soc_cm_print("INFO:PortAging simulation not enabled yet \n");	
		return SOC_E_NONE;
	}
	port_age_enable=FALSE;
        soc_cm_print("Performing PortAging simulation... STATIC %d \n",port_age_flags & BCM_L2_REPLACE_MATCH_STATIC);
	...
	switch (port_age_flags & (BCM_L2_REPLACE_MATCH_DEST | BCM_L2_REPLACE_MATCH_VLAN)) {
		...
		case BCM_L2_REPLACE_MATCH_DEST | BCM_L2_REPLACE_MATCH_VLAN:
			...
			break;
		default:
	        	soc_cm_print("Unknown PortAging operation !!! \n");	
			return SOC_E_PARAM;
	}
	...
	for (chnk_idx = soc_mem_index_min(unit, L2Xm); chnk_idx <= mem_idx_max; chnk_idx += chunksize) {
		...
		for (ent_idx = 0 ; ent_idx <= chnk_end; ent_idx ++) {
			...
			if ( (port_age_flags & BCM_L2_REPLACE_MATCH_STATIC) != BCM_L2_REPLACE_MATCH_STATIC) {
				if (static_bit_val == 1) {
					soc_cm_print("Skipped STATIC L2 Entry Deletion:port_val:%d mod_id : %d static_bit_val : %d pending_bit_val %d vlan_id : %d vfi_id : %d \n",port_val,mod_val,static_bit_val,pending_bit_val,vlan_val,vfi_val);
					continue;
				}
			}
			if( (port_age_flags & BCM_L2_REPLACE_PENDING) == BCM_L2_REPLACE_PENDING) {
				if (pending_bit_val != 1) {
					soc_cm_print("Skipped Non Pending L2 Entry Deletion:port_val:%d mod_id : %d static_bit_val : %d pending_bit_val %d vlan_id : %d vfi_id : %d \n",port_val,mod_val,static_bit_val,pending_bit_val,vlan_val,vfi_val);
					continue;
				}
			}
			...
			soc_cm_print("Deleted L2 Entry:port_val:%d mod_id : %d static_bit_val : %d pending_bit_val : %d vlan_id : %d vfi_id : %d \n",port_val,mod_val,static_bit_val,pending_bit_val,vlan_val,vfi_val);
        	}
	}
                       ...
                       soc_mem_t tmem, void *fn_data)
{
    ...
    chunksize = SOC_CONTROL(unit)->l2_traverse_size;
