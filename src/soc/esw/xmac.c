/*
 * $Id: xmac.c 1.112.2.13 Broadcom SDK $
...
struct {
    int speed;
    int pa_flag;
} _mac_x_hg_speed_list[] = {
    { 10000, SOC_PA_SPEED_10GB },
    { 15000, SOC_PA_SPEED_15GB },
    { 20000, SOC_PA_SPEED_20GB },
    { 30000, SOC_PA_SPEED_30GB },
    { 40000, SOC_PA_SPEED_40GB },
    { 40000, SOC_PA_SPEED_42GB }
};
...
void
_mac_x_speed_to_pa_flag(int unit, soc_port_t port, int speed, uint32 *pa_flag)
{
    uint32 idx;

    *pa_flag = 0;
    for (idx = 0; idx < sizeof(_mac_x_hg_speed_list) /
             sizeof(_mac_x_hg_speed_list[0]); idx++) {
        if (speed == _mac_x_hg_speed_list[idx].speed) {
            *pa_flag = _mac_x_hg_speed_list[idx].pa_flag;
            break;
        }
    }
}
...
_mac_x_drain_cells(int unit, soc_port_t port)
{
    ...
    SOC_IF_ERROR_RETURN
        (soc_mac_x.md_pause_set(unit, port, 0, pause_rx));

    SOC_IF_ERROR_RETURN
        (soc_mac_x.md_control_get(unit, port, SOC_MAC_CONTROL_PFC_TX_ENABLE,
                                  &pfc_tx));
    SOC_IF_ERROR_RETURN
        (soc_mac_x.md_control_set(unit, port, SOC_MAC_CONTROL_PFC_TX_ENABLE,
                                  0));
    ...
    /* Disable egress metering so that packet draining is not rate limited */
...
{
    soc_info_t *si;
    uint64 rx_ctrl, tx_ctrl, rval, data;
    ...
    if (IS_SCH_PORT(unit, port)) {
        SOC_IF_ERROR_RETURN(READ_XMAC_RX_CTRLr(unit, port, &rx_ctrl));
        soc_reg64_field32_set(unit, XMAC_RX_CTRLr, &rx_ctrl, RUNT_THRESHOLDf, 32);
        soc_reg64_field32_set(unit, XMAC_RX_CTRLr, &rx_ctrl, STRIP_CRCf, 1);
        SOC_IF_ERROR_RETURN(WRITE_XMAC_RX_CTRLr(unit, port, rx_ctrl));

        SOC_IF_ERROR_RETURN(READ_XMAC_PAUSE_CTRLr(unit, port, &data));
        soc_reg64_field32_set(unit, XMAC_PAUSE_CTRLr, &data, RX_PAUSE_ENf, 0);
        soc_reg64_field32_set(unit, XMAC_PAUSE_CTRLr, &data, TX_PAUSE_ENf, 0);
        soc_reg64_field32_set(unit, XMAC_PAUSE_CTRLr, &data, PAUSE_REFRESH_ENf, 0);
        SOC_IF_ERROR_RETURN(WRITE_XMAC_PAUSE_CTRLr(unit, port, data));

        
        SOC_IF_ERROR_RETURN(READ_XMAC_RX_MAX_SIZEr(unit, port, &data));
        soc_reg64_field32_set(unit, XMAC_RX_MAX_SIZEr, &data, RX_MAX_SIZEf, 9616);
    ...
    if (IS_SCH_PORT(unit, port)) {
        /* SCH mode */
        soc_reg64_field32_set(unit, XMAC_MODEr, &rval, HDR_MODEf, 4);
    }
...
mac_x_init(int unit, soc_port_t port)
{
    ...
    if (soc_reg_field_valid(unit, XMAC_CTRLr, XGMII_IPG_CHECK_DISABLEf)) {
        if (IS_HG_PORT(unit , port)) {
            soc_reg64_field32_set(unit, XMAC_CTRLr, &mac_ctrl, XGMII_IPG_CHECK_DISABLEf,1);
        } else {
            soc_reg64_field32_set(unit, XMAC_CTRLr, &mac_ctrl, XGMII_IPG_CHECK_DISABLEf,0);
        }
    }
    ...
    if (!IS_XE_PORT(unit, port) && !IS_GE_PORT(unit, port)) {
        encap = soc_property_port_get(unit, port, spn_HIGIG2_HDR_MODE, 0) ?
            2 : 1;
        soc_reg64_field32_set(unit, XMAC_MODEr, &rval, HDR_MODEf, encap);
    }
    if (soc_reg_field_valid(unit, XMAC_MODEr, SPEED_MODEf)) {
        if (si->port_speed_max[port] >= 10000) {
            soc_reg64_field32_set(unit, XMAC_MODEr, &rval, SPEED_MODEf, SOC_XMAC_SPEED_10000);
        } else if (si->port_speed_max[port] == 2500) {
            soc_reg64_field32_set(unit, XMAC_MODEr, &rval, SPEED_MODEf, SOC_XMAC_SPEED_2500);
        } else if (si->port_speed_max[port] == 1000) {
            soc_reg64_field32_set(unit, XMAC_MODEr, &rval, SPEED_MODEf, SOC_XMAC_SPEED_1000);
        } else if (si->port_speed_max[port] == 100) {
            soc_reg64_field32_set(unit, XMAC_MODEr, &rval, SPEED_MODEf, SOC_XMAC_SPEED_100);
        } else if (si->port_speed_max[port] == 10) {
            soc_reg64_field32_set(unit, XMAC_MODEr, &rval, SPEED_MODEf, SOC_XMAC_SPEED_10);
        }
    }
    SOC_IF_ERROR_RETURN(WRITE_XMAC_MODEr(unit, port, rval));
...
mac_x_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    soc_reg64_field32_set(unit, XMAC_CTRLr, &ctrl, RX_ENf, enable ? 1 : 0);
...
mac_x_pause_get(int unit, soc_port_t port, int *pause_tx, int *pause_rx)
{
    ...
    *pause_rx =
        soc_reg64_field32_get(unit, XMAC_PAUSE_CTRLr, rval, RX_PAUSE_ENf);
...
mac_x_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    if (soc_reg_field_valid(unit, XMAC_MODEr, SPEED_MODEf)) {
        ...
        /* Update the speed */
        SOC_IF_ERROR_RETURN(soc_reg_field32_modify(unit, XMAC_MODEr, port, SPEED_MODEf, speed_select));
    ...
    /* Enable XLGMII_ALIGN_ENB when using Multi-Lane Districution (MLD) 
     * always use IEEE 40G if connected with an external PHY device.
     */
    SOC_IF_ERROR_RETURN(soc_phyctrl_interface_get(unit,port,&intf));
    if ((IS_XE_PORT(unit, port) || PHY_EXTERNAL_MODE(unit,port)) && ((speed == 40000) ||
             ((speed == 20000) && 
             (intf == SOC_PORT_IF_KR))) ) {  /* mld-2 20G */
        xlgmii_align = 1;
    } else {
        xlgmii_align = 0;
    }
...
mac_x_loopback_set(int unit, soc_port_t port, int lb)
{
    ...
    return soc_reg_field32_modify(unit, XMAC_CTRLr, port, LINE_LOCAL_LPBKf,
                                  lb ? 1 : 0);
...
mac_x_frame_max_set(int unit, soc_port_t port, int size)
{
    ...
    /* Do we have such register for XMAC?
     * SOC_IF_ERROR_RETURN(WRITE_XMAC_TX_MAX_SIZEr(unit, port, rval));
     */
                ...
                soc_port_duplex_t duplex, int ifg)
{
    ...
    _mac_x_speed_to_pa_flag(unit, port, speed, &pa_flag); 
...
mac_x_loopback_remote_set(int unit, soc_port_t port, int lb)
{
    ...
    SOC_IF_ERROR_RETURN(READ_XMAC_CTRLr(unit, port, &ctrl));
    octrl = ctrl;
    soc_reg64_field32_set(unit, XMAC_CTRLr, &ctrl, RMTLOOPf,
                          lb ? 1 : 0);
    if (COMPILER_64_NE(ctrl, octrl)) {
        SOC_IF_ERROR_RETURN(WRITE_XMAC_CTRLr(unit, port, ctrl));
    }
                          ...
                          soc_port_ability_t *ability)
{
    ...
    if (soc_feature(unit, soc_feature_xport_convertible)) {
        ...
        if (IS_ST_PORT(unit, port)) {
            ability->encap |= SOC_PA_ENCAP_HIGIG | SOC_PA_ENCAP_HIGIG2;
        }
    }
