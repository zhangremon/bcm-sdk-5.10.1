/*
 * $Id: shadow.c 1.20.2.10 Broadcom SDK $
...
static uint32 iptdm0_xfi[31] = {1,9,5,13,31,1,10,5,15,11,
                            1,14,5,12,16,1,9,5,13,31,
                            1,10,5,15,11,1,14,5,12,16,0};

static uint32 mmutdm0_xfi[30] = {2,512,32,8192,1024,2,16384,32,2048,32768,
                             2,4096,32,65536,1,2,512,32,8192,1024,
                             2,16384,32,2048,32768,2,4096,32,65536,0};
...
soc_shadow_misc_init(int unit)
{
    ...
    PBMP_ALL_ITER(unit, port) {
        ...
        if (!SOC_PBMP_MEMBER(si->port.disabled_bitmap, port)) {
            ...
            soc_reg64_field32_set(unit, EGR_ENABLEr, &reg64, PRT_ENABLEf, 0);
            SOC_IF_ERROR_RETURN(WRITE_EGR_ENABLEr(unit, port, reg64));
        }
    }
    ...
    case 1:
        ...
        do {
            ...
            if (soc_timeout_check(&to)) {
                soc_cm_debug(DK_WARN, "unit %d : IL_MEMORY_INIT_DONE timeout\n", unit);
                break;
            }
        } while (TRUE);
...
soc_shadow_petrab_misc_init(int unit)
{
    ...
    soc_cm_print("Shadow-PetraB Initialization\n");
    ...
    if ((soc_property_get(unit, spn_BCM88732_2X40_1X40, 0)) ||
        ...
        soc_cm_print("Configuring Static Load Balancing(UFLOW) ( 1->[9,10,11,12] 5->[13,14,15,16] )\n");
        ...
        soc_cm_print("Configuring Static Load Balancing(UFLOW) (1->9, 2->10 ... 8->16)\n");
    ...
    PBMP_IL_ITER(unit, p) {
        if ((p == 9) || (p == 13)) {
            ...
            /* Set In-band flow controls */
            SOC_IF_ERROR_RETURN
                (soc_reg_field32_modify(unit, IL_GLOBAL_CONFIGr, p, 
                                        CFG_FLOWCONTROL_TYPEf, 0));
...
int _shadow_tdm0_xfi_mmu_init(int unit)
{
    ...
    PBMP_XE_ITER(unit, p) {
        ...
        if (p <= 8) {
            SOC_IF_ERROR_RETURN
                (soc_reg_field32_modify(unit, PORT_SHARED_LIMITr, p, 
                                    PORT_SHARED_LIMITf, 24713));
        } else {
            SOC_IF_ERROR_RETURN
                (soc_reg_field32_modify(unit, PORT_SHARED_LIMITr, p, 
                                    PORT_SHARED_LIMITf, 1500));
        }
        SOC_IF_ERROR_RETURN
            (soc_reg_fields32_modify(unit, PG_THRESH_SELr, p, 8,
                                    pg_thresh, xe_val));
    ...
    SOC_IF_ERROR_RETURN
        (soc_reg_field32_modify(unit, TOTAL_SHARED_AVAIL_THRESHr, 
                                REG_PORT_ANY, TOTAL_SHARED_AVAIL_THRESHf, 61320));
    ...
    SOC_IF_ERROR_RETURN
        (soc_reg_field32_modify(unit, INPUT_PORT_RX_ENABLEr, REG_PORT_ANY, 
                                INPUT_PORT_RX_ENABLEf, 0x1fe23));
    SOC_IF_ERROR_RETURN
        (soc_reg_field32_modify(unit, OUTPUT_PORT_RX_ENABLEr, REG_PORT_ANY, 
                                OUTPUT_PORT_RX_ENABLEf, 0x1fe23));
    SOC_IF_ERROR_RETURN
        (soc_reg_field32_modify(unit, PORTGRP_WDRR_CONFIGr, REG_PORT_ANY, 
                                PG_WDRR_MTU_QUANTA_SELECTf, 2));
