/*
 * $Id: hercules.c 1.10 Broadcom SDK $
...
/*
 * Hercules chip driver functions.
 */
soc_functions_t soc_hercules_drv_funs = {
...
soc_hercules_mmu_init(int unit)
{
    ...
    soc_cm_debug(DK_VERBOSE, "soc_hercules_mmu_init: unit %d\n", unit);

    /* LLA init before MMU init */
    rv = soc_mem_lla_init(unit);
    if (rv < 0) {
        soc_cm_print("soc_hercules_mmu_init: LLA initialization failed.\n");
    }

    /* Initialize MMU in CMIC */
    rv = soc_hercules_port_mmu_init(unit, CMIC_PORT(unit));
    if (rv < 0) {
        soc_cm_print("soc_hercules_mmu_init: CMIC MMU init failed.\n");
    }
    ...
    PBMP_PORT_ITER(unit, port) {
        ...
        if (rv < 0) {
            soc_cm_print("soc_hercules_mmu_init: "
    ...
    if (!SAL_BOOT_PLISIM) {
        PBMP_PORT_ITER(unit, port) {
            do {
                ...
                if (!done)
                    soc_cm_print("Notice: HW initializing (port %s)\n",
                                 SOC_PORT_NAME(unit, port));
            } while (!done);
        }
    }
    ...
    if (!SAL_BOOT_SIMULATION) { /* Way too slow for sim */
        ...
        if (rv < 0) {
            soc_cm_print("ERROR: clearing VLAN tables failed.\n");
        }
    }
    ...
    if (!SAL_BOOT_SIMULATION) { /* Way too slow for sim */
        ...
        if (rv < 0) {
            soc_cm_print("ERROR: clearing MC tables failed.\n");
        }
    }
	...
	if (rv < 0) {
	    soc_cm_print("soc_hercules_mmu_init: ING_MODMAP clear failed\n");
	}
	rv = soc_mem_clear(unit, MEM_EGR_MODMAPm, MEM_BLOCK_ALL, TRUE);
	if (rv < 0) {
	    soc_cm_print("soc_hercules_mmu_init: EGR_MODMAP clear failed\n");
	}
	rv = soc_mem_clear(unit, MEM_TRUNK_PORT_POOLm, MEM_BLOCK_ALL, TRUE);
	if (rv < 0) {
	    soc_cm_print("soc_hercules_mmu_init: "
			 "TRUNK_PORT_POOL clear failed\n");
	}
             ...
             void *entry_data)
{
    ...
    entry_addr = soc_mem_addr(unit, mem, copyno, 0);
...
soc_hercules_mem_read_word(int unit, uint32 addr, void *word_data)
{
    ...
    if (soc_cm_debug_check(DK_MEM)) {
        soc_cm_print("soc_hercules_mem_read_word: u=%d: "
             ...
             void *entry_data)
{
    ...
    SOC_MEM_BLOCK_ITER(unit, mem, blk) {
        ...
        entry_addr = soc_mem_addr(unit, mem, blk, 0);
        entry_addr += index * meminfo->gran;
...
soc_hercules_mem_write_word(int unit, uint32 addr, void *word_data)
{
    ...
    schan_msg.writecmd.header.datalen = sizeof(uint32);
    ...
    if (soc_cm_debug_check(DK_MEM)) {
        soc_cm_print("soc_hercules_mem_write_word: u=%d: "
                     "addr=0x%08x: value=0x%08x\n",
                     unit, addr, *(uint32 *)word_data);
    }
...
int
soc_hercules_mmu_limits_config(int unit, int port, int num_ports,
			       int num_cos, int lossless)
{
    ...
    soc_cm_debug(DK_VERBOSE,
                 "soc_hercules_mmu_limits_config: "
    ...
    for (cos = 0; cos < NUM_COS(unit); cos++) {
        if (cos < num_cos) {
            /* Packet Limit / COS */
            SOC_IF_ERROR_RETURN(WRITE_MMU_PKTLMTCOSr(unit, port, cos,
                                                     xq_entries / num_cos));
            /* Cell Limit / COS */
            SOC_IF_ERROR_RETURN(WRITE_MMU_CELLLMTCOSr(unit, port, cos,
                   lossless ? lla_cells_good : (lla_cells_good / num_cos)));
        } else {
            /* Packet Limit / COS */
            SOC_IF_ERROR_RETURN(WRITE_MMU_PKTLMTCOSr(unit, port, cos, 0));
            /* Cell Limit / COS */
            SOC_IF_ERROR_RETURN(WRITE_MMU_CELLLMTCOSr(unit, port, cos, 0));
        }
    }
    ...
    if (lossless) { /* Lossless mode */
        ...
        PBMP_ALL_ITER(unit, other_port) {
            ...
            if (other_port != port) {
                /* Per Ingress Packet Limit / Port / COS */
                SOC_IF_ERROR_RETURN(WRITE_MMU_PKTLMTINGr(unit, port,
                                          other_port,
                                          xq_entries / num_ports / num_cos));

                /* Per Ingress Cell Limit / Port */
                SOC_IF_ERROR_RETURN(
                       WRITE_MMU_CELLLMTINGr(unit, port, other_port,
                                             lla_cells_good / num_ports));
    ...
    if (IS_HG_PORT(unit, port)) {  /* CMIC has no ING_CTRLr */
        uint32 reg, oreg;
        SOC_IF_ERROR_RETURN(READ_ING_CNTLr(unit, port, &reg));
        oreg = reg;
        soc_reg_field_set(unit, ING_CNTLr, &reg, LOSSLESSENf,
                          lossless ? 1 : 0);
        if (reg != oreg) {
            SOC_IF_ERROR_RETURN(WRITE_ING_CNTLr(unit, port, reg));
        }
    }
...
soc_hercules_port_mmu_init(int unit, int port)
{
    ...
    if (SOC_IS_HERCULES1(unit)) {
        if (SOC_CONTROL(unit)->sbe_disable[port]) {
            SOC_IF_ERROR_RETURN(WRITE_MMU_INTCNTLr(unit, port,
                                                   SOC_ERR_HAND_MMU_ALL ^
        ...
        SOC_IF_ERROR_RETURN(
        WRITE_MMU_CELLLMTTOTALr(unit, port, lla_cells_good));
    } else {
        SOC_IF_ERROR_RETURN
            (WRITE_MMU_INTCLRr(unit, port, SOC_ERR_HAND_MMU_ALL_H15));
        if (SOC_CONTROL(unit)->sbe_disable[port]) {
...
soc_hercules_pie_init(int unit, int port, pbmp_t pbmp, int testmode)
{
	...
	reg = 0;
	if (SOC_IS_HERCULES1(unit)) {
	    soc_reg_field_set(unit, ING_CNTLr, &reg, TSTMODEENf, 1);
	    SOC_IF_ERROR_RETURN(WRITE_ING_CNTLr(unit, port, reg));
	} else {
	    soc_reg_field_set(unit, ING_CTRLr, &reg, TSTMODEENf, 1);
	    SOC_IF_ERROR_RETURN(WRITE_ING_CTRLr(unit, port, reg));
	}
        soc_cm_print("NOTICE: port %s: INGRESS test mode enabled.\n",
                     SOC_PORT_NAME(unit, port));
    }
