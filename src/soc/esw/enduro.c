/*
 * $Id: enduro.c 1.93.2.2 Broadcom SDK $
...
STATIC soc_enduro_oam_handler_t en_oam_handler[SOC_MAX_NUM_DEVICES] = {NULL};
                                       ...
                                       int schan, char *msg)
{
    ...
    soc_reg_t status_reg;
    uint32 addr, reg_val;
    int index, single_bit = 0, double_bit = 0, multiple = 0, error = 0;
...
_soc_enduro_process_parity_error(int unit)
{
    ...
    for (group = 0; _soc_en_parity_group_info[group].cpi_bit; group++) {
        ...
        soc_cm_debug(DK_VERBOSE, "unit %d %s parity processing\n",
        ...
        SOC_BLOCK_ITER(unit, blk,
            ...
            for (table = 0; info[table].error_field != INVALIDf; table++) {
                ...
                soc_cm_debug(DK_VERBOSE, "unit %d %s analysis\n",
                             unit, msg);
...
soc_enduro_mmu_init(int unit)
{
     ...
     */
    in_reserved_cells = NUM_PORT(unit) * TR_MMU_IN_PORT_MIN_CELLS +
        NUM_PORT(unit) * TR_MMU_PG_HDRM_LIMIT_CELLS +
        EN_MMU_GLOBAL_HDRM_LIMIT_CELLS;
    in_reserved_pkts = NUM_PORT(unit) * TR_MMU_IN_PORT_MIN_PKTS +
        NUM_PORT(unit) * TR_MMU_PG_HDRM_LIMIT_PKTS;
    out_reserved_cells =
        (NUM_PORT(unit) + 1) * TR_MMU_OUT_PORT_MIN_CELLS * TR_MMU_NUM_COS;
    out_reserved_pkts =
        (NUM_PORT(unit) + 1)  * TR_MMU_OUT_PORT_MIN_PKTS * TR_MMU_NUM_COS;
...
{
    int rv = SOC_E_NONE;
    uint32 old_bits, bits, rval, val2, to_usec, max_sz;
    int frm_max;
    ...
    SOC_IF_ERROR_RETURN(MAC_FRAME_MAX_GET(macd, unit, port, &frm_max));
    ...
    /* Bring the unicore out of reset */
    ...
    SOC_IF_ERROR_RETURN(MAC_FRAME_MAX_SET(macd, unit, port, frm_max));
