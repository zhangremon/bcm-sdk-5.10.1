/* $Id: tnl_term.c 1.25 Broadcom SDK $
...
typedef int (*_soc_tnl_term_hash_compare_fn)(_soc_tnl_term_hash_key_t *key1,
                                             _soc_tnl_term_hash_key_t *key2);

STATIC _soc_tnl_term_hash_t *_tnl_term_hash_tab[SOC_MAX_NUM_DEVICES];
 ...
 */ 
STATIC INLINE void 
_soc_tunnel_term_hash_entry_get(int unit, soc_tunnel_term_t *entry, int ipv6,
                                _soc_tnl_term_hash_key_t *entry_key)
{
    ...
    /* Initialization part. */
    sal_memset(entry_key, 0, sizeof(_soc_tnl_term_hash_key_t)); 
    ...
    if (ipv6) {
        ...
        soc_mem_ip6_addr_get(unit, L3_TUNNELm, 
                             ...
                             entry_key->sip, SOC_MEM_IP6_LOWER_ONLY);
        /* SIP [64-127] */
        soc_mem_ip6_addr_get(unit, L3_TUNNELm, 
                             (uint32 *)&entry->entry_arr[1], IP_ADDRf,
                             entry_key->sip, SOC_MEM_IP6_UPPER_ONLY);
        /* DIP [0-63] */
        soc_mem_ip6_addr_get(unit, L3_TUNNELm, 
                             (uint32 *)&entry->entry_arr[2], IP_ADDRf,
                             entry_key->dip, SOC_MEM_IP6_LOWER_ONLY);
        /* DIP [64-127] */
        soc_mem_ip6_addr_get(unit, L3_TUNNELm, 
                             (uint32 *)&entry->entry_arr[3], IP_ADDRf,
                             entry_key->dip, SOC_MEM_IP6_UPPER_ONLY);
        ...
        entry_key->sip_plen =  _shr_ip6_mask_length(sip_mask);
    } else {
        /* Get destination ip. */
        soc_mem_field_get(unit, L3_TUNNELm, entry_ptr, DIPf, 
                          (uint32 *)entry_key->dip);

        /* Get source ip. */
        soc_mem_field_get(unit, L3_TUNNELm, entry_ptr, SIPf, 
                          (uint32 *)entry_key->sip);

        /* Get source ip. */
        if (SOC_MEM_FIELD_VALID(unit, L3_TUNNELm, SIP_MASKf)) {
            soc_mem_field_get(unit, L3_TUNNELm, entry_ptr, SIP_MASKf, 
                              (uint32 *)sip_mask);
            entry_key->sip_plen =  _shr_ip_mask_length((*(uint32 *)sip_mask));
        } else {
            entry_key->sip_plen = (*(uint32 *)entry_key->sip) ? 32 : 0;
        }
    }

#if defined(BCM_EASYRIDER_SUPPORT)
    if (SOC_MEM_FIELD_VALID(unit, L3_TUNNELm, UDP_SRC_PORTf)) {
        entry_key->l4_src_port = 
            soc_mem_field32_get(unit, L3_TUNNELm, entry_ptr, UDP_SRC_PORTf);
    }

    if (SOC_MEM_FIELD_VALID(unit, L3_TUNNELm, UDP_DST_PORTf)) {
        entry_key->l4_dst_port = 
            soc_mem_field32_get(unit, L3_TUNNELm, entry_ptr, UDP_DST_PORTf);
    } 
...
_soc_tunnel_term_type_get(int unit, soc_tunnel_term_t *entry, 
                          int *type, int *ipv6)
{
    int v6;                              /* IPv6 tunnel terminator flag.    */
    _soc_tnl_term_hash_key_t key_hash; /* Tunnel termination hashing key. */
    ...
    if (SOC_MEM_FIELD_VALID(unit, L3_TUNNELm, MODEf)) {
        v6 = soc_mem_field32_get(unit, L3_TUNNELm, 
    ...
    SOC_TNL_TERM_HASH_ENTRY_GET(unit, entry, v6, &key_hash);

    if (SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit) || 
        SOC_IS_VALKYRIE2(unit)) {
        if (v6) {
            if (key_hash.l4_src_port && key_hash.l4_dst_port) {
                if (key_hash.ip_protocol) {
                    if (0 == key_hash.sip_plen) {
 ...
 */ 
STATIC INLINE int 
_soc_tunnel_term_hash_compare_key(_soc_tnl_term_hash_key_t *key1,
                                  _soc_tnl_term_hash_key_t *key2)
{
    return sal_memcmp(key1, key2, sizeof(_soc_tnl_term_hash_key_t));
}
...
_soc_tunnel_term_hash_create(int unit, 
                             int entry_count,
                             int index_count,
                             _soc_tnl_term_hash_t **tnl_term_hash_ptr)
{
    _soc_tnl_term_hash_t  *hash;        /* Tunnel termination hash. */
    ...
    alloc_size = sizeof (_soc_tnl_term_hash_t);
 ...
 */
STATIC int 
_soc_tunnel_term_hash_destroy(_soc_tnl_term_hash_t **tnl_term_hash)
{
    _soc_tnl_term_hash_t *ptr;   /* Hash pointer. */
...
_soc_tunnel_term_entry_read(int unit, int index, 
                            soc_tunnel_term_t *entry, int *ipv6)
{
    int idx;     /* Entry width iteration index. */

    /* Input parameters check. */
    if ((NULL == entry) || (NULL == ipv6)) {
        return (SOC_E_PARAM);
    }
    ...
    SOC_IF_ERROR_RETURN(_soc_tunnel_term_type_get(unit, entry, NULL, ipv6));
    if (*ipv6) {
                             ...
                             soc_tunnel_term_t *entry, int ipv6)
{
    ...
    idx_max = (ipv6) ? SOC_TNL_TERM_IPV6_ENTRY_WIDTH : \
 ...
 */
STATIC int 
_soc_tunnel_term_hash_lookup(_soc_tnl_term_hash_t *hash,
                             _soc_tnl_term_hash_compare_fn key_cmp_fn,
                             _soc_tnl_term_hash_key_t *entry,
                             uint16 *key_index)
{

    soc_tunnel_term_t hw_entry;          /* Tunnel terminator entry from HW.*/
    _soc_tnl_term_hash_key_t  r_entry;   /* Entry key portion.              */
    ...
    while(index != TNL_TERM_HASH_INDEX_NULL) {
        ...
        /* Read entry from HW.  */
        rv = _soc_tunnel_term_entry_read(unit, index, &hw_entry, &ipv6);
        if (SOC_FAILURE(rv)){
            return (SOC_E_INTERNAL);
        }
        SOC_TNL_TERM_HASH_ENTRY_GET(unit, &hw_entry, ipv6, &r_entry);
 ...
 */
STATIC int 
_soc_tunnel_term_hash_insert(_soc_tnl_term_hash_t *hash,
                             _soc_tnl_term_hash_compare_fn key_cmp_fn,
                             _soc_tnl_term_hash_key_t *entry,
                             uint16 old_index,
                             uint16 new_index)
 ...
 */
STATIC int 
_soc_tunnel_term_hash_delete(_soc_tnl_term_hash_t *hash,
                             _soc_tnl_term_hash_compare_fn key_cmp_fn,
                             _soc_tnl_term_hash_key_t *entry,
                             uint16 delete_index)
{
...
{
    _soc_tnl_term_hash_key_t    key_hash;      /* Entry tcam lookup key.   */
    int ipv6;                                  /* IPv6 tunnel terminator.  */
    int rv;                                    /* Operation return status. */
    ...
    SOC_IF_ERROR_RETURN(_soc_tunnel_term_type_get(unit, entry, NULL, &ipv6));
    SOC_TNL_TERM_HASH_ENTRY_GET(unit, entry, ipv6, &key_hash);
...
soc_tunnel_term_hash_delete(int unit, soc_tunnel_term_t *key_data, 
                         uint32 tab_index)
{
    _soc_tnl_term_hash_key_t  key_hash;    /* Entry lookup key.        */
    int                       ipv6;        /* IPv6 tunnel terminator.  */
    ...
    SOC_IF_ERROR_RETURN(_soc_tunnel_term_type_get(unit, key_data, NULL, &ipv6));
    SOC_TNL_TERM_HASH_ENTRY_GET(unit, key_data, ipv6, &key_hash);
...
{
    soc_tunnel_term_t hw_entry;       /* Tunnel terminator entry from hw. */
    int ipv6;                         /* Entry is ipv6 indicator.         */
    int rv;                           /* Operation return status.         */
    ...
    /* Read source entry. */
    rv = _soc_tunnel_term_entry_read(unit, from_ent, &hw_entry, &ipv6);
    if (SOC_FAILURE(rv)) {
    ...
    SOC_IF_ERROR_RETURN
        (_soc_tunnel_term_entry_write(unit, to_ent, &hw_entry, ipv6));

    /* Reset source entry. */ 
    sal_memset(&hw_entry, 0, sizeof(soc_tunnel_term_t));
    SOC_IF_ERROR_RETURN
        (_soc_tunnel_term_entry_write(unit, from_ent, &hw_entry, ipv6));
...
int _soc_tunnel_term_slot_allocate(int unit, soc_tunnel_term_t *entry, int *index)
{
    ...
    SOC_IF_ERROR_RETURN (_soc_tunnel_term_type_get(unit, entry, &type, &ipv6));
    entry_size = (ipv6) ? SOC_TNL_TERM_IPV6_ENTRY_WIDTH :
        SOC_TNL_TERM_IPV4_ENTRY_WIDTH;
...
_soc_tunnel_term_slot_free (int unit, soc_tunnel_term_t *entry, int index)
{
    ...
    SOC_IF_ERROR_RETURN
        (_soc_tunnel_term_entry_write(unit, from_ent, &null_entry, ipv6));
                       ...
                       soc_tunnel_term_t *result, int *index)
{
    ...
    if (SOC_SUCCESS(rv)) {
        *index = key_index;
        rv = _soc_tunnel_term_entry_read(unit, key_index, result, &ipv6);
    }
    return (rv);
}
...
soc_tunnel_term_reinit(int unit)
{
    ...
    while (idx <= idx_max) {
        rv = _soc_tunnel_term_entry_read(unit, idx, &entry, &ipv6);
        ...
        SOC_IF_ERROR_RETURN
            (_soc_tunnel_term_type_get(unit, &entry, &type, &ipv6));
...
soc_tunnel_term_insert(int unit, soc_tunnel_term_t *entry, uint32 *index)
{
    ...
    if (SOC_SUCCESS(rv)) {
        ...
        rv = _soc_tunnel_term_type_get(unit, entry, NULL, &ipv6);
        if (SOC_FAILURE(rv)) {
            SOC_TNL_TERM_UNLOCK(unit);
            return(rv);
        }
        ...
        rv = _soc_tunnel_term_entry_write(unit, temp_index, entry, ipv6);
        *index = temp_index;
    }
    SOC_TNL_TERM_UNLOCK(unit);
    return(rv);
