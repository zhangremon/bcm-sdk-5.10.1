/*
 * $Id: bcm_ces_sal.c 1.9.2.7 Broadcom SDK $
...
{
  SOC_DEBUG_PRINT((DK_VERBOSE,"%s: %p\n", __func__, (void*)sem->sal_sem));
			...
			AG_BOOL     b_periodic)
{
    if (b_periodic)
    {
/*	SOC_DEBUG_PRINT((DK_VERBOSE,"%s: Periodic:%lumS\n", __func__, n_timer_value));*/
	...
	SOC_DEBUG_PRINT((DK_VERBOSE,"%s - NOT YET IMPLEMENTED\n", __func__));
...
{
    SOC_DEBUG_PRINT((DK_VERBOSE,"%s: flags:0x%08lx\n", __func__, n_flags));
			    ...
			    AG_U32      n_timeout)
{
    ...
    SOC_DEBUG_PRINT((DK_VERBOSE,"%s: req_flags:0x%08lx\n", __func__, n_requested_flags));
...
AgResult agos_create_task(AgosTaskInfo *x_task_info,
    ...
    AG_U32 start)
{
    SOC_DEBUG_PRINT((DK_VERBOSE,"%s: name:%s priority:%lu\n", __func__, p_name, n_priority));
...
void *memset_word(void *dest, AG_U32 n, size_t size)
{
    ...
    /*SOC_DEBUG_PRINT((DK_VERBOSE,"%s: address:%p len:%d data:0x%08lx\n", __func__, address, size, n));*/
