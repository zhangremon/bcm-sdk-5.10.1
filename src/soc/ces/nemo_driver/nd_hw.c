/* $Id: nd_hw.c 1.10.2.4 Broadcom SDK $
 * Copyright 2011 BATM
 */
...
{  
    ...
    for (; n_word_count--; n_offset += 2)
    {
        ...
        n_high = (AG_U8)(n_rdata >> 8);
        n_low = (AG_U8)(n_rdata & 0xff);
        *((AG_U8*)p_data) = n_high;
	p_data++;
        *((AG_U8*)p_data) = n_low;
	p_data++;
      #else
        *((AG_U16*)p_data)++ = n_rdata;  /*BCM write word, need to take care about Endianess*/
      #endif 
    }
}
...
{  
	...
	p_data++;
        n_low = *((AG_U8*)p_data);
	p_data++;
        n_wdata = ((AG_U16)n_high << 8) | (AG_U16)n_low;
       #else
        n_wdata = *((AG_U16*)p_data)++;
       #endif
...
/*
 * Debug, leave in for now, remove later
 */
#if 0
#define MEM_SZ 0x80000
AG_U16 mem[16][MEM_SZ];
AG_U16 reg[MEM_SZ];
void hackMem(AgNdHwAccess  e_action, AG_U32 n_addr, AG_U16 *p_data) 
{
  static int block = 0;
  int ext = 0;
  ...
  if (e_action == AG_ND_HW_ACCESS_READ)
    {
      if (ext)
	*p_data = mem[block][n_addr/2];
      else
	{
	  if (n_addr == 0)
	    *p_data = 0x9000; /* Revision */
	  else
	    *p_data = reg[n_addr/2];
	}

      
      AG_ND_TRACE(0, AG_ND_TRACE_API, AG_ND_TRACE_DEBUG, "%s: Reading address:%p 0x%04x block:%d ext:%d\n", __func__, (AG_U16*)n_addr, *p_data, block, ext);
    }
  else
    {
      if (ext)
	mem[block][n_addr/2] = *p_data;
      else
	reg[n_addr/2] = *p_data;

      AG_ND_TRACE(0, AG_ND_TRACE_API, AG_ND_TRACE_DEBUG, "%s: Writing address:%p 0x%04x block:%d ext:%d\n", __func__, (AG_U16*)n_addr, *p_data, block, ext);
    }
              ...
              AG_U16        *p_data)
{
	...
	{
            soc_reg32_read(0, n_addr, &tempData);
    ...
    else if (AG_ND_HW_ACCESS_WRITE == e_action)
    {
        ...
        if (p_device->b_use_hw)
	{
	    soc_reg32_write(0, n_addr, (AG_U32)*p_data); 
	}
    }
    ...
    {
      if (AG_ND_HW_ACCESS_WRITE == e_action)
          printf("w: addr=0x%06lX data=0x%04hX\n", n_addr, *p_data);

      if (AG_ND_HW_ACCESS_READ == e_action)
          printf("r: addr=0x%06lX data=0x%04hX\n", n_addr, *p_data);
    }
#endif
}
