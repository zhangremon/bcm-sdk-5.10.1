...
soc_robo_dos_monitor_deinit(int unit)
{
    ...
    if (dm != NULL){
        ...
        if (dm->dm_thread != NULL){
            soc_cm_debug(DK_ERR, 
                    "HW DOS monitor disabled but still exist!,thread(%d)\n",
                     (int)(dm->dm_thread));
        }
...
soc_robo_dos_monitor_init(int unit)
{
 ...
 int 
drv_dos_enable_set(int unit, uint32 type, uint32 param)
{
    return SOC_E_UNAVAIL;
}

int 
drv_dos_enable_get(int unit, uint32 type, uint32 *param)
{
    return SOC_E_UNAVAIL;
}
