/*
 * $Id: mem.c 1.29.64.2 Broadcom SDK $
 ...
 */
int 
_drv_bcm5396_mem_search(int unit, uint32 mem, uint32 *key, 
                                    uint32 *entry, uint32 *entry_1, uint32 flags, int *index)
{
    ...
    uint32  vid_rw,vid1 = 0, vid2 = 0, valid, valid1;
...
mem_search_valid_start:        
        ...
        for (count = 0; count < SOC_TIMEOUT_VAL; count++) {
            ...
            soc_cm_debug (DK_ARL," key %d result0 %x result1 %x entry %x %x entry 1 %x %x\n",
                          *key, result, result1, 
            ...
            if ((valid1==1) && (!COMPILER_64_IS_ZERO(entry1))) {
                ...
                /* MAC Address */
                (DRV_SERVICES(unit)->reg_field_get)(unit, 
                    ARLA_SRCH_RSLT_MACVID_1r, (uint32 *)&entry1, 
                    ARLA_SRCH_MACADDRf, (uint32 *)&temp_mac_field);
                /* VLAN ID */
                (DRV_SERVICES(unit)->reg_field_get)(unit, 
                    ARLA_SRCH_RSLT_MACVID_1r, (uint32 *)&entry1, 
                ...
                if (flags & DRV_MEM_OP_SEARCH_PORT) {
                    ...
                    (DRV_SERVICES(unit)->reg_field_get)
                        (unit, ARLA_SRCH_RSLT_1r, &result1, 
                        PORTID_Rf, &temp);
                    if (temp != src_port) {
                        continue;
                    }
                ...
                if (multicast) { /* mcast address */
                    reg_enum = MARLA_SRCH_RSLT_1r;        
                    mcast_index = 0;
                    (DRV_SERVICES(unit)->reg_field_get)
                        (unit, reg_enum, &result1, 
                        FWD_PRT_MAPf, &mcast_index);
                    ((DRV_SERVICES(unit)->mem_field_set)
                        (unit, DRV_MEM_MARL, DRV_MEM_FIELD_DEST_BITMAP, 
                        entry_1, (uint32 *)&mcast_index)); 
...
mem_search_valid_get:
    ...
    /* delete by MAC */    
        ...
        int i, valid[ROBO_5396_L2_BUCKET_SIZE];

        if (flags & DRV_MEM_OP_SEARCH_CONFLICT){
            for (i = 0; i < ROBO_5396_L2_BUCKET_SIZE; i++){
        ...
        /* Not DRV_MEM_OP_REPLACE */
            ...
            /* Check the ARL Entry 1 Register */
            (DRV_SERVICES(unit)->reg_field_get)
                (unit, ARLA_FWD_ENTRY0r, &result1, ARL_VALIDf, &temp);
            if (temp) {
                ...
                if (flags & DRV_MEM_OP_SEARCH_CONFLICT) {
                    ...
                    (DRV_SERVICES(unit)->reg_field_get)
                        ...
                        (uint32 *)&temp_mac_field);
                    (DRV_SERVICES(unit)->reg_field_get)
                        (unit, ARLA_MACVID_ENTRY1r, (uint32 *)&entry0, VID_Rf,
                        &vid2);
                    if (!memcmp(temp_mac_addr, mac_addr_rw, 6)) {
                        if (flags & DRV_MEM_OP_BY_HASH_BY_VLANID) {
                            if (vid2 == vid_rw) {
        ...
        for (i = 0; i < ROBO_5396_L2_BUCKET_SIZE; i++){
            ...
            *index = i;
            ...
            if (temp_mac_addr[0] & 0x01) { /* The input is the mcast address */
                ...
                (DRV_SERVICES(unit)->reg_field_get)
                        (unit, MARLA_FWD_ENTRY0r, input, FWD_PRT_MAPf, &temp);
                (DRV_SERVICES(unit)->mem_field_set)
                        (unit, DRV_MEM_MARL, DRV_MEM_FIELD_DEST_BITMAP, 
 ...
 */
int
_drv_bcm5396_vlan_mem_read(int unit,
 ...
 */
int
_drv_bcm5396_vlan_mem_write(int unit,
    uint32 entry_id, uint32 count, uint32 *entry)
{
    int rv = SOC_E_NONE;
    int i, index;
 ...
 */
int
_drv_bcm5396_mstp_mem_read(int unit,
    uint32 entry_id, uint32 count, uint32 *entry)
{
    int rv = SOC_E_NONE;
    int i, index;
 ...
 */
int
_drv_bcm5396_mstp_mem_write(int unit,
    uint32 entry_id, uint32 count, uint32 *entry)
{
    int rv = SOC_E_NONE;
    int i, index;
...
int 
_drv_bcm5396_mem_arl_entry_delete(int unit, uint8 *mac_addr, uint32 vid, int static_entry)
{
    ...
    temp = MEM_TABLE_READ;
    (DRV_SERVICES(unit)->reg_field_set)
            (unit, ARLA_RWCTLr, &reg_val32, ARL_RWf, &temp);
    temp = 1;
    (DRV_SERVICES(unit)->reg_field_set)
    ...
    if ((rv = (DRV_SERVICES(unit)->reg_read)
        (unit, (DRV_SERVICES(unit)->reg_addr)
        (unit, ARLA_FWD_ENTRY0r, 0, 0), &result0, reg_len)) < 0) {
        goto arl_entry_delete_exit;
    }
    if ((rv = (DRV_SERVICES(unit)->reg_read)
        (unit, (DRV_SERVICES(unit)->reg_addr)
        ...
        goto arl_entry_delete_exit;
            ...
            /* Check the ARL Entry 0 Register */
            (DRV_SERVICES(unit)->reg_field_get)
                (unit, ARLA_FWD_ENTRY0r, (uint32 *)&result0, 
            ...
            if (temp_valid0) {
                ...
                /* check mac + vid */
                if (!memcmp(temp_mac_addr, mac_addr, 6) && 
                    (vid0 == vid)) {
                    /* select bin 0 to overwrite it */
                        binNum = 0;
                }
            ...
            if (binNum == 0) {
                if (!((static_entry==0) && (temp_static0 !=static_entry))) {
                    ...
                    SOC_IF_ERROR_RETURN(
                        (DRV_SERVICES(unit)->reg_field_set)
                        (unit, ARLA_FWD_ENTRY0r, &result0, ARL_VALIDf, &temp));
            ...
            /* can't find a entry to overwrite based on same mac + vid */
            if (binNum == -1) {
                soc_cm_debug(DK_WARN,"entry_not found\n");
                rv = SOC_E_NOT_FOUND;
            }    
...
int
 _drv_bcm5396_mem_delete_ARL_by_port(int unit, uint32 src_port, int static_entry)
{
    int rv = SOC_E_NONE;
    soc_control_t   *soc = SOC_CONTROL(unit);
        ...
        for (idx = index_min; idx < index_count; idx++) {
            ...
            if (valid){
                ...
                if (!(temp_mac_addr[0] & 0x01)) { /* unicast address */
                    rv = _drv_bcm5396_mem_arl_entry_delete(unit, 
                        temp_mac_addr, temp_vid, static_entry);
 ...
 */
int
drv_bcm5396_mem_read(int unit, uint32 mem, 
    uint32 entry_id, uint32 count, uint32 *entry)
{
    ...
    if (mem_id == L2_ARLm ) {        
        ...
        rv = _drv_bcm5396_vlan_mem_read(unit, entry_id, count, entry);
        goto mem_read_exit;
    } else {
        rv = _drv_bcm5396_mstp_mem_read(unit, entry_id, count, entry);
        goto mem_read_exit;
    }
    ...
    for (i = 0;i < count; i++ ) {
        ...
        if ((rv = (DRV_SERVICES(unit)->reg_read)
                (unit, (DRV_SERVICES(unit)->reg_addr)
                (unit, reg_low, 0, 0), (uint32 *)&mem_data0, 8)) < 0) {
            goto mem_read_exit;
        }
        if ((rv = (DRV_SERVICES(unit)->reg_read)
                (unit, (DRV_SERVICES(unit)->reg_addr)
                    (unit, reg_high, 0, 0), (uint32 *)&mem_data1, 8)) < 0) {
            goto mem_read_exit;
        }
            ...
            if (temp_mac_addr[0] & 0x01) {
                 (DRV_SERVICES(unit)->reg_field_get)(unit, reg_high, 
                    (uint32 *)&mem_data1, PORTBMPf, &temp);
                 (DRV_SERVICES(unit)->mem_field_set)
                    (unit, DRV_MEM_MARL, DRV_MEM_FIELD_DEST_BITMAP, 
                     (uint32 *)arl_mem, &temp);
            } else {
                (DRV_SERVICES(unit)->mem_field_set)
                    (unit, DRV_MEM_ARL, DRV_MEM_FIELD_AGE, 
                     (uint32 *)arl_mem, &temp);
                (DRV_SERVICES(unit)->reg_field_get)(unit, reg_high, 
                    (uint32 *)&mem_data1, STATICf, &temp);
                (DRV_SERVICES(unit)->mem_field_set)
                    (unit, DRV_MEM_ARL, DRV_MEM_FIELD_STATIC, 
                     (uint32 *)arl_mem, &temp);
                (DRV_SERVICES(unit)->reg_field_get)(unit, reg_high, 
                    (uint32 *)&mem_data1, PORTID_Rf, &temp);
                (DRV_SERVICES(unit)->mem_field_set)
                    (unit, DRV_MEM_ARL, DRV_MEM_FIELD_SRC_PORT, 
                     (uint32 *)arl_mem, &temp);                
            }
 ...
 */
int
drv_bcm5396_mem_write(int unit, uint32 mem, 
    uint32 entry_id, uint32 count, uint32 *entry)
{
    ...
    if (mem_id == L2_ARLm ){        
        ...
        rv = _drv_bcm5396_vlan_mem_write(unit, entry_id, count, entry);
        goto mem_write_exit;    
    } else {
        rv = _drv_bcm5396_mstp_mem_write(unit, entry_id, count, entry);
        goto mem_write_exit;
    }
    ...
    for (i = 0; i < count; i++) {
        ...
        data_addr0 = (DRV_SERVICES(unit)->reg_addr)
                    (unit, MEM_FRM_DATA0r, 0, 0);
        data_addr1 = (DRV_SERVICES(unit)->reg_addr)
                    (unit, MEM_FRM_DATA1r, 0, 0);
        ...
        if (mem_id == L2_ARLm) {
            ...
            (DRV_SERVICES(unit)->mem_field_get)
                ...
                (uint32 *)arl_mem, &temp); 
            (DRV_SERVICES(unit)->reg_field_set)(unit, reg_low, 
                (uint32 *)&mem_data0, VID_Rf, (uint32 *)&temp); 
            (DRV_SERVICES(unit)->mem_field_get)
                (unit, DRV_MEM_ARL, DRV_MEM_FIELD_PRIORITY, 
                (uint32 *)arl_mem, &temp); 
            (DRV_SERVICES(unit)->reg_field_set)(unit, reg_low, 
            ...
            if (temp_mac_addr[0] & 0x01) {
                ...
                (DRV_SERVICES(unit)->mem_field_get)
                    ...
                    (uint32 *)arl_mem, &temp); 
                (DRV_SERVICES(unit)->reg_field_set)(unit, reg_high, 
                    (uint32 *)&mem_data1, STATICf, (uint32 *)&temp); 
                (DRV_SERVICES(unit)->mem_field_get)
                    (unit, DRV_MEM_ARL, DRV_MEM_FIELD_SRC_PORT, 
                    (uint32 *)arl_mem, &temp); 
                (DRV_SERVICES(unit)->reg_field_set)(unit, reg_high, 
            ...
            if ((rv = (DRV_SERVICES(unit)->reg_write)
                (unit, data_addr0, (uint32 *)&mem_data0, 8)) < 0) {
                goto mem_write_exit;
            }              
            if ((rv = (DRV_SERVICES(unit)->reg_write)
                (unit, data_addr1, (uint32 *)&mem_data1, 8)) < 0) {
                goto mem_write_exit;
            }
            arl_mem ++;
        } 
 ...
 */
int
drv_bcm5396_mem_field_get(int unit, uint32 mem, 
 ...
 */
int
drv_bcm5396_mem_field_set(int unit, uint32 mem, 
    uint32 field_index, uint32 *entry, uint32 *fld_data)
{
    soc_mem_info_t  *meminfo;
    soc_field_info_t    *fieldinfo;
 ...
 */
int 
drv_bcm5396_mem_clear(int unit, uint32 mem)
{
    int rv = SOC_E_NONE;
    uint32 count;
    int mem_id;
 ...
 */
int 
drv_bcm5396_mem_search(int unit, uint32 mem, 
    uint32 *key, uint32 *entry, uint32 *entry1, uint32 flags)
{
    ...
    rv = _drv_bcm5396_mem_search(unit, mem, key, entry, 
        entry1, flags, &index);
 ...
 */
int 
drv_bcm5396_mem_insert(int unit, uint32 mem, uint32 *entry, uint32 flags)
{
    int                 rv = SOC_E_NONE;
    l2_arl_sw_entry_t       output;
    ...
    int index, reg_macvid, reg_fwd;
    int data_base_update = 0;
    ...
    /* write entry if entry does not exist or mem content changed*/
        ...
        if (mac_addr[0] & 0x01) { /* The input is the mcast address */
            ...
            (DRV_SERVICES(unit)->mem_field_get)
                (unit, DRV_MEM_MARL, DRV_MEM_FIELD_AGE, entry, &temp);
            (DRV_SERVICES(unit)->reg_field_set)
                (unit, MARLA_FWD_ENTRY0r, &reg32, ARL_AGEf, &temp);
            /* valid bit */
            temp = 1;
            (DRV_SERVICES(unit)->reg_field_set)
 ...
 */
int 
drv_bcm5396_mem_delete(int unit, uint32 mem, uint32 *entry, uint32 flags)
{
    int                 rv = SOC_E_NONE;
    l2_arl_sw_entry_t       output;
    uint32               entry_reg;
    ...
    if ((ag_port_mode) || (ag_vlan_mode)) {
        ...
        if (ag_port_mode) {
            ...
            if (flags & DRV_MEM_OP_DELETE_ALL_ARL) {
                ...
                _drv_bcm5396_mem_delete_ARL_by_port(unit, src_port, ag_static_mode);
                goto mem_delete_exit;
            }
            if ((rv = (DRV_SERVICES(unit)->reg_write)
                (unit, reg_addr, &reg_value, reg_len)) < 0) {
        ...
        if (rv == SOC_E_EXISTS) {
            ...
            if (index == 0) { 
                ...
                if ((rv = (DRV_SERVICES(unit)->reg_read)
                    (unit, (DRV_SERVICES(unit)->reg_addr)
                    (unit, ARLA_FWD_ENTRY0r, 0, 0), &entry_reg, 4)) < 0) {
                    goto mem_delete_exit;
                }
               /* check static bit if set */
                ...
                if ((rv = (DRV_SERVICES(unit)->reg_read)
                    (unit, (DRV_SERVICES(unit)->reg_addr)
                    (unit, ARLA_FWD_ENTRY1r, 0, 0), &entry_reg, 4)) < 0) {
                    goto mem_delete_exit;
                }
               /* check static bit if set */
                if (!ag_static_mode){
                ...
                if ((rv = (DRV_SERVICES(unit)->reg_write)
                    (unit, (DRV_SERVICES(unit)->reg_addr)
                    (unit, ARLA_FWD_ENTRY0r, 0, 0),
                    (uint32 *)&entry_reg, 4)) < 0) {
                    goto mem_delete_exit;
                }
            }
