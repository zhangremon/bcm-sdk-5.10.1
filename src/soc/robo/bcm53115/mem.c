/*
 * $Id: mem.c 1.34.56.4 Broadcom SDK $
    ...
    uint32 entry_id, uint32 count, uint32 *entry)
{
    ...
    for (i = 0;i < count; i++ ) {
        ...
        if ((index % 2) == 0) {
            ...
            /* Read bin 1 entry */
            if (++i < count) {
                if ((rv = REG_READ_MEM_DEBUG_DATA_0_1r(unit, gmem_entry)) < 0) {
                    goto mem_read_exit;
                }
                gmem_entry++;    
            }
    ...
    uint32 field_index, uint32 *entry, uint32 *fld_data)
{
    ...
    if (mem_id == INDEX(L2_MARL_SWm) || mem_id == INDEX(L2_ARL_SWm)) {
        ...
        entry[0] = entry [2];
        entry [2] = val32;
    } else if ((mem_id == INDEX(L2_ARLm))  || (mem_id == INDEX(MAC2VLANm))) {
        val32 = entry[0];
        entry[0] = entry[1];
        entry[1] = val32;
    }
