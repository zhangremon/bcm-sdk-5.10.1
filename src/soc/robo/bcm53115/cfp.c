/*
 * $Id: cfp.c 1.43.2.4 Broadcom SDK $
 *
...
int 
drv53115_cfp_chain_sub_qual_by_udf(int unit, int enable, int slice_id, uint32 sub_qual, 
 ...
 */
int
_drv53115_cfp_field_mapping(int unit, uint32 mem_type, uint32 field_type, uint32 *field_id)
{
    int i;
    int rv = SOC_E_NONE;
...
int
_drv53115_cfp_qual_by_udf(int unit, int *slice_id, uint32 field_type)
{
    int i;
...
int
_drv53115_cfp_field_in_chain_slice(uint32 field_type)
{
    int rv = FALSE;
 ...
 */
void
_drv53115_cfp_meter_rate2chip(int unit, uint32 kbits_sec, uint32 *chip_val)
{
 ...
 */
void
_drv53115_cfp_meter_chip2rate(int unit, uint32 *kbits_sec, uint32 chip_val)
{
    if (chip_val > 127) {
 ...
 */
void
_drv53115_cfp_meter_burst2chip(int unit, uint32 kbits_burst, uint32 *chip_val)
{
    uint32  burst;
    int i;
 ...
 */
void
_drv53115_cfp_meter_chip2burst(int unit, uint32 *kbits_burst, uint32 chip_val)
{
 ...
 */
int
_drv53115_cfp_read(int unit, uint32 ram_type, 
                         uint32 index, drv_cfp_entry_t *entry)
{
    ...
    case DRV_CFP_RAM_METER:
        ...
        if ((rv = REG_READ_RATE_METER0r(unit, &reg_val)) < 0) {
            goto cfp_read_exit;
        }
        *data_p = reg_val;
        if ((rv = REG_READ_RATE_METER1r(unit, &reg_val)) < 0) {
            goto cfp_read_exit;
        }
        *(data_p + 1) = reg_val;
        break;
 ...
 */
int
_drv53115_cfp_stat_read(int unit, uint32 counter_type, 
 ...
 */
int
_drv53115_cfp_stat_write(int unit, uint32 counter_type, 
                                      uint32 index, uint32 counter)
{
    int rv = SOC_E_NONE;
    uint32  mem_id  = 0;
 ...
 */
int
_drv53115_cfp_tcam_search(int unit, uint32 flags, uint32* index, 
 ...
 */
int
_drv53115_cfp_write(int unit, uint32 ram_type, 
                              uint32 index, drv_cfp_entry_t *entry)
{
    int i, rv = SOC_E_NONE;
    uint32  mem_id = 0;
...
drv53115_cfp_init(int unit)
{
    ...
    if (SOC_IS_ROBO53125(unit)) {
         ...
         */
        sal_memset(&null_entry.tcam_data, 0, sizeof(null_entry.tcam_data));
        sal_memset(&null_entry.tcam_mask, 0, sizeof(null_entry.tcam_mask));
        index_max = soc_robo_mem_index_max(unit, INDEX(CFP_TCAM_IPV4_SCm));
        for (i = 0; i <= index_max; i++) {
            _drv53115_cfp_write(unit,
                DRV_CFP_RAM_TCAM, i, &null_entry);
        }
...
/*
 * Function: drv53115_cfp_action_get
 *
 * Purpose:
 ...
 */
int
drv53115_cfp_action_get(int unit, uint32* action, 
...
/*
 * Function: drv53115_cfp_action_set
 *
 * Purpose:
 ...
 */
int
drv53115_cfp_action_set(int unit, uint32 action, 
 ...
 */
int
drv53115_cfp_control_get(int unit, uint32 control_type, uint32 param1, 
            uint32 *param2)
{
    int rv = SOC_E_NONE;
 ...
 */
int
drv53115_cfp_control_set(int unit, uint32 control_type, uint32 param1, 
            uint32 param2)
{
    int rv = SOC_E_NONE;
    uint32  reg_addr, reg_len, fld_32, reg_val;
    ...
    switch (control_type) {
        ...
        case DRV_CFP_OTHER_RAM_CLEAR:
            ...
            fld_32 = 1;
            (DRV_SERVICES(unit)->reg_field_set)
                (unit, CFP_ACCr, &reg_val, CFP_RAM_CLEARf, &fld_32);
            if ((rv = (DRV_SERVICES(unit)->reg_write)
                (unit, reg_addr, &reg_val, reg_len)) < 0) {
                MEM_UNLOCK(unit, CFP_TCAM_IPV4_SCm);
                return rv;
...
/*
 * Function: drv53115_cfp_entry_read
 *
 * Purpose:
 ...
 */
int
drv53115_cfp_entry_read(int unit, uint32 index, uint32 ram_type, 
            drv_cfp_entry_t *entry)
{
    int rv = SOC_E_NONE;
    ...
    switch (ram_type) {
        case DRV_CFP_RAM_ALL:
            if ((rv = _drv53115_cfp_read(unit, DRV_CFP_RAM_TCAM, index, entry)) 
                != SOC_E_NONE) {
                soc_cm_debug(DK_ERR,
"drv_cfp_entry_read : failed to read TCAM with index = 0x%x, rv = %d. \n", 
            ...
            if ( (rv = _drv53115_cfp_read(unit, DRV_CFP_RAM_ACT, index, entry)) 
                != SOC_E_NONE) {
                soc_cm_debug(DK_ERR,
"drv_cfp_entry_read : failed to read action ram with index = 0x%x, rv = %d. \n",
            ...
            if ((rv = _drv53115_cfp_read(unit, DRV_CFP_RAM_METER, index, entry)) 
                != SOC_E_NONE) {
                soc_cm_debug(DK_ERR,
...
/*
 * Function: drv53115_cfp_entry_search
 *
 * Purpose:
 ...
 */
int
drv53115_cfp_entry_search(int unit, uint32 flags, uint32 *index, 
            drv_cfp_entry_t *entry)
{
    int rv = SOC_E_NONE;

    if ((rv = _drv53115_cfp_tcam_search(unit, flags, 
...
/*
 * Function: drv53115_cfp_entry_write
 *
 * Purpose:
 ...
 */
int
drv53115_cfp_entry_write(int unit, uint32 index, uint32 ram_type, 
            drv_cfp_entry_t *entry)
{
    int rv = SOC_E_NONE;
    ...
    switch (ram_type) {
        case DRV_CFP_RAM_ALL:
            if ((rv = _drv53115_cfp_write(unit, DRV_CFP_RAM_TCAM, index, entry)) 
                != SOC_E_NONE) {
                soc_cm_debug(DK_ERR,
    "drv_cfp_entry_write : failed to write TCAM with index = 0x%x, rv = %d. \n",
            ...
            if ((rv = _drv53115_cfp_write(unit, DRV_CFP_RAM_ACT, index, entry)) 
                != SOC_E_NONE) {
                soc_cm_debug(DK_ERR,
"drv_cfp_entry_write:failed to write action ram with index = 0x%x, rv = %d. \n",
            ...
            if ((rv = _drv53115_cfp_write(unit, DRV_CFP_RAM_METER, index, entry)) 
                != SOC_E_NONE) {
                soc_cm_debug(DK_ERR,
"drv_cfp_entry_write :failed to write meter ram with index = 0x%x, rv = %d. \n",
    ...
    "drv_cfp_entry_write : failed to write TCAM with index = 0x%x, rv = %d. \n",
            ...
            break;
        case DRV_CFP_RAM_TCAM_INVALID:
             if ((rv = _drv53115_cfp_write(unit, DRV_CFP_RAM_TCAM_INVALID, index, entry)) 
                != SOC_E_NONE) {
                soc_cm_debug(DK_ERR,
 ...
 "drv_cfp_entry_write : failed to write ACT ram with index = 0x%x, rv = %d. \n",
            ...
            break;
        case DRV_CFP_RAM_METER:
             if ((rv = _drv53115_cfp_write(unit, 
...
/*
 * Function: drv53115_cfp_field_get
 *
 * Purpose:
 ...
 */
int
drv53115_cfp_field_get(int unit, uint32 mem_type, uint32 field_type, 
            drv_cfp_entry_t* entry, uint32* fld_val)
{
   ...
   mem_id = -1;
        ...
        switch (slice_id) {
            ...
            case 7:
                ...
                if (field_type == DRV_CFP_FIELD_VALID) {
                    ...
                    if (!_drv53115_cfp_field_in_chain_slice(field_type)) {
                        mem_id = INDEX(CFP_TCAM_CHAIN_SCm);
                        entry->flags |= _DRV_CFP_SLICE_CONFIG_SLICE_CHAIN;
                    } else {
    ...
    if (_drv53115_cfp_qual_by_udf(unit, &slice_id, field_type)){
        uint32 temp_val;
        ...
        for (i = 0; i < CFP_53115_UDF_NUM_MAX; i++) {
            if ((drv53115_udf_info[i].valid) && 
                ...
                rv = drv53115_cfp_field_get(unit, mem_type, 
    ...
    if (( rv = _drv53115_cfp_field_mapping(unit, mem_type, field_type, &fld_id)) < 0) {
        soc_cm_debug(DK_ERR,
...
/*
 * Function: drv53115_cfp_field_set
 *
 * Purpose:
 ...
 */
int
drv53115_cfp_field_set(int unit, uint32 mem_type, uint32 field_type, 
            drv_cfp_entry_t* entry, uint32* fld_val)
{
    int rv = SOC_E_NONE;
    int     mem_id;
                ...
                if (field_type == DRV_CFP_FIELD_VALID) {
                    ...
                    if (entry->cfp_chain != NULL) {
                        ...
                        entry->flags = _DRV_CFP_SLICE_CHAIN |
                            _DRV_CFP_SLICE_CONFIG_SLICE_MAIN;
                        SOC_IF_ERROR_RETURN(
                            drv53115_cfp_field_set(unit, mem_type, field_type,
                            entry, fld_val));
    ...
    if (_drv53115_cfp_qual_by_udf(unit, &slice_id, field_type)){
        ...
        for (i = 0; i < CFP_53115_UDF_NUM_MAX; i++) {
            if ((drv53115_udf_info[i].valid) && 
                ...
                rv = drv53115_cfp_field_set(unit, mem_type, 
                    drv53115_udf_info[i].self_field, entry, 
                    &temp_val);
                if (rv  < 0) {
...
/*
 * Function: drv53115_cfp_meter_get
 *
 * Purpose:
 ...
 */
int
drv53115_cfp_meter_get(int unit, drv_cfp_entry_t* entry, uint32 *kbits_sec, 
            uint32 *kbits_burst)    
{
    int rv = SOC_E_NONE;
    uint32  fld_val;
    ...
    if (fld_val) {
        ...
        _drv53115_cfp_meter_chip2rate(unit, kbits_sec, fld_val);
    } else {
        *kbits_sec = 0;
    }
    ...
    _drv53115_cfp_meter_chip2burst(unit, kbits_burst, fld_val);
 ...
 */
int
drv53115_cfp_meter_set(int unit, drv_cfp_entry_t* entry, uint32 kbits_sec, 
            uint32 kbits_burst)
{
    int rv = SOC_E_NONE;
    uint32  fld_val;
    ...
    if (kbits_sec) {
        ...
        _drv53115_cfp_meter_rate2chip(unit, kbits_sec, &fld_val);
    ...
    if (kbits_burst) {
        ...
        _drv53115_cfp_meter_burst2chip(unit, kbits_burst, &fld_val);
        (DRV_SERVICES(unit)->cfp_field_set)(unit, DRV_CFP_RAM_METER, 
                DRV_CFP_FIELD_REF_CAP, entry, &fld_val);
    } else {
        _drv53115_cfp_meter_burst2chip(unit, CFP_53115_METER_BURST_MIN, &fld_val);
        (DRV_SERVICES(unit)->cfp_field_set)(unit, DRV_CFP_RAM_METER, 
                DRV_CFP_FIELD_REF_CAP, entry, &fld_val);
    }
    return rv;
...
int
drv53115_cfp_meter_rate_transform(int unit, uint32 kbits_sec, uint32 kbits_burst, 
                uint32 *bucket_size, uint32 * ref_cnt, uint32 *ref_unit)
{
    ...
    if (kbits_sec) {
        ...
        if ((kbits_sec > CFP_53115_METER_RATE_MAX) ||
            (kbits_sec < CFP_53115_METER_RATE_MIN)) {
            soc_cm_debug(DK_ERR, 
                "drv53115_cfp_meter_rate_transform : rate unsupported. \n");
            rv = SOC_E_UNAVAIL;
            return rv;
        }
        _drv53115_cfp_meter_rate2chip(unit, kbits_sec, ref_cnt);
    } else {
        *ref_cnt = 0;
    }
    ...
    if (kbits_burst) {
        ...
        if ((kbits_burst > CFP_53115_METER_BURST_MAX) ||
            (kbits_burst < CFP_53115_METER_BURST_MIN)) {
            soc_cm_debug(DK_ERR, 
                "drv53115_cfp_meter_rate_transform : burst size unsupported. \n");
            rv = SOC_E_UNAVAIL;
            return rv;
        }
        
        _drv53115_cfp_meter_burst2chip(unit, kbits_burst, bucket_size);
    } else {
        *bucket_size = 0;
    }
...
int drv53115_cfp_slice_id_select_udf(int unit, drv_cfp_entry_t * entry, uint32 * slice_id);
int drv53115_cfp_chain_slice_id_select(int unit, drv_cfp_entry_t * entry, 
 ...
 */
int
drv53115_cfp_slice_id_select(int unit, drv_cfp_entry_t *entry, uint32 *slice_id, uint32 flags)
{
    uint32 i, j;
    int match;
    int slice;
    ...
    if (flags & DRV_FIELD_QUAL_CHAIN) {
        ...
        return drv53115_cfp_chain_slice_id_select(
            unit, entry, slice_id, flags);
    }
    
    if (flags & DRV_FIELD_QUAL_REPLACE_BY_UDF) {
        return drv53115_cfp_slice_id_select_udf(
            unit, entry, slice_id);
    }
...
int
drv53115_cfp_slice_id_select_udf(int unit, drv_cfp_entry_t *entry, uint32 *slice_id)
{
    uint32 i, j, k;
    int match, slice;
...
int
drv53115_cfp_chain_slice_id_select(int unit, drv_cfp_entry_t *entry, uint32 *slice_id, uint32 flags)
{
    uint32 i, j;
    int match;
 ...
 */
int
drv53115_cfp_slice_to_qset(int unit, uint32 slice_id, drv_cfp_entry_t *entry)
{
    uint32 i;
...
/*
 * Function: drv53115_cfp_stat_get
 *
 * Purpose:
 ...
 */
int
drv53115_cfp_stat_get(int unit, uint32 stat_type, uint32 index, uint32* counter)
{
    int rv = SOC_E_NONE;
   ...
   index_max = soc_robo_mem_index_max(unit, CFP_STAT_IBm);
    ...
    switch (stat_type) {
        case DRV_CFP_STAT_INBAND:
            _drv53115_cfp_stat_read(unit, DRV_CFP_RAM_STAT_IB, index, 
                counter);
            break;
        case DRV_CFP_STAT_OUTBAND:
            _drv53115_cfp_stat_read(unit, DRV_CFP_RAM_STAT_OB, index, 
                counter);
            break;
...
/*
 * Function: drv53115_cfp_stat_set
 *
 * Purpose:
 ...
 */
int
drv53115_cfp_stat_set(int unit, uint32 stat_type, uint32 index, uint32 counter)
{
    int rv = SOC_E_NONE;
    uint32  index_max;
   ...
   index_max = soc_robo_mem_index_max(unit, CFP_STAT_IBm);
    ...
    switch (stat_type) {
        case DRV_CFP_STAT_INBAND:
            _drv53115_cfp_stat_write(unit, DRV_CFP_RAM_STAT_IB, index, 
                counter);
            break;
        case DRV_CFP_STAT_OUTBAND:
            _drv53115_cfp_stat_write(unit, DRV_CFP_RAM_STAT_OB, index, 
                counter);
            break;
        case DRV_CFP_STAT_ALL:
            _drv53115_cfp_stat_write(unit, DRV_CFP_RAM_STAT_IB, index, 
 ...
 */
int 
drv53115_cfp_udf_get(int unit, uint32 port, uint32 udf_index, 
    uint32 *offset, uint32 *base)
{
    ...
    if (&drv53115_udf_reg_info[udf_index] == NULL) {
        return SOC_E_PARAM;
    }
 ...
 */
int 
drv53115_cfp_udf_set(int unit, uint32 port, uint32 udf_index, 
    uint32 offset, uint32 base)
{
    int rv = SOC_E_NONE;
    uint32  reg_addr, reg_len, temp;
 ...
 */
int 
drv53115_cfp_ranger(int unit, uint32 flags, uint32 min, uint32 max)
{
    return SOC_E_UNAVAIL;
}

/*
 * Function: drv53115_cfp_range_set
 *
 * Purpose:
 *     Check ranger type and parameters.
 *
 ...
 */
int 
drv53115_cfp_range_set(int unit, uint32 type, uint32 id, uint32 param1, uint32 param2)
{
    /* 
     * BCM53115 doesn't support range check by setting register.
     */
    return SOC_E_UNAVAIL;
}

/*
 * Function: drv53115_cfp_range_get
 *
 * Purpose:
 ...
 */
int 
drv53115_cfp_range_get(int unit, uint32 type, uint32 id, uint32 *param1, uint32 *param2)
{
    /* 
     * BCM53115 doesn't support range check by setting register.
     */
...
int 
drv53115_cfp_sub_qual_by_udf(int unit, int enable, int slice_id, uint32 sub_qual, 
    drv_cfp_qual_udf_info_t * qual_udf_info)
{
    int i;
    int match = FALSE;
    ...
    if (slice_id == CFP_53115_IPV6_CHAIN_SLICE_ID) {
        return drv53115_cfp_chain_sub_qual_by_udf(
    ...
    if (enable ) {
        ...
        for (i=0; i < CFP_53115_UDF_NUM_MAX; i++) {
            ...
            if ((drv53115_udf_info[i].valid == 0) && 
                ...
                drv53115_cfp_udf_set(unit, -1, i, drv53115_udf_info[i].offset_value,
