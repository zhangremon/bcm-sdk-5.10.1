/*
 * $Id: common.c 1.15.2.1 Broadcom SDK $
...
soc_robo_regaddrinfo_get(int unit, soc_regaddrinfo_t *ainfo, uint32 addr)
{
    ...
    if (SOC_IS_ROBO5324(unit)) {
        if ((page >= 0x10) && (page <= 0x2B)) { /* Port MII Registers */
            tmp_page = 0x10;
        }
        else if ((page >= 0x50) && (page <= 0x6a)) { /* Port MIB Registers */
            tmp_page = 0x50;
        }        
        ...
        if ((page >= 0x10) && (page <= 0x17)) { /* Port MII Registers */
            tmp_page = 0x10;
        } else if ((page >= 0x20) && (page <= 0x28)) { /* Port MIB Registers */
            tmp_page = 0x20;
        } else if ((page >= 0x87) && (page <= 0x88)) { /* External Port MII Registers */
            tmp_page = 0x87;
        } 
...
soc_robo_reg_sprint_addr(int unit, char *bp, soc_regaddrinfo_t *ainfo)
{
    ...
    if (ainfo->reg != INVALID_Rr && ainfo->field != INVALID_Rf) {
