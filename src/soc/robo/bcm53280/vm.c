...
/*
 * Function: _drv_bcm53280_vm_field_mapping
 *
 * Purpose:
 ...
 */
STATIC int
_drv_bcm53280_vm_field_mapping(int unit, uint32 field_type, uint32 *field_id)
{
    int rv = SOC_E_NONE;
 ...
 */
STATIC int
_drv53280_vm_read(int unit, uint32 ram_type, uint32 index, drv_vm_entry_t *entry)
{
    int rv = SOC_E_NONE;
    ...
    case DRV_VM_RAM_EVM_KEY_DATA:
        ...
        if (ram_type == DRV_VM_RAM_IVM_KEY_DATA) {
            rv = MEM_READ_IVM_KEY_DATA_MASKm(unit, index, (uint32 *)&vm);
            SOC_IF_ERROR_RETURN(rv);
            rv = soc_IVM_KEY_DATA_MASKm_field_get(unit, (uint32 *)&vm, 
                VM_KEY_DATAf, (uint32 *)&fld_data);
            SOC_IF_ERROR_RETURN(rv);
        } else {
            rv = MEM_READ_EVM_KEY_DATA_MASKm(unit, index, (uint32 *)&vm);
            SOC_IF_ERROR_RETURN(rv);
            rv = soc_IVM_KEY_DATA_MASKm_field_get(unit, (uint32 *)&vm, 
                VM_KEY_DATAf, (uint32 *)&fld_data);
            SOC_IF_ERROR_RETURN(rv);
        ...
        if (ram_type == DRV_VM_RAM_IVM_KEY_DATA) {
            rv = soc_IVM_KEY_DATA_MASKm_field_get(unit, (uint32 *)&vm, 
                VM_KEY_MASKf, (uint32 *)&fld_data);
            SOC_IF_ERROR_RETURN(rv);
        } else {
            rv = soc_EVM_KEY_DATA_MASKm_field_get(unit, (uint32 *)&vm, 
                VM_KEY_MASKf, (uint32 *)&fld_data);
            SOC_IF_ERROR_RETURN(rv);
        }
    ...
    case DRV_VM_RAM_EVM_ACT:
        if (ram_type == DRV_VM_RAM_IVM_ACT) {
            rv = MEM_READ_IVM_ACTm(unit, index, (uint32 *)&fld_data);
            SOC_IF_ERROR_RETURN(rv);
        } else {
            rv = MEM_READ_EVM_ACTm(unit, index, (uint32 *)&fld_data);
            SOC_IF_ERROR_RETURN(rv);
        }
#ifdef BE_HOST
 ...
 */
STATIC int
_drv53280_vm_write(int unit, uint32 ram_type, uint32 index, drv_vm_entry_t *entry)
{
    int rv = SOC_E_NONE;
    ivm_key_data_mask_entry_t vm;
    uint64 fld_data;
    ...
    case DRV_VM_RAM_EVM_KEY_DATA:
        ...
        if (ram_type == DRV_VM_RAM_IVM_KEY_DATA) {          
            rv = soc_IVM_KEY_DATA_MASKm_field_set(unit, (uint32 *)&vm, 
                VM_KEY_DATAf, (uint32 *)&fld_data);
            SOC_IF_ERROR_RETURN(rv);
        } else {
            rv = soc_EVM_KEY_DATA_MASKm_field_set(unit, (uint32 *)&vm, 
                VM_KEY_DATAf, (uint32 *)&fld_data);
            SOC_IF_ERROR_RETURN(rv);
        }
        ...
        if (ram_type == DRV_VM_RAM_IVM_KEY_DATA) {          
            rv = soc_IVM_KEY_DATA_MASKm_field_set(unit, (uint32 *)&vm, 
                VM_KEY_MASKf, (uint32 *)&fld_data);
            SOC_IF_ERROR_RETURN(rv);
            rv = MEM_WRITE_IVM_KEY_DATA_MASKm(unit, index, (uint32 *)&vm);

        } else {
            rv = soc_EVM_KEY_DATA_MASKm_field_set(unit, (uint32 *)&vm, 
                VM_KEY_MASKf, (uint32 *)&fld_data);
            SOC_IF_ERROR_RETURN(rv);
            rv = MEM_WRITE_EVM_KEY_DATA_MASKm(unit, index, (uint32 *)&vm);
        }
        break;
    ...
    case DRV_VM_RAM_EVM_ACT:
        ...
        COMPILER_64_SET(fld_data, val32_hi, val32_lo);
        if (ram_type == DRV_VM_RAM_IVM_ACT) {
            rv = MEM_WRITE_IVM_ACTm(unit, index, (uint32 *)&fld_data);
        } else {
            rv = MEM_WRITE_EVM_ACTm(unit, index, (uint32 *)&fld_data);
        }
        break;
    default:
        rv = SOC_E_PARAM;
...
STATIC int
_drv53280_vm_field_set(int unit, uint32 mem_type, uint32 field_type, 
            drv_vm_entry_t* entry, uint32* fld_val)
{
    ...
    if (( rv = _drv_bcm53280_vm_field_mapping(unit, field_type, &fld_id)) < 0) {
        soc_cm_debug(DK_ERR,
...
STATIC int
_drv53280_vm_field_get(int unit, uint32 mem_type, uint32 field_type, 
            drv_vm_entry_t* entry, uint32* fld_val)
{
    int rv = SOC_E_NONE;
    int mem_id;
...
/*
 * Function: drv_bcm53280_vm_deinit
 *
 * Purpose:
 ...
 */
int 
drv_bcm53280_vm_deinit(int unit)
{
...
/*
 * Function: drv_bcm53280_vm_qset_get
 *
 * Purpose:
 ...
 */
int
drv_bcm53280_vm_qset_get(int unit, uint32 qual, drv_vm_entry_t *entry, uint32 *val)
{
    int rv = SOC_E_NONE;
    uint32  wp, bp;
...
/*
 * Function: drv_bcm53280_vm_qset_set
 *
 * Purpose:
 ...
 */
int
drv_bcm53280_vm_qset_set(int unit, uint32 qual, drv_vm_entry_t *entry, uint32 val)
{
    int rv = SOC_E_NONE;
    uint32  wp, bp, temp = 0;
...
/*
 * Function: drv_bcm53280_vm_format_id_select
 *
 * Purpose:
 ...
 */
int
drv_bcm53280_vm_format_id_select(int unit, drv_vm_entry_t *entry, uint32 *id, uint32 flags)
{
...
/*
 * Function: drv_bcm53280_vm_format_to_qset
 *
 * Purpose:
 ...
 */
int
drv_bcm53280_vm_format_to_qset(int unit, uint32 mem_type, uint32 id, drv_vm_entry_t *entry)
{
    uint32 i;
...
/*
 * Function: drv_bcm53280_vm_field_get
 *
 * Purpose:
 ...
 */
int
drv_bcm53280_vm_field_get(int unit, uint32 mem_type, uint32 field_type, 
            drv_vm_entry_t* entry, uint32* fld_val)
{
    int rv = SOC_E_NONE;
    uint32  temp;
        ...
        case DRV_VM_RAM_IVM_KEY_MASK:
            ...
            if (field_type == DRV_VM_FIELD_IVM_INGRESS_SVID) {
                ...
                /* Get bit 0~1 of Ingress SVID from FrameType field */
                 rv = _drv53280_vm_field_get(unit, mem_type, DRV_VM_FIELD_IVM_FRAME_TYPE, entry, &temp);
                 if (rv < 0) {
                    *fld_val = 0;
                     return rv;
                 }
                *fld_val = temp;
                /* Get bit 2~4 of Ingress SVID from IngressCPCP field */
                 rv = _drv53280_vm_field_get(unit, mem_type, DRV_VM_FIELD_IVM_INGRESS_CPCP, entry, &temp);
                 if (rv < 0) {
                    *fld_val = 0;
                     return rv;
                 }
                *fld_val |= temp << 2;
                /* Get bit 5~7 of Ingress SVID from IngressSPCP field */
                 rv = _drv53280_vm_field_get(unit, mem_type, DRV_VM_FIELD_IVM_INGRESS_SPCP, entry, &temp);
                 if (rv < 0) {
                    *fld_val = 0;
                     return rv;
                 }
                *fld_val |= temp << 5;
                /* Get bit 8~11 of Ingress SVID from IVMReserved field */
                 rv = _drv53280_vm_field_get(unit, mem_type, DRV_VM_FIELD_IVM_RESERVED, entry, &temp);
                 if (rv < 0) {
                    *fld_val = 0;
                     return rv;
                 }
                *fld_val |= temp << 8;
                ...
                /* Get bit 0~11 of Ether_Type from Ingress_CVID field */
                 rv = _drv53280_vm_field_get(unit, mem_type, DRV_VM_FIELD_IVM_INGRESS_CVID, entry, &temp);
                 if (rv < 0) {
                    *fld_val = 0;
                     return rv;
                 }
                *fld_val = temp;
                /* Get bit 12~15 of Ether_Type from Ingress_CVID Range field */
                 rv = _drv53280_vm_field_get(unit, mem_type, DRV_VM_FIELD_IVM_INGRESS_CVID_RANGE, entry, &temp);
                 if (rv < 0) {
                    *fld_val = 0;
                     return rv;
                 }
    ...
    rv = _drv53280_vm_field_get(unit, mem_type, field_type, entry, fld_val);
    return rv;
}

/*
 * Function: drv_bcm53280_vm_field_set
 *
 * Purpose:
 ...
 */
int
drv_bcm53280_vm_field_set(int unit, uint32 mem_type, uint32 field_type, 
            drv_vm_entry_t* entry, uint32* fld_val)
{
    int rv = SOC_E_NONE;
    uint32  temp;
    ...
    switch (mem_type) {
        ...
        case DRV_VM_RAM_IVM_KEY_MASK:
            ...
            if (field_type == DRV_VM_FIELD_IVM_INGRESS_SVID) {
                ...
                temp = *fld_val & 0x3;
                 rv = _drv53280_vm_field_set(unit, mem_type, DRV_VM_FIELD_IVM_FRAME_TYPE, entry, &temp);
                 if (rv < 0) {
                     return rv;
                 }
                /* Set bit 2~4 of Ingress SVID to IngressCPCP field */
                temp = (*fld_val >> 2) & 0x7;
                 rv = _drv53280_vm_field_set(unit, mem_type, DRV_VM_FIELD_IVM_INGRESS_CPCP, entry, &temp);
                 if (rv < 0) {
                     return rv;
                 }
                /* Set bit 5~7 of Ingress SVID to IngressSPCP field */
                temp = (*fld_val >> 5) & 0x7;
                 rv = _drv53280_vm_field_set(unit, mem_type, DRV_VM_FIELD_IVM_INGRESS_SPCP, entry, &temp);
                 if (rv < 0) {
                     return rv;
                 }
                /* Set bit 8~11 of Ingress SVID to IVMReserved field */
                temp = (*fld_val >> 8) & 0xf;
                 rv = _drv53280_vm_field_set(unit, mem_type, DRV_VM_FIELD_IVM_RESERVED, entry, &temp);
                ...
                temp = *fld_val & 0xfff;
                 rv = _drv53280_vm_field_set(unit, mem_type, DRV_VM_FIELD_IVM_INGRESS_CVID, entry, &temp);
                 if (rv < 0) {
                     return rv;
                 }
                /* Set bit 12~15 of Ether_Type to Ingress_CVID Range field */
                temp = (*fld_val >> 12) & 0xf;
                 rv = _drv53280_vm_field_set(unit, mem_type, DRV_VM_FIELD_IVM_INGRESS_CVID_RANGE, entry, &temp);
    ...
    rv = _drv53280_vm_field_set(unit, mem_type, field_type, entry, fld_val);
    
    return rv;
}

/*
 * Function: drv_bcm53280_vm_action_get
 *
 * Purpose:
 ...
 */
int
drv_bcm53280_vm_action_get(int unit, uint32 action, 
...
/*
 * Function: drv_bcm53280_vm_action_set
 *
 * Purpose:
 ...
 */
int
drv_bcm53280_vm_action_set(int unit, uint32 action, 
            drv_vm_entry_t* entry, uint32 act_param)
{
    int rv = SOC_E_NONE;
    uint32  fld_val;
...
/*
 * Function: drv_bcm53280_vm_entry_read
 *
 * Purpose:
 ...
 */
int
drv_bcm53280_vm_entry_read(int unit, uint32 index, uint32 ram_type, 
            drv_vm_entry_t *entry)
{
    ...
    switch (ram_type) {
        case DRV_VM_RAM_IVM_ALL:
            if ((rv = _drv53280_vm_read(unit, DRV_VM_RAM_IVM_KEY_DATA, index, entry)) 
                != SOC_E_NONE) {
                soc_cm_debug(DK_ERR,
            ...
            if ( (rv = _drv53280_vm_read(unit, DRV_VM_RAM_IVM_ACT, index, entry)) 
                != SOC_E_NONE) {
                soc_cm_debug(DK_ERR,
        ...
        case DRV_VM_RAM_EVM_ALL:
            if ((rv = _drv53280_vm_read(unit, DRV_VM_RAM_EVM_KEY_DATA, index, entry)) 
                != SOC_E_NONE) {
                soc_cm_debug(DK_ERR,
            ...
            if ( (rv = _drv53280_vm_read(unit, DRV_VM_RAM_EVM_ACT, index, entry)) 
                != SOC_E_NONE) {
                soc_cm_debug(DK_ERR,
...
/*
 * Function: drv_bcm53280_vm_entry_write
 *
 * Purpose:
 ...
 */
int
drv_bcm53280_vm_entry_write(int unit, uint32 index, uint32 ram_type, 
            drv_vm_entry_t *entry)
{
    int rv = SOC_E_NONE;
    ...
    switch (ram_type) {
        case DRV_VM_RAM_IVM_ALL:
            if ((rv = _drv53280_vm_write(unit, DRV_VM_RAM_IVM_KEY_DATA, index, entry)) 
                != SOC_E_NONE) {
                soc_cm_debug(DK_ERR,
            ...
            if ((rv = _drv53280_vm_write(unit, DRV_VM_RAM_IVM_ACT, index, entry)) 
                != SOC_E_NONE) {
                soc_cm_debug(DK_ERR,
        ...
        case DRV_VM_RAM_EVM_ALL:
            if ((rv = _drv53280_vm_write(unit, DRV_VM_RAM_EVM_KEY_DATA, index, entry)) 
                != SOC_E_NONE) {
                soc_cm_debug(DK_ERR,
            ...
            if ((rv = _drv53280_vm_write(unit, DRV_VM_RAM_EVM_ACT, index, entry)) 
                != SOC_E_NONE) {
                soc_cm_debug(DK_ERR,
...
/*
 * Function: drv_bcm53280_vm_range_set
 *
 * Purpose:
 ...
 */
int 
drv_bcm53280_vm_range_set(int unit, uint32 id, uint32 min, uint32 max)
{
    int rv = SOC_E_NONE;
...
/*
 * Function: drv_bcm53280_vm_flow_alloc
 *
 * Purpose:
 ...
 */
int 
drv_bcm53280_vm_flow_alloc(int unit, uint32 type, int *flow_id)
{
...
/*
 * Function: drv_bcm53280_vm_flow_free
 *
 * Purpose:
 *    Free the flow id.
 *
 ...
 */
int 
drv_bcm53280_vm_flow_free(int unit, int flow_id)
{
    return DRV_FP_ID_CONTROL(unit, _DRV_FP_ID_VM_FLOW_ID, 
...
/*
 * Function: drv_bcm53280_vm_ranger_inc
 *
 * Purpose:
 ...
 */
int 
drv_bcm53280_vm_ranger_inc(int unit, int ranger_id)
{
    /* Increase applied count of the ranger_id. */
...
/*
 * Function: drv_bcm53280_vm_ranger_dec
 *
 * Purpose:
 ...
 */
int 
drv_bcm53280_vm_ranger_dec(int unit, int ranger_id)
{
    return DRV_FP_ID_CONTROL(unit, _DRV_FP_ID_VM_RANGE_ID, 
...
/*
 * Function: drv_bcm53280_vm_ranger_count_get
 *
 * Purpose:
 ...
 */
int 
drv_bcm53280_vm_ranger_count_get(int unit, int ranger_id, uint32 *count)
{
    return DRV_FP_ID_CONTROL(unit, _DRV_FP_ID_VM_RANGE_ID, 
