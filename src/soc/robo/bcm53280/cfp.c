/*
 * $Id: cfp.c 1.31.2.2 Broadcom SDK $
 *
 ...
 */
int
_drv53280_cfp_field_mapping(int unit, uint32 mem_type, uint32 field_type, uint32 *field_id)
{
    int rv = SOC_E_NONE;
    int i, rev;
...
int
_drv53280_cfp_chain_slice_id_mapping(int unit,uint32 field_type, 
 ...
 */
int
_drv53280_cfp_read(int unit, uint32 ram_type, 
 ...
 */
int
_drv53280_cfp_write(int unit, uint32 ram_type, 
                              uint32 index, drv_cfp_entry_t *cfp_entry)
{
    int rv = SOC_E_NONE;
    uint32 *mem_entry;
...
/*
 * Function: drv53280_cfp_action_get
 *
 * Purpose:
 ...
 */
int
drv53280_cfp_action_get(int unit, uint32* action, 
...
/*
 * Function: drv53280_cfp_action_set
 *
 * Purpose:
 ...
 */
int
drv53280_cfp_action_set(int unit, uint32 action, 
 ...
 */
int
drv53280_cfp_control_get(int unit, uint32 control_type, uint32 param1, 
            uint32 *param2)
{
    int rv = SOC_E_NONE;
...
/*
 * Function: drv53280_cfp_control_set
 *
 * Purpose:
 ...
 */
int
drv53280_cfp_control_set(int unit, uint32 control_type, uint32 param1, 
            uint32 param2)
{
    int rv = SOC_E_NONE;
    uint32  fld_32, reg_val;
    ...
    switch (control_type) {
        case DRV_CFP_ENABLE:
            ...
            rv = soc_CFP_EN_CTRLr_field_get(unit, &reg_val, CFP_EN_CTRLf, (uint32 *)&fld_32);
            SOC_IF_ERROR_RETURN(rv);
            ...
            rv = soc_CFP_EN_CTRLr_field_set(unit, &reg_val, CFP_EN_CTRLf, (uint32 *)&fld_32);
            SOC_IF_ERROR_RETURN(rv);
...
/*
 * Function: drv53280_cfp_entry_read
 *
 * Purpose:
 ...
 */
int
drv53280_cfp_entry_read(int unit, uint32 index, uint32 ram_type, 
            drv_cfp_entry_t *entry)
{
    ...
    switch (ram_type) {
        case DRV_CFP_RAM_ALL:
            ...
            if (SOC_IS_VO(unit)) {
                rv = _vo_cfp_data_mask_read(unit, DRV_MEM_CFP_DATA_MASK, 
                    index, entry);
            }
#endif
            if (SOC_IS_TB(unit)) {
                rv = _drv53280_cfp_read(unit, DRV_MEM_CFP_DATA_MASK, 
                    index, entry);
            }
            if (SOC_FAILURE(rv)) {
                soc_cm_debug(DK_ERR,
            ...
            rv = _drv53280_cfp_read(unit, DRV_MEM_CFP_ACT, index, entry);
            if (SOC_FAILURE(rv)) {
                soc_cm_debug(DK_ERR,
                    "drv_cfp_entry_read : \
            ...
            rv = _drv53280_cfp_read(unit, DRV_MEM_CFP_METER, index, entry);
            if (SOC_FAILURE(rv)) {
                soc_cm_debug(DK_ERR,
                    "drv_cfp_entry_read : \
 ...
 */
int
drv53280_cfp_entry_search(int unit, uint32 flags, uint32 *index, 
            drv_cfp_entry_t *entry)
{
    int rv = SOC_E_NONE;
 ...
 */
int
drv53280_cfp_entry_write(int unit, uint32 index, uint32 ram_type, 
            drv_cfp_entry_t *entry)
{
    int rv;
    ...
    switch (ram_type) {
        case DRV_CFP_RAM_ALL:
...
#ifdef BCM_VO_SUPPORT
            if (SOC_IS_VO(unit)) {
                rv = _vo_cfp_data_mask_write(unit, DRV_MEM_CFP_DATA_MASK,
                    index, entry);
            } 
#endif
            if (SOC_IS_TB(unit)) {
                rv = _drv53280_cfp_write(unit, DRV_MEM_CFP_DATA_MASK, 
                    index, entry);
            }
            if (SOC_FAILURE(rv)){
                soc_cm_debug(DK_ERR,
            ...
            rv = _drv53280_cfp_write(unit, DRV_MEM_CFP_ACT, index, entry);
            if (SOC_FAILURE(rv)){
                soc_cm_debug(DK_ERR,
            ...
            rv = _drv53280_cfp_write(unit, DRV_MEM_CFP_METER, index, entry);
            if (SOC_FAILURE(rv)){
                soc_cm_debug(DK_ERR,
...
#ifdef BCM_VO_SUPPORT
            if (SOC_IS_VO(unit)) {
                rv = _vo_cfp_data_mask_write(unit, DRV_MEM_CFP_DATA_MASK, index, entry);
            } 
#endif
            if (SOC_IS_TB(unit)) {
                rv = _drv53280_cfp_write(unit, DRV_MEM_CFP_DATA_MASK, index, entry);
            }
            if (SOC_FAILURE(rv)){
                soc_cm_debug(DK_ERR,
                    "drv_cfp_entry_write : failed to write TCAM with index \
 ...
 */
int
drv53280_cfp_field_get(int unit, uint32 mem_type, uint32 field_type, 
            drv_cfp_entry_t* entry, uint32* fld_val)
{
...
#ifdef BCM_VO_SUPPORT
        if (SOC_IS_VO(unit)) {
            return _vo_cfp_field_get(unit, mem_type, field_type, entry, 
                fld_val);
        }
#endif /* BCM_VO_SUPPORT */
        if (SOC_IS_TB(unit)) {
            rv = _drv53280_cfp_chain_slice_id_mapping(unit, field_type, entry, &slice_id);
            SOC_IF_ERROR_RETURN(rv);
            switch (slice_id) {
                case 0:
    ...
    SOC_IF_ERROR_RETURN(rv);
    if (( rv = _drv53280_cfp_field_mapping(unit, mem_type, field_type, &fld_id)) < 0) {
        soc_cm_debug(DK_ERR,
...
/*
 * Function: drv53280_cfp_field_set
 *
 * Purpose:
 ...
 */
int
drv53280_cfp_field_set(int unit, uint32 mem_type, uint32 field_type, 
            drv_cfp_entry_t* entry, uint32* fld_val)
{
    int rv = SOC_E_NONE;
    int     mem_id;
...
#ifdef BCM_VO_SUPPORT
        if (SOC_IS_VO(unit)) {
            return _vo_cfp_field_set(unit, mem_type, field_type, entry, 
                fld_val);
        }
#endif /* BCM_VO_SUPPORT */        
        if (SOC_IS_TB(unit)) {
...
/*
 * Function: drv53280_cfp_meter_get
 *
 * Purpose:
 ...
 */
int
drv53280_cfp_meter_get(int unit, drv_cfp_entry_t* entry, uint32 *kbits_sec, 
            uint32 *kbits_burst)    
{
    int rv = SOC_E_NONE;
...
/*
 * Function: _drv53280_cfp_meter_rate2chip
 *
 * Purpose:
 ...
 */
void
_drv53280_cfp_meter_rate2chip(int unit, uint32 kbits_sec, uint32 *ref_cnt)
{
    *ref_cnt = kbits_sec / TB_RATE_REF_COUNT_GRANULARITY(unit);
    return;
}
int
drv53280_cfp_meter_rate_transform(int unit, uint32 kbits_sec, uint32 kbits_burst, 
                uint32 *bucket_size, uint32 * ref_cnt, uint32 *ref_unit)
{
    ...
    /*    coverity[unsigned_compare]    */
        if ((kbits_sec > CFP_53280_METER_RATE_MAX(unit)) ||
            ...
            soc_cm_debug(DK_ERR, 
                "drv53280_cfp_meter_rate_transform : rate unsupported. \n");
            rv = SOC_E_UNAVAIL;
            return (rv);
        }
        _drv53280_cfp_meter_rate2chip(unit, kbits_sec, ref_cnt);
    } else {
        *ref_cnt = 0;
    }
    ...
    /*    coverity[unsigned_compare]    */
        if ((kbits_burst > CFP_53280_METER_BURST_MAX(unit)) ||
            ...
            soc_cm_debug(DK_ERR, 
                "drv53280_cfp_meter_rate_transform : burst size unsupported. \n");
            rv = SOC_E_UNAVAIL;
            return (rv);
        }
        /* tranfer to Byte */
...
/*
 * Function: drv53280_cfp_meter_set
 *
 * Purpose:
 ...
 */
int
drv53280_cfp_meter_set(int unit, drv_cfp_entry_t* entry, uint32 kbits_sec, 
            uint32 kbits_burst)
{
    int rv = SOC_E_NONE;
    uint32  bkt_size;    
...
/*
 * Function: drv53280_cfp_slice_id_select
 *
 * Purpose:
...
/*
 * Function: drv53280_cfp_slice_to_qset
 *
 * Purpose:
 ...
 */
int
drv53280_cfp_slice_to_qset(int unit, uint32 slice_id, drv_cfp_entry_t *entry)
{
    uint32 i;
...
/*
 * Function: drv53280_cfp_stat_get
 *
 * Purpose:
 ...
 */
int
drv53280_cfp_stat_get(int unit, uint32 stat_type, uint32 index, uint32* counter)
{
    int rv = SOC_E_NONE;
 ...
 */
int
drv53280_cfp_stat_set(int unit, uint32 stat_type, uint32 index, uint32 counter)
{           
    int rv = SOC_E_NONE;
    uint32 *mem_entry;
    int mem_len, size;
 ...
 */
int 
drv53280_cfp_udf_get(int unit, uint32 port, uint32 udf_index, 
 ...
 */
int 
drv53280_cfp_udf_set(int unit, uint32 port, uint32 udf_index, 
    uint32 offset, uint32 base)
{
    int rv = SOC_E_NONE;
    uint32  reg_32, temp;
...
/*
 * Function: drv53280_cfp_qset_get
 *
 * Purpose:
 ...
 */
int
drv53280_cfp_qset_get(int unit, uint32 qual, drv_cfp_entry_t *entry, uint32 *val)
{
    int rv = SOC_E_NONE;
    uint32  wp, bp;
...
/*
 * Function: drv53280_cfp_qset_set
 *
 * Purpose:
 ...
 */
int
drv53280_cfp_qset_set(int unit, uint32 qual, drv_cfp_entry_t *entry, uint32 val)
{
    int rv = SOC_E_NONE;
    uint32  wp, bp, temp = 0;
 ...
 */
int 
drv53280_cfp_ranger(int unit, uint32 flags, uint32 min, uint32 max)
{
    return SOC_E_UNAVAIL;
}

/*
 * Function: drv53280_cfp_range_set
 *
 * Purpose:
 *     Check ranger type and parameters.
 *
...
int 
drv53280_cfp_range_set(int unit, uint32 type, uint32 id, uint32 param1, uint32 param2)
{
    /* 
     * BCM53280 doesn't support range check by setting register.
     */
    return SOC_E_UNAVAIL;
}

/*
 * Function: drv53280_cfp_range_get
 *
 * Purpose:
...
int 
drv53280_cfp_range_get(int unit, uint32 type, uint32 id, uint32 *param1, uint32 *param2)
{
    /* 
     * BCM53280 doesn't support range check by setting register.
     */
