...
static int tb_vt_enable[SOC_MAX_NUM_SWITCH_DEVICES];
 ...
 */
int 
drv_bcm53280_port_vlan_pvid_set(int unit, uint32 port, uint32 outer_tag, uint32 inner_tag)
{
    uint64  reg_value;
    uint32 vid, pri;
    ...
    SOC_IF_ERROR_RETURN(REG_READ_DEF_PORT_QOS_CFGr(unit, port, (uint32 *)&reg_value));
    ...
    soc_DEF_PORT_QOS_CFGr_field_set(unit, (uint32 *)&reg_value, SVIDf, &vid);
    soc_DEF_PORT_QOS_CFGr_field_set(unit, (uint32 *)&reg_value, SPCPf, &pri);
    ...
    soc_DEF_PORT_QOS_CFGr_field_set(unit, (uint32 *)&reg_value, CVIDf, &vid);
    soc_DEF_PORT_QOS_CFGr_field_set(unit, (uint32 *)&reg_value, CPCPf, &pri);

    SOC_IF_ERROR_RETURN(REG_WRITE_DEF_PORT_QOS_CFGr(unit,port, (uint32 *)&reg_value));
 ...
 */
int 
drv_bcm53280_port_vlan_pvid_get(int unit, uint32 port, uint32 *outer_tag, uint32 *inner_tag)
{
    uint64  reg_value;
    ...
    soc_DEF_PORT_QOS_CFGr_field_get(unit, (uint32 *)&reg_value, SVIDf, &vid);
    soc_DEF_PORT_QOS_CFGr_field_get(unit, (uint32 *)&reg_value, SPCPf, &pri);
    *outer_tag = VLAN_CTRL(pri, cfi, vid);

    /* Read CVID, CDEI and CPCP fields from DEF_PORT_QOS_CFGr(0x28, 0x00~0xa8) */
    soc_DEF_PORT_QOS_CFGr_field_get(unit, (uint32 *)&reg_value, CVIDf, &vid);
    soc_DEF_PORT_QOS_CFGr_field_get(unit, (uint32 *)&reg_value, CPCPf, &pri);
 ...
 */
int 
drv_bcm53280_port_vlan_set(int unit, uint32 port, soc_pbmp_t bmp)
{
    int rv = SOC_E_NONE;
 ...
 */
int 
drv_bcm53280_port_vlan_get(int unit, uint32 port, soc_pbmp_t *bmp)
{
    int rv = SOC_E_NONE;
    soc_pbmp_t fwd_bmp;
    soc_pbmp_t block_bmp;
 ...
 */
int 
drv_bcm53280_vlan_mode_set(int unit, uint32 mode)
{
    ...
    switch (mode) 
    {
        case DRV_VLAN_MODE_TAG:
             ...
             * BCM53280's design doesn't have to enable/disable 1Q Vlan.
             */
 ...
 */
int 
drv_bcm53280_vlan_mode_get(int unit, uint32 *mode)
{
    /* 
     * BCM53280's design doesn't have to enable/disable 1Q Vlan.
 ...
 */
int 
drv_bcm53280_vlan_prop_set(int unit, uint32 prop_type, uint32 prop_val)
{
    uint32 reg_value = 0;
    uint32 temp = 0;
    int rv = SOC_E_NONE;
 ...
 */
int 
drv_bcm53280_vlan_prop_get(int unit, uint32 prop_type, uint32 *prop_val)
{
 ...
 */
int 
drv_bcm53280_vlan_prop_port_enable_set(int unit, uint32 prop_type, 
    ...
    switch(prop_type){
        ...
        case DRV_VLAN_PROP_EGRESS_VT_MISS_UNTAG:
            /* 
              ...
              */
            PBMP_ITER(bmp, port) {
                SOC_IF_ERROR_RETURN(REG_READ_DEF_PORT_QOS_CFGr(unit, port, (uint32 *)&reg_value64));
            
                /* Read port profile id from DEF_PORT_QOS_CFGr */
                soc_DEF_PORT_QOS_CFGr_field_get(unit, (uint32 *)&reg_value64, PORT_PROFILEf, &profile);
 ...
 */
int 
drv_bcm53280_vlan_prop_port_enable_get(int unit, uint32 prop_type, 
    ...
    switch(prop_type){
        ...
        case DRV_VLAN_PROP_EGRESS_VT_MISS_UNTAG:
            /* 
              ...
              * 2. Configure the port profile according to the parameter.
              */
            SOC_IF_ERROR_RETURN(REG_READ_DEF_PORT_QOS_CFGr(unit, port_n, (uint32 *)&reg_value64));
