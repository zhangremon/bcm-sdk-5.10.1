...
#define BCM53280_SUPPRESSION_DLF_MASK  0x20
#define BCM53280_SUPPRESSION_MLF_MASK  0x10
#define BCM53280_SUPPRESSION_BROADCAST_MASK  0x08
#define BCM53280_SUPPRESSION_BPDU_MASK  0x04
#define BCM53280_SUPPRESSION_MULTICAST_MASK  0x02
#define BCM53280_SUPPRESSION_UNICAST_MASK  0x01

#define BCM53280_STORM_CONTROL_PKT_MASK \
    (DRV_STORM_CONTROL_BCAST | DRV_STORM_CONTROL_MCAST | \
     DRV_STORM_CONTROL_DLF | DRV_STORM_CONTROL_RSV_MCAST | \
     DRV_STORM_CONTROL_UCAST)

/*
 *  Function : _drv_bcm53280_storm_control_type_enable_set
 *
 *  Purpose :
 ...
 */
int 
_drv_bcm53280_storm_control_type_enable_set(int unit, uint32 port, 
    uint32 type, uint32 enable)
{
    ...
    if (type & DRV_STORM_CONTROL_BUCKET_2) {
        bucket_field = INDEX(BKT2_PKT_MASKf);
    } else {
        bucket_field = INDEX(BKT1_PKT_MASKf);
    }
    ...
    rv = DRV_MEM_FIELD_GET(unit, INDEX(IRC_PORTm), 
        bucket_field, (uint32 *)&irc_entry, &field_val32);
    SOC_IF_ERROR_RETURN(rv);

    if (type & DRV_STORM_CONTROL_BCAST) {
        if (enable) {
            field_val32 |= BCM53280_SUPPRESSION_BROADCAST_MASK;
        } else {
            field_val32 &= ~BCM53280_SUPPRESSION_BROADCAST_MASK;
        }
    }
    if (type & DRV_STORM_CONTROL_MCAST) {
        if (enable) {
            field_val32 |= BCM53280_SUPPRESSION_MULTICAST_MASK |
                BCM53280_SUPPRESSION_MLF_MASK;
        } else {
            field_val32 &= ~(BCM53280_SUPPRESSION_MULTICAST_MASK |
                BCM53280_SUPPRESSION_MLF_MASK);
        }
    ...
    if (type & DRV_STORM_CONTROL_DLF) {
        if (enable) {
            field_val32 |= (BCM53280_SUPPRESSION_DLF_MASK);
        } else {
            field_val32 &= ~(BCM53280_SUPPRESSION_DLF_MASK);
        }
    }
    if (type & DRV_STORM_CONTROL_RSV_MCAST) {
        if (enable) {
            field_val32 |= BCM53280_SUPPRESSION_BPDU_MASK;
        } else {
            field_val32 &= ~BCM53280_SUPPRESSION_BPDU_MASK;
        }
    }
    if (type & DRV_STORM_CONTROL_UCAST) {
        if (enable) {
            field_val32 |= BCM53280_SUPPRESSION_UNICAST_MASK;
        } else {
            field_val32 &= ~BCM53280_SUPPRESSION_UNICAST_MASK;
        }
    }
...
/*
 *  Function : _drv_bcm53280_storm_control_type_enable_get
 *
 *  Purpose :
 ...
 */
int 
_drv_bcm53280_storm_control_type_enable_get(int unit, uint32 port, uint32 *type)
{
    irc_port_entry_t  irc_entry;
    uint32  index, field_val32;
    uint32  bucket_field;
    ...
    if (field_val32 & BCM53280_SUPPRESSION_BROADCAST_MASK) {
        *type |= DRV_STORM_CONTROL_BCAST;
    }
    
    if (field_val32 & BCM53280_SUPPRESSION_MULTICAST_MASK ||
        field_val32 & BCM53280_SUPPRESSION_MLF_MASK) {
        *type |= DRV_STORM_CONTROL_MCAST;
    }
    
    if (field_val32 & BCM53280_SUPPRESSION_DLF_MASK){
        *type |= DRV_STORM_CONTROL_DLF;
    }

    if (field_val32 & BCM53280_SUPPRESSION_BPDU_MASK) {
        *type |= DRV_STORM_CONTROL_RSV_MCAST;
    }

    if (field_val32 & BCM53280_SUPPRESSION_UNICAST_MASK) {
...
/*
 *  Function : drv_bcm53280_storm_control_enable_set
 *
 *  Purpose :
 ...
 */
int 
drv_bcm53280_storm_control_enable_set(int unit, uint32 port, uint8 enable)
{
    irc_port_entry_t  irc_entry;
    uint32  index, field_val32;
    int  rv = SOC_E_NONE;
...
/*
 *  Function : drv_bcm53280_storm_control_enable_get
 *
 *  Purpose :
 ...
 */
int 
drv_bcm53280_storm_control_enable_get(int unit, uint32 port, uint8 *enable)
{
    irc_port_entry_t  irc_entry;
    uint32  index, field_val32;
    int  rv = SOC_E_NONE;
...
/*
 *  Function : drv_bcm53280_storm_control_set
 *
 *  Purpose :
 ...
 *      burst_size  :  burst size value of storm control. (Kbits : kilobits (1000 bits))
 *
 *  Return :
 *      SOC_E_NONE
 *
 ...
 */
int 
drv_bcm53280_storm_control_set(int unit, soc_pbmp_t bmp, uint32 type, 
    uint32 limit, uint32 burst_size)
{
    ...
    soc_cm_debug(DK_PORT, 
        "drv_storm_control_set: unit = %d, bmp= %x, type = 0x%x, limit = %dK, burst = %dK\n",
    ...
    if ((limit > TB_RATE_METER_MAX(unit)) ||
        ...
        soc_cm_debug(DK_ERR, 
            "drv_bcm53280_storm_control_set : rate unsupported. \n");
        return SOC_E_PARAM;
    }
    ...
    if ((burst_size > TB_RATE_BURST_MAX(unit)) ||
        ...
        soc_cm_debug(DK_ERR, 
            "drv_bcm53280_storm_control_set : burst size unsupported. \n");
        return SOC_E_PARAM;
    }
    ...
    PBMP_ITER(bmp, port) {
        ...
        if (limit == 0) { /* Disable storm suppression type */
            SOC_IF_ERROR_RETURN(
                _drv_bcm53280_storm_control_type_enable_set(unit, port, type, FALSE));
        } else {
            disable_type = BCM53280_STORM_CONTROL_PKT_MASK;
            disable_type ^= type;
            /* set storm suppression type */
            SOC_IF_ERROR_RETURN(
                _drv_bcm53280_storm_control_type_enable_set(unit, port, type, TRUE));
            if (disable_type){
                SOC_IF_ERROR_RETURN(_drv_bcm53280_storm_control_type_enable_set
...
/*
 *  Function : drv_bcm53280_storm_control_get
 *
 *  Purpose :
 ...
 *      burst_size  :  (OUT) burst size value of storm control. (Kbits : kilobits (1000 bits))
 *
 *  Return :
 *      SOC_E_NONE
 *
 ...
 */
int 
drv_bcm53280_storm_control_get(int unit, uint32 port, uint32 *type, 
    uint32 *limit, uint32 *burst_size)
{
    irc_port_entry_t  irc_entry;
    uint32  index, field_val32;
    ...
    int  rv= SOC_E_NONE;
     ...
     SOC_IF_ERROR_RETURN(
        _drv_bcm53280_storm_control_type_enable_get(unit, port, type));
    loc_type = *type & BCM53280_STORM_CONTROL_PKT_MASK;
    if (!loc_type) {
        *limit = 0;
    } else {
        /*
    ...
    soc_cm_debug(DK_PORT, 
        "drv_storm_control_get: unit = %d, port= %d, type = 0x%x, limit = %dK, burst = %dK\n",
