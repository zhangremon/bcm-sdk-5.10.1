/*
 * $Id: mstp.c 1.7.64.1 Broadcom SDK $
 *
...
/*
 *  Function : drv_bcm53280_mstp_port_set
 *
 *  Purpose :
 ...
 */
int 
drv_bcm53280_mstp_port_set(int unit, uint32 mstp_gid, uint32 port, 
    uint32 port_state)
{
    uint32  shift;
    ...
    if (!soc_feature(unit, soc_feature_mstp)) {
        ...
        if(IS_CPU_PORT(unit, port)) {
            ...
            SOC_IF_ERROR_RETURN(soc_IMP_PCTLr_field_set
                (unit, &reg_value, STP_STATEf, (uint32 *)&temp));
            ...
            SOC_IF_ERROR_RETURN(soc_G_PCTLr_field_set
                (unit, &reg_value, G_STP_STATEf, (uint32 *)&temp));
            SOC_IF_ERROR_RETURN(REG_WRITE_G_PCTLr
                (unit, port, &reg_value));
        } else {
        ...
        rv = MEM_READ_MSPT_TABm(unit, mstp_gid, (uint32 *)&mstp_entry);
        ...
        rv = MEM_WRITE_MSPT_TABm(unit, mstp_gid, (uint32 *)&mstp_entry);
        SOC_IF_ERROR_RETURN(rv);
        return SOC_E_NONE;
    }
}
...
/*
 *  Function : drv_bcm53280_mstp_port_get
 *
 *  Purpose :
 ...
 */
int 
drv_bcm53280_mstp_port_get(int unit, uint32 mstp_gid, uint32 port, 
    uint32 *port_state)
{
    uint32  portstate;
    mspt_tab_entry_t  mstp_entry;
    ...
    if (!soc_feature(unit, soc_feature_mstp)){
        ...
        /* write mstp id to vlan entry */       
        SOC_IF_ERROR_RETURN(
            MEM_READ_MSPT_TABm(unit, mstp_gid, (uint32 *)&mstp_entry));
