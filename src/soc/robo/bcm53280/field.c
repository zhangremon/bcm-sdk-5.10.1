/*
 * $Id: field.c 1.49.2.1 Broadcom SDK $
...
int _tb_fp_entry_alloc(int unit, int stage_id, void **entry);
int _tb_fp_entry_copy(int unit, int stage_id, void *src_entry, void *dst_entry );
int _tb_cfp_qual_value_set(int unit, drv_field_qualify_t qual, void *drv_entry,
                      uint32 *p_data, uint32 *p_mask);
int _tb_cfp_qual_value_get(int unit, drv_field_qualify_t qual, void *drv_entry,
                      uint32 *p_data, uint32 *p_mask);

static int 
_tb_cfp_qual_field_get(int unit, drv_cfp_entry_t *cfp_entry, drv_field_qualify_t qual, int *cfp_field)
{
    int     rv = SOC_E_NONE;
    int slice_id;
...
int
_tb_ivm_qual_value_set(int unit, drv_field_qualify_t qual, void *entry,
                      uint32 *p_data, uint32 *p_mask)
{
    int     rv = SOC_E_NONE;
    uint32      fld_index = 0;
    ...
    DRV_FP_VERB(("_robo_field_ivm_qual_value_set : fld_index = 0x%x\n", fld_index));
    SOC_IF_ERROR_RETURN(
  ...
  DRV_FP_VERB(("_robo_field_ivm_qual_value_set : data= 0x%x, 0x%x, \n",
...
int
_tb_ivm_qual_value_get(int unit, drv_field_qualify_t qual, void *entry,
                      uint32 *p_data, uint32 *p_mask)
{
    int     rv = SOC_E_NONE;
    uint32      fld_index = 0;
    ...
    DRV_FP_VERB(("_robo_field_ivm_qual_value_get : fld_index = 0x%x\n", fld_index));
    SOC_IF_ERROR_RETURN(
  ...
  DRV_FP_VERB(("_robo_field_ivm_qual_value_get : data= 0x%x, 0x%x, \n",
        drv_entry->key_data[0], drv_entry->key_data[1]));
...
int
_tb_evm_qual_value_set(int unit, drv_field_qualify_t qual, void *entry,
                      uint32 *p_data, uint32 *p_mask)
{
    int     rv = SOC_E_NONE;
    uint32      fld_index = 0;
    ...
    DRV_FP_VERB(("_robo_field_evm_qual_value_set : fld_index = 0x%x\n", fld_index));
    SOC_IF_ERROR_RETURN(
  ...
  DRV_FP_VERB(("_robo_field_evm_qual_value_set : data= 0x%x, 0x%x,\n",
        drv_entry->key_data[0], drv_entry->key_data[1]));
...
int
_tb_evm_qual_value_get(int unit, drv_field_qualify_t qual, void *entry,
                      uint32 *p_data, uint32 *p_mask)
{
    int     rv = SOC_E_NONE;
    uint32      fld_index = 0;
    ...
    DRV_FP_VERB(("_robo_field_evm_qual_value_get : fld_index = 0x%x\n", fld_index));
    SOC_IF_ERROR_RETURN(
  ...
  DRV_FP_VERB(("_robo_field_evm_qual_value_get : data= 0x%x, 0x%x,\n",
        drv_entry->key_data[0], drv_entry->key_data[1]));
...
int
_tb_cfp_udf_value_set(int unit, uint32 udf_idx, drv_cfp_entry_t *drv_entry,
                      uint32 *p_data, uint32 *p_mask)
{
    uint32      fld_index = 0;
    uint32      temp;
...
int
_tb_cfp_udf_value_get(int unit, uint32 udf_idx, drv_cfp_entry_t *drv_entry,
                      uint32 *p_data, uint32 *p_mask)
{
    uint32      fld_index = 0;
    uint32      temp;
...
int
_tb_cfp_entry_slice_id_change(int unit, drv_cfp_entry_t *drv_entry, int new_id)
{
    int rv;
    int old_id, i, j, k;
    ...
    if (j > 0) {
        rv = _tb_fp_entry_alloc(unit, DRV_FIELD_STAGE_INGRESS, 
                        (void *)&temp_entry);
        SOC_IF_ERROR_RETURN(rv);
        rv = _tb_fp_entry_copy(unit, DRV_FIELD_STAGE_INGRESS, 
        ...
        for (i = 0; i < COUNTOF(cfp_qual_remove); i++) {
            if (cfp_qual_remove[i] != -1) {
                ...
                rv = _tb_cfp_qual_value_set(unit, cfp_qual_remove[i], 
        ...
        for (i = 0; i < COUNTOF(cfp_qual_add); i++) {
            if (cfp_qual_add[i] != -1) {
                sal_memset(p_data, 0, 4 * sizeof(uint32)); 
                sal_memset(p_mask, 0, 4 * sizeof(uint32)); 
                rv = _tb_cfp_qual_value_get(unit, cfp_qual_add[i], temp_entry, 
                        (uint32 *)&p_data, (uint32 *)&p_mask);
                if (SOC_FAILURE(rv)){
                    sal_free(temp_entry);
                    return rv;
                }
                rv = _tb_cfp_qual_value_set(unit, cfp_qual_add[i], drv_entry, 
                        (uint32 *)&p_data, (uint32 *)&p_mask);
                if (SOC_FAILURE(rv)){
                    sal_free(temp_entry);
                    return rv;
...
int
_tb_cfp_slice_id_update(int unit, drv_field_qualify_t qual,
            drv_cfp_entry_t *drv_entry, int *slice_id)
{
    int rv;
    int update, conflict, i, valid;
    uint32 flags, flags_mask,  match_slice;
    int frame, new_frame, id, new_id, id_map, sliceid_map;
    uint32 mask;
    int qual_count;
    ...
    id_map = CFP_53280_SLICE_ID_MAP(id);
    ...
    if (update) {
        *slice_id = (new_frame << 2) | new_id;    
        rv = _tb_cfp_entry_slice_id_change(unit, drv_entry, *slice_id);
        SOC_IF_ERROR_RETURN(rv);
        id = new_id;
...
int
_tb_cfp_slice_id_update_udf(int unit, uint32 udf_idex,
            drv_cfp_entry_t *drv_entry, int *slice_id)
{
    ...
    if (change == 1) {
        rv = _tb_cfp_entry_slice_id_change(unit, drv_entry, new_slice);
        SOC_IF_ERROR_RETURN(rv);
    } else if (change == 0){
        return SOC_E_UNAVAIL;
    } else {    
...
int 
_tb_cfp_qual_value_set(int unit, drv_field_qualify_t qual, void *drv_entry,
                      uint32 *p_data, uint32 *p_mask)
{
    int     rv = SOC_E_NONE;
    uint32      fld_index = 0;

    rv = _tb_cfp_qual_field_get(unit, drv_entry, qual, (int *)&fld_index);    
    SOC_IF_ERROR_RETURN(rv);
...
int 
_tb_cfp_qual_value_get(int unit, drv_field_qualify_t qual, void *drv_entry,
                      uint32 *p_data, uint32 *p_mask)
{
    int     rv = SOC_E_NONE;
    uint32      fld_index = 0;
...
static int
_tb_ingress_qualify_support(int unit, drv_field_qualify_t qual)    
{
...
static int
_tb_qualify_to_cfp(int unit, drv_field_qualify_t qual, 
...
static int
_tb_qset_to_cfp(int unit, drv_field_qset_t qset, drv_cfp_entry_t * drv_entry)
{
    int     idx;
    ...
    for (idx = 0; idx < drvFieldQualifyCount; idx++) {
        ...
        retval = _tb_ingress_qualify_support(unit, idx);
        if (SOC_SUCCESS(retval)){
            retval = _tb_qualify_to_cfp(unit, idx, drv_entry);
            if (SOC_FAILURE(retval)) {
                return retval;
            }            
        } else {
...
static int
_tb_cfp_selcode_to_qset(int unit, 
...
STATIC int
_tb_vm_qset_mapping(drv_vm_entry_t *entry, drv_field_qset_t *qset,
...
STATIC int
_tb_ivm_selcode_to_qset(int unit, 
    int format_id, drv_field_qset_t *qset)
{
    ...
    SOC_IF_ERROR_RETURN(
    _tb_vm_qset_mapping(&drv_entry, qset, 
...
static int
_tb_evm_selcode_to_qset(int unit, 
     int format_id, drv_field_qset_t *qset)
{
    int     rv = SOC_E_NONE;
    drv_vm_entry_t drv_entry;
             ...
             uint32 param1)
{
    ...
    switch (action)
    {
        ...
        case drvFieldActionRedirectPort:
            if ((param0 != 0)||(!SOC_PORT_VALID(unit, (bcm_port_t)param1))){
                return SOC_E_PARAM;
            }
            rv = DRV_CFP_ACTION_SET(unit, DRV_CFP_ACT_CHANGE_FWD, 
                drv_entry, 0, 0);
        ...
        case drvFieldActionRedirectVportPort:
            if ((param0 > 15)||(!SOC_PORT_VALID(unit, (bcm_port_t)param1))) {
                return SOC_E_PARAM;
            }
            rv = DRV_CFP_ACTION_SET(unit, DRV_CFP_ACT_CHANGE_FWD, 
                drv_entry, 0, 0);            
...
int
_tb_ivm_action_remove(int unit, 
...
int
_tb_evm_action_remove(int unit, 
        void *drv_entry,  drv_field_action_t action)
{
    int rv = SOC_E_NONE;    
    switch (action) {
...
int
_tb_cfp_action_remove(int unit, void *drv_entry,  
...
int
_tb_fp_entry_alloc(int unit, int stage_id, void **entry)
{
    int memsize;
...
int
_tb_fp_entry_copy(int unit, int stage_id, void *src_entry, void *dst_entry )
{
    int memsize;
...
int
_tb_fp_entry_clear(int unit, int stage_id, void* entry, int op)
{
    int memsize = 0;
...
int
_tb_fp_entry_tcam_slice_id_set(int unit, int stage_id, void *entry, int sliceId, void *slice_map)
{
    uint32 temp;
    uint32 id;
    drv_cfp_entry_t *drv_entry;
...
int
_tb_fp_entry_tcam_chain_mode_get(int unit, int stage_id, void *drv_entry, int sliceId, void *mode)    
{
    uint32 frame, id;
    drv_cfp_tcam_t *cfp_chain;
...
int
_tb_fp_entry_tcam_valid_control(int unit, int stage_id, void * drv_entry, int tcam_idx, uint32 *valid)
{
    uint32 temp;
...
int
_tb_fp_entry_tcam_block_move(int unit, int stage_id, void *tcam_idx_old, int dest_index, void *amount)
{
...
int
_tb_fp_entry_tcam_move(int unit, int stage_id, void *drv_entry, int amount, void *counter)
{
...
int
_tb_fp_entry_ivm_tcam_policy_install(int unit, void *entry, int tcam_idx)
{
    uint32 temp;
    drv_vm_entry_t *drv_entry = (drv_vm_entry_t *)entry;
    int retval;
...
int
_tb_fp_entry_evm_tcam_policy_install(int unit, void *entry, int tcam_idx)
{
    uint32 temp;
    drv_vm_entry_t *drv_entry = (drv_vm_entry_t *)entry;
    int retval;
        ...
        int tcam_chain_idx)
{
   ...
   if (drv_entry->flags & _DRV_CFP_FIX_UDF_ALL_VALID) {
    ...
    /* entry with chain {slice0,slice3}*/        
        ...
        drv_entry->flags |= _DRV_CFP_SLICE_CONFIG_SLICE_CHAIN;
        temp = 0xfff;
        retval = _tb_cfp_qual_value_set(unit, drvFieldQualifyClassId, 
...
int
_tb_fp_entry_cfp_tcam_meter_install(int unit, void *entry, int tcam_idx, 
        int tcam_chain_idx)
{    
    drv_cfp_entry_t *drv_entry = (drv_cfp_entry_t *)entry;
    uint32 temp_meter[2];
...
int
_tb_fp_entry_cfp_tcam_reinstall(int unit, void *entry, int tcam_idx, 
        int mode)
{
    drv_cfp_entry_t *drv_entry = (drv_cfp_entry_t *)entry;
    int retval;
...
int
_tb_fp_entry_tcam_policy_install(int unit, int stage_id, void *entry, 
            int tcam_idx, int *tcam_chain_idx)
{
    ...
    switch(stage_id) {
        case DRV_FIELD_STAGE_INGRESS:
            rv = SOC_E_UNAVAIL;
            if (SOC_IS_TB(unit)) {
                rv = _tb_fp_entry_cfp_tcam_policy_install(unit, entry, tcam_idx, 
                    *tcam_chain_idx);
            }
#ifdef BCM_VO_SUPPORT
            if (SOC_IS_VO(unit)) {
                rv = _vo_fp_entry_cfp_tcam_policy_install(unit, entry, tcam_idx, 
                    *tcam_chain_idx);
            }
#endif /* BCM_VO_SUPPORT */
            break;
        case DRV_FIELD_STAGE_LOOKUP:
            rv = _tb_fp_entry_ivm_tcam_policy_install(unit, entry, tcam_idx);
            break;
        case DRV_FIELD_STAGE_EGRESS:
            rv = _tb_fp_entry_evm_tcam_policy_install(unit, entry, tcam_idx);
            break;
        default:
            return SOC_E_PARAM;
    }
...
int
_tb_fp_entry_tcam_meter_install(int unit, int stage_id, void *entry, 
            int tcam_idx, int *tcam_chain_idx)
{
    int rv;
    switch(stage_id) {
        case DRV_FIELD_STAGE_INGRESS:
            rv = _tb_fp_entry_cfp_tcam_meter_install(unit, entry, tcam_idx, *tcam_chain_idx);
            break;
        case DRV_FIELD_STAGE_LOOKUP:
        case DRV_FIELD_STAGE_EGRESS:
        default:
...
int
_tb_fp_entry_tcam_reinstall(int unit, int stage_id, void *entry, 
            int tcam_idx, int *mode)
{
    int rv;
    switch(stage_id) {
        case DRV_FIELD_STAGE_INGRESS:
            rv = _tb_fp_entry_cfp_tcam_reinstall(unit, entry, tcam_idx, *mode);
            break;
        case DRV_FIELD_STAGE_LOOKUP:
            rv = _tb_fp_entry_ivm_tcam_policy_install(unit, entry, tcam_idx);
            break;
...
int 
_tb_fp_entry_tcam_remove(int unit, int stage_id, int tcam_idx, 
                                                            int *tcam_chain_idx)
{
    int rv;
    void *drv_entry=NULL;
    ...
    rv = _tb_fp_entry_alloc(unit, stage_id, &drv_entry);
    if (SOC_FAILURE(rv)) {
        return rv;
    }
    ...
    switch(stage_id) {
        case DRV_FIELD_STAGE_INGRESS:
            if (*tcam_chain_idx != -1) {
                rv = _tb_fp_entry_tcam_chain_mode_get(unit, stage_id, drv_entry, 
               ...
               ((drv_cfp_entry_t *)drv_entry)->slice_id = 
                    CFP_53280_SLICE_ID_WITH_CHAIN;
                temp = 0;
                rv = _tb_fp_entry_tcam_valid_control(unit, stage_id, 
    ...
    temp = 0;
    rv = _tb_fp_entry_tcam_valid_control(unit, stage_id, drv_entry, tcam_idx, &temp);
...
int 
_tb_fp_entry_tcam_clear(int unit, int stage_id, int tcam_idx, 
...
int
_tb_fp_cfp_qualify_support(int unit, drv_field_qset_t qset)
{
    int rv = SOC_E_NONE;
    int i;
    ...
    for (i = 0; i < drvFieldQualifyCount; i++) {
        if (DRV_FIELD_QSET_TEST(qset, i)) {
            rv = _tb_ingress_qualify_support(unit, i);
            if (SOC_FAILURE(rv)){
                return rv;
            }
        }            
...
int
_tb_fp_ivm_qualify_support(int unit, drv_field_qset_t qset)
{
    int rv = SOC_E_NONE;
    int i;
    /* Qualifier checking */
...
int
_tb_fp_evm_qualify_support(int unit, drv_field_qset_t qset)
{
    int rv = SOC_E_NONE;
    int i;
...
int
_tb_fp_id_control_deinit(int unit)
{
...
int
_tb_fp_id_control_init(int unit)
{
    uint32 temp = 0;
    uint32 cpu_fid = 0;
    int rv;
...
int 
drv_bcm53280_fp_init(int unit,int stage_id)
{
    int rv;
    ...
    rv = _tb_fp_id_control_init(unit);

    return rv;
}

int 
drv_bcm53280_fp_deinit(int unit,int stage_id)
{
    int rv;
    int port;
    ...
    rv =_tb_fp_id_control_deinit(unit);

    return rv;
}

int
drv_bcm53280_fp_id_control(int unit, int type, int op, int flags, int *id, uint32 *count)
{
    int rv = SOC_E_NONE;
    int loop, idx = -1;
 ...
 */
int
drv_bcm53280_fp_entry_mem_control (int unit, int stage_id, int op, void *src_entry, void *dst_entry, void **alloc_entry)
{
    ...
    switch (op) {
        case DRV_FIELD_ENTRY_MEM_ALLOC:
            rv = _tb_fp_entry_alloc(unit, stage_id, alloc_entry);    
            break;    
        case DRV_FIELD_ENTRY_MEM_COPY:
            rv = _tb_fp_entry_copy(unit, stage_id, src_entry, dst_entry);
            break;                
        case DRV_FIELD_ENTRY_MEM_CLEAR_DATAMASK:
            rv = _tb_fp_entry_clear(unit, stage_id, src_entry, op);
            break;
    }
    return rv;
}
...
*/
int
drv_bcm53280_fp_entry_tcam_control(int unit, int stage_id, void* drv_entry, int op, 
        int param1, void *param2)
{
    ...
    switch (op) {
        case DRV_FIELD_ENTRY_TCAM_SLICE_ID_SET:
            rv = SOC_E_UNAVAIL;
            if (SOC_IS_TB(unit)) {
                rv = _tb_fp_entry_tcam_slice_id_set(unit, stage_id, drv_entry, 
                    param1, param2);
            }
#ifdef BCM_VO_SUPPORT
            if (SOC_IS_VO(unit)) {
                rv = _vo_fp_entry_tcam_slice_id_set(unit, stage_id, drv_entry, 
                    param1, param2);
            }
#endif /* BCM_VO_SUPPORT */
            break;
        case DRV_FIELD_ENTRY_TCAM_MOVE:
            rv = _tb_fp_entry_tcam_move(unit, stage_id, drv_entry, param1, param2);
            break;
        case DRV_FIELD_ENTRY_TCAM_BLOCK_MOVE:
            rv = _tb_fp_entry_tcam_block_move(unit, stage_id, drv_entry, param1, param2);
            break;
        case DRV_FIELD_ENTRY_TCAM_REMOVE:
            rv = _tb_fp_entry_tcam_remove(unit, stage_id, param1, param2);
            break;                
        case DRV_FIELD_ENTRY_TCAM_CLEAR:
        case DRV_FIELD_ENTRY_TCAM_DATAMASK_CLEAR:            
            rv = _tb_fp_entry_tcam_clear(unit, stage_id, param1, param2, op);
            break;
        case DRV_FIELD_ENTRY_TCAM_POLICY_INSTALL:
            rv = _tb_fp_entry_tcam_policy_install(unit, stage_id, drv_entry, param1, param2);
            break;
        case DRV_FIELD_ENTRY_TCAM_METER_INSTALL:
            rv = _tb_fp_entry_tcam_meter_install(unit, stage_id, drv_entry, param1, param2);
            break;            
        case DRV_FIELD_ENTRY_TCAM_ENTRY_REINSTALL:
            rv = _tb_fp_entry_tcam_reinstall(unit, stage_id, drv_entry, param1, param2);
            break;                
...
#ifdef BCM_VO_SUPPORT
            if(SOC_IS_VO(unit)) {
                rv = _vo_fp_entry_tcam_chain_mode_get(unit, stage_id, drv_entry, 
                    param1, param2);
            }   
#endif /* BCM_VO_SUPPORT*/
            break;                
...
int
drv_bcm53280_fp_action_conflict(int unit, int stage_id, drv_field_action_t act1, drv_field_action_t act2)
{
...
int
drv_bcm53280_fp_action_support_check(int unit, int stage_id, 
            drv_field_action_t action)
{
...
int
drv_bcm53280_fp_action_add(int unit, int stage_id, void *drv_entry, 
        drv_field_action_t action, uint32 param0, uint32 param1)
{
    int rv = SOC_E_UNAVAIL;
    switch (stage_id){
        case DRV_FIELD_STAGE_INGRESS:  
            rv = _tb_cfp_action_add(unit, drv_entry, action, param0, param1);
            break;
        case DRV_FIELD_STAGE_LOOKUP:  
            rv = _tb_ivm_action_add(unit, drv_entry, action, param0, param1);
            break;
        case DRV_FIELD_STAGE_EGRESS:  
            rv = _tb_evm_action_add(unit, drv_entry, action, param0, param1);
            break;            
        default:
            break;
    }
...
int
drv_bcm53280_fp_action_remove(int unit, int stage_id, void *drv_entry, 
        drv_field_action_t action, uint32 param0, uint32 param1)
{
    int rv = SOC_E_UNAVAIL;
    switch (stage_id){
        case DRV_FIELD_STAGE_INGRESS:  
            rv = _tb_cfp_action_remove(unit, drv_entry, action, param0, param1);
            break;
        case DRV_FIELD_STAGE_LOOKUP:  
            rv = _tb_ivm_action_remove(unit, drv_entry, action);
            break;
        case DRV_FIELD_STAGE_EGRESS:  
            rv = _tb_evm_action_remove(unit, drv_entry, action);
            break;            
        default:
            break;
    }
...
int
drv_bcm53280_fp_selcode_mode_get(int unit, int stage_id, 
    ...
    switch (stage_id){
        case DRV_FIELD_STAGE_INGRESS:  
           drv_entry = sal_alloc(sizeof(drv_cfp_entry_t),"cfp entry");
            ...
            if (SOC_IS_TB(unit)) {
                rv = _tb_qset_to_cfp(unit, drv_qset, 
                            (drv_cfp_entry_t *)drv_entry);
            }
#ifdef BCM_VO_SUPPORT
            if (SOC_IS_VO(unit)) {
                rv = _vo_qset_to_cfp(unit, drv_qset, 
...
int
drv_bcm53280_fp_selcode_to_qset(int unit,  int stage_id, int slice_id, 
                void *qset) 
{
    int rv;
    ...
    switch (stage_id){
        case DRV_FIELD_STAGE_INGRESS:
            rv =_tb_cfp_selcode_to_qset(unit, slice_id, drv_qset);
            break;
        case DRV_FIELD_STAGE_LOOKUP:
            rv = _tb_ivm_selcode_to_qset(unit, slice_id, drv_qset);
            break;
        case DRV_FIELD_STAGE_EGRESS:
            rv = _tb_evm_selcode_to_qset(unit, slice_id, drv_qset);
            break;
        default:
            rv = SOC_E_PARAM;
    }
...
int
drv_bcm53280_fp_qualify_support(int unit, int stage_id, void* qset)
{
    int rv;
    drv_field_qset_t drv_qset;
    ...
    switch (stage_id){
        case DRV_FIELD_STAGE_INGRESS:
            rv = SOC_E_UNAVAIL;
            if (SOC_IS_TB(unit)) {
                rv = _tb_fp_cfp_qualify_support(unit, drv_qset);
            }
#ifdef BCM_VO_SUPPORT
            if (SOC_IS_VO(unit)) {
                rv = _vo_fp_cfp_qualify_support(unit, drv_qset);
            }
#endif /* BCM_VO_SUPPORT */
            break;
        case DRV_FIELD_STAGE_LOOKUP:
            rv = _tb_fp_ivm_qualify_support(unit, drv_qset);
            break;
        case DRV_FIELD_STAGE_EGRESS:
            rv = _tb_fp_evm_qualify_support(unit, drv_qset);
            break;
        default:
            rv = SOC_E_PARAM;
    }
...
int
drv_bcm53280_fp_qual_value_get(int unit, int stage_id, drv_field_qualify_t qual, void *drv_entry, uint32* p_data, uint32* p_mask)
{
    int rv;
    ...
    switch(stage_id){
        case DRV_FIELD_STAGE_INGRESS:
            rv = SOC_E_UNAVAIL;
            if (SOC_IS_TB(unit)) {
                rv = _tb_cfp_qual_value_get 
                            (unit, qual, drv_entry, p_data, p_mask);
            }
#ifdef BCM_VO_SUPPORT
            if (SOC_IS_VO(unit)) {
                rv = _vo_cfp_qual_value_get
                            (unit, qual, drv_entry, p_data, p_mask);            
            }
#endif /* BCM_VO_SUPPORT */            
            break;
        case DRV_FIELD_STAGE_LOOKUP:
            rv = _tb_ivm_qual_value_get 
                        (unit, qual, drv_entry, p_data, p_mask);
            break;
        case DRV_FIELD_STAGE_EGRESS:
            rv = _tb_evm_qual_value_get 
                (unit, qual, drv_entry, p_data, p_mask);
            break;
        default:
            rv = SOC_E_UNAVAIL;
...
int
drv_bcm53280_fp_qual_value_set(int unit, int stage_id, 
            ...
            uint32* p_mask)
{
    ...
    switch(stage_id) {
        case DRV_FIELD_STAGE_INGRESS:
            rv = SOC_E_UNAVAIL;
            if(SOC_IS_TB(unit)){
                rv = _tb_cfp_slice_id_update(unit, qual, drv_entry,&slice_id);
                SOC_IF_ERROR_RETURN(rv);
                
                rv = _tb_cfp_qual_value_set 
                            (unit, qual, drv_entry, p_data, p_mask);
                SOC_IF_ERROR_RETURN(rv);
            ...
            break;
        case DRV_FIELD_STAGE_LOOKUP:
            rv = _tb_ivm_qual_value_set 
                        (unit, qual, drv_entry, p_data, p_mask);
            break;
        case DRV_FIELD_STAGE_EGRESS:
            rv = _tb_evm_qual_value_set 
                (unit, qual, drv_entry, p_data, p_mask);
            break;
        default:
            rv = SOC_E_PARAM;
...
int
drv_bcm53280_fp_udf_value_set(int unit, int stage_id, uint32 udf_idex, void *drv_entry, uint32* p_data, uint32* p_mask)
{
    ...
    switch(stage_id) {
        case DRV_FIELD_STAGE_INGRESS:
            rv = SOC_E_UNAVAIL;
            if (SOC_IS_TB(unit)){
                rv = _tb_cfp_slice_id_update_udf(unit, udf_idex, drv_entry,&slice_id);
                SOC_IF_ERROR_RETURN(rv);
                
                rv = _tb_cfp_udf_value_set 
...
int
drv_bcm53280_fp_udf_value_get(int unit, int stage_id, uint32 udf_idex, void *drv_entry, uint32* p_data, uint32* p_mask)
{
    int rv;
    ...
    switch(stage_id) {
        case DRV_FIELD_STAGE_INGRESS:
            rv = SOC_E_UNAVAIL;
            if (SOC_IS_TB(unit)) {
                rv = _tb_cfp_udf_value_get 
                        (unit, udf_idex, drv_entry, p_data, p_mask);
            }
#ifdef BCM_VO_SUPPORT
            if (SOC_IS_VO(unit)) {
                rv = _vo_cfp_udf_value_get 
                        (unit, udf_idex, drv_entry, p_data, p_mask);
            }
#endif /* BCM_VO_SUPPORT */
            break;
...
int
drv_bcm53280_fp_tcam_parity_check(int unit, drv_fp_tcam_checksum_t *drv_fp_tcam_chksum)
{
    int rv;
    uint64  reg_val;
    ...
    rv = REG_READ_TCAM_CHKSUM_STSr(unit, (uint32 *)&reg_val);
    SOC_IF_ERROR_RETURN(rv);
    DRV_FP_VVERB(("DRV_FP: %s reg_val hi %x lo %x\n",
        FUNCTION_NAME(),COMPILER_64_HI(reg_val),COMPILER_64_LO(reg_val)));    

    rv = soc_TCAM_CHKSUM_STSr_field_get(unit, (uint32 *)&reg_val, 
...
int
_tb_fp_policer_support_check(int unit, int stage_id, drv_policer_config_t *policer_cfg)
{
    int rv;
...
int
_tb_fp_policer_config_b0(int unit, void *entry, drv_policer_config_t *policer_cfg)
{
    int rv = SOC_E_NONE;
    drv_cfp_entry_t *drv_entry = (drv_cfp_entry_t *)entry;
...
int
_tb_fp_policer_config(int unit,  int stage_id, void *entry, drv_policer_config_t *policer_cfg)
{
    int rv = SOC_E_NONE;
    drv_cfp_entry_t *drv_entry;
    ...
    if (SOC_IS_TB_AX(unit)) {
        ...
        return _tb_fp_policer_config_b0(unit, entry, policer_cfg);
    }

}
int
_tb_fp_policer_free(int unit,  int stage_id, void *entry)
{
    drv_cfp_entry_t *drv_entry;
...
int
drv_bcm53280_fp_policer_control(int unit,  int stage_id, int op, void *entry, drv_policer_config_t *policer_cfg)
{
    ...
    switch (op) {
        case DRV_FIELD_POLICER_MODE_SUPPORT:
            rv = _tb_fp_policer_support_check(unit, stage_id, policer_cfg);
            break;
        case DRV_FIELD_POLICER_CONFIG:
            rv =  _tb_fp_policer_config(unit, stage_id, entry, policer_cfg);
            break;                
        case DRV_FIELD_POLICER_FREE:
            rv =  _tb_fp_policer_free(unit, stage_id, entry);
            break;                
    }
    return rv;
}
...
int
drv_bcm53280_fp_stat_type_get(int unit, int stage_id, drv_policer_mode_t policer_mode,
...
int
_tb_counter_mode_support(int unit, int param0, void *mode)
{
    int policer_valid;
...
int
_tb_stat_mode_support(int unit, int stage_id, drv_policer_mode_t policer_mode, void *mode)    
{
    ...
    type1 = type2 = type3 =-1;
    rv = drv_bcm53280_fp_stat_type_get(unit, stage_id, f_stat, policer_mode, &type1, &type2, &type3);
    return rv;
}


int
drv_bcm53280_fp_stat_support_check(int unit, int stage_id, int op, int param0, void *mode)
{
        
    switch (op) {
        case _DRV_FP_STAT_OP_COUNTER_MODE:
            return _tb_counter_mode_support(unit, param0, mode);
        case _DRV_FP_STAT_OP_STAT_MODE:
            return _tb_stat_mode_support(unit, stage_id, param0, mode);
    }
    return SOC_E_NONE;
}
