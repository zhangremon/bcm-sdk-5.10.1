...
#define BCM53280_EAP_PORT_BLOCK_NONE    0
#define BCM53280_EAP_PORT_BLOCK_INGRESS    1
#define BCM53280_EAP_PORT_BLOCK_INGRESS_EGRESS    2

#define BCM53280_EAP_PORT_BLOCK_MASK    0x3

int
_drv_bcm53280_security_init(int unit)
{
...
/*
 *  Function : drv_bcm53280_security_set
 *
 *  Purpose :
 ...
 */
int 
drv_bcm53280_security_set(int unit, soc_pbmp_t bmp, uint32 state, uint32 mask)
{
    int  rv = SOC_E_NONE;
    ...
    switch (state) {
        case DRV_SECURITY_PORT_UNCONTROLLED:
            ...
            SOC_IF_ERROR_RETURN(soc_EAP_PORT_CTLr_field_get
                (unit, (uint32 *)&reg_value64, EAP_PORT_BLKf, (uint32 *)&temp64));

            if (BCM_PBMP_EQ(PBMP_E_ALL(unit), bmp)) {
                ...
                PBMP_ITER(bmp, port) {
                    temp = BCM53280_EAP_PORT_BLOCK_NONE;
                    /* Port is not blocked */
                    temp_mask = BCM53280_EAP_PORT_BLOCK_MASK;
                    if (port > 15) {
        ...
        case DRV_SECURITY_PORT_UNAUTHENTICATE:
            /* 1. Disbale l2 learnning
              ...
              */
            SOC_IF_ERROR_RETURN(_drv_bcm53280_security_init(unit));
            ...
            PBMP_ITER(bmp, port) {
                ...
                temp = (mask & DRV_SECURITY_BLOCK_INOUT) ? 
                            BCM53280_EAP_PORT_BLOCK_INGRESS_EGRESS : 
                            BCM53280_EAP_PORT_BLOCK_INGRESS;
...
/*
 *  Function : drv_bcm53280_security_get
 *
 *  Purpose :
 ...
 */
int 
drv_bcm53280_security_get(int unit, uint32 port, uint32 *state, uint32 *mask)
{
    int  rv = SOC_E_NONE;
    uint64  reg_value64, temp64;
...
int
drv_bcm53280_security_egress_set(int unit, soc_pbmp_t bmp, int enable)
{
    int  rv = SOC_E_NONE;
    uint64  reg_value64, temp64;
    uint32  temp, temp_mask, temp_lo, temp_hi;
    ...
    PBMP_ITER(bmp, port) {
        ...
        if (enable) {
             ...
             */
            temp = BCM53280_EAP_PORT_BLOCK_INGRESS;
        } else {
            temp = BCM53280_EAP_PORT_BLOCK_INGRESS_EGRESS;
        }
...
int
drv_bcm53280_security_egress_get(int unit, int port, int *enable)
{
    int  rv = SOC_E_NONE;
    uint64  reg_value64, temp64;
    uint32  temp, temp_mask, temp_lo, temp_hi;
    ...
    if (temp == BCM53280_EAP_PORT_BLOCK_INGRESS_EGRESS) {
        *enable = 0;
    } else {
        *enable = 1;
    }
...
/*
 *  Function : drv_bcm53280_security_eap_control_set
 *
 *  Purpose :
 ...
 */
int
drv_bcm53280_security_eap_control_set(int unit, uint32 type, uint32 value)
{
 ...
 */
int
drv_bcm53280_security_eap_control_get(int unit, uint32 type, uint32 *value)
{
    uint32 reg_value32 = 0, temp = 0;
