/*
 * $Id: mem.c 1.54.2.8 Broadcom SDK $
...
/* Local defintion for BCM53280 memory control */
/* Memory index */
...
static drv_mem_info_t drv_TB_mem_info_B0[]={
        {DRV_MEM_CFP_METER, INDEX(CFP_RATE_METERm), CFP_METER_TABLE, -1}
};


#ifdef BCM_VO_SUPPORT
static drv_mem_info_t drv_VO_mem_info[]={
        ...
        l2_arl_sw_entry_t *sw_arl, l2_arl_entry_t *hw_arl)
{
    ...
    /* set MAC */
    rv = soc_L2_ARL_SWm_field_get(unit, (uint32 *)sw_arl, 
        MACADDRf, (uint32 *)&full_mac);
    ...
    rv = soc_L2_ARLm_field_set(unit, (uint32 *)hw_arl, 
        MACADDRf, (uint32 *)&short_mac);
    SOC_IF_ERROR_RETURN(rv);

    /* set vport/port or MGID */
    if (is_marl){
        rv = soc_L2_MARL_SWm_field_get(unit, (uint32 *)sw_arl, 
            MGIDf, &temp);
        SOC_IF_ERROR_RETURN(rv);        
        rv = soc_L2_MARLm_field_set(unit, (uint32 *)hw_arl, 
            MGIDf, &temp);
        SOC_IF_ERROR_RETURN(rv);        
...
/*
 *  Function : _drv_bcm53280_search_key_op
 *
 *  Purpose :
 ...
 */
int
_drv_bcm53280_search_key_op(int unit, uint32 *sw_arl, uint32 int_flag,
        uint32 *result_entry, int  *arl_index)
{
    ...
    if (int_flag == _TB_ARL_SEARCH_CONFLICT){
        for (i = 0; i < ROBO_53280_L2_BUCKET_SIZE; i++){
...
/*
 *  Function : _drv_bcm53280_search_valid_op
 *
 *  Purpose :
 ...
 */
int
_drv_bcm53280_search_valid_op(int unit, uint32 *key, uint32 *entry, 
 ...
 */
static int
_drv_bcm53280_mem_table_reset(int unit, uint32 mem)
{
    int rv = SOC_E_NONE;
    uint32 retry;
 ...
 */
int 
_drv_tb_arl_hash_index_get(int unit, int is_crc16, uint32 *sw_arl, int *index)
{
    int         rv = SOC_E_NONE;
...
/*
 *  Function : drv_bcm53280_mem_clear
 *
 *  Purpose :
 ...
 */
int 
drv_bcm53280_mem_clear(int unit, uint32 mem)
{
    int rv = SOC_E_NONE;
    ...
    switch(mem) {
        ...
        */
        ...
        case DRV_MEM_CFP_METER:
        case DRV_MEM_CFP_STAT:            
            rv = _drv_bcm53280_mem_table_reset(unit, mem);
            break;
...
/*
 *  Function : drv_bcm53280_mem_delete
 *
 *  Purpose :
 ...
 */
int
drv_bcm53280_mem_delete(int unit, uint32 mem, uint32 *entry, uint32 flags)
{
    ...
    switch(mem) {
        ...
        case DRV_MEM_MARL:
            if (flags & DRV_MEM_OP_DELETE_ALL_ARL){
                ...
                rv = _drv_bcm53280_mem_table_reset(unit, DRV_MEM_ARL);                
                return rv;
            }            
            /* no break here */
            /* break;*/ 
    ...
    if (fastaging_flags) {
        ...
        rv = _drv_arl_database_delete_by_portvlan(unit, src_port,
                vlanid, flags);     
        return rv;
    } else {
        /* -- Normal ARL deletion -- 
         ...
         */
        rv = _drv_bcm53280_search_key_op(unit, entry, _TB_ARL_SEARCH_MATCH,
 ...
 */
int
_tb_mem_mapping(int unit, uint32 mem, uint32 field_index, int *map_mem_id, int *map_field_id)
{
    int i;
    int mem_id, field_id;
    ...
    if (!SOC_IS_TB_AX(unit)){
        for (i = 0; i < COUNTOF(drv_TB_mem_info_B0); i++) {
            if (drv_TB_mem_info_B0[i].drv_mem == mem){
                mem_id = drv_TB_mem_info_B0[i].soc_mem;
                break;
            }    
        }
    }
...
#ifdef BCM_VO_SUPPORT
    if (SOC_IS_VO(unit)) {
        for (i = 0; i < COUNTOF(drv_VO_mem_info); i++) {
            if (drv_VO_mem_info[i].drv_mem == mem){
                mem_id = drv_VO_mem_info[i].soc_mem;
                break;
            }   
        }    
    }        
...
/*
 *  Function : drv_bcm53280_mem_field_get
 *
 *  Purpose :
 ...
 */
int
drv_bcm53280_mem_field_get(int unit, uint32 mem, 
    uint32 field_index, uint32 *entry, uint32 *fld_data)
{
    ...
    if ((mem == MSPT_TABm) ||(mem == DRV_MEM_MSTP)){
    ...
    if (mem < DRV_MEM_ARL){
        ...
        if (!SOC_IS_TB_AX(unit)){
            for (i = 0; i < COUNTOF(drv_TB_mem_info_B0); i++) {
                if ((drv_TB_mem_info_B0[i].drv_mem == mem) || 
                    (drv_TB_mem_info_B0[i].soc_mem == mem)) {
                    mem_id = mem;
                    field_id = field_index;
                    break;
                }    
        ...
        rv = _tb_mem_mapping(unit, mem, field_index, &mem_id, &field_id);
        SOC_IF_ERROR_RETURN(rv);
    }    
...
/*
 *  Function : drv_bcm53280_mem_field_set
 *
 *  Purpose :
 ...
 */
int
drv_bcm53280_mem_field_set(int unit, uint32 mem, 
    uint32 field_index, uint32 *entry, uint32 *fld_data)
{
    soc_mem_info_t      *meminfo;
    soc_field_info_t    *fieldinfo;
...
/*
 *  Function : drv_bcm53280_mem_length_get
 *
 *  Purpose :
 ...
 */
int 
drv_bcm53280_mem_length_get(int unit, uint32 mem, uint32 *data)
{
    int mem_id;
    soc_mem_info_t *meminfo;
    int rv;

    rv = _tb_mem_mapping(unit, mem, -1, &mem_id, NULL);
    SOC_IF_ERROR_RETURN(rv);
...
/*
 *  Function : drv_bcm53280_mem_read
 *
 *  Purpose :
 ...
 */
int
drv_bcm53280_mem_read(int unit, uint32 mem, 
    uint32 entry_id, uint32 count, uint32 *entry)
{
    int rv = SOC_E_NONE;
    int i, j, k;
        ...
        if (!SOC_IS_TB_AX(unit)){
            for (i = 0; i < COUNTOF(drv_TB_mem_info_B0); i++) {
                if ((drv_TB_mem_info_B0[i].drv_mem == mem) 
                    || (drv_TB_mem_info_B0[i].soc_mem == mem)) {
                    mem_id = drv_TB_mem_info_B0[i].soc_mem;
                    table_index = drv_TB_mem_info_B0[i].table_index;
                    table_mask = drv_TB_mem_info_B0[i].table_valid_mask;
                    break;
                }    
            }
        }
        ...
        if (SOC_IS_VO(unit)) {
            for (i = 0; i < COUNTOF(drv_VO_mem_info); i++) {
                if ((drv_VO_mem_info[i].drv_mem == mem) || 
                    (drv_VO_mem_info[i].soc_mem == mem)) {
                    mem_id = drv_VO_mem_info[i].soc_mem;
                    table_index = drv_VO_mem_info[i].table_index;
                    table_mask = drv_VO_mem_info[i].table_valid_mask;
                    break;
                }    
            }
...
#ifdef BCM_VO_SUPPORT
        if (SOC_IS_VO(unit) && (field_change == 1)){
            rv = _vo_cfp_mem_hw2sw(unit, temp_entry, entry);
            if (rv < 0) {
                goto mem_read_exit;
            }            
        } 
...
/*
 *  Function : drv_bcm53280_mem_width_get
 *
 *  Purpose :
 *      Get the width of selected memory. 
 *      The value returned in data is width in bits.
 ...
 */
int 
drv_bcm53280_mem_width_get(int unit, uint32 mem, uint32 *data)
{
    int mem_id, memlen;
    int rv;
...
/*
 *  Function : drv_bcm53280_mem_write
 *
 *  Purpose :
 ...
 */
int
drv_bcm53280_mem_write(int unit, uint32 mem, 
    uint32 entry_id, uint32 count, uint32 *entry)
{
    int rv = SOC_E_NONE;
    uint32 retry, index_min, index_max;
...
/*
 *  Function : drv_bcm53280_mem_search
 *
 *  Purpose :
 ...
 */
int 
drv_bcm53280_mem_search(int unit, uint32 mem, 
    uint32 *key, uint32 *entry, uint32 *entry_1, uint32 flags)
{
    soc_control_t   *soc = SOC_CONTROL(unit);
    int     rv = SOC_E_NONE;
    int     mem_id, index, cf_index[ROBO_53280_L2_BUCKET_SIZE];

    soc_cm_debug(DK_MEM|DK_TESTS, "drv_mem_search : mem=0x%x, flags = 0x%x\n",
         mem, flags);
    switch(mem) {
    case DRV_MEM_ARL:
    case DRV_MEM_MARL:
    ...
    if ((flags & DRV_MEM_OP_SEARCH_DONE) ||
         ...
         */
        rv = _drv_bcm53280_search_valid_op(unit, key, entry, entry_1, flags);
    ...
    /* TB have no feature on searching OP by MAC key only without VID key */
        ...
        if (flags & DRV_MEM_OP_SEARCH_CONFLICT){
             ...
             *      hashed buckets.
             */
            rv = _drv_bcm53280_search_key_op(unit, key, 
             ...
             *      call _drv_bcm53280_index_read_op() 
             */
            rv = _drv_bcm53280_search_key_op(unit, key, 
...
/*
 *  Function : drv_bcm53280_mem_insert
 *
 *  Purpose :
 ...
 */
int 
drv_bcm53280_mem_insert(int unit, uint32 mem, uint32 *entry, 
        uint32 flags)
{
    soc_control_t       *soc = SOC_CONTROL(unit);
    l2_arl_sw_entry_t   temp_sw_arl;
    ...
    uint8       temp_mac[6];
    uint32      mem_id = 0, reg_val = 0, temp = 0;
    uint32      temp_hi = 0, temp_lo = 0, vid_key = 0;
    ...
    SOC_IF_ERROR_RETURN(soc_L2_ARL_SWm_field_get(unit, 
            entry, MACADDRf, (uint32 *)&temp_mem_data));
    ...
    if (rv == SOC_E_FULL) {
        ...
        soc_cm_debug(DK_ARL,"%s,%d, entry existed already!!!\n",
                FUNCTION_NAME(), __LINE__);
        bin_id = table_index & _TB_ARL_INDEX_BIN_ID_MASK;
        if (flags & DRV_MEM_OP_MODIFY){
            
            soc_cm_debug(DK_ERR,"%s,%d, Remove after Checked!!\n",
                    FUNCTION_NAME(),__LINE__);
            rv = MEM_DATA_READ(unit, bin_id * 2, &temp_mem_data);
            if (rv){
                goto mem_insert_exit;
            }
            ...
            COMPILER_64_AND(temp64, temp_mem_data);
            COMPILER_64_AND(temp64, data64_port_mask);
            COMPILER_64_SHR(temp64, _TB_ARL_KEY_SEARCH_RESULT_SHIFT_PORT);
            ori_port = (uint32)(COMPILER_64_LO(temp64));

            /* check origian static */
            temp = COMPILER_64_BITTEST(temp_mem_data, 
                    _TB_ARL_KEY_SEARCH_RESULT_REGSHIFT_STATIC);
            ori_dynamic = (!temp) ? TRUE : FALSE;

            /* check pending/valid */
            rv = MEM_DATA_READ(unit, bin_id * 2 + 1, &temp_mem_data);
            if (rv){
                soc_cm_debug(DK_ERR,"%s,%d,SOC driver problem!\n",
                        FUNCTION_NAME(),__LINE__);
                goto mem_insert_exit;
            }
            temp = COMPILER_64_LO(temp_mem_data) & 
                    _TB_ARL_KEY_SEARCH_RESULT_REGMASK_VALID;
            assert(temp == _TB_ARL_STATUS_VALID || 
                    temp == _TB_ARL_STATUS_PENDING);
            /* Modify + Pending */
            if (flags & DRV_MEM_OP_PENDING){
                /* this case is used to support L2 REPLACE BCM API with 
                 *  replace pending entry request >> BCM_L2_REPLACE_PENDING.
                 *  
                 *  P.S. BCM_L2_REPLACE_PENDING : replace the pending entry.
                 */
                soc_cm_debug(DK_ERR,"%s,%d, Remove after Checked!!\n",
                        FUNCTION_NAME(),__LINE__);
                if (temp == _TB_ARL_STATUS_VALID){
                    ...
                    rv = SOC_E_NOT_FOUND;
                    goto mem_insert_exit;
                } else if (temp == _TB_ARL_STATUS_PENDING){
                    /* remove pending flag to force the inserted entry status
                     *  will be "VALID" for this case.
                     */
            ...
            uint32  t_port = 0, t_vport = 0, t_static = 0;
            uint32  in_port = 0, in_vport = 0, in_static = 0;
            int     changed = 0;
            
            /* port, vport, static are allowed to be re-insert.  
             *  - AT_l2_42 designed for such case.
             */
            rv = soc_L2_ARL_SWm_field_get(unit, entry, STATICf, &in_static);
            if (SOC_FAILURE(rv)){
                goto mem_insert_exit;
            }

            rv = soc_L2_ARL_SWm_field_get(unit, entry, PORTIDf, &in_port);
            if (SOC_FAILURE(rv)){
                goto mem_insert_exit;
            }

            rv = soc_L2_ARL_SWm_field_get(unit, entry, V_PORTIDf, &in_vport);
            if (SOC_FAILURE(rv)){
                goto mem_insert_exit;
            }
            ...
            ori_dynamic = (!t_static) ? TRUE : FALSE;
            ...
            changed = (in_static ^ t_static) | (in_port ^ t_port) |
                    (in_vport ^ t_vport);
            
            if (!changed){            
                bin_id = -1;
                /* Return SOC_E_NONE instead of SOC_E_EXISTS for DV test. */
                rv = SOC_E_NONE;
                goto mem_insert_exit;
            }
        }
    ...
    if (bin_id == -1){
        ...
        /* age :
         *  - to keep the uper layer API with chip independent design, the 
         *      arl age value is reasigned here through flags definition. 
         *      (So far, all ROBO device before TB have no such feature.
         ...
         */
        temp = (flags & DRV_MEM_OP_HIT) ? _TB_ARL_AGE_HIT_VAL : 
                _TB_ARL_AGE_DEF_VAL;
        rv = soc_L2_ARL_SWm_field_set(unit, (uint32 *)&temp_sw_arl, 
            AGEf, &temp);
        if (SOC_FAILURE(rv)){
...
/*
 *  Function : drv_bcm53280_mem_move
 *
 *  Purpose :
 ...
 */
 int
drv_bcm53280_mem_move(int unit, uint32 mem,int src_index, 
    int dest_index, int count, int flag)
{
    ...
    val32_lo = count;
    COMPILER_64_SET(temp_data, val32_hi, val32_lo);
    soc_MEM_KEY_0r_field_set(unit, (uint32 *)&reg64, MEM_KEYf, (uint32 *)&temp_data);
...
/*
 *  Function : drv_bcm53280_mem_fill
 *
 *  Purpose :
 ...
 */
int
drv_bcm53280_mem_fill(int unit, uint32 mem, 
    uint32 entry_id, uint32 count, uint32 *entry)
{
    int rv = SOC_E_NONE;
 ...
 */
int 
_drv_bcm53280_mem_sa_lrncnt_control_set(int unit, uint32 entry_id, 
    ...
    uint32 entry_id, uint32 count, uint32 *entry)
{
    ...
    char *s;
    uint32 val32_hi, val32_lo, read_entry_id;
    int k=0; 
    int write_data =0;
    ...
    if (field_change == 1) {
        ...
        rv = _vo_cfp_mem_sw2hw(unit, entry, temp_entry);
        if (SOC_FAILURE(rv)) {
            sal_free(temp_entry);
            return rv;
        }
    ...
    if( (s != NULL) && (sal_strcmp(s, "bcm53280_fpga") == 0)) {
        if ((TABLE(table_index) == CFP_DATA_TABLE)||
            ...
            (table_index == CFP_METER_TABLE)||
            (table_index == CFP_STAT_TABLE)){
            read_entry_id = (entry_id/128)+entry_id%2;
        }
    }
