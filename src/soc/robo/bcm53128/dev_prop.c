/*
 * $Id: dev_prop.c 1.11 Broadcom SDK $
 *
...
int
_drv_bcm53128_station_move_drop_set(int unit, uint32 prop_val)
{
    int         rv = SOC_E_NONE;
 ...
 */
int 
drv_bcm53128_dev_prop_get(int unit, uint32 prop_type, uint32 *prop_val)
{
    ...
    switch (prop_type) {
        ...
        case DRV_DEV_PROP_LOW_POWER_SUPPORT_PBMP:
            if (SOC_IS_ROBO53128V(unit)) {
                *prop_val = DRV_MAC_LOW_POWER_SUPPORT_PBMP & 0x7f;
            } else {
 ...
 */
int 
drv_bcm53128_dev_prop_set(int unit, uint32 prop_type, uint32 prop_val)
{
    int         rv = SOC_E_NONE;
    uint32 reg_val = 0, temp = 0;
    soc_pbmp_t  pbmp;
    ...
    switch (prop_type) {
        case DRV_DEV_PROP_SA_STATION_MOVE_DROP:
            rv = _drv_bcm53128_station_move_drop_set(unit, prop_val);
            break;
        case DRV_DEV_PROP_EEE_GLOBAL_CONG_THRESH:
            if (prop_val > 512) {
                return SOC_E_CONFIG;
        ...
        case DRV_DEV_PROP_EEE_INIT:
            ...
            /* per device */
            SOC_IF_ERROR_RETURN(
                drv_bcm53128_dev_prop_set(unit, 
