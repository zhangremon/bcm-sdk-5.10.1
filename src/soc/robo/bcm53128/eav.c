...
#define EAV_BCM53128_MAX_TICK_INC    63
#define EAV_BCM53128_MAX_TICK_ADJUST_PERIOD    15
#define EAV_BCM53128_MAX_SLOT_ADJUST_PERIOD    15
#define EAV_BCM53128_MAX_TICK_ONE_SLOT    3126
#define EAV_BCM53128_MAX_SLOT_NUMBER    31

#define EAV_BCM53128_MAX_PCP_VALUE    0x7

/* Bytes count allowed for EAV Class4/Class5 bandwidth within a slot time */
#define EAV_53128_MAX_BANDWIDTH_VALUE 16383
#define EAV_53128_MIN_BANDWIDTH_VALUE 0

static int
_drv_bcm53128_eav_mmu_init(int unit)
{
    uint32 reg_val;
...
int 
drv_bcm53128_eav_control_set(int unit, uint32 type, uint32 param)
{
    int rv = SOC_E_NONE;
    ...
    switch (type) {
        ...
        case DRV_EAV_CONTROL_STREAM_CLASSA_PCP:
            ...
            if (param > EAV_BCM53128_MAX_PCP_VALUE) {
                return SOC_E_PARAM;
            }
        ...
        case DRV_EAV_CONTROL_MMU_INIT:
            rv = _drv_bcm53128_eav_mmu_init(unit);
            break;
        default:
            rv = SOC_E_PARAM;
    }
...
int 
drv_bcm53128_eav_control_get(int unit, uint32 type, uint32 *param)
{
    int rv = SOC_E_NONE;
...
int 
drv_bcm53128_eav_enable_set(int unit, uint32 port, uint32 enable)
{
    uint32 reg_value, temp = 0;
...
int 
drv_bcm53128_eav_enable_get(int unit, uint32 port, uint32 *enable)
{
    uint32 reg_value, temp = 0;
...
int 
drv_bcm53128_eav_link_status_set(int unit, uint32 port, uint32 link)
{
    uint32 reg_value, temp = 0;
...
int 
drv_bcm53128_eav_link_status_get(int unit, uint32 port, uint32 *link)
{
    uint32 reg_value, temp = 0;
...
int
drv_bcm53128_eav_egress_timestamp_get(int unit, uint32 port,
    uint32 *timestamp)
{
    uint32 reg_value, temp = 0;
...
int 
drv_bcm53128_eav_time_sync_set(int unit, uint32 type, uint32 p0, uint32 p1)
{
    int rv = SOC_E_NONE;
    uint32 reg_value, temp;
    ...
    switch (type) {
        ...
        case DRV_EAV_TIME_SYNC_TIME_ADJUST:
            ...
            if ((p0 > EAV_BCM53128_MAX_TICK_INC) || 
                (p1 > EAV_BCM53128_MAX_TICK_ADJUST_PERIOD)) {
                return SOC_E_PARAM;
            }
            temp = p0;
        ...
        case DRV_EAV_TIME_SYNC_TICK_COUNTER:
            ...
            if (p0 > EAV_BCM53128_MAX_TICK_ONE_SLOT) {
                return SOC_E_PARAM;
            }
            temp = p0;
        ...
        case DRV_EAV_TIME_SYNC_SLOT_NUMBER:
            SOC_IF_ERROR_RETURN(
                REG_READ_AVB_SLOT_TICK_CNTRr(unit, &reg_value));
            if (p0 > EAV_BCM53128_MAX_SLOT_NUMBER) {
                return SOC_E_PARAM;
            }
            temp = p0;
            soc_AVB_SLOT_TICK_CNTRr_field_set(
...
int 
drv_bcm53128_eav_time_sync_get(int unit, uint32 type, uint32 *p0, uint32 *p1)
{
    int rv = SOC_E_NONE;
    uint32 reg_value, temp = 0;
...
int 
drv_bcm53128_eav_queue_control_set(int unit, 
    uint32 port, uint32 type, uint32 param)
{
    ...
    switch (type) {
        case DRV_EAV_QUEUE_Q4_BANDWIDTH:
            ...
            if (param > EAV_53128_MAX_BANDWIDTH_VALUE) {
                soc_cm_debug(DK_ERR, 
                    "drv_bcm53128_eav_queue_control_set : BW unsupported. \n");
                return  SOC_E_PARAM;
            }
            SOC_IF_ERROR_RETURN(
...
int 
drv_bcm53128_eav_queue_control_get(int unit, 
    uint32 port, uint32 type, uint32 *param)
{
    int rv = SOC_E_NONE;
...
int 
drv_bcm53128_eav_time_sync_mac_set(int unit, uint8* mac, uint16 ethertype)
{
    uint32 reg_val;
...
int 
drv_bcm53128_eav_time_sync_mac_get(int unit, uint8* mac, uint16 *ethertype)
{
    uint32 reg_val;
    uint64 reg_val64, mac_field;
    uint32 temp = 0;
