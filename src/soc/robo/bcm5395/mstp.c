/*
 * $Id: mstp.c 1.7.120.1 Broadcom SDK $
 *
...
int bcm5395_mst_field[] = { 
                    INDEX(SPT_STA0f), INDEX(SPT_STA1f), INDEX(SPT_STA2f), 
                    INDEX(SPT_STA3f), INDEX(SPT_STA4f), INDEX(SPT_STA5f), 
                    INDEX(SPT_STA6f), INDEX(SPT_STA7f), INDEX(SPT_STA8f) };
/*
 *  Function : drv_bcm5395_mstp_port_set
 *
 *  Purpose :
 ...
 */
int 
drv_bcm5395_mstp_port_set(int unit, uint32 mstp_gid, 
                           uint32 port, uint32 port_state)
{
    ...
    if (!soc_feature(unit, soc_feature_mstp)){
        ...
        (DRV_SERVICES(unit)->reg_field_set)
            (unit, reg_index, (uint32 *)&reg_value, 
            bcm5395_mst_field[port], &temp);  
...
/*
 *  Function : drv_bcm5395_mstp_port_get
 *
 *  Purpose :
 ...
 */
int 
drv_bcm5395_mstp_port_get(int unit, uint32 mstp_gid, 
                                uint32 port, uint32 *port_state)
{
    int rv = SOC_E_NONE;
    uint32  portstate;
    ...
    if (!soc_feature(unit, soc_feature_mstp)){
      ...
      "drv_mstp_port_get : unit %d, STP id = %d, port = %d, port_state = %d \n",
        ...
        (DRV_SERVICES(unit)->reg_field_get)
            (unit, reg_index, (uint32 *)&reg_value, 
            bcm5395_mst_field[port], &portstate); 
