...
#define SUPPRESSION_5395_SALF_MASK   0x40
#define SUPPRESSION_5395_DLF_MASK    0x20
#define SUPPRESSION_5395_MCAST_DLF  0x10
#define SUPPRESSION_5395_BROADCAST  0x8
#define SUPPRESSION_5395_REV_MCAST   0x4
#define SUPPRESSION_5395_MULTICAST_HIT  0x2
#define SUPPRESSION_5395_UNICAST_HIT    0x1
    ...
    uint32 type, uint32 enable)
{
    ...
    temp = enable;
    if (rev_mcast_fld_index) {
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, PORT_IRC_CONr, &reg_value, rev_mcast_fld_index, &temp));
    }
    if (bcast_fld_index) {
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
 ...
 */
int 
_drv_bcm5395_storm_control_type_enable_get(int unit, uint32 port, uint32 *type)
{
    uint32	reg_addr, reg_len, temp;
    uint32  reg_value;
    int		rv= SOC_E_NONE;
    ...
    *type = 0;
    ...
    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
        (unit, PORT_IRC_CONr, &reg_value, EN_RSV_MLTCST_SUPf, &temp));
    if (temp) {
...
/*
 *  Function : drv_bcm5395_storm_control_enable_set
 *
 *  Purpose :
 ...
 */
int 
drv_bcm5395_storm_control_enable_set(int unit, uint32 port, uint8 enable)
{
    uint32  reg_addr, reg_len, temp;
    uint32  reg_value;
    int		rv= SOC_E_NONE;
...
/*
 *  Function : drv_bcm5395_storm_control_enable_get
 *
 *  Purpose :
 ...
 */
int 
drv_bcm5395_storm_control_enable_get(int unit, uint32 port, uint8 *enable)
{
    uint32	reg_addr, reg_len, temp;
    uint32  reg_value;
    int		rv= SOC_E_NONE;
...
/*
 *  Function : drv_bcm5395_storm_control_set
 *
 *  Purpose :
 ...
 */
int 
drv_bcm5395_storm_control_set(int unit, soc_pbmp_t bmp, uint32 type, 
    uint32 limit, uint32 burst_size)
{
    ...
    PBMP_ITER(bmp, port) {
        if (limit == 0) { /* Disable storm suppression type */
            SOC_IF_ERROR_RETURN(
                _drv_bcm5395_storm_control_type_enable_set(unit, port, type, FALSE));
            ...
            SOC_IF_ERROR_RETURN(
                _drv_bcm5395_storm_control_type_enable_set(unit, port, type, TRUE));
            if (disable_type){
                SOC_IF_ERROR_RETURN(
                    _drv_bcm5395_storm_control_type_enable_set(
                                    unit, port, 
                                    disable_type, FALSE));
            }
 ...
 */
int 
drv_bcm5395_storm_control_get(int unit, uint32 port, uint32 *type, 
    uint32 *limit, uint32 *burst_size)
{
    uint32		reg_addr, reg_value, reg_len, temp;
    int			rv= SOC_E_NONE;
    ...
    SOC_IF_ERROR_RETURN(
        _drv_bcm5395_storm_control_type_enable_get(unit, port, type));
    if (!(*type)) {
        *limit = 0;
    } else {
        reg_addr = (DRV_SERVICES(unit)->reg_addr)
