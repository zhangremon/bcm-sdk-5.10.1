/*
 * $Id: mirror.c 1.4.122.2 Broadcom SDK $
 *
 ...
 */
int 
drv_bcm53242_mirror_set(int unit, uint32 enable, soc_pbmp_t mport_bmp, 
    soc_pbmp_t ingress_bmp, soc_pbmp_t egress_bmp)
{
    ...
    soc_cm_debug(DK_PORT, 
        "drv_mirror_set : unit %d, %sable, mport_bmp = 0x%x, \
    ...
    if ((rv = REG_READ_IGMIRCTLr(unit, (uint32 *)&reg_value)) < 0) {
        return rv;
    }
    /* write ingress mirror mask */
    temp32 = SOC_PBMP_WORD_GET(ingress_bmp, 0);
    soc_IGMIRCTLr_field_set(unit, (uint32 *)&reg_value,
        IN_MIR_MSKf, &temp32);

    if ((rv = REG_WRITE_IGMIRCTLr(unit, (uint32 *)&reg_value)) < 0) {
        return rv;
    }
    /* check egress mirror */
    if ((rv = REG_READ_EGMIRCTLr(unit, (uint32 *)&reg_value)) < 0) {
        return rv;
    }
    /* write egress mirror mask */
    temp32 = SOC_PBMP_WORD_GET(egress_bmp, 0);
    soc_EGMIRCTLr_field_set(unit, (uint32 *)&reg_value,
        OUT_MIR_MSKf, &temp32);

    if ((rv = REG_WRITE_EGMIRCTLr(unit, (uint32 *)&reg_value)) < 0) {
        return rv;
    }

    /* check mirror control */
    if ((rv = REG_READ_MIRCAPCTLr(unit, (uint32 *)&reg_value)) < 0) {
        return rv;
    }

    if (enable) { /* enable mirror */
        temp32 = 1;
        soc_MIRCAPCTLr_field_set(unit, (uint32 *)&reg_value,
            MIR_ENf, &temp32);
    } else { /* disable mirror */
        temp32 = 0;
        soc_MIRCAPCTLr_field_set(unit, (uint32 *)&reg_value,
    ...
    soc_MIRCAPCTLr_field_set(unit, (uint32 *)&reg_value,
            SMIR_CAP_PORTf, &temp32);

    if ((rv = REG_WRITE_MIRCAPCTLr(unit, (uint32 *)&reg_value)) < 0) {
        return rv;
    }
 ...
 */
int 
drv_bcm53242_mirror_get(int unit, uint32 *enable, soc_pbmp_t *mport_bmp, 
    soc_pbmp_t *ingress_bmp, soc_pbmp_t *egress_bmp)
{
    uint64      reg_value;
    int         rv= SOC_E_NONE;
    ...
    soc_IGMIRCTLr_field_get(unit, (uint32 *)&reg_value,
        IN_MIR_MSKf, &temp32);
    ...
    soc_EGMIRCTLr_field_get(unit, (uint32 *)&reg_value,
        OUT_MIR_MSKf, &temp32);
    ...
    /* enable value */
    temp32 = 0;
    soc_MIRCAPCTLr_field_get(unit, (uint32 *)&reg_value,
        MIR_ENf, &temp32);
    if (temp32) {
        /* enable value */
        *enable = TRUE;
    ...
    soc_cm_debug(DK_PORT, 
        "drv_mirror_get : unit %d, %sable, mport_bmp = 0x%x, \
        ingress_bmp = 0x%x, egress_bmp = 0x%x\n",
        unit, *enable ? "en" : "dis", \
        SOC_PBMP_WORD_GET(*mport_bmp, 0), \
        SOC_PBMP_WORD_GET(*ingress_bmp, 0), \
