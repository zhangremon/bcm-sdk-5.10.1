/*
 * $Id: sec.c 1.6.244.2 Broadcom SDK $
 *
...
#define BCM53242_EAP_PORT_BLOCK_NONE    0
#define BCM53242_EAP_PORT_BLOCK_INGRESS    1
#define BCM53242_EAP_PORT_BLOCK_INGRESS_EGRESS    2


int
_drv_bcm53242_security_init(int unit)
{
...
/*
 *  Function : drv_bcm53242_security_set
 *
 *  Purpose :
 ...
 */
int 
drv_bcm53242_security_set(int unit, soc_pbmp_t bmp, uint32 state, uint32 mask)
{
    int             rv = SOC_E_NONE;
    ...
    switch (state) {
        case DRV_SECURITY_PORT_UNCONTROLLED:
            ...
            */                
            ...
            PBMP_ITER(bmp, port) {
                ...
                /* port is not blocked */
                temp = BCM53242_EAP_PORT_BLOCK_NONE;
        ...
        case DRV_SECURITY_PORT_UNAUTHENTICATE:
            ...
            */
            SOC_IF_ERROR_RETURN(_drv_bcm53242_security_init(unit));
            ...
            PBMP_ITER(bmp, port) {
                ...
                temp = (mask & DRV_SECURITY_BLOCK_INOUT) ? 
                            BCM53242_EAP_PORT_BLOCK_INGRESS_EGRESS : 
                            BCM53242_EAP_PORT_BLOCK_INGRESS;
                soc_PORT_SEC_CONr_field_set(unit, &reg_value, 
                    EAP_PORT_BLKf, &temp);
                if ((rv = REG_WRITE_PORT_SEC_CONr(
                    unit, port, &reg_value)) < 0) {
...
/*
 *  Function : drv_bcm53242_security_get
 *
 *  Purpose :
 ...
 */
int 
drv_bcm53242_security_get(int unit, uint32 port, uint32 *state, uint32 *mask)
{
    uint32          reg_value, temp;
    int             rv = SOC_E_NONE;
...
int
drv_bcm53242_security_egress_set(int unit, soc_pbmp_t bmp, int enable)
{
    int             rv = SOC_E_NONE;
    uint32          reg_value, temp;
    int             port;
    ...
    PBMP_ITER(bmp, port) {
        ...
        if (enable) {
             ...
             */
            temp = BCM53242_EAP_PORT_BLOCK_INGRESS;
        } else {
            temp = BCM53242_EAP_PORT_BLOCK_INGRESS_EGRESS;
        }
        soc_PORT_SEC_CONr_field_set(unit, &reg_value, 
            EAP_PORT_BLKf, &temp);
        if ((rv = REG_WRITE_PORT_SEC_CONr(
...
int
drv_bcm53242_security_egress_get(int unit, int port, int *enable)
{
    int             rv = SOC_E_NONE;
    ...
    if (temp == BCM53242_EAP_PORT_BLOCK_INGRESS_EGRESS) {
        *enable = 0;
    } else {
        *enable = 1;
    }
