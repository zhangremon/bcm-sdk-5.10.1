/*
 * $Id: cfp.c 1.49.20.3 Broadcom SDK $
 *
 ...
 */
int
_drv53242_cfp_field_mapping(int unit, uint32 mem_type, uint32 field_type, uint32 *field_id)
{
    int i, rv = SOC_E_NONE;
...
_drv53242_cfp_read(int unit, uint32 ram_type, uint32 index, drv_cfp_entry_t *entry)
{
    ...
    case DRV_CFP_RAM_TCAM:
        ...
        break;
    case DRV_CFP_RAM_ACT:
        if ((rv = REG_READ_ACT_POL_DATA0r(unit, (uint32 *)&reg_val64)) < 0) {
            goto cfp_read_exit;
        }
            ...
            drv_cfp_entry_t* entry, uint32* fld_val)
{
    ...
    if (( rv = _drv53242_cfp_field_mapping(unit, mem_type, field_type, &fld_id)) < 0) {
        soc_cm_debug(DK_ERR,
