/*
 * $Id: mem.c 1.50.14.6 Broadcom SDK $
                ...
                uint64 *data0, uint64 *data1)
{
    ...
    SOC_IF_ERROR_RETURN(
        REG_WRITE_ARLA_MACr(unit, (uint32 *)&reg_val64));
   ...
   /* always set arla_vid to zero to prevent the improper MAC2VLAN table
    ...
    SOC_IF_ERROR_RETURN(
        REG_WRITE_ARLA_VIDr(unit, &reg_val32));
...
_drv_bcm53242_mem_marl_delete_all(int unit)
{
    ...
    index_max = SOC_MEM_BASE(unit, INDEX(L2_ARLm)) + 
        SOC_MEM_SIZE(unit, INDEX(L2_ARLm));
    ...
    uint32 entry_id, uint32 count, uint32 *entry)
{
    ...
    for (i = 0;i < count; i++ ) {
        ...
        if ((rv = REG_READ_OTHER_TABLE_DATA0r(unit, mem_ptr)) < 0) {
            goto mem_read_exit;
        }
        if ((rv = REG_READ_OTHER_TABLE_DATA1r(unit, mem_ptr+8)) < 0) {
            goto mem_read_exit;
        }
        if ((rv = REG_READ_OTHER_TABLE_DATA2r(unit, mem_ptr+16)) < 0) {
            goto mem_read_exit;
        }
#ifdef BE_HOST
    temp_entry = (uint32 *)temp_data;
    if (mem_id == INDEX(L2_ARLm) ||mem_id == INDEX(L2_MARL_SWm) || 
        mem_id == INDEX(L2_ARL_SWm)) {
        val32 = temp_entry[0];
        temp_entry[0] = temp_entry [2];
        temp_entry [2] = val32;
    } else if (mem_id == INDEX(VLAN_1Qm) || mem_id == INDEX(MARL_PBMPm)){
        for (i = 0; i < 3; i++) {
            val32 = temp_entry[i];
    ...
    } else if (mem_id == INDEX(VLAN2VLANm) || mem_id == INDEX(PROTOCOL2VLANm) || 
                mem_id == INDEX(FLOW2VLANm) || mem_id == INDEX(MAC2VLANm)) {
        val32 = temp_entry[0];
    ...
    uint32 entry_id, uint32 count, uint32 *entry)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        /* write data */
        mem_ptr = (uint8 *)gmem_entry;
        if ((rv = REG_WRITE_OTHER_TABLE_DATA0r(unit, mem_ptr)) < 0) {
            goto mem_write_exit;
        }
        if ((rv = REG_WRITE_OTHER_TABLE_DATA1r(unit, mem_ptr+8)) < 0) {
            goto mem_write_exit;
        }
        if ((rv = REG_WRITE_OTHER_TABLE_DATA2r(unit, mem_ptr+16)) < 0) {
            goto mem_write_exit;
        }
    ...
    uint32 field_index, uint32 *entry, uint32 *fld_data)
{
    ...
    if (mem_id == INDEX(L2_MARL_SWm) || mem_id == INDEX(L2_ARL_SWm)) {
        ...
        entry[0] = entry [2];
        entry [2] = val32;
    } else if (mem_id == INDEX(VLAN_1Qm) || mem_id == INDEX(MSPT_TABm) || 
        mem_id == INDEX(MARL_PBMPm)){
        for (i = 0; i < 3; i++) {
           val32 = entry[i];
           entry[i] = entry[5-i];
    ...
    } else if (mem_id == INDEX(MAC2VLANm) || mem_id == INDEX(L2_ARLm)) {
        val32 = entry[0];
        entry[0] = entry [1];
        entry [1] = val32;
    }
