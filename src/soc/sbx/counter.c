/* 
 * $Id: counter.c 1.28.2.1 Broadcom SDK $
...
_soc_sbx_counter_bset_collect_autozero(int unit, int discard, int block, int set)
{
    ...
    for (counter = 0; counter < num_counters; counter++) {
        ...
        if(val_new != val_prev) soc_cm_debug(DK_VERBOSE + DK_COUNTER, "cntr %d/%d/%d val_new %llx %llx. %x... ",
                set, counter, num_counters, val_new, val_prev, (uint32)data);
        ...
        if (discard) {
            ...
            COUNTER_ATOMIC_END(s);
            soc_cm_debug(DK_COUNTER, "discard");
            continue;
        }
                     ...
                     int block_count)
{
    ...
    if (COUNTER_CONTROL(unit) == (uint32 *) soc->counter_buf64) {
        COUNTER_CONTROL(unit) = sal_alloc(sizeof(_counter_block_t),
                                          "soc_counter_control");
        ...
        /*
         * Stop counter thread, if running, and
         * remove any previous counter sets
         */
        rv = soc_sbx_counter_stop(unit);
        if (SOC_SUCCESS(rv)) {
            rv = _soc_sbx_counter_bset_remove_all(unit);
        }
        if (SOC_FAILURE(rv)) {
            COUNTER_UNLOCK(unit);
            return rv;
    ...
    soc_cm_debug(DK_VERBOSE, "soc_counter_init: unit=%d rv=%d\n",
                 unit, rv);
...
soc_sbx_counter_collect64(int unit, int discard)
{
    ...
    soc_port_t          port = 0;
    soc_reg_t           ctr_reg = 0;
    uint64              ctr_new = 0, ctr_prev = 0, ctr_diff = 0;
    int                 index = 0;
...
_soc_sbx_counter_thread(void *unit_vp)
{
    ...
    soc_cm_debug(DK_COUNTER, "soc_sbx_counter_thread: unit=%d\n", unit);
	...
	if (SOC_IS_SBX_SIRIUS(unit)) {
        ...
        if (err) {
            if (++err_count == soc_property_get(unit,
                ...
                soc_cm_debug(DK_ERR, "soc_sbx_counter_thread: Too many errors\n");
                rv = SOC_E_INTERNAL;
                goto done;
            }
        } else if (err_count > 0) {
    ...
    soc_cm_debug(DK_COUNTER, "soc_sbx_counter_thread: exiting\n");
...
soc_sbx_counter_start(int unit, uint32 flags, int interval, pbmp_t pbmp)
{
    ...
    if ((soc->counter_trigger == NULL) || (soc->counter_intr == NULL)) {
        soc_cm_debug(DK_ERR, "soc_sbx_counter_start: sem create failed\n");
        COUNTER_UNLOCK(unit);
    ...
    if (interval != 0) {
        ...
        if (soc->counter_pid == SAL_THREAD_ERROR) {
            soc->counter_interval = 0;
            soc_cm_debug(DK_ERR, "soc_sbx_counter_start: thread create failed\n");
            rv = SOC_E_INTERNAL;
        } else {
            soc_cm_debug(DK_COUNTER, "soc_sbx_counter_start: complete\n");
        }
    }
...
soc_sbx_counter_status(int unit, uint32 *flags, int *interval, pbmp_t *pbmp)
{
    ...
    soc_cm_debug(DK_COUNTER, "soc_sbx_counter_status: unit=%d\n", unit);
...
soc_sbx_counter_stop(int unit)
{
    ...
    soc_cm_debug(DK_COUNTER, "soc_counter_stop: unit=%d rv=%d\n",
                 unit, rv);
    return rv;
}
    ...
    for (block = 0; block < COUNTER_NUM_BLOCKS(unit); block++) {
        ...
        for (set = 0; set < COUNTER_BLOCK_NUM_SETS(unit, block); set++) {
            ...
            for (counter = 0; counter < COUNTER_BSET_NUM_COUNTERS(unit, block);
                ...
                soc_cm_print("        Counter %d - 0x%08x%08x \n",
