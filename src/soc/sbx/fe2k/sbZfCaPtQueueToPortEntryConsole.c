/*
 * $Id: sbZfCaPtQueueToPortEntryConsole.c 1.1.48.3 Broadcom SDK $
...
#include "sbZfCaPtQueueToPortEntryConsole.hx"
...
sbZfCaPtQueueToPortEntry_Print(sbZfCaPtQueueToPortEntry_t *pFromStruct) {
  SB_LOG("CaPtQueueToPortEntry:: pbsel=0x%01x", (unsigned int)  pFromStruct->m_uPbSel);
  ...
  SB_LOG("CaPtQueueToPortEntry:: destinterface=0x%01x", (unsigned int)  pFromStruct->m_uDestInterface);
...
char *
sbZfCaPtQueueToPortEntry_SPrint(sbZfCaPtQueueToPortEntry_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtQueueToPortEntry:: pbsel=0x%01x", (unsigned int)  pFromStruct->m_uPbSel);
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtQueueToPortEntry:: destinterface=0x%01x", (unsigned int)  pFromStruct->m_uDestInterface);
