/*
 * $Id: sbZfCaPmTimerCheckerState.c 1.1.48.4 Broadcom SDK $
...
sbZfCaPmTimerCheckerState_Pack(sbZfCaPmTimerCheckerState_t *pFrom,
  ...
  int i;
  int size = SB_ZF_CAPMTIMERCHECKERSTATE_SIZE_IN_BYTES;
  ...
  return SB_ZF_CAPMTIMERCHECKERSTATE_SIZE_IN_BYTES;
}
...
sbZfCaPmTimerCheckerState_Unpack(sbZfCaPmTimerCheckerState_t *pToStruct,
  ...
  (pToStruct)->m_bTimerStarted =  (uint8_t)  ((pFromData)[6] >> 4) & 0x01;

  /* Unpack Member: m_uTimer */
  (pToStruct)->m_uTimer =  (uint32_t)  (pFromData)[4] ;
  (pToStruct)->m_uTimer |=  (uint32_t)  (pFromData)[5] << 8;
  (pToStruct)->m_uTimer |=  (uint32_t)  ((pFromData)[6] & 0x0f) << 16;
