/*
 * $Id: sbZfCaQmQueueConfigEntryConsole.c 1.1.48.3 Broadcom SDK $
...
sbZfCaQmQueueConfigEntry_Print(sbZfCaQmQueueConfigEntry_t *pFromStruct) {
  SB_LOG("CaQmQueueConfigEntry:: enforcepolicemarkings=0x%01x", (unsigned int)  pFromStruct->m_uEnforcePoliceMarkings);
  SB_LOG(" dropthresh2=0x%03x", (unsigned int)  pFromStruct->m_uDropThresh2);
  SB_LOG("\n");

  SB_LOG("CaQmQueueConfigEntry:: dropthresh1=0x%03x", (unsigned int)  pFromStruct->m_uDropThresh1);
  SB_LOG(" flowcontrolthresh2=0x%03x", (unsigned int)  pFromStruct->m_uFlowControlThresh2);
  SB_LOG("\n");

  SB_LOG("CaQmQueueConfigEntry:: flowcontrolthresh1=0x%03x", (unsigned int)  pFromStruct->m_uFlowControlThresh1);
  SB_LOG(" flowcontrolenable=0x%01x", (unsigned int)  pFromStruct->m_uFlowControlEnable);
  SB_LOG("\n");

  SB_LOG("CaQmQueueConfigEntry:: flowcontroltarget=0x%01x", (unsigned int)  pFromStruct->m_uFlowControlTarget);
  SB_LOG(" flowcontrolport=0x%03x", (unsigned int)  pFromStruct->m_uFlowControlPort);
  SB_LOG("\n");

  SB_LOG("CaQmQueueConfigEntry:: maxpages=0x%03x", (unsigned int)  pFromStruct->m_uMaxPages);
...
char *
sbZfCaQmQueueConfigEntry_SPrint(sbZfCaQmQueueConfigEntry_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
