/*
 * $Id: sbZfCaPpAggregateHashConfigEntryConsole.c,v 1.1.48.3 2011/05/22 05:39:26 iakramov Exp $
 * $Copyright (c) 2011 Broadcom Corporation
 * All rights reserved.$
 */
#include "sbTypes.h"
#include <soc/sbx/sbWrappers.h>
#include "sbZfCaPpAggregateHashConfigEntryConsole.hx"
...
sbZfCaPpAggregateHashConfigEntry_Print(sbZfCaPpAggregateHashConfigEntry_t *pFromStruct) {
  SB_LOG("CaPpAggregateHashConfigEntry:: data9=0x%08x", (unsigned int)  pFromStruct->m_uData9);
  SB_LOG(" data8=0x%08x", (unsigned int)  pFromStruct->m_uData8);
  SB_LOG("\n");

  SB_LOG("CaPpAggregateHashConfigEntry:: data7=0x%08x", (unsigned int)  pFromStruct->m_uData7);
  SB_LOG(" data6=0x%08x", (unsigned int)  pFromStruct->m_uData6);
  SB_LOG("\n");

  SB_LOG("CaPpAggregateHashConfigEntry:: data5=0x%08x", (unsigned int)  pFromStruct->m_uData5);
  SB_LOG(" data4=0x%08x", (unsigned int)  pFromStruct->m_uData4);
  SB_LOG("\n");

  SB_LOG("CaPpAggregateHashConfigEntry:: data3=0x%08x", (unsigned int)  pFromStruct->m_uData3);
  SB_LOG(" data2=0x%08x", (unsigned int)  pFromStruct->m_uData2);
  SB_LOG("\n");

  SB_LOG("CaPpAggregateHashConfigEntry:: data1=0x%08x", (unsigned int)  pFromStruct->m_uData1);
...
sbZfCaPpAggregateHashConfigEntry_SPrint(sbZfCaPpAggregateHashConfigEntry_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPpAggregateHashConfigEntry:: data9=0x%08x", (unsigned int)  pFromStruct->m_uData9);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," data8=0x%08x", (unsigned int)  pFromStruct->m_uData8);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPpAggregateHashConfigEntry:: data7=0x%08x", (unsigned int)  pFromStruct->m_uData7);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," data6=0x%08x", (unsigned int)  pFromStruct->m_uData6);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPpAggregateHashConfigEntry:: data5=0x%08x", (unsigned int)  pFromStruct->m_uData5);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," data4=0x%08x", (unsigned int)  pFromStruct->m_uData4);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPpAggregateHashConfigEntry:: data3=0x%08x", (unsigned int)  pFromStruct->m_uData3);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," data2=0x%08x", (unsigned int)  pFromStruct->m_uData2);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPpAggregateHashConfigEntry:: data1=0x%08x", (unsigned int)  pFromStruct->m_uData1);
