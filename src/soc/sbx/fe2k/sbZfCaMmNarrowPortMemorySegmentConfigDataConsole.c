/*
 * $Id: sbZfCaMmNarrowPortMemorySegmentConfigDataConsole.c 1.1.48.3 Broadcom SDK $
...
sbZfCaMmNarrowPortMemorySegmentConfigData_Print(sbZfCaMmNarrowPortMemorySegmentConfigData_t *pFromStruct) {
  SB_LOG("CaMmNarrowPortMemorySegmentConfigData:: resv0=0x%02x", (unsigned int)  pFromStruct->m_uResv0);
  SB_LOG(" wacc=0x%01x", (unsigned int)  pFromStruct->m_uWriteAccess);
  SB_LOG(" racc=0x%01x", (unsigned int)  pFromStruct->m_uReadAccess);
  SB_LOG("\n");

  SB_LOG("CaMmNarrowPortMemorySegmentConfigData:: ding=0x%01x", (unsigned int)  pFromStruct->m_uDataIntegrity);
  SB_LOG(" trap=0x%01x", (unsigned int)  pFromStruct->m_uTrapNullPointer);
  SB_LOG(" wrap=0x%01x", (unsigned int)  pFromStruct->m_uWrap);
  SB_LOG("\n");

  SB_LOG("CaMmNarrowPortMemorySegmentConfigData:: lmask=0x%06x", (unsigned int)  pFromStruct->m_uLimitMask);
...
char *
sbZfCaMmNarrowPortMemorySegmentConfigData_SPrint(sbZfCaMmNarrowPortMemorySegmentConfigData_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
