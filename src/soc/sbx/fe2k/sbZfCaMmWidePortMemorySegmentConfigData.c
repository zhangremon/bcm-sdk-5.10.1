/*
 * $Id: sbZfCaMmWidePortMemorySegmentConfigData.c 1.1.48.4 Broadcom SDK $
...
#include "sbTypes.h"
#include "sbZfCaMmWidePortMemorySegmentConfigData.hx"
...
sbZfCaMmWidePortMemorySegmentConfigData_Pack(sbZfCaMmWidePortMemorySegmentConfigData_t *pFrom,
  ...
  int i;
  int size = SB_ZF_CAMMWIDEPORTMEMORYSEGMENTCONFIGDATA_SIZE_IN_BYTES;
  ...
  (pToData)[6] |= ((pFrom)->m_uWriteAccess & 0x01) <<7;
  (pToData)[7] |= ((pFrom)->m_uWriteAccess >> 1) & 0x01;

  /* Pack Member: m_uReadAccess */
  (pToData)[6] |= ((pFrom)->m_uReadAccess & 0x03) <<5;

  /* Pack Member: m_uDataIntegrity */
  (pToData)[6] |= ((pFrom)->m_uDataIntegrity & 0x07) <<2;
  ...
  return SB_ZF_CAMMWIDEPORTMEMORYSEGMENTCONFIGDATA_SIZE_IN_BYTES;
}
...
sbZfCaMmWidePortMemorySegmentConfigData_Unpack(sbZfCaMmWidePortMemorySegmentConfigData_t *pToStruct,
  ...
  (pToStruct)->m_uWriteAccess =  (uint32_t)  ((pFromData)[6] >> 7) & 0x01;
  (pToStruct)->m_uWriteAccess |=  (uint32_t)  ((pFromData)[7] & 0x01) << 1;

  /* Unpack Member: m_uReadAccess */
  (pToStruct)->m_uReadAccess =  (uint32_t)  ((pFromData)[6] >> 5) & 0x03;

  /* Unpack Member: m_uDataIntegrity */
  (pToStruct)->m_uDataIntegrity =  (uint32_t)  ((pFromData)[6] >> 2) & 0x07;
