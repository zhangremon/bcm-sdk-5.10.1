/*
 * $Id: sbZfCaPtPte1PortStateEntryConsole.c 1.1.48.3 Broadcom SDK $
...
sbZfCaPtPte1PortStateEntry_Print(sbZfCaPtPte1PortStateEntry_t *pFromStruct) {
  SB_LOG("CaPtPte1PortStateEntry:: pktinprogress=0x%01x", (unsigned int)  pFromStruct->m_uPktInProgress);
  ...
  SB_LOG("CaPtPte1PortStateEntry:: doubledequeued=0x%01x", (unsigned int)  pFromStruct->m_uDoubleDequeued);
  SB_LOG(" eopextraburst=0x%01x", (unsigned int)  pFromStruct->m_uEopExtraBurst);
  SB_LOG("\n");

  SB_LOG("CaPtPte1PortStateEntry:: residuebyte=0x%01x", (unsigned int)  pFromStruct->m_uResidueByte);
  ...
  SB_LOG("CaPtPte1PortStateEntry:: lastpage=0x%01x", (unsigned int)  pFromStruct->m_uLastPage);
  ...
  SB_LOG("CaPtPte1PortStateEntry:: nextlength=0x%02x", (unsigned int)  pFromStruct->m_uNextLength);
  SB_LOG(" page=0x%03x", (unsigned int)  pFromStruct->m_uPage);
  SB_LOG(" savedbytes=0x%01x", (unsigned int)  pFromStruct->m_uSavedBytes);
  SB_LOG("\n");

  SB_LOG("CaPtPte1PortStateEntry:: remainingbytes=0x%02x", (unsigned int)  pFromStruct->m_uRemainingBytes);
  SB_LOG(" eop=0x%01x", (unsigned int)  pFromStruct->m_uEop);
  SB_LOG(" sop=0x%01x", (unsigned int)  pFromStruct->m_uSop);
  SB_LOG("\n");

  SB_LOG("CaPtPte1PortStateEntry:: pktlength=0x%04x", (unsigned int)  pFromStruct->m_uPktLength);
  SB_LOG(" continuebyte=0x%02x", (unsigned int)  pFromStruct->m_uContinueByte);
  SB_LOG("\n");

  SB_LOG("CaPtPte1PortStateEntry:: sourcequeue=0x%02x", (unsigned int)  pFromStruct->m_uSourceQueue);
  SB_LOG("\n");
...
char *
sbZfCaPtPte1PortStateEntry_SPrint(sbZfCaPtPte1PortStateEntry_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
