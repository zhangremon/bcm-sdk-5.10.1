/*
 * $Id: sbZfCaPpStreamSelectionStateAddress.c 1.1.48.4 Broadcom SDK $
...
sbZfCaPpStreamSelectionStateAddress_Unpack(sbZfCaPpStreamSelectionStateAddress_t *pToStruct,
  ...
  (pToStruct)->m_uLocalStationMatch =  (uint32_t)  ((pFromData)[1] >> 3) & 0x01;

  /* Unpack Member: m_uHeaderTransitionNumber */
  (pToStruct)->m_uHeaderTransitionNumber =  (uint32_t)  ((pFromData)[0] >> 5) & 0x07;
  (pToStruct)->m_uHeaderTransitionNumber |=  (uint32_t)  ((pFromData)[1] & 0x07) << 3;

  /* Unpack Member: m_uLastState */
  (pToStruct)->m_uLastState =  (uint32_t)  ((pFromData)[0] ) & 0x1f;
