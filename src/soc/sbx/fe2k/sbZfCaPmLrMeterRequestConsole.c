/*
 * $Id: sbZfCaPmLrMeterRequestConsole.c 1.1.48.3 Broadcom SDK $
...
#include "sbZfCaPmLrMeterRequestConsole.hx"
...
sbZfCaPmLrMeterRequest_Print(sbZfCaPmLrMeterRequest_t *pFromStruct) {
  SB_LOG("CaPmLrMeterRequest:: resv0=0x%02x", (unsigned int)  pFromStruct->m_uResv0);
  SB_LOG(" pid=0x%07x", (unsigned int)  pFromStruct->m_uPolicerId);
  SB_LOG(" resv1=0x%04x", (unsigned int)  pFromStruct->m_uResv1);
  SB_LOG(" blnd=0x%01x", (unsigned int)  pFromStruct->m_uBlind);
  SB_LOG("\n");

  SB_LOG("CaPmLrMeterRequest:: dred=0x%01x", (unsigned int)  pFromStruct->m_uDropOnRed);
...
char *
sbZfCaPmLrMeterRequest_SPrint(sbZfCaPmLrMeterRequest_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPmLrMeterRequest:: resv0=0x%02x", (unsigned int)  pFromStruct->m_uResv0);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," pid=0x%07x", (unsigned int)  pFromStruct->m_uPolicerId);
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPmLrMeterRequest:: dred=0x%01x", (unsigned int)  pFromStruct->m_uDropOnRed);
...
sbZfCaPmLrMeterRequest_Validate(sbZfCaPmLrMeterRequest_t *pZf) {
  ...
  if (pZf->m_uPolicerId > 0x1ffffff) return 0;
