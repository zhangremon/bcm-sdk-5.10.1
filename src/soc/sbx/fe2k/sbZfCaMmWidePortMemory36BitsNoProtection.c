/*
 * $Id: sbZfCaMmWidePortMemory36BitsNoProtection.c 1.1.48.4 Broadcom SDK $
...
sbZfCaMmWidePortMemory36BitsNoProtection_Pack(sbZfCaMmWidePortMemory36BitsNoProtection_t *pFrom,
  ...
  int i;
  int size = SB_ZF_ZFCAMMWIDEPORTMEMORY36BITSNOPROTECTION_SIZE_IN_BYTES;
  ...
  return SB_ZF_ZFCAMMWIDEPORTMEMORY36BITSNOPROTECTION_SIZE_IN_BYTES;
}
...
sbZfCaMmWidePortMemory36BitsNoProtection_Unpack(sbZfCaMmWidePortMemory36BitsNoProtection_t *pToStruct,
  ...
  (pToStruct)->m_uData1ProtectionBits =  (uint32_t)  ((pFromData)[8] >> 4) & 0x0f;

  /* Unpack Member: m_uData1 */
  (pToStruct)->m_uData1 =  (uint32_t)  ((pFromData)[4] >> 4) & 0x0f;
  (pToStruct)->m_uData1 |=  (uint32_t)  (pFromData)[5] << 4;
  (pToStruct)->m_uData1 |=  (uint32_t)  (pFromData)[6] << 12;
  (pToStruct)->m_uData1 |=  (uint32_t)  (pFromData)[7] << 20;
  (pToStruct)->m_uData1 |=  (uint32_t)  ((pFromData)[8] & 0x0f) << 28;

  /* Unpack Member: m_uData0ProtectionBits */
  (pToStruct)->m_uData0ProtectionBits =  (uint32_t)  ((pFromData)[4] ) & 0x0f;
