/*
 * $Id: sbZfCaPeDiagPV2EtcConsole.c 1.1.48.3 Broadcom SDK $
...
sbZfCaPeDiagPV2Etc_Print(sbZfCaPeDiagPV2Etc_t *pFromStruct) {
  SB_LOG("CaPeDiagPV2Etc:: qid=0x%04x", (unsigned int)  pFromStruct->m_uQid);
  SB_LOG(" test=0x%01x", (unsigned int)  pFromStruct->m_uTest);
  SB_LOG(" dummy=0x%02x", (unsigned int)  pFromStruct->m_uDummy);
  SB_LOG(" vid=0x%03x", (unsigned int)  pFromStruct->m_uVid);
  SB_LOG("\n");
...
char *
sbZfCaPeDiagPV2Etc_SPrint(sbZfCaPeDiagPV2Etc_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
