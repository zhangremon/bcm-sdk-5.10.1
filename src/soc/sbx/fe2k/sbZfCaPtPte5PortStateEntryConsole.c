/*
 * $Id: sbZfCaPtPte5PortStateEntryConsole.c 1.1.48.3 Broadcom SDK $
...
#include "sbZfCaPtPte5PortStateEntryConsole.hx"



/* Print members in struct */
void
sbZfCaPtPte5PortStateEntry_Print(sbZfCaPtPte5PortStateEntry_t *pFromStruct) {
  SB_LOG("CaPtPte5PortStateEntry:: pktinprogress=0x%01x", (unsigned int)  pFromStruct->m_uPktInProgress);
  ...
  SB_LOG("CaPtPte5PortStateEntry:: doubledequeued=0x%01x", (unsigned int)  pFromStruct->m_uDoubleDequeued);
  SB_LOG(" eopextraburst=0x%01x", (unsigned int)  pFromStruct->m_uEopExtraBurst);
  SB_LOG("\n");

  SB_LOG("CaPtPte5PortStateEntry:: residuebyte=0x%01x", (unsigned int)  pFromStruct->m_uResidueByte);
  SB_LOG(" pktrequeststate=0x%01x", (unsigned int)  pFromStruct->m_uPktRequestState);
  SB_LOG(" line=0x%01x", (unsigned int)  pFromStruct->m_uLine);
  SB_LOG("\n");

  SB_LOG("CaPtPte5PortStateEntry:: lastpage=0x%01x", (unsigned int)  pFromStruct->m_uLastPage);
  SB_LOG(" nextpagestate=0x%01x", (unsigned int)  pFromStruct->m_uNextPageState);
  SB_LOG(" nextpage=0x%03x", (unsigned int)  pFromStruct->m_uNextPage);
  SB_LOG("\n");

  SB_LOG("CaPtPte5PortStateEntry:: nextlength=0x%02x", (unsigned int)  pFromStruct->m_uNextLength);
  SB_LOG(" page=0x%03x", (unsigned int)  pFromStruct->m_uPage);
  SB_LOG(" savedbytes=0x%01x", (unsigned int)  pFromStruct->m_uSavedBytes);
  SB_LOG("\n");

  SB_LOG("CaPtPte5PortStateEntry:: remainingbytes=0x%02x", (unsigned int)  pFromStruct->m_uRemainingBytes);
  SB_LOG(" eop=0x%01x", (unsigned int)  pFromStruct->m_uEop);
  SB_LOG(" sop=0x%01x", (unsigned int)  pFromStruct->m_uSop);
  SB_LOG("\n");

  SB_LOG("CaPtPte5PortStateEntry:: pktlength=0x%04x", (unsigned int)  pFromStruct->m_uPktLength);
  SB_LOG(" continuebyte=0x%02x", (unsigned int)  pFromStruct->m_uContinueByte);
  SB_LOG("\n");

  SB_LOG("CaPtPte5PortStateEntry:: sourcequeue=0x%02x", (unsigned int)  pFromStruct->m_uSourceQueue);
  SB_LOG("\n");
...
char *
sbZfCaPtPte5PortStateEntry_SPrint(sbZfCaPtPte5PortStateEntry_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte5PortStateEntry:: pktinprogress=0x%01x", (unsigned int)  pFromStruct->m_uPktInProgress);
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte5PortStateEntry:: doubledequeued=0x%01x", (unsigned int)  pFromStruct->m_uDoubleDequeued);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," eopextraburst=0x%01x", (unsigned int)  pFromStruct->m_uEopExtraBurst);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte5PortStateEntry:: residuebyte=0x%01x", (unsigned int)  pFromStruct->m_uResidueByte);
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte5PortStateEntry:: lastpage=0x%01x", (unsigned int)  pFromStruct->m_uLastPage);
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte5PortStateEntry:: nextlength=0x%02x", (unsigned int)  pFromStruct->m_uNextLength);
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte5PortStateEntry:: remainingbytes=0x%02x", (unsigned int)  pFromStruct->m_uRemainingBytes);
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte5PortStateEntry:: pktlength=0x%04x", (unsigned int)  pFromStruct->m_uPktLength);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," continuebyte=0x%02x", (unsigned int)  pFromStruct->m_uContinueByte);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte5PortStateEntry:: sourcequeue=0x%02x", (unsigned int)  pFromStruct->m_uSourceQueue);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");
...
int
sbZfCaPtPte5PortStateEntry_Validate(sbZfCaPtPte5PortStateEntry_t *pZf) {
...
int
sbZfCaPtPte5PortStateEntry_SetField(sbZfCaPtPte5PortStateEntry_t *s, char* name, int value) {
