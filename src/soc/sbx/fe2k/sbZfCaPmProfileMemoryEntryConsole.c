/*
 * $Id: sbZfCaPmProfileMemoryEntryConsole.c 1.1.48.3 Broadcom SDK $
...
sbZfCaPmProfileMemoryEntry_Print(sbZfCaPmProfileMemoryEntry_t *pFromStruct) {
  SB_LOG("CaPmProfileMemoryEntry:: type2=0x%01x", (unsigned int)  pFromStruct->m_uType2);
  SB_LOG(" droponred=0x%01x", (unsigned int)  pFromStruct->m_uDropOnRed);
  SB_LOG("\n");

  SB_LOG("CaPmProfileMemoryEntry:: excessbucketnodecrement=0x%01x", (unsigned int)  pFromStruct->m_uExcessBucketNoDecrement);
  SB_LOG("\n");

  SB_LOG("CaPmProfileMemoryEntry:: committedbucketnodecrement=0x%01x", (unsigned int)  pFromStruct->m_uCommittedBucketNoDecrement);
  SB_LOG(" lengthshift=0x%01x", (unsigned int)  pFromStruct->m_uLengthShift);
  SB_LOG("\n");

  SB_LOG("CaPmProfileMemoryEntry:: excessinformationrate=0x%03x", (unsigned int)  pFromStruct->m_uExcessInformationRate);
  SB_LOG("\n");

  SB_LOG("CaPmProfileMemoryEntry:: committedinformationrate=0x%03x", (unsigned int)  pFromStruct->m_uCommittedInformationRate);
  SB_LOG(" couplingflag=0x%01x", (unsigned int)  pFromStruct->m_uCouplingFlag);
  SB_LOG("\n");

  SB_LOG("CaPmProfileMemoryEntry:: rfcmode=0x%01x", (unsigned int)  pFromStruct->m_uRfcMode);
  ...
  SB_LOG("CaPmProfileMemoryEntry:: excessburstsize=0x%03x", (unsigned int)  pFromStruct->m_uExcessBurstSize);
  SB_LOG(" committedburstsize=0x%03x", (unsigned int)  pFromStruct->m_uCommittedBurstSize);
  SB_LOG("\n");
...
char *
sbZfCaPmProfileMemoryEntry_SPrint(sbZfCaPmProfileMemoryEntry_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
