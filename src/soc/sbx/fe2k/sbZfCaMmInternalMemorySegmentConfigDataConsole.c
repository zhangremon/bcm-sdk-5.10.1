/*
 * $Id: sbZfCaMmInternalMemorySegmentConfigDataConsole.c 1.1.48.3 Broadcom SDK $
...
sbZfCaMmInternalMemorySegmentConfigData_Print(sbZfCaMmInternalMemorySegmentConfigData_t *pFromStruct) {
  SB_LOG("CaMmInternalMemorySegmentConfigData:: resv0=0x%07x", (unsigned int)  pFromStruct->m_uResv0);
  SB_LOG(" wacc=0x%01x", (unsigned int)  pFromStruct->m_uWriteAccess);
  SB_LOG(" racc=0x%01x", (unsigned int)  pFromStruct->m_uReadAccess);
  SB_LOG("\n");

  SB_LOG("CaMmInternalMemorySegmentConfigData:: ding=0x%01x", (unsigned int)  pFromStruct->m_uDataIntegrity);
  SB_LOG(" trap=0x%01x", (unsigned int)  pFromStruct->m_uTrapNullPointer);
  SB_LOG(" wrap=0x%01x", (unsigned int)  pFromStruct->m_uWrap);
  SB_LOG("\n");

  SB_LOG("CaMmInternalMemorySegmentConfigData:: lmask=0x%04x", (unsigned int)  pFromStruct->m_uLimitMask);
  SB_LOG(" baddr=0x%04x", (unsigned int)  pFromStruct->m_uBaseAddress);
  SB_LOG("\n");
...
char *
sbZfCaMmInternalMemorySegmentConfigData_SPrint(sbZfCaMmInternalMemorySegmentConfigData_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
