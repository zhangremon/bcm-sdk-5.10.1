/*
 * $Id: sbZfCaQmDequeueState0EntryConsole.c 1.1.48.3 Broadcom SDK $
...
sbZfCaQmDequeueState0Entry_Print(sbZfCaQmDequeueState0Entry_t *pFromStruct) {
  SB_LOG("CaQmDequeueState0Entry:: drop=0x%01x", (unsigned int)  pFromStruct->m_uDrop);
  SB_LOG(" remainingpageshi=0x%01x", (unsigned int)  pFromStruct->m_uRemainingPagesHi);
  SB_LOG("\n");

  SB_LOG("CaQmDequeueState0Entry:: remainingpageslo=0x%02x", (unsigned int)  pFromStruct->m_uRemainingPagesLo);
  SB_LOG(" replicantnumber=0x%03x", (unsigned int)  pFromStruct->m_uReplicantNumber);
  SB_LOG("\n");

  SB_LOG("CaQmDequeueState0Entry:: replicantlength=0x%04x", (unsigned int)  pFromStruct->m_uReplicantLength);
  SB_LOG("\n");
...
char *
sbZfCaQmDequeueState0Entry_SPrint(sbZfCaQmDequeueState0Entry_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
