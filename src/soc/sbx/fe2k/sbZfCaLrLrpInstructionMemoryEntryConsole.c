/*
 * $Id: sbZfCaLrLrpInstructionMemoryEntryConsole.c 1.1.48.3 Broadcom SDK $
...
#include "sbZfCaLrLrpInstructionMemoryEntryConsole.hx"



/* Print members in struct */
void
sbZfCaLrLrpInstructionMemoryEntry_Print(sbZfCaLrLrpInstructionMemoryEntry_t *pFromStruct) {
  SB_LOG("CaLrLrpInstructionMemoryEntry:: thread=0x%01x", (unsigned int)  pFromStruct->m_uThread);
  ...
  SB_LOG("CaLrLrpInstructionMemoryEntry:: target=0x%05x", (unsigned int)  pFromStruct->m_uTarget);
  SB_LOG(" sourcea=0x%08x", (unsigned int)  pFromStruct->m_uSourceA);
  SB_LOG("\n");

  SB_LOG("CaLrLrpInstructionMemoryEntry:: sourceb=0x%04x", (unsigned int)  pFromStruct->m_uSourceB);
...
char *
sbZfCaLrLrpInstructionMemoryEntry_SPrint(sbZfCaLrLrpInstructionMemoryEntry_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaLrLrpInstructionMemoryEntry:: thread=0x%01x", (unsigned int)  pFromStruct->m_uThread);
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaLrLrpInstructionMemoryEntry:: target=0x%05x", (unsigned int)  pFromStruct->m_uTarget);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," sourcea=0x%08x", (unsigned int)  pFromStruct->m_uSourceA);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaLrLrpInstructionMemoryEntry:: sourceb=0x%04x", (unsigned int)  pFromStruct->m_uSourceB);
...
int
sbZfCaLrLrpInstructionMemoryEntry_Validate(sbZfCaLrLrpInstructionMemoryEntry_t *pZf) {
...
int
sbZfCaLrLrpInstructionMemoryEntry_SetField(sbZfCaLrLrpInstructionMemoryEntry_t *s, char* name, int value) {
