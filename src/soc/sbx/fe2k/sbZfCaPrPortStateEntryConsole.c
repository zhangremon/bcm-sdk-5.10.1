/*
 * $Id: sbZfCaPrPortStateEntryConsole.c 1.1.48.3 Broadcom SDK $
...
sbZfCaPrPortStateEntry_Print(sbZfCaPrPortStateEntry_t *pFromStruct) {
  SB_LOG("CaPrPortStateEntry:: reserve=0x%02x", (unsigned int)  pFromStruct->m_nReserved);
  ...
  SB_LOG("CaPrPortStateEntry:: npages=0x%02x", (unsigned int)  pFromStruct->m_nNumPages);
  ...
  SB_LOG("CaPrPortStateEntry:: drop=0x%01x", (unsigned int)  pFromStruct->m_bDropPkt);
  SB_LOG("\n");
...
char *
sbZfCaPrPortStateEntry_SPrint(sbZfCaPrPortStateEntry_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
