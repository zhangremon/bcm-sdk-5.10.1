/*
 * $Id: sbZfCaPtPte6PortStateEntryConsole.c 1.1.48.3 Broadcom SDK $
...
#include "sbZfCaPtPte6PortStateEntryConsole.hx"



/* Print members in struct */
void
sbZfCaPtPte6PortStateEntry_Print(sbZfCaPtPte6PortStateEntry_t *pFromStruct) {
  SB_LOG("CaPtPte6PortStateEntry:: pktinprogress=0x%01x", (unsigned int)  pFromStruct->m_uPktInProgress);
  ...
  SB_LOG("CaPtPte6PortStateEntry:: doubledequeued=0x%01x", (unsigned int)  pFromStruct->m_uDoubleDequeued);
  SB_LOG(" eopextraburst=0x%01x", (unsigned int)  pFromStruct->m_uEopExtraBurst);
  SB_LOG("\n");

  SB_LOG("CaPtPte6PortStateEntry:: residuebyte=0x%01x", (unsigned int)  pFromStruct->m_uResidueByte);
  ...
  SB_LOG("CaPtPte6PortStateEntry:: lastpage=0x%01x", (unsigned int)  pFromStruct->m_uLastPage);
  ...
  SB_LOG("CaPtPte6PortStateEntry:: nextlength=0x%02x", (unsigned int)  pFromStruct->m_uNextLength);
  SB_LOG(" page=0x%03x", (unsigned int)  pFromStruct->m_uPage);
  SB_LOG(" savedbytes=0x%01x", (unsigned int)  pFromStruct->m_uSavedBytes);
  SB_LOG("\n");

  SB_LOG("CaPtPte6PortStateEntry:: remainingbytes=0x%02x", (unsigned int)  pFromStruct->m_uRemainingBytes);
  SB_LOG(" eop=0x%01x", (unsigned int)  pFromStruct->m_uEop);
  SB_LOG(" sop=0x%01x", (unsigned int)  pFromStruct->m_uSop);
  SB_LOG("\n");

  SB_LOG("CaPtPte6PortStateEntry:: pktlength=0x%04x", (unsigned int)  pFromStruct->m_uPktLength);
  SB_LOG(" continuebyte=0x%02x", (unsigned int)  pFromStruct->m_uContinueByte);
  SB_LOG("\n");

  SB_LOG("CaPtPte6PortStateEntry:: sourcequeue=0x%02x", (unsigned int)  pFromStruct->m_uSourceQueue);
  SB_LOG("\n");
...
char *
sbZfCaPtPte6PortStateEntry_SPrint(sbZfCaPtPte6PortStateEntry_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte6PortStateEntry:: pktinprogress=0x%01x", (unsigned int)  pFromStruct->m_uPktInProgress);
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte6PortStateEntry:: doubledequeued=0x%01x", (unsigned int)  pFromStruct->m_uDoubleDequeued);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," eopextraburst=0x%01x", (unsigned int)  pFromStruct->m_uEopExtraBurst);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte6PortStateEntry:: residuebyte=0x%01x", (unsigned int)  pFromStruct->m_uResidueByte);
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte6PortStateEntry:: lastpage=0x%01x", (unsigned int)  pFromStruct->m_uLastPage);
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte6PortStateEntry:: nextlength=0x%02x", (unsigned int)  pFromStruct->m_uNextLength);
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte6PortStateEntry:: remainingbytes=0x%02x", (unsigned int)  pFromStruct->m_uRemainingBytes);
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte6PortStateEntry:: pktlength=0x%04x", (unsigned int)  pFromStruct->m_uPktLength);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," continuebyte=0x%02x", (unsigned int)  pFromStruct->m_uContinueByte);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte6PortStateEntry:: sourcequeue=0x%02x", (unsigned int)  pFromStruct->m_uSourceQueue);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");
...
int
sbZfCaPtPte6PortStateEntry_Validate(sbZfCaPtPte6PortStateEntry_t *pZf) {
...
int
sbZfCaPtPte6PortStateEntry_SetField(sbZfCaPtPte6PortStateEntry_t *s, char* name, int value) {
