/*
 * $Id: sbZfCaPrTagInfoConsole.c 1.1.48.3 Broadcom SDK $
...
sbZfCaPrTagInfo_Print(sbZfCaPrTagInfo_t *pFromStruct) {
  SB_LOG("CaPrTagInfo:: active=0x%01x", (unsigned int)  pFromStruct->m_bActive);
  ...
  SB_LOG("CaPrTagInfo:: lines=0x%01x", (unsigned int)  pFromStruct->m_nLineCount);
...
char *
sbZfCaPrTagInfo_SPrint(sbZfCaPrTagInfo_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
