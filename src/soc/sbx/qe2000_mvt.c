/*
 * $Id: qe2000_mvt.c 1.20 Broadcom SDK $
...
soc_qe2000_mvt_consistency_check(const int qeUnit)
{
    ...
    if (qeUnit >= BCM_MAX_NUM_UNITS) {
        SOC_ERROR_PRINT((DK_ERR, "%s, Invalid unit(%d)\n", FUNCTION_NAME(), qeUnit));
        return(SOC_E_INIT);
    }
    if (mvtState_p[qeUnit] == NULL) {
        SOC_ERROR_PRINT((DK_ERR, "%s, MVT (Ver1) Not Initialized, unit(%d)\n", FUNCTION_NAME(), qeUnit));
        return(SOC_E_INIT);
    }
    ...
    if (sbx_mvt_mode == SBX_MVT_MODE_VER0) {
        if (!g_mvtState) {
            ...
            SOC_ERROR_PRINT((DK_ERR, "%s, MVT (Ver0) Not Initialized, unit(%d)\n", FUNCTION_NAME(), qeUnit));
            return(SOC_E_INIT);
        }
    }
        ...
        SOC_ERROR_PRINT((DK_ERR, "%s, MVT (Ver Unknown), unit(%d)\n", FUNCTION_NAME(), qeUnit));
        return(SOC_E_INIT);
    }
                            ...
                            sbx_mvt_id_t pEntryIds[])
{
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        for (nCount = 1; nCount < num; nCount++) {
            ...
            SOC_DEBUG_PRINT((0, "Linking entry[%d]=%d to entry[%d]=%d\n",
			 ...
			 sbx_qe2000_mvt_entry_t *pEntry)
{
    ...
    sbxRv = hwQe2000MVTGet((sbhandle)qeUnit, &sbMvtEntry, entryId,
                           NULL, NULL, 0, 0, NULL);
    if( sbxRv ) {
        SOC_DEBUG_PRINT((DK_ERR, "MVTGet Failed:(%d)\n", sbxRv ));
        return SOC_E_FAIL;
    }
                          ...
                          const sbx_qe2000_mvt_entry_t *pEntry)
{
    ...
    sbxRv = hwQe2000MVTSet((sbhandle)qeUnit, sbMvtEntry, entryId,
                           NULL, NULL, 0, 0, NULL);
    if( sbxRv ) {
        SOC_DEBUG_PRINT((DK_ERR, "MVTSet Failed:(%d)\n", sbxRv ));
        return SOC_E_FAIL;
    }
...
soc_qe2000_mvt_init(const int qeUnit)
{
    ...
    if (!g_mvtState) {
        ...
        if (!BCM_FAILURE(rv)) {
            SOC_DEBUG_PRINT((DK_VERBOSE,
                             "Discard unit %d prior state %08X; reinit at %08X\n",
                             qeUnit,
                             (uint32)oldList,
    ...
    if (!SOC_WARM_BOOT(qeUnit)) {
        ...
        for(i=0; i<nNumInitMtv; i++)
        {
            ...
            if( sbRv ) {
                SOC_DEBUG_PRINT((DK_ERR, "Error on entry=%d  %d\n", i, sbRv));
            }
        }
        /* diagnostic */
        SOC_DEBUG_PRINT((DK_VERBOSE, "Initialized %d MVT entries, in %d uSecs\n",
        ...
        if ((SOC_E_NOT_FOUND != rv) && (SOC_E_NONE != rv)) {
            SOC_DEBUG_PRINT((DK_ERR,
                             "%s: Error(%s) checking scache handle on"
                             " unit:%d\n",
                             FUNCTION_NAME(),
                             soc_errmsg(rv),
                             qeUnit));
        ...
        if (SOC_E_NOT_FOUND == rv) {
            ...
            if (SOC_E_NONE != rv) {
                SOC_DEBUG_PRINT((DK_ERR,
                                 "%s: Error(%s) allocating WB scache handle"
                                 " on unit:%d \n",
                                 FUNCTION_NAME(),
                                 soc_errmsg(rv),
                                 qeUnit));
            ...
            if (SOC_E_NONE == rv) {
                ...
                if (SOC_E_NONE != rv) {
                    SOC_DEBUG_PRINT((DK_ERR,
                                     "%s: Error(%s) checking scache handle"
                                     " on unit:%d\n",
                                     FUNCTION_NAME(),
                                     soc_errmsg(rv),
                                     qeUnit));
        ...
        if ((SOC_E_NONE == rv) && (wbOldSize < wbSize)) {
            SOC_DEBUG_PRINT((DK_ERR,
                             "%s: unit %d scache block is too small (%d;"
                             " needed %d)\n",
                             FUNCTION_NAME(),
                             qeUnit,
                             wbOldSize,
        ...
        if (SOC_E_NONE != rv) {
            SOC_DEBUG_PRINT((DK_ERR,
                             "%s: Error(%s) getting scache handle on"
                             " unit:%d\n",
                             FUNCTION_NAME(),
                             soc_errmsg(rv),
                             qeUnit));
        ...
        if (SOC_E_NONE == rv) {
            ...
            if (SBX_MVT_MODE_VER0 != hdrPtr->mvtMode) {
                SOC_DEBUG_PRINT((DK_ERR,
                                 "%s: unit %d configuration has changed\n",
                                 FUNCTION_NAME(),
                                 qeUnit));
                rv = BCM_E_CONFIG;
            }
        ...
        if (SOC_E_NONE == rv) {
            ...
            for (i = 0; i < k; i++) {
                if (SHR_BITGET(g_mvtState->mvtUse, i)) {
                    ...
                    SOC_DEBUG_PRINT((DK_VERBOSE,
                                     "Reserve unit %d MVT entries %08X..%08X\n",
                                     qeUnit,
                                     i + MVT_NUM_STATIC,
                                     j + MVT_NUM_STATIC));
                    rv = shr_idxres_list_reserve(g_mvtState->freeList,
                    ...
                    if (BCM_E_NONE != rv) {
                        SOC_DEBUG_PRINT((DK_ERR,
                                         "%s: unable to reserve unit %d MVT"
                                         " elements %d..%d: %d (%s)\n",
                                         FUNCTION_NAME(),
                                         qeUnit,
                                         i + MVT_NUM_STATIC,
...
soc_qe2000_bd_mvt_init(const int qeUnit)
{
    ...
    if (SBX_MVT_ID_BD_SIZE == 0) {
        /* SOC_ERROR_PRINT((DK_ERR, "%s, no initialization required\n", FUNCTION_NAME())); */
        return(rv);
    }
    ...
    if (!g_mvtState) {
        ...
        if (tempState) {
            ...
            /* failed to allocate the needed space */
            SOC_ERROR_PRINT((DK_ERR, "%s, memory allocation failed\n", FUNCTION_NAME()));
            return(SOC_E_MEMORY);
        }
#ifdef BCM_WARM_BOOT_SUPPORT
        /* since the handle is fixed, always just fill in the computed value */
        ...
        if(BCM_FAILURE(rv)) {
            /* get rid of the working structure */
            SOC_ERROR_PRINT((DK_ERR, "%s, shr_idxres_list_create, failed, Err: 0x%x\n", FUNCTION_NAME(), rv));
            sal_free(tempState);
            return(rv);
        }
        /* if we're here, it all went well */
        ...
        if (!BCM_FAILURE(rv)) {
            SOC_DEBUG_PRINT((DK_VERBOSE,
                             "Discard unit %d prior BD state %08X; reinit at %08X\n",
                             qeUnit,
                             (uint32)oldList,
                             (uint32)g_mvtState->freeList));
            if (oldList) {
...
soc_qe2000_get_mvt_free_list_frm_block(const int qeUnit, int mvt_id_block)
{
    ...
    if ((mvt_id_block != SBX_MVT_ID_GLOBAL) && (mvt_id_block != SBX_MVT_ID_LOCAL) &&
        ...
        SOC_ERROR_PRINT((DK_ERR, "%s, Invalid mvt_id_block Unit(%d), MvtBlock(%d)\n", FUNCTION_NAME(), qeUnit, mvt_id_block));
        return(NULL);
    }
    ...
    if (free_list == NULL) {
        SOC_ERROR_PRINT((DK_ERR, "%s, NULL Free list, Unit(%d)\n", FUNCTION_NAME(), qeUnit));
    }
...
soc_qe2000_get_mvt_free_list_frm_id(const int qeUnit, int mvt_id)
{
    ...
    if (sbx_mvt_mode == SBX_MVT_MODE_VER0) {
        ...
        if ( (mvt_id >= SBX_MVT_ID_VSI_BASE) && (mvt_id <= SBX_MVT_ID_VSI_END) ) {
            ...
            SOC_ERROR_PRINT((DK_ERR, "%s, Inconsistency, unit(%d)\n", FUNCTION_NAME(), qeUnit));
            free_list = NULL;
        }
    }
    else {
...
soc_qe2000_mvt_initialization(const int qeUnit)
{
...
#endif
	if (mvtState_p[qeUnit] != NULL) {
            SOC_ERROR_PRINT((DK_ERR, "%s mvtState_p not null, unit(%d)\n", FUNCTION_NAME(), qeUnit));
	    return(SOC_E_INTERNAL);
	}
#ifdef BCM_EASY_RELOAD_SUPPORT
    }
...
#endif 
    if (sbx_mvt_mode == SBX_MVT_MODE_VER0) {
      SOC_ERROR_PRINT((DK_ERR, "%s, Inconsistency, unit(%d) sbx_mvt_mode(%d)\n", FUNCTION_NAME(), qeUnit, sbx_mvt_mode));
        return(SOC_E_INTERNAL);
    }
    ...
    if (SBX_MVT_ID_BD_SIZE > 0) {
        SOC_DEBUG_PRINT((DK_VERBOSE, "%s, is_local: %s, localStart: 0x%x localEnd; 0x%x\n",
			 FUNCTION_NAME(), ((is_local == TRUE) ? "TRUE" : "FALSE"), local_start, local_end));
	SOC_DEBUG_PRINT((DK_VERBOSE, "%s, is_global: %s, globalStart: 0x%x globalEnd; 0x%x\n",
    ...
    if (!SOC_WARM_BOOT(qeUnit)) {
        ...
        for (i = 0; i < k; i++) {
            if (SHR_BITGET(tempState->vlanUse, i)) {
                ...
                /* reserve elements [i..j] */
                SOC_DEBUG_PRINT((DK_VERBOSE,
                                 "Reserve unit %d VLAN entries %08X..%08X\n",
                                 qeUnit,
                                 i + hdrPtr->vlanLow,
                                 j + hdrPtr->vlanLow));
                ...
                if (BCM_E_NONE != rv) {
                    SOC_DEBUG_PRINT((DK_ERR,
                                     "%s: unable to reserve unit %d VLAN"
                                     " elements %d..%d: %d (%s)\n",
                                     FUNCTION_NAME(),
                                     qeUnit,
                                     i + hdrPtr->vlanLow,
        ...
        for (i = 0; i < k; i++) {
            if (SHR_BITGET(tempState->globalUse, i)) {
                ...
                /* reserve elements [i..j] */
                SOC_DEBUG_PRINT((DK_VERBOSE,
                                 "Reserve unit %d global entries %08X..%08X\n",
                                 qeUnit,
                                 i + hdrPtr->globalLow,
                                 j + hdrPtr->globalLow));
                ...
                if (BCM_E_NONE != rv) {
                    SOC_DEBUG_PRINT((DK_ERR,
                                     "%s: unable to reserve unit %d global"
                                     " elements %d..%d: %d (%s)\n",
                                     FUNCTION_NAME(),
                                     qeUnit,
                                     i + hdrPtr->globalLow,
        ...
        for (i = 0; i < k; i++) {
            if (SHR_BITGET(tempState->localUse, i)) {
                ...
                /* reserve elements [i..j] */
                SOC_DEBUG_PRINT((DK_VERBOSE,
                                 "Reserve unit %d local entries %08X..%08X\n",
                                 qeUnit,
                                 i + hdrPtr->localLow,
                                 j + hdrPtr->localLow));
                ...
                if (BCM_E_NONE != rv) {
                    SOC_DEBUG_PRINT((DK_ERR,
                                     "%s: unable to reserve unit %d local"
                                     " elements %d..%d: %d (%s)\n",
                                     FUNCTION_NAME(),
                                     qeUnit,
                                     i + hdrPtr->localLow,
                              ...
                              int mvt_id_block)
{
    ...
    for (nCount = 0, aCount = 0; (nCount < num) && BCM_SUCCESS(rv); nCount++, aCount++) {
        ...
        if (BCM_FAILURE(rv)) {
          SOC_ERROR_PRINT((DK_ERR, "%s, shr_idxres_list_alloc Unit(%d), nCount(%d), aCount(%d)\n", FUNCTION_NAME(), qeUnit, nCount, aCount));
        }
				...
				sbx_qe2000_mvt_entry_t *pEntry)
{
	...
	if (SOC_IS_RELOADING(qeUnit) == 0) {
		...
		rv = shr_idxres_list_elem_state(FreeList, entryId);
		if (rv != BCM_E_EXISTS ) {
		    SOC_ERROR_PRINT((DK_ERR, "%s, Element not allocated Unit(%d), Element(0x%x)\n", FUNCTION_NAME(), qeUnit, entryId));
		    return(rv);    /* could be some other idxres error... */
		}
		rv = SOC_E_NONE;
	    }
    ...
    if( sbxRv ) {
        SOC_DEBUG_PRINT((DK_ERR, "MVTGet Failed:(%d)\n", sbxRv ));
        SOC_ERROR_PRINT((DK_ERR, "%s, hwQe2000MVTGet, unit(%d), Err: 0x%x\n", FUNCTION_NAME(), qeUnit, sbxRv));
        return(SOC_E_FAIL);
    }
...
soc_qe2000_mvt_state_get(int unit, char *pbuf)
{
    ...
    if (BCM_FAILURE(rv)) {
	soc_cm_print("consistency check error\n");
        return(rv);
    }
	...
	if (FreeList == NULL) {
	    soc_cm_print("mvt list not found for mvtId(%d)\n", (int32)mvtId);
	    continue;
	}
