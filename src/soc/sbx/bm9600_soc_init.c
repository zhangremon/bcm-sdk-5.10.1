/*
 * $Id: bm9600_soc_init.c 1.89 Broadcom SDK $
...
soc_bm9600_unit9_error(int unit, uint32 ignored)
{
	...
	*/
	...
	if (SAND_HAL_GET_FIELD(PL, FO_EVENT, AUTO_FAILOVER, fo_intrs)) {
	    soc_cm_debug(DK_INTR, "Auto Failover Interrupt\n");
	}
	if (SAND_HAL_GET_FIELD(PL, FO_EVENT, AUTO_LINK_DIS, fo_intrs)) {
	    soc_cm_debug(DK_INTR, "Link disable Interrupt\n");
	}
	if (SAND_HAL_GET_FIELD(PL, FO_EVENT, AUTO_QE_DIS, fo_intrs)) {
	    soc_cm_debug(DK_INTR, "Qe disable Interrupt\n");
	}
	...
	if (((ifn_t)SOC_SBX_STATE(unit)->fabric_state->red_f) != NULL) {
	    ...
	    red_info.xbars = (uint64)SAND_HAL_GET_FIELD(PL, FO_STATUS1, GLOBAL_ENABLED_LINKS, uData);
...
soc_bm9600_isr(void *_unit)
{
  ...
  */
    ...
    for (i=0; i < BM9600_INTR_HANDLERS_COUNT; i++) {
      soc_cm_debug(DK_INTR, "soc_bm9600_isr unit %d: dispatch %s\n",
...
soc_bm9600_config_linkdriver(int unit, int port)
{
    ...
    if ( port < 0 || port >= SB_FAB_DEVICE_BM9600_NUM_SERIALIZERS ) {
        soc_cm_debug(DK_WARN, "Bad port(%d) requested for BM9600. "
                         "Valid range is [0,%d]\n", port, 
    ...
    if (status != SB_FAB_STATUS_OK) {
        soc_cm_debug(DK_WARN, "Could not get link specific configuration "
                     "for port(%d)\n", port);
                            ...
                            int dst_xbport)
{
    ...
    if (rv != SOC_E_NONE) {
        soc_cm_debug(DK_ERR, "bcm_bm9600_port_ability_get failed on xbport %d\n", src_xbport);
        return(BCM_E_FAIL);
    }
				     ...
				     int dst_xbport)
{
	...
	soc_cm_print("WARNING: mapping failure - src_iprt(%d) dest_iprt read(%d) expected value(%d)\n",
                     ...
                     uint *psigma)
{
    ...
    sbZfFabBm9600BwR0BwpEntry_Unpack(&zfBwpEntry, (uint8_t*)&read_value, 4);
                     ...
                     uint sigma)
{
    ...
    sbZfFabBm9600BwR0BwpEntry_Pack(&zfBwpEntry, (uint8_t*)&write_value, 4);
...
soc_bm9600_init(int unit, soc_sbx_config_t *cfg)
{
    ...
    * ToDo: verify the Epoch length setting for Vport and Vport legacy mode
    */
    switch ( SOC_SBX_CFG(unit)->uFabricConfig ) {
	...
	case SOC_SBX_SYSTEM_CFG_VPORT_MIX: /* Vport Mix * - Bm9600 + Qe2000 + Sirius */
	    ...
	    break;
	default:
	    BCM_FABRIC_DEBUG_ERR(("soc_bm9600_init: unit %d unknown "
    ...
    */
	...
	BCM_FABRIC_DEBUG_ERR(("BM9600 uLinkThresholdIndex(%d) initialization "
    ...
    */
    switch (SOC_SBX_CFG(unit)->uRedMode) {
        ...
        case bcmFabricRedManual:
            break;
        default:
            BCM_FABRIC_DEBUG_ERR(("soc_bm3200_init: unit %d unsupported "
                                  " redundancy mode %d",
    ...
    */
    ...
    if (uStatus != HW_BM9600_STATUS_OK_K) {
        soc_cm_print("Bm9600 initialization failed with uStatus(0x%x)\n",uStatus);
        /* NOTE: TODO, convert the low level error into a corresponding BCM error */
		     ...
		     uint32 bag_rate_bytes_per_epoch)
{
    ...
    sbZfFabBm9600BwR0BagEntry_Pack(&zfR0BagEntry, (uint8_t*)&write_value, 4);
    ...
    sbZfFabBm9600BwR1BagEntry_Pack(&zfR1BagEntry, (uint8_t*)&write_value, 4);
		    ...
		    uint32 *pbag_rate_bytes_per_epoch)
{
    ...
    sbZfFabBm9600BwR0BagEntry_Unpack(&zfR0BagEntry, (uint8_t*)&read_value, 4);
    ...
    *pbag_rate_bytes_per_epoch = (zfR0BagEntry.m_uBagRate * scale) / (scale + 1);
    ...
    sbZfFabBm9600BwR1BagEntry_Unpack(&zfR1BagEntry, (uint8_t*)&read_value, 4);
					...
					int32 new_row)
{
    ...
    if ( (sysport < 0)                         ||
	 ...
	 (offset < 0)                          ||
	 (offset >= 16) ) {
	BCM_FABRIC_DEBUG_ERR(("%s: Invalid params range, sysport(%d)"
                              " portset(%d) offset(%d)\n",
	...
	if (bSuccess != SOC_E_NONE) {
	    BCM_FABRIC_DEBUG_ERR(("%s: Failed to read NM Sysport Array Memory"
                                  " row %d. \n",
                                  FUNCTION_NAME(), (uint32) portset));
	    return bSuccess;
	}
	sbZfFabBm9600NmSysportArrayEntry_Unpack(&zfSysportArrayEntry, (uint8_t *)uData, 24);
    }
    ...
    sbZfFabBm9600NmSysportArrayEntry_Pack(&zfSysportArrayEntry, (uint8_t *)uData, 24);

    bSuccess = soc_bm9600_NmSysportArrayWrite(unit, portset, uData[0], uData[1], uData[2],
					      uData[3], uData[4], uData[5]);
    if (bSuccess != SOC_E_NONE) {
	BCM_FABRIC_DEBUG_ERR(("%s: Failed to write NM Sysport Array Memory"
                              " row %d. \n",
                              FUNCTION_NAME(), (uint32) portset));
	return bSuccess;
    }
				       ...
				       int32 *sysport)
{
    ...
    if ( (sysport == NULL)                     ||
	 ...
	 (offset < 0)                          ||
	 (offset >= 16) ) {
	BCM_FABRIC_DEBUG_ERR(("%s:  Invalid params range, portset(%d)"
                              " offset(%d)\n",
				    ...
				    int32 next)
{
    ...
    sbZfFabBm9600NmPortsetLinkEntry_Pack(&zfPortsetLink, (uint8_t *)&uData, 2);

    bSuccess = soc_bm9600_NmPortsetLinkWrite(unit, portset, uData);

    if (bSuccess != SOC_E_NONE) {
	BCM_FABRIC_DEBUG_ERR(("%s: Error writing portset link table"
                              " for entry %d\n",
                              FUNCTION_NAME(), (uint32)portset));
	return bSuccess;
    }
				    ...
				    int32 *next)
{
    ...
    if (bSuccess != SOC_E_NONE) {
	BCM_FABRIC_DEBUG_ERR(("%s: Error reading portset link table for"
                              " entry %d\n",
                              FUNCTION_NAME(), (uint32)portset));
	return bSuccess;
    }
    sbZfFabBm9600NmPortsetLinkEntry_Unpack(&zfPortsetLink, (uint8_t *)&uData, 2);
				    ...
				    int32 eg_node)
{
    ...
    sbZfFabBm9600NmPortsetInfoEntry_Pack(&zfPortsetInfo, (uint8_t *)&uData, 4);

    bSuccess = soc_bm9600_NmPortsetInfoWrite(unit, portset, uData);

    if (bSuccess != SOC_E_NONE) {
	BCM_FABRIC_DEBUG_ERR(("%s: Error writing portset info table"
                              " for entry %d\n",
                              FUNCTION_NAME(), (uint32)portset));
	return bSuccess;
    }
				    ...
				    int32 *p_eg_node)
{
    ...
    if (bSuccess != SOC_E_NONE) {
	BCM_FABRIC_DEBUG_ERR(("%s:  Error reading portset info table"
                              " for entry %d\n",
                              FUNCTION_NAME(), (uint32)portset));
	return bSuccess;
    }

    sbZfFabBm9600NmPortsetInfoEntry_Unpack(&zfPortsetInfo, (uint8_t *)&uData, 4);
					   ...
					   int32 offset)
{
    ...
    sbZfFabBm9600InaSysportMapEntry_Pack(&zfInaSysportMap, (uint8_t*)&uData, 4);

    for (ina = 0; ina < BM9600_NUM_LINKS; ina++) {
	if ( (ina_enabled[ina]) || (ina == cached_ina) ) {
	    bSuccess = soc_bm9600_InaSysportMapWrite(unit, sysport, ina, uData);
	    if (bSuccess != SOC_E_NONE) {
		BCM_FABRIC_DEBUG_ERR(("%s: Error writing ina sysport map"
                                      " for sysport %d\n",
				      ...
				      int32 *offset)
{
	...
	if (ina_enabled[ina]) {
	    ...
	    if (bSuccess != SOC_E_NONE) {
		BCM_FABRIC_DEBUG_ERR(("%s: Error reading ina sysport map"
                                      " for sysport %d on ina %d\n",
                                      FUNCTION_NAME(), (uint32)sysport,
                                      (uint32)ina));
		return bSuccess;
	    } else {
		sbZfFabBm9600InaSysportMapEntry_Unpack(&zfInaSysportMap, (uint8_t*)&uData, 4);
    ...
    BCM_FABRIC_DEBUG_ERR(("%s: Error writing ina sysport map for"
                          " sysport %d, no ina is enabled\n",
                          FUNCTION_NAME(), (uint32)sysport));
    return SOC_E_INTERNAL;
}
                            ...
                            int32  new_offset)
{
	...
	if (bDone) {
	    ...
	    count++;
	    if (count > 100) {
		BCM_FABRIC_DEBUG_ERR(("%s: Error moving sysport %d\n",
                                      FUNCTION_NAME(), (uint32)sysport));
		return SOC_E_INTERNAL;
	    }
	}
			...
			int32  portset)
{
	...
	if (bDone) {
	    ...
	    count++;
	    if (count > 100) {
		BCM_FABRIC_DEBUG_ERR(("%s: Error init portpri table for"
                                      " row %d\n",
                                      FUNCTION_NAME(), (uint32)portset));
		return SOC_E_INTERNAL;
	    }
 ...
 */
int soc_bm9600_mdio_hc_write(int unit, uint32 uPhyAddr, uint32_t uLane, uint32 uRegAddr, uint32 uData) {
    uint32 uBlkBaseAddr;
    uint32 uRegOffset;
    int status;
    ...
    if (uRegAddr > 0xffff) {
        SOC_ERROR_PRINT((DK_ERR,"Address must be less than or equal to 0xffff\n"));
	return SOC_E_PARAM;
    }
    /* 0x3ff is broadcast to all lanes */
    if (uLane > 0x3ff) {
        SOC_ERROR_PRINT((DK_ERR,"Invalid lane (0-3 or 0x3ff for broadcast)\n"));
	return SOC_E_PARAM;
    }
    if (uPhyAddr == 0x1f) {
        SOC_ERROR_PRINT((DK_ERR,"Use soc_bm9600_mdio_broadcast_write for broadcast writes to HC's instead\n"));
	return SOC_E_PARAM;
    }
...
int soc_bm9600_mdio_hc_cl22_write(int unit, uint32 uPhyAddr, uint32 uRegAddr, uint32 uData) {
    ...
    if ( uTimeOut == 150 ) {
	SOC_ERROR_PRINT((DK_ERR, "PI_MIIM_INT_STATUS-MIIM_OP_DONE, timeout waiting for ACK on MDIO write.\n"));
    ...
    SOC_DEBUG_PRINT(( DK_VERBOSE | DK_SOCMEM, "soc_bm9600_mdio_hc_cl22_write. uPhyAddr 0x%x uRegAddr 0x%x, uData 0x%x\n", uPhyAddr, uRegAddr, uData));
...
int soc_bm9600_mdio_hc_cl22_read(int unit, uint32 uPhyAddr, uint32 uRegAddr, uint32 *pReadData) {
    ...
    if ( uTimeOut == 150 ) {
	SOC_ERROR_PRINT((DK_ERR, "PI_MIIM_INT_STATUS-MIIM_OP_DONE, timeout waiting for ACK on MDIO read.\n"));
	return SOC_E_TIMEOUT;
    } else {
	/* clear interrupt status (W1TC) */
	SAND_HAL_WRITE_EASY_RELOAD(unit, PL, PI_MIIM_INT_STATUS, (1<<SAND_HAL_PL_PI_MIIM_INT_STATUS_MIIM_OP_DONE_SHIFT));
	...
	*pReadData = SAND_HAL_READ(unit, PL, PI_CMIC_MIIM_READ_DATA);
	SOC_DEBUG_PRINT((DK_VERBOSE | DK_SOCMEM, "soc_bm9600_mdio_hc_cl22_read uPhyAddr%d uRegAddr 0x%08x uData 0x%08x\n",uPhyAddr, uRegAddr, *pReadData));
...
soc_bm9600_xb_test_pkt_get(int unit, int egress, int xb_port, int *cnt)
{
    ...
    *cnt = -1;    
	...
	nnCutoffTime++;
	if( nnCutoffTime > 20 ) {
	    SOC_ERROR_PRINT((DK_ERR,"%s:  Error reading xb_test_cnt memory for uAddress=0x%08x\n", FUNCTION_NAME(), (uint32)uAddress));
	    return SOC_E_TIMEOUT;
	}
    }
...
soc_bm9600_xb_test_pkt_clear(int unit, int egress, int xb_port)
{
	...
	nnCutoffTime++;
	if( nnCutoffTime > 20 ) {
	    SOC_ERROR_PRINT((DK_ERR,"%s:  Error writing xb_test_cnt memory for uAddress=0x%08x\n", FUNCTION_NAME(), (uint32)uAddress));
	    return SOC_E_TIMEOUT;
	}
    }
...
soc_bm9600_port_check_policing_errors_reset_xb_port(int unit, uint32 *xb_iport_channel_ptr)
{
	...
	if (xb_iport_channel_ptr[j]) {
	    soc_cm_print("xb_nonempty_error%d =0x%08x\n", j, xb_iport_channel_ptr[j]);
	}
    }
#endif
	...
	for (i=0;i<16; i++) {
		...
		if (enabled == TRUE) {
#ifdef DEBUG_SDK30912
		    soc_cm_print("port(%d) xb error, disable and re-enable port\n", port);
#endif
