/*
 * $Id: sbZfC2PtPortStateEntry.c 1.1.34.4 Broadcom SDK $
...
#include "sbTypes.h"
#include "sbZfC2PtPortStateEntry.hx"
...
sbZfC2PtPortStateEntry_Pack(sbZfC2PtPortStateEntry_t *pFrom,
  ...
  int i;
  int size = SB_ZF_C2PTPORTSTATEENTRY_SIZE_IN_BYTES;
  ...
  (pToData)[11] |= ((pFrom)->m_uPktInProgress & 0x01) <<6;

  /* Pack Member: m_uMirror */
  (pToData)[11] |= ((pFrom)->m_uMirror & 0x01) <<5;

  /* Pack Member: m_uMirrorSrcDrop */
  (pToData)[11] |= ((pFrom)->m_uMirrorSrcDrop & 0x01) <<4;

  /* Pack Member: m_uDoubleDequeued */
  (pToData)[11] |= ((pFrom)->m_uDoubleDequeued & 0x01) <<3;

  /* Pack Member: m_uEopExtraBurst */
  (pToData)[11] |= ((pFrom)->m_uEopExtraBurst & 0x01) <<2;

  /* Pack Member: m_uResidueByte */
  (pToData)[10] |= ((pFrom)->m_uResidueByte & 0x03) <<6;
  (pToData)[11] |= ((pFrom)->m_uResidueByte >> 2) & 0x03;

  /* Pack Member: m_uPktRequestState */
  (pToData)[10] |= ((pFrom)->m_uPktRequestState & 0x0f) <<2;

  /* Pack Member: m_uLine */
  (pToData)[10] |= ((pFrom)->m_uLine & 0x01) <<1;

  /* Pack Member: m_uLastPage */
  (pToData)[10] |= ((pFrom)->m_uLastPage & 0x01);

  /* Pack Member: m_uNextPageState */
  (pToData)[9] |= ((pFrom)->m_uNextPageState & 0x03) <<6;

  /* Pack Member: m_uNextPage */
  (pToData)[8] |= ((pFrom)->m_uNextPage & 0x7f) <<1;
  (pToData)[9] |= ((pFrom)->m_uNextPage >> 7) & 0x3f;

  /* Pack Member: m_uNextLength */
  (pToData)[7] |= ((pFrom)->m_uNextLength & 0x7f) <<1;
  (pToData)[8] |= ((pFrom)->m_uNextLength >> 7) & 0x01;

  /* Pack Member: m_uPage */
  (pToData)[5] |= ((pFrom)->m_uPage & 0x0f) <<4;
  (pToData)[6] |= ((pFrom)->m_uPage >> 4) &0xFF;
  (pToData)[7] |= ((pFrom)->m_uPage >> 12) & 0x01;
  ...
  return SB_ZF_C2PTPORTSTATEENTRY_SIZE_IN_BYTES;
}
...
sbZfC2PtPortStateEntry_Unpack(sbZfC2PtPortStateEntry_t *pToStruct,
  ...
  (pToStruct)->m_uPktInProgress =  (uint32_t)  ((pFromData)[11] >> 6) & 0x01;

  /* Unpack Member: m_uMirror */
  (pToStruct)->m_uMirror =  (uint32_t)  ((pFromData)[11] >> 5) & 0x01;

  /* Unpack Member: m_uMirrorSrcDrop */
  (pToStruct)->m_uMirrorSrcDrop =  (uint32_t)  ((pFromData)[11] >> 4) & 0x01;

  /* Unpack Member: m_uDoubleDequeued */
  (pToStruct)->m_uDoubleDequeued =  (uint32_t)  ((pFromData)[11] >> 3) & 0x01;

  /* Unpack Member: m_uEopExtraBurst */
  (pToStruct)->m_uEopExtraBurst =  (uint32_t)  ((pFromData)[11] >> 2) & 0x01;

  /* Unpack Member: m_uResidueByte */
  (pToStruct)->m_uResidueByte =  (uint32_t)  ((pFromData)[10] >> 6) & 0x03;
  (pToStruct)->m_uResidueByte |=  (uint32_t)  ((pFromData)[11] & 0x03) << 2;

  /* Unpack Member: m_uPktRequestState */
  (pToStruct)->m_uPktRequestState =  (uint32_t)  ((pFromData)[10] >> 2) & 0x0f;

  /* Unpack Member: m_uLine */
  (pToStruct)->m_uLine =  (uint32_t)  ((pFromData)[10] >> 1) & 0x01;

  /* Unpack Member: m_uLastPage */
  (pToStruct)->m_uLastPage =  (uint32_t)  ((pFromData)[10] ) & 0x01;

  /* Unpack Member: m_uNextPageState */
  (pToStruct)->m_uNextPageState =  (uint32_t)  ((pFromData)[9] >> 6) & 0x03;

  /* Unpack Member: m_uNextPage */
  (pToStruct)->m_uNextPage =  (uint32_t)  ((pFromData)[8] >> 1) & 0x7f;
  (pToStruct)->m_uNextPage |=  (uint32_t)  ((pFromData)[9] & 0x3f) << 7;

  /* Unpack Member: m_uNextLength */
  (pToStruct)->m_uNextLength =  (uint32_t)  ((pFromData)[7] >> 1) & 0x7f;
  (pToStruct)->m_uNextLength |=  (uint32_t)  ((pFromData)[8] & 0x01) << 7;

  /* Unpack Member: m_uPage */
  (pToStruct)->m_uPage =  (uint32_t)  ((pFromData)[5] >> 4) & 0x0f;
  (pToStruct)->m_uPage |=  (uint32_t)  (pFromData)[6] << 4;
  (pToStruct)->m_uPage |=  (uint32_t)  ((pFromData)[7] & 0x01) << 12;
