/*
 * $Id: sbZfC2PrCcTcamConfigEntryFirst.c 1.1.22.4 Broadcom SDK $
...
sbZfC2PrCcTcamConfigEntryFirst_Unpack(sbZfC2PrCcTcamConfigEntryFirst_t *pToStruct,
  ...
  (pToStruct)->m_uMaskReserved =  (uint32_t)  ((pFromData)[27] >> 5) & 0x07;
  (pToStruct)->m_uMaskReserved |=  (uint32_t)  ((pFromData)[28] & 0x3f) << 3;

  /* Unpack Member: m_uMaskPort */
  (pToStruct)->m_uMaskPort =  (uint32_t)  ((pFromData)[27] ) & 0x1f;
  ...
  (pToStruct)->m_uReserved =  (uint32_t)  ((pFromData)[0] >> 7) & 0x01;
  (pToStruct)->m_uReserved |=  (uint32_t)  (pFromData)[1] << 1;

  /* Unpack Member: m_uPort */
  (pToStruct)->m_uPort =  (uint32_t)  ((pFromData)[0] >> 2) & 0x1f;
