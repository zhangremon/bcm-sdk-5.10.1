/*
 * $Id: sbZfC2QmBufferState1Entry.c 1.1.34.4 Broadcom SDK $
...
sbZfC2QmBufferState1Entry_Unpack(sbZfC2QmBufferState1Entry_t *pToStruct,
  ...
  (pToStruct)->m_uDrop =  (uint32_t)  ((pFromData)[4] ) & 0x01;

  /* Unpack Member: m_uCrcMode */
  (pToStruct)->m_uCrcMode =  (uint32_t)  ((pFromData)[3] >> 6) & 0x03;

  /* Unpack Member: m_uHeaderLength */
  (pToStruct)->m_uHeaderLength =  (uint32_t)  ((pFromData)[2] >> 6) & 0x03;
  (pToStruct)->m_uHeaderLength |=  (uint32_t)  ((pFromData)[3] & 0x3f) << 2;

  /* Unpack Member: m_uContinueByte */
  (pToStruct)->m_uContinueByte =  (uint32_t)  ((pFromData)[1] >> 6) & 0x03;
  (pToStruct)->m_uContinueByte |=  (uint32_t)  ((pFromData)[2] & 0x3f) << 2;
