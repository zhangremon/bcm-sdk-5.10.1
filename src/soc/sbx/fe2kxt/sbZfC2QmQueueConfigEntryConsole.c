/*
 * $Id: sbZfC2QmQueueConfigEntryConsole.c 1.1.34.3 Broadcom SDK $
...
sbZfC2QmQueueConfigEntry_Print(sbZfC2QmQueueConfigEntry_t *pFromStruct) {
  SB_LOG("C2QmQueueConfigEntry:: enforcepolicemarkings=0x%01x", (unsigned int)  pFromStruct->m_uEnforcePoliceMarkings);
  SB_LOG(" dropthresh2=0x%03x", (unsigned int)  pFromStruct->m_uDropThresh2);
  SB_LOG("\n");

  SB_LOG("C2QmQueueConfigEntry:: dropthresh1=0x%03x", (unsigned int)  pFromStruct->m_uDropThresh1);
  SB_LOG(" flowcontrolthresh2=0x%03x", (unsigned int)  pFromStruct->m_uFlowControlThresh2);
  SB_LOG("\n");

  SB_LOG("C2QmQueueConfigEntry:: flowcontrolthresh1=0x%03x", (unsigned int)  pFromStruct->m_uFlowControlThresh1);
  SB_LOG(" flowcontrolenable=0x%01x", (unsigned int)  pFromStruct->m_uFlowControlEnable);
  SB_LOG("\n");

  SB_LOG("C2QmQueueConfigEntry:: flowcontroltarget=0x%01x", (unsigned int)  pFromStruct->m_uFlowControlTarget);
  SB_LOG(" flowcontrolport=0x%03x", (unsigned int)  pFromStruct->m_uFlowControlPort);
  SB_LOG("\n");

  SB_LOG("C2QmQueueConfigEntry:: maxpages=0x%03x", (unsigned int)  pFromStruct->m_uMaxPages);
...
char *
sbZfC2QmQueueConfigEntry_SPrint(sbZfC2QmQueueConfigEntry_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
