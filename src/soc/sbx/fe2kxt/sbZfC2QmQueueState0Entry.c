/*
 * $Id: sbZfC2QmQueueState0Entry.c 1.1.34.4 Broadcom SDK $
...
sbZfC2QmQueueState0Entry_Pack(sbZfC2QmQueueState0Entry_t *pFrom,
  ...
  int i;
  int size = SB_ZF_C2QMQUEUESTATE0ENTRY_SIZE_IN_BYTES;
  ...
  (pToData)[2] |= ((pFrom)->m_uTailPtr) & 0xFF;
  (pToData)[3] |= ((pFrom)->m_uTailPtr >> 8) & 0x1f;

  /* Pack Member: m_uDepthInPages */
  (pToData)[0] |= ((pFrom)->m_uDepthInPages & 0x3f) <<2;
  (pToData)[1] |= ((pFrom)->m_uDepthInPages >> 6) &0xFF;
  ...
  return SB_ZF_C2QMQUEUESTATE0ENTRY_SIZE_IN_BYTES;
}
...
sbZfC2QmQueueState0Entry_Unpack(sbZfC2QmQueueState0Entry_t *pToStruct,
  ...
  (pToStruct)->m_uTailPtr =  (uint32_t)  (pFromData)[2] ;
  (pToStruct)->m_uTailPtr |=  (uint32_t)  ((pFromData)[3] & 0x1f) << 8;

  /* Unpack Member: m_uDepthInPages */
  (pToStruct)->m_uDepthInPages =  (uint32_t)  ((pFromData)[0] >> 2) & 0x3f;
  (pToStruct)->m_uDepthInPages |=  (uint32_t)  (pFromData)[1] << 6;
