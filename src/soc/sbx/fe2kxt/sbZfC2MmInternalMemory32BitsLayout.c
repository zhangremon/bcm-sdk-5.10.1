/*
 * $Id: sbZfC2MmInternalMemory32BitsLayout.c 1.1.34.4 Broadcom SDK $
...
sbZfC2MmInternalMemory32BitsLayout_Unpack(sbZfC2MmInternalMemory32BitsLayout_t *pToStruct,
  ...
  (pToStruct)->m_uProtectionBits0 =  (uint32_t)  ((pFromData)[8] >> 4) & 0x0f;

  /* Unpack Member: m_uData0 */
  (pToStruct)->m_uData0 =  (uint32_t)  ((pFromData)[4] >> 4) & 0x0f;
  (pToStruct)->m_uData0 |=  (uint32_t)  (pFromData)[5] << 4;
  (pToStruct)->m_uData0 |=  (uint32_t)  (pFromData)[6] << 12;
  (pToStruct)->m_uData0 |=  (uint32_t)  (pFromData)[7] << 20;
  (pToStruct)->m_uData0 |=  (uint32_t)  ((pFromData)[8] & 0x0f) << 28;

  /* Unpack Member: m_uProtectionBits1 */
  (pToStruct)->m_uProtectionBits1 =  (uint32_t)  ((pFromData)[4] ) & 0x0f;

  /* Unpack Member: m_uData1 */
  (pToStruct)->m_uData1 =  (uint32_t)  (pFromData)[0] ;
  (pToStruct)->m_uData1 |=  (uint32_t)  (pFromData)[1] << 8;
  (pToStruct)->m_uData1 |=  (uint32_t)  (pFromData)[2] << 16;
  (pToStruct)->m_uData1 |=  (uint32_t)  (pFromData)[3] << 24;
