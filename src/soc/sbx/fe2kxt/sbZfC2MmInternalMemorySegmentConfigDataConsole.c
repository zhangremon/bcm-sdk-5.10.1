/*
 * $Id: sbZfC2MmInternalMemorySegmentConfigDataConsole.c 1.1.34.3 Broadcom SDK $
...
sbZfC2MmInternalMemorySegmentConfigData_Print(sbZfC2MmInternalMemorySegmentConfigData_t *pFromStruct) {
  SB_LOG("C2MmInternalMemorySegmentConfigData:: resv0=0x%07x", (unsigned int)  pFromStruct->m_uResv0);
  SB_LOG(" wacc=0x%01x", (unsigned int)  pFromStruct->m_uWriteAccess);
  SB_LOG(" racc=0x%01x", (unsigned int)  pFromStruct->m_uReadAccess);
  SB_LOG("\n");

  SB_LOG("C2MmInternalMemorySegmentConfigData:: ding=0x%01x", (unsigned int)  pFromStruct->m_uDataIntegrity);
  SB_LOG(" trap=0x%01x", (unsigned int)  pFromStruct->m_uTrapNullPointer);
  SB_LOG(" wrap=0x%01x", (unsigned int)  pFromStruct->m_uWrap);
  SB_LOG("\n");

  SB_LOG("C2MmInternalMemorySegmentConfigData:: lmask=0x%04x", (unsigned int)  pFromStruct->m_uLimitMask);
...
char *
sbZfC2MmInternalMemorySegmentConfigData_SPrint(sbZfC2MmInternalMemorySegmentConfigData_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
