/*
 * $Id: sbZfC2PedDescriptor.c 1.1.34.4 Broadcom SDK $
...
#include "sbTypes.h"
#include "sbZfC2PedDescriptor.hx"
...
sbZfC2PedDescriptor_Pack(sbZfC2PedDescriptor_t *pFrom,
  ...
  int i;
  int size = SB_ZF_C2PEDDESCRIPTOR_SIZE_IN_BYTES;
  ...
  (pToData)[165] |= ((pFrom)->m_uBuffer & 0x01) <<7;
  (pToData)[166] |= ((pFrom)->m_uBuffer >> 1) &0xFF;
  (pToData)[167] |= ((pFrom)->m_uBuffer >> 9) & 0x1f;

  /* Pack Member: m_uExceptionIndex */
  (pToData)[165] |= ((pFrom)->m_uExceptionIndex & 0x7f);
  ...
  return SB_ZF_C2PEDDESCRIPTOR_SIZE_IN_BYTES;
}
...
sbZfC2PedDescriptor_Unpack(sbZfC2PedDescriptor_t *pToStruct,
  ...
  (pToStruct)->m_uContinueByte =  (uint32_t)  (pFromData)[171] ;

  /* Unpack Member: m_uHeaderLength */
  (pToStruct)->m_uHeaderLength =  (uint32_t)  (pFromData)[170] ;

  /* Unpack Member: m_uResv0 */
  (pToStruct)->m_uResv0 =  (uint32_t)  (pFromData)[168] ;
  (pToStruct)->m_uResv0 |=  (uint32_t)  (pFromData)[169] << 8;

  /* Unpack Member: m_uStreamSelector */
  (pToStruct)->m_uStreamSelector =  (uint32_t)  ((pFromData)[167] >> 5) & 0x07;

  /* Unpack Member: m_uBuffer */
  (pToStruct)->m_uBuffer =  (uint32_t)  ((pFromData)[165] >> 7) & 0x01;
  (pToStruct)->m_uBuffer |=  (uint32_t)  (pFromData)[166] << 1;
  (pToStruct)->m_uBuffer |=  (uint32_t)  ((pFromData)[167] & 0x1f) << 9;

  /* Unpack Member: m_uExceptionIndex */
  (pToStruct)->m_uExceptionIndex =  (uint32_t)  ((pFromData)[165] ) & 0x7f;

  /* Unpack Member: m_uSourceQueue */
  (pToStruct)->m_uSourceQueue =  (uint32_t)  (pFromData)[164] ;

  /* Unpack Member: m_uResv1 */
  (pToStruct)->m_uResv1 =  (uint32_t)  ((pFromData)[163] >> 4) & 0x0f;

  /* Unpack Member: m_uMirrorIndex */
  (pToStruct)->m_uMirrorIndex =  (uint32_t)  ((pFromData)[163] ) & 0x0f;

  /* Unpack Member: m_uCopyCount */
  (pToStruct)->m_uCopyCount =  (uint32_t)  ((pFromData)[161] >> 4) & 0x0f;
  (pToStruct)->m_uCopyCount |=  (uint32_t)  (pFromData)[162] << 4;

  /* Unpack Member: m_uCrcMode */
  (pToStruct)->m_uCrcMode =  (uint32_t)  ((pFromData)[161] >> 2) & 0x03;

  /* Unpack Member: m_uDropBit */
  (pToStruct)->m_uDropBit =  (uint32_t)  ((pFromData)[161] >> 1) & 0x01;

  /* Unpack Member: m_uEnqueue */
  (pToStruct)->m_uEnqueue =  (uint32_t)  ((pFromData)[161] ) & 0x01;

  /* Unpack Member: m_uDestinationQueue */
  (pToStruct)->m_uDestinationQueue =  (uint32_t)  (pFromData)[160] ;

  /* Unpack Member: m_uXferLength */
  (pToStruct)->m_uXferLength =  (uint32_t)  ((pFromData)[158] >> 2) & 0x3f;
  (pToStruct)->m_uXferLength |=  (uint32_t)  (pFromData)[159] << 6;

  /* Unpack Member: m_uPacketMarking */
  (pToStruct)->m_uPacketMarking =  (uint32_t)  ((pFromData)[158] ) & 0x03;

  /* Unpack Member: m_uResv3 */
  (pToStruct)->m_uResv3 =  (uint32_t)  ((pFromData)[157] >> 6) & 0x03;

  /* Unpack Member: m_uSourceFrameLength */
  (pToStruct)->m_uSourceFrameLength =  (uint32_t)  ((pFromData)[155] >> 7) & 0x01;
  (pToStruct)->m_uSourceFrameLength |=  (uint32_t)  (pFromData)[156] << 1;
  (pToStruct)->m_uSourceFrameLength |=  (uint32_t)  ((pFromData)[157] & 0x3f) << 9;
