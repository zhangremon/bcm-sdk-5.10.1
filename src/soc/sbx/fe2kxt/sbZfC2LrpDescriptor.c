/*
 * $Id: sbZfC2LrpDescriptor.c 1.1.34.4 Broadcom SDK $
...
#include "sbTypes.h"
#include "sbZfC2LrpDescriptor.hx"
...
sbZfC2LrpDescriptor_Pack(sbZfC2LrpDescriptor_t *pFrom,
  ...
  int i;
  int size = SB_ZF_ZFC2LRPDESCRIPTOR_SIZE_IN_BYTES;
  ...
  (pToData)[173] |= ((pFrom)->m_uBuffer & 0x01) <<7;
  (pToData)[174] |= ((pFrom)->m_uBuffer >> 1) &0xFF;
  (pToData)[175] |= ((pFrom)->m_uBuffer >> 9) & 0x1f;

  /* Pack Member: m_uExceptIndex */
  (pToData)[173] |= ((pFrom)->m_uExceptIndex & 0x7f);
  ...
  return SB_ZF_ZFC2LRPDESCRIPTOR_SIZE_IN_BYTES;
}
...
sbZfC2LrpDescriptor_Unpack(sbZfC2LrpDescriptor_t *pToStruct,
  ...
  (pToStruct)->m_uBuffer =  (uint32_t)  ((pFromData)[173] >> 7) & 0x01;
  (pToStruct)->m_uBuffer |=  (uint32_t)  (pFromData)[174] << 1;
  (pToStruct)->m_uBuffer |=  (uint32_t)  ((pFromData)[175] & 0x1f) << 9;

  /* Unpack Member: m_uExceptIndex */
  (pToStruct)->m_uExceptIndex =  (uint32_t)  ((pFromData)[173] ) & 0x7f;
