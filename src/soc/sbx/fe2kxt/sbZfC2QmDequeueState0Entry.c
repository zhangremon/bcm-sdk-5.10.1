/*
 * $Id: sbZfC2QmDequeueState0Entry.c 1.1.34.4 Broadcom SDK $
...
sbZfC2QmDequeueState0Entry_Unpack(sbZfC2QmDequeueState0Entry_t *pToStruct,
  ...
  (pToStruct)->m_uDrop =  (uint32_t)  ((pFromData)[4] >> 1) & 0x01;

  /* Unpack Member: m_uRemainingPagesHi */
  (pToStruct)->m_uRemainingPagesHi =  (uint32_t)  ((pFromData)[4] ) & 0x01;

  /* Unpack Member: m_uRemainingPagesLo */
  (pToStruct)->m_uRemainingPagesLo =  (uint32_t)  ((pFromData)[3] >> 2) & 0x3f;

  /* Unpack Member: m_uReplicantNumber */
  (pToStruct)->m_uReplicantNumber =  (uint32_t)  ((pFromData)[1] >> 6) & 0x03;
  (pToStruct)->m_uReplicantNumber |=  (uint32_t)  (pFromData)[2] << 2;
  (pToStruct)->m_uReplicantNumber |=  (uint32_t)  ((pFromData)[3] & 0x03) << 10;

  /* Unpack Member: m_uReplicantLength */
  (pToStruct)->m_uReplicantLength =  (uint32_t)  (pFromData)[0] ;
  (pToStruct)->m_uReplicantLength |=  (uint32_t)  ((pFromData)[1] & 0x3f) << 8;
