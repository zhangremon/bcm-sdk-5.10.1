/*
 * $Id: sbZfC2MmInternalMemory35BitsLayout.c 1.1.34.4 Broadcom SDK $
...
sbZfC2MmInternalMemory35BitsLayout_Pack(sbZfC2MmInternalMemory35BitsLayout_t *pFrom,
  ...
  int i;
  int size = SB_ZF_C2MMINTERNALMEMORY35BITSLAYOUT_SIZE_IN_BYTES;
  ...
  (pToData)[8] |= ((pFrom)->m_uProtectionBits0 & 0x01) <<7;
  ...
  (pToData)[8] |= ((pFrom)->m_uData0 >> 28) & 0x7f;

  /* Pack Member: m_uProtectionBits1 */
  (pToData)[4] |= ((pFrom)->m_uProtectionBits1 & 0x01) <<3;
  ...
  (pToData)[4] |= ((pFrom)->m_uData1 >> 32) & 0x07;

  return SB_ZF_C2MMINTERNALMEMORY35BITSLAYOUT_SIZE_IN_BYTES;
}
...
sbZfC2MmInternalMemory35BitsLayout_Unpack(sbZfC2MmInternalMemory35BitsLayout_t *pToStruct,
  ...
  (pToStruct)->m_uProtectionBits0 =  (uint32_t)  ((pFromData)[8] >> 7) & 0x01;

  /* Unpack Member: m_uData0 */
  COMPILER_64_SET((pToStruct)->m_uData0, 0,  (unsigned int) (pFromData)[4]);
  {
  ...
  (pToStruct)->m_uProtectionBits1 =  (uint32_t)  ((pFromData)[4] >> 3) & 0x01;

  /* Unpack Member: m_uData1 */
  COMPILER_64_SET((pToStruct)->m_uData1, 0,  (unsigned int) (pFromData)[0]);
  {
    VOL COMPILER_UINT64 *tmp0 = &(pToStruct)->m_uData1;
