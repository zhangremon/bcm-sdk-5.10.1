/*
 * $Id: sbZfC2PmProfileMemoryEntry.c 1.1.34.4 Broadcom SDK $
...
#include "sbTypes.h"
#include "sbZfC2PmProfileMemoryEntry.hx"
...
sbZfC2PmProfileMemoryEntry_Pack(sbZfC2PmProfileMemoryEntry_t *pFrom,
  ...
  int i;
  int size = SB_ZF_C2PMPROFILEMEMORYENTRY_SIZE_IN_BYTES;
  ...
  (pToData)[9] |= ((pFrom)->m_uType2 & 0x01) <<2;

  /* Pack Member: m_uLengthShift */
  (pToData)[8] |= ((pFrom)->m_uLengthShift & 0x01) <<7;
  (pToData)[9] |= ((pFrom)->m_uLengthShift >> 1) & 0x03;

  /* Pack Member: m_uCouplingFlag */
  (pToData)[8] |= ((pFrom)->m_uCouplingFlag & 0x01) <<6;

  /* Pack Member: m_uRfcMode */
  (pToData)[8] |= ((pFrom)->m_uRfcMode & 0x01) <<5;

  /* Pack Member: m_uCommitedBucketSize */
  (pToData)[8] |= ((pFrom)->m_uCommitedBucketSize & 0x1f);

  /* Pack Member: m_uColorBlind */
  (pToData)[7] |= ((pFrom)->m_uColorBlind & 0x01) <<7;

  /* Pack Member: m_uExcessBucketNoDecrement */
  (pToData)[7] |= ((pFrom)->m_uExcessBucketNoDecrement & 0x01) <<6;
  ...
  (pToData)[7] |= ((pFrom)->m_uExcessInformationRate >> 12) & 0x3f;

  /* Pack Member: m_uExcessBurstSize */
  (pToData)[4] |= ((pFrom)->m_uExcessBurstSize) & 0xFF;
  (pToData)[5] |= ((pFrom)->m_uExcessBurstSize >> 8) & 0x0f;

  /* Pack Member: m_uDropOnRed */
  (pToData)[3] |= ((pFrom)->m_uDropOnRed & 0x01) <<7;

  /* Pack Member: m_uCommittedBucketNoDecrement */
  (pToData)[3] |= ((pFrom)->m_uCommittedBucketNoDecrement & 0x01) <<6;

  /* Pack Member: m_uCommittedInformationRate */
  (pToData)[1] |= ((pFrom)->m_uCommittedInformationRate & 0x0f) <<4;
  (pToData)[2] |= ((pFrom)->m_uCommittedInformationRate >> 4) &0xFF;
  (pToData)[3] |= ((pFrom)->m_uCommittedInformationRate >> 12) & 0x3f;

  /* Pack Member: m_uCommittedBurstSize */
  (pToData)[0] |= ((pFrom)->m_uCommittedBurstSize) & 0xFF;
  (pToData)[1] |= ((pFrom)->m_uCommittedBurstSize >> 8) & 0x0f;

  return SB_ZF_C2PMPROFILEMEMORYENTRY_SIZE_IN_BYTES;
}
...
sbZfC2PmProfileMemoryEntry_Unpack(sbZfC2PmProfileMemoryEntry_t *pToStruct,
  ...
  (pToStruct)->m_uType2 =  (uint32_t)  ((pFromData)[9] >> 2) & 0x01;

  /* Unpack Member: m_uLengthShift */
  (pToStruct)->m_uLengthShift =  (uint32_t)  ((pFromData)[8] >> 7) & 0x01;
  (pToStruct)->m_uLengthShift |=  (uint32_t)  ((pFromData)[9] & 0x03) << 1;

  /* Unpack Member: m_uCouplingFlag */
  (pToStruct)->m_uCouplingFlag =  (uint32_t)  ((pFromData)[8] >> 6) & 0x01;

  /* Unpack Member: m_uRfcMode */
  (pToStruct)->m_uRfcMode =  (uint32_t)  ((pFromData)[8] >> 5) & 0x01;

  /* Unpack Member: m_uCommitedBucketSize */
  (pToStruct)->m_uCommitedBucketSize =  (uint32_t)  ((pFromData)[8] ) & 0x1f;

  /* Unpack Member: m_uColorBlind */
  (pToStruct)->m_uColorBlind =  (uint32_t)  ((pFromData)[7] >> 7) & 0x01;

  /* Unpack Member: m_uExcessBucketNoDecrement */
  (pToStruct)->m_uExcessBucketNoDecrement =  (uint32_t)  ((pFromData)[7] >> 6) & 0x01;
  ...
  (pToStruct)->m_uExcessInformationRate |=  (uint32_t)  ((pFromData)[7] & 0x3f) << 12;

  /* Unpack Member: m_uExcessBurstSize */
  (pToStruct)->m_uExcessBurstSize =  (uint32_t)  (pFromData)[4] ;
  (pToStruct)->m_uExcessBurstSize |=  (uint32_t)  ((pFromData)[5] & 0x0f) << 8;

  /* Unpack Member: m_uDropOnRed */
  (pToStruct)->m_uDropOnRed =  (uint32_t)  ((pFromData)[3] >> 7) & 0x01;

  /* Unpack Member: m_uCommittedBucketNoDecrement */
  (pToStruct)->m_uCommittedBucketNoDecrement =  (uint32_t)  ((pFromData)[3] >> 6) & 0x01;

  /* Unpack Member: m_uCommittedInformationRate */
  (pToStruct)->m_uCommittedInformationRate =  (uint32_t)  ((pFromData)[1] >> 4) & 0x0f;
  (pToStruct)->m_uCommittedInformationRate |=  (uint32_t)  (pFromData)[2] << 4;
  (pToStruct)->m_uCommittedInformationRate |=  (uint32_t)  ((pFromData)[3] & 0x3f) << 12;
