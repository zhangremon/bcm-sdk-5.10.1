/* 
 * $Id: qe2000_scoreboard.c 1.17.60.1 Broadcom SDK $
...
static uint32_t Qe2000_Debug = 0x00;
...
static uint32_t              Debug_pollIndex = 0;
static uint32_t              nTotalLost;
static uint32_t              nTotalFreed;
static uint32_t              nTotalDogs;
static uint32_t              nShortIntervals;
...
typedef struct _scoreboard_control_s {
    ...
    int32_t    nBuffersSbBuf;
    int32_t    nBuffers;
    int32_t    nBufsDelta;
    int32_t    bIsHalfBus;
    int32_t    bIs256MbMemPart;
    int32_t    bIsFaulted;
    int32_t    nScoreboardPolls;
    int32_t    nMinPollIntvlInUsecs;
    int32_t    nCurrentAgeValue;
    uint8_t     *pScoreboardBuf;
...
STATIC int _sbx_qe2000_scoreboard_read( int unit, uint32_t *num_buffers_to_free) ;

STATIC int _sbx_qe2000_get_buf_info(int  unit, uint32_t startBufAddr,
        uint32_t endBufAddr, uint32_t *pMask, uint32_t *pNbrBufs);
...

{
    ...
    soc_cm_print("RegVal %d assigning %d buffers\n",RegVal, nBuffers);
#endif
...
_sbx_qe2000_poll_intvl(sal_usecs_t *pLastTime, sal_usecs_t *pCurTime)
{
    ...
    nLastUsecs = *pLastTime;
    nCurUsecs = *pCurTime;

    /* consistency checks */
    if (nCurUsecs < nLastUsecs) {
#ifdef SCOREBOARD_DEBUG
        /* update the last poll time, This will result in only 1 poll being missed */

        BCM_FABRIC_DEBUG_ERR(("Sb Wraparound Detected\n"));
        
        soc_cm_print("- Prev 0x%llx  Cur 0x%llx ... Wraparound detected\n", nLastUsecs, nCurUsecs);
#endif
    ...
    return((((nCurUsecs - nLastUsecs)) > 0xFFFFFFFF) ? 0xFFFFFFFF : (nCurUsecs - nLastUsecs));
}
...
_sbx_qe2000_set_churner_state(int unit, sbBool_t bIsEnabled)
{
    ...
    BCM_FABRIC_DEBUG_VERB(("SetChurnerState (%s)\n",
...
STATIC int
_sbx_qe2000_qm_fb_write(int unit, uint32_t nBufAddress)
{
    int                        status = SB_FAB_STATUS_OK;
    uint32_t                   uData;
    uint32_t                   ulTimeOut;
    ...
    while(ulTimeOut--) {
        ...
        if (SAND_HAL_GET_FIELD(KA, QM_FB_CACHE_ACC_CTRL, ACK, uData) == 1) {
            ...
            if (SAND_HAL_GET_FIELD(KA, QM_ERROR2, FB_TAIL_CACHE_OVERFLOW, uData) == 1) {
                BCM_FABRIC_DEBUG_ERR(("QM_ERROR2, "
                                      " FB_TAIL_CACHE_OVERFLOW Set\n"));
#ifdef SCOREBOARD_DEBUG_2
                soc_cm_print("FB_TAIL_CACHE_OVERFLOW\n");
#endif
        ...
        uData = SAND_HAL_READ(unit, KA, QM_ERROR2);
        if (SAND_HAL_GET_FIELD(KA, QM_ERROR2, FB_TAIL_CACHE_OVERFLOW, uData) == 1) {
            BCM_FABRIC_DEBUG_ERR(("QM_ERROR2, FB_TAIL_CACHE_OVERFLOW Set\n"));
    ...
    BCM_FABRIC_DEBUG_ERR(("Timeout waiting for QM_FB_CACHE_ACC_CTRL ACK\n"));
    return(SB_FAB_STATUS_TARGET_INDIRECT_MEMORY_TIMEOUT); /* need to add specific error code */
}


STATIC int
_sbx_qe2000_qm_fb_read(int unit, uint32_t *pBufAddr)
{
    int              status = SB_FAB_STATUS_OK;
    uint32_t                   uData;
    uint32_t                   ulTimeOut;
    ...
    while(ulTimeOut--) {
        ...
        if (SAND_HAL_GET_FIELD(KA, QM_ERROR2, FB_HEAD_CACHE_UNDERFLOW, uData) == 1) {
            BCM_FABRIC_DEBUG_ERR(("QM_ERROR2, FB_HEAD_CACHE_UNDERFLOW Set\n"));
...
STATIC sbBool_t
_sbx_qe2000_is_buf_valid(int unit, uint32_t BufAddr)
{
    uint32_t                   nMaskBit;
...
STATIC int
_sbx_qe2000_scoreboard_recover_buffer(int unit, uint32_t nBufAddress)
{
    sbFabStatus_t              status = SB_FAB_STATUS_OK;

    /* Add this buffer into the Qm FreeBuffer Cache */
    if (Qe2000_Debug & 1) { /* for debugging */
        BCM_FABRIC_DEBUG_VERB(("Addr: 0x%x\n", nBufAddress));
    }

/*    soc_cm_print("Add 0x%x\n", nBufAddress); */
...
{
    int              status = SB_FAB_STATUS_OK;
    uint32_t         nBufs, nBufAddr, nRdBufAddr, RegVal;
    sbBool_t         bMarkedFound = FALSE;

    SB_FAB_ENTER();

    BCM_FABRIC_DEBUG_VERB(("Initializing Free Buffer Cache\n"));
#ifdef SCOREBOARD_DEBUG
    soc_cm_print("Initializing Free Buffer Cache\n");
#endif
    ...
    if (bHdCacheEmpty == FALSE) {
        ...
        if (SAND_HAL_GET_FIELD(KA, QM_ERROR2, FB_HEAD_CACHE_UNDERFLOW, RegVal) == 1) {
            ...
            BCM_FABRIC_DEBUG_WARN(("Poping Byffers from Free Buffer"
            ...
            for (; (SAND_HAL_GET_FIELD(KA, QM_ERROR2, FB_HEAD_CACHE_UNDERFLOW, RegVal) == 0); ) {
                ...
                if( status ) soc_cm_print("nRdBufAddr 0x%x status 0x%x\n", nRdBufAddr, status);
        ...
        if ((nBufs % QE2000_MARKED_BUFFER_CHECK_THRESHOLD) == 0) {
            ...
            for (; (SAND_HAL_GET_FIELD(KA, QM_ERROR2, FB_HEAD_CACHE_UNDERFLOW, RegVal) == 0); ) {
                ...
                if (nRdBufAddr == QE2000_MARKED_BUFFER_NUMBER) {
                    BCM_FABRIC_DEBUG_VERB(("Found Marked Buffer\n"));
                    bMarkedFound = TRUE;
                    break;
                }
...
{
    uint32_t                   hdCacheWptr, hdCacheRptr, nHeadDepth, hdCacheData;
    uint32_t                   tailCacheWptr, tailCacheRptr, nTailDepth, tailCacheData;
    uint32_t                   extMemWptr, extMemRptr, nExtMemDepth, extMemData1, extMemData2;
    ...
    BCM_FABRIC_DEBUG_WARN(("tailCacheData: 0x%x, hdCacheData: 0x%x,"
...
STATIC int
_sbx_qe2000_queue_state(int unit, uint8_t *pState, sbBool_t bIsSave)
{
    int                  status = SB_FAB_STATUS_OK;
    uint32_t                       nQueue, nOper;
    uint32_t                       uData0, uData1, uData2, uData3, nIndex, nBit;


    BCM_FABRIC_DEBUG_VERB(("QueueState(%s)\n",
    ...
    BCM_FABRIC_DEBUG_VERB(("QueueState(%s): Oper: 0x%x\n",
...
_sbx_qe2000_init_free_buffer_cache(int unit)
{
    ...
    uint8_t                       *pState;
    uint32_t                       RegVal, nFbHeadCacheVal, hdCacheWptr, hdCacheRptr;
    ...
    if ((SAND_HAL_GET_FIELD(KA, QM_ERROR2, FB_HEAD_CACHE_UNDERFLOW, RegVal) != 1) &&
        ...
        BCM_FABRIC_DEBUG_ERR(("Consistency Check Failed, Head Cache"
    ...
    if (pState == NULL) {
        BCM_FABRIC_DEBUG_ERR(("QueueState, Malloc Failed\n"));
        return(SB_FAB_STATUS_USER_MALLOC_FAILED);
    }
    sal_memset((uint8_t *)pState, 0, ((SB_FAB_DEVICE_QE2000_NUM_QUEUES / 8) + 1));
    ...
    if (rc != SB_FAB_STATUS_OK) {
        BCM_FABRIC_DEBUG_WARN(("Marking Checkboard State as Faulted\n"));
...
STATIC int
_sbx_qe2000_scoreboard_process(int unit, uint32_t *pNbrBufFreed)
{
    int                            status = SB_FAB_STATUS_OK;
    uint32_t                       i, j, nBufs;
    uint8_t                        nUsed;
    uint32_t                       nLostBuffer;
    uint32_t                       nAddr, nBuffers;
    uint8_t                       *pScoreboardBuf;
    uint32_t                       startBufAddr, endBufAddr, nMask, nbrBufs;
    int                            flag = 0;
...
#ifdef SCOREBOARD_DEBUG
    soc_cm_print("scoreboard process nBuffers 0x%x Scoreboard at 0x%x\n", nBuffers, (unsigned int) pScoreboardBuf);
#endif
    ...
    for (i = 0, startBufAddr = 0; (i < (nBuffers - SCOREBOARD_CONTROL(unit)->nBufsDelta)) && (status == SB_FAB_STATUS_OK); ) {
        ...
        for (j = 0; (j < nBufs) && (status == SB_FAB_STATUS_OK); j++) {
            ...
            if ((nUsed & (1 << j)) == 0) {
                ...
                /* recover buffer */
#ifdef SCOREBOARD_DEBUG
                if (nAddr < 0x20) soc_cm_print("* RECOVER * 0x%x:0x%x nUsed 0x%x 0x%x 0x%x 0x%x\n",
                ...
                if (status == SB_FAB_STATUS_OK) {
                    ...
                    soc_cm_print("recover error 0x%x\n", status);
                }
            }
        }
...
#ifdef SCOREBOARD_DEBUG
    if (flag) soc_cm_print("Scoreboard Freed Buffers %d, Lost Buffers %d \n",nTotalFreed, nTotalLost);
#endif
...

{
    int         rv = SOC_E_NONE;
    uint32_t    RegVal;
    uint32_t    fb_buf_alloc_per_ts;
    uint32_t    RegLnaConfigVal, AgeVal;
    uint32_t    OldAge, NewAge;
    uint64_t    ullAgeVal;
    uint64_t    nScoreBoardTimePeriodNs, nAgingTimePeriodNs;
    uint32      minPollTmp = 0;
    uint32      nBufTmp = 0;
    sbBool_t    bAgingJustConfigured = FALSE; 
    sal_usecs_t curScoreboardTime;
    uint32_t    nPollIntvlDiffInUsecs, nBufsFreed = 0, nBufToFree;
...
#ifdef SCOREBOARD_DEBUG
    soc_cm_print("start _sbx_qe2000_scoreboard_check....\n");
#endif
    ...
    if ((fb_buf_alloc_per_ts == QE2000_BUF_ALLOC_PER_TS_DISABLE) ||
        ...
        BCM_FABRIC_DEBUG_ERR(("Scoreboarding(unit %d) not performed,"
...
#ifdef SCOREBOARD_DEBUG
        soc_cm_print("fb_buf_alloc_per_ts %d\n", fb_buf_alloc_per_ts);
#endif
    }
    ...
    if ((SCOREBOARD_CONTROL(unit)->nBuffers == 0) || (SCOREBOARD_CONTROL(unit)->pScoreboardBuf == SB_FAB_NULL_POINTER)) {
        BCM_FABRIC_DEBUG_VERB(("Scoreboard(%d) First Poll\n", unit));
        AgeVal = SAND_HAL_READ(unit, KA, QM_CONFIG3);
        BCM_FABRIC_DEBUG_VERB(("Scoreboard(%d) AgeVal: 0x%x(%d)\n",
                           unit, AgeVal, AgeVal));
        ...
        if ( (AgeVal == 0) || (AgeVal == QE2000_HW_AGE_SCALER_DEF_VAL) ||
            ...
            if ((AgeVal == 0) || (AgeVal == QE2000_HW_AGE_SCALER_DEF_VAL) ) {
                if (SAND_HAL_GET_FIELD(KA, QS_LNA_CONFIG, TME_MODE, RegLnaConfigVal) == 0) { /* FIC Mode */
                    BCM_FABRIC_DEBUG_VERB(("QE (%d) in FIC Mode\n", unit));
                    AgeVal = QE2000_HW_AGE_FIC_SCALER_DEF_VAL;
                }
                else { /* TME MODE */
                    BCM_FABRIC_DEBUG_VERB(("QE ( %d) in TME Mode\n", unit));
                    AgeVal = QE2000_HW_AGE_TME_SCALER_DEF_VAL;
                }
                BCM_FABRIC_DEBUG_VERB(("Scoreboard(%d) Updated"
                                       " AgeVal: 0x%x(%d)\n",
                                       unit, AgeVal, AgeVal));
            ...
            /* enable aging */
#ifdef SCOREBOARD_DEBUG
            soc_cm_print("* Scoreboard(%d) Enable Aging 0x%x \n", unit, AgeVal);
#endif

            BCM_FABRIC_DEBUG_VERB(("Scoreboard(%d) Enable Aging\n", unit));
...
#ifdef SCOREBOARD_DEBUG
    soc_cm_print("calc poll _sbx_qe2000_scoreboard_check....\n");
#endif
	...
	nBufTmp = (SCOREBOARD_CONTROL(unit)->nBuffers / fb_buf_alloc_per_ts) + 1;
        nScoreBoardTimePeriodNs = nBufTmp;
        nScoreBoardTimePeriodNs *= SB_FAB_MAX_TIMESLOT_IN_NS;
        nAgingTimePeriodNs = AgeVal * 4 * 15;
	if (nScoreBoardTimePeriodNs > nAgingTimePeriodNs) {
        ...
        soc_cm_print("Scoreboard(%d) Time(nsecs): (0x%llx) Aging Time(nsecs): (0x%llx)\n",
                unit, nScoreBoardTimePeriodNs,  nAgingTimePeriodNs);
        BCM_FABRIC_DEBUG_VERB(("Scoreboard(%d) Time(nsecs): (0x%llx)"
                               " Aging Time(nsecs): (0x%llx)\n",
                               unit, nScoreBoardTimePeriodNs,
                               nAgingTimePeriodNs));
        BCM_FABRIC_DEBUG_VERB(("Scoreboard(unit %d) Current Time"
                              " usecs: (%d) (0x%x)\n", unit,
        ...
        if (SCOREBOARD_CONTROL(unit)->nMinPollIntvlInUsecs < QE2000_MIN_SCOREBOARD_POLL_INTVL_US) {
            BCM_FABRIC_DEBUG_VERB(("Scoreboard(%d) Setting Default"
                                   " Time: (%d)\n", unit,
                            SCOREBOARD_CONTROL(unit)->nMinPollIntvlInUsecs));
        ...
        BCM_FABRIC_DEBUG_VERB(("Scoreboard(%d) FinalTime in"
                               " usecs (0x%x) secs (0x%x)\n", unit,
                               SCOREBOARD_CONTROL(unit)->nMinPollIntvlInUsecs,
                SCOREBOARD_CONTROL(unit)->nMinPollIntvlInUsecs/(1000*1000)));

#ifdef SCOREBOARD_DEBUG
        soc_cm_print("Scoreboard(%d) FinalTime in usecs (0x%x) secs (0x%x)\n",
                unit, SCOREBOARD_CONTROL(unit)->nMinPollIntvlInUsecs,
                SCOREBOARD_CONTROL(unit)->nMinPollIntvlInUsecs/(1000*1000));
        soc_cm_print("modes _sbx_qe2000_scoreboard_check.... AgingTP 0x%llx, AgeVal 0x%x\n", nAgingTimePeriodNs, AgeVal);
#endif
        ...
        if (SCOREBOARD_CONTROL(unit)->bIsHalfBus == TRUE) {
            ...
            if ( (SCOREBOARD_CONTROL(unit)->nBuffers != QE2000_BUF_AVL_HALFBUS_512_CONFIG) &&
                ...
                BCM_FABRIC_DEBUG_ERR(("Scoreboarding(nId %d) not performed,"
	 ...
	 memory, we need 131072 bits, or 16384 bytes, or 4096 32-bit words*/
        ...
        if (SCOREBOARD_CONTROL(unit)->pScoreboardBuf == NULL) {
            BCM_FABRIC_DEBUG_ERR(("Scoreboard(%d) Malloc Failed\n", unit));
            return(SB_FAB_STATUS_USER_MALLOC_FAILED);
        }
#ifdef SCOREBOARD_DEBUG
        soc_cm_print("Scoreboard(%d) IsHalfBus: %d Is256MbPart: %d nBuffers: 0x%x NSbBuffers: 0x%x\n",
        ...
        soc_cm_print("**********   Aging 0x%x   **********************\n", AgeVal);
        soc_cm_print("**********   Scoreboard Alloc 0x%x  size 0x%x ********\n",
		     (unsigned int) SCOREBOARD_CONTROL(unit)->pScoreboardBuf,
		     QE2000_MARKED_BUFFER_NUMBER + 1);
#endif
        BCM_FABRIC_DEBUG_VERB(("Scoreboard(%d) IsHalfBus: %d Is256MbPart: %d"
        ...
        if(OldAge != NewAge) {
            ...
            soc_cm_print("Aging Old 0x%x New 0x%x\n", OldAge, NewAge);
	    ...
	    nScoreBoardTimePeriodNs *= SB_FAB_MAX_TIMESLOT_IN_NS;
            ...
            ullAgeVal = AgeVal;
            nAgingTimePeriodNs = ullAgeVal * 4 * 15;
		...
		SCOREBOARD_CONTROL(unit)->nMinPollIntvlInUsecs = minPollTmp + 1;
            ...
            soc_cm_print("Scoreboard(%d) FinalTime in usecs (0x%x) secs (0x%x) Default:%d\n",
                    unit, SCOREBOARD_CONTROL(unit)->nMinPollIntvlInUsecs,
                    SCOREBOARD_CONTROL(unit)->nMinPollIntvlInUsecs/(1000*1000), QE2000_MIN_SCOREBOARD_POLL_INTVL_US);

            BCM_FABRIC_DEBUG_VERB(("Scoreboard(%d) FinalTime in usecs"
                                   " (0x%x) secs (0x%x)\n", unit,
                               SCOREBOARD_CONTROL(unit)->nMinPollIntvlInUsecs,
                  SCOREBOARD_CONTROL(unit)->nMinPollIntvlInUsecs/(1000*1000)));

            soc_cm_print("************ Aging Reconfigured Was 0x%x, Now 0x%x. Updating Interval 0x%x. ********",
...
#ifdef SCOREBOARD_DEBUG_2
        soc_cm_print("Scoreboard(%d) Aging just configured      \n", unit);
#endif
        BCM_FABRIC_DEBUG_VERB(("Scoreboard(%d) Aging just configured\n",
                               unit));
    ...
    if (SCOREBOARD_CONTROL(unit)->lastScoreboardTime == 0) {
        /* update poll time */
#ifdef SCOREBOARD_DEBUG
        soc_cm_print("*******************Scoreboard(%d) First Poll ******************\n", unit);
#endif
        BCM_FABRIC_DEBUG_VERB(("Scoreboard(%d) First Poll\n", unit));
        Debug_StartScoreboardTime = SCOREBOARD_CONTROL(unit)->lastScoreboardTime;
        SCOREBOARD_CONTROL(unit)->lastScoreboardTime = sal_time_usecs();
    ...
    /* determine if the poll frequency is correct */
     ...
     if (nPollIntvlDiffInUsecs < (SCOREBOARD_CONTROL(unit)->nMinPollIntvlInUsecs )) {
         ...
         if(nPollIntvlDiffInUsecs) {
              soc_cm_print("Poll interval not met cur %d prev %d diff %d min %d\n",
              ...
              BCM_FABRIC_DEBUG_WARN(("Scoreboard(nId %d) Poll Interval"
                                     " not met\n", unit));
              nShortIntervals++;
         }
         return(rv);
...
#ifdef SCOREBOARD_DEBUG
     soc_cm_print("interval cur 0x%x diff 0x%x interval %d\n", curScoreboardTime,
             nPollIntvlDiffInUsecs,  SCOREBOARD_CONTROL(unit)->nMinPollIntvlInUsecs);
#endif

     SCOREBOARD_CONTROL(unit)->nScoreboardPolls++; /* for debugging */

     BCM_FABRIC_DEBUG_VERB(("Scoreboard INVOKED (%d)\n",
                            SCOREBOARD_CONTROL(unit)->nScoreboardPolls));
#ifdef SCOREBOARD_DEBUG
     soc_cm_print("Scoreboard INVOKED (%d)\n", SCOREBOARD_CONTROL(unit)->nScoreboardPolls);
#endif
     ...
     if (rv != SB_FAB_STATUS_OK) {
         BCM_FABRIC_DEBUG_ERR(("qe2000_scoreboard_read(), Error: 0x%x"
                               " Unit %d\n", rv, unit));
         goto ErrHandle;
     }

#ifdef SCOREBOARD_DEBUG
     soc_cm_print("read_scoreboard returns  nBufToFree 0x%x\n", nBufToFree);
#endif
     ...
     if (SAND_HAL_GET_FIELD(KA, QS_LNA_CONFIG, TME_MODE, RegVal) == 0) {
         ...
         if (SAND_HAL_GET_FIELD(KA, SC_ERROR, SOT_WATCHDOG_TIMEOUT, RegVal) == 1) {
...
#ifdef SCOREBOARD_DEBUG
             soc_cm_print("Scoreboard Clear Error Error 0x%x\n", RegVal);
             soc_cm_print("ATTEMPT to clear Watchdog\n");
#endif
             BCM_FABRIC_DEBUG_WARN(("SOT WatchDog Expired (0x%x),"
             ...
             BCM_FABRIC_DEBUG_WARN(("SOT WatchDog Cleared\n"));
             bForceBufInit = TRUE;
...
#ifdef SCOREBOARD_DEBUG
     soc_cm_print("bForceBufInit %d nBufToFree %d > (nBuffers %d - nBufsDelta %d - ACCESS_SZ %d)\n",
...
#ifdef SCOREBOARD_DEBUG
         soc_cm_print("****** Initialize Free Buffer Cache ******* \n");
#endif
         BCM_FABRIC_DEBUG_WARN(("Initialize Free Buffer Cache,"
                                " unit (%d), Reason: %s\n",
                                unit, ((bForceBufInit == TRUE) ?
                                    "Watchdog Cleared" : "Flush Tail Cache")));
         rv = _sbx_qe2000_init_free_buffer_cache(unit);
         if (rv != SB_FAB_STATUS_OK) {
             BCM_FABRIC_DEBUG_ERR(("Scoreboarding(nId %d) Stopped,"
...
#ifdef SCOREBOARD_DEBUG
     soc_cm_print("Process Scoreboard \n");
#endif
     rv = _sbx_qe2000_scoreboard_process(unit, &nBufsFreed);
#ifdef SCOREBOARD_DEBUG
    soc_cm_print("Process Scoreboard.. freed 0x%x status 0x%x\n", nBufsFreed, rv);
#endif
    if (rv != SB_FAB_STATUS_OK) {
#if 0
        rv = (nBufsFreed > 0) ? SB_FAB_STATUS_OK : rv;
#endif /* 0 */
        BCM_FABRIC_DEBUG_ERR(("sbx_qe2000_scoreboard_process,"
                              " Error: 0x%x nId %d\n", rv, unit));

        BCM_FABRIC_DEBUG_WARN(("Initialize Free Buffer Cache, (%d),"
                               " Reason: ERROR in Processing scoreboard"
                               " Buffer\n", unit));
        rv = _sbx_qe2000_init_free_buffer_cache(unit);
        if (rv != SB_FAB_STATUS_OK) {
            BCM_FABRIC_DEBUG_ERR(("Scoreboarding(%d) Stopped,"
                                  " Free Buffer Init failed, error: 0x%x\n",
                                  unit, rv));
            rv = SB_FAB_STATUS_INIT_FAILED;
        }
    ...
    BCM_FABRIC_DEBUG_VERB(("Scoreboard INVOKED (%d), FREED: 0x%x\n",
                           SCOREBOARD_CONTROL(unit)->nScoreboardPolls,
                           nBufsFreed));
...
#ifdef SCOREBOARD_DEBUG
    soc_cm_print("end _sbx_qe2000_scoreboard_check....\n");
#endif
...
STATIC int
_sbx_qe2000_scoreboard_read( int unit, uint32_t *num_buffers_to_free) 

{
    uint32_t   uData0, uData1, uData2, uData3;
    uint32_t   i, j,  nBufs, nAddr, nBuffers;
    uint32_t   start_buf_addr, end_buf_addr, nMask;
    uint32_t   nbrBufs;
    int        rv = SOC_E_NONE;
    ...
    *(uint32_t *)(pScoreboardBuf+4095) = 0x01234567;
     ...
     */
    if (cnt >= 50) {
	BCM_FABRIC_DEBUG_ERR(("scoreboard read failed after %d milliseconds, RegVal 0x%x\n", cnt, regval2));
	/* Double-check with the marker */
	if (*(uint32_t *)(pScoreboardBuf+4095) == 0x01234567) {
	  SCOREBOARD_CONTROL(unit)->bIsFaulted = TRUE;
	...
	BCM_FABRIC_DEBUG_WARN(("scoreboard read DMA succeeded\n"));
    }
#endif
        ...
        if ( (uData0 & nMask) != (0xFF & nMask) ) { /* Buffer Lost */
            ...
            if (bBufsNotFreed == FALSE) { /* for debugging */
                ...
                if ( (start_buf_addr + nBufs) < SCOREBOARD_CONTROL(unit)->nBuffersSbBuf)  {
                    BCM_FABRIC_DEBUG_WARN(("Buffers Lost, Addr: 0x%x"
                    ...
                    BCM_FABRIC_DEBUG_WARN(("Buffers Lost (END CONDITION),"
                                           " Addr: 0x%x Data: 0x%x,"
                                           " 1stBuf: 0x%x\n",
            ...
            if (Qe2000_Debug & 1) { /* for debugging */
                BCM_FABRIC_DEBUG_WARN(("Addr: 0x%x Data: 0x%x",
                                       (nAddr * 8), (uData0 & nMask)));
            }
...
ErrHandle:
#ifdef SCOREBOARD_DEBUG
    soc_cm_print("End scoreboard read\n");
#endif
    return(rv);
}
...
soc_sbx_qe2000_scoreboard_init(int unit)
{
    ...
    if (soc_property_get(unit, spn_SOC_SCOREBOARD_ENABLE, 0) == 0) {
        BCM_FABRIC_DEBUG_VERB(("Scoreboarding Disabled\n"));
        return(0);
        /* Get the environment variables */

    } else {
        BCM_FABRIC_DEBUG_VERB(("soc_scoreboard_enable set - Enabling Scoreboard\n"));
    }


    if ((env_interval = soc_property_get(unit, spn_SOC_SCOREBOARD_INTERVAL, 2000000)) == 0) {
        BCM_FABRIC_DEBUG_VERB(("soc_scoreboard_interval not set or 0 - using default %d\n", interval));
    } else {
        BCM_FABRIC_DEBUG_VERB(("soc_scoreboard_interval set %d. Applying... \n", env_interval));
    ...
    if (SCOREBOARD_CONTROL(unit) == NULL) {
        ...
        if (SOC_SUCCESS(rv)) {
               BCM_FABRIC_DEBUG_VERB((" _soc_sbx_scoreboard_set_remove_all?\n"));
        }
        if (SOC_FAILURE(rv)) {
            SCOREBOARD_UNLOCK(unit);
            return rv;
...
#ifdef SCOREBOARD_DEBUG
    soc_cm_print("*************** New Interval %d  ************\n",new_interval);
#endif

    if (new_interval > interval) {
        BCM_FABRIC_DEBUG_VERB(("Requested interval too low %d. Adjusting %d\n", interval, new_interval));
        interval = new_interval;
    }
...
#ifdef SCOREBOARD_DEBUG
    soc_cm_print("Starting scoreboard thread flags 0x%x Interval %d\n", flags, interval);
#endif
...
#ifdef SCOREBOARD_DEBUG
    soc_cm_print("soc_scoreboard_init: unit=%d rv=%d\n", unit, rv);
#endif
...
_soc_sbx_scoreboard_thread(void *unit_vp)
{
    ...
    soc_cm_debug(DK_VERBOSE, "soc_scoreboard_thread: unit=%d\n", unit);
...
#ifdef SCOREBOARD_DEBUG
    soc_cm_print("soc_scoreboard_thread: unit=%d, interval %d\n", unit, collect->interval);
#endif
    ...
    while ((interval = collect->interval) != 0) {
...
#ifdef SCOREBOARD_DEBUG
        soc_cm_print( "soc_scoreboard_thread: sleep %d\n", interval);
#endif
...
#ifdef SCOREBOARD_DEBUG
        soc_cm_print( "soc_scoreboard_thread: slept %d \n", collect->interval);
#endif
        ...
        now = sal_time_usecs();
#ifdef SCOREBOARD_DEBUG
         soc_cm_print("Thread 0x%x 0x%x delta %d  err %d\n", now, collect->cur, now - collect->cur, err);
#endif
...
#ifdef SCOREBOARD_DEBUG
         soc_cm_print("Thread 0x%x 0x%x err %d\n", collect->prev, collect->cur, err);
#endif

        if (!err) {
#ifdef SCOREBOARD_DEBUG
            soc_cm_print("SCOREBOARD Lock\n");
#endif
	    BCM_SBX_LOCK(unit);
            SCOREBOARD_LOCK(unit);
...
#ifdef SCOREBOARD_DEBUG
            soc_cm_print("scoreboard check returned nbrBuf 0x%x intvl 0x%x\n", nbr_buffers, new_interval);
#endif
	    ...
	    BCM_SBX_UNLOCK(unit);
#ifdef SCOREBOARD_DEBUG
            soc_cm_print("SCOREBOARD Unlock\n");
#endif
            if (rv != SOC_E_NONE ) {
                soc_cm_print("DEBUG Error on Scoreboard READ\n");
                soc_cm_debug(DK_ERR,
...
#ifdef SCOREBOARD_DEBUG
                soc_cm_print("DEBUG Scoreboard Check done\n");
#endif
        ...
        if (err) {
            if (++err_count == soc_property_get(unit,
                ...
                soc_cm_debug(DK_ERR, "soc_scoreboard_thread: Too many errors\n");
                rv = SOC_E_INTERNAL;
                goto done;
            }
        } else if (err_count > 0) {
...
#ifdef SCOREBOARD_DEBUG
        soc_cm_print("SB While at end Interval %d TotLost %d TotFreed %d Total Watchdog Resets %d\n\n",
...
#ifdef SCOREBOARD_DEBUG
    soc_cm_print("SCOREBOARD_DEBUG thread done\n");
#endif
    if (rv < 0) {
        soc_cm_debug(DK_ERR,
    ...
    soc_cm_debug(DK_COUNTER, "soc_scoreboard_thread: exiting\n");
...
soc_sbx_scoreboard_start(int unit, uint32 flags, int interval)
{
    ...
    if (SOC_FAILURE(rv)) {
        SCOREBOARD_UNLOCK(unit);
        soc_cm_print("Scoreboard Stop failure\n");
        return rv;
    }
    ...
    if (interval == 0) {
        SCOREBOARD_UNLOCK(unit);
        soc_cm_print("Scoreboard Interval 0\n");
        return SOC_E_NONE;
    }
    ...
    if ((collect->trigger == NULL) || (collect->intr == NULL)) {
        soc_cm_debug(DK_ERR, "soc_scoreboard_start: sem create failed\n");
        SCOREBOARD_UNLOCK(unit);
        soc_cm_print("Scoreboard semaphore failure 0\n");
        return SOC_E_INTERNAL;
    }
    ...
    if (interval != 0) {
...
#ifdef SCOREBOARD_DEBUG
        soc_cm_print("DEBUG: Start Scoreboard Thread %s. Interval %d \n",
        ...
        if (collect->thread_id == SAL_THREAD_ERROR) {
            collect->interval = 0;
            soc_cm_debug(DK_ERR, "soc_scoreboard_start: thread create failed\n");
            rv = SOC_E_INTERNAL;
        } else {
            soc_cm_debug(DK_VERBOSE, "soc_scoreboard_start: complete \n");
#ifdef SCOREBOARD_DEBUG
            soc_cm_print("soc_sbx_scoreboard_start: complete %d \n",
                    collect->interval );
#endif
        }
    }
...
soc_sbx_scoreboard_stop(int unit)
{
    ...
    soc_cm_debug(DK_COUNTER, "soc_scoreboard_stop: unit=%d\n", unit);

#ifdef SCOREBOARD_COUNTER
    soc_cm_print("soc_sbx_scoreboard_stop: unit=%d\n", unit);
#endif
    ...
    soc_cm_debug(DK_COUNTER, "soc_scoreboard_stop: unit=%d rv=%d\n",
                 unit, rv);
...
soc_qe2000_pkt_age_get(int unit, int *value)
{
    uint32_t    AgeVal;
        ...
        soc_cm_print("soc_qe2000_pkt_age_get 0%x\n", AgeVal);
        *value = AgeVal;
...
soc_qe2000_pkt_age_set(int unit, int value)
{
        ...
        soc_cm_print("soc_qe2000_pkt_age_set 0%x\n", AgeVal);
        SAND_HAL_WRITE(unit, KA, QM_CONFIG3, AgeVal);
...

{
    ...
    *pShortIntervals = nTotalDogs;
    soc_cm_print("Scoreboard stats BufErrs 0x%x, Buf Frees 0x%x SOT Watchdogs 0x%x Short Intervals 0x%x Scoreboard Ticks 0x%x\n",
