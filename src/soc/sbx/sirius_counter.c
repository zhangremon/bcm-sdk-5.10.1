/* 
 * $Id: sirius_counter.c 1.26.2.2 Broadcom SDK $
...
soc_sbx_sirius_process_global_stats(int unit)
{
      ...
      soc_sbx_sirius_cs_update_t *stat_update = NULL;
	...
	stat_update = (soc_sbx_sirius_cs_update_t *) &sir->cs.gbl_stats[gbl_idx * 2];
...
soc_sbx_sirius_process_fd_drop_stats(int unit, int clear)
{
	  ...
	  for (fd_idx = 0; fd_idx <= SOC_MEM_INFO(unit, EG_FD_PER_PORT_DROP_COUNT1m).index_max; fd_idx++) {
	      ...
	      stat_update = (soc_sbx_sirius_cs_update_t *) &sir->cs.fd_drop[drop1][fd_idx * 2];
	  ...
	  for (fd_idx = 0; fd_idx <= SOC_MEM_INFO(unit, EG_FD_PER_PORT_DROP_COUNT2m).index_max; fd_idx++) {
	      ...
	      stat_update = (soc_sbx_sirius_cs_update_t *) &sir->cs.fd_drop[drop2][fd_idx * 2];
...
soc_sbx_sirius_process_slq_stats(int unit)
{
	...
	for (slq_q_idx = 0; slq_q_idx < 16; slq_q_idx++) {
	  ...
	  stat_update = (soc_sbx_sirius_cs_update_t *) &sir->cs.slq_stats[slq_cntr_addr*2];
...
soc_sbx_sirius_process_cs_dma_fifo(int unit)
{
	...
	if ((stat_update = (soc_sbx_sirius_cs_update_t *) 
 ...
 */
int
soc_sbx_sirius_get_segment(uint32 unit, uint8 cu, int32_t *rSegment)
{
    soc_sbx_sirius_config_t *sir = SOC_SBX_CFG_SIRIUS(unit);
    uint8      segment = 0;
...
soc_sbx_sirius_free_segment(uint32 unit, uint32 segment)
{
    ...
    if (COMPILER_64_IS_ZERO(sir->cs.segment[segment].cntrIdMap) == FALSE) {
      SOC_DEBUG_PRINT((DK_COUNTER, "CS:Free segment unit %d segment %d cntrIdMap %llu\n",
		       unit, segment, sir->cs.segment[segment].cntrIdMap));
      return SOC_E_FAIL;
    }

    if (sir->cs.segment[segment].segmentSize != 0) {
      SOC_DEBUG_PRINT((DK_COUNTER, "CS:Free segment unit %d segment %d Segment Size %d\n",
...
cleanProvisionedGroup(uint32 unit, uint32 segment)
{
	...
	if ((grpCfg->segment == segment) &&
	  ...
	  if (curr_cfg_grp < 32)
	    COMPILER_64_SET(temp, 0, (uint64) 1 << curr_cfg_grp);
	  else
	    COMPILER_64_SET(temp, 1 << (curr_cfg_grp - 32), 0);
	  COMPILER_64_AND(segInfo->cntrIdMap, COMPILER_64_NOT(temp));
...
soc_sbx_sirius_create_group(uint32 unit, uint32 segment, uint32 cu_num, uint32 cntrId, uint32 size)
{
	...
	continue;
      ...
      SOC_DEBUG_PRINT((DK_COUNTER, "CS:Create Group Unit %d New Segment %d cu_num %d cntrId %d size %d\n",
	...
	for (group = 0; group < CS_NUM_GROUPS; group++) {
      ...
      SOC_DEBUG_PRINT((DK_COUNTER, "CS:Unit %d: Unable to Allocate Group Config %d for Segment %d\n",
...
got_group:
      ...
      SOC_DEBUG_PRINT((DK_COUNTER, "CS:Unit %d: Segment %d range %d - %d: Brick %d Group %d\n", 
		       unit, segment, 
    ...
    if (segInfo->ullCountOrig == NULL) {
      ...
      if (segInfo->ullCountOrig == NULL) {
	cleanProvisionedGroup(unit, segment);
	SOC_DEBUG_PRINT((DK_COUNTER,"CS: Unit %d failed to allocate ullCount[%d] \n", unit, segment));
	return SOC_E_MEMORY;
      }
...
soc_sbx_sirius_init_group(uint32 unit, uint32 segment, uint32 cntr_id, uint32 num_cntrs)
{
    ...
    if (!nInitDone) {
	SOC_ERROR_PRINT((DK_ERR, "CS BLOCK Initialization done timeout\n"));
    ...
    SOC_DEBUG_PRINT((DK_COUNTER, "CS:Group %d has been initialized on unit %d\n", segment, unit));
    return SOC_E_NONE;
}
...
soc_sbx_sirius_remove_group(uint32 unit, uint32 segment, uint32 cu_num, uint32 cntrId, uint32 size)
{
	...
	continue;
      ...
      SOC_DEBUG_PRINT((DK_COUNTER, "CS:Remove Group Unit %d Segment %d cu_num %d cntrId %d size %d\n",
		       unit, segment, cu_num, cntrId, size));
...
flush:
    ...
    SOC_DEBUG_PRINT((DK_COUNTER, "CS:Flush Segment Unit %d Segment %d Counter %d Range %d\n", 
    ...
    if (sir->cs.flags & CS_FLUSHING) {
      SOC_DEBUG_PRINT((DK_COUNTER, "CS:Flush Segment Unit %d BUSY\n", unit));
      return SOC_E_BUSY;
    }
    ...
    if ((rv = WRITE_CS_MANUAL_EJECT_CONFIG0r(unit,regval)) < 0) {
      SOC_DEBUG_PRINT((DK_COUNTER,
		       "CS: unable to write CS_MANUAL_EJECT_CONFIG0r register for unit %d with %d\n",
		       unit, regval));
      return rv;
    }
    ...
    if ((rv = WRITE_CS_MANUAL_EJECT_CONFIG1r(unit,regval)) < 0) {
      SOC_DEBUG_PRINT((DK_COUNTER,
		       "CS: unable to write CS_MANUAL_EJECT_CONFIG1r register for unit %d with %d\n",
		       unit, regval));
      return rv;
    }
    ...
    if ((rv = WRITE_CS_MANUAL_EJECT_CONFIG2r(unit,regval)) < 0) {
      SOC_DEBUG_PRINT((DK_COUNTER,
		       "CS: unable to write CS_MANUAL_EJECT_CONFIG2r register for unit %d with %d\n",
		       unit, regval));
      return rv;
    }
    ...
    if ((rv = WRITE_CS_MANUAL_EJECT_CTRLr(unit, regval)) < 0) {
      SOC_DEBUG_PRINT((DK_COUNTER,
		       "CS: unable to write CS_MANUAL_EJECT_CTRLr register for unit %d with %x\n",
		       unit, regval));
      return rv;
    }
    ...
    soc_cm_debug(DK_COUNTER + DK_VERBOSE,
		 "soc_sbx_sirius_flush_segment: sleep %d\n", to_usec);
    ...
    if ((rv = READ_CS_MANUAL_EJECT_CTRLr(unit, &regval)) < 0) {
      ...
      if (!nInitDone) {
	SOC_ERROR_PRINT((DK_ERR, "MANUAL EJECT done timeout\n"));
      } else {
...
#define REAL_COUNTER
        ...
        *puCounterBase++ = uByteCount;
        if (uPacketCount || uByteCount ) {
            SOC_DEBUG_PRINT((DK_COUNTER + DK_VERBOSE,"Q %d C %d p %d b %d\n", set, nCounter, uPacketCount, uByteCount));
        }
#else
                            ...
                            uint64 *val, int *width)
{
    ...
    SOC_DEBUG_PRINT((DK_VERBOSE + DK_COUNTER, "_soc_sirius_counter_qm_read set %d cntr %d enb %d\n",
    ...
    if (SOC_FAILURE(rv)) {
        SOC_DEBUG_PRINT((DK_ERR, "sirius counter read failure\n"));
        return rv;
    }
    ...
    for ( counter_index = 0 ; counter_index < SIRIUS_COUNTER_QM_COUNT; counter_index++) {
        if ( data[counter_index]) {
            SOC_DEBUG_PRINT((DK_COUNTER, " Count %d:%d: %08x, ", set, counter_index, data[counter_index]));
        } else {
            SOC_DEBUG_PRINT((DK_COUNTER + DK_VERBOSE, " Count %d:%d: %08x, ", set, counter_index, data[counter_index]));
        }
                                ...
                                int counter, uint64 val)
{
    ...
    rv = soc_sbx_counter_set(unit, block, set, counter, 0);
