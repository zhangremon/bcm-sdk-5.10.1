 ...
 * ===========================================================
 *
 * WORKING REVISION: $Id: sbQe2000ElibVlanMem.c 1.4.112.1 Broadcom SDK $
 *
 ...
 */
static sbQe2000ElibBfMemAvl_pst sbQe2000ElibVlanMemAllocDLLSearch(SB_QE2000_ELIB_CONTEXT_PST pEp, uint32_t ulAddr);
...
static void sbQe2000ElibVlanMemListTree(shr_avl_entry_t* entry,int m)
{
	...
	if(entry->right) sbQe2000ElibVlanMemListTree(entry->right, m+1);
	while(n--) SB_LOGV3("   ");
	SB_LOGV3("%d (%d)\n",pBfMemAvl->nNumLines, entry->balance);
	if(entry->left) sbQe2000ElibVlanMemListTree(entry->left, m+1);
}
...
sbElibStatus_et sbQe2000ElibVlanMemListOutput( SB_QE2000_ELIB_HANDLE Handle )
{
    ...
    pBfMemAvl = pEp->pBfMemList;
    SB_LOGV3("Listing VLAN Memory Nodes:\n");
    nIdx = 0;
    while(NULL != pBfMemAvl)
    {
        SB_LOGV3("%i:  Lines: %d  Addr: 0x%x Used: %i This: %p Prev: %p Next %p\n",
                  nIdx++,
                  pBfMemAvl->nNumLines,
                  pBfMemAvl->ulAddr & ~BF_MEM_USED,
