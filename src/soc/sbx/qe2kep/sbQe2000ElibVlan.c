 ...
 * =================================================
 *
 * WORKING REVISION: $Id: sbQe2000ElibVlan.c 1.2.226.1 Broadcom SDK $
 *
...
typedef struct sb_qe2000_elib_vit_s {
    ...
    uint32_t ulPtr;                  /**< Indirection record ptr.                  */
 ...
 */
static sbElibStatus_et sbQe2000ElibVITSet( SB_QE2000_ELIB_CONTEXT_PST pEp, uint32_t ulVLAN, SB_QE2000_ELIB_VIT_ST Entry );
 ...
 * @return           Status.  0 incates success.  Non-zero indicates failure.
 */
static sbElibStatus_et sbQe2000ElibVITGet( SB_QE2000_ELIB_CONTEXT_PST pEp, uint32_t ulVLAN, SB_QE2000_ELIB_VIT_PST pEntry );


static sbElibStatus_et sbQe2000ElibScIdxCountAccumulate( SB_QE2000_ELIB_CONTEXT_PST pEp, uint32_t ulScIdx, uint32_t *pulCounts );
static int sbQe2000ElibGetPopulationCount(uint64_t ullPortEnable);
static uint32_t sbQe2000ElibGetPopulationCount32( uint32_t uliValue );


static uint32_t sbQe2000ElibGetPopulationCount32( uint32_t uliValue )
{
  uint32_t uliResult = uliValue;
...
int sbQe2000ElibGetPopulationCount(uint64_t ullPortEnable)
{
    uint32_t res0, res1;

    res0 = sbQe2000ElibGetPopulationCount32((uint32_t)(ullPortEnable & 0xFFFFFFFF));
    res1 = (uint32_t)( (ullPortEnable >> 32) & 0xFFFFFFFF);
    res1 = sbQe2000ElibGetPopulationCount32(res1);
    return( res0 + res1 );
                                  ...
                                  SB_QE2000_ELIB_VIT_CMAP_T tCmap)
{
    ...
    uint32_t aulVRT[SB_ZF_SB_QE2000_ELIB_VRT_ENTRY_SIZE/sizeof(uint32_t)];
    VrtPtr_t tVrtPtr;
    uint64_t ullPortEnable;
    uint64_t ullPopMask;
    void *pTmpVoid;
    sbElibStatus_et nStatus;
    int nNumLines;
    ...
    if( (SB_QE2000_ELIB_VIT_CMAP_0_NO_PORT != tCmap) && (SB_QE2000_ELIB_VIT_CMAP_1_NO_PORT != tCmap) )
    {
        ...
        for( nPort = 0; nPort < pEp->nPorts; nPort++ )
        {
            ...
            if(SB_QE2000_ELIB_VRT_PS_UNMANAGED != tPortState)
            {
                nNumLines++;
                nNumPorts++;
                ullPortEnable |= ((uint64_t)1 << nPort);
            }
        }
        ...
        if( (1 == pEp->nOnePcwEnable) || (48 > pEp->nPorts) )
        {
            if(SB_QE2000_ELIB_VRT_PS_UNMANAGED != tPortState)
            {
                ullPortEnable |= ((uint64_t)1 << 48);
                ullPortEnable |= ((uint64_t)1 << 49);
                nNumLines += 2;
                nNumPorts += 2;
            }
        }
     ...
     */
    ullPopMask = ((1 << 24) -1);
    ullPopMask &= ullPortEnable;
    nPcw1Count = sbQe2000ElibGetPopulationCount(ullPortEnable & ullPopMask);
    nPcw1Count += (ullPortEnable >> 48) & 0x1;
    nPcw1Count += (ullPortEnable >> 49) & 0x1;
    if(1 == (nPcw1Count % 2))
    {
        nNumLines++;
        bDummyLine = TRUE;
                                  ...
                                  SB_QE2000_ELIB_VRT_PS_T tPortState )
{
        ...
        (SB_QE2000_ELIB_VRT_PS_UNMANAGED != tPortState) )
    {
         ...
         */
        pVrtMap->ullPortEnable |= (uint64_t)1 << ulPort;
        pVrtMap->ulNumPorts++;
             ...
             (SB_QE2000_ELIB_VRT_PS_UNMANAGED == tPortState)  )
    {
        ...
        pVrtMap->ullPortEnable &= ~(1 << ulPort);
        pVrtMap->ulNumPorts--;
        if( (SB_QE2000_ELIB_VIT_CMAP_0_NO_PORT != pVrtMap->tCmap) &&
            (SB_QE2000_ELIB_VIT_CMAP_1_NO_PORT != pVrtMap->tCmap) )
        {
    ...
    if( -1 != nAdd)
    {
           ...
           (SB_QE2000_ELIB_VIT_CMAP_1_NO_PORT != pVrtMap->tCmap) )
        {
                ...
                (SB_QE2000_ELIB_VIT_CMAP_1_NO_PORT != pVrtMap->tCmap) )
            {
                ...
                ullPopMask = ( ((uint64_t)1 << ulPort) - 1);
                nPort = sbQe2000ElibGetPopulationCount(ullPopMask & pVrtMap->ullPortEnable);
                ...
                if( 1 == nAdd )
                {
                    pTmpCounterShadow[nCountIdx].ullPktCount = 0;
                    pTmpCounterShadow[nCountIdx].ullByteCount = 0;
                    nCountIdx++;
                }
                else
                {
                ...
                ullPopMask = ~( ((uint64_t)1 << (ulPort + 1)) - 1 );
                ullPopMask &= (uint64_t)1 << ulPort;
    ...
    sbZfSbQe2000ElibVRT_Pack(&pVrtMap->sZfVrt, (uint8_t*)aulVRT, 0);
    ...
    if( -1 == nAdd )
    {
           ...
           (SB_QE2000_ELIB_VIT_CMAP_1_NO_PORT != pVrtMap->tCmap) )
        {
             ...
             * vice versa.  for now, just whack the count to zero
             */
            /* Address DEFECT 22280 */
            ullPopMask = (((uint64_t)1 << ulPort) -1);
            ullPopMask &= pVrtMap->ullPortEnable;
            nPort = sbQe2000ElibGetPopulationCount(pVrtMap->ullPortEnable & ullPopMask);
            if( SB_QE2000_ELIB_VIT_CMAP_MAP0 == pVrtMap->tCmap )
            {
            ...
            pVrtMap->pCntrShadow[nPort].ullPktCount = 0;
            pVrtMap->pCntrShadow[nPort].ullByteCount = 0;
        }
    }
...
sbElibStatus_et sbQe2000ElibVlanRecordDelete( SB_QE2000_ELIB_HANDLE Handle, uint32_t ulScIdx )
{
    ...
    SB_QE2000_ELIB_VIT_ST sVitEntry;
    sbDmaMemoryHandle_t tDmaHdl;
    uint32_t aulDetachScIdx[SBQE2000_ELIB_SCIDX_MAX_K];
    int nDetachIdx;
    sbElibStatus_et nStatus;
    int i;
                              ...
                              SB_QE2000_ELIB_COUNTER_PST pCounts)
{
    ...
    pCounts->ullPktCount  = 0;
    pCounts->ullByteCount = 0;
    ...
    if( (SB_QE2000_ELIB_CNT_CLASS0 <= tCntIdx) && (SB_QE2000_ELIB_CNT_CLASS15 >= tCntIdx) )
    {
        ...
        if( (pEp->nCMapClassCnt[nCmap] >= tCntIdx) && (tCntIdx < pEp->nCMapClassCnt[nCmap]) )
        {
            ...
            if(TRUE == bClrOnRd)
            {
                pVrtMap->pCntrShadow[tCntIdx].ullPktCount = 0;
                pVrtMap->pCntrShadow[tCntIdx].ullByteCount = 0;
            }
        }
        else
        {
    ...
    if( 0 == ( (1 << nPortIdx) & pVrtMap->ullPortEnable ) )
    {
        /*
    ...
    if(24 > nPortIdx)
    {
        ullPopMask = ((1 << nPortIdx) - 1);
    }
    else if(48 == nPortIdx)
    {
        ullPopMask = ( ((1 << 24) - 1) | ((uint64_t)1 << 48));
    }
    else if(49 == nPortIdx)
    {
        ullPopMask = ( ((1 << 24) - 1) | ((uint64_t)1 << 49));
    }
    else
    {
        ullPopMask = ((1 << nPortIdx) - 1) | ((uint64_t)1 << 49) | ((uint64_t)1 << 48);
    }

    /*
     * Get the actual index into the shadow table
     */
    nPortIdx = sbQe2000ElibGetPopulationCount(pVrtMap->ullPortEnable & ullPopMask);
    ...
    if(TRUE == bClrOnRd)
    {
        pVrtMap->pCntrShadow[nPortIdx].ullPktCount = 0;
        pVrtMap->pCntrShadow[nPortIdx].ullByteCount = 0;
    }
                                 ...
                                 SB_QE2000_ELIB_COUNTER_ST aCounts[SB_QE2000_ELIB_MAX_COUNTERS_K])
{
    ...
    for( i = 0; i < nCounts; i++)
    {
        ...
        if(TRUE == bClrOnRd)
        {
            pVrtMap->pCntrShadow[i].ullPktCount = 0;
            pVrtMap->pCntrShadow[i].ullByteCount = 0;
        }
    }
...
static sbElibStatus_et sbQe2000ElibScIdxCountAccumulate( SB_QE2000_ELIB_CONTEXT_PST pEp, uint32_t ulScIdx, uint32_t *pulCounts )
{
    ...
    uint64_t ullPopMask;
    uint32_t aulCntr[(SB_QE2000_ELIB_NUM_PORTS_K + SB_QE2000_ELIB_NUM_CLASSES_K) * 2];
    VrtPtr_t tVrtPtr;
    int nMemIdx;
    int i;
    sbElibStatus_et nStatus;
     ...
     */
    SB_MEMSET(aulCntr, 0, sizeof(uint32_t) * (SB_QE2000_ELIB_NUM_PORTS_K + SB_QE2000_ELIB_NUM_CLASSES_K) * 2);
    ...
    for( i = 0; i < nNumLines; i++)
    {
         ...
         * NOTE: In the case
         */
        sbZfSbQe2000ElibPCTSingle_Unpack(&sZfCount, (uint8_t *)&aulCntr[nMemIdx], 0);
        pVrtMap->pCntrShadow[i].ullPktCount += sZfCount.m_PktClass;
        pVrtMap->pCntrShadow[i].ullByteCount += sZfCount.m_ByteClass;
        nMemIdx += 2;
    }
     ...
     */
    ullPopMask = (1 << 24) - 1;
    ullPopMask |= ( ((uint64_t)1 << 48) | ((uint64_t)1 << 49) );
    nNumLinesFirstPCW = sbQe2000ElibGetPopulationCount(ullPopMask & pVrtMap->ullPortEnable);
    ...
    if( 0 == pEp->nOnePcwEnable )
    {
        ullPopMask =  ~(((uint64_t)1 << 48) | ((uint64_t)1 << 49) | ((1 << 24) - 1));
        nNumLinesSecondPCW = sbQe2000ElibGetPopulationCount(ullPopMask & pVrtMap->ullPortEnable);
        /* Address DEFECT 2280 */
...
sbElibStatus_et sbQe2000ElibVlanCountReset( SB_QE2000_ELIB_CONTEXT_PST pEp )
{
    ...
    for( ulScIdx = 0; ulScIdx <  SBQE2000_ELIB_SCIDX_MAX_K; ulScIdx++ )
    {
        ...
        {
            pVrtMap->pCntrShadow[nIdx].ullPktCount  = 0;
            pVrtMap->pCntrShadow[nIdx].ullByteCount = 0;
        }
    }
