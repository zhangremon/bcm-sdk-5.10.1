...
/*
 * $Id: bm9600_init.c 1.92.2.6 Broadcom SDK $
 *
...
#define BM9600_LOG(format, args...) do{printf(format, ##args); } while(0)
#else
#define BM9600_LOG(format, args...)
#endif

#define BM9600_ERR(format, args...) do{printf(format, ##args); } while(0)
...
int hwBm9600NmIngressRankerWrite( const bm9600InitParams_t *pInitParams, uint32 uAddress, uint32 uData ) {
  ...
  if (rv) {
      BM9600_ERR("error(%d) returned from soc_bm9600_NmIngressRankerWrite()\n", rv);
  }
  return rv;
}
...
int hwBm9600NmEgressRankerWrite( const bm9600InitParams_t *pInitParams, uint32 uAddress, uint32 uData ) {
    ...
    if (rv) {
	BM9600_ERR("error(%d) returned from soc_bm9600_NmEgressRankerWrite()\n", rv);
    }
    return rv;
}
...
int hwBm9600InaRandomNumGenWrite(const bm9600InitParams_t *pInitParams, uint32 uInstance, uint32 uAddress, uint32 uData ) {
    ...
    if (rv) {
	BM9600_ERR("InaRandomNumGenWrite Failed:  rv=%d\n", rv);
    }
    return rv;
}
...
int hwBm9600NmRandomNumGenWrite(const bm9600InitParams_t *pInitParams,  uint32 uAddress, uint32 uData ) {
    ...
    if (rv) {
	BM9600_ERR("NmRandomNumGenWrite Failed:  rv=%d\n", rv);
    }
    return rv;
}
...
int hwBm9600EsetSet(int nUnit, uint32 uEset, uint64 uuLowNodesMask, uint32 uHiNodesMask, uint32 uMcFullEvalMin, uint32 uEsetFullStatusMode)
{
    ...
    uint8_t                   data[(SB_ZF_FAB_BM9600_NMEMTENTRY_SIZE_IN_BYTES + 2)];
    uint32                    uSbRv;
    ...
    eset_info.m_uuEsetMember0 = uLowPhyNodesMask1; 
    eset_info.m_uuEsetMember0 = eset_info.m_uuEsetMember0 << 32; 
    eset_info.m_uuEsetMember0 |= (uLowPhyNodesMask0);
...
int hwBm9600InitSiStep0(int nNum, const bm9600InitParams_t *pInitParams){
    ...
    BM9600_LOG( "Bringing up Si%d Step0 \n", nNum);
...
hwBm9600InitSiStep1(int nNum, const bm9600InitParams_t *pInitParams){
    ...
    BM9600_LOG( "Bringing up Si%d Step1 \n", nNum);
	...
	if ( !uAck ) {
	    BM9600_ERR( "Error - no PLL lock for SI%d\n", nNum);
	    rv = SOC_E_TIMEOUT;
	}
    }
    return rv;
...
hwBm9600InitInaStep0(int nNum, const bm9600InitParams_t *pInitParams){
	...
	if ( pInitParams->ina[nNum].bIsEnabled ) {
	    BM9600_LOG( "Bringing up Ina %0d Step0\n", nNum);

	    if (pInitParams->bEnableEcc == 0){
		BM9600_LOG( "Backdoor allowed disable ECC/parity check for INA(%d)\n",nNum);
...
hwBm9600InitInaStep1(int nNum, const bm9600InitParams_t *pInitParams){
	...
	if ( pInitParams->ina[nNum].bIsEnabled ) {
	    ...
	    BM9600_LOG( "Bringing up Ina %d Step1\n", nNum);
	    if (pInitParams->nm.bInitRankerTables) {

		BM9600_LOG( "Initialize RandomGen Memory for INA %d. \n", nNum);
		...
		uAck = SAND_HAL_GET_FIELD(PL, INA_CONFIG, INIT_DONE, uValue);
              ...
              if ( !uAck ) {
		BM9600_ERR( "Timeout waiting for INA %d Init Done.\n", nNum);
		rv = SOC_E_TIMEOUT;
		return rv;
              }
            }
	...
	if ( pInitParams->bInaSysportMap == 1 ) {
		...
		if (!uAck) {
		    BM9600_ERR( "Error broadcast writing INA Sysport memory uAddress=0x%08x\n", uSysPort);
		    rv = SOC_E_TIMEOUT;
		    return rv;
		}
	    }
...
void hwBm9600InitFoStep0(const bm9600InitParams_t *pInitParams){
    ...
    for( i = 7; i <= 12; i++ ){
	BM9600_LOG(  "Writing FO_CONFIG%d with 0x%08x\n", i, pInitParams->fo.uForceLinkSelect[i-7] );
    }
    ...
    BM9600_LOG(  "Writing FO_CONFIG17 with 0x%08x\n", pInitParams->fo.uNumNullGrants );
...
hwBm9600InitFoStep1(const bm9600InitParams_t *pInitParams){
    ...
    BM9600_LOG(  "Writing FO_CONFIG18 with 0x%08x\n", uFoConfig18 );
...
hwBm9600InitBwStep0(const bm9600InitParams_t *pInitParams){
	...
	if ( pInitParams->bw.bBandwidthAllocationEnable ) {
		...
		BM9600_LOG( "Allow Kamino %d + 2 timeslots to process demand", uDemandTimeslots);
	    }
...
int
hwBm9600WredTableUpdate(uint32_t BaseAddr, uint32_t addr, uint32_t write_value)
{
    int rv = SOC_E_NONE;
    uint32_t nData;
    soc_timeout_t timeout;
    ...
    if (!uAck) {
	BM9600_ERR("%s:  Error writing Wred Table, addr(0x%x)\n", FUNCTION_NAME(), (uint32)addr);
	rv = SOC_E_TIMEOUT;
	return rv;
    }
    return(rv);
...
hwBm9600InitPi(const bm9600InitParams_t *pInitParams){
	...
	if (uIdentification != EXPECTED_IDENTIFICATION) {
	    BM9600_ERR("Did not get PI_REVISION::IDENTIFICATION of 0x%08x (got 0x%08x)\n",
...
{
    const uint32 NUM_LINKS_PER_ENABLE_REGISTER = 16;
    const uint32 NUM_IPRT_ENABLE_REGS = BM9600_NUM_LINKS / NUM_LINKS_PER_ENABLE_REGISTER;
    uint32 uIprtEnables[NUM_IPRT_ENABLE_REGS];
    ...
    uint32 uReg;
    uint32 uPort;
    uint32 uEprtPassThru[NUM_EPRT_PASSTHRU_REGS];
...
hwBm9600InitStep1(const bm9600InitParams_t *pInitParams) {
    ...
    int nAi = 0;
    int nSi = 0;
    uint32_t uQuadEnableDoneMask = 0;
    uint32_t uBaseSiInQuad;
    uint32_t uSiConfig2;
    int rv;
...
hwBm9600InitStep2(const bm9600InitParams_t *pInitParams)
{
	...
	if (SOC_SBX_CFG_BM9600(pInitParams->m_nBaseAddr)->bElectArbiterReconfig == TRUE) {
        ...
        if (bSuccess != SOC_E_NONE) {
            BM9600_ERR("ERROR: %s, Failed to init NM Sysport Array Memory row %d. \n",
    ...
    for (nIna = 0; nIna < BM9600_NUM_LINKS; nIna++) {
      if (pInitParams->ina[nIna].bIsEnabled) {
        for (sysport = 0; sysport < BM9600_MAX_NUM_SYSPORTS; sysport++) {
          ...
          if (bSuccess != SOC_E_NONE) {
            BM9600_ERR("ERROR: %s, Failed to init NM Sysport mapping table for sysport %d. \n",
                       __FUNCTION__, (uint32) sysport);
            return bSuccess;
          }
        }
      }
	...
	if (SOC_SBX_CFG_BM9600(pInitParams->m_nBaseAddr)->bElectArbiterReconfig == TRUE) {
        ...
        if (bSuccess != SOC_E_NONE) {
            BM9600_ERR("ERROR: %s, Failed to init portset link table row %d. \n",
                     FUNCTION_NAME(), (uint32) portset);
            return bSuccess;
        }
    }
	...
	if (SOC_SBX_CFG_BM9600(pInitParams->m_nBaseAddr)->bElectArbiterReconfig == TRUE) {
        ...
        if (bSuccess != SOC_E_NONE) {
            BM9600_ERR("ERROR: %s, Failed to init portset info table row %d. \n",
                     FUNCTION_NAME(), (uint32) portset);
            return bSuccess;
        }
    }
	...
	bSuccess = soc_bm9600_InaEsetPriClear(pInitParams->m_nBaseAddr, nIna);
        if (bSuccess != SOC_E_NONE) {
            BM9600_ERR("ERROR: %s, Failed to init Eset priority table for ina %d. \n",
...
void GetDefaultBmInitParams(int unit, bm9600InitParams_t *pInitParams) {
    ...
    /* TODO: This routine should not be invoked for an XBAR only device */
    ...
    /*  FO_CONFIG18.AC_TS_TO_GRANT_OFFSET = timeslot-5  */
    /*  FO_CONFIG18.AC_TS_TO_NM_TS_OFFSET = 17  */

    pInitParams->fo.uAcTsToGrantOffset = (  pInitParams->uTimeslotSizeNs / BM9600_BW_CLOCK_PERIOD_200MHZ ) - 7;
    ...
    * for now, setting all timeslot sizes to same value - would want to optimize this per link up */
	...
	if (uIndex == 1) {
	    ...
	    pInitParams->fo.uForceLinkActive[uIndex] = 0x3C0000;
	} else {
	    pInitParams->fo.uForceLinkActive[uIndex] = 0;
	}
    }
...
uint32 CheckInterruptReg(int nOffset, uint32 uValue, BOOL bCheck, BOOL bClear, BOOL bReturnMasked /* TRUE */ ){
  ...
  if( bCheck && ( (uData & ~uMask) != uValue) ) {
    ...
    if( uData ) {
      BM9600_LOG( "  %s data=0x%x (mask=0x%x) \n", _T sName, uData, uMask);
      BM9600_LOG( "    fields: %s\n",
                _T ZFoldString(m_RegSet.TranslateInterruptData(nOffset, uData, uMask, REGSET_SET_FIELDS), 25, "           "));
    }
  }
  ...
  if ( bClear && uData ) {
    ...
    if( (uReturnValue & ~uMask) != 0x0 ) {
      BM9600_ERR("%s CANNOT clear (return value: 0x%08x)\n",_T sName, uReturnValue);
    } else {
      BM9600_LOG( "  %s cleared\n", _T sName);
    }
  }
  if( bReturnMasked ) {
    uData = uData & ~uMask;
...
void hwBm9600TraceErrorRegs(char *sBlockName, BOOL bClear, BOOL bUnMask ) {
  BM9600_LOG( " called for block %s, Clear:%d, UnMask:%d\n", _T sBlockName, bClear, bUnMask);
    ...
    while(iter.GetNext((void*&)uAddress, (void*&)pRegItem)) {
      ...
      if ( pRegItem->m_bInterrupt ) {
        ...
        if( uRegData != 0 ) {
          ...
          BM9600_LOG("Trace non-zero interrupt register %s (%s)\n",
        ...
        if (bUnMask){
          ...
          if( uRegData != 0 ) { /* If mask isn't cleared, clear it */
            ...
            BM9600_LOG("Trace clear interrupt mask register %s (%s)\n",
...
void TraceInterruptReg(int nOffset, uint32 uValue, BOOL bClear) {
  ...
  BM9600_LOG( "Trace of register 0x%08x=0x%08x started\n", nOffset, uValue);
  BM9600_LOG( "%s", _T sTop);
