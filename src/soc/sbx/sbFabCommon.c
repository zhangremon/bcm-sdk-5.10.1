/*
 * $Id: sbFabCommon.c 1.50.20.2 Broadcom SDK $
				  ...
				  sbLinkSpecificDriverConfig_t *pSpecificConfig)
{
	...
	soc_cm_print("WARNING: Maximum drive strength setting is %d for hypercore, using this value instead of requested(%d)\n",
	...
	soc_cm_print("WARNING: Maximum drive equalization setting is %d for hypercore, using this value instead of requested(%d)\n",
		       ...
		       sbLinkThresholdConfig_t *pLinkThresholdConfig)
{
	...
	BCM_FABRIC_DEBUG_ERR(("Link threshold index should be between 0"
                              " and 100, value (0%d)\n", uLinkThresholdIndex));
...
static int32_t
GetLinkImplementationType(sbFabUserDeviceHandle_t devHandle, int32_t nLink) {
    SB_FAB_ENTER();
	...
	default:
            BCM_FABRIC_DEBUG_ERR(("Device type error, not an SBX device\n"));
            return -1;
    }
}
                      ...
                      sbLinkSpecificDriverConfig_t *pSpecificConfig)
{
    ...
    if ( pLinkDriverConfig->uDriverStrength > LINK_SPECIFIC_CONFIG_DRIVE_STRENGTH_MAX ) {
        BCM_FABRIC_DEBUG_WARN(("Found requested driver strength(%d) out of"
    ...
    if ( pLinkDriverConfig->uDriverEqualization > LINK_SPECIFIC_CONFIG_EQUALIZATION_MAX ) {
        BCM_FABRIC_DEBUG_WARN(("Found requested driver equalization(%d)"
        ...
        case SB_FAB_TARGET_DEVICE_LINK_IMPLEMENTATION_TYPE_HYPERCORE:
	{
        ...
        default:
        {
            BCM_FABRIC_DEBUG_ERR(("Unknown link type(0x%08x) found for"
                                  " TargetDevice(0x%08x) nLink(%d)\n",
                                  nLinkType, (uint32_t)devHandle, nLink));
                           ...
                           sbLinkDriverConfig_t *pLinkDriverConfig)
{
    ...
    switch (SOC_SBX_CONTROL((int)sbHandle)->fabtype) {
	    ...
	    break;
        default:
            BCM_FABRIC_DEBUG_WARN(("Unknown device type(0x%08x) for"
...
int
soc_sbx_fabric_get_timeslot_size_sirius(int unit, int32_t num_logical_xbars, 
                                        uint32 *ts_rv)
{
    ...
    uint32  link_speed, numerator, denominator;
    uint32  ts_ss_ss, ts_ss_qe, ts_opt, ts_base, opt_packlets;
    ...
    case SOC_SBX_SYSTEM_CFG_VPORT: 
        ts_base = SB_FAB_DEVICE_BM9600_MIN_TIMESLOT_IN_NS_VPORT;
        break;
    case SOC_SBX_SYSTEM_CFG_VPORT_MIX:
        ts_base = SB_FAB_DEVICE_BM9600_MIN_TIMESLOT_IN_NS_VPORT_MIX;
        break;
    default:
        return SOC_E_INTERNAL;
    }
     ...
     */
    fab_bw = ((opt_packlets * 16 * 8) * (ss_links * 2) * 1000)/ts_opt;
    ...
    ss_ss_bs = (fab_bw * ts_opt)/(16 * 8 * 2 * 1000);
    ...
    ss_qe_bs_ddr_limited = (ddr_bw * ts_opt)/(2 * 16 * 8); /* rounddown */
...
int
soc_sbx_fabric_get_timeslot_size_qe2k(int unit, int32_t num_logical_xbars, 
                                      uint32 *ts_rv)
{
    uint32  fabric_config = SOC_SBX_CFG(unit)->uFabricConfig;
...
int32_t
soc_sbx_fabric_get_timeslot_size(int unit, int32_t num_logical_xbars, 
                                 int32_t b_half_bus, int32_t b_hybrid)
{
    int     rv = SOC_E_NONE;
    uint32  fabric_config = SOC_SBX_CFG(unit)->uFabricConfig;
    ...
    case SOC_SBX_SYSTEM_CFG_VPORT_LEGACY:
        rv = soc_sbx_fabric_get_timeslot_size_qe2k(unit, num_logical_xbars, 
                                                   &ts_rv);
        if (SOC_FAILURE(rv)) {
            BCM_FABRIC_DEBUG_ERR(("Internal error calculating Timeslot size "
    ...
    default:
        BCM_FABRIC_DEBUG_ERR(("Invalid fabric mode (%d) configured\n", 
                              fabric_config));
        ts_rv = SB_FAB_DEVICE_BM9600_MIN_TIMESLOT_IN_NS_VPORT;
        break;
    }
    ...
    *exp = 0;
	...
	case L7_BPm:
		...
		case CREDITOR_RATE_EXPf:
			...
			*exp = 15;
			*mant = 0xFF;
			BCM_FABRIC_DEBUG_ERR(("Rate out of valid range\n"));
			return SOC_E_PARAM;
		    } else {
		        get_ts_exp_mant(num, exp, mant);
			if ((*exp > 15) || (*mant > 0xFF)) {
			    BCM_FABRIC_DEBUG_ERR(("Rate coversion result in"
		...
		case SHAPER_MAXBURST_EXPf:
			...
			*mant = 196;
			*exp = 15;
			BCM_FABRIC_DEBUG_ERR(("Number out of valid range\n"));
			return SOC_E_PARAM;
		    } else if (num == 0) {
			*mant = 0;
			*exp = 0;
