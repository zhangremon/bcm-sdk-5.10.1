 ...
 * ====================================================================
 *
 * WORKING REVISION: $Id: sbG2EplibCt.c 1.5.42.1 Broadcom SDK $
 *
...
typedef struct sbG2EplibPatternDesc_s {
    uint8_t  bValid;
    uint8_t  ucClass;
    uint8_t  aucPattern[SB_QE2000_ELIB_NUM_CRT_BITS_K];
    uint32_t ulEnMask;
    uint32_t ulEnPat;
} sbG2EplibPatternDesc_t;

static uint8_t
sbG2EplibGetClass(uint32_t nIdx);

bool_t
sbG2EplibIsClassValid(uint8_t nClass);
...
sbG2EplibSetupEnableMasks(void)
{
    uint32_t nPat, nBit, nEnMask, nEnPat;
...
/*
 * XXX -- TODO
 * We externally link to the EP class instruction stream
 * that was compiled into source form by KaEgAsm. In the
 * future, we should push this up, so this isn't a external
 * hard-coded link, but rather a variable passed through
 * the function call.
 */
extern uint32_t aulKaEpClassInstr[16][8];
extern uint32_t aulKaQeSsEpClassInstr[16][8];
...
{
    uint32_t ulSegIdx, ulBase, ulBase8, ulEnd, ulWidth, ulLength, ulSegReg;
