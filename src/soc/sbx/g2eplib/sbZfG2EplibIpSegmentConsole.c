/*
 * $Id: sbZfG2EplibIpSegmentConsole.c 1.6.12.3 Broadcom SDK $
...
sbZfG2EplibIpSegment_Print(sbZfG2EplibIpSegment_t *pFromStruct) {
  SB_LOG("G2EplibIpSegment:: start=0x%01x", (unsigned int)  pFromStruct->start);
  SB_LOG(" end=0x%01x", (unsigned int)  pFromStruct->end);
  SB_LOG(" width=0x%01x", (unsigned int)  pFromStruct->width);
  SB_LOG(" entsize=0x%01x", (unsigned int)  pFromStruct->entrysize);
  SB_LOG("\n");
...
char *
sbZfG2EplibIpSegment_SPrint(sbZfG2EplibIpSegment_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
