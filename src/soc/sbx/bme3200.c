/*
 * $Id: bme3200.c 1.25 Broadcom SDK $
...
soc_bm3200_init(int unit, soc_sbx_config_t *cfg)
{
	...
	BCM_FABRIC_DEBUG_ERR(("BM3200 uLinkThresholdIndex(%d) initialization "
    ...
    hwBm3200InitParams.seSerializerMask_ull     = SOC_SBX_CFG_BM3200(unit)->uSeSerializerMaskHi;
    hwBm3200InitParams.seSerializerMask_ull     = hwBm3200InitParams.seSerializerMask_ull << 32;
    hwBm3200InitParams.seSerializerMask_ull    |= SOC_SBX_CFG_BM3200(unit)->uSeSerializerMaskLo;
    hwBm3200InitParams.lcmSerializerMask_ull    = SOC_SBX_CFG_BM3200(unit)->uLcmSerializerMaskHi;
    hwBm3200InitParams.lcmSerializerMask_ull    = hwBm3200InitParams.lcmSerializerMask_ull << 32;
    hwBm3200InitParams.lcmSerializerMask_ull   |= SOC_SBX_CFG_BM3200(unit)->uLcmSerializerMaskLo;
    ...
    if (uStatus != HW_BM3200_STATUS_OK_K) {
        soc_cm_print("Bm3200 initialization failed with uStatus(0x%x)\n",uStatus);
        return (uStatus);
    }
...
soc_bm3200_fo_error(int unit, uint32 ignored)
{
    ...
    if (intrs) {
      ...
      if (SAND_HAL_GET_FIELD(PT, FO_ERROR, AUTO_SWITCHOVER_EVENT, intrs)) {
        soc_cm_debug(DK_INTR, "Auto Switchover Interrupt\n");
      }
	...
	red_info.xbars = (uint64)SAND_HAL_GET_FIELD(PT, FO_STATUS, ENABLED_LINKS, uData);
...
soc_bm3200_isr(void *_unit)
{
	...
	while (SOC_CONTROL(unit) &&
		(SOC_CONTROL(unit)->info.chip_type == SOC_INFO_BME3200)) {
...
soc_bm3200_isr2(void *_unit)
{
  ...
  */
    ...
    for (i=0; i < BM3200_INTR_HANDLERS_COUNT; i++) {
      soc_cm_debug(DK_INTR, "soc_bm3200_isr unit %d: dispatch %s\n",
...
soc_bm3200_config_linkdriver(int unit, int nLink, sbLinkDriverConfig_t *pLinkDriverConfig)
{
    ...
    if ( nLink < 0 || nLink > SB_FAB_DEVICE_BM3200_NUM_SERIALIZERS ) {
        BCM_FABRIC_DEBUG_ERR(("Bad link(%d) requested for BM3200."
                              " Valid range is [0,%d)\n",
    ...
    if ( status != SB_FAB_STATUS_OK ) {
        BCM_FABRIC_DEBUG_ERR(("Could not get link specific"
...
int soc_bm3200_eset_block_set(int nUnit, uint32 uBaseEset, uint32 uNodesMasks[4] )
{
    ...
    /* todo: set/clear mcFullEvalMin field in nEsetData[4] based
     *       upon parameter passed in for now, always set the
     *       mcFullEvalMin field for the eset if there are any entries.
     */
    for(i = 0; i < 4; i++) {
        if (uNodesMasks[i]) {
            uEsetData4 |= (0x1 << i);
...
soc_bm3200_bwp_read(int unit, int32 queue, int32 *pgamma, int32 *psigma) {
    ...
    sbZfFabBm3200BwBandwidthParameterEntry_Unpack(&zfBwpEntry, (uint8_t*)&data, 4);
...
soc_bm3200_bwp_write(int unit, int32 queue,
    ...
    sbZfFabBm3200BwBandwidthParameterEntry_Pack(&zfBwpEntry, (uint8_t*)&write_value, 4);
...
soc_bm3200_prt_read(int unit, int32 bw_group, int32 *psp_queues_in_bag, int32 *pqueues_in_bag,
    ...
    sbZfFabBm3200BwPortRateEntry_Unpack(&zfPrtEntry, (uint8_t*)&read_value, 8);
...
soc_bm3200_prt_write(int unit, int32 bw_group, int32 sp_queues_in_bag,
    ...
    sbZfFabBm3200BwPortRateEntry_Pack(&zfPrtEntry, (uint8_t*)&write_value, 8);
...
soc_bm3200_bw_mem_write(int unit, int32 bw_table_id,
	...
	if (nBwError) {
	    ...
	    soc_cm_print("bw_error occurred during write to repository(%d)\n", repository);
	}
    }
