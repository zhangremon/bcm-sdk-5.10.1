...
STATIC int32_t
_sbxred_get_timeslot_size(int unit, int32_t nTotalLogicalCrossbars, 
...
_sbxred_bm3200_bme_add(int unit, bcm_port_t port, int num, int active)
{
    ...
    if (active) {
        ...
        if (nRegister != 0) {
            BCM_DEBUG(BCM_DBG_ERR, ("bw_error after enabling scheduler (0x%08x)\n", 
                                    nRegister));
            rv = BCM_E_FAIL;
        }
