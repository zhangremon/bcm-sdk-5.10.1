/*
 * $Id: sirius_ddr23.c 1.9 Broadcom SDK $
...
soc_sbx_sirius_sw_ddr23_train_init(int unit, siriusInitParams_t *pInitParams)
{
  ...
  if (DDR_TRAINING_CONTROL(unit) == NULL) {
    ...
    if (SOC_FAILURE(rv)) {
      DDR_TRAINING_UNLOCK(unit);
      soc_cm_print("Error stopping training thread\n");
      return rv;
    }
  }  

  soc_cm_debug(DK_VERBOSE,"Starting ddr training thread flags 0x%x, Interval %d\n",flags,interval_us);

  /* start */
  SOC_IF_ERROR_RETURN(soc_sbx_sirius_sw_ddr23_train_start(unit,flags,interval_us));
  soc_cm_debug(DK_VERBOSE,"soc_sbx_sirius_sw_ddr23_train_init: unit=%d rv=%d\n"
	       ,unit, rv);
...
#define NO_ERRORS 0
  ...
  if (longest_span == 0) {
     ...
     */
    soc_cm_print("No delay settings worked using tap=%d least errors(%d)\n",
...
_soc_sbx_ddr23_training_thread(void *unit_vp)
{
    ...
    while ((interval = train->interval) != -1) {
      ...
      soc_cm_debug(DK_VERBOSE, "%s  sleep %d\n", FUNCTION_NAME(),interval);
			       ...
			       int interval)
{
  ...
  if (interval == -1) {
    DDR_TRAINING_UNLOCK(unit);
    soc_cm_print("train interval (-1) not starting\n");
    return SOC_E_NONE;
  }
  ...
  if (train->trigger == NULL) {
    soc_cm_print("%s: sem create failed\n",FUNCTION_NAME());
    DDR_TRAINING_UNLOCK(unit);
  ...
  if (interval >= 0) {
    ...
    soc_cm_debug(DK_VERBOSE,"start DDR Train Thread %s. Interval %d \n",
...
soc_sbx_sirius_sw_ddr23_train_stop(int unit)
{
  ...
  soc_cm_debug(DK_VERBOSE,"%s: unit=%d\n",FUNCTION_NAME(),unit);
  DDR_TRAINING_LOCK(unit);
  train = &DDR_TRAINING_THREAD(unit);
  ...
  soc_cm_debug(DK_VERBOSE, "%s: unit=%d rv=%d\n",
...
soc_sbx_sirius_ddr23_clear(int unit)
{
  ...
  if (uDoneMask != 0x3ff) {
    for(ci = 0; ci < SOC_MAX_NUM_CI_BLKS; ci++) {
      if (!(uDoneMask & ( 1 << ci ))) {
	soc_cm_print("CI%d did not finish mem clear\n",ci);
	uStatus = -1;
      }
    }
  }
  ...
  for (ci = 0; ci < SOC_MAX_NUM_CI_BLKS; ci++ ) {
	...
	soc_cm_print("CI%d finished mem clear with errors\n",ci);
	uStatus = -1;
      }
    }
  }
...
soc_sbx_sirius_ddr23_phy_tune_best_setting(int first_window, ci_tune_params_t *pCiTune, uint8 *p) {
  ...
  if (longest_span <= TREAD_EN_GAP_WIDTH ) {
    soc_cm_print("no passing results found from one tread_en window into the next\n");
    pCiTune->valid = 0;
  } else {   
    pCiTune->valid       = 1;
...
soc_sbx_sirius_ddr23_vdl_set(int unit, int ci, uint block, uint vdl, 
  ...
  if ((tap > 63) || (fine > 3)) {
    soc_cm_debug(DK_ERR, "%s: tap or fine out of range \n",FUNCTION_NAME());
    return SOC_E_PARAM;
  }
  ...
  case 1:
    ...
    case 3:
      ...
      break;
    default:
      soc_cm_debug(DK_ERR, "%s: unknown ddr phy vdl %d\n",FUNCTION_NAME(), vdl);
      return SOC_E_PARAM;
    }
    break;
  case 2:
    ...
    break;
  default:
    soc_cm_debug(DK_ERR, "%s: unknown ddr phy register block %d \n",FUNCTION_NAME(), block);
    return SOC_E_PARAM;
  }
