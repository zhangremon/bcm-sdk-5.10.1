/*
 * $Id: bm3200_init.c 1.16 Broadcom SDK $
...
#ifdef BM3200DEBUG
uint32_t bm3200Debug = 1;
#define BM3200PRINTF(flag, fmt) do{soc_cm_print("%s:%d: ",FUNCTION_NAME(),__LINE__);soc_cm_print fmt;}while(0)
#else
...
hwBm3200SetupPi(sbhandle userDeviceHandle,
                uint64_t     xbSerializerMask_ul,
                uint32_t     bmSerializerMask_ul,
                uint32_t     cmode_ul);
...
hwBm3200SetupCrossBar(sbhandle userDeviceHandle,
                      uint64_t xbSerializerMask_ull,
                      uint32_t xcfgMode_ul);
...
hwBm3200SetupAm(sbhandle userDeviceHandle,
                uint32_t     amLocalBmId_ul,
                uint32_t     amDefaultBmId_ul,
                uint32_t     bmSerializerMask_ul,
		uint32_t     bmEnableAutoFailover,
		uint32_t     bmEnableAutoLinkDisable,
		uint32_t     bmMaxFailedLinks,
                uint32_t     spMode);
...
hwBm3200SetupBwCmode(sbhandle userDeviceHandle,
                     uint32_t nQueues,
                     uint32_t nVirtualPorts,
                     uint32_t nNodes,
                     uint32_t epochLength_ul);
...
hwBm3200SetupBw(sbhandle userDeviceHandle,
                ...
                uint32_t nNodes,
                uint32_t epochLength_ul,
                uint32_t modeLatency_ul);

static uint32_t
hwBm3200SetupIf(sbhandle userDeviceHandle,
                uint64_t serializerMask_ull,
                uint64_t xbSerializerMask_ull,
...
hwBm3200SetXcfg(sbhandle userDeviceHandle,
                ...
                uint32_t planeActive[HW_BM3200_MAX_NUM_PLANES],
                uint32_t *xcfgMode_ulp,
                uint32_t lcmXcfgABInputPolarityReversed_ul);

static void
hwBm3200SetXcfgA(sbhandle userDeviceHandle,
                 uint32_t xcfg[HW_BM3200_PT_NUM_SERIALIZERS]);
...
hwBm3200Init(HW_BM3200_INIT_ST *hwBm3200Init_sp)
{
    ...
    xbSerializerMask_ull =
        (hwBm3200Init_sp->seSerializerMask_ull |
         hwBm3200Init_sp->lcmSerializerMask_ull);

    userDeviceHandle = hwBm3200Init_sp->userDeviceHandle;

    BM3200PRINTF(BM3200DEBUG, ("Begin init. Handle(0x%x) xbSerializerMask_ull(0x%llx)\n\n",(uint32_t)(userDeviceHandle), xbSerializerMask_ull));
...
#define HW_BM3200_XCFG_A_OR_B 3 /* Normal LCM mode */
        ...
        if (hwBm3200Init_sp->lcmSerializerMask_ull) {
    ...
    sts_ul = hwBm3200SetupIf(userDeviceHandle,
                             (xbSerializerMask_ull | hwBm3200Init_sp->bmSerializerMask_ul),
                              xbSerializerMask_ull,
...
static uint32_t hwBm3200CalcTsSize(uint32_t timeSlotSize_ul)
{
    uint32_t ts_size = 0;
    uint32_t ts_in_ps = 0, clk_in_ps = 0;
                ...
                uint32_t     cmode_ul)
{
    ...
    /* Set link modes. */
    SAND_HAL_WRITE(userDeviceHandle, PT, PI_XB_LINK_MODE, (uint32_t)xbSerializerMask_ul);
...
hwBm3200SetXcfg(sbhandle userDeviceHandle,
                uint32_t    lcmXcfg_ul[HW_BM3200_MAX_NUM_PLANES][HW_BM3200_PT_NUM_SERIALIZERS],
                uint32_t    planeValid[HW_BM3200_MAX_NUM_PLANES],
                uint32_t    *xcfgMode_ulp,
                uint32_t    lcmXcfgABInputPolarityReversed_ul)
{
                      ...
                      uint32_t xcfgMode_ul)
{
    ...
    int32_t status_ul;
    int32_t saved_status_ul = HW_BM3200_STATUS_OK_K;
	...
	status_ul = hwBm3200EnableSerializer(userDeviceHandle, i, (sbBool_t)((xbSerializerMask_ull >> i) & 1));
	if (status_ul) {
	    BM3200PRINTF(BM3200DEBUG, ("BM3200(%d) Error enabling serializer(%d)",(uint32_t)(userDeviceHandle), i));
	    saved_status_ul = status_ul;
	}
    }

    SAND_HAL_WRITE(userDeviceHandle, PT, XB_ERROR0_MASK, ~(uint32_t)xbSerializerMask_ull);
    SAND_HAL_WRITE(userDeviceHandle, PT, XB_ERROR1_MASK, ~(uint32_t)xbSerializerMask_ull);
    SAND_HAL_WRITE(userDeviceHandle, PT, XB_ERROR2_MASK, ~(uint32_t)xbSerializerMask_ull);

    reg = (uint32_t)(xbSerializerMask_ull >> 32);
...
{
    uint32_t reg;
    int32_t status_ul = HW_BM3200_STATUS_OK_K;
...
static uint32_t random_array[HW_BM3200_PT_RANDOM_ARRAY_SIZE] = {
...
hwBm3200NmEmapReadWrite(sbhandle userDeviceHandle,
                        ...
                        uint32_t  rd_wr_n)
{
    uint32_t reg_ul, sts_ul = HW_BM3200_STATUS_OK_K;
    soc_timeout_t timeout;
    uint32_t uAck;
...
hwBm3200QltInit(sbhandle userDeviceHandle)
{
    uint32_t data_ul;
    uint32_t addr_ul;
    int32_t  bank_l = 1; /* QLT is in bank 1 */
    uint32_t sts_ul;
...
hwBm3200Q2NpcInit(sbhandle userDeviceHandle)
{
    uint32_t data_ul;
    uint32_t addr_ul;
    int32_t  bank_l = 0; /* Q2NPC is in bank 0 */
    uint32_t sts_ul;
...
hwBm3200Npc2QInit(sbhandle userDeviceHandle)
{
    uint32_t data_ul;
    uint32_t addr_ul;
    int32_t  bank_l = 0; /* NPC2Q is in bank 0 */
    uint32_t sts_ul;
...
{
    int32_t  nBank = 1; /* WCT is in bank 1 */
    ...
    /* Initialize all templates to a value which turns WRED off */
    {
        ...
        for (nTemplate=0; nTemplate<HW_BM3200_WCT_TEMPLATES; nTemplate++) {
            uint32_t uAddr = 0;
            uint32_t uStartAddr =  nTemplate * (HW_BM3200_WCT_ENTRIES_PER_TEMPLATE + 2);
...
{
    uint32_t reg_ul;
    uint32_t sts_ul = HW_BM3200_STATUS_OK_K;
    uint32_t uAck;
    soc_timeout_t timeout;    
    int32_t nRegBwError;
                     ...
                     uint32_t epochLength_ul)
{
    ...
    uint32_t ts_size = hwBm3200CalcTsSize(HW_BM3200_STARTUP_TIMESLOT_SIZE_IN_NS);
    uint32_t qmon_deadline, wred_deadline, dmnd_deadline, alloc_deadline,
        dmnd_cycles;
    uint32_t grant_to_critical_delay, base_rx_sot, rx_sot;
    uint32_t sendmsg_offset;
    uint32_t status;
...
hwBm3200SetupBwDmode(sbhandle userDeviceHandle,
                     ...
                     uint32_t epochLength_ul)
{
    uint32_t wred_deadline, dmnd_deadline, alloc_deadline;
    uint32_t dmode_rxsot, dmode_cmd, ts_size;
    int32_t status;
    ...
    {
        int32_t nCosPerPort = (HW_BM3200_PT_MAX_DMODE_QUEUES + 1)/nQueues;
...
{
    uint32_t rv = HW_BM3200_STATUS_OK_K;
    int32_t nRegister;
    uint32_t uAck;
    soc_timeout_t timeout;
...
static void
config_serdes(sbhandle userDeviceHandle, uint32_t sd)
{
                ...
                uint32_t siLsWindow_ul)
{
    ...
    BM3200PRINTF(BM3200DEBUG, ("Serializers(0x%010llx) xbSerializers(0x%010llx) \n\n",serializerMask_ull, xbSerializerMask_ull));
    /*
    ...
    for (sd = 0; sd < HW_BM3200_PT_NUM_SERIALIZERS; sd += 4) { /* for ports (0,1), (4,5), (8,9), .... */
        if (serializerMask_ull & (3ULL << sd)) {
            BM3200PRINTF(BM3200DEBUG, ("line:%d Enabling serdes pair(0x%010llx)\n\n",__LINE__, (3ULL << sd)));
    ...
    for (sd = 2; sd < HW_BM3200_PT_NUM_SERIALIZERS; sd += 4) { /* for ports (2,3), (6,7), (10,11), .... */
        if (serializerMask_ull & (3ULL << sd)) {
            uint32_t neighbor_sd = sd - 2;
            sbBool_t shared_pll_done = !(serializerMask_ull & 3ULL << neighbor_sd);
	    uint32_t uAck = 0;
	    soc_timeout_t timeout;
