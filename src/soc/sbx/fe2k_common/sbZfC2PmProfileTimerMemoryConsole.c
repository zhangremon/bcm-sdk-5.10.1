/*
 * $Id: sbZfC2PmProfileTimerMemoryConsole.c 1.1.30.3 Broadcom SDK $
...
sbZfC2PmProfileTimerMemory_Print(sbZfC2PmProfileTimerMemory_t *pFromStruct) {
  SB_LOG("C2PmProfileTimerMemory:: resv0=0x%06x", (unsigned int)  pFromStruct->uResv0);
  SB_LOG(" type=0x%01x", (unsigned int)  pFromStruct->uType);
  SB_LOG(" profmode=0x%01x", (unsigned int)  pFromStruct->uProfMode);
  SB_LOG(" resv1=0x%02x", (unsigned int)  pFromStruct->uResv1);
  SB_LOG("\n");

  SB_LOG("C2PmProfileTimerMemory:: resv2=0x%08x", (unsigned int)  pFromStruct->uResv2);
  SB_LOG(" resv3=0x%02x", (unsigned int)  pFromStruct->uResv3);
  SB_LOG(" binterrupt=0x%01x", (unsigned int)  pFromStruct->bInterrupt);
  SB_LOG("\n");

  SB_LOG("C2PmProfileTimerMemory:: breset=0x%01x", (unsigned int)  pFromStruct->bReset);
  SB_LOG(" bstrict=0x%01x", (unsigned int)  pFromStruct->bStrict);
  SB_LOG(" bstarted=0x%01x", (unsigned int)  pFromStruct->bStarted);
  SB_LOG("\n");

  SB_LOG("C2PmProfileTimerMemory:: deadline=0x%05x", (unsigned int)  pFromStruct->uDeadline);
  SB_LOG("\n");
...
char *
sbZfC2PmProfileTimerMemory_SPrint(sbZfC2PmProfileTimerMemory_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
