/*
 * $Id: sbZfFabBm9600BwR1Wct0BEntry.c 1.1.44.4 Broadcom SDK $
...
sbZfFabBm9600BwR1Wct0BEntry_Unpack(sbZfFabBm9600BwR1Wct0BEntry_t *pToStruct,
  ...
  (pToStruct)->m_uTEcnDp0 =  (uint32_t)  (pFromData)[1] ;
  (pToStruct)->m_uTEcnDp0 |=  (uint32_t)  (pFromData)[0] << 8;

  /* Unpack Member: m_uScaleDp0 */
  (pToStruct)->m_uScaleDp0 =  (uint32_t)  ((pFromData)[2] >> 4) & 0x0f;

  /* Unpack Member: m_uSlopeDp0 */
  (pToStruct)->m_uSlopeDp0 =  (uint32_t)  (pFromData)[3] ;
  (pToStruct)->m_uSlopeDp0 |=  (uint32_t)  ((pFromData)[2] & 0x0f) << 8;
#else

  /* Unpack operation based on little endian */

  /* Unpack Member: m_uTEcnDp0 */
  (pToStruct)->m_uTEcnDp0 =  (uint32_t)  (pFromData)[2] ;
  (pToStruct)->m_uTEcnDp0 |=  (uint32_t)  (pFromData)[3] << 8;

  /* Unpack Member: m_uScaleDp0 */
  (pToStruct)->m_uScaleDp0 =  (uint32_t)  ((pFromData)[1] >> 4) & 0x0f;

  /* Unpack Member: m_uSlopeDp0 */
  (pToStruct)->m_uSlopeDp0 =  (uint32_t)  (pFromData)[0] ;
  (pToStruct)->m_uSlopeDp0 |=  (uint32_t)  ((pFromData)[1] & 0x0f) << 8;
#endif
