/*
 * $Id: sbZfFabBm9600NmEmtEntryConsole.c 1.1.38.3 Broadcom SDK $
...
sbZfFabBm9600NmEmtEntry_Print(sbZfFabBm9600NmEmtEntry_t *pFromStruct) {
  SB_LOG("FabBm9600NmEmtEntry:: esetfullstatusmode=0x%01x", (unsigned int)  pFromStruct->m_uEsetFullStatusMode);
  SB_LOG(" mfem=0x%01x", (unsigned int)  pFromStruct->m_uMfem);
  SB_LOG(" esetmember1=0x%02x", (unsigned int)  pFromStruct->m_uEsetMember1);
  SB_LOG("\n");

  SB_LOG("FabBm9600NmEmtEntry:: esetmember0=0x%016llx", (uint64_t)  pFromStruct->m_uuEsetMember0);
  SB_LOG("\n");
...
char *
sbZfFabBm9600NmEmtEntry_SPrint(sbZfFabBm9600NmEmtEntry_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"FabBm9600NmEmtEntry:: esetmember0=0x%016llx", (uint64_t)  pFromStruct->m_uuEsetMember0);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");
