/*
 * $Id: sbZfFabBm9600FoTestInfoConsole.c 1.1.38.3 Broadcom SDK $
...
sbZfFabBm9600FoTestInfo_Print(sbZfFabBm9600FoTestInfo_t *pFromStruct) {
  SB_LOG("FabBm9600FoTestInfo:: isactive=0x%01x", (unsigned int)  pFromStruct->m_bIsActive);
  SB_LOG(" dutstimeslotsize=0x%016llx", (uint64_t)  pFromStruct->m_uuDUTTimeslotsize);
  SB_LOG("\n");

  SB_LOG("FabBm9600FoTestInfo:: qefosimtime=0x%016llx", (uint64_t)  pFromStruct->m_uuSimTimeThatQEFailoverOccurred);
  SB_LOG("\n");

  SB_LOG("FabBm9600FoTestInfo:: dutbaseaddress=0x%08x", (unsigned int)  pFromStruct->m_uDUTBaseAddress);
  SB_LOG(" numfailedqe=0x%02x", (unsigned int)  pFromStruct->m_uNumQEsThatFailedOver);
  SB_LOG("\n");

  SB_LOG("FabBm9600FoTestInfo:: previousactivebm=0x%01x", (unsigned int)  pFromStruct->m_uPreviousActiveBm);
  SB_LOG(" fopreviouslyasserted=0x%01x", (unsigned int)  pFromStruct->m_bFailoverPreviouslyAsserted);
  SB_LOG("\n");

  SB_LOG("FabBm9600FoTestInfo:: expectfo=0x%01x", (unsigned int)  pFromStruct->m_bExpectFailover);
  SB_LOG("\n");
...
char *
sbZfFabBm9600FoTestInfo_SPrint(sbZfFabBm9600FoTestInfo_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"FabBm9600FoTestInfo:: isactive=0x%01x", (unsigned int)  pFromStruct->m_bIsActive);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," dutstimeslotsize=0x%016llx", (uint64_t)  pFromStruct->m_uuDUTTimeslotsize);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"FabBm9600FoTestInfo:: qefosimtime=0x%016llx", (uint64_t)  pFromStruct->m_uuSimTimeThatQEFailoverOccurred);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");
...
sbZfFabBm9600FoTestInfo_SetField(sbZfFabBm9600FoTestInfo_t *s, char* name, int value) {
  ...
  if (SB_STRCMP(name, "") == 0 ) {
    ...
    s->m_uuDUTTimeslotsize = value;
  } else if (SB_STRCMP(name, "m_uusimtimethatqefailoveroccurred") == 0) {
    s->m_uuSimTimeThatQEFailoverOccurred = value;
