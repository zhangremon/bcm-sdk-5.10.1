/*
 * $Id: sbZfFabBm3200BwWstEntry.c 1.2.28.4 Broadcom SDK $
...
sbZfFabBm3200BwWstEntry_Unpack(sbZfFabBm3200BwWstEntry_t *pToStruct,
  ...
  (pToStruct)->m_nState =  (uint32_t)  (pFromData)[3] ;
  (pToStruct)->m_nState |=  (uint32_t)  (pFromData)[2] << 8;
  (pToStruct)->m_nState |=  (uint32_t)  (pFromData)[1] << 16;
  (pToStruct)->m_nState |=  (uint32_t)  (pFromData)[0] << 24;
#else

  /* Unpack operation based on little endian */

  /* Unpack Member: m_nState */
  (pToStruct)->m_nState =  (uint32_t)  (pFromData)[0] ;
  (pToStruct)->m_nState |=  (uint32_t)  (pFromData)[1] << 8;
  (pToStruct)->m_nState |=  (uint32_t)  (pFromData)[2] << 16;
  (pToStruct)->m_nState |=  (uint32_t)  (pFromData)[3] << 24;
#endif
...
/*
 * $Id: sbZfFabBm3200BwWstEntry.c,v 1.2.28.4 2011/05/22 05:39:14 iakramov Exp $
 * $Copyright: Copyright 2011 Broadcom Corporation.
 * This program is the proprietary software of Broadcom Corporation
 * and/or its licensors, and may only be used, duplicated, modified
 * or distributed pursuant to the terms and conditions of a separate,
...
sbZfFabBm3200BwWstEntry_Print(sbZfFabBm3200BwWstEntry_t *pFromStruct) {
  SB_LOG("FabBm3200BwWstEntry:: state=0x%08x", (unsigned int)  pFromStruct->m_nState);
  SB_LOG("\n");
...
int
sbZfFabBm3200BwWstEntry_SPrint(sbZfFabBm3200BwWstEntry_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
