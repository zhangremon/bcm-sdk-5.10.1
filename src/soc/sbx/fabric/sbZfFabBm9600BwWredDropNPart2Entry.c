/*
 * $Id: sbZfFabBm9600BwWredDropNPart2Entry.c 1.1.44.4 Broadcom SDK $
...
sbZfFabBm9600BwWredDropNPart2Entry_Unpack(sbZfFabBm9600BwWredDropNPart2Entry_t *pToStruct,
  ...
  (pToStruct)->m_uDp2 =  (uint32_t)  ((pFromData)[2] >> 7) & 0x01;

  /* Unpack Member: m_uEcn2 */
  (pToStruct)->m_uEcn2 =  (uint32_t)  ((pFromData)[2] >> 6) & 0x01;

  /* Unpack Member: m_uReserved2 */
  (pToStruct)->m_uReserved2 =  (uint32_t)  ((pFromData)[2] >> 2) & 0x0f;

  /* Unpack Member: m_uPbDe2 */
  (pToStruct)->m_uPbDe2 =  (uint32_t)  (pFromData)[3] ;
  (pToStruct)->m_uPbDe2 |=  (uint32_t)  ((pFromData)[2] & 0x03) << 8;
#else

  /* Unpack operation based on little endian */

  /* Unpack Member: m_uDp2 */
  (pToStruct)->m_uDp2 =  (uint32_t)  ((pFromData)[1] >> 7) & 0x01;

  /* Unpack Member: m_uEcn2 */
  (pToStruct)->m_uEcn2 =  (uint32_t)  ((pFromData)[1] >> 6) & 0x01;

  /* Unpack Member: m_uReserved2 */
  (pToStruct)->m_uReserved2 =  (uint32_t)  ((pFromData)[1] >> 2) & 0x0f;

  /* Unpack Member: m_uPbDe2 */
  (pToStruct)->m_uPbDe2 =  (uint32_t)  (pFromData)[0] ;
  (pToStruct)->m_uPbDe2 |=  (uint32_t)  ((pFromData)[1] & 0x03) << 8;
#endif
