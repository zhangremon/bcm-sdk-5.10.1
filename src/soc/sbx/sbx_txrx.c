/******************************************************************************
*
* $Id: sbx_txrx.c 1.28 Broadcom SDK $
*
...
soc_sbx_txrx_intr (int unit, uint32 unused)
{
           ...
           != consumer)
    {
        ...
        SOC_DEBUG_PRINT((DK_VERBOSE | DK_TX, "sbx txrx unit %d completion,"
                         " prod=%d, cons=%d\n", unit, producer,
                         consumer));
        SOC_DEBUG_PRINT((DK_VERBOSE | DK_TX, "    tx active %d, rx active %d\n",
        ...
        while (producer != consumer) {
            ...
            SOC_DEBUG_PRINT((DK_VERBOSE | DK_TX,
            ...
            if (entry & COMPLETION_RING_TX) {
                if (sbx->tx_active_head == NULL) {
                    SOC_DEBUG_PRINT((DK_WARN, "sbx txrx unit=%d found tx "
    ...
    SOC_DEBUG_PRINT((DK_VERBOSE | DK_TX, "sbx txrx unit %d completion done,"
                     " prod=%d, cons=%d\n", unit, producer,
                     consumer));
    SOC_DEBUG_PRINT((DK_VERBOSE | DK_TX, "    tx active %d, rx active %d\n",
                     sbx->tx_active, sbx->rx_active));
