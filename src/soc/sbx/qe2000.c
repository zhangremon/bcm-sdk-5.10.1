/*
 * $Id: qe2000.c 1.90.20.2 Broadcom SDK $
...
nStatus = 0;\
while (1) {\
uint32_t uDataWaitForAck = SAND_HAL_READ_POLL((sbhandle)userDeviceHandle, KA, reg##_ACC_CTRL);\
...
nStatus = 0;\
while (1) {\
 uint32_t uDataWaitForAck = SAND_HAL_READ((sbhandle)userDeviceHandle, KA, reg##_ACC_CTRL);\
if (SAND_HAL_GET_FIELD(KA, reg##_ACC_CTRL, ACK, uDataWaitForAck)==1){\
uDataWaitForAck = SAND_HAL_MOD_FIELD(KA, reg##_ACC_CTRL, REQ, uDataWaitForAck, 0);\
uDataWaitForAck = SAND_HAL_MOD_FIELD(KA, reg##_ACC_CTRL, ACK, uDataWaitForAck, 1);\
...
soc_qe2000_spi_init(int unit, soc_sbx_config_t *cfg)
{
    ...
    qe2000Init_sp->Spi0JumboMask_ull    = 0xFFFF;
    ...
    */
    ...
    qe2000Init_sp->Spi1JumboMask_ull    = 0x3;
...
soc_qe2000_init(int unit, soc_sbx_config_t *cfg)
{
    ...
    if (SAL_BOOT_BCMSIM) {
        ...
        soc_cm_print("Skipping QE2000 HW init \n");
        return 0;
    }
#endif
	...
	case SOC_SBX_SYSTEM_CFG_VPORT_MIX:
	    ...
	    break;
	default:
	    BCM_FABRIC_DEBUG_ERR(("soc_sbx_init: unit %d unknown"
	...
	break;
    default:
	BCM_FABRIC_DEBUG_ERR(("soc_sbx_init: unit %d unknown tme_mode %d",
    ...
    if (hwQe2000InitParams_sp->uEgHiMcPri == (uint16_t)-1) {
    ...
    if (hwQe2000InitParams_sp->uEgLoMcPri == (uint16_t)-1) {
    ...
    status = soc_qe2000_burst_size_lines_get(unit, 
					     ...
					     &hwQe2000InitParams_sp->nLinesPerTimeslotCongested);
    if (status) {
    ...
    if (status == SB_FAB_STATUS_OK) {
        ...
        BCM_FABRIC_DEBUG_ERR(("QE200 uLinkThresholdIndex(%d) initialization "
    ...
    if (uStatus != HW_QE2000_STS_OK_K) {
        BCM_FABRIC_DEBUG_ERR(("Qe2000 initialization failed with"
    ...
    if (uStatus != HW_QE2000_STS_OK_K) {
        SOC_ERROR_PRINT((DK_ERR, "soc_qe2000_init: unit %d "
                         "unable to spi init e=%d\n", unit, uStatus));
        soc_cm_print("failed spi init\n");
        return uStatus;
    }
...
soc_qe2000_config_linkdriver(int unit, int nLink, sbLinkDriverConfig_t *pLinkDriverConfig)
{
    ...
    if ( nLink < 0 || nLink > SB_FAB_DEVICE_QE2000_NUM_SERIALIZERS ) {
        BCM_FABRIC_DEBUG_ERR(("Bad link(%d) requested for QE2000."
                              " Valid range is [0,%d)\n",
    ...
    if ( status != SB_FAB_STATUS_OK ) {
        BCM_FABRIC_DEBUG_ERR(("Could not get link specific configuration"
                              " for link(%d)\n", nLink));
        return status;
    }
		    ...
		    int nECos)
{
    ...
    /* soc_cm_print("Q2EC queue(%d) mapped to node(%d)/port(%d)/cos(%d)\n", nQueue, nNode, nEPort, nECos); */
		   ...
		   int nQueue, int nEnable)
{
    ...
    /* soc_cm_print("E2Q node(%d)/port(%d) mapped to queue(%d)\n", nENode, nEPort, nQueue); */
...
uint32_t  soc_qe2000_qs_lna_mem_read(int unit, int32_t nAddr, int32_t nTable,
				     uint32_t *puData0, uint32_t *puData1, uint32_t *puData2, uint32_t *puData3, uint32_t *puData4) {
    uint32_t ulTimeOut;
    uint32_t ulCntrl;
    ...
    soc_cm_print("lna memory read timed out addr(0x%x)\n", nAddr);
    return -1;
}


sbFabStatus_t soc_qe2000_qs_lna_mem_write(int unit, int32_t nAddr, int32_t nTable,
					  uint32_t uData0, uint32_t uData1, uint32_t uData2, uint32_t uData3, uint32_t uData4) {
    uint32_t ulTimeOut;
    uint32_t ulCntrl;
    uint32_t status = 0;

    {
	    ...
	    soc_cm_print("lna memory write timed out addr(0x%x)\n", nAddr);
        }
...
soc_qe2000_lna_mem_full_remap_table_write(int unit, int32_t nVirtualNodePort, int32_t nDestPortId) {
    int32_t nAddr;
    int32_t nPortLocation;
    uint32_t uData[6]; /* make a little large to simplify data word position calc */
    int32_t nVirtualNodeId = (nVirtualNodePort>>5)&0x1F;
    int32_t nVirtualPortId = nVirtualNodePort&0x1F;
    ...
    sbZfFabQe2000LnaFullRemapEntry_Unpack(&zfRemapEntry, (uint8_t*)uData, 20);
    zfRemapEntry.m_nRemap[nPortLocation] = nDestPortId;

    sbZfFabQe2000LnaFullRemapEntry_Pack(&zfRemapEntry, (uint8_t*)uData, 20);
...
uint32_t soc_qe2000_eb_mem_read( int unit,
			    uint32_t uOffset,
			    uint32_t puData[8])
{
    int nTimeout;
 ...
 *
 *****************************************************************************/
uint32_t soc_qe2000_qs_mem_write(int unit,
				uint32_t nTableId,
				uint32_t uAddress,
				uint32_t uData)
{
    uint32_t uStatus = 0;
    uint32_t uLocalAddr = 0;
    uint32_t uDataRead;
    int32_t nRetryCnt = 0;
	...
	if (uStatus) {
	    SOC_ERROR_PRINT((DK_ERR,"Timeout writing QS memory table(%d) address(0x%x)\n", nTableId, uAddress));
	    soc_cm_print("Timeout writing QS memory table(%d) address(0x%x)\n", nTableId, uAddress);
	    return uStatus;
	}
	
        if (qe2000_qs_mem[nTableId].bCheckMemory == FALSE || SOC_IS_RELOADING(unit)) {
	    break;
	}
	...
	if (uStatus) {
	    SOC_ERROR_PRINT((DK_ERR,"Timeout on readback of QS memory table(%d) address(0x%x)\n", nTableId, uAddress));
	    return uStatus;
	}
	
	if ( (uData     & qe2000_qs_mem[nTableId].nMemMask[0]) != 
	     (uDataRead & qe2000_qs_mem[nTableId].nMemMask[0])   ) {
	    SOC_ERROR_PRINT((DK_VERBOSE, "QS Table(0x%x) Not Updated, Retrying Write, "
...
uint32_t soc_qe2000_qs_mem_read(int unit,
			       uint32_t nTableId,
			       uint32_t uAddress,
			       uint32_t* pData)
{
...
uint32_t soc_qe2000_qs_mem_read_easy_reload(int unit,
                                            uint32_t nTableId,
                                            uint32_t uAddress,
                                            uint32_t* pData)
{
...
uint32_t soc_qe2000_qs_rank_read(int unit,
				uint32_t uAddress,
				uint32_t* pData)
{
    uint32_t uStatus = 0;
 ...
 *
 *****************************************************************************/
uint32_t soc_qe2000_qs_rank_write(int unit,
				 uint32_t uAddress,
				 uint32_t uData)
{
  uint32_t uStatus;
...
uint32_t soc_qe2000_qs_rand_write(int unit,
				 uint32_t uAddress,
				 uint32_t uData)
{
  uint32_t uStatus = 0;
...
uint32_t soc_qe2000_qs_rand_read(int unit,
				uint32_t uAddress,
				uint32_t* pData)
{
  uint32_t uStatus = 0;
...
uint32_t soc_qe2000_qs_lna_rank_write(int unit,
				    uint32_t uAddress,
				    uint32_t uData)
{
    uint32_t uStatus = 0;
...
uint32_t soc_qe2000_qs_lna_rank_read(int unit,
				   uint32_t uAddress,
				   uint32_t* pData)
{
    uint32_t uStatus = 0;
...
uint32_t soc_qe2000_qs_lna_rand_write(int unit,
				    uint32_t uAddress,
				    uint32_t uData)
{
    uint32_t uStatus = 0;
...
uint32_t soc_qe2000_qs_lna_rand_read(int unit,
				   uint32_t uAddress,
				   uint32_t* pData)
{
    uint32_t uStatus = 0;
 ...
 *
 *****************************************************************************/
uint32_t soc_qe2000_qs_mem_lna_write(int unit,
				   uint32_t nTableId,
				   uint32_t uAddress,
				   uint32_t uData4,
				   uint32_t uData3,
				   ...
				   uint32_t uData0)
{
    ...
    uint32_t uLocalAddr = 0;
    int32_t nRetryCnt;
    uint32_t uData0Read, uData1Read, uData2Read, uData3Read, uData4Read;
	...
	if (uStatus) {
        ...
        if (qe2000_qs_lna_mem[nTableId].bCheckMemory == FALSE || SOC_IS_RELOADING(unit)) {
	    break;
	}
	...
	if (uStatus) {
        ...
        if ( ((uData0Read & qe2000_qs_lna_mem[nTableId].nMemMask[0]) !=
	      ...
	      (uData4 & qe2000_qs_lna_mem[nTableId].nMemMask[4])) ) {
            ...
            SOC_ERROR_PRINT((DK_WARN, "QS_LNA Table(0x%x) Not Updated, Retrying Write, "
...
uint32_t soc_qe2000_qs_mem_lna_read(int unit,
				  uint32_t nTableId,
				  uint32_t uAddress,
				  uint32_t* pData4,
				  uint32_t* pData3,
 ...
 *
 *****************************************************************************/
uint32_t soc_qe2000_qm_fb_cache_fifo_write(int unit,
					uint32_t uData)
{
    uint32_t uStatus = 0;
...
uint32_t soc_qe2000_qm_fb_cache_fifo_read(int unit,
				       uint32_t* pData)
{
    uint32_t uStatus = 0;
			...
			uint32_t uData3)
{
...
#if 0
    soc_cm_print("Writing QM Table(0x%x), uData0: 0x%x, uData1: 0x%x, uData2: 0x%x, uData3: 0x%x\n",
	...
	soc_cm_print("Timeout waiting for QM indirect access ACK (table(%d) addr(0x%x/%d).\n",
		     nTable, nAddr, nAddr);
    }
		       ...
		       uint32_t *puData3)
{
    ...
    soc_cm_print("ERROR: Timeout waiting for QM indirect access ACK (table(%d) addr(0x%x/%d).\n",
		 nTable, nAddr, nAddr);
...
uint32_t soc_qe2000_qstate_mem_read(int unit,
                                    int32_t nQueue,
                                    int32_t *pbEnable,
				    int32_t *pnAnemicWatermarkSel,
                                    int32_t *pnMinBuffers,
				    int32_t *pnMaxBuffers)
{
...
uint32_t soc_qe2000_qwred_config_set(int unit,
				     int32_t queue,
				     int32_t template,
				     int32_t gain)
{
     ...
     *     definitions
     */
    uint32_t uData0 = ( ((template << 4) & 0x000000F0) | (gain & 0x0000000F) );
    return ( soc_qe2000_qm_mem_write(unit, queue, 0x0F, uData0, 0x00, 0x00, 0x00) );
}

uint32_t soc_qe2000_qdemand_config_read(int unit,
					int32_t queue,
...
uint32_t soc_qe2000_qdemand_config_set(int unit,
				       int32_t queue,
				       ...
				       int32_t qla_demand_mask)
{
     ...
     *     definitions
     */
    uint32_t uData0 = ( ((rate_delta_max_index << 1) & 0x0000007E) | (qla_demand_mask & 0x00000001) );
    return ( soc_qe2000_qm_mem_write(unit, queue, 0x08, uData0, 0x00, 0x00, 0x00) );
}

uint32_t soc_qe2000_qbyte_adjust_set(int unit,
				     int32_t queue,
				     int32_t sign,
				     int32_t bytes)
{
     ...
     *     definitions
     */
    uint32_t uData0 = ( ((sign << 6) & 0x00000040) | (bytes & 0x0000003F) );
...
soc_qe2000_eg_mem_write(int unit, int32_t nAddr, int32_t nTable,
    ...
    /**
     * soc_cm_print("*>soc_qe2000_eg_mem_write. Table 0x%x Addr 0x%x Data 0x%x 0x%x 0x%x\n",
...
uint32_t soc_qe2000_eg_mem_read(int unit,
			       uint32_t uAddress,
			       uint32_t uTableId,
			       uint32_t* pData0,
			       uint32_t* pData1,
			       uint32_t* pData2)
{
    ...
    SOC_DEBUG_PRINT(( DK_VERBOSE | DK_SOCMEM, "soc_qe2000_eg_mem_read. Table 0x%x Addr 0x%x\n", uTableId, uAddress));
    ...
    *pData2 = SAND_HAL_READ((sbhandle)unit, KA, EG_MEM_ACC_DATA2);
    SOC_DEBUG_PRINT(( DK_VERBOSE | DK_SOCMEM, "soc_qe2000_eg_mem_read. LocAddr 0x%x Data 0x%x 0x%x 0x%x\n",
...
uint32_t soc_qe2000_ei_mem_write(int unit,
				uint32_t uTableId,
				uint32_t uAddress,
				uint32_t uData0){
    uint32_t uStatus = 0;
...
uint32_t soc_qe2000_ei_mem_read(int unit,
			       uint32_t uTableId,
			       uint32_t uAddress,
			       uint32_t* pData0)
{
...
uint32_t soc_qe2000_pm_mem_write(int unit,
				uint32_t uAddress,
				uint32_t uData3,
				uint32_t uData2,
				uint32_t uData1,
...
uint32_t soc_qe2000_pm_mem_read(int unit,
			       uint32_t uAddress,
			       uint32_t* pData3,
			       uint32_t* pData2,
			       uint32_t* pData1,
...
uint32_t soc_qe2000_sf_mem_write(int unit,
				uint32_t uAddress,
				uint32_t uData)
{
    uint32_t uStatus = 0;
...
uint32_t soc_qe2000_sf_mem_read(int unit,
			       uint32_t uAddress,
			       uint32_t* pData)
{
    uint32_t uStatus = 0;
...
uint32_t soc_qe2000_ep_amcl_mem_write(int unit,
				    uint32_t uAddress,
				    uint32_t uData1,
				    uint32_t uData0,
...
uint32_t soc_qe2000_ep_amcl_mem_read(int unit,
				   uint32_t uAddress,
...
uint32_t soc_qe2000_ep_bmbf_mem_write(int unit,
				    uint32_t uAddress,
				    uint32_t uData0)
{
    uint32_t uStatus = 0;
...
uint32_t soc_qe2000_ep_bmbf_mem_read(int unit,
				   uint32_t uAddress,
				   uint32_t* pData0)
{
    uint32_t uStatus = 0;
...
uint32_t soc_qe2000_ep_mmip_mem_write(int unit,
				    uint32_t uAddress,
				    uint32_t uData1,
				    uint32_t uData0)
{
...
uint32_t soc_qe2000_ep_mmip_mem_read(int unit,
				   uint32_t uAddress,
				   uint32_t bClearOnRead,
				   uint32_t* pData1,
				   uint32_t* pData0)
...
uint32_t soc_qe2000_ep_mmbf_mem_write(int unit,
				    uint32_t uAddress,
				    uint32_t uData1,
				    uint32_t uData0)
{
...
uint32_t soc_qe2000_ep_mmbf_mem_read(int unit,
				   uint32_t uAddress,
...
uint32_t soc_qe2000_rb_pol_mem_write(int unit, uint32_t uTableId, uint32_t uAddress, uint32_t uData)
{
    uint32_t uStatus = 0;
    uint32_t uLocalAddr = 0;
...
uint32_t soc_qe2000_rb_pol_mem_read(int unit,
				  uint32_t uTableId,
				  uint32_t uAddress,
				  uint32_t* pData)
{
...
uint32_t soc_qe2000_rb_class_mem_write(int unit,
				     uint32_t nSpi,
				     uint32_t uTableId,
				     uint32_t uAddress,
				     uint32_t uData0,
 ...
 *
 *****************************************************************************/
uint32_t soc_qe2000_rb_class_mem_read(int unit,
				    uint32_t nSpi,
				    uint32_t uTableId,
				    uint32_t uAddress,
				    uint32_t* pData0,
...
uint32_t soc_qe2000_pm_dll_lut_write(int unit,
				   uint32_t uTableId,
				   uint32_t uAddress,
				   uint32_t uData)
{
...
uint32_t soc_qe2000_pm_dll_lut_read(int unit,
				  uint32_t uTableId,
				  uint32_t uAddress,
				  uint32_t* pData)
{
               ...
               int32_t enable)
{
    if (nBaseQueue > SOC_SBX_CFG(unit)->num_queues - 16) {
        SOC_ERROR_PRINT((DK_ERR,"Invalid Base Queue Requested %d [0x%x]\n", nBaseQueue, nBaseQueue));
        return SOC_E_PARAM;
    }



    SOC_DEBUG_PRINT((DK_VERBOSE, "Set Counter Base to %d [0x%x]\n", nBaseQueue, nBaseQueue));
		       ...
		       int32_t *nBaseQueue)
{
    ...
    *nBaseQueue = SAND_HAL_READ(unit, KA, QM_SELECTED_Q);
     SOC_DEBUG_PRINT((DK_VERBOSE, "Get Counter Base to %d [0x%x]\n", *nBaseQueue, *nBaseQueue));
    return SOC_E_NONE;
...
soc_qe2000_qm_counter_read(int unit,
		       int32_t set,
		       uint32_t *puCounterBase)
{
    uint32_t uCtrl;
    uint32_t uPacketCount;
    uint32_t uByteCount;
    int nQ;
    int nCounter;
...
#define REAL_COUNTER
        ...
        *puCounterBase++ = uByteCount;
        if (uPacketCount || uByteCount ) {
            SOC_DEBUG_PRINT((DK_COUNTER + DK_VERBOSE,"Q %d C %d p %d b %d 0x%x\n", nQ, nCounter, uPacketCount, uByteCount, uCtrl));
        }
#else
...
soc_qe2000_rate_delta_config(int unit, uint32 timeslot_size, uint32 epoch_size)
{
    ...
    ns_in_epoch = timeslot_size * epoch_size;
    ...
    for (cur_index = 0; cur_index < QE2000_RATE_DELTA_MAX_PORT_SPEEDS; cur_index++) {
	    ...
	    if (soc_sbx_div64((ns_in_epoch * soc_sbx_qe2000_rate_delta_port_speed[cur_index]), (1000000 * 8), &rate_delta) == -1) {
		soc_cm_print("ERROR: rate_delta_config > 64bits\n");
...
soc_qe2000_template_max_age_set(int unit, int32 template, int hwAge)
{
    ...
    if (rc != SOC_E_NONE) {
        BCM_FABRIC_DEBUG_ERR(("ERROR: Age threshold LUT write"
                              " timed out key 0x%x\n", template));
    }
    return(rc);
}
...
soc_qe2000_mc_pointer_fifo_set(int unit, int32 fifo, int32 size, int32 base, int32 drop_on_full)
{
    ...
    soc_cm_print("fifo(%d) size(2^%d) base(%d)\n", fifo, size, base);
#endif
