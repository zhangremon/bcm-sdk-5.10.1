...
/*
 * $Id: qe2000_spi.c 1.10 Broadcom SDK $
...
static uint32_t hwQe2000BringupSpi4Tx(sbhandle userDeviceHandle,
				      uint32_t numPorts_ul,
				      uint32_t maxBurst1_ul,
				      uint32_t maxBurst2_ul,
				      uint32_t dataMaxT_ul,
				      uint32_t alpha_ul,
				      uint32_t spiTsInvert_ul,
				      uint32_t spi_ul,
				      uint32_t txIgnStatus_ul,
				      uint64_t requeuePortsMask_ull,
                                      uint32_t cal_m );

static void hwQe2000ResetSpi4Clk(sbhandle userDeviceHandle, uint32_t spi_ul);

static uint32_t hwQe2000BringupSpi4Rx(sbhandle userDeviceHandle,
				      uint32_t numPorts_ul,
				      uint64_t jumboMask_ull,
				      uint32_t minFrameSz_ul,
				      uint32_t maxFrameSz_ul,
				      int32_t  alignDelay_ul,
				      uint32_t spiRsInvert_ul,
				      uint32_t spi_ul,
                                      uint32_t cal_m );

static uint32_t hwQe2000SpiClockSetup(sbhandle userDeviceHandle,
                                      uint32_t spiPort,
				      uint32_t uSpiRefClockSpeed,
				      uint32_t uSpiClockSpeed);
 ...
 *
 *****************************************************************************/
uint32_t hwQe2000InitSpi(HW_QE2000_INIT_ST *hwQe2000Init_sp)
{
    uint32_t sts_ul;
    uint32_t nSpi0TxChans, nSpi0RxChans, nSpi1TxChans, nSpi1RxChans;
                                      ...
                                      uint32_t      cal_m)
{
    ...
    uint32_t nData=0;
    uint32_t nArbLen = numPorts_ul-1;
    uint64_t nnTemp = 0;
    int32_t i;
	...
	if (COMPILER_64_AND(nnTemp, requeuePortsMask_ull)) {
	    nArbLen++;
	}
    }
...
static void hwQe2000ResetSpi4Clk(sbhandle userDeviceHandle, uint32_t spi_ul)
{
    uint32_t sr0_cfg1, st0_cfg1, reg;
 ...
 *
 *****************************************************************************/
void hwQe2000Spi4TxForceTrainingOn(sbhandle userDeviceHandle, uint32_t spi_ul)
{
    uint32_t reg;
...
void hwQe2000Spi4TxForceTrainingOff(sbhandle userDeviceHandle, uint32_t spi_ul)
{
    uint32_t reg;
    reg = SAND_HAL_READ_STRIDE(userDeviceHandle, KA, ST, spi_ul, ST0_CONFIG0);
...
uint32_t hwQe2000Spi4RxStatus(sbhandle userDeviceHandle, uint32_t spi_ul)
{
    uint32_t reg, rxsync;
    /*
 ...
 *
 *****************************************************************************/
uint32_t hwQe2000Spi4TxStatus(sbhandle userDeviceHandle, uint32_t spi_ul)
{
...
void hwQe2000Spi4RxEnableGet(sbhandle userDeviceHandle, uint32_t *enable_ul, uint32_t spi_ul)
{
    uint32_t sr_cfg0;
...
void hwQe2000Spi4RxEnable(sbhandle userDeviceHandle, uint32_t enable_ul, uint32_t spi_ul)
{
    uint32_t sr_cfg0;
    sr_cfg0 = SAND_HAL_READ_STRIDE(userDeviceHandle, KA, SR, spi_ul, SR0_CONFIG0);
...
void hwQe2000Spi4TxEnableGet(sbhandle userDeviceHandle, uint32_t *enable_ul, uint32_t spi_ul)
{
    uint32_t st_cfg0;
...
void hwQe2000Spi4TxEnable(sbhandle userDeviceHandle, uint32_t enable_ul, uint32_t spi_ul)
{
...
void hwQe2000Spi4RxRequestTraining(sbhandle userDeviceHandle, uint32_t enable_ul, uint32_t spi_ul)
{
    uint32_t sr_cfg0;
...
hwQe2000SpiClockSetup(sbhandle userDeviceHandle,
                      ...
                      uint32_t uSpiClockSpeed)
{
    uint32_t rc = HW_QE2000_STS_OK_K;
    uint32_t *freq_p;
    int index = 0;
    int txpll_m = 0;
    int txpll_n = 0;
    int txpll_p = 0;
    uint32_t refClk = uSpiRefClockSpeed; /* in KHz */
    uint32_t freq, numerator, denominator;
    uint32_t nData = 0;
    ...
    freq_p = sal_alloc((sizeof(uint32_t) * HW_QE2000_SFI_TXPLL_M_MAX *
