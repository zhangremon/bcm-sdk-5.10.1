/*
 * $Id: sbZfKaEgTmePortRemapAddr.c 1.1.44.4 Broadcom SDK $
...
sbZfKaEgTmePortRemapAddr_Unpack(sbZfKaEgTmePortRemapAddr_t *pToStruct,
  ...
  (pToStruct)->m_nNode =  (uint32_t)  ((pFromData)[3] >> 5) & 0x07;
  (pToStruct)->m_nNode |=  (uint32_t)  ((pFromData)[2] & 0x03) << 3;

  /* Unpack Member: m_nPort */
  (pToStruct)->m_nPort =  (uint32_t)  ((pFromData)[3] ) & 0x1f;
#else

  /* Unpack operation based on little endian */

  /* Unpack Member: m_nNode */
  (pToStruct)->m_nNode =  (uint32_t)  ((pFromData)[0] >> 5) & 0x07;
  (pToStruct)->m_nNode |=  (uint32_t)  ((pFromData)[1] & 0x03) << 3;

  /* Unpack Member: m_nPort */
  (pToStruct)->m_nPort =  (uint32_t)  ((pFromData)[0] ) & 0x1f;
#endif
