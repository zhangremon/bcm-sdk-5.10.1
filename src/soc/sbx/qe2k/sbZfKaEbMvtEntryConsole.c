/*
 * $Id: sbZfKaEbMvtEntryConsole.c 1.1.44.3 Broadcom SDK $
...
sbZfKaEbMvtEntry_Print(sbZfKaEbMvtEntry_t *pFromStruct) {
  SB_LOG("KaEbMvtEntry:: reserved=0x%01x", (unsigned int)  pFromStruct->m_nReserved);
  SB_LOG(" portmap=0x%013llx", (uint64_t)  pFromStruct->m_nPortMap);
  SB_LOG(" mvtda=0x%04x", (unsigned int)  pFromStruct->m_nMvtda);
  SB_LOG(" mvtdb=0x%01x", (unsigned int)  pFromStruct->m_nMvtdb);
  SB_LOG("\n");

  SB_LOG("KaEbMvtEntry:: next=0x%04x", (unsigned int)  pFromStruct->m_nNext);
...
char *
sbZfKaEbMvtEntry_SPrint(sbZfKaEbMvtEntry_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"KaEbMvtEntry:: reserved=0x%01x", (unsigned int)  pFromStruct->m_nReserved);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," portmap=0x%013llx", (uint64_t)  pFromStruct->m_nPortMap);
...
sbZfKaEbMvtEntry_Validate(sbZfKaEbMvtEntry_t *pZf) {
  ...
  if (pZf->m_nPortMap > 0x3FFFFFFFFFFFFULL) return 0;
