/*
 * $Id: sbZfKaRbPolicePortQMapEntry.c 1.1.44.4 Broadcom SDK $
...
sbZfKaRbPolicePortQMapEntry_Unpack(sbZfKaRbPolicePortQMapEntry_t *pToStruct,
  ...
  (pToStruct)->m_nReserved2 =  (uint32_t)  ((pFromData)[0] >> 1) & 0x7f;

  /* Unpack Member: m_nOddMeter */
  (pToStruct)->m_nOddMeter =  (uint32_t)  (pFromData)[1] ;
  (pToStruct)->m_nOddMeter |=  (uint32_t)  ((pFromData)[0] & 0x01) << 8;

  /* Unpack Member: m_nReserved1 */
  (pToStruct)->m_nReserved1 =  (uint32_t)  ((pFromData)[2] >> 1) & 0x7f;

  /* Unpack Member: m_nEvenMeter */
  (pToStruct)->m_nEvenMeter =  (uint32_t)  (pFromData)[3] ;
  (pToStruct)->m_nEvenMeter |=  (uint32_t)  ((pFromData)[2] & 0x01) << 8;
#else

  /* Unpack operation based on little endian */

  /* Unpack Member: m_nReserved2 */
  (pToStruct)->m_nReserved2 =  (uint32_t)  ((pFromData)[3] >> 1) & 0x7f;

  /* Unpack Member: m_nOddMeter */
  (pToStruct)->m_nOddMeter =  (uint32_t)  (pFromData)[2] ;
  (pToStruct)->m_nOddMeter |=  (uint32_t)  ((pFromData)[3] & 0x01) << 8;

  /* Unpack Member: m_nReserved1 */
  (pToStruct)->m_nReserved1 =  (uint32_t)  ((pFromData)[1] >> 1) & 0x7f;

  /* Unpack Member: m_nEvenMeter */
  (pToStruct)->m_nEvenMeter =  (uint32_t)  (pFromData)[0] ;
  (pToStruct)->m_nEvenMeter |=  (uint32_t)  ((pFromData)[1] & 0x01) << 8;
#endif
