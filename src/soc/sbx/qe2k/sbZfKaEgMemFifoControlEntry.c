/*
 * $Id: sbZfKaEgMemFifoControlEntry.c 1.1.44.4 Broadcom SDK $
...
sbZfKaEgMemFifoControlEntry_Unpack(sbZfKaEgMemFifoControlEntry_t *pToStruct,
  ...
  (pToStruct)->m_nTailPage =  (uint32_t)  ((pFromData)[0] >> 2) & 0x3f;
  (pToStruct)->m_nTailPage |=  (uint32_t)  ((pFromData)[7] & 0x0f) << 6;

  /* Unpack Member: m_nTailOffset */
  (pToStruct)->m_nTailOffset =  (uint32_t)  ((pFromData)[1] >> 5) & 0x07;
  (pToStruct)->m_nTailOffset |=  (uint32_t)  ((pFromData)[0] & 0x03) << 3;

  /* Unpack Member: m_nCurrDepth */
  (pToStruct)->m_nCurrDepth =  (uint32_t)  ((pFromData)[3] >> 5) & 0x07;
  (pToStruct)->m_nCurrDepth |=  (uint32_t)  (pFromData)[2] << 3;
  (pToStruct)->m_nCurrDepth |=  (uint32_t)  ((pFromData)[1] & 0x1f) << 11;

  /* Unpack Member: m_nHeadOffset */
  (pToStruct)->m_nHeadOffset =  (uint32_t)  ((pFromData)[3] ) & 0x1f;
#else

  /* Unpack operation based on little endian */

  /* Unpack Member: m_nTailPage */
  (pToStruct)->m_nTailPage =  (uint32_t)  ((pFromData)[3] >> 2) & 0x3f;
  (pToStruct)->m_nTailPage |=  (uint32_t)  ((pFromData)[4] & 0x0f) << 6;

  /* Unpack Member: m_nTailOffset */
  (pToStruct)->m_nTailOffset =  (uint32_t)  ((pFromData)[2] >> 5) & 0x07;
  (pToStruct)->m_nTailOffset |=  (uint32_t)  ((pFromData)[3] & 0x03) << 3;

  /* Unpack Member: m_nCurrDepth */
  (pToStruct)->m_nCurrDepth =  (uint32_t)  ((pFromData)[0] >> 5) & 0x07;
  (pToStruct)->m_nCurrDepth |=  (uint32_t)  (pFromData)[1] << 3;
  (pToStruct)->m_nCurrDepth |=  (uint32_t)  ((pFromData)[2] & 0x1f) << 11;

  /* Unpack Member: m_nHeadOffset */
  (pToStruct)->m_nHeadOffset =  (uint32_t)  ((pFromData)[0] ) & 0x1f;
#endif
