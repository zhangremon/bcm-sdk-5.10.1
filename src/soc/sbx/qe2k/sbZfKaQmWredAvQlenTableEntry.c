/*
 * $Id: sbZfKaQmWredAvQlenTableEntry.c 1.1.44.4 Broadcom SDK $
...
sbZfKaQmWredAvQlenTableEntry_Unpack(sbZfKaQmWredAvQlenTableEntry_t *pToStruct,
  ...
  (pToStruct)->m_nQueueAvg =  (uint32_t)  (pFromData)[3] ;
  (pToStruct)->m_nQueueAvg |=  (uint32_t)  (pFromData)[2] << 8;
  (pToStruct)->m_nQueueAvg |=  (uint32_t)  (pFromData)[1] << 16;
  (pToStruct)->m_nQueueAvg |=  (uint32_t)  ((pFromData)[0] & 0x01) << 24;
#else

  /* Unpack operation based on little endian */

  /* Unpack Member: m_nQueueAvg */
  (pToStruct)->m_nQueueAvg =  (uint32_t)  (pFromData)[0] ;
  (pToStruct)->m_nQueueAvg |=  (uint32_t)  (pFromData)[1] << 8;
  (pToStruct)->m_nQueueAvg |=  (uint32_t)  (pFromData)[2] << 16;
  (pToStruct)->m_nQueueAvg |=  (uint32_t)  ((pFromData)[3] & 0x01) << 24;
#endif
