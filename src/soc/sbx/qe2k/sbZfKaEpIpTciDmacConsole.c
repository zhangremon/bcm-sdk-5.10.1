/*
 * $Id: sbZfKaEpIpTciDmacConsole.c 1.1.44.3 Broadcom SDK $
...
sbZfKaEpIpTciDmac_Print(sbZfKaEpIpTciDmac_t *pFromStruct) {
  SB_LOG("KaEpIpTciDmac:: pri=0x%01x", (unsigned int)  pFromStruct->m_nPri);
  SB_LOG(" cfi=0x%01x", (unsigned int)  pFromStruct->m_nCfi);
  SB_LOG(" vid=0x%03x", (unsigned int)  pFromStruct->m_nVid);
  SB_LOG(" dmac=0x%012llx", (uint64_t)  pFromStruct->m_nnDmac);
  SB_LOG("\n");
...
char *
sbZfKaEpIpTciDmac_SPrint(sbZfKaEpIpTciDmac_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," dmac=0x%012llx", (uint64_t)  pFromStruct->m_nnDmac);
