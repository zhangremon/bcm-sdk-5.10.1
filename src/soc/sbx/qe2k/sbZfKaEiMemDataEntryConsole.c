/*
 * $Id: sbZfKaEiMemDataEntryConsole.c 1.1.44.3 Broadcom SDK $
...
sbZfKaEiMemDataEntry_Print(sbZfKaEiMemDataEntry_t *pFromStruct) {
  SB_LOG("KaEiMemDataEntry:: destchannel=0x%02x", (unsigned int)  pFromStruct->m_nDestChannel);
  SB_LOG(" sizemask=0x%03x", (unsigned int)  pFromStruct->m_nSizeMask);
  SB_LOG(" rb_only=0x%01x", (unsigned int)  pFromStruct->m_nRbOnly);
  SB_LOG("\n");

  SB_LOG("KaEiMemDataEntry:: lineptr=0x%03x", (unsigned int)  pFromStruct->m_nLinePtr);
  SB_LOG(" byteptr=0x%01x", (unsigned int)  pFromStruct->m_nBytePtr);
  SB_LOG("\n");
...
char *
sbZfKaEiMemDataEntry_SPrint(sbZfKaEiMemDataEntry_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
