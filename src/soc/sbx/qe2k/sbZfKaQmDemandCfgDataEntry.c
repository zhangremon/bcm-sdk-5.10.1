/*
 * $Id: sbZfKaQmDemandCfgDataEntry.c 1.1.44.4 Broadcom SDK $
...
sbZfKaQmDemandCfgDataEntry_Unpack(sbZfKaQmDemandCfgDataEntry_t *pToStruct,
  ...
  (pToStruct)->m_nRateDeltaMaxIdx =  (uint32_t)  ((pFromData)[3] >> 1) & 0x3f;

  /* Unpack Member: m_nQlaDemandMask */
  (pToStruct)->m_nQlaDemandMask =  (uint32_t)  ((pFromData)[3] ) & 0x01;
#else

  /* Unpack operation based on little endian */

  /* Unpack Member: m_nRateDeltaMaxIdx */
  (pToStruct)->m_nRateDeltaMaxIdx =  (uint32_t)  ((pFromData)[0] >> 1) & 0x3f;

  /* Unpack Member: m_nQlaDemandMask */
  (pToStruct)->m_nQlaDemandMask =  (uint32_t)  ((pFromData)[0] ) & 0x01;
#endif
