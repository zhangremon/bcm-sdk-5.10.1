/*
 * $Id: sbZfKaQsE2QEntryConsole.c 1.1.44.3 Broadcom SDK $
...
sbZfKaQsE2QEntry_Print(sbZfKaQsE2QEntry_t *pFromStruct) {
  SB_LOG("KaQsE2QEntry:: res=0x%05x", (unsigned int)  pFromStruct->m_nReserved);
  SB_LOG(" enable=0x%01x", (unsigned int)  pFromStruct->m_nEnable);
  SB_LOG(" queuenum=0x%04x", (unsigned int)  pFromStruct->m_nQueueNum);
  SB_LOG("\n");
...
char *
sbZfKaQsE2QEntry_SPrint(sbZfKaQsE2QEntry_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
