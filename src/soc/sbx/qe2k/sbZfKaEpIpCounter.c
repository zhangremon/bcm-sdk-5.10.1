/*
 * $Id: sbZfKaEpIpCounter.c 1.1.44.4 Broadcom SDK $
...
sbZfKaEpIpCounter_Pack(sbZfKaEpIpCounter_t *pFrom,
  ...
  (pToData)[3] |= ((pFrom)->m_nnByteCnt) & 0xFF;
  (pToData)[2] |= ((pFrom)->m_nnByteCnt >> 8) &0xFF;
  (pToData)[1] |= ((pFrom)->m_nnByteCnt >> 16) &0xFF;
  (pToData)[0] |= ((pFrom)->m_nnByteCnt >> 24) &0xFF;
  (pToData)[7] |= ((pFrom)->m_nnByteCnt >> 32) & 0x07;
#else
  int i;
  int size = SB_ZF_ZFKAEPIPCOUNTER_SIZE_IN_BYTES;
...
sbZfKaEpIpCounter_Unpack(sbZfKaEpIpCounter_t *pToStruct,
  ...
  (pToStruct)->m_nPktCnt =  (uint32_t)  ((pFromData)[7] >> 3) & 0x1f;
  (pToStruct)->m_nPktCnt |=  (uint32_t)  (pFromData)[6] << 5;
  (pToStruct)->m_nPktCnt |=  (uint32_t)  (pFromData)[5] << 13;
  (pToStruct)->m_nPktCnt |=  (uint32_t)  (pFromData)[4] << 21;

  /* Unpack Member: m_nnByteCnt */
  COMPILER_64_SET((pToStruct)->m_nnByteCnt, 0,  (unsigned int) (pFromData)[3]);
  {
    VOL COMPILER_UINT64 *tmp0 = &(pToStruct)->m_nnByteCnt;
  ...
  /* Unpack Member: m_nPktCnt */
  (pToStruct)->m_nPktCnt =  (uint32_t)  ((pFromData)[4] >> 3) & 0x1f;
  (pToStruct)->m_nPktCnt |=  (uint32_t)  (pFromData)[5] << 5;
  (pToStruct)->m_nPktCnt |=  (uint32_t)  (pFromData)[6] << 13;
  (pToStruct)->m_nPktCnt |=  (uint32_t)  (pFromData)[7] << 21;
