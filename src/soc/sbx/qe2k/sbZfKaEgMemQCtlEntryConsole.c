/*
 * $Id: sbZfKaEgMemQCtlEntryConsole.c 1.1.44.3 Broadcom SDK $
...
sbZfKaEgMemQCtlEntry_Print(sbZfKaEgMemQCtlEntry_t *pFromStruct) {
  SB_LOG("KaEgMemQCtlEntry:: droponfull=0x%01x", (unsigned int)  pFromStruct->m_nDropOnFull);
  SB_LOG(" wptr=0x%02x", (unsigned int)  pFromStruct->m_nWptr);
  SB_LOG(" rptr=0x%02x", (unsigned int)  pFromStruct->m_nRptr);
  SB_LOG(" ssize=0x%01x", (unsigned int)  pFromStruct->m_nSize);
  SB_LOG(" base=0x%03x", (unsigned int)  pFromStruct->m_nBase);
  SB_LOG("\n");
...
char *
sbZfKaEgMemQCtlEntry_SPrint(sbZfKaEgMemQCtlEntry_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
