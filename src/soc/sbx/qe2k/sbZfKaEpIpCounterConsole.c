/*
 * $Id: sbZfKaEpIpCounterConsole.c 1.1.44.3 Broadcom SDK $
...
sbZfKaEpIpCounter_Print(sbZfKaEpIpCounter_t *pFromStruct) {
  SB_LOG("KaEpIpCounter:: pktcnt=0x%08x", (unsigned int)  pFromStruct->m_nPktCnt);
  SB_LOG(" bytecnt=0x%09llx", (uint64_t)  pFromStruct->m_nnByteCnt);
  SB_LOG("\n");
...
char *
sbZfKaEpIpCounter_SPrint(sbZfKaEpIpCounter_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"KaEpIpCounter:: pktcnt=0x%08x", (unsigned int)  pFromStruct->m_nPktCnt);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," bytecnt=0x%09llx", (uint64_t)  pFromStruct->m_nnByteCnt);
...
sbZfKaEpIpCounter_Validate(sbZfKaEpIpCounter_t *pZf) {
  ...
  if (pZf->m_nnByteCnt > 0x7FFFFFFFFULL) return 0;
