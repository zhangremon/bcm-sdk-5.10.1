/*
 * $Id: sbZfKaQsPriAddrConsole.c 1.1.44.3 Broadcom SDK $
...
sbZfKaQsPriAddr_Print(sbZfKaQsPriAddr_t *pFromStruct) {
  SB_LOG("KaQsPriAddr:: res=0x%04x", (unsigned int)  pFromStruct->m_nReserved);
  SB_LOG(" cos=0x%01x", (unsigned int)  pFromStruct->m_nCos);
  SB_LOG(" mc=0x%01x", (unsigned int)  pFromStruct->m_nMc);
...
char *
sbZfKaQsPriAddr_SPrint(sbZfKaQsPriAddr_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
