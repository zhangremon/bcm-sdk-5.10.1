/*
 * $Id: sbZfKaPmLastLineConsole.c 1.1.44.3 Broadcom SDK $
...
sbZfKaPmLastLine_Print(sbZfKaPmLastLine_t *pFromStruct) {
  SB_LOG("KaPmLastLine:: res=0x%015llx", (uint64_t)  pFromStruct->m_nReserved);
  SB_LOG(" hec=0x%02x", (unsigned int)  pFromStruct->m_nHec);
  SB_LOG(" zero=0x%03x", (unsigned int)  pFromStruct->m_nZero);
  SB_LOG("\n");

  SB_LOG("KaPmLastLine:: next_buffer=0x%05x", (unsigned int)  pFromStruct->m_nNextBuffer);
  SB_LOG(" timestamp=0x%08x", (unsigned int)  pFromStruct->m_nTimestamp);
  SB_LOG("\n");
...
char *
sbZfKaPmLastLine_SPrint(sbZfKaPmLastLine_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"KaPmLastLine:: res=0x%015llx", (uint64_t)  pFromStruct->m_nReserved);
...
sbZfKaPmLastLine_Validate(sbZfKaPmLastLine_t *pZf) {
  ...
  if (pZf->m_nReserved > 0xFFFFFFFFFFFFFFFULL) return 0;
