/*
 * $Id: sbZfKaRbClassSourceIdEntry.c 1.1.44.4 Broadcom SDK $
...
sbZfKaRbClassSourceIdEntry_Unpack(sbZfKaRbClassSourceIdEntry_t *pToStruct,
  ...
  (pToStruct)->m_nReserved2 =  (uint32_t)  ((pFromData)[0] >> 4) & 0x0f;

  /* Unpack Member: m_nSrcIdOdd */
  (pToStruct)->m_nSrcIdOdd =  (uint32_t)  (pFromData)[1] ;
  (pToStruct)->m_nSrcIdOdd |=  (uint32_t)  ((pFromData)[0] & 0x0f) << 8;

  /* Unpack Member: m_nReserved1 */
  (pToStruct)->m_nReserved1 =  (uint32_t)  ((pFromData)[2] >> 4) & 0x0f;

  /* Unpack Member: m_nSrcIdEven */
  (pToStruct)->m_nSrcIdEven =  (uint32_t)  (pFromData)[3] ;
  (pToStruct)->m_nSrcIdEven |=  (uint32_t)  ((pFromData)[2] & 0x0f) << 8;
#else

  /* Unpack operation based on little endian */

  /* Unpack Member: m_nReserved2 */
  (pToStruct)->m_nReserved2 =  (uint32_t)  ((pFromData)[3] >> 4) & 0x0f;

  /* Unpack Member: m_nSrcIdOdd */
  (pToStruct)->m_nSrcIdOdd =  (uint32_t)  (pFromData)[2] ;
  (pToStruct)->m_nSrcIdOdd |=  (uint32_t)  ((pFromData)[3] & 0x0f) << 8;

  /* Unpack Member: m_nReserved1 */
  (pToStruct)->m_nReserved1 =  (uint32_t)  ((pFromData)[1] >> 4) & 0x0f;

  /* Unpack Member: m_nSrcIdEven */
  (pToStruct)->m_nSrcIdEven =  (uint32_t)  (pFromData)[0] ;
  (pToStruct)->m_nSrcIdEven |=  (uint32_t)  ((pFromData)[1] & 0x0f) << 8;
#endif
