/*
 * $Id: sbZfKaEgNotTmePortRemapAddrConsole.c 1.1.44.3 Broadcom SDK $
...
sbZfKaEgNotTmePortRemapAddr_Print(sbZfKaEgNotTmePortRemapAddr_t *pFromStruct) {
  SB_LOG("KaEgNotTmePortRemapAddr:: txdma=0x%01x", (unsigned int)  pFromStruct->m_nTxdma);
  SB_LOG(" ef=0x%01x", (unsigned int)  pFromStruct->m_nEf);
  SB_LOG(" qe1k=0x%01x", (unsigned int)  pFromStruct->m_nQe1k);
  SB_LOG(" dport=0x%02x", (unsigned int)  pFromStruct->m_nPort);
  SB_LOG("\n");
...
char *
sbZfKaEgNotTmePortRemapAddr_SPrint(sbZfKaEgNotTmePortRemapAddr_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
