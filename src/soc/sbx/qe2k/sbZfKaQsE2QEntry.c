/*
 * $Id: sbZfKaQsE2QEntry.c 1.1.44.4 Broadcom SDK $
...
sbZfKaQsE2QEntry_Unpack(sbZfKaQsE2QEntry_t *pToStruct,
  ...
  (pToStruct)->m_nReserved =  (uint32_t)  ((pFromData)[2] >> 7) & 0x01;
  (pToStruct)->m_nReserved |=  (uint32_t)  (pFromData)[1] << 1;
  (pToStruct)->m_nReserved |=  (uint32_t)  (pFromData)[0] << 9;

  /* Unpack Member: m_nEnable */
  (pToStruct)->m_nEnable =  (uint8_t)  ((pFromData)[2] >> 6) & 0x01;

  /* Unpack Member: m_nQueueNum */
  (pToStruct)->m_nQueueNum =  (uint32_t)  (pFromData)[3] ;
  (pToStruct)->m_nQueueNum |=  (uint32_t)  ((pFromData)[2] & 0x3f) << 8;
#else

  /* Unpack operation based on little endian */

  /* Unpack Member: m_nReserved */
  (pToStruct)->m_nReserved =  (uint32_t)  ((pFromData)[1] >> 7) & 0x01;
  (pToStruct)->m_nReserved |=  (uint32_t)  (pFromData)[2] << 1;
  (pToStruct)->m_nReserved |=  (uint32_t)  (pFromData)[3] << 9;

  /* Unpack Member: m_nEnable */
  (pToStruct)->m_nEnable =  (uint8_t)  ((pFromData)[1] >> 6) & 0x01;

  /* Unpack Member: m_nQueueNum */
  (pToStruct)->m_nQueueNum =  (uint32_t)  (pFromData)[0] ;
  (pToStruct)->m_nQueueNum |=  (uint32_t)  ((pFromData)[1] & 0x3f) << 8;
#endif
