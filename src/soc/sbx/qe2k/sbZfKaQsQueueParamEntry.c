/*
 * $Id: sbZfKaQsQueueParamEntry.c 1.1.44.4 Broadcom SDK $
...
sbZfKaQsQueueParamEntry_Unpack(sbZfKaQsQueueParamEntry_t *pToStruct,
  ...
  (pToStruct)->m_nLocal =  (uint32_t)  ((pFromData)[3] >> 7) & 0x01;

  /* Unpack Member: m_nMaxHoldTs */
  (pToStruct)->m_nMaxHoldTs =  (uint32_t)  ((pFromData)[3] >> 4) & 0x07;

  /* Unpack Member: m_nQType */
  (pToStruct)->m_nQType =  (uint32_t)  ((pFromData)[3] ) & 0x0f;
#else
  ...
  /* Unpack Member: m_nLocal */
  (pToStruct)->m_nLocal =  (uint32_t)  ((pFromData)[0] >> 7) & 0x01;

  /* Unpack Member: m_nMaxHoldTs */
  (pToStruct)->m_nMaxHoldTs =  (uint32_t)  ((pFromData)[0] >> 4) & 0x07;

  /* Unpack Member: m_nQType */
  (pToStruct)->m_nQType =  (uint32_t)  ((pFromData)[0] ) & 0x0f;
#endif
