/*
 * $Id: sbZfKaEpIpV6TciConsole.c 1.1.44.3 Broadcom SDK $
...
sbZfKaEpIpV6Tci_Print(sbZfKaEpIpV6Tci_t *pFromStruct) {
  SB_LOG("KaEpIpV6Tci:: rsvd=0x%012llx", (uint64_t)  pFromStruct->m_nReserved);
  SB_LOG(" pri=0x%01x", (unsigned int)  pFromStruct->m_nPri);
  SB_LOG(" cfi=0x%01x", (unsigned int)  pFromStruct->m_nCfi);
  SB_LOG(" vid=0x%03x", (unsigned int)  pFromStruct->m_nVid);
  SB_LOG("\n");
...
char *
sbZfKaEpIpV6Tci_SPrint(sbZfKaEpIpV6Tci_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"KaEpIpV6Tci:: rsvd=0x%012llx", (uint64_t)  pFromStruct->m_nReserved);
...
sbZfKaEpIpV6Tci_Validate(sbZfKaEpIpV6Tci_t *pZf) {
  ...
  if (pZf->m_nReserved > 0xFFFFFFFFFFFFULL) return 0;
