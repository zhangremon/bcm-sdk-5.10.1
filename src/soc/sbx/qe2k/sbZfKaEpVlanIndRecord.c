/*
 * $Id: sbZfKaEpVlanIndRecord.c 1.1.44.4 Broadcom SDK $
...
sbZfKaEpVlanIndRecord_Unpack(sbZfKaEpVlanIndRecord_t *pToStruct,
  ...
  (pToStruct)->m_nPtr =  (uint32_t)  ((pFromData)[3] >> 2) & 0x3f;
  (pToStruct)->m_nPtr |=  (uint32_t)  (pFromData)[2] << 6;

  /* Unpack Member: m_nCMap */
  (pToStruct)->m_nCMap =  (uint32_t)  ((pFromData)[3] ) & 0x03;
#else

  /* Unpack operation based on little endian */

  /* Unpack Member: m_nPtr */
  (pToStruct)->m_nPtr =  (uint32_t)  ((pFromData)[0] >> 2) & 0x3f;
  (pToStruct)->m_nPtr |=  (uint32_t)  (pFromData)[1] << 6;

  /* Unpack Member: m_nCMap */
  (pToStruct)->m_nCMap =  (uint32_t)  ((pFromData)[0] ) & 0x03;
#endif
