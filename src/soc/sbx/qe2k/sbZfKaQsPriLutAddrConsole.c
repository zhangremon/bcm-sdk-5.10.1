/*
 * $Id: sbZfKaQsPriLutAddrConsole.c 1.1.44.3 Broadcom SDK $
...
sbZfKaQsPriLutAddr_Print(sbZfKaQsPriLutAddr_t *pFromStruct) {
  SB_LOG("KaQsPriLutAddr:: res=0x%05x", (unsigned int)  pFromStruct->m_nReserved);
  SB_LOG(" shaped=0x%01x", (unsigned int)  pFromStruct->m_nShaped);
  SB_LOG(" depth=0x%01x", (unsigned int)  pFromStruct->m_nDepth);
  SB_LOG(" anemicaged=0x%01x", (unsigned int)  pFromStruct->m_nAnemicAged);
  SB_LOG(" qtype=0x%01x", (unsigned int)  pFromStruct->m_nQType);
  SB_LOG("\n");

  SB_LOG("KaQsPriLutAddr:: efaged=0x%01x", (unsigned int)  pFromStruct->m_nEfAged);
  SB_LOG(" creditlevel=0x%01x", (unsigned int)  pFromStruct->m_nCreditLevel);
  SB_LOG(" holdts=0x%01x", (unsigned int)  pFromStruct->m_nHoldTs);
  SB_LOG(" pktlen=0x%01x", (unsigned int)  pFromStruct->m_nPktLen);
  SB_LOG("\n");
...
char *
sbZfKaQsPriLutAddr_SPrint(sbZfKaQsPriLutAddr_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
