/*
 * $Id: sbZfKaEgMemFifoControlEntryConsole.c 1.1.44.3 Broadcom SDK $
...
sbZfKaEgMemFifoControlEntry_Print(sbZfKaEgMemFifoControlEntry_t *pFromStruct) {
  SB_LOG("KaEgMemFifoControlEntry:: tailpage=0x%03x", (unsigned int)  pFromStruct->m_nTailPage);
  SB_LOG(" tailoffset=0x%02x", (unsigned int)  pFromStruct->m_nTailOffset);
  SB_LOG(" currdepth=0x%04x", (unsigned int)  pFromStruct->m_nCurrDepth);
  SB_LOG("\n");

  SB_LOG("KaEgMemFifoControlEntry:: headoffset=0x%02x", (unsigned int)  pFromStruct->m_nHeadOffset);
  SB_LOG("\n");
...
char *
sbZfKaEgMemFifoControlEntry_SPrint(sbZfKaEgMemFifoControlEntry_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
