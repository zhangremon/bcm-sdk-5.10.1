/*
 * $Id: sbZfKaQsAgeThreshLutEntryConsole.c 1.1.44.3 Broadcom SDK $
...
sbZfKaQsAgeThreshLutEntry_Print(sbZfKaQsAgeThreshLutEntry_t *pFromStruct) {
  SB_LOG("KaQsAgeThreshLutEntry:: res=0x%04x", (unsigned int)  pFromStruct->m_nReserved);
  SB_LOG(" anemicthresh=0x%02x", (unsigned int)  pFromStruct->m_nAnemicThresh);
  SB_LOG(" efthresh=0x%02x", (unsigned int)  pFromStruct->m_nEfThresh);
  SB_LOG("\n");
...
char *
sbZfKaQsAgeThreshLutEntry_SPrint(sbZfKaQsAgeThreshLutEntry_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
