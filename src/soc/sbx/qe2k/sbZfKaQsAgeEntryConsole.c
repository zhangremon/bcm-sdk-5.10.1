/*
 * $Id: sbZfKaQsAgeEntryConsole.c 1.1.44.3 Broadcom SDK $
...
sbZfKaQsAgeEntry_Print(sbZfKaQsAgeEntry_t *pFromStruct) {
  SB_LOG("KaQsAgeEntry:: res=0x%06x", (unsigned int)  pFromStruct->m_nReserved);
  SB_LOG(" noempty=0x%01x", (unsigned int)  pFromStruct->m_nNoEmpty);
  SB_LOG(" anemicevent=0x%01x", (unsigned int)  pFromStruct->m_nAnemicEvent);
  SB_LOG(" efevent=0x%01x", (unsigned int)  pFromStruct->m_nEfEvent);
  SB_LOG("\n");

  SB_LOG("KaQsAgeEntry:: cnt=0x%02x", (unsigned int)  pFromStruct->m_nCnt);
  SB_LOG("\n");
...
char *
sbZfKaQsAgeEntry_SPrint(sbZfKaQsAgeEntry_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
