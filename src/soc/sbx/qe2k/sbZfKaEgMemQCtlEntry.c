/*
 * $Id: sbZfKaEgMemQCtlEntry.c 1.1.44.4 Broadcom SDK $
...
sbZfKaEgMemQCtlEntry_Unpack(sbZfKaEgMemQCtlEntry_t *pToStruct,
  ...
  (pToStruct)->m_nDropOnFull =  (uint32_t)  ((pFromData)[0] >> 1) & 0x01;

  /* Unpack Member: m_nWptr */
  (pToStruct)->m_nWptr =  (uint32_t)  ((pFromData)[1] >> 3) & 0x1f;
  (pToStruct)->m_nWptr |=  (uint32_t)  ((pFromData)[0] & 0x01) << 5;

  /* Unpack Member: m_nRptr */
  (pToStruct)->m_nRptr =  (uint32_t)  ((pFromData)[2] >> 5) & 0x07;
  (pToStruct)->m_nRptr |=  (uint32_t)  ((pFromData)[1] & 0x07) << 3;

  /* Unpack Member: m_nSize */
  (pToStruct)->m_nSize =  (uint32_t)  ((pFromData)[2] >> 2) & 0x07;

  /* Unpack Member: m_nBase */
  (pToStruct)->m_nBase =  (uint32_t)  (pFromData)[3] ;
  (pToStruct)->m_nBase |=  (uint32_t)  ((pFromData)[2] & 0x03) << 8;
#else

  /* Unpack operation based on little endian */

  /* Unpack Member: m_nDropOnFull */
  (pToStruct)->m_nDropOnFull =  (uint32_t)  ((pFromData)[3] >> 1) & 0x01;

  /* Unpack Member: m_nWptr */
  (pToStruct)->m_nWptr =  (uint32_t)  ((pFromData)[2] >> 3) & 0x1f;
  (pToStruct)->m_nWptr |=  (uint32_t)  ((pFromData)[3] & 0x01) << 5;

  /* Unpack Member: m_nRptr */
  (pToStruct)->m_nRptr =  (uint32_t)  ((pFromData)[1] >> 5) & 0x07;
  (pToStruct)->m_nRptr |=  (uint32_t)  ((pFromData)[2] & 0x07) << 3;

  /* Unpack Member: m_nSize */
  (pToStruct)->m_nSize =  (uint32_t)  ((pFromData)[1] >> 2) & 0x07;

  /* Unpack Member: m_nBase */
  (pToStruct)->m_nBase =  (uint32_t)  (pFromData)[0] ;
  (pToStruct)->m_nBase |=  (uint32_t)  ((pFromData)[1] & 0x03) << 8;
#endif
