/*
 * $Id: sbZfKaQmDemandCfgDataEntryConsole.c 1.1.44.3 Broadcom SDK $
...
sbZfKaQmDemandCfgDataEntry_Print(sbZfKaQmDemandCfgDataEntry_t *pFromStruct) {
  SB_LOG("KaQmDemandCfgDataEntry:: ratedeltamaxids=0x%02x", (unsigned int)  pFromStruct->m_nRateDeltaMaxIdx);
  SB_LOG(" qlademandmask=0x%01x", (unsigned int)  pFromStruct->m_nQlaDemandMask);
  SB_LOG("\n");
...
char *
sbZfKaQmDemandCfgDataEntry_SPrint(sbZfKaQmDemandCfgDataEntry_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
