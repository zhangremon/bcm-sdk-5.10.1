/*
 * $Id: sbZfKaEpVlanIndRecordConsole.c 1.1.44.3 Broadcom SDK $
...
sbZfKaEpVlanIndRecord_Print(sbZfKaEpVlanIndRecord_t *pFromStruct) {
  SB_LOG("KaEpVlanIndRecord:: ptr=0x%04x", (unsigned int)  pFromStruct->m_nPtr);
  SB_LOG(" cmap=0x%01x", (unsigned int)  pFromStruct->m_nCMap);
  SB_LOG("\n");
...
char *
sbZfKaEpVlanIndRecord_SPrint(sbZfKaEpVlanIndRecord_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
