/*
 * $Id: sbZfKaEbMvtEntry.c 1.1.44.4 Broadcom SDK $
...
sbZfKaEbMvtEntry_Pack(sbZfKaEbMvtEntry_t *pFrom,
  ...
  (pToData)[7] |= ((pFrom)->m_nPortMap & 0x1f) <<3;
  (pToData)[6] |= ((pFrom)->m_nPortMap >> 5) &0xFF;
  (pToData)[5] |= ((pFrom)->m_nPortMap >> 13) &0xFF;
  (pToData)[4] |= ((pFrom)->m_nPortMap >> 21) &0xFF;
  (pToData)[11] |= ((pFrom)->m_nPortMap >> 29) &0xFF;
  (pToData)[10] |= ((pFrom)->m_nPortMap >> 37) &0xFF;
  (pToData)[9] |= ((pFrom)->m_nPortMap >> 45) & 0x1f;
...
sbZfKaEbMvtEntry_Unpack(sbZfKaEbMvtEntry_t *pToStruct,
  ...
  {
    VOL COMPILER_UINT64 *tmp0 = &(pToStruct)->m_nPortMap;
  ...
  (pToStruct)->m_nMvtda =  (uint32_t)  ((pFromData)[1] >> 5) & 0x07;
  (pToStruct)->m_nMvtda |=  (uint32_t)  (pFromData)[0] << 3;
  (pToStruct)->m_nMvtda |=  (uint32_t)  ((pFromData)[7] & 0x07) << 11;

  /* Unpack Member: m_nMvtdb */
  (pToStruct)->m_nMvtdb =  (uint32_t)  ((pFromData)[1] >> 1) & 0x0f;

  /* Unpack Member: m_nNext */
  (pToStruct)->m_nNext =  (uint32_t)  ((pFromData)[3] >> 1) & 0x7f;
  (pToStruct)->m_nNext |=  (uint32_t)  (pFromData)[2] << 7;
  (pToStruct)->m_nNext |=  (uint32_t)  ((pFromData)[1] & 0x01) << 15;

  /* Unpack Member: m_nKnockout */
  (pToStruct)->m_nKnockout =  (uint32_t)  ((pFromData)[3] ) & 0x01;
#else
