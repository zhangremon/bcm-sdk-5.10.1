/*
 * $Id: sbZfKaQsPriAddr.c 1.1.44.4 Broadcom SDK $
...
sbZfKaQsPriAddr_Unpack(sbZfKaQsPriAddr_t *pToStruct,
  ...
  (pToStruct)->m_nCos =  (uint32_t)  ((pFromData)[2] >> 5) & 0x07;

  /* Unpack Member: m_nMc */
  (pToStruct)->m_nMc =  (uint32_t)  ((pFromData)[2] >> 4) & 0x01;

  /* Unpack Member: m_nNode */
  (pToStruct)->m_nNode =  (uint32_t)  ((pFromData)[3] >> 7) & 0x01;
  (pToStruct)->m_nNode |=  (uint32_t)  ((pFromData)[2] & 0x0f) << 1;

  /* Unpack Member: m_nPort */
  (pToStruct)->m_nPort =  (uint32_t)  ((pFromData)[3] ) & 0x7f;
#else
  ...
  /* Unpack Member: m_nCos */
  (pToStruct)->m_nCos =  (uint32_t)  ((pFromData)[1] >> 5) & 0x07;

  /* Unpack Member: m_nMc */
  (pToStruct)->m_nMc =  (uint32_t)  ((pFromData)[1] >> 4) & 0x01;

  /* Unpack Member: m_nNode */
  (pToStruct)->m_nNode =  (uint32_t)  ((pFromData)[0] >> 7) & 0x01;
  (pToStruct)->m_nNode |=  (uint32_t)  ((pFromData)[1] & 0x0f) << 1;

  /* Unpack Member: m_nPort */
  (pToStruct)->m_nPort =  (uint32_t)  ((pFromData)[0] ) & 0x7f;
#endif
