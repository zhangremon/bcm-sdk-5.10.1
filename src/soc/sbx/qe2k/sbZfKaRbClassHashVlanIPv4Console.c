/*
 * $Id: sbZfKaRbClassHashVlanIPv4Console.c 1.1.44.3 Broadcom SDK $
...
sbZfKaRbClassHashVlanIPv4_Print(sbZfKaRbClassHashVlanIPv4_t *pFromStruct) {
  SB_LOG("KaRbClassHashVlanIPv4:: protocol=0x%02x", (unsigned int)  pFromStruct->m_nProtocol);
  SB_LOG(" pad3=0x%011llx", (uint64_t)  pFromStruct->m_nPadWord3);
  SB_LOG("\n");

  SB_LOG("KaRbClassHashVlanIPv4:: pad1=0x%016llx", (uint64_t)  pFromStruct->m_nPadWord1);
  SB_LOG(" ipsa=0x%08x", (unsigned int)  pFromStruct->m_nIpSa);
  SB_LOG("\n");

  SB_LOG("KaRbClassHashVlanIPv4:: ipda=0x%08x", (unsigned int)  pFromStruct->m_nIpDa);
  SB_LOG(" skt=0x%08x", (unsigned int)  pFromStruct->m_nSocket);
  SB_LOG(" pad2=0x%012llx", (uint64_t)  pFromStruct->m_nPadWord2);
  SB_LOG("\n");

  SB_LOG("KaRbClassHashVlanIPv4:: word1=0x%016llx", (uint64_t)  pFromStruct->m_nSpareWord1);
...
char *
sbZfKaRbClassHashVlanIPv4_SPrint(sbZfKaRbClassHashVlanIPv4_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"KaRbClassHashVlanIPv4:: protocol=0x%02x", (unsigned int)  pFromStruct->m_nProtocol);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," pad3=0x%011llx", (uint64_t)  pFromStruct->m_nPadWord3);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"KaRbClassHashVlanIPv4:: pad1=0x%016llx", (uint64_t)  pFromStruct->m_nPadWord1);
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," pad2=0x%012llx", (uint64_t)  pFromStruct->m_nPadWord2);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"KaRbClassHashVlanIPv4:: word1=0x%016llx", (uint64_t)  pFromStruct->m_nSpareWord1);
...
sbZfKaRbClassHashVlanIPv4_Validate(sbZfKaRbClassHashVlanIPv4_t *pZf) {
  ...
  if (pZf->m_nPadWord3 > 0x1FFFFFFFFFFULL) return 0;
  if (pZf->m_nPadWord1 > 0x7FFFFFFFFFFFFFFFULL) return 0;
  ...
  if (pZf->m_nPadWord2 > 0xFFFFFFFFFFFFULL) return 0;
