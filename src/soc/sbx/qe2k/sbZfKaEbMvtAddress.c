/*
 * $Id: sbZfKaEbMvtAddress.c 1.1.44.4 Broadcom SDK $
...
sbZfKaEbMvtAddress_Unpack(sbZfKaEbMvtAddress_t *pToStruct,
  ...
  (pToStruct)->m_nReserved =  (uint32_t)  ((pFromData)[1] >> 6) & 0x03;
  (pToStruct)->m_nReserved |=  (uint32_t)  (pFromData)[0] << 2;

  /* Unpack Member: m_nEgress */
  (pToStruct)->m_nEgress =  (uint32_t)  ((pFromData)[1] ) & 0x3f;

  /* Unpack Member: m_nOffset */
  (pToStruct)->m_nOffset =  (uint32_t)  (pFromData)[3] ;
  (pToStruct)->m_nOffset |=  (uint32_t)  (pFromData)[2] << 8;
#else

  /* Unpack operation based on little endian */

  /* Unpack Member: m_nReserved */
  (pToStruct)->m_nReserved =  (uint32_t)  ((pFromData)[2] >> 6) & 0x03;
  (pToStruct)->m_nReserved |=  (uint32_t)  (pFromData)[3] << 2;

  /* Unpack Member: m_nEgress */
  (pToStruct)->m_nEgress =  (uint32_t)  ((pFromData)[2] ) & 0x3f;

  /* Unpack Member: m_nOffset */
  (pToStruct)->m_nOffset =  (uint32_t)  (pFromData)[0] ;
  (pToStruct)->m_nOffset |=  (uint32_t)  (pFromData)[1] << 8;
#endif
