/*
 * $Id: sbZfKaQsShapeMaxBurstEntryConsole.c 1.1.44.3 Broadcom SDK $
...
sbZfKaQsShapeMaxBurstEntry_Print(sbZfKaQsShapeMaxBurstEntry_t *pFromStruct) {
  SB_LOG("KaQsShapeMaxBurstEntry:: res=0x%02x", (unsigned int)  pFromStruct->m_nReserved);
  SB_LOG(" enable=0x%01x", (unsigned int)  pFromStruct->m_nEnable);
  SB_LOG(" shapemaxburst=0x%06x", (unsigned int)  pFromStruct->m_nShapeMaxBurst);
  SB_LOG("\n");
...
char *
sbZfKaQsShapeMaxBurstEntry_SPrint(sbZfKaQsShapeMaxBurstEntry_t *pFromStruct, char *pcToString, uint32_t lStrSize) {
  uint32_t WrCnt = 0x0;
