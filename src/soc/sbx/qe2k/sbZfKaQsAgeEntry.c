/*
 * $Id: sbZfKaQsAgeEntry.c 1.1.44.4 Broadcom SDK $
...
sbZfKaQsAgeEntry_Unpack(sbZfKaQsAgeEntry_t *pToStruct,
  ...
  (pToStruct)->m_nReserved =  (uint32_t)  ((pFromData)[2] >> 3) & 0x1f;
  (pToStruct)->m_nReserved |=  (uint32_t)  (pFromData)[1] << 5;
  (pToStruct)->m_nReserved |=  (uint32_t)  (pFromData)[0] << 13;

  /* Unpack Member: m_nNoEmpty */
  (pToStruct)->m_nNoEmpty =  (uint8_t)  ((pFromData)[2] >> 2) & 0x01;

  /* Unpack Member: m_nAnemicEvent */
  (pToStruct)->m_nAnemicEvent =  (uint8_t)  ((pFromData)[2] >> 1) & 0x01;

  /* Unpack Member: m_nEfEvent */
  (pToStruct)->m_nEfEvent =  (uint8_t)  ((pFromData)[2] ) & 0x01;

  /* Unpack Member: m_nCnt */
  (pToStruct)->m_nCnt =  (uint32_t)  (pFromData)[3] ;
#else

  /* Unpack operation based on little endian */

  /* Unpack Member: m_nReserved */
  (pToStruct)->m_nReserved =  (uint32_t)  ((pFromData)[1] >> 3) & 0x1f;
  (pToStruct)->m_nReserved |=  (uint32_t)  (pFromData)[2] << 5;
  (pToStruct)->m_nReserved |=  (uint32_t)  (pFromData)[3] << 13;

  /* Unpack Member: m_nNoEmpty */
  (pToStruct)->m_nNoEmpty =  (uint8_t)  ((pFromData)[1] >> 2) & 0x01;

  /* Unpack Member: m_nAnemicEvent */
  (pToStruct)->m_nAnemicEvent =  (uint8_t)  ((pFromData)[1] >> 1) & 0x01;

  /* Unpack Member: m_nEfEvent */
  (pToStruct)->m_nEfEvent =  (uint8_t)  ((pFromData)[1] ) & 0x01;

  /* Unpack Member: m_nCnt */
  (pToStruct)->m_nCnt =  (uint32_t)  (pFromData)[0] ;
#endif
