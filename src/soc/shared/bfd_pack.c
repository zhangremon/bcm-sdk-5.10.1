/* 
 * $Id: bfd_pack.c 1.1.2.8 Broadcom SDK $
...
uint8 *
shr_bfd_msg_ctrl_encap_pack(uint8 *buf, shr_bfd_msg_ctrl_encap_t *msg)
{
    uint32 i;

    _SHR_PACK_U32(buf, msg->offset);
    _SHR_PACK_U32(buf, msg->length);
    for (i = 0; i < msg->length; i++) {
        _SHR_PACK_U8(buf, msg->data[i]);
    }
...
uint8 *
shr_bfd_msg_ctrl_encap_unpack(uint8 *buf, shr_bfd_msg_ctrl_encap_t *msg)
{
    uint32 i;

    _SHR_UNPACK_U32(buf, msg->offset);
    _SHR_UNPACK_U32(buf, msg->length);
    for (i = 0; i < msg->length; i++) {
        _SHR_UNPACK_U8(buf, msg->data[i]);
    }
