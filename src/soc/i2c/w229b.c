...
w311_ss_print(char* pfx, uint8 cb)
{
	...
	modeStr = "unknown";
    soc_cm_print("w311: %s spread spectrum %s (0x%x)\n", pfx, modeStr, cb);
}
	   ...
	   int opcode, void* data, int len)
{
	...
	if ( speed < MHZ  )
	    speed *= MHZ;
	soc_cm_debug(DK_VERBOSE, "Speed requested = %2.2f\n", speed);
#else
	speed = *((int*)data);
	soc_cm_debug(DK_VERBOSE, "Speed requested = %d.%02d\n", 
...
#ifdef	COMPILER_HAS_DOUBLE
#ifdef CLOCK_DEBUG
	    soc_cm_debug(DK_VERBOSE, "w311: FS_Sel[0:4]=0x%x f=%2.2f\n",
			 n,0.0);
#endif
	    *((double*)data) = 0.0;
#else
...
#ifdef CLOCK_DEBUG
		soc_cm_debug(DK_VERBOSE, "n=0x%x tab=0x%x\n",
...
#ifdef	COMPILER_HAS_DOUBLE
#ifdef CLOCK_DEBUG
	    soc_cm_debug(DK_VERBOSE, "w311: m/n=0x%x m=0x%x n=0x%x, f=%2.2f\n",
			 nm, m, n,
	  ...
	  void* data, int len)
{
	...
	if((soc_i2c_read_byte_data(unit, saddr, W311_ID_REG, &rev)) < 0){
	    ...
	    soc_cm_debug(DK_VERBOSE, "%s: vendor 0x%x revision 0x%x\n",
