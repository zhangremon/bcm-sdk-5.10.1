/*
 * $Id: bsc_at24c64.c 1.15 Broadcom SDK $
...
eep24c64_get_params(uint32 unit, uint32 devno, eep24c64_t* config)
{
    ...
    if ((retv = soc_eep24c64_read(unit, devno,
        ...
        soc_cm_print("eep24c64_get_params: %s\n", soc_errmsg(retv));
        return retv;
    }
    ...
    if (sum != savesum) {
        soc_cm_print("EEPROM: bad chksum: 0x%04x calc: 0x%04x\n",
                savesum, sum);
...
eep24c64_set_params(uint32 unit, uint32 devno, eep24c64_t* config)
{
    ...
    soc_cm_print("EEPROM: set config at 0x%04x\n", (int)AT24C64_CONFIG_START);
...
at24c64_init(int unit, int devno)
{
    ...
    if (eep24c64_get_params(unit, devno, &config) < 0 ) {
        ...
        if(sal_strlen(devname) > LC2464_NAME_LEN) {
            ...
            sal_strcpy(config.name, devname);
        }
        soc_cm_print("EEPROM: initiailzing ... \n");
        retv = eep24c64_set_params(unit, devno, &config);
        soc_cm_print("EEPROM: done\n");
    }
    return retv;
}
...
soc_eep24c64_test(int unit, int devno)
{
    ...
    soc_cm_print(" testing data integrity\n");
    ...
    soc_cm_print(" writing %d bytes pattern=0x%x\n", len, pattern);

    retv = soc_eep24c64_write(unit, devno, 0, outbuf, len);
    if (retv != SOC_E_NONE) {
        soc_cm_print(" write failed: %s\n", soc_errmsg(len));
        goto err_exit;
    }

    soc_cm_print(" reading %d bytes\n", len);
    ...
    if ((len == AT24C64_DEVICE_RW_SIZE) && (retv == SOC_E_NONE)) {
        ...
        if (j == AT24C64_DEVICE_RW_SIZE) {
            soc_cm_print(" test passed (%d bytes verified)\n", len);
        }
    }
...
#ifndef PLISIM
static int
_my_mc_fpga_read8(int addr, uint8_t *v)
{
    int lcm0 = -1;
    int lcm1 = -1;
    int pl = -1;
    ...
    if (lcm1 != -1){
      ...
      soc_cm_print("LCM/BME with FPGA not found\n");
      return SOC_E_INTERNAL;
    }
...
static int get_board_info(uint8 *board_id, uint8 *slot_id)
{
        ...
        if (retv) {
                soc_cm_print("get_board_info: FPGA read error\n");
                return retv;
        }
...
err_exit:
    BSC_UNLOCK(unit);
    soc_cm_print("at24c64_write: error\n");
    return retv;
}
...
err_exit:
    BSC_UNLOCK(unit);
    soc_cm_print("at24c64_read: error\n");
    return retv;
}
