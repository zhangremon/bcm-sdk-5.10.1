...
static struct pca9505_port_config {
    ...
    uint8 op_config;         /* */
} pca9505_port_config[PCA9505_DEV_MAX][PCA9505_PORT_MAX] = {
    {   /* device @ SADDR0 */
        { 0xff, 0xff, 0x00 },  /* port 0: SFP_ABS[0..7]    */
        { 0x00, 0xff, 0x00 },  /* port 1: SFP_TXDIS[0..7]  */
        { 0xff, 0xff, 0x00 },  /* port 2: SFP_TXFLT[0..7]  */
        { 0xff, 0xff, 0x00 },  /* port 3: QSFP_PRS[0..3], QSFP_INT[4..7] */
        { 0xff, 0xff, 0x00 },  /* port 4: UNUSED */
    },
    {   /* device @ SADDR1 */
        { 0xff, 0xff, 0x00 },  /* port 0: XON_XOFF[0..7]  */ /* changed this
        to input as per Doug's request as these are overloaded for EB3 address
        */
        { 0xff, 0xff, 0x00 },  /* port 1: BRD_ID[0..7]    */
        { 0x00, 0xff, 0x00 },  /* port 2: SHAD_GPIO[0..7] */
        { 0xff, 0xff, 0x00 },  /* port 3: UNUSED          */
        { 0xff, 0xff, 0x00 },  /* port 4: JTAG_TCE[0]     */
    },
    {   /* device @ SADDR0 */
        { 0xe0, 0xff, 0x0e },  /* port 0: MOD_ABS, LO-PWR, PRG[3:1], TXDIS */
        { 0xe0, 0xff, 0x0e },  /* port 1: MOD_ABS, LO-PWR, PRG[3:1], TXDIS */
        { 0xff, 0xff, 0x00 },  /* port 2: SFP_TXFLT[0..7]  */
        { 0xff, 0xff, 0x00 },  /* port 3: QSFP_PRS[0..3], QSFP_INT[4..7] */
        { 0xff, 0xff, 0x00 },  /* port 4: UNUSED */
    }
...
pca9505_read(int unit, int devno, uint16 addr, uint8 *data, uint32 *len)
{
    ...
    *len = 1; /* Byte device */
    /*soc_cm_print("iop@0x%x: read 0x%x from addr 0x%x\n", saddr, *data, addr);*/
...
pca9505_write(int unit, int devno, uint16 addr, uint8 *data, uint32 len)
{
    ...
    /* soc_cm_print("iop@0x%x: wrote 0x%x to addr 0x%x\n", saddr, *data, addr);*/
...
pca9505_init(int unit, int devno, void* data, int len)
{
	...
	soc_cm_print("%s: invalid address 0x%x\n",
    ...
    for (port = 0; port < PCA9505_PORT_MAX; port++) {
        ...
        if (rv != SOC_E_NONE) {
            soc_cm_print("Failed initializing PCA9505 pin direction "
                         "addr=0x%x, port=%d\n", saddr, port);
            return SOC_E_FAIL;
        }
        ...
        rv = pca9505_write(unit, devno, PCA9505_IOP_BASE + port, 
                           &pca9505_port_config[dev_idx][port].op_config, 1);
        rv = pca9505_write(unit, devno, PCA9505_MSK_BASE + port, 
                           &pca9505_port_config[dev_idx][port].mask_intr, 1);
        if (rv != SOC_E_NONE) {
            soc_cm_print("Failed initializing PCA9505 interrupt mask "
                         "addr=0x%x, port=%d\n", saddr, port);
        }
    }        
    return SOC_E_NONE;
...
pca9505_dump(void)
{
    ...
    for(saddr = 0x24; saddr >= 0x22; saddr -= 2) {
        for(i = 0;i < 6;i++) {
