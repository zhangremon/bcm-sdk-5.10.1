 ...
 */
STATIC int
ltc1427_setmin_max(int unit, int devno, int set_max, int idx)
{
    if ( set_max)
	return ltc1427_write(unit, devno, 0,
			     (uint8*)&dac_params[idx].dac_max_hwval,
			     sizeof(short));
    else
	return ltc1427_write(unit, devno, 0,
			     (uint8*)&dac_params[idx].dac_min_hwval,
			     sizeof(short));
}

#define LTC1427_SETMAX(unit,devno, idx)\
   ltc1427_setmin_max(unit, devno, 1, idx)
#define LTC1427_SETMIN(unit,devno, idx)\
   ltc1427_setmin_max(unit, devno, 0, idx)
	   ...
	   int opcode, void* data, int len)
{
	...
	/* Calibrate stepsize */
	dac_params[len].step =
	    (dac_params[len].max - dac_params[len].min) / LTC1427_MAX_DACVAL;
