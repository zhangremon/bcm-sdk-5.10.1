...
static int zm73xx_read(int unit, int devno, uint32 reg, uint32 *data)
{
	...
	if (retv != SB_FE2000_STS_INIT_OK_K) {
		soc_cm_print("zm73xx: unable to read dpm reg rel0\n");
		goto err_exit;
	}
...
static int zm73xx_init(int unit, int devno)
{
	...
	if (retv != SOC_E_NONE) {
		soc_cm_print("zm73xx: unable to write dpm reg ZM73XX_DPM_REG_WP\n");
		goto err_exit;
	}
	...
	retv = zm73xx_read(unit, devno, ZM73XX_DPM_REG_REL0, &dpm_reg_rel0);
	if (retv != SOC_E_NONE) {
		soc_cm_print("zm73xx: unable to read dpm reg ZM73XX_DPM_REG_REL0\n");
		goto err_exit;
	}

	retv = zm73xx_read(unit, devno, ZM73XX_DPM_REG_REL1, &dpm_reg_rel1);
	if (retv != SOC_E_NONE) {
		soc_cm_print("zm73xx: unable to read dpm reg ZM73XX_DPM_REG_REL1\n");
		goto err_exit;
	}
	...
	sc->dpm_gen = dpm_reg_rel0 & 0x0F;
        /*
	if (sc->dpm_gen > 1) {
		soc_cm_print("zm73xx: DPM Generation (0x%X) not recognized!\n", sc->dpm_gen);
		goto err_exit;
	}
        */
	...
	case 14:
		...
		break;
	default:
		soc_cm_print("zm73xx: DPM Rel1 Reg decode error, 0x%x, number of POLs unknown\n",
									dpm_reg_rel1);
		goto err_exit;
	}

	soc_cm_print("Found ZM7%d%02d fw %c.%d\n",
		       sc->dpm_gen == 0 ? 1:3, sc->numpols, 'A' + sc->fw_ver, sc->fw_idx);
...
void soc_bsc_readdpm(int unit)
{
	...
	soc_cm_print(" -- ZM73XX_DPM_REG_REL0: 0x%02x\n", data);

	retv = sbFe2000UtilPciIICRead(unit, SB_FE2000_IIC_DATA_OUT1_REG, &data);
	if (retv != SB_FE2000_STS_INIT_OK_K) {
			goto err_exit;
	}
	soc_cm_print(" -- return byte: 0x%02x\n", data);
...
static int zm73xx_direct_pol_read(int unit, int devno, int pol, int reg, int *data)
{
	...
	retv = zm73xx_msg_rw(unit, devno, txbuf, txbuf_len, rxbuf, rxbuf_len);
	if (retv != SB_FE2000_STS_INIT_OK_K) {
		soc_cm_print("zm73xx: unable to read pol %d reg %x\n", pol, reg);
		return SOC_E_FAIL;
	}
	/* 1: good transaction and zm73 acked */
	if (rxbuf[1] != 1) {
...
static int zm73xx_direct_pol_write(int unit, int devno, int pol, int reg, int data)
{
	...
	soc_cm_print(" -- txbuf[0]: %x\n", txbuf[0]);
	soc_cm_print(" -- txbuf[1]: %x\n", txbuf[1]);
	soc_cm_print(" -- txbuf[2]: %x\n", txbuf[2]);
	soc_cm_print(" -- txbuf[3]: %x\n", txbuf[3]);
	soc_cm_print(" -- txbuf[4]: %x\n", txbuf[4]);
	soc_cm_print(" -- txbuf[5]: %x\n", txbuf[5]);
	soc_cm_print(" -- txbuf[6]: %x\n", txbuf[6]);
#endif
	retv = zm73xx_msg_rw(unit, devno, txbuf, txbuf_len, rxbuf, rxbuf_len);
	if (retv != SB_FE2000_STS_INIT_OK_K) {
		soc_cm_print("zm73xx: unable to write pol %d reg %x\n", pol, reg);
		return SOC_E_FAIL;
	}
	/* 1: good transaction and zm73 acked */
	if (rxbuf[0] != 1) {
...
int soc_bsc_zm73xx_set(int unit, int pol, float volt)
{
	...
	if (devno >= num_bsc_devices) {
		soc_cm_print("No DPM device found\n");
		return SOC_E_FAIL;
	}
	...
	if ((pol < 0) || (pol >= MAX_POLS) || (!sc->pids[pol])) {
		soc_cm_print("No pol for this DPM device found\n");
		return SOC_E_FAIL;
	}

	retv = zm73xx_direct_pol_read(unit, devno, pol, ZM73XX_POL_REG_VOS, &vos_cur);
	if (retv != SOC_E_NONE) {
		soc_cm_print("Error: zm73xx_direct_pol_read ZM73XX_POL_REG_VOS\n");
		return SOC_E_FAIL;
	}
	...
	if (volt_delta > volt_cur * 0.1) {
		soc_cm_print("Error: new setting has to be within 10%% of current : %6.4fv\n",
									volt_cur);
		return SOC_E_FAIL;
	}
	...
	if ((volt < vos_to_volt[0]) || (volt > vos_to_volt[num_volt - 1])) {
		soc_cm_print("Error: setting volt out of range: %6.4f .. %6.4f\n",
	...
	if (retv != SOC_E_NONE) {
		soc_cm_print("Error: writing 0x%02x ZM73XX_POL_REG_VOS\n", vos);
		return SOC_E_FAIL;
	}
	soc_cm_print("Set POL %d: %6.4fv\n", pol, vos_to_volt[vos & 0xff]);

#if 000 /* useful debug routines */
	soc_cm_print(" -- set VOS: 0x%02x vos_cur: 0x%02x\n", vos, vos_cur);
...
int soc_bsc_zm73xx_show(int unit)
{
	...
	for (i = 0; i < sc->numpols; i++) {
		...
		watts += watts_x;
		soc_cm_print(" POL %d (%s): %6.4fv(%6.4fv) %6.3f amps %5.2fC  %7.4f watts\n",
			i, pc->name,
	...
	soc_cm_print(" Total power consumption: %7.4f watts\n", watts);
...
void soc_bsc_zm73xx_watch(int unit, int enable, int nsecs)
{
        ...
        sal_thread_create("",   SAL_THREAD_STKSZ,
                                ...
                                INT_TO_PTR(unit));
        soc_cm_print("unit %d: dpm monitoring enabled\n", unit);
}
