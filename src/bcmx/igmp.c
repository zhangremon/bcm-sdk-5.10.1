...
#define BCMX_IGMP_INIT_CHECK    BCMX_READY_CHECK

#define BCMX_IGMP_ERROR_CHECK(_unit, _check, _rv)    \
    BCMX_ERROR_CHECK(_unit, _check, _rv)

#define BCMX_IGMP_SET_ERROR_CHECK(_unit, _check, _rv)    \
    BCMX_SET_ERROR_CHECK(_unit, _check, _rv)

#define BCMX_IGMP_GET_IS_VALID(_unit, _rv)    \
...
bcmx_igmp_snooping_init(void)
{
    ...
    BCMX_IGMP_INIT_CHECK;

    BCMX_UNIT_ITER(bcm_unit, i) {
        tmp_rv = bcm_igmp_snooping_init(bcm_unit);
        BCM_IF_ERROR_RETURN(BCMX_IGMP_ERROR_CHECK(bcm_unit, tmp_rv, &rv));
    }
...
bcmx_igmp_snooping_enable_set(int enable)
{
    ...
    BCMX_UNIT_ITER(bcm_unit, i) {
        ...
        BCM_IF_ERROR_RETURN(BCMX_IGMP_SET_ERROR_CHECK(bcm_unit, tmp_rv, &rv));
    }
...
bcmx_igmp_snooping_enable_get(int *enable)
{
    ...
    BCMX_UNIT_ITER(bcm_unit, i) {
        ...
        if (BCMX_IGMP_GET_IS_VALID(bcm_unit, rv)) {
            return rv;
        }
    }
