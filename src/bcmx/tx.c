/*
 * $Id: tx.c 1.34.122.3 Broadcom SDK $
...
bcmx_tx(bcm_pkt_t *pkt, uint32 flags)
{
    ...
    default:
        /* Error */
        BCMX_DEBUG(BCMX_DBG_WARN, ("bcmx_tx: packet resolve error (%d): %s\n",
                     rv, bcm_errmsg(rv)));
        break;
    }
               ...
               bcmx_lplist_t *untagged_ports, uint32 flags)
{
    ...
    if (tx_ports == NULL || BCMX_LPLIST_IS_EMPTY(tx_ports)) {
        if (untagged_ports == NULL || BCMX_LPLIST_IS_EMPTY(untagged_ports)) {
            BCMX_DEBUG(BCMX_DBG_WARN | BCMX_DBG_VERBOSE,
                       ("bcmx_tx_lplist: No TX ports given\n"));
            return BCM_E_NONE;
        }
        all_untagged = TRUE;
...
bcmx_tx_port_list(bcmx_lplist_t *lplist, bcm_pkt_t *pkt)
{
    ...
    if (pkt->call_back != NULL) {
        BCMX_DEBUG(BCMX_DBG_WARN,
                   ("WARNING: bcmx_tx_port_list: async not supported\n"));
    }
