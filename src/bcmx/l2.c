                     ...
                     bcmx_lplist_t *port_block)
{
    ...
    from_bcmx.mcast = source->l2mc_index;
                       ...
                       bcm_l2_addr_t *source)
{
    ...
    from_bcm.mcast       = source->l2mc_index;
    ...
    dest->l2mc_index = to_bcmx.mcast;

    /* Set remaining fields */
    dest->trunk_sel = source->rtag;
    dest->cos_dst = source->cos_dst;
    dest->cos_src = source->cos_src;
    dest->auth = source->auth;
    dest->group = source->group;
 ...
 *      bcmx_l2_bpdu_count
 * Notes:
 *      Returns the minimum > 0 found; 0 if none found.
 */

int
bcmx_l2_bpdu_count(void)
{
    int rv = BCM_E_NONE;
    int i, bcm_unit;
    int count = 0;
...
int
bcmx_l2_bpdu_set(int index, bcm_mac_t addr)
{
    int rv = BCM_E_UNAVAIL, tmp_rv;
    int i, bcm_unit;
    ...
    BCMX_UNIT_ITER(bcm_unit, i) {
        tmp_rv = bcm_l2_bpdu_set(bcm_unit, index, addr);
        BCM_IF_ERROR_RETURN(BCMX_L2_SET_ERROR_CHECK(bcm_unit, tmp_rv, &rv));
    }
...
int
bcmx_l2_bpdu_get(int index, bcm_mac_t *addr)
{
    int rv;
    int i, bcm_unit;
    ...
    BCMX_UNIT_ITER(bcm_unit, i) {
        rv = bcm_l2_bpdu_get(bcm_unit, index, addr);
        if (BCMX_L2_GET_IS_VALID(bcm_unit, rv)) {
            return rv;
        }
    }
