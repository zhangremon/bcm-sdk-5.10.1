 /*
 * $Id: field.c 1.107.42.2 Broadcom SDK $
                        ...
                        uint32 flags, uint32 offset)
{
    ...
    BCMX_PARAM_NULL_CHECK(udf_spec);
                             ...
                             bcm_port_ethertype_t *ethertype)
{
    ...
    BCMX_PARAM_NULL_CHECK(frametype);
    BCMX_PARAM_NULL_CHECK(ethertype);
                        ...
                        int *udf_count)
{
    ...
    BCMX_PARAM_NULL_CHECK(qset);
    BCMX_PARAM_NULL_CHECK(udf_count);
    BCMX_PARAM_ARRAY_NULL_CHECK(udf_max, udf_arr);

    BCMX_UNIT_ITER(bcm_unit, i) {
        rv = bcm_field_qset_udf_get(bcm_unit, qset, udf_max, udf_arr, udf_count);
        if (BCMX_FIELD_GET_IS_VALID(bcm_unit, rv)) {
            return rv;
        }
    }
