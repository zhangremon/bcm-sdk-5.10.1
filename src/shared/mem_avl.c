...
/*
 * $Id: mem_avl.c 1.10 Broadcom SDK $
...
static void shr_mem_avl_list_tree(shr_avl_entry_t* entry, int m)
{
    ...
    soc_cm_print("0x%x (size:0x%x) (0x%x)\n",mem_entry->addr, 
...
int shr_mem_avl_list_output( shr_mem_avl_t *mem_avl )
{
    ...
    entry = mem_avl->mem_list;
    soc_cm_print("Listing Memory Nodes:\n");
    index = 0;
    while (NULL != entry) {
        soc_cm_print("%i:  Units: 0x%x  Addr: 0x%x Used: %i ",
                  index++,
                  entry->size,
                  entry->addr,
                  entry->used);
        soc_cm_print("This: %p Prev: %p Self: %p Next %p\n",
                  (void *)entry,
                  (void *)entry->prev,
                  (void *)entry->self,
                  (void *)entry->next);
...
int shr_mem_avl_free_tree_list( shr_mem_avl_t *mem_avl )
{
    ...
    soc_cm_print("Listing Free Tree:\n");
