/*
 * $Id: bm9600_diags.c 1.7 Broadcom SDK $
...
extern int _mc_fpga_write8(int addr, uint8_t data);
static uint8_t gStopPrbsTest = 0;
...
uint32_t sbx_qe_pl_port_map_rev1[2][2] = {
  { 25, 24 }, /* QE(Unit0),SCI0,SCI1 */
  { 26, 27 }, /* QE(Unit3),SCI0,SCI1 */
};

/* mapping SCI links to Polaris Ports */
uint32_t sbx_qe_pl_port_map_rev0[2][2] = {
  { 23, 22 }, /* QE(Unit0),SCI0,SCI1 */
  { 56, 57 }, /* QE(Unit3),SCI0,SCI1 */
};

/* SCI0,SCI1 */
uint32_t sbx_fe2k_pl_port_map_rev[1][2] = {
  { 25, 24 }, /* QE(Unit0),SCI0,SCI1 */
};
...
int sbBme9600DiagsPrbsTest(sbxDiagsInfo_t *pDiagsInfo)
{
  ...
  if (pDiagsInfo->qe_prbs_link_mask == 0x0 ||
    ...
    debugk(DK_WARN,"Specify a non-zero mask\n");
    return 0;
  }
  ...
  case BOARD_TYPE_FE2KXT_QE2K_POLARIS_LC:
	...
	return (-1);
      ...
      if (( rv = bcm_port_control_set(unit,i,bcmPortControlRxEnable,1)) < 0) {
	printk("ERROR could not enable %s hypercore port control rx %d\n",SOC_CHIP_STRING(unit),i);
	return (-1);
      }
      if (( rv = bcm_port_control_set(unit,i,bcmPortControlTxEnable,1)) < 0) {
	printk("ERROR could not enable %s hypercore port control tx %d\n",SOC_CHIP_STRING(unit),i);
	return (-1);
      }
      if (( rv = bcm_stk_module_protocol_set(unit,BCM_MODULE_FABRIC_BASE+i,bcmModuleProtocol1)) < 0) {
	printk("ERROR could not set %s protocol for link:%d (%s)\n",SOC_CHIP_STRING(unit),i,bcm_errmsg(rv));
    ...
    break;
  default:
    debugk(DK_WARN,"Unsupported board type(%d). ",g_brdtype);
    return (-1);
  }

  soc_cm_print("Starting test.. \n");

  /* Locate all the QE's for this card  todo -- update to search on more than one card */
  rv = sbBme9600DiagsFindAllQExx(unit, g_qexx_lcm_ids);
  if (rv != 0) {
    debugk(DK_ERR,"No QExx type devices found.\n");
    return rv;
  }
  ...
  for (i = 0; i < MAX_QE_NODES; i++ ) {
    if (g_qexx_lcm_ids[i] != -1 && (g_qexx_mask & (1 << g_qexx_lcm_ids[i]))) {
      ...
      bTestRun = TRUE;
      printk("Testing using LSFR Polynomial %s\n",lsr_polynomial);
      rv = sbBme9600DiagsSFIPrbsTest(pDiagsInfo,
				     pcmd,
				     g_qexx_lcm_ids[i]);
      if (rv < 0) {
	debugk(DK_ERR,"PRBS test found errors testing with QE[u:%d] using %s polynomial\n",g_qexx_lcm_ids[i],lsr_polynomial);
	status = -1;
      }
    }
  }
  ...
  if (!bTestRun) {
    printk("No QE's found in mask:0x%x\n",g_qexx_mask);
    status = -1;
    goto done;
  }
  ...
  if (g_brdtype == BOARD_TYPE_FE2KXT_QE2K_POLARIS_LC) {
    ...
    if ((fpga_data1 & 0xf) != 0xa) {
      debugk(DK_WARN,"Setting SCI links to go to polaris for test\n");
    ...
    if ((fpga_data1 & 0xf0) != 0xa0) {
      debugk(DK_WARN,"Setting QE0 SCI links to go to polaris for test\n");
      bRestoreSCIMapping = TRUE;
      _mc_fpga_write8(16,0xa0);
    }
    _mc_fpga_read8(17,&fpga_data2);
    if ((fpga_data2 & 0xf) != 0xa) {
      debugk(DK_WARN,"Setting QE1 SCI links to go to polaris for test\n");
      bRestoreSCIMapping = TRUE;
  ...
  if (rv < 0) {
    debugk(DK_ERR,"PRBS test found errors testing SCI links\n");
    status = -1;
  }

  /* restore orginal SCI mapping */
  if (bRestoreSCIMapping) {
    printk("Restoring SCI Mapping to backplane.\n");
    if (g_brdtype == BOARD_TYPE_FE2KXT_QE2K_POLARIS_LC) {
			      ...
			      int qexx_unit)
{
  ...
  int tx_port = 0;
  uint32_t qe = 0;
  uint32_t qexx_sfi_link_mask=0;
  int failedCount = 0;

#ifndef __KERNEL__
  signal(SIGINT,(void *)polaris_sigcatcher);
#else
  ...
  if (BCM_FAILURE(rv)) {
    debugk(DK_ERR,"bcm_stk_modid_get failed(%s) for UNIT:%d\n",bcm_errmsg(rv),qexx_unit);
    return (-1);
  }
  ...
  for (dir = start; dir <= end; dir++) {
    for(i = 0; i < SB_FAB_DEVICE_QE2000_SFI_LINKS ; i++ ) {
      ...
      if (BCM_FAILURE(rv)) {
	debugk(DK_ERR,"bcm_fabric_crossbar_mapping_get (%d)(%s)\n",rv,bcm_errmsg(rv));
	return (-1);
      }
      ...
      if (nXbPort == -1 ) {
	debugk(DK_ERR,"Could not find xbPort mapping for QE[mod:%d,u:%d,link:%d]\n",
    ...
    LoopOnPrbsError:
      printk("Testing PRBS %s[MOD:%d,U:%d,SFI:%02d] %s %s[U:%d,port:%02d HC%d_lane%d] .. ",SOC_CHIP_STRING(qexx_unit),
    ...
    start:
      ...
      if (rv != 0) {
	debugk(DK_ERR,"(%s) Errors starting prbs test.\n",FUNCTION_NAME());
	return rv;
      }
	...
	if (!p->bForcePRBSError) {
	  ...
	  printk("PASSED, but should have failed with force_error=1\n");
	}
      } else {
	if (p->bForcePRBSError) {
	  printk("FAILED (expected), status = 0x%x\n",prbs_stat);
	} else {
	  failedCount++;
	  if (failedCount == 1) {
	    prbs_stat = 0;
	  ...
	  if (!p->bLoopOnError) { /* do not show if looping on this error */
	    printk("To re-test this link TR 110 qe_mask=0x%x qe_link_mask=0x%x dir=%d Lsfr=%d LoopOnError=[0|1]\n",
...

{
  ...
  if (BCM_FAILURE(rv)) {
    debugk(DK_ERR,"Failed to stop %s from sending prbs\n bcm_port_control_set (%d)(%s)\n",
  ...
  if (BCM_FAILURE(rv)) {
    debugk(DK_ERR,"Failed to stop %s from recieving prbs\n bcm_port_control_set (%d)(%s)\n",
...

{
  ...
  if (BCM_FAILURE(rv)) {
    debugk(DK_ERR,"Failed to set %s to force PRBS Error\n bcm_port_control_set (%d)(%s)\n",
	   SOC_CHIP_STRING(tx_unit),rv,bcm_errmsg(rv));
    return (-1);
  }  
  ...
  if (BCM_FAILURE(rv)) {
    debugk(DK_ERR,"Failed to get %s prbs status\n bcm_port_control_get (%d)(%s)\n",
	   SOC_CHIP_STRING(rx_unit),rv,bcm_errmsg(rv));
    *prbs_status = -1;
    return (-1);
  }
...
int sbBme9600DiagsSCIPrbsTest(sbxDiagsInfo_t *pDiagsInfo) 
{
  ...
  for (i = 0; i < MAX_QE_NODES; i++ ) {
	...
	for (sci = 0; sci < SB_FAB_DEVICE_QE2000_SCI_LINKS; sci++ ) {
	  ...
	  if (nXbPort == -1 ) {
	    printk("No SCI mapping found for this board\n");
	    return (-1);
	  }
	...
	LoopOnPrbsError:
	  printk("Testing PRBS %s[U:%d,SCI:%d] %s %s[U:%d,port:%02d HC%d_lane%d] .. ",
...
int sbBme9600DiagsRegTest(sbxDiagsInfo_t *pDiagsInfo)
{
  ...
  if (!pl_test_regs_l) {
    debugk(DK_ERR,"%s ERROR:  Out of Memory \n",FUNCTION_NAME());
    return CMD_FAIL;
  }
  ...
  for(i = 0; i < pl_test_regs_l->count; i++) {
    ...
    if (strstr(reg_info->name,"pi_revision")) {
      ...
      if (rv != CMD_OK) {
	debugk(DK_ERR,"sbx_diag_read_reg failed(%d)\n",rv);
	sal_free(pl_test_regs_l);
	return (-1);
      }
	...
	return (-1);
      ...
      if (rv != CMD_OK) {
	debugk(DK_ERR,"sbx_diag_write_reg failed(%d)\n",rv);
	sal_free(pl_test_regs_l);
	return (-1);
      }
      ...
      sbx_diag_read_reg(unit,reg_info,&tmp);
      if (tmp != ~regval) {
	debugk(DK_ERR,"Write failed exp:0x%x got:0x%x\n",~regval,tmp);
      } else {
	printk("Simple Read/Modify/Write test passed.\n");
	stat = 0;
      }
