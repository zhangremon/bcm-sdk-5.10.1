/*
 * $Id: rpacket.c 1.58.44.3 Broadcom SDK $
...
#include "testlist.h"
    ...
    defined(BCM_FE2000_SUPPORT) || defined(BCM_ROBO_SUPPORT)
...
static int imp_rate_bps = IMP_MIN_RATE_BPS;
...
rpacket_setup(int unit, p_t *p)
{
    ...
    if (soc_feature(unit, soc_feature_filter)) {
        pbmp_t  pbm;
        /* Install filter to redirect packet back out that port. */
        rv = bcm_filter_create(unit, &p->p_filter);
        if (BCM_SUCCESS(rv)) {
            BCM_PBMP_CLEAR(pbm);
            BCM_PBMP_PORT_ADD(pbm, p->p_port[0]);
            rv = bcm_filter_qualify_ingress(unit, p->p_filter, pbm);
        }
        if (BCM_SUCCESS(rv)) {
            rv = bcm_filter_action_match(unit, p->p_filter,
                                         bcmActionSetPortNonUcast, p->p_port[0]);
        }
        if (BCM_SUCCESS(rv)) {
            rv = bcm_filter_action_match(unit, p->p_filter,
                                         bcmActionCopyToCpu, p->p_port[0]);
        }
        if (BCM_SUCCESS(rv)) {
    ...
    if (soc_feature(unit, soc_feature_field)) {
        if (SOC_IS_ESW(unit)) {
            ...
            if (BCM_SUCCESS(rv)) {
                for (port_idx=0; port_idx <  num_test_ports; port_idx++) {
                    rv = bcm_field_counter_create(unit, fent[port_idx]);
                }
            }
            ...
            for (port_idx=0; port_idx < num_test_ports; port_idx++) {
                if (BCM_SUCCESS(rv)) {
                    rv = bcm_field_action_add(unit, fent[port_idx], 
                                              ...
                                              BCM_FIELD_COUNTER_MODE_DEFAULT, 0);
...
rpacket_test(int unit, args_t *a, void *pa)
{
    ...
    for (l = p->p_l_start; l <= p->p_l_end; l += p->p_l_inc) {
        ...
        if (soc_feature(unit, soc_feature_filter)) {
            /* Install filter */
            rv = bcm_filter_install(unit, p->p_filter);
        }
#endif /* BCM_FILTER_SUPPORT */
#if defined(BCM_FIELD_SUPPORT)
        if (soc_feature(unit, soc_feature_field)) {
...
#if defined(BCM_FILTER_SUPPORT)
        if (soc_feature(unit, soc_feature_filter)) {
            rv = bcm_filter_remove(unit, p->p_filter);
        }
#endif /* BCM_FILTER_SUPPORT */
#if defined(BCM_FIELD_SUPPORT)
        if (soc_feature(unit, soc_feature_field)) {
   ...
   if (SOC_IS_ROBO(unit)) {
...
#if defined(BCM_FILTER_SUPPORT)
    if (soc_feature(unit, soc_feature_filter)) {
        (void)bcm_filter_remove(unit, p->p_filter);
    }
#endif /* BCM_FILTER_SUPPORT */
#if defined(BCM_FIELD_SUPPORT)
    if (soc_feature(unit, soc_feature_field)) {
    ...
    if (SOC_IS_ROBO(unit)) {
        ...
        DRV_RATE_SET
            ...
            0, imp_rate_default, imp_burst_size_default);
    }
#endif /* BCM_ROBO_SUPPORT */
...
rpacket_init(int u, args_t *a, void **pa)
{
    ...
    *pa = (void *)p;
    ...
    /* Record IMP port previous egress rate : restore back while testing done */
        ...
        if (SOC_IS_ROBO53115(u) || SOC_IS_ROBO53125(u)) {
            SOC_IF_ERROR_RETURN(DRV_RATE_SET
            (u, t_pbm, no_que, DRV_RATE_CONTROL_DIRECTION_EGRESSS, 
            0, imp_rate_pps, IMP_BURST_SIZE));

        } else if (SOC_IS_ROBO5395(u)) {
            SOC_IF_ERROR_RETURN(DRV_RATE_SET
            (u, t_pbm, no_que, DRV_RATE_CONTROL_DIRECTION_EGRESSS, 
            0, imp_rate_bps, IMP_BURST_SIZE));
        }
    }
#endif /* BCM_ROBO_SUPPORT */
    ...
    return 0;
             ...
             * BCM_FE2000_SUPPORT || BCM_ROBO_SUPPORT */
