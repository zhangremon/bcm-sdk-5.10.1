/*
 * $Id: ctrtest.c 1.30.20.2 Broadcom SDK $
...
test_width32(int unit, soc_reg_t ctr_reg, soc_port_t port)
{
    ...
    if (SOC_IS_ROBO(unit)) {
        ...
        addr = soc_reg_addr(unit, ctr_reg, port, 0);
		...
		continue;
            ...
            for (i = 0; i < SOC_CTR_MAP_SIZE(unit, ctype); i++) {
                ...
                if (!SOC_COUNTER_INVALID(unit, reg) && 
                    ...
                    uint64		mask;
                    soc_regaddrinfo_get(unit, &ainfo,
		    ...
		    width = SOC_REG_INFO(unit, reg).fields[0].len;
                    ...
                    if (soc_feature(unit, soc_feature_counter_parity)) {
                        int j = 0;
                        while((SOC_REG_INFO(unit, reg).fields + j) != NULL) { 
                            if (SOC_REG_INFO(unit, reg).fields[j].field 
                                == COUNTf) {
                                width = SOC_REG_INFO(unit, reg).fields[j].len;
                                break;
                            }
                            j++;
                        }
