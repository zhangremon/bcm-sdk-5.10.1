/*
 * $Id: lb_util.c 1.171.14.6 Broadcom SDK $
...
{
#ifdef BCM_ESW_SUPPORT
    uint32          modport_addr, modport_reg, modport_mask;
    uint64          modport_reg64, modport_mask64;
    ...
    if (IS_ST_PORT(unit, src_port)) {
        modport_addr = 
            soc_reg_addr(unit, IMODPORT_7_0r, src_port, 0) + (modid / 8);
    } else {
        modport_addr = 
            soc_reg_addr(unit, MODPORT_7_0r, src_port, 0) + (modid / 8);
    }

    if (SOC_IS_TUCANA(unit)) { /* Gack! */
        SOC_IF_ERROR_RETURN(soc_reg64_read(unit, modport_addr, 
                                           &modport_reg64));

        COMPILER_64_SET(modport_mask64, 0, 0x3f);
        COMPILER_64_SHL(modport_mask64, 6 * (modid % 8));
        if ((modid % 8) > 4) { /* Align with high word */
            COMPILER_64_SHL(modport_mask64, 2);
        }
        COMPILER_64_NOT(modport_mask64);
        COMPILER_64_AND(modport_reg64, modport_mask64);

        COMPILER_64_SET(modport_mask64, 0, dst_port);
        COMPILER_64_SHL(modport_mask64, 6 * (modid % 8));
        if ((modid % 8) > 4) { /* Align with high word */
            COMPILER_64_SHL(modport_mask64, 2);
        }
        COMPILER_64_OR(modport_reg64, modport_mask64);

        SOC_IF_ERROR_RETURN(soc_reg64_write(unit, modport_addr, 
                                            modport_reg64));
    } else {
        SOC_IF_ERROR_RETURN(soc_reg32_read(unit, modport_addr, &modport_reg));
        modport_mask = 0xf << (4 * (modid % 8));
        modport_reg &= ~modport_mask;
        modport_reg |= dst_port << (4 * (modid % 8));
        SOC_IF_ERROR_RETURN(soc_reg32_write(unit, modport_addr, modport_reg));
    }
#endif /* BCM_ESW_SUPPORT */
    return 0;
}
...
STATIC int
lbu_filter_ether_port(int unit, lb2s_port_connect_t *pc_info, 
                      int connect)
{
    bcm_filterid_t      f;
    int                 rv;
    ...
    SOC_PBMP_CLEAR(pbm);
    SOC_PBMP_PORT_ADD(pbm, pc_info->this_port);
    BCM_IF_ERROR_RETURN(bcm_filter_qualify_ingress(unit, f, pbm));
    BCM_IF_ERROR_RETURN(BCM_FILTER_QUALIFY_MAC_DST(unit, f, pc_info->dst_mac));
    if (SOC_IS_XGS_SWITCH(unit)) {
        BCM_IF_ERROR_RETURN(bcm_filter_action_match(unit, f, bcmActionSetModule, 
                                                    pc_info->dst_modid));
    }

    if (MAC_IS_MCAST(pc_info->dst_mac)) {
        BCM_IF_ERROR_RETURN(bcm_filter_action_match(unit, f, 
    ...
    if (connect) {
        if ((rv = bcm_filter_install(unit, f)) < 0) {
            /* Destroy because test_error may not return */
            bcm_filter_destroy(unit, f);
            test_error(unit,
                       "Could not add new filter entry: %s\n",
                       bcm_errmsg(rv));
            return rv;
        }
    } else {
        if ((rv = bcm_filter_remove(unit, f)) < 0) {
            /* Destroy because test_error may not return */
            bcm_filter_destroy(unit, f);
            test_error(unit,
                       "Could not delete filter entry: %s\n",
                       bcm_errmsg(rv));
            return rv;
        }
    }
                  ...
                  int connect)
{
        ...
        if (IS_E_PORT(unit, pc_info->this_port)) {
            if (IS_E_PORT(unit, pc_info->to_port)) {
                ...
                if ((!SOC_IS_XGS3_SWITCH(unit)) &&
                    ...
                    rv = lbu_filter_ether_port(unit, pc_info, connect);
                } else
#endif
                {
                    rv = lbu_connect_ether_ports(unit, pc_info, connect);
    ...
    debugk(DK_TESTS, "lbu_connect_ports: rv = %d\n", rv);
    return rv;
}
...
#ifdef BCM_ESW_SUPPORT
    if (SOC_IS_XGS_SWITCH(unit)) {
        int     not_my_mod, modid = 0, modid_hi = 0, max_modules;
        /* Save current source port/module and change to != lp->d_mod */

        max_modules = SOC_MODID_MAX(unit)+1;
        ...
        if (SOC_IS_TUCANA(unit)) {
            ...
            not_my_mod = (lp->d_mod + 1) % max_modules;
            if (not_my_mod == modid) {
                not_my_mod = (not_my_mod + 1) % max_modules;
                if (not_my_mod == modid_hi) {
                    not_my_mod = (not_my_mod + 1) % max_modules;
                }
           } else if (not_my_mod == modid_hi) {
               not_my_mod = (not_my_mod + 1) % max_modules;
                if (not_my_mod == modid) {
                    not_my_mod = (not_my_mod + 1) % max_modules;
                }
...
lbu_setup_port(int unit, bcm_port_t port, int req_speed, int autoneg)
{
    ...
    switch (req_speed) {
        ...
        case LB_SPEED_16G_FD:
            ...
            req_abil = BCM_PORT_ABIL_16GB_FD;
            duplex = SOC_PORT_DUPLEX_FULL;
            break;
        ...
        case LB_SPEED_MAX:
            ...
            BCM_IF_ERROR_RETURN(
                bcm_port_advert_get(unit, port, &req_abil));
            BCM_IF_ERROR_RETURN(
    ...
    if (autoneg) {
        BCM_IF_ERROR_RETURN(
            bcm_port_advert_set(unit, port, req_abil));
        BCM_IF_ERROR_RETURN(
...
lbu_port_init(loopback2_test_t *lw, loopback2_testdata_t *lp)
{
    ...
    debugk(DK_TESTS, "Saving ports\n");
    if (lbu_save_port(lw, lp) < 0) {
        return (-1);
    }
    
    /* Port setup */
    debugk(DK_TESTS, "Port setup, bitmap %s\n",
    ...
    if (lp->loopback == LB2_MODE_MAC) {
        debugk(DK_TESTS, "MAC loopback at speed %s\n", 
        ...
        debugk(DK_TESTS, "PHY loopback at speed %s\n", 
               lb2_parse_speed[lp->speed]);
...
lbu_init(loopback2_test_t *lw, loopback2_testdata_t *lp)
{
    ...
    if (!lp->inject) {
        debugk(DK_TESTS, "Port monitor init\n");
 ...
 */
{
...
#ifdef BCM_XGS3_SWITCH_SUPPORT
        uint32  hc;
        debugk(DK_TESTS, "Hash select reset to CRC32 low\n");
    ...
    if (SOC_IS_TUCANA(unit)) {
        if ((ipic_my_modid == (lp->d_mod == 0)) || (ipic_my_modid == modid)
            || (ipic_my_modid == modid_hi)) {
            ipic_my_modid += lp->d_mod_inc;
        }

        ipic_my_modid_hi = ipic_my_modid + lp->d_mod_inc;
        if ((ipic_my_modid_hi == (lp->d_mod == 0)) || 
            (ipic_my_modid_hi == modid) || (ipic_my_modid_hi == modid_hi)) {
    ...
    PBMP_ITER(lp->pbm, port) {
        ...
        if (IS_ST_PORT(unit, port) && SOC_IS_XGS_SWITCH(unit)) {
            if (SOC_IS_TUCANA(unit)) {
                ...
                soc_reg64_field32_set(unit, ICONFIGr, &iconfig, MY_MODID0f, 
                                      ipic_my_modid);
                soc_reg64_field32_set(unit, ICONFIGr, &iconfig, MY_MODID1f, 
                                      ipic_my_modid_hi);
                ...
                soc_reg_field_set(unit, IGR_PORTr, &igr_port, MY_MODIDf, 
                                      ipic_my_modid);
                SOC_IF_ERROR_RETURN(WRITE_IGR_PORTr(unit, port, igr_port));
#endif /* BCM_EASYRIDER_SUPPORT */
                ...
                SOC_IF_ERROR_RETURN(READ_ICONFIGr(unit, port, &iconfig));
                soc_reg64_field32_set(unit, ICONFIGr, &iconfig, MY_MODIDf, 
                                      ipic_my_modid);
                SOC_IF_ERROR_RETURN(WRITE_ICONFIGr(unit, port, iconfig));
            }
        }
...
{
    int max_cells, num_cells, num_ports, num_cos, mmu_cell_size, port, cos;
    int len = *lene;
    uint32 num_cells_per_port_cos;
    ...
    *ppte = ppte_max;
    ...
    if (SOC_IS_XGS_SWITCH(unit) || SOC_IS_XGS12_FABRIC(unit)) {
        ...
        mmu_cell_size = SOC_IS_TUCANA(unit) ? 288 : 128;
        ...
        if (SOC_IS_TUCANA(unit)) {
            ...
            SOC_IF_ERROR_RETURN(soc_tucana_num_cells(unit, &num_cells));
#endif
        } else 
#if defined(BCM_EASYRIDER_SUPPORT)
        if (SOC_IS_EASYRIDER(unit)) {
            SOC_IF_ERROR_RETURN(soc_er_num_cells(unit, &num_cells));
        } else
#endif /* BCM_EASYRIDER_SUPPORT */ 
        if (SOC_IS_FBX(unit) && !SOC_IS_KATANA(unit)) {
        ...
        *ppte = (num_cells / (num_cos * num_ports * max_cells)) - 1;
        ...
        if (SOC_IS_HERCULES(unit)) {
            ...
            if (SOC_IS_HERCULES15(unit)) {
                ...
                SOC_IF_ERROR_RETURN(READ_MMU_CELLLMTCOSr(unit, port, cos, 
                                                     &num_cells_per_port_cos));
            }
            /* overwrite ppte */
        ...
        if (SOC_IS_LYNX(unit)) {
            /* look at what is configured for any port/cos 0,0 */
            SOC_IF_ERROR_RETURN(READ_HOLCOSCELLSETLIMITr(unit, port, cos, 
                                                     &num_cells_per_port_cos));
            /* overwrite ppte */
            *ppte = (num_cells_per_port_cos / max_cells) - 1; 
        }
        ...
        PBMP_ITER(pbm, port) {
            if (max_pkt > 1536) {
                if (soc_feature(unit, soc_feature_trimac)) {
                    ...
                    if (! soc_reg64_field32_get(unit, GMACC1r, gmacc1, JUMBOf)) {
                        soc_reg64_field32_set(unit, GMACC1r, &gmacc1, JUMBOf, 1);
                        SOC_IF_ERROR_RETURN
 ...
 */
{
    ...
    for (ix = 0; ix < lw->port_count; ix++) {
        ...
        debugk(DK_TESTS, "Port %s Tx Count = 0x%x\n", 
        ...
        if (((IS_FE_PORT(unit, thisPort) || IS_GE_PORT(unit, thisPort)) &&
            ...
            debugk(DK_TESTS, "Port %s Rx Count = 0x%x\n", 
                   SOC_PORT_NAME(unit, thisPort), tempStat);
                    ...
                    loopback2_test_t *lw)
{
    ...
    for (i = 0; i < ppt; i++) {
        ...
        if (SOC_IS_XGS3_SWITCH(lw->unit) && 
            ...
            if (SOC_IS_EASYRIDER(unit)) {
                /* Must use different source modid to get around CPU TX drop */
                pkt->src_mod = dmod + 1;
            }
            pkt->src_port =  CMIC_PORT(unit);
