/*
 * $Id: loopback.c 1.170.6.1 Broadcom SDK $
 ...
 */
{
    ...
    if (SOC_IS_DRACO(unit)) {
        /*
         *      No corresponding pair available for HG port on Draco
         *      for external loopback.
         */
        SOC_PBMP_REMOVE(lp->lp_pbm_tx, PBMP_HG_ALL(unit));
    }
    lp->lp_dst_inc      = 1;
    SOC_PBMP_CLEAR(lp->lp_ubm);
    lp->lp_pattern      = 0xa5a4a3a2;
...
lb_save_port(loopback_test_t *lw)
{
    ...
    if (SOC_IS_XGS_SWITCH(lw->lw_unit)) {
        ...
        max_modules = SOC_MODID_MAX(lw->lw_unit)+1;
        ...
        if (SOC_IS_TUCANA(lw->lw_unit)) {
            if ((rv = bcm_stk_my_modid_get(lw->lw_unit, &modid)) < 0) {
                test_error(lw->lw_unit,
            ...
            not_my_mod = (lw->lw_lp->lp_d_mod + 1) % max_modules;
            if (not_my_mod == modid) {
        ...
        if (SOC_IS_TUCANA(lw->lw_unit)) {
            ...
            if ((rv = READ_CMIC_HGTX_CTRLr(lw->lw_unit, &reg)) < 0) {
                test_error(lw->lw_unit,
                           "Could not read CMIC_HGTX_CTRL register %s\n",
                           bcm_errmsg(rv));
                return(-1);
            }
            oreg = reg;
            soc_reg_field_set(lw->lw_unit, CMIC_HGTX_CTRLr, &reg, SRC_MODIDf,
                              not_my_mod);
            soc_reg_field_set(lw->lw_unit, CMIC_HGTX_CTRLr, &reg,
                              SRC_PORT_TGIDf, CMIC_PORT(lw->lw_unit));
            if (reg != oreg) {
                if ((rv = WRITE_CMIC_HGTX_CTRLr(lw->lw_unit, reg)) < 0) {
                    test_error(lw->lw_unit,
                         "Could not write CMIC_HGTX_CTRL register %s\n",
                               bcm_errmsg(rv));
                    return(-1);
                }
            }
            ...
            if ((rv = READ_CMIC_HGTX_CTRL2r(lw->lw_unit, &reg)) < 0) {
                test_error(lw->lw_unit,
                           "Could not read CMIC_HGTX_CTRL2 register %s\n",
                           bcm_errmsg(rv));
                return(-1);
            }
            oreg = reg;
            soc_reg_field_set(lw->lw_unit, CMIC_HGTX_CTRL2r, &reg, SRC_MODf,
                              not_my_mod);
            soc_reg_field_set(lw->lw_unit, CMIC_HGTX_CTRL2r, &reg,
                              SRC_TGID_PORTf, CMIC_PORT(lw->lw_unit));
            if (reg != oreg) {
                if ((rv = WRITE_CMIC_HGTX_CTRL2r(lw->lw_unit, reg)) < 0) {
                    test_error(lw->lw_unit,
                         "Could not write CMIC_HGTX_CTRL2 register %s\n",
                               bcm_errmsg(rv));
                    return(-1);
                }
            }
 ...
 */
{
    ...
    while (rx_dcb < end_dcb) {
        ...
        /* Tucana is different than previous chips and may overwrite
         * the FCS coming out of the MMU, depending on mode settings.
         */
        if (SOC_IS_TUCANA(lw->lw_unit)) {
            rx_crc_valid = FALSE;
        } else {
        ...
        for (l_cur = lp->lp_l_start; l_cur <= lp->lp_l_end;
            ...
            for (c_cur = lp->lp_c_start; c_cur <= lp->lp_c_end;
                ...
                for (cos_cur = lp->lp_cos_start;
                     ...
                     cos_cur++) {
                    if ((0 > lb_do_rx(lw, l_cur, c_cur, cos_cur)) ||
                        (0 > lb_do_tx(lw, l_cur, c_cur, cos_cur,
                                      lp->lp_d_port,
                                      lp->lp_d_mod))) {
                        return(-1);
                    }
 ...
 */
{
    ...
    for (rx_chan = 0; rx_chan < N_DMA_CHAN; rx_chan++) {
        debugk(DK_TESTS, "Configuring RX-channel: %d\n", rx_chan);
        ...
        if(soc_feature(unit, soc_feature_cmicm)) {
            debugk(DK_TESTS, "Assign all COS to channel: %d\n", rx_chan);
            SOC_IF_ERROR_RETURN(
        ...
        for (tx_chan = 0; tx_chan < N_DMA_CHAN; tx_chan++) {
            ...
            debugk(DK_TESTS, "Configuring TX-channel: %d\n", tx_chan);
                     ...
                     soc_port_t src_port)
{
    ...
    debugk(DK_TESTS, "Setting up static path for port %s\n",
             ...
             soc_port_t src_port)
{
    ...
    if (SOC_IS_EASYRIDER(unit)) {
         ...
         * result.
         */
        if (IS_ST_PORT(unit, src_port) && (src_port != 0)) {
            return(0);
        }
    } else
#endif
    ...
    debugk(DK_TESTS, "Setting up ARL for port %s\n",
           SOC_PORT_NAME(unit, src_port));
                   ...
                   loopback_testdata_t *lp)
{
     ...
     */
    for (c = 0; c < N_DMA_CHAN; c++) {
        debugk(DK_TESTS, "lb_dma_init: Deconfiguring channel %d\n", c);
 ...
 */
{
    ...
    if (((rv = soc_dma_chan_config(unit, 0, DV_TX, 
                                   SOC_DMA_F_MBM | SOC_DMA_F_DEFAULT)) < 0) ||
     ...
     * it should be OK if we do this for CMICe as well */
    if(soc_feature(unit, soc_feature_cmicm)) {
        debugk(DK_TESTS, "Assign all COS to channel: 1\n" );
        SOC_IF_ERROR_RETURN(
 ...
 */
{
    ...
    debugk(DK_TESTS, "lb_mac_test: unit %d starting ....\n", lw->lw_unit);
 ...
 */
{
    ...
    while (!rv && (cur_packet < ppc)) {
        ...
        debugk(DK_TESTS, "lb_sg_check_rx : rxlen = %d txlen = %d\n",cur_len,length);
 ...
 */
{
    ...
    if (SOC_IS_XGS12_FABRIC(lw->lw_unit)) {
        /* Update length */
        debugk(DK_TESTS, "lb_sg_wait: length = %d\n", length);
 ...
 */
{
    ...
    if (SOC_IS_XGS12_FABRIC(lw->lw_unit)) {
        /* Update length */
        debugk(DK_TESTS, "lb_do_sg_rx: p_len = %d\n", p_len);
                ...
                for (cur_ppc = lp->lp_ppc_start;
                    ...
                    for (tx_dpp = lp->lp_dpp_start;
                        ...
                        for (rx_dpp = lp->lp_dpp_start;
                            ...
                            debugk(DK_TESTS, "Testing cnt=%d L=%d cos=%d "
 ...
 */
{
    ...
    if (!SOC_PBMP_MEMBER(lp->lp_pbm, CMIC_PORT(unit))) {
        debugk(DK_TESTS, "Warning: CPU port not set in bitmap.\n"
    ...
    *p = lw;                            /* Pass back cookie */
    ...
    if (SOC_IS_TUCANA(unit)) {
        if (READ_MISCCONFIGr(unit, &misc) >= 0) {
            soc_reg_field_set(unit, MISCCONFIGr, &misc, PRESERVECRCf, 1);
            if (WRITE_MISCCONFIGr(unit, misc) < 0) {
                rv = -1;
            }
        } else {
            rv = -1;
 ...
 */
{
    int         tx_dv_idx, rx_dv_idx, p_idx, dv_idx;
    ...
    if (SOC_IS_XGS12_FABRIC(lw->lw_unit)) {
        /* Update length */
        debugk(DK_TESTS, "lb_random_restore_dv: l = %d\n", l);
 ...
 */
{
    ...
    for (channel = 0; channel < N_DMA_CHAN; channel++) {
        debugk(DK_TESTS, "PBMP(0) testing Tx channel %d\n", channel);
        ...
        for (channel = 0; channel < N_DMA_CHAN; channel++) {
            ...
            for (d = lp->lp_dv_start; d <= lp->lp_dv_end; d += lp->lp_dv_inc) {
                ...
                while (!rv) {
                    ...
                    dma_desc = (sal_vaddr_t) soc_cm_p2l(unit, soc_pci_read(unit, CMIC_DMA_DESC(channel)));
