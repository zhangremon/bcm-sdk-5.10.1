/*
 * $Id: topo_pkt.c 1.32 Broadcom SDK $
...
bcm_stack_topo_update(cpudb_ref_t db_ref)
{
    ...
    if (TOPO_ACTIVE) {
        ST_TOPO_UNLOCK;
        TOPO_WARN(("TOPO: Activated twice\n"));
        return BCM_E_FAIL;
    }
    ...
    if (db_ref->local_entry == db_ref->master_entry) { /* Local is master */
        ...
        if ((rv = topology_mod_ids_assign(db_ref)) < 0) {
            TOPO_ERR(("Could not assign all mod ids: %s\n", bcm_errmsg(rv)));
        } else if ((rv = topology_create(db_ref)) < 0) {
            TOPO_ERR(("Failed to create topology: %s\n", bcm_errmsg(rv)));
        } else if ((rv = topo_pkt_send_depth_first(db_ref, 
                                            db_ref->master_entry)) < 0) {
            TOPO_ERR(("Failed to send topo pkt: %s\n", bcm_errmsg(rv)));
        }
...
topo_pkt_send(cpudb_ref_t db_ref, cpudb_entry_t *entry)
{
    ...
    rv = atp_tx(entry->base.key,
                ...
                NULL);
    if (rv < 0) {
        TOPO_WARN(("topo send %d:  %s. Failed to tx pkt to "
                   CPUDB_KEY_FMT_EOLN, rv, bcm_errmsg(rv),
...
topo_pkt_send_depth_first(cpudb_ref_t db_ref, cpudb_entry_t *entry)
{
    ...
    if (CPUDB_TOPO_VISITED(entry)) {
        TOPO_ERR(("TOPO ERROR: recursing infinitely"));
        return -1;
    }
...
_topo_pkt_process_thread(void *cookie)
{
    ...
    if (TOPO_ACTIVE && TOPO_PKT_PRESENT) {
        ...
        if (rv < 0) {
            TOPO_ERR(("TOPO PKT ERROR: parse failure: %s\n", bcm_errmsg(rv)));
        } else if ((rv = topo_board_program(topo_current_db, &topo_cpu)) < 0) {
            TOPO_ERR(("TOPO PKT ERROR: appl fail: %s\n", bcm_errmsg(rv)));
        }
        ...
        if (rv == BCM_E_NONE) {
            ...
            if ((topo_current_db->num_cpus > 1) && (delay > 0)) {
                TOPO_VERB(("TOPO: Sleeping for %d us\n", delay));
                sal_usleep(delay);
            }
                 ...
                 void *cookie)
{
    ...
    TOPO_VERB(("ST TOPO: pkt len %d in from "
               CPUDB_KEY_FMT_EOLN, payload_len, CPUDB_KEY_DISP(src_key)));
    ...
    uint8 *pkt_data, int len, int sp_idx, int tx)
{
    ...
    if ((cfm_slot = cpudb_sp_idx_to_slot(db_ref, entry, sp_idx, NULL)) < 0) {
        TOPO_VERB(("TOPO: LM %d no cxn on %d\n", slot_id, sp_idx));
        return 0;
    }

    TOPO_VERB(("Packing LM slot %d, sp idx %d to slot %d, %s; %d cfms\n",
    ...
    uint8 *pkt_data, int len, int sp_idx, int tx)
{
    ...
    if ((lm_slot = cpudb_sp_idx_to_slot(db_ref, entry, sp_idx, NULL)) < 0) {
        TOPO_VERB(("TOPO: CFM %d no cxn on %d\n", slot_id, sp_idx));
        return 0;
    }

    TOPO_VERB(("Packing CFM slot %d sp idx %d to slot %d, %s; %d cfms\n",
    ...
    int *ap_data_ofs)
{
    ...
    if (len < TOPO_HEADER_BYTES(db_ref->num_cpus) + 5) {
        TOPO_ERR(("TOPO PARSE ERROR: Packet too short (%d<%d)[#%d]\n",
                  len,
                  TOPO_HEADER_BYTES(db_ref->num_cpus) + 5,
                  db_ref->num_cpus));
        return BCM_E_FAIL;
    }
    ...
    for (sp_idx = 0; sp_idx < l_base->num_stk_ports; sp_idx++) {
        ...
        if (num_modids > TOPO_MODIDS_MAX) {
            TOPO_ERR(("TOPO PARSE ERROR: TX mod id count %d exceeds DB limit "
        ...
        for (i = 0; i < tp_sp->tx_mod_num; i++) {
            ...
            TOPO_VVERB(("ST TOPO: Map TX mod %d to stk idx %d\n",
        ...
        num_modids = topo_data[cur_ofs++];
        if (num_modids > TOPO_MODIDS_MAX) {
            TOPO_ERR(("TOPO PARSE ERROR: RX mod id count %d exceeds DB limit "
                      "%d\n", num_modids, TOPO_MODIDS_MAX));
            return BCM_E_FAIL;
        }
        ...
        for (i = 0; i < tp_sp->rx_mod_num; i++) {
            ...
            TOPO_VVERB(("ST TOPO: Map RX mod %d to stk idx %d\n",
                       tp_sp->rx_mods[i], sp_idx));
        }
        
        if (cur_ofs + 2 > len) {
            TOPO_ERR(("TOPO PARSE ERROR: Packet too short at %d\n",
                        cur_ofs));
            return BCM_E_FAIL;
        }
    }
    ...
    for (cpu_idx = 0; cpu_idx < cpu_count; cpu_idx++) {
        ...
        if (db_ent == NULL) {
            TOPO_ERR(("TOPO PARSE ERROR: Could not find DB key "
                        CPUDB_KEY_FMT_EOLN, CPUDB_KEY_DISP(db_key)));
            return BCM_E_FAIL;
        }
        db_ent->dest_mod = topo_data[cur_ofs++];
        db_ent->dest_port = topo_data[cur_ofs++];
        db_ent->flags |= CPUDB_F_DEST_KNOWN;
        TOPO_VERB(("TOPO: Mapped " CPUDB_KEY_FMT " to (%d, %d)\n",
