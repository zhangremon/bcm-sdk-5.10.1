/*
 * $Id: topo_brd.c 1.57 Broadcom SDK $
...
topo_cpu_trans_flush(cpudb_ref_t db_ref)
{
    ...
    CPUDB_FOREACH_ENTRY(db_ref, entry) {
        ...
        if (prev_entry != NULL) {
            ...
            TOPO_VERB(("TX path changed to CPU " CPUDB_KEY_FMT_EOLN,
...
topo_board_setup(cpudb_ref_t db_ref)
{
    ...
    TOPO_VERB(("Topology board setup\n"));
    ...
    if (db_ref->local_entry == NULL || db_ref->num_cpus <= 0 ||
        ...
        TOPO_WARN(("TOPO SETUP: Local entry %p, num cpus %d\n",
    ...
    if (db_ref->num_cpus == 1) {
        ...
        if ((rv = cpudb_entry_copy(&bcm_board_topo_data.local_entry,
            ...
            TOPO_WARN(("TOPO SETUP: Failed to copy entry\n"));
        } else {
            bcm_board_topo_data.num_cpus = 1;
            topo_data_set = TRUE;

            if ((rv = topo_board_program(db_ref, &bcm_board_topo_data)) < 0) {
                TOPO_WARN(("TOPO SETUP: Failed topo brd program: %s\n",
                           bcm_errmsg(rv)));
            }
        }
    } else {
    ...
    TOPO_VERB(("TOPO SETUP: Exit rv %d, data set %d\n", rv, topo_data_set));
    return rv;
}
...
_topo_board_default_program(cpudb_ref_t db_ref, topo_cpu_t *topo_cpu)
{
    ...
    TOPO_VERB(("Topology board programming db_ref %p. old_db %p\n",
    ...
    CPUDB_FOREACH_ENTRY(db_ref, entry) {
        ...
        if (!done) {
            TOPO_WARN(("TOPOLOGY WARNING: Did not find tx stack port for "
                CPUDB_KEY_FMT " mod %d\n", CPUDB_KEY_DISP(entry->base.key),
                entry->dest_mod));
        }
    ...
    if (soc_ndev == 2 &&
        SOC_IS_DRACO(0) &&
        SOC_IS_DRACO(1)) {
        rv = bcm_board_topo_24g(topo_cpu, db_ref);
    } else if (soc_ndev == 3 &&
               SOC_IS_XGS_FABRIC(0) &&
               SOC_IS_DRACO(1) &&
               SOC_IS_DRACO(2)) {
        rv = bcm_board_topo_24g_stk(topo_cpu, db_ref);
    } else if (soc_ndev == 5 &&
               SOC_IS_XGS_FABRIC(0) &&
               SOC_IS_DRACO15(1) &&
               SOC_IS_DRACO15(2) &&
               SOC_IS_DRACO15(3) &&
               SOC_IS_DRACO15(4)) {
        rv = bcm_board_topo_lm_xgs2_48g(topo_cpu, db_ref);
    } else if (soc_ndev == 5 &&
               SOC_IS_XGS_FABRIC(0) &&
               SOC_IS_DRACO(1) &&
               SOC_IS_DRACO(2) &&
               SOC_IS_DRACO(3) &&
               SOC_IS_DRACO(4)) {
        rv = bcm_board_topo_48g_stk(topo_cpu, db_ref);
    } else if (soc_ndev == 1 &&
               SOC_IS_TUCANA(0)) {
        uint32 flags;
        
        BCM_IF_ERROR_RETURN(bcm_stk_mode_get(0, &flags));
        if (flags & BCM_STK_SL) {
            rv = bcm_board_topo_48f4g_stk(topo_cpu, db_ref);
        } else {
            rv = bcm_board_topo_48f_stk(topo_cpu, db_ref);
        }
    } else if (soc_ndev == 5 &&
               SOC_IS_XGS_FABRIC(0) &&
               SOC_IS_DRACO(1) &&
               ...
               SOC_IS_LYNX(3) &&
               SOC_IS_LYNX(4)) {
        rv = bcm_board_topo_24g2x_stk(topo_cpu, db_ref);
    } else if (soc_ndev == 7 &&
               SOC_IS_LYNX(0) &&
               SOC_IS_LYNX(1) &&
               SOC_IS_XGS_FABRIC(2) &&
               ...
               SOC_IS_DRACO(5) &&
               SOC_IS_DRACO(6)) {
        rv = bcm_board_topo_48g2x_stk(topo_cpu, db_ref);
    } else if (soc_ndev == 1 &&
               SOC_IS_HERCULES15(0)) {
        ...
        rv = bcm_board_topo_8h(topo_cpu, db_ref);
               ...
               SOC_IS_LYNX(1) &&
               SOC_IS_LYNX(2)) {
        rv = bcm_board_topo_lm2x(topo_cpu, db_ref);
    } else if (soc_ndev == 7 &&
               SOC_IS_XGS_FABRIC(0) &&
               SOC_IS_LYNX(1) &&
               SOC_IS_LYNX(2) &&
               ...
               SOC_IS_LYNX(5) &&
               SOC_IS_LYNX(6)) {
        rv = bcm_board_topo_lm6x(topo_cpu, db_ref);
    } else if (soc_ndev == 1 &&
               SOC_IS_DRACO15(0)) {
        rv = bcm_board_topo_xgs2_12g(topo_cpu, db_ref);
    } else if (soc_ndev == 1 &&
               SOC_IS_EASYRIDER(0)) {
        ...
        rv = bcm_board_topo_sdk_xgs3_24g(topo_cpu, db_ref);
               ...
               SOC_IS_FIREBOLT(3) &&
               SOC_IS_FIREBOLT(4) ) {
        rv = bcm_board_topo_lm_xgs3_12x(topo_cpu, db_ref);
    } else {
        ...
        case cpudb_board_id_sdk_xgs3_48g2:
            ...
            break;
        default:
            TOPO_ERR(("TOPOLOGY ERROR: Unknown board\n"));
            rv = BCM_E_NOT_FOUND;
            break;
        }
    }
...
topo_board_program(cpudb_ref_t db_ref, topo_cpu_t *topo_cpu)
{
    ...
    if (rv < 0) {
        TOPO_WARN(("TOPO board prg: Board programming failed: %s\n",
                   bcm_errmsg(rv)));
        ...
        if ((current_modid & 1) == 0) {
            ...
            if (rv == BCM_E_BADID) {
                *preference = CPUDB_TOPO_MODID_EVEN;
                TOPO_VERB(("unit %d modid preference is even\n",
                           unit));
            }
...
#endif
        break;
    case cpudb_board_id_guenevere:  /* These use full duplex cxns only */
    case cpudb_board_id_lancelot:
    case cpudb_board_id_lynxalot:
    case cpudb_board_id_white_knight:
    case cpudb_board_id_sdk_xgs3_48f:
    case cpudb_board_id_sdk_xgs3_48g:
    case cpudb_board_id_sdk_xgs3_24g:
    case cpudb_board_id_sdk_xgs3_20x:
...
topo_sp_info_update(cpudb_entry_t *entry)
{
    ...
    for (i = 0; i < topo_sp_max; i++) {
        ...
        if (BCM_SUCCESS(bcm_trunk_find(unit, modid, port, &tid))) {
            topo_sp[i].tid = tid;
        } else {
            topo_sp[i].tid = -1;
        }
        TOPO_VERB(("Stack port %d: unit %d, port %d, tid %d\n",
...
topo_board_rapid_recovery(cpudb_ref_t db_ref, int unit, bcm_port_t fail_sp)
{
    ...
    if (topo_cpu == NULL) {
        TOPO_DATA_UNLOCK;
        TOPO_ERR(("Current topology is null\n"));
        return BCM_E_FAIL;
    } 
    ...
    if ((fail_sp_idx = topo_sp_idx_find(unit, fail_sp)) < 0) {
        TOPO_DATA_UNLOCK;
        TOPO_ERR(("Stack port unit %d, port %d, not found\n",
                  unit, fail_sp));
        return BCM_E_NOT_FOUND;
    }
    ...
    if (tid >= 0) {
        ...
        rv = bcm_trunk_get(unit, tid, &t_info);
        if (BCM_FAILURE(rv)) {
            TOPO_ERR(("Cannot get trunk information for unit %d, tid %d\n",
                      unit, tid));
            TOPO_DATA_UNLOCK;
            return rv;
        }
        ...
        for (i = 0; i < t_info.num_ports; i++) {
            if (t_info.tp[i] == fail_sp) {
                continue;
            }
            t_info.tp[j++] = t_info.tp[i];
        }
        t_info.num_ports = j;
        
        /* Trunk failover requires at least one port left in trunk */
        if (t_info.num_ports < 1) {
            TOPO_VERB(("Link failure on unit %d, port %d, is not a "
                       "trunk failover case\n", unit, fail_sp));
            TOPO_DATA_UNLOCK;
            return BCM_E_FAIL;
        }
        ...
        rv = bcm_board_topo_trunk_failover(topo_cpu, unit,
                                           ...
                                           tid, &t_info);
        if (BCM_FAILURE(rv)) {
            TOPO_ERR(("Fail to reconfigure system for trunk failover\n"));
            TOPO_DATA_UNLOCK;
            return BCM_E_FAIL;
        }
