/*
 * $Id: brd_chassis_ast.c 1.8 Broadcom SDK $
                      ...
                      void *cookie)
{
    ...
    return bcm_trunk_set(unit, tid, trunk);
}
                     ...
                     void *cookie)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_trunk_set(unit, tid, trunk));

    for (i=0; i<trunk->num_ports; i++) {
        for (j=0; j<cfm_info->count; j++) {
            slot = cfm_info->cfm[j].slot;
            modid = cfm_info->cfm[j].modid;
            TOPO_VERB(("slot:%d unit:%d tid:%d modid:%d port:%d n:%d\n",
                       slot,
                       unit,
                       tid,
                       modid,
            ...
            /* Override trunking of CFM modid if there is more than
               one CFM */
            BCM_IF_ERROR_RETURN(
                bcm_trunk_override_ucast_set(unit, trunk->tp[i], tid,
                                             modid,
                                             (cfm_info->count > 1)));
        }
    }
...
install_cfm_fp(int unit, bcm_board_cfm_info_t *cfm_info)
{
    ...
    if (cfm_info->count > 1) {
        ...
        */
        ...
        BCM_FIELD_QSET_ADD(qset, bcmFieldQualifyDstModid);
        BCM_FIELD_QSET_ADD(qset, bcmFieldQualifySrcModid);
        ...
        if (cfm_fp_entries > 0) {
            ...
            for ( i=0; i<cfm_fp_entries; i++) {
                ...
                TOPO_VERB
                    (("bcm_field_entry_remove(unit, cfm_fp_entry[i])=%d\n",
                      unit,
                      cfm_fp_entry[i],
                      rv));
            }
            ...
            TOPO_VERB
                (("bcm_field_group_destroy(unit, cfm_fp_group)=%d\n",
                      unit,
                      cfm_fp_group,
                      rv));
        }
        ...
        for ( i=0; i<cfm_info->count; i++) {
            ...
            BCM_IF_ERROR_RETURN
                (bcm_field_qualify_SrcModid(unit, cfm_fp_entry[i], 
                                            cfm_info->cfm[partner].modid,
                                            BCM_FIELD_EXACT_MATCH_MASK));
            }

            BCM_IF_ERROR_RETURN
                (bcm_field_qualify_DstModid(unit, cfm_fp_entry[i], 
...
{
    TOPO_VERB(("TOPO: XGS 5675 BCM956000 CFM (Asymmetric Trunk)\n"));
...
chassis_ast_xgs2_48g(topo_cpu_t *tp_cpu, cpudb_ref_t db_ref)
{
    ...
    TOPO_VERB(("TOPO: XGS 569x 48GE BCM956000 LM (Asymmetric Trunk)\n"));
...
chassis_ast_xgs2_6x(topo_cpu_t *tp_cpu, cpudb_ref_t db_ref)
{
    ...
    TOPO_VERB(("TOPO: XGS 5674 6XE BCM956000 LM (Asymmetric Trunk)\n"));
    _chassis_ast_default_lm_psc = XGS2_LM_HASH;
    return chassis_ast_lm(tp_cpu, db_ref, connection);
}
...
chassis_ast_xgs3_12x(topo_cpu_t *tp_cpu, cpudb_ref_t db_ref)
{
    ...
    TOPO_VERB(("TOPO: XGS3 56501 12XE BCM956000 LM (Asymmetric Trunk)\n"));
    _chassis_ast_default_lm_psc = XGS2_LM_HASH;
    return chassis_ast_lm(tp_cpu, db_ref, connection);
}
...
{
    TOPO_VERB(("TOPO: XGS 56700 BCM956000 CFM (Asymmetric Trunk)\n"));
    return chassis_ast_cfm(tp_cpu, db_ref);
}
...
{
    TOPO_VERB(("TOPO: XGS3 56800 BCM956000 LM (Asymmetric Trunk)\n"));
...
chassis_ast_xgs3_48g(topo_cpu_t *tp_cpu, cpudb_ref_t db_ref)
{
    ...
    TOPO_VERB(("TOPO: XGS3 56504 48GE BCM956000 LM (Asymmetric Trunk)\n"));
    ...
    if ((master < 0) || (master > 1) || (partner < 0) || (partner > 1)) {
        TOPO_VERB(("Parameter error: master=%d partner=%d\n",
                   master, partner));
    ...
    TOPO_VERB(("Master: %d,%d\n", master, master_modid));
    TOPO_VERB(("Partner: %d,%d\n", partner, partner_modid));
