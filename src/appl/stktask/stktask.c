/*
 * $Id: stktask.c 1.64.2.1 Broadcom SDK $
...
#define ST_OUT(stuff) TKS_DEBUG(TKS_DBG_STKTASK, stuff)
#define ST_VERB(stuff) TKS_DEBUG(TKS_DBG_STKTASK | TKS_DBG_VERBOSE, stuff)
#define ST_VVERB(stuff) TKS_DEBUG(TKS_DBG_STKV, stuff)
#define ST_WARN(stuff) TKS_DEBUG(TKS_DBG_STKTASK | TKS_DBG_WARN, stuff)
#define ST_ERR(stuff) TKS_DEBUG(TKS_DBG_STKTASK | TKS_DBG_ERR, stuff)
#else
#define ST_OUT(stuff)
#define ST_VERB(stuff)
#define ST_VVERB(stuff)
#define ST_WARN(stuff)
#define ST_ERR(stuff)
...
#define STK_PORT_LAST_LINK_SET(_sp, _link) \
    if (_link) st_stk_port_flags[_sp] |= ST_SPF_LAST_EVENT_UP;    \
...
st_bad_event_warn(bcm_st_state_t state, bcm_st_event_t event)
{
    ...
    if (warn_count < bcm_st_max_bad_event_warnings) {
        ++warn_count;
        ST_VERB(("ST ERR: Unexpected event %s in state %s.\n",
                 BCM_STE_VALID(event) ?
...
st_disc_event_process(bcm_st_event_t event)
{
    ...
    if (BCM_STE_DISC_STARTABLE(event)) { /* Re-start discovery */
        ST_VERB(("ST: State DISC, event %s restarting discovery\n",
...
discovery_start(void)
{
    ...
    if (st_disc_tid == SAL_THREAD_ERROR) {
        ...
        ST_ERR(("ST: Discovery thread not running on start; aborting\n"));
        return;
    }
    if (!(bcm_st_flags & BCM_STF_DISC_SLEEPING)) {
        ST_WARN(("ST: Discovery thread not sleeping on start\n"));
...
discovery_stop(int retry_max)
{
    ...
    if (rv < 0) {
        ST_WARN(("ST: Discovery abort (fail) returns %s\n", bcm_errmsg(rv)));
    }
    ...
    if (!(bcm_st_flags & BCM_STF_DISC_SLEEPING)) {
        ST_WARN(("ST: Discovery thread won't sleep; aborting\n"));
        ST_LOCK;
        bcm_st_flags |= BCM_STF_ABORT;
        ST_UNLOCK;
        rv = BCM_E_FAIL;
...
discovery_quit(int retry_max)
{
    ...
    if (st_disc_tid != SAL_THREAD_ERROR) {
        ST_WARN(("ST: Discovery thread won't quit\n"));
        rv = BCM_E_FAIL;
    }
    return rv;
}
...
st_state_change(bcm_st_state_t new_state, bcm_st_event_t event)
{
    ...
    ST_VERB(("ST: Trans from %s to %s on event %s [T=%u]\n",
    ...
    if (rv < 0) { /* Error from appl state transition */
        ST_ERR(("ST: Appl trans returns %s; State was %s.\n",
    ...
    if (new_state == st_state) {
        ST_VERB(("ST: Trans from %s cancelled\n",
                 bcm_st_state_strings[st_state]));
        return;
    }
    ...
    case BCM_STS_ATTACH: /* Switch database over; call stack attach update */
        ...
        if ((rv = st_config.st_attach(bcm_st_cur_db)) < 0) {
            ST_WARN(("ST: st_attach returns %s\n", bcm_errmsg(rv)));
        }
        break;
    default:
        ST_UNLOCK;
...
st_pending_events_handle(void)
{
    ...
    if (st_pending_events != 0) {
        ST_WARN(("ST: Clearing illegal pending flags: 0x%x\n",
                 st_pending_events));
        st_pending_events = 0;
    }
}
    ...
    for (i = 0; i < st_config.base.num_stk_ports; i++) {
        ...
        if (!found) {
            if (st_num_stk_units >= CPUDB_UNITS_MAX) {
                ST_ERR(("ST: cannot stack more than %d units\n",
                        CPUDB_UNITS_MAX));
                return BCM_E_FAIL;
            }
...
st_comm_setup(void)
{
    ...
    for (i = 0; i < st_config.base.num_stk_ports; i++) {
        ...
        if (rv < 0) {
            ST_WARN(("ST: stkport %d.%d nexthop add failed: %s\n",
    ...
    if (bcm_st_cfg_flags & BCM_STC_DISC_PKT_REGISTER) {
        ...
        if (rv < 0) {
            ST_WARN(("ST: nexthop register failed: %s\n", bcm_errmsg(rv)));
        }
    }
    if (!next_hop_running()) {
        ST_WARN(("ST: Nexthop is not running\n"));
    }
    if ((rv = next_hop_update((cpudb_base_t *)&st_config.base)) < 0) {
        ST_WARN(("ST: Nexthop update returns %s\n", bcm_errmsg(rv)));
    }
    ...
    if (bcm_st_cfg_flags & BCM_STC_START_ATP) { /* Start and bring up ATP */
        if (!atp_running()) {
            ...
            if ((rv = atp_start(bcm_st_atp_flags, unit_bmp,
                                BCM_RCO_F_ALL_COS)) < 0) {
                ST_WARN(("ST: ATP start returns %s\n", bcm_errmsg(rv)));
            }
        }
    }
    ...
    if ((rv = atp_key_add(st_config.base.key, TRUE)) < 0) {
        ST_WARN(("ST: ATP key add returns %s\n", bcm_errmsg(rv)));
    }
}
               ...
               void *cookie)
{
    ...
    if (idx < 0) {
        ST_WARN(("ST: Nexthop pkt on non-stk port (%d, %d)\n",
                  rx_unit, rx_port));
        return BCM_RX_NOT_HANDLED;
    }
    ...
    if (st_state == BCM_STS_TOPO) {
        ...
        */
        ST_VERB(("ST: Probe pkt in TOPO from " CPUDB_KEY_FMT_EOLN,
                 CPUDB_KEY_DISP(src_key)));

        bcm_st_event_send(BCM_STE_TOPO_FAILURE);

    } else {
        ST_VERB(("ST: Probe pkt in from " CPUDB_KEY_FMT_EOLN,
                 CPUDB_KEY_DISP(src_key)));
...
st_linkscan_handler(int unit, bcm_port_t port, bcm_port_info_t *info)
{
    ...
    if (bcm_st_flags & BCM_STF_ABORT) {      /* Task aborted */
        return;
    }
    ST_LOCK;
    ...
    STK_PORT_LAST_LINK_SET(idx, info->linkstatus);
    stk_port_last_event_us[idx] = sal_time_usecs();

    /* If port was up, send link down now; */
    if (!info->linkstatus && STK_PORT_LINK_UP(idx)) {
        int rv;
        ...
        if (BCM_SUCCESS(topo_board_rapid_recovery(bcm_st_cur_db,
                                                  unit, port))) {
            ST_UNLOCK;
            ST_VERB(("Rapid recovery successful for unit %d, port %d\n",
                     unit, port));
            return;
        }
        ...
        if (!bcm_st_reserved_modid_enable_get()) {
            if ((rv = nh_tx_src_mod_port_set(unit, port, -1, -1)) < 0) {
                ST_WARN(("ST: Link down, error clearing mod/port: %s\n",
                         bcm_errmsg(rv)));
            }
        }
    }
    ...
    ST_VERB(("ST: Link change idx %d, (%d, %d) %s\n", idx, unit, port,
...
{
    ST_WARN(("STACK: communication timeout to " CPUDB_KEY_FMT_EOLN,
             CPUDB_KEY_DISP(key)));
...
pre_discovery_prep(void)
{
    ...
    if (bcm_st_disc_db == CPUDB_REF_NULL) {
        ST_ERR(("ST ERR: Error creating DB.\n"));
        return -1;
    }
    st_config.base.dseq_num = st_ev_count;
    if (cpudb_local_base_info_set(bcm_st_disc_db,
                                  (cpudb_base_t *)&st_config.base) < 0) {
        ST_ERR(("ST ERR: Error creating local entry.\n"));
        CPUDB_TOPO_DESTROY(bcm_st_disc_db);
        bcm_st_disc_db = CPUDB_REF_NULL;
        return -1;
    }

    /* disable any fabric trunks during discovery */
    (void)bcm_board_trunk_destroy(bcm_st_disc_db->local_entry);
    bcm_st_disc_db->old_db = bcm_st_cur_db;
...
st_encap_check(cpudb_entry_t *local_entry)
{
    ...
    for (i = 0; i < local_entry->base.num_stk_ports; i++) {
        ...
        if (BCM_SUCCESS(bcm_port_encap_config_get(unit, port, &mode)) &&
            ...
            ST_VERB(("ST: sp(%d,%d) idx=%d encap=%d stkmode=%d\n",
            ...
            ST_VERB(("ST: sp(%d,%d): could not get encap/mode.\n", unit,port));
        }
    }
}
...
st_disc_thread(void *cookie)
{
    ...
    ST_VERB(("ST [T=%u]: Disc thread started, id %d\n",
    ...
    while (TRUE) {
        ...
        if (st_state != BCM_STS_DISC) { /* Bad state */
            bcm_st_flags |= BCM_STF_DISC_SLEEPING;
            ST_UNLOCK;
            ST_VERB(("ST: Disc thread awake, but state %s; continuing\n",
                     bcm_st_state_strings[st_state]));
            continue;
        }
    ...
    ST_VERB(("STACK: Disc thread exiting\n"));
    sal_thread_exit(rv);
}
...
stk_port_link_state_init(int unit, bcm_port_t port, int sp)
{
    ...
    if (rv < 0) {
        ST_ERR(("ST: activating stkport %d.%d link status failed: %s\n",
                unit, port, bcm_errmsg(rv)));
        link = 0;
    }
...
st_link_init(void)
{
    ...
    if (bcm_st_cfg_flags & BCM_STC_LINK_REGISTER) {
        for (i = 0; i < st_num_stk_units; i++) {
            ...
            if (rv < 0) {
                ST_ERR(("ST: link register failed: %s\n", bcm_errmsg(rv)));
            }
        }
    }
...
local_board_setup(cpudb_key_t key)
{
    ...
    if ((local_ref = cpudb_create()) == NULL) {
        ST_WARN(("ST: Could not create setup DB\n"));
        return BCM_E_MEMORY;
    }

    
    if ((rv = cpudb_local_base_info_set(local_ref, &st_config.base)) < 0) {
        ST_WARN(("ST: Setup could not set base info: %s\n", bcm_errmsg(rv)));
        return rv;
    }
    ...
    if ((rv = topology_mod_ids_assign(local_ref)) < 0) {
        ST_WARN(("ST: Could not setup modids: %s\n", bcm_errmsg(rv)));
        return rv;
    }
    
    if ((rv = topo_board_setup(local_ref)) < 0) {
        ST_WARN(("ST: Could not setup local topology: %s\n", bcm_errmsg(rv)));
        return rv;
    }
...
st_disc_thread_started(void)
{
    ...
    while (!(bcm_st_flags & (BCM_STF_DISC_SLEEPING|BCM_STF_ABORT))) {
        ...
        sal_usleep(10000);
        if (++count > 400) {
            ST_WARN(("ST: Discovery thread not alive on startup\n"));
...
bcm_st_start(bcm_st_config_t *config, int enable)
{
    ...
    if (st_disc_tid != SAL_THREAD_ERROR) {
        ST_WARN(("ST: Discovery thread is running\n"));
        return BCM_E_FAIL;
    }

    if (!atp_running() && !(bcm_st_cfg_flags & BCM_STC_START_ATP)) {
        ST_WARN(("ST: ATP is not running\n"));
    }
    ...
    ST_VERB(("Stack task started [T=%u]: %s with %d stk unit%s.\n",
    ...
    if (st_disc_tid == SAL_THREAD_ERROR) {
        bcm_st_flags &= ~BCM_STF_RUNNING;
        ST_UNLOCK;
        ST_VERB(("ST: Could not create discovery thread\n"));
        return BCM_E_FAIL;
    }
...
bcm_st_event_send(bcm_st_event_t event)
{
    ...
    ST_VERB(("ST: sending event %s [T=%u]\n",
             bcm_st_event_strings[event],
             sal_time_usecs()));
                  ...
                  cpudb_ref_t cur_db)
{
    if (event == BCM_STE_TIMEOUT && from != BCM_STS_BLOCKED) {
        ST_WARN(("TKS ST TIMEOUT in state %s\n", bcm_st_state_strings[from]));
    }
    ST_VERB(("TKS ST transition: disc_db %p. cur_db %p\n",
             disc_db, cur_db));
