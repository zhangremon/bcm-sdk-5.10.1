/* 
 * $Id: brd_common.c 1.13 Broadcom SDK $
                      ...
                      int start, int flags)
{
    ...
    trunk.psc = -1; /* Default hash algorithm */
    trunk.dlf_index = -1;
    trunk.mc_index = -1;
    trunk.ipmc_index = -1;
    tid = start;
    ...
    for (i = 0; i < entry->base.num_stk_ports; i++) {
        ...
        trunk.tp[trunk.num_ports] = devport;
        trunk.tm[trunk.num_ports] = -1;
        trunk.num_ports++;
        ...
        if (trunk.num_ports > 1) {
            ...
            if (bcm_board_trunk_callback_func) {
                BCM_IF_ERROR_RETURN(
                    bcm_board_trunk_callback_func(unit, tid, &trunk,
...
bcm_board_topo_trunk_failover(topo_cpu_t *topo_cpu,
                              ...
                              bcm_trunk_t tid, bcm_trunk_add_info_t *trunk)
{
    int              i, rv;
    bcm_module_t     mod;
    bcm_port_t       port;
    ...
    if (trunk->num_ports <= 0) {    /* Trunk should contain at least 1 port */
        return BCM_E_FAIL;
    }
    repl_sp = trunk->tp[0];
