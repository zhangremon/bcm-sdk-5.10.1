/*
 * $Id: i2c_cmds.c 1.94.2.4 Broadcom SDK $
 *
 ...
 */
STATIC 
int _i2c_mux_open(int unit, int mux_num, uint32 flags, int speed, 
                  char **mux_name)
{
    int fd = -1;
    char *name;

    name = I2C_LPT_0; /* initializing to get rid of compile errors */
    switch (mux_num) {
    case 0:
        /* First try I2C_LPT_0 */
        name = I2C_LPT_0;
...
#ifdef SHADOW_SVK
    case 1:
        /* Second mux is always on boards with I2C_MUX_7 */
        name = I2C_MUX_7;
        fd = bcm_i2c_open(unit, name, flags, speed);
        if (fd >= 0) {
            _i2c_mux_info_set(unit, name, I2C_MUX_F_MAP);
        }
        break;
#endif
    default:
...
uint8 _i2c_mux_channel_get(int unit, int mux)
{
    ...
    if (rv == 0) {
        soc_cm_debug(DK_VERBOSE, "name = %s flags %x\n", name, flags);
        if (flags & I2C_MUX_F_MAP) {
            channel = (1 << mux);
        }
    }
    
    soc_cm_debug(DK_VERBOSE, "mux %d -> channel %d\n", mux, channel);
    return channel;
}
#endif /* SHADOW_SVK */
...
cmd_muxsel(int unit, args_t *a)
{
    ...
    {
    int muxnum = 0;
    if (sal_strcasecmp(dev_name, "mux6") == 0) {
        muxnum = 0;
    } else if (sal_strcasecmp(dev_name, "mux7") == 0) {
        muxnum = 1;
    }
    
    if ( (fd = _i2c_mux_open(unit, muxnum, 0, 0, &dev_name)) < 0) {
 	printk("%s: cannot open I2C device %s: %s\n",
...
char cmd_pio_usage[] =
    "Usages:\n\t"
    "       pio [dev=0|1] [port=0..7] [data=value]\n\t"
...
cmd_pio(int unit, args_t *a)
{
    ...
    dev = port = data = -1;
    dev_mask = PIO_PORT_0 | PIO_PORT_1;
    port_mask = 0xff;
    ...
    for (dev = 0; dev < 2; dev++) {
        ...
        char *name = (dev == 0) ? I2C_IOP_0 : I2C_IOP_1;
        ...
        for (port = 0; port < 5; port++) {
            ...
            if (data != -1) {
                if (data > 255) {
                    printk("Data must be 0..255\n");
                    break;
                }
                wr_data = data;
                /*printk("writing port dev %d %d <- %d\n", dev, port,
                 * wr_data);*/
                rv = bcm_i2c_write(unit, fd, 0x8 + port, (uint8 *)&wr_data, 1);
                if (rv != SOC_E_NONE) {
                    printk("%s:Port%d Is INPUT Port\n", name, port);
                }
            } else {
                int i;
                for(i = 0; i < 2; i++) {
...
soc_i2c_detect_board(void)
{
        ...
        for( id = 0; id < soc_i2c_device_count(dev); id++) {
            ...
            if ( i2c_dev ) {
                ...
                if ( ( i2c_dev->driver->id == PCA9505_DEVICE_TYPE ) &&
                    ...
                    /* Read the board ID register */
                    /* soc_cm_print("reading id from 9505\n"); */
                    i2c_dev->driver->read(dev, id, 0, &board_id, &id_detected);
                }
#endif /* SHADOW_SVK */
                /* ADC */
    ...
    else if (id_detected && ((board_id == 0x65) || (board_id == 0x67))){
        /* Katana */
        board_type = G28;
    }
...
#ifdef SHADOW_SVK
                ( !sal_strncasecmp(function,"test",  strlen(function))) ||
#endif
...
#ifdef SHADOW_SVK
	    if (sal_strncasecmp(op, "shadow_40GSVK", strlen(op)) == 0) {
		svk_40g = 1;
		continue;
	    }
	    if (sal_strncasecmp(op, "shadow_10GSVK", strlen(op)) == 0) {
		svk_10g = 1;
		continue;
	    }
#endif /* SHADOW_SVK */
...
#ifdef SHADOW_SVK
        if ( !sal_strncasecmp(function,"test", strlen(function))) {
            int i = 0;
            uint8 lptVal = 0;
            int fd;
            char *name;
            ...
            if (rv < 0) {
                printk("I2C Test Failed: I2C probe Failed\n");
                return CMD_FAIL;
            }
            if (svk_40g) {
                ...
                for(i = 0; i < 5; i++) {
                    if (soc_i2c_device_present(unit, saddr[i]) < 0) {
                        printk("I2C Test Failed: Device NOT at slave address 0X%02X (%s)\n",
                            saddr[i], soc_i2c_saddr_to_string(unit, saddr[i]));
                        return CMD_FAIL;
                    }
                }

                if ( (fd = _i2c_mux_open(unit, 0, 0, 0, &name)) < 0) {
                    printk("I2C Test Failed: MUX at 0x76 Failed\n");
                    return CMD_FAIL;
                }
                ...
                for(i = 0; i < 4; i++) {
                    lptVal = (1 << i);
                    if( (rv = bcm_i2c_write(unit, fd, 0, &lptVal, 1)) < 0){
                        printk("I2C Test Failed:MUX Channel%d couldn't be selected\n", i);
                        return CMD_FAIL;
                    }
                    if (soc_i2c_device_present(unit, 0x50) < 0) {
                        printk("QSP not present in Channel %d \n", i);
                    }
                }
                printk("I2C Test Passed\n");
                return CMD_OK;
            } else if (svk_10g) {
                ...
                for(i = 5; i < 8; i++) {
                    ...
                    if (soc_i2c_device_present(unit, 0x2C) < 0) {
                        printk("DAC not present at Channel %d \n", i);
                    }
                }
                if ( (fd = _i2c_mux_open(unit, 1, 0, 0, &name)) < 0) {
                    printk("I2C Test Failed: MUX at 0x77 Failed\n");
                    return CMD_FAIL;
                }
                ...
                for(i = 0; i < 8; i++) {
                    ...
                    if (soc_i2c_device_present(unit, 0x50) < 0) {
                        printk("SFP+ not present in Channel %d \n", i);
                    }
                }
                printk("I2C Test: Passed\n");
                return CMD_OK;
            } else {
                    printk("I2C Test: Unknown Board type\n");
                    return CMD_FAIL;
            }
        }
#endif
...
#define _SHADOW_OPEN_TEST(_u, _nm, _fl, _sp, _fd, _pf)               \
    do {                                                             \
       if ((_fd = bcm_i2c_open(_u, _nm, _fl, _sp)) < 0) {            \
           _pf = 0;                                                  \
           soc_cm_debug(DK_VERBOSE, "Could not open %s: %s\n",       \
                        _nm, bcm_errmsg(_fd));                       \
       }                                                             \
    } while(0)
...
#define _SHADOW_WRITE_TEST(_u, _f, _r, _d, _l, _nm, _rv, _pf)        \
    do {                                                             \
       ...
       if (_rv < 0) {                                                \
          _pf = 0;                                                   \
          soc_cm_debug(DK_VERBOSE, "%s writing failed: %s\n",        \
                       _nm, bcm_errmsg(_rv));                        \
        }                                                            \
    } while (0)
...
#define _SHADOW_READ_TEST(_u, _f, _r, _d, _l, _nm, _rv, _pf)         \
   do {                                                              \
      ...
      if (_rv < 0) {                                                 \
          _pf = 0;                                                   \
          soc_cm_debug(DK_VERBOSE, "%s reading failed: %s\n",        \
                       _nm, bcm_errmsg(_rv));                        \
      }                                                              \
   } while (0)
...
#define _SHADOW_IOCTL_TEST(_u, _f, _op, _d, _l, _nm, _rv, _pf)       \
   do {                                                              \
      ...
      if (_rv < 0) {                                                 \
          _pf = 0;                                                   \
          soc_cm_debug(DK_VERBOSE, "%s ioctl failed: %s\n",          \
                       _nm, bcm_errmsg(_rv));                        \
      }                                                              \
   } while (0)
...
_bb_shadow_test(int unit, bb_type_t *baseboard)
{
    ...
    if (rv == SOC_E_NONE) {
        if (rdval != /* expected board id(s) */) {
            pass = 0;
            soc_cm_debug(DK_VERBOSE, "....Invalid board ID %x\n", buf[0]);
        }
    }
#endif
    _SHADOW_TEST_STATUS_PRINT(pass);

    printk("   Checking MAX127: ");
    pass = 1;
    _SHADOW_OPEN_TEST(unit, I2C_ADC_0, 0, 0, fd, pass);
    if (fd >= 0) {
        int i;
        int pass = 1;
        max127_t max127_data;
        
        for (i = 0; i < 8; i++) {
            _SHADOW_IOCTL_TEST(unit, fd, i, max127_data, len,
                               I2C_ADC_0, rv, pass);
        }
    }
    _SHADOW_TEST_STATUS_PRINT(pass);
    ...
    for (dev = 0; dev < 2; dev++) {
        ...
        if (fd >= 0) {
            ...
            for (i = 0; i < 8; i++) {
                ...
                soc_cm_debug(DK_VERBOSE, "writing 0x%02x ", wrval);
                _SHADOW_WRITE_TEST(unit, fd, 0, wrval, 1, name, rv, pass);
            }
            wrval = 0;
            soc_cm_debug(DK_VERBOSE, "2. Bit(0) -> Bit(7) read 0 write 1\n");
            for (i = 0; i < 8; i++) {
                _SHADOW_READ_TEST(unit, fd, 0, rdval, len, name, rv, pass);
                if (rv == SOC_E_NONE) { 
                    soc_cm_debug(DK_VERBOSE, "read 0x%02x\n", rdval);
            ...
            soc_cm_debug(DK_VERBOSE, "3. Bit(7) -> Bit(0) read 1 write 0\n");
            for (i = 7; i > 0 ; i--) {
                _SHADOW_READ_TEST(unit, fd, 0, rdval, len, name, rv, pass);
                if (rv == SOC_E_NONE) { 
                    soc_cm_debug(DK_VERBOSE, "read 0x%02x\n", rdval);
...
dac_params[NUM_BOARD_TYPES][4] = {
     ...
     * used */
    {{0,"Core",1.198,.805,0,0,LTC1427_MIN_DACVAL+((5*HVSPO)+50),LTC1427_MAX_DACVAL-((5*HVSPO)+50),1},
     {1,"1.0",1.198,.805,0,0,LTC1427_MIN_DACVAL+((5*HVSPO)+50),LTC1427_MAX_DACVAL-((5*HVSPO)+50),1},
     {2,"1.8",2.063,1.538,0,0,LTC1427_MIN_DACVAL,LTC1427_MAX_DACVAL,1},
     {3,"3.3",3.645,2.980,0,0,LTC1427_MIN_DACVAL,LTC1427_MAX_DACVAL,1}},
    /* Katana  SDK design: aka G28 */
    {{0,"Core",  -1,1,0,0,LTC1427_MIN_DACVAL,LTC1427_MAX_DACVAL,1},
     {1,"3.3",   -1,1,0,0,LTC1427_MIN_DACVAL,LTC1427_MAX_DACVAL,1},
     {2,"1.5",   -1,1,0,0,LTC1427_MIN_DACVAL,LTC1427_MAX_DACVAL,1},
     {3,"Analog",-1,1,0,0,LTC1427_MIN_DACVAL,LTC1427_MAX_DACVAL,1}},
    /* Enduro-2  SDK design: aka G29 */
    {{0,"Core",  -1,1,0,0,LTC1427_MIN_DACVAL,LTC1427_MAX_DACVAL,1},
     {1,"Analog", -1,1,0,0,LTC1427_MIN_DACVAL,LTC1427_MAX_DACVAL,1},
     {3,"3.3",-1,1,0,0,LTC1427_MIN_DACVAL,LTC1427_MAX_DACVAL,1},
     {0}},
    ...
    if(!sal_strcasecmp(function,"clock")  ||
        ...
        if( source && value ) { 
            ...
            if (bb_clocks[bx][i].mux != 0xFF) {
                ...
                uint8 channel;
                /* Open MUX device */
                if ( (mux = _i2c_mux_open(unit, 0, flags, 0, &name)) < 0) {
                    printk("%s: cannot open I2C device %s: %s\n",
            ...
            */
            ...
            for(  ; i < bb_clock_len[bx]; i++) {
                ...
                if (bb_clocks[bx][i].mux != 0xFF) {
                    ...
                    if( (rv = bcm_i2c_write(unit, mux, 0,
                               &channel, 1)) < 0){
                        printk("Error: write of %s byte failed:%s\n",
                                name, bcm_errmsg(rv));
                        return CMD_FAIL;
                    }
#else
...
#undef BB_DEBUG
	...
	if (value && (bb_vs_config[bx][i].mux != 0xFF)) {
	    ...
	    /* Open MUX (lpt0) device */
            dev_name = _i2c_mux_dev_name_get(I2C_MUX_VOLTAGE_CONTROL);
	    if ( (mux = _i2c_mux_open(unit, 0, flags, 0, &dev_name)) < 0) {
		printk("Could not open %s for mux selection:%s\n",
