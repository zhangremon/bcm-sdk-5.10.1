/*
 * $Id: parse.c 1.38.6.2 Broadcom SDK $
 ...
 */
{
    ...
    while ((*ts != term) && (*tp) && 
           (toupper((unsigned)*ts) == toupper((unsigned)*tp))) {
        ts++;
        tp++;
    }
 ...
 */
{
    ...
    case PQ_HEX:
        ...
        if (pq->pq_type & PQ_LAB) {
            printk("WARNING: using deprecated zero based port parsing\n");
        }
        break;
    case PQ_STRING:
        fv = *(void **)pq->pq_value;
    ...
    case PQ_MAC:
        if (parse_macaddr(s, mac_addr)) {
            printk("Expected MAC address in the format "
    ...
    case PQ_IP:
        if (parse_ipaddr(s, &ip_addr)) {
            printk("Expecting IP address in the format "
                   "w.x.y.z or 0x<value>\n");
            return(-1);
        }
    ...
    case PQ_IP6:
        if (parse_ip6addr(s, ip6_addr)) {
            printk("Expecting IPV6 address in the format "
    ...
    case PQ_PBMP:
        /* Check for ? */
        if (strcmp(s, "?") == 0 || parse_pbmp(pq->pq_unit, s, &pbm)) {
            /* AND with pbmp_valid */
            if (SOC_IS_RAPTOR(pq->pq_unit) || SOC_IS_RAVEN(pq->pq_unit)) {
                ...
                printk("Warning: continuing with invalid bitmap\n");
            } else {
                if (strcmp(s, "?") != 0) {
                    printk("Invalid port bitmap \"%s\"\n", s);
                }
                printk("Examples:\n");
                printk("  fe ge e fe5 fe8-fe15 fe0-fe46:2 fe:2 fe0,ge1,cpu\n");
                printk("  1,25,27 none 0x0 all 0xbffffff\n");
                return(-2);
            }
    ...
    case PQ_PORTMODE:
        /* Check for ? */
        if (strcmp(s, "?") == 0 || parse_port_mode(s, &pm)) {
            if (strcmp(s, "?") != 0) {
                printk("Invalid port mode string.\n");
            }
            printk("Examples: 1000full,100,10,pause 100,pause_tx,pause_rx\n");
            return(-2);
        }
        v = INT_TO_PTR(pm);
        break;
    case PQ_PORTABIL:
        /* Check for ? */
        if (strcmp(s, "?") == 0 || parse_port_ability(s, &pa)) {
            if (strcmp(s, "?") != 0) {
                printk("Invalid port mode string.\n");
            }
            printk("Examples: 21g,1000full,100,10,pause 100,pause_tx,pause_rx\n");
            return(-2);
        }
        v = (void *)&pa;
        vs = sizeof(pa);
    ...
    case PQ_PORT:
        /* Check for ? */
        if (!strcmp(s, "?") || parse_port(pq->pq_unit, s, &port)) {
            if (strcmp(s, "?"))
                printk("Invalid port \"%s\"\n", s);
            printk("Examples: fe0 ge1 3 cpu0 any\n");
            return(-2);
        }
    ...
    case PQ_MOD_PORT:   /* Parse as <mod.port>; if no '.', parse as port */
        /* Check for ? */
        if (!strcmp(s, "?") || parse_mod_port(pq->pq_unit, s, &mod_port)) {
            if (strcmp(s, "?"))
                printk("Invalid module.port \"%s\"\n", s);
            printk("<mod>.<port> or port:  2.4 fe0 cpu0 any\n");
            return(-2);
        }
        v = (void *)&mod_port;
        vs = sizeof(mod_port);
    ...
    case PQ_LR_PHYAB:
        /* Check for ? */
        if (strcmp(s, "?") == 0 || parse_phy_control_longreach_ability(s, &lr_pa)) {
            if (strcmp(s, "?") != 0) {
                printk("Invalid longreach phy ability string.\n");
            }
            printk("Examples: 100x4,100x2,100x1,50x2,50x1,33x2,33x1,25x2,25x1,20x2,20x1,10x2,10x1,pause_tx,pause_rx\n");
            return(-2);
        }
        v = INT_TO_PTR(lr_pa);
        break;
    default:
        /* Defensive default */
        printk("Unexpected parse qualifier 0x%03x\n", PQ_TYPE(pq->pq_type));
        return(-2);
    }
    /*
 ...
 */
{
    ...
    while (pq_cur < pt->pt_cnt) {
        ...
        case PQ_HEX:
            ...
            break;
        case PQ_BOOL:
            sal_strcpy(ds, PTR_TO_INT(defl) ? "True" : "False");
            break;
        case PQ_STRING:
 ...
 */
{
    ...
    for (i = 0; i < pq_table->pt_cnt; i++) {    /* Fill in defaults */
		...
		memcpy(pq->pq_value, pq->pq_default, vs);
	    } else {
		memset(pq->pq_value, 0, vs);
	    }
        } else {
 ...
 */
{
    ...
    if (pq_table->pt_cnt + 1 >= pq_table->pt_alloc) {
        ...
        if (npe == NULL) {
            soc_cm_print("parse_table_add: ERROR: "
