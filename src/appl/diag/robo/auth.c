/*
 * $Id: auth.c 1.15.2.1 Broadcom SDK $
...
cmd_robo_auth(int unit, args_t *a)
{
    ...
    if (sal_strcasecmp(subcmd, "mac") == 0) {
        ...
        if (sal_strcasecmp(subcmd, "init") == 0) {
            ...
            printk("All fe ports set to mac address ");
        ...
        if (sal_strcasecmp(subcmd, "add") == 0) {
            ...
            PBMP_ITER(pbmp, port) {
                if (SOC_IS_ROBO5396(unit)){
                    if (i >= 1) {
                        printk("ERROR : %s %s port %s failed:  \n",
                ...
                printk("port %s mac address ",SOC_PORT_NAME(unit, port));
                printk("%02x:%02x:%02x:%02x:%02x:%02x set.\n",
                       mac[0],mac[1],mac[2],mac[3],mac[4],mac[5]);
                i++;
            }
            return CMD_OK;
        ...
        if (sal_strcasecmp(subcmd, "del") == 0) {
            ...
            PBMP_ITER(pbmp, port) {
                ...
                printk("port %s mac address ",SOC_PORT_NAME(unit, port));
                printk("%02x:%02x:%02x:%02x:%02x:%02x deleted.\n",
                       mac[0],mac[1],mac[2],mac[3],mac[4],mac[5]);
            }
            return CMD_OK;
        }
        ...
        if (SOC_IS_TBX(unit)) {
            ...
            switch (auth_num) {
            	...
            	case AUTH_SEC_SA_OVERLIMIT_CPUCOPY:
                  ...
                  break;
            	default:
                  printk("ERROR: %s without expected parameter.\n",   
                      ARG_CMD(a));   
                  return CMD_FAIL;
            }
#endif
