    ...
    if (!(l2addr->flags & BCM_L2_MCAST)) {              
        if (BCM_GPORT_IS_SET(l2addr->port)){
            ...
            if (BCM_GPORT_IS_LOCAL(l2addr->port)) {
                ...
                printk(" modid=%d port=%d%s%s", l2addr->modid, gport_id, 
                ...
                if (mod_id != l2addr->modid){
                    printk(" (l2.modid=%d)", l2addr->modid);
                }
                port_id = BCM_GPORT_MODPORT_PORT_GET(l2addr->port);
                printk(" modid=%d port=%d%s%s", mod_id, port_id, 
                        ((local_modid) == mod_id) ? "/" : " ",
                        ((local_modid == mod_id) ? 
                ...
                printk(" port=%d(vp_group)", l2addr->port);
                ...
                if (SOC_IS_TBX(unit)){
                    ...
                    printk(" port=%d vport=%d", port_id, vp_id);
#endif  /* BCM_TB_SUPPORT */
                } else {
                    printk(" port=%d(subport_port)", gport_id);
                }
            } else {
                printk(" port=%d(UNEXPECTED GPORT)", l2addr->port);
            }
        } else {
...
if_robo_bpdu(int unit, args_t *a)
{
    ...
    static sal_mac_addr_t arg_macaddr, maczero, mac;
...
if_robo_l2(int unit, args_t *a)
{
    ...
    if (!sal_strcasecmp(subcmd, "replace")) {
        ...
        if (arg_stonly || arg_mcast || arg_mconly){
            printk(" Message: STOnly|MCast|MCOnly are not available!\n");
        }
#endif  /* BCM_TB_SUPPORT */
        ...
        if (!sal_strcasecmp(learncmd, "control")){
            ...
            if (!parseEndOk( a, &pt, &ret_code)) {
                printk("PortBitMap must be assigned!\n");
                return ret_code;
            }
            pbmp = arg_pbmp;
        ...
        if(soc->arl_table){
            soc_cm_debug(DK_ARL,"showing SW ARL table.. \n");
            ...
            soc_cm_debug(DK_ARL,"showing HW ARL table..\n");
        ...
        if(soc->arl_table){
            ...
            index_max = SOC_MEM_BASE(unit, L2_ARLm) + 
                    SOC_MEM_SIZE(unit, L2_ARLm);
