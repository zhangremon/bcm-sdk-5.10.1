...
if_robo_mirror(int unit, args_t *a)
{
    ...
    if ((c = ARG_CUR(a)) != NULL) {
        if (sal_strcasecmp(c, "mtp") == 0) {
            ...
            if (sal_strcasecmp(subcmd, "dump_sw") == 0) {
                ...
                if (rv < 0) {
                    printk("%s: bcm_mirror_to_pbmp_get failed: %s\n", ARG_CMD(a),
                	   bcm_errmsg(rv));
                    return CMD_FAIL;
                }

                format_pbmp(unit, pbmp_str, sizeof(pbmp_str), arg_mport_bmp);
                printk("MPortBitMap (SW) = %s ==> %s\n", 
                ...
                if (rv < 0) {
                    printk("%s: DRV_MIRROR_GET failed: %s\n", ARG_CMD(a),
                	   bcm_errmsg(rv));
                    return CMD_FAIL;
                }

                format_pbmp(unit, pbmp_str, sizeof(pbmp_str), arg_mport_bmp);
                printk("MPortBitMap (HW) = %s ==> %s\n", 
                    SOC_PBMP_FMT(arg_mport_bmp, pfmt), pbmp_str);
    ...
    if (rv < 0) {
        printk("%s: bcm_mirror_to_pbmp_set failed: %s\n", ARG_CMD(a),
           bcm_errmsg(rv));
...
_robo_dmirror_show(int unit, int port, int dstmod, int dstport, uint32 flags, int mymodid)
{
    ...
    if (show_dest) {
        if (flags & BCM_MIRROR_PORT_DEST_TRUNK) {
            ...
            if (count >= 2) {
                printk(" to local port bitmap %s ==> %s", 
...
if_robo_dmirror(int unit, args_t *a)
{
    ...
    if (rv < 0) {
        printk("%s: bcm_mirror_to_pbmp_set: %s\n", ARG_CMD(a),
               bcm_errmsg(rv));
        return CMD_FAIL;
    }
