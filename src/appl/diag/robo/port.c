/*
 * $Id: port.c 1.59.2.4 Broadcom SDK $
        ...
        if (operation & SHOW) {
                ...
                if (rv < 0) {
                    ...
                    if (rate) { 
                        ...
                        if (SOC_IS_TBX(unit)) {
                            ...
                            printk("\tEgress meter:  %8d %s %8d kbits max burst.\n", 
                ...
                if (queue_n >= 0) {
                    ...
                    for (i = 0; i < current_q; i++) {
                        ...
                        if (rate) { 
                            if (SOC_IS_TBX(unit)) {
#ifdef BCM_TB_SUPPORT
                                printk("\tQueue #%d Egress meter:  %8d %s %8d kbits max burst.\n",
            ...
            if (operation & INGRESS) {
                ...
                if (SOC_IS_TBX(unit)) {
                    ...
                    if (rv < 0) {
                        printk("%s: Error: bcm_cosq_control_set() failed: %s\n",
                           ARG_CMD(a), bcm_errmsg(rv));
                        return CMD_OK;
                    }
#endif
               ...
               uint32 flags)
{
    ...
    if (flags & BCM_PORT_ATTR_FRAME_MAX_MASK) {
        printk("MaxFrmSz(%d) ", info->frame_max);
    }
    ...
    PBMP_ITER(pbm, p) {
        ...
        if (given.frame_max) {
            ...
            if ((r = bcm_port_frame_max_set(u, p, given.frame_max)) < 0) {
                printk("%s: Error: Could not set port %s max frame size: %s\n",
                       ARG_CMD(a), SOC_PORT_NAME(u, p), bcm_errmsg(r));
                cmd_rv = CMD_FAIL;
                continue;
            }
...
cmd_robo_cablediag(int unit, args_t *a)
{
    ...
    PBMP_ITER(pbm, port) {
        if (SOC_IS_TB_B0(unit) && IS_FE_PORT(unit, port)) {
            printk("Waiting 6 seconds for phy status update.\n");
            sal_usleep(6000000);
        }
    ...
    if (sal_strcasecmp(subcmd, "set") == 0) {
        ...
        if (prio < 0) {
            ...
            if (cfi < 0) {
                ...
                printk("%s: ERROR: Either configure cfi or prio value.\
Or use the 'color map ' cmd to configure both\n",
                       ARG_CMD(a));
                return CMD_FAIL;
            }
            ...
            for (prio = prio_min; prio <= prio_max; prio++) {
                for (color = color_min; 
                    ...
                    if (pkt_dscp & BCM_DSCP_ECN) {
                        ...
                        printk("Internal Prio=%d, Color=%s, Packet dscp=%d ecn=%d\n ",
                            prio, diag_robo_parse_color[color], dscp, ecn);

                    } else {
                        printk("Internal Prio=%d, Color=%s, Packet dscp=%d \n ",
 ...
 bcm_err:
    ...
    if ((s = ARG_GET(a)) == NULL) {
        if (srccp < 0) {
            ...
            for (i = 0; i < count; i++) {
                ...
                if (SOC_IS_TBX(unit) && !SOC_IS_TB_AX(unit)) {
                printk(" pkt dscp %d  ecn %d prio=%d cng=%s\n",
                    ...
                    printk(" pkt dscp %d prio=%d cng=%s\n",
                       srccp + i, 
                           prio & BCM_PRIO_MASK,
                           (prio & BCM_PRIO_GREEN) ? diag_robo_parse_color[0] :
                           (prio & BCM_PRIO_YELLOW) ?diag_robo_parse_color[1]   :
    ...
    if (sal_strcasecmp(subcmd, "mode") == 0) {     
        ...
        if (BCM_FAILURE(rv)) {
            ...
            if (rv < 0) {
                printk("fail to get dscp mode!\n");
                return CMD_FAIL;
            }
            if (map == BCM_PORT_DSCP_MAP_ALL) {
                printk("DSCP MAP ALL mode \n");
            } else {
                printk("DSCP MAP Disabled !\n");
            }
        }
        return CMD_OK;
    }
    ...
    if (c == NULL) {
        ...
        PBMP_ITER(pbm, p){
            ...
            for (i = 0; pcontrol_names[i].name != NULL; i++) {
                ...
                printk("\t %-45s= ", pcontrol_names[i].name);
                if (rv){
                    printk("%s\n", bcm_errmsg(rv));
                } else {
                    printk("0x%x\n", val);
        ...
        PBMP_ITER(pbm, p){
            ...
            for (i = 0; pcontrol_names[i].name != NULL; i++) {
                if (sal_strcasecmp(c, pcontrol_names[i].name) == 0){
                    ...
                    if (val_str != NULL){   
                        ...
                        if (rv) {
                            printk("\t Failed on setting type=%s(%d),%s\n", 
                                    pcontrol_names[i].name, 
                                    pcontrol_names[i].type, 
                                    bcm_errmsg(rv));
                        } else {
                            printk("\t %-45s(%d) set value=%d\n", 
                                    pcontrol_names[i].name, 
                                    pcontrol_names[i].type, 
                                    val);
                        }
            ...
            if (pcontrol_names[i].name == NULL){
                printk("\t Unknown Port Control type at %s!\n", c);
                return CMD_FAIL;
            }
        }
    }
...
if_robo_port_cross_connect(int unit, args_t *a)
{
    ...
    if (parse_port(unit, c, &p) < 0) {
        printk("%s: ERROR: unrecognized port number: %s\n",
                ARG_CMD(a), c);
        return CMD_FAIL;
    }
    ...
    if (c == NULL) {
        ...
        if (rv) {
            ...
            if (val) {
                printk("Enabled, Egress port is %s\n", SOC_PORT_NAME(unit, egr_p));
            } else {
                printk("Disabled.\n");
            }
        }
        ...
        if (!sal_strcasecmp(c, "disable")) {
            ...
            if (parse_port(unit, c, &egr_p) < 0) {
                printk("%s: ERROR: unrecognized egress port number: %s\n",
                        ARG_CMD(a), c);
                return CMD_FAIL;
            }
            val = 0;
