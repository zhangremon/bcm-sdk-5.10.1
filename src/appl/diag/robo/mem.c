/*
 * $Id: mem.c 1.28.6.1 Broadcom SDK $
    ...
    if ((table_name == DRV_MEM_TCAM_DATA) ||
        ...
        switch(mem_field) {
            ...
            case ISO_MAPf:
                ...
                break;
            default:
                if ((mem_field >= MSP_TREE_PORT0f) && \
                           (mem_field<=MSP_TREE_PORT26f)) {
        ...
        if (mem_mapping) {
            if (table_name == DRV_MEM_ARL) {
                if (!sal_strcasecmp(fmod, "macaddr") ||
                    ...
                    printk("MAC or VID in ARL memory are not allowed to be modified\n");
                    goto error_return;
                }
            }            
...
cmd_robo_mem_write(int unit, args_t *a)
{
    ...
    if ( !sal_strcasecmp(tab, "int8051ram")) {
        ...
        soc_cm_print("Write the 8051 image file, 8051gm.bin, to 8051\n");
