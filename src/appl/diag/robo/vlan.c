/*
 * $Id: vlan.c 1.32.2.1 Broadcom SDK $
...
_robo_vlan_action_get(const char *action_name)
{
    ...
    printk("Invalid action <%s>. Valid actions are:\n   ", action_name);
    for (i = 0; i < names; i++) {
        printk("%s ", robo_vlan_action_names[i].name);
    }
    printk("\n");
    return -1;
}
...
_robo_pcp_action_get(const char *action_name)
{
    ...
    printk("Invalid pcp action <%s>. Valid pcp actions are:\n   ", action_name);
    for (i = 0; i < names; i++) {
        printk("%s ", robo_vlan_pcp_action_names[i].name);
    }
    printk("\n");
    return -1;
}
...
STATIC INLINE char *
_robo_vlan_pcp_action_name_get(bcm_vlan_action_t action)
{
...
_robo_vlan_action_set_print(const bcm_vlan_action_set_t *action)
{
    ...
    printk("  DT: Outer=%-7s  OuterPrio=%-7s  Inner=%-7s  InnerPrio=%-7s\n",
    ...
    printk("  OT: Outer=%-7s  OuterPrio=%-7s  Inner=%-7s\n",
    ...
    printk("  IT: Outer=%-7s  Inner=%-7s      InnerPrio=%-7s\n",
...
_robo_vlan_action_translate_key_get(const char *key_str)
{
    ...
    printk("Invalid key type <%s>. Valid key types are:\n   ", 
           key_str ? key_str : "");
    for (i = 1;  i < names; i++) {
        printk("%s ", robo_vlan_key_type_names[i].name);
        if (i % 7 == 0) {
            printk("\n   ");
        }
    }
                             ...
                             void *user_data)
{
    ...
    if (key_type >= bcmVlanTranslateKeyPortDouble) {
        ...
        if (BCM_GPORT_IS_TRUNK(gport)) {
            ...
            printk("Port=%d, Modid=%d, ", port, modid);
        }
    }
    printk("KeyType=%s, ",_robo_vlan_action_translate_key_name_get(key_type));
    if (outer_vlan == BCM_VLAN_INVALID) {
        printk("OldOuterVlan=--, ");
    } else {
        printk("OldOuterVlan=%d, ", outer_vlan);
    }
    if (inner_vlan == BCM_VLAN_INVALID) {
        printk("OldInnerVlan=--\n");
    } else {
        printk("OldInnerVlan=%d\n", inner_vlan);
    }
                                    ...
                                    void *user_data) 
{
    ...
    printk("PortClass=%d, OldOuterVlan=%d, OldInnerVlan=%d\n",
                                   ...
                                   void *user_data)
{
    ...
    printk("OuterVlanLo=%d, OuterVlanHi=%d, InnerVlanLo=%d, "
        ...
        if (sal_strcasecmp(subcmd, "delete") == 0) {
            ...
            ROBO_VLAN_IF_ERROR_RETURN(
                bcm_vlan_translate_action_range_get(unit, gport,
                                                    ...
                                                    inner_vlan_hi,
                                                    &action));
            printk("Port=%d, OuterVlanLo=%d, OuterVlanHi=%d, InnerVlanLo=%d, "
        ...
        if (sal_strcasecmp(subcmd, "delete") == 0) {
            ...
            ROBO_VLAN_IF_ERROR_RETURN(
                bcm_vlan_translate_action_get(unit, gport, key_type, 
                                              ...
                                              old_inner_vlan,
                                              &action));
            printk("Port=%d, KeyType=%s, OldOuterVlan=%d, OldInnerVlan=%d\n",
            ...
            PBMP_ITER(pbmp, port) {
                ...
                ROBO_VLAN_IF_ERROR_RETURN(
                    bcm_vlan_port_protocol_action_get(unit, port,
                                                  ...
                                                  ether,
                                                  &action));
                printk("Port=%d, Frame=0x%x, EtherType=0x%x\n",
                       port, frame, ether);
                _robo_vlan_action_set_print(&action);
            }
            return CMD_OK;
                               ...
                               bcm_port_t port_1, bcm_port_t port_2, void *user_data)
{
    ...
    printk("u = %d, outer vid = %d, inner vid = %d, port1 = %d, port2 = %d \n",
                      ...
                      bcm_vlan_t new_vid, int prio, void *user_data)
{
    ...
    if (BCM_FAILURE(rv)) {
        printk("ERROR: Invalid gport =0x%x\n", gport);
        return CMD_FAIL;
    }
    if (BCM_GPORT_IS_TRUNK(gport)) {
        printk("u = %d, tgid = %d, old_vid = %d, new_vid = %d, prio = %d \n",
               unit, trunk, old_vid, new_vid, prio);
    } else {
        printk("u = %d, modid = %d port = %d, old_vid = %d, new_vid = %d, prio = %d \n",
                            ...
                            int prio, void *user_data)
{
    ...
    if (BCM_GPORT_IS_TRUNK(gport)) {
        printk("u = %d, tgid = %d, old_vid_lo = %d, old_vid_hi = %d, new_vid = %d, prio = %d \n",
               unit, trunk, old_vid_lo, old_vid_hi, new_vid, prio);
    } else {
        printk("u = %d, modid = %d port = %d, old_vid_lo = %d, old_vid_hi = %d, new_vid = %d, prio = %d \n",
...
_robo_vlan_action_port(int unit, args_t *a)
{
    ...
    if (sal_strcasecmp(subcmd, "default") == 0) {
        ...
        if (sal_strcasecmp(subcmd, "add") == 0) {
            ...
            printk("Port %d: New OVLAN=%d, New IVLAN=%d, New Prio=%d\n",
...
_robo_vlan_policer(int unit, args_t *a)
{
    ...
    if (sal_strcasecmp(subcmd, "create") == 0) {
        ...
        if ((rv = bcm_vlan_port_policer_set(unit, vlan, port, polid)) != BCM_E_NONE) {
            printk("ERROR: bcm_vlan_port_policer_set(unit=%d, vlan=%d, port=%d,\
...
if_robo_vlan(int unit, args_t *a)
{
    ...
    if (sal_strcasecmp(subcmd, "test") == 0) {
        ...
        printk(" data 0 :\n");
        for(i=0;i<8;i++){
            printk("0x%02x ",*((uint8 *)&test_data0+i));
        }
        printk("\n data 1 :\n");
        for(i=0;i<8;i++){
            printk("0x%02x ",*((uint8 *)&test_data1+i));
        }
    ...
    if (sal_strcasecmp(subcmd, "clear") == 0) {
           ...
           parse_table_add(&pt, "PortBitMap", PQ_PBMP|PQ_BCM, 0, &pbmp, NULL);
           if (!parseEndOk( a, &pt, &ret_code)) {
               printk("Usages: vlan protocol clear PortBitMap=<pbmp>\n");
               return ret_code;
           }
    ...
    if (sal_strcasecmp(subcmd, "translate") == 0) {
        ...
        if (sal_strcasecmp(subcmd, "dtag") == 0) {
            ...
            if (sal_strcasecmp(subsubcmd, "range") == 0) {
                ...
                if (subsubsubcmd == NULL) {
                    printk("%s: ERROR: Missing translate dtag range subcommand\n",
                   ARG_CMD(a));
                    return CMD_FAIL;
                }
        ...
        if (sal_strcasecmp(subcmd, "range") == 0) {
            char * subsubcmd = ARG_GET(a);
            if (subsubcmd == NULL) {
                printk("%s: ERROR: Missing translate range subcommand\n",
               ARG_CMD(a));
                return CMD_FAIL;
            }
    ...
    if (sal_strcasecmp(subcmd, "crossconnect") == 0) {
        ...
        if (subcmd == NULL) {
            printk("%s: ERROR: Missing 'vlan crossconnect' subcommand\n",
            ARG_CMD(a));
            return CMD_FAIL;
        }
    ...
    if (sal_strcasecmp(subcmd, "block") == 0) {
        ...
        if (subcmd == NULL) {
            printk("%s: ERROR: Missing 'vlan block' subcommand\n",
            ARG_CMD(a));
            return CMD_FAIL;
        }
