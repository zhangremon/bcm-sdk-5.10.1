/*
 * $Id: dump.c 1.26.2.1 Broadcom SDK $
            ...
            while (an_arg) {
                if (sal_strcasecmp(an_arg, "addr") == 0) {
                    ...
                    if (count != NULL) {
                    	...
                    	printk("SOC_ROBO_MAX_NUM_PORTS %d\n",SOC_ROBO_MAX_NUM_PORTS);
                        if (dump_port >= SOC_ROBO_MAX_NUM_PORTS) {
                        	printk("ERROR: Exceed max port number.\n");
                        	return CMD_FAIL;
                        }
                  	} else {
                  	    printk("ERROR: port number needed.\n");
                  	    return CMD_FAIL;
                  	}
                } else {
                        ...
                        int index, int count, int flags)
{
       ...
       case CFP_TCAM_SCm:
        ...
        case CFP_TCAM_NONIP_SCm: 
    		...
    		mask_ptr= cfp_entry.tcam_mask;
              break;
		default:
		    printk("Unsupport CFP memory table.\n");\
		    return CMD_FAIL;
    }
    ...
    for (idx = index; idx < index + count; idx++) {
        ...
        if (flags & DUMP_TABLE_HEX) {
    		...
    		printk("%s[%d]: ", SOC_ROBO_MEM_UFNAME(unit, mem), idx);
            if (flags & DUMP_TABLE_RAW) {
                for (j = 0; j < entry_len; j++) {
                    printk("0x%08x ", data_ptr[j]);
                }
                if (tbl_name == DRV_CFP_RAM_TCAM) {
                    if (mask_ptr != NULL ) {
                        printk("\n");
                        printk("TCAM MASK[%d]: ", idx);
                        for (j = 0; j < entry_len; j++) {
                            printk("0x%08x ", mask_ptr[j]);
                        }
                    }
                }
            } else {
                printk("ERROR: all not support yet.\n");
            }
          ...
          int copyno, int index, int count, int flags)
{
    ...
    if (soc_feature(unit, soc_feature_field)) {
        if ((mem == CFP_ACT_POLm) || 
            (mem == CFP_METERm) || (mem == CFP_STAT_IBm) ||
            (mem == CFP_STAT_OBm) || (mem == CFP_TCAM_S0m) ||
            (mem == CFP_TCAM_S1m) || (mem == CFP_TCAM_S2m) ||
            (mem == CFP_TCAM_S3m) ||(mem == CFP_TCAM_S4m) ||
            (mem == CFP_TCAM_IPV4_SCm) ||(mem == CFP_TCAM_IPV4_MASKm) ||
            (mem == CFP_TCAM_IPV6_SCm) ||(mem == CFP_TCAM_IPV6_MASKm) ||
            (mem == CFP_TCAM_NONIP_SCm) ||(mem == CFP_TCAM_NONIP_MASKm) ||
            (mem == CFP_TCAM_CHAIN_SCm) ||(mem == CFP_TCAM_CHAIN_MASKm) ||
            (mem == CFP_TCAM_SCm)
            ) {
    ...
    if(mem_mapping) {
        switch(mem) {
            ...
            case VLAN_1Qm:
                ...
                if (dt_mode){
                    printk("\n Double tagging mode! ") ;
                    printk("Check 'VLAN show' for the real untagbitmap.\n\n");
                }
        		break;
            case MAC2VLANm:
    ...
    for (k = index; k < index + count; k++) {
        ...
        if (flags & DUMP_TABLE_HEX) {
    		...
    		printk("%s[%d]: ", SOC_ROBO_MEM_UFNAME(unit, mem), k);
            if (flags & DUMP_TABLE_RAW) {
                for (i = 0; i < entry_dw; i++) {
...
cmd_robo_dump(int unit, args_t *a)
{
	...
	soc_cm_print("Expect %x for register reg [0x%x 0x%x]\n", expect, page, addr);
	...
	do {
		...
		wrong = 1;
	        soc_cm_print("Found wrong register access: expect 0x%x,"
		       "got 0x%x after %d times\n",
		       expect, COMPILER_64_LO(regdata), loop);
	        break;
	    }
	    soc_cm_debug(DK_PCI, "dump reg loops [0x%x 0x%x] got 0x%x for %d times\n",
	            page, addr, expect, ++loop);
	} while ((loop < tloop) || !tloop);

	if (!wrong)
	    soc_cm_print("phydbg is successful for %d times\n", loop);
    ...
    if (parse_memory_name(unit, &mem, arg1, &copyno) >= 0) {
        ...
        if (mem > NUM_SOC_ROBO_MEM){
            debugk(DK_ERR, "Error: Memory %d is out of range for chip %s.\n",
            		(int)mem, SOC_UNIT_NAME(unit));
            goto done;
        }
        if (!SOC_MEM_IS_VALID(unit, mem)) {
            debugk(DK_ERR, "Error: Memory %d not valid for chip %s.\n",
            		(int)mem, SOC_UNIT_NAME(unit));
            goto done;
        }
