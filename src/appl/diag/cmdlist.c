/*
 * $Id: cmdlist.c 1.241.2.13 Broadcom SDK $
...
#ifdef BCM_KATANA_SUPPORT
    {"FLEX",            cmd_esw_flex,           cmd_esw_flex_usage,     "flex counters related commands " },
#endif
...
#endif
#ifdef INCLUDE_REGEX
    {"regex",        cmd_regex,           cmd_regex_usage,     "Load Regex patterns" },
#endif
};
...
cmdlist_init(void)
{
    ...
    for (i = 1; i < bcm_ea_cmd_cnt; i++) {
        ...
        bcm_ea_cmd_list[i - 1].c_cmd) <= 0) {
            printk("WARNING: bcm ethernet access command %s not alphabetized\n",
