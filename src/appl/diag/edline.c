/* $Id: edline.c 1.12.202.1 Broadcom SDK $
 *  ed - standard editor
 *  ^^
...
static	int doread(int lin, char *fname)
{
	...
	for(lines = 0, bytes = 0;(err = egets(str,MAXLINE,fp)) > 0;) {
		bytes += strlen(str);
		if(ins(str) < 0) {
			printk("file insert error\n");
			err++;
			break;
		}
		lines++;
	}
	printk("\"%s\" %u lines %lu bytes\n", fname, lines, bytes);
	sal_fclose(fp);
	if(err < 0)
		return(err);
	return( err );
...
static	int dowrite(int from, int to, char *fname, int apflg)
{
	...
	if((fp = sal_fopen(fname,(apflg?"a":"w"))) == NULL) {
		printk("%s can't be opened for writing!", fname);
		return( ERR );
	}
	...
	while (*arg && *arg != delim && *arg != '\n' && !error) {
		...
		case CCL:
			...
			if(( ntok->bitmap = makebitmap(CLS_SIZE) ) != 0)
				...
				printk("Not enough memory for pat\n");
				error = 1;
			}
			break;
...
{
	volatile int stat = ERR, pushed = 0, rv = 0;
	jmp_buf ctrl_c;
	char inlin[MAXLINE];
	...
	for (;;) {
		...
		case FATAL:
			...
			goto done;
		case CHANGED:
			printk("buffer modified (use q again to quit)\n");
			fchanged = FALSE;
			break;
		case SET_FAIL:
			printk("`set' command failed\n");
			break;
		case SUB_FAIL:
			printk("string substitution failed\n");
			break;
		default:
			printk("?\n");
		}
