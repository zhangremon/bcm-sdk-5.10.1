...

{
  ...
  if (p_cb_params == NULL) {
    BCM_ERR(("Callback params pointer is null\n"));
    return (-1);
  }
  ...
  if (p_cb_params->cmd == BCM_SBX_COSQ_GPORT_REMOVE_ALL) {
	...
	rv = bcm_cosq_gport_delete(unit,gport);
	if (rv != BCM_E_NONE) {
	  BCM_ERR(("remove gport(%d) with port=%d failed\n", gport,_port));
	  retval = -1;
	}
	...
	rv = bcm_cosq_gport_delete(unit,gport);
	if (rv != BCM_E_NONE) {
	  BCM_ERR(("remove gport(%d) with modid=%d failed\n", gport,_modid));
	  retval = -1;
	}
  ...
  if (BCM_GPORT_IS_MODPORT(gport)) {
    ...
    *port  = BCM_GPORT_EGRESS_MODPORT_PORT_GET(gport);
    ...
    if ((p_eg == NULL) || (eg_n < 0) || (eg_n >= SOC_SBX_CFG(unit)->num_egress_group)) {
      ...
      printk("Egress Group (0x%08x) is invalid\n",gport);
      *port = -1;
    } else {
...
#define QUEUE_LEVEL 0
  ...
  printk("L%d: gport=0x%08x ",level+1,parent);
  ...
  printk("   L%d: gport=0x%08x",level,sch[0]);
  printk("\t\t\t\t %-10s %-14s %07d \t %07d\n",sbx_get_gport_type_string(sch[0]),
  ...
  if (children[0] > 0) {
    for (child[0]=0; child[0] < children[0]; child[0]++) {
      ...
      printk("      L%d: gport=0x%08x",level-1,child_gport);
	...
	for (child[1] =0; child[1] < children[1]; child[1]++) {
	  ...
	  if (rv == BCM_E_NOT_FOUND) continue;
	  printk("         L%d: gport=0x%08x",level-2,child_gport);	
	  ...
	  if (children[2] > 0) {
	    for(child[2]=0; child[2] < children[2]; child[2]++) {
	      ...
	      if (rv == BCM_E_NOT_FOUND) continue;
	      printk("            L%d: gport=0x%08x",level-3,child_gport);	
		...
		for(child[3]=0;child[3] < ((level-4==QUEUE_LEVEL)?1:children[3]); child[3]++) {
		  ...
		  if (rv == BCM_E_NOT_FOUND) continue;
		  printk("               L%d: gport=0x%08x",level-4,child_gport);	
		...
		if (children[4] > 0) {
		  for(child[4]=0;child[4] < ((level-5==QUEUE_LEVEL)?1:children[4]) ; child[4]++) {
		    ...
		    if (rv == BCM_E_NOT_FOUND) continue;
		    printk("                  L%d: gport=0x%08x",level-5,child_gport);	
		  ...
		  if (children[5] > 0) {
		    for (child[5]=0;child[5] < 1; child[5]++) {
		      ...
		      if (rv == BCM_E_NOT_FOUND) continue;
		      printk("                     L%d: gport=0x%08x",level-6,child_gport);
		      printk("\t %-10s \n",sbx_get_gport_type_string(child_gport));		    
		    }
		  }
		}
...
sbx_gport_display(int unit, bcm_gport_t gport)
{
  ...
  if (BCM_GPORT_IS_SCHEDULER(gport)) {
    ...
    if (p_i_scheduler == NULL ) {
      printk("No Ingress scheduler state found.\n");
      return;
    }
    ...
    for (i=0;i<SOC_SBX_CFG(unit)->num_ingress_scheduler; i++) {
      ...
      if (scheduler == gport) {
	bcm_sbx_cosq_gport_attach_get(unit,scheduler,&parent,&cosq);
	printk(" gport \t\t type \t\tmode         node    level    min_rate(kbps)  max_rate(kbps)  parent \t type \n");
	printk(" 0x%08x \t %-10s %-14s   %04d    %04d\t%07d \t %07d      0x%08x %-10s \n",
	       gport,
    ...
    printk(" gport \t\t type \t\t     mod   port\n");
    sbx_cosq_gport_get_node_port(unit,gport,&modid,&port);
    printk(" 0x%08x \t %-12s        %04d  %04d\n",
	   gport,
	   sbx_get_gport_type_string(gport),
	   modid,
	   port);
...
sbx_gport_scheduler_show(int unit)
{
  ...
  if (num_ingress_schedulers == 0 ) {
    printk("No ingress schedulers found\n");
    return;
  }
  printk("\t\t\t\t\t\t Type \t    Mode         min_rate(kbps)  max_rate(kbps)\n");
  for (i=0;i<num_ingress_schedulers; i++) {
    ...
    if ((0 > node) || (0 > level)) {
        ...
        printk("skip - level = %d, node = %d, invalid\n", level, node);
        continue;
    }
    t_i_scheduler = &is_state[unit][level][node];
    if (t_i_scheduler == NULL ) {
        printk("skip scheduler is NULL\n");
        continue;
    }
...
sbx_gport_map_show(int unit) 
{
    ...
    int fabric_node = 0;
    int fabric_port = 0;
    printk(" Switch Gport     Type          Fabric Gport     Type  \t\t\t        Switch (Node|Port)  Fabric (Node|Port) \n");
    printk(" --------------------------------------------------------------------------------------------------------------------\n");
	...
	for(entry=0;entry<2*_BCM_SBX_STACK_PORTMAP_CHUCK_SIZE;entry+=2) {
	  ...
	  printk("  0x%08x \t %-s \t 0x%08x \t %-32s  (%05d|%04d)\t      (%05d|%04d)\n",
...
sbx_gport_map_delete(int unit) 
{
	...
	for(entry=0;entry<2*_BCM_SBX_STACK_PORTMAP_CHUCK_SIZE;entry+=2) {
	  ...
	  if (rv != BCM_E_NONE) {
	    BCM_ERR(("bcm_sbx_stk_fabric_map_set failed for switch_port(0x%08x)\n",map->portmap[entry],bcm_errmsg(rv)));
	  }
	}
      }
    }
		    ...
		    int verbose)
{
  ...
  if (strstr(type,"Invalid") != 0) {
    BCM_ERR(("ERROR: gport(0x%x) is invalid\n",gport));
    return (BCM_E_PARAM);
  }
  ...
  printk("  0x%08x   %2d     %32s    %5d      %2d %12d\n",gport, num_cos_levels, type, modid, port, base_queue);
  /* get mode and weighted value for each cos level */
  if (verbose == 1) {
      printk("               cos       mode           weight    kbps_min   kbps_max  min_size  max_size  flags\n");
...

{
  ...
  if (p_cb_params->cmd == BCM_SBX_COSQ_GPORT_SHOW_ALL) {
	...
	rv = sbx_cosq_gport_show(unit,gport,p_cb_params->verbose);
	if (rv != BCM_E_NONE) {
	  BCM_ERR(("show gport(%d) with port=%d failed\n", gport,_port));
	  retval = -1;
	}
	...
	rv = sbx_cosq_gport_show(unit,gport,p_cb_params->verbose);
	if (rv != BCM_E_NONE) {
	  BCM_ERR(("show gport(%d) with modid=%d failed\n", gport,_modid));
	  retval = -1;
	}
    ...
    if (level > (7 - levels)) {
        ...
        if (children) {
            if (7 == level) {
                ...
                if (SOC_E_NONE == result) {
                    printk("%1X.%04X %s %1X.%04X %1X.%04X+%04X %04X %s %04X\n",
                           level,
                           node,
                           prefix,
                           level + 1,
                ...
                printk("%1X.%04X %s %1X.%04X %1X.%04X+%04X\n",
                       level,
                       node,
                       prefix,
                       level + 1,
...
sbx_sirius_ingress_tree_show(int unit, int levels)
{
    ...
    if (6 < levels) {
        printk("Node             Parent Child  Cnt  QID   M Leaf\n");
        printk("------ --------- ------ ------ ---- ----- - ----\n");
    } else {
        printk("Node             Parent Child  Cnt\n");
        printk("------ --------- ------ ------ ----\n");
    }
    for (current = 0;
         (SOC_E_NONE == result) &&
