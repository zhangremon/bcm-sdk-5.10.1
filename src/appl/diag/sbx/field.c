...
static const char *ing_span_text[] = {"DISabled",
                                      "BLocK",
                                      "LeaRN",
                                      "ForWarD",
                                      NULL};
...
static const char *color_text[] = {"green",
                                   ...
                                   "red",
                                   NULL};
static const char *l2_text[]   = {"Ethernet2",
                                  "Snap",
                                  "LLC",
                                  "Other",
                                  NULL};
static const char *vlan_text[] = {"NOtag",
                                  "ONEtag",
                                  "TWOtag",
                                  NULL};
static const char *ip_text[]   = {"IP4HdrOnly",
                                  "IP6HdrOnly",
                                  "IP6Fragment",
                                  "IP4OverIP4",
                                  "IP6OverIP4",
...
static const char *data_mpls_text[] = {"Any",
                                       ...
                                       "TwoLabels",
                                       NULL};
static const char *true_false[] = {"True",
                                   "False",
                                   NULL};
 ...
 */
static void
fp_qprint_options(const char *options[], const char* prefix)
{
    int offset;
    offset = printk("%s", prefix);
    fp_print_options(options, offset);
    printk("\n");
}
...
fp_qual_LookupStatus_help(const char *prefix, int width_col1) {
    ...
    offset = printk("%-*s%s", width_col1, "", "IngSpan=");
    fp_print_options(ing_span_text, offset);
    printk("\n");
...
fp_list_udf_flags(int unit)
{
    ...
    fp_qprint_options(l2_text, "        L2=");
    fp_qprint_options(vlan_text, "        VLan=");
    fp_qprint_options(ip_text, "        IP=");
    fp_qprint_options(true_false, "        AdjustIpOptions=");
    fp_qprint_options(true_false, "        Higig=");
    fp_qprint_options(true_false, "        Higig2=");
    return CMD_OK;
}

/*
 *  Execute UDF related qualifier stuff
 */
static int
fp_qual_udf(int unit,
            ...
            char *qual_str)
{
    ...
    FP_VRFY_GET_NUMB(udf_id,     subcmd, args);
    FP_VRFY_GET_NUMB(data32,     subcmd, args);
    FP_VRFY_GET_NUMB(mask32,     subcmd, args);
...
/*
 *  Qualify based upon module ID
 */
static int
fp_qual_module(int unit,
               bcm_field_entry_t eid,
               args_t *args,
...
/*
 *  Qualify based upon LookupStatus information
 */
static int
...
fp_qual(int unit, args_t *args)
{
    ...
    case bcmFieldQualifySrcModid:
        rv = fp_qual_module(unit, eid, args, bcm_field_qualify_SrcModid, "SrcModid");
        break;
    case bcmFieldQualifySrcPortTgid:
        rv = fp_qual_port(unit, eid, args, bcm_field_qualify_SrcPortTgid, "SrcPortTgid");
        break;
    case bcmFieldQualifySrcPort:
    ...
    case bcmFieldQualifyDstModid:
        rv = fp_qual_module(unit, eid, args, bcm_field_qualify_DstModid, "DstModid");
        break;
    case bcmFieldQualifyDstPortTgid:
        rv = fp_qual_port(unit, eid, args, bcm_field_qualify_DstPortTgid, "DstPortTgid");
        break;
    case bcmFieldQualifyDstPort:
    ...
    case bcmFieldQualifyPacketFormat:
        rv = fp_qual_32(unit, eid, args, bcm_field_qualify_PacketFormat, "PacketFormat");
        break;
    ...
    while (NULL != qual_str) {
        ...
        if (isint(qual_str)) {
            ...
            if (qual >= bcmFieldQualifyCount) {
                FP_ERR(("FP(unit %d) Error: Unknown qualifier: %d\n", unit, qual));
                return CMD_FAIL;
            }
        } else {
...
fp_qset(int unit, args_t *args, bcm_field_qset_t *qset)
{
    ...
    if (CMD_USAGE == result) {
        printk(if_field_proc_qset_usage);
        result = CMD_NFND;
    }
    return result;
}
...
fp_action(int unit, args_t *args)
{
    ...
    if (CMD_USAGE == result) {
        printk(if_field_proc_action_usage);
        result = CMD_NFND;
    }
    return result;
}
...
fp_counter_share(int unit, args_t *args)
{
    ...
    FP_VRFY_GET_NUMB(dst_eid, subcmd, args);
...
fp_counter(int unit, args_t *args)
{
    ...
    if (CMD_USAGE == result) {
        printk(if_field_proc_counter_usage);
        result = CMD_NFND;
    }
    return result;
}
...
fp_entry_show(int unit, args_t *args)
{
    ...
    FP_VERB(("FP(unit %d) verb:bcm_field_entry_dump gid=%d\n", unit, gid));
...
fp_entry(int unit, args_t *args)
{
    ...
    if (CMD_USAGE == result) {
        printk(if_field_proc_entry_usage);
        result = CMD_NFND;
    }
    return result;
}
...
fp_group_status_get(int unit, args_t *args)
{
    ...
    printk("\tprio_min=%d,\n", gstat.prio_min);
    printk("\tprio_max=%d,\n", gstat.prio_max);
    printk("\tentries_total=%d,\n", gstat.entries_total);
    printk("\tentries_free=%d,\n", gstat.entries_free);
    printk("\tcounters_total=%d,\n", gstat.counters_total);
    printk("\tcounters_free=%d,\n", gstat.counters_free);
    printk("\tmeters_total=%d,\n", gstat.meters_total);
    printk("\tmeters_free=%d\n", gstat.meters_free);
    printk("}\n");
...
fp_group_install(int unit, args_t *args)
{
    ...
    FP_VERB(("FP(unit %d) verb:bcm_field_group_install gid=%d\n", unit, gid));
...
fp_group_remove(int unit, args_t *args)
{
    ...
    FP_VERB(("FP(unit %d) verb:bcm_field_group_remove gid=%d\n", unit, gid));
...
fp_group_show(int unit, args_t *args)
{
    ...
    FP_VERB(("FP(unit %d) verb:bcm_field_group_dump gid=%d\n", unit, gid));
...
fp_group(int unit, args_t *args, bcm_field_qset_t *qset)
{
    ...
    if (CMD_USAGE == result) {
        printk(if_field_proc_group_usage);
        result = CMD_NFND;
    }
    return result;
}
...
fp_range(int unit, args_t *args)
{
    ...
    if (CMD_USAGE == result) {
        printk(if_field_proc_range_usage);
        result = CMD_NFND;
    }
    return result;
}
...
/*
 *  Get committed rate from a meter
 */
static int
...
fp_meter_setc(int unit, args_t *args)
{
    ...
    FP_VRFY_GET_NUMB(eid, subcmd, args);
    FP_VRFY_GET_NUMB(rate, subcmd, args);
    FP_VRFY_GET_NUMB(burst, subcmd, args);
...
/*
 *  Share a meter from an entry with one to an entry without one
 */
static int
...
fp_meter(int unit, args_t *args)
{
    ...
    if ((subcmd = ARG_GET(args)) != NULL) {
        if(!sal_strcasecmp(subcmd, "create")) {
            ...
            result = fp_meter_create(unit, args);
        } else if(!sal_strcasecmp(subcmd, "destroy")) {
            /* BCM.0> fp meter destroy ... */
            result = fp_meter_destroy(unit, args);
        } else if(!sal_strcasecmp(subcmd, "getc")) {
            /* BCM.0> fp meter getc ... */
            result = fp_meter_getc(unit, args);
        } else if(!sal_strcasecmp(subcmd, "getp")) {
            /* BCM.0> fp meter getp ... */
            result = fp_meter_getp(unit, args);
        } else if(!sal_strcasecmp(subcmd, "setc")) {
            /* BCM.0> fp meter setc ... */
            result = fp_meter_setc(unit, args);
        } else if(!sal_strcasecmp(subcmd, "setp")) {
            /* BCM.0> fp meter setp ... */
            result = fp_meter_setp(unit, args);
        } else if(!sal_strcasecmp(subcmd, "share")) {
            /* BCM.0> fp meter share ... */
            result = fp_meter_share(unit, args);
        }
    }
    if (CMD_USAGE == result) {
        printk(if_field_proc_meter_usage);
        result = CMD_FAIL;
    }
    return result;
}
...
fp_policer(int unit, args_t *args)
{
    ...
    if (CMD_USAGE == result) {
        printk(if_field_proc_policer_usage);
        result = CMD_FAIL;
    }
    return result;
}
...
fp_stat(int unit, args_t *args)
{
    ...
    if (CMD_USAGE == result) {
        printk(if_field_proc_stat_usage);
        result = CMD_NFND;
    }
    return result;
}
...
fp_data(int unit, args_t *args)
{
    ...
    if (CMD_USAGE == result) {
        printk(if_field_proc_data_usage);
        result = CMD_NFND;
    }
    return result;
}
...
fp_user_get(int unit, args_t *args, bcm_field_udf_spec_t *udf_spec)
{
    ...
    if (NULL == (subcmd = ARG_GET(args))) {
        udf_id = Last_Created_UDF_Id;
    } else {
        udf_id = parse_integer(subcmd);
    }
...
fp_user_set(int unit, args_t *args, bcm_field_udf_spec_t *udf_spec)
{
    ...
    FP_VRFY_GET_NUMB(offset, subcmd, args);
...
fp_user(int unit, args_t *args, bcm_field_udf_spec_t *udf_spec)
{
    ...
    if ((subcmd = ARG_GET(args)) != NULL) {
        if(!sal_strcasecmp(subcmd, "get")) {
            /* BCM.0> fp user get ... */
            result = fp_user_get(unit, args, udf_spec);
        } else if(!sal_strcasecmp(subcmd, "init")) {
            /* BCM.0> fp user init ... */
            result = fp_user_init(unit, args, udf_spec);
        } else if(!sal_strcasecmp(subcmd, "set")) {
            /* BCM.0> fp user set ... */
            result = fp_user_set(unit, args, udf_spec);
        } else if(!sal_strcasecmp(subcmd, "create")) {
            /* BCM.0> fp user create ... */
            result = fp_user_create(unit, args, udf_spec);
        }
    }
    if (CMD_USAGE == result) {
        printk(if_field_proc_user_usage);
        result = CMD_NFND;
    }
    return result;
}
...
if_sbx_field_proc(int unit, args_t *args)
{
    ...
    if((!sal_strcasecmp(subcmd, "qual")) ||
       (!sal_strcasecmp(subcmd, "qualify"))) {
        ...
        if (CMD_USAGE == retval) {
            printk(if_field_proc_qual_usage);
            retval = CMD_NFND;
        }
        return retval;
    }
    ...
    if(!sal_strcasecmp(subcmd, "show")) {
        ...
        if (CMD_USAGE == retval) {
            printk(if_field_proc_show_usage);
            retval = CMD_NFND;
        }
        return retval;
    }
