/* $Id: l3.c 1.10.150.2 Broadcom SDK $
 * $Id: 
...
_cmd_sbx_l3_intf(int unit, args_t *args)
{
    ...
    if ((sal_strcasecmp(subcmd, "add") == 0) ||
        ...
        if (rv == BCM_E_NONE) {
            if (_l3_ifid)
                printk("Deleted if_id 0x%x, %s\n",
                       bcm_intf.l3a_intf_id, bcm_errmsg(rv));
            else
                printk("Deleted vid=%d mac=0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x %s\n",
                       bcm_intf.l3a_vid,
        ...
        if (rv == BCM_E_NONE) {
            printk("MacAddr=0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x "
...
_cmd_sbx_l3_egr(int unit, args_t *args)
{
    ...
    if ((sal_strcasecmp(subcmd, "add") == 0) ||
        ...
        if (rv == BCM_E_NONE) {
            printk("Created fte 0x%x ohi=0x%x\n", fte_idx, bcm_egr.encap_id);
        }
    } else if (sal_strcasecmp(subcmd, "get") == 0) {
        parse_table_add(&pt, "fte",    PQ_DFL|PQ_HEX, 0, &_l3_fte, NULL);
...
_cmd_sbx_l3_route(int unit, args_t *args)
{
    ...
    if (sal_strcasecmp(subcmd, "add") == 0) {
        ...
        printk("Route update returned: %s\n", bcm_errmsg(rv));

    } else if (sal_strcasecmp(subcmd, "get") == 0) {
        rv = bcm_l3_route_get(unit, &bcm_route);
        printk("Route get returned: %s\n", bcm_errmsg(rv));
...
_cmd_sbx_l3_mpath(int unit, args_t *args)
{
    ...
    if (sal_strcasecmp(subcmd, "create") == 0) {
        ...
        rv = bcm_l3_egress_multipath_create(unit,
                                            ...
                                            &mpbase);
        if (rv == BCM_E_NONE) {
            printk("Multipath add success: 0x%x\n", mpbase);
        }
        ...
        rv = bcm_l3_egress_multipath_add(unit,
                                         ...
                                         ftes[i]);
        if (rv == BCM_E_NONE) {
            printk("multipath add successfull\n");
        }
    } else if (sal_strcasecmp(subcmd, "delete") == 0) {
        /*
        ...
        rv = bcm_l3_egress_multipath_delete(unit,
                                            ...
                                            ftes[i]);
        if (rv == BCM_E_NONE) {
            printk("multipath delete successfull\n");
        }
        ...
        rv = bcm_l3_egress_multipath_destroy(unit,
                                             mpbase) ;
        if (rv == BCM_E_NONE) {
            printk("multipath destroy success\n");
        }
    } else if (sal_strcasecmp(subcmd, "get") == 0) {
        rv = bcm_l3_egress_multipath_get(unit,
                                         ...
                                         &count);
        if (rv == BCM_E_NONE) {
            printk("multipath get successfull\n");
            for (i = 0; i < count; i++) {
                printk("fte[%d] = 0x%x\n", i, ftes[i]);
            }
        }
...
_cmd_sbx_l3_mpls_tunnel(int unit, args_t *args)
{
    ...
    if (sal_strcasecmp(subcmd, "initiator-set") == 0) {
        ...
        for (i = 0; i < _FE2K_MAX_MPLS_TNNL_LABELS; i++) {
            if (label_array[i].label) {
                ...
                if (sal_strcasecmp(ttlop_str[i], "set") == 0) {
                    ...
                    printk("ttlop (%s) is invalid, [set|copy|decr] needs to be provided\n", ttlop_str[i]);
                    parse_arg_eq_done(&pt);
                    return(CMD_FAIL);
                }
            } else {
        ...
        rv = bcm_mpls_tunnel_initiator_set(unit,
                                           ...
                                           label_array);
        if (rv == BCM_E_NONE) {
            printk("mpls tunnel initiator set successfull\n");
        } 
        ...
        rv = bcm_mpls_tunnel_initiator_clear(unit,
                                             _l3_ifid);
        if (rv == BCM_E_NONE) {
            printk("mpls tunnel initiator clear successfull\n");
        }
        ...
        rv = bcm_mpls_tunnel_initiator_get (unit,
                                            ...
                                            &count);
        if (rv == BCM_E_NONE) {
            printk("mpls tunnel initiator get successfull\n");
            for (i = 0; i < count; i++) {
                printk("Label %d: value 0x%x ttl %d exp %d\n",
