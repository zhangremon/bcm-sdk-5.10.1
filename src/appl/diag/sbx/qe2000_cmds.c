/*
 * $Id: qe2000_cmds.c 1.10 Broadcom SDK $
...
cmd_sbx_qe2000_qsindirect(int unit, args_t *args)
{
    ...
    printk("cdp cmd_sbx_qe2000_qsindirect\n"); 
    ...
    if(!sal_strcasecmp(mem, "ratea")) {
        if(diag_parse_range(lo, hi, &start_idx, &end_idx, 0, 16383)) {
            printk("Invalid range. Valid range is : 0-16383\n");
            return CMD_FAIL;
        }
        if(!(value = ARG_GET(args))) {
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_rate_a_get(unit, idx, &val)) {
                    printk("Error getting Rate A for queue %d\n", idx);
                    return CMD_FAIL;
                }
                printk("Queue: %5d\tRate A: 0x%08X\n", idx, val);
            }
        } else {
            val = sal_ctoi(value, 0);
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_rate_a_set(unit, idx, val)) {
                    printk("Error setting Rate A for queue %d\n", idx);
                    return CMD_FAIL;
                }
            }
        }
        ...
        if(!(value = ARG_GET(args))) {
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_rate_b_get(unit, idx, (uint32 *)&val)) {
                    printk("Error getting Rate B for queue %d\n", idx);
                    return CMD_FAIL;
                }
                printk("Queue: %5d\tRate B: 0x%08X\n", idx, val);
            }
        } else {
            val = sal_ctoi(value, 0);
            for(idx = start_idx; idx <= end_idx; idx++) {
        ...
        if(!(value = ARG_GET(args))) {
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_credit_get(unit, idx, (uint32*)&val)) {
                    printk("Error getting credit for queue %d\n", idx);
                    return CMD_FAIL;
                }
                printk("Queue: %5d\tCredit: 0x%08X\n", idx, val);
            }
        } else {
            val = sal_ctoi(value, 0);
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_credit_set(unit, idx, (uint32)val)) {
                    printk("Error setting Credit for queue %d\n", idx);
                    return CMD_FAIL;
                }
            }
        }
        ...
        if(!ARG_CNT(args)) {
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_depth_length_get(unit, idx, &depth, &pktlen)) {
                    printk("Error getting depth_hplen for queue %d\n", idx);
                    return CMD_FAIL;
                }
                printk("Queue: %5d\tDepth: 0x%X\tPktlen: 0x%X\n", idx,
                       depth, pktlen);
            }
        } else {
            parse_table_t pt;
            ...
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_depth_length_set(unit, idx, depth, pktlen)) {
                    printk("Error setting depth_hplen for queue %d\n", idx);
                    return CMD_FAIL;
                }
            }
        }
        ...
        if(!ARG_CNT(args)) {
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_q2ec_get(unit, idx, &mc, &node, &port, &cos)) {
                    printk("Error getting q2ec for queue %d\n", idx);
                    return CMD_FAIL;
                }
                printk("Queue: %5d\tMc=%d  Node=0x%02X  Port=%d  Cos=%d\n", idx,
                       mc, node, port, cos);
            }
        } else {
            parse_table_t pt;
            ...
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_q2ec_set(unit, idx, mc, node, port, cos)) {
                    printk("Error setting q2ec for queue %d\n", idx);
                    return CMD_FAIL;
                }
            }
        }
        ...
        if(!ARG_CNT(args)) {
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_queue_para_get(unit, idx, &local, &hold_ts, &q_type)) {
                    printk("Error getting queue_para for queue %d\n", idx);
                    return CMD_FAIL;
                }
                printk("Queue: %5d\tlocal=%d  hold_ts=0x%X  q_type=0x%X\n", idx,
            ...
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_queue_para_set(unit, idx, local, hold_ts, q_type)) {
                    printk("Error setting queue_para for queue %d\n", idx);
                    return CMD_FAIL;
                }
            }
        }
...
printk("cdp shape_rate\n");
        ...
        if(!ARG_CNT(args)) {
            for(idx = start_idx; idx <= end_idx; idx++) {
printk("cdp shape_rate get\n");
                if(soc_qe2000_shape_rate_get(unit, idx, &shape_mode, &shape_rate)) {
                    printk("Error getting shape_rate for queue %d\n", idx);
                    return CMD_FAIL;
                }
                printk("Queue: %5d\tShape Mode=%d  shape_rates=0x%X\n", idx,
                       shape_mode, shape_rate);
            }
        } else {
            parse_table_t pt;
printk("cdp shape_rate set\n");
            ...
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_shape_rate_set(unit, idx, shape_mode, shape_rate)) {
                    printk("Error setting shape_rate for queue %d\n", idx);
                    return CMD_FAIL;
                }
            }
        }
        ...
        if(!ARG_CNT(args)) {
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_shape_maxburst_get(unit, idx, &shape_enable, &shape_maxburst)) {
                    printk("Error getting shape_max_burst for queue %d\n", idx);
                    return CMD_FAIL;
                }
                printk("Queue: %5d\tShape Enable=%d  Maxburst=0x%X\n", idx,
            ...
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_shape_maxburst_set(unit, idx, shape_enable, shape_maxburst)) {
                    printk("Error setting shape_max_burst for queue %d\n", idx);
                    return CMD_FAIL;
                }
            }
        }
        ...
        if(!(value = ARG_GET(args))) {
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_shape_bucket_get(unit, idx, &val)) {
                    printk("Error getting shape bucket for queue %d\n", idx);
                    return CMD_FAIL;
                }
                printk("Queue: %5d\tShape_bucket: 0x%08X\n", idx, val);
            }
        } else {
            val = sal_ctoi(value, 0);
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_shape_bucket_set(unit, idx, val)) {
                    printk("Error setting Shape bucket for queue %d\n", idx);
                    return CMD_FAIL;
                }
            }
        }
        ...
        if(!ARG_CNT(args)) {
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_age_get(unit, idx, &nonempty, &anemic_event, &ef_event, &cnt)) {
                    printk("Error getting age for queue %d\n", idx);
                    return CMD_FAIL;
                }
                printk("Queue: %5d\tNonempty=%d anemic_event=%d ef_event=%d cnt=0x%02X\n", idx,
            ...
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_age_set(unit, idx, nonempty, anemic_event, ef_event, cnt)) {
                    printk("Error setting age for queue %d\n", idx);
                    return CMD_FAIL;
                }
            }
        }
        ...
        if(!(value = ARG_GET(args))) {
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_age_thresh_key_get(unit, idx, &val)) {
                    printk("Error getting age_thresh_key for queue %d\n", idx);
                    return CMD_FAIL;
                }
                printk("Queue: %5d\tage_thresh_key: 0x%02X\n", idx, val);
            }
        } else {
            val = sal_ctoi(value, 0);
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_age_thresh_key_set(unit, idx, val)) {
                    printk("Error setting age_thresh_key for queue %d\n", idx);
                    return CMD_FAIL;
                }
            }
        }
        ...
        if(diag_parse_range(lo, hi, &start_idx, &end_idx, 0, 31)) {
            printk("Invalid range. Valid range is : 0-31\n");
            return CMD_FAIL;
        }
        if(!ARG_CNT(args)) {
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_age_thresh_get(unit, idx, &anemic_thresh, &ef_thresh)) {
                    printk("Error getting age_thres for idx %d\n", idx);
                    return CMD_FAIL;
                }
                printk("Idx: %5d\tanemic_thresh=%d ef_thresh=%d\n", idx,
            ...
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_age_thresh_set(unit, idx, anemic_thresh, ef_thresh)) {
                    printk("Error setting age_thres for idx %d\n", idx);
                    return CMD_FAIL;
                }
            }
        }
        ...
        if(diag_parse_range(lo, hi, &start_idx, &end_idx, 0, 8191)) {
            printk("Invalid range. Valid range is : 0-8191\n");
            return CMD_FAIL;
        }
        if(!ARG_CNT(args)) {
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_pri_lut_get(unit, idx, &pri, &next_pri)) {
                    printk("Error getting pri_lut for idx %d\n", idx);
                    return CMD_FAIL;
                }
                printk("Idx: %5d\tpri=%d next_pri=%d\n", idx,
                       pri, next_pri);
            }
        } else {
            parse_table_t pt;
            ...
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_pri_lut_set(unit, idx, pri, next_pri)) {
                    printk("Error setting pri_lut for idx %d\n", idx);
                    return CMD_FAIL;
                }
            }
        }
        ...
        if(diag_parse_range(lo, hi, &start_idx, &end_idx, 0, 16*4224-1)) {
            printk("Invalid range. Valid range is : 0-%d\n", 16*4224-1);
            return CMD_FAIL;
        }
        if(!ARG_CNT(args)) {
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_priority_get(unit, idx, &shaped, &pri, &next_pri)) {
                    printk("Error getting priority for idx %d\n", idx);
                    return CMD_FAIL;
                }
                printk("Idx: %5d\tshaped=%d pri=%d next_pri=%d\n", idx,
                       shaped, pri, next_pri);
            }
        } else {
            parse_table_t pt;
            ...
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_priority_set(unit, idx, shaped, pri, next_pri)) {
                    printk("Error setting priority for idx %d\n", idx);
                    return CMD_FAIL;
                }
            }
        }
        ...
        if(!ARG_CNT(args)) {
            for(idx = start_idx; idx <= end_idx; idx++) {
                ...
                if(soc_qe2000_e2q_get(unit, mc, node, port, &enable, &queue)) {
                    printk("Error getting e2q for idx %d\n", idx);
                    return CMD_FAIL;
                }
                printk("Idx: %5d\tqueue=%d enable=%d\n", idx,
                       queue, enable);
            }
        } else {
            parse_table_t pt;
            ...
            for(idx = start_idx; idx <= end_idx; idx++) {
                ...
                if(soc_qe2000_e2q_set(unit, mc, node, port, enable, queue)) {
                    printk("Error setting e2q for idx %d\n", idx);
                    return CMD_FAIL;
                }
            }
        }
        ...
        if(!ARG_CNT(args)) {
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_lastsentpri_get(unit, idx, &pri, &next_pri)) {
                    printk("Error getting lastsentpri for idx %d\n", idx);
                    return CMD_FAIL;
                }
                printk("Idx: %5d\tpri=%d next_pri=%d\n", idx,
                       pri, next_pri);
            ...
            for(idx = start_idx; idx <= end_idx; idx++) {
                if(soc_qe2000_lastsentpri_set(unit, idx, pri, next_pri)) {
                    printk("Error setting lastsentpri for idx %d\n", idx);
                    return CMD_FAIL;
                }
            }
        }
...
cmd_sbx_qe2000_mvtget(int unit, args_t *args)
{
    ...
    if (end_idx > mcgroup_size) {
        printk("Invalid high range. MVT entry max configured entry is %d\n", mcgroup_size);
        return CMD_FAIL;
    }
    ...
    for (i = start_idx; i <= end_idx; i++) {
        ...
        if(sbxRv) {
            printk("MVTGet Failed:(%d)\n", sbxRv);
            return CMD_FAIL;
        }
        printk("MVT entry %d: mvta=0x%x, mvtb=0x%x, ko=%d, next=0x%x\n",
...
cmd_sbx_qe2000_mvtset(int unit, args_t *args)
{
    ...
    int knockout = -1;
    uint32_t setportmask=0, portmaskhi = 0, portmasklo = 0;
