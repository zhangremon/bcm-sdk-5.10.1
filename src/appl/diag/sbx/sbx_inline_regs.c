/*
 * $Id: sbx_inline_regs.c,v 1.9 2010/05/20 02:37:35 iakramov Exp $
...
sbx_chip_info_add(int chip_id, char *dev)
{
    ...
    if (sbx_chip_info_get(chip_id, &chip_info,0) != CMD_OK) {
        ...
        if (soc_sbx_chip_count >= SOC_MAX_NUM_DEVICES) {
            printk("ERROR: sbx_chip_add. Number of devices > "
                   "SOC_MAX_NUM_DEVICES \n");
            assert(0);
        }
             ...
             int intreg)
{
    ...
    if (sbx_chip_info_get(chip_id, &chip_info,0 ) != CMD_OK) {
        /* new Device type */
        if (soc_sbx_chip_count >= SOC_MAX_NUM_DEVICES) {
            printk("ERROR: addRegister failed. Number of devices > "
                   "SOC_MAX_NUM_DEVICES \n");
            return;
        }
        chip_info = &soc_sbx_chip_list[soc_sbx_chip_count++];
    ...
    group = 0;
    inst = 1;
    if ( (chip_id == SOC_INFO_FE2000) ||
         (chip_id == SOC_INFO_FE2000XT) ){
        if (reg[0] == 'x' && reg[1] == 'm') {
            group_name = "xg";
            group = 1;
            inst = (chip_id == SOC_INFO_FE2000XT) ? 4 : 2;
    ...
    for (i = 0; i < inst; i++) {
        if (chip_info->nregs >= chip_info->maxreg) {
            printk("ERROR: addRegister failed. Number of regs greater"
        ...
        if ( (chip_id == SOC_INFO_FE2000 ||
              chip_id == SOC_INFO_FE2000XT) && group > 0) {

          if (chip_id == SOC_INFO_FE2000XT) {
        ...
        sprintf(buf, "%s_%s", dev, n);
        reg_info = sal_alloc(sizeof (soc_sbx_reg_info_t), buf);
        if (!reg_info) {
            printk("ERROR: alloc for register info failed\n");
            return;
        }
          ...
          uint32 shift, uint8 msb, uint8 lsb, uint32 type, uint32 default_val)
{
    ...
    if (sbx_chip_info_get(chip_id, &chip_info,0) != CMD_OK) {
        printk("ERROR: add_field failed. chip_id:%d not found\n", chip_id);
        return;
    }

    for (idx = 0; idx < chip_info->nregs; idx++) {
        n = chip_info->regs[idx]->name;
        if ( ((chip_id == SOC_INFO_FE2000) || (chip_id == SOC_INFO_FE2000XT))
    ...
    if (!inst) {
        printk("ERROR: addField failed. Adding field info before adding "
               "register type. \n");
        return;
    }

    if (inst == 2 && (!reg_info[0] || !reg_info[1])) {
        printk("ERROR: addField failed. only found 1 of 2 instances of"
               " register %s\n", reg);
        return;
    }
    if (inst == 4 && (!reg_info[0] || !reg_info[1] || !reg_info[2] || !reg_info[3])) {
        ...
        if (reg_info[2]) i++;
        if (reg_info[3]) i++;
        printk("ERROR: addField failed. only found %d of 4 instances of"
               " register %s\n", i, reg);
        return;
    }
    ...
    for (i = 0; i < inst; i++) {
        n = reg;
        if (inst > 1) {
            sprintf(name, "%c%c%d%s", reg[0], reg[1], i, &reg[2]);
            n = name;
        } else {
            n = reg;
        }

        sprintf(buf, "%s_%s_%s", dev, n, field);
        ...
        if (reg_info[i]->nfields >= SBX_REG_FIELDS_MAX) {
            printk("ERROR: addField failed. Number of fields greater than "
                   "SBX_REG_FIELDS_MAX \n");
        }
    }
}
...
sbx_qe2000_reg_list_init(void)
{
    ...
    soc_sbx_chip_info_t     *chip_info = sbx_chip_info_add(SOC_INFO_QE2000, "KA");
...
#define SAND_HAL_ADD_REG(device, sDevice, reg, sReg, offset, \
    ...
    add_register(SOC_INFO_QE2000, sDevice, sReg, offset, mask, msb, lsb, \
                intreg);
...
#define SAND_HAL_ADD_FIELD(device,sDevice, reg, sReg, field, sField, \
    ...
    add_field(SOC_INFO_QE2000, sDevice, sReg, sField, mask, shift, \
...
sbx_bme3200_reg_list_init(void)
{
    ...
    soc_sbx_chip_info_t     *chip_info = sbx_chip_info_add(SOC_INFO_BME3200, "PT");
...
#define SAND_HAL_ADD_REG(device, sDevice, reg, sReg, offset, \
                         no_test_mask, mask, msb, lsb, intreg) \
    add_register(SOC_INFO_BME3200, sDevice, sReg, offset, mask, msb, \
                lsb, intreg);
...
#define SAND_HAL_ADD_FIELD(device,sDevice, reg, sReg, field, sField, \
                           mask, shift, msb,lsb,type,defaultVal) \
    add_field(SOC_INFO_BME3200, sDevice, sReg, sField, mask, shift, \
             msb, lsb, type, defaultVal);
...
sbx_bm9600_reg_list_init(void)
{
    ...
    soc_sbx_chip_info_t     *chip_info = sbx_chip_info_add(SOC_INFO_BM9600, "PL");
...
#define SAND_HAL_ADD_REG(device, sDevice, reg, sReg, offset, \
                         no_test_mask, mask, msb, lsb, intreg) \
    add_register(SOC_INFO_BM9600, sDevice, sReg, offset, mask, msb, \
                lsb, intreg);
...
#define SAND_HAL_ADD_FIELD(device,sDevice, reg, sReg, field, sField, \
                           mask, shift, msb,lsb,type,defaultVal) \
    add_field(SOC_INFO_BM9600, sDevice, sReg, sField, mask, shift, \
             msb, lsb, type, defaultVal);
...
sbx_fe2000_reg_list_init(void)
{
    ...
    soc_sbx_chip_info_t     *chip_info = sbx_chip_info_add(SOC_INFO_FE2000, "CA");
...
#define SAND_HAL_ADD_REG(device, sDevice, reg, sReg, offset, \
                         no_test_mask, mask, msb, lsb, intreg) \
    add_register(SOC_INFO_FE2000, sDevice, sReg, offset, mask, msb, \
                lsb, intreg);
...
#define SAND_HAL_ADD_FIELD(device,sDevice, reg, sReg, field, sField, \
                           mask, shift, msb,lsb,type,defaultVal) \
    add_field(SOC_INFO_FE2000, sDevice, sReg, sField, mask, shift, \
             msb, lsb, type, defaultVal);
...
#if defined(BCM_FE2000_SUPPORT)
    soc_sbx_chip_info_t     *chip_info = sbx_chip_info_add(SOC_INFO_FE2000XT, "C2");
...
#define SAND_HAL_ADD_REG(device, sDevice, reg, sReg, offset, \
                         no_test_mask, mask, msb, lsb, intreg) \
    add_register(SOC_INFO_FE2000XT, sDevice, sReg, offset, mask, msb, \
                lsb, intreg);
...
#define SAND_HAL_ADD_FIELD(device,sDevice, reg, sReg, field, sField, \
                           mask, shift, msb,lsb,type,defaultVal) \
    add_field(SOC_INFO_FE2000XT, sDevice, sReg, sField, mask, shift, \
             msb, lsb, type, defaultVal);
