/*
 * $Id: sbQe2000MemAccess.c 1.23.20.2 Broadcom SDK $
...
sbQe2000ShowMemNames(void)
{
   ...
   for(i = 0; i < QE2000_MEM_MAX_INDEX; i++) {
       printk("%-32s",qe2kMemConfigTable[i].memname);
       if ((i+1)%3 == 0)
	 printk("\n");
   }
   printk("\n");
   ...
   uint    spi = 0;   /*  FIX THIS */
    ...
    case QE2000_KAQSLNAPRIENTRY:
	    ...
	    if ((pData[0]) || (pData[1]) || (pData[2]) || (pData[3]) || (pData[4])) {
	       sbZfKaQsLnaPriEntry_Unpack(&sbZfKaQsLnaPriEntryRecord,(uint8_t *)dbuf,sizeof(dbuf));
    ...
    case QE2000_KAQSLNANEXTPRIENTRY:
		...
		sbZfKaQsLnaNextPriEntry_Unpack(&sbZfKaQsLnaNextPriEntryRecord,(uint8_t *)dbuf,sizeof(dbuf));
    ...
    case QE2000_KAQMRATEDELTAMAXENTRY:
        status = sandDrvKaQmMemRead( unit, uTableId, uAddress,   pData);
        if (status == SAND_DRV_KA_STATUS_OK) {
            sbZfKaQmRateDeltaMaxTableEntry_Unpack(&sbZfKaQmRateDeltaMaxEntryRecord,(uint8_t *)dbuf,sizeof(dbuf));
