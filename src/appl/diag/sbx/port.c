...
sbx_port_disp_iter(int unit, pbmp_t pbm, pbmp_t pbm_mask, uint32 seen)
{
    ...
    BCM_PBMP_ITER(pbm, port) {
        ...
        if ((r = bcm_port_selective_get(unit, port, &info)) < 0) {
            printk("Error: %s Could not get port %s information: %s\n",
                   __PRETTY_FUNCTION__, SOC_PORT_NAME(unit, port), bcm_errmsg(r));
            return (CMD_FAIL);
        }
...
cmd_sbx_port(int unit, args_t *args)
{
    ...
    if (!sal_strcasecmp(c, "init")) {
        if ((rv = bcm_port_init(unit)) < 0) {
            printk("%s: Error: Failed to initialize port module unit=%d (%s)\n",
...
#define _call_bpi(pbm, pbm_mask) \
    ...
    PBMP_ITER(tmp_pbm, port) { \
        brief_port_info(SOC_PORT_NAME(unit, port), &info_all[port], \
...
cmd_sbx_pbmp(int unit, args_t *a)
{
    ...
    if (sal_strcasecmp(c, "port") == 0) {
        ...
        if (parse_port(unit, c, &port) < 0) {
            printk("%s: Invalid port string: %s (MAX allowable %d)\n", ARG_CMD(a), c, SOC_MAX_NUM_PORTS);
            return CMD_FAIL;
        }
        printk("    port %s ==> %s (%d)\n",
...
#define QE2000_PORT_COUNT 48
    ...
    if (!c) {   
        printk(" %s",cmd_sbx_port_rate_usage);
            return CMD_FAIL;
    } 
    ...
    if (sal_strcasecmp(c, "all") == 0) {
        ...
        for ( i = 0; i < QE2000_PORT_COUNT; i++) {
            if ((rv = bcm_port_rate_egress_get(unit, i, &kbits_sec, &kbits_burst)) < 0) {
                printk("%s: Error: Could not get port_rate_egress %s information: %s\n",
                        ARG_CMD(a), SOC_PORT_NAME(unit, i), bcm_errmsg(rv));
                /* sal_free(info_all); */
                return (CMD_FAIL);
            }
            printk("Port %d %s. Rate: %d kb/sec. Burst: %d kbits (0x%x 0x%x)\n", i,
    ...
    if (sal_strcasecmp(c, "port") == 0) {
     ...
     /*   return CMD_OK;  */
        ...
        printk(" command requires \"port <port#>\" argument\n");
        return CMD_FAIL;
  ...
  /* Ingress, egress or show both */
    if ((c = ARG_GET(a)) == NULL) {
        ...
        operation = INGRESS;
        printk("No ingress shaper support at this time\n");
        return CMD_FAIL;
        ...
        printk("%s: Error: unrecognized port rate type: %s\n",
                ARG_CMD(a), c);
        printk("include \"egress\" before rate values kbits_rate kbits_burst\n");
        printk("e.g. portrate port 1 egress 10 100\n");
        return CMD_FAIL;
    }
    ...
    if ((c = ARG_GET(a)) != NULL) {
        ...
        if ( kbits_sec == 0 ) {
            printk(" kb_sec is zero. Disable shaper\n");
            kbits_burst = 0;
        } else {
            if ((c = ARG_GET(a)) != NULL) {
     ...
     if (operation == SHOW) {
        ...
        printk("Port %d %s. Rate: %d kb/sec. Burst: %d kbits (0x%x 0x%x)\n", port,
            ...
            printk("Error: ingress not supported for QE2000\n");
            return (CMD_FAIL);

    } else if (operation == EGRESS) {
        printk("Set: ");
        /* printk("bcm_port_rate_egress_set 0x%x 0x%x\n", kbits_sec, kbits_burst); */
        if ((rv = bcm_port_rate_egress_set(unit, port, kbits_sec, kbits_burst)) < 0) {
            printk("%s: Error: Could not set port_rate_egress %s information: %s\n",
                   ARG_CMD(a), SOC_PORT_NAME(unit, port), bcm_errmsg(rv));
            /* sal_free(info_all); */
            return (CMD_FAIL);
        }
            ...
            printk("Error: Unknown command mode for QE2000\n");
            return (CMD_FAIL);
    }
