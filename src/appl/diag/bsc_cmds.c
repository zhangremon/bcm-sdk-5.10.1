/*
 * $Id: bsc_cmds.c 1.12 Broadcom SDK $
 *
...
cmd_bsc(int unit, args_t *a)
{
	...
	if (!function) {
		return CMD_USAGE;
	} else if (!sal_strncasecmp(function, "setmux", strlen(function))) {
		tmp = ARG_GET(a);
		if (!tmp) {
			return CMD_USAGE;
		}
	...
	} else if (!sal_strncasecmp(function, "readmax", strlen(function))) {
		soc_bsc_readmax(unit);
	} else if (!sal_strncasecmp(function, "readdpm", strlen(function))) {
	...
	} else if (!sal_strncasecmp(function,"epr", strlen(function))) {
                char buf_tmp[17];
                buf_tmp[16] = '\0';
                buf_tmp[0] = '\0';
		...
		sal_free(buf);
		printk("\n");
	} else if (!sal_strncasecmp(function,"epw", strlen(function))) {
		tmp = ARG_GET(a);
		if (!tmp) {
			return CMD_USAGE;
		}
		...
		retv = soc_eep24c64_write(unit, devno,
					addr, (uint8 *)tmp, strlen(tmp));
	} else if (!sal_strncasecmp(function,"eptest", strlen(function))) {
		devno = soc_bsc_devopen(unit, BSC_AT24C32_NAME);
                if (devno  < 0) {
                    printk("%s: cannot open I2C device %s: %s\n",
                           ARG_CMD(a), BSC_AT24C32_NAME, bcm_errmsg(devno));
	...
	} else if (!sal_strncasecmp(function,"sfpread", strlen(function))) {
		...
		sal_free(buf);
		printk("\n");
	} else if (!sal_strncasecmp(function,"sfpwrite", strlen(function))) {
                char sfp_dev_name[16];
                uint8 db;
		tmp = ARG_GET(a);
		if (!tmp) {
...
cmd_dpm(int unit, args_t *a)
{
	...
	} else if (!sal_strncasecmp(function,"setpol",strlen(function))) {
		p = ARG_GET(a);
		v = ARG_GET(a);
		if (p) {
			pol = parse_integer(p);
