/*
 * $Id: esw_diag.c 1.78.2.7 Broadcom SDK $
...
char cmd_esw_flex_usage[] = 
     "Usages:\n"
#ifdef COMPILER_STRING_CONST_LIMIT
     " flex <args>\n"
#else
     " stat-enable-debug - Enable flex counter traces \n"
     " stat-disable-debug - Disable flex counter traces \n"
     " stat-group-create \n" 
     "      {{ingress <object>             \n"
     "                Port=0               \n"
     "                Vlan=1               \n"
     ...
     "      TypedIntPri=6                  \n"
     "      SingleWithControl=7            \n"
     "      TrafficTypeWithControl=8       \n"
     "      DlfAllWithControl=9            \n"
     "      DlfIntPriWithControl=10        \n"
     "      TypedWithControl=11            \n"
     "      TypedAllWithControl=12         \n"
     "      TypedIntPriWithControl=13      \n"
     "      Dot1P=14                       \n"
     "      IntPri=15                      \n"
     "      IntPriCng=16                   \n"
     "      SvpType=17                     \n"
     "      Dscp=18                        \n"
     "      DvpType=19}                    \n"
     "   -Create stat group for given object & returns counter-id and n-count\n"
     " stat-group-destroy <stat_counter_id>\n"
     "   - Destroys created stat group identified by stat counter id \n"
     " stat-attach-port-counter \n"
     "      {{gport <value>} | {port-name <port-name>}} \n"
     "      <stat_counter_id>                           \n"
     "   - Attache Port to stat counter group identified by stat counter id \n"
     " stat-detach-port-counter \n"
     "      {{gport <value>} | {port-name <port-name>}} \n"
     "   - Detach Given Port to stat counter group \n"
     " stat-get-port-counter \n"
     "      {{gport <value>} | {port-name <port-name>}} \n"
     "      {ingress | egress}                          \n"
     "      {byte    | packet}                          \n"
     "      total-counters [index ...]                  \n"
     "   - Get Port Counters for given counter indexes  \n" 
     " stat-set-port-counter \n"
     "      {{gport <value>} | {port-name <port-name>}} \n"
     "      {ingress | egress}                          \n"
     "      {byte    | packet}                          \n"
     "      total-counters [ (index value) ...]         \n"
     "   - Set Port Counters for given counter indexes and values  \n" 
     " stat-attach-vlan-counter \n"
     "      <vlan_id>  <stat_counter_id>                \n"
     "   - Attache Vlan to stat counter group identified by stat counter id \n"
     " stat-detach-vlan-counter \n"
     "      <vlan_id>                                   \n"
     "   - Detach Given Vlan to stat counter group \n"
     " stat-get-vlan-counter \n"
     "      <vlan_id>                                   \n"
     "      {ingress | egress}                          \n"
     "      {byte    | packet}                          \n"
     "      total-counters [index ...]                  \n"
     "   - Get Vlan Counters for given counter indexes  \n" 
     " stat-set-vlan-counter \n"
     "      <vlan_id>                                   \n"
     "      {ingress | egress}                          \n"
     "      {byte    | packet}                          \n"
     "      total-counters [ (index value) ...]         \n"
     "   - Set Vlan Counters for given counter indexes and values  \n" 
#endif
     ;
extern cmd_result_t cmd_esw_flex(int unit, args_t *a);
#endif
