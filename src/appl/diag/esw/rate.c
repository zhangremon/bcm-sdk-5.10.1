...
cmd_esw_rate(int unit, args_t *a)
{
    ...
    if (ARG_CNT(a)) {
        ...
        if (limit < 0 && limit != _DFT_MAGIC_NUMBER) {
            printk("Error: Negative rate limit: %d is not " 
                                       "allowed\n", (int)limit);
        }
    ...
    if (displayOnly) {
        ...
        DPORT_BCM_PBMP_ITER(unit, pbmp, dport, p) {
            ...
            if ((r = bcm_rate_bcast_get(unit, &limit, &flags, p)) < 0) {
                printk(" Bcast = UNKNOWN, %s;", bcm_errmsg(r));
            } else if (flags == BCM_RATE_BCAST) {
                printk(" Bcast= TRUE,Limit=%d;", limit);
            } else {
                printk(" Bcast=FALSE ;");
            }
            if ((r = bcm_rate_mcast_get(unit, &limit, &flags, p)) < 0) {
                printk(" Mcast=UNKNOWN, %s;", bcm_errmsg(r));
            } else if (flags == BCM_RATE_MCAST) {
                printk(" Mcast= TRUE,Limit=%d;", limit);
            } else {
                printk(" Mcast=FALSE ;");
            }
            if ((r = bcm_rate_dlfbc_get(unit, &limit, &flags, p)) < 0) {
                printk(" Dlf=UNKNOWN, %s;", bcm_errmsg(r));
            } else if (flags == BCM_RATE_DLF) {
                printk(" Dlfbc= TRUE,Limit=%d;", limit);
            } else {
                printk(" Dlfbc=FALSE ;");
            }
            printk ("\n");
        ...
        if (bRateSet) {
            ...
            if (bRateFlGet || bRateLmGet) {
                if ((r = bcm_rate_bcast_get(unit, &oriLimit, &oriFlag, p)) < 0) {
                    printk("%4s Error: can not get Brocast limit info: %s\n", 
             ...
             if (!r && (r = bcm_rate_bcast_set(unit, 
                 ...
                 printk("%4s Error: can not set Broadcast rate: %s\n", 
                           BCM_PORT_NAME(unit, p), bcm_errmsg(r));
             }
          }
        /* Mcast meter control */
        if (mRateSet) {
            ...
            if (mRateFlGet || mRateLmGet) {
                if ((r = bcm_rate_mcast_get(unit, &oriLimit, &oriFlag, p)) < 0) {
                    printk("%4s Error: can not get Mcast limit info: %s\n", 
                        BCM_PORT_NAME(unit, p), bcm_errmsg(r));
                }
             }
             ...
             if (!r && (r = bcm_rate_mcast_set(unit, 
                                        limitPro, flagPro, p)) < 0) {
                 printk("%4s Error: can not set Mcast rate: %s\n", 
                           BCM_PORT_NAME(unit, p), bcm_errmsg(r));
             }
          }
        /* Dlf meter */
        if (dlfRateSet) {
            ...
            if (dlfRateFlGet || dlfRateLmGet) {
                if ((r = bcm_rate_dlfbc_get(unit, &oriLimit, &oriFlag, p)) < 0) {
                    printk("%4s Error: can not get DLF limit info: %s\n", 
                        BCM_PORT_NAME(unit, p), bcm_errmsg(r));
                }
             }
             ...
             if (!r && (r = bcm_rate_dlfbc_set(unit, 
                                        limitPro, flagPro, p)) < 0) {
                 printk("%4s Error: can not set DLF rate: %s\n", 
                           BCM_PORT_NAME(unit, p), bcm_errmsg(r));
             }
          }
    }
...
cmd_bw_rate(int unit, args_t *a)
{
    ...
    if (! soc_feature(unit, soc_feature_storm_control)) {
        printk("%s: Error: bandwidth limit unavailable for this device\n", 
	       ARG_CMD(a));
        return CMD_OK;
    }
    ...
    if ((c = ARG_GET(a)) == NULL) {
        printk("%s: Error: unspecified port bitmap: %s\n",
               ARG_CMD(a), c);
        return CMD_FAIL;
    ...
    if ((c = ARG_GET(a)) != NULL) {
        ...
        if (0 == sal_strncasecmp(c, "bcast", strlen(c))){
            flags = BCM_RATE_BCAST;
        } else if (0 == sal_strncasecmp(c, "mcast", strlen(c))) {
            flags = BCM_RATE_MCAST;
        } else if (0 == sal_strncasecmp(c, "dlf", strlen(c))) {
            flags = BCM_RATE_DLF;
        } else {
            printk("%s: Error: unrecognized rate type: %s\n",
                   ARG_CMD(a), c);
            return CMD_FAIL;
        }
    }
    else {
        printk("%s: Error: missing rate type: %s need bcast or mcast or dlf\n",
               ARG_CMD(a), c);
        return CMD_FAIL;
    }
    ...
    if ((c = ARG_GET(a)) == NULL) {     /* Get part */
        DPORT_BCM_PBMP_ITER(unit, pbm, dport, p) {
            ...
            if (rv < 0 ) {
                printk("%s port %s: ERROR: bcm_rate_bandwidth_get: "
                           "%s\n",
            ...
            if (rate) {
                printk("%4s:", BCM_PORT_NAME(unit, p));
                if (rate < 64) { 
                    printk("\tBandwidth meter: ? kbps ? kbits max burst.\n");
                } else {
                    printk("\tBandwidth meter: "
