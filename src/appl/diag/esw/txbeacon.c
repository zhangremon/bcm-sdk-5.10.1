/*
 * $Id: txbeacon.c 1.6.2.5 Broadcom SDK $
 * 
...
#if 0
char pkt[] = {0x00, 0x00, 0x00, 0x00, 0x01, 0x07, 0x00, 0x00,
              0x00, 0x00, 0x01, 0x0a, 0x81, 0x00, 0x00, 0x14,
              0x08, 0x00, 0x45, 0x00, 0x00, 0x34, 0x00, 0x00,
              0x00, 0x00, 0xff, 0x11, 0xde, 0xa5, 0x6e, 0x01,
              0x02, 0x0a, 0x6b, 0x01, 0x02, 0x07, 0xc3, 0x50,
              0x0e, 0xc8, 0x00, 0x20, 0x30, 0x18, 0x20, 0x48,
...
{
    printk("Got user=%p pktid=%x flags=%x len=%x port=%x interval=%x count=%x pkt_data=%x\n",
...
cmd_result_t cmd_esw_txbeacon(int unit, args_t *args)
{
    ...
    if (sal_strcasecmp(arg_string_p, "init") == 0) {
        ...
        printk("TXBeacon module initialized.\n");
        ...
        txb.pkt_data = sal_alloc(70, "TXB test packet");
        for (i = 0; i < txb.len; i++) {
            txb.pkt_data[i] = pkt[i];
        }
#endif        
        ...
        printk("Got flags=%x len=%x port=%x interval=%x count=%x pkt_data=%x\n",
        ...
        printk("Invalid TXBEACON subcommand: %s\n", arg_string_p);
