...
diag_mem_pp_check(int unit, int start, int end)
{
    ...
    for (ix = start; ix < end; ix++) {
        int entry_addr = soc_mem_addr(unit, MEM_PPm, 0, ix);
...
diag_dump_xq_packet(int unit, soc_block_t blk, int xq_ptr)
{
    ...
    soc_cm_print("XQ Packet:  index[%d]\n", xq_ptr);
    soc_dma_dump_pkt(unit, "  ", pkt_data, size, TRUE);
    soc_cm_print("\tCRC(len-4) = 0x%08x; CRC(len) = 0x%08x\n", 
...
diag_dump_cos_packets(int unit, soc_port_t port, int cos_start, int cos_end)
{
    ...
    for (cos = 0; cos < num_cos; cos++) {
        SOC_IF_ERROR_RETURN(READ_MMU_PKTLMTCOSr(unit, port, cos, &regval));
        limit = soc_reg_field_get(unit, MMU_PKTLMTCOSr, regval, LIMITf);
                   ...
                   int final_lanes)
{
    ...
    if (size > (9 * 1024)) { /* Jumbo packets are max size */
        soc_cm_print("IngBuf Packet oversized, skipping range[%d,%d]\n", 
                     start, end);
        return SOC_E_NONE;
    }
    ...
    soc_cm_print("IngBuf Packet:  range[%d,%d]\n", start, end);
    soc_dma_dump_pkt(unit, "  ", pkt_data, size, TRUE);
    sal_free(pkt_data);
...
diag_dump_ib_packets(int unit, soc_port_t port)
{
    ...
    while (!wrap) {
        ...
        do {
            ...
            info_addr = soc_mem_addr(unit, MEM_INGBUFm, blk, end_index) + 2;
                   ...
                   int errors, int start_bit)
{
    ...
    word_addr = soc_mem_addr(unit, MEM_PPm, blk, 0) + 
    ...
    while (errors--) {
        ...
        soc_cm_print(
            "Predicted error at entry %d, data word %d, bit %d\n",
