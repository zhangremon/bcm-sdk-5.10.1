/*
 * $Id: wlan.c 1.8.112.1 Broadcom SDK $
...
#define WLAN_ROE(op, arg)     \
    do {               \
        ...
        if (BCM_FAILURE(__rv__)) { \
            printk("WLAN_CLI: Error: " #op " failed, %s\n", \
                           bcm_errmsg(__rv__)); \
            return CMD_FAIL; \
        } \
    } while(0)
#define WLAN_CMD(_s)                         \
    if ((cmd = ARG_CUR(a)) == NULL) {        \
        ARG_PREV(a);  \
        printk("WLAN_CLI: Error: Missing arg after %s\n", ARG_CUR(a)); \
        return CMD_USAGE;                    \
...
#define PT_PARSE(_a, _pt) \
    do {     \
        if (parse_arg_eq(_a, &_pt) < 0) { \
            printk("WLAN_CLI: Error: Invalid option or expression: %s\n", \
                    ARG_CUR(_a)); \
            PT_DONE(_pt); \
            return CMD_USAGE; \
        } \
...
if_tr2_wlan(int unit, args_t * a)
{
    ...
    switch (action) {
        ...
        case WLAN_OPTIONS:
            ...
            break;
        default:
            printk("Error:Unknown WLAN command %d\n", action);
            return CMD_USAGE;
    }
...
_bcm_tr2_wlan_cli_port_add(int unit, args_t * a)
{
    ...
    if (ifl & (BCM_WLAN_PORT_REPLACE | BCM_WLAN_PORT_WITH_ID)) {
        ...
        soc_cm_debug(DK_TESTS, "wlan port=%08x\n", port);
    }
    ...
    printk("env var added : $wlan_port_id=0x%08x\n", wport.wlan_port_id);
...
_bcm_tr2_wlan_tunnel_init_create(int unit, args_t *a) 
{
    ...
    if ((rv = bcm_wlan_tunnel_initiator_create(unit, &tunnel_init)) < 0) {
        printk("ERROR %s: creating tunnel initiator %s\n",
...
_bcm_tr2_wlan_tunnel_init_destroy(int unit, args_t *a) 
{
    ...
    if ((rv = bcm_wlan_tunnel_initiator_destroy(unit, tunnel_id)) < 0) {
        printk("ERROR %s: destroying tunnel initiator for %d %s\n",
...
_bcm_tr2_wlan_tunnel_init_print(int unit, bcm_tunnel_initiator_t *info)
{
    ...
    printk("\tSRC MAC_ADDR    = %-18s\n", if_mac_str);
    format_macaddr(if_mac_str, info->dmac);
    printk("\tDST MAC_ADDR    = %-18s\n", if_mac_str);
...
_bcm_tr2_wlan_cli_mcast_group_addr(int unit, args_t * a)
{
    ...
    printk("WLAN_CLI: mc_group 0x%08x vlan 0x%08x\n", mcast_group, vlan);
    PT_DONE(pt);
...
_bcm_tr2_wlan_cli_parse_flags(char *input_s, int type)
{
    ...
    if(!flags) {
        printk("WLAN CLI: Error: Discarded unrecognized Flags\n\t %s\n", 
                        input_s);
        return 0;
    }
    ...
    while (!done) {
        ...
        if (!found) {
            printk("WLAN_CLI: flag %s not recognized, discarded\n", f_name);
        }
    }
    return result;
