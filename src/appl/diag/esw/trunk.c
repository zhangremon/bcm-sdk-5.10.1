/*
 * $Id: trunk.c 1.14.112.1 Broadcom SDK $
...
if_esw_trunk(int unit, args_t *a)
{
	...
	j = 0;
		...
		t_add_info.tm[j] = (soc_feature(unit, soc_feature_mod1) && i > 31) ? modid + 1 : modid;
		t_add_info.tp[j] = i;
        r = bcm_stk_modmap_map(unit, BCM_STK_MODMAP_GET, t_add_info.tm[j],
	...
	if (r >= 0) {
            BCM_PBMP_CLEAR(pbmp);
            SOC_IF_ERROR_RETURN(bcm_stk_my_modid_get(unit, &modid));
            for (i = 0; i < t_add_info.num_ports; i++) {
                if ((t_add_info.tm[i] != modid) && 
                    !soc_feature(unit, soc_feature_mod1))  {
                    t_add_info.tp[i] +=32;
                }
	        BCM_PBMP_PORT_ADD(pbmp, t_add_info.tp[i]);
            }
	...
	r = bcm_trunk_mcast_join(unit, tid, vid, mac);
        ...
        if (sal_strcasecmp(subcmd_s, "set") == 0) {
	       ...
	       if (r >= 0) {
                   printk("port %s programmable hash set to %d\n",
...
cmd_trunkpool(int unit, args_t *a)
{
    ...
    if (sal_strcasecmp(subcmd, "set") == 0) {
        ...
        DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
            ...
            printk("port %s trunk pool set:\n", BCM_PORT_NAME(unit, port));
            printk("     tid=%d   size=%d\n",arg_tid, arg_psize);
            r = bcm_trunk_get(unit, arg_tid,  &add_info);
            if (r >= 0) {
                printk("     ");
                for (i = 0; i < add_info.num_ports; i++) {
                     if (weights[add_info.tp[i]-1])
                         printk("W[%d]=%d ",
				add_info.tp[i] - 1,
    ...
    if (sal_strcasecmp(subcmd, "get") == 0) {
        ...
        DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
            ...
            printk("port %s trunk pool get:\n", BCM_PORT_NAME(unit, port));
            printk("     tid=%d   size=%d\n",arg_tid, size);
            r = bcm_trunk_get(unit, arg_tid,  &add_info);
            if (r >= 0) {
                printk("     ");
                for (i = 0; i <add_info.num_ports; i++) {
                     if (count[add_info.tp[i]-1])
                         printk("TotalCount[%d]=%d ",
				add_info.tp[i] - 1,
				count[add_info.tp[i] - 1]);
                }
                printk("\n");
            }
        }
