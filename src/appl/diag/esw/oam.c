/*
 * $Id: oam.c 1.30.2.3 Broadcom SDK $
 * 
...
#if defined(BCM_ENDURO_SUPPORT)
static int 
_cmd_esw_oam_tx(int unit, int flags, bcm_gport_t gport_dst, bcm_gport_t gport_src,
 bcm_mac_t *mac_dst, bcm_mac_t *mac_src, 
 bcm_oam_endpoint_info_t *endpoint_info)
{
    ...
    int vp = 0;        
    int rv = 0;
    bcm_module_t module_id, dst_module_id, src_module_id;
    bcm_port_t port_id, dst_port_id, src_port_id;
    bcm_trunk_t trunk_id, dst_trunk_id, src_trunk_id;
    int local_id, dst_local_id, src_local_id;
    ...
    if (BCM_GPORT_IS_TRUNK(endpoint_info->gport)) {
        if (endpoint_info->flags & BCM_OAM_ENDPOINT_UP_FACING) {
            /* Have to select one port in the trunk */
            BCM_IF_ERROR_RETURN(_bcm_esw_gport_resolve(unit, gport_src,
                &src_module_id, &src_port_id, &src_trunk_id, &src_local_id));
        } else {
            BCM_IF_ERROR_RETURN(_bcm_esw_gport_resolve(unit, gport_dst,
                &dst_module_id, &dst_port_id, &dst_trunk_id, &dst_local_id));
        }
    ...
    if (vp) {
        ...
        if (endpoint_info->flags & BCM_OAM_ENDPOINT_UP_FACING) {
            ...
            if (BCM_GPORT_IS_TRUNK(endpoint_info->gport)) {
                soc_higig_field_set(unit, xgh, HG_src_mod, src_module_id);
                soc_higig_field_set(unit, xgh, HG_src_port, src_port_id);
            } else {
...
char cmd_esw_oam_usage[] = 
    ...
    "\n"
    "  oam init\n"
    "  oam group add [ID=<id>] Name=<name> [RemoteDefect] [CopyToCpu=true|false]\n"
	"					[LowestAlarmPri=<0-6>]\n"
    "  oam group replace ID=<id> Name=<name> [RemoteDefect] [CopyToCpu=true|false]\n"
	"					[LowestAlarmPri=<0-6>]\n"
    "  oam group delete <id>\n"
    "  oam group show\n"
    ...
    "                   [OpcodeFlags=<flags>] [CPUPRI=<cpupri>]\n"
    ...
    "  oam events [on|off]\n"
#if defined(BCM_ENDURO_SUPPORT)
    "  oam tx ID=<endpointid> [DM] [LM] [DstPort=<port>] [SrcPort=<port>]\n"
...
cmd_result_t cmd_esw_oam(int unit, args_t *args)
{
    ...
    bcm_port_t port_src;
    bcm_gport_t gport_src = BCM_GPORT_INVALID;
    ...
    {    
        ...
        parse_table_add(&parse_table, "SrcPort", PQ_PORT, (void *)-1, &port_src,
                NULL);
        ...
        if (BCM_GPORT_IS_TRUNK(endpoint_info.gport)) {
            if (endpoint_info.flags & BCM_OAM_ENDPOINT_UP_FACING) {
                if (port_src != -1) {
                    BCM_GPORT_LOCAL_SET(gport_src, port_src);
                } else {
                    printk("Source port is required for UP MEP created on TRUNK.\n");
                    CLEAN_UP_AND_RETURN(CMD_FAIL);
                }
            } else {
                if (port != -1) {
                    ...
                    printk("Destination port is required for MEP created on TRUNK.\n");
                    CLEAN_UP_AND_RETURN(CMD_FAIL);
                }
            }
        }
        ...
        result = _cmd_esw_oam_tx(unit, pbbte, gport_dst, gport_src,
...
#if defined(BCM_KATANA_SUPPORT)
            parse_table_add(&parse_table, "OpcodeFlags", PQ_HEX, (void *) -1,
                                          &opcode_flags, NULL);
