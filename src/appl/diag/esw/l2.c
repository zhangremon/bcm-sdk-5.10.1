...
if_esw_l2(int unit, args_t *a)
{
    ...
    int          arg_start = 0, arg_valid = TRUE,
	arg_hash = XGS_HASH_COUNT,
	arg_end = 0;
#endif
	...
	parse_table_add(&pt, "LearnLimitExempt", PQ_DFL|PQ_BOOL,
        ...
        if (SOC_IS_EASYRIDER(unit)) {
            parse_table_add(&pt, "RemoteLookup",	PQ_DFL|PQ_BOOL,
                            0, &arg_rlk,	NULL);
        }
	if (!parseEndOk(a, &pt, &retCode)) {
	    return retCode;
	...
	return CMD_FAIL;
        ...
        if ((sscmd = ARG_GET(a)) != NULL) {
            if (!sal_strcasecmp(sscmd, "hit")) {
                if (soc_mem_is_valid(unit, L2X_HITm)) {
            fld = BUCKET_BITMAPf;
                mem = L2X_HITm;
                } 
            } else if (!sal_strcasecmp(sscmd, "valid")) {
                if (soc_mem_is_valid(unit, L2X_VALIDm)) {
                    fld = BUCKET_BITMAPf;
                mem = L2X_VALIDm;
                } 
            } else if (!sal_strcasecmp(sscmd, "static")) {
                if (soc_mem_is_valid(unit, L2X_STATICm)) {
                    fld = BUCKET_BITMAPf;
                mem = L2X_STATICm;
                } 
            } else {
                printk("Valid subcommands: hit, valid, static\n");
                return CMD_USAGE;
            }
        } else {
            mem = L2Xm;
        ...
        if ((INVALIDm == mem) || (INVALIDf == fld) ) {
            printk("Invalid command on a device\n");
            return CMD_FAIL;
        }
        ...
        for (i = soc_mem_index_min(unit, mem);
             i < soc_mem_index_max(unit, mem); i++) {
            if (!((i+1)%100)) {
                printk(".");
                if (!((i+1)%4000)) {
                    printk("\n");
...
#ifdef BCM_XGS_SWITCH_SUPPORT
    else if (!sal_strcasecmp(subcmd, "sanity")) {
	...
	uint32 regval;
        ...
        if (!SOC_IS_XGS12_SWITCH(unit)) {
            printk("Command only valid for XGS1 & XGS2 switches\n");
            return CMD_FAIL;
        }

	parse_table_init(unit, &pt);
	parse_table_add(&pt, "Start", 	PQ_DFL|PQ_INT,
			(void *)(0), &arg_start, NULL);
	parse_table_add(&pt, "End", 	PQ_DFL|PQ_INT,
			(void *)(0), &arg_end, NULL);
	parse_table_add(&pt, "Valid", 	PQ_DFL|PQ_BOOL,
			(void *)(0), &arg_valid, NULL);
	if (!parseEndOk(a, &pt, &retCode))
	    return retCode;
	...
	if ( arg_valid ) {
	    ...
	    printk("%s: Retrieving valid bits by DMA\n", ARG_CMD(a));
	    if ((rv = soc_mem_read_range
		 (unit, L2X_VALIDm, MEM_BLOCK_ANY,
		  low_index / SOC_L2X_BUCKET_SIZE,
		  high_index / SOC_L2X_BUCKET_SIZE, buf)) < 0) {
		printk("%s: ERROR: %s\n", ARG_CMD(a), soc_errmsg(rv));
		return CMD_FAIL;
	    }

	    valid_bits = buf[valid_ptr] >> (low_index % SOC_L2X_BUCKET_SIZE);
	}
	...
	for (ix = low_index; ix <= high_index; ix++) {
	    ...
	    if ( arg_valid ) {
		if ( (valid_bits & 0x1) == 0 ) {
		    soc_L2Xm_field32_set(unit, &(bucket_entry[offset]),
					 VALID_BITf, 0);
		    goto iter_stuff;
		}
	    }

	    if ( (rv = diag_l2x_entry_verify(unit, hash_select, ix,
					     &(bucket_entry[offset]))) < 0 ) {
		printk("\n%s: ERROR: %s at index %d\n",
		       ARG_CMD(a), soc_errmsg(rv), ix);
		soc_mem_entry_dump(unit, L2Xm, &(bucket_entry[offset]));
		printk("\n");
		cmd_rv = CMD_FAIL;
		goto iter_stuff;
	    }
	    ...
	    if ( soc_L2Xm_field32_get(unit, &(bucket_entry[offset]),
				      VALID_BITf)) {
		for (jx = 0; jx < offset; jx++ ) {
		    if (soc_L2Xm_field32_get(unit, &(bucket_entry[jx]),
					     VALID_BITf)) {
			if (soc_mem_compare_key(unit, L2Xm,
						&(bucket_entry[offset]),
						&(bucket_entry[jx])) == 0) {
			    /* Found a match!  Bad. */
			    printk("\n%s: ERROR: Duplicate keys: %d ~= %d\n",
				   ARG_CMD(a), ix, ((ix/SOC_L2X_BUCKET_SIZE) *
						    SOC_L2X_BUCKET_SIZE) + jx);
			    soc_mem_entry_dump(unit, L2Xm,
					       &(bucket_entry[offset]));
			    printk("\n");
			    soc_mem_entry_dump(unit, L2Xm,
					       &(bucket_entry[jx]));
	...
	iter_stuff:
	    if ( arg_valid ) {
		if ( offset == (SOC_L2X_BUCKET_SIZE - 1)) {
		    valid_bits = buf[++valid_ptr];
		} else {
		    valid_bits >>= 1;
		}
	    }
	...
	hash_select = arg_hash;
        ...
        defined(BCM_TRX_SUPPORT)
	        ...
	        hash_bucket = soc_er_l2_hash(unit, hash_select, key);
            } else
#endif
	    ...
	    soc_draco_l2x_param_to_key(arg_macaddr, arg_vlan, key);
	    hash_bucket = soc_draco_l2_hash(unit, hash_select, key);
        }
	...
	printk("Hash[%d] of key "
        ...
        if (!sal_strcasecmp(cachecmd, "add") ||
            ...
            if (SOC_IS_EASYRIDER(unit)) {
                parse_table_add(&pt, "RemoteLookup",   PQ_DFL|PQ_BOOL,
                                0, &arg_rlk,	NULL);
                parse_table_add(&pt, "TerminateTunnel",   PQ_DFL|PQ_BOOL,
                                0, &arg_tunnel,	NULL);
            }
...
if_esw_bpdu(int unit, args_t *a)
{
    int            idx, count;
    char           *subcmd = NULL;
    sal_mac_addr_t arg_macaddr, maczero, mac;
    int	           arg_index = 0;
    int	           rv;
    parse_table_t  pt;
