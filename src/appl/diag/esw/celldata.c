/*
 * $Id: celldata.c 1.65.2.1 Broadcom SDK $
...
cmd_cdt(int unit, args_t *a)
{
    ...
    if (!soc_mem_is_valid(unit, C1_CELLm) ||
        !soc_mem_is_valid(unit, C0_CELLm)) {
        printk("CellDataTest for unit %d memory %d not supported\n", unit, mem);
        rv = CMD_FAIL;
        goto exit_cleanup;
    }
...
_extt_l3_activities(int unit, int enable, int num_ge_pkt, int num_xe_pkt)
{
    ...
    if (enable) {
        ...
        if (rv < 0) {
            printk("fail to create Vlan %d : %s\n", vid, soc_errmsg(rv));
            return CMD_FAIL;
        }
        ...
        if (rv < 0) {
            printk("fail to add ports to Vlan %d : %s\n",
                   vid, soc_errmsg(rv));
            return CMD_FAIL;
        }
        ...
        rv = bcm_switch_control_get(unit, bcmSwitchL3EgressMode,
                                    &l3_egress_mode);
        if (rv < 0) {
            printk("fail to get current L3 egress mode : %s\n",
                   soc_errmsg(rv));
            return CMD_FAIL;
        }
        rv = bcm_switch_control_set(unit, bcmSwitchL3EgressMode, TRUE);
        if (rv < 0) {
            printk("fail to enable L3 egress mode : %s\n",
                   soc_errmsg(rv));
            return CMD_FAIL;
        }
        ...
        if (rv < 0) {
            printk("fail to create L3 interface : %s\n",
                   soc_errmsg(rv));
            return CMD_FAIL;
        }
        ...
        if (rv < 0) {
            printk("fail to create egress L3 object : %s\n",
                   soc_errmsg(rv));
            return CMD_FAIL;
        }
        ...
        rv = bcm_field_group_create(unit, qset, BCM_FIELD_GROUP_PRIO_ANY,
                                    &group);
        if (rv < 0) {
            printk("fail to create field group : %s\n",
                   soc_errmsg(rv));
            return CMD_FAIL;
        }
        ...
        PBMP_E_ITER(unit, port) {
            ...
            if (rv < 0) {
                printk("fail to create field entry : %s\n",
                       soc_errmsg(rv));
                return CMD_FAIL;
            }
        }
        ...
        PBMP_GE_ITER(unit, port) {
            ...
            if (rv < 0) {
                printk("fail to add field port qualifier : %s\n",
                       soc_errmsg(rv));
                return CMD_FAIL;
            }
            ...
            if (rv < 0) {
                printk("fail to add field action : %s\n",
                       soc_errmsg(rv));
                return CMD_FAIL;
            }
            prev_port = port;
        ...
        PBMP_E_ITER(unit, port) {
            ...
            if (rv < 0) {
                printk("fail to install field entry to hardware : %s\n",
                       soc_errmsg(rv));
                return CMD_FAIL;
            }
        }
        ...
        PBMP_E_ITER(unit, port) {
            ...
            if (rv < 0) {
                printk("fail to set port %d to MAC loopback : %s\n",
                       port, soc_errmsg(rv));
                return CMD_FAIL;
            }
        }
        ...
        if (pkt == NULL) {
            printk("fail to allocate packet : %s\n", soc_errmsg(rv));
            return CMD_FAIL;
        }
        ...
        PBMP_E_ITER(unit, port) {
            ...
            if (rv < 0) {
                printk("fail to remove field entry from hardware : %s\n",
                       soc_errmsg(rv));
                return CMD_FAIL;
            }
            ...
            if (rv < 0) {
                printk("fail to destroy field entry : %s\n",
                       soc_errmsg(rv));
                return CMD_FAIL;
            }
        }
        ...
        if (rv < 0) {
            printk("fail to destroy field group : %s\n",
                   soc_errmsg(rv));
            return CMD_FAIL;
        }
        ...
        if (rv < 0) {
            printk("fail to destroy egress L3 object : %s\n",
                   soc_errmsg(rv));
            return CMD_FAIL;
        }
        ...
        if (rv < 0) {
            printk("fail to delete L3 interface : %s\n",
                   soc_errmsg(rv));
            return CMD_FAIL;
        }
        ...
        rv = bcm_switch_control_set(unit, bcmSwitchL3EgressMode,
                                    l3_egress_mode);
        if (rv < 0) {
            printk("fail to restore L3 egress mode : %s\n",
                   soc_errmsg(rv));
            return CMD_FAIL;
        }
        ...
        if (rv < 0) {
            printk("fail to destroy Vlan %d : %s\n",
                   vid, soc_errmsg(rv));
            return CMD_FAIL;
        }
        ...
        PBMP_E_ITER(unit, port) {
            ...
            if (rv < 0) {
                printk("fail to remove port %d from loopback : %s\n",
                       port, soc_errmsg(rv));
                return CMD_FAIL;
            }
        }
...
cmd_extt(int unit, args_t *a)
{
    ...
    if (soc_feature(unit, soc_feature_esm_support) &&
        ...
        if (soc_triumph_esm_init_read_config(unit) < 0) {
            printk("ERROR: unit %d, restore TCAM config fail\n", unit);
            goto fail;
        }
        if (soc_tcam_init(unit) < 0) {
            printk("ERROR: unit %d, re-init TCAM fail\n", unit);
            goto fail;
        }
    }
#endif /* BCM_TRIUMPH_SUPPORT */
