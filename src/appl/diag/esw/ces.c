...
/*
 * $Id: ces.c 1.18.2.27 Broadcom SDK $
 * 
...
void bcm_diag_ces_show(int unit) {
	...
	printk("a_lops_threshold_table[1]: %d\n", config->ndGlobal.a_lops_threshold_table[1]);
	printk("a_aops_threshold_table[1]: %d\n", config->ndGlobal.a_aops_threshold_table[1]);
	printk("a_lops_threshold_table[2]: %d\n", config->ndGlobal.a_lops_threshold_table[2]);
	printk("a_aops_threshold_table[2]: %d\n", config->ndGlobal.a_aops_threshold_table[2]);
	printk("a_lops_threshold_table[3]: %d\n", config->ndGlobal.a_lops_threshold_table[3]);
	printk("a_aops_threshold_table[3]: %d\n", config->ndGlobal.a_aops_threshold_table[3]);
	...
	printk("---- RClock ----\n");
	printk("Index Service Enable OBRG Port Type\n");
	printk("----- ------- ------ ---- ---- ----\n");
	for (i = 0;i < BCM_CES_RCLOCK_MAX;i++) {
	    printk("%5d %6d %6d %4d %4d %4d\n",
		   config->rclock_record[i].config.rclock,
		   (config->rclock_record[i].service == NULL ? -1:config->rclock_record[i].service->ces_service),
		   config->rclock_record[i].config.enable,
		   config->rclock_record[i].config.output_brg,
		   config->rclock_record[i].config.port,
		   config->rclock_record[i].config.recovery_type);		   
	}
...
int bcm_diag_ces_tdm_set(int unit, int *port, bcm_tdm_port_config_t *config) {
    ...
    if (config->e_clk_rx_select < 0 || config->e_clk_rx_select > 1) {
	printk("Receive clock out of range\n");
	return BCM_E_PARAM;
    }
    if (config->e_clk_tx_select < 0 || config->e_clk_tx_select > 2) {
...
int bcm_diag_esw_liu_write(int unit, bcm_diag_ces_reg_config_t *config)
{
     ...
     * Select LIU
     */
    soc_mspi_select_device(unit, MSPI_LIU);
...
int bcm_diag_esw_ces_rclock_status_get(int unit, int *service) {
    ...
    ret = bcm_esw_ces_rclock_status_get(unit, *service, &status);
