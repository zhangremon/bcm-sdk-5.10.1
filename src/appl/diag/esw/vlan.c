/*
 * $Id: vlan.c 1.34.2.1 Broadcom SDK $
...
_parse_table_vlan_action_set_add(parse_table_t *pt, ivlan_action_set_t *act)
{
    ...
    parse_table_add(pt, "UtInner",     PQ_STRING, "None", 
                    &(act->ut_inner), NULL);   
#ifdef BCM_KATANA_SUPPORT
    parse_table_add(pt, "Policer",     PQ_HEX, "None", 
                    &(act->policer), NULL);   
#endif
}
...
_vlan_action_get(const char *action_name)
{
    ...
    for (i = 0; i < names; i++) {
        printk("%s ", vlan_action_names[i].name);
    }
    printk("\n");
    return -1;
}
...
_vlan_action_translate_key_get(const char *key_str)
{
    ...
    printk("Invalid key type <%s>. Valid key types are:\n   ", 
           key_str ? key_str : "");
    for (i = 1;  i < names; i++) {
        printk("%s ", vlan_key_type_names[i].name);
        if (i % 7 == 0) {
            printk("\n   ");
        }
    }
                             ...
                             void *user_data)
{
    ...
    printk("KeyType=%s, ",_vlan_action_translate_key_name_get(key_type));
...
if_esw_vlan(int unit, args_t *a)
{
    ...
    if (sal_strcasecmp(subcmd, "gport") == 0) {
        subcmd = ARG_GET(a);
        if (subcmd == NULL) {
            printk("%s: ERROR: Missing gport subcommand\n", ARG_CMD(a));
            return CMD_FAIL;
        }
        ...
        if ((sal_strcasecmp(subcmd, "delete") != 0) &&
            ...
            printk("%s: ERROR: Invalid gport subcommand\n", ARG_CMD(a));
            return CMD_FAIL;
        }
        ...
        if (sal_strcasecmp(subcmd, "delete") == 0) {
            ...
            int i, count;
            bcm_gport_t port_array[BCM_PBMP_PORT_MAX];
            int is_untagged_array[BCM_PBMP_PORT_MAX];
             ...
             */
            printk("Virtual and physical ports defined for VLAN: %d\n", id);
            for (i = 0; i < count; i++) {
	...
	subcmd = ARG_GET(a);
	if (subcmd == NULL) {
	    printk("%s: ERROR: missing policer subcommand\n", ARG_CMD(a));
	    return CMD_FAIL;
	}
	...
	if (sal_strcasecmp(subcmd, "set") == 0) {
	    ...
	    if (!parseEndOk( a, &pt, &retCode)) {
                printk("Usages: vlan policer set <id> POLICER=<policer>\n");
		return retCode;
	    }
            r = bcm_vlan_policer_set(unit, id, policer);
            if( 0 == r) {
                printk(" Policer id=%x is set to Vlan %x\n", policer,id);
            } else {
                printk(" Policer id=%x set to Vlan %x failed\n", policer,id);
            }
	    return CMD_OK;
	}
	if (sal_strcasecmp(subcmd, "get") == 0) {
	    if ((c = ARG_GET(a)) != NULL) {
            ...
            if(0 == r) {
                printk(" Policer id=%x is set in Vlan %x\n", policer,id);
            } else {
                printk(" Get Policer from Vlan %x failed\n", id);
            }
	    return CMD_OK;
	}

	printk("%s: ERROR: unknown policer subcommand: %s\n",
	       ARG_CMD(a), subcmd);
