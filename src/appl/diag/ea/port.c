/*
 * $Id: port.c 1.1.2.6 Broadcom SDK $
               ...
               uint32 flags)
{
    ...
    if (flags & BCM_PORT_ATTR_LINKSCAN_MASK) {
        if (info->linkscan) {
            printk("LS(%s) ", tk371x_linkscan_mode[info->linkscan]);
        }
    }

    if (hg_port) {
        printk("%s/XAUI(", tk371x_encap_mode[info->encap_mode & 0x3]);
    ...
    if (!no_an_props) {
        if (flags & BCM_PORT_ATTR_SPEED_MASK) {
            printk("%s", tk371x_if_fmt_speed(speed_buf, info->speed));
        }
    ...
    if (flags & BCM_PORT_ATTR_STP_STATE_MASK) {
        printk("STP(%s) ", tk371x_forward_mode[info->stp_state]);
    }
    ...
    if (flags & BCM_PORT_ATTR_INTERFACE_MASK) {
        if (info->interface >= 0 && info->interface < SOC_PORT_IF_COUNT) {
            printk("IF(%s) ", tk371x_interface_mode[info->interface]);
        }
    }
    ...
    if (flags & BCM_PORT_ATTR_PHY_MASTER_MASK) {
        ...
        info->phy_master != SOC_PORT_MS_NONE) {
            printk("PH(%s) ", tk371x_phymaster_mode[info->phy_master]);
        }
    }
        ...
        PBMP_ITER(pbm, p){
        	if (p == 0){
				...
				for (i = 0; poncontrol_names[i].name != NULL; i++) {
					...
					printk("\t %-45s= ", poncontrol_names[i].name);
					if (rv){
						printk("%s\n", bcm_errmsg(rv));
					} else {
						printk("0x%x\n", val);
				...
				for (i = 0; ethcontrol_names[i].name != NULL; i++) {
					...
					rv = bcm_port_control_get(unit, p, type, &val);
					printk("\t %-45s= ", ethcontrol_names[i].name);
					if (rv){
						printk("%s\n", bcm_errmsg(rv));
					} else {
						printk("0x%x\n", val);
				...
				for (i = 0; llidcontrol_names[i].name != NULL; i++) {
					...
					rv = bcm_port_control_get(unit, p, type, &val);
					printk("\t %-45s= ", llidcontrol_names[i].name);
					if (rv){
						printk("%s\n", bcm_errmsg(rv));
					} else {
						printk("0x%x\n", val);
    ...
    if (c == NULL) {
        ...
        PBMP_ITER(pbm, p){
            ...
            for (i = 0; ponphycontrol_names[i].name != NULL; i++) {
                ...
                printk("\t %-45s= ", ponphycontrol_names[i].name);
                if (rv){
                    printk("%s\n", bcm_errmsg(rv));
                } else {
                    printk("0x%x\n", val);
