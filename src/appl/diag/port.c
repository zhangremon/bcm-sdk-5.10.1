    /*
 * $Id: port.c 1.180.2.14 Broadcom SDK $
    ...
    if (sal_strcasecmp(cmd_str, "dsc") == 0) {
        ...
        DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
            if (soc_phyctrl_diag_ctrl(unit, port,PHY_DIAG_INT,
                   PHY_DIAG_CTRL_CMD,cmd,(void *)0) != SOC_E_NONE) {
                rv = CMD_FAIL;
            }
        }
...
if_esw_phy(int u, args_t *a)
{
    ...
    if (c != NULL && sal_strcasecmp(c, "timesync") == 0) {
        ...
        if ((c = ARG_CUR(a)) != NULL) {
            ...
            DPORT_SOC_PBMP_ITER(u, pbm, dport, p) {
                ...
                SOC_IF_ERROR_RETURN
                    (soc_phyctrl_offset_get(u, p, &offset));
    ...
    if (c != NULL && sal_strcasecmp(c, "dumpall") == 0) {
        ...
        if ((c = ARG_GET(a)) != NULL) {
            ...
            phy_addr_start = strtoul(c, &end, 0);
            if (*end) {
                printk("%s: Error: Expecting PHY start address [%s]\n", ARG_CMD(a), c);
                return CMD_USAGE;
            }
            if ((c = ARG_GET(a)) != NULL) {
                phy_addr_end = strtoul(c, &end, 0);
                if (*end) {
...
void config_i0e2_in(int unit, bcm_port_t port)
{
    ...
    rv = bcm_port_control_phy_timesync_set(unit, port, bcmPortControlPhyTimesyncLoadControl, BCM_PORT_PHY_TIMESYNC_TIMECODE_LOAD);
...
void config_i0e2_out(int unit, bcm_port_t port)
{
    ...
    config.original_timecode.seconds = 0xbeef8000;
...
void config_tc5p1(int unit, bcm_port_t port)
{
    ...
    config.syncout.syncout_ts = 80000;
    config.gmode = bcmPortPhyTimesyncModeCpu;
    ...
    rv = bcm_port_control_phy_timesync_set(unit, port, bcmPortControlPhyTimesyncLoadControl, 0xffffffff);
...
void cpu_sync(int unit, bcm_port_t port)
{
    ...
    rv = bcm_port_control_phy_timesync_set(unit, port, bcmPortControlPhyTimesyncFrameSync, 1);
...
void set_localtime(int unit, bcm_port_t port, uint64 localtime)
{
    ...
    rv = bcm_port_control_phy_timesync_set(unit, port, bcmPortControlPhyTimesyncLoadControl, BCM_PORT_PHY_TIMESYNC_LOCAL_TIME_LOAD);
...
set_localtime(0, port1, 0xa3344556677ULL);
