...
cmd_stkmode(int unit, args_t *a)
{
	...
	if (rv < 0) {
	    printk("%s: ERROR: bcm_stk_modid_get: %s\n",
	...
	if (bcm_stk_sl_simplex_count_get(unit, &slcount) < 0) {
	    slcount = 0;
	}
	if (modcount != 1) {
	    printk("%s: unit %d: module id %d (uses %d module ids)\n",
		   ARG_CMD(a), unit, modid, modcount);
	} else {
	    printk("%s: unit %d: module id %d\n",
		   ARG_CMD(a), unit, modid);
	}
	if (slmode || slcount) {
	    printk("%s: unit %d: SL mode %s, simplex hop count %d, "
		   "Higig/SL\n",
		   ARG_CMD(a), unit,
		   slmode ? "on" : "off",
		   slcount);
	...
	if (rv < 0) {
	    printk("%s: ERROR: bcm_stk_modid_set: %s\n",
		   ARG_CMD(a), bcm_errmsg(rv));
	    return CMD_FAIL;
	}
    }
	...
	if (rv < 0) {
	    printk("%s: ERROR: bcm_stk_modport_clear_all: %s\n",
		   ARG_CMD(a), bcm_errmsg(rv));
	    return CMD_FAIL;
	}
    }
	...
	if (rv < 0) {
	    printk("%s: ERROR: bcm_stk_port_add: %s\n",
		   ARG_CMD(a), bcm_errmsg(rv));
	    return CMD_FAIL;
	}
    }
#endif
    if (slcount >= 0) {
	rv = bcm_stk_sl_simplex_count_set(unit, slcount);
	if (rv < 0) {
	    printk("%s: ERROR: bcm_stk_sl_simplex_count_set: %s\n",
		   ARG_CMD(a), bcm_errmsg(rv));
	    return CMD_FAIL;
	}
    }
