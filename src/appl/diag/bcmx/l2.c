/*
 * $Id: l2.c 1.20.234.1 Broadcom SDK $
...
bcmxdiag_l2_notify(bcmx_l2_addr_t *addr, int insert, void *cookie)
{
    ...
    if (l2n_report) {
        soc_cm_print("BCMX L2 %s: V=0x%03x MAC=0x%02x%02x%02x"
    ...
    if (l2n_insert) {  /* Propagate native inserts/deletes */
        if (addr->flags & BCM_L2_NATIVE) {
            if (insert) {
                ...
                if (rv < 0) {
                    soc_cm_print("ERROR L2 notify insert %d: %s\n", rv,
                                 bcm_errmsg(rv));
                }
            } else {
                ++l2n_delete_calls;
                ...
                if (rv < 0) {
                    soc_cm_print("ERROR L2 notify delete %d: %s\n", rv,
                                 bcm_errmsg(rv));
                }
            }
        }
...
bcmx_cmd_l2(int unit, args_t *args)
{
    ...
    if (! initted) {
        ...
        if (bcmx_lplist_init(&plist, 0, 0) < 0) {
            soc_cm_print("%s: Error: could not init plist\n", ARG_CMD(args));
            return CMD_FAIL;
        }
    ...
    if (! sal_strcasecmp(subcmd, "add") || ! sal_strcasecmp(subcmd, "+")) {
        ...
        if ((argage = ARG_GET(args)) == NULL) {
            ...
            if (rv < 0) {
                ...
                soc_cm_print("Current age timer setting: %d seconds\n",
                        arg_age_seconds);
            }
        } else {
        ...
        if (rv < 0) {
            ...
            lport = l2addr.lport;
            soc_cm_print("Found on uport %s:", bcmx_lport_to_uport_str(lport));
        ...
        soc_cm_print("L2 dump not yet implemented.\n");
        return CMD_OK;
    }
        ...
        if ((subcmd = ARG_GET(args)) == NULL) {
            soc_cm_print("L2 notify: Ins %d (tot %d). "
            ...
            while (subcmd) {
                if (!sal_strcasecmp(subcmd, "start")) {
                    ...
                    if (rv < 0) {
                        soc_cm_print("Could not start BCMX L2 notify: %s",
                                   bcm_errmsg(rv));
                        return CMD_FAIL;
                    }
                    ...
                    if (rv < 0) {
                        soc_cm_print("Could not stop BCMX L2 notify: %s",
                                   bcm_errmsg(rv));
                        return CMD_FAIL;
                    }
                    ...
                    if (rv < 0) {
                        soc_cm_print("Could not register BCMX L2 notify "
                    ...
                    soc_cm_print("L2 notify report is %s\n", l2n_report ?
                                 "on" : "off");
                } else if (!sal_strcasecmp(subcmd, "+report")) {
                    l2n_report = TRUE;
                } else if (!sal_strcasecmp(subcmd, "-report")) {
                    ...
                    soc_cm_print("L2 notify insert is %s\n", l2n_insert ?
                                 "on" : "off");
                } else if (!sal_strcasecmp(subcmd, "+insert")) {
                    l2n_insert = TRUE;
                } else if (!sal_strcasecmp(subcmd, "-insert")) {
                    ...
                    soc_cm_print("Bad notify subcommand: %s\n", subcmd);
                    return CMD_USAGE;
                }
                subcmd = ARG_GET(args);
            }
        ...
        if (!sal_strcasecmp(cachecmd, "add") ||
            ...
            if (arg_cidx == -1) {
                printk("%s ERROR: no cache index specified\n", ARG_CMD(args));
                return CMD_FAIL;
            }
...
bcmx_dump_l2_addr(char *pfx, bcmx_l2_addr_t *l2addr, bcmx_lplist_t *port_block)
{
    ...
    printk("%smac=%02x:%02x:%02x:%02x:%02x:%02x vlan=%d uport=%s",
           pfx,
    ...
    if (l2addr->flags & BCM_L2_TRUNK_MEMBER) {
        printk(" Trunk=%d Rtag=%d", l2addr->tgid, l2addr->trunk_sel);
    }
...
bcmx_dump_l2_cache_addr(char *pfx, bcmx_l2_cache_addr_t *l2caddr)
{
    ...
    if (l2caddr->flags & BCM_L2_CACHE_TRUNK) {
        ...
        printk(" uport=%s", bcmx_lport_to_uport_str(l2caddr->dest_lport));
    }
