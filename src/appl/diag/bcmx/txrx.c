...
bcmx_cmd_rx(int unit, args_t *args)
{
    ...
    if ((ch = ARG_GET(args)) == NULL) {
        printk("BCMX RX is %srunning; seen %d packet%s (%d remote); "
               "pkt dump is %s\n",
               bcmx_rx_running() ? "" : "not ", 
    ...
    while (ch) {
        if (!strcmp("start", ch)) {
            rv = bcmx_rx_start();
            pkt_count = 0;
            printk("BCMX RX start returns %s\n", bcm_errmsg(rv));
        } else if (!strcmp("rlclear", ch)) {
#ifdef	BCM_RPC_SUPPORT
            bcm_rlink_clear();
#else
            printk("Function rlink_clear unavailable\n");
	    return CMD_FAIL;
#endif	/* BCM_RPC_SUPPORT */
        } else if (!strcmp("stop", ch)) {
            rv = bcmx_rx_stop();
            printk("BCMX RX stop returns %s\n", bcm_errmsg(rv));
            ...
            rv = bcmx_rx_register(
                ...
                NULL,
                BCM_RCO_F_ALL_COS);
            printk("BCMX RX register returns %s\n", bcm_errmsg(rv));
        } else if (!strcmp("unreg", ch)) {
            rv = bcmx_rx_unregister(_bcmx_rx_handler, 100);
            printk("BCMX RX unregister returns %s\n", bcm_errmsg(rv));
            ...
            pkt_count = 0;
            printk("BCM TX Tunnel Setup returns %s\n", bcm_errmsg(rv));
#else
            printk("BCM TX Tunnel Setup unavailable\n");
	    return CMD_FAIL;
#endif	/* BCM_RPC_SUPPORT */
            ...
            pkt_count = 0;
            printk("BCM TX Tunnel Stop returns %s\n", bcm_errmsg(rv));
#else
            printk("BCM TX Tunnel Stop unavailable\n");
	    return CMD_FAIL;
#endif	/* BCM_RPC_SUPPORT */
        } else {
            return CMD_USAGE;
...
_bcmx_rx_handler(int unit, bcm_pkt_t *pkt, void *cookie)
{
    ...
    pkt_count++;
    if (dump) {
        printk("BCMX RX %d: %p from: (%d, %d), len %d.\n", unit,
...
bcmx_cmd_tx(int unit, args_t *args)
{
    ...
    for ( xfer = 0; xfer < count; xfer++ ) {
        ...
        if (rv < 0) {
            printk("BCMX TX ERROR:  return value is %d (%s)\n",
                   rv, bcm_errmsg(rv));
            break;
        }
        sal_usleep(delay);
