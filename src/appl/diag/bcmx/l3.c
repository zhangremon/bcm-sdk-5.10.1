/*
 * $Id: l3.c 1.16.166.1 Broadcom SDK $
...
_bcmx_l3_ip_show(bcmx_l3_host_t *info)
{
    ...
    if (info->l3a_flags & BCM_L3_IP6) {
        printk("%-4s %-42s %-18s %-4s  %-11s %6s  %3s\n",
               "VRF", "IP address", "Mac Address", "INTF",
               "LPORT", "CLASS", "HIT");
    } else {
        printk("%-4s %-16s %-18s %-4s  %-11s %6s  %3s\n",
               "VRF", "IP address", "Mac Address", "INTF",
               "LPORT", "CLASS", "HIT");
    }
...
bcmx_cmd_l3(int unit, args_t *args)
{
	...
	sal_strcasecmp(table, "interface") == 0) {
	    ...
	    sal_strcasecmp(subcmd, "add") == 0) {
            ...
            if (intf_idx < 0) { /* Not specified */
                rv = bcmx_l3_interface_create(mac, vid, 1);
                if (rv == BCM_E_NONE) {
                    intf_idx = rv;
                }
            } else {
                rv = bcmx_l3_interface_id_create(mac, vid, intf_idx, 1);
            }
	    ...
	    sal_strcasecmp(subcmd, "destroy") == 0) {
            ...
            rv = bcmx_l3_interface_destroy(intf);
            if (rv < 0) {
    ...
    if (sal_strcasecmp(table, "defip") == 0) {
	    ...
	    sal_memset(&l3route, 0, sizeof(l3route));
            ...
            rv = bcmx_l3_defip_delete(&l3route);
            if (rv < 0) {
