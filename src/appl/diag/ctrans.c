...
rx_start(void)
{
    ...
    for (i = 0; i < soc_ndev; i++) {
        ...
        rv = bcm_rx_init(i);
        if (rv == BCM_E_UNAVAIL) {
            printk("WARNING: BCM RX init failed, unit %d: %s\n",
                   i, bcm_errmsg(rv));
            continue;
    ...
    for (i = 0; i < soc_ndev; i++) {
        ...
        if (!bcm_rx_active(i)) {
            printk("WARNING: skipping callback registration on unit %d\n", i);
            continue;
        }
        ...
        if (BCM_FAILURE(rv)) {
            printk("ERROR: Cannot register RX callback to initiate packet "
                   "processing, unit %d: %s\n",
                   i, bcm_errmsg(rv));
            /* Clean-up */
            for (; i > 0; i--) {
...
atptrans_socket_subcmd(int unit, args_t *args, cpudb_ref_t db_ref)
{
    ...
    if (parse_cmp("INStall", subcmd, '\0')) {
        ...
        if (BCM_FAILURE(rv)) {
            printk("Cannot install socket interface in ATP transport "
    ...
    if (BCM_FAILURE(rv)) {
        if (rv == BCM_E_UNAVAIL) {
            printk("Socket interface not available for current system\n");
        }
        else {
...
ct_setup(int unit, args_t *args)
{
    ...
    if ((subcmd = ARG_GET(args)) == NULL) {
        printk("Next Hop is%s running\n",
               next_hop_running() ? "" : " not");
        printk("ATP is%s running\n",
               atp_running() ? "" : " not");
        return CMD_OK;
    }
    ...
    if (sal_strcasecmp(subcmd, "RX") == 0) {
        ...
        if (ARG_CUR(args) == NULL) {
            for (idx = 0; idx < next_hop_num_ports_get(); idx++) {
                if (next_hop_port_get(idx, &s_unit, &s_port, &duplex)
                    == BCM_E_NONE) {
                    printk("NH idx %d:  Unit %d, Port %d: %sarked duplex.\n",
                           idx, s_unit, s_port, duplex ? "M" : "Not m");
                } else {
                    printk("NH idx %d:  INVALID?\n", idx);
                }
                did_print = TRUE;
            }
            for (idx = 0; idx < next_hop_max_entries_get(); idx++) {
                ...
                if (key != NULL) {
                    printk("KEY %x:%x:%x:%x:%x:%x is in NH DB\n",
            ...
            if (!did_print) {
                printk("Next Hop database and ports empty\n");
            }
        } else {
            int thrd_pri = -1;
            int rx_pri = -1;
            ...
            if (nhvlan >= 0) {
                ...
                if (rv < 0) {
                    printk("NH TX dest install failed: %s\n", bcm_errmsg(rv));
                    return CMD_FAIL;
                }
            }
            if (src_mod >= 0) {
            ...
            if (pkt_preinit() < 0) {
                printk("NH: Pre-init failed\n");
                return CMD_FAIL;
            }
            
            if (next_hop_running()) {
                printk("Next hop is running; stopping: %d\n",
                       next_hop_stop());
            }
            
            if (thrd_pri >= 0 || rx_pri >= 0) {
                if (next_hop_config_set(NULL, thrd_pri, rx_pri) < 0) {
                    printk("Warning: config set failed\n");
                }
            }
            rv = next_hop_start(&db_refs[db_idx]->local_entry->base);
            if (rv < 0) {
                printk("ERROR: next_hop_start failed: %s\n", bcm_errmsg(rv));
            }
            parse_arg_eq_done(&pt);
        }
        return CMD_OK;
        ...
        if (cos == -1) {
            for (cos = 0; cos <= BCM_COS_MAX; cos++) {
                ...
                printk("Limit for COS %d currently %d\n", cos, limit);
            }
            parse_arg_eq_done(&pt);
            return CMD_OK;
        }
        ...
        if (rv < 0) {
            printk("WARNING: limit set returned %d: %s\n", rv, bcm_errmsg(rv));
        }
        ...
        if ((subcmd = ARG_CUR(args)) != NULL) {
            if (parse_cmp("TRANSport", subcmd, '\0')) {
                ...
                if (db_idx < 0 || db_idx >= MAX_DBS ||
                    db_refs[db_idx] == CPUDB_REF_NULL) {
                    printk("Bad or empty DB index: %d\n", db_idx);
                    return CMD_FAIL;
                }
                if (db_refs[db_idx]->local_entry == NULL) {
                    printk("No local DB entry in cpudb %d\n", db_idx);
        ...
        if (rv < 0) {
            printk("ERROR: ATP config set failed: %s\n", bcm_errmsg(rv));
        }
        ...
        rv = atp_start(ATP_F_LEARN_SLF,
                       ...
                       BCM_RCO_F_ALL_COS);
        if (rv < 0) {
            printk("ERROR: ATP start failed: %s\n", bcm_errmsg(rv));
        }
        parse_arg_eq_done(&pt);
#if defined(BROADCOM_DEBUG)
        ...
        rv = atp_stop();
        if (rv < 0) {
            printk("ERROR: ATP stop failed: %s\n", bcm_errmsg(rv));
        }
        ...
        if (ARG_CUR(args) == NULL) {
            ...
            printk("Retries occur after %d usecs and will be made %d times\n",
                   retry_us, retries);
        } else {
            parse_table_init(unit, &pt);
            ...
            printk("Retry set(%d, %d) returns %d\n", retry_us, retries,
        ...
        if ((next_arg = ARG_GET(args)) == NULL) {
            ...
            printk("Current segmentation length is %d\n", seg_len);
        } else {
            seg_len = strtoul(next_arg, NULL, 0);
            printk("Seg_len set(%d) returns %d\n", seg_len,
        ...
        if (ARG_CUR(args) == NULL) {
            printk("Current TX pool: %d.  RX pool %d.\n", tx_pool, rx_pool);
        } else {
            parse_table_init(unit, &pt);
            ...
            printk("Pool size set returns %d\n",
...
ct_install(int unit, args_t *args)
{
    ...
    if (drv_mode == CTI_DRIVER_NONE) {
        printk("Note:  Clearing per CPU transport with NULL driver\n");
    } else if (cti_driver_ptrs[drv_mode] == NULL) {
        printk("Sorry, driver for %s is not yet implemented\n",
    ...
    if (trans_mode == CTI_TRANS_ALL || trans_mode == CTI_TRANS_NH) {
        ...
        if (rv < 0) {
            printk("WARNING:  nh_tx pointer set returned %d: %s\nContinuing\n",
                   rv, bcm_errmsg(rv));
        }
    }
    ...
    if (trans_mode == CTI_TRANS_ALL || trans_mode == CTI_TRANS_DBENTRY) {
        ...
        if (db_refs[db_idx] == CPUDB_REF_NULL) {
            printk("DB %d is not setup.  Try 'CPUDB create'.\n", db_idx);
            return CMD_FAIL;
        }
        CPUDB_KEY_SEARCH(db_refs[db_idx], key, entry);
        if (entry == NULL) {
            printk("ERROR: DB entry not found\n");
            return CMD_FAIL;
        }
...
_send_echo_pkt_cb(uint8 *pkt, void *cookie, int rv)
{
    ...
    if (cookie == NULL) {
        printk("Error in async callback routine; missing data\n");
        return;
    }
    
    async_data = (_echo_cb_async_t *)cookie;
    printk("Async callback routine, unit %d\n", async_data->unit);
               ...
               void *cookie)
{
    ...
    if (verbose) {
        printk("echo pkt: %d bytes to " CPUDB_KEY_FMT_EOLN, payload_len,
               CPUDB_KEY_DISP(dest_key));
    }
    PACK_SHORT(payload, depth);
    PACK_LONG(&payload[sizeof(uint16)], pkt_flags);
    PACK_LONG(payload+payload_len-sizeof(uint32),
              ~_shr_crc32(~0, payload, payload_len-sizeof(uint32)));
           ...
           void *cookie)
{
    ...
    if (payload == NULL) {  /* Just use first segment for string */
        if (pkt == NULL) {
            printk("CT echo error: payload and pkt both NULL, cli %d\n",
                   client_id);
            return BCM_RX_NOT_HANDLED;
        }
        payload = pkt->pkt_data[1].data;
        len = pkt->pkt_data[1].len;
        printk("CT echo warning: \n");
        printk("   Segmented packet; segmentation not maintained on echo\n");
    } else {
        len = payload_len;
    }
    ...
    if (mode < 0 || mode > 4) {
        printk("CT echo error: Bad client id: %d\n", client_id);
        return BCM_RX_NOT_HANDLED;
    }

    if (sal_strlen((char *)&payload[offset]) > len) {
        printk("CT echo error: Unterminated string in first segment\n");
        return BCM_RX_HANDLED;
    }

    if (_shr_crc32(~0, payload, len) != _SHR_CRC32_CORRECT) {
        printk("CT echo error: CRC failure.\n");
    }
    
    if (options->verbose) {
        printk("%s echo request from " CPUDB_KEY_FMT ": %s\n",
               cte_mode_list[mode],
               CPUDB_KEY_DISP(src_key),
               &payload[offset]);
        printk("       Depth %d, flags 0x%x, len %d. (total %d)\n",
...
_ct_echo_pkt_create(_echo_payload_t *ep,
                    char *str, int mode, int alloc_hdr_here, int minlen)
{
    int payload_len, len;
    uint32 pkt_flags = 0;
    ...
    if (pkt_buf == NULL) {
        printk("Could not alloc packet buffer\n");
        return 0;
    }
    ...
    sal_strcpy((char *)&payload_buf[ECHO_STR_OFFSET], str);
...
_ct_echo_async(int unit, cpudb_key_t dest_key,
               ...
               int alloc_hdr_here, int verbose, int minlen)
{
    void *cookie = NULL;
    ...
    if (!_ct_echo_pkt_create(&payload, str, mode, alloc_hdr_here, minlen)) {
        return;
    }
    payload.pkt_flags |= CTE_FLAGS_ASYNC_FREE;

    if ((async_data = sal_alloc(sizeof(*async_data), "ct-echo")) == NULL) {
        printk("Could not alloc memory for async data\n");
        return;
    }
    ...
    if (verbose) {
        printk("CT echo:  Sending out %s echo request to CPU key "
     ...
     */
    if (BCM_FAILURE(rv)) {
        printk("%s tx returns %d (%s)\n", cte_mode_list[mode],
               rv, bcm_errmsg(rv));
        sal_dma_free(payload.pkt_buf);
        sal_free(async_data);
    } else {
        printk("Echo sent successfully\n");
    }
}
...
_ct_echo_sync(int unit, cpudb_key_t dest_key,
              char *str, int depth, int mode,
              int alloc_hdr_here, int verbose, int minlen, int count)
{
    void *cookie = NULL;
    _echo_payload_t payload;
    int rv = BCM_E_NONE;
...
ct_echo(int unit, args_t *args)
{
    ...
    if (sal_strcasecmp(subcmd, "REG") == 0) {
        ...
        parse_arg_eq_done(&pt);
        if (rv < 0) {
            printk("Could not register echo as %s (%d) client (%d): %s\n",
        ...
        if (async) {
            _ct_echo_async(unit, dest_key, str, depth,
                          mode, alloc_hdr, verbose, minlen);
        } else {
            _ct_echo_sync(unit, dest_key, str, depth,
                          mode, alloc_hdr, verbose, minlen, count);
        }
        parse_arg_eq_done(&pt);
