/*
 * $Id: symtab.c 1.43.2.4 Broadcom SDK $
...
hash_func(char *s)
{
    ...
    while ((c = *s++) != 0)
	h = (h << 3) ^ h ^ (h >> 7) ^ (islower(c) ? toupper(c) : (c));
...
_parse_array(char *name, char **out1, char **out2)
{
    ...
    if ((idx1 = strchr(name, '[')) != 0) {
        rv = 1;
        *idx1++ = 0;
        if ((tmpchr = strchr(idx1, ']')) == 0) {
            debugk(DK_WARN, "Could not parse index in %s.\n", name);
        } else {
            *tmpchr = 0;
        }
        if ((idx2 = strchr(idx1, '-')) != 0) {
            *idx2++ = 0;
        }
    } else if ((idx1 = strchr(name, '(')) != 0) {
        rv = 1;
        *idx1++ = 0;
        if ((tmpchr = strchr(idx1, ')')) == 0) {
            debugk(DK_WARN, "Could not parse index in %s.\n", name);
...
parse_symbolic_reference(int unit, soc_regaddrlist_t *alist, char *ref)
{
    ...
    int numels, maxidx, minidx, idxfound = 0;
    char pfmt[SOC_PBMP_FMT_LEN];
    symtab_entry_t  *et;
    int idx = 0, match = 0, max_block=0;
#if defined(BCM_ROBO_SUPPORT)
    soc_regaddrinfo_t *a = NULL;
#endif
    
    debugk(DK_SYMTAB, "parsing: %s\n", ref);
    ...
    if ((range1 = strchr(rname, '.')) != 0) {
	*range1++ = '\0';
        idxfound = _parse_array(range1, &idx1, &idx2);
	if ((range2 = strchr(range1, '.')) != 0) {
	    *range2++ = '\0';
	    if (*range2 == '\0') {
		range2 = 0;
	    }
    ...
    if (!SOC_REG_IS_VALID(unit, reg)) {
        debugk(DK_WARN, "Register %s %d is not valid for chip %s\n",
    ...
    if (!SOC_REG_IS_ENABLED(unit, reg)) {
        if (SOC_IS_ROBO(unit)){
#ifdef BCM_ROBO_SUPPORT
            debugk(DK_WARN, "Register %s is not enabled for this configuration\n",
    ...
    bmask = sal_alloc(sizeof(uint32) * max_block, 
                      "parse_bmask");
    if (NULL == bmask){
        debugk(DK_WARN, "Fail to allocate bmask\n");
        return -1;
    }
    ...
    if (diag_parse_range(idx1, idx2, &minidx, &maxidx,
        ...
        debugk(DK_WARN, "Register %s: bad index specification\n", ref);
        goto _return_error;
    }
    ...
    while (regblktype[idx] != SOC_BLOCK_TYPE_INVALID) {
        soc_cm_debug(DK_SYMTAB, "%d ", regblktype[idx]);
        idx++;
    }
    soc_cm_debug(DK_SYMTAB, " ]\n");
