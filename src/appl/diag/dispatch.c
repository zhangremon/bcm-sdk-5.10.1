/*
 * $Id: dispatch.c 1.23.124.1 Broadcom SDK $
...
int
cmd_dispatch_custom_filt(int unit, bcm_port_t port, int setget, int type,
                         uint32 args[BCM_CUSTOM_ARGS_MAX])
{
    ...
    rv = BCM_E_NONE;
    for (codep = 0; codep < 64; codep++) {
        rv = bcm_filter_create(unit, &f);
        if (rv < 0) {
            (void)bcm_filter_destroy(unit, f);
            return rv;
        }
        rv = bcm_filter_qualify_data16(unit, f, 12, 0x8100, 0xffff);
        if (rv < 0) {
            (void)bcm_filter_destroy(unit, f);
            return rv;
        }
        rv = bcm_filter_qualify_data16(unit, f, 16, 0x0800, 0xffff);
        if (rv < 0) {
            (void)bcm_filter_destroy(unit, f);
            return rv;
        }
        rv = bcm_filter_qualify_data16(unit, f, 18, 0x4500 | codep, 0xffff);
        if (rv < 0) {
            (void)bcm_filter_destroy(unit, f);
            return rv;
        }
        ...
        rv = bcm_filter_qualify_ingress(unit, f, pbmp);
        if (rv < 0) {
            (void)bcm_filter_destroy(unit, f);
            return rv;
        }
        rv = bcm_filter_action_match(unit, f, bcmActionSetPrio, args[codep]);
        if (rv < 0) {
            (void)bcm_filter_destroy(unit, f);
            return rv;
        }
        rv = bcm_filter_reinstall(unit, f);
        if (rv < 0) {
            rv = bcm_filter_install(unit, f);
            if (rv < 0) {
                (void)bcm_filter_destroy(unit, f);
                return rv;
            }
        }
        rv = bcm_filter_destroy(unit, f);
        if (rv < 0) {
            return rv;
        }
    }
...
int
custom_unit_vlan_vector_get(int unit, uint32 args[BCM_CUSTOM_ARGS_MAX])
{
    int rv = BCM_E_NONE;
    int i, vid, count = 0;
    ...
    sal_memset(args, 0, sizeof(uint32)*BCM_CUSTOM_ARGS_MAX);    
    args[0] = count; /* store the number of active VLANS in word0 */
    for (i = 0; i < count; i++) {
        vid = list[i].vlan_tag;
        args[vid/32 + 1] |=  1 << (vid % 32);
    }
 ...
 */
int
custom_handler(int unit, bcm_port_t port, int setget, int type,
               uint32 args[BCM_CUSTOM_ARGS_MAX])
{
    int     rv, argi, s;
    uint64  val;
    ...
    case CUSTOM_HANDLER_VLAN_GET: /* gets vlan vector for a unit */
        rv = custom_unit_vlan_vector_get(unit, args);
        break;
                ...
                int dosync, uint64 stats[snmpValCount])
{
    ...
    rv = bcm_custom_port_get(unit, port, 
                             dosync ? CUSTOM_HANDLER_STAT_SYNC_GET \
                             : CUSTOM_HANDLER_STAT_GET, args);
    if (rv >= 0) {
        argi = 0;
        for (s = 0; s < snmpValCount; s++) {
                 ...
                 int dosync, uint64 stats[snmpValCount])
{
    ...
    rv = bcmx_custom_port_get(port, dosync ? 2 : 1, args);
    if (rv >= 0) {
        argi = 0;
        for (s = 0; s < snmpValCount; s++) {
            COMPILER_64_SET(stats[s], args[argi], args[argi+1]);
...
cmd_dispatch(int unit, args_t *args)
{
    ...
    /* dispatch ftest N */
    if (sal_strcasecmp(subcmd, "ftest") == 0) {
        int p, count, codep;
        bcm_filterid_t f;
        bcm_pbmp_t pbmp;
        uint32 btime, etime;
        ...
        for (codep = 0; codep < 64; codep++) {
            for (p = 0; p < 12; p++) {
                rv = bcm_filter_create(u, &f);
                if (rv < 0) {
                    sal_printf("ERROR: filter_create(%d): %s\n",
                               u, bcm_errmsg(rv));
                    break;
                }
                rv = bcm_filter_qualify_data16(u, f, 12, 0x8100, 0xffff);
                if (rv < 0) {
                    sal_printf("ERROR: filter_qual_data16(%d, 12): %s\n",
                               u, bcm_errmsg(rv));
                    break;
                }
                rv = bcm_filter_qualify_data16(u, f, 16, 0x0800, 0xffff);
                if (rv < 0) {
                    sal_printf("ERROR: filter_qual_data16(%d, 16): %s\n",
                               u, bcm_errmsg(rv));
                    break;
                }
                rv = bcm_filter_qualify_data16(u, f, 18, 0x4500 | codep,
                                               0xffff);
                if (rv < 0) {
                    sal_printf("ERROR: filter_qual_data16(%d, 18, %d): %s\n",
                               u, codep, bcm_errmsg(rv));
                    break;
                }
                BCM_PBMP_CLEAR(pbmp);
                BCM_PBMP_PORT_ADD(pbmp, p);
                rv = bcm_filter_qualify_ingress(u, f, pbmp);
                if (rv < 0) {
                    sal_printf("ERROR: filter_qual_ingress(%d, %d): %s\n",
                               u, p, bcm_errmsg(rv));
                    break;
                }
                rv = bcm_filter_action_match(u, f, bcmActionSetPrio, 1);
                /* cosq */
                if (rv < 0) {
                    sal_printf("ERROR: filter_action_match(%d): %s\n",
                               u, bcm_errmsg(rv));
                    break;
                }
                rv = bcm_filter_install(u, f);
                if (rv < 0) {
                    sal_printf("ERROR: filter_install(%d): %s\n",
                               u, bcm_errmsg(rv));
                    break;
                }
                rv = bcm_filter_destroy(u, f);
                if (rv < 0) {
                    sal_printf("ERROR: filter_destroy(%d): %s\n",
                               u, bcm_errmsg(rv));
                    break;
                }
                count += 1;
        ...
        etime = sal_time_usecs();
        sal_printf("%d port filters in %u usec (%d usec/call)\n",
                   count, etime-btime, (etime-btime)/count);
        if (rv < 0) {
            return CMD_FAIL;
        }
    ...
    /* dispatch ftest2 N */
    if (sal_strcasecmp(subcmd, "ftest2") == 0) {
        int p, count, codep;
        bcm_filterid_t f;
        bcm_pbmp_t pbmp;
        uint32 btime, etime;
    ...
    /* dispatch ftest3setup N */
    if (sal_strcasecmp(subcmd, "ftest3setup") == 0) {
        s = ARG_GET(args);
        if (s == NULL) {
            sal_printf("ERROR: %s %s: unit missing\n", cmd, subcmd);
            return CMD_FAIL;
        ...
        u = parse_integer(s);
        rv = bcm_custom_register(u, cmd_dispatch_custom_filt);
        if (rv < 0) {
            sal_printf("ERROR: %s %s: %s\n", cmd, subcmd, bcm_errmsg(rv));
            return CMD_FAIL;
        }
    ...
    /* dispatch ftest3 N */
    if (sal_strcasecmp(subcmd, "ftest3") == 0) {
        int p, count, codep;
        uint32 btime, etime;
        uint32 custom_args[BCM_CUSTOM_ARGS_MAX];
        ...
        sal_memset(custom_args, 0, sizeof(custom_args));
        for (codep = 0; codep < 64; codep++) {
            custom_args[codep] = 1;    /* cosq */
        }
        count = 0;
        rv = BCM_E_NONE;
        for (p = 0; p < 12; p++) {
            rv = bcm_custom_port_set(u, p, 0, custom_args);
            if (rv < 0) {
                sal_printf("ERROR: custom_port_set(%d,%d): %s\n",
                           u, p, bcm_errmsg(rv));
                break;
            }
            count += 1;
        ...
        etime = sal_time_usecs();
        sal_printf("%d port customs in %u usec (%d usec/call)\n",
                   count, etime-btime, (etime-btime)/count);
        if (rv < 0) {
            return CMD_FAIL;
        }
    ...
    if (sal_strcasecmp(subcmd, "custom_reg") == 0) {
        ...
        u = parse_integer(s);
        rv = bcm_custom_register(u, custom_handler);
        if (rv < 0) {
            sal_printf("ERROR: %s %s: %s\n", cmd, subcmd, bcm_errmsg(rv));
            return CMD_FAIL;
        }
