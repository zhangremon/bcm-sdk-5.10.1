/*
 * $Id: util.c 1.145.2.5 Broadcom SDK $
...
parse_macaddr(char *str, sal_mac_addr_t macaddr)
{
    ...
    } else if (str[0] == '0' && tolower((int)str[1]) == 'x') {
...
parse_pbmp(int unit, char *s, soc_pbmp_t *pbmp)
{
    ...
    int         matched, blk, blktype, is_ci = 0;
    char        *sn, *se;
    ...
    while (*s) {
        ...
        case '\0':
            ...
            for (port = pfirst; port <= plast; port += pstep) {  /* a range */
                ...
                if ((is_ci) || (SOC_PORT_VALID(unit, p))) {
                    if (complement) {
...
dump_l2_addr(int unit, char *pfx, bcm_l2_addr_t *l2addr)
{
    ...
    printk("%smac=%02x:%02x:%02x:%02x:%02x:%02x vlan=%d",
           pfx,
    ...
    if (BCM_GPORT_IS_SET(l2addr->port)) {
        ...
        case _SHR_GPORT_TYPE_MPLS_PORT:
            ...
            break;
        case _SHR_GPORT_TYPE_WLAN_PORT:
            printk(" port=%d(wlan)", l2addr->port);
            break;
        case _SHR_GPORT_TYPE_MIM_PORT:
            printk(" port=%d(mim)", l2addr->port);
            break;
        case _SHR_GPORT_TYPE_TRILL_PORT:
            printk(" port=%d(trill)", l2addr->port);
            break;
        case _SHR_GPORT_TYPE_SUBPORT_GROUP:
            printk(" port=%d(subport_group)", l2addr->port);
            break;
        case _SHR_GPORT_TYPE_SUBPORT_PORT:
            printk(" port=%d(subport_port)", l2addr->port);
            break;
        case _SHR_GPORT_TYPE_TRUNK:
                  ...
                  soc_pbmp_t pall)
{
    ...
    if (SOC_PBMP_EQ(bm, pall)) {        /* all ports of this type */
        sal_sprintf(dest + strlen(dest), ",%s", pname);
        return;
    }
    ...
    DPORT_SOC_PBMP_ITER(unit, pall, dport, tport) {        /* a subset of ports */
        ...
        if (pfirst >= 0) {
            sal_sprintf(dest + strlen(dest), ",%s%d", pname, pfirst);
            if (pfirst != plast) {
                sal_sprintf(dest + strlen(dest), "-%s%d", pname, plast);
            }
            pfirst = plast = -1;
        }
    }
...
format_pbmp(int unit, char *buf, int bufsize, soc_pbmp_t pbmp)
{
    ...
    if (SOC_PBMP_NOT_NULL(tpbm)) {
        strncpy(buf, SOC_PBMP_FMT(pbmp, tmp), bufsize);
...
bcm_port_name(int unit, int port)
{
    ...
    return SOC_PORT_NAME(unit, port);
}
...
STATIC int
_parse_block(int unit, char **s, int *nump)
{
    char        *sblk, *snum;
    int         blk, num;
    int         i, phy_port;
                  ...
                  soc_block_types_t mask)
{
    ...
    *max = SOC_INFO(unit).block_num;
    ...
    nummin = -1;
    if ((*min = _parse_block(unit, &s, &nummin)) < 0) {
        return -1;
    }
    ...
    nummax = -1;
    if ((*max = _parse_block(unit, &s, &nummax)) < 0) {
        return -1;
    }
    if (*s != '\0') {
        return -1;
...
int
parse_memory_name(int unit, soc_mem_t *result, char *str, int *copyno)
{
    soc_mem_t   mem;
    ...
    if ((cp = strchr(tmp, '.')) != NULL) {
        ...
        *cp++ = '\0';
        blk = _parse_block(unit, &cp, &numblk);
        if (blk < 0 || *cp != '\0') {
            return -1;
        }
    }
...
parse_field_qualifier(char *qual_str) 
{
    ...
    for (qual = 0; qual < bcmFieldQualifyCount; qual++) {
        ...
        if(!sal_strcasecmp(cannonical_str + strlen("bcmFieldQualify"),
                           qual_str)) {
            break;
        }
    }
...
parse_field_qset(char *str, bcm_field_qset_t *qset) {
    ...
    strncpy(str_copy, str, input_len + 1);

    buf = strtok(str_copy, delimiters);
...
format_field_qset(char *buf, bcm_field_qset_t qset, char *separator)
{
    ...
    for (qual = 0; qual < bcmFieldQualifyCount; qual++) {
        if(BCM_FIELD_QSET_TEST(qset, qual)) {
            ...
            strncat(buf, buf_local, strlen(buf_local));
        }
    }
    strncat(buf, "}", strlen("}"));
...
parse_field_group_mode(char *act_str) 
{
    ...
    for (mode = 0; mode < bcmFieldGroupModeCount; mode++) {
        ...
        /* Test for the suffix only */
        if(!sal_strcasecmp(cannonical_str + strlen("bcmFieldGroupMode"),
                           act_str)) {
            break;
        }
    }
...
parse_field_action(char *act_str) 
{
    ...
    for (action = 0; action < bcmFieldActionCount; action++) {
        ...
        /* Test for the suffix only */
        if(!sal_strcasecmp(cannonical_str + strlen("bcmFieldAction"),
                           act_str)) {
            break;
        }
    }
...
{
    char *action_text[bcmFieldActionCount + 1] = BCM_FIELD_ACTION_STRINGS;
...
parse_field_decap(char *str)
{
    ...
    for (decap = 0; decap < bcmFieldDecapCount; decap++) {
        ...
        /* Test for the suffix only */
        strncpy(tbl_str, decap_text[decap], FP_DECAP_STR_SZ - 1);
        ...
        strncat(lng_str, tbl_str,
                FP_DECAP_STR_SZ - 1 - strlen("bcmFieldDecap"));
...
parse_switch_control(char *str)
{
    ...
    for (control = 0; control < bcmSwitch__Count; control++) {
        ...
        /* Test for the suffix only */
        strncpy(tbl_str, control_text[control], SWITCH_CONTROL_STR_SZ - 1);
        if (!sal_strcasecmp(tbl_str, str)) {
            break;
        }
        /* Test for whole name of the action */
        sal_strcpy(lng_str, SWITCH_CONTROL_PREFIX);
        strncat(lng_str, tbl_str,
                SWITCH_CONTROL_STR_SZ - 1 - strlen(SWITCH_CONTROL_PREFIX));
        if (!sal_strcasecmp(lng_str, str)) {
            break;
        }
    }
...
parse_tunnel_type(char *str)
{
    ...
    for (control = 0; control < (int)bcmSwitch__Count; control++) {
        sal_memset(tbl_str, 0, sizeof(char) * TUNNEL_TYPE_STR_SZ);
        sal_memset(lng_str, 0, sizeof(char) * TUNNEL_TYPE_STR_SZ);
        /* Test for the suffix only */
        strncpy(tbl_str, control_text[control], TUNNEL_TYPE_STR_SZ - 1);
        if (!sal_strcasecmp(tbl_str, str)) {
            break;
        }
        /* Test for whole name of the action */
         ...
         */ 
        /*    coverity[buffer_size]    */
        strncpy(lng_str, TUNNEL_TYPE_PREFIX,
                strlen(TUNNEL_TYPE_PREFIX) + 1);
        strncat(lng_str, tbl_str,
                TUNNEL_TYPE_STR_SZ - 1 - strlen(TUNNEL_TYPE_PREFIX));
        if (!sal_strcasecmp(lng_str, str)) {
            break;
        }
    }
    ...
    *ability = 0;
    ...
    while (*s != 0) {
        for (i = 0; lr_pa_map[i].str != NULL; i++) {
            int len = strlen(lr_pa_map[i].str);
...
format_phy_control_longreach_ability(char *buf, int bufsize, soc_phy_control_longreach_ability_t ability)
{
    ...
    for (i = 0; ability != 0 && lr_pa_map[i].str != NULL; i++) {
        if ((ability & lr_pa_map[i].pa) == lr_pa_map[i].pa) {
            ...
            sal_strcpy(buf, lr_pa_map[i].str);
...
bcm_pkt_dump(int unit, bcm_pkt_t *pkt, int dump_data)
{
    ...
    for (i = 0; i < pkt->blk_count; i++) {
        sal_sprintf(&line[offset], "%s(0x%p, %d)", nl ? "" : ", ",
                    pkt->pkt_data[i].data, pkt->pkt_data[i].len);
        offset = sal_strlen(line);
        if (!((i+1) % 4)) {
            soc_cm_print("    %s\n", line);
            nl = 1;
