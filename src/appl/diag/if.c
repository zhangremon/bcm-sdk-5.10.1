/*
 * $Id: if.c 1.71.32.1 Broadcom SDK $
 ...
 */
{
    debugk(DK_END, "%s: Stopping %s %d %08x \n", pfx, if_name, if_unit, (uint32)eo);
    if (OK != muxDevStop(eo)) {
    ...
    debugk(DK_END, "%s: Clearing Routes\n", pfx);
 ...
 */
{
    ...
    debugk(DK_END, "%s: sal_if_deconfig complete: %s%d (%d)\n", 
 ...
 */
{
...
#if VX_VERSION == 64 || VX_VERSION == 65
#else
    M2_INTERFACETBL	m2;
#endif

    SAL_MAP_NETUNIT(if_unit);
    debugk(DK_END, "%s: sal_if_config *** if_name=%s if_unit=%d\n", 
	   pfx, if_name, if_unit);
	...
	debugk(DK_END, "%s: muxDevStart successful: 0x%x\n", pfx, (int)eo);
    }
#if VX_VERSION == 64 || VX_VERSION == 65
#else
    /*
