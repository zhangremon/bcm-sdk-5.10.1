...
char board_cmd_usage[] =
    ...
    "        If no board name is provided, probed all registered board\n"
    ...
    "    list\n"
    "        Show a list of all registered boards and attributes\n"
    "        supported by each board.\n"
    "    show\n"
    "        Show the current board state and configuration name,\n"
    ...
    "    clear\n"
    "        Clear devices on current board and preserves the current board\n"
    "        configuration.\n"
#endif
    ;
...
/* Decimal number stringbuffer length */
#define STRBUF 16
...
board_cmd(int unit, args_t *args)
{
    ...
    if (sal_strncasecmp(subcmd, "show", sublen) == 0) {
        ...
        if (builtin != NULL &&
            sal_strncasecmp(builtin, "builtin", strlen(builtin)) == 0) {
            ARG_NEXT(args);
