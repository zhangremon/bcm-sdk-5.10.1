...
#define _DISC_DEBUG(flags, stuff) TKS_DEBUG(TKS_DBG_DISCOVER | (flags), stuff)
#define DISC_OUT(stuff) _DISC_DEBUG(0, stuff)
#define DISC_VERB(stuff) _DISC_DEBUG(TKS_DBG_VERBOSE, stuff)
#define DISC_ERR(stuff) _DISC_DEBUG(TKS_DBG_ERR, stuff)
#define DISC_WARN(stuff) _DISC_DEBUG(TKS_DBG_WARN, stuff)
#define DISC_VVERB(stuff) _DISC_DEBUG(TKS_DBG_DISCV, stuff)
#else
#define DISC_OUT(stuff)
#define DISC_ERR(stuff)
#define DISC_WARN(stuff)
#define DISC_VERB(stuff)
#define DISC_VVERB(stuff)
...
int _check_start_values(cpudb_ref_t db_ref)
{
    ...
    if (!cpudb_valid(db_ref)) {
        DISC_ERR(("disc ERR:  Bad DB reference\n"));
        return BCM_E_PARAM;
    }

    if (db_ref->local_entry == NULL) {
        DISC_ERR(("disc ERR:  Can't find local DB entry\n"));
        return BCM_E_PARAM;
    }

    if (disc_retrx_min_us >= disc_retrx_us) {
        DISC_ERR(("disc ERR:  retrx mis-configuration. min %d max %d\n",
    ...
    if (db_ref->local_entry->base.num_units < 0 ||
        ...
        DISC_WARN(("disc WARN: Bad local DB info\n"));
    }

    if (disc_ttl_min == 1 && disc_ttl_max == 1) {
        DISC_WARN(("DISC WARN: TTL min == max == 1.  Discovery will fail\n"));
    }
...
disc_abort(int disc_rv, int timeout_us)
{
    ...
    DISC_VERB(("DISC abort: RV %d, to %d\n", disc_rv,
               timeout_us));
    ...
    if (!(disc_flags & DF_DISC_RUNNING)) {
        DISC_UNLOCK;
        DISC_VERB(("DISC abort: not running\n"));
        return BCM_E_NONE;
    }
    DISC_UNLOCK;
    ...
    if (timeout_us > 0) {  /* Block until restart noted */
        ...
        if (!(disc_flags & DF_ABORT_ACK)) {
            DISC_WARN(("DISC WARN: Discovery did not exit in "
                       "%d us\n", timeout_us));
            rv = BCM_E_FAIL;
        }
    }
...
disc_prep(cpudb_ref_t db_ref)
{
    ...
    for (i = 0; i < ROUTE_PKTS_MAX; i++) {
        if (route_pkt_buf[i] == NULL) {
            ...
            if (route_pkt_buf[i] == NULL) {
                DISC_WARN(("disc WARN: Route pkt alloc %d\n", i));
                return BCM_E_MEMORY;
            }
        }
    }
    ...
    if (CPUDB_ENTRY_COUNT_GET(db_ref) > 1) {
        DISC_WARN(("disc WARN: Local DB has > 1 entry\n"));
    }
#endif /* BROADCOM_DEBUG */
    ...
    if (!next_hop_running()) {
        ...
        if (BCM_FAILURE(rv)) {
            DISC_WARN(("disc WARN: Error starting next hop\n"));
...
STATIC void probe_pkt_forward(cpudb_ref_t db_ref,
                                     uint8 *start_buf, uint8 *pkt_buf,
                                     int entry_count, int rx_sp_idx);
...
routing_pkt_process(cpudb_ref_t db_ref, disc_pkt_t *routing_pkt)
{
    ...
    if (entry->base.dseq_num != src_dseq_num) {
        DISC_VERB(("disc: Rte src dseq num mismatch; pkt %d, db %d.\n",
    ...
    for (i = 0; i < entry_count; i++) {
        ...
        DISC_VVERB(("disc: Rte entry %p key " CPUDB_KEY_FMT_EOLN,
    ...
    if (dt < 0 || dt > disc_cfg_to_us) {
        ...
        for (stk_idx = 0; stk_idx < lentry->base.num_stk_ports;
            ...
            if (PP_SEEN_GET(stk_idx) == 0) {
                ...
                DISC_VERB(("DISC: Marking SP %d inactive\n", stk_idx));
            }
        }
...
lc_atp_setup(cpudb_ref_t db_ref)
{
    ...
    if (BCM_FAILURE(rv)) {
        DISC_WARN(("disc WARN: could not register cfg client\n"));
    }
...
{
    int rv;
    DISC_VERB(("Disc: Master is set to " CPUDB_KEY_FMT " w/ dseq num %d\n",
    ...
    if (db_ref->master_entry == db_ref->local_entry) {
        ...
        if (BCM_FAILURE(rv)) {
            DISC_ERR(("Disc ERR: cfg send returns %d: %s\n",
                        rv, bcm_errmsg(rv)));
            DISC_LOCK;
            disc_flags |= DF_DISC_ERROR;
            DISC_UNLOCK;
        } else {
            DISC_VERB(("Disc: master cfg pkts out sn %d\n",
                       db_ref->master_entry->base.dseq_num));
       }
    }
}
...
local_complete_set(cpudb_ref_t db_ref)
{
    ...
    DISC_VERB(("Disc: Local DB complete\n"));
...
global_complete_set(cpudb_ref_t db_ref)
{
    ...
    DISC_VERB(("Disc: All DB entries report complete\n"));

    if (db_ref->master_entry != NULL) {
        /* Assume this means that we're not the master. */
        DISC_VERB(("DISC: Global done, but master known.\n"));
    }
    ...
    if (rv != BCM_E_NONE) {
        DISC_VERB(("Disc: Master elect returns %d causing abort: %s\n",
        ...
        DISC_VERB(("Disc WARN:  Global done; no master\n"));
    } else {
...
{       
    ...
    if ((disc_flags & DF_SEND_CFG_PKT) &&
        ...
        rv = disc_config_send(db_ref);
        if (BCM_FAILURE(rv)) {
            DISC_ERR(("DISC:  Failed to respond with cfg pkt: %s\n",
                      bcm_errmsg(rv)));
        } else {
    ...
    if (!(LOCAL_FLAGS(db_ref) & CPUDB_F_GLOBAL_COMPLETE)) {
        ...
        DISC_VERB(("DISC:  All DB entries accept config.\n"));
        flags |= DF_DISC_SUCCESS;
        DISC_LOCK;
        disc_flags |= flags;
        DISC_UNLOCK;
                ...
                int remote_sp_idx, uint32 flags)
{
    if (!(STK_FLAGS(db_ref, sp) & CPUDB_SPF_RX_RESOLVED)) {
        ...
        DISC_VERB(("disc: Set RX for sp %d\n", sp));
    }
                ...
                int remote_sp_idx, uint32 flags)
{
    if (!(STK_FLAGS(db_ref, sp) & CPUDB_SPF_TX_RESOLVED)) {
        ...
        DISC_UNLOCK;
        CHECK_IS_DUPLEX(db_ref, sp);
        DISC_VERB(("disc: Set TX for sp %d\n", sp));
    }
...
disc_probe_pkt_new(cpudb_ref_t db_ref, cpudb_key_t src_key, uint8 *pkt_buf)
{
    ...
    for (i = 0; i < entry_count; i++) {
        ...
        if (entry == NULL) {
            DISC_VERB(("disc probe chk: New key "
                        CPUDB_KEY_FMT_EOLN, 
            ...
            if (dseq_num - entry->base.dseq_num > 0) {
                ...
                DISC_VERB(("disc probe chk: New SN %d for key "
                            CPUDB_KEY_FMT_EOLN, dseq_num,
...
probe_pkt_local_cpu_key(cpudb_ref_t db_ref, uint8 *buf, int entry_count)
{
    ...
    for (i = 0; i < entry_count; i++) {
        ...
        if (!CPUDB_KEY_COMPARE(key, LOCAL_KEY(db_ref))) {
            DISC_VVERB(("disc: Local key in probe pkt, %d\n", i));
            return i;
        }
    ...
    DISC_VVERB(("disc: Local key not in probe pkt\n"));
    return -1;
}
...
probe_pkt_process(cpudb_ref_t db_ref, disc_pkt_t *probe_pkt)
{
    ...
    /* data_start points to start of disc packet; pkt_start to start of pkt */
    pkt_start = probe_pkt->pkt_buf;
    data_start = pkt_start + CPUTRANS_HEADER_BYTES;
    ...
    entry_buf = data_start + PROBE_ENTRY_OFS(last_entry);
    CPUDB_KEY_UNPACK(entry_buf, key);
    ...
    rx_sp_idx = stk_port_find(db_ref, probe_pkt->rx_unit,
                              probe_pkt->rx_port);
    if (rx_sp_idx < 0) {
        DISC_ERR(("disc ERR: Could not find stk port for "
    ...
    if (!(STK_FLAGS(db_ref, rx_sp_idx) & CPUDB_SPF_RX_RESOLVED)) {
        ...
        if (remote_ent == NULL) {
            DISC_ERR(("disc ERR: Error adding KEY to DB\n"));
            ++disc_rx_pkt_err;
            ++disc_tot_err;
            _DROP_PKT;
            return;
    ...
    DISC_VVERB(("disc: Prb pkt Rmt TX %d RX %d key "
               CPUDB_KEY_FMT " cnt %d\n", remote_tx_idx,
    ...
    if (entry_count > disc_ttl) {
        ...
        DISC_VERB(("disc: probe entry count %d > %d - forcing forwarding\n",
    ...
    case PROBE_PKT_MEMORY:
        DISC_VERB(("disc: Could not add probe entry\n"));
        break;
    default:
    ...
    local_idx = probe_pkt_local_cpu_key(db_ref,
                                        &data_start[ENTRY_START_OFS],
                                        entry_count);

    if (!DISC_EXIT && !drop_pkt) {  /* Default behavior */
        if (local_idx < 0) {
            probe_pkt_forward(db_ref, pkt_start, data_start, entry_count,
                              rx_sp_idx);
        } else {
            /* Local CPU key is in packet; analyze it */
            probe_pkt_with_local_key(db_ref, probe_pkt, data_start,
         ...
         * dropped.
         */
        uint8 *ttl_ptr = &data_start[DISC_PROBE_TTL_OFS];
        ...
        probe_pkt_forward(db_ref, pkt_start, data_start, entry_count,
                          rx_sp_idx);
        DISC_VVERB(("Restart discovery, forward probe from "
                 ...
                 int version)
{
    ...
    if (new_entry || ((flags | new_flags) != flags)) { /* Unpack */
        ...
        disc_flags |= DF_DB_UPDATED;
        DISC_UNLOCK;
        DISC_VVERB(("disc:  Stk update flags %x\n", new_flags));
    }
}
...
_check_route_pkt(uint8 *buf, cpudb_entry_t *entry)
{
    ...
    if (entry->base.num_stk_ports > 0) {  /* Have seen before */
        ...
        if (entry->base.num_stk_ports != stk_port_count) {
            DISC_ERR(("disc ERR: stk port count mismatch for %x:%x\n",
                       entry->base.mac[4], entry->base.mac[5]));
            DISC_ERR(("    was %d. new %d\n",
...
route_entry_process(uint8 *buf, cpudb_entry_t *entry, int version)
{
    ...
    if (!(entry->flags & CPUDB_F_BASE_INIT_DONE)) {
        DISC_VVERB(("disc: Base init key " CPUDB_KEY_FMT_EOLN,
                   CPUDB_KEY_DISP(entry->base.key)));
        /* Init base info */
        ...
        if (dseq_num > entry->base.dseq_num) {
            DISC_VERB(("disc: new info for " CPUDB_KEY_FMT " seq %d->%d\n",
                       CPUDB_KEY_DISP(entry->base.key),
        ...
        if (num_units > CPUDB_UNITS_MAX) {
            DISC_ERR(("disc ERR: Rte unit count %d exceeds DB limit %d\n",
        ...
        if (num_stk_ports > CPUDB_STK_PORTS_MAX) {
            DISC_ERR(("disc ERR: Rte stk port count %d exceeds DB limit %d\n",
        ...
        if (stale) {
            ...
            DISC_VERB(("disc: stale info for " CPUDB_KEY_FMT " seq %d->%d\n",
                       CPUDB_KEY_DISP(entry->base.key),
                       entry->base.dseq_num, dseq_num));
        ...
        for (i = 0; i < entry->base.num_units; i++) {
            ...
            DISC_VVERB(("disc:  Rte unit %d. mr %d. pref %d\n",
        ...
        if (combined_flags != entry->flags) {
            DISC_VVERB(("disc:  Rte flag update %x->%x\n",
                ...
                int len, cpudb_ref_t db_ref)
{
    ...
    case DISC_PKT_TYPE_PROBE:          /* Probe packet */
        ...
        if (stack_port_active_mark(db_ref, unit, port) < 0) {
            DISC_VERB(("disc: Inactive Stack port %d %d now active.\n",
                       unit, port));
            DISC_UNLOCK;
            DISC_WAKE;
        ...
        if (enqueue_probe_pkt(src_key, unit, port, pkt_buf, len) < 0) {
            DISC_VERB(("disc: Discarding probe pkt\n"));
            DISC_UNLOCK;
            return BCM_RX_HANDLED;
        }
        break;
    case DISC_PKT_TYPE_ROUTING:        /* Routing packet */
        if (enqueue_routing_pkt(src_key, unit, port, pkt_buf, len) < 0) {
            DISC_VERB(("disc: Discarding routing pkt\n"));
            DISC_UNLOCK;
            return BCM_RX_HANDLED;
        }
        break;
    default:
        DISC_WARN(("disc WARN: Unsupported packet type %d", pkt_type));
        ++disc_tot_err;
        DISC_UNLOCK;
            ...
            int len, void *cookie)
{
    ...
    if (!cpudb_valid(db_ref)) {
        DISC_UNLOCK;
        DISC_VERB(("disc: Config pkt in when db_ref is invalid.\n"));
        return BCM_RX_NOT_HANDLED;
    }
    ...
    if (disc_fallback && (disc_version > d_ver)) {
        DISC_WARN(("DISC WARN: Protocol version changed from %d to %d\n",
                   disc_version, d_ver));
        disc_version = d_ver;
        DISC_UNLOCK;
        return BCM_RX_HANDLED;
    } else if (d_ver != disc_version && version_warn) {
        DISC_WARN(("DISC WARN: Received discovery version %d, sending %d\n",
                   d_ver, disc_version));
        version_warn = FALSE;
    }
    ...
    DISC_UNLOCK;
    DISC_VERB(("disc: Unknown discovery version %d in probe/route\n",
               d_ver));
    return BCM_RX_NOT_HANDLED;
}
...
probe_pkt_tx(cpudb_ref_t db_ref, uint8 *buf, int len, uint8 *cur_entry)
{
    ...
    for (i = 0; i < db_ref->local_entry->base.num_stk_ports; i++) {
        ...
        DISC_VVERB(("DISC:  Sending probe pkt cos %d to port %d (%d, %d)\n",
                    disc_cos, i, STKP_UNIT(db_ref, i), STKP_PORT(db_ref, i)));
        ...
        if (BCM_FAILURE(rv)) {
            ++disc_tx_pkt_err;
            DISC_ERR(("disc ERR %d: probe pkt tx to "
                        "stkport %d: %s\n", rv, i, bcm_errmsg(rv)));
            break;
        }
    }
    ...
    for (i = 0; i < entry_count; i++) {
        ...
        if (entry == NULL) {
            ...
            if (diff > 0) {  /* pkt SN is newer than local DB */
                DISC_VERB(("disc: New seq num %d for key "
                            CPUDB_KEY_FMT_EOLN, dseq_num,
                            CPUDB_KEY_DISP(entry->base.key)));
                return PROBE_PKT_DB_CLEAR;
            } else if (diff < 0) { /* pkt SN is older than DB */
                DISC_VERB(("disc: Old seq num. DB SN %d. Pkt SN %d. key "
                           CPUDB_KEY_FMT_EOLN, entry->base.dseq_num,
...
STATIC void
probe_pkt_forward(cpudb_ref_t db_ref, uint8 *pkt_start, uint8 *data_start,
                  int entry_count, int rx_sp_idx)
{
    uint8 *local_entry;
    int tx_len;
    ...
    ttl_rsvd = data_start[DISC_PROBE_TTL_OFS];
    switch (ttl_rsvd) {
    case 0:  /* Ignore TTL */
        break;
    case 1:  /* TTL expired, do not forward */
        DISC_VVERB(("DISC: Not forwarding probe pkt due to TTL == 1\n"));
        return;
    default: /* Decrement TTL and forward */
        data_start[DISC_PROBE_TTL_OFS] = ttl_rsvd - 1;
        break;
    }

    local_entry = &data_start[PROBE_ENTRY_OFS(entry_count++)];
    ...
    data_start[ENTRY_COUNT_OFS] = (uint8)entry_count;
    ...
    DISC_VVERB(("disc: Prb pkt out, ent %d len %d\n",
               entry_count, tx_len));
    probe_pkt_tx(db_ref, pkt_start, tx_len, local_entry);
}
                                          ...
                                          that is, >= local entry */
{
    ...
    if (entry == NULL) {
        ++disc_rx_pkt_err;
        DISC_ERR(("disc ERR: bad key, off board 0\n"));
        return;
    }
    ...
    for (i = 1; i < new_entries; i++) {  /* One done above */
        ...
        entry = _disc_key_resolve(db_ref, key, &buf[PROBE_MAC_OFS], dseq_num);
        if (entry == NULL) {
            DISC_ERR(("disc ERR: bad key, off board %d\n", i));
            ++disc_rx_pkt_err;
            return;
        }
        buf += PROBE_ENTRY_BYTES;  /* Next entry */
...
probe_pkt_with_local_key(cpudb_ref_t db_ref,
                         ...
                         uint8 *pkt_buf,  /* Start of probe pkt */
                         ...
                         int entry_count) /* How many entries in pkt */
{
    ...
    buf = &pkt_buf[PROBE_ENTRY_OFS(local_idx)];
    ...
    DISC_VVERB(("disc: Prb pkt local. RX %d. TX %d, ent cnt %d\n",
               rx_sp_idx, tx_sp_idx, entry_count));

#if defined(BROADCOM_DEBUG)
    if (tx_sp_idx < 0 || tx_sp_idx > db_ref->local_entry->base.num_stk_ports) {
        DISC_ERR(("disc ERR: Did not find stk port in probe pkt"));
        return;
    }
    if (rx_sp_idx < 0) {
        DISC_ERR(("disc ERR: Did not find rx stk port of probe pkt"));
        return;
    }
#endif /* BROADCOM_DEBUG */
...
route_pkt_tx(cpudb_ref_t db_ref, uint8 *buf, int len)
{
    ...
    for (i = 0; i < db_ref->local_entry->base.num_stk_ports; i++) {
        ...
        DISC_VVERB(("disc rte pkt out len %d to %d @ " CPUDB_KEY_FMT_EOLN,
        ...
        if (BCM_FAILURE(rv)) {
            ++disc_tx_pkt_err;
            DISC_ERR(("disc ERR: sending rte to stkport %d\n", i));
            break;
        }
        if (DISC_EXIT) {
            break;
...
routing_pkts_form(cpudb_ref_t db_ref)
{
    ...
    CPUDB_FOREACH_ENTRY(db_ref, entry) {
        ...
        if ((entry_bytes > pkt_bytes_left) && (num_entries == 0)) {
            DISC_ERR(("disc ERR: route pkt entry too long, need %d, left %d\n",
        ...
        if ((num_entries == 0) || (entry_bytes > pkt_bytes_left) ||
            ...
            if ((++cur_pkt_idx) >= ROUTE_PKTS_MAX) {
                DISC_ERR(("disc ERR: number of route pkts exceeds limit %d\n",
                          ROUTE_PKTS_MAX));
                ++disc_internal_err;
                ++disc_tot_err;
                return -1;
        ...
        if (entry_bytes < 0) {
            DISC_ERR(("disc ERR: cannot pack route entry %d in packet %d\n",
        ...
        DISC_VVERB(("disc: Packing route entry %d in packet %d, "
                    "buffer left %d\n",
    ...
    DISC_VVERB(("disc: Packed %d entries in %d pkts\n",
...
routing_pkts_send(cpudb_ref_t db_ref)
{
    ...
    if (!(disc_flags & DF_TX_KNOWN)) {
        DISC_VVERB(("disc: No route out; no TX known\n"));
        return;
    }
                  ...
                  int dseq_num)
{
    ...
    if (entry == NULL) {
        disc_external_err++;
        ++disc_tot_err;
        DISC_ERR(("disc ERR: Error adding key\n"));
        return NULL;
    }
    sal_memcpy(entry->base.mac, mac, sizeof(bcm_mac_t));
    db_entry_clear_mod_ids(entry);
    entry->base.dseq_num = dseq_num;
    DISC_VVERB(("disc: Added key " CPUDB_KEY_FMT
               ", mac %x:%x seq %d to DB\n", CPUDB_KEY_DISP(key),
                        ...
                        cpudb_ref_t db_ref)
{
    ...
    DISC_VERB(("disc: Config pkt in from " CPUDB_KEY_FMT " disc SN %d\n",
               CPUDB_KEY_DISP(src_key), src_dseq_num));
    DISC_VERB(("disc: Cfg pkt master key " CPUDB_KEY_FMT " disc SN %d\n",
    ...
    if (entry == NULL) {
        ++disc_rx_pkt_err;
        ++disc_tot_err;
        DISC_ERR(("disc ERR: Config pkt, bad source key "
                  CPUDB_KEY_FMT_EOLN,
                  CPUDB_KEY_DISP(src_key)));
        DISC_UNLOCK;
        return BCM_RX_HANDLED;
    ...
    if (entry->base.dseq_num != src_dseq_num) {
        ++disc_rx_pkt_err;
        ++disc_tot_err;
        DISC_WARN(("disc warn: Config pkt from " CPUDB_KEY_FMT
    ...
    if (db_ref->local_entry == db_ref->master_entry) {
        if (!CPUDB_KEY_EQUAL(master_key, db_ref->local_entry->base.key)) {
            DISC_UNLOCK;
            DISC_WARN(("disc ERR: Config pkt from " CPUDB_KEY_FMT
        ...
        if (db_ref->master_entry != NULL &&
            ...
            DISC_WARN(("disc WARN: Config pkt w/ different master; "
                       "overwriting\n"));
        }
        db_ref->master_entry = entry;
                        ...
                        void *cookie)
{
    ...
    if (!cpudb_valid(db_ref) || (db_ref->local_entry == NULL)) {
        DISC_UNLOCK;
        DISC_WARN(("disc: Config pkt in when db_ref is invalid.\n"));
        return BCM_RX_NOT_HANDLED;
    }

    if (!(db_ref->local_entry->flags & CPUDB_F_LOCAL_COMPLETE)) {
        DISC_UNLOCK;
        DISC_WARN(("disc: Config pkt in when local DB not complete\n"));
        return BCM_RX_NOT_HANDLED;
    }
    ...
    /* Unknown version - throw packet away */
    DISC_UNLOCK;
    DISC_WARN(("disc: Unknown discovery version %d in config\n",
               d_ver));
    return BCM_RX_HANDLED;
}
...
disc_config_send(cpudb_ref_t db_ref)
{
    ...
    DISC_VERB(("DISC:  Generating config packet(s)\n"));

    if (!cpudb_valid(db_ref) || (db_ref->master_entry == NULL)) {
        DISC_WARN(("disc WARN: cfg send; master is NULL\n"));
        return BCM_E_PARAM;
    }

    if (!(LOCAL_FLAGS(db_ref) & CPUDB_F_LOCAL_COMPLETE)) {
        DISC_WARN(("disc WARN: cfg send, but not local complete\n"));
    }
    ...
    if (db_ref->local_entry != db_ref->master_entry) {
        ...
        rv = atp_tx(db_ref->master_entry->base.key,
                    ...
                    NULL);
        if (BCM_FAILURE(rv)) {
            DISC_ERR(("disc ERR:  %s (%d) Failed to tx cfg pkt to "
                      CPUDB_KEY_FMT_EOLN, bcm_errmsg(rv), rv,
                      CPUDB_KEY_DISP(db_ref->master_entry->base.key)));
        }
    } else {
        CPUDB_FOREACH_ENTRY(db_ref, entry) {
            ...
            DISC_VVERB(("disc: Cfg pkt out to " CPUDB_KEY_FMT_EOLN,
                        CPUDB_KEY_DISP(entry->base.key)));
            rv = atp_tx(entry->base.key,
                        ...
                        NULL);
            if (BCM_FAILURE(rv)) {
                DISC_ERR(("disc ERR:  %s (%d) Failed to tx pkt to "
                          CPUDB_KEY_FMT_EOLN,
                          bcm_errmsg(rv), rv,
                          CPUDB_KEY_DISP(entry->base.key)));
                break;
...
disc_m_elect_default(cpudb_ref_t db_ref, void *user_data)
{
    ...
    if (db_ref == CPUDB_REF_NULL || db_ref->local_entry == NULL) {
        DISC_ERR(("disc m_elect ERR:  Bad DB or local entry\n"));
        return BCM_E_FAIL;
    }
    ...
    if (db_ref->master_entry != NULL && db_ref->master_entry != best_entry) {
        DISC_WARN(("DISC m_elect:  Master set, but changing\n"));
    }

    db_ref->master_entry = best_entry;
    best_entry->flags |= CPUDB_F_IS_MASTER;
    DISC_VERB(("disc m_elect: Master is "
...
{
    DISC_OUT(("disc_alloc_fail    = %d\n", disc_alloc_fail));
    DISC_OUT(("disc_resource_err  = %d\n", disc_resource_err));
    DISC_OUT(("disc_internal_err  = %d\n", disc_internal_err));
    DISC_OUT(("disc_tx_pkt_err    = %d\n", disc_tx_pkt_err));
    DISC_OUT(("disc_rx_pkt_err    = %d\n", disc_rx_pkt_err));
    DISC_OUT(("disc_tot_err       = %d\n", disc_tot_err));
    DISC_OUT(("disc_external_err  = %d\n", disc_external_err));
}
#endif /* BROADCOM_DEBUG */
...
_disc_run_prep(cpudb_ref_t db_ref)
{
    ...
    if (disc_flags & DF_DISC_RUNNING) {
        DISC_UNLOCK;
        DISC_VERB(("_disc_run_prep: terminating existing disc\n"));
        if (disc_abort(BCM_E_FAIL, 500000) != BCM_E_NONE) {
            DISC_VERB(("%s: disc_abort failed.\n",
                       FUNCTION_NAME()));
            return BCM_E_BUSY;
        }
        DISC_LOCK;
    ...
    if (disc_trans_ptr == NULL) {
        DISC_UNLOCK;
        DISC_ERR(("_disc_run ERR: Need transport pointers\n"));
        return BCM_E_FAIL;
    }

    if (db_ref == NULL) {
        DISC_UNLOCK;
        DISC_ERR(("%s ERR: Need DB ref pointers\n", FUNCTION_NAME()));
        return BCM_E_PARAM;
    }
...
_disc_run_reg(cpudb_ref_t *db_refp)
{
    ...
    if (BCM_FAILURE(rv)) {
        DISC_ERR(("disc ERR: Could not register RX callback"));
        DISC_LOCK;
        disc_flags |= DF_DISC_ERROR;
        DISC_UNLOCK;
    } else {
        DISC_VERB(("disc: registered with nexthop\n"));
    }
    return rv;
}
...
{
    DISC_VERB(("disc: unregistered with nexthop\n"));
...
disc_run_task(void)
{
    ...
    if ((_disc_tid = sal_thread_create("bcmDISC",
                                       ...
                                       NULL))
        == SAL_THREAD_ERROR) {
        DISC_ERR(("disc ERR: Could not create discovery thread\n"));
        rv = BCM_E_FAIL;
    }
    return rv;
}
