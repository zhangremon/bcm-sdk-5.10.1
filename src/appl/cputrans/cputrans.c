/*
 * $Id: cputrans.c 1.18 Broadcom SDK $
...
#define CT_DEBUG(stuff)        TKS_DEBUG(TKS_DBG_CTPKT, stuff)
#define CT_VERB(stuff)        TKS_DEBUG(TKS_DBG_CTPKT | TKS_DBG_VERBOSE, stuff)
#else
#define CT_DEBUG(stuff)
#define CT_VERB(stuff)
                           ...
                           uint32 ct_flags, int *num_segs)
{
    ...
    if (first_pkt->next == first_pkt) {
        CT_DEBUG(("CT pkt list alloc: Internal corruption in pointers\n"));
        CPUTRANS_ERROR_INCR;
    }
...
cputrans_tx_pkt_list_free(bcm_pkt_t *pkt)
{
    ...
    if (pkt == NULL) {
        CT_DEBUG(("CT pkt list free: Packet NULL\n"));
        return;
    }
    if (pkt->next == pkt) {
        CT_DEBUG(("CT pkt list free: Circular list\n"));
        pkt->next = NULL;
        pkt->_last_pkt = pkt;
        CPUTRANS_ERROR_INCR;
    }
    if (pkt->_last_pkt == NULL) {
        CT_DEBUG(("CT pkt list free: No last pkt\n"));
        pkt->_last_pkt = pkt;
        CPUTRANS_ERROR_INCR;
    }
    ...
    for (t_pkt = pkt; t_pkt != NULL; t_pkt = t_pkt->next) {
        if (count++ > tx_pool_size) {
            CT_DEBUG(("CT pkt list free: List too long\n"));
            CPUTRANS_ERROR_INCR;
            return;
        }
    }
...
cputrans_rx_pkt_free(bcm_pkt_t *pkt)
{
    ...
    if (pkt == NULL) {
        CT_DEBUG(("CT free: Packet NULL\n"));
        return;
    }

    if (!_rx_setup_done) {
        CT_DEBUG(("CT free: Not initialized\n"));
        return;
    }

    idx = PTR_TO_INT(pkt->cookie2);
    if (idx >= _rx_lists) {
        CT_DEBUG(("CT free: bad CT index: %d > %d\n", idx, _rx_lists));
        return;
    }
                     ...
                     uint32 flags)
{
    ...
    for (unit = 0; unit < BCM_CONTROL_MAX; unit++) {
        if (BCM_UNIT_VALID(unit) && BCM_IS_LOCAL(unit)) {
            ...
            tmp_rv = cputrans_rx_unit_register(unit, name, callback,
                                               priority, cookie, flags);
            if (tmp_rv < 0) {
                CT_VERB(("CPU Trans reg failed %d, unit %d: %s\n",
    ...
    if (!found) { /* Call transport pointers on unit -1 */
        tmp_rv = cputrans_rx_unit_register(-1, name, callback, priority,
                                           cookie, flags);
        if (tmp_rv < 0) {
            CT_VERB(("CPU Trans reg failed %d, dflt unit: %s\n",
                      rv, bcm_errmsg(rv)));
            rv = tmp_rv;
        }
    }
