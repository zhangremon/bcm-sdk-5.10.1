...
STATIC int _acl_range_nodes_combine(acl_node_t *first, acl_node_t *second,
                                    uint16 bit);
...
bcma_acl_init(void) 
{
    ...
    COMPILER_REFERENCE(retval_temp);    
...
#endif /* BCM_FIELD_SUPPORT */
#ifdef BCM_FILTER_SUPPORT
    retval_temp = _acl_filter_detach();
    ACL_RETVAL_REPLACE(retval_temp, retval);
#endif /* BCM_FILTER_SUPPORT */
...
#ifdef BCM_FILTER_SUPPORT
    /*
     * Since the filter module needs to know if packets are either IPv4 or
     * IPv6, in order to qualify on L4 ports, the rule must have enough
     * information to resolve IP version.
     */
...
#endif /* BCM_FIELD_SUPPORT */
#ifdef BCM_FILTER_SUPPORT
            retval_temp = _acl_filter_rule_remove(rule_id);
            ACL_RETVAL_REPLACE(retval_temp, retval);
#endif /* BCM_FILTER_SUPPORT */
...
#endif /* BCM_FIELD_SUPPORT */
#ifdef BCM_FILTER_SUPPORT
    retval_temp = _acl_filter_merge(_acl_control);
    ACL_RETVAL_REPLACE(retval_temp, retval);
#endif /* BCM_FILTER_SUPPORT */
...
#endif /* BCM_FIELD_SUPPORT */
#ifdef BCM_FILTER_SUPPORT
    retval_temp = _acl_filter_install();
    ACL_RETVAL_REPLACE(retval_temp, retval);
#endif /* BCM_FILTER_SUPPORT */
...
#endif /* BCM_FIELD_SUPPORT */
#ifdef BCM_FILTER_SUPPORT
    retval_temp = _acl_filter_uninstall();
    ACL_RETVAL_REPLACE(retval_temp, retval);
#endif /* BCM_FILTER_SUPPORT */
...
/*
 * Function:
 *     _acl_range_nodes_combine
 *
 * Purpose:
 *     Test to range nodes to see if they can be combined. If so, combine them
 *     and deallocate second node.
 *
 * Parameters:
 *
 * Returns:
 *     BCM_E_NONE - Successfully combined nodes
 *     BCM_E_FAIL - Did not combine nodes
 */
STATIC int
_acl_range_nodes_combine(acl_node_t *first, acl_node_t *second, uint16 bit) 
{
    uint16              data_first, data_second;
    ...
    ACL_VVERB(
       ("ACL _acl_range_nodes_combine({first->data=%#x, first->mask=%#x},"
        "{second->data=%#x, second->mask=%#x})\n", first->data, first->mask,
        second->data, second->mask ));

    /* Masks much match to be combined. */
    if (first->mask != second->mask) {
        return BCM_E_FAIL;
    }

    /* Mask first and second values. */
    data_first  = first->data  & first->mask  & ~bit;
    data_second = second->data & second->mask & ~bit;

    /* If the masked values match, they can be combined. */
    if (data_first == data_second) {
        first->mask &= ~bit; /* Turn off bit in first node mask. */
        /* Move first node next pointer to the node following the second node.*/
        first->next = second->next;
        /* Free the unneeded second node. */
        sal_free(second);
    } else {
        return BCM_E_FAIL;
    }
