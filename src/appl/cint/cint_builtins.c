/*
 * $Id: cint_builtins.c 1.36.2.1 Broadcom SDK $
...
CINT_FWRAPPER_CREATE_VP0(cint_variable_dump); 
...
CINT_FWRAPPER_CREATE_VP0(cint_reset); 
...
CINT_FWRAPPER_CREATE_RP0(unsigned long, unsigned long, 0, 0, 
                         cint_timer_get); 

CINT_FWRAPPER_CREATE_RP1(long, long, 0, 0, 
                         cint_timer_start,
                         long, long, t0, 0, 0); 
...
typedef void (*cint_print_driver_f)(cint_print_dest_t *dst,
                                    const char *fmt,
                                    const void *src,
                           ...
                           cint_print_driver_cvt_t cvt)
{
    switch (cvt) {
    case cvt_integer_v:
        CINT_PRINTF(fmt, cint_integer_value((cint_variable_t *)src)); 
        break;
    case cvt_long_v:
        CINT_PRINTF(fmt, cint_long_value((cint_variable_t *)src)); 
        break;
    case cvt_longlong_v:
        CINT_PRINTF(fmt, cint_longlong_value((cint_variable_t *)src)); 
        break;
    case cvt_double_v:
        CINT_PRINTF(fmt, cint_double_value((cint_variable_t *)src)); 
        break;
    case cvt_ptr_string:
        CINT_PRINTF(fmt, *(char**)src); 
        break;
    case cvt_string:
        CINT_PRINTF(fmt, (char*)src); 
        break;
    case cvt_ptr_void:
        CINT_PRINTF(fmt, *(void**)src); 
        break;
    case cvt_fmt:
        CINT_PRINTF(fmt); 
        break;
                    ...
                    case 'o':
                        {
                            ...
                            if(flags & (CINT_ATOMIC_TYPE_FLAGS_INTEGRAL)) {
                                ...
                                if(larg == 0) {
                                    print(dst, buffer, v, cvt_integer_v); 
                                }
                                else if(larg == 1) {
                                    print(dst, buffer, v, cvt_long_v); 
                                }
                                else if(larg > 1){
                                    print(dst, buffer, v, cvt_longlong_v); 
                                }
                                _break = 1; 
                            }
                            else {
                    ...
                    case 'E':
                        {
                            ...
                            if(flags & CINT_ATOMIC_TYPE_FLAGS_ARITH) {
                                print(dst, buffer, v, cvt_double_v); 
                                _break = 1; 
                            }
                            else {
                    ...
                    case 's':
                        {
                            ...
                            if(v && !CINT_STRCMP(v->dt.desc.basetype, "char")) {
                                if(v->dt.desc.pcount == 1 && v->dt.desc.array == 0) {
                                    print(dst, buffer, v->data, cvt_ptr_string); 
                                }       
                                else if(v->dt.desc.pcount == 0 && v->dt.desc.array) {
                                    print(dst, buffer, v->data, cvt_string); 
                                }       
                                _break = 1; 
                                break; 
                            }
                    ...
                    case 'p':
                        {
                            if(!v || v->dt.desc.pcount == 0) {
                                ...
                                print(dst, buffer, v->data, cvt_ptr_void); 
                                _break = 1; 
                            }
                            break; 
                        }
...
cint_builtin_sprintf(cint_ast_t* ast)
{
    ...
    v = cint_eval_ast(arg);
    if (v == NULL) {
        cint_ast_error(ast, CINT_E_BAD_VARIABLE, "variable not found");
        return NULL;
    }

    if (CINT_STRCMP(v->dt.desc.basetype, "char") ||
        (v->dt.desc.pcount != 0) || (v->dt.desc.array <= 0)) {
        cint_ast_error(ast, CINT_E_BAD_VARIABLE, "expected char array");
        return NULL;
    }

    /* Setup destination buffers. The staging buffer is the same size as the
       output buffer for the worst case. */
    dst.len = v->dt.desc.array;
    dst.out = (char *)v->data;
