/*
 * $Id: cint_operators.c 1.32.2.1 Broadcom SDK $
...
__cint_operator_OpenBracket(cint_ast_t* ast, cint_operand_type_t optype, cint_variable_t* left, cint_variable_t* right)
{
    ...
    {   
        ...
        /* Create a new variable of the dereferenced type */
        desc = left->dt.desc; 
        desc.pcount--; 
        
        rc = cint_variable_create(&rv, NULL, &desc, CINT_VARIABLE_F_AUTO|CINT_VARIABLE_F_SDATA, (void*)0x1); 

        if (rc == CINT_E_NONE) {
            /* Replace the address */
            p = *(char**)left->data; 
            p += (rv->size*cint_integer_value(right)); 
            rv->data = p;
        }
        return rv;
    }
...
__cint_operator_Dereference(cint_ast_t* ast, cint_operand_type_t optype, cint_variable_t* left, cint_variable_t* right)
{
    if(right->dt.desc.pcount == 0) {
        ...
        rc = cint_variable_create(&rv, NULL, &desc, CINT_VARIABLE_F_AUTO|CINT_VARIABLE_F_SDATA, *(void**)right->data); 
