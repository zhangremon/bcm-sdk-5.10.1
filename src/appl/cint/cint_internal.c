/*
 * $Id: cint_internal.c 1.41.2.1 Broadcom SDK $
...
{
    return (v && (v->dt.flags & CINT_DATATYPE_F_ENUM && v->dt.desc.pcount == 0 && v->dt.desc.array == 0)); 
}
...
{
    return v && (v->dt.desc.pcount > 0 && v->dt.desc.array == 0); 
}
...
{    
    ...
    if(v->dt.desc.pcount || v->dt.desc.array) {
        return 0; 
    }   
...
cint_type_check(cint_datatype_t* dt1, cint_datatype_t* dt2)
{
    ...
    if(dt1->desc.array != dt2->desc.array) {
