/*
 * $Id: cint_eval_asts.c 1.46.2.2 Broadcom SDK $
                                     ...
                                     int *array_p)
{
    ...
    assert(ast->ntype == cintAstDeclaration);
    ...
    if(ast->utype.declaration.array) {
        rv = cint_eval_ast(ast->utype.declaration.array); 
        if(rv == NULL || (cint_is_integer(rv) == 0)) {
            return cint_ast_error(ast,
                                  CINT_E_BAD_AST,
                                  "illegal array definition"); 
        }
        array = cint_integer_value(rv); 
        if(array <= 0) {
...
{    
    ...
    if(cint_variable_find(desc.name, 1) != NULL) {
    ...
    if(rc < 0) {
        ...
        if(basetype_flags & CINT_AST_TYPE_F_CONST || (desc.array != 0))  {
    ...
    for(count = 0, p = ast->utype.function.parameters, pd = fp->params+1; 
        ...
        if(cint_type_check(&v->dt, &pdt) == 0) {
            ...
            if((!CINT_STRCMP(v->dt.desc.basetype, pdt.desc.basetype) ||
               ...
               v->dt.desc.array == pdt.desc.array) {
...
__cint_eval_ast_StructureDef(cint_ast_t* ast)
{
    ...
    cst->name = CINT_STRDUP(ast->utype.structuredef.name->utype.identifier.s); 
