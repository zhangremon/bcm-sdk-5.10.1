/*
 * $Id: eth_drv.c 1.11.6.4 Broadcom SDK $
...
{
    return et_drvTx(tx_pkt, pkt_len);
}

static int 
_eth_drv_get_mac(int unit, unsigned char *mac)
{
    return et_drv_mac_addr_get(mac);
}
...
_eth_drv_rx_thread(void *p)
{
    ...
    while (eth_control[unit]->started) {
        pkt_len = recvfrom(sock_fd, rx_pkt, 
           ETH_DRV_MAX_BUF_LEN, 0, (struct sockaddr *)&sl, &fromlen);
...
_eth_drv_handle_rx(int unit, void *pkt, int pkt_len)
{
    ...
    if (eth_control[unit] && eth_control[unit]->rx_cb) {
        rv = (eth_control[unit]->rx_cb(unit, 
             (unsigned char*) pkt, pkt_len) == 0) ?  TRUE : FALSE;
    }
    return;
}
