/*
 * $Id: etc_robo_spi.c 1.12.114.2 Broadcom SDK $
...
#ifdef ROBODVT /* Enable error message reported for DVT test env */
#ifdef ET_ERROR
#undef ET_ERROR
#endif
...
#define DIRECT_PHY_ACCESS_PAGE_1X    1 /* 5395,5397,5398,53118*/
#define DIRECT_PHY_ACCESS_PAGE_1X_2X 2 /* 5324 */
#define DIRECT_PHY_ACCESS_PAGE_1X_8X 3 /* 5389,5396,53115,53101,53125*/
#define DIRECT_PHY_ACCESS_PAGE_AX_DX 4 /* 5347,5348,53242,53262,53280,53600 */
...
void *robo_attach(void *sbh, uint32 ssl, uint32 clk, uint32 mosi, uint32 miso, uint8 ss);
...
void *
robo_attach(void *sih, uint32 ssl, uint32 clk, uint32 mosi, uint32 miso, uint8 ss)
{
    robo_info_t *robo;
    si_info_t *sii;
...
{    
    ...
    SPI_MSG(("%s: flags = 0x%x, val = 0x%x, reg_val = 0x%x\n", 
        __FUNCTION__, flags, val, reg_val));
...
chipc_spi_io_select(robo_info_t *robo, cc_spi_id_t id, int en) 
{
    ...
    SPI_MSG(("%s: %s SPI_%d is DONE, reg_val = 0x%x\n", 
            __FUNCTION__, (en) ? "Enable" : "Disable", id, reg_val));
    return SPI_ERR_NONE;
}
...
chipc_spi_io_intr(robo_info_t *robo, int en) 
{
    ...
    SPI_MSG(("%s: %s is DONE, reg_val = 0x%x\n", 
             __FUNCTION__, (en) ? "Enable" : "Disable", reg_val));
    return SPI_ERR_NONE;
}
...
_chipc_spi_wait_for_iflg_set(robo_info_t *robo) 
{
    ...
    SPI_MSG(("%s: %d, spireg_val=0x%x, retry=%d\n",
            __FUNCTION__, __LINE__, spireg_val, retry));
...
{
    robo_info_t *robo = (robo_info_t *)rinfo;
    if (((phyidh == BCM5395_PHYID_HIGH) && \
        ((phyidl & 0xFFF0) == BCM5395_PHYID_LOW)) || \
        ((phyidh == BCM5397_PHYID_HIGH) && \
        ((phyidl & 0xFFF0) == BCM5397_PHYID_LOW)) || \
        ((phyidh == BCM5398_PHYID_HIGH) && \
        ((phyidl & 0xFFF0) == BCM5398_PHYID_LOW)) || \
        ((phyidh == BCM53118_PHYID_HIGH) && \
        ((phyidl & 0xFFF0) == BCM53118_PHYID_LOW))) {
        robo->phytype = DIRECT_PHY_ACCESS_PAGE_1X;
    } else if ((phyidh == BCM5324_PHYID_HIGH) && \
               ((phyidl & 0xFFF0) == BCM5324_PHYID_LOW)){
        robo->phytype = DIRECT_PHY_ACCESS_PAGE_1X_2X;
    } else if (((phyidh == BCM5389_PHYID_HIGH) && \
                ((phyidl & 0xFFF0) == BCM5389_PHYID_LOW)) || \
                ((phyidh == BCM5396_PHYID_HIGH) && \
                ((phyidl & 0xFFF0) == BCM5396_PHYID_LOW)) || \
                ((phyidh == BCM53115_PHYID_HIGH) && \
        ...
        robo->phytype = DIRECT_PHY_ACCESS_PAGE_1X_8X;
    } else if (((phyidh == BCM5347_PHYID_HIGH) && \
                ((phyidl & 0xFFF0) == BCM5347_PHYID_LOW)) || \
                ((phyidh == BCM5348_PHYID_HIGH) && \
                ((phyidl & 0xFFF0) == BCM5348_PHYID_LOW)) || \
                ((phyidh == BCM53242_PHYID_HIGH) && \
        ...
        robo->phytype = DIRECT_PHY_ACCESS_PAGE_AX_DX;
    } else {
        robo->phytype = DIRECT_PHY_ACCESS_UNKNOWN;
    }
...
robo_phy_direct_set(void *rinfo, uint8 cid, uint8 page, uint8 addr, uint8 *buf, uint len)
{
    ...
    if (robo->phytype == DIRECT_PHY_ACCESS_PAGE_1X) {
    if (page>= MII_PAGE_1Xmin && page <= MII_PAGE_1Xmax) {

        cid = cid<<3 |(page - MII_PAGE_1Xoffset);
        chipphy_wr(robo->sbh, cid, addr>>1, mMdioBuf[0]);

        rv = TRUE;
    }
    } else if (robo->phytype == DIRECT_PHY_ACCESS_PAGE_1X_2X) {
    if ((page>= MII_PAGE_1Xmin && page <= MII_PAGE_1Xmax) ||
        (page>= MII_PAGE_2Xmin && page <= MII_PAGE_2Xmax)) {
    ...
    } else if (robo->phytype == DIRECT_PHY_ACCESS_PAGE_1X_8X) {
    if (page>= MII_PAGE_1Xmin && page <= MII_PAGE_1Xmax) {
    ...
    if (page>= MII_PAGE_8Xmin && page <= MII_PAGE_8Xmax) {
        ...
        cid = cid<<3 | (page - MII_PAGE_8Xoffset); 
        chipphy_wr(robo->sbh, cid, addr>>1, mMdioBuf[0]);

        rv = TRUE;
    }
    } else if (robo->phytype == DIRECT_PHY_ACCESS_PAGE_AX_DX) {
    if (page>= MII_PAGE_AXmin && page <= MII_PAGE_AXmax) {

        cid = cid<<3 | (page - MII_PAGE_AXoffset); 
        chipphy_wr(robo->sbh, cid, addr>>1, mMdioBuf[0]);
    ...
    if (page>= MII_PAGE_DXmin && page <= MII_PAGE_DXmax) {
        ...
        cid = cid<<3 | (page - MII_PAGE_DXoffset); 
        chipphy_wr(robo->sbh, cid, addr>>1, mMdioBuf[0]);
