...
#define SBCC_GPIO_OUT_ADDR \
        ((uint *)OSL_UNCACHED(SB_ENUM_BASE + OFFSETOF(chipcregs_t, gpioout)))
#define SBCC_GPIO_IN_ADDR  \
        ((uint *)OSL_UNCACHED(SB_ENUM_BASE + OFFSETOF(chipcregs_t, gpioin)))
...
sb_gpioout(void *sbh, uint32 mask, uint32 val)
{
    ...
    if ((SB_INFO(sbh))->gpioid == SB_CC) {
		...
		W_REG(SBCC_GPIO_OUT_ADDR, (R_REG(SBCC_GPIO_OUT_ADDR) & ~mask) | val);
...
sb_gpioin(void *sbh)
{
    ...
    if ((SB_INFO(sbh))->gpioid == SB_CC) {
		...
		return R_REG(SBCC_GPIO_IN_ADDR);
