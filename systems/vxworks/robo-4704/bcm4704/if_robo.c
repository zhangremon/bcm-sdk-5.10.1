...
robosw_attach_pmii(void)
{
	...
	printk("robosw_attach_pmii: out of memory");
	return NULL;
    }
	...
	printk("robosw_attach_pmii: failed to get eth0 device handle");
	return NULL;
    }
...
robosw_pmii_poll(robo_info_pmii_t *robo)
{
    ...
    if (timeout == 0) {
	printk("robosw_pmii_poll: timeout");
	return -1;
    }
