...
/* $Id: bootConfig.c 1.10.172.1 Broadcom SDK $
...
#ifdef CPU_SPEED_264MHZ
    do {
	extern void *sb_kattach(unsigned int chip, unsigned int chiprev);
	extern void sb_detach(void *sbh);
	extern int sb_mips_setclock(void *sbh, unsigned int mipsclock, unsigned int sbclock, unsigned int pciclock);
        void *sbh;

    	sbh = sb_kattach(0x4704, 0);
	sb_mips_setclock(sbh, 264000000, 0, 0);
    	sb_detach(sbh);
    } while (0);
#endif
...
LOCAL STATUS bootLoad 
    ...
    if (strncmp (params.bootDev, "flsh", 4) == 0)
        {
	...
	if ((strncmp (params.bootDev, "bp", 2) != 0) &&
        ...
        {
            unsigned char       enetAdrs[6];
/*
 * Takes a pointer, returns true if a 48-bit multicast/broadcast/null address
 */
#define ETHER_ISMULTI(ea) ((ea)[0] & 1)
#define ETHER_ISBCAST(ea) (((ea)[0] &                \
                            (ea)[1] &                \
                            (ea)[2] &                \
                            (ea)[3] &                \
                            (ea)[4] &                \
                            (ea)[5]) == 0xff)
#define ETHER_ISNULLADDR(ea) (((ea)[0] |             \
                            (ea)[1] |                \
                            (ea)[2] |                \
                            (ea)[3] |                \
                            (ea)[4] |                \
            ...
            sysNvRamGet(enetAdrs, 6, NV_OFF_MACADDR);
            if (ETHER_ISMULTI(enetAdrs) || ETHER_ISBCAST(enetAdrs) ||
                ETHER_ISNULLADDR(enetAdrs)) {
                printf("Invalid MAC Address %02x:%02x:%02x:%02x:%02x:%02x\n",
                    enetAdrs[0], enetAdrs[1], enetAdrs[2],
                    enetAdrs[3], enetAdrs[4], enetAdrs[5]);
                return (ERROR);
            }

#undef ETHER_ISMULTI
#undef ETHER_ISBCAST
#undef ETHER_ISNULLADDR
        }
