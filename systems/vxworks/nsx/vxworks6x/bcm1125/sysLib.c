...
/* $Id: sysLib.c 1.5.108.1 Broadcom SDK $
        ...
        unsigned int nEntry, int idx)
{
    ...
    for (i = idx ; i < (idx + nEntry); i++) {
        /* printk("setTlbs i:%d virt:%16llX phy:%16llX "
...
mem_map(unsigned int virt, unsigned int phy, unsigned int size)
{
    ...
    /*
    printk ("mem_map(virt=0x%x, phy=0x%x, size=%d)\n", virt, phy, size);
     */
    ...
    for (idx = low ; idx < (low + nEntry); idx++) {
        ...
        /* printk("Entry %2d  %016llX %016llx %016llx %08x\n",
