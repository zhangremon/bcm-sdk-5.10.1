...
#if 0
int
bcm_vlan_destroy(int unit, bcm_vlan_t vid)
{
    /* this is actually the same as creating, just zero entry */
    return bcm_vlan_create(unit,vid);
}

int
bcm_vlan_destroy_all(int unit)
{
    int i, retVal;

    for (i=0;i<=VLAN_ID_MAX;i++)
        if (BCM_RET_SUCCESS != (retVal = bcm_vlan_destroy(unit,i)))
            return retVal;
    return 0;
}
#endif
