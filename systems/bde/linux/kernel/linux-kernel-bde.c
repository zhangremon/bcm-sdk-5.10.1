/*
 * $Id: linux-kernel-bde.c 1.237.2.22 Broadcom SDK $
...
fixup_p2p_bridge(void)
{
    ...
    if ((dev = PCI_FIND_DEV(BCM4716_VENDOR_ID, BCM4716PCIE_DEVICE_ID, NULL)) != NULL ||
        ...
        uint32 tmp, maxpayld, fixup_bmp=0, mask;
        unsigned long addr;
        uint16 tmp16, tmp161;
         ...
         * Writes to the PCIE capability device control register
         */
        if ((dev = PCI_FIND_DEV(BCM53000_VENDOR_ID, 
                BCM53000PCIE_DEVICE_ID, NULL)) != NULL) {
            pci_write_config_dword(BCM53000PCIE_DEV(0), 0x0004, 
                    BCM53000PCIE_DEFAULT_STATUS);
        } 
                ...
                if (!(tmp16 & MAX_PAYLOAD_256B) || !(tmp16 & MAX_READ_REQ_256B)) {                         
                    ...
                    *((uint16 *)addr) = tmp161;                                                                                                          
                    if (debug >= 1) {
                        tmp16 = 0;                                                                             
                        tmp16 = *((uint16 *)addr);   
                        gprintk("addr %x Enable DevControl MaxPayloadSize to 1 (256 bytes): "
                                "0x%x (%s w/ 0x%x)\n", addr, tmp161,
     ...
     */
    dev = NULL;
    while ((dev = PCI_FIND_DEV(PCI_VENDOR_ID_PLX, PLX_PEX8608_DEV_ID, dev)) != NULL) {
        uint16 ctrl_reg;
...
_pci_probe(struct pci_dev *dev, const struct pci_device_id *ent)
{
    ...
    case BCM53000PCIE_DEVICE_ID:
        ...
        break;
        /*
         * The device ID for 56500, 56100, 56300 and 56600 family of devices
         * may be incorrect just after a PCI HW reset. It needs to be read
    ...
    if (plx_dev) {
        ...
        if (cap_base) {
            while (cap_base) {
                ...
                pci_write_config_word(dev, cap_base + 0x08, rval);
                break;
            }
        } else {
    ...
    ctrl->phys_address = pci_resource_start(dev, baroff);
     ...
     */
    if (gmac_base) {
        ctrl->base_address = (sal_vaddr_t)IOREMAP(gmac_base, 0x300000);

        /* Record the base address of GMAC core */
        ctrl->alt_base_addr = ctrl->base_address + (ctrl->phys_address - gmac_base);
...
#define FE2000_REVISION_OFFSET      (0x0)
    ...
    pci_read_config_dword(dev, PCI_BASE_ADDRESS_0, &obar);
    pci_write_config_dword(dev, PCI_BASE_ADDRESS_0, ~0);
    pci_read_config_dword(dev, PCI_BASE_ADDRESS_0, &bar);
    pci_write_config_dword(dev, PCI_BASE_ADDRESS_0, obar);
    if ((~(bar & ~0xf) + 1) == 0x40000) {
        ctrl->dev_type |= BDE_256K_REG_SPACE; /* CMICm */
    }
...
static struct pci_device_id _id_table[] = {
    ...
    { BROADCOM_VENDOR_ID, BCM56444_DEVICE_ID, PCI_ANY_ID, PCI_ANY_ID },
...
/* Optionally swap PHY IDs */
#ifdef BE_HOST
#define ROBO_DTOHS(_w)  _w = (_w >> 8) | (_w << 8);
#else
#define ROBO_DTOHS(_w)
#endif
...
probe_robo_switch(void)
{
    ...
    for (dev = 0; dev < LINUX_BDE_MAX_SWITCH_DEVICES; dev++) {
        ...
        if (robo == NULL) {
#ifdef KEYSTONE
            sbh = (void *)ai_soc_kattach(NULL);
#else
            sbh = (void *)sb_soc_kattach();
#endif
            if (sbh == NULL) {
                return -ENODEV;
            }
...
#ifdef KEYSTONE
            robo = (void *)robo_attach(sbh, (1<<1), (1<<0), (1<<2), (1<<3), 0);
#else
            robo = (void *)robo_attach(sbh, (1<<2), (1<<3), (1<<4), (1<<5));
#endif
        }
        if (robo == NULL) {
            return -ENODEV;
...
#ifdef MDC_MDIO_SUPPORT
        if (phyidh == BCM5348_PHYID_HIGH && phyidl_nr == BCM5348_PHYID_LOW) {
            robo_mdio_reg_read(0,0x388, &part_32, 1);
            part_8 = (part_32 & 0xFF);
            part_16 = (part_32 & 0xFFFF);
        } else if ((phyidh == BCM53242_PHYID_HIGH && phyidl_nr == BCM53242_PHYID_LOW) ||
                   (phyidh == BCM53262_PHYID_HIGH && phyidl_nr == BCM53262_PHYID_LOW)) {
                part_8 = 0;
                part_16 = part_8;
                rev = 0;
        } else {
            /* Register Model ID len = 4 for BCM53115, BCM53118 */
            if ((phyidh == BCM53115_PHYID_HIGH && phyidl_nr == BCM53115_PHYID_LOW) ||
                (phyidh == BCM53118_PHYID_HIGH && phyidl_nr == BCM53118_PHYID_LOW) ||
                (phyidh == BCM53101_PHYID_HIGH && phyidl_nr == BCM53101_PHYID_LOW) ||
                (phyidh == BCM53125_PHYID_HIGH && phyidl_nr == BCM53125_PHYID_LOW) ||
                (phyidh == BCM53128_PHYID_HIGH && phyidl_nr == BCM53128_PHYID_LOW)) {
                robo_mdio_reg_read(0,0x230, &part_32, 4);
                part_16 = (part_32 & 0x0000FFFF);
            } else if ((phyidh == BCM53280_PHYID_HIGH && phyidl_nr == BCM53280_PHYID_LOW) ||
                       (phyidh == BCM53600_PHYID_HIGH && phyidl_nr == BCM53600_PHYID_LOW)) {
        ...
        if (phyidh == BCM5348_PHYID_HIGH && phyidl_nr == BCM5348_PHYID_LOW) {
            ...
            part_16 = part_8;
                ...
                part_16 = part_8;
                rev = 0;
        } else if ((phyidh == BCM53115_PHYID_HIGH && phyidl_nr == BCM53115_PHYID_LOW) ||
                   (phyidh == BCM53118_PHYID_HIGH && phyidl_nr == BCM53118_PHYID_LOW) ||
                   (phyidh == BCM53101_PHYID_HIGH && phyidl_nr == BCM53101_PHYID_LOW) ||
                   (phyidh == BCM53125_PHYID_HIGH && phyidl_nr == BCM53125_PHYID_LOW) ||
                   (phyidh == BCM53128_PHYID_HIGH && phyidl_nr == BCM53128_PHYID_LOW)) {
        ...
        for (_ids = _spi_id_table; _ids->phyid_high && _ids->phyid_low; _ids++) {

            if (_ids->phyid_high == phyidh && _ids->phyid_low == phyidl_nr) {
                ...
                ctrl->bde_dev.device = phyidl_nr;
                ctrl->bde_dev.rev = phyidl & 0xF;
                if (phyidh == BCM5396_PHYID_HIGH && phyidl == BCM5396_PHYID_LOW) {
                    ctrl->bde_dev.device = part_16;
                    ctrl->bde_dev.rev = rev;
                }
                if (phyidh == BCM5398_PHYID_HIGH && phyidl == BCM5398_PHYID_LOW) {
                    ctrl->bde_dev.device = part_16;
                    ctrl->bde_dev.rev = rev;
                }
                if (phyidh == BCM5324_A1_PHYID_HIGH && phyidl == BCM5324_PHYID_LOW) {
                    ctrl->bde_dev.rev = 0x1;
                }
                if (phyidh == BCM5325_PHYID_HIGH && phyidl == BCM5325_PHYID_LOW) {
                    ctrl->bde_dev.rev = 0x1;
                }
                if (phyidh == BCM5348_PHYID_HIGH && phyidl_nr == BCM5348_PHYID_LOW) {
                    ctrl->bde_dev.device = part_16;
                }
                if ((phyidh == BCM53280_PHYID_HIGH && phyidl_nr == BCM53280_PHYID_LOW) ||
                    (phyidh == BCM53600_PHYID_HIGH && phyidl_nr == BCM53600_PHYID_LOW)) {
                    ctrl->bde_dev.device = part_16 | phyidl_nr;
                    part_8 = (uint8)(phyidl&0xF);
                    ctrl->bde_dev.rev =part_8 ;
                }
                if ((phyidh == BCM53128_PHYID_HIGH && phyidl_nr == BCM53128_PHYID_LOW) ||
                    (phyidh == BCM53125_PHYID_HIGH && phyidl_nr == BCM53125_PHYID_LOW) ||
                    (phyidh == BCM53101_PHYID_HIGH && phyidl_nr == BCM53101_PHYID_LOW)) {
                    ctrl->bde_dev.rev = rev;
                }
...
#if defined(KEYSTONE) && !defined(MDC_MDIO_SUPPORT)
        /* If uninitialized, set to highest supported SPI frequency, as of now 20MHz */
        if (spi_freq == 0) {
...
map_local_bus(uint64_t addr, uint32_t size)
{
    ...
    ctrl->base_address = (sal_vaddr_t)IOREMAP(addr, size);
    ctrl->bde_dev.base_address = ctrl->base_address;
...
probe_plx_local_bus(void)
{
    ...
    printk(KERN_ERR "Found PLX %04x:%04x vir: 0x%08x phy: 0x%s%08x\n",
           ...
           plx_ctrl.base_address, addr_hi_str,
...
probe_ea_dev(void)
{
    ...
    for (ea_uid = 0; ea_uid < eadevices; ea_uid++) {
            ctrl = _devices + _ndevices++;
            _switch_ndevices++;
            ctrl->dev_type = (BDE_ET_DEV_TYPE | BDE_SWITCH_DEV_TYPE);
...
_init(void)
{
     ...
     * (specifically the interrupt IOCTLs) the first device *must*
     * be a switch device. If this is not the case (due to the
     * probing order), we let the switch device(s) bubble up to
     * the top of the device array.
     */
    if (_switch_ndevices > 0) {
        bde_ctrl_t tmp_dev;
        int i;

        while ((_devices[0].dev_type & BDE_SWITCH_DEV_TYPE) == 0) {
            tmp_dev = _devices[0];
            for (i = 0; i < _ndevices - 1; i++) {
...
_isr(int irq, void *dev_id)
{
    ...
    if ((ctrl->bde_dev.device == BCM53000_GMAC_ID) && 
        (ctrl->dev_type & BDE_ETHER_DEV_TYPE)) {
        if ((regs = (gmac0regs_t *)ctrl->alt_base_addr) == NULL) {
            return IRQ_NONE;
        }
                   ...
                   void *isr_data)
{
    ...
    if (!(_devices[d].dev_type & (BDE_PCI_DEV_TYPE | BDE_ICS_DEV_TYPE |
                                   BDE_EB_DEV_TYPE))) {
...
_i2c_read(int d, uint16 addr, uint8 *buf, int len)
{
    ...
    if (!(_devices[d].dev_type & BDE_SPI_DEV_TYPE)) {
        gprintk("_i2c_read: Not SPI device %d, type %x\n",
                d, _devices[d].dev_type);
        return -1;
    }
...
_i2c_write(int d, uint16 addr, uint8 *buf, int len)
{
    ...
    if (!(_devices[d].dev_type & BDE_SPI_DEV_TYPE)) {
        gprintk("_i2c_write: Not SPI device %d, type %x\n",
                d, _devices[d].dev_type);
        return -1;
    }
...
lkbde_get_dev_virt(int d)
{
    ...
    return (void *)_devices[d].base_address;
}
