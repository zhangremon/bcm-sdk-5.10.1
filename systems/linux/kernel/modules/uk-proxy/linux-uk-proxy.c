...
/*
 * $Id: linux-uk-proxy.c 1.15 Broadcom SDK $
...
#define LDBG(l, str) gprintk("%d:%s:%s - %s\n", current->pid, l->service, l->name, str)
    ...
    if (down_interruptible(&source_list->count_s) < 0) {
        return -1; 
    }
    ...
    if (down_interruptible(&source_list->access_s) < 0) {
...
_linux_uk_proxy_send_to_user(const char *service, void *data, int *len)
{
    ...
    return _list_op(&s->send_q.data, &s->send_q.free, data, len, 
