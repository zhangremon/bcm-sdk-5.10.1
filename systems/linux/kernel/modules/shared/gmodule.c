/*
 * $Id: gmodule.c 1.11.118.3 Broadcom SDK $
...
gmodule_pprintf(char** page_ptr, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    return gmodule_vpprintf(page_ptr, fmt, args); 
}
...
{  
    va_list args;
    va_start(args, fmt);
    return gmodule_vpprintf(&_proc_buf, fmt, args); 
}
...
{
    if(_gmodule->compat_ioctl) {
	return _gmodule->compat_ioctl(cmd, arg);
    } else {
	return -1;
    }
}
