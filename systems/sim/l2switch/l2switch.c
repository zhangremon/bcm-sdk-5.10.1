/*
 * $Id: l2switch.c 1.3 Broadcom SDK $
...
_interrupt_connect(soc_cm_dev_t *dev, soc_cm_isr_func_t handler, void *data)
{
    ...
    debugk(DK_PCI,
	   "sysconf_probe: connecting INT%c# to IRQ %d\n",
...
_setup_pci(int unit)
{
    ...
    printk("PCI Discovery: found unit %d: %s "
    ...
    debugk(DK_PCI, "Set PCI MBAR 0x%x\n", PCI_SOC_MBAR(unit));
#endif
    ...
    debugk(DK_PCI, "Read back PCI MBAR 0x%x\n", soc->pciMemBaseAddr);
...
sysconf_probe(void)
{
    ...
    debugk(DK_PCI, "sysconf_probe: checking for SOC devices\n");

    if (_ndevices > 0) {
	debugk(DK_ERR, "sysconf_probe: probe already done\n");
    ...
    if (_ndevices == 0) {
	debugk(DK_ERR, "WARNING: No StrataSwitch unit found.\n");
	goto done;
    }
...
_sysconf_dump(soc_cm_dev_t *cookie)
{
    ...
    printk("PCI: Base=0x%x Bus=%d Dev=0x%x iPin=%d iLine=%d\n",
	   dev->pciMemBaseAddr,
...
# endif
    ...
    printk("WARNING: continuing: correct behavior no longer guaranteed!\n");
}
#endif
...
packet_process(int unit, bcm_pkt_t *pkt, void *cookie)
{
    ...
    printk("=== Packet received: port %d length %d ===\n",
	   pkt->rx_port, pkt->tot_len);

    p = BCM_PKT_IEEE(pkt);
    for (i = 0; i < pkt->tot_len; i++) {
	printk("%02x%s", p[i], (i % 16) == 15 ? "\n" : " ");
    }
    if ((pkt->tot_len % 16) != 0) {
	printk("\n");
    }
...
int main(int argc, char *argv[])
{
	...
	printk("SAL Initialization failed\n");
	exit(1);
    }
    ...
    printk("Example L2 Switching Application\n");
    printk("Version %s built %s\n", _build_release, _build_date);

    sysconf_init();

    if ((rv = sysconf_probe()) < 0) {
	printk("ERROR: PCI device probe failed\n");
	return -1;
    }

    unit = 0;

    printk("Attaching unit %d...\n", unit);

    if (sysconf_attach(unit) < 0) {
	printk("ERROR: PCI device attach failed\n");
	return -1;
    }

    printk("Initializing unit %d...\n", unit);

    if ((rv = system_init(unit)) < 0) {
	printk("ERROR: Driver initialization failed: %s\n", bcm_errmsg(rv));
	return -1;
    }

    printk("Initializing packet receiver...\n");

    rv = bcm_rx_start(unit, NULL);
    if (rv < 0) {
	printk("ERROR: Packet receiver not started: %s\n", bcm_errmsg(rv));
    }

    rv = bcm_rx_register(unit, "l2switch", packet_process,
			 BCM_RX_PRIO_MAX, (void *)0, BCM_RCO_F_ALL_COS);
    if (rv < 0) {
	printk("ERROR: Packet monitor registration failed: %s\n",
	       bcm_errmsg(rv));
    }

    printk("Switching started.\n");
