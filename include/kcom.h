...
#define KCOM_VERSION            1  /* Protocol version */
 ...
 * Once DMA is done a DMA event is returned to the application.
 *
 ...
 * Packet info types:
 *
 *  KCOM_DMA_INFO_T_TX_DCB_CHAIN
 *  Data is physical start address of Tx DCB chain.
 *
 *  KCOM_DMA_INFO_T_RX_DCB_CHAIN
 *  Data is physical start address of Rx DCB chain.
 *
 * Packet info flags:
 *
 *  KCOM_DMA_INFO_F_DONE
 *  This flag is set by the kernel module.
 *  For Tx packets this flag means that the packet has been sent.
 *  For Rx packets this flag means that the associated buffer contains
 *  valid packet data.
 *
 *  KCOM_DMA_INFO_F_ASYNC
 *  This flag is set by the application to request the Tx completion
 *  message to be sent as an asynchronous event.
 */
...
#define KCOM_DMA_INFO_F_BDE     (1U << 2)
