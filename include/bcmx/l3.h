...
/* Create a new L3 interface. */
extern int bcmx_l3_interface_create(
    bcm_mac_t mac_addr, 
    bcm_vlan_t vid, 
    int add_to_arl);

/* Create a new L3 interface with specified interface ID. */
extern int bcmx_l3_interface_id_create(
    bcm_mac_t mac_addr, 
    bcm_vlan_t vid, 
    int intf_idx, 
    int add_to_arl);
...
extern int bcmx_l3_interface_id_update(
    bcm_mac_t mac_addr, 
    bcm_vlan_t vid, 
    int intf_idx, 
    int add_to_arl);
...
extern int bcmx_l3_interface_lookup(
    bcm_mac_t mac_addr, 
    bcm_vlan_t vid, 
    int *intf_id);

/* Delete a L3 interface. */
extern int bcmx_l3_interface_destroy(
    int interface);

/* Given the L3 interface number, return the MAC and VLAN. */
extern int bcmx_l3_interface_find(
    int interface, 
    bcm_vlan_t *vid, 
    bcm_mac_t mac_addr);

/* Find the L3 interface by VLAN ID. */
extern int bcmx_l3_interface_find_by_vlan(
    bcm_vlan_t vid, 
    int *interface);

/* Lookup L3 host entry based on IP address. */
extern int bcmx_l3_ip_find(
    bcmx_l3_ip_t *info);

/* bcm_l3_ip_find_index */
extern int bcmx_l3_ip_find_index(
    int index, 
    bcmx_l3_ip_t *info);

/* Add an entry into L3 host table. */
extern int bcmx_l3_ip_add(
    bcmx_l3_ip_t *info);

/* Delete an entry from the L3 host table. */
extern int bcmx_l3_ip_delete(
    bcm_ip_t ip_addr);

/* Delete L3 host entries based on IP prefix (network). */
extern int bcmx_l3_ip_delete_by_prefix(
    bcm_ip_t ip_addr, 
    bcm_ip_t mask);

/* Delete L3 host entries that match L3 interface number. */
extern int bcmx_l3_ip_delete_by_interface(
    int intf);
 ...
 * unicast.
 */
extern int bcmx_l3_ip_update_entry_by_key(
    bcmx_l3_host_t *info);

/* Return list of conflicts in the L3 table. */
extern int bcmx_l3_conflict_get(
    bcm_l3_key_t *ipkey, 
    bcm_l3_key_t *cf_array, 
    int cf_max, 
    int *cf_count);
...
extern int bcmx_l3_invalidate_entry(
    bcm_ip_t l3_addr);

/* Invalidate L3 entry without clearing so it can be re-validated later. */
extern int bcmx_l3_validate_entry(
    bcm_ip_t l3_addr);

/* Run L3 table aging. */
extern int bcmx_l3_age(
    uint32 flags, 
    bcm_l3_age_cb age_out);

/* Return L3 status information. */
extern int bcmx_l3_status(
    int *free_l3intf, 
    int *free_l3, 
    int *free_defip, 
    int *free_lpm_blk);
...
extern int bcmx_l3_defip_add(
    bcmx_l3_route_t *info);

/* Delete an IP route from the route table. */
extern int bcmx_l3_defip_delete(
    bcmx_l3_route_t *info);

/* Given the net/prefix, return route info. */
extern int bcmx_l3_defip_get(
    bcmx_l3_route_t *info);

/* Delete routes based on matching L3 interface number. */
extern int bcmx_l3_defip_delete_by_interface(
    int intf);
...
/* Age the route table. */
extern int bcmx_l3_defip_age(
    bcm_l3_route_traverse_cb age_out);

/* Set the maximum ECMP paths allowed for a route (XGS only). */
extern int bcmx_l3_defip_max_ecmp_set(
    int max);

/* Get the maximum ECMP paths allowed for a route (XGS only). */
extern int bcmx_l3_defip_max_ecmp_get(
    int *max);
