/*
 * $Id: sbFabStatus.h 1.6 Broadcom SDK $
...
#define _BCM_FABRIC_LOG(flags, stuff) BCM_DEBUG(flags | BCM_DBG_FABRIC, stuff)

#define BCM_FABRIC_DEBUG(stuff) BCM_DEBUG(BCM_DBG_FABRIC, stuff)
#define BCM_FABRIC_DEBUG_WARN(stuff) _BCM_FABRIC_LOG(BCM_DBG_WARN, stuff)
#define BCM_FABRIC_DEBUG_ERR(stuff) _BCM_FABRIC_LOG(BCM_DBG_ERR, stuff)
#define BCM_FABRIC_DEBUG_VERB(stuff) _BCM_FABRIC_LOG(BCM_DBG_VERBOSE, stuff)

#ifdef SB_FAB_DEBUG_TRACE
#define SB_FAB_ENTER() _SB_FAB_LOGF(0, "Enter.\n");
#define SB_FAB_LEAVE() _SB_FAB_LOGF(0, "Leave.\n");
#define SB_FAB_VERIFY(x...) do{_SB_FAB_LOGF(0,"%d: Assert if 0x0 == 0x%08x (%s)\n",__LINE__,(uint32_t)(x), #x);if((x)==0){assert(0);}}while(0)
#else
#define SB_FAB_ENTER()
#define SB_FAB_LEAVE()
