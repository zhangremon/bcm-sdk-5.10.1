/*
 * $Id: sandHalFrameHwQe2000FMVT.h 1.2 Broadcom SDK $
...
#define SAND_ZF_HWQE2000FMVT_GET_PORT_MAP2(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint64_t) ((pFromData)[25] >> 5) & 0x07; \
           (nToData) |= (uint64_t) (pFromData)[26] << 3; \
           (nToData) |= (uint64_t) (pFromData)[27] << 11; \
           (nToData) |= (uint64_t) (pFromData)[28] << 19; \
           (nToData) |= (uint64_t) (pFromData)[29] << 27; \
           (nToData) |= (uint64_t) (pFromData)[30] << 35; \
           (nToData) |= (uint64_t) ((pFromData)[31] & 0x7f) << 43; \
          } while(0)
...
#define SAND_ZF_HWQE2000FMVT_GET_PORT_MAP1(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint64_t) (pFromData)[15] ; \
           (nToData) |= (uint64_t) (pFromData)[16] << 8; \
           (nToData) |= (uint64_t) (pFromData)[17] << 16; \
           (nToData) |= (uint64_t) (pFromData)[18] << 24; \
           (nToData) |= (uint64_t) (pFromData)[19] << 32; \
           (nToData) |= (uint64_t) (pFromData)[20] << 40; \
           (nToData) |= (uint64_t) ((pFromData)[21] & 0x03) << 48; \
          } while(0)
...
#define SAND_ZF_HWQE2000FMVT_GET_PORT_MAP0(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint64_t) ((pFromData)[4] >> 3) & 0x1f; \
           (nToData) |= (uint64_t) (pFromData)[5] << 5; \
           (nToData) |= (uint64_t) (pFromData)[6] << 13; \
           (nToData) |= (uint64_t) (pFromData)[7] << 21; \
           (nToData) |= (uint64_t) (pFromData)[8] << 29; \
           (nToData) |= (uint64_t) (pFromData)[9] << 37; \
           (nToData) |= (uint64_t) ((pFromData)[10] & 0x1f) << 45; \
          } while(0)
