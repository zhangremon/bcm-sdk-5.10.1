/*
 * $Id: sbx_drv.h 1.224.2.5 Broadcom SDK $
...
#define SOC_IS_SBX_SIRIUS(unit) \
        (SOC_INFO(unit).chip_type == SOC_INFO_SIRIUS)
#else
...
/**
 * Modid to Nodeid mapping
 */
#define BCM_MODULE_FABRIC_BASE  (10000)  /* base for node id */

#define BCM_STK_MOD_IS_NODE(modid) \
    (((modid) >= BCM_MODULE_FABRIC_BASE) &&  \
     ((modid) < (BCM_MODULE_FABRIC_BASE+BCM_STK_MAX_MODULES)))

#define BCM_STK_MOD_TO_NODE(modid) \
    ((modid) - BCM_MODULE_FABRIC_BASE)

#define BCM_STK_NODE_TO_MOD(nodeid) \
    ((nodeid) + BCM_MODULE_FABRIC_BASE)

#define SBX_GET_XBAR_MAP_SFI(unit, xbar, node) \
(SOC_SBX_STATE(unit)->fabric_state->xbar_map[xbar][node])

#define SBX_SET_XBAR_MAP_SFI(unit, xbar, node, port) \
(SOC_SBX_STATE(unit)->fabric_state->xbar_map[xbar][node] = port)
...
extern char* soc_sbx_fte_segment_names[];
#endif
...
typedef struct soc_sbx_sirius_config_s {
    ...
    int32              nMaxVoq;
    uint32             uSubportSpeed[SB_FAB_DEVICE_SIRIUS_NUM_HG_PORTS][SB_FAB_DEVICE_SIRIUS_MAX_PER_HG_SUBPORTS][SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_MAX][SB_FAB_DEVICE_SIRIUS_FIFO_GROUP_MAX_SIZE+1];
    ...
    uint32             type_res_flags[2][16];                                      /* information about frame parsers */
...
#else
#define SBX_QE_CPU_PORT        #error undefined
#endif /* BCM_QE2000_SUPPORT */
 ...
 */
#define SBX_EXC_QID_BASE(unit)    (SBX_UC_QID_BASE +                      \
                                  (SBX_QE_CPU_PORT * NUM_COS(unit)))
#define SBX_EXC_QID_END(unit)     (SBX_EXC_QID_BASE +                     \
                                  (SBX_MAX_EXC_QID * NUM_COS(unit)) - 1)

#ifdef BCM_QE2000_SUPPORT
#define SBX_QID_END              (SBX_MAX_QID - 1)
#else
#define SBX_QID_END              #error undefined
#endif /* BCM_QE2000_SUPPORT */
...
#define SBX_RAW_OHI_END          (SBX_RAW_OHI_BASE + 1)
...
#define SBX_MPLS_GPORT_OHI_BASE  (SBX_TRUNK_OHI_END + 1)
#define SBX_MPLS_GPORT_OHI_END   (SBX_MPLS_GPORT_OHI_BASE + SBX_MAX_GPORTS -1)
#define SBX_USER_OHI_BASE        (SBX_MPLS_GPORT_OHI_END + 1)
#define SBX_USER_OHI_END         (SBX_USER_OHI_BASE + SBX_MAX_USER_OHI - 1)
#define SBX_DYNAMIC_OHI_BASE     (SBX_USER_OHI_END + 1)
...
#define SOC_SBX_PORT_FTE(_unit, _node, _port) \
          (SBX_PORT_FTE_BASE(_unit) +                                        \
           ((_node) * SOC_SBX_CFG(unit)->nMaxFabricPortsOnModule) + (_port))
 ...
 * Externs
 */
#define SOC_SBX_NUM_SUPPORTED_CHIPS 10
...
#define SOC_SBX_WB_MODULE_COMMON    40
#define SOC_SBX_WB_MODULE_SOC    41
...
extern int soc_sbx_wb_common_state_sync(int unit, int sync);
#endif

extern int soc_sbx_div64(uint64_t x, uint32_t y, uint32_t *result);
