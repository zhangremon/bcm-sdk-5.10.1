/*
 * $Id: sirius.h 1.84.2.1 Broadcom SDK $
...
extern int
soc_sirius_hypercore_loopback_set(int unit, int32_t uPhyAddr, int32_t uLaneAddr, int32_t bEnable);

extern int
soc_sirius_hypercore_loopback_get(int unit, int32_t uPhyAddr, int32_t uLaneAddr, int32_t *pEnable);

extern int
soc_sirius_hypercore_8b10b_set(int unit, int32_t uPhyAddr, int32_t uLaneAddr, int32_t bEnable);
...
extern int
soc_sirius_mdio_hc_write(int unit, uint32 uPhyAddr, uint32_t uLane, uint32 uRegAddr, uint32 uData);
...
#define SOC_SIRIUS_API_PARAM_NO_CHANGE                 (-1)
...
typedef struct siriusInitParamsSc_s {
  ...
  uint32_t uLinkEnRemap[SB_FAB_DEVICE_SIRIUS_LINKS];
  uint32_t uLinkStatusRemap[SB_FAB_DEVICE_SIRIUS_INTERNAL_LINKS];
  uint32_t uJitTolerance[SB_FAB_DEVICE_SIRIUS_LINKS];
  uint32_t bEvenChannelOn[SB_FAB_DEVICE_SIRIUS_LINKS];
  uint32_t bOddChannelOn[SB_FAB_DEVICE_SIRIUS_LINKS];
  uint32_t uQuadDone;
...
typedef struct siriusInitParamsSf_s {
  sbBool_t bBringUp;
  sbBool_t bTmeOnly;
  uint32_t uSfiPortRemap[SB_FAB_DEVICE_SIRIUS_SFI_PORTS];
  uint32_t uSfiPortLoopback[SB_FAB_DEVICE_SIRIUS_SFI_PORTS];
  uint32_t uSfiPortBackpressure[SB_FAB_DEVICE_SIRIUS_SFI_PORTS];
  uint32_t uSfiPortIdleFreq[SB_FAB_DEVICE_SIRIUS_SFI_PORTS];
  uint32_t uSfiPortEnable[SB_FAB_DEVICE_SIRIUS_SFI_PORTS];
...
typedef struct siriusInitParams_s {
    ...
    uint32_t     bResetOnly;                         /* only reset all blocks and config cmic block if non-zero */
    ...
    uint32_t     uQmMaxArrivalRateMbs;               /* maximum arrival rate in Mbps */
    uint32_t     uSiLsThreshold;                     /* si (sf/sc blocks) error threshold */
    uint32_t     uSiLsWindow;                        /* si (sf/sc blocks) error window */
    ...
    uint32_t     uBufferSize;                        /* external DDR3 buffer size */
    uint32_t     uMaxBuffer;                         /* external DDR3 max number of buffer */
    uint32_t     uNumColBits;                        /* external DDR3 number of column bits */
    uint32_t     uDdr3NumMemories;                   /* external DDR3 devices */
    uint32_t     uDdr3ClockMhz;                      /* clock frequency for DDR3 interface */
    siriusInitParamsCi_t ci[SOC_MAX_NUM_CI_BLKS];
...
extern int
soc_sirius_defrag_ff(uint32 unit);
 ...
 */
extern int 
soc_sirius_get_ingress_rate_info(int unit, uint32_t rate,
                                 uint32_t *rate_mantissa, uint32_t *rate_exponent,
                                 uint32_t *burst_mantissa, uint32_t *burst_exponent);
