...
/*
 * $Id: sbZfFabBm9600LinkFailureInfo.hx 1.1.44.4 Broadcom SDK $
...
typedef struct _sbZfFabBm9600LinkFailureInfo {
  uint32_t m_bQeLinkState;
  uint8_t m_bExpectingLinkError;
  uint64_t m_uDigestScanStartTime;
  uint64_t m_uDigestScanEndTime;
  uint32_t m_uDUTBaseAddress;
  uint32_t m_uActiveBmeFoMode;
  uint32_t m_uActiveBmeSciLink;
  uint32_t m_uExpectedGlobalLinkState;
  uint32_t m_uInaNumber;
...
#define SB_ZF_FABBM9600LINKFAILUREINFO_GET_DUTBASEADDRESS(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           ...
           (nToData) |= (uint32_t) (pFromData)[4] << 14; \
           (nToData) |= (uint32_t) (pFromData)[11] << 22; \
           (nToData) |= (uint32_t) ((pFromData)[10] & 0x03) << 30; \
          } while(0)
...
#define SB_ZF_FABBM9600LINKFAILUREINFO_GET_EXPECTEDLINKSTATE(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           ...
           (nToData) |= (uint32_t) (pFromData)[1] << 9; \
           (nToData) |= (uint32_t) (pFromData)[0] << 17; \
           (nToData) |= (uint32_t) ((pFromData)[7] & 0x7f) << 25; \
          } while(0)
...
#define SB_ZF_FABBM9600LINKFAILUREINFO_GET_QELINKSTATE(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32_t) ((pFromData)[25] >> 3) & 0x1f; \
          } while(0)

#define SB_ZF_FABBM9600LINKFAILUREINFO_GET_EXPECTINGLINKERROR(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint8_t) ((pFromData)[25] >> 2) & 0x01; \
          } while(0)
...
#define SB_ZF_FABBM9600LINKFAILUREINFO_GET_DUTBASEADDRESS(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           ...
           (nToData) |= (uint32_t) (pFromData)[7] << 14; \
           (nToData) |= (uint32_t) (pFromData)[8] << 22; \
           (nToData) |= (uint32_t) ((pFromData)[9] & 0x03) << 30; \
          } while(0)
...
#define SB_ZF_FABBM9600LINKFAILUREINFO_GET_EXPECTEDLINKSTATE(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           ...
           (nToData) |= (uint32_t) (pFromData)[2] << 9; \
           (nToData) |= (uint32_t) (pFromData)[3] << 17; \
           (nToData) |= (uint32_t) ((pFromData)[4] & 0x7f) << 25; \
          } while(0)
