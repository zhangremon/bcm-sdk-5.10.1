...
/*
 * $Id: sbZfKaRbClassDmacMatchEntry.hx 1.1.44.4 Broadcom SDK $
...
typedef struct _sbZfKaRbClassDmacMatchEntry {
  uint32_t m_nDmacDataLsb;
  uint32_t m_nDmacDataRsv;
  uint32_t m_nDmacDataMsb;
  uint32_t m_nDmacMaskLsb;
  uint32_t m_nDmacMaskRsv;
  uint32_t m_nDmacMaskMsb;
  uint32_t m_nDmacReserve;
  uint8_t m_nDmacEnable;
  uint32_t m_nDmacDp;
  uint32_t m_nDmacLsb;
