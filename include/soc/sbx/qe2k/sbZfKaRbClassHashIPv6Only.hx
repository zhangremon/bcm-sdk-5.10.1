...
/*
 * $Id: sbZfKaRbClassHashIPv6Only.hx 1.1.44.4 Broadcom SDK $
...
#define SB_ZF_KARBCLASSHASHIPV6ONLY_GET_PAD1(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32_t) (pFromData)[41] ; \
           (nToData) |= (uint32_t) (pFromData)[40] << 8; \
           (nToData) |= (uint32_t) ((pFromData)[47] & 0x0f) << 16; \
          } while(0)
...
#define SB_ZF_KARBCLASSHASHIPV6ONLY_GET_SKT(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           ...
           (nToData) |= (uint32_t) (pFromData)[11] << 16; \
...
#define SB_ZF_KARBCLASSHASHIPV6ONLY_GET_PAD1(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32_t) (pFromData)[42] ; \
           (nToData) |= (uint32_t) (pFromData)[43] << 8; \
           (nToData) |= (uint32_t) ((pFromData)[44] & 0x0f) << 16; \
          } while(0)
...
#define SB_ZF_KARBCLASSHASHIPV6ONLY_GET_SKT(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           ...
           (nToData) |= (uint32_t) (pFromData)[8] << 16; \
           (nToData) |= (uint32_t) (pFromData)[9] << 24; \
          } while(0)
