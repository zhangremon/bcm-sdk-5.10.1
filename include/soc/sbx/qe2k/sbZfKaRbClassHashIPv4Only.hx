...
/*
 * $Id: sbZfKaRbClassHashIPv4Only.hx 1.1.44.4 Broadcom SDK $
...
#define SB_ZF_KARBCLASSHASHIPV4ONLY_GET_IPSA(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32_t) (pFromData)[34] ; \
           (nToData) |= (uint32_t) (pFromData)[35] << 8; \
           (nToData) |= (uint32_t) (pFromData)[36] << 16; \
           (nToData) |= (uint32_t) (pFromData)[37] << 24; \
          } while(0)

#define SB_ZF_KARBCLASSHASHIPV4ONLY_GET_IPDA(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32_t) (pFromData)[30] ; \
           (nToData) |= (uint32_t) (pFromData)[31] << 8; \
           (nToData) |= (uint32_t) (pFromData)[32] << 16; \
           (nToData) |= (uint32_t) (pFromData)[33] << 24; \
          } while(0)

#define SB_ZF_KARBCLASSHASHIPV4ONLY_GET_SKT(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32_t) (pFromData)[26] ; \
           (nToData) |= (uint32_t) (pFromData)[27] << 8; \
           (nToData) |= (uint32_t) (pFromData)[28] << 16; \
           (nToData) |= (uint32_t) (pFromData)[29] << 24; \
          } while(0)
