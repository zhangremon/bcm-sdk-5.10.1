...
/*
 * $Id: sbZfC2PtPortStateEntry.hx 1.3.16.4 Broadcom SDK $
...
#define SB_ZF_C2PTPORTSTATEENTRY_GET_NEXTPAGE(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32_t) ((pFromData)[8] >> 1) & 0x7f; \
           (nToData) |= (uint32_t) ((pFromData)[9] & 0x3f) << 7; \
          } while(0)

#define SB_ZF_C2PTPORTSTATEENTRY_GET_NEXTLENGTH(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32_t) ((pFromData)[7] >> 1) & 0x7f; \
           (nToData) |= (uint32_t) ((pFromData)[8] & 0x01) << 7; \
          } while(0)
...
#define SB_ZF_C2PTPORTSTATEENTRY_GET_PAGE(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           ...
           (nToData) |= (uint32_t) ((pFromData)[7] & 0x01) << 12; \
          } while(0)
