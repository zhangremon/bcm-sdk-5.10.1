...
/*
 *$Id: hal_user_common.h 1.2 Broadcom SDK $
...
/* verify that there is only one Non-Zero field */
#ifndef SAND_HAL_VERIFY_ONZ_FIELD
#define SAND_HAL_VERIFY_ONZ_FIELD(device,reg,field,regValue) do{UINT nExpected=((regValue)&~SAND_HAL_FIELD_MASK(device,reg,field));if(nExpected!=0x0){SAND_HAL_ERROR("VERIFY_ONZ_FIELD: Found unexpected non-zero fields.  Expected(0x%x) Found(0x%x)\n",nExpected,(regValue));}}while(0)
#endif

#ifndef SAND_HAL_VERIFY_FIELD
#define SAND_HAL_VERIFY_FIELD(device,reg,field,regValue,expectValue) do{UINT nFieldValue=SAND_HAL_GET_FIELD(device,reg,field,(regValue));if((UINT)nFieldValue!=(UINT)(expectValue)){SAND_HAL_ERROR("VERIFY_FIELD: (%s:%d) value(0x%x) not as expected(0x%x)\n",__FILE__,__LINE__, nFieldValue, (expectValue));}else{ZSIM_LOG(NULL,"Verified (%s:%d) field value 0x%x\n",__FILE__,__LINE__,(expectValue));}}while(0)
#endif

#ifndef SAND_HAL_VERIFY_VALUE
#define SAND_HAL_VERIFY_VALUE(sMsg, regValue,expectValue) do{if((UINT)(regValue)!=(UINT)(expectValue)){SAND_HAL_ERROR("VERIFY_VALUE: (%s:%d) %s regValue(0x%x) not as expected(0x%x)\n",__FILE__,__LINE__, (sMsg), (regValue), (expectValue));}else{ZSIM_LOG(NULL,"Verified (%s:%d) %s field value 0x%x\n",__FILE__,__LINE__,(sMsg),(expectValue));}}while(0)
#endif
