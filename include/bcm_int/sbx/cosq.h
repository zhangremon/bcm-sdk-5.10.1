...
#define BCM_INT_SBX_SCHEDULER_GPORT_SET(gport, egress, id)  \
    (BCM_GPORT_SCHEDULER_SET(gport, (((((egress)?1:0) & 0x1) << (_SHR_GPORT_TYPE_SHIFT-1)) | ((id) & (_SHR_GPORT_SCHEDULER_MASK >> 1)))))
...
#define BCM_INT_SBX_SCHEDULER_IS_EGRESS(gport)             \
    (((BCM_GPORT_SCHEDULER_GET(gport) >> (_SHR_GPORT_TYPE_SHIFT-1)) & 0x1))
...
#define BCM_INT_SBX_QUEUE_STATE_AVAILABLE      (0x23230000)
#define BCM_INT_SBX_QUEUE_STATE_IN_USE         (0x23230001)
#define BCM_INT_SBX_QUEUE_STATE_RESERVED_TME   (0x23230003)
...
typedef struct bcm_sbx_cosq_queue_params_ingress_s {
    ...
    int32_t given_weight; /* WFQ only, this is the 128 adjusted weight value for this queue */
...
typedef struct bcm_sbx_cosq_queue_state_s {
    int32 state;   /* one of BCM_INT_SBX_QUEUE_STATE_AVAILABLE, IN_USE... */
...
extern int
bcm_sbx_cosq_fcd_get_from_fifo(int unit, int fifo, int *fcd);
