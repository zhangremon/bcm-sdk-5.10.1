/*
 * $Id: field.h 1.326.2.9 Broadcom SDK $
...
#define _FP_PORT_BITWIDTH(_unit_)    ((SOC_IS_TD_TT(_unit_)) ? 7 : 6)
...
#define _FP_QUAL_EXT_ADD(_unit_, _stage_fc_, _qual_id_,                       \
                        ...
                        _intraslice_,  _offset_0_, _width_0_,                 \
                        _offset_1_, _width_1_, _offset_2_, _width_2_,         \
                        _secondary_)                                          \
            do {                                                              \
...
typedef enum _bcm_field_stat_e {
    _bcmFieldStatBytesEven = bcmFieldStatCount, 
                                       /* Byte count of traffic of all colors.   */
    _bcmFieldStatBytesOdd,             /* Byte count of traffic of all colors.   */
    _bcmFieldStatPacketsEven,          /* Packet count of traffic of all colors. */
    _bcmFieldStatPacketsOdd,           /* Packet count of traffic of all colors. */
    _bcmFieldStatCount                 /* Always Last. Not a usable value.       */
} _bcm_field_stat_t;
...
typedef struct _field_stat_s {
    ...
    uint64               *stat_values;  /* Stat value after it was detached */
                                        /* from a last entry.               */
    uint32               flex_mode;     /* Flexible Counter Mode */
    uint32               tcam_index;    /* Index to VFP_POLICY_TABLE */
} _field_stat_t;
...
typedef struct _field_funct_s {
    ...
    int(*fp_udf_delete)(int, bcm_field_udf_spec_t *, uint32, uint32);
...
#ifdef BCM_WARM_BOOT_SUPPORT
    uint32                 scache_size;   /* Size of Level2 warm boot cache */
    uint32                 scache_usage;  /* Actual scache usage            */
    uint32                 scache_pos;    /* Current position in scache     */
    uint8                  *scache_ptr;   /* Pointer to scache section      */
...
#define _FIELD_D_TYPE_POS   (SOC_IS_TD_TT(unit) ? (16) : (14))
