/*
 * $Id: policer.h 1.1.2.1 Broadcom SDK $
...
#define BCM_POLICER_SVC_METER_INNER_DOT1P_ATTR_SIZE                     4
#define BCM_POLICER_SVC_METER_ING_PORT_ATTR_SIZE                        6
#define BCM_POLICER_SVC_METER_TOS_ATTR_SIZE                             8
...
/* Packet resolution bit positions */
...
#define _GLOBAL_METER_XGS3_ALLOC(_ptr_,_size_,_descr_)                 \
    do {                                             \
         ...
         if((_ptr_) != NULL) {                        \
              ...
              BCM_DEBUG(BCM_DBG_ERR, ("Error:Alloc failure %s\n", (_descr_))); \
         }                                          \
    } while (0)
...
typedef struct bcm_policer_svc_meter_compressed_attr_selectors_s {
    ...
    compressed_pkt_res_attr_map_t 
         compressed_pkt_res_attr_map_v[BCM_SVC_METER_MAP_SIZE_256]; 
    offset_table_entry_t offset_map[BCM_SVC_METER_MAP_SIZE_256];
...
bcm_error_t _bcm_policer_svc_meter_get_available_mode( uint32 unit,
    bcm_policer_svc_meter_mode_type_t *bcm_policer_svc_meter_mode_type_t
);
