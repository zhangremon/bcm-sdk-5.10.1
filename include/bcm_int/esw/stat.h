/*
 * $Id: stat.h 1.21.120.2 Broadcom SDK $
...
extern int **_bcm_stat_ovr_threshold;
...
#define COUNT_OVR_ERRORS(unit)                                    \
    (soc_feature(unit, soc_feature_stat_jumbo_adj) ||             \
     ((_bcm_stat_ovr_threshold != NULL) &&                        \
      ...
      (_bcm_stat_ovr_threshold[unit][port] <= 1518)))
...
extern int _bcm_stat_generic_get(int unit, bcm_port_t port,
                                 bcm_stat_val_t type, uint64 *val);
